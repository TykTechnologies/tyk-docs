...
Refer to the [Tyk Gateway Configuration Reference](/docs/tyk-oss-gateway/configuration#external_services) for more details on this setting.
...
| :-------------- | :------------- | :------------ |
| `oauth` | [OAuth/JWT token validation and introspection](/docs/basic-config-and-security/security/authentication-authorization/json-web-tokens) | JWT auth interactions with Auth Server (Identity Provider) |
| `storage` | [External storage operations](/docs/planning-for-production/database-settings) | Redis connections, database interactions |
| `webhooks` | [Webhook event notifications](/docs/api-management/gateway-events#event-handling-with-webhooks) | Event handlers, notification delivery |
| `health` | [Health check requests](/docs/planning-for-production/ensure-high-availability/health-check) | Host checker, uptime monitoring |  
| `discovery` | [Service discovery requests](/docs/planning-for-production/ensure-high-availability/service-discovery) | Load balancer, service registry |
...
| `cert_id` | String | No | Certificate ID from [Tyk Certificate Store](/docs/api-management/certificates#using-tyk-certificate-storage) |
| `ca_cert_ids` | Array | No | Array of CA certificate IDs from [Tyk Certificate Store](/docs/api-management/certificates#using-tyk-certificate-storage) |
...
Refer to the [Tyk Gateway Configuration Reference](/docs/tyk-oss-gateway/configuration#external_services) for more details.
...
4. **Default settings** - If a setting is not specified, Tyk uses sensible defaults. You can find these in the [Tyk Gateway Configuration Reference](/docs/tyk-oss-gateway/configuration#external_services).
...
**What this does:**
- Similar to previous example, but uses [Tyk Certificate Store](/docs/api-management/certificates#using-tyk-certificate-storage) instead of file paths
...
**What this does:**
- Similar to example 4, but uses [Tyk Certificate Store](/docs/api-management/certificates#using-tyk-certificate-storage) for all mTLS configurations
