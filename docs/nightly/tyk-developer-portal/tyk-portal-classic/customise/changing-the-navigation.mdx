...
canonical: "https://tyk.io/docs/docs/nightly/tyk-developer-portal/tyk-portal-classic/customise/changing-the-navigation"
---
...
<ul class="nav navbar-nav">
  ...
  <li><a href="/docs/nightly/portal/{{$menuItem.URL}}">{{$menuItem.Title}}</a></li>
  {{ end }}
  <li><a href="/docs/nightly/portal/apis/">API Catalog</a></li>
  {{ if not .PortalConfig.DisableSignup }}
    {{ if not .UserData }}
    <li><a href="/docs/nightly/portal/register/">Register</a></li>
    {{ end }}
  {{ end }}
  {{ if not .PortalConfig.DisableLogin }}
    {{ if not .UserData }}
    <li><a href="/docs/nightly/portal/login/">Log in</a></li>
    {{ end }}
  {{ end }}
</ul>
```
