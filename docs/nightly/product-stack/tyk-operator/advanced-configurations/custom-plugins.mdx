...
canonical: "https://tyk.io/docs/docs/nightly/product-stack/tyk-operator/advanced-configurations/custom-plugins"
---

## Introduction

Using Tyk Classic APIs, developers can implement API-level custom plugins that can be optionally setup to execute for
each of the following [hooks](/docs/nightly/api-management/plugins/plugin-types#plugin-and-hook-types) in the API request
lifecycle: [Pre (Request)](/docs/nightly/api-management/plugins/plugin-types#request-plugins), [Authentication](/docs/nightly/api-management/plugins/plugin-types#authentication-plugins),
[Post (Request)](/docs/nightly/api-management/plugins/plugin-types#request-plugins), [Post
Authentication](/docs/nightly/api-management/plugins/plugin-types#request-plugins), [Response](/docs/nightly/api-management/plugins/plugin-types#response-plugins)
and [Analytics](/docs/nightly/api-management/plugins/plugin-types#analytics-plugins). Subsequently, users can execute, or “hook”, their
...
Please refer to [Analytics Plugins](/docs/nightly/api-management/plugins/plugin-types#analytics-plugins) to learn how to configure Analytics
...
languages](/docs/nightly/api-management/plugins/overview#plugin-driver-names) section for list of supported plugin driver names.
...
[per-endpoint](/docs/nightly/api-management/plugins/plugin-types#per-endpoint-custom-plugins) level plugins are not currently
...
2. **Create API Definition Resource**
    ...
    *http://httpbin.org*.
    ...
    - The `driver` configuration parameter is set to `otto` at line 15, since our plugin is a Javascript plugin. For other
      valid values please refer to the [plugins driver page](/docs/nightly/api-management/plugins/overview#plugin-driver-names).
    - A plugin hook configuration block is specified at line 16, containing the `name` and `path` for our plugin. The plugin
      ...
      middleware. Valid values are the same as the [Tyk Classic API Definition](/docs/nightly/#how-it-works) equivalent, i.e.
...
request lifecycle](/docs/nightly/api-management/traffic-transformation#request-middleware-chain).
...
bundles](/docs/nightly/api-management/plugins/overview#plugin-bundles), load them into the Tyk Gateway, and configure an API
