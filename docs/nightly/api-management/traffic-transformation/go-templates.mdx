...
canonical: "https://tyk.io/docs/docs/nightly/api-management/traffic-transformation/go-templates"
---

Tyk's [request](/docs/nightly/api-management/traffic-transformation/request-body) and [response](/docs/nightly/api-management/traffic-transformation/response-body) body transform middleware use the [Go template language](https://golang.org/pkg/text/template/) to parse and modify the provided input.

Go templates are also used by Tyk's [webhook event handler](/docs/nightly/api-management/gateway-events#event-handling-with-webhooks) to produce the payload for the HTTP request sent to the target system.
...
- `jsonMarshal` performs JSON style character escaping on an XML field and, for complex objects, serialises them to a JSON string ([example](/docs/nightly/#xml-to-json-conversion-using-jsonmarshal))
- `xmlMarshal` performs the equivalent conversion from JSON to XML ([example](/docs/nightly/#json-to-xml-conversion-using-xmlmarshal))
...
Here we provide worked examples for both [JSON](/docs/nightly/#example-json-transformation-template) and [XML](/docs/nightly/#example-xml-transformation-template) formatted inputs. We also explain examples using the [jsonMarshal](/docs/nightly/#xml-to-json-conversion-using-jsonmarshal) and [xmlMarshal](/docs/nightly/#json-to-xml-conversion-using-xmlmarshal) helper functions.
