...
canonical: "https://tyk.io/docs/docs/5.8/api-management/traffic-transformation/response-headers"
---
...
This middleware changes only the headers and not the payload. You can, however, combine this with the [Response Body Transform](/docs/5.8/api-management/traffic-transformation/response-body) to apply more complex transformation to responses.

There are related [Request Header Transform](/docs/5.8/api-management/traffic-transformation/request-headers) middleware (at API-level and endpoint-level) that provide the same functionality on the request from a client, prior to it being proxied to the upstream.
...
- [context variables](/docs/5.8/api-management/traffic-transformation/request-context-variables), extracted from the request at the start of the middleware chain, can be injected into added headers using the `$tyk_context.` namespace
- [session metadata](/docs/5.8/api-management/policies#what-is-a-session-metadata), from the Tyk Session Object linked to the request, can be injected into added headers using the `$tyk_meta.` namespace
- values from [key-value (KV) storage](/docs/5.8/tyk-configuration-reference/kv-store#transformation-middleware) can be injected into added headers using the notation appropriate to the location of the KV store
...
Tyk's [response header transform](/docs/5.8/api-management/traffic-transformation/response-headers) middleware enables you to append or delete headers on responses received from the upstream service before sending them to the client.
...
When working with Tyk OAS APIs the transformation is configured in the [Tyk OAS API Definition](/docs/5.8/api-management/gateway-config-tyk-oas#operation). You can do this via the Tyk Dashboard API or in the API Designer.

If you're using the legacy Tyk Classic APIs, then check out the [Tyk Classic](/docs/5.8/#response-headers-using-classic) page.
...
- `X-Request-ID` with a dynamic value taken from the `request_id` [context variable](/docs/5.8/api-management/traffic-transformation/request-context-variables)
- `X-User-ID` with a dynamic value taken from the `uid` field in the [session metadata](/docs/5.8/api-management/policies#what-is-a-session-metadata)
...
If the example [API-level](/docs/5.8/#api-level-transform) and [endpoint-level](/docs/5.8/#endpoint-level-transform) transforms are applied to the same API, then the `X-Secret` header will be added (by the endpoint-level transform first) and then removed (by the API-level transform). Subsequently, the result of the two transforms for a call to `GET /status/200` would be to add four headers:
- `X-Request-ID`
- `X-User-ID`
- `X-Static`
- `X-New`
...
If you're using the newer Tyk OAS APIs, then check out the [Tyk OAS](/docs/5.8/#response-headers-using-tyk-oas) page.

If you're using Tyk Operator then check out the [configuring the Response Header Transform in Tyk Operator](/docs/5.8/#tyk-operator) section below.
...
This feature is rarely used and has not been implemented in the Tyk Dashboard UI, nor in the [Tyk OAS API](/docs/5.8/#response-headers-using-tyk-oas).
