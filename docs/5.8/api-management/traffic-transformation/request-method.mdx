...
Tyk's [request method transform](/docs/5.8/api-management/traffic-transformation/request-method) middleware is configured at the endpoint level, where it modifies the HTTP method used in the request to a configured value.
...
If you're using the legacy Tyk Classic APIs, then check out the [Tyk Classic](/docs/5.8/#request-method-using-classic) page.
...
If you're using the newer Tyk OAS APIs, then check out the [Tyk OAS](/docs/5.8/#request-method-using-tyk-oas) page.

If you're using Tyk Operator then check out the [configuring a Request Method Transform in Tyk Operator](/docs/5.8/#tyk-operator) section below.
