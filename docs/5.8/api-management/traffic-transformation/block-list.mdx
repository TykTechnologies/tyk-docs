...
canonical: "https://tyk.io/docs/docs/5.8/api-management/traffic-transformation/block-list"
---
...
Note that this is not the same as Tyk's [IP block list](/docs/5.8/api-management/gateway-config-tyk-classic#ip-access-control) feature, which is used to restrict access to APIs based upon the IP of the requestor.
...
You can also set case sensitivity for the entire [gateway](/docs/5.8/tyk-oss-gateway/configuration#ignore_endpoint_case) in the Gateway configuration file `tyk.conf`. If case insensitivity is configured at the gateway level, this will override the endpoint-level setting.

#### Endpoint parsing

When using the block list middleware, we recommend that you familiarize yourself with Tyk's [URL matching](/docs/5.8/getting-started/key-concepts/url-matching) options.

<br />

<Note>
Tyk recommends that you use [exact](/docs/5.8/getting-started/key-concepts/url-matching#exact-match) matching for maximum security, though prefix and wildcard strategies might also apply for your particular deployment or use case.
</Note>
...
The [block list](/docs/5.8/api-management/traffic-transformation/block-list) is a feature designed to block access to specific API endpoints. Tyk Gateway rejects all requests made to endpoints with the block list enabled, returning `HTTP 403 Forbidden`. 
...
If you're using the legacy Tyk Classic APIs, then check out the [Tyk Classic](/docs/5.8/#block-list-using-classic) page.
...
Note also that the endpoint path has not been terminated with `$`. Requests to, for example, `GET /anything/foobar` will be rejected as the [regular expression pattern match](/docs/5.8/#endpoint-parsing) will recognize this as `GET /anything`.
...
If you're using the newer Tyk OAS APIs, then check out the [Tyk OAS](/docs/5.8/#block-list-using-tyk-oas) page.

If you're using Tyk Operator then check out the [configuring the block list in Tyk Operator](/docs/5.8/#tyk-operator) section below.
...
- `method_actions`: a shared object used to configure the [mock response](/docs/5.8/api-management/traffic-transformation/mock-response#when-is-it-useful) middleware
...
Note also that the endpoint path has not been terminated with `$`. Requests to, for example, `GET /status/200/foobar` will be rejected as the [regular expression pattern match](/docs/5.8/#endpoint-parsing) will recognize this as `GET /status/200`.

Consult section [configuring the Allow List in Tyk Operator](/docs/5.8/#tyk-operator) for details on how to configure allow lists for endpoints using Tyk Operator.
...
Note also that the endpoint path has not been terminated with `$`. Requests to, for example, `GET /get/foobar` will be rejected as the [regular expression pattern match](/docs/5.8/#endpoint-parsing) will recognize this as `GET /get`.
