...
canonical: "https://tyk.io/docs/docs/5.8/tyk-stack/tyk-developer-portal/enterprise-developer-portal/api-access/dynamic-client-registration"
---
...
Go directly to [Configure Tyk Enterprise Developer Portal to work with an identity provider](/docs/5.8/#configure-tyk-enterprise-developer-portal-to-work-with-an-identity-provider).
</Note>
...
Tyk requires any API that uses the scope to policy mapping to have [a default policy](/docs/5.8/basic-config-and-security/security/authentication-authorization/json-web-tokens). Access rights and rate limits defined in the default policy take priority over other policies, including policies for the API Product and plan.
...
Then you need to specify the connection settings: [the initial access token and the well-known endpoint](/docs/5.8/#create-an-initial-access-token). If your Identity Provider uses certificates that are not trusted, the portal will not work with it by default. To bypass certificate verification, you can select the `SSL secure skip verify` checkbox.
...
After that, specify the scope for this API product. You should have at least one scope that was created in [the Prerequisites for getting started](/docs/5.8/tyk-stack/tyk-developer-portal/enterprise-developer-portal/api-access/dynamic-client-registration#prerequisites). If you need to specify more than one scope, you can separate them with spaces.

Finally, select one or multiple types of clients that were created in [the Create client configurations](/docs/5.8/tyk-stack/tyk-developer-portal/enterprise-developer-portal/api-access/dynamic-client-registration#create-client-configurations) section of this guide to associate them with that product.
...
The last step is to configure the plans you want to use with the DCR flow. To do this, go to the portal's `Plans` menu section and specify the OAuth2.0 scope to use with each plan. You should have at least one scope that was created in [the Prerequisites for getting started](/docs/5.8/tyk-stack/tyk-developer-portal/enterprise-developer-portal/api-access/dynamic-client-registration#prerequisites). If you need to specify more than one scope, you can separate them with spaces.
...
Once the access request is approved, the developer should receive an email informing them of the approval. Please refer to [the email customization section](/docs/5.8/portal/customization/email-notifications) if you wish to change the email template.
