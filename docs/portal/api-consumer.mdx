...
Unlike [API Owners](/docs/portal/api-owner) who administer the portal, API Consumers interact with your APIs through the Live Portal. They browse catalogs, request access to API Products, create applications, and use the credentials you provide to integrate with your services. The Developer Portal gives you powerful tools to organize these users, control their access, and facilitate collaboration among development teams.

This guide focuses on managing individual API Consumer accounts. For information on organizing users into groups, see the [Organisation and Teams](/docs/tyk-stack/tyk-developer-portal/enterprise-developer-portal/managing-access/manage-api-consumer-organisations) page.
...
Please refer to the [email server configuration](/docs/product-stack/tyk-enterprise-developer-portal/getting-started/setup-email-notifications) section for setup instructions.
<br />
...
5. If the portal requires an admin to [approve registration requests](/docs/portal/api-owner#user-account-status), they will see the following message.
...
6. If the portal is configured for [automatic approval](/docs/portal/api-owner#automatically-approve-user-registrations), they will see a message that indicates that their account is active:
...
After completing authentication, as an API Consumer, you will be taken to the [Live Portal](/docs/portal/overview/concepts#developer-portal-views).
...
- Admins will can edit the details of other users, including switching between Team Member and Admin role, and reassignment to other teams
  ...
  - Admins cannot change the [active](/docs/portal/api-owner#user-account-status) status of other API Consumer users
...
- Personal: Only visible to the [owner](/docs/portal/developer-app#app-owner) (typically the creator) of the app
...
- Regular Audits: Periodically review user accounts and [deactivate](/docs/portal/api-owner#user-account-status) or [delete](/docs/portal/api-owner#deleting-users) users as appropriate
