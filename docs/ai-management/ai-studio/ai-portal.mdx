...
*   **Chat Interface:** Access to all [Chat Experiences](/docs/ai-management/ai-studio/chat-interface) the user has permission to use, with a clean, intuitive UI for conversational interactions.
*   **Resource Catalogues:** Browse and subscribe to available LLMs, Data Sources, and [Tools](/docs/ai-management/ai-studio/tools) through dedicated catalogue interfaces.
*   **Application Management:** Create and manage [Apps](/docs/ai-management/ai-studio/apps) that integrate LLMs, tools, and data sources for API access.
