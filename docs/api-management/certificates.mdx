...
1. **[Enabling TLS in Tyk components](/docs/api-management/certificates#enable-tlsssl-in-tyk-components)**: 
    Learn how to enable and configure TLS/SSL for Tyk Gateway and Dashboard to secure your communication.
2. **[TLS Support in Tyk](/docs/api-management/certificates#tlsssl-configuration)**: 
    Understand the supported TLS versions, cipher suites, their configurations, and best practices for secure communication.
3. **[Configuring Tyk Certificate Storage](/docs/api-management/certificates#using-tyk-certificate-storage)**: 
...
4. **[Self Signed Certificates](/docs/api-management/certificates#self-signed-certificates)**: 
    Learn how to configure and use self-signed certificates for secure communication in Tyk.
5. **[Configuring Internal Proxy Setup](/docs/api-management/certificates#internal-proxy-setup)**: 
...
1. Certificates without public keys used for [client authorization and authentication](/docs/basic-config-and-security/security/mutual-tls/client-mtls#why-use-mutual-tls)
2. Certificates with private keys used for [upstream access](/docs/api-management/upstream-authentication/mtls), and server certificates (in other words when we need to sign and encrypt the request or response).
...
For self signed certificates additional consideration has to be taken place, [refer to the section below](/docs/#self-signed-certificates).
...
Set the [host_config.generate_secure_paths](/docs/tyk-dashboard/configuration#host_configgenerate_secure_paths) flag to `true` so that your Dashboard URL starts with HTTPS.

If you are using self-signed certs or are in a test environment, [you can tell Tyk to ignore validation on certs Mutual TLS support](/docs/#self-signed-certificates)
...
- `min_version` and `max_version` are optional and allow you to configure the [versions of TLS](/docs/#supported-tls-versions) from which Tyk will accept connections
- `ssl_ciphers` allows you to select the [cipher suite](/docs/#supported-tls-cipher-suites) that will be used to negotiate connections
...
You may notice that you can't get the raw certificate back, only its meta information. This is to ensure security. Certificates with private keys have special treatment and are encoded before storing. If a private key is found it will be encrypted with AES256 algorithm 3 using the `security.private_certificate_encoding_secret` secret, defined in `tyk.conf` file. Otherwise, the certificate will use the [secret](/docs/tyk-oss-gateway/configuration#secret) value in `tyk.conf`.

### Using Tyk Certificate Storage

In Tyk Gateway 2.4 and Tyk Dashboard 1.4 we added [Mutual TLS support](/docs/basic-config-and-security/security/mutual-tls/client-mtls#why-use-mutual-tls) including special Certificate storage, which is used to store all kinds of certificates from public to server certificates with private keys.
...
For the server in `common name` specify a domain, or just pass `-subj "/CN=localhost"` to OpenSSL command. Then follow our [TLS and SSL Guide](/docs/api-management/certificates).
...
From v2.9.3 you can also specify a custom proxy and set the minimum TLS versions and any SSL ciphers within your API definitions. See [Internal Proxy Setup](/docs/api-management/gateway-config-tyk-classic#proxy-transport-settings) for more details.
