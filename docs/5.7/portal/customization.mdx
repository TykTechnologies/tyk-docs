...
2. Upload the theme package that is created in the previous step to the portal. You can use the portal's [Admin dashboard](/docs/5.7/portal/customization#create-a-theme) or the [admin API](/docs/5.7/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) to do it.
![image](https://github.com/TykTechnologies/tyk-docs/assets/14009/f0e547b2-b521-4c3e-97ce-fd3a2a3b170b)
3. Finally, you need to [activate](/docs/5.7/portal/customization#activate-a-theme) the theme so that it will be applied to the portal.
...
[section](/docs/5.7/portal/customization#edit-page-content)
...
Returns a map of all [menus](/docs/5.7/portal/customization#configure-menus).
...
Please refer to [email workflow](/docs/5.7/portal/customization#configure-email-notifications)
...
We suggest you read the [portal concepts](/docs/5.7/portal/overview#developer-portal-concepts) first. If you've already done that, we will show you an example on how to:
...
Both of them are defined as [partials](/docs/5.7/portal/customization#file-structure-of-a-theme) in the portal directory in `/themes/default/partials/`.
...
Admin users can create additional navigational menus and render them on any page of the live portal. This customization requires changes to a theme and is covered in the [Full customization section](/docs/5.7/portal/customization#configure-menus).
...
2. Select a menu that you want to modify. By default, the Developer portal has only one **Primary** menu. If you want to add more menus and render them on the live portal, please refer to [Full customization section](/docs/5.7/portal/customization#configure-menus).
...
- A Tyk Self-Managed [installation](/docs/5.7/tyk-self-managed#installation-options-for-tyk-self-managed)
...
- **Attribute ID**: A string that consists of letters (a-zA-Z), numbers (0-9), dashes, and underscores. This is used to reference the attribute in the [Admin APIs](/docs/5.7/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) screen.
...
When creating a user account from the admin UI or via [the admin APIs](/docs/5.7/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api), an admin user can create the user account as active or inactive.
...
- [UserRegistered](/docs/5.7/portal/customization#new-user-registered) when a new user is registered.
- [UserAccountActivated](/docs/5.7/portal/customization#user-account-activated) when a user is activated.
- [UserAccountDeactivated](/docs/5.7/portal/customization#user-account-deactivated) when a user is deactivated.
- [PasswordReset](/docs/5.7/portal/customization#password-reset) when a user tries to reset a password.
- [ApplicationRegistered](/docs/5.7/portal/customization#new-application-registered) when a new API consumer application is created.
- [CredentialRegistered](/docs/5.7/portal/customization#new-credential-is-created) when a new API credential is created.
- [AccessRequestCreated](/docs/5.7/portal/customization#new-access-request-created) when a new API access request is created.
- [AccessRequestApproved](/docs/5.7/portal/customization#an-access-request-is-approved) when an API access request is approved.
- [AccessRequestRejected](/docs/5.7/portal/customization#an-access-request-is-rejected) when an API access request is rejected.
- [OrganizationRegistered](/docs/5.7/portal/customization#new-organization-registered) when an API consumer organization is created.
- [OrganizationRequestCreated](/docs/5.7/portal/customization#new-organization-registration-request-created) when a new API consumer organization registration request is created.
- [OrganizationRequestApproved](/docs/5.7/portal/customization#organization-registration-request-is-approved) when an API consumer organization registration request is approved.
- [OrganizationRequestRejected](/docs/5.7/portal/customization#organization-request-is-rejected) when an API consumer organization registration request is rejected.
...
This event is fired whenever a new consumer organization is created via the admin APIs, the live portal ([the become an organization flow](/docs/5.7/portal/api-consumer#self-registration)) or the admin UI.
...
This event is fired whenever a new organization request is created via the live portal ([the become an organization flow](/docs/5.7/portal/api-consumer#self-registration)) or the admin UI.
