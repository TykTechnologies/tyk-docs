...
This page introduces general features of dashboard and how to configure them. If you are looking for global configurations of the Dashboard deployment refer this [config file](/docs/5.7/tyk-dashboard/configuration).
...
To get a tour of the Dashboard UI, refer to this [document](/docs/5.7/getting-started/using-tyk-dashboard).
...
The Dashboard exposes two APIs:
 - **Dashboard API**: Is used for operational management of Tyk resources (APIs, policies, keys, etc.). This API offers granular permissions based on user roles.
    ...
    - [Postman / Swagger / Open API specification](/docs/5.7/tyk-dashboard-api)
 ...
 - **Dashboard Admin API**: Is used for system-level administration and initial setup tasks like managing organizations, initial user creation, backups/migrations and SSO setup. 
    ...
    - [Postman / Swagger / Open API specification](/docs/5.7/dashboard-admin-api)
...
The [Tyk Dashboard API](/docs/5.7/tyk-dashboard-api) is secured using an `Authorization` header that must be added to each request that is made. The **Tyk Dashboard API Access Credentials** `Authorization` key can be found within the Dashboard UI at the bottom of the **Edit User** section for a user.
...
The [Tyk Dashboard API](/docs/5.7/tyk-dashboard-api) is a superset of the Tyk Gateway API, enabling (almost) all of the core features and adding many more. The Dashboard API is also more granular and supports [Role Based Access Control](/docs/5.7/api-management/user-management#) (RBAC) on both a multi-tenant, and user basis.
...
<Note>
See [API Definition Objects](/docs/5.7/api-management/gateway-config-tyk-classic) section for an explanation of each field in the request & response.
</Note>
...
You need to have the `users` [Permission object](/docs/5.7/api-management/user-management#user-permissions) set to write to use **Add User**.
...
You need to have the `users` [Permission object](/docs/5.7/api-management/user-management#user-permissions) set to **read** to use **Set User Password**.
...
You need to have the `users` [Permission object](/docs/5.7/api-management/user-management#user-permissions) set to write to use **Update User**.
...
You need to have the `users` [Permission object](/docs/5.7/api-management/user-management#user-permissions) set to write to use this call.
...
This API returns by default the initial set of additional permissions defined in your Tyk Dashboard configuration, under [security.additional_permissions](/docs/5.7/tyk-dashboard/configuration#securityadditional_permissions).
...
You can control how long you want to store expired tokens in this list using `oauth_token_expired_retain_period` which specifies retain period for expired tokens stored in Redis. By default expired token not get removed. See [here](/docs/5.7/tyk-oss-gateway/configuration#oauth_token_expired_retain_period) for more details.
...
This endpoint is used in the [Authorization Code Grant](/docs/5.7/api-management/authentication/oauth-2#using-the-authorization-code-grant) flow, generating an authorization code that can be used by the client to request an access token.
...
<Note>
This functionality is available from [v2.9.0](/docs/5.7/developer-support/release-notes/archived#single-sign-on-for-the-tyk-saas). If you have an older version please using the [admin api](/docs/5.7/api-management/dashboard-configuration#single-sign-on-api-1)
</Note>
...
See [Single Sign On](/docs/5.7/api-management/external-service-integration#single-sign-on-sso) documentation for how to use this token and more details.
...
For more information on how to configure OPA see [Open Policy Agent](/docs/5.7/api-management/dashboard-configuration#extend-permissions-using-open-policy-agent-opa).
#### List OPA rules and settings

This endpoint returns by defaul the initial set of OPA rules defined in your Tyk Dashboard, which are located in [schema/dashboard.rego](/docs/5.7/api-management/dashboard-configuration#dashboard-opa-rules) (accessible in Self-Managed installations).
...
For the official Tyk Dashboard Admin API Reference, please visit our [API Documentation](/docs/5.7/dashboard-admin-api).
...
The Dashboard Admin SSO API endpoint allows you to implement custom authentication schemes for the Dashboard and Portal. Our Tyk Identity Broker (TIB) internally also uses this API. See [Single Sign On](/docs/5.7/api-management/external-service-integration#single-sign-on-sso) for more details.
...
- [Detailed logging](/docs/5.7/api-management/logs-metrics#capturing-detailed-logs)
...
- Upstream settings including
   ...
   - [Service Discovery](/docs/5.7/tyk-self-managed#service-discovery)
- [API Ownership](/docs/5.7/api-management/user-management#api-ownership)
- [API level rate limiting](/docs/5.7/api-management/rate-limit#configuring-the-rate-limiter-at-the-api-level)
- [Authentication](/docs/5.7/api-management/client-authentication)
...
The **Versions** tab allows you to create and manage [API versioning](/docs/5.7/api-management/gateway-config-tyk-classic#tyk-classic-api-versioning) for the API.
...
The **Endpoint Designer** is where you can define endpoints for your API so that you can enable and configure Tyk middleware to [perform checks and transformations](/docs/5.7/api-management/traffic-transformation#) on the API traffic.

In some cases, you will want to set global settings that affect all paths that are managed by Tyk. The **Global Version Settings** section will enable you to configure API-level [request](/docs/5.7/api-management/traffic-transformation#tyk-classic-api) and [response](/docs/5.7/api-management/traffic-transformation#tyk-classic-api) header transformation.
...
- [API-level caching](/docs/5.7/api-management/response-caching#configuring-the-cache-via-the-dashboard) including a button to invalidate (flush) the cache for the API
- [CORS](/docs/5.7/api-management/gateway-config-tyk-classic#cross-origin-resource-sharing-cors)
- Add custom attributes to the API definition as *config data* that can be accessed by middleware
- Enable [context variables](/docs/5.7/api-management/traffic-transformation#request-context-variables) so that they are extracted from requests and made available to middleware
- Manage *segment tags* if you are working with [sharded gateways](/docs/5.7/api-management/multiple-environments#gateway-sharding)
- Manage client IP address [allow](/docs/5.7/api-management/gateway-config-tyk-classic#ip-access-control) and [block](/docs/5.7/api-management/gateway-config-tyk-classic#ip-access-control) lists
- Attach [webhooks](/docs/5.7/api-management/gateway-events#event-handling-with-webhooks) that will be triggered for different events
...
In the **Uptime Tests** tab you can configure Tyk's [Uptime Test](/docs/5.7/api-management/gateway-config-tyk-classic#uptime-tests) functionality
...
[User Owned Analytics](/docs/5.7/api-management/user-management#user-permissions), introduced in Tyk v5.1, can be used to limit the visibility of aggregate statistics to users when API Ownership is enabled. Due to the way that the analytics data are aggregated, not all statistics can be filtered by API and so may be inaccessible to users with the Owned Analytics permission.


<Note>
For the Tyk Dashboard's analytics functionality to work, you must configure both per-request and aggregated pumps for the database platform that you are using. For more details see the [Setup Dashboard Analytics](/docs/5.7/api-management/tyk-pump#setup-dashboard-analytics) section.
</Note>
...
<Note>
From Tyk v5.1 (and LTS patches v4.0.14 and v5.0.3) the Error Breakdown and Endpoint Popularity charts will not be visible to a user if they are assigned the [Owned Analytics](/docs/5.7/api-management/user-management#user-permissions) permission.
</Note>
...
To enable request and response logging, please take a look at [useful debug modes](/docs/5.7/api-management/troubleshooting-debugging#capturing-detailed-logs) .
...
<Note>
From Tyk v5.1 (and LTS patches v4.0.14 and v5.0.3) the <b>Traffic per Key</b> screen will not be visible to a user if they are assigned the [Owned Analytics](/docs/5.7/api-management/user-management#user-permissions) permission.
</Note>
...
You can alternatively select only a subset of the endpoints to include in the aggregated data by setting `track_all_paths` to `false` and identifying specific endpoints to be "tracked". These are identified by the `TrackPath` [flag](/docs/5.7/api-management/tyk-pump#trackpath) being set to `true` in the record. In this configuration, the Pump will only include transaction records from "tracked" endpoints in the aggregated data.
...
<Note>
The *track endpoint* middleware only affects the inclusion of endpoints in the per-endpoint aggregates, it does not have any impact on other [aggregated data](/docs/5.7/api-management/logs-metrics#aggregated-analytics) nor the [per-request data](/docs/5.7/api-management/dashboard-configuration#activity-logs).
</Note>
...
<Note>
From Tyk v5.1 (and LTS patches v4.0.14 and v5.0.3) the <b>Geographic Distribution</b> screen will not be visible to a user if they are assigned the [Owned Analytics](/docs/5.7/api-management/user-management#user-permissions) permission.
</Note>


**MaxMind Settings**

To use a MaxMind database, see [MaxMind Database Settings](/docs/5.7/tyk-oss-gateway/configuration#analytics_configenable_geo_ip) in the Tyk Gateway Configuration Options.
...
<Note>
From Tyk v5.1 (and LTS patches v4.0.14 and v5.0.3) the Errors by Category data will not be visible to a user if they are assigned the [Owned Analytics](/docs/5.7/api-management/user-management#user-permissions) permission.
</Note>
...
<Note>
From Tyk v5.1 (and LTS patches v4.0.14 and v5.0.3) the Traffic per OAuth Client ID charts will not be visible to a user if they are assigned the [Owned Analytics](/docs/5.7/api-management/user-management#user-permissions) permission.
</Note>
...
Multi-tenant deployments with [role-based access control](/docs/5.7/api-management/user-management#) enabled allows an admin user to give different users or groups access to a sub-set of the entire API portfolio. Categories can be aligned with the API ownership rules that you have deployed to allow filtering the list of APIs for those visible to each separate user group/team.
...
API categories can be added and removed from APIs within the [API Designer](#api-designer), via the [Tyk Dashboard API](#tyk-dashboard-api), or via [Tyk Operator](/docs/5.7/api-management/automations/operator#what-is-tyk-operator).
...
#### Tyk Dashboard API
The [Tyk Dashboard API](/docs/5.7/tyk-dashboard-api) provides endpoints to manage categories directly, if you are not using the API Designer.
...
You can manage categories using Tyk Operator custom resources. Please refer to [Tyk Operator](/docs/5.7/api-management/automations/operator#api-categories) documentation to see how to manage API categories for Tyk OAS APIs and Tyk Classic APIs.
...
<Note>
API Templates are exclusive to [Tyk OAS APIs](/docs/5.7/api-management/gateway-config-introduction#api-definitions) and can be managed via the Tyk Dashboard API or within the Tyk Dashboard UI.
</Note>
...
When you use a template during the [creation](/docs/5.7/api-management/gateway-config-managing-oas#creating-an-api) of an API, the fields configured in `data` will be pre-set in your new API. You are able to modify these during and after creation of the template. No link is created between the API and the template, so changes made to the API will not impact the template.

#### Merging with an OpenAPI description or Tyk OAS API definition
When you use a template during the creation of an API where you [import](/docs/5.7/api-management/gateway-config-managing-oas#importing-an-openapi-description-to-create-an-api) the OpenAPI document or a full Tyk OAS API definition, the template is combined with the imported OAS description. If the `x-tyk-api-gateway` extension exists in the template, it will be applied to the newly created API.
...
<Note>
API Templates are exclusive to [Tyk OAS APIs](/docs/5.7/api-management/gateway-config-introduction#api-definitions).
</Note>
...
You can then control OPA functionality on a global level via your `tyk_analytics.conf` file, or at an organization level using either the [OPA API](/docs/5.7/api-management/dashboard-configuration#open-policy-agent-api) or the [Dashboard](#using-the-open-policy-agent-in-the-dashboard).
...
Additionally, users can modify OPA rules, and define their own, through the [OPA API](/docs/5.7/api-management/dashboard-configuration#additional-permissions-api). For Self-Managed installations you can access and modify the OPA rules from your Tyk installation directory from [schemas/dashboard.rego](/docs/5.7/api-management/dashboard-configuration#dashboard-opa-rules).
...
To include the `API Editor` role with additional permissions, send a PUT Request to the [Dashboard Additional Permissions API endpoint](/docs/5.7/api-management/dashboard-configuration#additional-permissions-api) `/api/org/permissions`
...
The Tyk Dashboard Admin API provides the following administrator level functions:
 ...
 - Creating initial [users](/docs/5.7/api-management/dashboard-configuration#users-api-1) during boot-strapping of the system.
 - Forcing a [URL reload](/docs/5.7/api-management/dashboard-configuration#url-reload-api).
...
Self-Managed users should use [API tagging](/docs/5.7/api-management/multiple-environments#api-tagging-with-on-premises) and enforce a tagging standard across all organizations.
...
Please consult [Tyk Dashboard Configuration Options](/docs/5.7/tyk-dashboard/configuration#audit) for equivalent configuration with environment variables.
...
Since Tyk 5.7.0 a new API endpoint has been added to allow authorized users to retrieve audit logs from the database storage. To know more about the API specifications, check out the swagger [documentation](/docs/5.7/tyk-dashboard-api).
...
Please check [here](/docs/5.7/tyk-self-managed#mongodb) for MongoDB driver and production configurations.
...
<Note>
Tyk no longer supports SQLite as of Tyk 5.7.0. To avoid disruption, please transition to [PostgreSQL](/docs/5.7/tyk-self-managed#postgresql), [MongoDB](/docs/5.7/tyk-self-managed#mongodb), or one of the listed compatible alternatives.
</Note>
...
3. **Logs**: When [detailed logging](/docs/5.7/api-management/troubleshooting-debugging#capturing-detailed-logs) is enabled, request and response data is logged to storage. These logs can previewed in the Dashboard [log browser](#activity-logs).
...
Please refer to the [storage configuration](/docs/5.7/tyk-dashboard/configuration#storage) section to explore the parameters for configuring Tyk Dashboard to read from different storage layers.


### Configure Pump to Write to Data Storage Layers?

Please consult the Pump configuration [guide](/docs/5.7/api-management/tyk-pump#sql-uptime-pump) for an explanation of how to configure Tyk Pump to write to different storage layers.
...
Further details for configuring an uptime SQL database are available [here](/docs/5.7/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#uptime_pump_configuptime_type)
...
Further details for configuring a Tyk Mongo Pump are available [here](/docs/5.7/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#uptime_pump_config)
