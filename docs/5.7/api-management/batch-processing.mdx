...
If sequential execution is in use, Tyk will work through the entire `requests` array regardless of whether any requests return errors. All responses (success and failure) will be logged and returned to the client as described [below](/docs/5.7/api-management/batch-processing#batch-response-format).
...
You can make requests to the logical batch request endpoint from within [custom JavaScript middleware](/docs/5.7/api-management/plugins/javascript) via the `TykBatchRequest` function that is included in Tyk's  [JavaScript API](/docs/5.7/api-management/plugins/javascript#javascript-api).
...
- using [Internal Looping](/docs/5.7/advanced-configuration/transform-traffic/looping) to put the batch request API behind a protected API
