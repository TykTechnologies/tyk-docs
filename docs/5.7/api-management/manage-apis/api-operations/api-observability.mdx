...
Explore our guides for [Datadog](/docs/5.7/api-management/logs-metrics#datadog), [Dynatrace](/docs/5.7/api-management/logs-metrics#dynatrace), [Jaeger](/docs/5.7/api-management/logs-metrics#using-docker) and [New Relic](/docs/5.7/api-management/logs-metrics#new-relic) for further info on how to integrate with 3rd party observability vendors.
...
Tyk allows you to capture and analyze logs related to API requests and responses in the [Log Browser](/docs/5.7/api-management/dashboard-configuration#activity-logs) . You can optionally enable detailed recording for the requests per API level or per Key level to store inbound request and outbound response data. You can [enable debug modes](/docs/5.7/api-management/troubleshooting-debugging#capturing-detailed-logs) for selected APIs and send the detail logs to one or more Pump backend instances.
