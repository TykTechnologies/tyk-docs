...
2. ##### Users receive 502 error in the Gateway
    ...
    **Solution**
    ...
    See [Tyk Gateway configuration](/docs/5.7/tyk-oss-gateway/configuration) and [Tyk Gateway API](/docs/5.7/api-management/gateway-config-tyk-classic) for further information regarding API definition settings.
...
3. ##### Gateway proxy error "context canceled"
    ...
    See [this guide](/docs/5.7/tyk-self-managed#resource-limits).
...
4. ##### Invalid memory address or nil pointer dereference error
    ...
    *   [API Definition Object Details](/docs/5.7/api-management/gateway-config-tyk-classic)
    *   [API Management](/docs/5.7/tyk-gateway-api)
...
5. ##### Users receive this error message when attempting to make API calls to an existing key.
    ...
    **Solution**
    The user will need to add the key-value pair `meta_data: {}` to their key as per the [Tyk Gateway REST API Documentation](/docs/5.7/tyk-gateway-api).
...
6. ##### There was a problem proxying the request
    ...
    **Solution**
    ...
    Users are advised to upgrade to the latest versions of any Tyk packages at their earliest convenience as a patch was released to resolve this issue. Packages are available to download from [Packagecloud.io][1]. See [Upgrading Tyk](/docs/5.7/developer-support/upgrading) for details on upgrading to the latest version. It may also be worth checking if any TLS certificates associated with the domain have expired.
...
10. ##### How to import existing keys into Tyk CE
    ...
    See also the Keys section of the [Tyk Gateway API documentation](/docs/5.7/tyk-gateway-api).
...
11. ##### DRL not ready, skipping this notification
    ...
    **Cause**
    ...
    There can be a couple of reasons for seeing this error about the [Distributed Rate Limiter](/docs/5.7/api-management/rate-limit#rate-limiting-layers):
...
12. ##### "Index out of range“ error in logs
    ...
    **Solution**
    ...
    Users are advised to upgrade to the latest versions of any Tyk packages as a patch was released to resolve this issue. Packages are available to download from [Packagecloud.io](https://packagecloud.io/tyk) and further details on how to upgrade can be found [here](/docs/5.7/developer-support/upgrading).
...
Tyk supports [error templating](/docs/5.7/api-management/gateway-events#error-templates), allowing you to configure the Gateway to return customised messages for certain HTTP error codes.

We also support limited customisation of the error codes and messages returned by custom authentication middleware through the use of [override messages](/docs/5.7/tyk-oss-gateway/configuration#override_messages).
...
1. ##### Can't update policy. Please ensure at least one access rights setting is set
    ...
    **Solution**
    ...
    Users should first [create a new API](/docs/5.7/api-management/gateway-config-managing-classic#create-an-api) and then [create a new policy](/docs/5.7/api-management/gateway-config-managing-classic#secure-an-api) with an existing API in the Access Rights.
...
12. ##### How to Connect to DocumentDB with X.509 client cert
    ...
    Additionally DocumentDB can't be exposed to the local machine outside of the Amazon Virtual Private Cloud (VPC), which means that even if verification is turned on, it will always fail since if we use a SSH tunnel or a similar method, the domain will differ from the original. Also, it can have [Mutual TLS](/docs/5.7/api-management/client-authentication#use-mutual-tls) enabled.

    So, in order to support it, we provide the following variables for both our [Tyk Analytics Dashboard](/docs/5.7/tyk-dashboard/configuration) and [Tyk Pump](/docs/5.7/api-management/tyk-pump#tyk-pump-configuration):
    ...
    **Capped Collections**
    ...
    If you are using DocumentDB, [capped collections](/docs/5.7/api-management/tyk-pump#tyk-pump-capping-analytics-data-storage) are not supported. See [here](https://docs.aws.amazon.com/documentdb/latest/developerguide/mongo-apis.html) for more details.
...
13. ##### How to disable an API
    ...
    See [Dashboard API - API Definitions](/docs/5.7/api-management/dashboard-configuration#manage-apis-api-definition) for more details on how to GET and PUT an API definition.
...
14. ##### How to Setup CORS
    ...
    **Upstream does not handle CORS**
    ...
    To learn more, look for `CORS.options_passthrough` [here](/docs/5.7/api-management/gateway-config-tyk-classic#cross-origin-resource-sharing-cors).
    ...
    **CORS middleware is allowing headers which I did not allow**
    ...
    The same can happen with Allowed Origins and Allowed Methods. Read more about it [here](/docs/5.7/api-management/gateway-config-tyk-classic#cross-origin-resource-sharing-cors).
...
18. ##### How to run two Gateways with docker-compose
    ...
    Managing a second Tyk Gateway with our [Tyk Pro Docker Demo](/docs/5.7/tyk-self-managed#docker-compose-setup) is a case of mounting the `tyk.conf` file into a new volume and declaring a new Gateway service but exposed on a different port.
...
19. ##### “Payload signature is invalid!“ error
    ...
    **Solution**
    ...
    See [System Payloads](/docs/5.7/api-management/security-best-practices#sign-payloads) for more details.

## Pump

1. ##### Capturing detailed logs

    If you've seen the documentation for Tyk Dashboard's [log browser](/docs/5.7/api-management/dashboard-configuration#activity-logs), then you'll also be wondering how to set up your Tyk configuration to enable detailed request logging.

    **What is detailed request logging?**

    When [detailed request logging](/docs/5.7/api-management/logs-metrics#capturing-detailed-logs) is enabled, Tyk will record the request and response in wire-format in the analytics database. This can be very useful when trying to debug API requests to see what went wrong for a user or client.

    This mode is configured in the gateway and can be enabled at the [system](/docs/5.7/api-management/logs-metrics#configure-at-gateway-level), [API](/docs/5.7/api-management/logs-metrics#configure-at-api-level) or [access key](/docs/5.7/api-management/logs-metrics#configure-at-key-level) level.
...
3. ##### Data Seen in Log Browser but No Reports
    ...
    **Solution**
    ...
    If your Pump is configured to use `mongo_selective_pump` (e.g. store data in a collection per organization), ensure that the [Dashboard configuration setting](/docs/5.7/tyk-dashboard/configuration) `use_sharded_analytics` is set to `true`. 

    The same applies in the reverse direction. If you are using `mongo-pump-aggregate` in your [pump configuration](/docs/5.7/api-management/tyk-pump#tyk-pump-configuration), set `use_sharded_analytics` to false.
...
5. ##### Tyk Pump Panic “stack exceeds 1000000000-byte limit“
    ...
    **Solution**
    ...
    Users are advised to upgrade to the latest version of Tyk. They must also ensure that their Pump is configured with a `purge_delay` and an `optimisation_max_active` value that's greater than 0. Packages are available to download from [Packagecloud.io](https://packagecloud.io/tyk) and further details on how to upgrade can be found [here](/docs/5.7/developer-support/upgrading)
...
3. ##### Mongo version
    ...
    Does Tyk support the version of Mongo that you’re using? Read more about that [here](/docs/5.7/tyk-self-managed#mongodb).
...
4. ##### Capped collections
    ...
    You can determine each collection's cap size by visiting our [MongoDB sizing calculator](/docs/5.7/tyk-self-managed#mongodb-sizing-guidelines).
...
8. ##### How to Cap analytics data storage
    ...
    [Time Based Caps](/docs/5.7/api-management/tyk-pump#time-based-cap-in-single-tenant-environments)

    [Size Based Caps](/docs/5.7/api-management/tyk-pump#size-based-cap)
...
9. ##### MongoDB X.509 Client Authentication
    ...
    You can use the *MongoDB X509 Certificate* flow to authenticate the *Tyk Dashboard*, *Tyk Pump*, and *Tyk MDCB* with your *MongoDB* install.  This is slightly different from [AWS DocumentDB setup instructions](/docs/5.7/api-management/troubleshooting-debugging#how-to-connect-to-documentdb-with-x509-client-cert).
...
1. ##### Gateway `/hello` endpoint
    ...
    Querying the gateway's `/hello` health endpoint is the quickest way to determine the status of your Tyk instance. You can find more information in our docs about the [Gateway Liveness health check](/docs/5.7/tyk-self-managed#set-up-liveness-health-checks).
...
2. ##### Dashboard Debug Settings
    ...
    You can find the full [log levels](/docs/5.7/api-management/logs-metrics#system-logs) in our documentation.

#### Versions

You can access all Tyk release information on the [release notes](/docs/5.7/developer-support/release-notes/overview) overview page.

We recommend always using the [Long-Term Support (LTS) release](/docs/5.7/developer-support/release-types/long-term-support) for stability and long term support.
...
For the best experience when experimenting with Tyk and exploring its latest capabilities, you can use our latest version. You can access all Tyk releases on the [release notes summary](/docs/5.7/developer-support/release-notes/overview) page. 

#### Dashboard

The Dashboard front-end (GUI included) uses [Tyk Dashboard API](/docs/5.7/tyk-dashboard-api) to retrieve data to display or update. This means you can use the [developer tools on your browser](https://developer.mozilla.org/en-US/docs/Learn/Common_questions/Tools_and_setup/What_are_browser_developer_tools) to access the API and its information. Looking into the API details, the URL, the headers, the payload and the response can help you investigate the source of the issue and replicate it with API calls using an HTTP client such as [cURL](https://curl.se/) or [Postman](https://www.postman.com/).
As a next step to this investigation, if that specific endpoint exists also in [Tyk Gateway API](/docs/5.7/tyk-gateway-api), you can compare the responses from both gateway and dashboard requests. 
...
If it works with the gateway API only, then the issue is likely to be in the Dashboard. It could be that you need to set in the Dashboard some [configuration parameters](/docs/5.7/tyk-dashboard/configuration) (using the config file or via environment variables).
