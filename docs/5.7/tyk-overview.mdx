...
- **[Tyk Gateway](/docs/5.7/tyk-oss-gateway)**: The core of Tyk’s platform, providing high-performance API routing, traffic management, and security.
- **[Tyk Dashboard](/docs/5.7/tyk-dashboard)**: A graphical control panel that simplifies API management, configuration, and monitoring.
- **[Tyk Developer Portal](/docs/5.7/tyk-developer-portal)**: A self-service portal that enables developers to access, understand, and integrate with APIs.
- **[Tyk Multi Data Centre Bridge (MDCB)](/docs/5.7/tyk-multi-data-centre)**: Allows centralized control over APIs distributed across multiple data centers or cloud regions.
- **[Tyk Pump](/docs/5.7/tyk-pump)**: Collects and streams analytics from the Tyk Gateway to various storage backends for performance monitoring and reporting.
- **[Tyk Operator](/docs/5.7/api-management/automations/operator#what-is-tyk-operator)**: Kubernetes-native API management that allows teams to manage APIs as Kubernetes resources.
- **[Tyk Streams](/docs/5.7/api-management/event-driven-apis#)**: Enables real-time data streaming and push-based communication for applications requiring live data.
- **[Tyk Sync](/docs/5.7/api-management/automations/sync)**: Synchronizes API configurations across environments, supporting DevOps practices and CI/CD workflows.
- **[Tyk Identity Broker](/docs/5.7/api-management/external-service-integration#what-is-tyk-identity-broker-tib)**: Integrates with external identity providers for single sign-on (SSO) and centralized identity management.
- **[Tyk Helm Charts](/docs/5.7/product-stack/tyk-charts/overview)**: Simplifies the deployment of Tyk components within Kubernetes environments.
- **[Universal Data Graph](/docs/5.7/api-management/data-graph#overview)**: Provides a single GraphQL endpoint that aggregates data from multiple sources, simplifying access to complex data.
...
You can learn more about the components that make up Tyk, [here](/docs/5.7/tyk-components).
...
Whether you’re a startup looking to build a simple API or a global enterprise deploying complex, multi-region architectures, Tyk has the tools to support your growth at every stage. If you face problems with scaling your solutions, learn more about how Tyk can support you by [getting started with Tyk Cloud](/docs/5.7/getting-started/create-account).
