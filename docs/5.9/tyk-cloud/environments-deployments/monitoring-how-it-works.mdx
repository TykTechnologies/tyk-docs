...
canonical: "https://tyk.io/docs/docs/5.9/tyk-cloud/environments-deployments/monitoring-how-it-works"
---
...
#### Tyk Cloud Storage
When a client makes a request to a Tyk Gateway deployment, the details of the request and response are captured and [stored in Redis](/docs/5.9/api-management/dashboard-configuration#traffic-analytics). Tyk Pump processes the records from Redis and forwards them to MongoDB. Finally, Tyk Cloud reads that data from MongoDB and displays its size(bytes) in the _Storage_ section of _Monitoring_. 
