...
canonical: "https://tyk.io/docs/docs/5.9/api-management/automations/sync"
---
...
Starting with Sync v1.5+ and Dashboard v5.3.2+, Tyk Sync supports both [Tyk OAS APIs](/docs/5.9/api-management/gateway-config-tyk-oas) and [Tyk Classic APIs](/docs/5.9/api-management/gateway-config-tyk-classic) when working with the Tyk Dashboard, without requiring special flags or configurations.

For Sync versions v1.4.1 to v1.4.3, enabling Tyk Sync for Tyk OAS APIs requires the [allow-unsafe-oas](/docs/5.9/tyk-dashboard/configuration#allow_unsafe_oas) configuration in the Dashboard, along with the `--allow-unsafe-oas` flag when invoking Tyk Sync. Note that Tyk Sync versions v1.4.1 to 1.4.3 do not support API Category for Tyk OAS APIs.
...
To install Tyk Sync, follow the instructions in the [Tyk Sync installation guide](/docs/5.9/product-stack/tyk-sync/installing-tyk-sync). You can install Tyk Sync using Docker or download the binary directly.

## Quick Start Guide

For a quick start guide to using Tyk Sync, refer to the [Tyk Sync Quick Start Guide](/docs/5.9/api-management/sync/quick-start). This guide will help you set up Tyk Sync, dump API configurations, and synchronize them with your Tyk Dashboard.
