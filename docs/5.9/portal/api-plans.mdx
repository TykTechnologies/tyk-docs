...
canonical: "https://tyk.io/docs/docs/5.9/portal/api-plans"
---

## Introduction

API Plans, sometimes referred to as Subscription Plans, define the terms and conditions under which developers can access your [API Products](/docs/5.9/portal/api-products). They establish the usage limits, approval requirements, and commercial tiers that govern API consumption. While API Products define what functionality is available, API Plans determine how much of that functionality can be used and under what conditions.
...
- When you [synchronize](/docs/5.9/portal/api-provider#synchronizing-developer-portal-with-providers) the Developer Portal with the Provider, a new API Plan will be created automatically
...
- **Unlimited Requests**: Select the checkbox to set no rate limit for this Plan; note that an [API-level rate limit](/docs/5.9/api-management/rate-limit#rate-limiting-scopes-api-level-vs-key-level) would still be applied if configured.
...
- **Behavior**: This configuration is required when the Plan is used with an API Product for which the [Dynamic Client Registration](/docs/5.9/tyk-stack/tyk-developer-portal/enterprise-developer-portal/api-access/dynamic-client-registration) flow has been activated
    ...
    - when a JWT is presented containing the Plan's scope, the Gateway will [apply the Plan to the session](/docs/5.9/basic-config-and-security/security/authentication-authorization/json-web-tokens)
