...
canonical: "https://tyk.io/docs/docs/5.9/tyk-developer-portal/tyk-portal-classic/customise/changing-the-navigation"
---
...
<ul class="nav navbar-nav">
  ...
  <li><a href="/docs/5.9/portal/{{$menuItem.URL}}">{{$menuItem.Title}}</a></li>
  {{ end }}
  <li><a href="/docs/5.9/portal/apis/">API Catalog</a></li>
  {{ if not .PortalConfig.DisableSignup }}
    {{ if not .UserData }}
    <li><a href="/docs/5.9/portal/register/">Register</a></li>
    {{ end }}
  {{ end }}
  {{ if not .PortalConfig.DisableLogin }}
    {{ if not .UserData }}
    <li><a href="/docs/5.9/portal/login/">Log in</a></li>
    {{ end }}
  {{ end }}
</ul>
```
