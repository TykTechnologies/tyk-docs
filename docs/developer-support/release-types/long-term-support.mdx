...
We provide [full support](/docs/#full-support) for the first 12 months, including regular maintenance intervals every eight weeks. Following this period, we transition to [maintenance support](/docs/#maintenance-support) for an additional 12 months, focusing on critical fixes and essential security patching as needed.
...
The compatibility matrix table shown above will be part of upcoming [Gateway release notes](/docs/developer-support/release-notes/gateway) for versions x.x and beyond. Additionally, these release notes will list tested third-party dependencies like *PostgreSQL, MongoDB, Redis* and more. */}
...
When selecting the "Latest" option, users can only choose the latest feature branch ([Gateway Release Notes](/docs/developer-support/release-notes/gateway)).
...
We have step-by-step install guides for various architectures and installation types. Refer to [upgrading tyk](/docs/developer-support/upgrading) for further details.
