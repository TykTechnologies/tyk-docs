...
#### Release Highlights
Tyk Charts 4.0.1 is a version alignment release that ensures compatibility with the most recent Tyk LTS release [5.8.6](/docs/developer-support/release-notes/dashboard#586-release-notes) and Developer Portal [1.14.1](/docs/developer-support/release-notes/portal#1141-release-notes). No functional changes have been implemented in this release.
...
This release includes improvements to support for Redis Sentinel deployments and updates the default charts to install the most recent Tyk LTS release [5.8.5](/docs/developer-support/release-notes/dashboard#585-release-notes) and Developer Portal [1.14.0](/docs/developer-support/release-notes/portal#1140-release-notes).

For a comprehensive list of changes, please refer to the detailed [changelog](/docs/#Changelog-v4.0.0) below.
...
Resolved an issue where the Redis Sentinel password ([TYK_GW_STORAGE_SENTINELPASSWORD](/docs/tyk-oss-gateway/configuration#storagesentinel_password)) was not correctly picked up by Helm charts. Previously, both Redis and Sentinel passwords referenced the same secret key, leading to the Sentinel password defaulting to the regular Redis password. This fix ensures correct handling of distinct Sentinel passwords.
</Accordion>
</AccordionGroup>
</li>
</ul>
...
For a comprehensive list of changes, please refer to the detailed [changelog](/docs/#Changelog-v3.0.0) below.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/docs/#Changelog-v2.2.0) below.
...
To configure, see [Tyk Stack](/docs/product-stack/tyk-charts/tyk-stack-chart#audit-log-configurations) documentation.
</Accordion>
...
To configure, see [Tyk Stack](/docs/product-stack/tyk-charts/tyk-stack-chart#opa-configurations) documentation.
</Accordion>
...
To configure, see [Tyk Stack](/docs/product-stack/tyk-charts/tyk-stack-chart#gateway-probes) documentation.
</Accordion>
...
For a comprehensive list of changes, please refer to the detailed [changelog](/docs/#Changelog-v2.1.0) below.

#### Breaking Changes
{/* Required. Use the following statement if there are no breaking changes, or explain if there are */}
This release has no breaking changes.

However, if you are upgrading to [Tyk Operator v1.0](/docs/developer-support/release-notes/operator#100-release-notes) using the Helm Chart, please read the [license requirement](/docs/developer-support/release-notes/operator#breaking-changesv1.0.0) and Tyk Operator [installation and upgrade instructions](/docs/tyk-stack/tyk-operator/installing-tyk-operator) carefully.
...
- Familiarize yourself with URL matching in Tyk [here](/docs/getting-started/key-concepts/url-matching).
- For production setup guidance, see [this guide](/docs/planning-for-production#ensure-you-are-matching-only-the-url-paths-that-you-want-to-match).
...
Learn more about the settings in the [URL Path Matching](/docs/getting-started/key-concepts/url-matching) documentation.
</Expandable>
</li>
...
MDCB v2.7.0 release introduces `/liveness` and `/readiness` probes which give more accurate and detail health check information. MDCB deployment has been updated to use the new endpoints. See [MDCB Health Check](/docs/api-management/mdcb#health-check) section for information about the new probes.
...
For more details about new endpoints, check [MDCB Health check](/docs/api-management/mdcb#health-check) section.
</Expandable>
</li>
</ul>
...
For a comprehensive list of changes, please refer to the detailed [changelog](/docs/#Changelog-v1.5.0) below.
...
To leverage this stable release and simplify your Tyk deployments, we invite you to explore our example setup for MDCB Control Plane using Helm Chart. Simply follow our [MDCB Control Plane setup guide](/docs/api-management/mdcb#setup-mdcb-control-plane) to get started.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/docs/#Changelog-v1.4.0) below.
...
#### Breaking Changes
{/* Required. Use the following statement if there are no breaking changes, or explain if there are */}
For MongoDB users: Tyk Charts 1.3.0 uses `mongo-go` as the default driver to connect to MongoDB. `mongo-go` driver is compatible with MongoDB 4.4.x and above. For MongoDB versions prior to 4.4, please set `global.mongo.driver` to `mgo`. We recommend reading [Choose a MongoDB driver](/docs/planning-for-production/database-settings#choose-a-mongodb-driver) when you need to change driver setting.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/docs/#Changelog-v1.3.0) below.
...
##### Graph Pump
Tyk Charts 1.3 adds support for Graph MongoDB Pump, Graph SQL Pump and Graph SQL Aggregate Pump. see [Graph Pump setup](/docs/api-management/tyk-pump#graph-pump-setup) to learn more about the GraphQL-specific metrics available.

##### Enable Tyk Identity Broker (TIB) in Tyk Dashboard
Tyk Charts 1.3 adds a field to enable Internal [Tyk Identity Broker (TIB)](/docs/api-management/external-service-integration#what-is-tyk-identity-broker-tib) in Tyk Dashboard by field `tyk-dashboard.tib.enabled` to `true`.
...
When enabled at `gateway.control.ingress.enabled`, an Ingress resource will be created to allow external access to gateway's [control service](/docs/planning-for-production#change-your-control-port).
</Accordion>

<Accordion title={'Gateway: Configure Gateway to work with MDCB synchroniser'}>
Allow users to configure worker gateway to work with [Tyk MDCB synchroniser](/docs/api-management/mdcb#synchroniser-feature-with-mdcb) easily by setting `global.mdcbSynchronizer.enabled` in `tyk-data-plane`.
...
You can enable Internal [Tyk Identity Broker (TIB)](/docs/api-management/external-service-integration#what-is-tyk-identity-broker-tib) in Tyk Dashboard by field `tyk-dashboard.tib.enabled` to `true`.
</Accordion>
...
read [Tyk Control Plane chart](/docs/product-stack/tyk-charts/tyk-control-plane-chart).
</Accordion>
