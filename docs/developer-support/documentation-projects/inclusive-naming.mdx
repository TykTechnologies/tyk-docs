...
- [allow_master_keys](/docs/tyk-oss-gateway/configuration#allow_master_keys)
- [analytics_storage.master_name](/docs/tyk-oss-gateway/configuration#analytics_storagemaster_name)
- [cache_storage.master_name](/docs/tyk-oss-gateway/configuration#cache_storagemaster_name)
- [storage.master_name](/docs/tyk-oss-gateway/configuration#storagemaster_name)
- [slave_options](/docs/tyk-oss-gateway/configuration#slave_options)
- [blacklisted_ips](/docs/api-management/gateway-config-tyk-classic#ip-access-control)
- [disable_ports_whitelist](/docs/key-concepts/tcp-proxy#allowing-specific-ports)
- [enable_ip_blacklisting](/docs/api-management/gateway-config-tyk-classic#ip-access-control)
- [ports_whitelist](/docs/key-concepts/tcp-proxy#allowing-specific-ports)
...
- [version_data.versions.{version-name}.extended_paths.black_list](/docs/api-management/traffic-transformation/block-list#api-definition). There is also a parameter with equivalent functionality under the `paths` object (`version_data.versions.{version_name}.paths.black_list`).
- [version_data.versions.{version-name}.extended_paths.white_list](/docs/api-management/traffic-transformation/allow-list#api-definition). There is also a parameter with equivalent functionality under the `paths` object (`version_data.versions.{version_name}.paths.while_list`).

##### Tyk Dashboard

**Config parameters**
- [enable_master_keys](/docs/tyk-dashboard/configuration#enable_master_keys)
- [redis_master_name](/docs/tyk-dashboard/configuration#redis_master_name)

**Tyk Classic API Definition**

The [Tyk Dashboard OpenAPI Document](https://github.com/TykTechnologies/tyk-docs/blob/master/tyk-docs/assets/others/dashboard-swagger.yml) (Tyk Dashboard swagger), contains references to [the parameters from the above Tyk Classic API Definition list](/docs/#gw-classic-api-definition).

**Dashboard UI**

The Tyk Classic APIs *Endpoint Designer* shows configuration of [blacklist](/docs/api-management/traffic-transformation/block-list#api-designer) and [whitelist](/docs/api-management/traffic-transformation/allow-list#api-definition) middleware plugins.
    
##### Tyk MDCB

The following MDCB configuration parameters contain tier 1 word occurrences:
- [analytics_storage.master_name](/docs/tyk-multi-data-centre/mdcb-configuration-options#analytics_storagemaster_name)
- [storage.master_name](/docs/tyk-multi-data-centre/mdcb-configuration-options#storagemaster_name)

##### Tyk Pump

The following Tyk Pump configuration parameters contain tier 1 word occurrences:
- [analytics_storage_config.master_name](/docs/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#analytics_storage_configmaster_name)
...
- Tyk Classic Developer Portal provides [documentation](/docs/tyk-developer-portal/tyk-portal-classic/keycloak-dcr) that explains how to integrate Tyk with Keycloak using the [OpenID Connect Dynamic Client Registration](https://tools.ietf.org/html/rfc7591) protocol. The example in the guide uses the Keycloak default `master` realm.
