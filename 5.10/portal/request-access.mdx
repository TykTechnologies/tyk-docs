...
- [Created](/5.10/portal/overview/getting-started#step-3-create-an-api-consumer-admin-user) an API Consumer Admin user
- [Published](/5.10/portal/publish-api-catalog) an API Catalog with at least one API Product and Plan
...
3. Enter the email address and password for the API consumer admin user you created [previously](/5.10/portal/overview/getting-started#step-3-create-an-api-consumer-admin-user)
4. Select **Log In**

You should now be logged in to the Live Portal as an API consumer. You'll see the *default theme* provided by Tyk, all of which is [customisable](/5.10/portal/customization) for your brand and workflows.
...
2. Select the API Product you created [previously](/5.10/portal/publish-api-catalog#step-2-create-an-api-product)
...
You need to create a [Developer App](/5.10/portal/developer-app) to contain your API credentials for any API Products to which you are granted access.
...
1. Return to the API product page in the catalog, which we did in [Step 2](/5.10/portal/request-access#step-2-browse-the-api-catalog).
...
4. Construct your API request using the access credentials issued in [Step 5](/5.10/portal/request-access#step-5-view-your-api-access-credentials)
