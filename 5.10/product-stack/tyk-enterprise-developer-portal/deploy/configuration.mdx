...
However, if you have  already created [a theme](/5.10/portal/customization#) and want the portal to use when it starts for the first time, then you can use this setting to achieve that.
...
**Type:** `string` <br/>
**Description**: API secret for enabling [Single Sign-on (SSO) flow](/5.10/tyk-stack/tyk-developer-portal/enterprise-developer-portal/managing-access/enable-sso) with the Tyk Identity Broker.
