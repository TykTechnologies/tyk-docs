...
With Tyk Gateway you can store configuration data (typically authentication secrets or upstream server locations) in Key-Value (KV) systems such as [Vault](/5.10/#vault), and [Consul](/5.10/#consul) and then reference these values during configuration of the Tyk Gateway or APIs deployed on the Gateway.
...
Storing local settings within the Tyk Gateway's configuration file allows you to have per instance variables, such as a machine ID, and inject these into API requests and responses using [transformation middleware](/5.10/api-management/traffic-transformation).
...
The exception to this is for specific [transformation middleware](/5.10/#transformation-middleware) where the value will be retrieved for each call to the API, during the processing of the API request or response.
...
- to retrieve the value assigned to a `KEY` in Consul you will use `consul://KEY` or `$secret_consul.KEY` notation depending on the [location](/5.10/#how-to-access-the-externally-stored-data) of the reference
...
- to retrieve the value assigned to a `KEY` in Vault you will use `vault://KEY` or `$secret_vault.KEY` notation depending on the [location](/5.10/#how-to-access-the-externally-stored-data) of the reference

**Tyk Gateway config file**

The `secrets` section in the [Tyk Gateway configuration file](/5.10/tyk-oss-gateway/configuration#secrets) allows you to store settings that are specific to a single Tyk Gateway instance. This is useful for storing instance-specific configuration to be injected into API middleware or if you prefer using configuration files.
- to retrieve the value assigned to a `KEY` in the `secrets` config you will use `secrets://KEY` or `$secret_conf.KEY` notation depending on the [location](/5.10/tyk-oss-gateway/configuration#secrets) of the reference
...
- if you want to set the local "secrets" section (equivalent to the `secrets` section in the [Gateway config file](/5.10/#how-to-access-the-externally-stored-data)) using environment variables, you should use the following notation: `TYK_GW_SECRETS=key:value,key2:value2`
...
- to retrieve the value assigned to an environment variable `VAR_NAME` you will use `env://VAR_NAME` or `$secret_env.VAR_NAME` notation depending on the [location](/5.10/#how-to-access-the-externally-stored-data) of the reference
...
In Tyk Gateway's configuration file (`tyk.conf`), you can retrieve values from KV stores for the following [fields](/5.10/tyk-oss-gateway/configuration):
- `secret`
- `node_secret`
- `storage.password`
...
For example, if you create a Key-Value pair in [Vault](/5.10/#vault) with the *key* `shared-secret` in *secret* `gateway-dashboard` within directory `tyk-secrets/` then you could use the *Value* as the `node_secret` in your Gateway config by including the following in your `tyk.conf` file:
``` .json
{
  "node_secret":"vault://tyk-secrets/gateway-dashboard.shared-secret"
}
```
When the Gateway starts, Tyk will read the *Value* from Vault and use this as the `node_secret`, which is used to [secure connection to the Tyk Dashboard](/5.10/tyk-oss-gateway/configuration#node_secret).
...
From Tyk Gateway v5.3.0 onwards, you can store [any string field](/5.10/#other-string-fields) from the API definition in any of the supported KV storage options; for earlier versions of Tyk Gateway only the [Target URL and Listen Path](/5.10/#target-url-and-listen-path) fields and [certain transformation middleware](/5.10/#transformation-middleware) configurations were supported. 
...
When the Gateway starts, Tyk will read the *Value* from the environment variable and use this as the [Target URL](/5.10/api-management/gateway-config-tyk-oas#upstream).
...
- [request body transform](/5.10/api-management/traffic-transformation/request-body)
- [request header transform](/5.10/api-management/traffic-transformation/request-headers)
- [URL rewrite](/5.10/transform-traffic/url-rewriting#url-rewrite-middleware)
- [response body transform](/5.10/api-management/traffic-transformation/response-body)
- [response header transform](/5.10/api-management/traffic-transformation/response-headers)
...
- Environment variables: `$secret_env.key` or `env://key` (see [here](/5.10/#using-environment-variables-with-transformation-middleware))
...
When accessing KV references from the `/tyk-apis` directory on Consul or Vault, you should not provide the `path/to/` the key except for Target URL and Listen Path (as described [above](/5.10/#target-url-and-listen-path)).
</Note>
...
When the Gateway starts, Tyk will read the *Value* from the `secrets` section in the Gateway config file and use this to identify the header where Tyk Gateway should look for the [Authentication](/5.10/api-management/gateway-config-tyk-oas#authentication) token in requests to your Tyk OAS API.
...
Alternatively, you can configure it using the equivalent [environment variables](/5.10/tyk-oss-gateway/configuration#kvconsuladdress).

### Where to store data in Consul

When you want to reference KV data from Tyk Gateway config or transform middleware, you can store your KV pairs wherever you like within the Consul KV store. You can provide the Consul path to the key in the reference using the notation appropriate to the calling [location](/5.10/#consul).
...
As described [here](/5.10/#from-the-tyk-gateway-configuration-file), from Tyk Gateway's configuration file (`tyk.conf`) you can retrieve values from Consul using the following notation:
- `consul://path/to/KEY`
...
For [certain transformation middleware](/5.10/#transformation-middleware) because the secret resolution happens during the request context, a different notation is used to retrieve values from Consul:
- `$secret_consul.KEY`
...
Alternatively, you can configure it using the equivalent [environment variables](/5.10/tyk-oss-gateway/configuration#kvvaulttoken).
...
As explained [below](/5.10/#vault), you could retrieve this value from within your Tyk Gateway config file using: 
   `TYK_GW_SECRET=vault://my-secret/tyk.gw`

### Where to store data in Vault

When you want to reference KV data from Tyk Gateway config or transform middleware, you can store your Vault *secrets* wherever you like within the KV store. You can provide the Vault path to the key in the reference using the notation appropriate to the calling [location](/5.10/#vault).
...
As described [here](/5.10/#from-the-tyk-gateway-configuration-file), from Tyk Gateway's configuration file (`tyk.conf`) you can retrieve values from Vault using the following notation:
...
For [certain transformation middleware](/5.10/#transformation-middleware) because the secret resolution happens during the request context, a different notation is used to retrieve values from Vault:
- `$secret_vault.KEY`
