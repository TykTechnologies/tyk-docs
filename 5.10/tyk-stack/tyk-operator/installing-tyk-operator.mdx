...
Cloud](/5.10/tyk-cloud#quick-start-tyk-cloud) or [Tyk Self
Managed](/5.10/tyk-self-managed) page. [Tyk Helm
Chart](/5.10/product-stack/tyk-charts/overview) is the preferred (and easiest) way to install Tyk on Kubernetes.
...
Tyk Operator needs a [user credential](/5.10/api-management/automations/operator#operator-user) to connect with
...
[Using Tyk Operator to enable GitOps with Tyk](/5.10/api-management/automations) about
...
Alternatively, you have the option to manually handle TLS certificates by disabling the `cert-manager` requirement. For more details, please refer to this [configuration](/5.10/#webhook-configuration).
...
If you are using [Tyk Stack](/5.10/product-stack/tyk-charts/tyk-stack-chart), [Tyk Control
Plane](/5.10/product-stack/tyk-charts/tyk-control-plane-chart), or [Tyk Open
Source Chart](/5.10/product-stack/tyk-charts/tyk-oss-chart), you can install Tyk Operator alongside other Tyk
...
another name, configure it through Helm Chart [envFrom](/5.10/#install-tyk-operator-and-custom-resource-definitions-crds) value.
...
You can configure [Tyk Operator as Ingress Controller](/5.10/product-stack/tyk-operator/tyk-ingress-controller) so
...
[WATCH_INGRESS_CLASS](/5.10/#configure-tyk-operator-via-environment-variable-or-tyk-operator-conf-secret) through `tyk-operator-conf` or the environment variable.
