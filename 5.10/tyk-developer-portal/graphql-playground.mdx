...
description: Learn how to set up and use the GraphQL Playground in the Tyk Developer
  Portal
keywords: Tyk Developer Portal, GraphQL Playground, GraphQL API, API Documentation,
...
| :------------- | :---------------------- | :------------------------- |
| Developer Portal | Available since [v1.15.0](/5.10/developer-support/release-notes/portal#1-15-0-release-notes) | Enterprise |
...
- **Working Tyk Environment:** You need access to a running Tyk instance that includes both the Tyk Gateway and Tyk Dashboard components. For setup instructions using Docker, please refer to the [Tyk Getting Started Guide](/5.10/getting-started/quick-start).
...
1. In the **Tyk Dashboard**, create a new **GraphQL API** by
    ...
    - **Star Wars GQL API** uses [Auth Token](/5.10/api-management/authentication/bearer-token) for authentication. Copy the **Key ID** from the pop-up and save it for later use. We will set the authorization header in the Playground using this Key ID.
...
The Tyk Getting Started Guide creates an API Catalog (`Public catalogue`) by default. If you don't have one, refer to this [guide](/5.10/portal/publish-api-catalog#step-1:-create-the-api-catalog) to create an API Catalog in the Developer Portal.
...
3. On the **Details** tab, enter the basic product information:
    ...
    - Publish API product to catalogue: Select the catalog you created [previously](/5.10/#2-create-the-api-catalog)
    - You can leave the other fields empty for now
4. On the **APIs** tab, select the `Star Wars GQL API` created in the [first step](/5.10/#1-create-a-graphql-api):
