...
Tyk cloud is a fully managed service that makes it easy for API teams to create, secure, publish and maintain APIs at any scale, anywhere in the world. Tyk Cloud includes everything you need to manage your global API ecosystem: [Tyk Gateways](/5.10/tyk-oss-gateway), [Tyk Dashboard](/5.10/api-management/dashboard-configuration), [Tyk Developer Portal](/5.10/portal/overview/intro) and [Universal Data Graph](/5.10/api-management/data-graph#overview).
...
The Tyk Cloud trial does not include access to [Hybrid deployments](/5.10/tyk-cloud/environments-deployments/hybrid-gateways) or the [Developer Portal](/5.10/portal/overview/intro).
To try out these capabilities, please get in touch for a [guided evaluation](https://tyk.io/guided-evaluation/) with our team.
</Note>


Welcome to the [Tyk Cloud Platform](/5.10/tyk-cloud)!
This guide will lead you through the following steps:
1. Signing up with [Tyk Cloud ](/5.10/tyk-cloud).
2. Creating your first [API](/5.10/api-management/gateway-config-introduction) using the [Tyk Dashboard](/5.10/api-management/dashboard-configuration).
3. Setting up a [Policy](/5.10/api-management/policies#what-is-a-security-policy) and Key to secure your APIs.
...
To create Tyk Cloud account follow this [guide](/5.10/getting-started/create-account).

2. **Get started with your first API with Tyk Dashboard**

To create your API using Tyk Dashboard follow this [guide](/5.10/getting-started/configure-first-api).

## Comprehensive Tyk Cloud Setup

This section walks you through how to start using Tyk Cloud, creating organization, environment and users before creating an API. If you are in a hurry, try the [Quick Start guide](/5.10/#quick-start-tyk-cloud) for a 5 min version of this tutorial. 
...
* We have some specific terminology used within Tyk Cloud. It would be useful to checkout our [Glossary](/5.10/#glossary) so you understand what we are referring to.
...
* Assign the account creator as a [Billing admin](/5.10/#user-roles-in-tyk-cloud) for the Organization. This user role allows you to manage the billing and plans for your org. You can also add other billing admins as required.
...
* **Step 2 - Select a Home Region:** Select a region from the drop-down list where your [Control Plane](/5.10/#glossary) will be deployed and your data stored. The number of regions available will depend on your license. Further regions can be added as an upgrade option.
...
For a manual setup you'll get started by [setting up your first team](/5.10/#create-your-first-team).
...
* **Step 1 - Name your Team:** Give your [Team](/5.10/#glossary) a name. You may find it useful to reflect the names used within your organization.

* **Step Two - Invite your Users:** Invite your [users](/5.10/#glossary) to your team. You'll only need their email address and which of the available [roles](/5.10/#glossary) you want to assign to them. This step is optional and can be completed within the dashboard later.
...
Next you'll create an [Environment](/5.10/#configure-environment-and-deployments).
...
* **Step 1 - Name your Environment:** Give your [Environment](/5.10/#glossary) a name. You may find it useful to reflect the names used within your organization such as Development, Production etc.
  
* **Step 2 - Name your Control Plane:** Give your [Control Plane](/5.10/#glossary) a name. Again, this is up to you and you may already have an infrastructure you want to re-create in Tyk Cloud.
  
* **Step 3 - Configure your first Cloud Data Plane:** Select the region you want to locate your [Cloud Data Plane](/5.10/#glossary) in from the drop-down list. Your Cloud Data Plane is not confined to the same region as your Organization and Control Plane but the amount of regions you have to choose from can be limited depending on your subscription plan. Give your Cloud Data Plane a name. 
...
1. Click [Deploy Control Plane and Create a Cloud Data Plane](/5.10/#glossary). You can watch your Control Plane being deployed and your Cloud Data Plane being created. You will then be taken to the Control Plane overview screen within the Tyk Cloud dashboard.
...
Next you'll [set up your first API](/5.10/#deploy-and-add-your-first-api) from the Tyk Dashboard.
...
For further details, please refer to the [release notes](/5.10/developer-support/release-notes/gateway#530-release-notes) for Tyk Gateway v5.3.0.
</Warning>


##### Steps to add an API in Tyk Cloud

* **Step 1 - Access the Dashboard:** Go to the Control Plane overview and click the dashboard link in the Ingress list. You'll be redirected to the Tyk Dashboard for your [Control Plane](/5.10/#glossary).
...
Your first API has been added. What's next? Testing it! This page shows how you can test an API that you have added to Tyk Cloud, to ensure that it’s functioning correctly. You'll now access the API you setup in [Task 5](/5.10/#deploy-and-add-your-first-api) from the Cloud Data Plane within Tyk Cloud.
...
* **Step 2 - Append the URL with your API:** You created a API named **my app** in [Task 5](/5.10/#deploy-and-add-your-first-api). Add `/my-app/` to the end of the URL. You should be taken to [https://httpbin.org/](https://httpbin.org/), which you added as the **Target URL** for the API in [Task 5](/5.10/#steps-to-add-an-api-in-tyk-cloud). 


Next you'll [view the analytics](/5.10/#view-analytics) for your API in the Dashboard.
...
* Adding [Plugins and Middleware](/5.10/tyk-cloud/using-plugins) to your Control Plane
...
A set of data and access permissions that can be applied to a user or team of users. See [User Roles](/5.10/#user-roles-in-tyk-cloud) for more details.
...
Where an Organization has expired due to either a subscription failure or cancelation and is now within a "retirement" period of 30 days, during which an [Billing Admin](/5.10/tyk-cloud/teams-users#assign-user-roles) can reinstate full functionality by updating or creating a new subscription.
...
See [User Roles](/5.10/#user-roles-in-tyk-cloud) for more details
