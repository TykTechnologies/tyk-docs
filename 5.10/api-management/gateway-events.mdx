...
- [API events](/5.10/#api-events)
- [Token lifecycle events](/5.10/#token-lifecycle-events)
- [Advanced quota usage events](/5.10/#advanced-quota-usage-events)
- [Custom events](/5.10/#custom-events)

### API events

Tyk can generate (or *fire*) a variety of built-in API events due to activity triggered by an API request, such as exceeded rate limits, depleted quotas or attempts to access using expired keys. The full list of standard API events is available [here](/5.10/api-management/gateway-events#api-events).

### Token lifecycle events

Alongside the events that are fired in response to API requests, Tyk will also mark the creation, update or deletion of access tokens (keys) with dedicated events as indicated [here](/5.10/api-management/gateway-events#token-lifecycle-events).

### Advanced quota usage events

Tyk will generate [standard quota events](/5.10/api-management/gateway-events#standard-quota-events) when a client quota has been consumed, but what if you want to have more granular notification of quota usage as your clients are approaching their quota limit?

For this, Tyk provides [advanced quota monitoring](/5.10/api-management/gateway-events#monitoring-quota-consumption) that can be configured to trigger a dedicated event handler when the API usage exceeds different thresholds approaching the quota limit.
...
Tyk has a simple event handling system where *event handlers* are assigned (or registered) to the different [events](/5.10/api-management/gateway-events#event-types) that Tyk can generate. These handlers are assigned per-API so when an event is generated for an API and there is an *event handler* registered for that *event*, the handler will be triggered.

Three different categories of *event handler* can be registered for each event:
- a [webhook](/5.10/api-management/gateway-events#event-handling-with-webhooks) that will call out to an external endpoint
- an [event log](/5.10/api-management/gateway-events#logging-api-events-1) that will write to the configured [log output](/5.10/api-management/logs-metrics#system-logs)
- your own [custom event handler](/5.10/api-management/gateway-events#custom-api-event-handlers) that will run in a JavaScript virtual machine on the Tyk server



    <Note>
    Remember that <b>quota usage monitoring</b> has a [dedicated mechanism](/5.10/api-management/gateway-events#monitoring-quota-consumption) for handling these special events.
    </Note>
...
- `RateLimitSmoothingUp`: the [intermediate rate limit allowance](/5.10/api-management/rate-limit#rate-limit-smoothing) has been increased for a specific key
- `RateLimitSmoothingDown`: the [intermediate rate limit allowance](/5.10/api-management/rate-limit#rate-limit-smoothing) has been decreased for a specific key
...
When an event is fired, and an *event handler* is registered for that specific API and event combination, Tyk Gateway provides the handler with a rich set of [metadata](/5.10/api-management/gateway-events#event-metadata). The external system (webhook) or custom (JavaScript) code can then use this metadata to decide what action to take.
...
- `originating_request` (string): a Base64-encoded [raw inbound request](/5.10/#raw-request-data)
...
The HTTP method, body, header values, and target URL can all be configured in the API definition. The [request body](/5.10/#webhook-payload) is generated using a Tyk template file that has access to the [event metadata](/5.10/api-management/gateway-events#event-metadata).
...
When your webhook event handler is triggered, it will send an HTTP request to the configured target. For HTTP methods that support a request body, for example `POST`, the event handler will process a [Go template](/5.10/api-management/traffic-transformation/go-templates) to produce the payload.
...
The event handler has access to the [event metadata](/5.10/api-management/gateway-events#event-metadata) and this can be accessed by the template using the `{{.Meta.XXX}}` namespace.

The [event type](/5.10/api-management/gateway-events#event-types) that triggered the event handler can be accessed as `{{.Type}}`.
...
*Global webhook definitions* are registered with the Dashboard using the [UI](/5.10/#creating-a-global-webhook-definition-using-tyk-dashboard) or [Dashboard API](/5.10/api-management/dashboard-configuration#web-hooks-api) and assigned a unique *webhook id* that can be obtained via the [Dashboard API](/5.10/api-management/dashboard-configuration#list-web-hooks) or via drop-down selection within the UI.
...
If you're using Tyk OAS APIs, then you can find details and examples of how to configure webhook event handlers [here](/5.10/api-management/gateway-events#webhook-event-handlers-with-tyk-oas-apis).

If you're using Tyk Classic APIs, then you can find details and examples of how to configure webhook event handlers [here](/5.10/api-management/gateway-events#webhook-event-handlers-with-tyk-classic-apis).

### Webhook event handlers with Tyk OAS APIs

[Webhooks](/5.10/api-management/gateway-events#event-handling-with-webhooks) are event handlers that can be registered against API Events. The webhook will be triggered when the corresponding event is fired and will send a customizable fixed payload to any open endpoint.

Webhooks are configured in the [Tyk OAS API Definition](/5.10/api-management/gateway-config-tyk-oas#operation). You can do this via the Tyk Dashboard API or in the API Designer.

If you're using the legacy Tyk Classic APIs, then check out the [Tyk Classic](/5.10/api-management/gateway-events#webhook-event-handlers-with-tyk-classic-apis) page.
...
- `cooldownPeriod`: the [webhook cooldown](/5.10/api-management/gateway-events#webhook-cooldown) for duplicate events (in duration format, e.g. 10s, 1m30s); use this to prevent flooding of the target endpoint when multiple events are fired in quick succession
...
- `bodyTemplate`: this is the path to the [webhook template](/5.10/api-management/gateway-events#webhook-payload) that will be used to construct the request body
...
- `type`: the type of event handler, in this case should be set to `webhook`
- `cooldownPeriod`: the [webhook cooldown](/5.10/api-management/gateway-events#webhook-cooldown) for duplicate events (in duration format, e.g. 10s, 1m30s); use this to prevent flooding of the target endpoint when multiple events are fired in quick succession
- `id`: the *webhook id* assigned by Tyk to the global webhook when it was created (this can be determined using the [list webhooks](/5.10/api-management/dashboard-configuration#list-web-hooks) endpoint in the Tyk Dashboard API)
...
If you want to use a *global webhook* then you'll need to declare it first, following [these instructions](/5.10/api-management/gateway-events#creating-a-global-webhook-definition-using-tyk-dashboard).
...
[Webhooks](/5.10/api-management/gateway-events#event-handling-with-webhooks) are event handlers that can
...
OAS](/5.10/api-management/gateway-events#webhook-event-handlers-with-tyk-oas-apis)
page.
...
API definition for the appropriate [API event](/5.10/api-management/gateway-events#event-types).
...
- `template_path`: this is the path to the [webhook
  template](/5.10/api-management/gateway-events#webhook-payload) that will be
...
- `event_timeout`: the [webhook
  cooldown](/5.10/api-management/gateway-events#webhook-cooldown) for duplicate
...
[global
webhook](/5.10/api-management/gateway-events#using-webhooks-with-tyk-dashboard).
</Note>
...
to [locally defined webhook handlers](/5.10/#set-up-a-webhook-event-handler-in-the-tyk-classic-api-definition) within the Tyk
...
1. **Define the webhook**
    ...
    **Webhooks** screen in the **API Management** menu, as described
    [here](/5.10/api-management/gateway-events#creating-a-global-webhook-definition-using-tyk-dashboard).
...
API events](/5.10/api-management/gateway-events#event-types).

The process for configuring webhook event handlers using Tyk Operator is similar to that explained in
[Set up a webhook event handler in the Tyk Classic API Definition](/5.10/#set-up-a-webhook-event-handler-in-the-tyk-classic-api-definition).
...
It is important to note that unlike custom JavaScript [plugins](/5.10/api-management/plugins/javascript#), custom event handlers execute in a *global* JavaScript environment. This means that you need to be careful when naming the event handlers: if you use the same event handler name for different event handling code across two APIs, only one of them will execute, as the other will be overridden when loaded.

Custom event handlers have access to the [JavaScript API](/5.10/api-management/plugins/javascript#javascript-api) which gives access to the session object and enables your code to make HTTP calls. This is particularly useful if you want to interface with another API with a complex request/response cycle.
...
Creating an event handler is very similar to [creating custom JS plugins](/5.10/api-management/plugins/javascript#using-javascript-with-tyk), simply invoke the correct constructors with a closure in the TykJS namespace:
...
This contains the [event metadata](/5.10/api-management/gateway-events#event-metadata) in the following structure:
...
Unlike API event [webhooks](/5.10/api-management/gateway-events#event-handling-with-webhooks) the quota monitor is configured at the Gateway level.
...
- `configuration`: a [webhook configuration](/5.10/api-management/gateway-events#event-handling-with-webhooks) object
...
<Note>
If you are using our [Classic Developer Portal](/5.10/tyk-developer-portal/tyk-portal-classic/portal-events-notifications), developers registered in the portal will also receive emails about quota threshold limits being reached.
</Note>
