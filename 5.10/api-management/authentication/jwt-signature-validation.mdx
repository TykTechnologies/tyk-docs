...
When storing the key or secret in the API definition, it is first base64 encoded and then configured in `server.authentication.securitySchemes.<jwtAuthScheme>.source` (in Tyk Classic, this is [jwt_source](/5.10/api-management/gateway-config-tyk-classic#configuring-authentication-for-tyk-classic-apis).
...
Refer to the [Tyk OAS API Definition](/5.10/api-management/gateway-config-tyk-oas#jwt) reference for details.

### External Key Value Store

For improved separation of concerns and flexibility, the key/secret can be placed in an [external key value store](/5.10/tyk-configuration-reference/kv-store), with the appropriate reference configured in the API definition.
...
Tyk can retrieve public keys from JSON Web Key Sets (JWKS) endpoints to validate the signature of incoming JWTs. Tyk supports configuring [single](/5.10/#single-jwks-endpoint) or [multiple](/5.10/#multiple-jwks-endpoints) JWKS endpoints with [caching](/5.10/#jwks-caching) capabilities.
...
From **Tyk 5.10.0** onwards, we have introduced enhanced JWKS caching for [Tyk OAS APIs](/5.10/api-management/gateway-config-tyk-oas) with the following improvements:
...
For more details, refer to the [Tyk OAS API definition reference](/5.10/api-management/gateway-config-tyk-oas#jwk).
...
<Note>
The Dashboard API endpoint is restricted to users with `admin` privileges and can only be used to flush the cache for APIs in the user's [Organisation](/5.10/tyk-dashboard-api#organisations%2C-apis-and-users).
</Note>
