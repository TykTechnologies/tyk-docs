...
When Tyk receives an API request, it works through a [chain](/5.10/api-management/traffic-transformation#request-middleware-chain) of processing *middleware* that is configured using the API definition. There are a large number of built-in middleware in the processing chain that are dedicated to performing [client authentication](/5.10/api-management/client-authentication), [request transformation](/5.10/api-management/traffic-transformation), [caching](/5.10/api-management/response-caching) and many other processes before proxying the request to the upstream.
...
There are several different stages of the [API request lifecycle](/5.10/api-management/traffic-transformation#request-middleware-chain) where custom plugins can be attached (or *hooked*) into the middleware chain allowing significant customization to meet your specific requirements.
...
1. [Pre (Request)](/5.10/api-management/plugins/plugin-types#request-plugins)
2. [Authentication](/5.10/api-management/plugins/plugin-types#authentication-plugins)
3. [Post-Auth (Request)](/5.10/api-management/plugins/plugin-types#request-plugins)
4. [Post (Request)](/5.10/api-management/plugins/plugin-types#request-plugins)
5. [Response](/5.10/api-management/plugins/plugin-types#response-plugins)
6. [Analytics (Response)](/5.10/api-management/plugins/plugin-types#analytics-plugins)
...
1. **[Request Plugin](/5.10/api-management/plugins/plugin-types#request-plugins)**
2. **[Authentication Plugin](/5.10/api-management/plugins/plugin-types#authentication-plugins)**
3. **[Response Plugin](/5.10/api-management/plugins/plugin-types#response-plugins)**
4. **[Analytics Plugin](/5.10/api-management/plugins/plugin-types#analytics-plugins)**

To know more about plugin types and it's advanced configuration, refer the following [docs](/5.10/api-management/plugins/plugin-types).
...
<Card href="/5.10/api-management/plugins/overview#dashboard-plugins" img="/img/logos/tyk-logo-selfmanaged.svg">
**Read time: 15 mins**

Dashboard Tutorial
</Card>

<Card href="/5.10/api-management/plugins/overview#open-source-plugins" img="/img/logos/tyk-logo-selfmanaged.svg">
**Read time: 15 mins**
...
This page provides an overview on how to register one or more custom plugins to be executed at different stages or [hooks](/5.10/api-management/plugins/plugin-types#plugin-and-hook-types) in the API request/response lifecycle. If you wish to learn how to register custom plugins to be executed on the traffic logs generated by the Gateway please refer to the [analytics plugins](/5.10/api-management/plugins/plugin-types#analytics-plugins) page. 

If you need fine-grained control at the endpoint level then it is also possible to configure [per-endpoint plugins](/5.10/api-management/plugins/plugin-types#per-endpoint-custom-plugins). These are custom Golang plugins that are triggered at the end of the request processing chain before API-level *Post* plugins are executed.
...
1. **gRPC plugins**: Plugin functions are implemented by a gRPC server with the associated configuration specified with the API definition. For further details on how to configure gRPC plugins, please refer to our [gRPC](/5.10/api-management/plugins/rich-plugins#overview-1) documentation.
...
3. **Plugin bundles**: The plugin source code and configuration are bundled into a zip file that is served by a remote web server. For further details see the [plugin bundles](/5.10/api-management/plugins/overview#plugin-bundles) page.
...
This guide explains how to configure plugins for Tyk OAS APIs within the [Tyk OAS API definition](/5.10/#tyk-oas-apidef) or via the [API designer](/5.10/#tyk-oas-dashboard) in Tyk Dashboard.

If you’re using the legacy Tyk Classic APIs, then check out the [Tyk Classic](/5.10/api-management/plugins/overview#tyk-classic-apis) page.
...
The `pluginConfig` section contains the `driver` parameter that is used to configure the plugin implementation [language](/5.10/api-management/plugins/overview#plugin-driver-names):
...
The configuration above is a complete and valid Tyk OAS API Definition that you can use as a basis for trying out custom plugins. You will need to update the [driver](/5.10/api-management/plugins/overview#plugin-driver-names) parameter to reflect the target language type of your plugins. You will also need to update the `path` and `functionName` parameters for each plugin to reflect the source code.
...
This guide explains how to configure plugins for Tyk Classic APIs within the [Tyk Classic API definition](/5.10/#tyk-classic-apidef) or via the [API designer](/5.10/#tyk-classic-dashboard) in Tyk Dashboard.

If you’re using the newer Tyk OAS APIs, then check out the [Tyk OAS](/5.10/api-management/plugins/overview#tyk-oas-apis) page.
...
The `driver` configuration parameter describes the plugin implementation language. Please refer to the [supported languages](/5.10/api-management/plugins/overview#plugin-driver-names) section for list of supported plugin driver names.
...
2. **Edit the Tyk Classic API Definition to configure plugins**
    ...
    Use the editor to edit the `custom_middleware` section of the [Tyk Classic API Definition](/5.10/api-management/plugins/overview#tyk-classic-apis).
...
The source code and a [manifest file](/5.10/#manifest) are bundled into a zip file and uploaded to an external remote web server. The manifest file references the source code file path and the function name within the code that should be invoked for each [plugin type](/5.10/api-management/plugins/plugin-types#plugin-types). Within the API definition, custom plugins are configured simply using the name of the bundle (zip file). Tyk Gateway downloads, caches, extracts and executes plugins from the downloaded bundle according to the configuration in the manifest file. 
...
-  `public_key_path`: Sets a public key, used for verifying signed bundles. If unsigned bundles are used you may omit this.
    ...
    <Note>
    Remember to set `"enable_coprocess": true` in your `tyk.conf` when using [rich plugins](/5.10/api-management/plugins/overview#plugin-bundles)!
    </Note>
...
Tyk provides the Bundle CLI tool as part of the `tyk` binary. For further details please visit the [Bundle CLI tool](/5.10/api-management/plugins/overview#bundler-cli-tool) page.

### Tyk OAS API Configuration

For API plugins that are deployed as [plugin bundles](/5.10/api-management/plugins/overview#plugin-bundles), the API should be configured with the name of the plugin bundle file to download from your remote web server. Furthermore, the Gateway should be [configured](/5.10/api-management/plugins/overview#gateway-configuration) to enable downloading plugin bundles.
...
For custom plugins that are deployed as [plugin bundles](/5.10/api-management/plugins/overview#plugin-bundles), the API should be configured with the name of the plugin bundle file to download from your remote web server. Furthermore, the Gateway should be [configured](/5.10/api-management/plugins/overview#gateway-configuration) to enable downloading plugin bundles.
...
If you’re using the newer Tyk OAS APIs, then check out the [Tyk OAS](/5.10/api-management/plugins/overview#tyk-oas-api-configuration) page.
...
Tyk Gateway should be configured for downloading plugin bundles from a secured web server. Please consult the [plugin bundles](/5.10/api-management/plugins/overview#plugin-bundles) documentation for further details.
...
[plugin bundles](/5.10/api-management/plugins/overview#plugin-bundles).
...
- **Manifest.json**: The [manifest.json](/5.10/api-management/plugins/overview#manifest) file
...
*   [Golang](/5.10/api-management/plugins/golang#): A plugin written in Golang is called a **Native Plugin**. Tyk recommends using Go plugins for performance, flexibility, and nativity reasons (all Tyk components are written in Go).
*   [JavaScript](/5.10/api-management/plugins/javascript#): A plugin written in Javascript uses JavaScript Virtual Machine (JSVM) interpreter.
*   [Rich Plugins](/5.10/api-management/plugins/rich-plugins#) includes Python, Lua, gRPC - With gRPC, you can write plugins in Java, .NET, C++ / C#, PHP, and all other [gRPC supported languages](https://grpc.io/docs/languages/).
...
* Used to stop the request and return a [custom response](/5.10/api-management/plugins/plugin-types#return-overrides--returnoverrides)
* Be served using [Bundles](/5.10/api-management/plugins/overview#plugin-deployment-types) or by files on the file system, except gRPC of course which by definition is served by some webserver in the language of your choosing

### Plugin Hook Types

Tyk provide 5 different phases, i.e. hooks to inject custom plugin throughout the [API execution lifecycle](/5.10/api-management/traffic-transformation#request-middleware-chain).
...
More reading on the [hook types](/5.10/api-management/plugins/rich-plugins#coprocess-dispatcher---hooks) in rich plugins and explanation with common use case for each [hook type](/5.10/api-management/plugins/plugin-types#plugin-types) 
...
| Built-in SDK | ✅<br />All Gateway Functionality | ✅<br />[Yes](/5.10/api-management/plugins/javascript#javascript-api)	| ❌	| ✅<br />[Yes](/5.10/api-management/plugins/rich-plugins#tyk-python-api-methods)	| ❌
...
- For [gRPC plugins](/5.10/api-management/plugins/rich-plugins#overview-1), Tyk Gateway can only be configured to integrate with one gRPC server.
...
[Tyk Plugins](/5.10/api-management/plugins/overview#) are a powerful tool that allows you to develop custom middleware that can intercept requests at different stages of the request lifecycle, modifying/transforming headers and body content.
