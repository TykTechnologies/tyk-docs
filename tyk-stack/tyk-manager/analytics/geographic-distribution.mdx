...
<Note>
From Tyk v5.1 (and LTS patches v4.0.14 and v5.0.3) the <b>Geographic Distribution</b> screen will not be visible to a user if they are assigned the [Owned Analytics](/basic-config-and-security/security/dashboard/user-roles#user-owned-analytics) permission.
</Note>


### MaxMind Settings

To use a MaxMind database, see [MaxMind Database Settings](/tyk-oss-gateway/configuration#a-name-enable-geo-ip-a-enable-geo-ip) in the Tyk Gateway Configuration Options.
