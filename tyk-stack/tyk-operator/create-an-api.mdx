...
This setup requires an [auth token](/api-management/authentication/auth-token) for access.
...
- `validate_signature`: Enable [signature validation](/api-management/authentication/auth-token#auth-token-with-signature).
...
- `jwt_signing_method`: Specify the method used to sign the JWT. Refer to the documentation on [JWT Signatures](/basic-config-and-security/security/authentication-authorization/json-web-tokens#signature-validation) for supported methods.
