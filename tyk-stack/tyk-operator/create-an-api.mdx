...
First, specify the details of your API using the [ApiDefinition CRD](/api-management/automations/operator#apidefinition-crd), then deploy it to create the corresponding Kubernetes resource. Tyk Operator will take control of the CRD and create the actual API in the Tyk data plane.
...
- [HTTP Proxy example](/tyk-stack/tyk-operator/create-an-api#set-up-manifest-for-http)
...
You can visit the [ApiDefinition CRD](/api-management/automations/operator#apidefinition-crd) page to see all the latest API Definitions fields and features we support.
...
<Note>
Tyk Operator supported authentication types are listed in the [API Definition features](/api-management/automations/operator#apidefinition-crd) section.
</Note>
...
This setup requires an [auth token](/api-management/authentication/bearer-token) for access.
...
- `jwt_signing_method`: Specify the method used to sign the JWT. Refer to the documentation on [JWT Signatures](/basic-config-and-security/security/authentication-authorization/json-web-tokens) for supported methods.
...
Below, we take you through how to define the security policy but you can also find [Security Policy Example](/tyk-stack/tyk-operator/create-an-api#security-policy-example) below.

### Define the Security Policy manifest

To create a security policy, you must define a Kubernetes manifest using the `SecurityPolicy` CRD. The following example illustrates how to configure a default policy for trial users for a Tyk Classic API named `httpbin`, a Tyk OAS API named `petstore`, a Tyk Streams API named `http-to-kafka`, and a Gateway API `HTTPRoute` named `orders`.
...
spec:
  ...
  access_rights_array:
    ...
    - name: orders                      # Use `HTTPRoute` to cover every API the operator synthesizes from a
      namespace: team-a                 # Gateway API HTTPRoute; ids are resolved automatically, no versions needed
      kind: HTTPRoute
  quota_max: 1000
...
**Define Access Lists for APIs**
  ...
  - **`access_rights_array`**: Defines the list of APIs that the security policy applies to and the versions of those APIs.
    ...
    - **`kind`**: Tyk OAS APIs (`TykOasApiDefinition`), Tyk Streams (`TykStreamsApiDefinition`), Tyk Classic APIs (`ApiDefinition`), and Gateway API `HTTPRoute`s (`HTTPRoute`) can be referenced here. The API format can be specified by `kind` field. If omitted, `ApiDefinition` is assumed.
...
In this example, the security policy will apply to an `ApiDefinition` resource named `httpbin` in the `default` namespace, a `TykOasApiDefinition` resource named `petstore` in the `default` namespace, a `TykStreamsApiDefinition` resource named `http-to-kafka` in the `default` namespace, and an `HTTPRoute` resource named `orders` in the `team-a` namespace. Note that you do not need to specify the API ID; Tyk Operator will automatically retrieve the API ID of referenced API Definition resources for you.

A `kind: HTTPRoute` entry covers every Tyk API definition the operator synthesizes from that route, so a single entry keeps covering the route as its rules and hostnames fan out or narrow. The operator resolves the synthesized API IDs from the route and requeues until the route has produced them, so you never handle API IDs yourself; the IDs are not written back into the policy manifest, which keeps GitOps workflows clean. Fields such as `limit` set on the entry apply to every covered API. Referencing routes requires the operator to run with `--enable-gateway-api`.
...
You can secure your APIs by specifying [allowed URLs](/api-management/access-control/sessions-and-keys/access-rights#granular-endpoint-access) (methods and paths) for each API within a security policy. This is done using the `allowed_urls` field under `access_rights_array`.
