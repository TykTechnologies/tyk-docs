...
It must be explicitly enabled in Tyk Developer Portal by setting [`TYK_PORTAL_ENABLEIDPREGISTRY=true`](/product-stack/tyk-enterprise-developer-portal/deploy/configuration#tyk-portal-enableidpregistry) (or `EnableIDPRegistry = true` in the config file) and restarting the Portal.
...
1. Set [`TYK_PORTAL_ENABLEIDPREGISTRY=true`](/product-stack/tyk-enterprise-developer-portal/deploy/configuration#tyk-portal-enableidpregistry) (or `EnableIDPRegistry = true` in the config file) and restart the Portal.
...
3. **Remove IdP configuration from API definitions.**
    ...
    Once the Registry is verified as complete and correct, remove the JWKS URIs and scope-to-policy mappings from each API definition. Until this step is completed, the API definition configuration takes precedence over the Registry and the Registry entries are not used for those APIs.
