...
Tyk requires any API that uses the scope to policy mapping to have [a default policy](/api-management/authentication-authorization#use-json-web-tokens-jwt ). Access rights and rate limits defined in the default policy take priority over other policies, including policies for the API Product and plan.
