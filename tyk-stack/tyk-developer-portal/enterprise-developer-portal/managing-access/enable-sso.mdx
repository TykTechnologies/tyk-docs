...
If you use [the Tyk helm chart](/tyk-self-managed/install/kubernetes#install-more-tyk-components), it is required to add the `PORTAL_API_SECRET` to extraEnvs:
```.yaml
extraEnvs:
...
*Available from Portal v1.16.0. See the [v1.16.0 release notes](/developer-support/release-notes/portal#1-16-0-release-notes).*
...
For the full reference, see [`PORTAL_SSO_CUSTOM_LOGIN_URL`](/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_sso_custom_login_url) in the configuration reference.

For a complete end-to-end walkthrough of setting up IDP-based SSO, from creating a TIB profile to testing the full login flow, see the End-to-End SSO Setup guide in the How-To Guides section.
