...
The system automatically creates a pre-configured "Default Organisation" during the [bootstrap](/portal/install#bootstrapping-developer-portal) process that serves as the initial home for:
...
[Team and Organisation level app visibility](/portal/developer-app#visibility) is not applied within the Default Organisation. All apps are treated as Personal (private). This behavior prevents accidental exposure of Developer Apps if a user is removed from a custom Organisation and automatically reverts to the Default Org. Any apps owned by that user will revert to Personal visibility. For the full visibility rules, see [App Visibility Controls](/portal/app-visibility-controls).
...
Keep the following access control rules in mind when managing team membership:

- **Admin Users can only add users to teams they belong to.** If you need to add a user to a team you are not a member of, ask an API Owner to make the change.
- **If no team is selected during invitation**, the user is automatically assigned to the default Organisation team.
...
- Remove users from it and move them to purpose-built Teams so they can only see their own team members rather than all users in the Organisation
- Use it as a holding area for inactive users awaiting deactivation or reassignment

<Note>
As of v1.14, users can be removed from the Default Team. Keep the Default Team for inactive users only. Note that Org Admins cannot add users to other Organisations, reinforcing the Default Team as a holding area rather than an active working team.
</Note>

#### Developer App visibility

[Team level app visibility](/portal/developer-app#visibility) is not applied within the Default Team. Users who are only in the Default Team cannot set their apps to Team visibility. Apps are restricted to Personal or Organisation level. This prevents accidental exposure of Developer Apps when a user is removed from a team and reverts to the Default Team. Users who belong to additional Teams alongside the Default Team can still use Team visibility for apps shared with those other Teams.
...
- **Minimise Default Team usage**: Move active users out of the Default Team into purpose-built Teams. This ensures users only see their own team members and can use Team-level app visibility for their Developer Apps.
