...
- **Authentication mode**: This is the security method to use with your API. First, you can set it to `Open(Keyless)`, but that option is not advised for production APIs. See [Authentication and Authorization](/api-management/authentication-authorization) for more details on securing your API.
...
- **Enable client certificate**: Select this to use Mutual TLS. See [Mutual TLS](/api-management/authentication-authorization#enable-mutual-tls) for details on implementing mutual TLS.
