...
All [user roles](/5.9/tyk-cloud/teams-users#assign-user-roles) can edit, deploy, undeploy, restart, redeploy all deployments within a team. Only the Organization Admin and the Team Admin can create or delete deployments.
...
<Note>
The number of Cloud Data Planes you can add is dependent on your [plan](/5.9/tyk-cloud/account-billing#select-a-payment-plan)
</Note>
...
5. (Optional) Enter a [custom domain](/5.9/tyk-cloud/using-custom-domains) if required
6. (Optional) Enable [plugins](/5.9/tyk-cloud/using-plugins) if required

## Edit Cloud Data Planes

You can edit the following Control Plane settings:
* Change the Gateway name
* Add a [custom domain](/5.9/tyk-cloud/using-custom-domains)
* Change the Bundle Channel and Bundle Version
* Enable [plugins](/5.9/tyk-cloud/using-plugins)



    <Note>
    The use of custom domains is dependent on your [plan](/5.9/tyk-cloud/account-billing#select-a-payment-plan)
    </Note>
...
The Auto Upgrade feature is only available for Control Plane deployments. When enabled on a Control Plane, it will automatically upgrade the corresponding data planes related to this control plane. For more information, see the [Tyk Cloud Control Plane Auto Upgrade Documentation](/5.9/tyk-cloud/environments-deployments/managing-control-planes).
