...
Internal endpoints are frequently used to make complex routing decisions that cannot be handled by the standard routing features. A single externally published endpoint can receive requests and then, based on inspection of the requests, the [URL rewrite](/5.9/transform-traffic/url-rewriting#url-rewrite-middleware) middleware can route them to different internal endpoints and on to the appropriate upstream services.
...
The [Internal Endpoint](/5.9/#internal-endpoint-overview) middleware instructs Tyk Gateway not to process external requests to the endpoint (which is a combination of HTTP method and path). Internal requests from other APIs will be processed.
...
If you're using the legacy Tyk Classic APIs, then check out the [Tyk Classic](/5.9/#internal-endpoint-using-classic) page.
...
- the [URL rewrite](/5.9/transform-traffic/url-rewriting#url-rewrite-middleware) middleware has been configured for requests to the `GET /redirect` endpoint
...
If you're using the newer Tyk OAS APIs, then check out the [Tyk OAS](/5.9/#internal-endpoint-using-tyk-oas) page.
