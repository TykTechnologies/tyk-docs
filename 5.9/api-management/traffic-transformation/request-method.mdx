...
Tyk's [request method transform](/5.9/api-management/traffic-transformation/request-method) middleware is configured at the endpoint level, where it modifies the HTTP method used in the request to a configured value.
...
If you're using the legacy Tyk Classic APIs, then check out the [Tyk Classic](/5.9/#request-method-using-classic) page.
...
If you're using the newer Tyk OAS APIs, then check out the [Tyk OAS](/5.9/#request-method-using-tyk-oas) page.

If you're using Tyk Operator then check out the [configuring a Request Method Transform in Tyk Operator](/5.9/#tyk-operator) section below.
