...
This middleware changes only the payload and not the headers. You can, however, combine this with the [Request Header Transform](/5.9/api-management/traffic-transformation/request-headers) middleware to apply more complex transformation to requests.

There is a closely related [Response Body Transform](/5.9/api-management/traffic-transformation/response-body) middleware that provides the same functionality on the response from the upstream, prior to it being returned to the client.
...
A common use of the request body transform middleware is to surface a legacy SOAP service with a REST API. Full details of how to perform this conversion using Tyk are provided [here](/5.9/advanced-configuration/transform-traffic/soap-rest).
...
The Go template can be defined within the API Definition or can be read from a file that is accessible to Tyk, for example alongside your [error templates](/5.9/api-management/gateway-events#error-templates).

We have provided more detail, links to reference material and some examples of the use of Go templating [here](/5.9/api-management/traffic-transformation/go-templates).
...
The middleware has direct access to the request body and also to dynamic data as follows:
 - [context variables](/5.9/api-management/traffic-transformation/request-context-variables), extracted from the request at the start of the middleware chain, can be injected into the template using the `._tyk_context.KEYNAME` namespace
 - [session metadata](/5.9/api-management/policies#what-is-a-session-metadata), from the Tyk Session Object linked to the request, can be injected into the template using the `._tyk_meta.KEYNAME` namespace 
 ...
 - values from [key-value (KV) storage](/5.9/tyk-configuration-reference/kv-store#transformation-middleware) can be injected into the template using the notation appropriate to the location of the KV store
...
The Request Body Transform supports two helper functions that you can use in your Go templates to facilitate this:
 - `jsonMarshal` performs JSON style character escaping on an XML field and, for complex objects, serialises them to a JSON string ([example](/5.9/api-management/traffic-transformation/go-templates#xml-to-json-conversion-using-jsonmarshal))
 - `xmlMarshal` performs the equivalent conversion from JSON to XML ([example](/5.9/api-management/traffic-transformation/go-templates#json-to-xml-conversion-using-xmlmarshal))
...
{/* proposed "summary box" to be shown graphically on each middleware page
 # Request Body Transform middleware summary
  ...
  - Request Body Transform can access both [session metadata](/5.9/api-management/policies#what-is-a-session-metadata) and [request context variables](/5.9/api-management/traffic-transformation/request-context-variables). */}

## Using Tyk OAS
<a id="request-body-using-tyk-oas"></a>

The [request body transform](/5.9/api-management/traffic-transformation/request-body) middleware provides a way to modify the payload of API requests before they are proxied to the upstream.
...
If you're using the legacy Tyk Classic APIs, then check out the [Tyk Classic](/5.9/#request-body-using-classic) page.
...
If you're using the newer Tyk OAS APIs, then check out the [Tyk OAS](/5.9/#request-body-using-tyk-oas) page.
