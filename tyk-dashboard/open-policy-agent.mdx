...
You can then control OPA functionality on a global level via your `tyk_analytics.conf` file, or at an organization level using either the [OPA API](/tyk-dashboard-api/org/opa) or the [Dashboard](#using-the-open-policy-agent-in-the-dashboard).
