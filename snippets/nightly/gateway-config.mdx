...
Minimum TLS version. Possible values: https://tyk.io/docs/api-management/certificates#tls-or-ssl
...
Note: This feature works with API definitions up to the configured `max_decompressed_size` when decompressed.
...
This feature reduces Redis memory usage in MDCB deployments where policies are cached locally on Data Plane Gateways. A conservative estimate is a 70-75% reduction in size.

The Gateway can read both compressed and uncompressed formats for backward compatibility.

You can safely enable this setting on existing deployments. The Gateway continues to load previously stored uncompressed backups and stores all new backups in compressed form.

Note: This feature works with policies up to the configured `max_decompressed_size` when decompressed.
...
Maximum decompressed size (in bytes) for API definitions and policies respectively when using compression.  This applies to API Definitions and policies separately so each collection when decompressed in isolation must be at or under this max size.   This limit prevents memory exhaustion during decompression.
