...
This field controls the exposure of the API on the Gateway. When set to `true`, the API will not be made available for external access and will not be included in API listings returned by the Gateway's management APIs; it will be accessible only via [internal looping](/nightly/docs/advanced-configuration/transform-traffic/looping) or as a [child API version](/nightly/docs/api-management/api-versioning#base-and-child-apis).
