...
The RPC port to which Data Plane Gateways will connect. You must open this port to accept connections via your firewall.<br />Default if not set: 9091.

### healthcheck_port
ENV: <b>TYK_MDCB_HEALTHCHECKPORT</b><br />
Type: `int`<br />

This port lets MDCB allow standard health checks.<br />Default if not set: 8181.
...
The min_version setting should be the minimum TLS protocol version required from the client.<br /> For TLS 1.0 use 769<br />For TLS 1.1 use 770<br />For TLS 1.2 use 771<br />For TLS 1.3 use 772<br />See [Controlling TLS Version & Cipher Suites](https://tyk.io/docs/api-management/implement-tls#controlling-tls-version--cipher-suites) for more details.
...
### server_options.max_connection_idle_time
ENV: <b>TYK_MDCB_SERVEROPTIONS_MAXCONNECTIONIDLETIME</b><br />
Type: `int`<br />

MaxConnectionIdleTime is the maximum time, in seconds, that an RPC connection may wait for client data before MDCB closes it. A value of 0 means idle connections are never closed.
Set it above the Gateway's [slave_options.call_timeout](/nightly/tyk-oss-gateway/configuration#slave_options-call_timeout) (the longest RPC call duration) and [slave_options.ping_timeout](/nightly/tyk-oss-gateway/configuration#slave_options-ping_timeout) (the data plane keepalive interval), to reduce reconnect and TLS handshake churn.

### server_options.max_connections
ENV: <b>TYK_MDCB_SERVEROPTIONS_MAXCONNECTIONS</b><br />
Type: `int`<br />

MaxConnections is the maximum number of concurrent RPC connections MDCB accepts from Data Plane Gateways and Pumps. A value of 0 means unlimited.
Size it for the number of client processes times the Gateway's [slave_options.rpc_pool_size](/nightly/tyk-oss-gateway/configuration#slave_options-rpc_pool_size), plus reconnect headroom. Excess connections are closed before TLS negotiation.
...
If you are using encrypted private keys, this is the secret to use to decrypt them. This must match the secret used for Gateway and Dashboard as explained [here](/nightly/api-management/certificates#certificate-data).
...
ENV: <b>TYK_MDCB_STORAGE_IAMAUTH_PROVIDER</b><br />
Type: `string`<br />
...
ENV: <b>TYK_MDCB_STORAGE_IAMAUTH_SERVICEACCOUNT</b><br />
Type: `string`<br />
...
ENV: <b>TYK_MDCB_STORAGE_IAMAUTH_TOKENREFRESHBEFOREEXPIRY</b><br />
Type: `string`<br />
...
ENV: <b>TYK_MDCB_STORAGE_IAMAUTH_ENABLED</b><br />
Type: `bool`<br />
...
ENV: <b>TYK_MDCB_ANALYTICSCONFIG_MONGOIGNOREAGGREGATIONS</b><br />
Type: `[]string`<br />

List of aggregate types to exclude from storage. Aggregates in this list are dropped before being written to the collection. See [pumps.mongoaggregate.meta.ignore_aggregations](/nightly/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#pumps-mongoaggregate-meta-ignore_aggregations) for detailed behavior.

### analytics.mongo_enable_aggregate_self_healing
ENV: <b>TYK_MDCB_ANALYTICSCONFIG_MONGOENABLEAGGREGATESELFHEALING</b><br />
Type: `bool`<br />

If set to true, enables self-healing for aggregate data. See [pumps.mongoaggregate.meta.enable_aggregate_self_healing](/nightly/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#pumps-mongoaggregate-meta-enable_aggregate_self_healing) for detailed behavior.
...
ENV: <b>TYK_MDCB_INVALIDKEYCACHETTL</b><br />
Type: `int`<br />

Number of seconds to cache invalid session keys. When a gateway attempts to login with a non-existent or invalid key,
it is cached to prevent repeated Redis lookups. Default is 180 seconds (3 minutes).
Lower values (e.g., 30) allow faster recovery after Redis outages but increase Redis load from retries.
Higher values better protect Redis from bad key hammering but delay recovery from temporary failures.
...
ENV: <b>TYK_MDCB_ANALYTICSSTORAGE_IAMAUTH_PROVIDER</b><br />
Type: `string`<br />
...
ENV: <b>TYK_MDCB_ANALYTICSSTORAGE_IAMAUTH_SERVICEACCOUNT</b><br />
Type: `string`<br />
...
ENV: <b>TYK_MDCB_ANALYTICSSTORAGE_IAMAUTH_TOKENREFRESHBEFOREEXPIRY</b><br />
Type: `string`<br />
...
ENV: <b>TYK_MDCB_ANALYTICSSTORAGE_IAMAUTH_ENABLED</b><br />
Type: `bool`<br />
...
Enables [API Ownership](/nightly/platform-management/api-ownership) in MDCB. If set, the Data Plane Gateways will load only APIs that are accessible by the user and user group associated with the Gateways' [slave_options.api_key](/nightly/tyk-oss-gateway/configuration#slave_options-api_key) used to connect to MDCB.
