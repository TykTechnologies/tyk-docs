...
A [Policy](/nightly/api-management/policies) encapsulates several options that can be applied to a key. It acts as a template that can override individual sections of an API key (or identity) in Tyk.


#### Create a Policy with the Dashboard

We have a video walkthrough for creating a Policy with the Dashboard.
...
4. **Setting Global Rate Limits and Quota**
    ...
    **Rate Limiting**
    ...
    A rate limit is enforced on all keys, set the number of requests per second that a user of a key with this policy is allowed to use. See [Rate Limiting](/nightly/api-management/rate-limit#rate-limiting-layers) for more details. **Note: The Rate Limit set by a policy will override the limits applied to an individual key.**

    **Throttling**

    When hitting rate limits, you can set Tyk Gateway to automatically queue and auto-retry client requests. Throttling can be configured at a key or policy level. See [Request Throttling](/nightly/api-management/request-throttling) for more details.

    **Usage Quotas**

    Usage quotas limit the number of total requests a user is allowed to have over a longer period of time. So while a rate limit is a rolling window, a quota is an absolute maximum that a user is allowed to have over a week, a day or a month. See [Request Quotas](/nightly/api-management/request-quotas) for more details.
...
5. **Add Configuration Details**
    ...
    2. Set the policy state. You can set your policy to one of the following states:
        ...
        * Draft
        * Access Denied 
    3. Set a time after which any Keys subscribed to your policy expire. Select a value from the drop-down list. This is a required setting. See [Session Lifecycle](/nightly/api-management/access-control/sessions-and-keys/session-lifecycle) for more details.
    4. Configure Behaviour on key expiry. When key expiry is set, the **Behaviour on key expiry** options appear:

        <img src="/img/behaviour_on_key_expiry_policy.png" alt="Behaviour on key expiry options on the Add Policy page" />

        * **Delete key** (Default): Immediately deletes session record from Redis upon expiration.
        * **Retain key**: Keeps the session in Redis post-expiration. Choose one retention mode:
            * **Retain for specific period**: Define a specific post-expiry duration (for example, 1h, 30d).
            * **Retain forever**: Keeps the session in Redis indefinitely (no post-expiry TTL is set) until manually deleted.
            * **Use API or gateway default** (Default): Falls back to the [legacy controls](/nightly/api-management/access-control/sessions-and-keys/session-lifecycle#legacy-controls).
    5. Add Tags to your policy. Any tags you add can be used when filtering Analytics Data. Tags are case sensitive.
    6. Add Metadata to your policy. Adding metadata such as User IDs can be used by middleware components. See [Session Metadata](/nightly/api-management/access-control/sessions-and-keys/understanding-sessions#metadata-and-context) for more details.
...
For more information on how Policies are constructed and applied to Sessions, please see the [Access Control](/nightly/api-management/access-control/overview) section.
