...
If you are using encrypted private keys, this is the secret to use to decrypt them. This must match the secret used for Gateway and Dashboard as explained [here](/5.13/api-management/certificates#encryption-of-the-private-key).
...
List of aggregate types to exclude from storage. Aggregates in this list are dropped before being written to the collection. See [pumps.mongoaggregate.meta.ignore_aggregations](/5.13/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#pumps-mongoaggregate-meta-ignore_aggregations) for detailed behavior.

### analytics.mongo_enable_aggregate_self_healing
ENV: <b>TYK_MDCB_ANALYTICSCONFIG_MONGOENABLEAGGREGATESELFHEALING</b><br />
Type: `bool`<br />

If set to true, enables self-healing for aggregate data. See [pumps.mongoaggregate.meta.enable_aggregate_self_healing](/5.13/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#pumps-mongoaggregate-meta-enable_aggregate_self_healing) for detailed behavior.
...
Enables [API Ownership](/5.13/api-management/user-management#enabling-api-ownership) in MDCB. If set, the Data Plane Gateways will load only APIs that are accessible by the user and user group associated with the Gateways' [slave_options.api_key](/5.13/tyk-oss-gateway/configuration#slave_options-api_key) used to connect to MDCB.
