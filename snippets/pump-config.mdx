...
Controls whether MCP analytics are aggregated and sent to MDCB via a separate RPC call. Only applies when `aggregated` is set to `true`. Defaults to `false`.
...
Custom Prometheus metrics. Each custom metric supports the following fields:
- `name`: The Prometheus metric name.
- `description`: Help text for the metric.
- `metric_type`: Either `counter` or `histogram`.
- `labels`: List of label names. Available values: `host`, `method`, `path`, `response_code`, `api_key`, `time_stamp`, `api_version`, `api_name`, `api_id`, `org_id`, `oauth_id`, `request_time`, `ip_address`, `alias`, `mcp_method`, `mcp_primitive_type`, `mcp_primitive_name`. The MCP labels are only populated for MCP analytics records; non-MCP records produce empty strings.
- `buckets`: (histogram only) Custom bucket boundaries. Defaults to `[1, 2, 5, 7, 10, 15, 20, 25, 30, 40, 50, 60, 70, 80, 90, 100, 200, 300, 400, 500, 1000, 2000, 5000, 10000, 30000, 60000]`.
- `mcp_only`: When set to `true`, the metric is only updated for MCP analytics records. Non-MCP records are skipped entirely.
- `latency_breakdown`: When set to `true` on a histogram metric, each record produces three observations (total, upstream, gateway) with the `type` label. Requires the `type` label in `labels`.
