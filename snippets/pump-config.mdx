<!-- Documents: SYS-REQ-001, SYS-REQ-007, INT-REQ-001, INT-REQ-005, SW-REQ-006 -->
### purge_chunk
...
<!-- Documents: STK-REQ-005, SYS-REQ-014, SYS-REQ-021, INT-REQ-002 -->
### uptime_pump_config
...
### pumps
<!-- Documents: SYS-REQ-020, SW-REQ-002, SW-REQ-017, INT-REQ-008 -->
...
`sql-graph`, `sql-graph-aggregate`, `mongo-mcp`, `mongo-mcp-aggregate`, `sql-mcp`, `sql-mcp-aggregate`,
`sqs`, `kinesis`, `resurfaceio`.

### pumps.csv.filters
<!-- Documents: SYS-REQ-005, SYS-REQ-009, SYS-REQ-010, SYS-REQ-015, SYS-REQ-016, SW-REQ-010, SW-REQ-016 -->
...
<!-- Documents: SW-REQ-001, SW-REQ-076 -->
...
Each pump can also be configured with per-backend `raw_request_decoded` and `raw_response_decoded` controls. When enabled, Pump attempts to base64-decode the corresponding raw payload field before dispatching the record to that backend. If the payload is malformed base64, the current behavior is to preserve the original field unchanged; this residual behavior is tracked by the known issue `filterdata-base64-decode-silent-noop`.
<!-- Documents: SYS-REQ-011, SW-REQ-088 -->
...
The DogStatsD and StatsD pumps project configured analytics fields and tags into metric names, tags, and timing values. Latency fields can include total, upstream, and gateway latency when those fields are present on the analytics record.
- `api_version`
- `api_name`
- `api_id`
- `org_id`
...
<!-- Documents: SW-REQ-068, SW-REQ-069, SW-REQ-070, SW-REQ-100 -->
...
### pumps.elasticsearch.meta.ssl_ca_file
ENV: <b>TYK_PMP_PUMPS_ELASTICSEARCH_META_SSLCAFILE</b><br />
Type: `string`<br />

Path to the PEM file with trusted CA certificates that will be used to verify the Elasticsearch server's certificate.

### pumps.elasticsearch.meta.mcp_index_name
ENV: <b>TYK_PMP_PUMPS_ELASTICSEARCH_META_MCPINDEXNAME</b><br />
Type: `string`<br />
...
Each analytics record sent by the Graylog pump is encoded as a GELF JSON message and includes the configured tag projections.
...
### pumps.hybrid.meta.enable_mcp_aggregation
ENV: <b>TYK_PMP_PUMPS_HYBRID_META_ENABLEMCPAGGREGATION</b><br />
Type: `bool`<br />
...
<!-- Documents: SW-REQ-022, SW-REQ-046, SW-REQ-101 -->
### pumps.influx.name
...
The InfluxDB v1 pump converts each purge batch into InfluxDB points using the configured field and tag lists and submits the batch with a single write operation.

### pumps.influx2
<!-- Documents: SW-REQ-047 -->
Type: `influx2`<br />

Configure an InfluxDB v2 pump with type `influx2`. The pump writes analytics records to an InfluxDB v2 bucket using the configured organization name, bucket, token, address, fields, and tags. It follows the same common pump filters, timeout, detailed-recording omission, maximum-record-size, and ignored-field controls as the other analytics pumps.

<!-- Documents: SW-REQ-021, SW-REQ-081, SW-REQ-106 -->
### pumps.kafka.name
...
### pumps.kafka.meta.ssl_ca_file
ENV: <b>TYK_PMP_PUMPS_KAFKA_META_SSLCAFILE</b><br />
Type: `string`<br />
...
<!-- Documents: SW-REQ-056, SW-REQ-105 -->
### pumps.kinesis.name
...
The Logz.io pump marshals each analytics record to JSON and enqueues it through the Logz.io sender. The sender uses `queue_dir`, `drain_duration`, `disk_threshold`, and `check_disk_space` to buffer and drain records.
...
When bulk mode is enabled, the pump queues analytics events up to the configured queue size and batch size before sending them to Moesif. If the queue is full during upstream connectivity problems, new events are skipped instead of growing memory without bound.

### pumps.segment
<!-- Documents: SW-REQ-053 -->
Type: `segment`<br />

Configure a Segment pump with type `segment`. The pump converts analytics records into Segment tracking events and sends them with the configured Segment write key. It uses the same common pump filters, timeout, detailed-recording omission, maximum-record-size, and ignored-field controls as the other analytics pumps.
...
Set to true to disable the default tyk index creation.
<!-- Documents: SW-REQ-063, SW-REQ-099 -->
...
<!-- Documents: SW-REQ-039, SW-REQ-084, SW-REQ-102 -->
...
<!-- Documents: SW-REQ-058, SW-REQ-107, SW-REQ-108, SW-REQ-109 -->
...
Maximum document size. If the document exceed this value, it will be skipped.
Defaults to 10Mb.
The size estimate counts `RawRequest` and `RawResponse` once each, plus metadata overhead, before deciding whether the record exceeds the configured maximum.
<!-- Documents: SW-REQ-092 -->

<!-- Documents: SW-REQ-024, SW-REQ-104 -->
...
Disabled metrics apply to built-in base metric family names. Custom metrics remain controlled by their own `custom_metrics` configuration.
...
<!-- Documents: SW-REQ-078, SW-REQ-079 -->
...
Each valid custom metric entry creates a distinct runtime metric with its own label schema. Histogram latency metrics use a `type` label to distinguish total, upstream, and gateway latency observations.
Custom metric entries can enable `obfuscate_api_keys`; when enabled, the `api_key` and `key` labels are masked so only the configured trailing characters are retained, or the fully masked sentinel is emitted for short keys.
When aggregate observations are enabled, Prometheus stores the original label tuple separately from its internal aggregation key so label values containing the internal separator remain a single label value when exposed.
<!-- Documents: SW-REQ-074, SW-REQ-083, SW-REQ-091, SW-REQ-094 -->
...
### pumps.splunk.meta.ssl_ca_file
ENV: <b>TYK_PMP_PUMPS_SPLUNK_META_SSLCAFILE</b><br />
Type: `string`<br />
...
Default value is `0`
<!-- Documents: SYS-REQ-006, SYS-REQ-023, SW-REQ-030 -->

<!-- Documents: SW-REQ-040, INT-REQ-007 -->
### pumps.sql.name
...
Specifies if all the analytics records are going to be stored in one table or in multiple
tables (one per day). By default, `false`. If `false`, all the records are going to be
stored in `tyk_analytics` table. Instead, if it's `true`, all the records of the day are
going to be stored in `tyk_analytics_YYYYMMDD` table, where `YYYYMMDD` is going to change
depending on the date.
...
<!-- Documents: SW-REQ-041, SW-REQ-064, SW-REQ-065, SW-REQ-066, SW-REQ-067, INT-REQ-007 -->
...
StatsD emits timing metrics only for supported timing fields such as request time and latency values. Unsupported field names are ignored instead of producing arbitrary metric shapes.
...
### pumps.stdout.meta.use_legacy_payload_format
ENV: <b>TYK_PMP_PUMPS_STDOUT_META_USELEGACYPAYLOADFORMAT</b><br />
Type: `bool`<br />

Use the legacy formatting of raw_request and raw_response as escaped strings rather than JSON formatting.

The stdout pump prints analytics records for debugging and local inspection. The `dummy` pump type is a discard sink for deployments or tests that need Pump to consume records without forwarding them to an external data store.
...
The syslog pump writes one analytics record per syslog message. When detailed payload fields are omitted, raw request and response bodies are removed before the message is sent.
...
The Timestream pump converts analytics records to Timestream multi-measure records and sends them in batches of at most 100 records per write request. If the backend rejects a write, the failed write is surfaced to Pump instead of being silently treated as successful.

### pumps.sqs
<!-- Documents: SW-REQ-055 -->
Type: `SQSConf`<br />

Configure an AWS SQS pump with type `sqs`. The SQS pump resolves the queue URL
from `aws_queue_name` and sends analytics records with `SendMessageBatch`.

### pumps.sqs.meta.EnvPrefix
ENV: <b>TYK_PMP_PUMPS_SQS_META_ENVPREFIX</b><br />
Type: `string`<br />

The prefix for the environment variables that will be used to override the configuration.
Defaults to `TYK_PMP_PUMPS_SQS_META`

### pumps.sqs.meta.aws_queue_name
ENV: <b>TYK_PMP_PUMPS_SQS_META_AWSQUEUENAME</b><br />
Type: `string`<br />

The AWS SQS queue name. The pump uses this value to resolve the queue URL.

### pumps.sqs.meta.aws_region
ENV: <b>TYK_PMP_PUMPS_SQS_META_AWSREGION</b><br />
Type: `string`<br />

AWS Region for the SQS queue.

### pumps.sqs.meta.aws_key
ENV: <b>TYK_PMP_PUMPS_SQS_META_AWSKEY</b><br />
Type: `string`<br />

AWS access key ID. If omitted, the AWS SDK default credential chain is used.

### pumps.sqs.meta.aws_secret
ENV: <b>TYK_PMP_PUMPS_SQS_META_AWSSECRET</b><br />
Type: `string`<br />

AWS secret access key. If omitted, the AWS SDK default credential chain is used.

### pumps.sqs.meta.aws_token
ENV: <b>TYK_PMP_PUMPS_SQS_META_AWSTOKEN</b><br />
Type: `string`<br />

AWS session token for temporary credentials.

### pumps.sqs.meta.aws_endpoint
ENV: <b>TYK_PMP_PUMPS_SQS_META_AWSENDPOINT</b><br />
Type: `string`<br />

Custom SQS endpoint URL.

### pumps.sqs.meta.aws_sqs_batch_limit
ENV: <b>TYK_PMP_PUMPS_SQS_META_AWSSQSBATCHLIMIT</b><br />
Type: `int`<br />

Maximum number of messages to send in each SQS batch.

### pumps.sqs.meta.aws_message_group_id
ENV: <b>TYK_PMP_PUMPS_SQS_META_AWSMESSAGEGROUPID</b><br />
Type: `string`<br />

Message group ID used when sending to FIFO queues.

### pumps.sqs.meta.aws_message_id_deduplication_enabled
ENV: <b>TYK_PMP_PUMPS_SQS_META_AWSMESSAGEIDDEDUPLICATIONENABLED</b><br />
Type: `bool`<br />

When true, the generated message ID is also used as the FIFO message deduplication ID.

### pumps.sqs.meta.aws_delay_seconds
ENV: <b>TYK_PMP_PUMPS_SQS_META_AWSDELAYSECONDS</b><br />
Type: `int32`<br />

Delay, in seconds, before sent messages become available for processing.

### pumps.resurfaceio
<!-- Documents: SW-REQ-054 -->
Type: `ResurfacePumpConfig`<br />

Configure a Resurface.io pump with type `resurfaceio`. The pump sends analytics records
to the configured Resurface capture endpoint using the configured logging rules.
The Resurface pump uses its configured queue while forwarding records to the capture endpoint, so queue sizing should be chosen for the expected traffic volume and backend latency.

### pumps.resurfaceio.meta.EnvPrefix
ENV: <b>TYK_PMP_PUMPS_RESURFACEIO_META_ENVPREFIX</b><br />
Type: `string`<br />

The prefix for the environment variables that will be used to override the configuration.
Defaults to `TYK_PMP_PUMPS_RESURFACEIO_META`

### pumps.resurfaceio.meta.capture_url
ENV: <b>TYK_PMP_PUMPS_RESURFACEIO_META_CAPTUREURL</b><br />
Type: `string`<br />

Resurface capture endpoint URL.

### pumps.resurfaceio.meta.Rules
ENV: <b>TYK_PMP_PUMPS_RESURFACEIO_META_RULES</b><br />
Type: `string`<br />

Resurface logging rules.

### pumps.resurfaceio.meta.Queue
ENV: <b>TYK_PMP_PUMPS_RESURFACEIO_META_QUEUE</b><br />
Type: `[]string`<br />

Resurface logger queue configuration.

<!-- Documents: SW-REQ-038 -->
### pumps.mongo-mcp
Type: `mongo-mcp`<br />

The `mongo-mcp` pump uses the same Mongo connection settings as `pumps.mongo.meta.*`.
It writes only MCP analytics records to the configured Mongo collection and skips non-MCP
records. Its default meta environment prefix is `TYK_PMP_PUMPS_MONGOMCP_META`.

<!-- Documents: SW-REQ-012 -->
### pumps.mongo-mcp-aggregate
Type: `mongo-mcp-aggregate`<br />

The `mongo-mcp-aggregate` pump uses the same Mongo aggregate settings as
`pumps.mongoaggregate.meta.*`. It aggregates only MCP analytics records and writes MCP
method, primitive type, and primitive name dimensions. Its default meta environment prefix
is `TYK_PMP_PUMPS_MONGOMCPAGGREGATE_META`.

<!-- Documents: SW-REQ-044 -->
### pumps.sql-mcp
Type: `sql-mcp`<br />

The `sql-mcp` pump uses the same SQL connection, batching, and sharding settings as
`pumps.sql.meta.*`. It writes only MCP analytics records. Its default meta environment
prefix is `TYK_PMP_PUMPS_MCP_SQL_META`.

### pumps.sql-mcp.meta.table_name
ENV: <b>TYK_PMP_PUMPS_MCP_SQL_META_TABLENAME</b><br />
Type: `string`<br />

SQL table name for MCP analytics records. In sharding mode, this value is used as the
table prefix. Defaults to `tyk_analytics_mcp`.

<!-- Documents: SW-REQ-045 -->
### pumps.sql-mcp-aggregate
Type: `sql-mcp-aggregate`<br />

The `sql-mcp-aggregate` pump uses the same SQL aggregate settings as
`pumps.sqlaggregate.meta.*`. It aggregates MCP analytics records into
`tyk_mcp_aggregated`, or `tyk_mcp_aggregated_YYYYMMDD` when table sharding is enabled.
Its default meta environment prefix is `TYK_PMP_PUMPS_SQLMCPAGGREGATE_META`.

### analytics_storage_config
<!-- Documents: SYS-REQ-006, SYS-REQ-025, SW-REQ-007, SW-REQ-031 -->
...
Tyk Pump uses this Redis-compatible temporal storage to read traffic-log batches written by the Gateway. Reads use the configured Redis key prefix and atomic Redis operations so multiple Pump processes can share the same temporal store without duplicating records. Connection failures are retried according to the Redis client and Pump retry behavior, and persistent failures are surfaced in Pump logs.
<!-- Documents: SYS-REQ-028, SYS-REQ-030, SYS-REQ-031, INT-REQ-003, SW-REQ-006, SW-REQ-008 -->
...
<!-- Documents: STK-REQ-004, SYS-REQ-013, SYS-REQ-017, SW-REQ-005 -->
...
StatsD instrumentation is best-effort operational telemetry. A StatsD sink connection or send failure must not stop Pump from purging or forwarding analytics records to configured pumps.
...
<!-- Documents: SYS-REQ-012, SW-REQ-032 -->
### Health Check
From v2.9.4, we have introduced a `/health` endpoint for liveness checks. You
need to configure the following settings. This returns a HTTP 200 OK response if the Pump
process is running. The endpoint does not report readiness of configured pumps or backends.
