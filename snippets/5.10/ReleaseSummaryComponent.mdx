export const ReleaseSummaryComponent = ({ releaseData }) => {
  ...
  const renderComponent = (component) => (
    ...
    >
      Click to <a href={`/docs/${component.home}`} style={linkStyle} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>discover more</a>
      ...
      <div style={{marginTop: '12px', paddingTop: '8px', borderTop: '1px solid #eee'}}>
        <a href={`/docs/${component.releaseNotesPath}`} style={linkStyle} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
          Release Notes →
        </a>
      </div>
    </Card>
