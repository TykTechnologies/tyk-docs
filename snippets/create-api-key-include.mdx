...
4. **Add Configuration Details**
    ...
    3. Set an Expiry time after which the key will expire. Select a value from the drop-down list. This is a required setting. See [Key Expiry](/api-management/access-control/sessions-and-keys/session-lifecycle) for more details.
    4. Configure Behaviour on key expiry. When an expiry is set, the **Behaviour on key expiry** options appear:

        {/* TODO: Screenshot — Dashboard "Behaviour on key expiry" section on the Add Key page, showing the Delete key / Retain key radio choice and the three retention mode options */}

        * **Delete key**: Removes the session data from Redis immediately upon expiration.
        * **Retain key** (Default): Keeps the session in Redis post-expiration. Choose one retention mode:
            * **Retain for specific period**: Specify a duration (for example, 1h, 30d, 6h30m20s) to retain the session before deletion.
