...
4. **Add Configuration Details**
    ...
    4. Configure Behaviour on key expiry. When an expiry is set, the **Behaviour on key expiry** options appear:
        ...
        * **Retain key** (Default): Keeps the session in Redis post-expiration. Choose one retention mode:
            ...
            * **Retain forever**: Keeps the session in Redis indefinitely until manual deletion.
            * **Use API or gateway default** (Default): Adheres to the API definition's custom key lifetime or Gateway's global session lifetime settings.
