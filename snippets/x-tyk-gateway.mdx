...
**Field: `errorOverrides` ([ErrorOverrides](#erroroverrides))**
...
**Field: `value` ([ErrorOverridesMap](#erroroverridesmap))**
...
**Field: `match` ([ErrorMatcher](#errormatcher))**
Match contains optional additional matching criteria.

**Field: `response` ([ErrorResponse](#errorresponse))**
...
**Field: `name` (`string`)**
Name is an operator-assigned label used in audit logs and metrics. Must be unique within `providers`.
...
**Field: `actorToken` ([OAuth2ActorToken](#oauth2actortoken))**
ActorToken, when set, makes exchange requests carry `actor_token` (RFC 8693 delegation). When omitted, requests carry only `subject_token` (impersonation).
...
OAuth2ActorToken describes how Tyk obtains the actor token for delegation exchanges (RFC 8693 §4.4). Exactly one source sub-block is used, selected by `source`.

**Field: `source` (`string`)**
Source selects the actor token origin: `"client_credentials"`, `"header"`, or `"static"`.

**Field: `actorTokenType` (`string`)**
ActorTokenType is the RFC 8693 token-type URN advertised for the actor token. Defaults to `urn:ietf:params:oauth:token-type:access_token`.

**Field: `requireMayAct` (`boolean`)**
RequireMayAct, when `true`, verifies the inbound subject token carries a `may_act` claim (RFC 8693 §4.4) before calling the IdP. Defaults to `false`.

**Field: `clientCredentials` ([OAuth2ActorClientCredentials](#oauth2actorclientcredentials))**
ClientCredentials — used when `source` is `"client_credentials"`.

**Field: `header` ([OAuth2ActorHeader](#oauth2actorheader))**
Header — used when `source` is `"header"`.

**Field: `static` ([OAuth2ActorStatic](#oauth2actorstatic))**
Static — used when `source` is `"static"`. For testing only.

---

### **OAuth2ActorClientCredentials**

OAuth2ActorClientCredentials drives the client credentials flow Tyk uses to mint a gateway-as-actor token.

**Field: `tokenEndpoint` (`string`)**
TokenEndpoint is the IdP token endpoint used to acquire the actor token.

**Field: `clientId` (`string`)**
ClientID is the client identifier for the actor.

**Field: `clientSecret` (`string`)**
ClientSecret accepts `env://`, `secrets://`, `vault://`, `consul://` prefixes.

**Field: `scopes` (`[]string`)**
Scopes requested when minting the actor token.

---

### **OAuth2ActorHeader**

OAuth2ActorHeader pulls the actor token from an inbound request header.

**Field: `name` (`string`)**
Name is the request header name. Defaults to `"X-Actor-Token"`.

**Field: `strip` (`boolean`)**
Strip removes the header from the proxied request. Defaults to `true`.

**Field: `required` (`boolean`)**
Required rejects requests missing the header. Defaults to `true`.

---

### **OAuth2ActorStatic**

OAuth2ActorStatic carries a pre-configured static actor token. For testing only — static tokens do not rotate.

**Field: `token` (`string`)**
Token is the static actor token value. Accepts `env://`, `secrets://`, `vault://`, `consul://` prefixes.
