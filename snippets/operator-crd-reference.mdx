{/* DO NOT EDIT. Auto-generated from the Tyk Operator CRD schemas (config/crd/bases) by the exp tyk-docs sync workflow. To change a field description, edit the Go doc-comments in tyk-operator-internal. */}
...
<ParamField body="tykOAS.configmapRef" type="object">
  ConfigmapRef points at the ConfigMap storing the Tyk OAS document. Required in standalone mode. Nil in Gateway API attachment mode (targetRefs set): the document is rendered from the attached object instead.
</ParamField>
...
<ParamField body="targetRefs[]" type="object">
  TargetRefs attaches this CR to a Kubernetes Gateway API object (HTTPRoute or Gateway) as a GEP-713 policy. When set, the CR runs in attachment mode and the routing fields of the attached OAS document are ignored.
</ParamField>

<ParamField body="targetRefs[].group" type="string" required>
  Group is the group of the target resource.
</ParamField>

<ParamField body="targetRefs[].kind" type="string" required>
  Kind is kind of the target resource.
</ParamField>

<ParamField body="targetRefs[].name" type="string" required>
...
<ParamField body="access_rights.{key}.kind" type="string" default="ApiDefinition">
  Kind represents the kind of the linked API Definition resource. It can be "ApiDefinition", "TykOasApiDefinition", "TykStreamsApiDefinition", "TykMcpProxyDefinition", or "HTTPRoute" — a Gateway API HTTPRoute whose synthesized API definitions are covered by one entry (case sensitive way). By default, Kind is set to "ApiDefinition". **Allowed values:** `ApiDefinition`, `TykOasApiDefinition`, `TykStreamsApiDefinition`, `TykMcpProxyDefinition`, `HTTPRoute`.
</ParamField>
...
<ParamField body="dataPlane" type="object">
  DataPlane binds this context to a data plane group (environment). Gateway API mode only; read on Gateway-level contexts, ignored on class-level ones.
</ParamField>

<ParamField body="dataPlane.serviceRef" type="object" required>
  ServiceRef is the k8s Service fronting this environment's data plane gateways, in this object's own namespace (the type carries no namespace, so cross-namespace is impossible). Gateway status addresses and listener port validation derive from it, so a data plane without its Service is not functional.
</ParamField>

<ParamField body="dataPlane.serviceRef.name" type="string">
  Name of the referent. More info: https://kubernetes.io/docs/concepts/overview/working-with-objects/names/#names
</ParamField>

<ParamField body="dataPlane.tags[]" type="string">
  Tags are the segment tags of this environment, stamped into server.gatewayTags of every synthesized API.
</ParamField>
...
TykRuleset is the Schema for the tykrulesets API.

### Top-Level Fields

<ParamField body="name" type="string" required>
  Name is the Dashboard-visible name of the ruleset.
</ParamField>

<ParamField body="action" type="string" default="none">
  Action governs what happens when the Operator finds violations for this ruleset. **Allowed values:** `none`, `warn`, `block`.
</ParamField>
...
<ParamField body="resourceType" type="string" default="oas-api">
  ResourceType is what this ruleset applies to. **Allowed values:** `oas-api`, `mcp-api`.
</ParamField>

### ruleset

<ParamField body="ruleset" type="object" required>
  Ruleset is the source of the Spectral JSON/YAML body. Exactly one of raw or objRef must be set.
</ParamField>

<ParamField body="ruleset.objRef" type="object">
  ObjRef points at a ConfigMap or Secret whose data[keyName] holds the Spectral JSON/YAML body.
</ParamField>

<ParamField body="ruleset.objRef.keyName" type="string" required>
  KeyName is the data key holding the ruleset body.
</ParamField>

<ParamField body="ruleset.objRef.kind" type="string" required>
  Kind is the referenced object kind. **Allowed values:** `ConfigMap`, `Secret`.
</ParamField>

<ParamField body="ruleset.objRef.name" type="string" required>
  Name is the object name.
</ParamField>

<ParamField body="ruleset.objRef.namespace" type="string">
  Namespace must be omitted; cross-namespace references are not permitted.
</ParamField>

<ParamField body="ruleset.raw" type="string">
