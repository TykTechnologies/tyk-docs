...
A [Policy](/api-management/policies) encapsulates several options that can be applied to a key. It acts as a template that can override individual sections of an API key (or identity) in Tyk.
...
5. **Add Configuration Details**
    ...
    2. Set the policy state. You can set your policy to one of the following states:
        ...
        * Draft
        * Access Denied 
    3. Set a time after which any Keys subscribed to your policy expire. Select a value from the drop-down list. This is a required setting. See [Session Lifecycle](/api-management/access-control/sessions-and-keys/session-lifecycle) for more details.
    4. Configure Behaviour on key expiry. When key expiry is set, the **Behaviour on key expiry** options appear:

        {/* TODO: Screenshot — Dashboard "Behaviour on key expiry" section on the Add Policy page, showing the Delete key / Retain key radio choice and the three retention mode options */}

        * **Delete key**: Immediately deletes session record from Redis upon expiration.
        * **Retain key** (Default): Keeps the session in Redis post-expiration. Choose one retention mode:
            ...
            * **Use API or gateway default** (Default): Falls back to the [legacy controls](/api-management/access-control/sessions-and-keys/session-lifecycle#legacy-controls) (custom API key lifetime or Gateway global session lifetime).
    5. Add Tags to your policy. Any tags you add can be used when filtering Analytics Data. Tags are case sensitive.
    6. Add Metadata to your policy. Adding metadata such as User IDs can be used by middleware components. See [Session Metadata](/api-management/access-control/sessions-and-keys/understanding-sessions#metadata-and-context) for more details.
...
For more information on how Policies are constructed and applied to Sessions, please see the [Access Control](/api-management/access-control/overview) section.
