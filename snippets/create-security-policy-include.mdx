...
A [Policy](/api-management/access-control/policies/understanding-policies) encapsulates several options that can be applied to a key. It acts as a template that can override individual sections of an API key (or identity) in Tyk.
...
5. **Add Configuration Details**
    ...
    5. Add Metadata to your policy. Adding metadata such as User IDs can be used by middleware components. See [Session Metadata](/api-management/access-control/sessions-and-keys/understanding-sessions#metadata-and-context) for more details.
