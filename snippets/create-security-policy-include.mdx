...
See [What is a Security Policy?](/docsv2/nightly/api-management/policies#what-is-a-security-policy)
...
4. **Setting Global Rate Limits and Quota**
    ...
    **Rate Limiting**
    ...
    A rate limit is enforced on all keys, set the number of requests per second that a user of a key with this policy is allowed to use. See [Rate Limiting](/docsv2/nightly/api-management/rate-limit#rate-limiting-layers) for more details. **Note: The Rate Limit set by a policy will override the limits applied to an individual key.**

    **Throttling**

    When hitting rate limits, you can set Tyk Gateway to automatically queue and auto-retry client requests. Throttling can be configured at a key or policy level. See [Request Throttling](/docsv2/nightly/api-management/request-throttling) for more details.

    **Usage Quotas**

    Usage quotas limit the number of total requests a user is allowed to have over a longer period of time. So while a rate limit is a rolling window, a quota is an absolute maximum that a user is allowed to have over a week, a day or a month. See [Request Quotas](/docsv2/nightly/api-management/request-quotas) for more details.
...
5. **Add Configuration Details**
    ...
    2. Set the policy state. You can set your policy to one of the following states:
        ...
        * Draft
        * Access Denied 
    3. Set a time after which any Keys subscribed to your policy expire. Select a value from the drop-down list. This is a required setting. See [Key Expiry](/docsv2/nightly/api-management/policies#key-expiry) for more details.
    4. Add Tags to your policy. Any tags you add can be used when filtering Analytics Data. Tags are case sensitive.
    5. Add Metadata to your policy. Adding metadata such as User IDs can be used by middleware components. See [Session Metadata](/docsv2/nightly/api-management/policies#what-is-a-session-metadata) for more details.
...
For more information on how policies are constructed and a detailed explanation of their properties, please see the [Security Policies](/docsv2/nightly/api-management/policies) section.
