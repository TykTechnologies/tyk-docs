...
Set it above the longest RPC timeout and comfortably above the data plane keepalive interval (~10s) to reduce reconnect and TLS handshake churn. It applies only to server_options.
...
Size it for the number of client processes times rpc_pool_size, plus reconnect headroom. Excess connections are closed before TLS negotiation. It applies only to server_options.
