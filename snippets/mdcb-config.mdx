...
Set it above the Gateway's [slave_options.call_timeout](/tyk-oss-gateway/configuration#slave_options-call_timeout), the longest RPC call duration.
Set it above [slave_options.ping_timeout](/tyk-oss-gateway/configuration#slave_options-ping_timeout) too, the data plane keepalive interval (~10s), to reduce reconnect and TLS handshake churn.
This field takes effect only under server_options; the same field under http_server_options has no effect.
...
Size it for the number of client processes times the Gateway's [slave_options.rpc_pool_size](/tyk-oss-gateway/configuration#slave_options-rpc_pool_size), plus reconnect headroom. Excess connections are closed before TLS negotiation.
This field takes effect only under server_options; the same field under http_server_options has no effect.
...
### http_server_options.max_connection_idle_time
ENV: <b>TYK_MDCB_HTTPSERVEROPTIONS_MAXCONNECTIONIDLETIME</b><br />
Type: `int`<br />
...
### http_server_options.max_connections
ENV: <b>TYK_MDCB_HTTPSERVEROPTIONS_MAXCONNECTIONS</b><br />
Type: `int`<br />
...
List of aggregate types to exclude from storage. Aggregates in this list are dropped before being written to the collection. See [pumps.mongoaggregate.meta.ignore_aggregations](/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#pumps-mongoaggregate-meta-ignore_aggregations) for detailed behavior.

### analytics.mongo_enable_aggregate_self_healing
ENV: <b>TYK_MDCB_ANALYTICSCONFIG_MONGOENABLEAGGREGATESELFHEALING</b><br />
Type: `bool`<br />

If set to true, enables self-healing for aggregate data. See [pumps.mongoaggregate.meta.enable_aggregate_self_healing](/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#pumps-mongoaggregate-meta-enable_aggregate_self_healing) for detailed behavior.
