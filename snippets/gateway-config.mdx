...
When set to `true`, enables [Zstandard (zstd)](https://facebook.github.io/zstd/) compression for API definitions stored in Redis RPC backups. This feature significantly reduces Redis memory usage in [MDCB](/api-management/mdcb) deployments where API definitions are cached locally on Data Plane Gateways.
