...
<Note>
The environment variable `TYK_GW_LOCALSESSIONCACHE_DISABLECACHESESSIONSTATE` is generated from the Go configuration struct field name (`DisableCacheSessionState`), not the JSON field name (`disable_cached_session_state`). This is the expected behavior for how Tyk Gateway processes environment variables.
</Note>
