...
When set to `true`, enables [Zstandard (zstd)](https://facebook.github.io/zstd/) compression for API definitions stored in Redis RPC backups. This feature significantly reduces Redis memory usage in [MDCB](/api-management/mdcb) deployments where API definitions are cached locally on worker Gateways.
...
**Key benefits:**
- Reduces Redis memory consumption for API definition storage
- Particularly beneficial for deployments with many or large API definitions
- Compression is applied transparently when storing backups, and decompression occurs automatically when loading

**Compatibility:**
- Compressed and uncompressed data are handled automatically - the Gateway detects the format when loading backups
- Safe to enable on existing deployments; existing uncompressed backups will continue to work
- New backups will be stored compressed once enabled
