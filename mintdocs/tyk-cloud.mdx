...
The Tyk Cloud trial does not include access to [Hybrid deployments](#deploy-hybrid-gateways) or the [Developer Portal](/mintdocs/portal/overview#).
To try out these capabilities, please get in touch for a [guided evaluation](https://tyk.io/guided-evaluation/) with our team.
</Note>


Welcome to the [Tyk Cloud Platform](/mintdocs/tyk-cloud)!
This guide will lead you through the following steps:
1. Signing up with [Tyk Cloud ](/mintdocs/tyk-cloud).
2. Creating your first [API](/mintdocs/api-management/gateway-config-introduction) using the [Tyk Dashboard](/mintdocs/tyk-dashboard).
3. Setting up a [Policy](/mintdocs/api-management/policies#what-is-a-security-policy) and Key to secure your APIs.
...
To create Tyk Cloud account follow this [guide](/mintdocs/getting-started/create-account).

2. **Get started with your first API with Tyk Dashboard**

To create your API using Tyk Dashboard follow this [guide](/mintdocs/getting-started/configure-first-api).
...
For further details, please refer to the [release notes](/mintdocs/developer-support/release-notes/gateway#530-release-notes) for Tyk Gateway v5.3.0.
</Warning>
...
See [Tyk Classic API versioning](/mintdocs/api-management/gateway-config-tyk-classic#tyk-classic-api-versioning) for more details.
...
Your API will appear in your APIs list. If you select **EDIT** from the **ACTIONS** drop-down list, you can see the endpoints (from the [Endpoint Designer](/mintdocs/api-management/dashboard-configuration#endpoint-designer)) that have been created as part of the import process.
...
Your Organization is your "container" for all your Environments, Control Planes and Cloud Data Planes. When you setup your Organization when [creating your account](/mintdocs/getting-started/create-account), you assign it to a Home Region where all your data is stored. You cannot change this home region after creating your organization.
...
Control Planes are situated in your Organization's home region and provide links to an instance of the [Tyk Dashboard](/mintdocs/tyk-dashboard) and the [Developer Portal](/mintdocs/tyk-developer-portal). The Dashboard is where you perform all your API tasks. The developer portal allows your 3rd party developers access to your APIs. Cloud Data Planes are then connected to your Control Planes.
...
The following quick start guide explains how to use the [Tyk Data Plane Helm chart](/mintdocs/product-stack/tyk-charts/tyk-data-plane-chart) to configure Tyk Gateway that includes:
...
For the complete installation guide and configuration options, please see [Tyk Data Plane Chart](/mintdocs/product-stack/tyk-charts/tyk-data-plane-chart).
...
* [Adding APIs](/mintdocs/api-management/gateway-config-managing-classic#create-an-api) to Tyk, including REST and GraphQL APIs
* Applying Quotas and Rate limits via [Security Policies](/mintdocs/api-management/gateway-config-managing-classic#secure-an-api) and [Keys](/mintdocs/api-management/gateway-config-managing-classic#access-an-api)
* [Securing](/mintdocs/api-management/security-best-practices#securing-apis-with-tyk) your APIs
* Viewing granular [Analytics](/mintdocs/api-management/dashboard-configuration#traffic-analytics) for your Tyk managed APIs
* [Transform traffic](/mintdocs/api-management/traffic-transformation#) with the Tyk API Designer
* Add integration options such as [SSO](/mintdocs/api-management/external-service-integration#single-sign-on-sso) and [3rd Party IdentityProviders](/mintdocs/api-management/external-service-integration)
...
* [Team Admin](#assign-user-roles) - Can only manage the users of the team they are a member of.
    ...
    You can also [add users to the Tyk Dashboard](/mintdocs/api-management/user-management#manage-tyk-dashboard-users) itself instead of inviting them as Tyk Cloud users. These users would likely be your API Developers and Engineers who manage the APIs.
    </Note>
...
You can [add users to the Tyk Dashboard](/mintdocs/api-management/user-management#manage-tyk-dashboard-users) itself instead of inviting them as Tyk Cloud users. These users would likely be your API Developers and Engineers who manage the APIs.   
...
| Grow <p>[Standard Support SLA](/mintdocs/developer-support/support)</p> | For single teams with high traffic. | This plan includes all of the features of the Tyk Stack. In this plan, you have **[Hybrid Gateways](#deploy-hybrid-gateways)** as an add on, along with standard support. |
| Scale <p>[Enhanced Support SLA](/mintdocs/developer-support/support)</p> | For global organizations with multiple teams, requiring gateway deployments in multiple locations. | This plan includes all of the features of the Tyk Stack. **Enhanced(silver) support** will be provided. |
...
Our Developer Portal is completely customizable. See [Portal Customization](/mintdocs/tyk-developer-portal/customise) for more details.
...
* [Python Plugins](/mintdocs/api-management/plugins/rich-plugins#overview)
* [JSVM](/mintdocs/api-management/plugins/javascript#)
...
Next you'll [set up the Python authentication code bundle](/mintdocs/tyk-cloud#create-a-python-code-bundle).
...
| File              | Description                                                                                                                                                                                                                                                                                       |
|-------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| custom_middleware | contains the middleware settings like the plugin driver you want to use (driver) and the hooks that your plugin will expose. We use the   **auth_check** for this tutorial. For other hooks see [here](/mintdocs/api-management/plugins/rich-plugins#coprocess-dispatcher---hooks). |
...
* You import decorators from the Tyk module that gives us the Hook decorator, and we import [Tyk Python API helpers](/mintdocs/api-management/plugins/rich-plugins#tyk-python-api-methods)
...
* A plugin bundle is a packaged version of the plugin, it may also contain a cryptographic signature of its contents. The -y flag tells the Tyk CLI tool to skip the signing process in order to simplify this tutorial. For more information on the Tyk CLI tool, see [here](/mintdocs/api-management/plugins/overview#how-plugin-bundles-work).
...
1. Firstly you will need a local Tyk Gateway installation that allows you to create your Python plugin bundle. We recommend [installing our Self-Managed version on Ubuntu Bionic 18.04](/mintdocs/tyk-self-managed#install-tyk-on-debian-or-ubuntu).
...
We recommend that all users to migrate to the `tyk-data-plane` Chart. Please review the [Configuration](/mintdocs/product-stack/tyk-charts/tyk-data-plane-chart#configuration) section of the new helm chart and cross-check with your existing configurations while planning for migration.
</Warning>
...
<Note>
Please make sure you are installing Redis versions that are supported by Tyk. Please refer to Tyk docs to get list of [supported versions](/mintdocs/tyk-self-managed#redis-1).
</Note>
...
#### Tyk Cloud Storage
When a client makes a request to a Tyk Gateway deployment, the details of the request and response are captured and [stored in Redis](/mintdocs/api-management/dashboard-configuration#traffic-analytics). Tyk Pump processes the records from Redis and forwards them to MongoDB. Finally, Tyk Cloud reads that data from MongoDB and displays its size(bytes) in the _Storage_ section of _Monitoring_. 
...
Since this functionality is powered by Tyk Gateway's OpenTelemetry integration, we recommend reviewing our comprehensive [OpenTelemetry documentation](/mintdocs/api-management/logs-metrics#opentelemetry)
