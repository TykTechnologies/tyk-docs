...
However, if you already created [a theme](/mintdocs/portal/customization#) and want the portal to use when it's starts for the first time, then you can use this setting to achieve that.
...
**Type:** `string` <br/>
**Description**: API secret for enabling [Single Sign-on (SSO) flow](/mintdocs/portal/settings#configure-developer-portal-sso) with the Tyk Identity Broker.
