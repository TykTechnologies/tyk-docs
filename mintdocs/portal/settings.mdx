...
Tyk Enterprise Developer Portal uses the [Tyk Identity Broker (TIB)](/mintdocs/api-management/external-service-integration#what-is-tyk-identity-broker-tib) to integrate Tyk authentication with 3rd party identity providers (IDPs). 
...
To enable Embedded TIB in the portal, add the `PORTAL_TIB_ENABLED` variable to [the portal .env file](/mintdocs/product-stack/tyk-enterprise-developer-portal/deploy/configuration#sample-env-file):
```.ini
...
1. **Developers**:
    ...
    Developers created by the sso flow are portal users that belong to an organization and team/s, if a user group mapping is not specified, they are assigned to the default organization and default team. Developers created by the sso flow are always assinged the **Consumer Super Admin** role. If part of an organization and a team, this means that the developer is a super admin for that organization. Read more about managing api consumer organizations [here](/mintdocs/portal/api-consumer#manage-api-consumer-organizations).
...
To define the user group mapping for your developer audience, you need to add the UserGroupMapping object to the corresponding [TIB profile](/mintdocs/api-management/external-service-integration#exploring-tib-profiles):
```yaml
  "UserGroupMapping": {
...
The Tyk Identity Broker (TIB) uses [profiles](/mintdocs/api-management/external-service-integration#exploring-tib-profiles) to define details related to the identity provider such as its type and access credentials, and instructs TIB on how to treat users that try log in with that provider.
...
Please refer to the [TIB configuration section](/mintdocs/api-management/external-service-integration#single-sign-on-sso) for step-by-step instructions for setting up the UseProviders section.
...
You can use any value for the portal SSO API secret, but it should be consistent with [TIB configuration](/mintdocs/portal/settings#configure-tyk-identity-broker-to-work-with-tyk-enterprise-developer-portal).

To specify the portal SSO API secret, add the `PORTAL_API_SECRET` variable to [the portal .env file](/mintdocs/product-stack/tyk-enterprise-developer-portal/deploy/configuration#sample-env-file):
```.ini
PORTAL_API_SECRET=your-portal-api-secret
```

If you use [the Tyk helm chart](/mintdocs/tyk-self-managed#install-more-tyk-components), it is required to add the `PORTAL_API_SECRET` to extraEnvs:
```.yaml
extraEnvs:
...
The Tyk Enterprise Developer portal uses the [Tyk Identity Broker](/mintdocs/api-management/external-service-integration#what-is-tyk-identity-broker-tib) to work with various Identity Management Systems, such as LDAP,
...
Please refer to [the TIB installation guide documentation](/mintdocs/api-management/external-service-integration#install-standalone-tib) for different installation options:
...
- [Tyk helm chart](/mintdocs/api-management/external-service-integration#install-standalone-tib)
...
The full reference for the configuration file is in [the TIB section of the documentation](/mintdocs/tyk-configuration-reference/tyk-identity-broker-configuration).
###### Helm charts
If you wish ot deploy TIB in Kubernetes via [Tyk helm chart](/mintdocs/api-management/external-service-integration#install-standalone-tib), you need to specify TIB config as extraVars:
```.yaml
extraEnvs:
