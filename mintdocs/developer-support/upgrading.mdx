...
- **Long Term Support:** Refer to our [versioning and long-term support policies](/mintdocs/developer-support/release-types/long-term-support) for details on major and minor releases, patches, and support dates.
...
- **Backups:** Follow our [comprehensive backup guide](/mintdocs/developer-support/faq#tyk-configuration) before starting the upgrade.
...
Use our [migration tool](/mintdocs/tyk-self-managed#migrating-from-an-existing-mongodb-instance) to manage the switch from MongoDB to SQL.

##### Backup APIs Script

Utilize our bash [backup script](/mintdocs/developer-support/upgrading#backup-apis-and-policies) to export and restore all Tyk API Definitions and Policies.
...
Have backups been performed?
  ...
  - Backup guide: Have you checke our [comprehensive guide for backing up Tyk](/mintdocs/developer-support/faq#tyk-configuration)?
...
3. **Upgrade Process:**
   ...
   - If using a new deployment process, follow the [deployment instructions](/mintdocs/tyk-self-managed) appropriate for your platform.
...
Follow our guide for [upgrading Cloud Control Planes](/mintdocs/tyk-cloud#upgrade-control-planes).
...
2. Update the [custom_middleware_bundle](/mintdocs/api-management/plugins/overview#plugin-bundles) field in the API Definitions of all APIs that use your plugin. The field should be updated to use the new bundle file containing your upgrade plugin.
3. Validate that your plugin is working per your expectations.

**Gateway Versions >= 4.1.0**

1. Update the [custom_middleware_bundle](/mintdocs/api-management/plugins/overview#plugin-bundles) field in the API Definitions of all APIs that use your plugin. The field should be updated to use the new bundle file containing your upgraded plugin.
...
Follow our guide for [upgrading Cloud Data Planes](/mintdocs/tyk-cloud#managing-cloud-data-plane).
...
See Tyk Guide for how to [upgrade Control Planes](/mintdocs/tyk-cloud#upgrade-control-planes)
...
2. Update the [custom_middleware_bundle](/mintdocs/api-management/plugins/overview#plugin-bundles)
...
1. Update the [custom_middleware_bundle](/mintdocs/api-management/plugins/overview#plugin-bundles)
field in the API Definitions of all APIs that use your plugin. The field should be updated to use the new bundle file
...
1. [Create a plugin bundle](/mintdocs/api-management/plugins/overview#plugin-bundles) that includes the newly compiled version
...
2. [Upload this bundle](/mintdocs/tyk-cloud#uploading-your-bundle) to your configured S3 bucket if using Cloud SaaS. If you're using Hybrid SaaS, upload this bundle to your bundle server.


**Path 2 - Current Version >= 4.1.0 and Target Version >= 4.2.0**

1. [Create a plugin bundle](/mintdocs/api-management/plugins/overview#plugin-bundles) that includes both your current version’s plugin along with the newly compiled version
...
the addition of the [Multi Data Center Bridge](/mintdocs/tyk-multi-data-centre) (MDCB).
...
For full installation details, check the usual [installation page](/mintdocs/tyk-open-source#install-tyk-gateway-with-docker).
...
If you are using Self Managed deployment then we recommend that you use [Tyk Sync](/mintdocs/api-management/automations/sync) to backup your Tyk APIs and policies. 
