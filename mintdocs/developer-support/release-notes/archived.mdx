...
See our [TCP Proxy Docs](/mintdocs/key-concepts/tcp-proxy) for more details.
...
See our [JWT Scope docs](/mintdocs/basic-config-and-security/security/authentication-authorization/json-web-tokens) for more details.
...
See our [Golang plugin documentation](/mintdocs/api-management/plugins/golang#) for more details.
...
See our [Distributed Tracing documentation](/mintdocs/api-management/logs-metrics#opentelemetry) for more details.
...
See our updated [Getting Started](/mintdocs/tyk-self-managed#installation-options-for-tyk-self-managed) section and [new CLI documentation](/mintdocs/tyk-self-managed).
...
See our [Dashboard SSO documentation](/mintdocs/api-management/dashboard-configuration#single-sign-on-api) for more details.

### Importing WSDL APIs

WSDL now is a first class citizen at Tyk. You can take your WSDL definition and simply import to the Dashboard, creating a nice boilerplate for your service. See [Import APIs](/mintdocs/api-management/gateway-config-managing-classic#import-an-api) for more details.
...
See [Debugging Tab](/mintdocs/api-management/dashboard-configuration#debugging) for more information.
...
There is a [new section](/mintdocs/api-management/dashboard-configuration#dashboard-audit-logs) in the Tyk Dashboard config file where you can specify parameters for the audit log (containing audit records for all requests made to all endpoints under the `/api` route).
...
> **TLDR**
> To get benefit or performance improvements ensure that you have `close_connections` set to `false` and set `max_idle_connections_per_host` according to our [production perfomance guide](/mintdocs/tyk-self-managed#planning-for-production)
...
Instead of setting permissions per user, you can now [create a user group](/mintdocs/api-management/user-management#manage-tyk-dashboard-user-groups), and assign it to multiple users. It works for Single Sign-On too, just specify group ID during [SSO API](/mintdocs/api-management/dashboard-configuration#single-sign-on-api-1) flow.
...
From an API standpoint, user groups can be managed by [new Dashboard API](/mintdocs/api-management/dashboard-configuration#user-groups-api). The User object now has a new `group_id` field, and if it is specified, all permissions will be inherited from the specified group. [SSO API](/mintdocs/api-management/dashboard-configuration#single-sign-on-api-1) has been updated to include `group_id` field as well.

#### Added SMTP support
Now you can configure the Dashboard to send transactional emails using your SMTP provider. See [Outbound Email Configuration](/mintdocs/configure/outbound-email-configuration) for details.

#### <a name="upgrade"></a>Upgrading all new Components

For details on upgrading all Tyk versions, see [Upgrading Tyk](/mintdocs/developer-support/upgrading).
...
See the Keys section of the [Tyk Gateway REST API](/mintdocs/tyk-gateway-api) Swagger doc for more details.
...
See [List Key Requests](/mintdocs/tyk-apis/tyk-dashboard-api/manage-key-requests#list-key-requests) for more details.
...
See [Developer Profiles](/mintdocs/tyk-developer-portal/tyk-portal-classic/developer-profiles#reset-developer-password) for more details.
...
[Docs](/mintdocs/api-management/logs-metrics#statsd-instrumentation)
...
[Docs](/mintdocs/api-management/gateway-config-tyk-classic#tyk-classic-api-versioning)
...
[Docs](/mintdocs/tyk-oss-gateway/configuration#http_server_options)
...
[Docs](/mintdocs/tyk-oss-gateway/configuration#enable_key_logging)
...
[Docs](/mintdocs/api-management/certificates)
...
[JSVM Docs](/mintdocs/api-management/plugins/javascript#using-javascript-with-tyk)
[Plugin Data Structure Docs](/mintdocs/api-management/plugins/rich-plugins#rich-plugins-data-structures)
...
* [API Docs](/mintdocs/api-management/dashboard-configuration#manage-apis-api-definition)
* [Policy Docs](/mintdocs/tyk-apis/tyk-dashboard-api/portal-policies)
...
[Docs](/mintdocs/transform-traffic/url-rewriting#url-rewrite-middleware)
...
[Docs](/mintdocs/tyk-oss-gateway/configuration)
...
A major feature of this release is the implementation of Mutual TLS. Now you can protect your APIs by allow listing certificates, idenitfy users based on them, and increase security between Tyk and upstream API. For details, see [Mutual TLS](/mintdocs/api-management/client-authentication#use-mutual-tls).
...
[Docs](/mintdocs/api-management/policies#partitioned-policies)
...
[Docs](/mintdocs/api-management/dashboard-configuration#activity-logs)
...
* Enable Tyk Identity Broker to apply LDAP filters to user search [Docs](/mintdocs/api-management/external-service-integration#advance-ldap-configuration)
* Set permissions for your users, logged via SSO, via `sso_permission_defaults` in Dashboard config file. [Docs](/mintdocs/api-management/external-service-integration)
* Setup a login page redirect, using `sso_custom_login_url` and `sso_custom_portal_login_url` Dashboard config options to enable users login using a custom SSO login page. [Docs](/mintdocs/api-management/external-service-integration)
* For those who love to build everything in-house, we have added new API for custom dashboard authentication integrations. [Docs](/mintdocs/api-management/external-service-integration#custom-proxy-identify-provider)
...
#### Mutual TLS support
[Docs](/mintdocs/api-management/client-authentication#use-mutual-tls)

#### Global API rate limits
[Docs](/mintdocs/api-management/rate-limit#rate-limiting-layers)
...
[Docs](/mintdocs/tyk-oss-gateway/configuration#control_api_port)
...
[Docs](/mintdocs/tyk-oss-gateway/configuration#log_level)
...
[Docs](/mintdocs/api-management/traffic-transformation#request-body-overview)
...
```
[Docs](/mintdocs/api-management/external-service-integration)
...
[Docs](/mintdocs/api-management/external-service-integration#custom-proxy-identify-provider)
...
Added support for Mutual TLS, mentioned by Gateway and Dashboard above. See [Docs](/mintdocs/api-management/client-authentication#use-mutual-tls)
...
Tyk Identity Broker now fully support LDAP search with complex filters! [Docs](/mintdocs/api-management/external-service-integration#advance-ldap-configuration)
...
Hybrid users should follow the upgrade instructions [here](/mintdocs/developer-support/upgrading#tyk-upgrade-guides-for-different-deployment-models).
