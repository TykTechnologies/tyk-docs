...
The **[Tyk Gateway](/5.14/tyk-oss-gateway)** is the core of Tyk’s API management platform. This open-source, high-performance API gateway handles crucial API tasks:
...
The **[Tyk Dashboard](/5.14/api-management/dashboard-configuration)** is a GUI interface for managing, monitoring, and configuring APIs. It integrates with the Tyk Gateway and offers centralized control for all API operations:
...
The **[Tyk Developer Portal](/5.14/portal/overview/intro)** provides a self-service interface for developers to access and interact with APIs. Key functionalities include:
...
The **[Tyk Multi Data Centre Bridge (MDCB)](/5.14/api-management/mdcb)** enables centralized control over API configurations across distributed regions or data centers. It’s ideal for global API infrastructure, offering:
...
**[Tyk Pump](/5.14/api-management/tyk-pump)** is a lightweight tool that aggregates API analytics from the Tyk Gateway and exports them to various storage systems for analysis:
...
The **[Tyk Operator](/5.14/api-management/automations/operator#what-is-tyk-operator)** is a Kubernetes-native API management solution, allowing teams to deploy and manage APIs as Kubernetes resources:
...
**[Tyk Streams](/5.14/api-management/event-driven-apis#)** is Tyk’s real-time data streaming tool for APIs, enabling applications to receive data as events happen:
...
**[Tyk Sync](/5.14/api-management/automations/sync)** facilitates configuration synchronization, helping teams manage API configurations across different environments:
...
The **[Tyk Identity Broker](/5.14/tyk-identity-broker/overview)** simplifies authentication by connecting APIs with external identity providers (IDPs), supporting single sign-on (SSO) capabilities:
...
**[Tyk Helm Charts](/5.14/product-stack/tyk-charts/overview)** provide templated configurations for deploying Tyk components in Kubernetes, enabling efficient setup and management:
...
The **[Universal Data Graph (UDG)](/5.14/api-management/data-graph#overview)** provides a single API endpoint for accessing data from multiple sources using GraphQL, offering:
...
1. **[Getting Started with Tyk Gateway](/5.14/tyk-oss-gateway)**  
...
2. **[Set Up and Configure the Tyk Dashboard](/5.14/api-management/dashboard-configuration)**  
...
3. **[Explore API Security](/5.14/api-management/client-authentication)**  
...
4. **[Implement Multi-Region Deployments with MDCB](/5.14/api-management/mdcb)**  
...
5. **[Use Tyk Sync for Consistent Configuration Management](/5.14/api-management/automations/sync)**  
...
6. **[Explore Tyk Developer Portal for Enhanced Developer Experience](/5.14/portal/overview/intro)**  
