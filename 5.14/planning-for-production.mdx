...
To secure your Tyk installation, you can configure the following settings in your [tyk.conf](/5.14/tyk-oss-gateway/configuration):
...
Under high traffic, consider isolating Redis workloads across dedicated instances. Tyk Gateway supports a [separate analytics store](/5.14/planning-for-production/database-settings#separate-analytics-storage) for traffic log buffering and a [separate cache store](/5.14/planning-for-production/database-settings#separate-cache-storage) for API response caching, keeping them isolated from session and rate limiting data.

For guidance on configuring Redis, including clustering, Sentinel, and TLS, see [Configure Redis](/5.14/tyk-configuration-reference/redis-cluster-sentinel). For database sizing guidelines and memory management options, see [Database Management](/5.14/planning-for-production/database-settings).
...
To avoid this, apply any license change to every Dashboard instance that shares the same backend. See the [Upgrading Guide](/5.14/developer-support/upgrading#upgrade-strategies) for upgrade guidance and [Troubleshooting](/5.14/api-management/troubleshooting-debugging#dashboard) to resolve the error.

## Other Dashboard Security Considerations

In addition to changing the default secrets (see [Change all the shared secrets](/5.14/#change-all-the-shared-secrets)) if you change the Control API port (see [Change your Control Port](/5.14/#change-your-control-port)), you also need to change the connection string settings in your `tyk_analytics.conf` file.

## Ensure you are matching only the URL paths that you want to match

We recommend that you configure Tyk Gateway to use [exact URL path matching](/5.14/getting-started/key-concepts/url-matching#matching-modes) and to enforce [strict route matching](/5.14/tyk-oss-gateway/configuration#http_server_options-enable_strict_routes) to avoid accidentally invoking your unsecured `/health` endpoint when a request is made to `/customer/{customer_id}/account/health`...
