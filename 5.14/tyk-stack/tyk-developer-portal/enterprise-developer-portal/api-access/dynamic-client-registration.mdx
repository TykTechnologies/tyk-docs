...
At runtime, the API Consumer uses their credentials to obtain a JWT from the IdP, then presents it to Tyk Gateway when calling an API. The Gateway [validates the token's signature](/5.14/api-management/authentication/jwt-signature-validation) using the public keys published by the IdP at its JWKS endpoint, and maps the token's [scopes](/5.14/#oauth-2-0-scopes) to Tyk policies to enforce access control.
...
<Note>
Refer to [Migrating to the Identity Provider Registry](/5.14/#migrating-to-the-identity-provider-registry) if you have an existing installation using DCR and wish to use the IdP Registry.
</Note>
...
Create scopes in the IdP for each API Product and Plan you intend to use with DCR, following the naming and uniqueness requirements in the [OAuth 2.0 Scopes](/5.14/#oauth-2-0-scopes) section above. Scopes must exist before the Product or Plan is published; see that section for details.
...
It must be explicitly enabled in Tyk Developer Portal by setting [`TYK_PORTAL_ENABLEIDPREGISTRY=true`](/5.14/product-stack/tyk-enterprise-developer-portal/deploy/configuration#tyk_portal_enableidpregistry) (or `EnableIDPRegistry = true` in the config file) and restarting the Portal.
...
| **Scope claim name** (optional) | The JWT claim that contains the token's scopes. Different IdPs use different claim names: `scope` is the most common, but some use `scp` or another custom claim. Defaults to `scope`. Used by Tyk Gateway when [mapping token scopes to policies](/5.14/api-management/authentication/jwt-authorization#scope-policies). |
| **Registration access token** (optional) | The token obtained in the [Authorize the Portal](/5.14/#authorize-the-portal) section. |
...
Each API Product and each Plan used with DCR must have a unique OAuth 2.0 scope assigned, matching a scope you created in the IdP. See the [OAuth 2.0 Scopes](/5.14/#oauth-2-0-scopes) section for the full requirements.

See [API Products](/5.14/portal/api-products#dynamic-client-registration) and [API Plans](/5.14/portal/api-plans#dynamic-client-registration) for configuration instructions.
...
- At least one [OAuth 2.0 Provider](/5.14/#configure-your-identity-provider) is configured in the Portal, with at least one [Client Profile](/5.14/#client-profiles) defined.
...
- Both scopes [exist in the IdP](/5.14/#oauth-2-0-scopes).
...
To migrate to the Identity Provider Registry, first upgrade to Portal 1.18.0, then follow the [Migration Steps](/5.14/#migration-steps) below. Note that the automatic backfill does not cover manually configured API definitions. You will need to create OAuth 2.0 Providers in the Portal for each IdP, then populate the Registry manually via `POST /api/clientidps`.
...
To migrate to the Identity Provider Registry, first upgrade to Portal 1.18.0, then follow the [Migration Steps](/5.14/#migration-steps) below.

### Migration Steps

1. Set [`TYK_PORTAL_ENABLEIDPREGISTRY=true`](/5.14/product-stack/tyk-enterprise-developer-portal/deploy/configuration#tyk_portal_enableidpregistry) (or `EnableIDPRegistry = true` in the config file) and restart the Portal.
    ...
    If moving from a [legacy version](/5.14/#legacy-setup-before-portal-1-13-0) there will be nothing for the backfill to process.
...
2. **Audit your API definitions.**
    ...
    <Note>
    There is no Tyk Dashboard UI for the Identity Provider Registry. All inspection and manual management must be done via the [Tyk Dashboard API](/5.14/tyk-dashboard-api).
    </Note>

3. **Remove IdP configuration from API definitions.**

    Once the Registry is verified as complete and correct, you can remove the JWKS URIs and scope-to-policy mappings from each API definition. The configuration in the API definition [takes precedence over the Registry](/5.14/api-management/client-idp-registry#what-is-the-identity-provider-registry)) so if an issuer and scope match is found in the API definition, this will be used rather than the configuration in the Registry.
