...
For detailed information about custom rate limiting concepts and configuration, see the [Custom Rate Limiting](/5.14/api-management/rate-limit#custom-rate-limiting) section in the main Rate Limiting documentation.

**Prerequisites**

This capability works with [Tyk 5.3.0](/5.14/developer-support/release-notes/dashboard#5-3-0-release-notes) or higher.
...
For general configuration of custom rate limit keys in policies, refer to the [Custom Rate Limiting](/5.14/api-management/rate-limit#custom-rate-limiting) documentation.
...
Additionally, it's possible to attach [custom attribute values](/5.14/portal/customization/user-model#add-custom-attributes-to-the-user-model) defined in a developer profile as metadata fields to credentials.
