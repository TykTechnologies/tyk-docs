...
Tyk Cloud supports the use of [Python](/5.14/api-management/plugins/rich-plugins#overview), [JavaScript Middleware](/5.14/api-management/plugins/javascript) and [Golang](/5.14/tyk-cloud/using-plugins) based plugins.
...
If you use a **Hybrid Data Plane** (a self-managed gateway that you deploy and connect to your Tyk Cloud Control Plane), use the Gateway's built-in [plugin bundle downloader](/5.14/api-management/plugins/overview#plugin-bundles) instead. You can either host your own bundle on your own web server or S3 bucket, or point the Hybrid Gateway directly at your Tyk Cloud Control Plane's File Server. See [Serving Plugin Bundles To Hybrid Gateways](/5.14/#serving-plugin-bundles-to-hybrid-gateways) for the File Server option.
</Note>
...
Next you'll [set up your plugin development environment](/5.14/#set-up-your-plugin-development-environment).
...
1. A local Tyk Gateway installation to create your Python plugin bundle. We recommend [installing our Self-Managed version on Ubuntu Bionic 18.04](/5.14/tyk-self-managed/install/linux#install-tyk-on-debian-or-ubuntu).
...
Next you'll [create a Python code bundle](/5.14/#create-a-python-code-bundle).
...
| File              | Description                                                                                                                                                                                                                                                                                       |
| :------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| custom_middleware | contains the middleware settings like the plugin driver you want to use (driver) and the hooks that your plugin will expose. We use the   **auth_check** for this tutorial. For other hooks see [here](/5.14/api-management/plugins/rich-plugins#coprocess-dispatcher-hooks). |
...
* You import decorators from the Tyk module that gives us the Hook decorator, and we import [Tyk Python API helpers](/5.14/api-management/plugins/rich-plugins#tyk-python-api-methods)
...
* A plugin bundle is a packaged version of the plugin, it may also contain a cryptographic signature of its contents. The -y flag tells the Tyk CLI tool to skip the signing process in order to simplify this tutorial. For more information on the Tyk CLI tool, see [here](/5.14/api-management/plugins/overview#how-plugin-bundles-work).
* You should now have a `bundle.zip` file in the plugin working directory.
* Next you will configure [uploading your plugin bundle file](/5.14/#uploading-your-bundle) to your Amazon S3 bucket.
...
Next you will [create an API](/5.14/#test-middleware) from our Control Plane and see our middleware in action.
...
Plugins must be enabled on your Tyk Cloud Control Plane, as described in [Setup Control Plane](/5.14/#setup-control-plane). Tyk Cloud only deploys the File Server, and exposes its API, once plugins are enabled on the Control Plane.
...
`bundle_base_url` must point at your Tyk Cloud Control Plane's File Server endpoint, with the `/mserv/api/mw/bundle` path and your File Server API key passed as the `api_key` query parameter. You can find both values on your Control Plane, as described in [Uploading your Bundle](/5.14/#uploading-your-bundle).
...
If your plugin bundle includes rich plugins (Python, Lua, or gRPC), also set `"enable_coprocess": true` in the Hybrid Gateway's `tyk.conf`, as described in [Gateway configuration](/5.14/api-management/plugins/overview#gateway-configuration).
