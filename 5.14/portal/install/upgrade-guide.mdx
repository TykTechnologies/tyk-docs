...
{/* 1. **Check the [Compatibility Matrix](/5.14/portal/overview/compatibility-matrix)** to confirm the new Portal version is compatible with your Tyk Dashboard version. */}

2. **Review the release notes** for every version between your current version and the target version. Pay particular attention to the "Breaking Changes" section. See [Portal Release Notes](/5.14/developer-support/release-notes/portal).
...
4. **Back up your theme files.** If you have a custom theme, export it from the Admin Portal or copy the theme directory before upgrading. See [Upgrade Your Theme](/5.14/#upgrade-your-theme).
...
3. **Apply the update.**
    ...
    After the upgrade completes, proceed to [Post-Upgrade Verification](/5.14/#post-upgrade-verification).
...
See [Upgrading Themes](/5.14/portal/customization/themes#upgrading-themes) for the full procedure.
...
For detailed troubleshooting steps for common upgrade failure scenarios, including Admin Portal inaccessible after upgrade, broken Product and Plan sync, and user login failures, see [Troubleshooting > Upgrade Failures](/5.14/portal/troubleshooting/upgrade-failures).
