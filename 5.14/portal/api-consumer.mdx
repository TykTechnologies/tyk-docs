...
Unlike [API Owners](/5.14/portal/api-owner) who administer the portal, API Consumers interact with your APIs through the Live Portal. They browse catalogs, request access to API Products, create applications, and use the credentials you provide to integrate with your services. The Developer Portal gives you powerful tools to organize these users, control their access, and facilitate collaboration among development teams.

This guide focuses on managing individual API Consumer accounts. For information on organizing users into groups, see the [Organisation and Teams](/5.14/tyk-stack/tyk-developer-portal/enterprise-developer-portal/managing-access/manage-api-consumer-organisations) page.
...
Please refer to the [email server configuration](/5.14/product-stack/tyk-enterprise-developer-portal/getting-started/setup-email-notifications) section for setup instructions.
<br />
...
8. Invitee receives an email with registration instructions
    ...
    <Note>
    All API Consumer users invited from the Live Portal will be [inactive](/5.14/portal/api-owner#activating-or-deactivating-user-accounts) until they complete the registration process. They will be assigned to the same Organisation as the Admin that invited them, as the Admin's scope is limited to their Organisation.
    </Note>
...
5. If the portal requires an admin to [approve registration requests](/5.14/portal/api-owner#user-account-status), they will see the following message.
...
6. If the portal is configured for [automatic approval](/5.14/portal/api-owner#automatically-approve-user-registrations), they will see a message that indicates that their account is active:
...
After completing authentication, as an API Consumer, you will be taken to the [Live Portal](/5.14/portal/overview/concepts#developer-portal-views).
...
- Admins will can edit the details of other users, including switching between Team Member and Admin role, and reassignment to other teams
  ...
  - Admins cannot change the [active](/5.14/portal/api-owner#user-account-status) status of other API Consumer users
...
- Personal: Only visible to the [owner](/5.14/portal/developer-app#app-owner) (typically the creator) of the app
...
- Regular Audits: Periodically review user accounts and [deactivate](/5.14/portal/api-owner#user-account-status) or [delete](/5.14/portal/api-owner#deleting-users) users as appropriate
