...
<Note>
When Markdown is enabled for a blog post, the template uses the `markdownify` function to render the content. See the [blog post template data](/5.14/portal/customization/templates#blog-detail) for details on `MarkdownEnabled` and `MarkdownContent`.
</Note>
...
See [Customize Pages](/5.14/portal/customization/pages) for how to create and edit pages and their content blocks.
...
This works well for API guides, authentication instructions, and quickstart tutorials. See [Getting Started Guide](/5.14/portal/api-products#getting-started-guide) for more details.
...
| Image as link | `[![Alt](/5.14/image-url)](link-url)` |
...
<Note>
The default theme does not render table borders. See [Table styling workarounds](/5.14/#table-styling) below for workarounds.
</Note>
