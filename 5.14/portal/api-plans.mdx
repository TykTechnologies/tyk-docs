...
API Plans, sometimes referred to as Subscription Plans, define the terms and conditions under which developers can access your [API Products](/5.14/portal/api-products). They establish the usage limits, approval requirements, and commercial tiers that govern API consumption. While API Products define what functionality is available, API Plans determine how much of that functionality can be used and under what conditions.
...
- When you [synchronize](/5.14/portal/api-provider#synchronizing-developer-portal-with-providers) the Developer Portal with the Provider, a new API Plan will be created automatically
...
<Warning>
The scope must [exist in your Identity Provider](/5.14/tyk-stack/tyk-developer-portal/enterprise-developer-portal/api-access/dynamic-client-registration#define-oauth-2-0-scopes) when the API Plan is published, so that the IdP is able to issue tokens containing the scope.
</Warning>
...
- **Unlimited Requests**: Select the checkbox to set no rate limit for this Plan; note that an [API-level rate limit](/5.14/api-management/rate-limit#rate-limiting-scopes-api-level-vs-key-level) would still be applied if configured.
...
- **Behavior**: This configuration is required when the Plan is used with an API Product for which the [Dynamic Client Registration](/5.14/tyk-stack/tyk-developer-portal/enterprise-developer-portal/api-access/dynamic-client-registration) flow has been activated
    ...
    - When a JWT presenting the Plan's scope is received by Tyk Gateway, the Gateway [applies the Plan to the session](/5.14/basic-config-and-security/security/authentication-authorization/json-web-tokens).
