...
In the Tyk Developer Portal, API Products serve as the primary unit of discovery, access control, and [monetization](/5.14/portal/overview/intro#api-monetization-with-tyk-developer-portal) of your API portfolio.

Since Tyk 5.7.0, you have been able to create API Products to publish your [Tyk Streams APIs](/5.14/api-management/event-driven-apis) on the Developer Portal.
...
- When you [synchronize](/5.14/portal/api-provider#synchronizing-developer-portal-with-providers) the Developer Portal with the Provider, a new API Product will be created automatically
...
1. Log in to the Developer Portal as an [API Owner](/5.14/portal/api-owner)
...
Refer to [JWT Signature Validation](/5.14/api-management/authentication/jwt-signature-validation) for API configuration details.
...
3. Select one or more [Client Profiles](/5.14/tyk-stack/tyk-developer-portal/enterprise-developer-portal/api-access/dynamic-client-registration#client-profiles) that describe the methods API Consumers will use to obtain access tokens from the IdP.
4. Select **Save Changes** and then publish the Product to a Catalog to make it available to API Consumers.

<Warning>
The scope must [exist in your Identity Provider](/5.14/tyk-stack/tyk-developer-portal/enterprise-developer-portal/api-access/dynamic-client-registration#define-oauth-2-0-scopes) when the API Product is published, so that the IdP is able to issue tokens containing the scope.
</Warning>
...
- **Note**: [Documentation-only Products](/5.14/portal/api-products#documentation-only-products) do not require any APIs to be selected
...
- **Note**: For a [Documentation-only Product](/5.14/portal/api-products#documentation-only-products) the *Specification Alias* will be used as the reference for each OpenAPI document, rather than the API name
...
- **Format**: [Markdown](/5.14/portal/customization/markdown) or HTML content with multiple pages
...
For full details, step-by-step creation instructions, and known limitations, see [Documentation-Only Products](/5.14/portal/api-products/documentation-only-products).
