...
See our [TCP Proxy Docs](/5.14/key-concepts/tcp-proxy) for more details.
...
See our [JWT Scope docs](/5.14/basic-config-and-security/security/authentication-authorization/json-web-tokens) for more details.
...
See our [Golang plugin documentation](/5.14/api-management/plugins/golang#) for more details.
...
See our [Distributed Tracing documentation](/5.14/api-management/traces) for more details.
...
See our updated [Getting Started](/5.14/tyk-self-managed/install) section and [new CLI documentation](/5.14/tyk-self-managed).
...
WSDL now is a first class citizen at Tyk. You can take your WSDL definition and simply import to the Dashboard, creating a nice boilerplate for your service. See [Import APIs](/5.14/api-management/gateway-config-managing-classic#import-an-api) for more details.
...
See [Debugging Tab](/5.14/api-management/endpoint-designer#debugging) for more information.
...
There is a [new section](/5.14/api-management/logs/audit-logs) in the Tyk Dashboard config file where you can specify parameters for the audit log (containing audit records for all requests made to all endpoints under the `/api` route).
...
> **TLDR**
> To get benefit or performance improvements ensure that you have `close_connections` set to `false` and set `max_idle_connections_per_host` according to our [production perfomance guide](/5.14/planning-for-production)
...
Instead of setting permissions per user, you can now [create a user group](/5.14/platform-management/user-groups), and assign it to multiple users. It works for Single Sign-On too, just specify group ID during [SSO API](https://tyk.io/docs/api-reference/single-sign-on/generate-authentication-token) flow.
...
#### Added SMTP support
Now you can configure the Dashboard to send transactional emails using your SMTP provider. See [Outbound Email Configuration](/5.14/configure/outbound-email-configuration) for details.

#### <a name="upgrade"></a>Upgrading all new Components

For details on upgrading all Tyk versions, see [Upgrading Tyk](/5.14/developer-support/upgrading).
...
See the Keys section of the [Tyk Gateway REST API](/5.14/tyk-gateway-api) Swagger doc for more details.
...
See [List Key Requests](/5.14/tyk-apis/tyk-dashboard-api/manage-key-requests#list-key-requests) for more details.
...
See [Developer Profiles](/5.14/tyk-developer-portal/tyk-portal-classic/developer-profiles#reset-developer-password) for more details.
...
[Docs](/5.14/api-management/logs-metrics#statsd-instrumentation)
...
[Docs](/5.14/api-management/gateway-config-tyk-classic#tyk-classic-api-versioning)
...
[Docs](/5.14/tyk-oss-gateway/configuration#http_server_options)
...
[Docs](/5.14/tyk-oss-gateway/configuration#enable_key_logging)
...
[Docs](/5.14/api-management/certificates)
...
[JSVM Docs](/5.14/api-management/plugins/javascript#using-javascript-with-tyk)
[Plugin Data Structure Docs](/5.14/api-management/plugins/rich-plugins#rich-plugins-data-structures)
...
* [Policy Docs](/5.14/tyk-apis/tyk-dashboard-api/portal-policies)
...
[Docs](/5.14/transform-traffic/url-rewriting)
...
[Docs](/5.14/tyk-oss-gateway/configuration)
...
A major feature of this release is the implementation of Mutual TLS. Now you can protect your APIs by allow listing certificates, idenitfy users based on them, and increase security between Tyk and upstream API. For details, see [Mutual TLS](/5.14/api-management/implement-tls#secure-hosted-apis-with-mtls).
...
[Docs](/5.14/api-management/access-control/policies/applying-policies#partitioned-policies)
...
[Docs](/5.14/api-management/dashboard-analytics#activity-logs)
...
* Enable Tyk Identity Broker to apply LDAP filters to user search [Docs](/5.14/api-management/single-sign-on-ldap)
* Set permissions for your users, logged via SSO, via `sso_permission_defaults` in Dashboard config file. [Docs](/5.14/tyk-identity-broker/overview)
* Setup a login page redirect, using `sso_custom_login_url` and `sso_custom_portal_login_url` Dashboard config options to enable users login using a custom SSO login page. [Docs](/5.14/tyk-identity-broker/overview)
* For those who love to build everything in-house, we have added new API for custom dashboard authentication integrations. [Docs](/5.14/api-management/custom-auth-with-proxy-identity-provider)
...
#### Mutual TLS support
[Docs](/5.14/api-management/implement-tls#secure-hosted-apis-with-mtls)

#### Global API rate limits
[Docs](/5.14/api-management/rate-limit#rate-limiting-layers)
...
[Docs](/5.14/tyk-oss-gateway/configuration#control_api_port)
...
[Docs](/5.14/tyk-oss-gateway/configuration#log_level)
...
[Docs](/5.14/api-management/traffic-transformation/request-body)
...
#### Attaching Multiple Policies to the Keys
[Docs](/5.14/api-management/access-control/policies/applying-policies#applying-multiple-policies)
...
```
[Docs](/5.14/tyk-identity-broker/overview)
...
[Docs](/5.14/api-management/custom-auth-with-proxy-identity-provider)
...
Added support for Mutual TLS, mentioned by Gateway and Dashboard above. See [Docs](/5.14/api-management/implement-tls#secure-hosted-apis-with-mtls)
...
Tyk Identity Broker now fully support LDAP search with complex filters! [Docs](/5.14/api-management/single-sign-on-ldap)
...
Hybrid users should follow the upgrade instructions [here](/5.14/developer-support/upgrading#tyk-upgrade-guides-for-different-deployment-models).
