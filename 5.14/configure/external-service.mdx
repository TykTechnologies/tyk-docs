...
keywords: "Configuration, External Services, Proxy, mTLS, OAuth, Security"
sidebarTitle: "External Service Config"
canonical: "https://tyk.io/docs/configure/external-service"
---
...
External Service Configuration gives you control over how Tyk Gateway securely connects to the [external services](/5.14/configure/external-service#external-service) that it needs to function correctly. You can configure proxy settings, mTLS authentication, and other connection parameters for Tyk's outbound connections to services such as OAuth providers, analytics systems, and webhook endpoints.
...
Refer to the [Tyk Gateway Configuration Reference](/5.14/tyk-oss-gateway/configuration#external_services) for more details on this setting.
...
| :-------------- | :------------- | :------------ |
| `oauth` | [OAuth/JWT token validation and introspection](/5.14/basic-config-and-security/security/authentication-authorization/json-web-tokens) | JWT auth interactions with Auth Server (Identity Provider) |
| `storage` | mTLS configuration for the rate limiter's Redis connection. Proxy settings are not supported for this service type. | Rate limiter Redis TLS |
| `webhooks` | [Webhook event notifications](/5.14/api-management/gateway-events#event-handling-with-webhooks) | Event handlers, notification delivery |
| `health` | [Health check requests](/5.14/planning-for-production/ensure-high-availability/health-check) | Host checker, uptime monitoring |  
| `discovery` | [Service discovery requests](/5.14/planning-for-production/ensure-high-availability/service-discovery) | Load balancer, service registry |

For each type of service, you can configure:

- specific proxies for HTTP, HTTPS and lists of hosts that should not be proxied (not applicable for `storage`)
...
| `cert_id` | String | No | Certificate ID from [Tyk Certificate Store](/5.14/api-management/certificates#tyk-certificate-store) |
| `ca_cert_ids` | Array | No | Array of CA certificate IDs from [Tyk Certificate Store](/5.14/api-management/certificates#tyk-certificate-store) |
...
From Tyk 5.13.0, the certificate paths (`cert_file`, `key_file`, `ca_file`) for service type `oauth`, used for integration with external Identity Providers (IdPs) for [client](/5.14/api-management/authentication/jwt-signature-validation#configuring-idps-in-the-api-definition) and [upstream](/5.14/api-management/upstream-authentication/oauth#upstream-oauth-2-0) authentication, can be set using [external references](/5.14/tyk-configuration-reference/kv-store#from-the-tyk-gateway-configuration-file). These references are resolved when the Gateway starts up and on each hot reload. This allows certificate rotation without a Gateway restart; however, updated certificates are applied only after a hot reload.

The certificate paths for other integrations do not support external references at this time.

<Note>
If [multiple Identity Providers](/5.14/api-management/authentication/jwt-signature-validation#configuring-idps-in-the-api-definition) are configured, they share the same External Service Configuration.
</Note>
...
Refer to the [Tyk Gateway Configuration Reference](/5.14/tyk-oss-gateway/configuration#external_services) for more details.
...
4. **Default settings** - If a setting is not specified, Tyk uses sensible defaults. You can find these in the [Tyk Gateway Configuration Reference](/5.14/tyk-oss-gateway/configuration#external_services).
...
**What this does:**
- Similar to previous example, but uses [Tyk Certificate Store](/5.14/api-management/certificates#tyk-certificate-store) instead of file paths
...
- **Global configuration**: Sets a baseline proxy (port 3128) for all external services with a general bypass list.
- **Storage service customization**: Adds mTLS security for the rate limiter's Redis connection, ensuring encrypted and authenticated communication. Note that proxy settings are not supported for the `storage` service type.
...
**What this does:**
- Similar to example 4, but uses [Tyk Certificate Store](/5.14/api-management/certificates#tyk-certificate-store) for all mTLS configurations
...
- **Comprehensive mTLS deployment**:
  - Every applicable service type (OAuth, storage, webhooks) has its own dedicated client certificate.
  - For `storage`, this applies mTLS to the rate limiter's Redis connection specifically.
...
> **Note:** These connection pooling settings are built into Tyk and are not configurable through the configuration file. They are optimized defaults based on the typical usage patterns of each service type. "Per Host" refers to the maximum number of idle connections maintained per upstream host. The `storage` service type uses a direct Redis TCP connection managed by go-redis and does not use HTTP connection pooling.
...
> **Note:** These timeout values are built into Tyk and are not configurable through the configuration file. They represent the maximum time Tyk will wait for a response from the external service before timing out the request. These values are optimized based on the expected response times and criticality of each service type. The `storage` service type connects to Redis directly via TCP; its timeout is controlled by the `timeout` field in the `storage` block of `tyk.conf`, not by this table.
