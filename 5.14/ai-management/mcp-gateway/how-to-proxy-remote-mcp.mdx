...
After completing the [quickstart](/5.14/ai-management/mcp-gateway/quickstart), you have a working MCP proxy, but it accepts connections from any client. This guide secures your remote MCP server so that only agents with a valid key can reach it.
...
- The Mock MCP Server running on `http://localhost:7878`. See the [quickstart](/5.14/ai-management/mcp-gateway/quickstart).
...
For more demanding scenarios, Tyk supports a range of [client authentication methods](/5.14/api-management/client-authentication), including JWT, mutual TLS, and OAuth 2.1. For MCP specifically, Tyk extends OAuth 2.1 with Protected Resource Metadata so MCP-aware clients can discover authentication requirements automatically. See [MCP Gateway: OAuth 2.1 authentication](/5.14/ai-management/mcp-gateway/oauth-2-1).
