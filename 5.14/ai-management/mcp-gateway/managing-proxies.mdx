...
For scripted or automated management, use the Dashboard API or Gateway API. See [MCP API extensions](/5.14/ai-management/mcp-gateway/mcp-api-extensions) for the full endpoint reference.

<Note>
MCP OAS definitions use the Tyk OAS format. They are not available as Tyk Classic API definitions. For the full definition structure, see [MCP OAS definition](/5.14/ai-management/mcp-gateway/mcp-proxy-definitions).
</Note>
...
**Authentication**: the authentication method applied to all inbound requests. Select a method from the **Authentication type** dropdown. See [Authentication](/5.14/api-management/client-authentication) for all supported methods and configuration options.
...
These options map to `x-tyk-api-gateway.middleware.global` in the proxy definition. For full configuration details, see [MCP middleware: proxy level](/5.14/ai-management/mcp-gateway/mcp-middleware#dashboard-settings-tab-proxy-level).
...
Each middleware option maps to the corresponding configuration block inside the primitive's entry in the proxy definition. See [MCP middleware](/5.14/ai-management/mcp-gateway/mcp-middleware) for what each option does and how it is configured.
...
**Configuring authentication**: The proxy is created with bearer token authentication enabled. To switch to a different method, open the Settings tab and select from the **Authentication type** dropdown. To use the external IdP integration — including scope check, PRM, and token exchange — select **OAuth 2.0**. See [Authentication](/5.14/api-management/client-authentication) for all supported methods.

**Enabling PRM for OAuth discovery**: Open the Settings tab, select **OAuth 2.0** as the authentication type, and enable the **Protected Resource Metadata** toggle. Set the resource URL and add at least one authorization server URL. See [OAuth 2.0 authentication](/5.14/api-management/authentication/oauth2-authentication) for full configuration details.

**Restricting which tools clients can call**: Use the Primitives tab: add the tool as a primitive, then add **Allow** middleware to it. Once any tool in the proxy has an Allow rule, all unlisted tools are blocked. For definition-based configuration, see [MCP middleware: access control](/5.14/ai-management/mcp-gateway/mcp-middleware#access-control).

**Applying rate limits to a specific tool**: Use the Primitives tab: open the tool primitive and add **Rate Limit** middleware. For definition-based configuration, see [MCP middleware: traffic management](/5.14/ai-management/mcp-gateway/mcp-middleware#traffic-management).

**Configuring upstream OAuth**: Add an `authentication.oauth.clientCredentials` block to `x-tyk-api-gateway.upstream` to have Tyk obtain and forward OAuth tokens to your upstream MCP server. See [MCP Gateway: OAuth 2.1 authentication](/5.14/ai-management/mcp-gateway/oauth-2-1#upstream-oauth).
...
For a complete explanation of every field in the definition, see [MCP OAS definition](/5.14/ai-management/mcp-gateway/mcp-proxy-definitions).
