...
OpenTelemetry must be enabled on your Tyk Gateway. See [OpenTelemetry configuration](/5.14/api-management/traces) for setup instructions.
...
|---|---|---|
| **Metrics** | Custom OTel metric instruments with MCP dimensions for counters and histograms | [MCP metrics](/5.14/ai-management/mcp-gateway/mcp-metrics) |
| **Access logs** | Structured per-request log records with MCP fields included when non-empty | [MCP access logs](/5.14/ai-management/mcp-gateway/mcp-access-logs) |
...
Alongside these OpenTelemetry signals, Tyk Dashboard has a dedicated **Activity by MCP** analytics page, covering proxy-level and primitive-level traffic and error charts. See [MCP Analytics](/5.14/ai-management/mcp-gateway/mcp-analytics).
