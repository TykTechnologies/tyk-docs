...
* [Redis](/5.14/planning-for-production/database-settings#redis)
* [MongoDB](/5.14/planning-for-production/database-settings#mongodb-sizing-guidelines)
* [PostgreSQL](/5.14/planning-for-production/database-settings#postgresql)

Please consult the [data storage configuration](/5.14/api-management/dashboard-configuration#data-storage-solutions) guide for further information relating to how to configure Tyk's data storage across different database engines.
...
- API client [Session](/5.14/api-management/access-control/sessions-and-keys/understanding-sessions) data (the source of truth for the deployment).
- [OAuth tokens issued by Tyk's OAuth 2.0 authorization server](/5.14/api-management/authentication/oauth-2#client-application).
- [Tyk Certificate Store](/5.14/api-management/certificates#tyk-certificate-store).
...
- [Traffic logs](/5.14/api-management/logs/traffic-logs) buffered before Tyk Pump processes them.
...
For configuration details, see [Configure Redis](/5.14/tyk-configuration-reference/redis-cluster-sentinel).
...
Tyk Gateways can generate a large volume of analytics data. Be sure to read about [capping your Dashboard analytics](/5.14/api-management/dashboard-analytics/analytics-storage-management).

#### Compressing API Definitions and Policies

In [distributed deployments](/5.14/api-management/mdcb), Data Plane Gateways store API definition and policy backups in Redis to ensure resilience when the Control Plane is unavailable. For deployments with many or large API definitions, this can consume significant Redis memory.
...
- [`storage.compress_api_definitions`](/5.14/tyk-oss-gateway/configuration#storage-compress_api_definitions) is available from Tyk 5.12.0.
- [`storage.compress_policies`](/5.14/tyk-oss-gateway/configuration#storage-compress_policies) is available from Tyk 5.13.0.
...
The `analytics_storage` block accepts the same fields as the main `storage` block. See [Configure Redis](/5.14/tyk-configuration-reference/redis-cluster-sentinel) for the full field reference.
...
The `cache_storage` block accepts the same fields as the main `storage` block. See [Configure Redis](/5.14/tyk-configuration-reference/redis-cluster-sentinel) for the full field reference.
...
For recommended deployment topology, see [Planning for Production](/5.14/planning-for-production#recommended-topology).

**Special notes for DocumentDB**

<Note>
If you are using [DocumentDB](https://aws.amazon.com/documentdb/), [capped collections](/5.14/api-management/dashboard-analytics/analytics-storage-management) are not supported. See [here](https://docs.aws.amazon.com/documentdb/latest/developerguide/mongo-apis.html) for more details.
</Note>
...
More information on these configuration variables [here](/5.14/tyk-dashboard/configuration). 
