...
<Note>
The availability of this feature [depends on your license](/5.14/api-management/user-management).
</Note>
...
The [user owned analytics](/5.14/platform-management/api-ownership#owned-analytics) feature allows you to prevent users from seeing the traffic to (and existence of) APIs for which they are not responsible. This reduces the opportunity for data leakage within your business.
...
To implement this structure, you would create three user groups and assign [permissions](/5.14/platform-management/user-permissions) as indicated:
...
Currently, only [API Usage](/5.14/api-management/dashboard-analytics#activity-by-api) and [Error Counts](/5.14/api-management/dashboard-analytics#activity-by-error) are available to users with the `owned_analytics` permission.
...
The [Tyk Dashboard API](/5.14/tyk-dashboard-api) provides endpoints to manage API ownership directly, if you are not using the API Designer.
