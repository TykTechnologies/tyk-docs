...
<Note>
  **Prerequisite:** Complete the [Getting Started](/5.14/getting-started/quick-start) guide to set up your Tyk Self-Managed environment before continuing.
</Note>
...
API security is an important aspect of API management. Tyk provides [multiple authentication methods](/5.14/api-management/client-authentication#what-does-tyk-support) to secure your APIs and control access. In this section, we'll explore the security features available in your trial environment.

### Exploring Authentication Methods

Tyk supports various authentication methods including [Auth Token](/5.14/api-management/authentication/bearer-token), [JWT](/5.14/basic-config-and-security/security/authentication-authorization/json-web-tokens), [OAuth 2.0](/5.14/api-management/authentication/oauth-2), and [more](/5.14/api-management/client-authentication#what-does-tyk-support). In your trial environment, the httpbingo API is pre-configured with Auth Token authentication.

[Auth Tokens](/5.14/api-management/authentication/bearer-token) are the simplest form of authentication. They're easy to implement and understand, making them perfect for your first exploration of Tyk.
...
Tyk API [Gateway](/5.14/tyk-oss-gateway) can control and transform incoming API traffic. It provides [various mechanisms](/5.14/api-management/traffic-transformation) to modify requests and responses, control traffic flow, and optimize performance. Let's explore these capabilities in your trial environment.
...
1. **Make a Request to the XML Endpoint:**
   - Using your API key from the [previous section](/5.14/getting-started/secure-api#exploring-authentication-methods), make a request to the XML endpoint (on the Tyk Gateway):
     ```
...
- [Creating and Managing APIs](/5.14/api-management/gateway-config-managing-oas#creating-an-api),
- [JWT Authentication](/5.14/basic-config-and-security/security/authentication-authorization/json-web-tokens),
- [Publishing APIs to Developer Portal](/5.14/portal/publish-api-catalog),
