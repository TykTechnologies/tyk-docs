...
[Okta](https://www.okta.com/) supports both [OpenID Connect (OIDC)](/5.14/#sso-with-openid-connect) and [SAML 2.0](/5.14/#sso-with-saml).
...
Before configuring your IdP and TIB profile, read [Dashboard SSO](/5.14/tyk-identity-broker/dashboard-sso) or [Portal SSO](/5.14/tyk-stack/tyk-developer-portal/enterprise-developer-portal/managing-access/enable-sso) to understand the `ActionType`, `ReturnURL`, and `IdentityHandlerConfig` fields required for your use case.
...
With this configuration, registered users (with a Tyk Dashboard user account) get their own permissions; unregistered users fall back to the group specified in `sso_default_group_id`. See [Dashboard SSO](/5.14/tyk-identity-broker/dashboard-sso) for full details.

**TIB profile**

The TIB profile is created via the [Tyk Identity Broker API](/5.14/tyk-identity-broker/tib-rest-api) or the [Tyk Dashboard UI](/5.14/tyk-identity-broker/dashboard-sso#create-a-tib-profile-using-dashboard-ui).
...
- set `DashboardCredential` to the [TIB service account's](/5.14/tyk-identity-broker/dashboard-sso#tib-service-account) Dashboard credentials
...
See [Dashboard SSO](/5.14/tyk-identity-broker/dashboard-sso) for details on session behavior, permissions, and user group mapping.
...
- set `Key` to the Okta **Client ID**
- set `Secret` to the Okta **Client Secret**
- set `DashboardCredential` to the [PORTAL_API_SECRET](/5.14/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_api_secret) used to authenticate with the Portal's management API
...
For details on user group mapping and admin vs developer profiles, see [Portal SSO](/5.14/tyk-stack/tyk-developer-portal/enterprise-developer-portal/managing-access/enable-sso).
