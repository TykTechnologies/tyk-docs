...
- **Configuration**: via `tib.conf` (pass the path with `-c`, defaults to `tib.conf` in the current directory) or [environment variables](/5.14/tyk-configuration-reference/tyk-identity-broker-configuration). See [Configuration](#configuration).
