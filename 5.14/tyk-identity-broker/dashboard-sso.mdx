...
| [Default User Group](#default-user-group) | Tyk Dashboard configuration | Assigns a fallback [Tyk Dashboard user group](/5.14/platform-management/user-groups) when no group mapping applies. |
...
In addition to the [default user group](#default-user-group) mechanism, setting [`sso_permission_defaults`](/5.14/tyk-dashboard/configuration#sso_permission_defaults) in the Tyk Dashboard config assigns SSO users that have not been assigned to a user group by TIB to a specific set of permissions, for example:
