...
2. Enable the Governance feature in Tyk Cloud Control Plane as described in [Enabling Governance Feature for Cloud Control Planes](/5.14/#enabling-governance-feature-for-cloud-control-planes)
...
1. Requesting Access to Tyk Governance
2. Generate agent tokens for each provider as described in [Installing a Local Agent](/5.14/#installing-a-local-agent)
3. Create configuration files for each agent
4. Deploy each agent using Docker or Kubernetes as described in [Installing a Local Agent](/5.14/#installing-a-local-agent)
