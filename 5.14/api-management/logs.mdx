...
- **[Application Log](/5.14/api-management/logs/application-logs):** Internal system events such as health-checks, configuration changes, and errors.
- **[API Traffic Log](/5.14/api-management/dashboard-analytics#where-the-data-comes-from):** <Badge color="orange">Gateway</Badge> A record of every API request, written into Redis and processed by Tyk Pump for analytics and reporting.
- **[Access Log](/5.14/api-management/logs/access-logs):** <Badge color="orange">Gateway</Badge> Per-request server logs intended for external log aggregators. Similar to API Traffic Logs in that both record individual requests, but Access Logs are lightweight and real-time whereas API Traffic Logs are richer and processed asynchronously.
- **[Audit Log](/5.14/api-management/logs/audit-logs):** <Badge color="blue">Dashboard</Badge> A record of user actions in Tyk Dashboard, such as API changes and login events.
...
| :--- | :--- | :--- | :--- |
| [Tyk Gateway](/5.14/tyk-oss-gateway/configuration#log_level) | `TYK_GW_LOGLEVEL` | `TYK_GW_LOGFORMAT` | `text`, `json`, `legacy` |
| [Tyk Pump](/5.14/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#log_level) | `TYK_PMP_LOGLEVEL` | `TYK_PMP_LOGFORMAT` | `text`, `json`, `legacy` |
| [Tyk Dashboard](/5.14/tyk-dashboard/configuration) | `TYK_DB_LOGLEVEL` (from `v5.14.0`) | `TYK_DB_LOGFORMAT` | `text`, `json` |
| [Tyk MDCB](/5.14/tyk-multi-data-centre/mdcb-configuration-options#log_level) | `TYK_MDCB_LOGLEVEL` | N/A | Legacy text only |
| [Tyk Developer Portal](/5.14/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_log_level) | `PORTAL_LOG_LEVEL` | `PORTAL_LOG_FORMAT` | `text`, `json` |

<Note>
The `legacy` format was introduced in Tyk Gateway 5.14.0 and Tyk Pump 5.14.0 to maintain backward compatibility for existing users with log pipelines reliant on the precise log content. See [legacy format](/5.14/api-management/logs#legacy-format).
</Note>
...
<Note>
Access Logs have `info` level severity. Setting Tyk Gateway's log level to `warn` or `error` therefore suppresses Access Log output. There are additional controls for the generation and content of access logs as described in [Access Logs](/5.14/api-management/logs/access-logs).
</Note>
...
Traffic Logs are generated by Tyk Gateway and written to Redis, not `stderr`, so none of the settings above apply to them. Enable them by setting [`enable_analytics`](/5.14/tyk-oss-gateway/configuration#enable_analytics) in the Gateway configuration; see [Dashboard Analytics](/5.14/api-management/dashboard-analytics) for how Tyk Pump then processes and routes them from Redis.

## Configuring Audit Logs

Audit Logs are generated by Tyk Dashboard and configured through its own `audit.*` settings block, unrelated to the Application Log settings above. See [Audit Logs](/5.14/api-management/logs/audit-logs) for the full configuration reference, or [Enable and View Audit Logs in Tyk Dashboard](/5.14/api-management/enable-audit-logs-dashboard) for a step-by-step guide.
