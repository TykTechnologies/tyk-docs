...
This page explains how to promote APIs, keys and policies from one environment to another. If you instead want to segment a single Tyk cluster into zones, so that specific Gateways selectively load specific APIs, see [Gateway and API Sharding](/5.14/api-management/api-sharding).
...
See [API Tagging](/5.14/api-management/api-sharding#api-tagging-with-on-premises) for more details.

#### API Sharding

You can also use [API Sharding](/5.14/api-management/api-sharding#what-is-api-sharding-) to move APIs in a Shards (and or MDCB) Tyk Self-Managed installation.
...
You can also use our new Tyk-Sync tool which allows you to sync your APIs (and Policies) with a Version Control System (VCS). You can then move your APIs between environments. See [Tyk-Sync](/5.14/api-management/automations/sync) for more details.
...
To create a key with a custom identifier, ie Token, simply use the [Gateway (OSS)](/5.14/tyk-gateway-api) or [Dashboard (Pro)](https://tyk.io/docs/api-reference/keys/create-custom-key) REST APIs to import a custom key.
...
You can also use our new Tyk-Sync tool which allows you to sync your Policies (and APIs) with a Version Control System (VCS). You can then move your Policies between environments. See [Tyk-Sync](/5.14/api-management/automations/sync) for more details.
