...
From Tyk 5.13.0, you can control the behavior of these headers using the [`rate_limit_response_headers`](/5.14/tyk-oss-gateway/configuration#rate_limit_response_headers) configuration in your `tyk.conf` (or the equivalent environment variable):
...
These guides include explanation of how to configure key-level rate limits when using [API Keys](/5.14/api-management/gateway-config-managing-classic#access-an-api) and [Security Policies](/5.14/api-management/gateway-config-managing-classic#secure-an-api).
...
1. [Distributed Rate Limiter](/5.14/#distributed-rate-limiter): recommended for most use cases, implements the [token bucket algorithm](https://en.wikipedia.org/wiki/Token_bucket)
2. [Redis Rate Limiter](/5.14/#redis-rate-limiter): implements the [sliding window log algorithm](https://developer.redis.com/develop/dotnet/aspnetcore/rate-limiting/sliding-window)
3. [Fixed Window Rate Limiter](/5.14/#fixed-window-rate-limiter): implements the [fixed window algorithm](https://redis.io/learn/develop/dotnet/aspnetcore/rate-limiting/fixed-window)
...
It can be configured to switch dynamically between two algorithms depending on the request rate, as explained [here](/5.14/#dynamic-algorithm-selection-based-on-request-rate).
</Note>
...
- if [configured to return rate limits](/5.14/api-management/rate-limit#rate-limit-response-headers), the `X-RateLimit-Remaining` response header will only reflect the current Gateway's share of the limit, rather than the true global remaining limit across the cluster
...
This algorithm implements a sliding window log algorithm and can be enabled via the [enable_redis_rolling_limiter](/5.14/tyk-oss-gateway/configuration#enable_redis_rolling_limiter) configuration option.
...
If you wish to avoid spike arrest behavior but the DRL is not suitable, you might use the [Fixed Window Rate Limiter](/5.14/#fixed-window-rate-limiter) algorithm.

You can configure [Rate Limit Smoothing](/5.14/#rate-limit-smoothing) to manage the traffic spike, allowing time to increase upstream capacity if required.

The [Redis Sentinel Rate Limiter](/5.14/#redis-sentinel-rate-limiter) reduces latency for clients, however increases resource usage on Redis and Tyk Gateway.
...
[enable_sentinel_rate_limiter](/5.14/tyk-oss-gateway/configuration#enable_sentinel_rate_limiter).
...
Performance can be improved by enabling the [enable_non_transactional_rate_limiter](/5.14/tyk-oss-gateway/configuration#enable_non_transactional_rate_limiter). This leverages Redis Pipelining to enhance the performance of the Redis operations. Please consult the [Redis documentation](https://redis.io/docs/manual/pipelining/) for more information.

Please consider the [Fixed Window Rate Limiter](/5.14/#fixed-window-rate-limiter) algorithm as an alternative, if Redis performance is an issue.
...
This algorithm can be enabled using the following configuration option [enable_fixed_window_rate_limiter](/5.14/tyk-oss-gateway/configuration#enable_fixed_window_rate_limiter).

If you need spike arrest behavior, the [Redis Rate Limiter](/5.14/#redis-rate-limiter) should be used.
...
See [DRL Threshold](/5.14/tyk-oss-gateway/configuration#drl_threshold) for details on how to configure this feature.
...
To enable this, Tyk introduced support for custom rate limit keys in [Tyk 5.3.0](/5.14/developer-support/release-notes/dashboard#5-3-0-release-notes). This feature allows you to define custom patterns for rate limiting that go beyond the default credential-based approach.
...
As mentioned above, we can easily configure custom rate limit keys for simple scenarios that do not require awareness of the request context. When more complex logic or integration with external services is required to determine the rate-limiting key, for example when you want to rate limit per requester IP address, a [custom authentication plugin](/5.14/api-management/plugins/plugin-types#authentication-plugins) can be used to identify and generate the rate limiter key.

We use an authentication plugin because [it lets us modify the session object](/5.14/api-management/plugins/plugin-types#hook-capabilities). 
...
1. **Build and Deploy plugin**: Build the plugin and deploy it to your Tyk Gateway. Refer to the [Go Plugin Development Guide](/5.14/api-management/plugins/golang#setting-up-your-environment) for instructions on building and deploying Go plugins.

2. **Configure your API**: Create an authenticated API and set up your API to use the [custom authentication plugin](/5.14/api-management/plugins/golang#loading-custom-go-plugins-into-tyk). Note that you will need to select the [multiple authentication](/5.14/basic-config-and-security/security/authentication-authorization/multiple-auth) method to invoke both the plugin and your chosen auth method.
...
As explained in the [Rate Limiting Concepts](/5.14/api-management/rate-limit#introduction) section, Tyk supports configuration of rate limits at both the API-Level and Key-Level for different use cases.
...
The following assumptions are made:
 - The [ignore authentication](/5.14/api-management/traffic-transformation/ignore-authentication) middleware should not be enabled for the relevant endpoints.
 - If [path-based permissions](/5.14/api-management/gateway-config-managing-classic#path-based-permissions) are configured, they must grant access to these endpoints for keys generated from the policies.
</Note>
...
Not yet, though IP-based rate limiting is possible using custom pre-processor middleware JavaScript that generates tokens based on IP addresses. See our [Middleware Scripting Guide](/5.14/api-management/plugins/javascript#using-javascript-with-tyk) for more details.
...
The per-endpoint rate limit middleware allows you to enforce rate limits on specific endpoints. This middleware is configured in the [Tyk OAS API Definition](/5.14/api-management/gateway-config-tyk-oas#operation), either via the Tyk Dashboard API or in the API Designer.

If you’re using the legacy Tyk Classic APIs, then check out the [Tyk Classic](/5.14/api-management/rate-limit#tyk-classic-api-definition) page.
