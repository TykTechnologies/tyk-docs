...
Note that this is not the same as Tyk's [IP block list](/5.14/api-management/gateway-config-tyk-classic#ip-access-control) feature, which is used to restrict access to APIs based upon the IP of the requestor.
...
You can also set case sensitivity for the entire [gateway](/5.14/tyk-oss-gateway/configuration#ignore_endpoint_case) in the Gateway configuration file `tyk.conf`. If case insensitivity is configured at the gateway level, this will override the endpoint-level setting.

#### Endpoint parsing

When using the block list middleware, we recommend that you familiarize yourself with [Request Matching](/5.14/getting-started/key-concepts/url-matching) options.

<br />

<Note>
Tyk recommends that you use [exact matching](/5.14/getting-started/key-concepts/url-matching#matching-modes) for maximum security, though prefix and wildcard strategies might also apply for your particular deployment or use case.
</Note>
...
The [block list](/5.14/api-management/traffic-transformation/block-list) is a feature designed to block access to specific API endpoints. Tyk Gateway rejects all requests made to endpoints with the block list enabled, returning `HTTP 403 Forbidden`. 
...
If you're using the legacy Tyk Classic APIs, then check out the [Tyk Classic](/5.14/#block-list-using-classic) page.
...
Note also that the endpoint path has not been terminated with `$`. Requests to, for example, `GET /anything/foobar` will be rejected as the [regular expression pattern match](/5.14/#endpoint-parsing) will recognize this as `GET /anything`.
...
If you're using the newer Tyk OAS APIs, then check out the [Tyk OAS](/5.14/#block-list-using-tyk-oas) page.

If you're using Tyk Operator then check out the [configuring the block list in Tyk Operator](/5.14/#tyk-operator) section below.
...
- `method_actions`: a shared object used to configure the [mock response](/5.14/api-management/traffic-transformation/mock-response#tyk-classic) middleware for Tyk Classic APIs
...
Note also that the endpoint path has not been terminated with `$`. Requests to, for example, `GET /status/200/foobar` will be rejected as the [regular expression pattern match](/5.14/#endpoint-parsing) will recognize this as `GET /status/200`.

Consult section [configuring the Allow List in Tyk Operator](/5.14/#tyk-operator) for details on how to configure allow lists for endpoints using Tyk Operator.
...
Note also that the endpoint path has not been terminated with `$`. Requests to, for example, `GET /get/foobar` will be rejected as the [regular expression pattern match](/5.14/#endpoint-parsing) will recognize this as `GET /get`.
