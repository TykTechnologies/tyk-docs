...
To get a tour of the Dashboard UI, refer to this [document](/5.14/getting-started/using-tyk-dashboard).
...
The Dashboard exposes two APIs:
 - **Dashboard API**: Is used for operational management of Tyk resources (APIs, policies, keys, etc.). This API offers granular permissions based on user roles.
    ...
    To know more about the Dashboard API, refer to the [Tyk Dashboard API documentation](/5.14/tyk-dashboard-api).
 ...
 - **Dashboard Admin API**: Is used for system-level administration and initial setup tasks like managing organizations, initial user creation, backups/migrations and SSO setup.
    ...
    To know more about the Dashboard Admin API, refer to the [Tyk Dashboard Admin API documentation](/5.14/dashboard-admin-api).

### Authenticating with Dashboard APIs

**Dashboard API**

The [Tyk Dashboard API](/5.14/tyk-dashboard-api) is secured using an `Authorization` header that must be added to each request that is made. The **Tyk Dashboard API Access Credentials** `Authorization` key can be found within the Dashboard UI at the bottom of the **Edit User** section for a user.
...
Tyk Dashboard requires a persistent datastore for its operations. For supported database engines and versions, see [Installation Requirements](/5.14/tyk-self-managed/install#requirements).
...
2. **Aggregate Analytics**: Data used to display Dashboard charts and [analytics](/5.14/api-management/dashboard-analytics#traffic-analytics).
3. **Logs**: When [detailed logging](/5.14/api-management/troubleshooting-debugging#capturing-detailed-logs) is enabled, request and response data is logged to storage. These logs can previewed in the Dashboard [log browser](/5.14/api-management/dashboard-analytics#activity-logs).
...
Please refer to the [storage configuration](/5.14/tyk-dashboard/configuration#storage) section to explore the parameters for configuring Tyk Dashboard to read from different storage layers.

### Configure Pump to Write to Data Storage Layers

Tyk Pump writes traffic logs, aggregated analytics and uptime test results to these same storage layers. See [Control Plane Pumps](/5.14/api-management/dashboard-analytics/control-plane-pumps) for configuring Tyk Pump to write logs and analytics, or [Uptime Tests](/5.14/planning-for-production/ensure-high-availability/uptime-tests#monitoring-uptime-tests-in-tyk-dashboard) for the uptime storage layer specifically.
