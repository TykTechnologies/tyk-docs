...
- [Detailed logging](/5.14/api-management/logs/traffic-logs#detailed-recording)
- API Settings including
   - Listen path
   - [API Categories](/5.14/platform-management/api-categories)
- Upstream settings including
   - Upstream service (target) URL
   - [Service Discovery](/5.14/planning-for-production/ensure-high-availability/service-discovery)
- [API Ownership](/5.14/platform-management/api-ownership)
- [API level rate limiting](/5.14/api-management/rate-limit#configuring-the-rate-limiter-at-the-api-level)
- [Authentication](/5.14/api-management/client-authentication)
...
The **Versions** tab allows you to create and manage [API versioning](/5.14/api-management/gateway-config-tyk-classic#tyk-classic-api-versioning) for the API.
...
The **Endpoint Designer** is where you can define endpoints for your API so that you can enable and configure Tyk middleware to [perform checks and transformations](/5.14/api-management/traffic-transformation) on the API traffic.

In some cases, you will want to set global settings that affect all paths that are managed by Tyk. The **Global Version Settings** section will enable you to configure API-level [request](/5.14/api-management/traffic-transformation/request-headers#tyk-classic-api) and [response](/5.14/api-management/traffic-transformation/request-headers#tyk-classic-api) header transformation.
...
- [API-level caching](/5.14/api-management/response-caching#configuring-the-cache-via-the-dashboard) including a button to invalidate (flush) the cache for the API
- [CORS](/5.14/api-management/gateway-config-tyk-classic#cross-origin-resource-sharing-cors)
- Add custom attributes to the API definition as *config data* that can be accessed by middleware
- Enable [context variables](/5.14/api-management/traffic-transformation/request-context-variables) so that they are extracted from requests and made available to middleware
- Manage *segment tags* if you are working with [sharded gateways](/5.14/api-management/api-sharding#gateway-sharding)
- Manage client IP address [allow](/5.14/api-management/gateway-config-tyk-classic#ip-access-control) and [block](/5.14/api-management/gateway-config-tyk-classic#ip-access-control) lists
- Attach [webhooks](/5.14/api-management/gateway-events#event-handling-with-webhooks) that will be triggered for different events
...
In the **Uptime Tests** tab you can configure Tyk's [Uptime Test](/5.14/api-management/gateway-config-tyk-classic#uptime-tests) functionality
