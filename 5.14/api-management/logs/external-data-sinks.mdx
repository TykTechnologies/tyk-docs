...
<Note>
This page describes forwarding traffic logs to external tools using Tyk Pump. [OpenTelemetry tracing](/5.14/api-management/traces) is an alternative for new integrations: it exports per-request span data directly from Tyk Gateway, without the Redis and Tyk Pump hop. It's not a like-for-like replacement, though: OTel spans use a different schema from Tyk's traffic log fields, and not every destination on this page has an OTel exporter. Use this page if you need Tyk's traffic log format, a destination without OTel support, or already have an existing deployment built around Tyk Pump.
</Note>

Tyk Pump can forward traffic logs to a range of external tools, each its own pump type, in parallel with, or instead of, the [pumps that populate Tyk Dashboard Analytics](/5.14/api-management/dashboard-analytics). Every pump on this page is declared the same way, under the `pumps` object in `pump.conf`, and every pump also accepts the [common pump settings](/5.14/api-management/tyk-pump#common-pump-settings) (`filters`, `timeout`, and so on) in addition to its own fields shown below.

Every pump on this page forwards each traffic log largely as-is, mapped into that destination's native format.

<Note>
None of the pumps on this page ever receive aggregated analytics, whichever way you deploy them. Aggregation only ever feeds Tyk Dashboard's Traffic Analytics graphs; see [How Aggregation Works](/5.14/api-management/dashboard-analytics#how-aggregation-works). External sinks only ever see individual traffic logs.
</Note>

This page covers every External Data Sink pump. StatsD, DogStatsD, and Prometheus are metrics pumps, not sink pumps, so they're covered on [Metrics Pumps](/5.14/api-management/metrics/metrics-pumps) instead; see the [pump type catalog](/5.14/api-management/tyk-pump#pump-type-catalog) for the full breakdown.
...
- **Run the sink pump directly on the data plane**, alongside the [Hybrid Pump](/5.14/api-management/dashboard-analytics/data-plane-pump), reading from the same local Redis. This is the simpler option.
- **Run the sink pump on the control plane**, as a [Control Plane Pump](/5.14/api-management/dashboard-analytics/control-plane-pumps), receiving traffic logs the Hybrid Pump has forwarded via Tyk MDCB. This requires `forward_analytics_to_pump: true`. See [Data Plane Pump](/5.14/api-management/dashboard-analytics/data-plane-pump#how-tyk-mdcb-handles-the-data) for that mechanism in full.

## CSV

Tyk Pump can create or append to a CSV file to track API traffic logs.

In addition to the [common pump settings](/5.14/api-management/tyk-pump#common-pump-settings), the `csv` pump accepts the following `meta` field:
...
In addition to the [common pump settings](/5.14/api-management/tyk-pump#common-pump-settings), the `stdout` pump accepts the following `meta` fields:
...
In addition to the [common pump settings](/5.14/api-management/tyk-pump#common-pump-settings), the `syslog` pump accepts the following `meta` fields:
...
In addition to the [common pump settings](/5.14/api-management/tyk-pump#common-pump-settings), the `elasticsearch` pump accepts the following `meta` fields:
...
In addition to the [common pump settings](/5.14/api-management/tyk-pump#common-pump-settings), the `graylog` pump accepts the following `meta` fields:
...
Tyk Pump can write traffic logs to [InfluxDB](https://www.influxdata.com/) 1.x. For InfluxDB 2.x, use [Influx2](/5.14/#influx2) below instead.

In addition to the [common pump settings](/5.14/api-management/tyk-pump#common-pump-settings), the `influx` pump accepts the following `meta` fields:
...
For InfluxDB 1.x, use [InfluxDB](/5.14/#influxdb) above instead. Uses the official Go client library for InfluxDB 2.x.

In addition to the [common pump settings](/5.14/api-management/tyk-pump#common-pump-settings), the `influx2` pump accepts the following `meta` fields:
...
In addition to the [common pump settings](/5.14/api-management/tyk-pump#common-pump-settings), the `kafka` pump accepts the following `meta` fields:
...
In addition to the [common pump settings](/5.14/api-management/tyk-pump#common-pump-settings), the `logzio` pump accepts the following `meta` fields:
...
In addition to the [common pump settings](/5.14/api-management/tyk-pump#common-pump-settings), the `moesif` pump accepts the following `meta` fields:
...
- Moesif needs [detailed recording](/5.14/api-management/logs/traffic-logs#detailed-recording) enabled to log HTTP headers and bodies.
...
In addition to the [common pump settings](/5.14/api-management/tyk-pump#common-pump-settings), the `resurfaceio` pump accepts the following `meta` fields:
...
- Requires [detailed recording](/5.14/api-management/logs/traffic-logs#detailed-recording) enabled to capture full API call details.

### Segment

Tyk Pump can send traffic logs to [Segment](https://segment.com/) as `Track` events.

In addition to the [common pump settings](/5.14/api-management/tyk-pump#common-pump-settings), the `segment` pump accepts the following `meta` field:
...
In addition to the [common pump settings](/5.14/api-management/tyk-pump#common-pump-settings), the `splunk` pump accepts the following `meta` fields:
...
In addition to the [common pump settings](/5.14/api-management/tyk-pump#common-pump-settings), the `kinesis` pump accepts the following `meta` fields:
...
In addition to the [common pump settings](/5.14/api-management/tyk-pump#common-pump-settings), the `sqs` pump accepts the following `meta` fields:
...
In addition to the [common pump settings](/5.14/api-management/tyk-pump#common-pump-settings), the `timestream` pump accepts the following `meta` fields:
