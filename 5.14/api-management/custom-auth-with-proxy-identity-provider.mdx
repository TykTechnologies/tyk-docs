...
Before configuring your TIB profile, read [Dashboard SSO](/5.14/tyk-identity-broker/dashboard-sso) or [Portal SSO](/5.14/tyk-stack/tyk-developer-portal/enterprise-developer-portal/managing-access/enable-sso) to understand the `ActionType`, `ReturnURL`, and `IdentityHandlerConfig` fields required for your use case.
...
- set `DashboardCredential` to the [`PortalAPISecret`](/5.14/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_api_secret) used to authenticate with the Portal's management API
