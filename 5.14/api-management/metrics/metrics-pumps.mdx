...
<Note>
For new deployments, we recommend [OpenTelemetry Metrics](/5.14/api-management/logs-metrics#opentelemetry-metrics) instead: Tyk Gateway pushes Rate, Errors, and Duration (RED) metrics via an OpenTelemetry Collector. This page remains relevant for existing deployments already using Tyk Pump for metrics.
</Note>

## Introduction

[Tyk Pump](/5.14/api-management/tyk-pump) can independently expose metrics, derived from the traffic logs it reads out of Redis, the same traffic logs used for [Dashboard Analytics](/5.14/api-management/dashboard-analytics). These metrics are computed by Tyk Pump after the fact from traffic log records, so it can only produce traffic-derived counters and histograms (status codes, latency, and whatever traffic log fields you tag). It has no visibility into Gateway's own runtime health or configuration state, unlike [OpenTelemetry metrics](/5.14/api-management/metrics/default-metrics).

Three pump types expose metrics this way, all deriving from the same traffic log fields:

| Pump Type | Format |
| :-- | :-- |
| [Prometheus](/5.14/#prometheus) | Exposes an HTTP endpoint for Prometheus to scrape |
| [StatsD](/5.14/#statsd) | Sends per-request metrics to a StatsD server |
| [DogStatsD](/5.14/#dogstatsd-datadog) | Sends per-request metrics in the DogStatsD format, for example to Datadog |

<Note>
Not to be confused with [Tyk Gateway's native StatsD instrumentation](/5.14/api-management/logs-metrics#statsd-instrumentation): that's a completely different, built-in mechanism exporting internal system metrics (request rate, goroutine health, and so on), not traffic-log-derived request metrics. The [StatsD pump](/5.14/#statsd) on this page is unrelated to that feature.
</Note>
...
You can add further custom metrics of your own; see [Custom Metrics](/5.14/#custom-metrics) below.

### Configuring the Pump

In addition to the [common pump settings](/5.14/api-management/tyk-pump#common-pump-settings), the `prometheus` pump accepts the following `meta` fields. Add them to `pump.conf`:
...
| `custom_metrics` | (none) | Additional counters and histograms, on top of the [base metrics](/5.14/#base-metrics) above. See [Custom Metrics](/5.14/#custom-metrics) below. |
...
| `mcp_only` | `false` | Restrict this metric to [MCP Gateway](/5.14/ai-management/mcp-gateway/overview) traffic. When `false`, the metric processes all traffic, and the `mcp_*` labels resolve to an empty string for non-MCP requests. |
...
In addition to the [common pump settings](/5.14/api-management/tyk-pump#common-pump-settings), the `statsd` pump accepts the following `meta` fields:
...
- A [Tyk Self-Managed](/5.14/tyk-self-managed/install) or [Tyk Open Source](/5.14/apim/open-source/installation) installation with [Tyk Pump](/5.14/api-management/tyk-pump) configured.
...
In addition to the [common pump settings](/5.14/api-management/tyk-pump#common-pump-settings), the `dogstatsd` pump accepts the following `meta` fields:
