...
canonical: "https://tyk.io/docs/api-management/client-idp-registry"
---
...
| :-------- | :------ | :------ |
| Tyk Gateway | Available since [v5.14.0](/5.14/developer-support/release-notes/gateway#5-14-0-release-notes) | Enterprise |
| Tyk Dashboard | Available since [v5.14.0](/5.14/developer-support/release-notes/dashboard#5-14-0-release-notes) | Enterprise |
| Tyk Sync | Available since [v2.2.0](/5.14/developer-support/release-notes/sync#2-2-0-release-notes) | Enterprise |
...
- Deleting an [API Product using Dynamic Client Registration (DCR)](/5.14/tyk-stack/tyk-developer-portal/enterprise-developer-portal/api-access/dynamic-client-registration) from the Developer Portal does not reliably clean up the IdP config it added, leaving orphaned JWKS URLs and scope mappings.
...
<Note>
The Identity Provider Registry manages IdPs used to authenticate *clients accessing your APIs* (inbound JWT validation). It is not related to the configuration of upstream services that the Gateway authenticates to, which is covered in [Upstream Authentication](/5.14/api-management/upstream-authentication).
</Note>
...
- The Tyk Developer Portal manages API Product access via [Dynamic Client Registration](/5.14/tyk-stack/tyk-developer-portal/enterprise-developer-portal/api-access/dynamic-client-registration), where multiple systems write IdP config for the same API.
...
The IdP registry is managed exclusively via the Tyk Dashboard API or the [Tyk Developer Portal](/5.14/tyk-stack/tyk-developer-portal/enterprise-developer-portal/api-access/dynamic-client-registration). There is no Dashboard UI for the registry at this time.
...
Full endpoint reference and request/response schemas are available in the [Tyk Dashboard API reference](/5.14/tyk-dashboard-api).
