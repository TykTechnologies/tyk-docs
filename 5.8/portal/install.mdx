...
- **3rd party identity provider.** To [enable oAuth2.0 for your API Products](/5.8/tyk-stack/tyk-developer-portal/enterprise-developer-portal/api-access/dynamic-client-registration), you'll need to utilize an OpenID-compliant third-party identity provider.
It's essential to note that the [Tyk Stack](/5.8/tyk-stack) doesn't include third-party identity providers, so you should refer to your Identity Provider's documentation for instructions on configuring and deploying it.
This component is optional and required only for enabling oAuth2.0
- **[Tyk Identity Broker](/5.8/api-management/external-service-integration)**. You only need this component if you want to configure Single Sign-On for the Tyk Enterprise Developer Portal.
For more guidance on this topic, please consult [the Single Sign-On section](/5.8/tyk-stack/tyk-developer-portal/enterprise-developer-portal/managing-access/enable-sso) of the documentation
...
For step-by-step instructions, please refer to [the Email Settings section](/5.8/portal/customization/email-notifications)
...
Tyk Developer Portal no longer supports SQLite as of the release of version 1.14.0. To ensure a smooth experience, please migrate to [PostgreSQL](/5.8/planning-for-production/database-settings#postgresql) or another supported alternative.
</Note>
...
For further guidance on launching the portal, please refer to one of the installation options: [Docker container](/5.8/portal/install#docker), [Docker Compose](/5.8/portal/install#docker-compose), [Helm chart](/5.8/portal/install#using-legacy-helm-chart), or [RPM package](/5.8/portal/install#linux-redhat-centos).
2. **[Bootstrap the portal](/5.8/#bootstrapping-enterprise-developer-portal)** After you've launched the portal, it will wait for you to provide credentials for the super admin user before it starts accepting traffic.
Once you've created the super admin user, the portal will complete its installation process by creating the necessary database structure and initialising the required assets for its operations. You can [bootstrap](/5.8/#bootstrapping-enterprise-developer-portal) the portal either through the UI or using the bootstrap API.
Please refer to [the Bootstrapping section](/5.8/portal/install#bootstrapping-enterprise-developer-portal) for implementing this step.
...
<Card href="/5.8/portal/install#docker" img="/img/docker.png">
**Read time: 10 mins**

Install with Docker
</Card>

<Card href="/5.8/portal/install#docker-compose" img="/img/docker.png">
**Read time: 10 mins**

Install with Docker Compose
</Card>

<Card href="/5.8/portal/install#using-new-helm-chart" img="/img/k8s.png">
**Read time: 10 mins**

Install on Kubernetes
</Card>

<Card href="/5.8/portal/install#linux-redhat-centos" img="/img/redhat-logo2.png">
**Read time: 10 mins**
...
4. **Create an environment variables file**
    ...
    Here is an example of a sample environment file. For a comprehensive reference of environment variables, please refer to the [configuration section](/5.8/product-stack/tyk-enterprise-developer-portal/deploy/configuration) in the Tyk Enterprise Developer Portal documentation.
    ```ini
...
6. **Bootstrap the portal**
    ...
    Follow the [bootstrapping section](/5.8/#bootstrapping-enterprise-developer-portal) of the documentation to bootstrap the portal via the UI or the admin API.
...
3. **Create an environment variables file**
    ...
    Here is an example of a sample environment file. For a comprehensive reference of environment variables, please refer to the [configuration](/5.8/product-stack/tyk-enterprise-developer-portal/deploy/configuration) section in the Tyk Enterprise Developer Portal documentation.
    ```ini
...
4. **Pull and launch the portal container**
    ...
    You can browse all available versions on [Docker Hub](https://hub.docker.com/r/tykio/portal/tags) and in the [release notes](/5.8/developer-support/release-notes/portal#170-release-notes) section.
    ```console
    docker run -d \
        -p 3001:3001 \
        --env-file .env \
...
5. **Bootstrap the portal**
    ...
    Now the portal is running on port 3001, but it needs to be bootstrapped by providing credentials for the super admin user since it's the first time you are launching it. Follow the [bootstrapping](/5.8/#bootstrapping-enterprise-developer-portal) section of the documentation to bootstrap the portal via the UI or the admin API.
...
5. **Bootstrap the portal**
    ...
    Now the portal is running on port 3001, but it needs to be bootstrapped by providing credentials for the super admin user since it's the first time you are launching it. Follow the [bootstrapping section](/5.8/#bootstrapping-enterprise-developer-portal) of the documentation to bootstrap the portal via the UI or the admin API. 
...
1. **Create an environment variables file for configuring the portal and the database**
    ...
    Here is an example of a sample environment file. For a comprehensive reference of environment variables, please refer the [configuration section](/5.8/product-stack/tyk-enterprise-developer-portal/deploy/configuration) in the Tyk Enterprise Developer Portal documentation.
    ```ini
    MYSQL_ROOT_PASSWORD=sup3rsecr3t
    MYSQL_DATABASE=portal
    MYSQL_USER=admin
...
2. **Create a docker-compose file**
    ...
    Ensure that you replace `<tag>` with the specific version of the portal you intend to launch before executing the command, e.g. `tykio/portal:v1.7` for the portal v1.7.
    You can browse all available versions on [Docker Hub](https://hub.docker.com/r/tykio/portal/tags) and in the [release notes section](/5.8/developer-support/release-notes/portal#170-release-notes).
...
4. **Bootstrap the portal**
    ...
    Now the portal is running on port 3001, but it needs to be bootstrapped by providing credentials for the super admin user since it's the first you are launching it. Follow the [bootstrapping section](/5.8/#bootstrapping-enterprise-developer-portal) of the documentation to bootstrap the portal via the UI or the admin API.
...
There are two ways to install Tyk Enterprise Developer Portal. You can enable `global.components.devPortal` during Tyk Self-Managed deployment by following the [Tyk Self-Managed installation instruction](/5.8/product-stack/tyk-charts/tyk-stack-chart) using our `tyk-stack` umbrella chart. It will install Tyk Enterprise Developer Portal together with Tyk Gateway and Dashboard in the same namespace.
...
###### Configuration
Please refer to this [guide](/5.8/product-stack/tyk-charts/tyk-stack-chart) for an explanation of all configuration options.
...
It is recommended to use new helm charts instead of legacy charts. Guide for new charts can be found [here](/5.8/portal/install#using-new-helm-chart)
</Warning>
...
3. **Launch the portal using the helm chart**
    ...
    <Note>
    In case this is the first time you are launching the portal, it will be necessary to bootstrap it before you can use it. For detailed instructions, please refer to the [bootstrapping documentation](/5.8/#bootstrapping-enterprise-developer-portal).
    </Note>
...
3. **Update the configuration file with your license**
    ...
    You can, however, customize the provided example and make more suitable for your need using the [configuration](/5.8/product-stack/tyk-enterprise-developer-portal/deploy/configuration) reference.
    ```json
    {
    "HostPort": 3001,
...
5. **Bootstrap the portal**
    ...
    Now the portal is running on port 3001, but it needs to be bootstrapped by providing credentials for the super admin user since it's the first you are launching it. Follow the [bootstrapping](/5.8/#bootstrapping-enterprise-developer-portal) section of the documentation to bootstrap the portal via the UI or the admin API.
...
You can continue configuring and customizing it either via the UI or the portal admin API. Please refer to [the Tyk Enterprise Developer Portal Concepts section](/5.8/portal/overview/concepts) for further guidance.

## Environment Variable Reference

For global configurations of the Developer Portal deployment refer this [config file](/5.8/product-stack/tyk-enterprise-developer-portal/deploy/configuration).

## API Documentation

The Dashboard exposes two APIs:

1. [Enterprise Developer Portal API](/5.8/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api)
2. [Enterprise Developer Portal Admin API](/5.8/product-stack/tyk-enterprise-developer-portal/api-documentation/list-of-endpoints/portal-1.13.0-list-of-endpoints)