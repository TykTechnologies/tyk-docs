...
The Tyk Cloud trial does not include access to [Hybrid deployments](/5.8/tyk-cloud/environments-deployments/hybrid-gateways) or the [Developer Portal](/5.8/portal/overview/intro).
...
2. Creating your first [API](/5.8/api-management/gateway-config-introduction) using the [Tyk Dashboard](/5.8/api-management/dashboard-configuration).
...
* Adding [Plugins and Middleware](/5.8/tyk-cloud/using-plugins) to your Control Plane
...
Where an Organization has expired due to either a subscription failure or cancelation and is now within a "retirement" period of 30 days, during which an [Billing Admin](/5.8/tyk-cloud/teams-users#assign-user-roles) can reinstate full functionality by updating or creating a new subscription.
