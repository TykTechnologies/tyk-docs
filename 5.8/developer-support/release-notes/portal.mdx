...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.8/#Changelog-v1.14.1) below.
...
To upgrade the Portal's theme, please follow the [upgrade instructions](/5.8/portal/customization/themes#upgrading-themes).
...
We’ve added support for [Tyk Streams APIs](/5.8/api-management/event-driven-apis) in the Developer Portal, allowing API Providers to expose access to event-driven APIs alongside traditional REST APIs.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.8/#Changelog-v1.14.0) below.

<h4 className="breaking-changes-v1.14.0">Breaking Changes</h4>
There are no breaking changes in this release.

For the complete API specification, see [Tyk EDP API documentation](/5.8/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api).
...
To upgrade the portal's theme, please follow the [upgrade instructions](/5.8/portal/customization/themes#upgrading-themes) for the portal's themes.
...
This patch release contains a bug fix. For a comprehensive list of changes, please refer to the detailed [changelog](/5.8/#Changelog-v1.13.2) below.
...
Fixed an issue where middleware configuration for an API could be corrupted (for example, [Request header transform middleware](/5.8/api-management/traffic-transformation#request-header-transform) settings were changed in Developer Portal. This was due to a misalignment between Dashboard and Developer Portal in their interpretation of the API definition and has now been corrected.
</Expandable>
</li>
</ul>
...
#### Release Highlights
This release is focused mainly on bug fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.8/#Changelog-v1.13.1) below.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.8/#Changelog-v1.13.0) below.
...
1. Portal will automatically migrate the new Custom IDs to most of the existing resources. For more information, please refer to the [changelog](/5.8/#Changelog-v1.13.0).
2. Users are advised to take appropriate actions in their system regarding the `Plans` API object changes as described in the [breaking changes section](/5.8/#breaking-changes-v1.13.0).
...
- Delete plans
    ...
    <Note>
    The Plans API object has schema changes; please refer to the [breaking changes section](/5.8/#breaking-changes-v1.13.0) for details.
    </Note>
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.8/#Changelog-v1.12.0) below.
...
You can read more about the supported SSO providers [here](/5.8/tyk-identity-broker).
</Accordion>
...
- [22 bugs fixed](/5.8/#fixed)
- [19 CVEs fixed](/5.8/#fixed)
...
- Added new TLS variables to set MinVersion ([portal_tls_min_version](/5.8/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_tls_min_version), MaxVersion ([PORTAL_TLSCONFIG_MAXVERSION](/5.8/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_tls_max_version), and CipherSuites ([PORTAL_TLS_CIPHER_SUITES](/5.8/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_tls_cipher_suites).
- Added a new configuration to manage the idle timeout of the portal's session ([PORTAL_SESSION_IDLE_TIMEOUT](/5.8/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_session_idle_timeout).
...
- Changed the default value of [PORTAL_DATABASE_CONNECTION_MAX_LIFETIME](/5.8/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_database_connection_max_lifetime) to 1800000 milliseconds.
...
- [Webhooks](/5.8/portal/customization) for events that happen in the portal.
- [Admin APIs](/5.8/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) for OAuth2.0 configuration. 
...
#### Changelog
##### Added
- Added [the webhooks](/5.8/portal/customization) capability that  enable real-time, automated data updates between the portal and 3rd party applications.
- Added [admin APIs](/5.8/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) for managing OAuth2.0 configuration. 
...
If you are on 1.8.3 or older version please follow the [upgrade instructions](/5.8/portal/customization/themes#upgrading-themes) to upgrade the portal's themes.
...
#### Changelog
##### Added
- Added [new configuration option](/5.8/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) for setting the `SameSite` attribute on the portal's cookie.
- Added [new welcome email for admin users](/5.8/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) that is sent when new admin account is created.
- Added [new welcome email for developers](/5.8/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) that is sent when new developer account is created.
...
- Changed the credential provisioning flow to automatically include DeveloperID, OrganizationID, ApplicationID, and TeamIDs in [the credential metadata](/5.8/portal/customization/user-model).
- Added warning regarding potential PII exposure to the [custom attributes menu](/5.8/portal/customization/user-model).
...
- Extended [filesize limit](/5.8/portal/customization/themes#create-a-theme) for individual files in themes to 5 MB.
- Made the organization invite email's subject configurable via [the emails settings section](/5.8/portal/customization/email-notifications#supported-email-notifications).
...
#### Changelog
##### Added
- Added [new admin API](/5.8/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) for managing tags attached to API Products.
- Added [new admin API](/5.8/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) for managing OAuth2.0 client types attached to API Products.
...
##### Added
- Add a [config option](/5.8/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_enable_http_profiler) to expose the Golang profiling information which allows for debugging issues related to resource consumption.
...
- When the portal theme [manifest](/5.8/portal/customization/themes#manifest-file) has a reference to a template that is not present in the theme then the theme won't be loaded. This check that prevents admin users from uploading themes with potential errors was introduced in version [1.7.0](/5.8/#content-blocks-validation).
...
All settings are available via the [admin API](/5.8/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) and the UI.
...
To set the CORS configuration please refer to the Portal's [configuration documentation](/5.8/product-stack/tyk-enterprise-developer-portal/deploy/configuration#cors-settings).
...
Now when the [PORTAL_DCR_LOG_ENABLED](/5.8/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_dcr_log_enabled) environment variable is set to `true`, the portal will output not only the status and status code of the request to the IdP, but also actual payload returned by the IdP: 
```yaml
...
- When [PORTAL_DCR_LOG_ENABLED](/5.8/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_dcr_log_enabled) is enabled the portal now returns not only the status and status code of the request to the IdP but also actual payload returned by the IdP
...
- Fixed the bug where the [Forgot Password page](/5.8/tyk-developer-portal/tyk-enterprise-developer-portal/api-consumer-portal/reset-password) did not reflect the current theme.
...
The Tyk Enterprise Developer Portal is now available in Kubernetes and customer can launch it using our [helm charts](/5.8/portal/install#using-legacy-helm-chart). This feature makes the portal Kubernetes friendly by adding liveness, readiness probes, graceful shutdown and changing the portal lifecycle so that it's possible to set an initial user and bootstrap the portal via APIs.

##### SSO for API Consumers and admins
API Providers can [configure Single Sign-on](/5.8/tyk-stack/tyk-developer-portal/enterprise-developer-portal/managing-access/enable-sso) for the Enterprise developer portal so that it's possible to login developers and admins to the portal user 3rd party IdP.
...
##### Add TLS support
This feature enables API Provides to secure the portal with [HTTPs](/5.8/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_tls_enable).

##### Add enhanced logging configuration
This new setting allows API Providers to set the logging [level](/5.8/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_log_level) and [format](/5.8/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_log_format). This offers API Providers more control over the logging behavior of their APIs.
...
##### Added
- Added Kubernetes support and [helm charts](/5.8/portal/install#using-legacy-helm-chart).
- Added [Single Sign-on](/5.8/tyk-stack/tyk-developer-portal/enterprise-developer-portal/managing-access/enable-sso) for API Consumers and admin users so that they can use their IdPs for managing admin users and developers.
...
- Added [TLS](/5.8/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_tls_enable) support for the portal's UI.
...
Now API Consumers can [create organizations](/5.8/tyk-stack/tyk-developer-portal/enterprise-developer-portal/managing-access/manage-api-consumer-organisations) and securely share credentials between their teammates. In greater detail:
...
##### Get started guides
API Providers can add [Get started guides](/5.8/tyk-stack/tyk-developer-portal/enterprise-developer-portal/getting-started-with-enterprise-portal/manage-get-started-guides-for-api-products) to API Products for better developer experiences:
...
##### Added
- Added the [organization management capability](/5.8/tyk-stack/tyk-developer-portal/enterprise-developer-portal/managing-access/manage-api-consumer-organisations) for API Consumers to safely share API access credentials between team members.
- Added the [Get started guides](/5.8/tyk-stack/tyk-developer-portal/enterprise-developer-portal/getting-started-with-enterprise-portal/manage-get-started-guides-for-api-products) for API Products so that admins can explain to their consumers how use their API Products.
- Added support for [S3 storage](/5.8/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_storage) for the portal's assets storage. Now our customers can use `s3` storage in addition to the filesystem which is especially important in Kubernetes environments.
- Added [tags](/5.8/tyk-stack/tyk-developer-portal/enterprise-developer-portal/getting-started-with-enterprise-portal/manage-get-started-guides-for-api-products) for API Products and blog posts so that API Providers have greater control over which blog posts to display on their API Product page.
...
### Upgrading Tyk
Please refer to the [upgrading Tyk](/5.8/developer-support/upgrading) page for further guidance with respect to the upgrade strategy.
