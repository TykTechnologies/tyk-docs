...
Tyk does not have an API-level circuit breaker that can be applied across all endpoints. If you are using the Tyk Dashboard, however, then you are able to use an [Open Policy Agent](/5.8/api-management/dashboard-configuration#extend-permissions-using-open-policy-agent-opa) to append a circuit breaker to every API/Service using the regex `.*` path.

<hr />

If you're using Tyk OAS APIs, then you can find details and examples of how to configure the circuit breaker middleware [here](/5.8/#configuring-the-circuit-breaker-in-the-tyk-oas-api-definition).

If you're using Tyk Classic APIs, then you can find details and examples of how to configure the circuit breaker middleware [here](/5.8/#configuring-the-circuit-breaker-in-the-tyk-classic-api-definition).
...
When working with Tyk OAS APIs the circuit breaker is configured in the [Tyk OAS API Definition](/5.8/api-management/gateway-config-tyk-oas#operation). You can do this via the Tyk Dashboard API or in the API Designer.

If you're using the legacy Tyk Classic APIs, then check out the [Tyk Classic](/5.8/#configuring-the-circuit-breaker-in-the-tyk-classic-api-definition) page.
...
- `halfOpenStateEnabled`: if set to `true` then the circuit breaker will operate in [half-open mode](/5.8/#half-open-mode) once it has been tripped
...
It will configure the circuit breaker so that if a minimum of 10 requests (`sampleSize`) to this endpoint are received during the [rolling sampling window](/5.8/#how-the-circuit-breaker-works) then it will calculate the ratio of failed requests (those returning `HTTP 500` or above) within that window.
...
- the minimum number of requests that must be received during the [rolling sampling window](/5.8/#how-the-circuit-breaker-works) before the circuit breaker can trip
- the cooldown period for which the breaker will remain _open_ after being tripped before returning to service (in seconds)
- optionally enable [half-open mode](/5.8/#half-open-mode) for upstream services with variable recovery times
...
If you're using the newer Tyk OAS APIs, then check out the [Tyk OAS](/5.8/#configuring-the-circuit-breaker-in-the-tyk-oas-api-definition) page.

If you're using Tyk Operator then check out the [configuring the Circuit Breaker in Tyk Operator](/5.8/#configuring-the-circuit-breaker-in-tyk-operator) section below.
...
- `disable_half_open_state`: by default the Tyk circuit breaker will operate in [half-open mode](/5.8/#half-open-mode) when working with Tyk Classic APIs, set this to `true` if you want Tyk to wait the full cooldown period before closing the circuit
...
The Dashboard supports the separate `BreakerTripped` and `BreakerReset` events, but not the combined `BreakerTriggered` [event type](/5.8/api-management/gateway-events#event-types). You should use **API Designer > Advanced Options** to add a Webhook plugin to your endpoint for each event.
