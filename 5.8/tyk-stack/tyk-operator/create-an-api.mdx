...
sidebarTitle: "Create an API"
canonical: "https://tyk.io/docs/5.8/tyk-stack/tyk-operator/create-an-api"
---
...
First, specify the details of your API using the [ApiDefinition CRD](/5.8/api-management/automations/operator#apidefinition-crd), then deploy it to create the corresponding Kubernetes resource. Tyk Operator will take control of the CRD and create the actual API in the Tyk data plane.
...
- [HTTP Proxy example](/5.8/tyk-stack/tyk-operator/create-an-api#set-up-manifest-for-http)
...
You can visit the [ApiDefinition CRD](/5.8/api-management/automations/operator#apidefinition-crd) page to see all the latest API Definitions fields and features we support.
...
<Note>
Tyk Operator supported authentication types are listed in the [API Definition features](/5.8/api-management/automations/operator#apidefinition-crd) section.
</Note>
...
Below, we take you through how to define the security policy but you can also find [Security Policy Example](/5.8/tyk-stack/tyk-operator/create-an-api#security-policy-example) below.
...
<h3 className="security-policy-example">Security policy example</h3>
<h4 className="key-level-per-api-rate-limits-and-quotas">Key-level per-API rate limits and quota</h4>
...
<h4 className="per-endpoint-rate-limit">Key-level per-endpoint rate limits</h4>
...
<h4 className="path-based-permissions">Path based permissions</h4>
...
<h4 className="partitioned-policies">Partitioned policies</h4>
