...
<Card href="/5.8/tyk-self-managed/install#install-with-docker" img="/img/docker.png">
**Read time: 10 mins**

Install with Docker
</Card>

<Card href="/5.8/tyk-self-managed/install#install-on-kubernetes" img="/img/k8s.png">
**Read time: 10 mins**

Install on K8s
</Card>

<Card href="/5.8/tyk-self-managed/install#install-with-ansible" img="/img/ansible.png">
**Read time: 10 mins**

Install with Ansible
</Card>

<Card href="/5.8/tyk-self-managed/install#install-tyk-on-redhat-rhel-centos" img="/img/redhat-logo2.png">
**Read time: 10 mins**

Install on Red Hat
</Card>

<Card href="/5.8/tyk-self-managed/install#install-tyk-on-debian-or-ubuntu" img="/img/debian-nd-753.png">
**Read time: 10 mins**

Install on Ubuntu
</Card>

<Card href="/5.8/tyk-self-managed/install#install-on-aws-marketplace" img="/img/aws.png">
**Read time: 10 mins**

Install on Amazon AWS
</Card>

<Card href="/5.8/tyk-self-managed/install#install-on-heroku" img="/img/heroku-logo.png">
**Read time: 10 mins**

Install Tyk on Heroku
</Card>

<Card href="/5.8/tyk-self-managed/install#install-on-microsoft-azure" img="/img/azure-2.png">
**Read time: 10 mins**
...
- [Quick Start with PostgreSQL](/5.8/#install-tyk-stack-with-helm-chart-postgresql)
- [Quick Start with MongoDB](/5.8/tyk-self-managed/install#install-tyk-stack-with-helm-chart-mongodb)

Or go to [Tyk Stack helm chart](/5.8/product-stack/tyk-charts/tyk-stack-chart) for detailed installation instructions and configuration options.
...
<Note>
Ensure that you are installing PostgreSQL versions that are supported by Tyk. Please consult the list of [supported versions](/5.8/api-management/dashboard-configuration#supported-database) that are compatible with Tyk.
</Note>
...
You are now ready to [create an API](/5.8/api-management/gateway-config-managing-classic#create-an-api).

For the complete installation guide and configuration options, please see [Tyk Stack Helm Chart](/5.8/product-stack/tyk-charts/tyk-stack-chart).
...
* [Helm 3+](https://helm.sh/docs/intro/install/)
    ...
    <Note>
    If you want to enable Tyk Enterprise Developer Portal, please use [PostgreSQL](/5.8/#install-tyk-stack-with-helm-chart-postgresql). MongoDB is not supported in Developer Portal.
    </Note>
...
<Note>
Please make sure you are installing Redis versions that are supported by Tyk. Please refer to Tyk docs to get list of [supported versions](/5.8/planning-for-production/database-settings#redis).
</Note>
...
<Note>
Please make sure you are installing MongoDB versions that are supported by Tyk. Please refer to Tyk docs to get list of [supported versions](/5.8/api-management/dashboard-configuration#supported-database).
</Note>
...
<Note>
Ensure that you are installing MongoDB versions that are supported by Tyk. Please consult the list of [supported versions](/5.8/api-management/dashboard-configuration#supported-database) that are compatible with Tyk.
</Note>
...
<Warning>
This deployment is NOT designed for production use or performance testing. The Tyk Pro Docker Demo is our full, [Self-Managed](/5.8/tyk-self-managed/install) solution, which includes our Gateway, Dashboard and analytics processing pipeline. 

This demo will run Tyk Self-Managed on your machine, which contains 5 containers: Tyk Gateway, Tyk Dashboard, Tyk Pump, Redis and either MongoDB or one of our supported [SQL databases](/5.8/api-management/dashboard-configuration#supported-database).
...
Now you have your prerequisites, follow the instructions from our [Tyk Helm Chart](/5.8/#use-legacy-helm-chart) page.

### Use Legacy Helm Chart


<Warning>
`tyk-pro` chart is deprecated. Please use our [Tyk Stack helm chart](/5.8/product-stack/tyk-charts/tyk-stack-chart) instead. 

We recommend all users migrate to the `tyk-stack` Chart. Please review the [Configuration](/5.8/product-stack/tyk-charts/tyk-stack-chart) section of the new helm chart and cross-check with your existing configurations while planning for migration.
</Warning>
...
2. **Data stores**
    ...
    You can find supported MongoDB and SQL versions [here](/5.8/planning-for-production/database-settings).
...
<Note>
Please make sure you are installing PostgreSQL versions that are supported by Tyk. Please refer to Tyk docs to get list of [supported versions](/5.8/api-management/dashboard-configuration#supported-database).
</Note>
...
Check [Tyk Gateway Sharding](/5.8/api-management/multiple-environments#what-is-api-sharding-) for more details.
...
If you are deploying the **Tyk Enterprise Developer Portal**, set the appropriate values under the `enterprisePortal` section in your `values.yaml`. Please visit [Tyk Enterprise Developer Portal installation](/5.8/portal/install#using-legacy-helm-chart) for a step by step guide.
...
Check [Tyk Control plane](/5.8/tyk-multi-data-centre) for more configuration details.
...
See [TIB](/5.8/api-management/external-service-integration#installing-tyk-identity-broker-tib) for more details.
...
To get started go to [Tyk Operator](/5.8/api-management/automations/operator).
...
5. Run ansible-playbook to install the following:
    ...
    <Note>
    For a production environment, we recommend that the Gateway, Dashboard and Pump are installed on separate machines. If installing multiple Gateways, you should install each on a separate machine. See [Planning for Production](/5.8/planning-for-production) For more details.
    </Note>
...
You can override the config values by environment variables. See [how to configure an environment variable](/5.8/tyk-oss-gateway/configuration). 
...
We also have a [Docker Tyk Pro Demo](/5.8/deployment-and-operations/tyk-self-managed/tyk-demos-and-pocs/overview#docker-compose-setup), which installs our full Self-Managed solution, which includes our Gateway, Dashboard, and analytics processing pipeline. This demo will run Tyk Self-Managed on your machine.
...
The [Dashboard configuration](/5.8/tyk-dashboard/configuration) can be changed by either editing the `tyk_analytics.conf` file or injecting them as [environment variables](/5.8/tyk-oss-gateway/configuration) via `heroku config`. In this guide we'll use the latter for simplicity of demonstration but there is merit to both methods.
...
All these files serve the same purpose as with the Dasboard and the Pump. [Configuration](/5.8/tyk-oss-gateway/configuration) can either be edited in `tyk.conf` or [injected](/5.8/tyk-oss-gateway/configuration) with `heroku config`.
...
You're ready to follow the guide on [creating and managing your APIs](/5.8/api-management/gateway-config-managing-classic#create-an-api) with this Heroku deployment.


<Note>
To use the [geographic log distribution](/5.8/api-management/dashboard-configuration#activity-by-location) feature in the Dashboard please supply the GeoLite2 DB in the `gateway` directory, uncomment the marked line in `Dockerfile.web` and set the `analytics_config.enable_geo_ip` setting (or `TYK_GW_ANALYTICSCONFIG_ENABLEGEOIP` env var) to `true`.
</Note>
...
In order to enable [rich plugins](/5.8/api-management/plugins/rich-plugins#) for the Gateway, please set the following Heroku config option to either `python` or `lua` depending on the type of plugins used:
```bash
...
1. Via our [Ubuntu Setup](/5.8/tyk-self-managed/install#debian-ubuntu-install-gateway) on an installed Ubuntu Server on Azure.
2. Via our [Docker Installation](/5.8/deployment-and-operations/tyk-self-managed/tyk-demos-and-pocs/overview#docker-compose-setup) using Azure's Docker support.
...
1. Via our [Ubuntu Setup](/5.8/tyk-self-managed/install#debian-ubuntu-install-gateway) on an installed Ubuntu Server within Google Cloud.
2. Via our [Docker Installation](/5.8/deployment-and-operations/tyk-self-managed/tyk-demos-and-pocs/overview#docker-compose-setup) using Google Cloud's Docker support.
...
Tyk Gateway has a [dependency](/5.8/planning-for-production/database-settings#redis) on Redis. Follow the steps provided by Red Hat to make the installation of Redis, conducting a [search](https://access.redhat.com/search/?q=redis) for the correct version and distribution.

**Storage Database**

Tyk Dashboard has a dependency on a storage database that can be [PostgreSQL](/5.8/planning-for-production/database-settings#postgresql) or [MongoDB](/5.8/planning-for-production/database-settings#mongodb-sizing-guidelines).
  

**Option 1: Install PostgreSQL**

Check the PostgreSQL supported [versions](/5.8/planning-for-production/database-settings#postgresql). Follow the steps provided by [PostgreSQL](https://www.postgresql.org/download/linux/redhat/) to install it.
...
<br />
Check the MongoDB supported [versions](/5.8/planning-for-production/database-settings#mongodb-sizing-guidelines). Follow the steps provided by [MongoDB](https://www.mongodb.com/docs/manual/tutorial/install-mongodb-on-red-hat/) to install it.
...
- [Dashboard](/5.8/#install-dashboard)
- [Pump](/5.8/#install-pump)
- [Gateway](/5.8/#install-gateway)



    <Note>
    For a production environment, we recommend that the Tyk Gateway, Tyk Dashboard and Tyk Pump are installed on separate machines. If installing multiple Tyk Gateways, you should install each on a separate machine. See [Planning for Production](/5.8/planning-for-production) for more details.
    </Note>
...
*   Follow the steps provided in this link [Getting started on Red Hat (RHEL / CentOS)](/5.8/#install-tyk-on-redhat-rhel-centos) to install and configure Tyk dependencies.
...
10. **Login to the Dashboard**
    ...
    **Configure your Developer Portal**
    ...
    To set up your [Developer Portal](/5.8/tyk-developer-portal) follow our Self-Managed [tutorial on publishing an API to the Portal Catalog](/5.8/getting-started/tutorials/publish-api).
...
See [SQL configuration](/5.8/planning-for-production/database-settings#postgresql) for details on installing SQL in a production environment.
</Tab>
</Tabs>
...
- [Dashboard](/5.8/tyk-self-managed/install#Debian-Ubuntu-install-dashboard)
- [Pump](/5.8/tyk-self-managed/install#Debian-Ubuntu-install-pump)
- [Gateway](/5.8/tyk-self-managed/install#debian-ubuntu-install-gateway)
...
**Prerequisites**
- Have MongoDB/SQL and Redis installed - follow the guide for [installing databases on Debian/Ubuntu](/5.8/#install-tyk-on-debian-or-ubuntu).
...
Follow the [Gateway installation instructions](/5.8/#using-shell-7) to connect to your Dashboard instance before you continue on to step 4.
