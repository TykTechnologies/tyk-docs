...
Here you can see the various fields as they are applied to Tyk keys, these are all described in the Keys section of the [Gateway API](/5.8/tyk-gateway-api).
...
[With the Dashboard API](/5.8/api-management/gateway-config-managing-classic#create-a-security-policy-with-the-api)

[With the Gateway API - Open Source tab](/5.8/api-management/gateway-config-managing-classic#secure-an-api)

[With the Dashboard](/5.8/api-management/gateway-config-managing-classic#create-a-security-policy-with-the-dashboard)
...
See the [Gateway Configuration Options](/5.8/tyk-oss-gateway/configuration) for more details.
...
See the Keys section of [Tyk Gateway API Swagger page](/5.8/tyk-gateway-api) for more details.

### Access Key Level Security

Tyk supports the concept of access control at the key level. Access control is managed via three important settings in a session object. In order to be fully clear on how Tyk handles access control, it's worth looking at the key settings that go into a user session object. A full description of each of the options can be found in the [Tyk Gateway API documentation](/5.8/tyk-gateway-api).
...
Creating new keys is done by POSTing an object such as the above to the Tyk create key API endpoint. See the keys section of the [Tyk Gateway API OpenAPI/Swagger](/5.8/tyk-gateway-api) page.
...
A session object is just a JSON object. For more details of each parameter in the session object, see [Tyk Token Session Object Details](/5.8/api-management/policies#session-object).
...
* `org_id`: The organization this user belongs to, this can be used in conjunction with the `org_id` setting in the API Definition object to have tokens "owned" by organizations. See the Organizations Quotas section of the [Tyk Gateway API](/5.8/tyk-gateway-api).
...
As described in [What is a Session Object?](/5.8/api-management/policies#what-is-a-session-object), all Tyk tokens can contain a metadata field. This field is a string key/value map that can store any kind of information about the underlying identity of a session.
...
- values can be used in dynamic [JavaScript](/5.8/api-management/plugins/javascript#accessing-external-and-dynamic-data) middleware and Virtual Endpoints for further validation or request modification
...
- [URL Rewrite](/5.8/transform-traffic/url-rewriting#pattern)
- [Request Header Transformation](/5.8/api-management/traffic-transformation/request-headers#injecting-dynamic-data-into-headers)
- [Response Header Transformation](/5.8/api-management/traffic-transformation/request-headers#injecting-dynamic-data-into-headers)
- [Request Body Transformation](/5.8/api-management/traffic-transformation/request-body#data-accessible-to-the-middleware)
- [Response Body Transformation](/5.8/api-management/traffic-transformation/request-body#data-accessible-to-the-middleware)
- [Virtual Endpoints](/5.8/api-management/traffic-transformation/virtual-endpoints)

You can also access and update metadata from your [custom plugins](/5.8/api-management/plugins/overview#).  For an example of this, take a look at this [gRPC enabled GO Server](https://github.com/TykTechnologies/tyk-grpc-go-basicauth-jwt).  It's a PoC middleware that injects a JWT value into metadata and then accesses it later in the stream.
...
Note that where we talk about keys here, we are referring to [Session Objects](/5.8/api-management/policies#what-is-a-session-object), also sometimes referred to as Session Tokens

### Key expiry

Tyk's API keys ([token session objects](/5.8/api-management/policies#session-object)) have an `expires` field. This is a UNIX timestamp and, when this date/time is reached, the key will automatically expire; any subsequent API request made using the key will be rejected.
...
<Note>
There is a risk, when configuring API-level lifetime, that a key will be deleted before it has expired, as `session_lifetime` is applied regardless of whether the key is active or expired. To protect against this, you can configure the [session_lifetime_respects_key_expiration](/5.8/tyk-oss-gateway/configuration#session_lifetime_respects_key_expiration) parameter in your `tyk.conf`, so that keys that have exceeded their lifetime will not be deleted from Redis until they have expired.
</Note>


This feature works nicely with [JWT](/5.8/basic-config-and-security/security/authentication-authorization/json-web-tokens) or [OIDC](/5.8/api-management/client-authentication#integrate-with-openid-connect-deprecated) authentication methods, as the keys are created in Redis the first time they are in use so you know when they will be removed. Be extra careful in the case of keys created by Tyk (Auth token or JWT with individual secrets) and set a long `session_lifetime`, otherwise the user might try to use the key **after** it has already been removed from Redis.

#### Gateway-level key lifetime control

You can set a global lifetime for all keys created in the Redis by setting [global_session_lifetime](/5.8/tyk-oss-gateway/configuration#global_session_lifetime) in the `tyk.conf` file; this parameter is an integer value in seconds.

To enable this global lifetime, you must also set the [force_global_session_lifetime](/5.8/tyk-oss-gateway/configuration#force_global_session_lifetime) parameter in the `tyk.conf` file.
