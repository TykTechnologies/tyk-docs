...
Tyk's [Request Header Transform](/5.8/api-management/traffic-transformation/request-headers) middleware can be configured to add this header to the request prior to it being proxied to the upstream. To enhance security by restricting visibility of the access token, the key/token can be stored in a [key-value store](/5.8/tyk-self-managed/install), with only the reference included in the middleware configuration.
