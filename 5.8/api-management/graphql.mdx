...
With Tyk OSS, it is possible to create GQL APIs using Tyk's Gateway API or to generate a file with the same object and store it in the `/apps` folder of the Tyk Gateway installation folder. This is demonstrated [here](/5.8/api-management/manage-apis/deploy-apis/deploy-apis-overview#file-based-configurations).
