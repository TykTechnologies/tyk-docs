...
*   [Golang native plugins](/plugins/supported-languages/golang) - fast, native performance
*   [JavaScript Plugins](/plugins/supported-languages/javascript-middleware) (JSVM Middleware) - simple with limited direct API when performance not important (same with Python / Lua)
*   [Rich Plugins](/plugins/supported-languages/rich-plugins) includes Python, Lua, gRPC - With gRPC, you can write plugins in Java, .NET, C++ / C#, PHP, and all other [gRPC supported languages](https://grpc.io/docs/languages/).
...
* Used to stop the request and return a [custom response](/plugins/plugin-types/request-plugins#return-overrides-returnoverrides)
* Be served using [Bundles](/plugins/how-to-serve-plugins) or by files on the file system, except gRPC of course which by definition is served by some webserver in the language of your choosing


## Plugin Hook Types

Tyk provide 5 different phases, i.e. hooks to inject custom plugin throughout the [API execution lifecycle](/concepts/middleware-execution-order). There are performance advantages to picking the correct phase, and of course that depends on your use case and what functionality you need.
...
More reading on the [hook types](/plugins/supported-languages/rich-plugins/rich-plugins-work#coprocess-dispatcher---hooks) in rich plugins and explanation with common use case for each [hook type](/plugins/plugin-types/plugintypes) 
...
| Built-in SDK | ✅<br />All Gateway Functionality | ✅<br />[Yes](/plugins/supported-languages/javascript-middleware/javascript-api)	| ❌	| ✅<br />[Yes](/plugins/supported-languages/rich-plugins/python/tyk-python-api-methods)	| ❌
