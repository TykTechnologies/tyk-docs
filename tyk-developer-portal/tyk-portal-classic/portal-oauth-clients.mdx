...
1. An API created in your Dashboard using Tyk's ability to act as a OAuth provider. You need to have [OAuth 2.0](/api-management/client-authentication#use-tyk-as-an-oauth-20-authorization-server) selected as the Authentication mode. See [Create an API](/api-management/gateway-config-managing-classic#create-an-api) for more details. 
...
See [Revoke OAuth Tokens](/api-management/client-authentication#revoking-access-tokens) for more details.
