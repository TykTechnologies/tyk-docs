...
See [Monitors](/basic-config-and-security/report-monitor-trigger-events/monitors) for details of how to configure quota consumption monitors.
...
To configure them, add an `event_options` section to an Organization when you are creating them. See [Creating an Organization via the Dashboard Admin API](/dashboard-admin-api/organisations#create-an-organization) for more details.
