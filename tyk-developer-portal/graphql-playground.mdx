...
- A working [Tyk Gateway](/tyk-gateway/overview) and [Tyk Dashboard](/tyk-dashboard/overview). See the [installation guide](/getting-started/quick-start) if you need to set up a new environment.
- A [GraphQL API](/api-management/graphql) in Tyk Dashboard. If you don't have one, follow **Step 1** below to create a test API using the built-in Star Wars example.
- A GraphQL SDL schema file in `.graphql`, `.graphqls`, `.gql`, or `.json` format. If you follow **Step 1**, you can download the schema directly from the Dashboard.
...
5. Set the specification type to **GraphQL SDL**, then upload your schema file.
   ...
   Accepted formats: `.graphql`, `.graphqls`, `.gql`, `.json`. The maximum upload size is 32MB, configurable via the `PORTAL_MAX_UPLOAD_SIZE` environment variable.

   <img src="/img/graphql-docs-tab-playground-setup.png" alt="Documentation tab with GraphQL SDL upload and Server URL field" />

6. Set the **GraphQL Server URL** to the live endpoint of your GraphQL API on the Tyk Gateway (for example, `http://gateway.example.com/my-graphql-api/`). From v1.16.0, this is [auto-populated](#graphql-server-url-and-baseline-url) from the Provider's Gateway Base URL and the API's listen path; you can always edit the value manually.
...
9. You should now see the **GraphQL Playground**, ready for interactive query testing.
   ...
   <Note>
   To test authenticated APIs, set the required headers in the Playground. The Star Wars GQL API uses [Auth Token](/api-management/authentication/bearer-token) authentication. Add the `Authorization` header using the Key ID you saved in step 1:
   ...
   The `Bearer ` prefix is optional; the Tyk Gateway strips it automatically if present. The exact header name depends on your API's authentication configuration.
   </Note>
...
When an API Consumer's [Developer App](/portal/developer-app) has been approved to access a Product, their credentials are automatically pre-populated in the Playground's HTTP headers. No additional configuration is required.

The header format matches the authentication scheme of the APIs in the Product. For example, for an [Auth Token](/api-management/authentication/bearer-token) API:
