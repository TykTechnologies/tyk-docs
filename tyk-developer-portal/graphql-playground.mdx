...
1. In the **Tyk Dashboard**, create a new **GraphQL API** by
    ...
    - **Star Wars GQL API** uses [Auth Token](/api-management/authentication/auth-token) for authentication. Copy the **Key ID** from the pop-up and save it for later use. We will set the authorization header in the Playground using this Key ID.
