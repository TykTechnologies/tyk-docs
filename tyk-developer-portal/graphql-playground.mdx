...
9. You should now see the **GraphQL Playground**, ready for interactive query testing.
   ...
   <Note>
   To test authenticated APIs, set the required headers in the Playground. The Star Wars GQL API uses [Auth Token (Bearer Token)](/api-management/authentication/bearer-token) authentication. Add the `Authorization` header using the Key ID you saved in step 1:
   ...
   The exact header name and format depend on your API's authentication configuration.
   </Note>
...
The header format matches the authentication scheme of the APIs in the Product. For example, for an [Auth Token (Bearer Token)](/api-management/authentication/bearer-token) API:
