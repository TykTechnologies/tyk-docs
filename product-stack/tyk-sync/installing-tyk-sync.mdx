...
<br />
<br />In order for policy ID matching to work correctly, your Dashboard must have [allow_explicit_policy_id](/tyk-dashboard/configuration#allow_explicit_policy_id) and [enable_duplicate_slugs](/tyk-dashboard/configuration#enable_duplicate_slugs) set to `true` and your Gateway must have [policies.allow_explicit_policy_id](/tyk-oss-gateway/configuration#policiesallow_explicit_policy_id) set to `true`.
</Note>
