...
<Note>
Homebrew installation is available from Tyk Sync **v[TODO: first version published with a working Homebrew formula]** onwards. For earlier versions, use [Packagecloud](#install-via-package-manager-packagecloud) or [Docker](#install-on-docker) instead. `brew install` always installs the latest version - it cannot install an older version.
</Note>
