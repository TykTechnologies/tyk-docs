...
Follow the [bootstrapping section](/product-stack/tyk-enterprise-developer-portal/deploy/bootstrapping-portal) of the documentation to bootstrap the portal via the UI or the admin API.
...
Here is an example of a sample environment file. For a comprehensive reference of environment variables, please refer to the [configuration](/content/product-stack/tyk-enterprise-developer-portal/deploy/configuration.md) section in the Tyk Enterprise Developer Portal documentation.
```ini
...
You can browse all available versions on [Docker Hub](https://hub.docker.com/r/tykio/portal/tags) and in the [release notes](/product-stack/tyk-enterprise-developer-portal/release-notes/portal-1.7.0.md) section.
```console
docker run -d \
    -p 3001:3001 \
    --env-file .env \
...
#### Bootstrap the portal
Now the portal is running on port 3001, but it needs to be bootstrapped by providing credentials for the super admin user since it's the first time you are launching it. Follow the [bootstrapping](/product-stack/tyk-enterprise-developer-portal/deploy/bootstrapping-portal) section of the documentation to bootstrap the portal via the UI or the admin API.
