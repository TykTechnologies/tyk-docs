...
**Type:** `boolean` <br/>
**Description**: Enables the [Identity Provider Registry](/api-management/client-idp-registry) for Dynamic Client Registration. When set to `true`, the Portal stores IdP configuration centrally in the Tyk Dashboard rather than writing it directly into API definitions. Requires Tyk 5.14.0 or later. The default value is `false`.
