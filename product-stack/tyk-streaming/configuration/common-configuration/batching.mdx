...
Tyk Streams is able to join sources and sinks with sometimes conflicting batching behaviours without sacrificing its strong delivery guarantees. It's also able to perform powerful [processing functions](/product-stack/tyk-streaming/configuration/common-configuration/windowed_processing) across batches of messages such as grouping and reduction. Therefore, batching within Tyk Streams is a mechanism that serves multiple purposes:
...
However, a small number of inputs such as [kafka](/product-stack/tyk-streaming/configuration/inputs/kafka) must be consumed sequentially (in this case by partition) and therefore benefit from specifying your batch policy at the input level instead:
...
Sometimes you may prefer to create your batches before processing in order to benefit from [batch wide processing](#grouped-message-processing), in which case if your input doesn't already support [a batch policy](#batch-policy) you can instead use a [broker](/product-stack/tyk-streaming/configuration/inputs/broker), which also allows you to combine inputs with a single batch policy:
...
This also works the same with [output brokers](/product-stack/tyk-streaming/configuration/outputs/broker).

## Grouped Message Processing

Some processors, such as [while](/product-stack/tyk-streaming/configuration/processors/while) are executed once across a whole batch. You can avoid this behaviour with the [for_each](/product-stack/tyk-streaming/configuration/processors/for-each) processor:
...
There's a vast number of processors that specialise in operations across batches such as [grouping](/product-stack/tyk-streaming/configuration/processors/group-by). For example, the following processors group a batch of messages according to a metadata field and compresses them into separate `.tar.gz` archives:
...
Consult our [windowing](/product-stack/tyk-streaming/configuration/common-configuration/windowed_processing) guide for further examples of batched (or windowed) processing.
...
A message batch (or multiple part message) can be broken down into smaller batches using the [split](/product-stack/tyk-streaming/configuration/processors/split) processor:
...
- A message added to the batch causes the [check](/product-stack/tyk-streaming/guides/bloblang/overview) to return to `true`.
...
If you are affected by this limitation then consider breaking the batches down with a [split](/product-stack/tyk-streaming/configuration/processors/split) processor before they reach the batch policy.
