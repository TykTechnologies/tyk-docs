...
Once our window has been grouped the next step is to calculate the aggregated passenger and unique cars counts. For this purpose the [Bloblang query language](/product-stack/tyk-streaming/guides/bloblang/overview) comes in handy as the method [from_all](/product-stack/tyk-streaming/guides/bloblang/methods/general#from_all) executes the target function against the entire batch and returns an array of the values, allowing us to mutate the result with chained methods such as [sum](/product-stack/tyk-streaming/guides/bloblang/methods/general#sum):
...
[Bloblang](/product-stack/tyk-streaming/guides/bloblang/overview) is very powerful, and by using [from](/product-stack/tyk-streaming/guides/bloblang/methods/general#from) and [from_all](/product-stack/tyk-streaming/guides/bloblang/methods/general#from_all) it's possible to perform a wide range of batch-wide processing.
