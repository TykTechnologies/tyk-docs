...
A [Bloblang query](/product-stack/tyk-streaming/guides/bloblang/overview) that should return a boolean value indicating whether a message should be routed to the case output. If left empty the case always passes.
...
An [output](/product-stack/tyk-streaming/configuration/outputs/overview) for messages that pass the check to be routed to.
