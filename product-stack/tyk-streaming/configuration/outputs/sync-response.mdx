...
It is safe to combine this output with others using broker types. For example, with the [http_server](/product-stack/tyk-streaming/configuration/inputs/http-server) input we could send the payload to a Kafka topic and also send a modified payload back with:
...
For more information please read the [Synchronous Responses](/product-stack/tyk-streaming/guides/sync-responses) guide.
