...
Splits a [batch of messages](/product-stack/tyk-streaming/configuration/common-configuration/batching) into N batches, where each resulting batch contains a group of messages determined by a [Bloblang query](/product-stack/tyk-streaming/guides/bloblang/overview).
...
The functionality of this processor depends on being applied across messages that are [batched](/product-stack/tyk-streaming/configuration/common-configuration/batching).

## Fields

### [].check

A [Bloblang query](/product-stack/tyk-streaming/guides/bloblang/overview) that should return a boolean value indicating whether a message belongs to a given group.
