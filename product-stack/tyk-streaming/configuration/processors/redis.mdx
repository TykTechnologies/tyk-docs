...
Performs actions against Redis that aren't possible using a [cache](/product-stack/tyk-streaming/configuration/processors/cache) processor.

Actions are performed for each message and the message contents are replaced with the result.

In order to merge the result into the original message compose this processor within a [branch processor](/product-stack/tyk-streaming/configuration/processors/branch).
...
each message using a [branch processor](/product-stack/tyk-streaming/configuration/processors/branch) in order to augment rather than replace
...
A [Bloblang](/product-stack/tyk-streaming/guides/bloblang/overview) mapping which should evaluate to an array of values matching in size to the number of arguments required for the specified Redis command.
