...
For use cases where you wish to cache the result of processors consider using the [cached processor](/product-stack/tyk-streaming/configuration/processors/cached) instead.
...
Deduplication can be done using the add operator with a key extracted from the message payload, since it fails when a key already exists we can remove the duplicates using a [mapping processor](/product-stack/tyk-streaming/configuration/processors/mapping):
...
Sometimes it's necessary to deduplicate a batch of messages (AKA a window) by a single identifying value. This can be done by introducing a [branch processor](/product-stack/tyk-streaming/configuration/processors/branch), which executes the cache only once on behalf of the batch, in this case with a value make from a field extracted from the first and last messages of the batch:
...
It's possible to enrich payloads with content previously stored in a cache by using the [branch](/product-stack/tyk-streaming/configuration/processors/branch) processor:
...
with a 'key already exists' error, which can be detected with processor [error handling](/product-stack/tyk-streaming/configuration/common-configuration/error-handling).
