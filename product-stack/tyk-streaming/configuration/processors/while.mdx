...
A processor that checks a [Bloblang query](/product-stack/tyk-streaming/guides/bloblang/overview) against each batch of messages and executes child processors on them for as long as the query resolves to true.
...
The conditions of this processor are applied across entire message batches using [batching](/product-stack/tyk-streaming/configuration/common-configuration/batching).
...
A [Bloblang query](/product-stack/tyk-streaming/guides/bloblang/overview) that should return a boolean value indicating whether the while loop should execute again.
