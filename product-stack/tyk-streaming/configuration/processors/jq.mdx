...
<Note>
For better performance and improved capabilities try out native Tyk Streams mapping with the [mapping processor](/product-stack/tyk-streaming/configuration/processors/mapping).
</Note>
...
Queries can fail, in which case the message remains unchanged, errors are logged, and the message is flagged as having failed, allowing you to use [standard processor error handling patterns](/product-stack/tyk-streaming/configuration/common-configuration/error-handling).
