...
You can access these metadata fields using [function interpolation](/product-stack/tyk-streaming/configuration/common-configuration/interpolation#bloblang-queries)
...
If you're seeing issues writing to or reading from Kafka with this component then it's worth trying out the newer [kafka_franz input](/product-stack/tyk-streaming/configuration/inputs/kafka-franz).
...
A [Bloblang](/product-stack/tyk-streaming/guides/bloblang/overview) mapping that attempts to extract an object containing tracing propagation information, which will then be used as the root tracing span for the message. The specification of the extracted fields must match the format used by the service wide tracer.
