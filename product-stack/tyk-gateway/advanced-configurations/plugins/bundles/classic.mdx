...
For custom plugins that are deployed as [plugin bundles](/plugins/how-to-serve-plugins/plugin-bundles), the API should be configured with the name of the plugin bundle file to download from your remote web server. Furthermore, the Gateway should be [configured](/plugins/how-to-serve-plugins/plugin-bundles#gateway-configuration) to enable downloading plugin bundles.
...
If you’re using the newer Tyk OAS APIs, then check out the [Tyk OAS](/product-stack/tyk-gateway/advanced-configurations/plugins/bundles/oas) page.
...
Tyk Gateway should be configured for downloading plugin bundles from a secured web server. Please consult the [plugin bundles](/plugins/how-to-serve-plugins/plugin-bundles) documentation for further details.
