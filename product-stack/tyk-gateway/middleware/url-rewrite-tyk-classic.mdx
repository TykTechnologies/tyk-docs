...
Tyk's [URL rewriter](/transform-traffic/url-rewriting) uses the concepts of triggers and rules to determine if the request (target) URL should be modified. These can be combined in flexible ways to create sophisticated logic to direct requests made to a single endpoint to various upstream services (or other APIs internally exposed within Tyk).

URL rewrite triggers and rules are explained in detail [here](/product-stack/tyk-gateway/middleware/url-rewrite-middleware).
...
If you're using the newer Tyk OAS APIs, then check out [this](/product-stack/tyk-gateway/middleware/url-rewrite-tyk-oas) page.
...
For further examples check out the [internal looping](/api-management/automations#internal-looping-with-tyk-operator) page.
