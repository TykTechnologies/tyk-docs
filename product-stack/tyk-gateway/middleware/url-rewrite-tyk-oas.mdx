...
When working with Tyk OAS APIs the rules and triggers are configured in the [Tyk OAS API Definition](/tyk-apis/tyk-gateway-api/oas/x-tyk-oas-doc#operation); this can be done manually within the `.json` file or from the API Designer in the Tyk Dashboard.

If you're using the legacy Tyk Classic APIs, then check out [this](/product-stack/tyk-gateway/middleware/url-rewrite-tyk-classic) page.
