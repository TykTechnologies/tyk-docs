...
The [per-endpoint custom plugin](/product-stack/tyk-gateway/middleware/endpoint-plugin) provides the facility to attach a custom Go plugin at the end of the request processing chain.
...
It can [terminate the request](/product-stack/tyk-gateway/advanced-configurations/plugins/golang/writing-go-plugins#terminating-the-request) if required,
and provides a [Virtual Endpoint](/advanced-configuration/compose-apis/virtual-endpoints) style capability using the Go language, rather than JavaScript (as supported by the virtual endpoint middleware).
...
If you're using the legacy Tyk Classic APIs, then check out the [Tyk Classic](/product-stack/tyk-gateway/middleware/endpoint-plugin-tyk-classic) page.
