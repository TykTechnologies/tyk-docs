...
The [Internal Endpoint](/product-stack/tyk-gateway/middleware/internal-endpoint-middleware) middleware instructs Tyk Gateway not to process external requests to the endpoint (which is a combination of HTTP method and path). Internal requests from other APIs will be processed.
...
If you're using the legacy Tyk Classic APIs, then check out the [Tyk Classic](/product-stack/tyk-gateway/middleware/internal-endpoint-tyk-classic) page.
...
- the [URL rewrite](/transform-traffic/url-rewriting) middleware has been configured for requests to the `GET /redirect` endpoint
