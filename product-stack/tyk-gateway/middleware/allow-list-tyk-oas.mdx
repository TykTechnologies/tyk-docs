...
If you're using the legacy Tyk Classic APIs, then check out the [Tyk Classic](/product-stack/tyk-gateway/middleware/allow-list-tyk-classic) page.
...
Note also that the endpoint path has not been terminated with `$`. Requests to, for example, `GET /anything/foobar` will be allowed as the [regular expression pattern match](/product-stack/tyk-gateway/middleware/allow-list-middleware#endpoint-parsing) will recognize this as `GET /anything`.
