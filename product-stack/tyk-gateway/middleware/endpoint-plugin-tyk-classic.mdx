...
The [per-endpoint custom plugin](/product-stack/tyk-gateway/middleware/endpoint-plugin) provides the facility to attach a custom Golang plugin at the end of the request processing chain.
...
and hence can provide a [Virtual Endpoint](/advanced-configuration/compose-apis/virtual-endpoints) style capability using the Go language, rather than JavaScript (as supported by the virtual endpoint middleware).
...
If you're using the newer Tyk OAS APIs, then check out the [Tyk OAS](/product-stack/tyk-gateway/middleware/endpoint-plugin-tyk-oas) page.
