...
<Note>
The availability of this feature [depends on your license](/tyk-dashboard/rbac).
</Note>
...
To implement this structure, you would create three user groups and assign [permissions](/basic-config-and-security/security/dashboard/user-roles) as indicated:
...
Currently, only [API Usage](/tyk-dashboard-analytics/traffic-per-api) and [Error Counts](/tyk-dashboard-analytics/error-overview) are available to users with the `owned_analytics` permission.
