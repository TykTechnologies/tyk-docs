...
2. **Aggregate Analytics**: Data used to display Dashboard charts and [analytics](/tyk-dashboard-analytics).
3. **Logs**: When [detailed logging](/tyk-stack/tyk-pump/useful-debug-modes#what-is-detailed-request-logging) is enabled, request and response data is logged to storage. These logs can previewed in the Dashboard [log browser](/tyk-stack/tyk-manager/analytics/log-browser).
...
Please consult the Pump configuration [guide](/tyk-pump/tyk-pump-configuration/tyk-pump-dashboard-config#3-sql-uptime-pump) for an explanation of how to configure Tyk Pump to write to different storage layers.
...
Further details for configuring an uptime SQL database are available [here](/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#sql-uptime-pump)
...
Further details for configuring a Tyk Mongo Pump are available [here](/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#mongo-uptime-pump>)
...
Aggregated analytics corresponds to data that is used for the display of charts and graphs in [dashboard](/tyk-dashboard-analytics). Tyk Pump can be configured to write aggregated analytics data to SQL based databases or MongoDB.
