...
See how to [Choose a MongoDB driver](/planning-for-production/database-settings/mongodb#choose-a-mongodb-driver)
...
- via command line [using tyk-sync](/universal-data-graph/udg-examples)
...
With this release we are also giving our users [improved headers for GQL APIs](/graphql/gql-headers). It is now possible to use context variables in request headers and persist headers needed for introspection separately for improved security.
...
- Now you can turn a Kafka topic into a GraphQL subscription by simply [importing your AsyncAPI definition](/tyk-apis/tyk-dashboard-api/data-graphs-api)
