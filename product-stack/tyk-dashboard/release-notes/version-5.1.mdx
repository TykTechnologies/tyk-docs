...
documentation for a full description of this new [user role](/basic-config-and-security/security/dashboard/user-roles).

##### Import API examples from within the Dashboard

In 5.0 we introduced the possibility to import API examples manually or via
[_Tyk Sync_](/api-management/automations#synchronize-tyk-environment-with-github-repository). We have now extended this feature and it is now possible to do this without
...
We added a [Dashboard API Endpoint](/universal-data-graph/datasources/rest#automatically-creating-rest-udg-configuration-based-on-oas-specification) that is capable of taking an OAS 3.0.0 document and converting it into a UDG API.
...
- Added a feature to display a full [_Tyk Sync_](/api-management/automations#synchronize-tyk-environment-with-github-repository) command that will allow a user to import an example UDG into their Dashboard
