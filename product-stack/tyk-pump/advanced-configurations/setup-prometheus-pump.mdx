...
Also you can customize Prometheus Custom Metrics based on your analytics needs. We are using `tyk_http_requests_total` and `tyk_http_latency` described [here](/tyk-stack/tyk-pump/other-data-stores/monitor-apis-prometheus) for illustration:
...
You can check our [Guide on Monitoring API with Prometheus](/tyk-stack/tyk-pump/other-data-stores/monitor-apis-prometheus#useful-queries) for a list of useful queries you can setup and use.
