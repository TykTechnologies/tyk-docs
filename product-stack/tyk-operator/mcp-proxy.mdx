...
<AccordionGroup>
  <Accordion title="Resource stays in a failed sync state">
    Check the Operator logs and the events on the CR:

    ```bash
    kubectl logs -n tyk-operator-system deploy/tyk-operator-controller-manager
    kubectl describe tykmcp <name>
    ```
  </Accordion>
  <Accordion title="OAS document not found">
    A missing or misnamed key in `tykMCP.configmapRef.keyName` is a common cause. Make sure `keyName` matches a key under the ConfigMap's `data` map.
  </Accordion>
  <Accordion title="End-to-end testing against a local mock MCP server">
    See the MCP testing notes in the Tyk Operator development guide.
  </Accordion>
</AccordionGroup>
