...
- `upstream.url` is an adapter target (for example `tyk://orders-rest/mcp`) instead of a remote `https://` URL — see [The upstream adapter target](/ai-management/mcp-gateway/api-to-mcp-definitions#the-upstream-adapter-target) for what this value means and why it takes this form.
