...
description: "Use the TykMcpProxyDefinition CRD to proxy and secure MCP servers through Tyk in Kubernetes"
...
A `TykMcpProxyDefinition` points at a `ConfigMap` that holds the MCP OAS document. The Operator reads the document from the ConfigMap and synchronises it to Tyk. Access to tools, resources, prompts and the underlying JSON-RPC methods is controlled through the existing `SecurityPolicy` resource, extended with MCP-specific fields.
...
- If the resource stays in a failed sync state, check the Operator logs (`kubectl logs -n tyk-operator-system deploy/tyk-operator-controller-manager`) and the events on the CR.
- A missing or misnamed key in `tykMCP.configmapRef.keyName` is a common cause of "OAS document not found" errors — make sure `keyName` matches a key under the ConfigMap's `data` map.
- For end-to-end testing against a local mock MCP server, see the MCP testing notes in the Tyk Operator development guide.
