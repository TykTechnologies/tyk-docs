...
The example above proxies a remote MCP server. The same `TykMcpProxyDefinition` and `ConfigMap` pattern also manages a proxy [generated directly from a Tyk-managed REST API](/ai-management/mcp-gateway/api-to-mcp), which the Operator reconciles identically. Two things differ in the OAS document itself:
