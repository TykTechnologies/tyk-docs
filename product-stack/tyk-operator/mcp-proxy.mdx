...
description: "Use the TykMcpProxyDefinition CRD in Tyk Operator v1.4.0 to proxy, secure, and apply rate limits to Model Context Protocol (MCP) servers in Kubernetes using a GitOps workflow, including MCP proxies generated directly from a Tyk-managed REST API."
...
A `TykMcpProxyDefinition` points at a `ConfigMap` that holds the MCP OAS document. The Operator reads the document from the ConfigMap and synchronizes it to Tyk as-is; it does not inspect what kind of MCP proxy the document describes. The same CRD manages a proxy fronting a remote MCP server or a proxy [generated directly from a Tyk-managed REST API](/ai-management/mcp-gateway/api-to-mcp) — only the document's own content differs between the two. Access to tools, resources, prompts, and the underlying JSON-RPC methods is controlled through the existing `SecurityPolicy` resource, extended with MCP-specific fields.
...
The example above proxies a remote MCP server. The same `TykMcpProxyDefinition` and `ConfigMap` pattern also manages a proxy [generated directly from a Tyk-managed REST API](/ai-management/mcp-gateway/api-to-mcp) — the Operator reconciles it identically. Two things differ in the OAS document itself:

- `upstream.url` is an adapter target (for example `tyk://orders-rest/mcp`) instead of a remote `https://` URL — see [MCP OAS definition: info, server, and upstream](/ai-management/mcp-gateway/mcp-proxy-definitions#info-server-and-upstream) for what this value means and why it takes this form.
- The document carries an `x-tyk-mcp-server` extension alongside `x-tyk-api-gateway`, holding the tool catalogue: which of the source API's operations are exposed as tools, and any name, description, or parameter overrides.

<Note>
Authoring this via the Operator means hand-writing the tool catalogue in the `x-tyk-mcp-server` extension yourself. The Dashboard's guided wizard — which selects all valid endpoints by default, lets you deselect unwanted ones, and previews the generated tools before saving — has no Operator equivalent. See [REST API to MCP x-tyk-mcp-server extension](/ai-management/mcp-gateway/mcp-proxy-definitions#rest-api-to-mcp-x-tyk-mcp-server-extension) for the full field reference.
</Note>

### Example

This assumes a Tyk-managed REST API with ID `orders-rest` already exists — managed via the Dashboard, the Gateway API, or its own `TykOasApiDefinition` resource.
