...
each of the following [hooks](/plugins/plugin-types/plugintypes#plugin-and-hook-types) in the API request
lifecycle: [Pre (Request)](/plugins/plugin-types/request-plugins), [Authentication](/plugins/plugin-types/auth-plugins/auth-plugins),
[Post (Request)](/plugins/plugin-types/request-plugins), [Post
Authentication](/plugins/plugin-types/request-plugins), [Response](/plugins/plugin-types/response-plugins)
and [Analytics](/plugins/plugin-types/analytics-plugins). Subsequently, users can execute, or “hook”, their
...
Please refer to [Analytics Plugins](/plugins/plugin-types/analytics-plugins) to learn how to configure Analytics
...
languages](/plugins/supported-languages#plugin-driver-names) section for list of supported plugin driver names.
...
[per-endpoint](/product-stack/tyk-gateway/middleware/endpoint-plugin) level plugins are not currently
...
- The `driver` configuration parameter is set to `otto` at line 15, since our plugin is a Javascript plugin. For other
  valid values please refer to the [plugins driver page](/plugins/supported-languages#plugin-driver-names).
- A plugin hook configuration block is specified at line 16, containing the `name` and `path` for our plugin. The plugin
  ...
  middleware. Valid values are the same as the [Tyk Classic API Definition](#tyk-classic) equivalent, i.e.
...
bundles](/plugins/how-to-serve-plugins/plugin-bundles), load them into the Tyk Gateway, and configure an API
