...
Using Tyk Classic APIs, developers can implement API-level custom plugins that can be optionally setup to execute for each of the following [hooks](/plugins/plugin-types/plugintypes#plugin-and-hook-types) in the API request lifecycle: [Pre (Request)](/plugins/plugin-types/request-plugins), [Authentication](/plugins/plugin-types/auth-plugins/auth-plugins), [Post (Request)](/plugins/plugin-types/request-plugins), [Post Authentication](/plugins/plugin-types/request-plugins), [Response](/plugins/plugin-types/response-plugins) and [Analytics](/plugins/plugin-types/analytics-plugins). Subsequently, users can execute, or “hook”, their plugin into these phases of the API request lifecycle based on their specific use case.
...
Please refer to [Analytics Plugins](/plugins/plugin-types/analytics-plugins) to learn how to configure Analytics plugins using Tyk Operator.
...
The `driver` configuration parameter describes the plugin implementation language. Please refer to the [supported languages](/plugins/supported-languages#plugin-driver-names) section for list of supported plugin driver names.
...
- The `driver` configuration parameter is set to `otto` at line 15, since our plugin is a Javascript plugin. For other valid values please refer to the [plugins driver page](/plugins/supported-languages#plugin-driver-names).
