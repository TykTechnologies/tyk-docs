...
2. **Create API Definition Resource**
    ...
    *http://httpbin.org*.
    ...
    spec:
      ...
      custom_middleware:
        driver: javascript # JavaScript driver name (goja engine, Tyk Gateway v5.14+); use otto for the legacy engine
        pre:
    ...
    - The `driver` configuration parameter is set to `javascript` at line 15, since our plugin is a JavaScript plugin. The
      `javascript` value selects the [goja engine](/api-management/plugins/javascript#the-goja-engine), introduced in Tyk
      Gateway v5.14. The legacy value `otto` selects the older otto engine, retained for backward compatibility. For other
