...
| `cert_id` | String | No | Certificate ID from [Tyk Certificate Store](/api-management/certificates#tyk-certificate-store) |
| `ca_cert_ids` | Array | No | Array of CA certificate IDs from [Tyk Certificate Store](/api-management/certificates#tyk-certificate-store) |
...
From Tyk 5.13.0, the certificate paths (`cert_file`, `key_file`, `ca_file`) for service type `oauth`, used for integration with external Identity Providers (IdPs) for [client](/api-management/authentication/jwt-signature-validation#configuring-idps-in-the-api-definition) and [upstream](/api-management/upstream-authentication/oauth#upstream-oauth-2-0) authentication, can be set using [external references](/tyk-configuration-reference/kv-store#from-the-tyk-gateway-configuration-file). These references are resolved when the Gateway starts up and on each hot reload. This allows certificate rotation without a Gateway restart; however, updated certificates are applied only after a hot reload.

The certificate paths for other integrations do not support external references at this time.

<Note>
If [multiple Identity Providers](/api-management/authentication/jwt-signature-validation#configuring-idps-in-the-api-definition) are configured, they share the same External Service Configuration.
</Note>
...
**What this does:**
- Similar to previous example, but uses [Tyk Certificate Store](/api-management/certificates#tyk-certificate-store) instead of file paths
...
**What this does:**
- Similar to example 4, but uses [Tyk Certificate Store](/api-management/certificates#tyk-certificate-store) for all mTLS configurations
