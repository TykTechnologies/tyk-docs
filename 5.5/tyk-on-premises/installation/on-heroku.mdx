...
<Note>
`--space` flag must be added to the command if the app is being created in a private space, see more details in the [section on Heroku private spaces](/5.5/#private-spaces).
</Note>
...
The [Dashboard configuration](/5.5/tyk-dashboard/configuration) can be changed by either editing the `tyk_analytics.conf` file or injecting them as [environment variables](/5.5/tyk-environment-variables) via `heroku config`. In this guide we'll use the latter for simplicity of demonstration but there is merit to both methods.
...
All these files serve the same purpose as with the Dasboard and the Pump. [Configuration](/5.5/tyk-oss-gateway/configuration) can either be edited in `tyk.conf` or [injected](/5.5/tyk-environment-variables) with `heroku config`.
...
In order to enable [rich plugins](/5.5/plugins/supported-languages/rich-plugins) for the Gateway, please set the following Heroku config option to either `python` or `lua` depending on the type of plugins used:
```{.copyWrapper}
