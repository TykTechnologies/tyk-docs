...
## Introduction
The [Tyk control plane](/5.5/tyk-multi-data-centre/mdcb-components.md#control-plane) contains all the
...
The MDCB component will however, by default, expose an RPC service on port 9091, to which the [Tyk Data Plane](/5.5/tyk-multi-data-centre/mdcb-components.md#data-plane) data centers, i.e. the worker gateway(s) that serves API traffic, will need connectivity.
...
* MongoDB or SQL (check [supported versions](/5.5/planning-for-production/database-settings))
* Redis (check [supported versions](/5.5/planning-for-production/redis))
* Tyk Dashboard
* Tyk Gateway / Gateways Cluster
* Working Tyk-Pro [Self-Managed installation](/5.5/tyk-self-managed/install)
...
The [Tyk Control Plane](/5.5/product-stack/tyk-charts/tyk-control-plane-chart) helm chart is pre-configured to install Tyk control plane for multi data center API management from a single Dashboard with the MDCB component.

Below is a concise instruction on how to set up an MDCB Control Plane with Redis and PostgreSQL.

To access the comprehensive installation instructions and configuration options, please see [Tyk Control Plane Helm Chart](/5.5/product-stack/tyk-charts/tyk-control-plane-chart).
...
<Note>
Ensure that you are installing Redis versions that are supported by Tyk. Please consult the list of [supported versions](/5.5/planning-for-production/redis#supported-versions) that are compatible with Tyk.
</Note>
...
<Note>
Ensure that you are installing PostgreSQL versions that are supported by Tyk. Please consult the list of [supported versions](/5.5/tyk-dashboard/database-options) that are compatible with Tyk.
</Note>
...
You can use the MDCB connection details included in the installation output, to install the [MDCB Data Plane](/5.5/tyk-multi-data-centre/setup-worker-data-centres).
...
<Note>
From MDCB 2.0+, you can choose between Mongo or SQL databases to setup your `analytics` storage. In order to setup your PostgreSQL storage, you can use the same configuration from your [Tyk Dashboard main storage](/5.5/content/planning-for-production/database-settings/postgresql.md).
...
See [MDCB API](/5.5/tyk-mdcb-api) for details of the endpoints.
...
To make things easier, we will first set a few [environment variables](/5.5/tyk-environment-variables):
