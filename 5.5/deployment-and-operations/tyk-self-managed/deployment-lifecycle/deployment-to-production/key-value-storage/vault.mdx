...
Alternatively, you can configure it using the equivalent [environment variables](/5.5/tyk-oss-gateway/configuration#kvvaulttoken).
...
As explained [below](/5.5/deployment-and-operations/tyk-self-managed/deployment-lifecycle/deployment-to-production/key-value-storage/vault#tyk-gateway-configuration-file), you could retrieve this value from within your Tyk Gateway config file using: 
...
As described [here](/5.5/tyk-configuration-reference/kv-store#from-the-tyk-gateway-configuration-file), from Tyk Gateway's configuration file (`tyk.conf`) you can retrieve values from Vault using the following notation:
...
For [certain transformation middleware](/5.5/tyk-configuration-reference/kv-store#transformation-middleware) because the secret resolution happens during the request context, a different notation is used to retrieve values from Vault:
- `$secret_vault.KEY`
