...
Tyk allows you to create versions of your APIs. When using Tyk OAS APIs versioning works slightly differently from with Tyk Classic APIs, to find out more please take a look at the dedicated [page](/5.5/getting-started/key-concepts/oas-versioning).
...
You need to create a new API that will be the [Base API](/5.5/getting-started/key-concepts/oas-versioning#key-concepts) for the future versions. You do this by sending a Tyk OAS API Definition to the Tyk Gateway API's `apis/oas` endpoint. Note that there is no special command required to create this new API as a Base API - i.e. any Tyk OAS API can be used as a Base API.
