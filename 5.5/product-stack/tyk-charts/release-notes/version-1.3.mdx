...
'og:description': "Release notes documenting updates, enhancements and changes for Tyk Charts versions within the 1.3 series."
sidebarTitle: "Tyk Charts 1.3"
...
For MongoDB users: Tyk Charts 1.3.0 uses `mongo-go` as the default driver to connect to MongoDB. `mongo-go` driver is compatible with MongoDB 4.4.x and above. For MongoDB versions prior to 4.4, please set `global.mongo.driver` to `mgo`. We recommend reading [Choose a MongoDB driver](/5.5/planning-for-production/database-settings/mongodb#choose-a-mongodb-driver) when you need to change driver setting.
...
<br/>
{/* Go to the [Upgrading Tyk](/5.5/#upgrading-tyk) section for detailed upgrade Instructions. */}
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.5/#Changelog-v1.3.0) below.
...
##### Graph Pump
Tyk Charts 1.3 adds support for Graph MongoDB Pump, Graph SQL Pump and Graph SQL Aggregate Pump. see [Graph Pump setup](/5.5/tyk-stack/tyk-pump/tyk-pump-configuration/graph-pump) to learn more about the GraphQL-specific metrics available.

##### Enable Tyk Identity Broker (TIB) in Tyk Dashboard
Tyk Charts 1.3 adds a field to enable Internal [Tyk Identity Broker (TIB)](/5.5/tyk-identity-broker) in Tyk Dashboard by field `tyk-dashboard.tib.enabled` to `true`.
...
When enabled at `gateway.control.ingress.enabled`, an Ingress resource will be created to allow external access to gateway's [control service](/5.5/planning-for-production#change-your-control-port).
</Accordion>

<Accordion title={'Gateway: Configure Gateway to work with MDCB synchroniser'}>
Allow users to configure worker gateway to work with [Tyk MDCB synchroniser](/5.5/product-stack/tyk-enterprise-mdcb/advanced-configurations/synchroniser) easily by setting `global.mdcbSynchronizer.enabled` in `tyk-data-plane`.
...
You can enable Internal [Tyk Identity Broker (TIB)](/5.5/tyk-identity-broker) in Tyk Dashboard by field `tyk-dashboard.tib.enabled` to `true`.
</Accordion>
...
read [Tyk Control Plane chart](/5.5/product-stack/tyk-charts/tyk-control-plane-chart).
</Accordion>
