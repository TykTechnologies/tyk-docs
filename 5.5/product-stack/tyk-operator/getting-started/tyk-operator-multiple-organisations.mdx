...
Please consult the [key concepts for Tyk Operator](/5.5/product-stack/tyk-operator/key-concepts/operator-context) documentation for an overview of the fundamental concepts of organizations in Tyk and the use of OperatorContext to manage resources for different teams effectively.
...
An [OperatorContext](/5.5/product-stack/tyk-operator/key-concepts/operator-context) specifies the parameters for connecting and authenticating with a Tyk Dashboard. Below is an example of how to define an `OperatorContext`:
