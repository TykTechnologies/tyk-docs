...
'og:description': "Guide for installing the Tyk Enterprise Developer Portal in Red Hat environment using RPM"
...
You can, however, customize the provided example and make more suitable for your need using the [configuration](/5.5/product-stack/tyk-enterprise-developer-portal/deploy/configuration) reference.
```json
{
  "HostPort": 3001,
...
Now the portal is running on port 3001, but it needs to be bootstrapped by providing credentials for the super admin user since it's the first you are launching it. Follow the [bootstrapping](/5.5/product-stack/tyk-enterprise-developer-portal/deploy/bootstrapping-portal) section of the documentation to bootstrap the portal via the UI or the admin API.