...
- [22 bugs fixed](/5.5/#fixed)
- [19 CVEs fixed](/5.5/#fixed)
...
- [PORTAL_DATABASE_MAX_OPEN_CONNECTIONS](/5.5/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_database_max_open_connections): Set this value based on your database’s maximum connection limit divided by the number of portal instances. For example, if your database allows 200 connections and you are running 4 portal instances, set PORTAL_DATABASE_MAX_OPEN_CONNECTIONS to 50 per instance. This ensures that all instances can share the available connections without exceeding the database's limit, which could otherwise lead to performance degradation or errors.
- [PORTAL_DATABASE_MAX_IDLE_CONNECTIONS](/5.5/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_database_max_idle_connections): Set to 15 or a lower value based on your expected load. This setting keeps a reasonable number of connections readily available without tying up resources unnecessarily.
...
- Added new TLS variables to set MinVersion ([PORTAL_TLSCONFIG_MINVERSION](/5.5/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_tlsconfig_minversion), MaxVersion ([PORTAL_TLSCONFIG_MAXVERSION](/5.5/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_tlsconfig_maxversion), and CipherSuites ([PORTAL_TLS_CIPHER_SUITES](/5.5/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_tls_cipher_suites).
- Added a new configuration to manage the idle timeout of the portal's session ([PORTAL_SESSION_IDLE_TIMEOUT](/5.5/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_session_idle_timeout).
...
- Changed the default value of [PORTAL_DATABASE_CONNECTION_MAX_LIFETIME](/5.5/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_database_connection_max_lifetime) to 1800000 milliseconds.
