...
An input is a source of data piped through an array of optional [processors](/5.5/product-stack/tyk-streaming/configuration/processors/overview):
...
Only one input is configured at the root of a Tyk Streams config. However, the root input can be a [broker](/5.5/product-stack/tyk-streaming/configuration/inputs/broker) which combines multiple inputs and merges the streams:
...
It's possible to generate data with Tyk Streams using the [generate](/5.5/product-stack/tyk-streaming/configuration/inputs/generate) input, which is also a convenient way to trigger scheduled pipelines.
