---
title: "Http Server"
'og:description': "Explains an overview of configuring Http server input"
...
{/* TODO add link to service wide HTTP server If the `address` config field is left blank the [service-wide HTTP server](/5.5/docs/components/http/about) will be used. */}

{/* TODO add rate limit The field `rate_limit` allows you to specify an optional [`rate_limit` resource](/5.5/docs/components/rate_limits/about), which will be applied to each HTTP request made and each websocket payload received.
...
It's possible to return a response for each message received using synchronous responses. When doing so you can customise headers with the `sync_response` field `headers`, which can also use [function interpolation](/5.5/product-stack/tyk-streaming/configuration/common-configuration/interpolation#bloblang-queries) in the value based on the response message contents.
...
You can access these metadata fields using [interpolation functions](/5.5/product-stack/tyk-streaming/configuration/common-configuration/interpolation#bloblang-queries).
...
An optional [rate limit](/5.5/docs/components/rate_limits/about) to throttle requests by. */}
