---
title: "Outputs"
'og:description': "Explains an overview of outputs"
sidebarTitle: "Overview"
tags: ['Tyk Streams', 'Stream Outputs', 'Outputs']
---

An output is a sink where we wish to send our consumed data after applying an optional array of [processors](/5.5/product-stack/tyk-streaming/configuration/processors/overview). Only one output is configured at the root of a Tyk Streams config. However, the output can be a [broker](/5.5/product-stack/tyk-streaming/configuration/outputs/broker) which combines multiple outputs under a chosen brokering pattern, or a [switch](/5.5/product-stack/tyk-streaming/configuration/outputs/switch) which is used to multiplex against different outputs.
...
It's possible to instead have Tyk Streams indefinitely retry an output until success with a [retry](/5.5/product-stack/tyk-streaming/configuration/outputs/retry) output. Some other outputs, such as the [broker](/5.5/product-stack/tyk-streaming/configuration/outputs/broker), might also retry indefinitely depending on their configuration.

## Dead Letter Queues

It's possible to create fallback outputs for when an output target fails using a [fallback](/5.5/product-stack/tyk-streaming/configuration/outputs/fallback) output:
...
Some output fields support [field interpolation](/5.5/product-stack/tyk-streaming/configuration/common-configuration/interpolation), which is a super easy way to multiplex messages based on their contents in situations where you are multiplexing to the same service.
...
A more advanced form of multiplexing is to route messages to different output configurations based on a query. This is easy with the [switch](/5.5/product-stack/tyk-streaming/configuration/outputs/switch) output:
