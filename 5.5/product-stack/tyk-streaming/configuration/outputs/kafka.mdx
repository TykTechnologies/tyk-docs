---
title: "Kafka"
'og:description': "Explains an overview of configuring Kafka output"
sidebarTitle: "Kafka"
...
{/* Add links to bloblang queries : Both the `key` and `topic` fields can be dynamically set using function interpolations described [here](/5.5/docs/configuration/interpolation#bloblang-queries). */}

Metadata will be added to each message sent as headers (version 0.11+), but can be restricted using the field [metadata](/5.5/#metadata).
...
If you're seeing issues writing to or reading from Kafka with this component then it's worth trying out the newer [kafka_franz output](/5.5/product-stack/tyk-streaming/configuration/outputs/kafka-franz).
...
This output benefits from sending messages as a [batch](/5.5/product-stack/tyk-streaming/configuration/common-configuration/batching) for improved performance. Batches can be formed at both the input and output level. 
...
This field supports [interpolation functions](/5.5/product-stack/tyk-streaming/configuration/common-configuration/interpolation#bloblang-queries).
...
A [Bloblang](/5.5/product-stack/tyk-streaming/guides/bloblang/overview) mapping used to inject an object containing tracing propagation information into outbound messages. The specification of the injected fields will match the format used by the service wide tracer.
