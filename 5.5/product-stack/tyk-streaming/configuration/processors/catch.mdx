...
Behaves similarly to the [for_each](/5.5/product-stack/tyk-streaming/configuration/processors/for-each) processor, where a list of child processors are applied to individual messages of a batch. However, processors are only applied to messages that failed a processing step prior to the catch.
...
Consult the [error handling](/5.5/product-stack/tyk-streaming/configuration/common-configuration/error-handling) guide for further information related to handling errors in Tyk Streams.
