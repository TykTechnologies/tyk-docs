...
This guide explains how to configure plugins for Tyk OAS APIs within the [Tyk OAS API definition](/5.5/#tyk-oas-apidef) or via the [API designer](/5.5/#tyk-oas-dashboard) in Tyk Dashboard.

If you’re using the legacy Tyk Classic APIs, then check out the [Tyk Classic](/5.5/product-stack/tyk-gateway/advanced-configurations/plugins/api-config/classic) page.
...
The `pluginConfig` section contains the `driver` parameter that is used to configure the plugin implementation [language](/5.5/plugins/supported-languages#plugin-driver-names):
...
The configuration above is a complete and valid Tyk OAS API Definition that you can use as a basis for trying out custom plugins. You will need to update the [driver](/5.5/plugins/supported-languages#plugin-driver-names) parameter to reflect the target language type of your plugins. You will also need to update the `path` and `functionName` parameters for each plugin to reflect the source code.
