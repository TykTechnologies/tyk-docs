...
'og:description': "An introduction to API versioning"
...
- [URL path](/5.5/#request-url-path)
- [Query parameter](/5.5/#query-parameter)
- [Request header](/5.5/#request-header)
...
The API definition (and Tyk Dashboard's API Designer) makes it easy for you to specify - and change - the *default* version for your APIs, as explained in the appropriate section for [Tyk OAS APIs](/5.5/getting-started/key-concepts/oas-versioning) and [Tyk Classic APIs](/5.5/getting-started/key-concepts/versioning).
...
The upstream (target) URL will be constructed by combining the configured `upstream.url` (`target_url` for Tyk Classic APIs) with the full request path unless configured otherwise (for example, by using the [strip listen path](/5.5/tyk-apis/tyk-gateway-api/oas/x-tyk-oas-doc#listenpath) feature).
...
When using the [Request URL](/5.5/#request-url-path) for the versioning identifier, if Tyk is configured to strip the versioning identifier then the first URL fragment after the `listenPath` (`listen_path` for Tyk Classic APIs) will be deleted prior to creating the proxy URL. If the request does not include a versioning identifier and Tyk is configured to [fallback to default](/5.5/#fallback-to-default), this may lead to undesired behaviour as the first URL fragment of the endpoint will be deleted.
...
<Note>
API tokens (keys) created from a [Security Policy](/5.5/basic-config-and-security/security/security-policies) will be granted access to the APIs named in the policy, so whilst we have discussed access keys here, the same rules apply to the policies from which keys might have been created.
</Note>


<hr />

If you're using Tyk OAS APIs, then you can find details of how to use versioning [here](/5.5/getting-started/key-concepts/oas-versioning).

If you're using Tyk Classic APIs, then you can find details of how to use versioning [here](/5.5/getting-started/key-concepts/versioning).
