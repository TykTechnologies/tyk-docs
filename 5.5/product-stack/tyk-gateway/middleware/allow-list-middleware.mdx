...
Note that this is not the same as Tyk's [IP allow list](/5.5/tyk-apis/tyk-gateway-api/api-definition-objects/ip-whitelisting) feature, which is used to restrict access to APIs based upon the IP of the requestor.
...
When using the allow list middleware, we recommend that you familiarize yourself with Tyk's [URL matching](/5.5/getting-started/key-concepts/url-matching) options.
...
If you're using Tyk OAS APIs, then you can find details and examples of how to configure the allow list middleware [here](/5.5/product-stack/tyk-gateway/middleware/allow-list-tyk-oas).

If you're using Tyk Classic APIs, then you can find details and examples of how to configure the allow list middleware [here](/5.5/product-stack/tyk-gateway/middleware/allow-list-tyk-classic).
