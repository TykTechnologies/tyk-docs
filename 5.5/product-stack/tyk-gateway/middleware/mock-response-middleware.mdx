...
When working with Tyk OAS APIs, Tyk Gateway can parse the [examples and schema](/5.5/product-stack/tyk-gateway/middleware/mock-response-openapi) in the OpenAPI description and use this to automatically generate responses using those examples. Where multiple examples are defined, for example for different response codes, Tyk enables you to [configure special headers](/5.5/product-stack/tyk-gateway/middleware/mock-response-tyk-oas#multiple-mock-responses-for-a-single-endpoint) in the request to select the desired mock response.
...
If you’re using Tyk OAS APIs, then you can find details and examples of how to configure the mock response middleware [here](/5.5/product-stack/tyk-gateway/middleware/mock-response-tyk-oas).

If you’re using Tyk Classic APIs, then you can find details and examples of how to configure the response body transformation middleware [here](/5.5/product-stack/tyk-gateway/middleware/mock-response-tyk-classic).
