...
See [Debugging Tab](/5.5/advanced-configuration/transform-traffic/endpoint-designer#debugging) for more information.
...
There is a [new section](/5.5/product-stack/tyk-dashboard/advanced-configurations/analytics/audit-log) in the Tyk Dashboard config file where you can specify parameters for the audit log (containing audit records for all requests made to all endpoints under the `/api` route).
