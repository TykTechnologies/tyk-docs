...
Multi-tenant deployments with [role-based access control](/5.5/tyk-dashboard/rbac) enabled allows an admin user to give different users or groups access to a sub-set of the entire API portfolio. Categories can be aligned with the API ownership rules that you have deployed to allow filtering the list of APIs for those visible to each separate user group/team.
...
API categories can be added and removed from APIs within the [API Designer](/5.5/product-stack/tyk-dashboard/advanced-configurations/api-categories#api-designer) or via the [Tyk Dashboard API](/5.5/product-stack/tyk-dashboard/advanced-configurations/api-categories#tyk-dashboard-api).
...
### Tyk Dashboard API
The [Tyk Dashboard API](/5.5/tyk-dashboard-api) provides endpoints to manage categories directly, if you are not using the API Designer.
