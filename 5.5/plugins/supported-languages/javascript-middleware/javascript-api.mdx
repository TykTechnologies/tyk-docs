...
- `TykBatchRequest` this function is similar to `TykMakeHttpRequest` but makes use of the Tyk Batch API. See the Batch Requests section of the [Tyk Gateway API](/5.5/tyk-gateway-api) for more details.
...
- `TykGetKeyData(api_key, api_id)`: Use this method to retrieve a [session object](/5.5/getting-started/key-concepts/what-is-a-session-object) for the key and the API provided:
