...
We see that upon every pull request, a section of steps are taken to "Build, [Bundle](/5.5/plugins/how-to-serve-plugins/plugin-bundles), Release Go Plugin".
...
The next step of the workflow is to "[bundle](/5.5/plugins/how-to-serve-plugins/plugin-bundles)" the plugin.
...
<Note>
For added security, please consider signing your [bundles](/5.5/plugins/how-to-serve-plugins.md#global-parameters), especially if the connection between the Gateways and the Bundler server traverses the internet.
</Note>
...
<Note>
For seamless deployments, take a look at multi-version [plugin support](/5.5/plugins/supported-languages/golang.md#upgrading-tyk) to enable zero downtime deployments of your Tyk Gateway installs
</Note>
...
1. The root server - The server which all bundles will be downloaded from.  This is set globally in the Tyk conf file [here](/5.5/tyk-oss-gateway/configuration#enable_bundle_downloader).
