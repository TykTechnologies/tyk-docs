...
There are two approaches to configure caching for an API deployed with Tyk:
 ...
 - [Basic](/5.5/basic-config-and-security/reduce-latency/caching/global-cache) or [Safe Request](/5.5/basic-config-and-security/reduce-latency/caching#global-cache-safe-requests) caching is applied at the API level for all requests for which it is safe to do so.
 - [Advanced](/5.5/basic-config-and-security/reduce-latency/caching/advanced-cache) caching options can be applied at the endpoint level.
...
Tyk does support safe request caching at the more granular, per-endpoint level, as described [here](/5.5/basic-config-and-security/reduce-latency/caching/advanced-cache#selective-caching-by-endpoint-all-safe-requests) - but `cache_all_safe_requests` must be set to `false` in that scenario.
