...
The HTTP method, body, header values, and target URL can all be configured in the API definition. The [request body](/5.5/#webhook-payload) is generated using a Tyk template file that has access to the [event metadata](/5.5/basic-config-and-security/report-monitor-trigger-events/event-data).
...
When your webhook event handler is triggered, it will send an HTTP request to the configured target. For HTTP methods that support a request body, for example `POST`, the event handler will process a [Go template](/5.5/product-stack/tyk-gateway/references/go-templates) to produce the payload.
...
The event handler has access to the [event metadata](/5.5/basic-config-and-security/report-monitor-trigger-events/event-data) and this can be accessed by the template using the `{{.Meta.XXX}}` namespace.

The [event type](/5.5/basic-config-and-security/report-monitor-trigger-events/event-types) that triggered the event handler can be accessed as `{{.Type}}`.
...
*Global webhook definitions* are registered with the Dashboard using the [UI](/5.5/#creating-a-global-webhook-definition-using-tyk-dashboard) or [Dashboard API](/5.5/tyk-apis/tyk-dashboard-api/web-hooks) and assigned a unique *webhook id* that can be obtained via the [Dashboard API](/5.5/tyk-apis/tyk-dashboard-api/web-hooks#list-web-hooks) or via drop-down selection within the UI.
...
If you're using Tyk OAS APIs, then you can find details and examples of how to configure webhook event handlers [here](/5.5/product-stack/tyk-gateway/basic-config-and-security/report-monitor-and-trigger-events/event-webhook-tyk-oas).

If you're using Tyk Classic APIs, then you can find details and examples of how to configure webhook event handlers [here](/5.5/product-stack/tyk-gateway/basic-config-and-security/report-monitor-and-trigger-events/event-webhook-tyk-classic).
