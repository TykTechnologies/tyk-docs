...
As explained in the [Rate Limiting Concepts](/5.5/getting-started/key-concepts/rate-limiting) section, Tyk supports configuration of rate limits at both the API-Level and Key-Level for different use cases.
...
Not yet, though IP-based rate limiting is possible using custom pre-processor middleware JavaScript that generates tokens based on IP addresses. See our [Middleware Scripting Guide](/5.5/plugins/supported-languages/javascript-middleware/middleware-scripting-guide) for more details.
