...
Selecting the **Account is Admin** checkbox from the Dashboard gives the user full access (it has the same [effect](/5.5/basic-config-and-security/security/dashboard/user-roles#admin-users) as the `IsAdmin` property).

### Custom user permissions
You can create your own custom permissions for use with the [Open Policy Agent (OPA)](/5.5/tyk-dashboard/open-policy-agent) using the [Additional Permissions](/5.5/tyk-dashboard-api/org/permissions) endpoint in the Tyk Dashboard Admin API. This allows you to add and delete (CRUD) a list of additional (custom) permissions for your Dashboard users. Once created, a custom permission will be added to standard list of user permissions. 

You can also configure these custom permissions in the `security.additional_permissions` [map](/5.5/tyk-dashboard/configuration#securityadditional_permissions) in the Tyk Dashboard configuration file. 