...
'og:description': "Using HMAC signatures"
...
Creating HMAC keys is the same as creating regular access tokens - by using the [Tyk Gateway API](/5.5/tyk-apis/tyk-gateway-api/api-definition-objects/authentication). Setting the `hmac_enabled` flag to `true`, Tyk will generate a secret key for the key owner (which should not be modified), but will be returned by the API so you can store and report it to your end-user.
