...
To enable [Dashboard Analytics](/5.5/tyk-dashboard-analytics), you would need to configure Tyk Pump to send analytic data to the Dashboard storage MongoDB / SQL.
...
### Capping
This collection [should be capped](/5.5/tyk-pump/configuration#capping-analytics-data) due to the number of individual documents. This is especially important if the `detailed_recording` in the Gateway is turned on which means that the Gateway records the full payload of the request and response. 
...
The field [use_sharded_analytics](/5.5/tyk-dashboard/configuration#use_sharded_analytics) controls the collection that the dashboard will query.
...
1. The [enable_aggregate_lookups: true](/5.5/tyk-dashboard/configuration#enable_aggregate_lookups) field must be set in the Dashboard configuration file, in order for the Dashboard to query and display the aggregated data that `mongo-pump-aggregate` saved to MongoDB.
...
This collection [should be capped](/5.5/tyk-stack/tyk-manager/analytics/capping-analytics-data-storage) due to the number of individual documents.
```yaml
{
  ...
  "pumps": {
...
To enable Uptime Pump, modify gateway configuration [enable_uptime_analytics](/5.5/tyk-oss-gateway/configuration#uptime_testsconfigenable_uptime_analytics) to true.

</Tab>
<Tab title="SQL">

When using one of our [supported SQL platforms](/5.5/tyk-dashboard/database-options#introduction), Tyk offers 3 types of SQL pumps:
...
1. The [enable_aggregate_lookups: true](/5.5/tyk-dashboard/configuration#enable_aggregate_lookups) field must be set in the Dashboard configuration file, in order for the Dashboard to query and display the aggregated data that `sql-aggregate` saved to the database.
