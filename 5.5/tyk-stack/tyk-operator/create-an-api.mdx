...
sidebarTitle: "Create an API with Tyk Operator"
order: 2
---

### Tutorial: Create an API with Tyk Operator
Creating an API takes the same approach whether you are using Tyk Open Source or Self Managed. First, specify the details of your API using the [ApiDefinition CRD](/5.5/product-stack/tyk-operator/reference/api-definition), then deploy it to create the corresponding Kubernetes resource. Tyk Operator will take control of the CRD and create the actual API in the Tyk data plane.
...
You can visit the [ApiDefinition CRD](/5.5/product-stack/tyk-operator/reference/api-definition) page to see all the latest API Definitions fields and features we support.
