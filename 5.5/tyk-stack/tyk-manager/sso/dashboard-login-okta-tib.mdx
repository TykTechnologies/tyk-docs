...
* You already have authorized access to Tyk's Dashboard. If you haven't, [get the authorization key by following this doc](/5.5/basic-config-and-security/security/dashboard/create-users#a-name-with-api-a-create-a-dashboard-user-with-the-api).
...
7. Start TIB by running the binary (`profiles.json` is in the same CWD)
   See [Install TIB](/5.5/advanced-configuration/integrate/3rd-party-identity-providers#tib) for detailed instructions on how to install TIB
...
```
  ...
  - See [TIB REST API](/5.5/tyk-identity-broker/tib-rest-api) for more details.
...
### SSO login into the Dashboard via a login page
   You will need to:
    ...
    Explicit details are in [steps 6-7](/5.5/advanced-configuration/integrate/3rd-party-identity-providers/dashboard-login-ldap-tib#6-create-a-login-page)
