...
'og:description': "How to transform the body for an API request"
...
This middleware changes only the payload and not the headers. You can, however, combine this with the [Request Header Transform](/5.5/transform-traffic/request-headers) middleware to apply more complex transformation to requests.

There is a closely related [Response Body Transform](/5.5/advanced-configuration/transform-traffic/response-body) middleware that provides the same functionality on the response from the upstream, prior to it being returned to the client.
...
A common use of the request body transform middleware is to surface a legacy SOAP service with a REST API. Full details of how to perform this conversion using Tyk are provided [here](/5.5/advanced-configuration/transform-traffic/soap-rest).
...
If you're using Tyk OAS APIs, then you can find details and examples of how to configure the request body transformation middleware [here](/5.5/product-stack/tyk-gateway/middleware/request-body-tyk-oas).

If you're using Tyk Classic APIs, then you can find details and examples of how to configure the request body transformation middleware [here](/5.5/product-stack/tyk-gateway/middleware/request-body-tyk-classic).
...
{/* proposed "summary box" to be shown graphically on each middleware page
 ## Request Body Transform middleware summary
  ...
  - Request Body Transform can access both [session metadata](/5.5/getting-started/key-concepts/session-meta-data) and [request context variables](/5.5/context-variables). */}
