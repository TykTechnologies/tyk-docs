...
2. [Compile your custom Go plugins](/5.6/developer-support/upgrading-tyk/go-plugins).
...
| :--- | :--- | :--- |
| [Path 1](/5.6/#path1)    | < 4.1.0 | < 4.1.0 |
| [Path 2](/5.6/#path2)    | >= 4.1.0 | >= 4.2.0 |

<h2 id="path-1">Path 1 - Current Version < 4.1.0 and Target Version < 4.1.0</h2>
1. [Create a plugin bundle](/5.6/plugins/how-to-serve-plugins/plugin-bundles) that includes the newly compiled version
...
2. [Upload this bundle](/5.6/tyk-cloud/configuration-options/using-plugins/uploading-bundle) to your configured S3 bucket if using Cloud SaaS. If you're using Hybrid SaaS, upload this bundle to your bundle server.


<h2 id="path-2">Path 2 - Current `Version >= 4`.1.0 and Target `Version >= 4`.2.0</h2>
1. [Create a plugin bundle](/5.6/plugins/how-to-serve-plugins/plugin-bundles) that includes both your current version’s plugin along with the newly compiled version
