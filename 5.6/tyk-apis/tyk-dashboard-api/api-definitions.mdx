...
<Note>
See [API Definition Objects](/5.6/tyk-gateway-api/api-definition-objects) section for an explanation of each field in the request & response.
</Note>
...
<Note>
`{id}` can either be the internal or public ID ( see `api_id` in the [sample response](/5.6/#sample-response-1) )
</Note>
