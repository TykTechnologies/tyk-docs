...
'og:description': "Tyk Tools that help with automating deployment and API Management operations"
...
- **A Tyk installation** (Self-Managed or Cloud)
  - If you don't have Tyk installed, follow our [installation guide](/5.6/tyk-self-managed/install)
...
- **Tyk Dashboard v3+ access** (for Tyk Sync setup)
  - Learn how to set up the Tyk Dashboard [here](/5.6/tyk-dashboard)
...
- **Basic knowledge of Kubernetes, YAML, and API concepts** (important for Tyk Operator and Tyk Sync)
  ...
  - For API concepts, review our [API management basics](/5.6/getting-started/key-concepts)
...
The following custom resources can be used to configure APIs and policies at [Tyk Gateway](/5.6/tyk-oss-gateway) or [Tyk Dashboard](/5.6/tyk-dashboard).

| Kind               | Group       | Version   | Description                                                                                       |
| :-------------------- | :------------- | :----------- | :--------------------------------------------------------------------------------------------------- |
| TykOasApiDefinition| tyk.tyk.io  | v1alpha1  | Defines configuration of [Tyk OAS API Definition object](/5.6/tyk-apis/tyk-gateway-api/oas/x-tyk-oas-doc)                                 |
| ApiDefinition      | tyk.tyk.io  | v1alpha1  | Defines configuration of [Tyk Classic API Definition object](/5.6/tyk-gateway-api/api-definition-objects)                                 |
| SecurityPolicy     | tyk.tyk.io  | v1alpha1  | Defines configuration of [security policies](/5.6/getting-started/key-concepts/what-is-a-security-policy). Operator supports linking ApiDefinition custom resources in SecurityPolicy's access list so that API IDs do not need to be hardcoded in the resource manifest.        |
| SubGraph           | tyk.tyk.io  | v1alpha1  | Defines a [GraphQL federation subgraph](/5.6/getting-started/key-concepts/graphql-federation#subgraphs-and-supergraphs).                                           |
| SuperGraph         | tyk.tyk.io  | v1alpha1  | Defines a [GraphQL federation supergraph](/5.6/getting-started/key-concepts/graphql-federation#subgraphs-and-supergraphs).                                        |
| OperatorContext    | tyk.tyk.io  | v1alpha1  | Manages the context in which the Tyk Operator operates, affecting its overall behavior and environment. See [Operator Context](/5.6/#multi-tenancy-in-tyk) for details. |

##### Tyk Classic Developer Portal

The following custom resources can be used to configure [Tyk Classic Developer Portal](/5.6/tyk-developer-portal/tyk-portal-classic).

| Kind               | Group       | Version   | Description                                                                                       |
| :-------------------- | :------------- | :----------- | :--------------------------------------------------------------------------------------------------- |
| APIDescription     | tyk.tyk.io  | v1alpha1  | Configures [Portal Documentation](/5.6/tyk-apis/tyk-portal-api/portal-documentation). |
| PortalAPICatalogue | tyk.tyk.io  | v1alpha1  | Configures [Portal API Catalogue](/5.6/getting-started/key-concepts/api-catalogue). |
| PortalConfig       | tyk.tyk.io  | v1alpha1  | Configures [Portal Configuration](/5.6/tyk-apis/tyk-portal-api/portal-configuration). |
...
#### Operator User
Tyk Operator is a Kubernetes Controller that manages Tyk Custom Resources (CRs) such as API Definitions and Security Policies. Developers define these resources as [Custom Resource (CRs)](/5.6/#custom-resources-in-tyk), and Tyk Operator ensures that the desired state is reconciled with the Tyk Gateway or Dashboard. This involves creating, updating, or deleting API configurations until the target state matches the desired state.
...
Tyk Dashboard is multi-tenant capable, which means you can use a single Tyk Dashboard instance to host separate [organizations](/5.6/basic-config-and-security/security/dashboard/organisations) for each team or department. Each organization is a completely isolated unit with its own:
...
| :------------------ | :------------- | :--------- |
| Client certifates | ✅ [Client mTLS](/5.6/api-management/authentication-authorization#setup-static-mtls-in-tyk-operator-using-the-tyk-classic-api-definition) | ✅ [Client mTLS](/5.6/api-management/authentication-authorization#setup-static-mtls-in-tyk-operator-using-tyk-oas-api-definition) |
| Custom domain certificates | ✅ [TLS and SSL](/5.6/basic-config-and-security/security/tls-and-ssl#tyk-operator-classic) | ✅ [TLS and SSL](/5.6/basic-config-and-security/security/tls-and-ssl#tyk-operator-oas) |
| Public keys pinning | ✅ [Certificate pinning](/5.6/security/certificate-pinning#tyk-operator-classic) | ✅ [Certificate pinning](/5.6/security/certificate-pinning#tyk-operator-oas) |
| Upstream mTLS | ✅ [Upstream mTLS via Operator](/5.6/api-management/authentication-authorization#via-tyk-operator-using-the-tyk-classic-api-definition) | ✅ [Upstream mTLS via Operator](/5.6/api-management/authentication-authorization#tyk-operator-oas) |


### Install and Configure Tyk Operator

We assume you have already installed Tyk. If you don’t have it, check out [Tyk
Cloud](/5.6/deployment-and-operations/tyk-cloud-platform/quick-start) or [Tyk Self
Managed](/5.6/getting-started/installation) page. [Tyk Helm
Chart](/5.6/product-stack/tyk-charts/overview) is the preferred (and easiest) way to install Tyk on Kubernetes.
...
Tyk Operator needs a [user credential](/5.6/#operator-user) to connect with
...
[Using Tyk Operator to enable GitOps with Tyk](/5.6/getting-started/key-concepts/gitops-with-tyk) about
...
If you are using [Tyk Stack](/5.6/product-stack/tyk-charts/tyk-stack-chart), [Tyk Control
Plane](/5.6/product-stack/tyk-charts/tyk-control-plane-chart), or [Tyk Open
Source Chart](/5.6/product-stack/tyk-charts/tyk-oss-chart), you can install Tyk Operator alongside other Tyk
...
another name, configure it through Helm Chart [envFrom](/5.6/#install-tyk-operator-and-custom-resource-definitions-crds) value.
...
You can configure [Tyk Operator as Ingress Controller](/5.6/#control-kubernetes-ingress-resources) so
...
[WATCH_INGRESS_CLASS](/5.6/#configure-tyk-operator-via-environment-variable-or-tyk-operator-conf-secret) through `tyk-operator-conf` or the environment variable.
...
You can configure your API for any Tyk supported authentication method by following [Authentication with Tyk OAS](/5.6/getting-started/key-concepts/authentication) documentation.
...
First, specify the details of your API using the [ApiDefinition CRD](/5.6/#apidefinition-crd), then deploy it to create the corresponding Kubernetes resource. Tyk Operator will take control of the CRD and create the actual API in the Tyk data plane.
...
- [HTTP Proxy example](/5.6/#set-up-manifest-for-http)
- [TCP Proxy example](/5.6/#set-up-manifest-for-tcp)
- [GraphQL Proxy example](/5.6/#set-up-manifest-for-graphql)
- [UDG example](/5.6/#set-up-manifest-for-udg)
...
You can visit the [ApiDefinition CRD](/5.6/#apidefinition-crd) page to see all the latest API Definitions fields and features we support.
...
<Note>
Tyk Operator supported authentication types are listed in the [API Definition features](/5.6/#apidefinition-crd) section.
</Note>
...
This configuration allows [keyless (open)](/5.6/api-management/authentication-authorization#use-open-keyless-authentication) access to the API without any authentication.
...
This setup requires a [bearer token](/5.6/api-management/authentication-authorization#use-bearer-tokens) for access.
...
- `validate_signature`: Enable [signature validation](/5.6/api-management/authentication-authorization#use-bearer-tokens).
...
This configuration uses [JWT tokens](/5.6/api-management/authentication-authorization#use-json-web-tokens-jwt) for authentication.

Users can configure JWT authentication by defining the following fields:

- `jwt_signing_method`: Specify the method used to sign the JWT. Refer to [JWT Signing Method](/5.6/api-management/authentication-authorization#set-up-jwt-signing-method) for supported methods.
...
This configuration uses [Basic Authentication](/5.6/api-management/authentication-authorization#use-basic-authentication), requiring a username and password for access.
...
This configuration uses a [Golang plugin](/5.6/plugins/supported-languages/golang) for custom authentication. The following example shows how to create an API definition with a Golang custom plugin for `httpbin-go-auth`.

For an example of Golang authentication middleware, see [Performing custom authentication with a Golang plugin](/5.6/product-stack/tyk-gateway/advanced-configurations/plugins/golang/go-plugin-examples#performing-custom-authentication-with-a-golang-plugin).
...
This configuration uses a [gRPC plugin](/5.6/plugins/supported-languages/golang) for custom authentication. The following example shows how to create an API definition with a gRPC custom plugin for `httpbin-grpc-auth`.
...
This setup allows for [multiple authentication](/5.6/api-management/authentication-authorization#multiple-chained-authentication) methods to be chained together, requiring clients to pass through each specified authentication provider.

To enable multiple (chained) auth, you should set `base_identity_provided_by` field to one of the supported chained enums. Consult [Enable Multi (Chained) Authentication in your API Definition](/5.6/api-management/authentication-authorization#enable-multi-chained-authentication-in-your-api-definition) for the supported auths.
...
To enable [IP Allowlist](/5.6/tyk-apis/tyk-gateway-api/api-definition-objects/ip-whitelisting), set the following fields:
...
To enable [IP Blocklist](/5.6/tyk-apis/tyk-gateway-api/api-definition-objects/ip-blacklisting), set the following fields:
...
In the example below we can see that the configuration is contained within the `graphql` configuration object. A GraphQL schema is specified within the `schema` field and the execution mode is set to `proxyOnly`. The [GraphQL public playground](/5.6/graphql/graphql-playground#enabling-public-graphql-playground) is enabled with the path set to `/playground`.
...
If you are on Tyk 3.2 and above, you can use the following manifest to create an UDG API. This example configures a Universal Data Graph from a [GraphQL datasource](/5.6/universal-data-graph/datasources/graphql) and a [REST Datasource](/5.6/universal-data-graph/datasources/rest).
...
Below, we take you through how to define the security policy but you can also find [Security Policy Example](/5.6/#security-policy-example) below.
...
In this example, the security policy will apply to an `ApiDefinition` resource named `httpbin` in the `default` namespace and a `TykOasApiDefinition` resource named `petstore` in the `default` namespace. Note that with Tyk Operator, you do not need to specify API ID as in the raw [Policy definition](/5.6/basic-config-and-security/security/security-policies/policies-guide). Tyk Operator will automatically retrieve the API ID of referenced API Definition resources for you.
...
You can secure your APIs by specifying [allowed URLs](/5.6/security/security-policies/secure-apis-method-path) (methods and paths) for each API within a security policy. This is done using the `allowed_urls` field under `access_rights_array`.
...
Tyk Operator by default looks for the value `tyk` and will ignore all other ingress classes. If you wish to override this default behavior,
 you may do so by setting the environment variable `WATCH_INGRESS_CLASS` in the operator manager deployment. [See Installing Tyk Operator](/5.6/#install-and-configure-tyk-operator) for further information.
...
Each path in an Ingress must have its own particular path type. Kubernetes offers three types of path types: `ImplementationSpecific`, `Exact`, and `Prefix`. Currently, not all path types are supported. The below table shows the unsupported path types for [Sample HTTP Ingress Resource](/5.6/#set-up-manifest-for-http) based on the examples in the [Kubernetes Ingress documentation](https://kubernetes.io/docs/concepts/services-networking/ingress/#examples).
...
Please bear in mind that if `proxy.strip_listen_path` is set to true on API Definition, Tyk strips the listen-path (for example, the listen-path for the Ingress under [Sample HTTP Ingress Resource](/5.6/#set-up-manifest-for-http) is /httpbin) with an empty string.
...
An [OperatorContext](/5.6/#multi-tenancy-in-tyk) specifies the parameters for connecting and authenticating with a Tyk Dashboard. Below is an example of how to define an `OperatorContext`:
...
The concept of [internal looping](/5.6/advanced-configuration/transform-traffic/looping) allows you to use URL Rewriting to redirect your URL to *another API endpoint* or to *another API* in the Gateway. In Tyk, looping is generally targeted using the `tyk://<API_ID>/<path>` scheme, which requires prior knowledge of the `API_ID`. Tyk Operator simplifies the management and transformation of API traffic within Kubernetes environments by abstracting APIs as objects, managing them and dynamically assigning `API_ID`s by its Kubernetes metedata name and namespace.

---

#### Configuring looping to internal ApiDefinition resources

Looping can be configured within Tyk Operator for [URL Rewrites](/5.6/#url-rewrites), [URL Rewrite Triggers](/5.6/#url-rewrite-triggers) and [Proxy to internal APIs](/5.6/#proxy-to-internal-apis) by configuring the `rewrite_to_internal` in `url_rewrite`, `rewrite_to_internal` in `triggers`, and `proxy.target_internal` fields respectively with these properties:
...
[URL rewriting](/5.6/transform-traffic/url-rewriting) in Tyk enables the alteration of incoming API request paths to align with the expected endpoint format of your backend services.
...
[Triggers](/5.6/product-stack/tyk-gateway/middleware/url-rewrite-middleware#url-rewrite-triggers) are configurations that specify actions based on certain conditions present in HTTP headers, query parameters, path parameters etc.
...
The process for configuring internal looping in triggers to is similar to that explained in section [URL Rewrites](/5.6/#url-rewrites).
...
Internal looping can also be used for [proxying to internal APIs](/5.6/advanced-configuration/transform-traffic/looping).
...
API name can be set through `x-tyk-api-gateway.info.name` field in [Tyk OAS API Definition](/5.6/tyk-apis/tyk-gateway-api/oas/x-tyk-oas-doc) object.
...
API active state can be set through `x-tyk-api-gateway.info.state.active` field in [Tyk OAS API Definition](/5.6/tyk-apis/tyk-gateway-api/oas/x-tyk-oas-doc) object.
...
API accessibility can be set through `x-tyk-api-gateway.info.state.internal` field in [Tyk OAS API Definition](/5.6/tyk-apis/tyk-gateway-api/oas/x-tyk-oas-doc) object.
...
If you already have API configurations created in the Tyk Dashboard and want to start using Tyk Operator to manage these APIs, you can include the existing API ID in the manifest under the `x-tyk-api-gateway.info.id` field in [Tyk OAS API Definition](/5.6/tyk-apis/tyk-gateway-api/oas/x-tyk-oas-doc) object.
...
#### API Categories
[API categories](/5.6/product-stack/tyk-dashboard/advanced-configurations/api-categories) are configured differently for Tyk OAS APIs and Tyk Classic APIs. Please see below for examples.
...
For a Tyk Classic API, you can specify the category name using the `name` field with a `#` qualifier. This will categorize the API in the Tyk Dashboard. See [How API categories work](/5.6/product-stack/tyk-dashboard/advanced-configurations/api-categories#tyk-classic-apis) to learn about limitations on API names.
...
#### API Versioning
[API versioning](/5.6/product-stack/tyk-gateway/advanced-configurations/api-versioning/api-versioning) are configured differently for [Tyk OAS APIs](/5.6/#tyk-oas-api) and [Tyk Classic APIs](/5.6/#tyk-classic-api). Please see below for examples.

##### Configuring API Version in Tyk OAS API Definition

In the [Tyk OAS API Definition](/5.6/getting-started/key-concepts/oas-versioning#configuring-api-versioning-in-the-tyk-oas-api-definition), versioning can be configured via `x-tyk-api-gateway.versioning` object of the Base API, where the child API's IDs are specified. In the Kubernetes environment with Tyk Operator, where we reference API resources through its Kubernetes name and namespace, this is not desired. Therefore, we add support for versioning configurations through the field `versioning` in `TykOasApiDefinition` custom resource definition (CRD).
...
`versioning` is configured at `order-api` (v1), the Base API, and it has similiar structure as [Tyk OAS API Definition](/5.6/getting-started/key-concepts/oas-versioning#configuring-api-versioning-in-the-tyk-oas-api-definition):
...
For Tyk Classic API, versioning can be configured via `ApiDefinition` custom resource definition (CRD). See [Tyk Classic versioning](/5.6/getting-started/key-concepts/versioning) for a comprehensive example of configuring API versioning for Tyk Classic API with Tyk Operator.

#### API Ownership

Please consult the [API Ownership](/5.6/product-stack/tyk-dashboard/advanced-configurations/user-management/api-ownership) documentation for the fundamental concepts of API Ownership in Tyk and [Operator Context](/5.6/#multi-tenancy-in-tyk) documentation for an overview of the use of OperatorContext to manage resources for different teams effectively.
...
Starting with Sync v1.5+ and Dashboard v5.3.2+, Tyk Sync supports both [Tyk OAS APIs](/5.6/getting-started/key-concepts/high-level-concepts) and [Tyk Classic APIs](/5.6/getting-started/key-concepts/what-is-an-api-definition#api-definition-types) when working with the Tyk Dashboard, without requiring special flags or configurations.

For Sync versions v1.4.1 to v1.4.3, enabling Tyk Sync for Tyk OAS APIs requires the [allow-unsafe-oas](/5.6/tyk-dashboard/configuration#allow_unsafe_oas) configuration in the Dashboard, along with the `--allow-unsafe-oas` flag when invoking Tyk Sync. Note that Tyk Sync versions v1.4.1 to 1.4.3 do not support API Category for Tyk OAS APIs.
...
The TykOasApiDefinition Custom Resource Definition (CRD) manages [Tyk OAS API Definition object](/5.6/tyk-apis/tyk-gateway-api/oas/x-tyk-oas-doc) within a Kubernetes environment. This CRD enables the integration and management of Tyk API definitions using Kubernetes-native tools, simplifying the process of deploying and managing OAS APIs on the Tyk Dashboard.

##### TykOasApiDefinition Features

`TykOasApiDefinition` can support all [features of the Tyk OAS API](/5.6/getting-started/using-oas-definitions/oas-reference). You just need to provide the Tyk OAS API definition via a ConfigMap. In addition to managing the CRUD (Create, Read, Update, Delete) of Tyk OAS API resources, the Tyk Operator helps you better manage resources through object linking to Ingress, Security Policies, and certificates stored as Kubernetes secrets. See below for a list of Operator features and examples:

| Features | Support | Supported From | Comments | Example |
| :---------- | :--------- | :----------------- | :---------- | :-------- |
| API Category | ✅      | v1.0 | - | [Manage API Categories](/5.6/#api-categories) |
| API Version | ✅      | v1.0 | - | [Manage API versioning](/5.6/#api-versioning) |
| API Ownership via OperatorContext | ✅      | v1.0 | - | [API Ownership](/5.6/product-stack/tyk-dashboard/advanced-configurations/user-management/api-ownership#when-to-use-api-ownership) |
| Client Certificates | ✅      | v1.0 | - | [Manage TLS certificate](/5.6/#tls-certificates) |
| Custom Domain Certificates | ✅      | v1.0 | - | [Manage TLS certificate](/5.6/#tls-certificates) |
| Public keys pinning | ✅      | v1.0 | - | [Manage TLS certificate](/5.6/#tls-certificates) |
| Upstream mTLS | ✅      | v1.0 | - | [Manage TLS certificate](/5.6/#tls-certificates) |
| Kubernetes Ingress | ✅      | v1.0 | - | [Kubernetes Ingress Controller](/5.6/#control-kubernetes-ingress-resources) |
| Link with SecurityPolicy | ✅      | v1.0 | - | [Protect an API](/5.6/#add-a-security-policy-to-your-api) |
...
See [Release notes](/5.6/product-stack/tyk-operator/release-notes/overview.md) to check for each Tyk Operator release,
...
#### Security Policy CRD
The SecurityPolicy custom resource defines configuration of [Tyk Security Policy object](/5.6/basic-config-and-security/security/security-policies).

Here are the supported features:

| Features                       | Support   | Supported From | Example |
| :-------------------------------- | :----------- | :---------------- | :--------- |
| API Access                     | ✅        | v0.1           | [API Access](/5.6/#define-the-security-policy-manifest)        |
| Rate Limit, Throttling, Quotas | ✅        | v0.1           | [Rate Limit, Throttling, Quotas](/5.6/#define-the-security-policy-manifest)        |
| Meta Data & Tags               | ✅        | v0.1           | [Tags and Meta-data](/5.6/#define-the-security-policy-manifest)        |
| Path and Method based permissions | ✅     | v0.1           | [Path based permission](/5.6/#security-policy-example)        |
| Partitions                     | ✅        | v0.1           | [Partitioned policies](/5.6/#security-policy-example)       |
| Per API limit                  | ✅        | v1.0           | [Per API Limit](/5.6/#security-policy-example)        |
| Per-Endpoint limit             | ✅        | v1.0           | [Per Endpoint Limit](/5.6/#security-policy-example)        |
...
- **Advanced Tyk API Management**: Leverage more of Tyk’s API capabilities for custom integrations and further automation possibilities. You can learn more about Tyk's custom integrations [here](/5.6/product-stack/tyk-operator/advanced-configurations/custom-plugins)
- **GraphQL Support**: You can learn more about how to support GraphQL resources via Tyk Operator [here](/5.6/product-stack/tyk-operator/advanced-configurations/custom-plugins)