...
Alternatively, you can configure it using the equivalent [environment variables](/5.6/tyk-oss-gateway/configuration#kvconsuladdress).
...
As described [here](/5.6/tyk-configuration-reference/kv-store#from-the-tyk-gateway-configuration-file), from Tyk Gateway's configuration file (`tyk.conf`) you can retrieve values from Consul using the following notation:
- `consul://path/to/KEY`
...
For [certain transformation middleware](/5.6/tyk-configuration-reference/kv-store#transformation-middleware) because the secret resolution happens during the request context, a different notation is used to retrieve values from Consul:
- `$secret_consul.KEY`
