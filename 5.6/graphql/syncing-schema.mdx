...
Syncing schemas is only available for proxy-only GraphQL APIs and **not** for UDG.
 ...
 If your upstream is protected then you need to make sure you provide Tyk with the authorization details to execute introspection query correctly. You can add those detail while [creating GQL API](/5.6/graphql/introspection#introspection-for-protected-upstreams) or using [Introspection headers](/5.6/graphql/gql-headers#introspection-headers) later on.