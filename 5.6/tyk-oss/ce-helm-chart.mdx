...
<Warning>
`tyk-headless` chart is deprecated. Please use our Tyk Chart for Tyk Open Source at [tyk-oss](/5.6/tyk-oss/ce-helm-chart-new) instead. 

We recommend all users migrate to the `tyk-oss` Chart. Please review the [Configuration](/5.6/tyk-oss/ce-helm-chart-new) section of the new helm chart and cross-check with your existing configurations while planning for migration.
</Warning>
...
The following are required for a Tyk OSS installation:
 ...
 - MongoDB/SQL - Required only if you chose to use the MongoDB/SQL Tyk pump with your Tyk OSS installation. Same goes with any
             [other pump](/5.6/tyk-stack/tyk-pump/other-data-stores) you choose to use.
...
1. [Add an API](/5.6/getting-started/create-api.md)
2. [Create a Security Policy](/5.6/getting-started/create-security-policy.md)
3. [Create an API Key](/5.6/getting-started/create-api-key.md)
