...
sidebarTitle: "Rich Plugins - Data Structures"
order: 3
---
...
The class diagram below illustrates the structure of the [Object](/5.6/#object) message, dispatched by Tyk to a gRPC server that handles custom plugins.
...
`response`
Contains information populated from the upstream HTTP response data, for response hooks. See [ResponseObject](/5.6/#responseobject-coprocess_response_objectproto) for more details. All the field contents can be modified.
...
The `ResponseObject` exists within an [object](/5.6/#object-coprocess_objectproto) for response hooks. The fields are populated with the upstream HTTP response data. All the field contents can be modified.
...
`access_rights`
Defined as a `map<string, APIDefinition>` instance, that maps the session's API ID to an [AccessDefinition](/5.6/#access-definition). The AccessDefinition defines the [access rights](/5.6/security/security-policies/secure-apis-method-path#setting-granular-paths-on-a-per-key-basis) for the API in terms of allowed: versions and URLs(endpoints). Each URL (endpoint) has a list of allowed methods. For further details consult the tutorials for how to create a [security policy](/5.6/getting-started/create-security-policy) for Tyk Cloud, Tyk Self Managed and Tyk OSS platforms.
...
For further details see [BasicAuthData](/5.6/#basicauthdata-coprocess_session_stateproto).

`jwt_data`
Added to sessions where a Tyk key (embedding a shared secret) is used as the public key for signing the JWT. The JWT token's KID header value references the ID of a Tyk key. See [JWTData](/5.6/#jwtdata-session_stateproto) for an example.

`hmac_enabled`
When set to `true` this indicates generation of a [HMAC signature](/5.6/api-management/authentication-authorization#sign-requests-with-hmac) using the secret provided in `hmac_secret`. If the generated signature matches the signature provided in the *Authorization* header then authentication of the request has passed.
...
<Note>
Although `apply_policy_id` is still supported, it is now deprecated. `apply_policies` is now used to list your policy IDs as an array. This supports the **[Multiple Policy](/5.6/basic-config-and-security/security/security-policies/partitioned-policies#a-name-multiple-a-multiple-policies)** feature introduced in the  **v2.4 - 1.4** release.
</Note>
...
`monitor`
Defines a [quota monitor](/5.6/basic-config-and-security/report-monitor-trigger-events/monitors) containing a list of percentage threshold limits in descending order. These limits determine when webhook notifications are triggered for API users or an organization. Each threshold represents a percentage of the quota that, when reached, triggers a notification. See [Monitor](/5.6/#monitor) for further details and an example.
...
This is a UNIX timestamp that signifies when a cached key or ID will expire. This relates to custom authentication, where authenticated keys can be cached to save repeated requests to the gRPC server. See [id_extractor](/5.6/plugins/plugin-types/auth-plugins/id-extractor) and [Auth Plugins](/5.6/plugins/plugin-types/auth-plugins/auth-plugins) for additional information.

`session_lifetime`
UNIX timestamp that denotes when the key will automatically expire. Any·subsequent API request made using the key will be rejected. Overrides the global session lifetime. See [Key Expiry and Deletion](/5.6/api-management/authentication-authorization#set-physical-key-expiry-and-deletion) for more information.

---

<h2 id="access-definition">AccessDefinition</h2>
```protobuf
...
Defined as an attribute within a [SessionState](/5.6/#session-state) instance. Contains the allowed versions and URLs (endpoints) for the API that the session request relates to. Each URL (endpoint) specifies an associated list of allowed methods. See also [AccessSpec](/5.6/#access-spec).
...
`allowed_urls` List of [AccessSpec](/5.6/#access-spec) instances. Each instance defines a URL (endpoint) with an associated allowed list of methods. If all URLs (endpoints) are allowed then the attribute is not set.

---

<h2 id="access-spec">AccessSpec</h2>
...
The `BasicAuthData` contains a hashed password and the name of the hashing algorithm used. This is represented by the `basic_auth_data` attribute in [SessionState](/5.6/#session-state) message.
...
`hash`
Name of the [hashing algorithm](/5.6/basic-config-and-security/security/key-hashing) used to hash the password.

---

## JWTData

Added to [sessions](/5.6/#sessionstate-session_stateproto) where a Tyk key (embedding a shared secret) is used as the public key for signing the JWT. This message contains the shared secret.
...
Added to a [session](/5.6/#session-state) when [monitor quota thresholds](/5.6/basic-config-and-security/report-monitor-trigger-events/monitors) are defined within the Tyk key. This message contains the quota percentage threshold limits, defined in descending order, that trigger webhook notification.
