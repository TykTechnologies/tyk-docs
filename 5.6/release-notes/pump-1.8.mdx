...
- When [`omit_configfile`](/5.6/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables.md#omit_config_file) is set to `true`, Pump will not try to load the config file and spit out error logs

#### Updated
- Updated the default Hybrid Pump RPC pool size from 20 to 5 connections in order to reduce default CPU and memory footprint. See [Pump configurations](/5.6/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables.md#pumpshybridmetarpcpoolsize)
