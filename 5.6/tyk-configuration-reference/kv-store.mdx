...
Storing local settings within the Tyk Gateway's configuration file allows you to have per instance variables, such as a machine ID, and inject these into API requests and responses using [transformation middleware](/5.6/advanced-configuration/transform-traffic).
...
The exception to this is for specific [transformation middleware](/5.6/tyk-configuration-reference/kv-store#transformation-middleware) where the value will be retrieved for each call to the API, during the processing of the API request or response.
...
- to retrieve the value assigned to a `KEY` in Consul you will use `consul://KEY` or `$secret_consul.KEY` notation depending on the [location](/5.6/tyk-configuration-reference/kv-store#how-to-access-the-externally-stored-data) of the reference
...
- to retrieve the value assigned to a `KEY` in Vault you will use `vault://KEY` or `$secret_vault.KEY` notation depending on the [location](/5.6/tyk-configuration-reference/kv-store#how-to-access-the-externally-stored-data) of the reference

#### Tyk Gateway config file

The `secrets` section in the [Tyk Gateway configuration file](/5.6/tyk-oss-gateway/configuration#secrets) allows you to store settings that are specific to a single Tyk Gateway instance. This is useful for storing instance-specific configuration to be injected into API middleware or if you prefer using configuration files.
- to retrieve the value assigned to a `KEY` in the `secrets` config you will use `secrets://KEY` or `$secret_conf.KEY` notation depending on the [location](/5.6/tyk-configuration-reference/kv-store#how-to-access-the-externally-stored-data) of the reference
...
- if you want to set the local "secrets" section (equivalent to the `secrets` section in the [Gateway config file](/5.6/tyk-configuration-reference/kv-store#tyk-gateway-config-file)) using environment variables, you should use the following notation: `TYK_GW_SECRETS=key:value,key2:value2`
...
- to retrieve the value assigned to an environment variable `VAR_NAME` you will use `env://VAR_NAME` or `$secret_env.VAR_NAME` notation depending on the [location](/5.6/tyk-configuration-reference/kv-store#how-to-access-the-externally-stored-data) of the reference
...
In Tyk Gateway's configuration file (`tyk.conf`), you can retrieve values from KV stores for the following [fields](/5.6/tyk-oss-gateway/configuration):
- `secret`
- `node_secret`
- `storage.password`
...
From Tyk Gateway v5.3.0 onwards, you can store [any string field](/5.6/tyk-configuration-reference/kv-store#other-string-fields) from the API definition in any of the supported KV storage options; for earlier versions of Tyk Gateway only the [Target URL and Listen Path](/5.6/tyk-configuration-reference/kv-store#target-url-and-listen-path) fields and [certain transformation middleware](/5.6/tyk-configuration-reference/kv-store#transformation-middleware) configurations were supported. 
...
When the Gateway starts, Tyk will read the *Value* from the environment variable and use this as the [Target URL](/5.6/tyk-apis/tyk-gateway-api/oas/x-tyk-oas-doc#upstream).
...
- [request body transform](/5.6/transform-traffic/request-body)
- [request header transform](/5.6/transform-traffic/request-headers)
- [URL rewrite](/5.6/transform-traffic/url-rewriting)
- [response body transform](/5.6/advanced-configuration/transform-traffic/response-body)
- [response header transform](/5.6/advanced-configuration/transform-traffic/response-headers)
...
- Environment variables: `$secret_env.key` or `env://key` (see [here](/5.6/tyk-configuration-reference/kv-store#using-environment-variables-with-transformation-middleware))
...
When accessing KV references from the `/tyk-apis` directory on Consul or Vault, you should not provide the `path/to/` the key except for Target URL and Listen Path (as described [above](/5.6/tyk-configuration-reference/kv-store#target-url-and-listen-path)).
</Note>
...
When the Gateway starts, Tyk will read the *Value* from the `secrets` section in the Gateway config file and use this to identify the header where Tyk Gateway should look for the [Authentication](/5.6/tyk-apis/tyk-gateway-api/oas/x-tyk-oas-doc#authentication) token in requests to your Tyk OAS API.