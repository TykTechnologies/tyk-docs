...
If you're using the newer Tyk OAS APIs, then check out the [Tyk OAS](/5.6/product-stack/tyk-gateway/middleware/virtual-endpoint-tyk-oas) page.
...
If you want to use Virtual Endpoints, you must [enable Tyk's JavaScript Virtual Machine](/5.6/tyk-oss-gateway/configuration#enable_jsvm) by setting `enable_jsvm` to `true` in your `tyk.conf` file.
...
The process for configuring a virtual endpoint using Tyk Operator is similar to that explained in [configuring the middleware in the Tyk Classic API Definition](/5.6/#tyk-classic)
