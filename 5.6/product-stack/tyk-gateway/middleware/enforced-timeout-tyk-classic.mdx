...
Tyk's [enforced timeout](/5.6/planning-for-production/ensure-high-availability/enforced-timeouts) middleware is configured at the endpoint level, where it sets a limit on the response time from the upstream service. If the upstream takes too long to respond to a request, Tyk will terminate the request and return `504 Gateway Timeout` to the client.
...
If you're using the newer Tyk OAS APIs, then check out the [Tyk OAS](/5.6/product-stack/tyk-gateway/middleware/enforced-timeout-tyk-oas) page.

If you're using Tyk Operator then check out the [configuring an enforced timeout in Tyk Operator](/5.6/#tyk-operator) section below.
