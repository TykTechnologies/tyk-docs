...
'og:description': "Tutorial to configuring and using Tyk OAS  Mock Response middleware"
sidebarTitle: "Tyk OAS configuration"
tags: ['mock response', 'middleware', 'per-endpoint', 'Tyk OAS', 'Tyk OAS API']
---

This tutorial is for Tyk OAS API definition users. If you're using the legacy Tyk Classic APIs, please refer to the [Tyk Classic Mock Response tutorial](/5.6/product-stack/tyk-gateway/middleware/mock-response-tyk-classic).

The [Mock Response](/5.6/product-stack/tyk-gateway/middleware/mock-response-middleware) middleware allows you to configure Tyk to return a response for an API endpoint without requiring an upstream service. 
...
The middleware is defined in the [Tyk OAS API Definition](/5.6/tyk-apis/tyk-gateway-api/oas/x-tyk-oas-doc#operation). To create this definition, you can use the Tyk Dashboard API or the API Designer in the Tyk Dashboard UI.
...
Tyk will parse the [examples and schema](/5.6/product-stack/tyk-gateway/middleware/mock-response-openapi) in the OpenAPI document and use them to generate mock responses automatically.
...
<Note>
If multiple `examples` are defined in the OpenAPI description but no default `exampleName` is set in the middleware configuration `fromOASExamples` Tyk will select randomly from the multiple `examples`. Yes, that means the response may change with every request. You can [control which response](/5.6/#working-with-multiple-mock-responses-for-an-endpoint) will be returned using special headers in the request.
</Note>
...
- [manual configuration](/5.6/#manual-configuration) of the middleware config
- [automatic configuration](/5.6/#automatic-configuration) from the OpenAPI description
...
- choose a default Response Body example that will be provided (unless [overridden in the request](/5.6/#working-with-multiple-mock-responses-for-an-endpoint))
- add a description for your mock response
