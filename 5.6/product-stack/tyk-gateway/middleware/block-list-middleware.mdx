...
Note that this is not the same as Tyk's [IP block list](/5.6/tyk-apis/tyk-gateway-api/api-definition-objects/ip-blacklisting) feature, which is used to restrict access to APIs based upon the IP of the requestor.
...
When using the block list middleware, we recommend that you familiarize yourself with Tyk's [URL matching](/5.6/getting-started/key-concepts/url-matching) options.
...
If you're using Tyk OAS APIs, then you can find details and examples of how to configure the block list middleware [here](/5.6/product-stack/tyk-gateway/middleware/block-list-tyk-oas).

If you're using Tyk Classic APIs, then you can find details and examples of how to configure the block list middleware [here](/5.6/product-stack/tyk-gateway/middleware/block-list-tyk-classic).
