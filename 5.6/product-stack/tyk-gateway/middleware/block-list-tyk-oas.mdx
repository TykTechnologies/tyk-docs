...
If you're using the legacy Tyk Classic APIs, then check out the [Tyk Classic](/5.6/product-stack/tyk-gateway/middleware/block-list-tyk-classic) page.
...
Note also that the endpoint path has not been terminated with `$`. Requests to, for example, `GET /anything/foobar` will be rejected as the [regular expression pattern match](/5.6/product-stack/tyk-gateway/middleware/block-list-middleware#endpoint-parsing) will recognize this as `GET /anything`.
