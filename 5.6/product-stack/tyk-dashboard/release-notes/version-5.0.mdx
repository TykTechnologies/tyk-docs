...
sidebarTitle: "Tyk Dashboard v5.0"
...
<h3 id="upgrade-5.0.15">Upgrade instructions</h3>
If you are upgrading to 5.0.15, please follow the detailed [upgrade instructions](/5.6/#upgrading-tyk). 

<h3 id="Changelog-v5.0.15">Changelog</h3>
...
See how to [Choose a MongoDB driver](/5.6/planning-for-production/database-settings/mongodb#choose-a-mongodb-driver)
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.6/#Changelog-v5.0.1) below.
...
We’ve improved support for [OAS Mock Responses](/5.6/product-stack/tyk-gateway/middleware/mock-response-middleware), with the Tyk OAS API definition now allowing you to register multiple Mock Responses in a single API, providing you with increased testing flexibility.
...
- via command line [using tyk-sync](/5.6/universal-data-graph/udg-examples)
...
With this release we are also giving our users [improved headers for GQL APIs](/5.6/graphql/gql-headers). It is now possible to use context variables in request headers and persist headers needed for introspection separately for improved security.
...
- Now you can turn a Kafka topic into a GraphQL subscription by simply [importing your AsyncAPI definition](/5.6/tyk-apis/tyk-dashboard-api/data-graphs-api)
