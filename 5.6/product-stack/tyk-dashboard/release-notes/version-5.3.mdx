...
'og:description': "Release notes documenting updates enhancements, and changes for Tyk Dashboard versions within the 5.3.X series."
sidebarTitle: "Tyk Dashboard v5.3 LTS"
...
As outlined in the [LTS policy](/5.6/developer-support/special-releases-and-features/long-term-support-releases), version 5.3 is a long-term support release with full support available until May 2025. Maintenance support will continue until May 2026. Our next long-term support release will be announced at the end of April 2025.
...
### Release Highlights
This release focuses mainly on bug fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.6/#Changelog-v5.3.8) below.

### Breaking Changes

This release has no breaking changes.

<h3 id="dependencies-5.3.8">Dependencies</h3>
...
<h4 id="3rdPartyTools-v5.3.8">3rd Party Dependencies & Tools</h4>
...
| :---------------------------------------------------------- | :---------------------- | :---------------------- | :-------- |
| [GoLang](https://go.dev/dl/)                               | 1.22       | 1.22       | [Go plugins](/5.6/plugins/supported-languages/golang) must be built using Go 1.22 |
...
This is an advanced notice that the dedicated External OAuth, OpenID Connect (OIDC) authentication options, and SQLite support will be deprecated starting in version 5.7.0. We recommend that users of the [External OAuth](/5.6/api-management/authentication-authorization#integrate-external-oauth-middleware) and [OpenID Connect](/5.6/api-management/authentication-authorization#use-openid-connect) methods migrate to Tyk's dedicated [JWT Auth](/5.6/api-management/authentication-authorization#use-json-web-tokens-jwt) method. Please review your API configurations, as the Gateway logs will provide notifications for any APIs utilizing these methods.

### Upgrade Instructions
If you are upgrading to 5.3.8, please follow the detailed [upgrade instructions](/5.6/#upgrading-tyk).
...
<h3 id="Changelog-v5.3.8">Changelog</h3>
...
The UI now displays a deprecation notice for the dedicated [External OAuth](/5.6/api-management/authentication-authorization#integrate-external-oauth-middleware) and [OpenID Connect (OIDC)](/5.6/api-management/authentication-authorization#use-openid-connect) authentication mechanisms. This provides advanced notification that these authentication options will be deprecated in version 5.7.0. Users are advised to migrate to the [JWT Auth](/5.6/api-management/authentication-authorization#use-json-web-tokens-jwt) method, which supports integration with both OAuth and OIDC providers, in preparation for future upgrade.
</Expandable>
</li>
</ul>
...
<h3 id="dependencies-5.3.7">Dependencies</h3>
#### Compatibility Matrix For Tyk Components
{/* Required. Version compatibility with other components in the Tyk stack. This takes the form of a compatibility matrix and is only required for Gateway and Portal.
An illustrative example is shown below. */}
| Dashboard Version | Recommended Releases | Backwards Compatibility |
...
<h4 id="3rdPartyTools-v5.3.7">3rd Party Dependencies & Tools</h4>
{/* Required. Third-party dependencies encompass tools (GoLang, Helm etc.), databases (PostgreSQL, MongoDB etc.) and external software libraries. This section should be a table that presents the third-party dependencies and tools compatible with the release. Compatible is used in the sense of those versions tested with the releases. Such information assists customers considering upgrading to a specific release.
Additionally, a disclaimer statement was added below the table, for customers to check that the third-party dependency they decide to install remains in support.
An example is given below for illustrative purposes only. Tested Versions and Compatible Versions information will require discussion with relevant squads and QA. */}
...
<h3 id="upgrade-5.3.7">Upgrade instructions</h3>
If you are upgrading to 5.3.7, please follow the detailed [upgrade instructions](/5.6/#upgrading-tyk). 
...
<h3 id="Changelog-v5.3.7">Changelog</h3>
...
This release primarily focuses on bug fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.6/#Changelog-v5.3.6) below.
...
If moving from a version of Tyk older than 5.3.0 please read the explanation provided with [5.3.0 release](/5.6/#TykOAS-v5.3.0).

### Deprecations
There are no deprecations in this release.

### Upgrade Instructions
When upgrading to 5.3.6, please follow the [detailed upgrade instructions](/5.6/#upgrading-tyk).

<h3 id="dependencies-5.3.6">Dependencies</h3>
{/* Required. Use this section to announce the following types of dependencies compatible with the release:
Version compatibility with other components in the Tyk stack. This takes the form of a compatibility matrix and is only required for Gateway and Portal.
3rd party dependencies and tools */}

With MongoDB 4.4 reaching [EOL](https://www.mongodb.com/legal/support-policy/lifecycles) in February 2024, we can no longer guarantee full compatibility with this version of the database. If you are [using MongoDB](/5.6/planning-for-production/database-settings/mongodb) we recommend that you upgrade to a version that we have tested with, as indicated [below](/5.6/#3rdPartyTools-v5.3.6).


With PostgreSQL v11 reaching [EOL](https://www.postgresql.org/support/versioning/) in November 2023, we can no longer guarantee full compatibility with this version of the database. If you are [using PostgreSQL](/5.6/planning-for-production/database-settings/postgresql) we recommend that you upgrade to a version that we have tested with, as indicated [below](/5.6/#3rdPartyTools-v5.3.6).
...
<h4 id="3rdPartyTools-v5.3.6">3rd Party Dependencies & Tools</h4>
{/* Required. Third-party dependencies encompass tools (GoLang, Helm etc.), databases (PostgreSQL, MongoDB etc.) and external software libraries. This section should be a table that presents the third-party dependencies and tools compatible with the release. Compatible is used in the sense of those versions tested with the releases. Such information assists customers considering upgrading to a specific release.
Additionally, a disclaimer statement was added below the table, for customers to check that the third-party dependency they decide to install remains in support.
An example is given below for illustrative purposes only. Tested Versions and Compatible Versions information will require discussion with relevant squads and QA. */}
...
- [Docker Image to pull](https://hub.docker.com/r/tykio/tyk-dashboard/tags?page=&page_size=&ordering=&name=v5.3.6)
 - ```bash
   ...
   ```
- Helm charts
 - [tyk-charts v2.0](/5.6/product-stack/tyk-charts/release-notes/version-2.0)

<h3 id="Changelog-v5.3.6">Changelog</h3>
{/* Required. The change log should include the following ordered set of sections below that briefly summarise the features, updates and fixed issues of the release.
Here it is important to explain the benefit of each changelog item. As mentioned by James in a previous Slack message (https://tyktech.slack.com/archives/C044R3ZTN6L/p1686812207060839?thread_ts=1686762128.651249&cid=C044R3ZTN6L):
"...it is important to document the customer impact for the work delivered, so we can share it with prospects/install base. For example:
"New Chart delivers x and y benefit to a and b customer use cases. The business impact for them will be this and that" */}
...
There are no breaking changes in this release, however, if moving from a version of Tyk older than 5.3.0 please read the explanation provided with [5.3.0 release](/5.6/#TykOAS-v5.3.0).
...
When upgrading to 5.3.5, please follow the [detailed upgrade instructions](/5.6/#upgrading-tyk).


<h3 id="dependencies-5.3.5">Dependencies</h3>
{/* Required. Use this section to announce the following types of dependencies compatible with the release:
Version compatibility with other components in the Tyk stack. This takes the form of a compatibility matrix and is only required for Gateway and Portal.
3rd party dependencies and tools */}

With MongoDB 4.4 reaching [EOL](https://www.mongodb.com/legal/support-policy/lifecycles) in February 2024, we can no longer guarantee full compatibility with this version of the database. If you are [using MongoDB](/5.6/planning-for-production/database-settings/mongodb) we recommend that you upgrade to a version that we have tested with, as indicated [below](/5.6/#3rdPartyTools-v5.3.5).
...
<h4 id="3rdPartyTools-v5.3.5">3rd Party Dependencies & Tools</h4>
{/* Required. Third-party dependencies encompass tools (GoLang, Helm etc.), databases (PostgreSQL, MongoDB etc.) and external software libraries. This section should be a table that presents the third-party dependencies and tools compatible with the release. Compatible is used in the sense of those versions tested with the releases. Such information assists customers considering upgrading to a specific release.
Additionally, a disclaimer statement was added below the table, for customers to check that the third-party dependency they decide to install remains in support.
An example is given below for illustrative purposes only. Tested Versions and Compatible Versions information will require discussion with relevant squads and QA. */}
...
<h3 id="Changelog-v5.3.5">Changelog</h3>
 No changes in this release.
...
### Upgrade Instructions
When upgrading to 5.3.4 please follow the [detailed upgrade instructions](/5.6/#upgrading-tyk).


### Release Highlights
For a comprehensive list of changes, please refer to the detailed [changelog](/5.6/#Changelog-v5.3.4) below.

<h3 id="dependencies-5.3.0">Dependencies</h3>
{/* Required. Use this section to announce the following types of dependencies compatible with the release:
Version compatibility with other components in the Tyk stack. This takes the form of a compatibility matrix and is only required for Gateway and Portal.
3rd party dependencies and tools */}

With MongoDB 4.4 reaching [EOL](https://www.mongodb.com/legal/support-policy/lifecycles) in February 2024, we can no longer guarantee full compatibility with this version of the database. If you are [using MongoDB](/5.6/planning-for-production/database-settings/mongodb) we recommend that you upgrade to a version that we have tested with, as indicated [below](/5.6/#3rdPartyTools-v5.3.3).
...
<h4 id="3rdPartyTools-v5.3.4">3rd Party Dependencies & Tools</h4>
{/* Required. Third-party dependencies encompass tools (GoLang, Helm etc.), databases (PostgreSQL, MongoDB etc.) and external software libraries. This section should be a table that presents the third-party dependencies and tools compatible with the release. Compatible is used in the sense of those versions tested with the releases. Such information assists customers considering upgrading to a specific release.
Additionally, a disclaimer statement was added below the table, for customers to check that the third-party dependency they decide to install remains in support.
An example is given below for illustrative purposes only. Tested Versions and Compatible Versions information will require discussion with relevant squads and QA. */}
...
- [Docker Image to pull](https://hub.docker.com/r/tykio/tyk-dashboard/tags?page=&page_size=&ordering=&name=v5.3.4)
 - ```bash
   ...
   ```
- Helm charts
 - [tyk-charts v1.4](/5.6/product-stack/tyk-charts/release-notes/version-1.4)

<h3 id="Changelog-v5.3.4">Changelog</h3>
{/* Required. The change log should include the following ordered set of sections below that briefly summarise the features, updates and fixed issues of the release.
Here it is important to explain the benefit of each changelog item. As mentioned by James in a previous Slack message (https://tyktech.slack.com/archives/C044R3ZTN6L/p1686812207060839?thread_ts=1686762128.651249&cid=C044R3ZTN6L):
"...it is important to document the customer impact for the work delivered, so we can share it with prospects/install base. For example:
"New Chart delivers x and y benefit to a and b customer use cases. The business impact for them will be this and that" */}
...
### Upgrade Instructions
When upgrading to 5.3.3 please follow the [detailed upgrade instructions](/5.6/#upgrading-tyk).

### Release Highlights

For a comprehensive list of changes, please refer to the detailed [changelog](/5.6/#Changelog-v5.3.3) below.
...
<h4 id="3rdPartyTools-v5.3.3">3rd Party Dependencies & Tools</h4>
{/* Required. Third-party dependencies encompass tools (GoLang, Helm etc.), databases (PostgreSQL, MongoDB etc.) and external software libraries. This section should be a table that presents the third-party dependencies and tools compatible with the release. Compatible is used in the sense of those versions tested with the releases. Such information assists customers considering upgrading to a specific release.
Additionally, a disclaimer statement was added below the table, for customers to check that the third-party dependency they decide to install remains in support.
An example is given below for illustrative purposes only. Tested Versions and Compatible Versions information will require discussion with relevant squads and QA. */}
...
<h3 id="Changelog-v5.3.3">Changelog</h3>
{/* Required. The change log should include the following ordered set of sections below that briefly summarise the features, updates and fixed issues of the release.
Here it is important to explain the benefit of each changelog item. As mentioned by James in a previous Slack message (https://tyktech.slack.com/archives/C044R3ZTN6L/p1686812207060839?thread_ts=1686762128.651249&cid=C044R3ZTN6L):
"...it is important to document the customer impact for the work delivered, so we can share it with prospects/install base. For example:
"New Chart delivers x and y benefit to a and b customer use cases. The business impact for them will be this and that" */}
...
### Upgrade Instructions
When upgrading to 5.3.2 please follow the [detailed upgrade instructions](/5.6/#upgrading-tyk).


### Release Highlights
This release primarily focuses on bug fixes.
For a comprehensive list of changes, please refer to the detailed [changelog](/5.6/#Changelog-v5.3.2) below.
...
With MongoDB 4.4 reaching [EOL](https://www.mongodb.com/legal/support-policy/lifecycles) in February 2024, we can no longer guarantee full compatibility with this version of the database. If you are [using MongoDB](/5.6/planning-for-production/database-settings/mongodb) we recommend that you upgrade to a version that we have tested with, as indicated [below](/5.6/#3rdPartyTools-v5.3.2).
...
<h4 id="3rdPartyTools-v5.3.2">3rd Party Dependencies & Tools</h4>
{/* Required. Third-party dependencies encompass tools (GoLang, Helm etc.), databases (PostgreSQL, MongoDB etc.) and external software libraries. This section should be a table that presents the third-party dependencies and tools compatible with the release. Compatible is used in the sense of those versions tested with the releases. Such information assists customers considering upgrading to a specific release.
...
<h3 id="Changelog-v5.3.2">Changelog</h3>
{/* Required. The change log should include the following ordered set of sections below that briefly summarise the features, updates and fixed issues of the release.
Here it is important to explain the benefit of each changelog item. As mentioned by James in a previous Slack message (https://tyktech.slack.com/archives/C044R3ZTN6L/p1686812207060839?thread_ts=1686762128.651249&cid=C044R3ZTN6L):
"...it is important to document the customer impact for the work delivered, so we can share it with prospects/install base. For example:
"New Chart delivers x and y benefit to a and b customer use cases. The business impact for them will be this and that" */}
...
### Upgrade Instructions
When upgrading to 5.3.1, please follow the [detailed upgrade instructions](/5.6/#upgrading-tyk).


### Release Highlights
This release primarily focuses on bug fixes.
For a comprehensive list of changes, please refer to the detailed [changelog](/5.6/#Changelog-v5.3.1) below.
...
With MongoDB 4.4 reaching [EOL](https://www.mongodb.com/legal/support-policy/lifecycles) in February 2024, we can no longer guarantee full compatibility with this version of the database. If you are [using MongoDB](/5.6/planning-for-production/database-settings/mongodb) we recommend that you upgrade to a version that we have tested with, as indicated [below](/5.6/#3rdPartyTools-v5.3.1).
...
<h4 id="3rdPartyTools-v5.3.1">3rd Party Dependencies & Tools</h4>
{/* Required. Third-party dependencies encompass tools (GoLang, Helm etc.), databases (PostgreSQL, MongoDB etc.) and external software libraries. This section should be a table that presents the third-party dependencies and tools compatible with the release. Compatible is used in the sense of those versions tested with the releases. Such information assists customers considering upgrading to a specific release.
...
- ```bash
  ...
  ```
- Helm charts
  - [tyk-charts v1.3](/5.6/product-stack/tyk-charts/release-notes/version-1.3)

<h3 id="Changelog-v5.3.1">Changelog</h3>
#### Fixed
...
<h4 id="TykOAS-v5.3.0">Tyk OAS APIs Compatibility Caveats</h4>
This upgrade transitions Tyk OAS APIs out of [Early Access](/5.6/developer-support/special-releases-and-features/early-access-features).
...
- **Not Backwards Compatible**
  ...
  - This means that if you wish to downgrade or revert to your previous version of Tyk, you will need to restore these API definitions from a backup. Please go to the [backup](/5.6/#upgrade-instructions) section for detailed instructions on backup before upgrading to v5.3.0.
...
- **Not Forward Compatible**
  ...
  - This means that any Tyk OAS APIs created in any previous release (4.1.0-5.2.x) cannot work with the new Tyk Dashboard v5.3.X without being migrated to its [latest format](/5.6/tyk-apis/tyk-gateway-api/oas/x-tyk-oas-doc).
...
- **After upgrade (the good news)**
  - If you had a Tyk OAS API prior to v5.3.0 then Tyk Dashboard will automatically update the API definition to [latest format](/5.6/tyk-apis/tyk-gateway-api/oas/x-tyk-oas-doc).
...
**Important:** Please go to the [backup](/5.6/#upgrade-instructions) section for essential instructions on how to backup before upgrading to v5.3.0
...
With MongoDB 4.4 reaching [EOL](https://www.mongodb.com/legal/support-policy/lifecycles) in February 2024, we can no longer guarantee full compatibility with this version of the database and recommend upgrading to a version that we have tested with, as indicated [below](/5.6/#3rdPartyTools-v5.3.0).
...
<h4 id="3rdPartyTools-v5.3.0">3rd Party Dependencies & Tools</h4>
{/* Required. Third-party dependencies encompass tools (GoLang, Helm etc.), databases (PostgreSQL, MongoDB etc.) and external software libraries. This section should be a table that presents the third-party dependencies and tools compatible with the release. Compatible is used in the sense of those versions tested with the releases. Such information assists customers considering upgrading to a specific release.
...
<h3 id="upgrade-5.3.0">Upgrade Instructions</h3>
**The following steps are essential to follow before upgrading**

1. For Self Managed deployments - Backup Your environment using the [usual guidance](/5.6/upgrading-tyk#tyk-self-managed) documented with every release (this includes backup config file and database).
2. For all deployments - Backup all your API definitions (Tyk OAS API and Classic Definitions):
   - For Tyk Cloud deployments - To perform the backup please use our guide for [exporting APIs and policies](/5.6/developer-support/backups/backup-apis-and-policies).
   - For Self-Managed deployments -  To perform the backup please use [Tyk Sync](/5.6/api-management/automations#synchronize-tyk-environment-with-github-repository).
4. Performing the upgrade - For all deployments, follow the instructions in the [upgrade guide](/5.6/#upgrading-tyk) when upgrading Tyk.

### Release Highlights

We are excited to announce the release of 5.3.0, packed with new features, improvements and bug fixes to enhance your experience with Tyk Dashboard. For a comprehensive list of changes, please refer to the detailed [changelog](/5.6/#Changelog-v5.3.0) below.

#### Tyk OAS Feature Maturity

Tyk OAS is now out of [Early Access](/5.6/developer-support/special-releases-and-features/early-access-features) as we have reached feature maturity. You are now able to make use of the majority of Tyk's features from your Tyk OAS APIs, so they are a credible alternative to the legacy Tyk Classic APIs.
...
<h3 id="Changelog-v5.3.0">Changelog</h3>
{/* Required. The change log should include the following ordered set of sections below that briefly summarise the features, updates and fixed issues of the release.
...
Relaxed the strict validation for mock response so that the `Description` field is now optional for `response`, `responses` and `schema` within the OpenAPI description. Automatically configuring mock responses when using [Tyk OAS APIs](/5.6/product-stack/tyk-gateway/middleware/mock-response-openapi) is now even easier.
</Accordion>
