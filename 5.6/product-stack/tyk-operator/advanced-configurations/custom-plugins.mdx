...
each of the following [hooks](/5.6/plugins/plugin-types/plugintypes#plugin-and-hook-types) in the API request
lifecycle: [Pre (Request)](/5.6/plugins/plugin-types/request-plugins), [Authentication](/5.6/plugins/plugin-types/auth-plugins/auth-plugins),
[Post (Request)](/5.6/plugins/plugin-types/request-plugins), [Post
Authentication](/5.6/plugins/plugin-types/request-plugins), [Response](/5.6/plugins/plugin-types/response-plugins)
and [Analytics](/5.6/plugins/plugin-types/analytics-plugins). Subsequently, users can execute, or “hook”, their
...
Please refer to [Analytics Plugins](/5.6/plugins/plugin-types/analytics-plugins) to learn how to configure Analytics
...
languages](/5.6/plugins/supported-languages#plugin-driver-names) section for list of supported plugin driver names.
...
[per-endpoint](/5.6/product-stack/tyk-gateway/middleware/endpoint-plugin) level plugins are not currently
...
This will be configured as a pre request [hook](/5.6/#).
...
- The `driver` configuration parameter is set to `otto` at line 15, since our plugin is a Javascript plugin. For other
  valid values please refer to the [plugins driver page](/5.6/plugins/supported-languages#plugin-driver-names).
- A plugin hook configuration block is specified at line 16, containing the `name` and `path` for our plugin. The plugin
  ...
  middleware. Valid values are the same as the [Tyk Classic API Definition](/5.6/#tyk-classic) equivalent, i.e.
...
request lifecycle](/5.6/concepts/middleware-execution-order).
...
bundles](/5.6/plugins/how-to-serve-plugins/plugin-bundles), load them into the Tyk Gateway, and configure an API
