...
Bloblang is available as a [processor](/5.6/product-stack/tyk-streaming/configuration/processors/bloblang) and it's also possible to use Bloblang queries in [function interpolations](/5.6/product-stack/tyk-streaming/configuration/common-configuration/interpolation).

This document outlines the core features of the [Bloblang language](/5.6/product-stack/tyk-streaming/guides/bloblang/methods/overview).
...
Bloblang is able to map data that is unstructured, whether it's a log line or a binary blob, by referencing it with the [content function](/5.6/product-stack/tyk-streaming/guides/bloblang/functions#content), which returns the raw bytes of the input document:
...
For more information about these operators and how they work check out the [arithmetic page](/5.6/product-stack/tyk-streaming/guides/bloblang/arithmetic).
...
You can find a full list of functions and their parameters in [the functions page](/5.6/product-stack/tyk-streaming/guides/bloblang/functions).
...
You can find a full list of methods and their parameters in the [methods page](/5.6/product-stack/tyk-streaming/guides/bloblang/methods/overview).

## Maps

Defining named maps allows you to reuse common mappings on values with the [apply method](/5.6/product-stack/tyk-streaming/guides/bloblang/methods/overview#apply):
...
Functions and methods can fail under certain circumstances, such as when they receive types they aren't able to act upon. These failures, when not caught, will cause the entire mapping to fail. However, the method [catch](/5.6/product-stack/tyk-streaming/guides/bloblang/methods/overview#catch) can be used in order to return a value when a failure occurs instead:
...
However, the `catch` method only acts on errors, sometimes it's also useful to set a fall back value when a query returns `null` in which case the method [or](/5.6/product-stack/tyk-streaming/guides/bloblang/methods/overview#or) can be used the same way:
