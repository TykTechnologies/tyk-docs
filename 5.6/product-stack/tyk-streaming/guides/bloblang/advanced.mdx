...
This section explains some advanced [Bloblang](/5.6/product-stack/tyk-streaming/guides/bloblang/overview) patterns
...
We can pull `items` out to the root with `root = items` with a [mapping processor](/5.6/product-stack/tyk-streaming/configuration/processors/mapping) and follow it with an unarchive processor to expand each element into its own independent message:
...
Tyk Streams has a few different ways of outputting a stream of CSV data. However, the best way to do it is by converting the documents into CSV rows with [Bloblang](/5.6/product-stack/tyk-streaming/guides/bloblang/overview) as this gives you full control over exactly how the schema is generated, erroneous data is handled, and escaping of column data is performed.
...
With this mapping we can write the data to an output such as [kafka](/5.6/product-stack/tyk-streaming/configuration/outputs/kafka), [kafka_franz](/5.6/product-stack/tyk-streaming/configuration/outputs/kafka-franz) or [redis_pubsub](/5.6/product-stack/tyk-streaming/configuration/outputs/redis-pubsub)
