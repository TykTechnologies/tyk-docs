...
For example, HTTP is a request/response protocol, and so our [http_server](/5.6/product-stack/tyk-streaming/configuration/inputs/http-server) input is capable of returning a response payload after consuming a message from a request.
...
It's possible to route the result of any Tyk Streams processing pipeline directly back to an input with a [sync_response](/5.6/product-stack/tyk-streaming/configuration/outputs/sync-response) output:
...
It's also possible to combine a [sync_response](/5.6/product-stack/tyk-streaming/configuration/outputs/sync-response) output with other outputs using a [broker](/5.6/product-stack/tyk-streaming/configuration/outputs/broker):
...
Some outputs, such as [http_client](/5.6/product-stack/tyk-streaming/configuration/outputs/http-client), have the potential to propagate payloads received from their destination after sending a message back to the input:
