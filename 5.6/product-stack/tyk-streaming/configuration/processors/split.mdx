---
title: "Split"
'og:description': "Explains an overview of split processor"
sidebarTitle: "Split"
tags: ['Tyk Streams', 'Stream Processors', 'Processors', 'Split']
---

Breaks message batches (synonymous with multiple part messages) into smaller batches. The size of the resulting batches are determined either by a discrete size or, if the field [byte_size](/5.6/#byte_size) is non-zero, then by total size in bytes (which ever limit is reached first).
...
In order to break a single message out into multiple messages use the [`unarchive` processor](/5.6/docs/components/processors/unarchive). */}
