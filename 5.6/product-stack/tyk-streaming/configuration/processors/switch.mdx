...
For each switch case a [Bloblang query](/5.6/product-stack/tyk-streaming/guides/bloblang/overview) is checked and, if the result is true (or the check is empty) the child processors are executed on the message.

## Fields

### [].check

A [Bloblang query](/5.6/product-stack/tyk-streaming/guides/bloblang/overview) that should return a boolean value indicating whether a message should have the processors of this case executed on it. If left empty the case always passes. If the check mapping throws an error the message will be flagged [as having failed](/5.6/product-stack/tyk-streaming/configuration/common-configuration/error-handling) and will not be tested against any other cases.
