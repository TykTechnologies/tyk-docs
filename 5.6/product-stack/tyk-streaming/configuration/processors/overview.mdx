...
Tyk Streams processors are functions applied to messages passing through a pipeline. The function signature allows a processor to mutate or drop messages depending on the content of the message. There are many types on offer but the most powerful are the [mapping](/5.6/product-stack/tyk-streaming/configuration/processors/mapping) and [mutation](/5.6/product-stack/tyk-streaming/configuration/processors/mutation) processors.
...
The `threads` field in the pipeline section determines how many parallel processing threads are created. You can read more about parallel processing in the [pipeline guide](/5.6/product-stack/tyk-streaming/configuration/common-configuration/processing-pipelines).
...
The reason for this behavior is to prevent noisy logging in cases where logical errors are expected and will likely be [handled in config](/5.6/product-stack/tyk-streaming/configuration/common-configuration/error-handling). However, this can also sometimes make it easy to miss logical errors in your configs when they lack error handling. 
...
It might be the case that a processor that results in a side effect, such as the [redis](/5.6/product-stack/tyk-streaming/configuration/processors/redis) processor, is the only side effect of a pipeline, and therefore could be considered the output.

In such cases it's possible to place these processors within a [reject](/5.6/product-stack/tyk-streaming/configuration/outputs/reject) output so that they behave the same as regular outputs, where success results in dropping the message with an acknowledgement and failure results in a nack (or retry):
...
Some processors such as [dedupe](/5.6/product-stack/tyk-streaming/configuration/processors/dedupe) act across an entire batch, when instead we might like to perform them on individual messages of a [batch](/5.6/product-stack/tyk-streaming/configuration/common-configuration/batching). In this case the [for_each](/5.6/product-stack/tyk-streaming/configuration/processors/for-each) processor can be used.
