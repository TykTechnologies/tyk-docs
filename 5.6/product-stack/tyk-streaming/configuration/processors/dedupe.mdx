...
Caches must be configured as resources, for more information check out the [cache documentation here](/5.6/product-stack/tyk-streaming/configuration/processors/cache).

When using this processor with an output target that might fail you should always wrap the output within an indefinite [retry](/5.6/product-stack/tyk-streaming/configuration/outputs/retry) block. This ensures that during outages your messages aren't reprocessed after failures, which would result in messages being dropped.

## Batch Deduplication

This processor enacts on individual messages only, in order to perform a deduplication on behalf of a batch (or window) of messages instead use the [cache processor](/5.6/product-stack/tyk-streaming/configuration/processors/cache#examples).
...
This field supports [interpolation functions](/5.6/#bloblang-queries).
