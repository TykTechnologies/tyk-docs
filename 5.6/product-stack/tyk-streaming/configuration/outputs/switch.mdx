---
title: "Switch"
'og:description': "Explains an overview of configuring switch output"
sidebarTitle: "Switch"
...
Messages that do not pass the check of a single output case are effectively dropped. In order to prevent this outcome set the field [strict_mode](/5.6/#strict_mode) to `true`, in which case messages that do not pass at least one case are considered failed and will be nacked and/or reprocessed depending on your input.
...
A [Bloblang query](/5.6/product-stack/tyk-streaming/guides/bloblang/overview) that should return a boolean value indicating whether a message should be routed to the case output. If left empty the case always passes.
...
An [output](/5.6/product-stack/tyk-streaming/configuration/outputs/overview) for messages that pass the check to be routed to.
