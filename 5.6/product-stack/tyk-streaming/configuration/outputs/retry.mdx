...
Rather than retrying the same output you may wish to retry the send using a different output target (a dead letter queue). In which case you should instead use the [fallback](/5.6/product-stack/tyk-streaming/configuration/outputs/retry) output type.
