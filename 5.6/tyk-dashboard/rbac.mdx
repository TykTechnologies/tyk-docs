...
You can create additional "users" for your colleagues who need to access Dashboard features. These additional users can be assigned [granular permissions](/5.6/basic-config-and-security/security/dashboard/user-roles) so that they only have access to the Dashboard pages (and corresponding Dashboard API endpoints) that they require. For each permission (or "role") you can "deny access", make it "read only", or allow "write access".
...
You can integrate your existing identity management server with the Tyk Dashboard, as explained in our detailed [Single Sign-On (SSO) guide](/5.6/advanced-configuration/integrate/sso). **This functionality is available with all Tyk licenses except Tyk Classic Cloud.**

By default all users who login via SSO are granted admin permissions. You can change this behavior by setting either default permissions for *[users](/5.6/basic-config-and-security/security/dashboard/create-users)* or by creating a default *[user group](/5.6/basic-config-and-security/security/dashboard/create-user-groups)* to which all new users are assigned. With some IDPs you can automatically assign different SSO users to different *user groups* by dynamically mapping the IDP's user groups, for example with [Azure AD](/5.6/tyk-stack/tyk-manager/sso/dashboard-login-azure-sso#user-group-mapping).
