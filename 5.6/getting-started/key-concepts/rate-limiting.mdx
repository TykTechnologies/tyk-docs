...
These guides include explanation of how to configure key-level rate limits when using [API Keys](/5.6/getting-started/create-api-key) and [Security Policies](/5.6/getting-started/create-security-policy).
...
1. [Distributed Rate Limiter](/5.6/#distributed-rate-limiter): recommended for most use cases, implements the [token bucket algorithm](https://en.wikipedia.org/wiki/Token_bucket)
2. [Redis Rate Limiter](/5.6/#redis-rate-limiter): implements the [sliding window log algorithm](https://developer.redis.com/develop/dotnet/aspnetcore/rate-limiting/sliding-window)
3. [Fixed Window Rate Limiter](/5.6/#fixed-window-rate-limiter): implements the [fixed window algorithm](https://redis.io/learn/develop/dotnet/aspnetcore/rate-limiting/fixed-window)
...
It can be configured to switch dynamically between two algorithms depending on the request rate, as explained [here](/5.6/#dynamic-algorithm-selection-based-on-request-rate).
</Note>
...
This algorithm implements a sliding window log algorithm and can be enabled via the [enable_redis_rolling_limiter](/5.6/tyk-oss-gateway/configuration.md#enable_redis_rolling_limiter) configuration option.
...
If you wish to avoid spike arrest behavior but the DRL is not suitable, you might use the [Fixed Window Rate Limiter](/5.6/#fixed-window-rate-limiter) algorithm.

You can configure [Rate Limit Smoothing](/5.6/#rate-limit-smoothing) to manage the traffic spike, allowing time to increase upstream capacity if required.

The [Redis Sentinel Rate Limiter](/5.6/#redis-sentinel-rate-limiter) reduces latency for clients, however increases resource usage on Redis and Tyk Gateway.
...
[enable_sentinel_rate_limiter](/5.6/tyk-oss-gateway/configuration.md#enable_sentinel_rate_limiter).
...
Performance can be improved by enabling the [enable_non_transactional_rate_limiter](/5.6/tyk-oss-gateway/configuration.md#enable_non_transactional_rate_limiter). This leverages Redis Pipelining to enhance the performance of the Redis operations. Please consult the [Redis documentation](https://redis.io/docs/manual/pipelining/) for more information.

Please consider the [Fixed Window Rate Limiter](/5.6/#fixed-window-rate-limiter) algorithm as an alternative, if Redis performance is an issue.
...
This algorithm can be enabled using the following configuration option [enable_fixed_window_rate_limiter](/5.6/tyk-oss-gateway/configuration.md#enable_fixed_window_rate_limiter).

If you need spike arrest behavior, the [Redis Rate Limiter](/5.6/#redis-rate-limiter) should be used.
...
See [DRL Threshold](/5.6/tyk-oss-gateway/configuration.md#drl_threshold) for details on how to configure this feature.
