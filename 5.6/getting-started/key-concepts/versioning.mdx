...
If you're using the newer Tyk OAS APIs, then check out the [Tyk OAS](/5.6/getting-started/key-concepts/oas-versioning) page.

If you're using Tyk Operator then check out the [configuring API versioning in Tyk Operator](/5.6/#tyk-operator) section below.

### Controlling access to Tyk Classic API versions

You can explicitly grant access to specific version(s) of an API by specifying only those version(s) in the [key](/5.6/tyk-apis/tyk-gateway-api/token-session-object-details) (also known as an *authorization token*, *bearer token*, *access token*, *API token* or *token session object* - see [here](/5.6/api-management/authentication-authorization#use-bearer-tokens)).
...
- `default_version`: this must contain the `name` of the version that shall be treated as `default` (for [access control](/5.6/#controlling-access-to-tyk-classic-api-versions) and [default fallback](/5.6/product-stack/tyk-gateway/advanced-configurations/api-versioning/api-versioning#default-api-version))
...
- `paths`: location for configuration of endpoint [ignore](/5.6/product-stack/tyk-gateway/middleware/ignore-middleware), [allow](/5.6/product-stack/tyk-gateway/middleware/allow-list-middleware) and [block](/5.6/product-stack/tyk-gateway/middleware/block-list-middleware) lists
- `use_extended_paths`: set to `true` to enable the `extended_paths` config
- `extended_paths`: location for configuration of additional [endpoint-level middleware](/5.6/advanced-configuration/transform-traffic)
- `global_*`: configuration of [API-level middleware](/5.6/advanced-configuration/transform-traffic). The wildcard can be replaced by any of the API-level settings e.g. `global_size_limit`
- `override_target`: alternative upstream (target) URL that should be used for this version, overriding the `target_url` configured in the `proxy` [section](/5.6/tyk-apis/tyk-gateway-api/api-definition-objects/proxy-settings#proxytarget_url) of the API definition; this can be used to redirect to a different hostname or domain if required
...
- `strip_versioning_data`: set this to `true` to [remove the versioning identifier](/5.6/product-stack/tyk-gateway/advanced-configurations/api-versioning/api-versioning#stripping-version-identifier) prior to creating the upstream (target) URL)
- `fallback_to_default`: set this to `true` to [invoke the default version](/5.6/product-stack/tyk-gateway/advanced-configurations/api-versioning/api-versioning#fallback-to-default) if an invalid version is requested
- `url_versioning_pattern`: if you are using `strip_versioning_data` and `fallback_to_default` with `location=url` [with Tyk 5.5.0 or later](/5.6/product-stack/tyk-gateway/advanced-configurations/api-versioning/api-versioning#stripping-url-path-version-and-default-fallback) you can configure this with a regex that matches the format that you use for the versioning identifier (`versions.name`)
...
You can choose any of your API versions to act as the [default](/5.6/product-stack/tyk-gateway/advanced-configurations/api-versioning/api-versioning#default-api-version).
...
As [explained](/5.6/product-stack/tyk-gateway/advanced-configurations/api-versioning/api-versioning#comparison-between-tyk-oas-and-tyk-classic-api-versioning) much of the Tyk Classic API definition is shared between the different versions, but some middleware can be configured differently.
...
The process for configuring API versioning is similar to that defined in section [Configuring API versioning in the Tyk Classic API Definition](/5.6/#configuring-api-versioning-in-the-tyk-classic-api-definition).
