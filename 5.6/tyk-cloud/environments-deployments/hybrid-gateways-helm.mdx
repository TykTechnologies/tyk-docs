...
'og:description': "How to deploy Hybrid Data Plane on Kubernetes using legacy Helm Chart"
sidebarTitle: "Deploy hybrid gateways using legacy helm chart"
...
<Warning>
`tyk-hybrid` chart is deprecated. Please use our [Tyk Data Plane helm chart](/5.6/tyk-cloud/environments-deployments/hybrid-gateways#deploy-in-kubernetes-with-helm-chart) instead. 

We recommend that all users to migrate to the `tyk-data-plane` Chart. Please review the [Configuration](/5.6/product-stack/tyk-charts/tyk-data-plane-chart#configuration) section of the new helm chart and cross-check with your existing configurations while planning for migration.
</Warning>
...
Now it is time to publish a new API [Task 5 - Deploy your Cloud Data Plane and add your first API](/5.6/tyk-cloud/getting-started-tyk-cloud/first-api.md)
