...
date: 2025-01-10
tags: ["plugins", "javascript", "goja", "jsvm", "custom middleware", "virtual endpoints", "inline code"]
...
keywords: ["javascript plugins", "goja", "jsvm", "enable_jsvm", "custom middleware", "inline code", "virtual endpoints", "tyk gateway"]
aliases:
  ...
  - /api-management/plugins/goja-inline
---
...
2. **Dynamic event handlers**: components that fire on API events. They run asynchronously and are documented in [Gateway events]({{< ref "api-management/gateway-events#set-up-a-webhook-event-handler-in-the-tyk-oas-api-definition" >}}).
3. **Virtual endpoints**: programmable middleware invoked towards the end of the request chain. Unlike custom plugins, a [virtual endpoint]({{< ref "api-management/traffic-transformation/virtual-endpoints" >}}) *terminates* the request and returns its own response.
...
Tyk Gateway runs JavaScript on [goja](https://github.com/dop251/goja), a pure-Go ECMAScript engine embedded in the gateway binary. goja is **ES5.1-complete and supports much of ES6+** — including `let`/`const`, arrow functions, template literals, destructuring and default parameters. (Earlier gateway releases ran JavaScript on [otto](https://github.com/robertkrimen/otto), which was limited to ES5; otto is still selectable for backward compatibility — see [Selecting the engine](#selecting-the-engine).)
...
The JavaScript Virtual Machine (JSVM) is disabled by default. Enable it by setting [`enable_jsvm`]({{< ref "tyk-oss-gateway/configuration#enable_jsvm" >}}) to `true` in `tyk.conf`. This single flag enables JavaScript for custom middleware, event handlers and virtual endpoints alike.
...
Each plugin entry attaches to exactly one hook. For a fuller treatment of the request chain and what each stage can do, see [Plugin types]({{< ref "api-management/plugins/plugin-types" >}}). Virtual endpoints are documented separately under [Virtual endpoints]({{< ref "api-management/traffic-transformation/virtual-endpoints" >}}).
...
See the [virtual endpoint examples]({{< ref "api-management/traffic-transformation/virtual-endpoints#examples" >}}) for complete handlers.

## The request, session and config objects

Your function receives three Tyk data objects. This section covers their behaviour and the fields you'll use most; for the full struct shapes, see the [data structures in Rich Plugins]({{< ref "api-management/plugins/rich-plugins#rich-plugins-data-structures" >}}).
...
[Virtual endpoint]({{< ref "api-management/traffic-transformation/virtual-endpoints" >}}) functions receive this request shape instead:
...
- `TykBatchRequest(...)`: like `TykMakeHttpRequest` but uses Tyk's [batch request feature]({{< ref "api-management/batch-processing" >}}).
...
Two functions read and write the [session object]({{< ref "api-management/policies#what-is-a-session-object" >}}) for a key:
    ...
    global:
      ...
      prePlugins:
        - enabled: true
          ...
          code: dmFyIGhhbmRsZXIgPSBuZXcgVHlrSlMuVHlrTWlkZGxld2FyZS5OZXdNaWRkbGV3YXJlKHt9KTsK # base64 of your plugin.js
          rawBodyOnly: false
...
Virtual endpoints honour the same `driver` flag as custom middleware. A virtual endpoint runs on goja when its API sets the driver to `javascript`; with no driver set it runs on the legacy otto engine. Set the driver explicitly to move virtual endpoints onto goja. See [Virtual endpoints]({{< ref "api-management/traffic-transformation/virtual-endpoints" >}}) for configuration.
...
- [Plugin types]({{< ref "api-management/plugins/plugin-types" >}}) — hook stages and capabilities
- [Rich plugins]({{< ref "api-management/plugins/rich-plugins" >}}) — plugin data structures and bundles
- [Virtual endpoints]({{< ref "api-management/traffic-transformation/virtual-endpoints" >}}) — programmable terminating middleware
- [Go plugins]({{< ref "api-management/plugins/golang" >}}) — native plugins for higher performance
