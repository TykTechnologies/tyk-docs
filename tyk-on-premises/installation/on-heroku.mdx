...
The [Dashboard configuration](/tyk-dashboard/configuration) can be changed by either editing the `tyk_analytics.conf` file or injecting them as [environment variables](/tyk-environment-variables) via `heroku config`. In this guide we'll use the latter for simplicity of demonstration but there is merit to both methods.
...
All these files serve the same purpose as with the Dasboard and the Pump. [Configuration](/tyk-oss-gateway/configuration) can either be edited in `tyk.conf` or [injected](/tyk-environment-variables) with `heroku config`.
...
In order to enable [rich plugins](/plugins/supported-languages/rich-plugins) for the Gateway, please set the following Heroku config option to either `python` or `lua` depending on the type of plugins used:
```{.copyWrapper}
