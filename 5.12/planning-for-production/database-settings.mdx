...
import RedisCalculator from '/snippets/5.12/RedisCalculator.mdx';
import DatabaseCalculator from '/snippets/5.12/DatabaseCalculator.mdx';

Visit the following sections to see how to configure the Database for Production:
* [Redis](/5.12/planning-for-production/database-settings#redis)
* [MongoDB](/5.12/planning-for-production/database-settings#mongodb-sizing-guidelines)
* [PostgreSQL](/5.12/planning-for-production/database-settings#postgresql)

Please consult the [data storage configuration](/5.12/api-management/dashboard-configuration#data-storage-solutions) guide for further information relating to how to configure Tyk's data storage across different database engines.
...
**Capping Analytics**
Tyk Gateways can generate a lot of analytics data. Be sure to read about [capping your Dashboard analytics](/5.12/api-management/tyk-pump#tyk-pump-capping-analytics-data-storage)

**API Definition Compression for MDCB Deployments**

In [MDCB](/5.12/api-management/mdcb) deployments, Data Plane Gateways store API definition backups in Redis to ensure resilience when the Control Plane is unavailable. For deployments with many or large API definitions, this can consume significant Redis memory.

You can enable Zstandard compression to reduce the storage footprint by setting [`storage.compress_api_definitions`](/5.12/tyk-oss-gateway/configuration#storage-compress_api_definitions) to `true` in your Data Plane Gateway configuration:
...
* [Configure Dashboard MongoDB driver](/5.12/tyk-dashboard/configuration#mongo_driver)
* [Configure MDCB MongoDB driver](/5.12/tyk-multi-data-centre/mdcb-configuration-options#analytics-driver)
...
<Note>
If you are using [DocumentDB](https://aws.amazon.com/documentdb/), [capped collections](/5.12/api-management/tyk-pump#tyk-pump-capping-analytics-data-storage) are not supported. See [here](https://docs.aws.amazon.com/documentdb/latest/developerguide/mongo-apis.html) for more details.
</Note>
...
More information on these configuration variables [here](/5.12/tyk-dashboard/configuration). 
