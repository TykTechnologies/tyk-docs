...
See [Monitors](/5.12/api-management/gateway-events#monitoring-quota-consumption) for details of how to configure quota consumption monitors.
...
To configure them, add an `event_options` section to an Organization when you are creating them. See [Creating an Organization via the Dashboard Admin API](/5.12/api-management/dashboard-configuration#create-an-organization) for more details.
