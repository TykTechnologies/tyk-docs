...
keywords: "Developer Portal, Release notes, changelog, v1.13.0, v1.14.0, v1.15.0, v1.16.0"
order: 7
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.12/#Changelog-v1.17.1) below.
...
If you are upgrading to 1.17.1, please follow the detailed [upgrade instructions](/5.12/#upgrading-tyk).
...
- [Docker Image to pull](https://hub.docker.com/r/tykio/portal/tags?page=&page_size=&ordering=&name=v1.17.1)
  - ```bash
    ...
    ```
- Helm charts
  - [tyk-charts](/5.12/developer-support/release-notes/helm-chart)
...
We've added a new Certificate Management function in the Admin Portal, where you can upload client certificates, retrieve existing certificates from the [Tyk Certificate Store](/5.12/api-management/certificates#tyk-certificate-store), and associate certificates with Organisations.

When an Auth Token ([API Key](/5.12/portal/overview/concepts#access-credentials)) is issued to a Developer App, the appropriate certificate or certificates will automatically be linked to that token to ensure that the API Consumer can access the Product's APIs using a combination of the token with their client certificate.

For a comprehensive list of changes, please refer to the detailed [changelog](/5.12/#Changelog-v1.17.0) below.
...
To upgrade the Portal's theme, please follow the [upgrade instructions](/5.12/portal/customization/themes#upgrading-themes).
...
The Developer Portal now properly supports rotation and revocation operations for [Tyk Managed Custom Credentials](/5.12/tyk-developer-portal/import-custom-credentials). Custom credentials now support the same lifecycle management as standard Tyk tokens.
</Accordion>
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.12/#Changelog-v1.16.0) below.
...
For more information, please visit [Requesting Access to an API Product](/5.12/tyk-developer-portal/direct-access-flow)
</Accordion>
...
For more information, please visit [Credential Management and Reusability](/5.12/tyk-developer-portal/single-credentials-multiple-api-products)
</Accordion>
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.12/#Changelog-v1.15.0) below.
...
Learn more about managing [API products with GraphQL documentation](/5.12/portal/api-products#api-reference-documentation)
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.12/#Changelog-v1.14.1) below.
...
We’ve added support for [Tyk Streams APIs](/5.12/api-management/event-driven-apis) in the Developer Portal, allowing API Providers to expose access to event-driven APIs alongside traditional REST APIs.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.12/#Changelog-v1.14.0) below.

#### Breaking Changes

There are no breaking changes in this release.

For the complete API specification, see [Tyk EDP API documentation](/5.12/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api).
...
To upgrade the portal's theme, please follow the [upgrade instructions](/5.12/portal/customization/themes#upgrading-themes) for the portal's themes.
...
This patch release contains a bug fix. For a comprehensive list of changes, please refer to the detailed [changelog](/5.12/#Changelog-v1.13.2) below.
...
Fixed an issue where middleware configuration for an API could be corrupted (for example, [Request header transform middleware](/5.12/api-management/traffic-transformation#request-header-transform) settings were changed in Developer Portal. This was due to a misalignment between Dashboard and Developer Portal in their interpretation of the API definition and has now been corrected.
</Expandable>
...
#### Release Highlights
This release is focused mainly on bug fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.12/#Changelog-v1.13.1) below.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.12/#Changelog-v1.13.0) below.
...
1. Portal will automatically migrate the new Custom IDs to most of the existing resources. For more information, please refer to the [changelog](/5.12/#Changelog-v1.13.0).
2. Users are advised to take appropriate actions in their system regarding the `Plans` API object changes as described in the [breaking changes section](/5.12/#breaking-changes-v1.13.0).
...
- Delete plans
    ...
    <Note>
    The Plans API object has schema changes; please refer to the [breaking changes section](/5.12/#breaking-changes-v1.13.0) for details.
    </Note>
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.12/#Changelog-v1.12.0) below.
...
You can read more about the supported SSO providers [here](/5.12/api-management/external-service-integration).
</Accordion>
...
- [22 bugs fixed](/5.12/#fixed)
- [19 CVEs fixed](/5.12/#fixed)
...
- [PORTAL_DATABASE_MAX_OPEN_CONNECTIONS](/5.12/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_database_max_open_connections): Set this value based on your database's maximum connection limit divided by the number of portal instances. For example, if your database allows 200 connections and you are running 4 portal instances, set PORTAL_DATABASE_MAX_OPEN_CONNECTIONS to 50 per instance. This ensures that all instances can share the available connections without exceeding the database's limit, which could otherwise lead to performance degradation or errors.
- [PORTAL_DATABASE_MAX_IDLE_CONNECTIONS](/5.12/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_database_max_idle_connections): Set to 15 or a lower value based on your expected load. This setting keeps a reasonable number of connections readily available without tying up resources unnecessarily.
...
- Added new TLS variables to set MinVersion ([portal_tls_min_version](/5.12/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_tls_min_version), MaxVersion ([PORTAL_TLSCONFIG_MAXVERSION](/5.12/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_tls_max_version), and CipherSuites ([PORTAL_TLS_CIPHER_SUITES](/5.12/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_tls_cipher_suites).
- Added a new configuration to manage the idle timeout of the portal's session ([PORTAL_SESSION_IDLE_TIMEOUT](/5.12/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_session_idle_timeout).
...
- Changed the default value of [PORTAL_DATABASE_CONNECTION_MAX_LIFETIME](/5.12/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_database_connection_max_lifetime) to 1800000 milliseconds.
...
- Added an assets cache for improved performance on database-backed themes. This speeds up the portal's pages loading time by 30%. It's enabled by default and you can disable using [PORTAL_ASSETS_CACHE_DISABLE](/5.12/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_assets_cache_disable).
- Added three new configuration options to manage database connections lifecycle: [PORTAL_DATABASE_MAX_OPEN_CONNECTIONS](/5.12/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_database_max_open_connections), [PORTAL_DATABASE_MAX_IDLE_CONNECTIONS](/5.12/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_database_max_idle_connections), and [PORTAL_DATABASE_CONNECTION_MAX_LIFETIME](/5.12/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_database_connection_max_lifetime).
...
- [Webhooks](/5.12/portal/customization) for events that happen in the portal.
- [Admin APIs](/5.12/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) for OAuth2.0 configuration. 
...
#### Changelog
##### Added
- Added [the webhooks](/5.12/portal/customization) capability that  enable real-time, automated data updates between the portal and 3rd party applications.
- Added [admin APIs](/5.12/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) for managing OAuth2.0 configuration. 
...
If you are on 1.8.3 or older version please follow the [upgrade instructions](/5.12/portal/customization/themes#upgrading-themes) to upgrade the portal's themes.
...
#### Changelog
##### Added
- Added [new configuration option](/5.12/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) for setting the `SameSite` attribute on the portal's cookie.
- Added [new welcome email for admin users](/5.12/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) that is sent when new admin account is created.
- Added [new welcome email for developers](/5.12/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) that is sent when new developer account is created.
...
- Changed the credential provisioning flow to automatically include DeveloperID, OrganizationID, ApplicationID, and TeamIDs in [the credential metadata](/5.12/portal/customization/user-model).
- Added warning regarding potential PII exposure to the [custom attributes menu](/5.12/portal/customization/user-model).
...
- Changed the behavior of the `Secure` cookie attribute that is set by [PORTAL_SESSION_SECURE](/5.12/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_session_secure) so that the `Secure` attribute is always add to the `Set-Cookie` header whenever `PORTAL_SESSION_SECURE` is set to `true` or when TLS is enabled.
...
- Extended the `DELETE /users/:id` API endpoint by adding the [?force](/5.12/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) query parameter to force removal of a user even if some of their credentials have been removed from the credentials provider, or if the provider itself is down or unreachable.
...
- Extended [filesize limit](/5.12/portal/customization/themes#create-a-theme) for individual files in themes to 5 MB.
- Made the organization invite email's subject configurable via [the emails settings section](/5.12/portal/customization/email-notifications#supported-email-notifications).
...
#### Changelog
##### Added
- Added [new admin API](/5.12/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) for managing tags attached to API Products.
- Added [new admin API](/5.12/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) for managing OAuth2.0 client types attached to API Products.
...
##### Added
- Add a [config option](/5.12/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_enable_http_profiler) to expose the Golang profiling information which allows for debugging issues related to resource consumption.
...
- When the portal theme [manifest](/5.12/portal/customization/themes#manifest-file) has a reference to a template that is not present in the theme then the theme won't be loaded. This check that prevents admin users from uploading themes with potential errors was introduced in version [1.7.0](/5.12/#content-blocks-validation).
...
All settings are available via the [admin API](/5.12/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) and the UI.
...
To set the CORS configuration please refer to the Portal's [configuration documentation](/5.12/product-stack/tyk-enterprise-developer-portal/deploy/configuration#cors-settings).
...
Now when the [PORTAL_DCR_LOG_ENABLED](/5.12/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_dcr_log_enabled) environment variable is set to `true`, the portal will output not only the status and status code of the request to the IdP, but also actual payload returned by the IdP: 
```yaml
...
- When [PORTAL_DCR_LOG_ENABLED](/5.12/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_dcr_log_enabled) is enabled the portal now returns not only the status and status code of the request to the IdP but also actual payload returned by the IdP
...
- Fixed the bug where the [Forgot Password page](/5.12/tyk-developer-portal/tyk-enterprise-developer-portal/api-consumer-portal/reset-password) did not reflect the current theme.
...
- Simplified the connection settings to the portal assets storage (where all images, themes, and other CMS files are stored) to help our customers get up to speed quicker. We are well aware that installing and configuring on-premise software can be tricky, especially when it comes to infrastructure, storage and databases. Hence, we have decided to ease this burden for you:
  - By default, the portal uses the `db` [storage type](/5.12/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_storage) for storing its themes and other CMS assets and it doesn't require any additional configuration. This means, you can start the portal right away without specifying any additional setting for the assets storage.
...
With this new feature, customers can simply use the same SQL database (MySQL, MariaDB and PostgreSQL) for both assets and metadata. To use the `db` [storage type](/5.12/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_storage) just set the `PORTAL_STORAGE=db` for environment variables or `"Storage": "db"` in a config file and you are good to go!
...
The Tyk Enterprise Developer Portal is now available in Kubernetes and customer can launch it using our [helm charts](/5.12/portal/install/kubernetes#legacy-helm-chart). This feature makes the portal Kubernetes friendly by adding liveness, readiness probes, graceful shutdown and changing the portal lifecycle so that it's possible to set an initial user and bootstrap the portal via APIs.

##### SSO for API Consumers and admins
API Providers can [configure Single Sign-on](/5.12/tyk-stack/tyk-developer-portal/enterprise-developer-portal/managing-access/enable-sso) for the Enterprise developer portal so that it's possible to login developers and admins to the portal user 3rd party IdP.
...
##### Add TLS support
This feature enables API Provides to secure the portal with [HTTPs](/5.12/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_tls_enable).

##### Add enhanced logging configuration
This new setting allows API Providers to set the logging [level](/5.12/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_log_level) and [format](/5.12/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_log_format). This offers API Providers more control over the logging behavior of their APIs.
...
##### Added
- Added Kubernetes support and [helm charts](/5.12/portal/install/kubernetes#legacy-helm-chart).
- Added [Single Sign-on](/5.12/tyk-stack/tyk-developer-portal/enterprise-developer-portal/managing-access/enable-sso) for API Consumers and admin users so that they can use their IdPs for managing admin users and developers.
...
- Added [TLS](/5.12/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_tls_enable) support for the portal's UI.
...
Now API Consumers can [create organizations](/5.12/tyk-stack/tyk-developer-portal/enterprise-developer-portal/managing-access/manage-api-consumer-organisations) and securely share credentials between their teammates. In greater detail:
...
##### Get started guides
API Providers can add [Get started guides](/5.12/portal/api-products#documentation-only-products) to API Products for better developer experiences:
...
API Providers can select which blogs posts to display on an API Product page using [the tags feature](/5.12/portal/api-products#documentation-only-products). To achieve that, an API Provider can specify tags for both API Products and blog posts. Blog posts that match tags with an API Product are displayed in the 'Related blog content' section in the API Product page. This offers API Providers greater control over what blog posts to display on their API Product page.

##### S3 support
We added [S3 support](/5.12/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_storage) for the portal assets storage (themes, images, OAS files). This update enhances the extensibility of our platform, allowing you to choose different storage solutions to better align with your specific needs.
...
##### Added
- Added the [organization management capability](/5.12/tyk-stack/tyk-developer-portal/enterprise-developer-portal/managing-access/manage-api-consumer-organisations) for API Consumers to safely share API access credentials between team members.
- Added the [Get started guides](/5.12/portal/api-products#documentation-only-products) for API Products so that admins can explain to their consumers how use their API Products.
- Added support for [S3 storage](/5.12/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_storage) for the portal's assets storage. Now our customers can use `s3` storage in addition to the filesystem which is especially important in Kubernetes environments.
- Added [tags](/5.12/portal/api-products#documentation-only-products) for API Products and blog posts so that API Providers have greater control over which blog posts to display on their API Product page.
...
### Upgrading Tyk
Please refer to the [upgrading Tyk](/5.12/developer-support/upgrading) page for further guidance with respect to the upgrade strategy.
