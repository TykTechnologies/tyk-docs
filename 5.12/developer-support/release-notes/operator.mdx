...
<Note>
The initial `Helm chart` for Operator 1.3.0 contained an installation bug. Please ensure you use tyk-charts version [5.1.1](/5.12/developer-support/release-notes/helm-chart#5-1-1-release-notes) or later to install Operator 1.3.0.
</Note>
...
Go to the [Upgrading Tyk Operator](/5.12/tyk-stack/tyk-operator/installing-tyk-operator) section for detailed upgrade instructions.
...
- [Docker image v1.3.0](https://hub.docker.com/r/tykio/tyk-operator/tags?page=&page_size=&ordering=&name=v1.3.0)
  - ```bash
    ...
    ```
- Helm chart
  - [tyk-charts v5.1.0](/5.12/developer-support/release-notes/helm-chart#5-0-0-release-notes)
...
The Tyk Operator now supports configuration of [upstream uptime tests](/5.12/planning-for-production/ensure-high-availability/uptime-tests) directly in your API definitions through both Tyk OAS and Tyk Classic CRDs.
</Accordion>
...
The ingress controller now respects the [`listenPath.strip`](/5.12/api-management/gateway-config-tyk-oas#listenpath) configuration defined in your Tyk OAS API templates. This fix ensures that template configurations are preserved while maintaining backward compatibility, as templates without an explicit strip setting will continue to default to `true`, and ACME challenge paths will still force `strip: false` as required.
</Accordion>
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.12/#Changelog-v1.2.0) below.
...
[Learn More](/5.12/api-management/upstream-authentication/request-signing)
</Accordion>
...
[Learn More](/5.12/tyk-stack/tyk-operator/create-an-api#create-a-tykstreamsapidefinition-custom-resource)
</Expandable>
...
For details please refer to the [changelog](/5.12/#Changelog-v1.0.0) below.
...
{/* Required. Use the following statement if there are no breaking changes, or explain if there are */}
{/* This release has no breaking changes. */}
**License Requirement:** Tyk Operator is now a closed-source product and requires a valid license key to operate. Please follow our [Installation and Upgrade Guide](/5.12/tyk-stack/tyk-operator/installing-tyk-operator) to set your license key before installation or upgrade.
...
Go to the [Upgrading Tyk Operator](/5.12/tyk-stack/tyk-operator/installing-tyk-operator#upgrading-tyk-operator) section for detailed upgrade instructions.
...
- [Docker image v1.0.0](https://hub.docker.com/r/tykio/tyk-operator/tags?page=&page_size=&ordering=&name=v1.0.0)
  - ```bash
    ...
    ```
- Helm chart
  - [tyk-charts v2.1.0](/5.12/developer-support/release-notes/helm-chart#2-1-0-release-notes) {/* This is the link to the Helm charts links. Please be mindful that this URL is only available a few hours or day/s after we release the main release, so this link needs to be updated in a separate iteration */}
{/* source code tarball for oss projects */}
...
Learn More: [Create Tyk OAS API](/5.12/tyk-stack/tyk-operator/create-an-api#set-up-tyk-oas-api)
</Accordion>
...
Learn More: [Tyk Ingress Controller](/5.12/product-stack/tyk-operator/tyk-ingress-controller)
</Accordion>
...
Learn More: [Key-Level Per-API Rate Limits and Quota](/5.12/tyk-stack/tyk-operator/create-an-api#security-policy-example)
</Accordion>
...
Learn More: [Key-Level Per-Endpoint Rate Limits](/5.12/tyk-stack/tyk-operator/create-an-api#security-policy-example)
</Accordion>
...
Learn More: [TykOasApiDefinition in Security Policy](/5.12/tyk-stack/tyk-operator/create-an-api#add-a-security-policy-to-your-api)
</Accordion>
...
Learn More: [Event Webhook with Tyk Classic](/5.12/api-management/gateway-events#webhook-event-handlers-with-tyk-classic-apis)
</Accordion>
...
For details please refer to the [changelog](/5.12/#Changelog-v0.18.0) below.
...
- [introspection](/5.12/api-management/graphql#turning-off-introspection) option to enable/disable GraphQL introspection
- [graphql.proxy.auth_headers](/5.12/api-management/graphql#creating-a-graphql-api-via-the-dashboard-ui)
- [graphql.proxy.subscription_type](/5.12/api-management/graphql#graphql-subscriptions)
- [graphql.proxy.request_headers](/5.12/api-management/graphql#request-headers)
...
#### Release Highlights
This release is focused on bug fixes. For details please refer to the [changelog](/5.12/#Changelog-v0.17.1) below.
...
For details please refer to the [changelog](/5.12/#Changelog-v0.17.0) below.
...
#### Release Highlights
This release added support for analytics plugin, UDG global header, and detailed tracing setting in ApiDefinition as detailed in the [changelog](/5.12/#Changelog-v0.16.0) below.
