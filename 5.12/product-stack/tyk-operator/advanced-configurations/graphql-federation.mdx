...
Tyk, with release *v4.0* offers [GraphQL federation](/5.12/api-management/graphql#federation-version-support) that allows you to divide GraphQL implementation across multiple back-end
...
Tyk Operator uses Custom Resources called [SubGraph](/5.12/#subgraph) and [SuperGraph](/5.12/#supergraph), that allows users to model the relationship between Subgraphs and Supergraphs.
...
Let's assume that a developer responsible for the [Users SubGraph](/5.12/#users-subgraph) would like to delete `username` field from the Users SubGraph.
Also, the [Supergraph](/5.12/#supergraph-1) called Social Media already uses the Users Subgraph.
