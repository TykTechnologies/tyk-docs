...
import OpaRules from '/snippets/5.12/opa-rules.mdx';
import MongodbVersionsInclude from '/snippets/5.12/mongodb-versions-include.mdx';
import SqlVersionsInclude from '/snippets/5.12/sql-versions-include.mdx';
...
This page introduces general features of dashboard and how to configure them. If you are looking for global configurations of the Dashboard deployment refer this [config file](/5.12/tyk-dashboard/configuration).

We will delve into the following key topics:

1. **[Exploring the Dasbhoard UI](/5.12/#exploring-the-dashboard-ui)**: A tour of the Dashboard UI.

2. **[Exploring the Dasbhoard API](/5.12/#exploring-the-dashboard-api)**: Explore the Dashboard APIs, including their classification, authentication methods, and usage examples with Swagger and Postman collections.

3. **[API Management using API Endpoint Designer](/5.12/#exploring-api-endpoint-designer)**: A graphical environment for configuring your Tyk APIs.

4. **[Monitoring and Traffic Analytics](/5.12/#traffic-analytics)**: Exploration of Tyk's traffic analytics capabilities, including logging mechanisms, error tracking, endpoint analysis, and various activity type measurements.

5. **[API Governance using API Templates and API Categories](/5.12/#governance-using-api-categories)**

6. **[System Management](/5.12/#system-administration)**: Detailed overview of Tyk's system management capabilities, including Admin API functionalities, organization management and configuting audit logs.
  
7. **[Supported Database](/5.12/#supported-database)**: We will examine Dashboard's storage requirement, compatible database versions and how to configure them.

7. **[Exploring Data Storage Solution](/5.12/#data-storage-solutions)**: We will explore Dashboard's multi-layered storage architecture and understand how to configure each storage tier effectively.

## Exploring the Dashboard UI

To get a tour of the Dashboard UI, refer to this [document](/5.12/getting-started/using-tyk-dashboard).
...
The Dashboard exposes two APIs:
 - **Dashboard API**: Is used for operational management of Tyk resources (APIs, policies, keys, etc.). This API offers granular permissions based on user roles.
    ...
    - [Postman / Swagger / Open API specification](/5.12/tyk-dashboard-api)
    - [Dashboard API Usage Examples](/5.12/#dashboard-api-resources-and-usage)
 ...
 - **Dashboard Admin API**: Is used for system-level administration and initial setup tasks like managing organizations, initial user creation, backups/migrations and SSO setup. 
    ...
    - [Postman / Swagger / Open API specification](/5.12/dashboard-admin-api)
    - [Dashboard Admin API Usage Examples](/5.12/#dashboard-admin-api-resources-and-usage)

### Authenticating with Dashboard APIs

**Dashboard API**

The [Tyk Dashboard API](/5.12/tyk-dashboard-api) is secured using an `Authorization` header that must be added to each request that is made. The **Tyk Dashboard API Access Credentials** `Authorization` key can be found within the Dashboard UI at the bottom of the **Edit User** section for a user.
...
The [Tyk Dashboard API](/5.12/tyk-dashboard-api) is a superset of the Tyk Gateway API, enabling (almost) all of the core features and adding many more. The Dashboard API is also more granular and supports [Role Based Access Control](/5.12/api-management/user-management#) (RBAC) on both a multi-tenant, and user basis.
...
<Note>
See [API Definition Objects](/5.12/api-management/gateway-config-tyk-classic) section for an explanation of each field in the request & response.
</Note>
...
<Note>
You can add a user via the API without a password by leaving out the `password` field. You then use [Set User Password](/5.12/#set-user-password) request to add a password.
</Note>


You need to have the `users` [Permission object](/5.12/api-management/user-management#user-permissions) set to write to use **Add User**.
...
You need to have the `users` [Permission object](/5.12/api-management/user-management#user-permissions) set to **read** to use **Set User Password**.
...
You need to have the `users` [Permission object](/5.12/api-management/user-management#user-permissions) set to write to use **Update User**.
...
You need to have the `users` [Permission object](/5.12/api-management/user-management#user-permissions) set to write to use this call.
...
This API returns by default the initial set of additional permissions defined in your Tyk Dashboard configuration, under [security.additional_permissions](/5.12/tyk-dashboard/configuration#security-additional_permissions).
...
You can control how long you want to store expired tokens in this list using `oauth_token_expired_retain_period` which specifies retain period for expired tokens stored in Redis. By default expired token not get removed. See [here](/5.12/tyk-oss-gateway/configuration#oauth_token_expired_retain_period) for more details.
...
This endpoint is used in the [Authorization Code Grant](/5.12/api-management/authentication/oauth-2#using-the-authorization-code-grant) flow, generating an authorization code that can be used by the client to request an access token.
...
<Note>
This functionality is available from [v2.9.0](/5.12/developer-support/release-notes/archived#single-sign-on-for-the-tyk-saas). If you have an older version please using the [admin api](/5.12/api-management/dashboard-configuration#single-sign-on-api-1)
</Note>
...
See [Single Sign On](/5.12/api-management/external-service-integration#single-sign-on-sso) documentation for how to use this token and more details.
...
For more information on how to configure OPA see [Open Policy Agent](/5.12/api-management/dashboard-configuration#extend-permissions-using-open-policy-agent-opa).
#### List OPA rules and settings

This endpoint returns by defaul the initial set of OPA rules defined in your Tyk Dashboard, which are located in [schema/dashboard.rego](/5.12/api-management/dashboard-configuration#dashboard-opa-rules) (accessible in Self-Managed installations).
...
For the official Tyk Dashboard Admin API Reference, please visit our [API Documentation](/5.12/dashboard-admin-api).
...
When you add a new user, they are created without a password being set. After adding a user, you need to use the [Set Password](/5.12/#set-user-password) call to set a password using the `user-id` created.
...
The Dashboard Admin SSO API endpoint allows you to implement custom authentication schemes for the Dashboard and Portal. Our Tyk Identity Broker (TIB) internally also uses this API. See [Single Sign On](/5.12/api-management/external-service-integration#single-sign-on-sso) for more details.
...
To export APIs and Policies you should use the standard `GET APIS` endpoint and `GET POLICIES` list endpoints. The output from these endpoints can be used by the [Import API](/5.12/#import-assets-api).
...
The [Organization object](/5.12/#organizations) is the most fundamental object in a Tyk setup, all other ownership properties hang off the relationship between an Organization and its APIs, Policies and API Tokens.
...
- [Detailed logging](/5.12/api-management/tyk-pump#configure-detailed-recording)
- API Settings including
   - Listen path
   - [API Categories](/5.12/#governance-using-api-categories)
- Upstream settings including
   - Upstream service (target) URL
   - [Service Discovery](/5.12/planning-for-production/ensure-high-availability/service-discovery)
- [API Ownership](/5.12/api-management/user-management#api-ownership)
- [API level rate limiting](/5.12/api-management/rate-limit#configuring-the-rate-limiter-at-the-api-level)
- [Authentication](/5.12/api-management/client-authentication)
...
The **Versions** tab allows you to create and manage [API versioning](/5.12/api-management/gateway-config-tyk-classic#tyk-classic-api-versioning) for the API.
...
The **Endpoint Designer** is where you can define endpoints for your API so that you can enable and configure Tyk middleware to [perform checks and transformations](/5.12/api-management/traffic-transformation) on the API traffic.

In some cases, you will want to set global settings that affect all paths that are managed by Tyk. The **Global Version Settings** section will enable you to configure API-level [request](/5.12/api-management/traffic-transformation/request-headers#tyk-classic-api) and [response](/5.12/api-management/traffic-transformation/request-headers#tyk-classic-api) header transformation.
...
- [API-level caching](/5.12/api-management/response-caching#configuring-the-cache-via-the-dashboard) including a button to invalidate (flush) the cache for the API
- [CORS](/5.12/api-management/gateway-config-tyk-classic#cross-origin-resource-sharing-cors)
- Add custom attributes to the API definition as *config data* that can be accessed by middleware
- Enable [context variables](/5.12/api-management/traffic-transformation/request-context-variables) so that they are extracted from requests and made available to middleware
- Manage *segment tags* if you are working with [sharded gateways](/5.12/api-management/multiple-environments#gateway-sharding)
- Manage client IP address [allow](/5.12/api-management/gateway-config-tyk-classic#ip-access-control) and [block](/5.12/api-management/gateway-config-tyk-classic#ip-access-control) lists
- Attach [webhooks](/5.12/api-management/gateway-events#event-handling-with-webhooks) that will be triggered for different events
...
In the **Uptime Tests** tab you can configure Tyk's [Uptime Test](/5.12/api-management/gateway-config-tyk-classic#uptime-tests) functionality
...
[User Owned Analytics](/5.12/api-management/user-management#user-permissions), introduced in Tyk v5.1, can be used to limit the visibility of aggregate statistics to users when API Ownership is enabled. Due to the way that the analytics data are aggregated, not all statistics can be filtered by API and so may be inaccessible to users with the Owned Analytics permission.


<Note>
For the Tyk Dashboard's analytics functionality to work, you must configure both per-request and aggregated pumps for the database platform that you are using. For more details see the [Setup Dashboard Analytics](/5.12/api-management/tyk-pump#setup-dashboard-analytics) section.
</Note>
...
<Note>
From Tyk v5.1 (and LTS patches v4.0.14 and v5.0.3) the Error Breakdown and Endpoint Popularity charts will not be visible to a user if they are assigned the [Owned Analytics](/5.12/api-management/user-management#user-permissions) permission.
</Note>
...
To enable request and response logging, please take a look at [useful debug modes](/5.12/api-management/troubleshooting-debugging#capturing-detailed-logs) .
...
<Note>
From Tyk v5.1 (and LTS patches v4.0.14 and v5.0.3) the <b>Traffic per Key</b> screen will not be visible to a user if they are assigned the [Owned Analytics](/5.12/api-management/user-management#user-permissions) permission.
</Note>
...
You can alternatively select only a subset of the endpoints to include in the aggregated data by setting `track_all_paths` to `false` and identifying specific endpoints to be "tracked". These are identified by the `TrackPath` [flag](/5.12/api-management/tyk-pump#trackpath) being set to `true` in the record. In this configuration, the Pump will only include transaction records from "tracked" endpoints in the aggregated data.
...
<Note>
The *track endpoint* middleware only affects the inclusion of endpoints in the per-endpoint aggregates, it does not have any impact on other [aggregated data](/5.12/api-management/tyk-pump#aggregated-analytics) nor the [per-request data](/5.12/api-management/dashboard-configuration#activity-logs).
</Note>
...
<Note>
From Tyk v5.1 (and LTS patches v4.0.14 and v5.0.3) the <b>Geographic Distribution</b> screen will not be visible to a user if they are assigned the [Owned Analytics](/5.12/api-management/user-management#user-permissions) permission.
</Note>


**MaxMind Settings**

To use a MaxMind database, see [MaxMind Database Settings](/5.12/tyk-oss-gateway/configuration#analytics_config-enable_geo_ip) in the Tyk Gateway Configuration Options.
...
<Note>
From Tyk v5.1 (and LTS patches v4.0.14 and v5.0.3) the Errors by Category data will not be visible to a user if they are assigned the [Owned Analytics](/5.12/api-management/user-management#user-permissions) permission.
</Note>
...
<Note>
From Tyk v5.1 (and LTS patches v4.0.14 and v5.0.3) the Traffic per OAuth Client ID charts will not be visible to a user if they are assigned the [Owned Analytics](/5.12/api-management/user-management#user-permissions) permission.
</Note>
...
Multi-tenant deployments with [role-based access control](/5.12/api-management/user-management#) enabled allows an admin user to give different users or groups access to a sub-set of the entire API portfolio. Categories can be aligned with the API ownership rules that you have deployed to allow filtering the list of APIs for those visible to each separate user group/team.
...
API categories can be added and removed from APIs within the [API Designer](/5.12/#api-designer), via the [Tyk Dashboard API](/5.12/#tyk-dashboard-api), or via [Tyk Operator](/5.12/api-management/automations/operator#what-is-tyk-operator).
...
#### Tyk Dashboard API
The [Tyk Dashboard API](/5.12/tyk-dashboard-api) provides endpoints to manage categories directly, if you are not using the API Designer.
...
You can manage categories using Tyk Operator custom resources. Please refer to [Tyk Operator](/5.12/api-management/automations/operator#api-categories) documentation to see how to manage API categories for Tyk OAS APIs and Tyk Classic APIs.
...
<Note>
API Templates are exclusive to [Tyk OAS APIs](/5.12/api-management/gateway-config-introduction#api-definitions) and can be managed via the Tyk Dashboard API or within the Tyk Dashboard UI.
</Note>
...
When you use a template during the [creation](/5.12/api-management/gateway-config-managing-oas#creating-an-api) of an API, the fields configured in `data` will be pre-set in your new API. You are able to modify these during and after creation of the template. No link is created between the API and the template, so changes made to the API will not impact the template.

#### Merging with an OpenAPI description or Tyk OAS API definition
When you use a template during the creation of an API where you [import](/5.12/api-management/gateway-config-managing-oas#importing-an-openapi-description-to-create-an-api) the OpenAPI document or a full Tyk OAS API definition, the template is combined with the imported OAS description. If the `x-tyk-api-gateway` extension exists in the template, it will be applied to the newly created API.
...
If you're using the API Designer in the Tyk Dashboard UI, then you can find details and examples of how to work with API templates [here](/5.12/#working-with-api-templates-using-the-template-designer).

If you're using the Tyk Dashboard API, then you can find details and examples of how to work with API templates [here](/5.12/#working-with-api-templates-using-the-dashboard-api).

### Working with API Templates using the Template Designer

[API Templates](/5.12/#governance-using-api-templates) are an API governance feature provided to streamline the process of creating Tyk OAS APIs. An API template is an asset managed by Tyk Dashboard that is used as the starting point - a blueprint - from which you can create a new Tyk OAS API definition.

The Tyk Dashboard UI provides the following functionality to support working with API templates:
 - Creating templates
   - [new template](/5.12/#creating-a-new-api-template)
   - [from an existing API](/5.12/#creating-a-template-from-an-existing-api)
 - Using templates
   - [when creating an API](/5.12/#using-a-template-when-creating-a-new-api)
   - [when importing an OpenAPI description or API definition](/5.12/#using-a-template-when-importing-an-openapi-description-or-api-definition)
 - [Managing templates](/5.12/#managing-templates)
...
<Note>
API Templates are exclusive to [Tyk OAS APIs](/5.12/api-management/gateway-config-introduction#api-definitions).
</Note>
...
Returning to the **API Template** screen you will see your new template has been added to the list and assigned a unique `id` that can be used to access the template from the [Tyk Dashboard API](/5.12/#structure-of-an-api-template):
...
In this example, we applied "My first template" that we created [here](/5.12/#creating-a-new-api-template). You can see that the Gateway Status and Access fields have been configured:
...
In this example we used this simple OpenAPI description and selected "My second template" that we created [here](/5.12/#creating-a-template-from-an-existing-api):
...
The Tyk Dashboard API provides the following functionality to support working with API templates:
 - [registering a template with Tyk Dashboard](/5.12/#registering-a-template-with-tyk-dashboard)
 - [applying a template when creating an API from an OpenAPI document](/5.12/#applying-a-template-when-creating-an-api-from-an-openapi-document)
 - [applying a template when creating an API from a Tyk OAS API definition](/5.12/#applying-a-template-when-creating-an-api-from-a-tyk-oas-api-definition)
...
You can then control OPA functionality on a global level via your `tyk_analytics.conf` file, or at an organization level using either the [OPA API](/5.12/api-management/dashboard-configuration#open-policy-agent-api) or the [Dashboard](/5.12/#using-the-open-policy-agent-in-the-dashboard).
...
Additionally, users can modify OPA rules, and define their own, through the [OPA API](/5.12/api-management/dashboard-configuration#additional-permissions-api). For Self-Managed installations you can access and modify the OPA rules from your Tyk installation directory from [schemas/dashboard.rego](/5.12/api-management/dashboard-configuration#dashboard-opa-rules).
...
To include the `API Editor` role with additional permissions, send a PUT Request to the [Dashboard Additional Permissions API endpoint](/5.12/api-management/dashboard-configuration#additional-permissions-api) `/api/org/permissions`
...
The Tyk Dashboard Admin API provides the following administrator level functions:
 - Managing [organizations](/5.12/#organizations).
 - Creating initial [users](/5.12/api-management/dashboard-configuration#users-api-1) during boot-strapping of the system.
 - Forcing a [URL reload](/5.12/api-management/dashboard-configuration#url-reload-api).
 - [Exporting](/5.12/#export-assets-api) and [importing](/5.12/#import-assets-api) Tyk assets (orgs, APIs, policies) for backup or when migrating between environments.
 - Setting up [SSO integration](/5.12/#single-sign-on-api-1).
...
Additional organizations can be created and managed using the [Dashboard Admin API](/5.12/#organizations-api).
...
Self-Managed users should use [API tagging](/5.12/api-management/multiple-environments#api-tagging-with-on-premises) and enforce a tagging standard across all organizations.
...
Please consult [Tyk Dashboard Configuration Options](/5.12/tyk-dashboard/configuration#audit) for equivalent configuration with environment variables.
...
Since Tyk 5.7.0 a new API endpoint has been added to allow authorized users to retrieve audit logs from the database storage. To know more about the API specifications, check out the swagger [documentation](/5.12/tyk-dashboard-api).
...
Please check [here](/5.12/planning-for-production/database-settings#mongodb) for MongoDB driver and production configurations.
...
Please check [here](/5.12/#configuring-postgresql) for production configurations.

See the following pages for configuring your SQL installation with Tyk:

* [Configuring Tyk Dashboard](/5.12/#configuring-postgresql)
* [Configuring Tyk Pumps](/5.12/#configuring-postgresql)
...
<Note>
Tyk no longer supports SQLite as of Tyk 5.7.0. To avoid disruption, please transition to [PostgreSQL](/5.12/planning-for-production/database-settings#postgresql), [MongoDB](/5.12/planning-for-production/database-settings#mongodb), or one of the listed compatible alternatives.
</Note>
...
2. **Aggregate Analytics**: Data used to display Dashboard charts and [analytics](/5.12/#traffic-analytics).
3. **Logs**: When [detailed logging](/5.12/api-management/troubleshooting-debugging#capturing-detailed-logs) is enabled, request and response data is logged to storage. These logs can previewed in the Dashboard [log browser](/5.12/#activity-logs).
...
Please refer to the [storage configuration](/5.12/tyk-dashboard/configuration#storage) section to explore the parameters for configuring Tyk Dashboard to read from different storage layers.


### Configure Pump to Write to Data Storage Layers?

Please consult the Pump configuration [guide](/5.12/api-management/tyk-pump#sql-uptime-pump) for an explanation of how to configure Tyk Pump to write to different storage layers.
...
Further details for configuring an uptime SQL database are available [here](/5.12/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#uptime_pump_config-uptime_type)
...
Further details for configuring a Tyk Mongo Pump are available [here](/5.12/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#uptime_pump_config)
...
Aggregated analytics corresponds to data that is used for the display of charts and graphs in [dashboard](/5.12/#traffic-analytics). Tyk Pump can be configured to write aggregated analytics data to SQL based databases or MongoDB.
