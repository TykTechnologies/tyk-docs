...
Note that this is not the same as Tyk's [IP block list](/5.12/api-management/gateway-config-tyk-classic#ip-access-control) feature, which is used to restrict access to APIs based upon the IP of the requestor.
...
When using the block list middleware, we recommend that you familiarize yourself with Tyk's [URL matching](/5.12/getting-started/key-concepts/url-matching) options.
...
The [block list](/5.12/api-management/traffic-transformation/block-list) is a feature designed to block access to specific API endpoints. Tyk Gateway rejects all requests made to endpoints with the block list enabled, returning `HTTP 403 Forbidden`. 
...
If you're using the legacy Tyk Classic APIs, then check out the [Tyk Classic](/5.12/#block-list-using-classic) page.
...
Note also that the endpoint path has not been terminated with `$`. Requests to, for example, `GET /anything/foobar` will be rejected as the [regular expression pattern match](/5.12/#endpoint-parsing) will recognize this as `GET /anything`.
...
If you're using the newer Tyk OAS APIs, then check out the [Tyk OAS](/5.12/#block-list-using-tyk-oas) page.

If you're using Tyk Operator then check out the [configuring the block list in Tyk Operator](/5.12/#tyk-operator) section below.
...
- `method_actions`: a shared object used to configure the [mock response](/5.12/api-management/traffic-transformation/mock-response#when-is-it-useful) middleware
...
Note also that the endpoint path has not been terminated with `$`. Requests to, for example, `GET /status/200/foobar` will be rejected as the [regular expression pattern match](/5.12/#endpoint-parsing) will recognize this as `GET /status/200`.
...
Note also that the endpoint path has not been terminated with `$`. Requests to, for example, `GET /get/foobar` will be rejected as the [regular expression pattern match](/5.12/#endpoint-parsing) will recognize this as `GET /get`.
