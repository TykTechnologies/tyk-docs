...
[JSON Web Tokens (JWT)](https://www.jwt.io/introduction) are a popular method for client authentication and authorization that can be used to secure access to your APIs via Tyk's [JWT Auth](/5.12/basic-config-and-security/security/authentication-authorization/json-web-tokens) method.

After the JWT signature has been [validated](/5.12/basic-config-and-security/security/authentication-authorization/json-web-tokens), Tyk uses the **claims** within the token to determine which security policies (access rights, rate limits and quotas) should be applied to the request.

From Tyk 5.10, Tyk can perform optional [validation](/5.12/api-management/authentication/jwt-claim-validation) of these claims.
...
2. **Policy Resolution**: Tyk determines which [policy](/5.12/api-management/policies#what-is-a-security-policy) to apply to the request:
   - From scope-to-policy mapping
   - From default policies

3. **Update Session**: The [session](/5.12/api-management/policies#what-is-a-session-object) is updated with the identity and policies.
...
2. The subject identity claim identified by the value(s) stored in `subjectClaims` (which allows API administrators to designate any JWT claim as the identity source (e.g., user_id, email, etc.).
    ...
    <Note>
    Prior to Tyk 5.10, the subject identity claim was retrieved from `identityBaseField`; see [using multiple identity providers](/5.12/#using-multiple-identity-providers) for details and for the Tyk Classic API alternative.
    </Note>


3. The `sub` [registered claim](/5.12/api-management/authentication/jwt-claim-validation#registered-vs-custom-claims).
...
[Security Policies](/5.12/api-management/policies) are applied (or mapped) to the session object to configure authorization for the request. Policies must be [registered](/5.12/api-management/policies#how-you-can-create-policies) with Tyk, such that they have been allocated a unique *Tyk Policy Id*.
...
<Note>
Prior to Tyk 5.10, the base policy claim was retrieved from `policyFieldName`; see [using multiple identity providers](/5.12/#using-multiple-identity-providers) for details and for the Tyk Classic API alternative.
</Note>
...
You **must** configure one or more default policies unless using [scope policies](/5.12/api-management/authentication/jwt-authorization#scope-policies).
...
<Note>
Prior to Tyk 5.10, the authorization scopes claim was retrieved from `scopes.claimName`; see [using multiple identity providers](/5.12/#using-multiple-identity-providers) for details and for the Tyk Classic API alternative.
</Note>
...
When creating multiple policies that might be applied to the same JWT, we recommend using [partitioned policies](/5.12/api-management/policies#partitioned-policies) - policies that control specific aspects of API access rather than trying to configure everything in a single policy.
...
The following [session attributes](/5.12/api-management/policies#session-object) are modified based on the policies:
...
In addition to updating the session, Tyk extracts claims from the JWT and makes them available as context variables for use in other [middleware](/5.12/api-management/traffic-transformation).
