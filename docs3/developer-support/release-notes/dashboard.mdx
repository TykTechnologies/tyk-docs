...
With MongoDB 4.4 reaching [EOL](https://www.mongodb.com/legal/support-policy/lifecycles) in February 2024, we can no longer guarantee full compatibility with this version of the database. If you are [using MongoDB](/docs3/tyk-self-managed#mongodb) we recommend that you upgrade to a version that we have tested with, as indicated [below](/docs3/#3rdPartyTools-v5.3.6).


With PostgreSQL v11 reaching [EOL](https://www.postgresql.org/support/versioning/) in November 2023, we can no longer guarantee full compatibility with this version of the database. If you are [using PostgreSQL](/docs3/tyk-self-managed#postgresql) we recommend that you upgrade to a version that we have tested with, as indicated [below](/docs3/#3rdPartyTools-v5.3.6).
...
Tyk uses `mongo-go` as the default MongoDB driver from v5.3. This provides support for MongoDB 4.4.x, 5.0.x, 6.0.x and 7.0.x. If you are using older MongoDB versions e.g. 3.x, please set MongoDB driver to `mgo`. The [MongoDB supported versions](/docs3/tyk-self-managed#supported-versions) page provides details on how to configure MongoDB drivers in Tyk.
</Accordion>
</AccordionGroup>
</li>
</ul>
...
Fixed an issue where the [Enforced Timeout](/docs3/tyk-self-managed#enforced-timeouts) configuration parameter of an API endpoint accepted negative values, without displaying validation errors. With this fix, users receive clear feedback and prevent unintended configurations.
</Accordion>
...
See how to [Choose a MongoDB driver](/docs3/tyk-self-managed#choose-a-mongodb-driver)
...
If you want switch from MongoDB to SQL, you can [use our migration tool](/docs3/tyk-self-managed#migrating-from-an-existing-mongodb-instance), but keep in mind that it does not yet support the migration of your analytics data.
...
While SQL support for Tyk products does not depend on specific database features, with this release, we will provide official support for [PostgreSQL DB for production purposes](/docs3/tyk-self-managed#postgresql), and SQLite for development and PoC environments. Note that SQL support is available for self-managed setups only.
...
[MongoDB to SQL migration docs](/docs3/tyk-self-managed#migrating-from-an-existing-mongodb-instance)
...
You're also able to configure [upstream Headers dynamically](/docs3/api-management/data-graph#header-forwarding), that is, you’re able to inject Headers from the client request into UDG upstream requests. For example, it can be used to acccess protected upstreams. 
...
[Read more](/docs3/tyk-self-managed#manage-multi-environment-and-distributed-setups)
...
[Read More](/docs3/tyk-self-managed#set-up-liveness-health-checks)
...
[Read More](/docs3/tyk-self-managed#configure-load-balancing-and-weighting-via-the-dashboard)
...
If you are upgrading from version 2.8, pls [read this guide](/docs3/developer-support/release-notes/archived#upgrading-from-version-28)
