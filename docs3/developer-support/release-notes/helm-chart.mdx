...
For a comprehensive list of changes, please refer to the detailed [changelog](/docs3/#Changelog-v3.0.0) below.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/docs3/#Changelog-v2.2.0) below.
...
To configure, see [Tyk Stack](/docs3/product-stack/tyk-charts/tyk-stack-chart#audit-log-configurations) documentation.
</Accordion>
...
To configure, see [Tyk Stack](/docs3/product-stack/tyk-charts/tyk-stack-chart#opa-configurations) documentation.
</Accordion>
...
To configure, see [Tyk Stack](/docs3/product-stack/tyk-charts/tyk-stack-chart#gateway-probes) documentation.
</Accordion>
...
For a comprehensive list of changes, please refer to the detailed [changelog](/docs3/#Changelog-v2.1.0) below.

#### Breaking Changes
{/* Required. Use the following statement if there are no breaking changes, or explain if there are */}
This release has no breaking changes.

However, if you are upgrading to [Tyk Operator v1.0](/docs3/developer-support/release-notes/operator#100-release-notes) using the Helm Chart, please read the [license requirement](/docs3/developer-support/release-notes/operator#breaking-changesv1.0.0) and Tyk Operator [installation and upgrade instructions](/docs3/api-management/automations/operator#install-and-configure-tyk-operator) carefully.
...
- Familiarize yourself with URL matching in Tyk [here](/docs3/getting-started/key-concepts/url-matching).
- For production setup guidance, see [this guide](/docs3/planning-for-production#ensure-you-are-matching-only-the-url-paths-that-you-want-to-match).
...
Learn more about the settings in the [URL Path Matching](/docs3/getting-started/key-concepts/url-matching) documentation.
</Expandable>
</li>
...
MDCB v2.7.0 release introduces `/liveness` and `/readiness` probes which give more accurate and detail health check information. MDCB deployment has been updated to use the new endpoints. See [MDCB Health Check](/docs3/api-management/mdcb#health-check) section for information about the new probes.
...
For more details about new endpoints, check [MDCB Health check](/docs3/api-management/mdcb#health-check) section.
</Expandable>
</li>
</ul>
...
For a comprehensive list of changes, please refer to the detailed [changelog](/docs3/#Changelog-v1.5.0) below.
...
This feature adds a dependency on the Tyk Operator to the umbrella charts, facilitating the installation of the Tyk Operator component. Users can now easily install the Tyk Operator component by setting the `global.components.operator` parameter. Note that the Tyk Operator requires `cert-manager` to be installed beforehand. It also expects secret `tyk-operator-conf` is present in the installation namespace. You can enable bootstrapping at `global.components.bootstrap` if you are working on a new installation to have this secret created for you. Refer to the Tyk Operator [installation guide](/docs3/api-management/automations/operator#install-and-configure-tyk-operator) for detailed information on pre-requisites.
...
To leverage this stable release and simplify your Tyk deployments, we invite you to explore our example setup for MDCB Control Plane using Helm Chart. Simply follow our [MDCB Control Plane setup guide](/docs3/api-management/mdcb#setup-mdcb-control-plane) to get started.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/docs3/#Changelog-v1.4.0) below.
...
#### Breaking Changes
{/* Required. Use the following statement if there are no breaking changes, or explain if there are */}
For MongoDB users: Tyk Charts 1.3.0 uses `mongo-go` as the default driver to connect to MongoDB. `mongo-go` driver is compatible with MongoDB 4.4.x and above. For MongoDB versions prior to 4.4, please set `global.mongo.driver` to `mgo`. We recommend reading [Choose a MongoDB driver](/docs3/planning-for-production/database-settings#choose-a-mongodb-driver) when you need to change driver setting.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/docs3/#Changelog-v1.3.0) below.
...
##### Graph Pump
Tyk Charts 1.3 adds support for Graph MongoDB Pump, Graph SQL Pump and Graph SQL Aggregate Pump. see [Graph Pump setup](/docs3/api-management/tyk-pump#graph-pump-setup) to learn more about the GraphQL-specific metrics available.

##### Enable Tyk Identity Broker (TIB) in Tyk Dashboard
Tyk Charts 1.3 adds a field to enable Internal [Tyk Identity Broker (TIB)](/docs3/api-management/external-service-integration#what-is-tyk-identity-broker-tib) in Tyk Dashboard by field `tyk-dashboard.tib.enabled` to `true`.
...
When enabled at `gateway.control.ingress.enabled`, an Ingress resource will be created to allow external access to gateway's [control service](/docs3/planning-for-production#change-your-control-port).
</Accordion>

<Accordion title={'Gateway: Configure Gateway to work with MDCB synchroniser'}>
Allow users to configure worker gateway to work with [Tyk MDCB synchroniser](/docs3/api-management/mdcb#synchroniser-feature-with-mdcb) easily by setting `global.mdcbSynchronizer.enabled` in `tyk-data-plane`.
...
You can enable Internal [Tyk Identity Broker (TIB)](/docs3/api-management/external-service-integration#what-is-tyk-identity-broker-tib) in Tyk Dashboard by field `tyk-dashboard.tib.enabled` to `true`.
</Accordion>
...
read [Tyk Control Plane chart](/docs3/product-stack/tyk-charts/tyk-control-plane-chart).
</Accordion>
