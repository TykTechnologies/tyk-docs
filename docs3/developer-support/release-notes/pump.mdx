...
Tyk Pump now offers [FIPS 140-2](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.140-2.pdf) compliance. For further details please consult [Tyk API Management FIPS support](/docs3/developer-support/release-types/fips-release)
...
Added [FIPS compliance](/docs3/developer-support/release-types/fips-release) for Tyk Pump.
</Expandable>
</li>
</ul>
...
Users are strongly advised to follow the recommended [upgrade instructions](/docs3/#upgrading-tyk) provided by Tyk before applying any updates.
...
- When [`omit_configfile`](/docs3/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#omit_config_file) is set to `true`, Pump will not try to load the config file and spit out error logs

##### Updated
- Updated the default Hybrid Pump RPC pool size from 20 to 5 connections in order to reduce default CPU and memory footprint. See [Pump configurations](/docs3/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#pumpshybridmetarpcpoolsize)
