...
- **Long Term Support:** Refer to our [versioning and long-term support policies](/docs3/5.8/developer-support/release-types/long-term-support) for details on major and minor releases, patches, and support dates.
- **Preparations:** Review the [preparation guidelines](/docs3/5.8/#upgrade-guidelines) before starting the upgrade.
- **Release notes:** Always check the "Upgrade Instructions`" section in the relevant release notes.
- **Backups:** Follow our [comprehensive backup guide](/docs3/5.8/developer-support/faq#tyk-configuration) before starting the upgrade.
...
|----------------------| ---------------- | ----------- |
| **Tyk Cloud**        | [Cloud SaaS](/docs3/5.8/#tyk-cloud) | Guide to Tyk Cloud SaaS |
|                      | [Hybrid](/docs3/5.8/#tyk-hybrid) | Guide for Hybrid environments with Gateway Data Plane(s) deployed locally or with a third-party cloud provider |
|                      | [Go plugin](/docs3/5.8/#go-plugin) | Guide for upgrading Go plugin on the Tyk Cloud |
| **Tyk Self Managed** | [RHEL and CentOS](/docs3/5.8/#upgrade-linux-redhat-centos) | Guide for RPM-based Linux distributions |
|                      | [Debian and Ubuntu](/docs3/5.8/#upgrade-linux-debian) | Guide for DEB-based Linux distributions |
|                      | [Docker](/docs3/5.8/#docker) | Guide for upgrading Docker images |
|                      | [Helm](/docs3/5.8/#helm) | Guide for upgrading Helm Charts |
|                      | [Kubernetes](/docs3/5.8/#kubernetes) | Guide for upgrading Kubernetes environment |
| **Tyk MDCB Self Managed** | [MDCB](/docs3/5.8/#upgrade-mdcb) | Guide for upgrading Mutli Data Center Bridge (MDCB) |
| **Tyk Open Source**  | [Tyk Gateway](/docs3/5.8/#open-source-upgrade-guide) | Guide for upgrading Tyk open source environment |
...
Use our [migration tool](/docs3/5.8/planning-for-production/database-settings#migrating-from-an-existing-mongodb-instance) to manage the switch from MongoDB to SQL.

##### Backup APIs Script

Utilize our bash [backup script](/docs3/5.8/developer-support/upgrading#backup-apis-and-policies) to export and restore all Tyk API Definitions and Policies.
...
Which strategy do you intend to use?
  ...
  - If following the [Blue-Green upgrade](/docs3/5.8/#blue-green-upgrade) strategy, has the green environment been configured and verified as production-ready?
  - If pursuing the [Rolling upgrade](/docs3/5.8/#rolling-upgrade) strategy, do all Tyk components have a second instance?
...
Have backups been performed?
  ...
  - Backup guide: Have you checke our [comprehensive guide for backing up Tyk](/docs3/5.8/developer-support/faq#tyk-configuration)?

#### Go plugins
Do you use custom go plugins with your APIs?

  - Go plugin must be [recompiled](/docs3/5.8/#go-plugins-upgrade-guide) for the new version.
...
Use the [Upgrade Guides ToC](/docs3/5.8/#tyk-upgrade-guides-for-different-deployment-models) to choose the appropriate upgrade guide for
your platform.
...
3. **Upgrade Process:**
   ...
   - If using a new deployment process, follow the [deployment instructions](/docs3/5.8/tyk-self-managed) appropriate for your platform.
...
the [upgrade guidelines](/docs3/5.8/#upgrade-guidelines).
...
Follow our guide for [upgrading Cloud Control Planes](/docs3/5.8/tyk-cloud#upgrade-control-planes).

**Step 2. Upgrade Go Plugins**

Follow our guide for deploying your [Go plugins on Tyk Cloud](/docs3/5.8/#go-plugin). Subsequently, follow the steps below according to the target upgrade version of the Gateway.

**Gateway Versions < 4.1.0.**

1. Proceed with [upgrading your Tyk Data Plane (Gateway)](/docs3/5.8/#upgrading-cloud-data-planes)
2. Update the [custom_middleware_bundle](/docs3/5.8/api-management/plugins/overview#plugin-bundles) field in the API Definitions of all APIs that use your plugin. The field should be updated to use the new bundle file containing your upgrade plugin.
3. Validate that your plugin is working per your expectations.

**Gateway Versions >= 4.1.0**

1. Update the [custom_middleware_bundle](/docs3/5.8/api-management/plugins/overview#plugin-bundles) field in the API Definitions of all APIs that use your plugin. The field should be updated to use the new bundle file containing your upgraded plugin.
...
3. Proceed with [upgrading your Tyk Data Plane (Gateway)](/docs3/5.8/#upgrading-cloud-data-planes). Given that you loaded your target version plugin in step 1, this version will be loaded automatically once you upgrade.
...
Follow our guide for [upgrading Cloud Data Planes](/docs3/5.8/tyk-cloud#managing-cloud-data-plane).
...
After following the guidelines for [preparing for upgrade](/docs3/5.8/#upgrade-guidelines),
...
See Tyk Guide for how to [upgrade Control Planes](/docs3/5.8/tyk-cloud#upgrade-control-planes)

**Step 2 Upgrade Go plugins**

Follow our guide for [upgrading your Go plugins on Tyk Cloud](/docs3/5.8/#go-plugin). Subsequently, follow the steps below according to the target upgrade version of the Gateway.

**Gateway Versions < 4.1.0**

1. Proceed with upgrading your [Tyk Data Plane Hybrid Gateways](/docs3/5.8/#upgrading-data-plane-hybrid-gateways).
2. Update the [custom_middleware_bundle](/docs3/5.8/api-management/plugins/overview#plugin-bundles)
...
1. Update the [custom_middleware_bundle](/docs3/5.8/api-management/plugins/overview#plugin-bundles)
field in the API Definitions of all APIs that use your plugin. The field should be updated to use the new bundle file
...
3. Proceed with upgrading your [Tyk Data Plane Hybrid Gateways](/docs3/5.8/#upgrading-data-plane-hybrid-gateways). Given that you
...
- [Docker](/docs3/5.8/#docker)
- [Helm](/docs3/5.8/#helm)
- [Linux Debian](/docs3/5.8/#upgrade-linux-debian)
- [Linux RHEL/CENTOS](/docs3/5.8/#upgrade-linux-redhat-centos)
...
2. [Compile your custom Go plugins](/docs3/5.8/#go-plugins-upgrade-guide).
...
| ---  | --- | --- |
| [Path 1](/docs3/5.8/#path-1)    | < 4.1.0 | < 4.1.0 |
| [Path 2](/docs3/5.8/#path-2)    | >= 4.1.0 | >= 4.2.0 |

**Path 1 - Current Version < 4.1.0 and Target Version < 4.1.0**

1. [Create a plugin bundle](/docs3/5.8/api-management/plugins/overview#plugin-bundles) that includes the newly compiled version
...
2. [Upload this bundle](/docs3/5.8/tyk-cloud#uploading-your-bundle) to your configured S3 bucket if using Cloud SaaS. If you're using Hybrid SaaS, upload this bundle to your bundle server.


**Path 2 - Current `Version >= 4`.1.0 and Target `Version >= 4`.2.0**

1. [Create a plugin bundle](/docs3/5.8/api-management/plugins/overview#plugin-bundles) that includes both your current version’s plugin along with the newly compiled version
...
the addition of the [Multi Data Center Bridge](/docs3/5.8/tyk-multi-data-centre) (MDCB).
...
Follow our [Tyk OSS guide](/docs3/5.8/#go-plugins-upgrade-guide) for upgrading Tyk Gateway and Tyk Pump
...
After reviewing guidelines for [preparing for upgrade](/docs3/5.8/#upgrade-guidelines),
follow the instructions below to upgrade your Tyk components and plugins.

**Upgrade order:**

Please note that upgrade order is as explained in the upgrade [overview](/docs3/5.8/#tyk-self-managed-upgrade-guide)
...
Before executing the upgrade, ensure that you have consulted and performed all the necessary steps in the [pre upgrade checklist](/docs3/5.8/#upgrade-guidelines).
...
the [self managed](/docs3/5.8/#tyk-self-managed-upgrade-guide) section, and look for the instruction regarding Tyk Gateway.

Before proceeding with the upgrade process, ensure that you have thoroughly reviewed and completed the steps outlined in
the [pre-upgrade guidelines](/docs3/5.8/#upgrade-guidelines). Once you
...
For full installation details, check the usual [installation page](/docs3/5.8/apim/open-source/installation#install-tyk-gateway-with-docker).
...
3. Review and complete the steps outlined in the [pre-upgrade guidelines](/docs3/5.8/#upgrade-guidelines).
...
|-----------------|-----------------|----------------|
| [Path 1](/docs3/5.8/#path-1)    | < 4.1.0         | < 4.1.0        |
| [Path 2](/docs3/5.8/#path-2)    | < 4.1.0         | >= 4.1.0       |
| [Path 3](/docs3/5.8/#path-3)    | >= 4.1.0        | >= 5.1.0       |
...
If you are using Self Managed deployment then we recommend that you use [Tyk Sync](/docs3/5.8/api-management/automations/sync) to backup your Tyk APIs and policies. 
