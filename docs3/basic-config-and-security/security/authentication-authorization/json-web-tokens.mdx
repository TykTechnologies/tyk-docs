...
As noted in the Tyk Classic API [documentation](/docs3/api-management/gateway-config-tyk-classic#configuring-authentication-for-tyk-classic-apis), you can select JSON Web Token authentication using the `use_jwt` option.
...
1. [Select JSON Web Tokens](/docs3/basic-config-and-security/security/authentication-authorization/json-web-tokens#configuring-your-api-to-use-jwt-authentication) as the Authentication mode
2. [Set the cryptographic signing method](/docs3/basic-config-and-security/security/authentication-authorization/json-web-tokens#set-up-jwt-signing-method) to `HMAC (shared)` and the public secret as `tyk123`
...
If Tyk cannot find a `pol` claim, it will apply this Default Policy. Select a policy that gives access to this API we are protecting, or [create one](/docs3/api-management/gateway-config-managing-classic#secure-an-api) if none exists.
...
If you are looking to POC this functionality in Kubernetes, you can run a fully worked-out example using our tyk-k8s-demo library. You can read more [here](/docs3/deployment-and-operations/tyk-self-managed/tyk-demos-and-pocs/overview#kubernetes-demo).
...
- HMAC shared secret
- ECDSA
- [Public JWKS URL](/docs3/basic-config-and-security/security/authentication-authorization/json-web-tokens#enable-dynamic-public-key-rotation-using-jwks)
