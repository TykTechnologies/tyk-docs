...
You can retrieve a value from KV storage by including a reference in the [appropriate notation](/docs3/tyk-self-managed#transformation-middleware) for the KV location where the key-value pair is stored.

If you use a value retrieved from [Consul](/docs3/tyk-self-managed#consul) or [Vault](/docs3/tyk-self-managed#vault), this must be the <b>last</b> part in the `rewriteTo` URL.
...
This limitation does not apply to KV accessed from the other [supported KV stores](/docs3/tyk-self-managed#store-configuration-with-key-value-store) (environment variable or Gateway `secrets`).
