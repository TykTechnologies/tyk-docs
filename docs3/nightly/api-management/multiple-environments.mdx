...
See [Tyk Gateway Configuration Options](/docs3/nightly/tyk-oss-gateway/configuration) for more details on node tags.
...
See [API Tagging](/docs3/nightly/api-management/multiple-environments#api-tagging-with-on-premises) for more details.

#### API Sharding

You can also use [API Sharding](/docs3/nightly/api-management/multiple-environments#what-is-api-sharding-) to move APIs in a Shards (and or MDCB) Tyk Self-Managed installation.
...
You can also use our new Tyk-Sync tool which allows you to sync your APIs (and Policies) with a Version Control System (VCS). You can then move your APIs between environments. See [Tyk-Sync](/docs3/nightly/api-management/automations/sync) for more details.
...
To create a key with a custom identifier, ie Token, simply use the [Gateway (OSS)](/docs3/nightly/tyk-gateway-api) or [Dashboard (Pro)](/docs3/nightly/api-management/dashboard-configuration#create-a-custom-key) REST APIs to import a custom key.
...
You can also use our new Tyk-Sync tool which allows you to sync your Policies (and APIs) with a Version Control System (VCS). You can then move your Policies between environments. See [Tyk-Sync](/docs3/nightly/api-management/automations/sync) for more details.
...
Couple this functionality with the Tyk [Multi Data Center Bridge](/docs3/nightly/api-management/mdcb#managing-geographically-distributed-gateways-to-minimize-latency-and-protect-data-sovereignty) to achieve a global, multi-cloud deployment.
...
Imagine the case where you have two sets of APIs, Internal & External.  You want to prevent your Internal APIs from being accessed or visible outside your protected network.  Well, [sharding](/docs3/nightly/api-management/multiple-environments#what-is-api-sharding-) makes it extremely easy to configure your Tyk Gateways from the Dashboard.
...
<Note>
In order to expose more details about the Gateway to the Dashboard, you can now configure the [edge_endpoints](/docs3/nightly/tyk-dashboard/configuration#edge_endpoints) section in the tyk-analytics.conf, and the Dashboard UI will pick that up and present you a list of Gateways you can chose from when creating an API.
</Note>
...
From version 3.2.2 of the Tyk Dashboard, if [edge_endpoints](/docs3/nightly/tyk-dashboard/configuration#edge_endpoints) are being configured in tyk-analytics.conf, your Dashboard will automatically pick that list up for you, and display it in the UI when you create your API.
