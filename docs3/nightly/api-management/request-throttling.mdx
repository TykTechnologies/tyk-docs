...
Tyk's Request Throttling feature provides a mechanism to manage traffic spikes by queuing and automatically retrying client requests that exceed [rate limits](/docs3/nightly/api-management/rate-limit), rather than immediately rejecting them. This helps protect upstream services from sudden bursts and improves the resilience of API interactions during temporary congestion.
...
Request Throttling is configured within Tyk [Security Policies](/docs3/nightly/api-management/policies) or directly on individual [Access Keys](/docs3/nightly/api-management/authentication/bearer-token).
...
Tyk's Request Throttling intercepts API requests *after* they have exceeded a configured [Rate Limit](/docs3/nightly/api-management/rate-limit). 
...
Refer to this [documentation](/docs3/nightly/#configuration-options).
</Accordion>
