...
Tyk can act as an OAuth 2.0 *authorization server*, performing token generation and management for *clients* accessing APIs deployed on Tyk. There are many great resources on the Internet that will help you to understand the OAuth 2.0 Authorization Framework, which we won't attempt to duplicate here. We will provide a basic introduction to the [concepts and terminology](/docs3/nightly/#oauth-20-core-concepts) before we dive into the details of using Tyk as your *auth server*.
...
- **Multi-API Access**: Enable access to multiple APIs using a single OAuth token; configure one API for OAuth 2.0 token issuance and the other APIs with the [Auth Token](/docs3/nightly/api-management/authentication/bearer-token) method, linking them through a common policy

*Tyk as OAuth authorization server* supports the following *grant types*:

- [Authorization Code Grant](/docs3/nightly/#using-the-authorization-code-grant): the *client* is redirected to an *identity server* where the *user* must approve access before an *access token* will be issued
- [Client Credentials Grant](/docs3/nightly/#using-the-client-credentials-grant): used for machine-to-machine access, authentication is performed using only the *client Id* and *client secret*
- [Resource Owner Password Grant](/docs3/nightly/#using-the-resource-owner-password-grant) (a.k.a. Password Grant): only for use where the *client* is highly trusted, as the *client* must provide the *Resource Owner*'s own credentials during authentication
...
- [register a client app](/docs3/nightly/#client-app-registration) for each client that needs to access the API
- [configure your API proxy](/docs3/nightly/#configuring-your-api-proxy) to use the *Tyk OAuth 2.0* authentication method
...
- redirect URI
- [optional] [security policies](/docs3/nightly/#manage-client-access-policies) to be applied to access tokens generated for the client
- [optional] [metadata](/docs3/nightly/api-management/policies#what-is-a-session-metadata) to be added to the access tokens
...
| :--- | :--- | :--- |
| Register a new client app | `POST /api/apis/oauth/{{api-id}}` | [link](/docs3/nightly/api-management/dashboard-configuration#create-a-new-oauth20-client) |
| Get a list of registered client apps | `GET /api/apis/oauth/{{api-id}}` | [link](/docs3/nightly/api-management/dashboard-configuration#list-oauth-clients) |
| Get the details of a client app | `GET /api/apis/oauth/{{api-id}}/{{client_id}}` | [link](/docs3/nightly/api-management/dashboard-configuration#get-an-oauth20-client) |
| Delete a client app | `DELETE /api/apis/oauth/{{api-id}}/{{client_id}}` | [link](/docs3/nightly/api-management/dashboard-configuration#delete-oauth-client) |


## Using the Authorization Code Grant

When using Tyk as the Authorization Server with the Authorization Code grant, the following steps are followed after [registering the Client App](/docs3/nightly/#client-app-registration):

<img src="/img/diagrams/diagram_docs_authorization-code-grant-type@2x.png" alt="Authorization grant type flow" />

**Explanatory notes:**

(1) *client* makes a request to the [authorization endpoint](/docs3/nightly/#authorization-request) on the *Auth Server*
...
(6) when the *user* successfully authenticates and authorizes the request, the *Identity Server* must request an [Authorization Code](/docs3/nightly/#authorization-code-request) from the *Auth Server*

(8) The *Identity Server* provides the *Authorization Code* to the *client*

(9) The *client* exchanges the *Authorization Code* for an [Access Token](/docs3/nightly/#exchange-the-authorization-code-for-an-access-token) from the *Auth Server*

(10) The *client* uses the *Access Token* to authenticate with the protected API using the [Auth Token](/docs3/nightly/api-management/authentication/bearer-token) method
...
The identity server will need access to the Tyk Dashboard API to [obtain an Authorization Code](/docs3/nightly/api-management/dashboard-configuration#oauth20-authorization-code).
...
This endpoint is protected using [Basic Authentication](/docs3/nightly/api-management/authentication/basic-authentication) where the username is the *client Id* and the password is the *client secret*.
...
This command, issued by the *client* is the final step to obtain an access token for the `/my-api` proxy deployed on a Tyk Gateway at `https://tyk.cloud.tyk.io`. The basic auth key is the base64 encoded representation of `my-client-id:my-client-secret` The `client_id` and `redirect_uri` match those provided in the initial [authorization request](/docs3/nightly/#authorization-request). The `code` is the *authorization code* provided to the *identity server* in the [authorization code request](/docs3/nightly/#authorization-code-request).
...
- `token_type`: set to `bearer` indicating that the access token should be provided in an [Auth Token](/docs3/nightly/api-management/authentication/bearer-token) request to the protected API
- `refresh_token`: [optional] a special token that can be used in the [Refresh Token](/docs3/nightly/#using-refresh-tokens) flow
...
When using Tyk as the *authorization server* with the Client Credentials grant, the *client* accesses resources on behalf of itself rather than on behalf of a *user*, so there is no user login/authorization step (as seen with [Authorization Code grant](/docs3/nightly/#using-the-authorization-code-grant)). This flow is ideal for server-to-server interactions.

After [registering the Client App](/docs3/nightly/#client-app-registration), the *client* simply requests an access token directly from the authorization server:
...
When using Tyk as the *authorization server* with the Resource Owner Password grant, the *client* provides the *user's* credentials when requesting an access token. There is no user login/authorization step (as seen with [Authorization Code grant](/docs3/nightly/#using-the-authorization-code-grant)). **This flow is not recommended and is provided only for integration with legacy clients.**
...
You can optionally [strip the user credentials](/docs3/nightly/api-management/client-authentication#managing-authorization-data) from the request prior to proxying to the upstream using the `authentication.stripAuthorizationData` field (Tyk Classic: `strip_auth_data`).

With the OAuth method selected, you'll need to configure Tyk to handle the specific configuration of OAuth grants that you will support. All of the OAuth specific configuration is performed within the [authentication.securitySchemes.oauth](/docs3/nightly/api-management/gateway-config-tyk-oas#oauth) object in the Tyk Vendor Extension.
...
- [Notifications](/docs3/nightly/#oauth-token-notifications) of token issuance will be sent to `http://notifyme.com` with the `X-Tyk-Shared-Secret` header set to `oauth-shared-secret`
...
As noted in the Tyk Classic API [documentation](/docs3/nightly/api-management/gateway-config-tyk-classic#configuring-authentication-for-tyk-classic-apis), you can select the Tyk as OAuth Server method using the `use_oauth2` option.
...
You configure whether Tyk should issue a refresh token within the [API proxy definition](/docs3/nightly/#configuring-your-api-proxy).
...
- [retrieve a list of all tokens for a client app](/docs3/nightly/api-management/dashboard-configuration#retrieve-all-current-tokens-for-specified-oauth20-client)
- [revoke a single token](/docs3/nightly/api-management/dashboard-configuration#revoke-a-single-oauth-client-token)
- [revoke all tokens for a client app](/docs3/nightly/api-management/dashboard-configuration#revoke-all-oauth-client-tokens)
...
The example given [above](/docs3/nightly/#using-the-api-definition) includes the configuration necessary to issue notifications for token issuance (see lines 48-51 in the example).
