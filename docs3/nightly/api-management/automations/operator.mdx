...
sidebarTitle: "API Management in Kubernetes"
...
**TykOasApiDefinition**: Available from Tyk Operator v1.0. It represents a [Tyk OAS API configuration](/docs3/nightly/api-management/gateway-config-tyk-oas). Tyk OAS API is based on the OpenAPI specification (OAS) and is the recommended format for standard HTTP APIs.

**ApiDefinition**: Available on all versions of Tyk Operator. It represents a [Tyk Classic API configuration](/docs3/nightly/api-management/gateway-config-tyk-classic). Tyk Classic API is the traditional format used for defining all APIs in Tyk, and now the recommended format for non-HTTP APIs such as TCP, GraphQL, and Universal Data Graph (UDG). Tyk Operator supports the major features of Tyk Classic API and the feature support details can be tracked [here](/docs3/nightly/#apidefinition-crd).

**TykStreamsApiDefinition**: Available from Tyk Operator v1.1. It represents an [Async API configuration](/docs3/nightly/api-management/event-driven-apis#configuration-options) which is based on [Tyk OAS API Definition](/docs3/nightly/api-management/gateway-config-tyk-oas). Tyk Operator supports all [Tyk Streams](/docs3/nightly/api-management/event-driven-apis#) features as they become available on the Gateway.

**SecurityPolicy**: Available on all versions of Tyk Operator. It represents a [Tyk Security Policy configuration](/docs3/nightly/#security-policy-example). Security Policies in Tyk provide a way to define and enforce security controls, including authentication, authorization, and rate limiting for APIs managed in Tyk. Tyk Operator supports essential features of Security Policies, allowing users to centrally manage access control and security enforcement for all APIs across clusters.
...
The following custom resources can be used to configure APIs and policies at [Tyk Gateway](/docs3/nightly/tyk-oss-gateway) or [Tyk Dashboard](/docs3/nightly/tyk-dashboard).

| Kind               | Group       | Version   | Description                                                                                       |
|--------------------|-------------|-----------|---------------------------------------------------------------------------------------------------|
| TykOasApiDefinition| tyk.tyk.io  | v1alpha1  | Defines configuration of [Tyk OAS API Definition object](/docs3/nightly/api-management/gateway-config-tyk-oas)                                 |
| ApiDefinition      | tyk.tyk.io  | v1alpha1  | Defines configuration of [Tyk Classic API Definition object](/docs3/nightly/api-management/gateway-config-tyk-classic)                                 |
| TykStreamsApiDefinition| tyk.tyk.io  | v1alpha1  | Defines configuration of [Tyk Streams](/docs3/nightly/api-management/event-driven-apis#configuration-options)                                 |
| SecurityPolicy     | tyk.tyk.io  | v1alpha1  | Defines configuration of [security policies](/docs3/nightly/api-management/policies#what-is-a-security-policy). Operator supports linking ApiDefinition custom resources in SecurityPolicy's access list so that API IDs do not need to be hardcoded in the resource manifest.        |
| SubGraph           | tyk.tyk.io  | v1alpha1  | Defines a [GraphQL federation subgraph](/docs3/nightly/api-management/graphql#subgraphs-and-supergraphs).                                           |
| SuperGraph         | tyk.tyk.io  | v1alpha1  | Defines a [GraphQL federation supergraph](/docs3/nightly/api-management/graphql#subgraphs-and-supergraphs).                                        |
| OperatorContext    | tyk.tyk.io  | v1alpha1  | Manages the context in which the Tyk Operator operates, affecting its overall behavior and environment. See [Operator Context](/docs3/nightly/#multi-tenancy-in-tyk) for details. |

#### Tyk Classic Developer Portal

The following custom resources can be used to configure [Tyk Classic Developer Portal](/docs3/nightly/tyk-developer-portal/tyk-portal-classic).

| Kind               | Group       | Version   | Description                                                                                       |
|--------------------|-------------|-----------|---------------------------------------------------------------------------------------------------|
| APIDescription     | tyk.tyk.io  | v1alpha1  | Configures [Portal Documentation](/docs3/nightly/tyk-apis/tyk-portal-api/portal-documentation). |
| PortalAPICatalogue | tyk.tyk.io  | v1alpha1  | Configures [Portal API Catalogue](/docs3/nightly/getting-started/key-concepts/api-catalogue). |
| PortalConfig       | tyk.tyk.io  | v1alpha1  | Configures [Portal Configuration](/docs3/nightly/tyk-apis/tyk-portal-api/portal-configuration). |
...
### Operator User
Tyk Operator is a Kubernetes Controller that manages Tyk Custom Resources (CRs) such as API Definitions and Security Policies. Developers define these resources as [Custom Resource (CRs)](/docs3/nightly/#custom-resources-in-tyk), and Tyk Operator ensures that the desired state is reconciled with the Tyk Gateway or Dashboard. This involves creating, updating, or deleting API configurations until the target state matches the desired state.
...
Tyk Dashboard is multi-tenant capable, which means you can use a single Tyk Dashboard instance to host separate [organizations](/docs3/nightly/api-management/dashboard-configuration#organizations) for each team or department. Each organization is a completely isolated unit with its own:
...
|------------------|-------------|---------|---------|
| Client certifates | ✅ [Client mTLS](/docs3/nightly/basic-config-and-security/security/mutual-tls/client-mtls#setup-static-mtls-in-tyk-operator-using-the-tyk-classic-api-definition) | ✅ [Client mTLS](/docs3/nightly/basic-config-and-security/security/mutual-tls/client-mtls#setup-static-mtls-in-tyk-operator-using-tyk-oas-api-definition) | Certificate ID can be set in the API Definition but configuring certificates from Secrets in CRD is not supported. |
| Custom domain certificates | ✅ [TLS and SSL](/docs3/nightly/api-management/certificates#dynamically-setting-ssl-certificates-for-custom-domains) | ✅ [TLS and SSL](/docs3/nightly/api-management/certificates#dynamically-setting-ssl-certificates-for-custom-domains) | Certificate ID can be set in the API Definition but configuring certificates from Secrets in CRD is not supported. |
| Public keys pinning | ✅ [Certificate pinning](/docs3/nightly/api-management/upstream-authentication/mtls#using-tyk-operator-to-configure-mtls-for-tyk-classic-apis) | ✅ [Certificate pinning](/docs3/nightly/api-management/upstream-authentication/mtls#certificate-pinning) | Certificate ID can be set in the API Definition but configuring certificates from Secrets in CRD is not supported. |
| Upstream mTLS | ✅ [Upstream mTLS via Operator](/docs3/nightly/api-management/upstream-authentication/mtls#using-tyk-operator-to-configure-mtls-for-tyk-classic-apis) | ✅ [Upstream mTLS via Operator](/docs3/nightly/api-management/upstream-authentication/mtls#using-tyk-operator-to-configure-mtls) | Certificate ID can be set in the API Definition but configuring certificates from Secrets in CRD is not supported. |


## Install and Configure Tyk Operator

We assume you have already installed Tyk. If you don’t have it, check out [Tyk
Cloud](/docs3/nightly/tyk-cloud#quick-start-tyk-cloud) or [Tyk Self
Managed](/docs3/nightly/tyk-self-managed) page. [Tyk Helm
Chart](/docs3/nightly/product-stack/tyk-charts/overview) is the preferred (and easiest) way to install Tyk on Kubernetes.
...
Tyk Operator needs a [user credential](/docs3/nightly/#operator-user) to connect with
...
[Using Tyk Operator to enable GitOps with Tyk](/docs3/nightly/api-management/automations) about
...
Alternatively, you have the option to manually handle TLS certificates by disabling the `cert-manager` requirement. For more details, please refer to this [configuration](/docs3/nightly/#webhook-configuration).
...
If you are using [Tyk Stack](/docs3/nightly/product-stack/tyk-charts/tyk-stack-chart), [Tyk Control
Plane](/docs3/nightly/product-stack/tyk-charts/tyk-control-plane-chart), or [Tyk Open
Source Chart](/docs3/nightly/product-stack/tyk-charts/tyk-oss-chart), you can install Tyk Operator alongside other Tyk
...
another name, configure it through Helm Chart [envFrom](/docs3/nightly/#install-tyk-operator-and-custom-resource-definitions-crds) value.
...
You can configure [Tyk Operator as Ingress Controller](/docs3/nightly/#control-kubernetes-ingress-resources) so
...
[WATCH_INGRESS_CLASS](/docs3/nightly/#configure-tyk-operator-via-environment-variable-or-tyk-operator-conf-secret) through `tyk-operator-conf` or the environment variable.
...
You can configure your API for any Tyk supported authentication method by following the [Client Authentication](/docs3/nightly/api-management/client-authentication) documentation.
...
First, specify the details of your API using the [ApiDefinition CRD](/docs3/nightly/#apidefinition-crd), then deploy it to create the corresponding Kubernetes resource. Tyk Operator will take control of the CRD and create the actual API in the Tyk data plane.
...
- [HTTP Proxy example](/docs3/nightly/#set-up-manifest-for-http)
- [TCP Proxy example](/docs3/nightly/#set-up-manifest-for-tcp)
- [GraphQL Proxy example](/docs3/nightly/#set-up-manifest-for-graphql)
- [UDG example](/docs3/nightly/#set-up-manifest-for-udg)
...
You can visit the [ApiDefinition CRD](/docs3/nightly/#apidefinition-crd) page to see all the latest API Definitions fields and features we support.
...
<Note>
Tyk Operator supported authentication types are listed in the [API Definition features](/docs3/nightly/#apidefinition-crd) section.
</Note>


#### Create an API key

You need to generate a key to access the `httpbin` API now. Follow [this guide](/docs3/nightly/getting-started/configure-first-api#create-an-api-key) to see how to create an API key for your installation. 
...
This configuration allows [keyless (open)](/docs3/nightly/basic-config-and-security/security/authentication-authorization/open-keyless) access to the API without any authentication.
...
This setup requires a [bearer token](/docs3/nightly/api-management/authentication/bearer-token) for access.
...
- `validate_signature`: Enable [signature validation](/docs3/nightly/api-management/authentication/bearer-token#auth-token-with-signature).
...
This configuration uses [JWT tokens](/docs3/nightly/basic-config-and-security/security/authentication-authorization/json-web-tokens) for authentication.

Users can configure JWT authentication by defining the following fields:

- `jwt_signing_method`: Specify the method used to sign the JWT. Refer to [JWT Signing Method](/docs3/nightly/basic-config-and-security/security/authentication-authorization/json-web-tokens#set-up-jwt-signing-method) for supported methods.
...
This configuration uses [Basic Authentication](/docs3/nightly/api-management/authentication/basic-authentication), requiring a username and password for access.
...
This configuration uses a [Golang plugin](/docs3/nightly/api-management/plugins/golang#) for custom authentication. The following example shows how to create an API definition with a Golang custom plugin for `httpbin-go-auth`.

For an example of Golang authentication middleware, see [Performing custom authentication with a Golang plugin](/docs3/nightly/api-management/plugins/golang#performing-custom-authentication-with-a-golang-plugin).
  ...
  custom_middleware:
    ...
    pre:
      - name: "AddFooBarHeader"
        path: "/docs3/nightly/mnt/tyk-gateway/example-go-plugin.so"
    auth_check:
...
This configuration uses a [gRPC plugin](/docs3/nightly/api-management/plugins/golang#) for custom authentication. The following example shows how to create an API definition with a gRPC custom plugin for `httpbin-grpc-auth`.
...
This setup allows for [multiple authentication](/docs3/nightly/basic-config-and-security/security/authentication-authorization/multiple-auth) methods to be chained together, requiring clients to pass through each specified authentication provider.

To enable multiple (chained) auth, you should set `base_identity_provided_by` field to one of the supported chained enums. Consult the [Multi (Chained) Authentication](/docs3/nightly/basic-config-and-security/security/authentication-authorization/multiple-auth) section for the supported auths.
...
To enable [IP Allowlist](/docs3/nightly/api-management/gateway-config-tyk-classic#ip-access-control), set the following fields:
...
To enable [IP Blocklist](/docs3/nightly/api-management/gateway-config-tyk-classic#ip-access-control), set the following fields:
...
In the example below we can see that the configuration is contained within the `graphql` configuration object. A GraphQL schema is specified within the `schema` field and the execution mode is set to `proxyOnly`. The [GraphQL public playground](/docs3/nightly/api-management/graphql#enabling-public-graphql-playground) is enabled with the path set to `/playground`.
...
If you are on Tyk 3.2 and above, you can use the following manifest to create an UDG API. This example configures a Universal Data Graph from a [GraphQL datasource](/docs3/nightly/api-management/data-graph#graphql) and a [REST Datasource](/docs3/nightly/api-management/data-graph#rest).
...
To secure your Tyk Streams API, configure security fields in the OAS definition just as you would for a standard Tyk OAS API. For more details, refer to the [Secure your Tyk OAS API](/docs3/nightly/#secure-your-tyk-oas-api) guide.

## Add a Security Policy to your API
To further protect access to your APIs, you will want to add a security policy. 
Below, we take you through how to define the security policy but you can also find [Security Policy Example](/docs3/nightly/#security-policy-example) below.
...
In this example, the security policy will apply to an `ApiDefinition` resource named `httpbin` in the `default` namespace, a `TykOasApiDefinition` resource named `petstore` in the `default` namespace, and a `TykStreamsApiDefinition` resource named `http-to-kafka` in the `default` namespace. Note that with Tyk Operator, you do not need to specify API ID as in the raw [Policy definition](/docs3/nightly/api-management/policies#policies-guide). Tyk Operator will automatically retrieve the API ID of referenced API Definition resources for you.
...
<h4 id="security-policy-example">Security policy example</h4>
<h5 id="key-level-per-api-rate-limits-and-quotas">Key-level per-API rate limits and quota</h5>
...
<h5 id="per-endpoint-rate-limit">Key-level per-endpoint rate limits</h5>
...
<h5 id="path-based-permissions">Path based permissions</h5>
You can secure your APIs by specifying [allowed URLs](/docs3/nightly/api-management/policies#secure-your-apis-by-method-and-path) (methods and paths) for each API within a security policy. This is done using the `allowed_urls` field under `access_rights_array`.
...
Tyk Operator by default looks for the value `tyk` and will ignore all other ingress classes. If you wish to override this default behavior,
 you may do so by setting the environment variable `WATCH_INGRESS_CLASS` in the operator manager deployment. [See Installing Tyk Operator](/docs3/nightly/#install-and-configure-tyk-operator) for further information.
...
Each path in an Ingress must have its own particular path type. Kubernetes offers three types of path types: `ImplementationSpecific`, `Exact`, and `Prefix`. Currently, not all path types are supported. The below table shows the unsupported path types for [Sample HTTP Ingress Resource](/docs3/nightly/#set-up-manifest-for-http) based on the examples in the [Kubernetes Ingress documentation](https://kubernetes.io/docs/concepts/services-networking/ingress/#examples).
...
Please bear in mind that if `proxy.strip_listen_path` is set to true on API Definition, Tyk strips the listen-path (for example, the listen-path for the Ingress under [Sample HTTP Ingress Resource](/docs3/nightly/#set-up-manifest-for-http) is /httpbin) with an empty string.
...
Tyk, with release *v4.0* offers [GraphQL federation](/docs3/nightly/api-management/graphql#federation-version-support) that allows you to divide GraphQL implementation across multiple back-end
...
Tyk Operator uses Custom Resources called [SubGraph](/docs3/nightly/#subgraph) and [SuperGraph](/docs3/nightly/#supergraph), that allows users to model the relationship between Subgraphs and Supergraphs.
...
Let's assume that a developer responsible for the [Users SubGraph](/docs3/nightly/#users-subgraph) would like to delete `username` field from the Users SubGraph.
Also, the [Supergraph](/docs3/nightly/#supergraph-1) called Social Media already uses the Users Subgraph.
...
each of the following [hooks](/docs3/nightly/api-management/plugins/plugin-types#plugin-and-hook-types) in the API request
lifecycle: [Pre (Request)](/docs3/nightly/api-management/plugins/plugin-types#request-plugins), [Authentication](/docs3/nightly/api-management/plugins/plugin-types#authentication-plugins),
[Post (Request)](/docs3/nightly/api-management/plugins/plugin-types#request-plugins), [Post
Authentication](/docs3/nightly/api-management/plugins/plugin-types#request-plugins), [Response](/docs3/nightly/api-management/plugins/plugin-types#response-plugins)
and [Analytics](/docs3/nightly/api-management/plugins/plugin-types#analytics-plugins). Subsequently, users can execute, or “hook”, their
...
Please refer to [Analytics Plugins](/docs3/nightly/api-management/plugins/plugin-types#analytics-plugins) to learn how to configure Analytics
...
languages](/docs3/nightly/api-management/plugins/overview#plugin-driver-names) section for list of supported plugin driver names.
...
[per-endpoint](/docs3/nightly/api-management/plugins/plugin-types#per-endpoint-custom-plugins) level plugins are not currently
...
2. **Create API Definition Resource**
    ...
    *http://httpbin.org*.
      ...
      custom_middleware:
        ...
        pre:
          - name: "exampleJavaScriptMiddlewarePreHook"
            path: "/docs3/nightly/middleware/example-javascript-middleware.js"
    ```
    ...
    - The `driver` configuration parameter is set to `otto` at line 15, since our plugin is a Javascript plugin. For other
      valid values please refer to the [plugins driver page](/docs3/nightly/api-management/plugins/overview#plugin-driver-names).
    - A plugin hook configuration block is specified at line 16, containing the `name` and `path` for our plugin. The plugin
      ...
      middleware. Valid values are the same as the [Tyk Classic API Definition](/docs3/nightly/#how-it-works) equivalent, i.e.
...
request lifecycle](/docs3/nightly/api-management/traffic-transformation#request-middleware-chain).
...
bundles](/docs3/nightly/api-management/plugins/overview#plugin-bundles), load them into the Tyk Gateway, and configure an API
...
An [OperatorContext](/docs3/nightly/#multi-tenancy-in-tyk) specifies the parameters for connecting and authenticating with a Tyk Dashboard. Below is an example of how to define an `OperatorContext`:
...
The concept of [internal looping](/docs3/nightly/advanced-configuration/transform-traffic/looping) allows you to use URL Rewriting to redirect your URL to *another API endpoint* or to *another API* in the Gateway. In Tyk, looping is generally targeted using the `tyk://<API_ID>/<path>` scheme, which requires prior knowledge of the `API_ID`. Tyk Operator simplifies the management and transformation of API traffic within Kubernetes environments by abstracting APIs as objects, managing them and dynamically assigning `API_ID`s by its Kubernetes metedata name and namespace.

### Configuring looping to internal ApiDefinition resources

Looping can be configured within Tyk Operator for [URL Rewrites](/docs3/nightly/#url-rewrites), [URL Rewrite Triggers](/docs3/nightly/#url-rewrite-triggers) and [Proxy to internal APIs](/docs3/nightly/#proxy-to-internal-apis) by configuring the `rewrite_to_internal` in `url_rewrite`, `rewrite_to_internal` in `triggers`, and `proxy.target_internal` fields respectively with these properties:
...
[URL rewriting](/docs3/nightly/transform-traffic/url-rewriting#url-rewrite-middleware) in Tyk enables the alteration of incoming API request paths to align with the expected endpoint format of your backend services.
...
[Triggers](/docs3/nightly/transform-traffic/url-rewriting#url-rewrite-triggers) are configurations that specify actions based on certain conditions present in HTTP headers, query parameters, path parameters etc.
...
The process for configuring internal looping in triggers to is similar to that explained in section [URL Rewrites](/docs3/nightly/#url-rewrites).
...
Internal looping can also be used for [proxying to internal APIs](/docs3/nightly/advanced-configuration/transform-traffic/looping).
...
spec:
  ...
  protocol: http
  proxy:
    listen_path: "/docs3/nightly/basic"
    target_url: http://example.com
  active: true
  use_keyless: true
  version_data:
...
spec:
  ...
  protocol: http
  proxy:
    listen_path: "/docs3/nightly/token"
    target_url: http://example.com
  active: true
  use_keyless: true
  version_data:
...
spec:
  ...
  internal: true
  proxy:
    listen_path: "/docs3/nightly/proxy"
    target_url: http://httpbin.org
    strip_listen_path: true
  use_keyless: true
```
...
API name can be set through `x-tyk-api-gateway.info.name` field in [Tyk OAS API Definition](/docs3/nightly/api-management/gateway-config-tyk-oas) object.
...
API active state can be set through `x-tyk-api-gateway.info.state.active` field in [Tyk OAS API Definition](/docs3/nightly/api-management/gateway-config-tyk-oas) object.
...
API accessibility can be set through `x-tyk-api-gateway.info.state.internal` field in [Tyk OAS API Definition](/docs3/nightly/api-management/gateway-config-tyk-oas) object.
...
If you already have API configurations created in the Tyk Dashboard and want to start using Tyk Operator to manage these APIs, you can include the existing API ID in the manifest under the `x-tyk-api-gateway.info.id` field in [Tyk OAS API Definition](/docs3/nightly/api-management/gateway-config-tyk-oas) object.
...
### API Categories
[API categories](/docs3/nightly/api-management/dashboard-configuration#governance-using-api-categories) are configured differently for Tyk OAS APIs and Tyk Classic APIs. Please see below for examples.
...
For a Tyk Classic API, you can specify the category name using the `name` field with a `#` qualifier. This will categorize the API in the Tyk Dashboard. See [How API categories work](/docs3/nightly/api-management/dashboard-configuration#tyk-classic-apis) to learn about limitations on API names.
...
`versioning` is configured at `order-api` (v1), the Base API, and it has similiar structure as [Tyk OAS API Definition](/docs3/nightly/api-management/api-versioning):
...
For Tyk Classic API, versioning can be configured via `ApiDefinition` custom resource definition (CRD). See [Tyk Classic versioning](/docs3/nightly/api-management/gateway-config-tyk-classic#tyk-classic-api-versioning) for a comprehensive example of configuring API versioning for Tyk Classic API with Tyk Operator.

### API Ownership

Please consult the [API Ownership](/docs3/nightly/api-management/user-management#api-ownership) documentation for the fundamental concepts of API Ownership in Tyk and [Operator Context](/docs3/nightly/#multi-tenancy-in-tyk) documentation for an overview of the use of OperatorContext to manage resources for different teams effectively.
...
Tyk supports secure upstream connections through mutual TLS, certificate pinning, and public key verification to ensure data integrity between the gateway and backend services. For full details, please see the [Upstream Authentication](/docs3/nightly/api-management/upstream-authentication) section.
...
The TykOasApiDefinition Custom Resource Definition (CRD) manages [Tyk OAS API Definition objects](/docs3/nightly/api-management/gateway-config-tyk-oas) within a Kubernetes environment. This CRD enables the integration and management of Tyk API definitions using Kubernetes-native tools, simplifying the process of deploying and managing OAS APIs on the Tyk Dashboard.
...
|----------|---------|-----------------|----------|--------|
| API Category | ✅      | v1.0 | - | [Manage API Categories](/docs3/nightly/#api-categories) |
| API Version | ✅      | v1.0 | - | [Manage API versioning](/docs3/nightly/#api-versioning) |
| API Ownership via OperatorContext | ✅      | v1.0 | - | [API Ownership](/docs3/nightly/api-management/user-management#when-to-use-api-ownership) |
| Client Certificates | ✅      | v1.0 | - | [Manage TLS certificate](/docs3/nightly/#tls-certificates) |
| Custom Domain Certificates | ✅      | v1.0 | - | [Manage TLS certificate](/docs3/nightly/#tls-certificates) |
| Public keys pinning | ✅      | v1.0 | - | [Manage TLS certificate](/docs3/nightly/#tls-certificates) |
| Upstream mTLS | ✅      | v1.0 | - | [Manage TLS certificate](/docs3/nightly/#tls-certificates) |
| Kubernetes Ingress | ✅      | v1.0 | - | [Kubernetes Ingress Controller](/docs3/nightly/#control-kubernetes-ingress-resources) |
| Link with SecurityPolicy | ✅      | v1.0 | - | [Protect an API](/docs3/nightly/#add-a-security-policy-to-your-api) |

#### TykStreamsApiDefinition CRD
The TykStreamsApiDefinition Custom Resource Definition (CRD) manages [Async API configuration](/docs3/nightly/api-management/event-driven-apis#configuration-options) within a Kubernetes environment.

##### TykStreamsApiDefinition Features

`TykStreamsApiDefinition` can support all features of [Tyk Streams](/docs3/nightly/api-management/event-driven-apis#). You just need to provide the Tyk Streams API definition via a ConfigMap. In addition to managing the CRUD (Create, Read, Update, Delete) of Tyk Streams API resources, the Tyk Operator helps you better manage resources through object linking to Security Policies. See below for a list of Operator features and examples:
...
See [Release notes](/docs3/nightly/developer-support/release-notes/operator) to check for each Tyk Operator release,
...
#### Security Policy CRD
The SecurityPolicy custom resource defines configuration of [Tyk Security Policy object](/docs3/nightly/api-management/policies).

Here are the supported features:

| Features                       | Support   | Supported From | Example |
|--------------------------------|-----------|----------------|---------|
| API Access                     | ✅        | v0.1           | [API Access](/docs3/nightly/#define-the-security-policy-manifest)        |
| Rate Limit, Throttling, Quotas | ✅        | v0.1           | [Rate Limit, Throttling, Quotas](/docs3/nightly/#define-the-security-policy-manifest)        |
| Meta Data & Tags               | ✅        | v0.1           | [Tags and Meta-data](/docs3/nightly/#define-the-security-policy-manifest)        |
| Path and Method based permissions | ✅     | v0.1           | [Path based permission](/docs3/nightly/#security-policy-example)        |
| Partitions                     | ✅        | v0.1           | [Partitioned policies](/docs3/nightly/#security-policy-example)       |
| Per API limit                  | ✅        | v1.0           | [Per API Limit](/docs3/nightly/#security-policy-example)        |
| Per-Endpoint limit             | ✅        | v1.0           | [Per Endpoint Limit](/docs3/nightly/#security-policy-example)        |
