...
sidebarTitle: "Docs Contribution"
tags: ['Inclusive Naming Initiative', 'Inclusivity', 'Inclusive']
---

## How to Contribute to our Docs

We appreciate any form of engagement and contribution to our documentation. You can do it in a few ways:
...
- Update the code yourself:
  ...
  2. To find the page that needs editing in the actual [GH docs repo](https://github.com/TykTechnologies/tyk-docs/), the best way would be to copy a sentence from that HTML page on the [docs website](https://tyk.io/docs) and look it up using the Github search box, on the top left corner. (Since the structure of our docs is not the same as the structure in the rendered docs website, we can't easily link you to this page).
...
- [allow_master_keys](/docs3/nightly/tyk-oss-gateway/configuration#allow_master_keys)
- [analytics_storage.master_name](/docs3/nightly/tyk-oss-gateway/configuration#analytics_storagemaster_name)
- [cache_storage.master_name](/docs3/nightly/tyk-oss-gateway/configuration#cache_storagemaster_name)
- [storage.master_name](/docs3/nightly/tyk-oss-gateway/configuration#storagemaster_name)
- [slave_options](/docs3/nightly/tyk-oss-gateway/configuration#slave_options)
- [blacklisted_ips](/docs3/nightly/api-management/gateway-config-tyk-classic#ip-access-control)
- [disable_ports_whitelist](/docs3/nightly/key-concepts/tcp-proxy#allowing-specific-ports)
- [enable_ip_blacklisting](/docs3/nightly/api-management/gateway-config-tyk-classic#ip-access-control)
- [ports_whitelist](/docs3/nightly/key-concepts/tcp-proxy#allowing-specific-ports)
...
- [version_data.versions.`{version-name}`.extended_paths.black_list](/docs3/nightly/api-management/traffic-transformation/block-list#api-definition). There is also a parameter with equivalent functionality under the `paths` object (`version_data.versions.{version_name}.paths.black_list`).
- [version_data.versions.`{version-name}`.extended_paths.white_list](/docs3/nightly/api-management/traffic-transformation/allow-list#api-definition). There is also a parameter with equivalent functionality under the `paths` object (`version_data.versions.{version_name}.paths.while_list`).

##### Tyk Dashboard

**Config parameters**
- [enable_master_keys](/docs3/nightly/tyk-dashboard/configuration#enable_master_keys)
- [redis_master_name](/docs3/nightly/tyk-dashboard/configuration#redis_master_name)

**Tyk Classic API Definition**

The [Tyk Dashboard OpenAPI Document](https://github.com/TykTechnologies/tyk-docs/blob/master/tyk-docs/assets/others/dashboard-swagger.yml) (Tyk Dashboard swagger), contains references to [the parameters from the above Tyk Classic API Definition list](/docs3/nightly/#gw-classic-api-definition).

**Dashboard UI**

The Tyk Classic APIs *Endpoint Designer* shows configuration of [blacklist](/docs3/nightly/api-management/traffic-transformation/block-list#api-designer) and [whitelist](/docs3/nightly/api-management/traffic-transformation/allow-list#api-definition) middleware plugins.
    
##### Tyk MDCB

The following MDCB configuration parameters contain tier 1 word occurrences:
- [analytics_storage.master_name](/docs3/nightly/tyk-multi-data-centre/mdcb-configuration-options#analytics_storagemaster_name)
- [storage.master_name](/docs3/nightly/tyk-multi-data-centre/mdcb-configuration-options#storagemaster_name)

##### Tyk Pump

The following Tyk Pump configuration parameters contain tier 1 word occurrences:
- [analytics_storage_config.master_name](/docs3/nightly/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#analytics_storage_configmaster_name)
...
- Tyk Classic Developer Portal provides [documentation](/docs3/nightly/tyk-developer-portal/tyk-portal-classic/keycloak-dcr) that explains how to integrate Tyk with Keycloak using the [OpenID Connect Dynamic Client Registration](https://tools.ietf.org/html/rfc7591) protocol. The example in the guide uses the Keycloak default `master` realm.
