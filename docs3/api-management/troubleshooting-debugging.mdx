...
3. ##### Gateway proxy error "context canceled"
    ...
    See [this guide](/docs3/tyk-self-managed#resource-limits).
...
18. ##### How to run two Gateways with docker-compose
    ...
    Managing a second Tyk Gateway with our [Tyk Pro Docker Demo](/docs3/tyk-self-managed#docker-compose-setup) is a case of mounting the `tyk.conf` file into a new volume and declaring a new Gateway service but exposed on a different port.
...
3. ##### Mongo version
    ...
    Does Tyk support the version of Mongo that you’re using? Read more about that [here](/docs3/tyk-self-managed#mongodb).
...
4. ##### Capped collections
    ...
    You can determine each collection's cap size by visiting our [MongoDB sizing calculator](/docs3/tyk-self-managed#mongodb-sizing-guidelines).
...
1. ##### Gateway `/hello` endpoint
    ...
    Querying the gateway's `/hello` health endpoint is the quickest way to determine the status of your Tyk instance. You can find more information in our docs about the [Gateway Liveness health check](/docs3/tyk-self-managed#set-up-liveness-health-checks).
