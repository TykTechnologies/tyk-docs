...
- `OriginatingRequest` (string): Based64-encoded [raw inbound request](/docs3/#raw-request-data)
    ...
    <Note>
    Circuit breaker events provide different metadata, see [Circuit Breakers](/docs3/planning-for-production/ensure-high-availability/circuit-breakers) to see what is provided when the `BreakerTripped`, `BreakerReset` or `BreakerTriggered` events are generated.
    </Note>
