...
We see that upon every pull request, a section of steps are taken to "Build, [Bundle](/docs3/api-management/plugins/overview#plugin-bundles), Release Go Plugin".
...
The next step of the workflow is to "[bundle](/docs3/api-management/plugins/overview#plugin-bundles)" the plugin.
...
<Note>
For added security, please consider signing your [bundles](/docs3/api-management/plugins/overview#plugin-deployment-types), especially if the connection between the Gateways and the Bundler server traverses the internet.
</Note>
...
<Note>
For seamless deployments, take a look at multi-version [plugin support](/docs3/tyk-cloud#configure-plugins) to enable zero downtime deployments of your Tyk Gateway installs
</Note>
...
1. The root server - The server which all bundles will be downloaded from.  This is set globally in the Tyk conf file [here](/docs3/tyk-oss-gateway/configuration#enable_bundle_downloader).
...
Add this field within your [Gateway config file](/docs3/tyk-oss-gateway/configuration):
...
And this field within your [API definition](/docs3/api-management/gateway-config-introduction):
...
You can find more information about enabling OpenTelemetry [here](/docs3/api-management/logs-metrics#opentelemetry).
