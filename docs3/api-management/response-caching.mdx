...
There are two approaches to configure caching for an API deployed with Tyk:
 ...
 - [Basic](/docs3/api-management/response-caching#basic-caching) or [Safe Request](/docs3/api-management/response-caching#global-cache-safe-requests) caching is applied at the API level for all requests for which it is safe to do so.
 - [Advanced](/docs3/api-management/response-caching#endpoint-caching) caching options can be applied at the endpoint level.
...
Tyk does support safe request caching at the more granular, per-endpoint level, as described [here](/docs3/api-management/response-caching#request-selective-cache-control) - but `cache_all_safe_requests` must be set to `false` in that scenario.
...
_On this page we describe the use of Tyk's API response cache at the API level (Global); for details on the more advanced Endpoint level cache you should refer to [this](/docs3/api-management/response-caching#endpoint-caching) page._
...
For more advanced use of the API-level cache we also have:
 - `cache_by_headers`: used to create multiple cache entries based on the value of a [header value](/docs3/#selective-caching-by-header-value) of your choice
 - `enable_upstream_cache`: used to allow your [upstream service](/docs3/api-management/response-caching#upstream-cache-control-1) to identify the responses to be cached
...
2. **Set the Cache Options for the Global Cache**
    ...
    2.  **Cache timeout** to set the [TTL](/docs3/api-management/response-caching#cache-timeout) (in seconds) for cached requests
    3.  **Cache only these status codes** to set which [response codes](/docs3/api-management/response-caching#cache-response-codes) to cache (ensure that you click **ADD** after entering each response code so that it is added to the list)
    4.  **Cache all safe requests** to enable the [global cache](/docs3/api-management/response-caching#global-cache-safe-requests)

## Endpoint Caching

### Overview

On this page we describe how to configure Tyk's API response cache per endpoint within an API. This gives granular control over which paths are cached and allows you to vary cache configuration across API versions. For details on the API level (Global) cache you should refer to the [global-cache](/docs3/api-management/response-caching#basic-caching) configuration page.
...
For details on the API-level cache you should refer to the [API-level cache](/docs3/api-management/response-caching#basic-caching) configuration page.
...
If you have an API that's providing search capability (for example into a catalog of products) and want to optimize the performance for the most frequently requested search terms, you could use the endpoint cache's [request-selective](/docs3/#request-selective-cache-control) capability to cache only a subset of all requests to an endpoint.
...
<Note>
It's important to note that the [cache all safe requests](/docs3/api-management/response-caching#global-cache-safe-requests) feature of the API-level cache will overrule the per-endpoint configuration so you must ensure that both are not enabled for the same API.
</Note>
...
If you're using Tyk OAS APIs, then you can find details and examples of how to configure the Endpoint Caching middleware [here](/docs3/api-management/response-caching#using-tyk-oas-api).

If you're using Tyk Classic APIs, then you can find details and examples of how to configure the Endpoint Caching middleware [here](/docs3/api-management/response-caching#using-classic-api).
...
The [Endpoint Caching](/docs3/api-management/response-caching#endpoint-caching) middleware allows you to perform selective caching for specific endpoints rather than for the entire API, giving you granular control over which paths are cached.

When working with Tyk OAS APIs the middleware is configured in the [Tyk OAS API Definition](/docs3/api-management/gateway-config-tyk-oas#operation). You can do this via the Tyk Dashboard API or in the API Designer.

If you're using the legacy Tyk Classic APIs, then check out the [Tyk Classic](/docs3/api-management/response-caching#using-classic-api) page.
...
2. **Enable and configure the middleware for the specific endpoint**
    ...
    - `cacheByRegex`: Pattern match for [selective caching by body value](/docs3/api-management/response-caching#request-selective-cache-control)
...
3. **Configure the middleware**
    ...
    <Note>
    Body value match or [request selective](/docs3/api-management/response-caching#request-selective-cache-control) caching is not currently exposed in the Dashboard UI, so it must be enabled though either the raw API editor or the Dashboard API.
    </Note>
...
If you're using the newer Tyk OAS APIs, then check out the [Tyk OAS](/docs3/api-management/response-caching#using-tyk-oas-api) page.

If using Tyk Operator please refer to section [configuring the middleware in the Tyk Operator](/docs3/#tyk-operator).

#### Configuring the middleware in the Tyk Classic API Definition

When using the Tyk Classic API Definition, there are two options for endpoint caching - simple and advanced.

The [simple](/docs3/#simple-endpoint-cache) option works with the API-level cache and allows you to select which endpoints are cached, but relies upon the cache timeout (refresh) configured at the API-level. It will cache all responses received from the endpoint regardless of the HTTP response code for all [safe requests](/docs3/api-management/response-caching#global-cache-safe-requests).

The [advanced](/docs3/#advanced-endpoint-cache) option allows you to cache more selectively, giving control over the HTTP response codes to be cached, a per-endpoint cache timeout and also the possibility of caching responses only to requests containing specific data in the request body.
...
- a pattern match to cache only requests containing specific data in the [request body](/docs3/api-management/response-caching#request-selective-cache-control)
...
3. **Configure the Advanced Cache plugin**
    ...
    <Note>
    Body value match or [request selective](/docs3/api-management/response-caching#request-selective-cache-control) caching is not currently exposed in the Dashboard UI, so it must be configured through either the raw API editor or the Dashboard API.
    </Note>
...
For [multi-data center](/docs3/api-management/mdcb#redis) deployments, the Data Planes have a locally deployed Redis. This enables them to have a localised cache close to the traffic-serving Gateways.

The [cache key](/docs3/api-management/response-caching#cache-key) is used as the Redis key, for quick lookups.
...
The configuration of the separate Redis Cache is the same (and uses the same underlying driver) as the regular configuration, so [Redis Cluster](/docs3/tyk-configuration-reference/redis-cluster-sentinel#configure-redis-cluster) is fully supported. If you set `enable_cluster` to `false`, you only need to set one entry in `addrs`.
