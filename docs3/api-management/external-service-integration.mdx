...
This page introduces general features of Tyk Identity Broker (TIB) and how to configure them. If you are looking for global configurations of the TIB deployment refer this [config file](/docs3/tyk-configuration-reference/tyk-identity-broker-configuration).

We will delve into the following key topics:

1. **[Introduction to Tyk Identity Broker](/docs3/#what-is-tyk-identity-broker-tib)**: Explore key concepts, configuration options, and implementation steps for TIB. You'll learn how to set up profiles for different identity providers, understand the flow of authentication requests, and customize the integration to fit your specific needs.

2. **[Single Sign On with Tyk](/docs3/#single-sign-on-sso)**: We will learn how to implement seamless SSO experiences for Tyk Dashboard and Developer Portal.

3. **[SSO with different Protocols and Systems](/docs3/#sso-in-tyk)**: We will explore SSO integrations with LDAP, Social OAuth, SAML, and OpenID Connect.

4. **[Handling Custom Authentication](/docs3/#custom-proxy-identify-provider)**: We will learn how to configure TIB for unique authentication scenarios using the Proxy Provider.
...
Refer to this [document](/docs3/tyk-identity-broker/tib-rest-api)
...
SSO is sometimes complicated to understand or set up but can be easily accomplished by using the built-in [Tyk Identity Broker (TIB)](/docs3/#what-is-tyk-identity-broker-tib).
...
1. [Login with 3rd party social providers](/docs3/#sso-with-social-identity-providers)
2. [Login with any IdP that supports OIDC](/docs3/#sso-with-openid-connect-oidc)
3. [Login with any IdP that supports SAML](/docs3/#sso-with-saml)
3. [Login with LDAP](/docs3/#sso-with-ldap)
...
7. More Docs for the flow can be found on our [GitHub TIB repo README](https://github.com/TykTechnologies/tyk-identity-broker) and our [3rd Party integration docs](/docs3/api-management/external-service-integration)
...
- `/admin/sso` - See [Dashboard Admin API SSO](/docs3/api-management/dashboard-configuration#single-sign-on-api-1) for more details.
- `/api/sso` -  See [Dashboard API SSO](/docs3/api-management/dashboard-configuration#single-sign-on-api) for more details.
...
- Instruction on setting [SSO with Okta](/docs3/#oidc-with-okta)
- Instructions on setting [SSO with Auth0](/docs3/#oidc-with-auth0)
- Instructions on setting [SSO with Keycloak](/docs3/#oidc-with-keycloak)
- Instructions on setting [SSO with AzureAD](/docs3/#oidc-with-azure-ad)
...
You already have authorized access to Tyk's Dashboard. If you haven't, get the authorization key by following this [guide](/docs3/api-management/user-management#using-dashboard-api).
...
For more information on how to set and change user permissions, head to this [guide](/docs3/api-management/user-management#using-dashboard-ui-1)
...
* You already have authorized access to Tyk's Dashboard. If you haven't, [get the authorization key by following this doc](/docs3/api-management/user-management#using-dashboard-api).
...
7. Start TIB by running the binary (`profiles.json` is in the same CWD)
   See [Install TIB](/docs3/api-management/external-service-integration) for detailed instructions on how to install TIB
...
```
  ...
  - See [TIB REST API](/docs3/tyk-identity-broker/tib-rest-api) for more details.
...
##### SSO login into the Dashboard via a login page
   You will need to:
    ...
    Explicit details are in [steps 6-7](/docs3/#create-login-page)
...
* Our Tyk Identity Broker (TIB). You can use the internal version included with a Tyk Self-Managed installation and Tyk Cloud, or an external version. See [Tyk Identity Broker](/docs3/#what-is-tyk-identity-broker-tib) for more details.
...
The Tyk Identity Broker (TIB) is an open-source project which can be used to integrate Tyk authentication with 3rd party identity providers (IDPs). You can use this to enable your Dashboard to authenticate users with your LDAP-powered identity providers such as Active Directory. TIB has been designed as a glue-code solution, so it can integrate with almost any identity provider (IDP). See [Tyk Identity Broker Configuration](/docs3/tyk-configuration-reference/tyk-identity-broker-configuration) for details on configuring the TIB.
...
This guide assumes you already have a Tyk environment set up, with a Gateway and Dashboard. If you don't, please follow the [Tyk Self-Managed getting started guide](/docs3/tyk-self-managed#installation-options-for-tyk-self-managed).
...
4. **Set up the LDAP profile**
    ...
    * `OrgId`: The organization id which the profile is connected to - make sure this is the correct id for your organization (see the [Dashboard Admin API documentation](/docs3/api-management/dashboard-configuration#organizations-api) for details on how to retrieve this)
...
3. **Setup OIDC Profile**
  - Create a new [TIB profile](/docs3/#exploring-tib-profiles):
...
4. **Enable JWE**
  - [Updated the TIB profile via API](/docs3/tyk-identity-broker/tib-rest-api#update-profile)
