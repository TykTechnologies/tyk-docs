...
canonical: "https://tyk.io/docs/platform-management/api-templates"
---
...
> Categories are not part of API Templates. They can't be pre-configured in a template. Categories must be set when the API is created (or edited afterwards).
...
Here `Meta` contains the database ID (where the asset has been registered in the persistent database) and `ID` contains the unique identifier for the template. This unique identifier will be automatically generated by Tyk if none was provided in the `id` of the template asset provided in the `curl` request.
