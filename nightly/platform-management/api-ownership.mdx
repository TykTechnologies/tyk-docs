---
title: "API Ownership"
description: "Learn how to use API Ownership in Tyk Dashboard to govern API visibility and manageability for specific teams or users in multi-team setups"
keywords: "API Ownership, Multi Team, RBAC, API Governance, Owned Analytics, Tyk Dashboard"
sidebarTitle: "API Ownership"
canonical: "https://tyk.io/docs/platform-management/api-ownership"
---
...
The [user owned analytics](/nightly/platform-management/api-ownership#owned-analytics) feature allows you to prevent users from seeing the traffic to (and existence of) APIs for which they are not responsible. This reduces the opportunity for data leakage within your business.
...
To implement this structure, you would create three user groups and assign [permissions](/nightly/platform-management/user-permissions) as indicated:
...
Currently, only [API Usage](/nightly/api-management/dashboard-analytics#activity-by-api) and [Error Counts](/nightly/api-management/dashboard-analytics#activity-by-error) are available to users with the `owned_analytics` permission.
