...
canonical: "https://tyk.io/docs/platform-management/user-permissions"
---
...
Selecting the **Account is Admin** checkbox from the Dashboard gives the user full access (it has the same [effect](/nightly/platform-management/user-permissions#admin-users) as the `IsAdmin` property).

## Custom User Permissions

You can create your own custom permissions for use with the [Open Policy Agent (OPA)](/nightly/platform-management/open-policy-agent) using the [Additional Permissions](https://tyk.io/docs/api-reference/additional-permissions/list-additional-permissions) endpoint in the Tyk Dashboard Admin API. This allows you to add and delete (CRUD) a list of additional (custom) permissions for your Dashboard users. Once created, a custom permission will be added to standard list of user permissions. 
