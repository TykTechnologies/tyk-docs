...
**Capping Analytics**
Tyk Gateways can generate a lot of analytics data. Be sure to read about [capping your Dashboard analytics](/nightly/api-management/tyk-pump#tyk-pump-capping-analytics-data-storage)

**API Definition Compression for MDCB Deployments**

In [MDCB](/nightly/api-management/mdcb) deployments, Data Plane Gateways store API definition backups in Redis to ensure resilience when the Control Plane is unavailable. For deployments with many or large API definitions, this can consume significant Redis memory.
...
Set [`storage.compress_api_definitions`](/nightly/tyk-oss-gateway/configuration#storage-compress_api_definitions) and [`storage.compress_policies`](/nightly/tyk-oss-gateway/configuration#storage-compress_api_policies) in your Data Plane Gateway configuration to compress your API definition and policy backups respectively:
