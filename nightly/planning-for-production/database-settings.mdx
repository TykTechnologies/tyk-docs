...
import RedisCalculator from '/snippets/nightly/RedisCalculator.mdx';
import DatabaseCalculator from '/snippets/nightly/DatabaseCalculator.mdx';

Visit the following sections to see how to configure the Database for Production:
* [Redis](/nightly/planning-for-production/database-settings#redis)
* [MongoDB](/nightly/planning-for-production/database-settings#mongodb-sizing-guidelines)
* [PostgreSQL](/nightly/planning-for-production/database-settings#postgresql)

Please consult the [data storage configuration](/nightly/api-management/dashboard-configuration#data-storage-solutions) guide for further information relating to how to configure Tyk's data storage across different database engines.
...
- API client [Session](/nightly/api-management/access-control/sessions-and-keys/understanding-sessions) data (the source of truth for the deployment).
- [OAuth tokens issued by Tyk's OAuth 2.0 authorization server](/nightly/api-management/authentication/oauth-2#client-application).
- [Tyk Certificate Store](/nightly/api-management/certificates#tyk-certificate-store).
...
- [Traffic logs](/nightly/api-management/tyk-pump#api-traffic-logs) buffered before Tyk Pump processes them.
...
For configuration details, see [Configure Redis](/nightly/tyk-configuration-reference/redis-cluster-sentinel).
...
Tyk Gateways can generate a large volume of analytics data. Be sure to read about [capping your Dashboard analytics](/nightly/api-management/tyk-pump#tyk-pump-capping-analytics-data-storage).

#### Compressing API Definitions and Policies

In [distributed deployments](/nightly/api-management/mdcb), Data Plane Gateways store API definition and policy backups in Redis to ensure resilience when the Control Plane is unavailable. For deployments with many or large API definitions, this can consume significant Redis memory.
...
- [`storage.compress_api_definitions`](/nightly/tyk-oss-gateway/configuration#storage-compress_api_definitions) is available from Tyk 5.12.0.
- [`storage.compress_policies`](/nightly/tyk-oss-gateway/configuration#storage-compress_policies) is available from Tyk 5.13.0.
...
The `analytics_storage` block accepts the same fields as the main `storage` block. See [Configure Redis](/nightly/tyk-configuration-reference/redis-cluster-sentinel) for the full field reference.
...
The `cache_storage` block accepts the same fields as the main `storage` block. See [Configure Redis](/nightly/tyk-configuration-reference/redis-cluster-sentinel) for the full field reference.
...
* [Configure Dashboard MongoDB driver](/nightly/tyk-dashboard/configuration#mongo_driver)
* [Configure MDCB MongoDB driver](/nightly/tyk-multi-data-centre/mdcb-configuration-options#analytics-driver)
* [Configure Pump MongoDB driver](https://github.com/TykTechnologies/tyk-pump#driver-type)

For recommended deployment topology, see [Planning for Production](/nightly/planning-for-production#recommended-topology).

**Special notes for DocumentDB**

<Note>
If you are using [DocumentDB](https://aws.amazon.com/documentdb/), [capped collections](/nightly/api-management/tyk-pump#tyk-pump-capping-analytics-data-storage) are not supported. See [here](https://docs.aws.amazon.com/documentdb/latest/developerguide/mongo-apis.html) for more details.
</Note>
...
More information on these configuration variables [here](/nightly/tyk-dashboard/configuration). 
