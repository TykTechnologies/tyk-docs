...
Tyk does not have an API-level circuit breaker that can be applied across all endpoints. If you are using the Tyk Dashboard, however, then you are able to use an [Open Policy Agent](/nightly/platform-management/open-policy-agent) to append a circuit breaker to every API/Service using the regex `.*` path.
