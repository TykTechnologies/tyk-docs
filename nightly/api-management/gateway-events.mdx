...
- an [event log](/nightly/api-management/gateway-events#logging-api-events-1) that will write to the configured [log output](/nightly/api-management/logs#application-logs)
...
When an event is fired, and an *event handler* is registered for that specific API and event combination, Tyk Gateway provides the handler with a rich set of [metadata](/nightly/api-management/gateway-events#event-metadata). The external system (webhook) or custom (JavaScript) code can then use this metadata to decide what action to take.
...
- `originating_request` (string): a Base64-encoded [raw inbound request](/nightly/#raw-request-data)
...
The HTTP method, body, header values, and target URL can all be configured in the API definition. The [request body](/nightly/#webhook-payload) is generated using a Tyk template file that has access to the [event metadata](/nightly/api-management/gateway-events#event-metadata).
...
The event handler has access to the [event metadata](/nightly/api-management/gateway-events#event-metadata) and this can be accessed by the template using the `{{.Meta.XXX}}` namespace.
...
This contains the [event metadata](/nightly/api-management/gateway-events#event-metadata) in the following structure:
