...
The Dashboard exposes two APIs:
 - **Dashboard API**: Is used for operational management of Tyk resources (APIs, policies, keys, etc.). This API offers granular permissions based on user roles.
    ...
    To know more about the Dashboard API, refer to the [Tyk Dashboard API documentation](/nightly/tyk-dashboard-api).
 ...
 - **Dashboard Admin API**: Is used for system-level administration and initial setup tasks like managing organizations, initial user creation, backups/migrations and SSO setup.
    ...
    To know more about the Dashboard Admin API, refer to the [Tyk Dashboard Admin API documentation](/nightly/dashboard-admin-api).
...
Tyk Dashboard requires a persistent datastore for its operations. For supported database engines and versions, see [Installation Requirements](/nightly/tyk-self-managed/install#requirements).
...
2. **Aggregate Analytics**: Data used to display Dashboard charts and [analytics](/nightly/api-management/dashboard-analytics#traffic-analytics).
3. **Logs**: When [detailed logging](/nightly/api-management/troubleshooting-debugging#capturing-detailed-logs) is enabled, request and response data is logged to storage. These logs can previewed in the Dashboard [log browser](/nightly/api-management/dashboard-analytics#activity-logs).
...
Tyk Pump writes traffic logs, aggregated analytics and uptime test results to these same storage layers. See [Control Plane Pumps](/nightly/api-management/dashboard-analytics/control-plane-pumps) for configuring Tyk Pump to write logs and analytics, or [Uptime Tests](/nightly/planning-for-production/ensure-high-availability/uptime-tests#monitoring-uptime-tests-in-tyk-dashboard) for the uptime storage layer specifically.
