...
Tyk Dashboard requires a persistent datastore for its operations, shared with Tyk MDCB and Tyk Pump. See [Core Platform Persistent Database](/nightly/api-management/persistent-database) for the categories of data it stores, supported engines, connection configuration, and connection pool tuning.
