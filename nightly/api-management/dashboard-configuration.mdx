...
- [Detailed logging](/nightly/api-management/logs/traffic-logs#detailed-recording)
...
<Note>
For the Tyk Dashboard's analytics functionality to work, you must configure both per-request and aggregated pumps for the database platform that you are using. For more details see the [Control Plane Pumps](/nightly/api-management/dashboard-analytics/control-plane-pumps#choosing-a-pump-type) section.
</Note>
...
Not every endpoint necessarily appears here: see [Controlling Which Endpoints Are Tracked](/nightly/api-management/dashboard-analytics#controlling-which-endpoints-are-tracked) for what controls that.
...
For more information, see [MCP Analytics](/nightly/ai-management/mcp-gateway/mcp-analytics).
...
Please consult [Control Plane Pumps](/nightly/api-management/dashboard-analytics/control-plane-pumps) for an explanation of how to configure Tyk Pump to write logs and analytics to different storage layers, or [Uptime Tests](/nightly/planning-for-production/ensure-high-availability/uptime-tests#monitoring-uptime-tests-in-tyk-dashboard) for the uptime storage layer specifically.
