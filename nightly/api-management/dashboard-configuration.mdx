...
This API returns by default the initial set of additional permissions defined in your Tyk Dashboard configuration, under [security.additional_permissions](/nightly/tyk-dashboard/configuration#securityadditional_permissions).
...
- [Detailed logging](/nightly/api-management/logs-metrics#capturing-detailed-logs)
...
<Note>
The *track endpoint* middleware only affects the inclusion of endpoints in the per-endpoint aggregates, it does not have any impact on other [aggregated data](/nightly/api-management/logs-metrics#aggregated-analytics) nor the [per-request data](/nightly/api-management/dashboard-configuration#activity-logs).
</Note>
...
To use a MaxMind database, see [MaxMind Database Settings](/nightly/tyk-oss-gateway/configuration#analytics_configenable_geo_ip) in the Tyk Gateway Configuration Options.
...
Further details for configuring an uptime SQL database are available [here](/nightly/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#uptime_pump_configuptime_type)
