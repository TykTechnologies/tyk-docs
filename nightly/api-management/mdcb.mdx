...
title: 'Tyk Multi Data Center Bridge (MDCB): Centralized API Governance Across Distributed
  Environments'
---
...
The MDCB component will however, by default, expose an RPC service on port 9091, to which the [Tyk Data Plane](/nightly/api-management/mdcb#data-plane) data centers, i.e. the worker gateway(s) that serves API traffic, will need connectivity.
...
Before a worker gateway can connect to MDCB, it is important to enable the organization that owns all the APIs to be distributed to be allowed to utilize Tyk MDCB. To do this, the organization record needs to be modified with two flags using the [Tyk Dashboard Admin API](/nightly/dashboard-admin-api).
...
You may configure an unlimited number of [Tyk Data Planes](/nightly/api-management/mdcb#data-plane) containing Worker Gateways for ultimate High Availablity (HA). We recommend that you deploy your worker gateways as close to your upstream services as possible in order to reduce latency.
...
1. **Worker Gateway configuration**
    ...
    Please see [Gateway configuration options](/nightly/tyk-oss-gateway/configuration#slave_optionssynchroniser_enabled) for reference.
    ...
    Please see [Gateway configuration options](/nightly/tyk-oss-gateway/configuration#slave_optionskey_space_sync_interval) for reference.
    ...
    Please see [Gateway configuration options](/nightly/tyk-oss-gateway/configuration#slave_optionsgroup_id) for reference
...
2. **Control Plane configuration**
    ...
    Please see [MDCB configuration options](/nightly/tyk-multi-data-centre/mdcb-configuration-options#sync_worker_config) for reference.
    ...
    Please see [MDCB configuration options](/nightly/tyk-multi-data-centre/mdcb-configuration-options#securityprivate_certificate_encoding_secret) for reference
...
2. **Can I trigger a re-synchronisation?**
    ...
    Synchronisation will be triggered once the Time To Live (TTL) of a worker Gateway has expired. The default expiry duration is 3 minutes. The Time To Live (TTL) value can be set via [sync_worker_config.group_key_ttl](/nightly/tyk-multi-data-centre/mdcb-configuration-options#sync_worker_configgroup_key_ttl)