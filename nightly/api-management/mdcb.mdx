---
title: "Using Tyk In Distributed Environments"
description: "Learn about Tyk Multi Data Center Bridge (MDCB) and how it enables you to manage multiple API gateways across different data centers"
...
- You can physically [segment teams and environments](/nightly/api-management/api-sharding#gateway-sharding) within a single physical data center, giving each team full control of its own API gateway and server resources without the noisy neighbors you might experience in a standard self-managed deployment.
...
#### Optional Components
- One or more **Tyk Pumps** can be deployed within the Control Plane to export analytics data (request/response logs) to your [data sink of choice](/nightly/api-management/logs/external-data-sinks) for further analytics and visualisation.
...
#### Optional Components
- A **Tyk Pump** specifically configured as a [Hybrid Pump](/nightly/product-stack/tyk-charts/tyk-data-plane-chart#hybrid-pump) can be deployed with the Data Plane gateways to export analytics data (request/response logs) to your [data sink of choice](/nightly/api-management/logs/external-data-sinks) for further analytics and visualisation.
...
<Note>
Ensure that you are installing PostgreSQL versions that are supported by Tyk. Please consult the list of [supported versions](/nightly/tyk-self-managed/install#requirements) that are compatible with Tyk.
</Note>
...
The Synchroniser behaviour is configured in [`slave_options`](/nightly/tyk-oss-gateway/configuration#slave_options) object  in the Data Plane Gateway's `tyk.conf` file or equivalent environment variables, in addition to the [standard Data Plane Gateway settings](/nightly/api-management/mdcb#data-plane-gateway-configuration):
...
The Synchroniser behaviour is configured in the [`sync_worker_config`](/nightly/tyk-multi-data-centre/mdcb-configuration-options#sync_worker_config) object in the MDCB's `tyk_sink.conf` file or equivalent environment variables, in addition to the [standard MDCB settings](/nightly/api-management/mdcb#configuration):
...
| `hash_keys`      | Set to `true` if [key hashing](/nightly/api-management/access-control/sessions-and-keys/key-hashing) is also enabled in your Tyk Dashboard and Gateway configurations. This ensures that the Synchroniser correctly handles hashed API keys. |
