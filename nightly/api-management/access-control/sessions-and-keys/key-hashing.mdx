...
canonical: "https://tyk.io/docs/api-management/access-control/sessions-and-keys/key-hashing"
---
...
Hashing is enabled in the [Gateway](/nightly/tyk-oss-gateway/configuration#hash_keys) and [Dashboard](/nightly/tyk-dashboard/configuration#hash_keys) configuration using the common setting `hash_keys` (or the equivalent environment variables).
...
You can configure the algorithm using the [`hash_key_function`](/nightly/tyk-oss-gateway/configuration#hash_key_function) setting in your `tyk.conf` (or the equivalent environment variable). The available options are:
...
You must explicitly enable these endpoints by setting `enable_hashed_keys_listing: true` in the [Gateway](/nightly/tyk-oss-gateway/configuration#enable_hashed_keys_listing) and [Dashboard](/nightly/tyk-dashboard/configuration#enable_hashed_keys_listing) configurations respectively.

### 3. Analytics Using Hashed Keys

All analytics records, logs, and quota tracking will use the hashed version of the key. If you need to correlate analytics data with a specific client, you should use the Session's `alias` or `meta_data` [fields](/nightly/api-management/access-control/sessions-and-keys/understanding-sessions#metadata-and-context), rather than relying on the key itself.
...
- **Dashboard**: 
    ...
    - You must explicitly set [`enable_update_key_by_hash: true`](/nightly/tyk-dashboard/configuration#enable_update_key_by_hash) and [`enable_delete_key_by_hash: true`](/nightly/tyk-dashboard/configuration#enable_delete_key_by_hash) in your tyk_analytics.conf to allow modification of keys via their hash.
...
It is of course not possible to convert a one-way hashed key to another hashing algorithm, but you do not need to invalidate all your existing keys. Instead, you can use the [`hash_key_function_fallback array`](/nightly/tyk-oss-gateway/configuration#hash_key_function_fallback) in your Gateway configuration. 
