...
[Tyk Identity Broker](/nightly/api-management/external-service-integration) (TIB) can act as a bridge between external Identity Providers (such as LDAP, Google, or Azure AD) and Tyk Gateway to issue API tokens for client applications (such as Single Page Applications or mobile apps). 

This is distinct from using TIB to log into the [Tyk Dashboard](/nightly/api-management/external-service-integration#single-sign-on-sso) or [Developer Portal](/nightly/tyk-stack/tyk-developer-portal/enterprise-developer-portal/managing-access/enable-sso). Instead, this flow allows a user to authenticate with a third-party provider, and upon success, TIB will grant them a token to access your Tyk-protected APIs.
...
This flow is used when your API is secured using the [Tyk OAuth 2.0](/nightly/api-management/authentication/oauth-2) authentication method. TIB acts as a delegate: it authenticates the user, then uses a pre-configured Tyk OAuth Client's credentials to request an OAuth token from the Tyk Gateway on behalf of the user.

### Prerequisites

You must first create an OAuth Client for the target API (typically done via [Tyk Dashboard](/nightly/api-management/authentication/oauth-2#client-app-registration)).
