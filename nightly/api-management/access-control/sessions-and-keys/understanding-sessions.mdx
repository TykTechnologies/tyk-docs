...
canonical: "https://tyk.io/docs/api-management/access-control/sessions-and-keys/understanding-sessions"
---
...
2. **Traffic Management**: Enforcing session-level rate limits (requests per second) to prevent individual "noisy neighbor" clients from monopolizing resources, which complements your global [API-level protections](/nightly/api-management/rate-limit#api-level-rate-limiting).
...
A Session contains several categories of configuration options. While you can configure these directly on individual sessions, it is highly recommended to manage them via [**Policies**](/nightly/api-management/policies).

### Access Control

This field determines what the client is allowed to access:

- `access_rights`: A map defining the specific APIs the client can access. The key is the API ID, and the value contains detailed [access control](/nightly/api-management/access-control/sessions-and-keys/access-rights) rules, such as granular path/method restrictions.
...
- `throttle_interval`: The time in seconds to [hold requests before retrying](/nightly/api-management/request-throttling) if the client exceeds the rate limit.
...
- `smoothing`: Configuration for [rate limit smoothing](/nightly/api-management/rate-limit#rate-limit-smoothing) (dynamic adjustment of rate limits to mitigate against traffic spikes).

These settings will be applied to all requests made using the Session except where [API](/nightly/api-management/access-control/sessions-and-keys/access-rights#api-level-limits-and-quotas) or [endpoint-level](/nightly/api-management/access-control/sessions-and-keys/access-rights#endpoint-level-rate-limits) limits have been configured in the access rights.
...
These settings will be applied to all requests made using the Session except where [API-level quotas](/nightly/api-management/access-control/sessions-and-keys/access-rights#api-level-limits-and-quotas) have been configured in the access rights.
...
- `monitor`: Configuration for the [quota monitor](/nightly/api-management/gateway-events#monitoring-quota-consumption), which Tyk uses to fire webhook events when a session's quota usage reaches those specific thresholds.

### Metadata and Context

These fields store custom information about the client:

- `meta_data`: A key-value map for storing custom data. This data can be injected into headers sent to your upstream API or used in transform middleware. For more details see the [session metadata](/nightly/api-management/access-control/sessions-and-keys/session-metadata) section.
...
Instead of configuring access rights and limits directly on the session, you can link the session to one or more [Policies](/nightly/api-management/policies). This is the recommended approach for managing access at scale.
...
- Learn how to manage the [Session Lifecycle](/nightly/api-management/access-control/sessions-and-keys/session-lifecycle) to control how long a key will be valid.
- Discover how to use [Policies](/nightly/api-management/policies) to manage Sessions at scale.
```