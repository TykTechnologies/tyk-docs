...
- `post_expiry_grace_period`: If the action is `retain`, this defines how long (in seconds) the Session is kept in Redis after expiration. 
    ...
    - If the value is `0` then the [legacy controls](/nightly/api-management/access-control/sessions-and-keys/session-lifecycle#legacy-controls) will be applied to the Session
