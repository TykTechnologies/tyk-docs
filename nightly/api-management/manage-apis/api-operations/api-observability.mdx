---
canonical: https://tyk.io/docs/api-management/manage-apis/api-operations/api-observability
description: Explains how to achieve API observability through Open Telemetry signals
  such as traces, metrics and logs
keywords: API Observability, Distributed Tracing, Metrics, Logs, Logging, Open Telemetry,
  OTel
noindex: noindex, follow
sidebarTitle: Monitoring and Troubleshooting APIs
...
Tyk allows you to capture and analyze logs related to API requests and responses in the [Log Browser](/nightly/api-management/dashboard-configuration#activity-logs) . You can optionally enable detailed recording for the requests per API level or per Key level to store inbound request and outbound response data. You can [enable debug modes](/nightly/api-management/troubleshooting-debugging#capturing-detailed-logs) for selected APIs and send the detail logs to one or more Pump backend instances.
