...
<Note>
Tyk Gateway also generates **API Traffic Logs**, which are written into Redis for detailed information about API traffic and analysis. For more information on API Traffic Logs, refer to this [documentation](/nightly/api-management/tyk-pump#api-traffic-logs).
</Note>
...
| [Tyk Gateway](/nightly/tyk-oss-gateway/configuration#log_level)   | `log_level`   | `TYK_GW_LOGLEVEL`    | `info`                   |
| [Tyk Pump](/nightly/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#log_level)     | `log_level`   | `TYK_PMP_LOGLEVEL`   | `info`                   |
| [Tyk Developer Portal](/nightly/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_log_level) | `logLevel`  | `PORTAL_LOG_LEVEL`   | `info`   |
...
The [track_404_logs](/nightly/tyk-oss-gateway/configuration#track_404_logs) is a configuration in Tyk Gateway. When you set it to `true`, the gateway will log all incoming HTTP requests that result in a 404 `Not Found` error because they don't match any of your configured API listen paths.
...
You can enable this feature by setting the [track_404_logs](/nightly/tyk-oss-gateway/configuration#track_404_logs) option to `true` in your `tyk.conf` file or by setting the [TYK_GW_TRACK404LOGS](/nightly/tyk-oss-gateway/configuration#track_404_logs) environment variable to `true`.
...
[Custom logging event handlers](/nightly/api-management/gateway-events#logging-api-events) can be registered against **Gateway events** to customise the logs that are generated for those events.
