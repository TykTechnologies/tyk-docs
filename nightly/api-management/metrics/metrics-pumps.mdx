...
canonical: "https://tyk.io/docs/api-management/metrics/metrics-pumps"
noindex: true
robots: "noindex, follow"
---

<Note>
For new deployments, we recommend [OpenTelemetry Metrics](/nightly/api-management/logs-metrics#opentelemetry-metrics) instead: Tyk Gateway pushes Rate, Errors, and Duration (RED) metrics via an OpenTelemetry Collector. This page remains relevant for existing deployments already using Tyk Pump for metrics.
</Note>

## Introduction

[Tyk Pump](/nightly/api-management/tyk-pump) can independently expose metrics, derived from the traffic logs it reads out of Redis, the same traffic logs used for [Dashboard Analytics](/nightly/api-management/dashboard-analytics). These metrics are computed by Tyk Pump after the fact from traffic log records, so it can only produce traffic-derived counters and histograms (status codes, latency, and whatever traffic log fields you tag). It has no visibility into Gateway's own runtime health or configuration state, unlike [OpenTelemetry metrics](/nightly/api-management/metrics/default-metrics).
...
<Note>
Not to be confused with [Tyk Gateway's native StatsD instrumentation](/nightly/api-management/logs-metrics#statsd-instrumentation): that's a completely different, built-in mechanism exporting internal system metrics (request rate, goroutine health, and so on), not traffic-log-derived request metrics. The [StatsD pump](#statsd) on this page is unrelated to that feature.
</Note>
...
In addition to the [common pump settings](/nightly/api-management/tyk-pump#common-pump-settings), the `prometheus` pump accepts the following `meta` fields. Add them to `pump.conf`:
...
The base metrics above cover a fixed set of dimensions. You can define your own counters and histograms instead, built from any of the following [traffic log fields](/nightly/api-management/logs/traffic-logs#traffic-log-field-reference):

| Custom Metric Label Name | Traffic Log Field | Description |
|---|---|---|
| host | Host | Inbound request host header |
| method | Method | HTTP method (GET, POST, etc.) |
| path | Path | Request path (or "unknown" if path tracking is disabled) |
| response_code | ResponseCode | HTTP response status code |
| code | ResponseCode | Alias for response_code |
| api_key | APIKey | API key (obfuscated if `obfuscate_api_keys` is set) |
| key | APIKey | Alias for api_key |
| time_stamp | TimeStamp | Request timestamp |
| api_name | APIName | Human-readable name of the API |
| api_version | APIVersion | Version of the API |
| api_id | APIID | Internal API ID |
| api | APIID | Alias for api_id |
| org_id | OrgID | Organisation ID |
| oauth_id | OauthID | OAuth client ID |
| client_id | OauthID | Alias for oauth_id |
| request_time | RequestTime | Total request latency |
| latency_total | Latency.Total | Total request latency from Latency struct |
| latency_upstream | Latency.Upstream | Time spent waiting for upstream backend |
| latency_gateway | Latency.Gateway | Gateway overhead latency |
| ip_address | IPAddress | Client IP address |
| alias | Alias | API alias configured in API definition |
| mcp_method | MCPStats.JSONRPCMethod | Model Context Protocol JSON-RPC method |
| mcp_primitive_type | MCPStats.PrimitiveType | MCP primitive type |
| mcp_primitive_name | MCPStats.PrimitiveName | MCP primitive name |
...
"custom_metrics": [
  ...
  {
    "name": "tyk_custom_requests_by_key",
    "description": "Number of API requests per API access key",
    "metric_type": "counter",
    "labels": ["response_code", "api_key"],
...
| `labels` | - | Traffic log fields to expose as metric labels. See the [table above](#custom-metrics). |
...
| `obfuscate_api_keys` | `false` | When set to true, masks API key values when `api_key` (or `key`) is included in labels. Keys longer than `obfuscate_api_keys_length` characters display *** followed by the trailing characters (e.g., ***abcd), while keys with `obfuscate_api_keys_length` or fewer characters are replaced with --. |
| `obfuscate_api_keys_length` | 4 | Defines the number of unmasked characters retained from the end of the API key when obfuscate_api_keys is true. Defaults to 4. |
...
<Note>
`mcp_method`, `mcp_primitive_type`, and `mcp_primitive_name` are only populated for MCP Gateway traffic. Use `mcp_only` to keep a metric exclusively for MCP records, for example to count MCP tool and prompt invocations separately from your REST API traffic:
...
In addition to the [common pump settings](/nightly/api-management/tyk-pump#common-pump-settings), the `statsd` pump accepts the following `meta` fields:
...
- A [Tyk Self-Managed](/nightly/tyk-self-managed/install) or [Tyk Open Source](/nightly/apim/open-source/installation) installation with [Tyk Pump](/nightly/api-management/tyk-pump) configured.
...
In addition to the [common pump settings](/nightly/api-management/tyk-pump#common-pump-settings), the `dogstatsd` pump accepts the following `meta` fields:
