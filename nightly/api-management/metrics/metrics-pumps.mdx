...
| Pump Type | Format |
| :-- | :-- |
| [Prometheus](/nightly/#prometheus) | Exposes an HTTP endpoint for Prometheus to scrape |
| [StatsD](/nightly/#statsd) | Sends per-request metrics to a StatsD server |
| [DogStatsD](/nightly/#dogstatsd-datadog) | Sends per-request metrics in the DogStatsD format, for example to Datadog |

<Note>
Not to be confused with [Tyk Gateway's native StatsD instrumentation](/nightly/api-management/logs-metrics#statsd-instrumentation): that's a completely different, built-in mechanism exporting internal system metrics (request rate, goroutine health, and so on), not traffic-log-derived request metrics. The [StatsD pump](/nightly/#statsd) on this page is unrelated to that feature.
</Note>
...
You can add further custom metrics of your own; see [Custom Metrics](/nightly/#custom-metrics) below.
...
| `custom_metrics` | (none) | Additional counters and histograms, on top of the [base metrics](/nightly/#base-metrics) above. See [Custom Metrics](/nightly/#custom-metrics) below. |
