...
canonical: "https://tyk.io/docs/api-management/metrics/default-metrics"
---
...
1. [Request metrics](/nightly/#request-metrics): Rate, Errors, and Duration (RED) for every API request
2. [Go runtime metrics](/nightly/#go-runtime-metrics): Memory, goroutines, and GC health
3. [Configuration state metrics](/nightly/#configuration-state-metrics): API/policy load counts and reload events
4. [Token exchange metrics](/nightly/#token-exchange-metrics): Volume, latency, and cache hit-rate for [token exchange](/nightly/api-management/authentication/token-exchange) (Enterprise only)

All metrics are exported as OpenTelemetry-semantic-convention-compliant instruments and include [resource attributes](/nightly/#resource-attributes) that identify the gateway instance.
...
| `tyk.response_flag` | Tyk 3-letter error classification code (e.g., `AKI`, `URS`, `CBO`), or the HTTP status code string (e.g., `"200"`) for successful requests or when error classification is absent. See the [full list of response flags](/nightly/api-management/logs/access-logs#error-classification). |

You can add more dimensions to any instrument using [custom metrics](/nightly/api-management/metrics/custom-metrics).
...
<Note>
Token exchange is an [Enterprise-only](/nightly/api-management/authentication/token-exchange) feature, available from Tyk 5.15.0. These metrics are only emitted on APIs with `oauth2.tokenExchange` configured.
</Note>

These metrics cover every [token exchange](/nightly/api-management/authentication/token-exchange) decision the Gateway makes when exchanging an inbound token for a downstream token (RFC 8693).

| Metric Name | Type | Unit | Description |
|-------------|------|------|-------------|
| `tyk.oauth2.exchange.requests` | Counter | exchanges | Total token exchange decisions, labelled by outcome and provider. Recorded on every attempt, including cache hits. |
...
| Dimension | Description |
|-----------|-------------|
| `outcome` | One of `ok`, `idp_error`, `misconfig`, `no_matching_provider`, or `step_up_required` ([jwt-bearer grant](/nightly/api-management/authentication/token-exchange#grant-type) only). |
...
Identity, audience, and requested scopes are never metric labels or dimensions — they're only available in structured logs and audit events, see [Token exchange observability](/nightly/api-management/authentication/token-exchange#observability).
...
| `tyk.gw.tags` | No | `["region:eu", "env:prod"]` | Present only when the Gateway has [segment tags](/nightly/api-management/api-sharding#what-is-api-sharding-) configured. |
