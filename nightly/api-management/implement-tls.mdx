...
canonical: "https://tyk.io/docs/api-management/implement-tls"
---
...
For more details on security protocols and public key cryptography, see our [TLS and Certificates](/nightly/api-management/certificates) guide.
...
1.  *Control API Interface*: The [Tyk Gateway API](/nightly/tyk-gateway-api) used for managing the Gateway (adding APIs, hot reloads, etc.)
...
Optionally, expose the Control API on a separate hostname from Hosted APIs. This is required to [secure the Control API with mTLS](/nightly/api-management/implement-tls#secure-the-gateway-control-api-with-mtls). See the [Planning For Production](/nightly/planning-for-production#change-your-control-port) guide for details.
...
| `certificates` | The server certificate pairs to associate with the API, each specified as a [certificate reference](/nightly/api-management/certificates#certificate-management) |
...
With [TLS enabled](/nightly/api-management/implement-tls#basic-server-configuration) for the Gateway you can enforce mutual TLS for requests to the **Control API** if it is [exposed on a different hostname](/nightly/planning-for-production#change-your-control-port) by setting the following options in the Gateway config file (`tyk.conf`) or equivalent environment variables:
...
| `security.certificates.control_api` | An array of CA certificates, each specified as a [certificate reference](/nightly/api-management/certificates#certificate-management) |
...
With [TLS enabled](/nightly/api-management/implement-tls#basic-server-configuration) for the Gateway you can selectively enforce mTLS per API.
...
- a dynamic list in the Redis in-memory database (historically referred to by Tyk as dynamic mTLS)
...
| `clientCertificates.allowlist` | A list of [certificate references](/nightly/api-management/certificates#certificate-management) for the client certificates that are authorized to access the API |
...
<Note>
  From Tyk 5.12, this is called Certificate Authentication. Please see the [dedicated section](/nightly/api-management/authentication/certificate-auth) for more details.
</Note>
...
The dynamic allow list comprises session objects in the Gateway's in-memory database (typically Redis) linked to the authorized client certificates.
...
For full details, see the [Certificate Auth](/nightly/api-management/authentication/certificate-auth#registering-certificate-authentication-user-credentials) section.
...
A key concept of the [mTLS handshake](/nightly/api-management/certificates#mutual-tls-mtls--two-way-authentication) is that it ensures that the client has possession of the private key. Possession of the certificate (i.e., the public key) does not confirm the client's identity (since this is by its nature public information).
...
From Tyk 5.12, the option to authenticate using only the token has been placed behind a Gateway configuration option [`allow_unsafe_dynamic_mtls_token`](/nightly/tyk-oss-gateway/configuration#security-allow_unsafe_dynamic_mtls_token). If this is not set (the default behavior), then requests presenting just the token will be rejected; the certificate must be presented and the mTLS handshake is enforced.
...
  Tyk uses Golang libraries to provide TLS functionality, so the range of supported TLS versions depends on the underlying library. Support for TLS 1.0 and 1.1 was removed in Go 1.22 (which was adopted in Tyk 5.3.6/5.6.0), so these are no longer supported by Tyk.
  ...
  The default minimum and maximum TLS versions are inherited from the Golang library and thus are presently 1.2 and 1.3 respectively.

  Prior to Tyk 5.8.14/5.13.0 the default for the maximum TLS version was set to 1.2.
</Note>
...
If TLS is in use for the Dashboard's [Control API](/nightly/api-management/implement-tls#using-tls-with-tyk-dashboard), i.e., the Dashboard is the TLS server, then the Gateway will need to verify the server certificate presented by the Dashboard (unless configured to [skip verification](/nightly/api-management/implement-tls#skipping-server-certificate-verification) of the server certificate).
...
| `proxy_ssl_disable_renegotiation` | Prevent [TLS renegotiation](/nightly/api-management/implement-tls#tls-1-2-renegotiation-behavior) (TLS 1.2 only) |
| `security.pinned_public_keys` | Specify an allow list of server certificates that will be accepted; if the upstream server presents a certificate that isn't on the list then the TLS handshake will fail; see [server certificate pinning](/nightly/api-management/upstream-authentication/mtls#certificate-pinning) |
| `security.certificate.upstream` | Map of upstream domains to [certificate references](/nightly/api-management/certificates#certificate-management) for mTLS client authentication |
| `ssl_force_common_name_check` | Validate that the upstream hostname matches the Common Name (CN) in the server certificate ([legacy support](/nightly/api-management/implement-tls#common-name-cn-check)) |

#### API-Specific Configuration

Granular configuration of the TLS connection is controlled via the [`upstream.tlsTransport`](/nightly/api-management/gateway-config-tyk-oas#tlstransport) object in the Tyk Vendor Extension:
...
| `forceCommonNameCheck` | Validate that the upstream hostname matches the Common Name (CN) in the server certificate ([legacy support](/nightly/api-management/implement-tls#common-name-cn-check)) |

Full details for configuring certificates for mTLS with upstream services are in the dedicated [upstream mTLS](/nightly/api-management/upstream-authentication/mtls) section.
...
The configuration is handled within the [`upstream.proxy`](/nightly/api-management/gateway-config-tyk-oas#proxy) object of the Tyk Vendor Extension (`proxy` in the Tyk Classic API definition):
...
Tyk's [External Services Configuration guide](/nightly/configure/external-service) provides the details required to secure these connections using mTLS.
...
- [Proxy connections](/nightly/api-management/implement-tls#routing-through-a-forward-proxy): Tyk performs standard certificate chain verification then additionally validates the CN
...
You must set the [host_config.generate_secure_paths](/nightly/tyk-dashboard/configuration#host_config-generate_secure_paths) flag to `true` so that your Dashboard URL starts with HTTPS. This will also set the default protocol for Target URLs to `https://` when creating APIs in the Dashboard's API Designer if no protocol is explicitly set.
...
- **Gateway Sync**: Ensure your Tyk Gateway is also configured with the same [Redis mTLS settings](/nightly/configure/external-service#service-specific-configuration)

For more details on configuring Tyk Dashboard with Redis, see the [Database Management](/nightly/planning-for-production/database-settings) guide.


#### Integrating with MongoDB

To enable mTLS for the connection between Dashboard and MongoDB persistent database, add the following settings to your `tyk_analytics.conf` file:
...
| `driver` | Choose the [MongoDB driver](/nightly/tyk-dashboard/configuration#mongo_driver) to be used, typically `mongo-go` |
...
For more details on using Tyk Dashboard with MongoDB, see the [Database Management](/nightly/planning-for-production/database-settings) guide.


#### Integrating with PostgreSQL

PostgreSQL uses connection string parameters for TLS configuration. To enable mTLS for the connection between Dashboard and PostgreSQL persistent database, add the following settings to your `tyk_analytics.conf` file:
...
For more details on using Tyk Dashboard with PostgreSQL, see the [Database Management](/nightly/planning-for-production/database-settings) guide.
...
Each server has a similar configuration to Dashboard, with settings described in the [reference documentation](/nightly/tyk-multi-data-centre/mdcb-configuration-options).

Note that you must set `security.private_certificate_encoding_secret` in the MDCB configuration file to the same value as specified in your Control Plane Gateway and Dashboard configuration files, to ensure that MDCB can [decode private keys](/nightly/api-management/certificates#encryption-of-the-private-key) that have been added to the Tyk Certificate Store.
...
| `security.certificates.mdcb_api` | A list of client certificate-key pairs, each specified as a [certificate reference](/nightly/api-management/certificates#certificate-management) |
