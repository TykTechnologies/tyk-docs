...
| --------- | ----------- |
| [`use_ssl`](/nightly/tyk-oss-gateway/configuration#http-server-options-use-ssl) | Enables the use of TLS for the Gateway server; if set to true then client requests to the control and hosted APIs must use HTTPS |
| [`server_name`](/nightly/tyk-oss-gateway/configuration#http-server-options-server-name) | (optional) If provided this will be used to identify the Gateway for SNI (Server Name Indication) in the TLS handshake |
| [`listen_port`](/nightly/tyk-oss-gateway/configuration#listen-port) | Set the port on which Tyk will listen; typically set to 443 for HTTPS |
...
| Parameter | Description |
| --------- | ----------- |
| [`ssl_certificates`](/nightly/tyk-oss-gateway/configuration#http-server-options-ssl-certificates) | An array of references to server certificate pairs, which can be either Certificate IDs from the Tyk Certificate Store or paths to PEM files |
...
| Parameter | Description |
| --------- | ----------- |
| [`enable_custom_domains`](/nightly/tyk-oss-gateway/configuration#enable-custom-domains) | Allow the use of custom domains for Hosted APIs |
...
A key concept of the [mTLS handshake](/nightly/api-management/certificates#mutual-tls-mtls-%3A-two-way-authentication) is that it ensures that the client has possession of the private key. Possession of the certificate (i.e., the public key) does not confirm the client's identity (since this is by its nature public information).
...
From Tyk 5.12, the option to authenticate using only the token has been placed behind a Gateway configuration option [`allow_unsafe_dynamic_mtls_token`](/nightly/tyk-oss-gateway/configuration#allow-unsafe-dynamic-mtls-token). If this is not set (the default behavior), then requests presenting just the token will be rejected; the certificate must be presented and the mTLS handshake is enforced.
...
| Parameter | Description |
| --------- | ----------- |
| [`proxy.enable_http2`](/nightly/tyk-oss-gateway/configuration#proxy-enable-http2) | Use HTTP/2 for upstream connections |
| [`proxy.ssl_insecure_skip_verify`](/nightly/tyk-oss-gateway/configuration#proxy-ssl-insecure-skip-verify) | Do not verify server certificates presented by the upstream; not recommended for production environments |
| [`proxy.ssl_min_version`](/nightly/tyk-oss-gateway/configuration#proxy-ssl-min-version) | Minimum TLS version allowed |
| [`proxy.ssl_max_version`](/nightly/tyk-oss-gateway/configuration#proxy-ssl-max-version)| Maximum TLS version allowed |
| [`proxy.ssl_ciphers`](/nightly/tyk-oss-gateway/configuration#proxy-ssl-ciphers) | Allowed cipher suites (TLS 1.2 only) |
| [`proxy.ssl_disable_renegotiation`](/nightly/tyk-oss-gateway/configuration#proxy-ssl-disable-renegotiation) | Prevent [TLS renegotiation](/nightly/api-management/implement-tls#tls-1-2-renegotiation-behavior) (TLS 1.2 only) |
| [`security.pinned_public_keys`](/nightly/tyk-oss-gateway/configuration#security-pinned-public-keys) | Specify an allow list of server certificates that will be accepted; if the upstream server presents a certificate that isn't on the list then the TLS handshake will fail; see [server certificate pinning](/nightly/api-management/upstream-authentication/mtls#certificate-pinning) |
| [`security.certificate.upstream`](/nightly/tyk-oss-gateway/configuration#security-certificates-upstream) | Map client certificates to upstream domains (for mTLS connections) |
| [`ssl_force_common_name_check`](/nightly/tyk-oss-gateway/configuration#ssl-force-common-name-check) | Validate that the upstream hostname matches the Common Name (CN) in the server certificate ([legacy support](/nightly/api-management/implement-tls#common-name-cn-check)) |
...
You must set the [host_config.generate_secure_paths](/nightly/tyk-dashboard/configuration#host_configgenerate_secure_paths) flag to `true` so that your Dashboard URL starts with HTTPS. This will also set the default protocol for Target URLs to `https://` when creating APIs in the Dashboard's API Designer if no protocol is explicitly set.
...
| `driver` | Choose the [MongoDB driver](/nightly/tyk-dashboard/configuration#mongo-driver) to be used, typically `mongo-go` |
