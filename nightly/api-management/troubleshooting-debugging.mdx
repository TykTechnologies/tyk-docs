...
Tyk supports comprehensive [error response customization](/nightly/api-management/custom-error-responses), allowing you to configure the Gateway to return customized messages for different HTTP error codes.
...
12. ##### How to Connect to DocumentDB with X.509 client cert
    ...
    Additionally DocumentDB can't be exposed to the local machine outside of the Amazon Virtual Private Cloud (VPC), which means that even if verification is turned on, it will always fail since if we use a SSH tunnel or a similar method, the domain will differ from the original. Also, it can have [Mutual TLS](/nightly/api-management/implement-tls#secure-hosted-apis-with-mtls) enabled.

    So, in order to support it, we provide the following variables for both our [Tyk Analytics Dashboard](/nightly/tyk-dashboard/configuration) and [Tyk Pump](/nightly/api-management/tyk-pump#configuring-tyk-pump):
    ...
    **Capped Collections**
    ...
    If you are using DocumentDB, [capped collections](/nightly/api-management/dashboard-analytics/analytics-storage-management) are not supported. See [here](https://docs.aws.amazon.com/documentdb/latest/developerguide/mongo-apis.html) for more details.
...
1. ##### Capturing detailed logs
    ...
    **What is detailed request logging?**
    ...
    When [detailed request logging](/nightly/api-management/logs/traffic-logs#detailed-recording) is enabled, Tyk will record the request and response in wire-format in the analytics database. This can be very useful when trying to debug API requests to see what went wrong for a user or client.

    This mode is configured in the gateway and can be enabled at the [system](/nightly/api-management/logs/traffic-logs#gateway-level), [API](/nightly/api-management/logs/traffic-logs#api-level) or [access key](/nightly/api-management/logs/traffic-logs#key-level) level.
...
3. ##### Data Seen in Log Browser but No Reports
    ...
    **Solution**
    ...
    The same applies in the reverse direction. If you are using `mongo-pump-aggregate` in your [pump configuration](/nightly/api-management/tyk-pump#configuring-tyk-pump), set `use_sharded_analytics` to false.
...
8. ##### How to Cap analytics data storage
    ...
    [TTL Indexes](/nightly/api-management/dashboard-analytics/analytics-storage-management#ttl-indexes)

    [Capped Collections](/nightly/api-management/dashboard-analytics/analytics-storage-management#capped-collections)
