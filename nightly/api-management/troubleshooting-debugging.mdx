...
18. ##### How to run two Gateways with docker-compose
    ...
    Managing a second Tyk Gateway with our [Tyk Pro Docker Demo](/nightly/tyk-self-managed/install/docker) is a case of mounting the `tyk.conf` file into a new volume and declaring a new Gateway service but exposed on a different port.
...
1. ##### Capturing detailed logs
    ...
    **What is detailed request logging?**
    ...
    When [detailed request logging](/nightly/api-management/tyk-pump#configure-detailed-recording) is enabled, Tyk will record the request and response in wire-format in the analytics database. This can be very useful when trying to debug API requests to see what went wrong for a user or client.

    This mode is configured in the gateway and can be enabled at the [system](/nightly/api-management/tyk-pump#gateway-level), [API](/nightly/api-management/tyk-pump#api-level) or [access key](/nightly/api-management/tyk-pump#key-level) level.
...
9. ##### MongoDB X.509 Client Authentication
    ...
    You can use the *MongoDB X509 Certificate* flow to authenticate the *Tyk Dashboard*, *Tyk Pump*, and *Tyk MDCB* with your *MongoDB* install.  This is slightly different from AWS DocumentDB setup instructions.
...
2. ##### Dashboard Debug Settings
    ...
    You can find the full [log levels](/nightly/api-management/logs#application-logs) in our documentation.
