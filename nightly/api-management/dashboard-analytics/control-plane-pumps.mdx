...
This page covers the [Tyk Pump types](/nightly/api-management/tyk-pump#pump-type-catalog) that write traffic logs and aggregated analytics into Tyk Dashboard's persistent storage (MongoDB or SQL): the pumps that ultimately feed Tyk Dashboard's Log Browser and Traffic Analytics screens.
...
| Protocol | MongoDB | SQL |
| :-- | :-- | :-- |
| REST | already covered by [`mongo`](/nightly/#standard-mongo-pump)/[`mongo-pump-selective`](/nightly/#per-organisation-mongo-pump) above | already covered by [`sql`](/nightly/#standard-sql-pump) above |
...
<Note>
Tyk no longer supports SQLite as of Tyk 5.7.0. Transition to [PostgreSQL](/nightly/planning-for-production/database-settings#postgresql), [MongoDB](/nightly/planning-for-production/database-settings#mongodb), or a listed compatible alternative.
</Note>
...
| `table_sharding` | `false` | If `true`, records are stored in one table per day instead of a single table. See [Table Sharding](/nightly/api-management/dashboard-analytics/analytics-storage-management#table-sharding). |
...
| `migrate_sharded_tables` | `false` | See [Table Sharding](/nightly/api-management/dashboard-analytics/analytics-storage-management#table-sharding). |
...
Accepts the same `track_all_paths`, `ignore_tag_prefix_list`, `store_analytics_per_minute`, and `omit_index_creation` fields as the [SQL Aggregate Pump](/nightly/#sql-aggregate-pump) above. The table name is fixed at `tyk_mcp_aggregated`; there's no `table_name` override, though [table sharding](/nightly/api-management/dashboard-analytics/analytics-storage-management#table-sharding) still applies, storing per-day tables such as `tyk_mcp_aggregated_20230327`.
