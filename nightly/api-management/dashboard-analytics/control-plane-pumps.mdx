...
canonical: "https://tyk.io/docs/api-management/dashboard-analytics/control-plane-pumps"
---

## Introduction

This page covers the [Tyk Pump types](/nightly/api-management/tyk-pump#pump-type-catalog) that write traffic logs and aggregated analytics into Tyk Dashboard's [persistent database](/nightly/api-management/persistent-database): the pumps that ultimately feed Tyk Dashboard's Log Browser and Traffic Analytics screens.

How you deploy these pumps depends on your deployment topology:

- In a combined control and data plane, such as the `tyk-stack` chart, you configure them directly: Tyk Pump already has access to both Redis and the persistent database.
- In a distributed deployment, with separate control and data planes connected via Tyk MDCB, these same pump types are instead configured as the Control Plane Pump, running on the control plane, which receives data forwarded by the Hybrid Pump on each data plane. See [Data Plane Pump](/nightly/api-management/dashboard-analytics/data-plane-pump) for that mechanism.

For an explanation of how traffic logging works in general, including detailed recording and aggregation, see the [Dashboard Analytics overview](/nightly/api-management/dashboard-analytics).

For settings common to every pump type, such as `filters` and `timeout`, see [Common Pump Settings](/nightly/api-management/tyk-pump#common-pump-settings).
...
For Tyk Dashboard's Traffic Analytics screens (Traffic Analytics graphs, Activity by Graph, Activity by MCP), data is written to the [Aggregate Analytics](/nightly/api-management/persistent-database#aggregate-analytics) category in the database using:

| Protocol | MongoDB | SQL |
| :-- | :-- | :-- |
| REST | [`mongo-pump-aggregate`](/nightly/#mongo-aggregate-pump) | [`sql_aggregate`](/nightly/#sql-aggregate-pump) |
| GraphQL | not available | [`sql-graph-aggregate`](/nightly/#sql-graphql-aggregate-pump) (PostgreSQL only) |
| MCP | [`mongo-mcp-aggregate`](/nightly/#mongo-mcp-aggregate-pump) | [`sql-mcp-aggregate`](/nightly/#sql-mcp-aggregate-pump) |

For Tyk Dashboard's Log Browser, data is written to the [Logs](/nightly/api-management/persistent-database#traffic-logs) category in the database using:

| Protocol | MongoDB | SQL |
| :-- | :-- | :-- |
| REST | [`mongo`](/nightly/#standard-mongo-pump) (or [`mongo-pump-selective`](/nightly/#per-organisation-mongo-pump), which splits Log Browser data into a separate collection per Organisation) | [`sql`](/nightly/#standard-sql-pump) |
| GraphQL | not available | not available |
| MCP | not available | not available |

For your own downstream querying, REST traffic reuses the same [Logs](/nightly/api-management/persistent-database#traffic-logs) category storage as the Log Browser above. GraphQL and MCP traffic instead write to their own dedicated collections or tables, which Tyk Dashboard never reads - typically pointed at the same database instance by convention, but not part of any category Tyk Dashboard itself recognizes:

| Protocol | MongoDB | SQL |
| :-- | :-- | :-- |
| REST | [`mongo`](/nightly/#standard-mongo-pump)/[`mongo-pump-selective`](/nightly/#per-organisation-mongo-pump) | [`sql`](/nightly/#standard-sql-pump) |
| GraphQL | [`mongo-graph`](/nightly/#mongo-graphql-pump) | [`sql-graph`](/nightly/#sql-graphql-pump) |
| MCP | [`mongo-mcp`](/nightly/#mongo-mcp-pump) | [`sql-mcp`](/nightly/#sql-mcp-pump) |

These pumps write the data into a MongoDB collection or SQL table, where it's queryable by your own tooling. For REST, that's the same `mongo`/`mongo-pump-selective`/`sql` pump you're already running for the Log Browser; GraphQL and MCP need a dedicated pump instead, since the plain pumps either pass GraphQL through without the extra structured detail, or exclude MCP entirely. Tyk Dashboard itself doesn't read any of this back out, so you'll need your own tooling, such as a BI tool or a scheduled export job, connected directly to that database. This is different from the [External Data Sink Pumps](/nightly/api-management/tyk-pump#external-data-sink-pumps), which actively forward traffic logs to a separate third-party system such as Splunk or Kafka.

## MongoDB

Tyk Pump offers MongoDB pump types for REST, MCP proxy and GraphQL traffic. All share the same connection settings.

The pumps are declared and configured as described in the [Tyk Pump configuration guide](/nightly/api-management/tyk-pump#declaring-pumps). Each pump to be deployed has its own entry in that configuration and has both [common](/nightly/api-management/tyk-pump#common-pump-settings) and specific configuration options. The specific config is held in the `meta` object - some of this is common, some is dependent on the specific pump as outlined in the following sections.
...
In addition to the [common pump settings](/nightly/api-management/tyk-pump#common-pump-settings) and [common `meta` fields](/nightly/#common-mongo-meta), the pump has the following settings:
...
| `collection_cap_enable` | `false` | If `true`, caps the collection at `collection_cap_max_size_bytes`, turning it into a FIFO buffer. See [Analytics Storage Management](/nightly/api-management/dashboard-analytics/analytics-storage-management). |
| `collection_cap_max_size_bytes` | 5GB | Maximum collection size, in bytes, when `collection_cap_enable` is `true`. |

Because this collection grows with every request, especially with detailed recording enabled, it [should be capped](/nightly/api-management/dashboard-analytics/analytics-storage-management).

**Tyk Dashboard Setting:** set [`use_sharded_analytics: false`](/nightly/tyk-dashboard/configuration#use_sharded_analytics) in the Tyk Dashboard configuration so that it queries the `tyk_analytics` collections to populate the Log Browser.
...
There's no `collection_name` field: the collection name is always computed per-Organisation. There's also no `collection_cap_*` support, so, unlike the [Standard Mongo Pump](/nightly/#standard-mongo-pump), Tyk Pump can't cap these collections for you. Given the volume of individual documents, cap each Organisation's collection manually instead, using MongoDB's `convertToCapped` command; see [Analytics Storage Management](/nightly/api-management/dashboard-analytics/analytics-storage-management#capped-collections) for the exact command.

**Tyk Dashboard Setting:** set [`use_sharded_analytics: true`](/nightly/tyk-dashboard/configuration#use_sharded_analytics) in the Tyk Dashboard configuration so that it queries `z_tyk_analyticz_{ORG_ID}` collections to populate the Log Browser.
...
| `track_all_paths` | `false` | If `true`, stores aggregated data for every endpoint rather than only [tracked endpoints](/nightly/api-management/dashboard-analytics#controlling-which-endpoints-are-tracked). |
| `ignore_tag_prefix_list` | (none) | Prefixes of [custom aggregation tags](/nightly/api-management/dashboard-analytics#custom-aggregation-tags) to exclude from aggregation. |
...
| `enable_aggregate_self_healing` | `false` | See [Self-Healing](/nightly/#self-healing) below. |
...
**Tyk Dashboard Setting:** this pump supplies the **API Usage Data** screens. Set [enable_aggregate_lookups: true](/nightly/tyk-dashboard/configuration#enable_aggregate_lookups) in the Tyk Dashboard configuration so it reads this pre-computed data, rather than computing it live from traffic logs on every request; see [Pre-Computed vs Live Aggregation](/nightly/api-management/dashboard-analytics#pre-computed-vs-live-aggregation) for the trade-off.

<Note>
Tyk Dashboard's [`use_sharded_analytics`](/nightly/tyk-dashboard/configuration#use_sharded_analytics) controls which collection(s) it queries. If `use_sharded_analytics: false`, you must set `use_mixed_collection: true` so the pump populates the shared collection `tyk_analytics_aggregates`, which Dashboard always reads in this mode. If `use_sharded_analytics: true`, ordinary per-Organisation queries read the per-Organisation collections, which the pump populates regardless of `use_mixed_collection`, but a superuser session not attached to a single Organisation still falls back to the shared collection, so set `use_mixed_collection: true` too if you need those cross-org views to show data.
</Note>
...
Enable [detailed recording](/nightly/api-management/logs/traffic-logs#detailed-recording) first, so that GraphQL information can be parsed from the request body and response; this applies globally across all APIs on the Gateway.
...
Accepts the same `collection_name`, `max_insert_batch_size_bytes`, `max_document_size_bytes`, and `collection_cap_*` fields as the [Standard Mongo Pump](/nightly/#standard-mongo-pump); `collection_name` is required, with no default.

<Note>
The [Standard Mongo Pump](/nightly/#standard-mongo-pump) doesn't filter out traffic logs for GraphQL requests, so they already reach `tyk_analytics` as ordinary, undifferentiated entries. If you point `mongo-graph`'s `collection_name` at the same collection as your Standard Mongo Pump, you'll get duplicate entries in the Log Browser for every GraphQL request, one plain copy and one GraphQL-enriched copy, without gaining anything: Log Browser can't display the extra fields anyway. Always use a separate, dedicated collection.
</Note>
...
The `mongo-mcp` pump stores MCP (Model Context Protocol) tool-call traffic logs in their own collection, mirroring the [Standard Mongo Pump](/nightly/#standard-mongo-pump). **Unlike GraphQL records, MCP records are excluded from the Standard Mongo Pump, so this is the only way to get MCP traffic logs into MongoDB.**

As with the [Mongo GraphQL Pump](/nightly/#mongo-graphql-pump), Tyk Dashboard doesn't read this data anywhere, including the Activity by MCP screen; it exists for your own downstream querying.
...
The `mongo-mcp-aggregate` pump computes aggregated MCP analytics, mirroring the [Mongo Aggregate Pump](/nightly/#mongo-aggregate-pump), and stores them in a collection called `z_tyk_mcp_analyticz_aggregate_{ORG_ID}`. Tyk Dashboard's MCP Traffic Analytics graphs are built on this aggregated analytics.
...
Accepts the same `use_mixed_collection`, `track_all_paths`, `ignore_tag_prefix_list`, `threshold_len_tag_list`, `store_analytics_per_minute`, `aggregation_time`, `enable_aggregate_self_healing`, and `ignore_aggregations` fields as the [Mongo Aggregate Pump](/nightly/#mongo-aggregate-pump) above. `use_mixed_collection: true` writes to the org-less `tyk_mcp_analytics_aggregate` collection instead of `tyk_analytics_aggregates`.

**Tyk Dashboard Setting:** this pump supplies the **Activity by MCP** screen. [enable_aggregate_lookups: true](/nightly/tyk-dashboard/configuration#enable_aggregate_lookups) is required in the Tyk Dashboard configuration, not just recommended: unlike the REST case, there's no working [live fallback](/nightly/api-management/dashboard-analytics#pre-computed-vs-live-aggregation) if it's left `false`, since that fallback has no MCP-specific logic and queries the plain REST traffic log collection, returning nothing useful. See [MCP Analytics](/nightly/ai-management/mcp-gateway/mcp-analytics) for the Dashboard side of this feature.
...
<Note>
Tyk no longer supports SQLite as of Tyk 5.7.0. Transition to [PostgreSQL](/nightly/tyk-configuration-reference/sql), [MongoDB](/nightly/tyk-configuration-reference/mongodb), or a listed compatible alternative.
</Note>
...
| `table_sharding` | `false` | If `true`, records are stored in one table per day instead of a single table. See [Table Sharding](/nightly/tyk-configuration-reference/sql#table-sharding). |
...
| `migrate_sharded_tables` | `false` | See [Table Sharding](/nightly/tyk-configuration-reference/sql#table-sharding). |
...
It has no pump-specific fields beyond [Common SQL Meta](/nightly/#common-sql-meta) and the [common pump settings](/nightly/api-management/tyk-pump#common-pump-settings): set `type` to `postgres` or `mysql` and configure those.
...
In addition to the [common pump settings](/nightly/api-management/tyk-pump#common-pump-settings) and [common `meta` fields](/nightly/#common-sql-meta), the pump has the following settings:
...
**Tyk Dashboard Setting:** supplies **Activity by API**, **Activity by Key**, and **Errors**. Set [enable_aggregate_lookups: true](/nightly/tyk-dashboard/configuration#enable_aggregate_lookups) in the Tyk Dashboard configuration so it reads this pre-computed data, rather than computing it live from traffic logs on every request, and configure matching SQL connection settings, as above. See [Pre-Computed vs Live Aggregation](/nightly/api-management/dashboard-analytics#pre-computed-vs-live-aggregation) for the trade-off.
...
<Note>
The [Standard SQL Pump](/nightly/#standard-sql-pump) doesn't filter out traffic logs for GraphQL requests, so they already reach `tyk_analytics` as ordinary, undifferentiated rows. If you point `sql-graph`'s `table_name` at the same table as your Standard SQL Pump, you'll get duplicate rows in the Log Browser for every GraphQL request, one plain copy and one GraphQL-enriched copy, without gaining anything: Log Browser can't display the extra fields anyway. Always use a separate, dedicated table.
</Note>

The same limitations apply as for the [Mongo GraphQL Pump](/nightly/#mongo-graphql-pump) above.

### SQL GraphQL Aggregate Pump

The `sql-graph-aggregate` pump computes aggregated GraphQL-specific analytics, mirroring the [SQL Aggregate Pump](/nightly/#sql-aggregate-pump), and stores them in a fixed table, `tyk_graph_aggregated`. Unlike the GraphQL pumps that store traffic logs, this one does feed Tyk Dashboard. Unlike Mongo, there's no `mongo-graph-aggregate` equivalent.
...
Accepts the same `track_all_paths`, `ignore_tag_prefix_list`, `store_analytics_per_minute`, and `omit_index_creation` fields as the [SQL Aggregate Pump](/nightly/#sql-aggregate-pump) above. The table name is fixed at `tyk_graph_aggregated`; there's no `table_name` override, though [table sharding](/nightly/tyk-configuration-reference/sql#table-sharding) still applies, storing per-day tables such as `tyk_graph_aggregated_20230327`.
...
The `sql-mcp` pump stores MCP tool-call traffic logs in a dedicated table, `tyk_analytics_mcp` by default, mirroring the [Standard SQL Pump](/nightly/#standard-sql-pump). **Unlike GraphQL records, MCP records are excluded from the [Standard SQL Pump](/nightly/#standard-sql-pump), so this is the only way to get MCP traffic logs into SQL.**

As with the [SQL GraphQL Pump](/nightly/#sql-graphql-pump), Tyk Dashboard doesn't read this data anywhere, including the Activity by MCP screen; it exists for your own downstream querying.
...
The `sql-mcp-aggregate` pump computes aggregated MCP analytics, mirroring the [SQL Aggregate Pump](/nightly/#sql-aggregate-pump), and stores them in a fixed table, `tyk_mcp_aggregated`; unlike the [SQL MCP Pump](/nightly/#sql-mcp-pump), this table name isn't configurable.
...
Accepts the same `track_all_paths`, `ignore_tag_prefix_list`, `store_analytics_per_minute`, and `omit_index_creation` fields as the [SQL Aggregate Pump](/nightly/#sql-aggregate-pump) above. The table name is fixed at `tyk_mcp_aggregated`; there's no `table_name` override, though [table sharding](/nightly/tyk-configuration-reference/sql#table-sharding) still applies, storing per-day tables such as `tyk_mcp_aggregated_20230327`.

**Tyk Dashboard Setting:** this pump supplies the **Activity by MCP** screen. [enable_aggregate_lookups: true](/nightly/tyk-dashboard/configuration#enable_aggregate_lookups) is required in the Tyk Dashboard configuration, not just recommended: unlike the REST case, there's no working [live fallback](/nightly/api-management/dashboard-analytics#pre-computed-vs-live-aggregation) if it's left `false`, since that fallback has no MCP-specific logic and queries the plain REST traffic log table, returning nothing useful. See [MCP Analytics](/nightly/ai-management/mcp-gateway/mcp-analytics) for the Dashboard side of this feature.
