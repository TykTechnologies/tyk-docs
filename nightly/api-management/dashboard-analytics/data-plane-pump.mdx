...
| `rpc_pool_size` | `5` | RPC connection pool size, see [Tuning the RPC Connection Pool](#tuning-the-rpc-connection-pool) below. |
...
Each connection has a real cost on both ends: the Hybrid Pump and Tyk MDCB each allocate a 64KB send buffer and a 64KB receive buffer per connection. This cost accumulates across every data plane connected to Tyk MDCB, not just your own. From Tyk MDCB 2.13.0, the control plane can cap the connections it accepts, with [server_options.max_connections](/nightly/tyk-multi-data-centre/mdcb-configuration-options#server_options-max_connections).
...
Tyk MDCB receives either [traffic logs or aggregated analytics](#traffic-logs-or-aggregated-analytics) from each Hybrid Pump, and writes it into the Control Plane's persistent database that Tyk Dashboard reads from.
