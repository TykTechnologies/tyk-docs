...
The [`timeout`](/nightly/tyk-oss-gateway/configuration#jwks-cache-timeout) is given in seconds. If set, this will override the default period of 240 seconds.
...
<Note>
The Dashboard API endpoint is restricted to users with `admin` privileges and can only be used to flush the cache for APIs in the user's [Organisation](/nightly/tyk-dashboard-api#organisations%2C-apis-and-users).
</Note>
