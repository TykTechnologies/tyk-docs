...
After the JWT signature has been [validated](/nightly/basic-config-and-security/security/authentication-authorization/json-web-tokens#signature-validation), Tyk uses the **claims** within the token to determine which security policies (access rights, rate limits and quotas) should be applied to the request.
...
2. **Policy Resolution**: Tyk determines which [policy](/nightly/api-management/policies#what-is-a-security-policy) to apply to the request:
   - From scope-to-policy mapping
   - From default policies

3. **Update Session**: The [session](/nightly/api-management/policies#what-is-a-session-object) is updated with the identity and policies.
...
[Security Policies](/nightly/api-management/policies) are applied (or mapped) to the session object to configure authorization for the request. Policies must be [registered](/nightly/api-management/policies#how-you-can-create-policies) with Tyk, such that they have been allocated a unique *Tyk Policy Id*.
...
When creating multiple policies that might be applied to the same JWT, we recommend using [partitioned policies](/nightly/api-management/policies#partitioned-policies) - policies that control specific aspects of API access rather than trying to configure everything in a single policy.
...
The following [session attributes](/nightly/api-management/policies#session-object) are modified based on the policies:
