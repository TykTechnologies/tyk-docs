...
canonical: "https://tyk.io/docs/api-management/authentication/oauth2-authentication"
noindex: true
...
- **Tyk OAuth 2.0**: Tyk Gateway acts as the authorization server, issuing and managing tokens itself. See [Tyk OAuth 2.0](/nightly/api-management/authentication/oauth-2).
...
Tyk serves the PRM document at `{listenPath}/{wellKnownPath}`. The default well-known path is `.well-known/oauth-protected-resource`. The endpoint does not require authentication, because clients read it before they have a token.

When PRM is enabled, Tyk adds a `WWW-Authenticate: Bearer resource_metadata="<PRM URL>"` header to `401 Unauthorized` responses, as defined in RFC 9728. OAuth 2.1 client libraries use this header to find the PRM document.
...
| `authorizationServers` | array of strings | - | Issuer URLs published in the PRM document. Clients use these to discover where to obtain a token. Tyk OAS API definitions must have at least one entry. |
...
Token exchange is Enterprise Edition only. For full configuration details, see [Token exchange](/nightly/api-management/authentication/token-exchange).
