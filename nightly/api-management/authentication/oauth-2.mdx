...
Tyk can act as an OAuth 2.0 *authorization server*, performing token generation and management for *clients* accessing APIs deployed on Tyk. There are many great resources on the Internet that will help you to understand the OAuth 2.0 Authorization Framework, which we won't attempt to duplicate here. We will provide a basic introduction to the [concepts and terminology](/nightly/#oauth-20-core-concepts) before we dive into the details of using Tyk as your *auth server*.
...
- **Multi-API Access**: Enable access to multiple APIs using a single OAuth token; configure one API for OAuth 2.0 token issuance and the other APIs with the [Auth Token](/nightly/api-management/authentication/bearer-token) method, linking them through a common policy

*Tyk as OAuth authorization server* supports the following *grant types*:

- [Authorization Code Grant](/nightly/#using-the-authorization-code-grant): the *client* is redirected to an *identity server* where the *user* must approve access before an *access token* will be issued
- [Client Credentials Grant](/nightly/#using-the-client-credentials-grant): used for machine-to-machine access, authentication is performed using only the *client Id* and *client secret*
- [Resource Owner Password Grant](/nightly/#using-the-resource-owner-password-grant) (a.k.a. Password Grant): only for use where the *client* is highly trusted, as the *client* must provide the *Resource Owner*'s own credentials during authentication
...
- [register a client app](/nightly/#client-app-registration) for each client that needs to access the API
- [configure your API proxy](/nightly/#configuring-your-api-proxy) to use the *Tyk OAuth 2.0* authentication method
...
The *access tokens* issued to clients by *Tyk Authorization Server* are lookup keys to [Sessions](/nightly/api-management/access-control/sessions-and-keys/understanding-sessions) and are usually associated with [Policies](/nightly/api-management/policies) at the point of creation. These allow the application of quotas, rate limits and access rights in the normal manner.
...
1. *Client apps* are registered per-API, so the first step is to [configure Tyk OAuth 2.0](/nightly/#configuring-your-api-proxy) as the security method to be used for the API. With this done, you can navigate to the OAuth Client management screen for the API from the **Actions** menu on the **Created APIs** screen:
...
- redirect URI
- [optional] [security policies](/nightly/#manage-client-access-policies) to be applied to access tokens generated for the client
- [optional] [metadata](/nightly/api-management/access-control/sessions-and-keys/understanding-sessions#metadata-and-context) to be added to the access tokens
...
When using Tyk as the Authorization Server with the Authorization Code grant, the following steps are followed after [registering the Client App](/nightly/#client-app-registration):

<img src="/img/diagrams/diagram_docs_authorization-code-grant-type@2x.png" alt="Authorization grant type flow" />

**Explanatory notes:**

(1) *client* makes a request to the [authorization endpoint](/nightly/#authorization-request) on the *Auth Server*
...
(6) when the *user* successfully authenticates and authorizes the request, the *Identity Server* must request an [Authorization Code](/nightly/#authorization-code-request) from the *Auth Server*

(8) The *Identity Server* provides the *Authorization Code* to the *client*

(9) The *client* exchanges the *Authorization Code* for an [Access Token](/nightly/#exchange-the-authorization-code-for-an-access-token) from the *Auth Server*

(10) The *client* uses the *Access Token* to authenticate with the protected API using the [Auth Token](/nightly/api-management/authentication/bearer-token) method
...
The identity server will need access to the Tyk Dashboard API to [obtain an Authorization Code](/nightly/api-management/authentication/oauth-2#oauth2-0-authorization-code).

[Tyk Identity Broker (TIB)](/nightly/tyk-identity-broker/overview) can fulfill this role, authenticating users against an external IdP and handling the authorization code exchange with Tyk Dashboard on their behalf. See [Issue API Access Tokens via TIB](/nightly/api-management/access-control/sessions-and-keys/issuing-tokens-via-tib) for details.
...
This endpoint is protected using [Basic Authentication](/nightly/api-management/authentication/basic-authentication) where the username is the *client Id* and the password is the *client secret*.
...
This command, issued by the *client* is the final step to obtain an access token for the `/my-api` proxy deployed on a Tyk Gateway at `https://tyk.cloud.tyk.io`. The basic auth key is the base64 encoded representation of `my-client-id:my-client-secret` The `client_id` and `redirect_uri` match those provided in the initial [authorization request](/nightly/#authorization-request). The `code` is the *authorization code* provided to the *identity server* in the [authorization code request](/nightly/#authorization-code-request).
...
- `token_type`: set to `bearer` indicating that the access token should be provided in an [Auth Token](/nightly/api-management/authentication/bearer-token) request to the protected API
- `refresh_token`: [optional] a special token that can be used in the [Refresh Token](/nightly/#using-refresh-tokens) flow
...
When using Tyk as the *authorization server* with the Client Credentials grant, the *client* accesses resources on behalf of itself rather than on behalf of a *user*, so there is no user login/authorization step (as seen with [Authorization Code grant](/nightly/#using-the-authorization-code-grant)). This flow is ideal for server-to-server interactions.

After [registering the Client App](/nightly/#client-app-registration), the *client* simply requests an access token directly from the authorization server:
...
When using Tyk as the *authorization server* with the Resource Owner Password grant, the *client* provides the *user's* credentials when requesting an access token. There is no user login/authorization step (as seen with [Authorization Code grant](/nightly/#using-the-authorization-code-grant)). **This flow is not recommended and is provided only for integration with legacy clients.**
...
- [Notifications](/nightly/#oauth-token-notifications) of token issuance will be sent to `http://notifyme.com` with the `X-Tyk-Shared-Secret` header set to `oauth-shared-secret`
...
As noted in the Tyk Classic API [documentation](/nightly/api-management/gateway-config-tyk-classic#configuring-authentication-for-tyk-classic-apis), you can select the Tyk as OAuth Server method using the `use_oauth2` option.
...
You configure whether Tyk should issue a refresh token within the [API proxy definition](/nightly/#configuring-your-api-proxy).
...
- [revoke a single token](/nightly/api-management/authentication/oauth-2#revoke-a-single-oauth-client-token)
- [revoke all tokens for a client app](/nightly/api-management/authentication/oauth-2#revoke-all-oauth-client-tokens)
...
- [`oauth_token_expire`](/nightly/tyk-oss-gateway/configuration#oauth_token_expire): Overrides the default expiration time (in seconds) for newly generated access tokens. This value sets the logical `expires` timestamp in the access token's [Session](/nightly/api-management/access-control/sessions-and-keys/understanding-sessions).
- [`oauth_refresh_token_expire`](/nightly/tyk-oss-gateway/configuration#oauth_refresh_token_expire): Overrides the default expiration time (in seconds) for newly generated refresh tokens. If not set, it defaults to 14 days (1,209,600 seconds). When this TTL expires, the refresh token is physically deleted from Redis.
- [`oauth_token_expired_retain_period`](/nightly/tyk-oss-gateway/configuration#oauth_token_expired_retain_period): Controls a background cleanup job that removes expired tokens from an internal tracking list (a Redis sorted set used to track all tokens issued to a specific OAuth Client). It does **not** control the Redis TTL or the retention of the actual Session data.
...
For more details on how to control the physical retention of access token session data, see the [Session Lifecycle](/nightly/api-management/access-control/sessions-and-keys/session-lifecycle) documentation.
...
The example given [above](/nightly/#using-the-api-definition) includes the configuration necessary to issue notifications for token issuance (see lines 48-51 in the example).

## Managing OAuth Tokens via the Tyk Dashboard API

In addition to the OAuth client management endpoints in the [Tyk Dashboard API](/nightly/tyk-dashboard-api), the following endpoints are available to manage OAuth tokens and obtain authorization codes.
