...
Each entry in `providers` matches inbound tokens by `iss` claim and routes exchange requests to the corresponding token endpoint. Provider names must be unique, and issuer values must not overlap across providers. If no provider matches the inbound token's issuer, the exchange fails and Tyk returns an error before the request reaches the upstream.
