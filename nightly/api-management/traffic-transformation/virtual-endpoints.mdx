...
description: "Learn how to terminate requests with a Virtual Endpoint"
keywords: "Traffic Transformation, Virtual Endpoints"
sidebarTitle: "Virtual Endpoints"
canonical: "https://tyk.io/docs/api-management/traffic-transformation/virtual-endpoints"
noindex: true
...
Tyk provides a very flexible [middleware chain](/nightly/api-management/traffic-transformation#request-middleware-chain) where you can combine functions to implement the access controls you require to protect your upstream services. Of course, not all scenarios can be covered by Tyk's standard middleware functions, but you can use a virtual endpoint to apply whatever custom logic you require to optimize your API experience.

#### Dynamic Routing

With a virtual endpoint you can implement complex [dynamic routing](/nightly/transform-traffic/url-rewriting) of requests made to a single external endpoint on to different upstream services. The flexibility of the virtual endpoint gives access to data within the request (including the key session) and also the ability to make calls to other APIs to make decisions on the routing of the request.

### Working

The virtual endpoint middleware provides a JavaScript engine that runs the custom code that you provide either inline within the API definition or in a source code file accessible to Tyk Gateway. Virtual endpoints use the same [JavaScript engine choice](/nightly/api-management/plugins/javascript#choosing-a-javascript-engine) as custom plugins, and default to the legacy `otto` engine if `driver` is left unset. To use the newer `javascript` engine, set `driver` to `javascript` explicitly. In Tyk OAS, this field is `x-tyk-api-gateway.middleware.global.pluginConfig.driver`. In a Tyk Classic API definition, it is `custom_middleware.driver`.
...
<Note>
You will need to enable Tyk's JavaScript Virtual Machine by setting `enable_jsvm` to `true` in your `tyk.conf` [file](/nightly/tyk-oss-gateway/configuration#enable_jsvm) for your virtual endpoints to work.
</Note>


### Scripting virtual endpoint functions

The [middleware scripting guide](/nightly/api-management/plugins/javascript#using-javascript-with-tyk) provides guidance on writing JS functions for your virtual endpoints, including how to access key session data and custom attributes from the API definition.
...
All virtual endpoints on an API share that API's virtual machine, so give each virtual endpoint on the same API a distinct function name. If two virtual endpoints declare the same function name, the last one loaded shadows the others. Only the surviving definition is live in the VM, so a request can end up running the wrong function. You can reuse names freely across different APIs, since each API has its own virtual machine. Unlike virtual endpoints, [custom middleware plugins](/nightly/api-management/plugins/javascript#using-javascript-with-tyk) on the `javascript` engine are compiled in isolated scopes, so this naming constraint applies only to virtual endpoints.
...
## Using Classic
<a id="virtual-endpoints-using-classic"></a>
...
If you want to use Virtual Endpoints, you must [enable Tyk's JavaScript Virtual Machine](/nightly/tyk-oss-gateway/configuration#enable_jsvm) by setting `enable_jsvm` to `true` in your `tyk.conf` file.
...
In this example, we demonstrate how you can access different [external Tyk objects](/nightly/api-management/plugins/javascript#accessing-external-and-dynamic-data) (API request, session key, API definition).
...
You can add [custom attributes](/nightly/api-management/plugins/javascript#adding-custom-attributes-to-the-api-definition) to the API definition and access these from within your Virtual Endpoint.
...
One of the most common use cases for virtual endpoints is to provide some form of aggregate data to your users, combining the responses from multiple upstream service calls. This virtual endpoint function will do just that using the batch processing function from the [JavaScript API](/nightly/api-management/plugins/javascript#javascript-api)
