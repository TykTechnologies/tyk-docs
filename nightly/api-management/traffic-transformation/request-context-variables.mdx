...
description: How to configure Request Context Variables traffic transformation middleware
  in Tyk
...
<Note>
When using Tyk Classic APIs, you must [enable](/nightly/#enabling-context-variables-for-use-with-tyk-classic-apis) context variables for the API to be able to access them. When using Tyk OAS APIs, the context variables are always available to the context-aware middleware.
</Note>
...
1.   URL Rewriter - Syntax is `$tyk_context.CONTEXTVARIABLES`. See [URL Rewriting](/nightly/transform-traffic/url-rewriting#url-rewrite-middleware) for more details.
2.   Modify Headers - Syntax is `$tyk_context.CONTEXTVARIABLES`. See [Request Headers](/nightly/api-management/traffic-transformation/request-headers) for more details.
3.   Body Transforms - Syntax is `{{ ._tyk_context.CONTEXTVARIABLES }}`. See [Body Transforms](/nightly/api-management/traffic-transformation/request-body) for more details.
