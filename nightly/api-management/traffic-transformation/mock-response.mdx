...
| :--- | :--- | :--- |
| [Mock Response](/nightly/#mock-response) | Fixed response configured in the API definition | Static responses, deprecation notices, health checks, development mocking |
| [Virtual Endpoint](/nightly/#virtual-endpoint) | JavaScript function runs in the Gateway | Dynamic responses, conditional logic, aggregation from multiple upstreams |
| [Custom Plugins](/nightly/#custom-plugins) | Custom code in Go or via gRPC (Python, Lua, others) | High-performance termination, complex business logic, access to external systems |
...
This section applies to mock responses generated from the OpenAPI description (`fromOASExamples`). Note that when the response is [configured directly](/nightly/#tyk-oas) in the Tyk Vendor Extension, Tyk returns it as-is and response selection does not apply.
...
For full configuration detail, the JavaScript function API, and examples, see [Virtual Endpoints](/nightly/api-management/traffic-transformation/virtual-endpoints).
...
- [Go Plugins](/nightly/api-management/plugins/golang)
- [Rich Plugins](/nightly/api-management/plugins/rich-plugins) (gRPC, Python, Lua)
- [Plugin Types](/nightly/api-management/plugins/plugin-types)

## Worked Example

Acme Publishing is retiring the legacy single-book download endpoint `GET /books/{category}/{id}/download` in favor of the authenticated download flow introduced in [Internal Routing](/nightly/advanced-configuration/transform-traffic/looping). The endpoint is configured to return `HTTP 410 Gone` with a message directing clients to the replacement flow.
