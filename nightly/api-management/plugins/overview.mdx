...
canonical: "https://tyk.io/docs/api-management/plugins/overview"
---
...
When Tyk receives an API request, it works through a [chain](/nightly/api-management/traffic-transformation#request-middleware-chain) of processing *middleware* that is configured using the API definition. There are a large number of built-in middleware in the processing chain that are dedicated to performing [client authentication](/nightly/api-management/client-authentication), [request transformation](/nightly/api-management/traffic-transformation), [caching](/nightly/api-management/response-caching) and many other processes before proxying the request to the upstream.
...
There are several different stages of the [API request lifecycle](/nightly/api-management/traffic-transformation#request-middleware-chain) where custom plugins can be attached (or *hooked*) into the middleware chain allowing significant customization to meet your specific requirements.
...
1. [Pre (Request)](/nightly/api-management/plugins/plugin-types#request-plugins)
2. [Authentication](/nightly/api-management/plugins/plugin-types#authentication-plugins)
3. [Post-Auth (Request)](/nightly/api-management/plugins/plugin-types#request-plugins)
4. [Post (Request)](/nightly/api-management/plugins/plugin-types#request-plugins)
5. [Response](/nightly/api-management/plugins/plugin-types#response-plugins)
6. [Analytics (Response)](/nightly/api-management/plugins/plugin-types#analytics-plugins)
...
1. **[Request Plugin](/nightly/api-management/plugins/plugin-types#request-plugins)**
2. **[Authentication Plugin](/nightly/api-management/plugins/plugin-types#authentication-plugins)**
3. **[Response Plugin](/nightly/api-management/plugins/plugin-types#response-plugins)**
4. **[Analytics Plugin](/nightly/api-management/plugins/plugin-types#analytics-plugins)**

To know more about plugin types and it's advanced configuration, refer the following [docs](/nightly/api-management/plugins/plugin-types).
...
<Card href="/nightly/api-management/plugins/overview#dashboard-plugins" img="/img/logos/tyk-logo-selfmanaged.svg">
**Read time: 15 mins**

Dashboard Tutorial
</Card>

<Card href="/nightly/api-management/plugins/overview#open-source-plugins" img="/img/logos/tyk-logo-selfmanaged.svg">
**Read time: 15 mins**
...
3. **View the analytics**
    ...
    As we can see, when we send API requests, the Tyk Pump will scrape them from Redis and then send them to a persistent database as configured in the Tyk Pump env file. 

    In this example, we've configured a simple `STDOUT` Pump where the records will be printed to the Standard OUT (docker logs!)

## API Configuration

This page provides an overview on how to register one or more custom plugins to be executed at different stages or [hooks](/nightly/api-management/plugins/plugin-types#plugin-and-hook-types) in the API request/response lifecycle. If you wish to learn how to register custom plugins to be executed on the traffic logs generated by the Gateway please refer to the [analytics plugins](/nightly/api-management/plugins/plugin-types#analytics-plugins) page. 

If you need fine-grained control at the endpoint level then it is also possible to configure [per-endpoint plugins](/nightly/api-management/plugins/plugin-types#per-endpoint-custom-plugins). These are custom Golang plugins that are triggered at the end of the request processing chain before API-level *Post* plugins are executed.
...
1. **gRPC plugins**: Plugin functions are implemented by a gRPC server with the associated configuration specified with the API definition. For further details on how to configure gRPC plugins, please refer to our [gRPC](/nightly/api-management/plugins/rich-plugins#overview-1) documentation.
...
3. **Plugin bundles**: The plugin source code and configuration are bundled into a zip file that is served by a remote web server. For further details see the [plugin bundles](/nightly/api-management/plugins/overview#plugin-bundles) page.
...
This guide explains how to configure plugins for Tyk OAS APIs within the [Tyk OAS API definition](/nightly/#tyk-oas-apidef) or via the [API designer](/nightly/#tyk-oas-dashboard) in Tyk Dashboard.

If you’re using the legacy Tyk Classic APIs, then check out the [Tyk Classic](/nightly/api-management/plugins/overview#tyk-classic-apis) page.
...
The `pluginConfig` section contains the `driver` parameter that is used to configure the plugin implementation [language](/nightly/api-management/plugins/overview#plugin-driver-names):
...
The configuration above is a complete and valid Tyk OAS API Definition that you can use as a basis for trying out custom plugins. You will need to update the [driver](/nightly/api-management/plugins/overview#plugin-driver-names) parameter to reflect the target language type of your plugins. You will also need to update the `path` and `functionName` parameters for each plugin to reflect the source code.
...
1. **Configure plugin type and custom data**
    ...
    In the *Plugins Configuration* section, select the *Plugin Driver*, which tells Tyk which type of plugin to expect: Go, gRPC, JavaScript (from Tyk 5.15.0 we recommend the `javascript` option for better performance), Lua or Python.
...
This guide explains how to configure plugins for Tyk Classic APIs within the [Tyk Classic API definition](/nightly/#tyk-classic-apidef) or via the [API designer](/nightly/#tyk-classic-dashboard) in Tyk Dashboard.

If you’re using the newer Tyk OAS APIs, then check out the [Tyk OAS](/nightly/api-management/plugins/overview#tyk-oas-apis) page.
...
The `driver` configuration parameter describes the plugin implementation language. Please refer to the [supported languages](/nightly/api-management/plugins/overview#plugin-driver-names) section for list of supported plugin driver names.
...
2. **Edit the Tyk Classic API Definition to configure plugins**
    ...
    Use the editor to edit the `custom_middleware` section of the [Tyk Classic API Definition](/nightly/api-management/plugins/overview#tyk-classic-apis).
...
The plugin source code and associated configuration are co-located with Tyk Gateway in the same file system. The configuration is located within the API Definition. For further details please consult [API configuration](/nightly/api-management/plugins/overview#api-configuration).

Local plugins can be supplied to Tyk Gateway in one of two ways:

- **Volume-mounted files**: the plugin file is mounted into the Gateway's file system at runtime, for example using a Kubernetes ConfigMap, Secret, or CSI volume. See [Custom Plugins with Tyk Operator](/nightly/product-stack/tyk-operator/advanced-configurations/custom-plugins) for an example that mounts a plugin this way.
- **Baked into the container image**: the plugin file is copied into a custom Tyk Gateway image at build time, so that it ships as part of the container.

Both variants share the same underlying constraint: Tyk Gateway only loads the plugin file that is present in its file system when the API definition referencing it is loaded. Any change to the plugin therefore requires the file to be replaced and the affected API definition to be reloaded. If the plugin is baked into the container image, that still means restarting or rolling out the Gateway pods, since the image itself has to change. If it is volume-mounted, updating the file in place followed by a Gateway reload can be enough, without a pod restart.

#### Advantages Of Local Plugins

- No external bundle server, network access, or signing keys are required, as the plugin ships with the Gateway.
- Tyk Gateway fails to load the API if the plugin file is missing, which makes startup issues easy to spot.
- You can roll out a new plugin version alongside the old one, for example during an upgrade, by shipping both files and pointing the relevant API definitions at the new file path. This avoids rebuilding a shared bundle for every API.

#### Trade-Offs Of Local Plugins

- Every plugin change requires the file to be replaced and the API definition reloaded. If the plugin is baked into the image, this also means restarting or rolling out the Gateway pods to deploy the new image, which can interrupt live traffic if it is not managed carefully.
- Each cluster or Gateway deployment needs its own copy of the image or mounted files, so a multi-cluster fleet can end up with several copies to keep in sync.
- Tyk Gateway does not version local plugins for you, so tracking which Gateway is running which plugin build depends on your own image or file naming discipline.

<Note>
Tyk Cloud Data Planes only support plugin bundles for custom plugins. If you use Tyk Cloud, see [Configure Custom Plugins in Tyk Cloud](/nightly/tyk-cloud/using-plugins).
</Note>

### Plugin Bundles (Remote)

The plugin source code and associated configuration are bundled into a zip file and uploaded to a remote webserver. Multiple plugins can be stored in a single *plugin bundle*. Tyk Gateway will download the plugin bundle from the remote webserver and then extract, cache and execute plugins for each of the configured phases of the API request / response lifecycle. For further details on plugin bundles and how to configure them, please refer to the [plugin bundles](/nightly/api-management/plugins/overview#plugin-bundles) page.

### gRPC Plugins (Remote)

Custom plugins can be hosted on a remote server and executed from the Tyk Gateway middleware chain via gRPC. These plugins can be written in any language you prefer, as they are executed on the gRPC server. You'll configure your API definition so that Tyk Gateway will send requests to your gRPC server at the appropriate points in the API request / response lifecycle. For further details please consult our [gRPC](/nightly/api-management/plugins/rich-plugins#overview-1) documentation.

### Inline Code (JavaScript Only)

As an alternative to a file path or bundle, a JavaScript plugin's source can be embedded directly in the API definition using the `code` field. This is useful for developing and testing a plugin directly from the API Designer, without needing to host a file or build a bundle first.

<Warning>
Inline code only works with the `javascript` driver, which must be set explicitly. Setting `code` while `driver` is `otto` loads nothing, silently - no error is raised. See [Inline Plugin Code](/nightly/api-management/plugins/javascript#inline-plugin-code) for details.
</Warning>

### Choosing Between Local Plugins And Plugin Bundles

Neither approach is universally better. Use the table below to compare them against your operational requirements.

| Dimension | Local Plugins (Volume-Mounted Or Baked-In) | Plugin Bundles (Remote) |
| --- | --- | --- |
| Distribution scope | Per Gateway deployment. The image or mounted files must be duplicated across clusters | Shared. Any Gateway that can reach the bundle server uses the same bundle |
| Update workflow | Rebuild the image and roll out the Gateway, or update the mounted files and reload the Gateway | Publish a new bundle file, update the API definition, then trigger a Gateway reload |
| External dependencies | None beyond your own image registry or cluster storage | Requires an HTTP(S) server to host the bundle, and optionally signing keys |
| Adding or changing a single plugin file | Copy or mount the new file directly | Rebuild and republish the entire bundle, as all files in a bundle are fixed by its `manifest.json` at build time |
| Rollback | Roll the Gateway back to the previous image or mounted files | Point the API definition back to the previous bundle filename |
| Behavior if the plugin source is unavailable at startup | The Gateway fails to load the affected API | Tyk Gateway can keep using a previously cached copy of the same bundle if the download fails. A first-time download or a new bundle filename still fails to load the affected API |
| Best fit | Air-gapped or restricted-network environments, or teams with a mature image or configuration promotion workflow | Fleets that share plugins across many APIs or Gateways, or that need frequent plugin updates without restarting pods |

Consider the following when deciding between the two:

- Use **local plugins** if you operate in a restricted network, want to avoid a runtime dependency on an external server, or already have a strong image or configuration promotion workflow.
- Use **plugin bundles** if you need the same plugin shared across many APIs or Gateways, want to roll out plugin changes without restarting pods, or require [signed bundles](/nightly/api-management/plugins/overview#gateway-configuration) for supply-chain integrity.
- The two approaches are not mutually exclusive. Different APIs on the same Tyk Gateway fleet can use either approach, as long as each API definition clearly states where its plugin lives.
- If you use [Tyk Cloud](/nightly/tyk-cloud/using-plugins), plugin bundles are the only supported option for custom plugins.

## Plugin Bundles

For Tyk Gateway to execute local custom plugins during the processing of API requests and responses, the plugin source code must be loaded into the Gateway. The source is usually stored in files and the API definition is used to point the Gateway at the correct file for each [plugin type](/nightly/api-management/plugins/plugin-types#plugin-types). To simplify the management of plugins, you can group (or *bundle*) multiple plugin files together in a ZIP file that is referred to as a *plugin bundle*.
...
The source code and a [manifest file](/nightly/#manifest) are bundled into a zip file and uploaded to an external remote web server. The manifest file references the source code file path and the function name within the code that should be invoked for each [plugin type](/nightly/api-management/plugins/plugin-types#plugin-types). Within the API definition, custom plugins are configured simply using the name of the bundle (zip file). Tyk Gateway downloads, caches, extracts and executes plugins from the downloaded bundle according to the configuration in the manifest file. 

<img src="/img/plugins/plugin-bundles-overview.png" alt="plugin bundles architectural overview" />

### Composing Multiple Bundles

From Tyk 5.15.0, you can attach more than one plugin bundle to a single API by providing a comma-separated list of bundle filenames, for example `"auth-bundle.zip,logging-bundle.zip"`, wherever you would otherwise give a single bundle filename. Tyk Gateway downloads and extracts each bundle independently, then merges their manifests in the order the filenames are listed:

- The `pre`, `post`, `post_key_auth` and `response` hook lists are concatenated, in the order the bundles are listed.
- `auth_check` is single-valued: if more than one bundle in the list configures it, the API fails to load.
- All bundles in the list must use the same `driver`; a mismatch causes the API to fail to load.

A single bundle filename (no comma) continues to work exactly as before.

This lets you combine standard, centrally-managed bundles (for example, a security team's authentication plugin) with a team's own additional plugins on the same API, without having to rebuild a combined bundle every time either side changes.
...
-  `public_key_path`: Sets a public key, used for verifying signed bundles. If unsigned bundles are used you may omit this.
    ...
    <Note>
    Remember to set `"enable_coprocess": true` in your `tyk.conf` when using [rich plugins](/nightly/api-management/plugins/overview#plugin-bundles)!
    </Note>
...
Tyk provides the Bundle CLI tool as part of the `tyk` binary. For further details please visit the [Bundle CLI tool](/nightly/api-management/plugins/overview#bundler-cli-tool) page.

### Tyk OAS API Configuration

For API plugins that are deployed as [plugin bundles](/nightly/api-management/plugins/overview#plugin-bundles), the API should be configured with the name of the plugin bundle file to download from your remote web server. Furthermore, the Gateway should be [configured](/nightly/api-management/plugins/overview#gateway-configuration) to enable downloading plugin bundles.
...
For custom plugins that are deployed as [plugin bundles](/nightly/api-management/plugins/overview#plugin-bundles), the API should be configured with the name of the plugin bundle file to download from your remote web server. Furthermore, the Gateway should be [configured](/nightly/api-management/plugins/overview#gateway-configuration) to enable downloading plugin bundles.
...
If you’re using the newer Tyk OAS APIs, then check out the [Tyk OAS](/nightly/api-management/plugins/overview#tyk-oas-api-configuration) page.
...
Tyk Gateway should be configured for downloading plugin bundles from a secured web server. Please consult the [plugin bundles](/nightly/api-management/plugins/overview#plugin-bundles) documentation for further details.
...
[plugin bundles](/nightly/api-management/plugins/overview#plugin-bundles).
...
- **Manifest.json**: The [manifest.json](/nightly/api-management/plugins/overview#manifest) file
...
*   [Golang](/nightly/api-management/plugins/golang#): A plugin written in Golang is called a **Native Plugin**. Tyk recommends using Go plugins for performance, flexibility, and nativity reasons (all Tyk components are written in Go).
*   [JavaScript](/nightly/api-management/plugins/javascript#): A plugin written in Javascript uses JavaScript Virtual Machine (JSVM) interpreter.
*   [Rich Plugins](/nightly/api-management/plugins/rich-plugins#) includes Python, Lua, gRPC - With gRPC, you can write plugins in Java, .NET, C++ / C#, PHP, and all other [gRPC supported languages](https://grpc.io/docs/languages/).
...
* Used to stop the request and return a [custom response](/nightly/api-management/plugins/plugin-types#return-overrides-%2F-returnoverrides)
* Be served using [Bundles](/nightly/api-management/plugins/overview#plugin-deployment-types) or by files on the file system, except gRPC of course which by definition is served by some webserver in the language of your choosing

### Plugin Hook Types

Tyk provide 5 different phases, i.e. hooks to inject custom plugin throughout the [API execution lifecycle](/nightly/api-management/traffic-transformation#request-middleware-chain).

The following table shows which phases/hooks are supported by each plugin language:
...
1. JavaScript gained support for the Auth, Post-Auth and Response hooks in Tyk 5.15.0 (previously limited to Pre and Post) - see [Choosing a JavaScript Engine](/nightly/api-management/plugins/javascript#choosing-a-javascript-engine). This applies to both the `otto` and `javascript` drivers. Lua is the only language not available on the Response hook.

More reading on the [hook types](/nightly/api-management/plugins/rich-plugins#coprocess-dispatcher-hooks) in rich plugins and explanation with common use case for each [hook type](/nightly/api-management/plugins/plugin-types#plugin-types) 
...
| :---------- | :--------- |
| GoLang     | goplugin  |
| JavaScript | `javascript` (from Tyk 5.15.0, recommended) or `otto` (legacy) - see [Choosing a JavaScript Engine](/nightly/api-management/plugins/javascript#choosing-a-javascript-engine) |
| gRPC       | grpc      |
| Python 		 | python    |
| Lua        | lua	     |
...
| Built-in SDK | ✅<br />All Gateway Functionality | ✅<br />[Yes](/nightly/api-management/plugins/javascript#javascript-api)	| ❌	| ✅<br />[Yes](/nightly/api-management/plugins/rich-plugins#tyk-python-api-methods)	| ❌
| TCP Connections<p>(DBs, Redis, etc)</p> | ✅ | ❌<br />No general access<sup>1</sup> | ✅ | ✅ | ✅ | 

1. `javascript` (Goja) plugins can perform a small set of atomic operations (get/set/delete/increment) against Tyk's own Redis instance - see [Atomic Storage Bindings](/nightly/api-management/plugins/javascript#atomic-storage-bindings). This isn't general TCP or socket access, doesn't extend to other databases, and isn't available on `otto`.
...
*   **Performance**: on the `otto` driver, JSVM is performant but not easy to optimize, and requires a copy of the interpreter object for each request, which can increase memory footprint. The `javascript` driver (Goja) is significantly faster to start and execute - see [Choosing a JavaScript Engine](/nightly/api-management/plugins/javascript#choosing-a-javascript-engine).
...
*   **TCP Access**: The JSVM has no general socket access, so working with DB drivers directly is not possible. The `javascript` driver is a partial exception: it can perform a small set of atomic operations against Tyk's own Redis instance - see [Atomic Storage Bindings](/nightly/api-management/plugins/javascript#atomic-storage-bindings) - but this doesn't extend to other databases, and isn't available on `otto`.
...
- For [gRPC plugins](/nightly/api-management/plugins/rich-plugins#overview-1), Tyk Gateway can only be configured to integrate with one gRPC server.
- JavaScript [dynamic event handlers](/nightly/api-management/gateway-events) always run on the legacy `otto` engine, regardless of an API's configured plugin driver. This doesn't affect custom plugins or virtual endpoints, both of which support either driver.
...
[Tyk Plugins](/nightly/api-management/plugins/overview#) are a powerful tool that allows you to develop custom middleware that can intercept requests at different stages of the request lifecycle, modifying/transforming headers and body content.
