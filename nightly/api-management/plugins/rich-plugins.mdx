...
canonical: "https://tyk.io/docs/nightly/api-management/plugins/rich-plugins"
---
...
<h3 className="id-extractor-auth-plugin">ID Extractor & Auth Plugins</h3>
...
<h3 className="session-state">SessionState</h3>
...
<h3 className="access-definition">AccessDefinition</h3>
```protobuf
...
<h3 className="access-spec">AccessSpec</h3>
...
<h3 className="monitor">Monitor</h3>
...
To implement this you must first specify the address of the load balanced service using the `dns:///` (note: triple slash) [protocol](https://github.com/grpc/grpc/blob/master/doc/naming.md) in Tyk Gateway's [gRPC server address](/nightly/tyk-oss-gateway/configuration#coprocess_optionscoprocess_grpc_server) configuration (`TYK_GW_COPROCESSOPTIONS_COPROCESSGRPCSERVER`). Tyk will retrieve the list of addresses for each gRPC server from that service.

You can control whether Tyk will implement load balancing using the [gRPC round robin load balancing](/nightly/tyk-oss-gateway/configuration#coprocess_optionsgrpc_round_robin_load_balancing)) config  (`TYK_GW_COPROCESSOPTIONS_GRPCROUNDROBINLOADBALANCING`):
...
<h4 className="what-is-next">What's Next?</h4>
...
<h3 className="create-custom-auth-plugin-with-dotnet">Create Custom Authentication Plugin with .NET</h3>
...
<h5 className="create-dotnet-project">Create .NET Project</h5>
...
<h5 className="install-grpc-tools">Install the gRPC Tools</h5>
