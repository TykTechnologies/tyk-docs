...
- `Session`  - the [Tyk session object](/nightly/api-management/policies#session-object).
...
<Note>
Although `apply_policy_id` is still supported, it is now deprecated. `apply_policies` is now used to list your policy IDs as an array. This supports the **[Multiple Policy](/nightly/api-management/policies#partitioned-policy-functionality)** feature introduced in the  **v2.4 - 1.4** release.
</Note>
...
`session_lifetime`
UNIX timestamp that denotes when the key will automatically expire. Any·subsequent API request made using the key will be rejected. Overrides the global session lifetime. See [Key Expiry and Deletion](/nightly/api-management/policies#set-physical-key-expiry-and-deletion) for more information.
...
`hash`
Name of the [hashing algorithm](/nightly/api-management/policies#access-key-hashing) used to hash the password.
