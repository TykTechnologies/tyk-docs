...
canonical: "https://tyk.io/docs/api-management/plugins/rich-plugins"
---

import GrpcInclude from '/snippets/nightly/grpc-include.mdx';
...
*   [Python](/nightly/api-management/plugins/rich-plugins#overview)
*   [gRPC](/nightly/api-management/plugins/rich-plugins#overview-1)
*   [Lua](/nightly/api-management/plugins/rich-plugins#using-lua)
...
See [ID Extractor](/nightly/api-management/plugins/plugin-types#plugin-caching-mechanism) for more details.
...
- `Session`  - the client's [Session](/nightly/api-management/access-control/sessions-and-keys/understanding-sessions).
...
`response`
Contains information populated from the upstream HTTP response data, for response hooks. See [ResponseObject](/nightly/#responseobject) for more details. All the field contents can be modified.
...
The `ResponseObject` exists within an [object](/nightly/#coprocess-object) for response hooks. The fields are populated with the upstream HTTP response data. All the field contents can be modified.
...
`access_rights`
Defined as a `map<string, APIDefinition>` instance, that maps the session's API ID to an [AccessDefinition](/nightly/#access-definition). The AccessDefinition defines the [access rights](/nightly/api-management/access-control/sessions-and-keys/access-rights) for the API in terms of allowed: versions and URLs(endpoints). Each URL (endpoint) has a list of allowed methods. For further details consult the tutorials for how to create a [security policy](/nightly/api-management/gateway-config-managing-classic#secure-an-api) for Tyk Cloud, Tyk Self Managed and Tyk OSS platforms.
...
For further details see [BasicAuthData](/nightly/#basicauthdata).

`jwt_data`
Added to sessions where a Tyk key (embedding a shared secret) is used as the public key for signing the JWT. The JWT token's KID header value references the ID of a Tyk key. See [JWTData](/nightly/#jwtdata) for an example.

`hmac_enabled`
When set to `true` this indicates generation of a [HMAC signature](/nightly/basic-config-and-security/security/authentication-authorization/hmac-signatures) using the secret provided in `hmac_secret`. If the generated signature matches the signature provided in the *Authorization* header then authentication of the request has passed.
...
<Note>
Although `apply_policy_id` is still supported, it is now deprecated. `apply_policies` is now used to list your policy IDs as an array. See [here](/nightly/api-management/access-control/policies/applying-policies#policy-ids) for more details.
</Note>
...
`monitor`
Defines a [quota monitor](/nightly/api-management/gateway-events#monitoring-quota-consumption) containing a list of percentage threshold limits in descending order. These limits determine when webhook notifications are triggered for API users or an organization. Each threshold represents a percentage of the quota that, when reached, triggers a notification. See [Monitor](/nightly/#monitor) for further details and an example.
...
This is a UNIX timestamp that signifies when a cached key or ID will expire. This relates to custom authentication, where authenticated keys can be cached to save repeated requests to the gRPC server. See [id_extractor](/nightly/api-management/plugins/plugin-types#plugin-caching-mechanism) and [Auth Plugins](/nightly/api-management/plugins/plugin-types#authentication-plugins) for additional information.

`session_lifetime`
UNIX timestamp that denotes when the key will automatically expire. Any·subsequent API request made using the key will be rejected. Overrides the global session lifetime. See [Key Expiry and Deletion](/nightly/api-management/access-control/sessions-and-keys/session-lifecycle) for more information.
...
Defined as an attribute within a [SessionState](/nightly/#session-state) instance. Contains the allowed versions and URLs (endpoints) for the API that the session request relates to. Each URL (endpoint) specifies an associated list of allowed methods. See also [AccessSpec](/nightly/#access-spec).
...
`allowed_urls` List of [AccessSpec](/nightly/#access-spec) instances. Each instance defines a URL (endpoint) with an associated allowed list of methods. If all URLs (endpoints) are allowed then the attribute is not set.
...
The `BasicAuthData` contains a hashed password and the name of the hashing algorithm used. This is represented by the `basic_auth_data` attribute in [SessionState](/nightly/#session-state) message.
...
`hash`
Name of the [hashing algorithm](/nightly/api-management/access-control/sessions-and-keys/key-hashing#hashing-algorithms) used to hash the password.

---

### JWTData

Added to [sessions](/nightly/#session-state) where a Tyk key (embedding a shared secret) is used as the public key for signing the JWT. This message contains the shared secret.
...
Added to a [session](/nightly/#session-state) when [monitor quota thresholds](/nightly/api-management/gateway-events#monitoring-quota-consumption) are defined within the Tyk key. This message contains the quota percentage threshold limits, defined in descending order, that trigger webhook notification.
...
In order to integrate with Python custom plugins, the *libpython3.x.so* shared object library is used to embed a Python interpreter directly in the Tyk Gateway. Further details can be found [here](/nightly/api-management/plugins/rich-plugins#coprocess-gateway-api)
...
<br />
Whilst Python plugins are still supported by Tyk Gateway, if you want to use them you must extend the image to add support for Python. For further details, please refer to the [release notes](/nightly/developer-support/release-notes/gateway) for Tyk Gateway `v5.3.0`.
</Note>
...
The project implements a simple middleware for header injection, using a Pre hook (see [Tyk custom middleware hooks](/nightly/api-management/plugins/rich-plugins#coprocess-dispatcher-hooks). A single Python script contains the code for it, see [middleware.py](https://github.com/TykTechnologies/tyk-plugin-demo-python/blob/master/middleware.py).
...
* Tyk API Gateway: This can be installed using standard package management tools like Yum or APT, or from source code. See [here](/nightly/tyk-self-managed/install) for more installation options.
...
* The `custom_middleware` block contains the middleware settings like the plugin driver we want to use (`driver`) and the hooks that our plugin will expose. You use the `auth_check` for this tutorial. For other hooks see [here](/nightly/api-management/plugins/rich-plugins#coprocess-dispatcher-hooks).
...
You import decorators from the Tyk module as this gives you the `Hook` decorator, and you import [Tyk Python API helpers](/nightly/api-management/plugins/rich-plugins#tyk-python-api-methods)
...
A plugin bundle is a packaged version of the plugin, it may also contain a cryptographic signature of its contents. The `-y` flag tells the Tyk CLI tool to skip the signing process in order to simplify the flow of this tutorial. For more information on the Tyk CLI tool, see [here](/nightly/api-management/plugins/overview#plugin-bundles).
...
The [Protocol Buffers](https://github.com/TykTechnologies/tyk/tree/master/coprocess/proto ) and [bindings](https://github.com/TykTechnologies/tyk/tree/master/coprocess/bindings) provided by Tyk should be used in order for successful ommunication between Tyk Gateway and your gRPC plugin server. Documentation for the protobuf messages is available in the [Rich Plugins Data Structures](/nightly/api-management/plugins/rich-plugins#rich-plugins-data-structures) page.
...
To implement this you must first specify the address of the load balanced service using the `dns:///` (note: triple slash) [protocol](https://github.com/grpc/grpc/blob/master/doc/naming.md) in Tyk Gateway's [gRPC server address](/nightly/tyk-oss-gateway/configuration#coprocess_options-coprocess_grpc_server) configuration (`TYK_GW_COPROCESSOPTIONS_COPROCESSGRPCSERVER`). Tyk will retrieve the list of addresses for each gRPC server from that service.

You can control whether Tyk will implement load balancing using the [gRPC round robin load balancing](/nightly/tyk-oss-gateway/configuration#coprocess_options-grpc_round_robin_load_balancing)) config  (`TYK_GW_COPROCESSOPTIONS_GRPCROUNDROBINLOADBALANCING`):
...
If you are not load balancing, you can alternatively provide the `tcp://` address of the gRPC server in Tyk Gateway's [gRPC server address](/nightly/tyk-oss-gateway/configuration#coprocess_options-coprocess_grpc_server) configuration (`TYK_GW_COPROCESSOPTIONS_COPROCESSGRPCSERVER`) and set [gRPC round robin load balancing](/nightly/tyk-oss-gateway/configuration#coprocess_options-grpc_round_robin_load_balancing)  (`TYK_GW_COPROCESSOPTIONS_GRPCROUNDROBINLOADBALANCING`) to `false`.
...
Example tutorials are available that explain how to generate the protobuf bindings and implement a server for [Java](/nightly/api-management/plugins/rich-plugins#create-a-request-transformation-plugin-with-java), [.NET](/nightly/api-management/plugins/rich-plugins#create-custom-auth-plugin-with-dotnet) and [NodeJS](/nightly/api-management/plugins/rich-plugins#create-custom-auth-plugin-with-dotnet).
...
Tyk Gateway can be configured to download the gRPC plugin configuration for an API from a web server. For further details related to the concept of bundling plugins please refer to [plugin bundles](/nightly/api-management/plugins/overview#plugin-bundles).
...
- Consult the [Protocol Buffer messages](/nightly/api-management/plugins/rich-plugins#rich-plugins-data-structures) that Tyk Gateway uses when making a request to a gRPC server.
- Visit tutorial guides that explain how to implement a [Java](/nightly/api-management/plugins/rich-plugins#create-a-request-transformation-plugin-with-java), [.NET](/nightly/api-management/plugins/rich-plugins#create-custom-auth-plugin-with-dotnet) and [NodeJS](/nightly/api-management/plugins/rich-plugins#create-custom-auth-plugin-with-dotnet) gRPC server.
- Visit our [plugins hub](/nightly/api-management/plugins/overview#plugins-hub) to explore further gRPC development examples and resources.
...
We are going to setup a Python virtual environment and install some supporting dependencies. Assuming that you have Python [virtualenv](https://virtualenv.pypa.io/en/latest/) already installed, then issue the following commands to setup a Python virtual environment containing the grpcio and grpcio-tools libraries:
...
To quickly get started, a Tyk OAS API schema can be created by importing the infamous [pet store](https://petstore3.swagger.io/api/v3/openapi.json) OAS schema. Then the [findByStatus](https://petstore3.swagger.io/#/pet/findPetsByStatus) endpoint can be used for testing.
...
We need to download the Tyk Protocol Buffers definition files, these files contains the data structures used by Tyk. See [Data Structures](/nightly/api-management/plugins/rich-plugins#rich-plugins-data-structures) for more information:
...
- The `custom_middleware` block contains the middleware settings like the plugin driver we want to use (`driver`) and the hooks that our plugin will expose. We use the `pre` hook for this tutorial. For other hooks see [here](/nightly/api-management/plugins/rich-plugins#coprocess-dispatcher-hooks).
...
For more information on the Tyk CLI tool, see [here](/nightly/api-management/plugins/overview#plugin-bundles).
...
<a id="create-custom-auth-plugin-with-dotnet"></a>
...
Now that we can safely run `protoc`, we can download the Tyk Protocol Buffers definition files. These files contain the data structures used by Tyk. See [Data Structures](/nightly/api-management/plugins/rich-plugins#rich-plugins-data-structures) for more information:
...
- The `custom_middleware` block contains the middleware settings like the plugin driver we want to use (`driver`) and the hooks that our plugin will expose. We use the `auth_check` hook for this tutorial. For other hooks see [here](/nightly/api-management/plugins/rich-plugins#coprocess-dispatcher-hooks).
...
- Tyk Gateway: This can be installed using standard package management tools like Yum or APT, or from source code. See [here](/nightly/tyk-self-managed/install) for more installation options.
- The Tyk CLI utility, which is bundled with our RPM and DEB packages, and can be installed separately from [https://github.com/TykTechnologies/tyk-cli](https://github.com/TykTechnologies/tyk-cli)
...
While Tyk Gateway offers built-in support for HMAC-signed authentication, this tutorial serves as a practical guide for developers looking to extend Tyk's capabilities through custom authentication plugins. It extends the gRPC server that we developed in our [getting started guide](/nightly/api-management/plugins/rich-plugins#using-python).
...
The Tyk API definition above can be imported via Tyk Dashboard. Alternatively, if using Tyk Gateway OSS, a POST request can be made to the *api/apis* endpoint of Tyk Gateway. Consult the [Tyk Gateway Open API Specification documentation](/nightly/tyk-gateway-api) for usage.
...
You can use Tyk Gateway’s API to create the key by issuing a POST request to the *tyk/keys* endpoint. Consult the [Tyk Gateway Open API Specification documentation](/nightly/tyk-gateway-api) for usage.
...
We have a demo plugin hosted in the repo [tyk-plugin-demo-lua](https://github.com/TykTechnologies/tyk-plugin-demo-lua). The project implements a simple middleware for header injection, using a Pre hook (see [Tyk custom middleware hooks](/nightly/api-management/plugins/javascript#using-javascript-with-tyk)) and [mymiddleware.lua](https://github.com/TykTechnologies/tyk-plugin-demo-lua/blob/master/mymiddleware.lua).
#### Lua Performance
