...
keywords: "security, upstream authentication, gateway to upstream, oauth"
sidebarTitle: "OAuth 2.0"
canonical: "https://tyk.io/docs/api-management/upstream-authentication/oauth"
---
...
Tyk supports OAuth 2.0 as a method for authenticating **clients** with the **Gateway** - you can use Tyk's own auth server functionality via the [Tyk OAuth 2.0](/nightly/api-management/authentication/oauth-2) auth method or obtain the access token via a third party auth server and use the [JWT Auth](/nightly/basic-config-and-security/security/authentication-authorization/json-web-tokens) method.
...
To enhance security by restricting visibility of the credentials, these can be stored in a [key-value store](/nightly/tyk-self-managed/install), with only references included in the API definition.

Some auth servers will return *additional metadata* with the access token (for example, the URL of the upstream server that should be addressed using the token if this can vary per client). Tyk can accommodate this using the optional `extraMetadata` field in the API definition. The response from the auth server will be parsed for any fields defined in `extraMetadata`; any matches will be saved to the request context where they can be accessed from other middleware (for our example, the URL rewrite middleware could be used to [transform the upstream target URL](/nightly/transform-traffic/url-rewriting)).

#### Authentication Method

When Tyk requests a token from the auth server, it must authenticate itself using its client ID and client secret. The OAuth standard allows these credentials to be sent either as HTTP Basic Authentication in the request header, or as form fields in the request body; different auth servers accept different methods.

By default, Tyk tries the header first and, if the auth server rejects it, retries with the credentials in the body - this is standards-compliant, but for an auth server that only accepts body credentials, it means every token request needlessly fails once before succeeding, producing avoidable `400 Bad Request` errors in your logs and monitoring.

If you know which method your auth server expects, set the optional `method` field to `client_secret_basic` (header only) or `client_secret_post` (body only) so Tyk sends a single, correct request. Leave `method` unset to keep the default automatic behavior.
...
| Field | Description |
|---|---|
| `enabled` | Set to `true` to enable upstream OAuth authentication. |
| `allowedAuthorizeTypes` | Should include the value `clientCredentials`. |
| `clientCredentials.tokenUrl` | The URL of the `/token` endpoint on the *auth server*. |
| `clientCredentials.clientId` | The client ID to be provided to the *auth server*. |
| `clientCredentials.clientSecret` | The client secret to be provided to the *auth server*. |
| `clientCredentials.method` | Optional. Controls how the client ID and secret are sent to the *auth server*'s token endpoint: `client_secret_basic` sends them in the request header, `client_secret_post` sends them in the request body. Leave unset for Tyk's automatic detection (tries the header first, then the body). |
| `clientCredentials.scopes` | Optional array of authorization scopes to be requested. |
| `clientCredentials.extraMetadata` | Optional array of additional fields to be extracted from the *auth server* response. |
| `clientCredentials.header.enabled` | Set to `true` if your upstream expects the credentials to be in a custom header. Otherwise, omit to use the `Authorization` header. |
| `clientCredentials.header.name` | The custom header to use, if `header.enabled` is `true`. |

Note that if you use the [Tyk API Designer](/nightly/api-management/upstream-authentication/basic-auth#configuring-upstream-basic-auth-using-the-api-designer) in Tyk Dashboard it will always configure the `header` parameter - even if you are using the default `Authorization` value.

For example:

```json {hl_lines=["43-63"],linenos=true, linenostart=1}
{
    "info": {
...
Use the **Client authentication method** drop-down to control how the client ID and secret are sent to the auth server: **Header then body** (the default) preserves Tyk's automatic detection; **Header** sends them as HTTP Basic Authentication only; **Body** sends them as form fields only. Leave the default selected unless you know which method your auth server requires.
...
"oauth": {
    "enabled": true,
    "allowedAuthorizeTypes": ["password"],
    "password": {
        "tokenUrl": "http://<my-auth-server>/token",
        "clientId": "client123",
        "clientSecret": "secret123",
...
| `password.tokenUrl` | The URL of the `/token` endpoint on the *auth server*. |
| `password.clientId` | The client ID to be provided to the *auth server*. |
| `password.clientSecret` | The client secret to be provided to the *auth server*. |
| `password.method` | Optional. Controls how the client ID and secret are sent to the *auth server*'s token endpoint: `client_secret_basic` sends them in the request header, `client_secret_post` sends them in the request body. Leave unset for Tyk's automatic detection (tries the header first, then the body). |
| `password.username` | The Resource Owner username to be provided to the *auth server*. |
| `password.password` | The Resource Owner password to be provided to the *auth server*. |
| `password.scopes` | Optional array of authorization scopes to be requested. |
| `password.extraMetadata` | Optional array of additional fields to be extracted from the *auth server* response. |
| `password.header.enabled` | Set to `true` if your upstream expects the credentials to be in a custom header. Otherwise, omit to use the `Authorization` header. |
| `password.header.name` | The custom header to use, if `header.enabled` is `true`. |
...
In this example upstream authentication has been enabled (line 44). The authentication method to be used is indicated in lines 46 (OAuth) and 48 (password grant). When a request is made to the API, Tyk will request an access token from the *authorization server* at `http://<my-auth-server>` providing client credentials, resource owner credentials and the scope `scope1`. The `method` field is left unset, so Tyk uses its default automatic detection: it tries the header first and falls back to the body if the auth server rejects it.
...
<Note>
Any error encountered in the communication with the OAuth server will generate an `UpstreamOAuthError` event. This event can be used to trigger an event handler, for example you could use a [webhook](/nightly/api-management/gateway-events) to alert the system administrator of the issue.
</Note>
