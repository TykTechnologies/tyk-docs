...
This page explains how to promote APIs, keys and policies from one environment to another. If you instead want to segment a single Tyk cluster into zones, so that specific Gateways selectively load specific APIs, see [Gateway and API Sharding](/nightly/api-management/api-sharding).
...
See [API Tagging](/nightly/api-management/api-sharding#api-tagging-with-on-premises) for more details.

#### API Sharding

You can also use [API Sharding](/nightly/api-management/api-sharding#what-is-api-sharding-) to move APIs in a Shards (and or MDCB) Tyk Self-Managed installation.
...
To create a key with a custom identifier, ie Token, simply use the [Gateway (OSS)](/nightly/tyk-gateway-api) or [Dashboard (Pro)](https://tyk.io/docs/api-reference/keys/create-custom-key) REST APIs to import a custom key.
