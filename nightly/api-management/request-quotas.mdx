...
The Tyk Dashboard provides a straightforward interface to set request quota parameters on [Security Policies](/nightly/api-management/policies) and [Access Keys](/nightly/api-management/policies#access-keys).
...
*   **Policy Precedence:** Quotas set on a [Security Policy](/nightly/api-management/policies) apply to all keys using that policy *unless* overridden by a specific quota set directly on the key (using the "Set per API Limits and Quota" option).
