...
keywords: GraphQL, Federation, Entities, Grapqhl Proxy, Validation, Schema, Complexity
  Limiting, Persisted Queries, Migration Guide, GraphQL Playground, GQL Headers
...
First, you need to learn [how to create a security policy with Tyk API](/nightly/api-management/gateway-config-managing-classic#secure-an-api) or [how to create an API Key with Tyk API](/nightly/api-management/policies#access-key-level-security).
...
Before this feature can be used, WebSockets need to be enabled in the Tyk Gateway configuration. To enable it set [http_server_options.enable_websockets](/nightly/tyk-oss-gateway/configuration#http_server_optionsenable_websockets) to `true` in your `tyk.conf` file.
