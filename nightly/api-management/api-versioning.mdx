...
Tyk's access control model supports very granular permissions to versioned APIs using the `access_rights` assigned in the [access keys/tokens](/nightly/api-management/policies#what-is-a-session-object) or [security policies](/nightly/api-management/policies#what-is-a-security-policy) that are applied to keys.
