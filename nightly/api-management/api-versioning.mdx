...
description: "Learn how to create and manage multiple versions of an API in Tyk"
keywords: "API versioning, version, Tyk Classic, Tyk OAS, API, versioning"
sidebarTitle: "API Versioning"
canonical: "https://tyk.io/docs/api-management/api-versioning"
---
...
For more details, see [this comparison](/nightly/#comparison-between-tyk-oas-and-tyk-classic-api-versioning).

Some key concepts that are important to understand when versioning your APIs are:

- [Version identifiers](/nightly/api-management/api-versioning#version-identifiers)
- [Default version](/nightly/api-management/api-versioning#default-version)
- [Base and child versions](/nightly/api-management/api-versioning#base-and-child-apis)
- [Controlling access to versions](/nightly/api-management/api-versioning#controlling-access-to-versioned-apis)

### Version Identifiers

The version identifier is the method by which the API client specifies which version of an API it is addressing with each request. Tyk supports multiple [locations](/nightly/api-management/api-versioning#version-identifier-location) within the request where this identifier can be placed. Typically the value assigned in the version identifier will be matched to the list of versions defined for the API and, assuming that the client is [authorized to access](/nightly/api-management/api-versioning#controlling-access-to-versioned-apis) that version, Tyk will apply the version specific processing to the request.
...
- [Request URL (path)](/nightly/api-management/api-versioning#request-url-path)
- [Query parameter](/nightly/api-management/api-versioning#query-parameter)
- [Request header](/nightly/api-management/api-versioning#request-header)
...
The upstream (target) URL will be constructed by combining the configured `upstream.url` (`target_url` for Tyk Classic APIs) with the full request path unless configured otherwise (for example, by using the [strip listen path](/nightly/api-management/gateway-config-tyk-oas#listenpath) feature).
...
When using the [Request URL](/nightly/api-management/api-versioning#request-url-path) for the versioning identifier, if Tyk is configured to strip the versioning identifier then the first URL fragment after the `listenPath` (`listen_path` for Tyk Classic APIs) will be deleted prior to creating the proxy URL. If the request does not include a versioning identifier and Tyk is configured to [fallback to default](/nightly/api-management/api-versioning#fallback-to-default), this may lead to undesired behaviour as the first URL fragment of the endpoint will be deleted.
...
Note that any version (*child* or *base*) can be set as the [default version](/nightly/api-management/api-versioning#default-version).
...
- the [Dashboard API](/nightly/api-management/api-versioning#changing-the-base-version)
- the Dashboard application's [API Designer](/nightly/api-management/api-versioning#change-the-base-api)

### Controlling access to versioned APIs

Tyk's access control model supports very granular permissions to versioned APIs using the [`access_rights`](/nightly/api-management/access-control/sessions-and-keys/access-rights) assigned when a Key is authenticated.

This means that you could restrict client access to only the [base API](/nightly/api-management/api-versioning#base-and-child-apis), while allowing developers to create and test new versions independently. These will only be added to the "routing table" in the base API when the API owner is ready and access keys could then be updated to grant access to the new version(s).

Note that an access key will only have access to the `default` version if it explicitly has access to that version (e.g. if `v2` is set as default, a key must have access to `v2` to be able to [fallback to the default](/nightly/api-management/api-versioning#fallback-to-default) if the versioning identifier is not correctly provided in the request.

When using Tyk OAS APIs each version of the API has a unique [API Id](/nightly/api-management/gateway-config-tyk-oas#info), so you simply need to identify the specific versions in the `access_rights` list in the key in the same way that you would add multiple different APIs to a single key.
...
These are largely due to the fact that a separate API definition is generated for each version of a Tyk OAS API, with one designated as the [base](/nightly/api-management/api-versioning#base-and-child-apis) version (which should be exposed on Tyk Gateway with the other (child) versions set to [internal](/nightly/advanced-configuration/transform-traffic/looping#internal-only-apis) visibility) whereas all versions of a Tyk Classic API are described by a single API definition.
...
You can configure a Tyk OAS API as a [base API](/nightly/api-management/api-versioning#base-and-child-apis) by adding the `info.versioning` [object](/nightly/api-management/gateway-config-tyk-oas#versioning) to the [Tyk Vendor Extension](/nightly/api-management/gateway-config-tyk-oas#tyk-vendor-extension).
...
<Note>
  **Note**
  ...
  If you are using Tyk Classic APIs, please see [this section](/nightly/api-management/api-versioning#versioning-with-tyk-classic-apis).
</Note>
...
<Note>
  **Note**
  ...
  If you are using Tyk Classic APIs, please see [this section](/nightly/api-management/api-versioning#tyk-classic-api-versioning-in-the-api-designer).
</Note>

### Configure versioning

From Tyk 5.10, you can pre-configure the [versioning metadata](/nightly/#how-api-versioning-works-with-tyk) for an API before you've created the first [child API](/nightly/api-management/api-versioning#base-and-child-apis).
...
2. Select **Edit** and you can pre-fill the following metadata:
    ...
    - [Version identifier location](/nightly/api-management/api-versioning#version-identifier-location)
    - Version identifier name (or [pattern](/nightly/api-management/api-versioning#version-identifier-pattern) for URL versioning)
    - Version name for the base API
    - [Stripping identifier from upstream request](/nightly/api-management/api-versioning#stripping-version-identifier)
    - [Default fallback behaviour](/nightly/api-management/api-versioning#fallback-to-default)
...
3. If you have not already [configured](/nightly/api-management/api-versioning#configure-versioning) the versioning for this API you will prompted to complete this information.
...
If you are not using the Dashboard's [API Designer](/nightly/api-management/api-versioning#api-versioning-in-the-tyk-dashboard-api-designer), you can use the Tyk Dashboard API to manage your APIs, including creating and linking base and child versions and, from Tyk 5.12.0, promoting a child API to become the base version.

If you are not using Tyk Dashboard, you can use the Tyk Gateway API to create base and child APIs.

You can either:
- manually [configure the version metadata](/nightly/api-management/api-versioning#configuring-api-versioning-in-the-api-definition) using the `info.versioning` section of the Tyk Vendor Extension before [importing the API definition](/nightly/api-management/gateway-config-managing-oas#loading-the-api-definition-into-tyk) via the Dashboard or Gateway API
...
| `base_api_version_name` | The version name (e.g. `v1`) that should be used to call the *base API*. If this has been [pre-configured](/nightly/api-management/api-versioning#creating-base-version) or this is not the first child API then it can be omitted. If provided, this will overwrite any existing base API version name. |
| `new_version_name`      | The version name (e.g. `v2`) that will be used to route to this new child API. |
| `set_default`           | Set this to `true` to make the new child API the [default version](/nightly/api-management/api-versioning#default-version). |

These options are also available when [updating an existing API definition](/nightly/api-management/gateway-config-managing-oas#updating-an-api) using the `PATCH /api/apis/oas` or `PATCH /tyk/apis/oas` endpoints.

#### Configuring the Versioning Identifier

When using the control API to create the first new child version, if you have not already [pre-configured](/nightly/api-management/api-versioning#creating-base-version) the versioning identifier in the base API, the folowing settings will be used:

- versioning identifier location: header
- versioning identifier key: `x-tyk-version`

If you need to change these, you should do so within the `info.versioning` section of the API definition for the base API as explained [previously](/nightly/api-management/api-versioning#configuring-api-versioning-in-the-api-definition).
...
- if [API ownership](/nightly/api-management/user-management#api-ownership) is in use, the user calling this endpoint must have *write* access to both APIs.
...
We also added a new optional `purge` parameter to the [delete API](https://tyk.io/docs/api-reference/oas-apis/delete-oas-api) endpoint in the Tyk Dashboard API which will delete all child APIs linked to the API being deleted, allowing you to remove all versions of an API with one command.


## Versioning with Tyk Classic APIs

All configuration for versioning of Tyk Classic APIs is documented [here](/nightly/api-management/gateway-config-tyk-classic#tyk-classic-api-versioning).

For details on how to create a Tyk Classic API, check out [this guide](/nightly/api-management/gateway-config-managing-classic).
...
4. **Set the default version**
    ...
    You can choose any of your API versions to act as the [default](/nightly/api-management/api-versioning#default-version).
