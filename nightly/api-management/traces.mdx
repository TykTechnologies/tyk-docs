...
sidebarTitle: "Traces"
canonical: "https://tyk.io/docs/api-management/traces"
---
...
<img src="/img/observability/tyk-otel-tracing.png" alt="Client request flowing through Tyk Gateway, which exports traces via OTLP to an OpenTelemetry Collector, which forwards to Jaeger, Datadog, Dynatrace, or any OTLP-compatible backend" />

Tyk also supports the legacy [OpenTracing](/nightly/#opentracing-deprecated) approach (now deprecated). Migrate to OpenTelemetry for vendor-neutral, actively maintained tracing.
...
<Note>
The root-level `opentelemetry.enabled`, `opentelemetry.exporter`, and `opentelemetry.endpoint` fields are deprecated from Tyk 5.13.0. 

They continue to work for backward compatibility but should be replaced with the `opentelemetry.traces.*` equivalents in new deployments.
</Note>

| Field | Description | Default |
|-------|-------------|---------|
| [opentelemetry.traces.enabled](/nightly/tyk-oss-gateway/configuration#opentelemetry-traces-enabled) | Enable distributed tracing | `false` |
| [opentelemetry.traces.exporter](/nightly/tyk-oss-gateway/configuration#opentelemetry-traces-exporter) | Export protocol: `grpc` or `http` | `grpc` |
| [opentelemetry.traces.endpoint](/nightly/tyk-oss-gateway/configuration#opentelemetry-traces-endpoint) | OTLP collector endpoint | `localhost:4317` |
| [opentelemetry.traces.connection_timeout](/nightly/tyk-oss-gateway/configuration#opentelemetry-traces-connection_timeout) | Connection timeout in seconds | `1` |
| [opentelemetry.traces.headers](/nightly/tyk-oss-gateway/configuration#opentelemetry-traces-headers) | Additional HTTP headers sent with each OTLP export request | — |
| [opentelemetry.traces.tls](/nightly/tyk-oss-gateway/configuration#opentelemetry-traces-tls) | TLS configuration for the OTLP connection | — |
| [opentelemetry.traces.context_propagation](/nightly/tyk-oss-gateway/configuration#opentelemetry-traces-context_propagation) | Trace context format: `tracecontext`, `b3`, `custom`, or `composite` | `tracecontext` |
| [opentelemetry.traces.custom_trace_header](/nightly/tyk-oss-gateway/configuration#opentelemetry-traces-custom_trace_header) | Custom propagation header name (used with `custom` or `composite` modes) | — |
| [opentelemetry.traces.span_processor_type](/nightly/tyk-oss-gateway/configuration#opentelemetry-traces-span_processor_type) | Span processing mode: `batch` or `simple` | `batch` |
| [opentelemetry.traces.span_batch_config.max_queue_size](/nightly/tyk-oss-gateway/configuration#opentelemetry-traces-span_batch_config-max_queue_size) | Maximum number of spans buffered before export | `2048` |
| [opentelemetry.traces.span_batch_config.max_export_batch_size](/nightly/tyk-oss-gateway/configuration#opentelemetry-traces-span_batch_config-max_export_batch_size) | Maximum number of spans per export batch | `512` |
| [opentelemetry.traces.span_batch_config.batch_timeout](/nightly/tyk-oss-gateway/configuration#opentelemetry-traces-span_batch_config-batch_timeout) | Seconds to wait before forcing an export | `5` |
| [opentelemetry.traces.sampling.type](/nightly/tyk-oss-gateway/configuration#opentelemetry-traces-sampling-type) | Sampling strategy: `AlwaysOn`, `AlwaysOff`, or `TraceIDRatioBased` | `AlwaysOn` |
| [opentelemetry.traces.sampling.rate](/nightly/tyk-oss-gateway/configuration#opentelemetry-traces-sampling-rate) | Fraction of traces to sample when using `TraceIDRatioBased` (0.0–1.0) | `0.5` |
| [opentelemetry.traces.sampling.parent_based](/nightly/tyk-oss-gateway/configuration#opentelemetry-traces-sampling-parent_based) | Inherit the parent span's sampling decision | `false` |
...
Set the [server.detailedTracing](/nightly/api-management/gateway-config-tyk-oas#detailedtracing) flag in the Tyk OAS API definition, or toggle **OpenTelemetry Tracing** in the Tyk OAS API Designer.
...
For Tyk Classic APIs, use the [detailed_tracing](/nightly/api-management/gateway-config-tyk-classic#opentelemetry) field in the API definition.
...
|-------|---------|-------------|
| [`max_queue_size](/nightly/tyk-oss-gateway/configuration#opentelemetry-span_batch_config-max_queue_size) | `2048` | maximum number of spans buffered before export |
| [`max_export_batch_size](/nightly/tyk-oss-gateway/configuration#opentelemetry-span_batch_config-max_export_batch_size) | `512` | maximum number of spans per export batch |
| [`batch_timeout](/nightly/tyk-oss-gateway/configuration#opentelemetry-span_batch_config-batch_timeout) | `5` | seconds to wait before forcing an export |
...
[opentelemetry.traces.context_propagation](/nightly/tyk-oss-gateway/configuration#opentelemetry-traces-context_propagation) controls which trace context format the gateway reads from incoming requests and writes to upstream requests.
...
```
  ...
  <Tab title="Environment Variable">
     Set the environment variable [TYK_GW_OPENTELEMETRY_TRACES_CONTEXTPROPAGATION](/nightly/tyk-oss-gateway/configuration#opentelemetry-context_propagation).
  </Tab>
</Tabs>
...
<Note>
`custom` and `composite` are available from Tyk Gateway v5.12.0 and require [opentelemetry.custom_trace_header](/nightly/tyk-oss-gateway/configuration#opentelemetry) to be set.
</Note>
...
Tyk supports configuring the following sampling strategies using [opentelemetry.sampling](/nightly/tyk-oss-gateway/configuration#opentelemetry) in `tyk.conf`.
...
Set the [opentelemetry.traces.sampling.type](/nightly/tyk-oss-gateway/configuration#opentelemetry-traces-sampling-type) field in the Tyk Gateway configuration file or use the equivalent environment variable. Allowed values for this setting are:

| Value | Behavior |
|-------|-----------|
| `AlwaysOn` (default) | All traces are sampled |
| `AlwaysOff` | No traces are sampled |
| `TraceIDRatioBased` | Samples a fraction of traces based on the configured [sampling rate](/nightly/api-management/traces#sampling-rate) |

#### Sampling Rate

The [opentelemetry.traces.sampling.rate](/nightly/tyk-oss-gateway/configuration#opentelemetry-traces-sampling-rate) field is used to control what fraction of total traces will be sampled when the `TraceIDRatioBased` sampling is configured. It accepts a value between 0.0 and 1.0. For example, a `rate` set to 0.5 implies that approximately 50% of the traces will be sampled. The default value is 0.5.
...
Enable parent based sampling by setting [opentelemetry.traces.sampling.parent_based](/nightly/tyk-oss-gateway/configuration#opentelemetry-traces-sampling-parent_based) or the equivalent environment variable. The default value is `false`.

## Tracing Backends

For step-by-step setup guides connecting Tyk Gateway traces to a specific backend:

- [Datadog](/nightly/api-management/traces/datadog)
- [Dynatrace](/nightly/api-management/traces/dynatrace)
- [Elasticsearch](/nightly/api-management/traces/elasticsearch)
- [New Relic](/nightly/api-management/traces/new-relic)
- [Jaeger](/nightly/api-management/traces/jaeger)

All configuration options are documented in the [Tyk Gateway configuration reference](/nightly/tyk-oss-gateway/configuration#opentelemetry).
...
The CNCF has archived the OpenTracing project. Tyk introduced [OpenTelemetry](/nightly/#opentelemetry) support in v5.2.
...
<Tabs>
  <Tab title="Jaeger">
    <Note>
    Tyk's OpenTelemetry tracing works with Jaeger. Follow the [Jaeger guide](/nightly/api-management/traces/jaeger) instead.
    </Note>
  ...
  <Tab title="New Relic">
    <Note>
    Tyk's OpenTelemetry tracing works with New Relic. Follow the [New Relic guide](/nightly/api-management/traces/new-relic) instead.
    </Note>
