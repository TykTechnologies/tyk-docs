...
canonical: "https://tyk.io/docs/api-management/observability-getting-started"
---
...
- Structured JSON access logs from Tyk Gateway shipping to [Loki](https://grafana.com/oss/loki/)
- Distributed traces exporting to [Tempo](https://grafana.com/oss/tempo/)
- Gateway metrics (request rate, latency, error rate) flowing to [Prometheus](https://grafana.com/oss/prometheus/)
...
- A running Tyk deployment. We recommend following the [Tyk getting started guide](https://tyk.io/docs/getting-started/quick-start#getting-started-with-tyk-self-managed) to set up a local Docker environment
...
<Note>
`sampling.rate: 1.0` captures every request, which is suitable for getting started. In production, lower this to `0.1` (10%) or use `ParentBased` sampling. See the [Tyk Gateway configuration reference](/nightly/tyk-oss-gateway/configuration#opentelemetry-sampling) for all options.
</Note>
...
- **Explore default metrics**: see all Gateway metrics available out of the box in [Default Metrics](/nightly/api-management/logs-metrics)
- **Add custom metrics**: attach request headers, JWT claims, and response codes as metric dimensions using [Custom Metrics](/nightly/api-management/metrics/custom-metrics)
- **Kubernetes deployment**: for log collection in Kubernetes using the OTel Collector DaemonSet, see [Collecting Gateway Logs with OTel on Kubernetes](/nightly/api-management/collecting-gateway-logs-otel-kubernetes)
