...
<Note>
`sampling.rate: 1.0` captures every request, which is suitable for getting started. In production, lower this to `0.1` (10%) or use `ParentBased` sampling. See the [Tyk Gateway configuration reference](/nightly/tyk-oss-gateway/configuration#opentelemetrysampling) for all options.
</Note>
