...
As explained in the dedicated [Data Plane Pump](/nightly/api-management/dashboard-analytics/data-plane-pump) section, once data reaches Tyk MDCB, it must be transferred to the control plane's persistent database via one of two options: Tyk MDCB's own built-in writer or a [Control Plane Pump](#control-plane-pumps): an ordinary Tyk Pump instance running one of the pump types listed above.
