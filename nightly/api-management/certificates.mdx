...
- **Gateway**: [`TYK_GW_SECURITY_PRIVATECERTIFICATEENCODINGSECRET`](/nightly/tyk-oss-gateway/configuration#security-private-certificate-encoding-secret)
- **Dashboard**: [`TYK_DB_SECURITY_PRIVATECERTIFICATEENCODINGSECRET`](/nightly/tyk-dashboard/configuration#security-private-certificate-encoding-secret)
...
The behavior of this Certificate Expiry Monitor is configured at the Gateway level using the following fields in the `security.certificate_expiry_monitor` section of the [Gateway configuration](/nightly/tyk-oss-gateway/configuration#security-certificate-expiry-monitor) (or equivalent environment variables):

| Field | Effect |
|-------|--------|
| [`warning_threshold_days`](/nightly/tyk-oss-gateway/configuration#security-certificate-expiry-monitor-warning-threshold-days) | The number of days before certificate expiry that the Gateway will start generating CertificateExpiringSoon events when the certificate is used |
| [`check_cooldown_seconds`](/nightly/tyk-oss-gateway/configuration#security-certificate-expiry-monitor-check-cooldown-seconds) | A cool-off period after checking a certificate's expiry before another check is made, to avoid unnecessary log generation when an API is called repeatedly |
| [`event_cooldown_seconds`](/nightly/tyk-oss-gateway/configuration#security-certificate-expiry-monitor-event-cooldown-seconds) | A cool-off period after generating `CertificateExpired` or `CertificateExpiringSoon` for a certificate, to avoid unnecessary event generation when an API is called repeatedly |
...
The Tyk Dashboard API exposes [endpoints](https://tyk.io/docs/api-reference/certificates/list-certificates) to manage the Tyk Certificate Store allowing you to:
