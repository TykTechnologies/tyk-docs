...
An input is a source of data piped through an array of optional [processors](/nightly/api-management/stream-config#overview-3):
...
Only one input is configured at the root of a Tyk Streams config. However, the root input can be a [broker](/nightly/api-management/stream-config#broker) which combines multiple inputs and merges the streams:
...
It's possible to configure a [batch policy](/nightly/api-management/stream-config#batch-policy) with a broker using the `batching` fields. When doing this the feeds from all child inputs are combined. Some inputs do not support broker based batching and specify this in their documentation.
...
Allows you to configure a [batching policy](/nightly/api-management/stream-config#batch-policy).
...
{/* TODO add link to service wide HTTP server If the `address` config field is left blank the [service-wide HTTP server](/nightly/docs/components/http/about) will be used. */}

{/* TODO add rate limit The field `rate_limit` allows you to specify an optional [`rate_limit` resource](/nightly/docs/components/rate_limits/about), which will be applied to each HTTP request made and each websocket payload received.
...
An optional [rate limit](/nightly/docs/components/rate_limits/about) to throttle requests by. */}
...
The Kafka input allows parallel processing of messages from different topic partitions, and messages of the same topic partition are processed with a maximum parallelism determined by the field [checkpoint_limit](/nightly/#checkpoint_limit).

In order to enforce ordered processing of partition messages set the [checkpoint_limit](/nightly/#checkpoint_limit) to `1` and this will force partitions to be processed in lock-step, where a message will only be processed once the prior message is delivered.

Batching messages before processing can be enabled using the [batching](/nightly/#batching) field, and this batching is performed per-partition such that messages of a batch will always originate from the same partition. This batching mechanism is capable of creating batches of greater size than the [checkpoint_limit](/nightly/#checkpoint_limit), in which case the next batch will only be created upon delivery of the current one.
...
Unfortunately this error message will appear for a wide range of connection problems even when the broker endpoint can be reached. Double check your authentication configuration and also ensure that you have [enabled TLS](/nightly/#tlsenabled) if applicable.
...
This field contains sensitive information that usually shouldn't be added to a config directly, read our [secrets page for more info](/nightly/docs/configuration/secrets).
::: */}
...
| `PLAIN` | Plain text authentication. NOTE: When using plain text auth it is extremely likely that you'll also need to [enable TLS](/nightly/#tlsenabled). |
...
Instead of using a static `access_token` allows you to query a [`cache`](/nightly/docs/components/caches/about) resource to fetch OAUTHBEARER tokens from */}
...
An output is a sink where we wish to send our consumed data after applying an optional array of [processors](/nightly/api-management/stream-config#overview-3). Only one output is configured at the root of a Tyk Streams config. However, the output can be a [broker](/nightly/api-management/stream-config#broker-1) which combines multiple outputs under a chosen brokering pattern.
...
Allows you to route messages to multiple child outputs using a range of brokering [patterns](/nightly/#patterns).
...
The body of the HTTP request is the raw contents of the message payload. If the message has multiple parts (is a batch) the request will be sent according to [RFC1341](https://www.w3.org/Protocols/rfc1341/7_2_Multipart.html). This behavior can be disabled by setting the field [batch_as_multipart](/nightly/#batch_as_multipart) to `false`.
...
This output benefits from sending messages as a [batch](/nightly/api-management/stream-config#batching-6) for improved performance. Batches can be formed at both the input and output level.
...
Allows you to configure a [batching policy](/nightly/api-management/stream-config#batching-6).
...
{/* TODO add link here  If the `address` config field is left blank the [service-wide HTTP server](/nightly/docs/components/http/about) will be used. */}
...
{/* TODO add link here - This behavior can be overridden by [archiving your batches](/nightly/docs/configuration/batching#post-batch-processing). */}
...
{/* Add links to bloblang queries : Both the `key` and `topic` fields can be dynamically set using function interpolations described [here](/nightly/docs/configuration/interpolation#bloblang-queries). */}

Metadata will be added to each message sent as headers (version 0.11+), but can be restricted using the field [metadata](/nightly/#metadata).
...
The `threads` field in the pipeline section determines how many parallel processing threads are created. You can read more about parallel processing in the [pipeline guide](/nightly/api-management/stream-config#processing-pipelines).
...
The [operator](/nightly/#operators) to execute
...
However, a small number of inputs such as [kafka](/nightly/api-management/stream-config#kafka) must be consumed sequentially (in this case by partition) and therefore benefit from specifying your batch policy at the input level instead:
...
Sometimes you may prefer to create your batches before processing, in which case if your input doesn't already support [a batch policy](/nightly/#batch-policy) you can instead use a [broker](/nightly/api-management/stream-config#broker), which also allows you to combine inputs with a single batch policy:
...
This also works the same with [output brokers](/nightly/api-management/stream-config#broker-1).
