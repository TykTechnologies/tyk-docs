...
7. Tyk can automatically configure the request processing middleware chain based upon configuration defined by the OpenAPI Specification. If your OpenAPI desription contains the relevant data then select the characteristics you would like to configure.
    ...
    | [Mock response](/nightly/api-management/traffic-transformation/mock-response#tyk-oas) | Endpoints with `examples` or `schema` |
...
| `mockResponse`    | Configure [mock response](/nightly/api-management/traffic-transformation/mock-response#tyk-oas) for all endpoints with `examples` or `schema` defined | Mock response not configured |
