...
There are three methods by which API definitions can be deployed to Tyk: using the [Tyk Dashboard API Designer](/nightly/api-management/dashboard-configuration), using the [Tyk Dashboard API](/nightly/tyk-dashboard-api) and using the [Tyk Gateway API](/nightly/tyk-gateway-api).
...
| Apply API [Templates](/nightly/platform-management/api-templates)  | ✅ | ✅ | ❌ |
| Export the OpenAPI description                       | ✅ | ✅ | ✅ |
| Export the Tyk OAS API definition                    | ✅ | ✅ | ✅ |
| Update API with new OpenAPI description              | ✅ | ✅ | ✅ |
| Manage API versions                                  | ✅ | ✅ | ✅ | 
| Assign APIs to [Categories](/nightly/platform-management/api-categories) | ✅ | ✅ | ❌ |
| Assign API [Owners](/nightly/platform-management/api-ownership) | ✅ | ✅ | ❌ |
...
3. Now complete the basic configuration for your new API following the guided steps providing:
    ...
    - API type (**HTTP**)
    - API style (**OpenAPI**)
    - [API template](/nightly/platform-management/api-templates#working-with-api-templates-using-the-template-designer) (optional)
    - Upstream URL
...
2. Now select **Add New API** and then, choose **Import**.
    ...
    Note that you can optionally apply an [API template](/nightly/platform-management/api-templates) by choosing **Start from template** as explained [here](/nightly/platform-management/api-templates#using-a-template-when-creating-a-new-api), however in this explanation we will not be applying a template.
...
You can use the optional `templateId` parameter to apply an [API Template](/nightly/platform-management/api-templates#applying-a-template-when-creating-an-api-from-a-tyk-oas-api-definition) to your API definition when creating the API.
...
5. You can optionally apply an [API template](/nightly/platform-management/api-templates) from the drop-down.
...
| `templateId`    | Apply the selected [API template](/nightly/platform-management/api-templates#applying-a-template-when-creating-an-api-from-a-tyk-oas-api-definition) when creating the API | No template is applied |
