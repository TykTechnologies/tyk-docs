---
title: "gRPC Proxy"
description: "Learn how to proxy gRPC with Tyk"
keywords: "gRPC, Other Protocol"
sidebarTitle: "gRPC Proxy"
canonical: "https://tyk.io/docs/key-concepts/grpc-proxy"
---
...
- You can target specific methods of the gRPC service using the format: `/{Service-Name}/{method name}`, for example: `/google.pubsub.v2.PublisherService/CreateTopic`. You can use this feature to apply standard ACL rules via Keys and Policies, or use URL rewrite middleware to [transform the upstream path](/nightly/transform-traffic/url-rewriting). 
...
You can also perform [gRPC load balancing](/nightly/#grpc-load-balancing).
...
Tyk supports Mutual Authentication in gRPC. See [Mutual TLS](/nightly/basic-config-and-security/security/mutual-tls/client-mtls#why-use-mutual-tls) to configure Mutual Authentication in Tyk. 

## Basic Authentication
Tyk supports Basic Authentication in gRPC. See [Basic Authentication](/nightly/api-management/authentication/basic-authentication) to configure Basic Authentication in Tyk. 
...
Tyk supports Token Based Authentication in gRPC. See [Auth Tokens](/nightly/api-management/authentication/bearer-token) to configure Token Based Authentication in Tyk. 
...
Tyk is able to perform load balancing on gRPC traffic using an approach similar to our native [Load Balancing](/nightly/planning-for-production/ensure-high-availability/load-balancing) functionality.
