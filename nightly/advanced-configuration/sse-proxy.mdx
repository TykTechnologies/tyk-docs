---
title: "Server-Sent Events (SSE) Proxy in Tyk Gateway"
description: "Configure Tyk Gateway as a Server-Sent Events (SSE) proxy to stream real-time events to clients while preserving authentication and rate limiting."
keywords: "SSE Proxy, Other Protocol"
sidebarTitle: "Server Side Events"
canonical: "https://tyk.io/docs/advanced-configuration/sse-proxy"
---
...
<Note>
`flush_interval` does not need to be configured for SSE APIs. The Gateway automatically handles immediate flushing for `text/event-stream` responses.
</Note>
