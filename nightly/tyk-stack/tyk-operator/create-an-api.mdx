...
canonical: "https://tyk.io/docs/nightly/tyk-stack/tyk-operator/create-an-api"
---
...
First, specify the details of your API using the [ApiDefinition CRD](/nightly/api-management/automations/operator#apidefinition-crd), then deploy it to create the corresponding Kubernetes resource. Tyk Operator will take control of the CRD and create the actual API in the Tyk data plane.
...
- [HTTP Proxy example](/nightly/tyk-stack/tyk-operator/create-an-api#set-up-manifest-for-http)
...
You can visit the [ApiDefinition CRD](/nightly/api-management/automations/operator#apidefinition-crd) page to see all the latest API Definitions fields and features we support.
...
<Note>
Tyk Operator supported authentication types are listed in the [API Definition features](/nightly/api-management/automations/operator#apidefinition-crd) section.
</Note>
...
- `jwt_signing_method`: Specify the method used to sign the JWT. Refer to the documentation on [JWT Signatures](/nightly/basic-config-and-security/security/authentication-authorization/json-web-tokens#jwt-signatures) for supported methods.
...
Below, we take you through how to define the security policy but you can also find [Security Policy Example](/nightly/tyk-stack/tyk-operator/create-an-api#security-policy-example) below.
