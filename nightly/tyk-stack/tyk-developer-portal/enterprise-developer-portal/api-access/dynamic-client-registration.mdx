...
It must be explicitly enabled in Tyk Developer Portal by setting [`TYK_PORTAL_ENABLEIDPREGISTRY=true`](/nightly/product-stack/tyk-enterprise-developer-portal/deploy/configuration#tyk_portal_enableidpregistry) (or `EnableIDPRegistry = true` in the config file) and restarting the Portal.
...
1. Set [`TYK_PORTAL_ENABLEIDPREGISTRY=true`](/nightly/product-stack/tyk-enterprise-developer-portal/deploy/configuration#tyk_portal_enableidpregistry) (or `EnableIDPRegistry = true` in the config file) and restart the Portal.
