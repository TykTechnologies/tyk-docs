...
You are now ready to [create an API](/nightly/api-management/gateway-config-managing-classic#create-an-api), or manage APIs declaratively with [Tyk Operator](/nightly/api-management/automations/operator), which is installed as part of this stack.
...
The bundled `values.yaml` is tuned for trials and evaluation, with Pump analytics to PostgreSQL, audit logging, hashed-key listing, OPA, and Dashboard security settings enabled. Inline comments mark the options to change for production and performance. For production deployments, also review the [Planning for Production](/nightly/planning-for-production) guide and the [helm-self-managed README](https://github.com/TykTechnologies/tyk-install/tree/main/kubernetes/helm-self-managed) for Ingress, TLS, autoscaling, and troubleshooting.

### Hybrid Control Plane and Data Plane

The installation above deploys a single, self-contained Tyk stack. To distribute API traffic across multiple data centers or regions, you can instead run a hybrid topology: a central Control Plane that hosts the management components and one or more remote Data Planes whose Gateways serve traffic locally and sync configuration from the Control Plane over [Tyk MDCB](/nightly/api-management/mdcb).

To set up a hybrid deployment with Helm:

- Install the [Control Plane](/nightly/api-management/mdcb#installing-in-a-kubernetes-cluster-with-our-helm-chart) first to provision the Dashboard, MDCB, and supporting services.
- Install each [Data Plane](/nightly/api-management/mdcb#installing-in-a-kubernetes-cluster-with-our-helm-chart-2) using the connection details produced by the Control Plane installation.
