...
When the Developer Portal is first started, you will go through a [bootstrapping](/nightly/portal/install#bootstrapping-enterprise-developer-portal) process during which an initial API Owner user will be created. When the bootstrapped user logs into the Portal, they will reach the Admin Portal. They can create additional API Owners from the **Settings > Admin Users** screen:
...
- You will be taken to the log in page (`/auth/password/login`) where you enter the email address and password registered for your account.
    ...
    <Note>
    If you are using Single Sign-On (SSO) then Tyk does not provide a log in page and you should [create one](/nightly/tyk-stack/tyk-developer-portal/enterprise-developer-portal/managing-access/enable-sso#testing-sso).
    </Note>
