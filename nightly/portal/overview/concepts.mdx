---
canonical: https://tyk.io/docs/portal/overview/concepts
description: Understand the fundamental concepts behind the Tyk Developer Portal,
  including APIs, access control, and customisation.
...
API Owners are the internal users who manage the publication of API Catalogs onto the Live Portal. They can configure the visual appearance of the portal, create [Catalogs, Products and Plans](/nightly/portal/overview/concepts#api-packaging-and-access-control), and manage the Organisation, Teams, and Users granted access to the Live Portal. They operate within the Admin Portal and have read-only access to the Live Portal.
...
The **Live Portal** is the public-facing website where [API Consumers](/nightly/portal/overview/concepts#api-consumers) can:
...
The **Admin Portal** is the private administrative view where [API Owners](/nightly/portal/overview/concepts#api-owners) can manage the content displayed in the Live Portal, approve access requests, and configure API Products.. It is also where users, Teams, and Organisation are administered.
...
The Developer Portal leverages Tyk's [partitioned policies](/nightly/api-management/policies#partitioned-policy-functionality) in two distinct ways:
