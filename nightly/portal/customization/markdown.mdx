...
canonical: "https://tyk.io/docs/portal/customization/markdown"
---
...
<Note>
When Markdown is enabled for a blog post, the template uses the `markdownify` function to render the content. See the [blog post template data](/nightly/portal/customization/templates#blog-detail) for details on `MarkdownEnabled` and `MarkdownContent`.
</Note>
...
See [Customize Pages](/nightly/portal/customization/pages) for how to create and edit pages and their content blocks.
...
This works well for API guides, authentication instructions, and quickstart tutorials. See [Getting Started Guide](/nightly/portal/api-products#getting-started-guide) for more details.
...
| Image | `![Alt text](/nightly/url)` |
| Reference image | `![Alt][ref]` with `[ref]: url` on a separate line |
| Image as link | `[![Alt](/nightly/image-url)](link-url)` |

<Note>
Markdown images (`![alt](/nightly/url)`) do not support specifying width or height. To control image dimensions, use an HTML `<img>` tag instead: `<img src="url" width="400" height="200" alt="description">`.
...
<Note>
The default theme does not render table borders. See [Table styling workarounds](/nightly/#table-styling) below for workarounds.
</Note>
