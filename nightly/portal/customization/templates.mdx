...
canonical: "https://tyk.io/docs/portal/customization/templates"
---
...
[section](/nightly/portal/customization/pages#edit-page-content)
...
- [Template Data](/nightly/#template-data): Detailed explanation of the data structures available in different templates.
- [Global Helper Functions](/nightly/#global-helper-functions): A list of global functions that can be used across templates to
manipulate and display data.
- [Email Templates](/nightly/#email-templates): Information about email-specific templates and their available data.
...
- [Analytics](/nightly/#analytics)
- [Application Create](/nightly/#application-create)
- [Application Detail](/nightly/#application-detail)
- [Blogs](/nightly/#blogs)
- [Blog Detail](/nightly/#blog-detail)
- [Cart Checkout](/nightly/#cart-checkout)
- [Organization User Detail](/nightly/#organization-user-detail)
- [Organization User Edit](/nightly/#organization-user-edit)
- [Organization Users List](/nightly/#organization-users-list)
- [Product Detail](/nightly/#product-detail)
- [Product OAS Documentation](/nightly/#product-oas-documentation)
...
| `{{ .MarkdownContent }}` | Markdown content (see [Markdown reference](/nightly/portal/customization/markdown)) |
...
{{ range .posts }}
<div class="blog-post">
  <h2><a href="/nightly/{{ .URL }}">{{ .Title }}</a></h2>
...
{{ .post.Content }}
{{ end }}
<p>Read more at: <a href="/nightly/{{ .post.URL }}">{{ .post.URL }}</a></p>
<h2>Latest Posts</h2>
{{ range .latest_posts }}
<div>
  <h3><a href="/nightly/{{ .URL }}">{{ .Title }}</a></h3>
  <p>{{ .Lede }}</p>
</div>
{{ end }}
```
...
<form action="edit" method="post" id="user-edit">
  ...
  {{ end }}
  <div>
    <a href="/nightly/portal/private/users">Cancel</a>
...
<div class="product-detail">
  ...
  <div class="related-post">
    <h3><a href="/nightly/{{ .Path }}">{{ .Title }}</a></h3>
    <img src="{{ .HeaderImage.URL }}" alt="{{ .Title }}">
    <p>{{ .Lede }}</p>
  </div>
  {{ end }}
...
<div class="docs-container">
  <div class="card mt-4">
    <div class="card-body">
      <h3 class="card-title">
        <a href="/nightly/portal/catalogue-products/{{ .Path }}">{{ .Name }}</a>
      </h3>
      <p class="card-text">
        {{ .Description }}
      </p>
...
- [CanCreateOrganisation](/nightly/#cancreateorganisation)
- [Clients](/nightly/#clients)
- [Current User](/nightly/#currentuser)
- [FeaturedProducts](/nightly/#featuredproducts)
- [FilterUserInvites](/nightly/#filteruserinvites)
- [FormatTime](/nightly/#formattime)
- [GetCart](/nightly/#getcart)
- [GetCatalogueList](/nightly/#getcataloguelist)
- [GetCataloguesForProduct](/nightly/#getcataloguesforproduct)
- [GetClientDescription](/nightly/#getclientdescription)
- [GetClientName](/nightly/#getclientname)
- [GetMenus](/nightly/#getmenus)
- [GetProducts](/nightly/#getproducts)
- [IsPortalDisabled](/nightly/#isportaldisabled)
- [IsPortalPrivate](/nightly/#isportalprivate)
- [ProductDocRenderer](/nightly/#productdocrenderer)
- [ProviderUpstreamURL](/nightly/#providerupstreamurl)
- [SplitStrings](/nightly/#splitstrings)
- [TruncateString](/nightly/#truncatestring)
- [TypeOfCredential](/nightly/#typeofcredential)
...
<div class="featured-products-container">
  ...
  <div class="product-card">
    ...
    <div class="product-actions">
      <a href="/nightly/portal/catalogue-products/{{ .Path }}" class="btn">More Info</a>
      <div class="dropdown-content">
        {{ range .APIDetails }}
        {{ if or (gt (.OASDocument.Base.Url | trim | length) 0) (gt (.OASUrl | trim | length) 0) }}
        <a href="/nightly/portal/catalogue-products/{{ $.Path }}/{{ .APIID }}/docs" target="blank">
          {{ .Name }}
        </a>
        {{ end }}
        {{ end }}
...
Returns a map of all [menus](/nightly/portal/customization/menus).
...
{{ if GetMenus.Primary }}
  {{ range GetMenus.Primary.Children }}
    {{ range .Children }}
    <li class="nav-item">
      <a class="dropdown-item" href="/nightly/{{ .Path }}">{{ .Tag }}</a>
    </li>
    {{ end }}
  {{ end }}
{{ end }}
...
<div class="col-lg-12 card-container">
  <div class="card d-flex flex-row {{ if .Logo.URL }}has-logo{{ end }}">
    ...
    <div class="card-cta d-flex flex-column align-self-center justify-content-between align-items-baseline w-100">
      <div>
        <a href="/nightly/portal/catalogue-products/{{ .Path }}" class="btn btn-secondary">More Info</a>
      </div>
    </div>
  </div>
</div>
...
Please refer to [email workflow](/nightly/portal/customization/email-notifications)
for additional detail on email notifications sent by the portal.


### Available Email Templates

- [Access Request Approve/Reject](/nightly/#access-request-approvereject)
- [Access Request Submitted](/nightly/#access-request-submitted)
- [Activate and Deactivate](/nightly/#activate-and-deactivate)
- [Key Expiration Warning](/nightly/#key-expiration-warning)
- [Key Expired](/nightly/#key-expired)
- [New User Request](/nightly/#new-user-request)
- [Organization Approve](/nightly/#organization-approve)
- [Organization Reject](/nightly/#organization-reject)
- [Organization Request](/nightly/#organization-request)
- [Reset Password](/nightly/#reset-password)
- [Targeted Invite](/nightly/#targeted-invite)
- [Welcome User](/nightly/#welcome-user)
...
**Template Path**: `themes/default/mailers/keytoexpire.tmpl`

This template is used to notify a developer that one of their credentials is about to expire.

##### Available Objects

- `{{ .user }}`: Returns the credential owner. Refer to the User Attributes table below for accessible attributes and methods.
- `{{ .credential }}`: Returns the credential that is about to expire (for example, `{{ .credential.CredentialHash }}` returns the credential hash).
...
```
<p>Hi {{ .user.First }},</p>
<p>Your credential {{ .credential.CredentialHash }} is about to expire. Please log in to the developer portal to renew it.</p>
```

#### Key Expired

**Template Path**: `themes/default/mailers/keyexpired.tmpl`

This template is used to notify a developer that one of their credentials has expired.

##### Available Objects

- `{{ .user }}`: Returns the credential owner. Refer to the Key Expiration Warning section for the available `{{ .user }}` attributes.
- `{{ .credential }}`: Returns the credential that has expired (for example, `{{ .credential.CredentialHash }}` returns the credential hash).

##### Example Usage
```
<p>Hi {{ .user.First }},</p>
<p>Your credential {{ .credential.CredentialHash }} has expired.</p>
```
...
**Template Path**: `themes/default/mailers/organisation_approve.tmpl`
...
- `{{ confirm_url }}`: Alias for `reset_password_url`; returns the same password-setting URL.
- `{{ login_url }}`: Returns the portal login URL (the custom SSO login URL if configured, otherwise the default login URL).
