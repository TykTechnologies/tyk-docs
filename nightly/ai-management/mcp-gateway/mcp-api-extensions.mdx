...
canonical: "https://tyk.io/docs/ai-management/mcp-gateway/mcp-api-extensions"
---
...
<Note>
For the full OpenAPI specifications of the Gateway API and Dashboard API, see [Tyk APIs](/nightly/tyk-apis).
</Note>
...
Returns the full Tyk OAS API definition. See [Get an MCP proxy: Gateway API](/nightly/#get-an-mcp-proxy) for the response shape.
...
For a conceptual explanation of what the policy fields do and how to configure access tiers, see [MCP proxy policies](/nightly/ai-management/mcp-gateway/policies).
...
If `x-tyk-api-gateway.server.authentication.securitySchemes[name].oauth2.protectedResourceMetadata` is present and enabled, Tyk validates the PRM configuration. For MCP proxies specifically, at least one entry in `authorizationServers` is required.
