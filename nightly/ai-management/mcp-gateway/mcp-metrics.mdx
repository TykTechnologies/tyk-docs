...
The examples below use Tyk's [custom metrics](/nightly/api-management/metrics/custom-metrics) system. Each instrument is a JSON object in the `opentelemetry.metrics.api_metrics` array in `tyk.conf`.
...
<Note>
The `tyk.mcp.requests.total` counter uses 6 dimensions. Keep the total dimension count at 10 or fewer per instrument to stay on the OTel SDK fast path. See [Performance Considerations](/nightly/api-management/logs/access-logs#performance-considerations) for guidance.
</Note>
