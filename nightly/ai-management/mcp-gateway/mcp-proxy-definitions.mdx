...
keywords: "MCP, Model Context Protocol, MCP proxy definition, x-tyk-api-gateway, x-tyk-mcp-server, API to MCP, JSON-RPC, primitives, tools, resources, prompts, operations, Tyk OAS, Streamable HTTP"
sidebarTitle: "MCP Proxy Definition"
canonical: "https://tyk.io/docs/ai-management/mcp-gateway/mcp-proxy-definitions"
---

## Overview

An MCP proxy definition is the configuration object that tells Tyk how to front an MCP server. It is built on the **[Tyk OAS API definition](/nightly/api-management/gateway-config-tyk-oas)** format (an OpenAPI 3.0 document extended with the `x-tyk-api-gateway` vendor extension) and adds the MCP-specific structures that allow Tyk to inspect JSON-RPC traffic and apply middleware at the method and primitive level.
...
<Note>
If you are not familiar with Tyk OAS API definitions, read [Tyk OAS](/nightly/api-management/gateway-config-tyk-oas) first. This page focuses on the MCP-specific aspects and assumes knowledge of the base format.
</Note>

## Structure Overview

An MCP proxy definition has two parts that work together:
...
Three core MCP concepts shape how you write an MCP proxy definition: primitives, JSON-RPC methods, and transport endpoints. Understanding them before reading the definition structure makes the configuration decisions much clearer.
...
The OpenAPI specification in an MCP proxy definition documents the server's HTTP interface. For MCP, this has a standard structure that you can treat as a template.
...
There are no MCP-specific fields in these three sections. You configure authentication, TLS, load balancing, upstream rate limits, and all other standard gateway capabilities exactly as you would for a REST API. See [Tyk OAS](/nightly/api-management/gateway-config-tyk-oas) for the full field reference for these sections.

For an MCP proxy [generated directly from a Tyk-managed REST API](/nightly/ai-management/mcps/api-to-mcp), `upstream.url` holds an adapter target instead of a remote server's url. See [The upstream adapter target](/nightly/ai-management/mcp-gateway/api-to-mcp-definitions#the-upstream-adapter-target) for what this value means and why it takes this form.

The example above uses `bearerAuth` as the security scheme, a simple bearer token check. For full OAuth 2.1 compliance (token validation, scope enforcement, Protected Resource Metadata, and token exchange), use the `oauth2` scheme instead. See [MCP OAuth 2.1](/nightly/ai-management/mcp-gateway/oauth-2-1).

### The middleware Section

The `middleware` section is where MCP proxy definitions diverge from standard Tyk OAS API definitions. It contains the same `global` block for API-wide middleware, but adds two new concepts: an `operations` map keyed by JSON-RPC method, and three primitive maps: `mcpTools`, `mcpResources`, and `mcpPrompts`. This section covers where each middleware option lives in the definition; for what each option actually does, see [MCP middleware](/nightly/ai-management/mcp-gateway/mcp-middleware).
...
| **Protected Resource Metadata (PRM)** | 2025-03-26 | ✅ Full | Tyk serves `/.well-known/oauth-protected-resource` automatically. Configured under `authentication.securitySchemes[name].oauth2.protectedResourceMetadata` from Tyk 5.14.0. See [OAuth 2.1 authentication](/nightly/ai-management/mcp-gateway/oauth-2-1). |
...
| API to MCP tool catalog and overrides | `x-tyk-mcp-server` (REST API to MCP proxies only) |
