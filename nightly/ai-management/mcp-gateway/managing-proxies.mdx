...
For scripted or automated management, use the Dashboard API or Gateway API. See [MCP API extensions](/nightly/ai-management/mcp-gateway/mcp-api-extensions) for the full endpoint reference.

<Note>
MCP proxy definitions use the Tyk OAS format. They are not available as Tyk Classic API definitions. For the full definition structure, see [MCP proxy definition](/nightly/ai-management/mcp-gateway/mcp-proxy-definitions).
</Note>
...
The **Add MCP Proxy** button opens the [Create an MCP proxy](/nightly/#create-an-mcp-proxy) screen.
...
- **Tyk API**: [convert an existing Tyk OAS API into an MCP proxy](/nightly/ai-management/mcps/api-to-mcp). Operations from its OpenAPI spec become MCP tools.

![Create MCP Proxy choice screen](/img/ai-management/mcp-create-proxy-choice-screen.png)
...
<Note>
For most deployments you'll want to open the MCP Designer to configure authentication, add per-primitive middleware, or set up OAuth discovery. See the [Settings tab](/nightly/#settings-tab) and [Primitives tab](/nightly/#primitives-tab) below.
</Note>
...
Only Tyk OAS APIs appear in this list. Tyk Classic API definitions cannot be used to generate MCP proxies. See [Requirements](/nightly/ai-management/mcp-gateway/overview#requirements-and-limitations).

Click **Continue** to proceed.

![Create MCP proxy, Select API step](/img/ai-management/create-mcp-proxy-select-api.png)

#### Step 3: Map endpoints to tools

All valid endpoints from the source API's OpenAPI spec are mapped as tools by default. Deselect any endpoint you don't want to expose, following Tyk's [Deny by Default](/nightly/api-management/security-best-practices#deny-by-default) guidance: favor building up an explicit allow-list of only the operations an agent needs, rather than leaving every operation exposed. Use the search box and HTTP method filter to narrow the list; the counter shows how many of the total endpoints are currently selected.

<Note>
Mapped tools inherit the source endpoint's name and description. To override a tool's name, description, or parameters, use the **Tool mapping** tab of the MCP Designer after creating the proxy. See [Tool mapping tab](/nightly/#tool-mapping-tab) below.
</Note>

Click **Continue** to proceed.

![Create MCP proxy, Map endpoints to tools step](/img/ai-management/create-mcp-proxy-map-endpoints.png)
...
Click **Finish**. This does not create the proxy immediately: Tyk previews the generated tool catalog and takes you to the MCP Designer with the preview pre-populated. Review the tools on the [Tool mapping tab](/nightly/#tool-mapping-tab), then click **Save MCP Proxy** to create the proxy.

![Create MCP proxy, Connect your gateways step](/img/ai-management/create-mcp-proxy-connect-gateways.png)

<Note>
The wizard creates the proxy with no authentication (keyless access) by default. Before deploying to production, open the [Settings tab](/nightly/#settings-tab) to configure an authentication method, or the [Tool mapping tab](/nightly/#tool-mapping-tab) to rename tools and parameters before saving.
</Note>
...
![MCP Designer, showing the read-only Source API panel on a REST API to MCP proxy](/img/ai-management/mcp-designer-source-api-panel.png)
...
**Authentication**: the authentication method applied to all inbound requests. Select a method from the **Authentication type** dropdown. See [Authentication](/nightly/api-management/client-authentication) for all supported methods and configuration options.
...
These options map to `x-tyk-api-gateway.middleware.global` in the proxy definition. For full configuration details, see [MCP middleware: proxy level](/nightly/ai-management/mcp-gateway/mcp-middleware#dashboard-settings-tab-proxy-level).
...
![Tool mapping tab, operation list and detail pane](/img/ai-management/mcp-designer-tool-mapping-tab.png)

Click an operation to open its detail pane and configure:

| Field | What it does |
|---|---|
| **Tool Name** | Overrides the tool's caller-facing name. Defaults to the name Tyk derived from the operation (see [MCP Gateway: Core Concepts](/nightly/ai-management/mcp-gateway/core-concepts#tool-naming-and-discovery)). |
...
Click **Save MCP Proxy** to apply your changes. These map to `x-tyk-mcp-server.primitives[]` in the proxy definition. See [REST API to MCP x-tyk-mcp-server extension](/nightly/ai-management/mcp-gateway/api-to-mcp-definitions) for the full field reference.
...
Each middleware option maps to the corresponding configuration block inside the primitive's entry in the proxy definition. See [MCP middleware](/nightly/ai-management/mcp-gateway/mcp-middleware) for what each option does and how it is configured.
...
| `x-tyk-api-gateway.upstream` | Upstream URL, load balancing, upstream authentication, and mTLS. For a REST API to MCP proxy, `upstream.url` holds an adapter target (for example `tyk://a1b2c3d4e5f647a8b9c0d1e2f3a4b5c6/mcp`) instead of a remote `https://` URL. See [The upstream adapter target](/nightly/ai-management/mcp-gateway/api-to-mcp-definitions#the-upstream-adapter-target). |
| `x-tyk-mcp-server` | REST API to MCP proxies only. Holds the tool catalog: which source operations are exposed, and any name, description, or parameter overrides. See [REST API to MCP x-tyk-mcp-server extension](/nightly/ai-management/mcp-gateway/api-to-mcp-definitions). |
...
**Configuring authentication**: The proxy is created with bearer token authentication enabled. To switch to a different method, open the Settings tab and select from the **Authentication type** dropdown. To use the external IdP integration — including scope check, PRM, and token exchange — select **OAuth 2.0**. See [Authentication](/nightly/api-management/client-authentication) for all supported methods.

**Enabling PRM for OAuth discovery**: Open the Settings tab, select **OAuth 2.0** as the authentication type, and enable the **Protected Resource Metadata** toggle. Set the resource URL and add at least one authorization server URL. See [OAuth 2.0 authentication](/nightly/api-management/authentication/oauth2-authentication) for full configuration details.

**Restricting which tools clients can call**: Use the Primitives tab: add the tool as a primitive, then add **Allow** middleware to it. Once any tool in the proxy has an Allow rule, all unlisted tools are blocked. For definition-based configuration, see [MCP middleware: access control](/nightly/ai-management/mcp-gateway/mcp-middleware#access-control).

**Applying rate limits to a specific tool**: Use the Primitives tab: open the tool primitive and add **Rate Limit** middleware. For definition-based configuration, see [MCP middleware: traffic management](/nightly/ai-management/mcp-gateway/mcp-middleware#traffic-management).

**Configuring upstream OAuth**: Add an `authentication.oauth.clientCredentials` block to `x-tyk-api-gateway.upstream` to have Tyk obtain and forward OAuth tokens to your upstream MCP server. See [MCP Gateway: OAuth 2.1 authentication](/nightly/ai-management/mcp-gateway/oauth-2-1#upstream-oauth).
...
For a complete explanation of every field in the definition, see [MCP proxy definition](/nightly/ai-management/mcp-gateway/mcp-proxy-definitions).
