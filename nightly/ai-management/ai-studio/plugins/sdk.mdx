...
| **[Custom Endpoints](/nightly/ai-management/ai-studio/plugins/custom-endpoints)** | `CustomEndpointHandler` | Gateway | Serve custom HTTP endpoints under `/plugins/{slug}/` |
| **UI Provider** | `UIProvider` | Studio only | Serve admin web UI assets |
| **[Portal UI](/nightly/ai-management/ai-studio/plugins/portal-ui)** | `PortalUIProvider` | Studio only | Portal-facing pages and forms with user context |
...
| **[Resource Provider](/nightly/ai-management/ai-studio/plugins/resource-types)** | `ResourceProvider` | Studio only | Register custom resource types for Apps with privacy scoring |
...
Conversational AI agent with streaming support. See [Agent Plugins Guide](/nightly/ai-management/ai-studio/plugins/studio-agent) for details.
...
Intercept CRUD operations on AI Studio objects (LLMs, Datasources, Tools, Users). See [Object Hooks Guide](/nightly/ai-management/ai-studio/plugins/object-hooks) for complete details.
...
Serve web UI assets for AI Studio plugins. See [UI Plugins Guide](/nightly/ai-management/ai-studio/plugins/studio-ui) for details.
...
Receive data from edge (Edge Gateway) plugins. This enables the hub-and-spoke communication pattern where edge plugins can send data back to the control plane. See [Edge-to-Control Communication](/nightly/ai-management/ai-studio/plugins/edge-to-control) for complete details.
...
- See [Service API Reference](/nightly/ai-management/ai-studio/plugins/service-api#event-service) for complete documentation
