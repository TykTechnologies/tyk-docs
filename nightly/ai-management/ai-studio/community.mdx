...
description: "How AI Portal users submit Data Sources and Tools for admin review, and how governance, privacy scoring, versioning, and rollback work"
...
canonical: "https://tyk.io/docs/ai-management/ai-studio/community"
---
...
Community Submissions let [AI Portal](/nightly/ai-management/ai-studio/ai-portal) users propose their own [Data Sources](/nightly/ai-management/ai-studio/datasources-rag) and [Tools](/nightly/ai-management/ai-studio/tools) for admin review. An admin checks each submission, sets its privacy score, and approves it before it becomes a usable resource. Your resource catalog grows. Admin governance stays in place.
...
```mermaid
graph LR
    A[AI Portal user drafts a submission] --> B[Submits for review]
    B --> C[Admin reviews]
...
Community Submissions solves both problems. An AI Portal user can propose a resource themselves. The resource only goes live after an admin reviews it. This keeps governance in place and removes the bottleneck.

You can only submit Data Sources and Tools this way. LLM providers always need direct admin configuration. Every submission goes through the same review queue.

<Note>
A plugin can also register a custom resource type that supports community submissions. See [Resource Provider Plugins](/nightly/ai-management/ai-studio/plugins/resource-types#community-submissions) for that variant of the workflow.
</Note>
...
<img src="/img/ai-management/community-submit-resource-form.png" alt="Submit a Resource form in the AI Portal, showing the Data Source resource type with Basic Information and Vector Database Access Details sections" />

For a Tool, the form shows an OpenAPI Specification section instead, where the submitter pastes a spec and validates it before submitting.

<img src="/img/ai-management/community-submit-resource-form-tool.png" alt="Submit a Resource form in the AI Portal, showing the Tool (OpenAPI) resource type with an OpenAPI spec pasted into the OpenAPI Specification section" />
...
An admin works submissions from the **Submissions** review queue. Access to the review queue, and to every review action on this page, requires the [Admin role](/nightly/ai-management/ai-studio/user-management#core-concepts). There is no separate reviewer or moderator role. Any user with `Admin` can review submissions, and no other role can.
...
<img src="/img/ai-management/community-submission-queue.png" alt="Submission Queue admin page listing a Data Source submission with Pending Review status" />
...
<img src="/img/ai-management/community-review-submission.png" alt="Submission review screen showing the resource configuration, suggested privacy score, support details, and Approve, Request Changes, and Reject actions" />

## Attestation Templates

An Attestation Template is an admin-authored statement. A submitter must read and accept it before they can submit a resource for review.
...
<img src="/img/ai-management/community-attestation-templates.png" alt="Attestation Templates admin page listing a required template that applies to all resource types" />
...
A privacy score is a number from 0 (public) to 100 (highly sensitive). It governs which LLMs a resource can be used with. Tyk AI Studio enforces this rule when a resource is added to an App. A Data Source or Tool's privacy score can never exceed the privacy score of its paired LLMs. See [Privacy Level Validation](/nightly/ai-management/ai-studio/admin-apps#privacy-level-validation) for details.
...
Every submission keeps a chronological log of the actions taken on it: submitted, review claimed, approved, rejected, changes requested, and resubmitted. Each entry records who acted, when, and any submitter-facing feedback or internal note attached to that action.
