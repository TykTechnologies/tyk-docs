...
### Use cases
- **Administrative Management**: Super admins can create user accounts for other administrators to help manage AI Studio configurations, [Teams](/nightly/ai-management/ai-studio/teams), and [Catalogs](/nightly/ai-management/ai-studio/catalogs).
- **Developer Access**: Developers can be granted access to the [AI Portal](/nightly/ai-management/ai-studio/ai-portal) to consume LLM APIs using their generated API keys.
- **End-User Chat**: Non-technical users can be given access to the [AI Chat](/nightly/ai-management/ai-studio/chat-interface) interface to interact with approved LLMs and data sources safely.

### Community vs Enterprise Edition
In the **Community Edition**, basic user management is available, and all users are automatically assigned to a single, built-in "Default" [Team](/nightly/ai-management/ai-studio/teams). 
...
Users do not directly get assigned to AI resources (like LLMs or Data Sources). Instead, their access is governed by the [Teams](/nightly/ai-management/ai-studio/teams) they belong to. When a user is added to a Team, they inherit access to all the [Catalogs](/nightly/ai-management/ai-studio/catalogs) associated with that Team.
...
This API key is specifically used for programmatic access to the Tyk [AI Studio management APIs](/nightly/ai-management/ai-studio/ai-studio-swagger) (the Admin API) and the AI Portal.
...
1. Navigate to **Governance > Users** in the admin UI sidebar.
...
6. (Optional) After creation, you can view the user's details to generate or copy their API Key, or assign them to specific [Teams](/nightly/ai-management/ai-studio/teams).
