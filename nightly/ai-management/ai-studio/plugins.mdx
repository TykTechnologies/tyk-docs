...
canonical: "https://tyk.io/docs/ai-management/ai-studio/plugins"
---
...
{/* [Learn more →](/nightly/plugins-edge gateway) */}
...
{/* [Learn more →](/nightly/plugins-studio-ui) */}
...
{/* [Learn more →](/nightly/plugins-studio-agent) */}
...
{/* [Learn more →](/nightly/plugins-object-hooks) */}
...
{/* Plugins can also run as standalone gRPC services in a sidecar or elsewhere on the network, rather than as local sub-processes. See [Plugin Deployment](/nightly/./plugins-deployment) for details. */}
...
└──────────┬──────────┘
           ...
           │                      │                      │
┌──────────▼──────────┐ ┌─────────▼────────┐ ┌─────────▼────────┐
│  Edge Gateway       │ │   UI Plugin      │ │  Agent Plugin    │
...
{/* [Learn more about Service API →](/nightly/[plugins-service-api](https://docs.claude.com/en/docs/plugins-service-api))) */}
...
{/* [Learn more about deployment →](/nightly/[plugins-deployment](https://docs.claude.com/en/docs/plugins-deployment))) */}
...
{/* [Learn more about manifests →](/nightly/[plugins-manifests](https://docs.claude.com/en/docs/plugins-manifests))) */}
...
5. Build and test with `file://` deployment (see [Development Workflow Guide](/nightly/) for fast iteration)
6. Deploy with `grpc://` or `oci://` for production

**Pro tip**: Use the reload API (`POST /api/v1/plugins/{id}/reload`) to test changes instantly without reinstalling. See the [Development Workflow Guide](/nightly/) for the fastest iteration loop.
...
**Note**: If you have existing plugins using the old SDKs (`edge gateway/plugins/sdk` or `pkg/ai_studio_sdk`), see the [Migration Guide](/nightly/) for upgrade instructions.
