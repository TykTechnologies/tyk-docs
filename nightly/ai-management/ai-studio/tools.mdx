...
canonical: "https://tyk.io/docs/ai-management/ai-studio/tools"
---
...
Tools bridge the gap between conversational AI and external functionalities. By defining tools, you allow LLMs interacting via the [Chat Interface](/nightly/ai-management/ai-studio/chat-interface) or API to:
...
*   **Authentication:** Tools often require authentication to access the target API. Tyk AI Studio handles this securely by integrating with [Secrets Management](/nightly/ai-management/ai-studio/secrets). You configure the authentication method (e.g., Bearer Token, Basic Auth) defined in the OAS. Reference a stored Secret that contains the actual credentials.
*   **Privacy Levels:** Each Tool is assigned a privacy level. This level is compared against the privacy level of the [LLM Configuration](/nightly/ai-management/ai-studio/llm-management) being used. A Tool can only be used if its privacy level is less than or equal to the LLM's level. This prevents sensitive tools from use with less secure or external LLMs.

    Privacy levels define how data is protected by controlling LLM access based on its sensitivity. See [Privacy Levels](/nightly/ai-management/ai-studio/privacy-levels) for the full score mapping and how the comparison works.
...
*   **Filters:** Optional [Filters](/nightly/ai-management/ai-studio/filters) can pre-process requests sent to the tool. They can also post-process responses received from it, for example for data sanitization.
...
2. **REST API** — Each tool is also available as a direct REST API endpoint for developers. They can call it programmatically, independent of LLM interactions.
3. **[MCP Interface](/nightly/ai-management/mcps/overview)** — An MCP-compliant shim wraps the OpenAPI-generated tools, providing an MCP-API compatible interface. This works with tools that require authentication and provides MCP compatibility without a separate MCP proxy server.
## How it Works

When a user interacts with an LLM via the [Chat Interface](/nightly/ai-management/ai-studio/chat-interface):
...
4.  **Configure Authentication:** Select the OAS security scheme and link to a stored [Secret](/nightly/ai-management/ai-studio/secrets) for credentials.
...
<img src="/img/ai-management/import-openapi-select-provider.png" alt="Import OpenAPI wizard with Tyk Dashboard selected as the import method" />
...
*   **Access Token or Secret Reference:** Your Tyk Dashboard API access token, or a [secret reference](/nightly/ai-management/ai-studio/secrets) such as `$SECRET/DashboardKey`.
...
<img src="/img/ai-management/import-openapi-connect-dashboard.png" alt="Connect to the Dashboard step with URL and Access Token fields" />
...
<img src="/img/ai-management/import-openapi-select-api.png" alt="Select an API step listing active Tyk OAS APIs from the Dashboard" />
...
*   **Privacy Level:** Defaults to 50 (Confidential). Change it to match the sensitivity of the data the API returns. See [Privacy Levels](/nightly/ai-management/ai-studio/privacy-levels) for more details.
...
<img src="/img/ai-management/import-openapi-configure-tool.png" alt="Configure Tool step with fields pre-filled from the selected API" />
...
*   **Assign to Teams:** Assign Tool Catalogues to specific [teams](/nightly/ai-management/ai-studio/user-management). This grants users in those groups *potential* access to the tools within the catalogue.

    <img src="/img/ai-management/tool-catalog-config.png" alt="Catalogue Config" />

## Using Tools (User)

Tools become available to end-users within the [Chat Interface](/nightly/ai-management/ai-studio/chat-interface) if:
