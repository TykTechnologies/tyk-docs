...
sidebarTitle: "Agents"
canonical: "https://tyk.io/docs/ai-management/ai-studio/agents"
---
...
An **Agent** is an administrator-managed object in Tyk AI Studio. It connects an [Agent plugin](/nightly/ai-management/ai-studio/plugins/studio-agent) to an [App](/nightly/ai-management/ai-studio/admin-apps), and controls which Teams can use it. Once you activate an Agent, it appears as a conversational AI option for its users in the AI Portal.

This page covers the **Agent** object, managed in the **Agents** section of the AI Studio dashboard. For the plugin code that powers an Agent's conversation logic, see [AI Studio Agent Plugins](/nightly/ai-management/ai-studio/plugins/studio-agent).
...
|------|-----------|---------------------|
| **Agent plugin** | The gRPC plugin code that implements the conversation logic. A developer builds and deploys it. | The **Plugins** section of the AI Studio dashboard, with hook type `agent`. See [AI Studio Agent Plugins](/nightly/ai-management/ai-studio/plugins/studio-agent). |
...
- An active Agent plugin (hook type `agent`) in the **Plugins** section of the AI Studio dashboard. See [AI Studio Agent Plugins](/nightly/ai-management/ai-studio/plugins/studio-agent) to build or install one.
- An active [App](/nightly/ai-management/ai-studio/admin-apps) with an active credential and at least one LLM assigned. The App supplies the LLMs, [Tools](/nightly/ai-management/ai-studio/tools), and [Data Sources](/nightly/ai-management/ai-studio/datasources-rag) that the Agent can use.
...
- **App**: The [App](/nightly/ai-management/ai-studio/admin-apps) that supplies LLMs, Tools, and Data Sources to this Agent. Required.
...
- **Teams**: The [Teams](/nightly/ai-management/ai-studio/teams) that can use this Agent. Leave this field empty to make the Agent available to all users. Some Agent screens label this same access control **Groups**.
...
LLM calls that the Agent makes still go through the [AI Gateway](/nightly/ai-management/ai-studio/proxy) and count against the App's [budgets](/nightly/ai-management/ai-studio/budgeting).
