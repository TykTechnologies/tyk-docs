...
*   **Tool Integration:** Seamlessly uses configured [Tools](/nightly/ai-management/ai-studio/tools) when the LLM determines they are necessary to fulfill a user's request. The available tools depend on the Chat Experience configuration and the user's team permissions.
*   **Data Source (RAG) Integration:** Can automatically query configured [Data Sources](/nightly/ai-management/ai-studio/datasources-rag) to retrieve relevant information (Retrieval-Augmented Generation) to enhance LLM responses. The available data sources depend on the Chat Experience configuration and the user's team permissions.
...
*   **Access Control:** Users only see and can interact with Chat Experiences assigned to their [Teams](/nightly/ai-management/ai-studio/user-management).
...
*   **Assigning LLM:** Linking to a specific [LLM Configuration](/nightly/ai-management/ai-studio/llm-management).
*   **Selecting Call Settings:** Choosing an existing [Call Setting](/nightly/ai-management/ai-studio/call-settings) for the assigned LLM.
*   **Enabling Tools:** Selecting which [Tool Catalogues](/nightly/ai-management/ai-studio/tools) are available.
*   **Enabling Data Sources:** Selecting which [Data Source Catalogues](/nightly/ai-management/ai-studio/datasources-rag) are available.
*   **Setting System Prompt:** Defining the guiding prompt for the LLM.
*   **Applying Filters:** Associating specific [Filters](/nightly/ai-management/ai-studio/filters) for governance.
*   **Assigning Teams:** Determining which [Teams](/nightly/ai-management/ai-studio/user-management) can access this Chat Experience.
