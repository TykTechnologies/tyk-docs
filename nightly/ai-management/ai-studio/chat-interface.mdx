...
*   **Tool Integration:** Seamlessly uses configured [Tools](/nightly/ai-management/ai-studio/tools) when the LLM determines they are necessary to fulfill a user's request. The available tools depend on the Chat Experience configuration and the user's group permissions.
