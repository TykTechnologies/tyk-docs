...
2.  **Admin Login:** Log in using the administrator account you created during registration:
    ...
    *   **Password:** The password you set during the registration process
    ...
    <Note>
    **Reminder**: If you haven't completed the initial registration yet, go back to your [installation guide](/nightly/ai-management/ai-studio/quickstart) and follow the "First User Registration" section.
    </Note>
...
4.  **Enter Details:**
    ...
    *   **API Key (Using Secrets):**
        *   **IMPORTANT:** Do *not* paste your API key directly here. Instead, use [Secrets Management](/nightly/ai-management/ai-studio/secrets).
...
This LLM is now available for use within Tyk AI Studio, subject to [User/Group permissions](/nightly/ai-management/ai-studio/user-management).
...
*   **Email Configuration:** If using features like user invites or notifications, ensure SMTP settings are correctly configured and test email delivery if possible ([Notifications](/nightly/ai-management/ai-studio/notifications)).
...
Refer to the **Configuration Options** detailed within the [Installation Guide](/nightly/ai-management/ai-studio/deployment-k8s) for specifics on setting these values during the deployment process.
...
*   Try out the [Chat Interface](/nightly/ai-management/ai-studio/chat-interface).
