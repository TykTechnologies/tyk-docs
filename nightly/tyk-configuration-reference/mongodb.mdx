...
Tyk Dashboard, Tyk MDCB and Tyk Pump connect to the same MongoDB instance as part of the Core Platform's shared [persistent database](/nightly/api-management/persistent-database). This page covers how to point each component at MongoDB.

<Note>
Tyk Developer Portal does not support MongoDB for storage. See [Configure SQL Storage](/nightly/tyk-configuration-reference/sql) for its supported engines.
</Note>
...
Tyk also connects to a managed [MongoDB Atlas](https://www.mongodb.com/atlas/database) cluster the same way as any other MongoDB instance - see [MongoDB Atlas](/nightly/#mongodb-atlas) below for connection details.

## Configuration Reference

Each component configures MongoDB differently:

- Tyk Dashboard uses a `storage.<category>` block per category of data. `<category>` is one of `main`, `analytics`, `logs`, or `uptime` - each of the [four categories of data](/nightly/api-management/persistent-database#how-tyk-uses-persistent-storage) is configured independently, though all four normally point at the same database instance. Note that Aggregate Analytics is configured as `analytics`, not `aggregate_analytics`.
- Tyk MDCB uses a top-level `analytics` block.
- Tyk Pump uses its own [Mongo pump types](/nightly/api-management/dashboard-analytics/control-plane-pumps#mongodb), each with a `meta` object.
...
|---|---|---|---|
| Storage type | `type` = `mongo` | `type` = `mongo` | Implied by pump type (`mongo`, `mongo-pump-selective`, `mongo-pump-aggregate`, and so on) - see [Choosing a Pump Type](/nightly/api-management/dashboard-analytics/control-plane-pumps#choosing-a-pump-type) |
...
Tyk does not expose connection pool tuning for a MongoDB backend - only [PostgreSQL](/nightly/tyk-configuration-reference/sql#connection-pool-management) has configurable pool settings. Instead, each connection uses its [driver's](/nightly/#configuration-reference) own default, unmodified:
...
Tyk Pump connects to MongoDB through its [Mongo pump types](/nightly/api-management/dashboard-analytics/control-plane-pumps#choosing-a-pump-type), each declared in `pump.conf` (or using equivalent environment variables).

For full details of the MongoDB pump types and their configuration see [Control Plane Pumps](/nightly/api-management/dashboard-analytics/control-plane-pumps#mongodb).
...
For 1 million requests per day - roughly 1GB of Traffic Logs per day and 30MB of Aggregate Analytics per month (see [Sizing Guidelines](/nightly/api-management/persistent-database#sizing-guidelines)) - keeping 7 days of Traffic Logs and 3 months of Aggregate Analytics in your working set requires:
