...
Tyk does not expose connection pool tuning for a MongoDB backend - only [PostgreSQL](/nightly/tyk-configuration-reference/sql#connection-pool-management) has configurable pool settings. Instead, each connection uses its [driver's](#configuration-reference) own default, unmodified:
