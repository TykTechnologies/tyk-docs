...
description: Learn how to use external Key-Value storage with Tyk Gateway, including
  Consul and Vault, to manage configuration data and secrets.
...
With Tyk Gateway you can store configuration data (typically authentication secrets or upstream server locations) in Key-Value (KV) systems such as [Vault](/nightly/#vault), and [Consul](/nightly/#consul) and then reference these values during configuration of the Tyk Gateway or APIs deployed on the Gateway.
...
The exception to this is for specific [transformation middleware](/nightly/#transformation-middleware) where the value will be retrieved for each call to the API, during the processing of the API request or response.
...
- [URL rewrite](/nightly/transform-traffic/url-rewriting#url-rewrite-middleware)
...
Alternatively, you can configure it using the equivalent [environment variables](/nightly/tyk-oss-gateway/configuration#kvconsuladdress).
...
Alternatively, you can configure it using the equivalent [environment variables](/nightly/tyk-oss-gateway/configuration#kvvaulttoken).
