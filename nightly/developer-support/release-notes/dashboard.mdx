...
See [Secrets Management](/nightly/docs/tyk-configuration-reference/kv-store) for full details.
</Accordion>
...
{/* TT-17497 */}
The new `file://` storage option added to Tyk Gateway's [Key-Value secret storage](/nightly/tyk-configuration-reference/kv-store#supported-storage-options) feature can be configured in the API editor.
...
- The Tyk OAS API designer now fully supports the use of Key-Value storage [references](/nightly/tyk-configuration-reference/kv-store#from-api-definitions) that are valid in the API definition.
...
See how to [Choose a MongoDB driver](/nightly/planning-for-production/database-settings#mongodb)
...
While SQL support for Tyk products does not depend on specific database features, with this release, we will provide official support for [PostgreSQL DB for production purposes](/nightly/planning-for-production/database-settings#postgresql), and SQLite for development and PoC environments. Note that SQL support is available for self-managed setups only.
...
[MongoDB to SQL migration docs](/nightly/planning-for-production/database-settings#migrating-from-an-existing-mongodb-instance)
