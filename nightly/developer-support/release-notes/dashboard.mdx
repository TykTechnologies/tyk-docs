...
Only the most recent Tyk Dashboard release and the most recent on the [Long Term Support (LTS)](/nightly/developer-support/release-types/long-term-support) branch are supported. We recommend staying current on the feature or LTS branch to minimise exposure to security vulnerabilities from third party dependencies.
...
You can now create an MCP proxy directly from an API already registered in Tyk, instead of only pointing at a remote MCP server. From **Add MCP Proxy**, choosing **API to MCP** launches a dedicated wizard: pick the source Tyk OAS API, then select which of its endpoints should be exposed as tools (every endpoint is selected by default, with search, a method filter, and a live count of exposed endpoints). A preview step calls the Gateway to expand the full set of configurable tools before handing off to the API Designer.

For an API-to-MCP proxy, the deployment target and API version are inherited from the source API and shown as read-only in the designer's Settings tab, and upstream-only sections such as Certificate Pinning, Rate Limit, and Upstream Authentication are hidden, since these proxies have no upstream configuration of their own.

The designer also gains a new **Tool mapping** tab for these proxies, listing every tool the source API could expose, including any left unselected in the wizard. Each row has an allow toggle, and selecting a tool opens a detail pane where you can override its name, description, and parameters. Only the tools you actually change, together with which tools are allowed, are saved, so anything left untouched keeps following the source API's own defaults as it evolves.

The MCP proxy list now shows a **Type** column so you can tell Tyk API and external server proxies apart at a glance.

**Introducing Tyk Governance**
 
This release introduces Tyk Governance, a new capability built directly into the Dashboard for defining API standards and continuously checking your Tyk OAS APIs against them — catching design, security, and compliance issues before an API reaches production instead of after.

Platform teams can create rulesets from a ready-to-use template or their own definition, apply them to APIs via categories, and see results directly on a new Governance tab for every API, complete with severity-graded issues and specific remediation guidance. Compliance status is also surfaced across the API list and on the Dashboard home page, and the same checks can be run from a CI/CD pipeline before an API is ever imported.

Tyk Governance is enabled by a new claim in the Dashboard license; contact your Tyk account team to turn it on.

**Configuration Secrets Management**
 
This release adds [Secrets Management](/nightly/tyk-configuration-reference/kv-store) to the Tyk Dashboard configuration file for the first time, letting you reference values from external secrets stores in place of plain-text values.
 
**IAM-based authentication for Google Cloud Memorystore**

Tyk Dashboard now supports IAM-based authentication for Google Cloud Memorystore for Redis Cluster and Valkey, as an alternative to a static password.

For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.15.0) below.
...
#### Dependencies 
<a id="dependencies-5.15.0"></a>

<a id="dependencies-5.15.0"></a>

| Dashboard Version | Recommended Releases | Backwards Compatibility |
| :------ | :---------------- | :---------------- |
| 5.15.0  | MDCB v2.13.0      | MDCB v2.13.0      |
...
<a id="3rdPartyTools-v5.15.0"></a>

| Third Party Dependency                                    | Tested Versions     | Compatible Versions | Comments | 
| :-------------------------------------------------------- | :------------------ | :------------------ | :------- | 
| [Golang](https://go.dev/dl/)                              | 1.26                |  1.26               |  | 
...
| [OpenAPI Specification](https://spec.openapis.org/oas/v3.1.2.html) | v3.1.x, v3.0.x | v3.1.x, v3.0.x | Supported by [Tyk OAS](/nightly/api-management/gateway-config-tyk-oas)|

Given the potential time difference between your upgrade and the release of this version, we recommend users verify the ongoing support of third-party dependencies they install, as their status may have changed since the release.

#### Upgrade instructions
<a id="upgrade-5.15.0"></a>

<a id="upgrade-5.15.0"></a>

If you are upgrading to 5.15.0, please follow the detailed [upgrade instructions](/nightly/#upgrading-tyk).

#### Downloads

- [Docker Image to pull](https://hub.docker.com/r/tykio/tyk-dashboard/tags?page=&page_size=&ordering=&name=v5.15.0)
  - ```bash
    docker pull tykio/tyk-dashboard:v5.15.0
    ```
- Helm charts
  - [tyk-charts](/nightly/developer-support/release-notes/helm-chart)

<Note>
Tyk Helm Charts are configured to install the LTS version of Tyk Dashboard. You will need to modify them to install v5.15.0.
</Note>
...
<Accordion title='Added support for creating MCP proxies from existing Tyk OAS APIs'>
{/* TT-17595, TT-17596, TT-17597, TT-17598, TT-17599, TT-17600 */}
You can now create an MCP proxy directly from an API already registered in Tyk, instead of only pointing at a remote MCP server. From **Add MCP Proxy**, choosing **API to MCP** launches a dedicated wizard: pick the source Tyk OAS API, then select which of its endpoints should be exposed as tools (every endpoint is selected by default, with search, a method filter, and a live count of exposed endpoints). A preview step calls the Gateway to expand the full set of configurable tools before handing off to the API Designer.
...
<Accordion title='Introduced Tyk Governance: rulesets and continuous compliance checks for your Tyk OAS APIs'>
{/* TT-17019, TT-17020, TT-17021, TT-17022, TT-17023, TT-17024, TT-17025, TT-17041, TT-17074, TT-17075, TT-17093, TT-17097, TT-17142, TT-17143, TT-17144, TT-17149, TT-17150, TT-17151, TT-17152, TT-17241, TT-17256, TT-17275, TT-17325, TT-17511, TT-17512, TT-17513, TT-17574, TT-17634, TT-17657, TT-17682, TT-17733, TT-17745, TT-17753, TT-17764, TT-17806, TT-17835, TT-17876, TT-17885, TT-17911 */}
Tyk Governance is a new Enterprise Edition capability, built directly into the Dashboard, for defining API standards and continuously checking your Tyk OAS APIs against them — so design, security, and compliance issues are caught before an API reaches production rather than after.
- **Create rulesets** — a set of governance rules — either from a ready-to-use template (an OWASP API Security ruleset, or a general OpenAPI design-best-practices ruleset), or from your own definition by uploading or pasting a JSON or YAML ruleset file.
- **Apply rulesets via API categories** — link a ruleset to one or more of your existing API categories, and every API carrying that category is automatically governed by it; an API can be governed by more than one ruleset at once.
- **Continuous and on-demand evaluation** — APIs are evaluated automatically whenever a service is saved, moves category, or a linked ruleset changes. You can also test any ruleset against any API on demand, without affecting its stored compliance status, before deciding to apply it.
- **A new Governance tab** on every API shows its compliance status, a breakdown by ruleset, and a filterable table of issues, each with a severity level (Error, Warn, or Info) and specific "how to fix" guidance you can configure per rule.
- **Portfolio-level visibility** — the API list gains a compliance status column and filter, and the Dashboard home page shows a notification banner whenever an API becomes non-compliant, so issues are visible without opening every API individually.
- **An optional Deployment Warning** can be enabled per ruleset, surfacing a warning (not a hard block) if you try to save a non-compliant change to a governed API.
- **CI/CD governance checks** — run the same evaluation from a pipeline (GitHub Actions, GitLab CI, Jenkins, or any tool that can call an HTTP endpoint) directly against an OpenAPI document, before the API is even imported into Tyk.
- Access to creating and managing rulesets is controlled by a dedicated "Rulesets" permission, separate from your existing API permissions.
In this release, Tyk Governance supports Tyk OAS APIs only. Tyk Governance is disabled by default — contact your Tyk account team to enable it for your organization. For more information, visit our documentation at [Tyk Governance Documentation](https://tyk.io/docs/tyk-governance/overview).
</Accordion>

<Accordion title='Secrets Management'>
{/* TT-16268, TT-17102, TT-17623, TT-17697, TT-17698, TT-17702 */}
Tyk Dashboard now supports Secrets Management, letting you reference values from external secrets stores: the Tyk config file, environment variables, local files (including Kubernetes-mounted secrets), Consul, Vault, AWS Secrets Manager, Azure Key Vault, and GCP Secret Manager, in place of plain-text values in Dashboard's configuration, using a consistent reference syntax:

- `kv://<store>/<path>[#fragment]` for whole-value references.
- `$kv{<store>:<path>[#fragment]}` for embedding a reference inside a larger string.

The secrets management configuration block (`kv`) can itself contain references, for example pulling a Vault token from an environment variable.
 
References are resolved once, at startup. Individual secrets stores can be marked as `required` and, if they cannot be reached or initialized, Dashboard will fail to start with a clear error rather than starting with that store silently unavailable.

See [Secrets Management](/nightly/tyk-configuration-reference/kv-store) for full details.
</Accordion>
 
<Accordion title='Custom plugin enhancements in API Designer'>
{/* TT-16948, TT-17761 */}
We have made some changes in the custom plugins section of the API Designer to give access to new features in Tyk Gateway:

- choose Tyk Gateway's new JavaScript plugin engine by selecting the `JavaScript` driver
- when using JavaScript plugins, provide your plugin code inline as an alternative to giving a path to a file containing the code
- assign JavaScript plugins to any plugin hook
- assign multiple plugin bundles to an API with a simple comma separated list
- include analytics plugins in the custom plugin bundle
</Accordion>

<Accordion title='Added GCP IAM authentication for Redis/Valkey'>
{/* TT-17611 */}
Tyk Dashboard can now authenticate to Google Cloud Memorystore for Redis/Valkey using GCP IAM-based authentication, as an alternative to a static Redis password.

Dashboard obtains a Google OAuth2 access token from the workload's identity, using Application Default Credentials (ADC) and authenticates with the Memorystore using this token.

This is supported for use with Memorystore for Valkey and Memorystore for Redis Cluster. The legacy non-cluster Memorystore for Redis has no IAM support from Google and continues to use a static password.

The IAM configuration is set in the new `iam_auth` section of MDCB's storage config.
</Accordion>

<Accordion title='PostgreSQL connection pool management'>
{/* TT-16212 */}
Tyk Dashboard now exposes configuration options to manage its pools of PostgreSQL database connections.

The following options are available in the `storage.<category>.postgres` configuration and can be configured separately for each category of data.

- `max_open_connections` sets the maximum number of open connections in the pool (default: unlimited)
- `max_idle_connections` sets the maximum number of idle connections kept in the pool (default: 2)
- `connection_max_lifetime` sets how long a connection can be reused before being closed (default: unlimited)
- `connection_max_idle_time` sets how long a connection can sit idle before being closed (default: unlimited)

<Note>
Tyk Dashboard maintains two connection pools (read and write) for each of the four categories of data and the maximum number of connections is configured **per pool**.
</Note>

For more details see [SQL Connection Pool Management](/nightly/tyk-configuration-reference/sql#connection-pool-management).
</Accordion>

<Accordion title='Select client authentication method when requesting token from upstream OAuth authorization server'>
{/* TT-16039 */}
When requesting an OAuth token to use in Upstream OAuth, Tyk follows RFC 6749 in presenting the client credentials first in the request header and, if this fails, in the request body.

If the OAuth authorization server explicitly requires the credentials in the request body, this will mean that the first request will always fail.

You can now select the behavior that Tyk should adopt via a dropdown in the API Designer:

- **Header** will send the credentials only in the request header
- **Body** will send the credentials only in the request body
- **Header then Body** will try header and then body (default)
</Accordion>

<Accordion title='Dashboard now shows the exact license expiry date and time'>
{/* TT-16617 */}
The Dashboard's Nodes & Licensing page now displays the exact license expiry date and time instead of an approximate relative string (such as "in 5 months"), making it easier to plan for license renewal.
</Accordion>
...
A full CRUD API has been implemented in Tyk Dashboard for creating and managing Client IdP assets. See the [changelog](/nightly/#Changelog-v5.14.0) for the endpoint reference.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.14.0) below.
...
If you are upgrading to 5.14.0, please follow the detailed [upgrade instructions](/nightly/#upgrading-tyk).
...
You can now configure an [enforced (request) timeout](/nightly/planning-for-production/ensure-high-availability/enforced-timeouts) at API level. This API-level timeout takes precedence over the Gateway default (`proxy_default_timeout`). The more granular endpoint-level timeout overrides the API-level value.
...
{/* TT-17497 */}
The new `file://` storage option added to Tyk Gateway's [Key-Value secret storage](/nightly/tyk-configuration-reference/kv-store#supported-secret-stores) feature can be configured in the API editor.
...
{/* TT-17453 */}
Tyk Dashboard embeds Tyk Identity Broker. This has been updated to [version 1.7.3](/nightly/developer-support/release-notes/tib#1-7-3-release-notes).
</Accordion>
...
{/* TT-16816 */}
We have resolved an issue where importing a [multipart OpenAPI document](/nightly/api-management/gateway-config-managing-oas#multi-part-openapi-documents) from a ZIP archive left temporary files on the Dashboard's local disk after the import completed. Over time this could exhaust disk space and, because the temporary location was derived from the archive filename, a later import using the same filename could reuse stale contents resulting in unexpected API configuration.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.13.2) below.
...
| Third Party Dependency                                    | Tested Versions     | Compatible Versions | Comments | 
| :-------------------------------------------------------- | :------------------ | :------------------ | :------- | 
| [Golang](https://go.dev/dl/)                              | 1.26                |  1.26               | [Go plugins](/nightly/api-management/plugins/golang) must be built using Go 1.26 | 
| [Redis](https://redis.io/download/)                       | 6.2.x, 7.x, 7.4.x, 8.x (up to 8.4)   | 6.2.x, 7.x, 7.4.x, 8.x (up to 8.4)   | | 
| [Valkey](https://valkey.io/download/)                     | 7.2.x, 8.0.x, 8.1.x | 7.2.x, 8.0.x, 8.1.x | | 
| [MongoDB](https://www.mongodb.com/try/download/community)  | 7.0.x, 8.0.x  | 6.0.x, 7.0.x, 8.0.x  | |
| [DocumentDB](https://aws.amazon.com/documentdb/)          | 4, 5                | 4, 5                | | 
...
If you are upgrading to 5.13.2, please follow the detailed [upgrade instructions](/nightly/#upgrading-tyk).
...
For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.13.1) below.
...
If you are upgrading to 5.13.1, please follow the detailed [upgrade instructions](/nightly/#upgrading-tyk).
...
<Accordion title='Fixed issue with temporary files left behind after importing a multipart OpenAPI document'>
{/* TT-16816 */}
Resolved an issue where importing a [multipart OpenAPI document](/nightly/api-management/gateway-config-managing-oas#multi-part-openapi-documents) from a ZIP archive left temporary files on the Dashboard's local disk after the import completed. Over time this could exhaust disk space and, because the temporary location was derived from the archive filename, a later import using the same filename could reuse stale contents resulting in unexpected API configuration.
...
Dashboard 5.13.0 introduces policies that grant access to MCP Proxies. Tyk Developer Portal versions prior to 1.17.2 may automatically retrieve these policies and present them as API Products when [synchronizing with the Dashboard](/nightly/portal/api-products#dashboard-first-approach).
...
See [MCP Gateway upgrade considerations](/nightly/developer-support/upgrading#mcp-upgrade-considerations) for the full sequence.
...
For details, see the [Managing MCP Proxies documentation](/nightly/ai-management/mcp-gateway/managing-proxies).
</Accordion>
...
- [post_expiry_action](/nightly/api-management/access-control/sessions-and-keys/session-lifecycle#2-define-the-post-expiry-action) - determines what happens to the data in Redis after the `expires` timestamp is reached.
- [post_expiry_grace_period](/nightly/api-management/access-control/sessions-and-keys/session-lifecycle#3-configure-the-grace-period) - defines how long (in seconds) the Session is kept in Redis after expiration (if the `post_expiry_actions` is to retain the Session)
...
- **Configurable toast notification duration**: The display duration of toast notifications can now be configured separately for success, warning, info, and error messages via the new [ui.notifications.duration](/nightly/tyk-dashboard/configuration#ui-notifications-duration) configuration option, replacing the previous fixed three-second duration. This helps reduce visual noise from frequent notifications while keeping critical messages visible for longer.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.12.1).
...
| Third Party Dependency | Tested Versions | Compatible Versions | Comments | 
| ---------------------- | --------------- | ------------------- | -------- | 
| [GoLang](https://go.dev/dl/)          | 1.25 | 1.25 | [Go plugins](/nightly/api-management/plugins/golang) must be built using Golang 1.25 | 
| [Redis](https://redis.io/download/)   | 5.x, 6.x, 7.x, 8.x (up to 8.4) | 5.x, 6.x, 7.x, 8.x (up to 8.4) | | 
| [Valkey](https://valkey.io/download/) | 8.0.x, 8.1.x    | 7.2.x, 8.0.x, 8.1.x    | | 
| [MongoDB](https://www.mongodb.com/try/download/community)  | 6, 7, 8  | 5, 6, 7, 8  | | 
| [DocumentDB](https://aws.amazon.com/documentdb/)  | 4, 5  | 4, 5  | | 
| [PostgreSQL](https://www.postgresql.org/download/) | 13.x - 17.x | 13.x - 17.x  | | 
| [OpenAPI Specification](https://spec.openapis.org/oas/v3.1.2.html) | v3.1.x | v3.1.x | Supported by [Tyk OAS](/nightly/api-management/gateway-config-tyk-oas)|
...
To provide an additional layer of security for your APIs, we've introduced Dashboard support for [Client Certificate-Token Binding](/nightly/api-management/authentication/bearer-token#client-certificate-token-binding). This feature allows you to form a strict binding association between an Auth Token issued to a client and their specific client certificate.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.12.0).
...
The Tyk Dashboard now supports [application logging](/nightly/api-management/logs/application-logs) in structured JSON format. To enable this feature, set the environment variable `TYK_DB_LOGFORMAT` to `json`.
...
In this release, we have fixed some priority CVEs. For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.11.1).
...
In this release we've deprecated the [policies.allow_explicit_policy_id](/nightly/tyk-oss-gateway/configuration#policies-allow_explicit_policy_id) configuration option. This was previously added to allow the use of custom policy IDs, which is now the default behaviour so this option is redundant.
...
This patch release fixes an issue in the API Designer where JWKS cache timeout values could not be saved properly. For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.10.2).
...
- [Docker Image to pull](https://hub.docker.com/r/tykio/tyk-dashboard/tags?page=&page_size=&ordering=&name=v5.10.2)
  - ```bash
    ...
    ```
- Helm charts
  - [tyk-charts v4.1.0](/nightly/developer-support/release-notes/helm-chart#4-1-0-release-notes)
...
This patch release upgrades the Go build environment and delivers UI, analytics, and security fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.10.1).
...
Both interfaces now consistently accept either `null` or `[]` (empty array) values in the `versions` field of the access control list, ensuring seamless interoperability between API and UI workflows for policy and key management. Tyk OAS APIs use a [different approach](/nightly/api-management/api-versioning#how-api-versioning-works-with-tyk) to versioning, with each (base or child) version having a unique API ID that is added to the access list.
</Accordion>
...
For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.9.2).
...
| Third Party Dependency | Tested Versions | Compatible Versions | Comments | 
| :---------------------- | :--------------- | :------------------- | :-------- | 
| [GoLang](https://go.dev/dl/)          | 1.23 | 1.23 | [Go plugins](/nightly/api-management/plugins/golang) must be built using Go 1.23 | 
| [Redis](https://redis.io/download/)   | 5.x, 6.x, 7.x, 8.x (up to 8.2) | 5.x, 6.x, 7.x, 8.x (up to 8.2) | | 
| [Valkey](https://valkey.io/download/) | 8.0.x, 8.1.x    | 7.2.x, 8.0.x, 8.1.x    | | 
| [MongoDB](https://www.mongodb.com/try/download/community)  | 6, 7, 8  | 5, 6, 7, 8  | | 
| [DocumentDB](https://aws.amazon.com/documentdb/)  | 4, 5  | 4, 5  | | 
...
Resolved a compatibility issue introduced via a fix in Dashboard v5.9.0 (LTS v5.8.3) to account for DocumentDB's handling of endpoint paths containing dots (`.`). The Dashboard now escapes dot characters (e.g., `\u002e`) when storing Tyk OAS API definitions in the database, but MDCB failed to unescape them when reading from the database. This caused the [request validation](/nightly/api-management/traffic-transformation/request-validation) and [mock response](/nightly/api-management/traffic-transformation/mock-response#mock-response) middleware configured on affected endpoints not to be applied. 

To align MDCB's dot handling mechanism with Tyk Dashboard, a new configuration option, `escape_dots_in_oas_paths`, has been introduced in both [Dashboard](/nightly/tyk-dashboard/configuration#escape_dots_in_oas_paths) and [MDCB](/nightly/tyk-multi-data-centre/mdcb-configuration-options#escape_dots_in_oas_paths):
...
Check the [Upgrade and Compatibility section](/nightly/#upgrade-5.9.2) for details on the recommended upgrade path.
</Expandable>
...
This release builds on the recent release of [Tyk 5.8.3](/nightly/developer-support/release-notes/dashboard#5-8-3-release-notes), adding a collection of new capabilities. For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.9.0).
...
For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.8.15) below.
...
If you are upgrading to 5.8.15, please follow the detailed [upgrade instructions](/nightly/#upgrading-tyk).
...
Official FIPS 140-3 readiness, which additionally requires Docker Hardened Images and enterprise supply chain assurances, is available exclusively from 5.13.x onward. See [FIPS 140-3](/nightly/developer-support/release-types/fips-release#fips-standard-by-version) for details.
</Note>
</Accordion>
...
In this release, we have fixed some priority CVEs. For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.8.11).
...
This patch release addresses a CVE present in 5.8.9. For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.8.10).
...
In this release we've deprecated the [allow_explicit_policy_id](/nightly/tyk-dashboard/configuration#allow_explicit_policy_id) configuration option. This was previously added to allow the use of custom policy IDs, which is now the default behaviour so this option is redundant.
...
This patch release contains various bug fixes and introduces new OPA helper functions to identify the type of API being processed (Tyk OAS, Tyk Streams, Tyk Classic). For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.8.9).
...
This patch release upgrades the Go build environment and fixes some high-priority CVEs. For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.8.8).
...
Resolved an issue where the [Dashboard API](/nightly/tyk-dashboard-api) allowed creation of empty or duplicate categories when updating Tyk OAS APIs through the `PUT /api/apis/oas/{API_ID}/categories` endpoint.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.8.5).
...
Check the [Upgrade and Compatibility section](/nightly/#upgrade-5.8.5) for details on the recommended upgrade path.
</Expandable>
...
- The Tyk OAS API designer now fully supports the use of Key-Value storage [references](/nightly/tyk-configuration-reference/kv-store#how-it-works) that are valid in the API definition.
...
With Tyk 5.8.0 we are delighted to unlock the power and flexibility of Tyk OAS for all users, with full feature parity with the legacy Tyk Classic style for REST APIs. We are thrilled to announce new updates and improvements in Tyk 5.8.0, delivering more control, flexibility, and performance.  For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.8.0) below.
...
| Third Party Dependency                                     | Tested Versions        | Compatible Versions    | Comments | 
| :---------------------------------------------------------- | :---------------------- | :---------------------- | :-------- | 
| [GoLang](https://go.dev/dl/)                               | 1.22       | 1.22       | [Go plugins](/nightly/api-management/plugins/golang) must be built using Go 1.22 | 
| [Redis](https://redis.io/download/)  | 6.2.x, 7.x  | 6.2.x, 7.x  | | 
| [MongoDB](https://www.mongodb.com/try/download/community)  | 5.0.x, 6.0.x, 7.0.x  | 5.0.x, 6.0.x, 7.0.x  | | 
| [DocumentDB](https://aws.amazon.com/documentdb/)  | 4, 5  | 4, 5  | | 
...
This release focuses mainly on a security fix. For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.7.2) below.
...
* **[External OAuth](/nightly/api-management/client-authentication#integrate-with-external-authorization-server-deprecated)**

  * Tyk Classic: `external_oauth`
  * Tyk OAS: `server.authentication.securitySchemes.externalOAuth`

* **[OpenID Connect (OIDC)](/nightly/api-management/client-authentication#integrate-with-openid-connect-deprecated)**

  * Tyk Classic: `auth_configs.oidc`
  * Tyk OAS: `server.authentication.oidc`

We recommend migrating to **[JWT Authentication](/nightly/basic-config-and-security/security/authentication-authorization/json-web-tokens)** for improved flexibility and long-term support.
...
[Setup guide for JWE OIDC SSO](/nightly/api-management/single-sign-on-social-idp#json-web-encryption-jwe)
</Accordion>
...
For detail usage of the `/audit-logs` endpoint, please see [Dashboard API documentation](/nightly/tyk-dashboard-api).
</Accordion>
...
We are thrilled to announce new updates and improvements in Tyk 5.6.0, bringing more control, flexibility, and performance.  For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.6.0) below.

##### Per endpoint Rate Limiting for clients

Now you can configure rate limits at the [endpoint level per client](/nightly/api-management/rate-limit#key-level-rate-limiting), using new configuration options in the access key. Use [Policies](/nightly/api-management/policies) to create templates to set appropriate rate limits for your different categories of user.
...
We encourage users to switch to the recommended alternatives. For more detailed information, please refer to the [Documentation](/nightly/api-management/client-authentication#integrate-with-openid-connect-deprecated)
...
| Third Party Dependency                                     | Tested Versions        | Compatible Versions    | Comments | 
| :---------------------------------------------------------- | :---------------------- | :---------------------- | :-------- | 
| [GoLang](https://go.dev/dl/)                               | 1.21       | 1.21       | [Go plugins](/nightly/api-management/plugins/golang#) must be built using Go 1.21 | 
| [Redis](https://redis.io/download/)  | 6.2.x, 7.x  | 6.2.x, 7.x  | | 
| [MongoDB](https://www.mongodb.com/try/download/community)  | 5.0.x, 6.0.x, 7.0.x  | 5.0.x, 6.0.x, 7.0.x  | | 
| [DocumentDB](https://aws.amazon.com/documentdb/)  | 4, 5  | 4, 5  | | 
| [PostgreSQL](https://www.postgresql.org/download/)         | 12.x - 16.x        | 12.x - 16.x            | | 
...
We are excited to announce Tyk Dashboard 5.5, featuring a brand-new dashboard identity, advanced rate-limiting capabilities, and enhanced security options. For a comprehensive list of changes, please refer to the [changelog](/nightly/#Changelog-v5.5.0) below.
...
In this release, we upgraded the Golang version to `v1.23` and fixed a [CVE-2025-21613](https://nvd.nist.gov/vuln/detail/CVE-2025-21613). For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.3.10) below.
...
The UI now displays a deprecation notice for the dedicated [External OAuth](/nightly/api-management/client-authentication#integrate-with-external-authorization-server-deprecated) and [OpenID Connect (OIDC)](/nightly/api-management/client-authentication#integrate-with-openid-connect-deprecated) authentication mechanisms. This provides advanced notification that these authentication options will be deprecated in version 5.7.0. Users are advised to migrate to the [JWT Auth](/nightly/basic-config-and-security/security/authentication-authorization/json-web-tokens) method, which supports integration with both OAuth and OIDC providers, in preparation for future upgrade.
</Expandable>
...
If you are upgrading to 5.3.7, please follow the detailed [upgrade instructions](/nightly/#upgrading-tyk). 
...
This release primarily focuses on bug fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.3.6) below.
...
If moving from a version of Tyk older than 5.3.0 please read the explanation provided with [5.3.0 release](/nightly/#TykOAS-v5.3.0).
...
With MongoDB 4.4 reaching [EOL](https://www.mongodb.com/legal/support-policy/lifecycles) in February 2024, we can no longer guarantee full compatibility with this version of the database. If you are [using MongoDB](/nightly/tyk-configuration-reference/mongodb) we recommend that you upgrade to a version that we have tested with, as indicated [below](/nightly/#3rdPartyTools-v5.3.6).


With PostgreSQL v11 reaching [EOL](https://www.postgresql.org/support/versioning/) in November 2023, we can no longer guarantee full compatibility with this version of the database. If you are [using PostgreSQL](/nightly/tyk-configuration-reference/sql) we recommend that you upgrade to a version that we have tested with, as indicated [below](/nightly/#3rdPartyTools-v5.3.6).
...
- [Docker Image to pull](https://hub.docker.com/r/tykio/tyk-dashboard/tags?page=&page_size=&ordering=&name=v5.3.6)
 - ```bash
   ...
   ```
- Helm charts
 - [tyk-charts v2.0](/nightly/developer-support/release-notes/helm-chart#2-0-0-release-notes)
...
There are no breaking changes in this release, however, if moving from a version of Tyk older than 5.3.0 please read the explanation provided with [5.3.0 release](/nightly/#TykOAS-v5.3.0).
...
With MongoDB 4.4 reaching [EOL](https://www.mongodb.com/legal/support-policy/lifecycles) in February 2024, we can no longer guarantee full compatibility with this version of the database. If you are [using MongoDB](/nightly/tyk-configuration-reference/mongodb) we recommend that you upgrade to a version that we have tested with, as indicated [below](/nightly/#3rdPartyTools-v5.3.5).
...
#### Release Highlights
For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.3.4) below.
...
With MongoDB 4.4 reaching [EOL](https://www.mongodb.com/legal/support-policy/lifecycles) in February 2024, we can no longer guarantee full compatibility with this version of the database. If you are [using MongoDB](/nightly/tyk-configuration-reference/mongodb) we recommend that you upgrade to a version that we have tested with, as indicated [below](/nightly/#3rdPartyTools-v5.3.3).
...
For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.3.3) below.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.3.2) below.
...
With MongoDB 4.4 reaching [EOL](https://www.mongodb.com/legal/support-policy/lifecycles) in February 2024, we can no longer guarantee full compatibility with this version of the database. If you are [using MongoDB](/nightly/tyk-configuration-reference/mongodb) we recommend that you upgrade to a version that we have tested with, as indicated [below](/nightly/#3rdPartyTools-v5.3.2).
...
#### Release Highlights
This release primarily focuses on bug fixes.
For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.3.1) below.
...
With MongoDB 4.4 reaching [EOL](https://www.mongodb.com/legal/support-policy/lifecycles) in February 2024, we can no longer guarantee full compatibility with this version of the database. If you are [using MongoDB](/nightly/tyk-configuration-reference/mongodb) we recommend that you upgrade to a version that we have tested with, as indicated [below](/nightly/#3rdPartyTools-v5.3.1).
...
- **Not Backwards Compatible**
  ...
  - This means that if you wish to downgrade or revert to your previous version of Tyk, you will need to restore these API definitions from a backup. Please go to the [backup](/nightly/#upgrade-instructions) section for detailed instructions on backup before upgrading to v5.3.0.
...
**Important:** Please go to the [backup](/nightly/#upgrade-instructions) section for essential instructions on how to backup before upgrading to v5.3.0
...
With MongoDB 4.4 reaching [EOL](https://www.mongodb.com/legal/support-policy/lifecycles) in February 2024, we can no longer guarantee full compatibility with this version of the database and recommend upgrading to a version that we have tested with, as indicated [below](/nightly/#3rdPartyTools-v5.3.0).
...
1. For Self Managed deployments - Backup Your environment using the [usual guidance](/nightly/developer-support/upgrading#tyk-upgrade-guides-for-different-deployment-models) documented with every release (this includes backup config file and database).
2. For all deployments - Backup all your API definitions (Tyk OAS API and Classic Definitions):
   - For Tyk Cloud deployments - To perform the backup please use our guide for [exporting APIs and policies](/nightly/developer-support/upgrading#backup-apis-and-policies).
   - For Self-Managed deployments -  To perform the backup please use [Tyk Sync](/nightly/api-management/automations/sync).
4. Performing the upgrade - For all deployments, follow the instructions in the [upgrade guide](/nightly/#upgrading-tyk) when upgrading Tyk.

#### Release Highlights

We are excited to announce the release of 5.3.0, packed with new features, improvements and bug fixes to enhance your experience with Tyk Dashboard. For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.3.0) below.

##### Tyk OAS Feature Maturity

Tyk OAS is now out of [Early Access](/nightly/developer-support/release-types/early-access-feature) as we have reached feature maturity. You are now able to make use of the majority of Tyk's features from your Tyk OAS APIs, so they are a credible alternative to the legacy Tyk Classic APIs.
...
Tyk uses `mongo-go` as the default MongoDB driver from v5.3. This provides support for MongoDB 4.4.x, 5.0.x, 6.0.x and 7.0.x. If you are using older MongoDB versions e.g. 3.x, please set MongoDB driver to `mgo`. The [MongoDB supported versions](/nightly/tyk-configuration-reference/mongodb#supported-versions) page provides details on how to configure MongoDB drivers in Tyk.
</Accordion>
</AccordionGroup>
...
Relaxed the strict validation for mock response so that the `Description` field is now optional for `response`, `responses` and `schema` within the OpenAPI description. Automatically configuring mock responses when using [Tyk OAS APIs](/nightly/api-management/traffic-transformation/mock-response#tyk-oas) is now even easier.
</Accordion>
...
#### Release Highlights
Dashboard 5.2.5 was version bumped only, to align with Gateway 5.2.5. Subsequently, no changes were encountered in release 5.2.5. Gateway 5.2.5 was a critical patch release. For further information please see the release notes for Gateway [v5.2.5](/nightly/developer-support/release-notes/gateway) 
...
Fixed two UI issues with the [OPA editor](/nightly/platform-management/open-policy-agent#using-the-open-policy-agent-in-the-dashboard) in the Tyk Dashboard to improve experience when using this feature. Scrolling beyond the end of the OPA window does not now start to scroll the API Designer window, and minimizing then re-expanding the OPA editor no longer limits the text to one line.
</Accordion>
...
Fixed an issue where the [OPA editor](/nightly/platform-management/open-policy-agent#using-the-open-policy-agent-in-the-dashboard) was not resizable. The fix ensures the floating OPA editor is now resizable and the resizing operation is smooth, improving user experience.
</Accordion>

<Accordion title='User Search not working unless you enter the full email address'>
Fixed an issue where the [User Search](/nightly/platform-management/dashboard-users#search-users) was not working unless the full email address was entered. The fix restores the functionality of showing suggestions for names as they are typed in, improving user experience and search efficiency.
</Accordion>
...
Fixed an issue in the Classic API Designer where the 'use_standard_auth' value was constantly reverting to 'true' when editing an API with an [external OAuth flow](/nightly/api-management/client-authentication#integrate-with-external-authorization-server-deprecated). This fix ensures the 'use_standard_auth' value remains consistent, enabling the use of external OAuth via the Raw API editor.
</Accordion>
...
Added a new Dashboard configuration option `allow_unsafe_oas`. This permits the modification of Tyk OAS APIs via the Tyk Classic API endpoints. This is not a recommended action due to the risk of inconsistent behavior and potential for breaking changes while Tyk OAS is in [Early Access](/nightly/developer-support/release-types/early-access-feature). This is provided for early adopters and will be deprecated later, once Tyk OAS reaches full maturity.
</Expandable>
...
We're thrilled to bring you some exciting enhancements and crucial fixes to improve your experience with Tyk Dashboard. For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.2.0) below.

Configure Caching Timeouts Per API Endpoint and Enable Advanced Caching Options From Within Dashboard

We’ve added the ability to [configure](/nightly/api-management/response-caching#configuring-the-middleware-in-the-tyk-oas-api-definition) per-endpoint timeouts for Tyk’s response cache, giving you increased flexibility to tailor your APIs to your upstream services. While doing this, we’ve also fixed a longstanding issue within the *Tyk Dashboard* so that you can configure more of the [advanced caching](/nightly/api-management/response-caching#configuring-the-middleware-in-the-api-designer) options from within the UI.

##### Added Body Transform Middleware to Tyk OAS API Definition

With this release, we are adding the much requested *Body Transformations* to *Tyk OAS API Definition*. You can now [configure](/nightly/api-management/gateway-config-tyk-oas#transformbody) middleware for both [request](/nightly/api-management/traffic-transformation/request-body) and [response](/nightly/api-management/traffic-transformation/response-body) *Body Transformations* and - as a *Tyk Dashboard* user - you’ll be able to do so from within our simple and elegant API Designer tool. Visually test and preview *Body Transformations* from within the API Designer.
...
Added a new [endpoint](/nightly/tyk-dashboard-api), */system/stats*, to provide insight and operational statistics on total and active APIs deployed. The endpoint's flexible date filtering options, equip users to obtain comprehensive insights into usage trends.
</Expandable>
...
documentation for a full description of this new [user role](/nightly/platform-management/user-permissions).

##### Import API examples from within the Dashboard

In 5.0 we introduced the possibility to import API examples manually or via
[_Tyk Sync_](/nightly/api-management/automations/sync). We have now extended this feature and it is now possible to do this without
...
We added a [Dashboard API Endpoint](/nightly/api-management/data-graph#automatically-creating-rest-udg-configuration-based-on-oas-specification) that is capable of taking an OAS 3.0.0 document and converting it into a UDG API.
...
size using [slave_options.rpc_pool_size](/nightly/tyk-oss-gateway/configuration#slave_options-rpc_pool_size)
...
- Added a feature to display a full [_Tyk Sync_](/nightly/api-management/automations/sync) command that will allow a user to import an example UDG into their Dashboard
...
If you are upgrading to 5.0.15, please follow the detailed [upgrade instructions](/nightly/#upgrading-tyk). 
...
Go to the [Upgrading Tyk](/nightly/developer-support/release-notes/gateway#upgrading-tyk) section for detailed upgrade instructions.
...
See how to [Choose a MongoDB driver](/nightly/tyk-configuration-reference/mongodb)
...
##### Release Highlights
This release primarily focuses on bug fixes. 
For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v5.0.1) below.
...
We’ve improved support for [OAS Mock Responses](/nightly/api-management/traffic-transformation/mock-response#mock-response), with the Tyk OAS API definition now allowing you to register multiple Mock Responses in a single API, providing you with increased testing flexibility.
...
- via command line [using tyk-sync](/nightly/api-management/data-graph#udg-examples)
...
With this release we are also giving our users [improved headers for GQL APIs](/nightly/api-management/graphql#graphql-apis-headers). It is now possible to use context variables in request headers and persist headers needed for introspection separately for improved security.
...
- Now you can turn a Kafka topic into a GraphQL subscription by simply [importing your AsyncAPI definition](/nightly/api-management/data-graph#data-graphs-api)
...
[Importing OAS v3 via the Dashboard](/nightly/api-management/gateway-config-managing-oas#importing-an-openapi-description-to-create-an-api)
...
If you want switch from MongoDB to SQL, you can [use our migration tool](/nightly/tyk-configuration-reference/sql#migrating-tyk-dashboard-from-mongodb-to-sql), but keep in mind that it does not yet support the migration of your analytics data.
...
- Added option to configure certificates for Tyk Dashboard using [environment variables](/nightly/tyk-dashboard/configuration#http_server_options-certificates).
...
While SQL support for Tyk products does not depend on specific database features, with this release, we will provide official support for [PostgreSQL DB for production purposes](/nightly/tyk-configuration-reference/sql), and SQLite for development and PoC environments. Note that SQL support is available for self-managed setups only.
...
[MongoDB to SQL migration docs](/nightly/tyk-configuration-reference/sql#migrating-tyk-dashboard-from-mongodb-to-sql)
...
Check our DCR docs [here](/nightly/tyk-developer-portal/tyk-portal-classic/dynamic-client-registration)
...
See our updated [Tutorials](/nightly/tyk-self-managed) section.
...
See updated tutorials on how to [create a policy](/nightly/api-management/gateway-config-managing-classic#secure-an-api) and [keys](/nightly/api-management/gateway-config-managing-classic#access-an-api)
...
See [updated flow details](/nightly/tyk-identity-broker/overview)
...
[Read More](/nightly/planning-for-production/ensure-high-availability/load-balancing#load-balancing)
...
[Read More](/nightly/api-management/tyk-pump#configuring-tyk-pump)
...
If you are upgrading from version 2.8, please [read this guide](/nightly/developer-support/release-notes/archived#upgrading-from-version-2-8)
