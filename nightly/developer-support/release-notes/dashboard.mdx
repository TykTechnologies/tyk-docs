...
This release introduces [Tyk Governance](/nightly/tyk-governance/overview), a new capability built directly into the Dashboard for defining API standards and continuously checking your Tyk OAS APIs against them, catching design, security, and compliance issues before an API reaches production instead of after.

Platform teams can create [rulesets](/nightly/tyk-governance/rulesets) from a [ready-to-use template](/nightly/tyk-governance/ruleset-templates) or their own definition, [apply them to APIs via categories](/nightly/tyk-governance/scoping-rulesets), and see results directly on a new Governance tab for every API, complete with severity-graded issues and specific [remediation guidance](/nightly/tyk-governance/remediate-issues). Compliance status is also surfaced across the API list and on the Dashboard home page, and the same checks can be [run from a CI/CD pipeline](/nightly/tyk-governance/cicd-checks) before an API is ever imported.
...
Tyk Dashboard now supports [IAM-based authentication](/nightly/tyk-configuration-reference/redis-cluster-sentinel#iam-authentication) for Google Cloud Memorystore for Redis Cluster and Valkey, as an alternative to a static password.
...
If you are upgrading to 5.15.0, please follow the detailed [upgrade instructions](#upgrading-tyk).
...
For more details see [Managing MCP proxies using the Dashboard](/nightly/ai-management/mcp-gateway/managing-proxies).
</Accordion>
...
- **[Create rulesets](/nightly/tyk-governance/rulesets)** — a set of governance rules — either from a [ready-to-use template](/nightly/tyk-governance/ruleset-templates) (an OWASP API Security ruleset, or a general OpenAPI design-best-practices ruleset), or from your own definition by uploading or pasting a JSON or YAML ruleset file.
- **[Apply rulesets via API categories](/nightly/tyk-governance/scoping-rulesets)** — link a ruleset to one or more of your existing API categories, and every API carrying that category is automatically governed by it; an API can be governed by more than one ruleset at once.
...
- **[CI/CD governance checks](/nightly/tyk-governance/cicd-checks)** — run the same evaluation from a pipeline (GitHub Actions, GitLab CI, Jenkins, or any tool that can call an HTTP endpoint) directly against an OpenAPI document, before the API is even imported into Tyk.
...
In this release, Tyk Governance supports Tyk OAS APIs only. Tyk Governance is disabled by default — contact your Tyk account team to enable it for your organization. For more information, see [Tyk Governance](/nightly/tyk-governance/overview).
</Accordion>
...
- choose Tyk Gateway's new [JavaScript plugin engine](/nightly/api-management/plugins/javascript#choosing-a-javascript-engine) by selecting the `JavaScript` driver
- when using JavaScript plugins, provide your [plugin code inline](/nightly/api-management/plugins/overview#inline-code-javascript-only) as an alternative to giving a path to a file containing the code
- assign JavaScript plugins to [any plugin hook](/nightly/api-management/plugins/overview#plugin-hook-types)
- assign [multiple plugin bundles](/nightly/api-management/plugins/overview#composing-multiple-bundles) to an API with a simple comma separated list
- include [analytics plugins](/nightly/api-management/plugins/plugin-types#loading-from-a-bundle) in the custom plugin bundle

For more details see [Custom Plugins](/nightly/api-management/plugins/overview).
</Accordion>
...
The IAM configuration is set in Tyk Dashboard's new top-level `redis_iam_auth` configuration block.

For more details see [IAM Authentication](/nightly/tyk-configuration-reference/redis-cluster-sentinel#iam-authentication).
</Accordion>
...
For more details see [Upstream OAuth 2.0](/nightly/api-management/upstream-authentication/oauth#authentication-method).
</Accordion>
...
A full CRUD API has been implemented in Tyk Dashboard for creating and managing Client IdP assets. See the [changelog](#Changelog-v5.14.0) for the endpoint reference.
...
Tyk Dashboard 5.13.0 adds comprehensive support for the MCP (Model Context Protocol) Gateway. A dedicated `/api/mcps` endpoint set provides full CRUD operations for MCP Proxy definitions. See [MCP API extensions](/nightly/ai-management/mcp-gateway/mcp-api-extensions).

- A new `mcp` RBAC permission group (deny/read/write) controls access to MCP management independently of the existing `apis` permission. Visibility is controlled solely by the `mcp` RBAC permission. See [Permissions](/nightly/ai-management/mcp-gateway/managing-proxies#permissions).
- Sessions (Keys) and Policies can now carry four new MCP-specific access-right fields for configuring tool-based access control and per-primitive rate limiting. See [MCP Gateway Policies](/nightly/ai-management/mcp-gateway/policies).
...
For details, see [MCP API extensions](/nightly/ai-management/mcp-gateway/mcp-api-extensions).
</Accordion>
...
For details, see [Permissions](/nightly/ai-management/mcp-gateway/managing-proxies#permissions).
</Accordion>
...
This release builds on the recent release of [Tyk 5.8.3](/nightly/developer-support/release-notes/dashboard#5-8-3-release-notes), adding a collection of new capabilities. For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v5.9.0).
