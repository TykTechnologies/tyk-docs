...
In this release we've deprecated the [policies.allow_explicit_policy_id](/nightly/tyk-oss-gateway/configuration#policies-allow-explicit-policy-id) configuration option. This was previously added to allow the use of custom policy IDs, which is now the default behaviour so this option is redundant.
...
In this release we've deprecated the [allow_explicit_policy_id](/nightly/tyk-dashboard/configuration#allow-explicit-policy-id) configuration option. This was previously added to allow the use of custom policy IDs, which is now the default behaviour so this option is redundant.
