...
This release adds [Secrets Management](/nightly/tyk-configuration-reference/kv-store) to the Tyk Pump configuration file for the first time, letting you reference values from external secrets stores in place of plain-text values.

Tyk Pump now supports [IAM-based authentication](/nightly/tyk-configuration-reference/redis-cluster-sentinel#iam-authentication) for Google Cloud Memorystore for Redis Cluster and Valkey, as an alternative to a static password.

We have also fixed two issues affecting the [Kinesis pump](/nightly/api-management/logs/external-data-sinks#kinesis) and deprecated the legacy `PMP_MONGO*` environment variable format in lieu of the standard `TYK_PMP_*` format (see [Deprecations](#deprecations) for the full list).
...
| Legacy (deprecated) | Use for [standard Mongo pump](/nightly/api-management/dashboard-analytics/control-plane-pumps#standard-mongo-pump) | Use for [Mongo uptime pump](/nightly/planning-for-production/ensure-high-availability/uptime-tests#mongodb) |
|---|---|---|
...
**Mitigation:** A new `legacy` option has been added to the [`log_format`](/nightly/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#log_format) configuration. This setting preserves the format used prior to Pump 1.16.0, with the historical timestamp format and the `msg` field name.
...
The [`log_format`](/nightly/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#log_format) configuration option now accepts three explicit values:
...
Added support for server-side encryption in the Kinesis Pump by introducing a new configuration option [TYK_PMP_PUMPS_KINESIS_META_KMSKEYID](/nightly/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#pumps-kinesis-meta-kmskeyid) that allows users to specify an AWS KMS key for encrypting data at rest in Kinesis Data Streams.
...
Added a new configuration option [TYK_PMP_PUMPS_KAFKA_META_BATCHBYTES](/nightly/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#pumps-kafka-meta-batch_bytes) to the Kafka pump that allows users to configure the maximum size (in bytes) of a batch before it is sent to a Kafka partition. 
