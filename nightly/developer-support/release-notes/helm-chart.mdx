...
#### Downloads
- [Source code](https://github.com/TykTechnologies/tyk-charts/archive/refs/tags/v5.1.1.tar.gz)
...
Resolved an issue where the Redis Sentinel password ([TYK_GW_STORAGE_SENTINELPASSWORD](/nightly/tyk-oss-gateway/configuration#storagesentinel_password)) was not correctly picked up by Helm charts. Previously, both Redis and Sentinel passwords referenced the same secret key, leading to the Sentinel password defaulting to the regular Redis password. This fix ensures correct handling of distinct Sentinel passwords.
</Accordion>
</AccordionGroup>
...
Allow users to configure worker gateway to work with [Tyk MDCB synchroniser](/nightly/api-management/mdcb#synchroniser-feature-with-mdcb) easily by setting `global.mdcbSynchronizer.enabled` in `tyk-data-plane`.
