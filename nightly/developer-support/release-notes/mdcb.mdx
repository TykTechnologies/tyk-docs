---
title: "Tyk MDCB Release Notes"
description: "Release notes documenting updates, enhancements and changes for Tyk Multi Data Center Bridge (MDCB)."
keywords: "release notes, MDCB, Tyk Multi Data Center, changelog"
sidebarTitle: "Tyk MDCB"
canonical: "https://tyk.io/docs/developer-support/release-notes/mdcb"
---

Licensed Protected Product

**This page contains all release notes for Tyk MDCB displayed in reverse chronological order**

## Support Lifetime
Only the most recent Tyk MDCB release is supported. We recommend staying current to minimise exposure to security vulnerabilities from third party dependencies.
...
This release adds [Secrets Management](/nightly/tyk-configuration-reference/kv-store) to the Tyk MDCB configuration file for the first time, letting you reference values from external secrets stores in place of plain-text values.

MDCB now supports IAM-based authentication for Google Cloud Memorystore for Redis Cluster and Valkey, as an alternative to a static password.

The RPC connection pool is now configurable to limit both the maximum number of active connections from Data Plane Gateways and maximum connection idle time.

For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v2.13.0).

#### Breaking Changes
This release has no breaking changes.

#### Dependencies
<a id="dependencies-2.13.0"></a>
...
If you are upgrading to 2.13.0, please follow the detailed [upgrade instructions](/nightly/#upgrading-tyk).

#### Downloads
- [Docker image v2.13.0](https://hub.docker.com/r/tykio/tyk-mdcb-docker/tags?page=&page_size=&ordering=&name=v2.13.0)
  - ```bash
    docker pull tykio/tyk-mdcb-docker:v2.13.0
    ```
#### Changelog
<a id="Changelog-v2.13.0" data-scroll-offset></a>
...
{/* TT-17868 */}
Tyk MDCB has been updated to build with Go 1.26, keeping the toolchain current with upstream Go security and performance updates.
</Accordion>
</AccordionGroup>
...
{/* TT-16268, TT-17699 */}
Tyk MDCB now supports Secrets Management, letting you reference values from external secrets stores: the Tyk config file, environment variables, local files (including Kubernetes-mounted secrets), Consul, Vault, AWS Secrets Manager, Azure Key Vault, and GCP Secret Manager, in place of plain-text values in MDCB's configuration, using a consistent reference syntax:
...
References are resolved once, at startup. Individual secrets stores can be marked as `required` and, if they cannot be reached or initialized, MDCB will fail to start with a clear error rather than starting with that store silently unavailable.
...
{/* TT-17611 */}
Tyk MDCB can now authenticate to Google Cloud Memorystore for Redis/Valkey using GCP IAM-based authentication, as an alternative to a static Redis password.

MDCB obtains a Google OAuth2 access token from the workload's identity, using Application Default Credentials (ADC) and authenticates with the Memorystore using this token.
...
<Accordion title='Configurable RPC connection pool'>
{/* TT-17967 */}
New configuration options have been introduced to manage the behavior of RPC connections from Data Plane Gateways.

- `max_connections` sets the maximum number of concurrent RPC connections MDCB will accept from Data Plane Gateways. 
- `max_connection_idle_time` sets the maximum time an idle RPC connection is kept open before MDCB closes it; this is configured with a readable duration (e.g. "5m").

For both settings, the default (`0` or omitted) means unlimited - this is the existing behavior to avoid any breaking change.
</Accordion>
</AccordionGroup>
...
In this release, we have added support for the Client Identity Provider (IdP) registry, introduced in [Tyk 5.14.0](/nightly/developer-support/release-notes/dashboard#5-14-0-release-notes). This feature provides centralized management of IdPs and scope-to-policy mappings used with [JWT Auth](/nightly/basic-config-and-security/security/authentication-authorization/json-web-tokens). Adding support to Tyk MDCB allows the use of the new Client IdP resource in distributed deployments.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v2.12.0).

#### Breaking Changes
This release has no breaking changes.

#### Dependencies
<a id="dependencies-2.12.0"></a>
...
#### Upgrade instructions
If you are upgrading to 2.12.0, please follow the detailed [upgrade instructions](/nightly/#upgrading-tyk).

#### Downloads
- [Docker image v2.12.0](https://hub.docker.com/r/tykio/tyk-mdcb-docker/tags?page=&page_size=&ordering=&name=v2.12.0)
  - ```bash
    docker pull tykio/tyk-mdcb-docker:v2.12.0
    ```
#### Changelog
...
For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v2.11.0).

#### Breaking Changes
This release has no breaking changes.

#### Dependencies
<a id="dependencies-2.11.0"></a>
...
#### Upgrade instructions
If you are upgrading to 2.11.0, please follow the detailed [upgrade instructions](/nightly/#upgrading-tyk).

For distributed deployments, deploy MDCB 2.11.0 before upgrading Tyk Pump to 1.15.0 and Tyk Gateway to 5.13.0. MCP analytics support is additive and does not affect existing Pump or Gateway versions until they are also upgraded. See [MCP Gateway upgrade considerations](/nightly/developer-support/upgrading#mcp-upgrade-considerations) for the full deployment order.

#### Downloads
- [Docker image v2.11.0](https://hub.docker.com/r/tykio/tyk-mdcb-docker/tags?page=&page_size=&ordering=&name=v2.11.0)
  - ```bash
    docker pull tykio/tyk-mdcb-docker:v2.11.0
    ```
#### Changelog
...
No additional MDCB configuration is required. For details, see the [MCP analytics documentation](/nightly/ai-management/mcp-gateway/mcp-observability).
</Accordion>
...
For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v2.10.0).

#### Breaking Changes
This release has no breaking changes.

#### Dependencies
<a id="dependencies-2.10.0"></a>
...
#### Upgrade instructions
If you are upgrading to 2.10.0, please follow the detailed [upgrade instructions](/nightly/#upgrading-tyk).

#### Downloads
- [Docker image v2.10.0](https://hub.docker.com/r/tykio/tyk-mdcb-docker/tags?page=&page_size=&ordering=&name=v2.10.0)
  - ```bash
    docker pull tykio/tyk-mdcb-docker:v2.10.0
    ```
#### Changelog
...
The [`omit_analytics_index_creation`](/nightly/tyk-multi-data-centre/mdcb-configuration-options#omit_analytics_index_creation) configuration option is used to prevent deadlocks in horizontally scaled deployments that could occur if multiple MDCB instances created the same index concurrently.
When this option is enabled, MDCB does not [automatically create indexes](/nightly/api-management/dashboard-analytics/control-plane-pumps#standard-mongo-pump), eliminating database contention issues, while maintaining full analytics functionality.
...
This release resolves an issue that caused MDCB memory usage to climb sharply every time an API was updated and never recover, with each further update pushing it higher until the process could run out of memory. Deployments with many Tyk OAS APIs, frequent Dashboard updates, or a large number of connected Data Plane Gateways were most exposed. This release also fixes high-priority CVEs. 

For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v2.9.1).

#### Breaking Changes
This release has no breaking changes.

#### Dependencies
<a id="dependencies-2.9.1"></a>
...
#### Upgrade instructions
If you are upgrading to 2.9.1, please follow the detailed [upgrade instructions](/nightly/#upgrading-tyk).

#### Downloads
- [Docker image v2.9.1](https://hub.docker.com/r/tykio/tyk-mdcb-docker/tags?page=&page_size=&ordering=&name=v2.9.1)
  - ```bash
    docker pull tykio/tyk-mdcb-docker:v2.9.1
    ```
#### Changelog
<a id="Changelog-v2.9.1" data-scroll-offset></a>

##### Fixed

<AccordionGroup>

<Accordion title='Fix unbounded memory growth and out-of-memory crashes on API updates'>

Resolved an issue where MDCB memory usage rose sharply each time an API was updated from the Dashboard and was never released, with every subsequent update driving it higher until MDCB could exhaust available memory and crash. Deployments with a large number of Tyk OAS APIs, a busy Dashboard, or many connected Data Plane Gateways were most affected.
MDCB now releases memory between updates and only reprocesses what has actually changed. As a result, memory usage stays stable and substantially lower under sustained load.
</Accordion>
...
For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v2.9.0).

#### Breaking Changes
This release has no breaking changes.

#### Dependencies
<a id="dependencies-2.9.0"></a>
...
#### Upgrade instructions
If you are upgrading to 2.9.0, please follow the detailed [upgrade instructions](/nightly/#upgrading-tyk).

#### Downloads
- [Docker image v2.9.0](https://hub.docker.com/r/tykio/tyk-mdcb-docker/tags?page=&page_size=&ordering=&name=v2.9.0)
  - ```bash
    docker pull tykio/tyk-mdcb-docker:v2.9.0
    ```
#### Changelog
...
In this release, we have fixed some priority CVEs. For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v2.8.8). 

#### Breaking Changes
This release has no breaking changes.

#### Dependencies
<a id="dependencies-2.8.8"></a>
...
#### Upgrade instructions
If you are upgrading to 2.8.8, please follow the detailed [upgrade instructions](/nightly/#upgrading-tyk).

#### Downloads
- [Docker image v2.8.8](https://hub.docker.com/r/tykio/tyk-mdcb-docker/tags?page=&page_size=&ordering=&name=v2.8.8)
  - ```bash
    docker pull tykio/tyk-mdcb-docker:v2.8.8
    ```
#### Changelog
...
In this patch release, we've resolved MongoDB connection leaks, and faster gateway recovery after Redis outages. For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v2.8.7).

#### Breaking Changes
This release has no breaking changes.

#### Dependencies
<a id="dependencies-2.8.7"></a>
...
#### Upgrade instructions
If you are upgrading to 2.8.7, please follow the detailed [upgrade instructions](/nightly/#upgrading-tyk).

#### Downloads
- [Docker image v2.8.7](https://hub.docker.com/r/tykio/tyk-mdcb-docker/tags?page=&page_size=&ordering=&name=v2.8.7)
  - ```bash
    docker pull tykio/tyk-mdcb-docker:v2.8.7
    ```
#### Changelog
...
In this patch release, we fixed high-priority CVEs. For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v2.8.6).

#### Breaking Changes
This release has no breaking changes.

#### Dependencies
<a id="dependencies-2.8.6"></a>
...
#### Upgrade instructions
If you are upgrading to 2.8.6, please follow the detailed [upgrade instructions](/nightly/#upgrading-tyk).

#### Downloads
- [Docker image v2.8.6](https://hub.docker.com/r/tykio/tyk-mdcb-docker/tags?page=&page_size=&ordering=&name=v2.8.6)
  - ```bash
    docker pull tykio/tyk-mdcb-docker:v2.8.6
    ```
#### Changelog
...
For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v2.8.5).

#### Breaking Changes
This release has no breaking changes.

#### Dependencies
<a id="dependencies-2.8.5"></a>
...
#### Upgrade instructions
If you are upgrading to 2.8.5, please follow the detailed [upgrade instructions](/nightly/#upgrading-tyk).
...
For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v2.8.4).
...
If you are upgrading to 2.8.4, please follow the detailed [upgrade instructions](/nightly/#upgrading-tyk).
...
- [Docker image v2.8.4](https://hub.docker.com/r/tykio/tyk-mdcb-docker/tags?page=&page_size=&ordering=&name=v2.8.4)
  - ```bash
    ...
    ```
#### Changelog
<a id="Changelog-v2.8.4" data-scroll-offset></a>
...
This release updates MDCB to maintain compatibility with recent changes to the [URL Rewrite Middleware schema](/nightly/developer-support/release-notes/gateway#release-highlights-1), ensuring seamless interoperability with the latest Gateway and Dashboard versions.

For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v2.8.3).
...
#### Upgrade instructions
If you are upgrading to 2.8.3, please follow the detailed [upgrade instructions](/nightly/#upgrading-tyk).
...
- [Docker image v2.8.3](https://hub.docker.com/r/tykio/tyk-mdcb-docker/tags?page=&page_size=&ordering=&name=v2.8.3)
  - ```bash
    ...
    ```
#### Changelog
<a id="Changelog-v2.8.3" data-scroll-offset></a>

##### Changed



<Expandable title='MDCB Compatibility Update for URL Rewrite Middleware Changes'>
We have updated MDCB to align with the recent [URL Rewrite Middleware schema](/nightly/developer-support/release-notes/gateway#release-highlights-1). This update ensures that MDCB remains fully compatible and behaves consistently with the latest Gateway and Dashboard releases.
</Expandable>
...
#### Upgrade instructions
If you are upgrading to 2.8.2, please follow the detailed [upgrade instructions](/nightly/#upgrading-tyk).
...
<Expandable title='Valkey Database Compatibility'>
...
#### Upgrade instructions
If you are upgrading to 2.8.1, please follow the detailed [upgrade instructions](/nightly/#upgrading-tyk).
...
For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v2.8.0) below.
...
- [Docker image v2.8.0](https://hub.docker.com/r/tykio/tyk-mdcb-docker/tags?page=&page_size=&ordering=&name=v2.8.0)
  - ```bash
    ...
    ```
#### Changelog
<a id="Changelog-v2.8.0" data-scroll-offset></a>
...
- Migrate to new [health check](/nightly/api-management/mdcb#health-check) endpoints in order to get more detailed information. For Kubernetes users, use Helm Charts v1.6 to upgrade MDCB to set liveness and readiness probes of MDCB deployment to the new health check endpoints.
...
Two new [health check](/nightly/api-management/mdcb#health-check) endpoints have been added to improve monitoring and diagnostics:
...
Two new `/config` and `/env` [endpoints](/nightly/api-management/mdcb#check-mdcb-configurations) have been implemented, allowing developers to access the current configuration state of the MDCB instance in real-time. This feature provides:
...
Please refer to the [changelog](/nightly/#Changelog-v2.7.0) below.
...
#### Changelog
<a id="Changelog-v2.7.0" data-scroll-offset></a>
...
Please refer to the [changelog](/nightly/#Changelog-v2.6.0) below.
...
#### Changelog
<a id="Changelog-v2.6.0" data-scroll-offset></a>
...
#### Release Highlights
This release contains bug fixes as detailed in the [changelog](/nightly/#Changelog-v2.5.1) below.
...
#### Changelog
<a id="Changelog-v2.5.1" data-scroll-offset></a>
...
Please refer to the [changelog](/nightly/#Changelog-v2.5.0) below.
...
#### Changelog
<a id="Changelog-v2.5.0" data-scroll-offset></a>
...
[MongoDB supported versions](/nightly/planning-for-production/database-settings#supported-versions) 
...
#### Release Highlights
This release enhances compatibility as detailed in the [changelog](/nightly/#Changelog-v2.4.2) below.
...
#### Changelog
<a id="Changelog-v2.4.2" data-scroll-offset></a>
...
#### Release Highlights
This release enhances compatibility as detailed in the [changelog](/nightly/#Changelog-v2.4.1) below.
...
#### Changelog
<a id="Changelog-v2.4.1" data-scroll-offset></a>
...
#### Release Highlights
MDCB 2.4.0 is an update for compatibility for synchronisation with Tyk v5.2 API Definitions. It also enables gateway information visualisation on Tyk Dashboard v5.2+. Please refer to the [changelog](/nightly/#Changelog-v2.4.0) below.
...
#### Changelog
<a id="Changelog-v2.4.0" data-scroll-offset></a>
...
#### Added
- Added a new configuration option [enable_ownership](/nightly/tyk-multi-data-centre/mdcb-configuration-options#enable_ownership) that allows MDCB filter APIs by API Ownership. 
