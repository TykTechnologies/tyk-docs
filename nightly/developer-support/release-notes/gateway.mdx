...
Only the most recent Tyk Gateway release and the most recent on the [Long Term Support (LTS)](/nightly/developer-support/release-types/long-term-support) branch are supported. We recommend staying current on the feature or LTS branch to minimise exposure to security vulnerabilities from third party dependencies.
...
Tyk Gateway can now turn an existing Tyk OAS API into a fully functional [MCP proxy](/nightly/ai-management/mcps/api-to-mcp), so REST APIs already registered with Tyk can be exposed to AI agents as MCP tools without rebuilding them. This release adds the catalogue and [configuration foundation](/nightly/ai-management/mcp-gateway/api-to-mcp-definitions) for defining MCP proxies, REST-to-MCP protocol translation for tool calls, session lifecycle and Gateway runtime support, and [policy enforcement](/nightly/ai-management/mcp-gateway/policies) over MCP proxy traffic.

**Enhanced Secrets Management**
 
This release also introduces an enhanced [Secrets Management](/nightly/tyk-configuration-reference/kv-store) approach, replacing the [legacy Key-Value reference syntax](/nightly/tyk-configuration-reference/kv-store-legacy) for most use cases.
...
For Enterprise Edition users, the new system adds support for new [providers](/nightly/tyk-configuration-reference/kv-store-providers): AWS Secrets Manager, Azure Key Vault, and GCP Secret Manager.
...
RFC 8693 Token Exchange now supports the [Microsoft Entra On-Behalf-Of (OBO)](/nightly/api-management/authentication/token-exchange-microsoft-entra-obo) grant, allowing Tyk Gateway to exchange an inbound client token for a downstream-scoped token issued by Microsoft Entra ID before forwarding the request upstream.
Token Exchange also gains [improved observability](/nightly/api-management/authentication/token-exchange#observability) into the exchange process (Enterprise Edition).

**Enhanced experience with JavaScript custom plugins**
 
We've added a new [JavaScript plugin engine](/nightly/api-management/plugins/javascript#choosing-a-javascript-engine) that uses the Goja Golang JavaScript runtime. This is available as a new `driver` option and provides significant improvement in JS plugin performance. We've also extended the usability of JS plugins so that they can be assigned to [all plugin hooks](/nightly/api-management/plugins/overview#plugin-hook-types). 
 
 **Next-generation (-ng) plugin compiler with native arm64 support**
 
A new next-generation (`-ng`) [plugin compiler](/nightly/api-management/plugins/golang/plugin-compiler) is now available, as an alternative to the existing compiler, with a significantly reduced the OS-level CVE surface compared to the current compiler.

This compiler adds [native linux/arm64 support](/nightly/api-management/plugins/golang/plugin-compiler#native-arm64-support). Plugins built on Apple Silicon or other arm64 hosts no longer need to run the compiler under emulation.

**IAM-based authentication for Google Cloud Memorystore**

Tyk Gateway now supports [IAM-based authentication](/nightly/tyk-configuration-reference/redis-cluster-sentinel#iam-authentication) for Google Cloud Memorystore for Redis Cluster and Valkey, as an alternative to a static password.
...
- Tyk OAS APIs can be configured as MCP proxies, giving each API a catalogue entry and [configuration foundation](/nightly/ai-management/mcp-gateway/api-to-mcp-definitions) that describes it as a set of MCP tools, resources, and prompts.
...
- [Access policies](/nightly/ai-management/mcp-gateway/policies) can be enforced against MCP proxy traffic, so the same policy-based access controls used for REST APIs apply consistently to the MCP tools, resources, and prompts an MCP proxy exposes.

For more details see [REST API to MCP](/nightly/ai-management/mcps/api-to-mcp).
</Accordion>
...
For more details see [Microsoft Entra ID On-Behalf-Of](/nightly/api-management/authentication/token-exchange-microsoft-entra-obo).
</Accordion>
...
For more details see [Token Exchange Observability](/nightly/api-management/authentication/token-exchange#observability).
</Accordion>
...
For more details see [Next-Generation Plugin Compiler](/nightly/api-management/plugins/golang/plugin-compiler) and [migrating from the existing compiler](/nightly/api-management/plugins/golang/plugin-compiler#migrating-from-the-existing-compiler).
</Accordion>
...
For more details see [Analytics Plugins](/nightly/api-management/plugins/plugin-types#loading-from-a-bundle).
</Accordion>
...
JavaScript plugins can also now be attached to [post-auth and response hooks](/nightly/api-management/plugins/overview#plugin-hook-types) increasing the viability of JS as the plugin language of choice.

The existing `otto` driver remains available, so existing API definitions will be unaffected by these changes.

For more details see [JavaScript Plugins](/nightly/api-management/plugins/javascript#choosing-a-javascript-engine).
</Accordion>
...
The introduction of granular per-endpoint [enforced (request) timeouts](/nightly/planning-for-production/ensure-high-availability/enforced-timeouts) means that the existing controls (`value` for Tyk OAS and `timeout` for Tyk Classic) should now be considered as deprecated.
...
<Note>
This is distinct from [Tyk OAuth 2.0](/nightly/api-management/authentication/oauth-2), in which Tyk acts as the OAuth 2.0 Authorization Server and issues tokens to clients.

It replaces and significantly improves upon the deprecated [External OAuth](/nightly/api-management/client-authentication#integrate-with-external-authorization-server-deprecated) method, which performed limited validation of tokens provided by an external IdP.
</Note>
...
See [OAuth 2.0 with an External IdP](/nightly/api-management/authentication/oauth2-authentication).
</Accordion>
...
See [Protected Resource Metadata](/nightly/api-management/authentication/oauth2-authentication#protected-resource-metadata).
</Accordion>
...
See [Token Exchange](/nightly/api-management/authentication/token-exchange).
</Accordion>

<Accordion title='Local file-based KV/secret references'>
{/* TT-17297 */}
Local filesystem has been added to Tyk Gateway's Key Value secret storage, using the [`file://<key>` and `$secret_file.<key>` notation](/nightly/tyk-configuration-reference/kv-store-legacy#notation). Configure the underlying `file` store under [Local Files](/nightly/tyk-configuration-reference/kv-store-providers#local-files-file). This allows Gateway to resolve KV/secret references from the contents of a local file, such as [Kubernetes Secrets](https://kubernetes.io/docs/concepts/configuration/secret/) mounted on the Data Plane pod.

The base path for all external `file` references must be configured in the Tyk Gateway config as [`kv.file.base_path`](/nightly/tyk-oss-gateway/configuration). This acts as an enable/disable control for external `file` references.
...
{/* TT-12339 */}
You can now configure more granular [enforced (request) timeouts](/nightly/planning-for-production/ensure-high-availability/enforced-timeouts) using human-readable values such as `500ms`, `1.5s`, or `2m`.

Configure the per-endpoint timeout in the Tyk Vendor Extension using the new `duration` field in the per-operation [`enforceTimeout`](/nightly/api-management/gateway-config-tyk-oas#enforcetimeout) object. This accepts human-readable durations in milliseconds, seconds, or minutes (for example `500ms`, `1.5s`, or `2m`).
...
Configure the timeout in the Tyk Vendor Extension using [`upstream.enforceTimeout`](/nightly/api-management/gateway-config-tyk-oas#upstream) using human-readable durations in milliseconds, seconds, or minutes (for example `500ms`, `1.5s`, or `2m`).
...
A new [`graceful_shutdown_delay_seconds`](/nightly/tyk-oss-gateway/configuration) configuration option controls the wait (default: `0` seconds). As soon as a `SIGTERM`, `SIGINT`, or `SIGQUIT` is received, the readiness endpoint (`/ready`) immediately begins to return `503 Service Unavailable`, but the Gateway keeps accepting and serving traffic for the configured delay period. 

Following the delay period, the existing [graceful shutdown](/nightly/planning-for-production/ensure-high-availability/graceful-shutdown#tyk-gateway) process continues.
</Accordion>
...
See [Tool naming and discovery](/nightly/ai-management/mcp-gateway/core-concepts#tool-naming-and-discovery).
</Accordion>
...
{/* TT-4023 */}
When the Gateway control API is moved to a separate port via [`control_api_port`](/nightly/tyk-oss-gateway/configuration#control_api_port), requests to the proxy/data API port that did not match any configured API (generating an `HTTP 404` response) were silently dropped: no log entry was generated even with [`track_404_logs`](/nightly/tyk-oss-gateway/configuration#track_404_logs) enabled. This has been fixed; 404 logs are now correctly emitted for unmatched requests on all listening ports.
</Accordion>
...
- [`regexp_cache_max_entries`](/nightly/tyk-oss-gateway/configuration)  caps the regular-expression cache (default: `5000`)
- [`disable_regexp_cache_bound`](/nightly/tyk-oss-gateway/configuration) restores the previous unbounded behavior for deployments with a naturally limited set of patterns (default:`false`)
- [`disable_auto_max_procs`](/nightly/tyk-oss-gateway/configuration) restores the previous default behavior of GOMAXPROCS (default:`false`)
</Accordion>
...
See [Distributed tracing](/nightly/ai-management/mcp-gateway/mcp-observability#distributed-tracing).
</Accordion>
...
**Mitigation:** A new `legacy` option has been added to the [`log_format`](/nightly/tyk-oss-gateway/configuration#log_format) configuration. This setting preserves the format used in Tyk Gateway 5.13.0 and earlier, with the historical timestamp format and lack of a field identifying for the originating API.
...
Additionally, a `host` field has been added to 404 log entries (when using `text` or `json` [`log_format`](/nightly/tyk-oss-gateway/configuration#log_format)). This records the Host header from the request, allowing users to distinguish whether the unmatched request was directed at the proxy API or the control API.
</Accordion>
...
- [`regexp_cache_max_entries`](/nightly/tyk-oss-gateway/configuration)  caps the regular-expression cache (default: `5000`)
- [`disable_regexp_cache_bound`](/nightly/tyk-oss-gateway/configuration) restores the previous unbounded behaviour for deployments with a naturally limited set of patterns (default:`false`)
- [`disable_auto_max_procs`](/nightly/tyk-oss-gateway/configuration) restores the previous default behavior of GOMAXPROCS (default:`false`)
</Accordion>
...
The MCP Gateway is not a new component - the capabilities are generally available in this release of Tyk Gateway. See [Tyk MCP Gateway](/nightly/ai-management/mcp-gateway/overview).
...
Per-primitive rate limiting sets independent rate limits for each method or named primitive within a Session or Policy. When multiple Policies apply, access lists merge as a union and rate limits use the most permissive value. See [MCP Gateway Policies](/nightly/ai-management/mcp-gateway/policies).

MCP traffic appears in Gateway access logs with dedicated fields (`mcp_method`, `mcp_primitive_type`, `mcp_primitive_name`) alongside a new `api_type` field present on all access log entries. See [MCP Gateway Access Logs](/nightly/ai-management/mcp-gateway/mcp-access-logs).

OpenTelemetry custom metrics expose the same four MCP dimensions, including `mcp_error_code`, aligned with the OpenTelemetry `mcp.*` semantic conventions. See [MCP Gateway Metrics](/nightly/ai-management/mcp-gateway/mcp-metrics).
...
You must set [http_server_options.max_version](/nightly/tyk-oss-gateway/configuration#http_server_options-max_version) (or the equivalent environment variable)  to `771` if you require an upper limit of TLS 1.2.
...
**Configuration:** Error overrides use a map-based structure indexed by HTTP status code in the new [error_overrides](/nightly/tyk-oss-gateway/configuration#error_overrides) configuration.
...
This release adds a new Gateway configuration option [rate_limit_response_headers](/nightly/tyk-oss-gateway/configuration#rate_limit_response_headers). When set to "rate_limits", the `X-RateLimit-*` headers are populated from rate-limit data instead of quota data:
...
To change the maximum TLS version, you must explicitly set [TYK_GW_HTTPSERVEROPTIONS_MAXVERSION](/nightly/tyk-oss-gateway/configuration#http_server_options-max_version) for client-to-Gateway connections or [TYK_GW_PROXYSSLMAXVERSION](/nightly/tyk-oss-gateway/configuration#proxy_ssl_max_version) for Gateway-to-upstream connections.

To change the minimum TLS version, you must explicitly set [TYK_GW_HTTPSERVEROPTIONS_MINVERSION](/nightly/tyk-oss-gateway/configuration#http_server_options-min_version) for client-to-Gateway connections or [TYK_GW_PROXYSSLMINVERSION](/nightly/tyk-oss-gateway/configuration#proxy_ssl_min_version) for Gateway-to-upstream connections.
...
This release builds on the recent release of [Tyk 5.8.3](/nightly/developer-support/release-notes/gateway#5-8-3-release-notes), adding a collection of new capabilities. For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v5.9.0).
...
**Mitigation:** A new `legacy` option has been added to the [`log_format`](/nightly/tyk-oss-gateway/configuration#log_format) configuration. This setting preserves the format used prior to Tyk Gateway 5.8.15, with the historical timestamp format and lack of a field identifying for the originating API.
...
For deployments that do not use custom Go plugins, this is a build-time dependency change only and does not change Tyk's official FIPS readiness target for the 5.8.x release line, which remains FIPS 140-2.
 ...
 If you build [custom Go plugins](/nightly/api-management/plugins/golang) for use with `tyk-gateway-fips`, you must rebuild them with a `tyk-plugin-compiler-fips` image for the matching Gateway version: plugins compiled against an older plugin compiler image are not binary-compatible with `tyk-gateway-fips:v5.8.15` and will fail to load. 
...
Fixed an issue where an [enforced timeout](/nightly/planning-for-production/ensure-high-availability/enforced-timeouts) set for a specific API endpoint could be overruled by the configured [proxy_default_timeout](/nightly/tyk-oss-gateway/configuration#proxy_default_timeout). Now if an endpoint-level timeout is set then this will be honoured, regardless of any default timeout that is configured.
</Accordion>
...
The [Logstash](/nightly/api-management/logs#log-output) formatter timestamp is now in [RFC3339Nano](https://www.rfc-editor.org/rfc/rfc3339) format.
</Accordion>
...
Fixed a bug where the Gateway did not correctly close idle upstream connections (sockets) when configured to generate a new connection after a configurable period of time (using the [max_conn_time](/nightly/tyk-oss-gateway/configuration#max_conn_time) configuration option). This could lead to the Gateway eventually running out of sockets under heavy load, impacting performance.
</Accordion>
...
Added configuration parameter to enable/disable [detailed_tracing](/nightly/api-management/logs/traffic-logs#detailed-recording) for *Tyk Classic API*.
</Accordion>
