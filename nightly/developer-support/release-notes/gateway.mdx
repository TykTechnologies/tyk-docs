...
You must set [http_server_options.max_version](/nightly/tyk-oss-gateway/configuration#http_server_options-max_version) (or the equivalent environment variable)  to `771` if you require an upper limit of TLS 1.2.
  ...
  See [here](/nightly/api-management/implement-tls#controlling-tls-version-&-cipher-suites) for details how to control TLS version and cipher suites.
...
Compression is opt-in per asset type via independent configuration flags: [`storage.enable_api_definition_compression`](/nightly/tyk-oss-gateway/configuration#storage-enable_api_definition_compression) (introduced in 5.12.0) for API definitions, and the new [`storage.enable_policy_compression`](/nightly/tyk-oss-gateway/configuration#storage-enable_policy_compression) for security policies. Both are disabled by default and remain backward compatible — Gateways transparently load both compressed and uncompressed data from Redis.
...
We offer support for integrating *OpenTelemetry* traces with supported open source tools such [Jaeger](/nightly/api-management/traces#using-docker), [Dynatrace](/nightly/api-management/traces#dynatrace) or [New Relic](/nightly/api-management/traces#new-relic). This allows API owners and developers to gain troubleshooting and performance insights from error logs, response times etc.
