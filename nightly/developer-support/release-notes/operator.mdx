...
- **`TykSession`**: Client access control and tracking [Session](/nightly/api-management/access-control/sessions-and-keys/understanding-sessions).
...
- **`TykIdentityProfile`**: Tyk Identity Broker (TIB) profile for [Dashboard Single Sign-On](/nightly/tyk-identity-broker/dashboard-sso).
- **`TykOAuthClient`**: OAuth 2.0 client used with [Tyk OAuth 2.0](/nightly/api-management/authentication/oauth-2) authentication method.
- **`TykWebhook`**:  [Webhook handler](/nightly/api-management/gateway-events#event-handling-with-webhooks) for Gateway events.
- **`TykUserGroup`**: Dashboard [user groups](/nightly/platform-management/user-groups)
...
- [Docker image v1.5.0](https://hub.docker.com/r/tykio/tyk-operator/tags?page=&page_size=&ordering=&name=v1.5.0)
  - ```bash
    ...
    ```
- Helm chart
  - [tyk-charts v5.4.0](/nightly/developer-support/release-notes/helm-chart#5-4-release-notes)
...
For more information, visit [Governance via Operator](/nightly/tyk-governance/tyk-operator)
</Accordion>
...
[Policies](/nightly/api-management/policies) associated with the Session (declared using the existing `SecurityPolicy` CRD) are referenced by their Kubernetes `metadata.name`, consistent with the Operator's existing cross-resource reference model, so no raw Tyk IDs are needed in the manifest.
</Accordion>
 
<Accordion title='Added TykCertificate CRD for declarative TLS certificate management'>
{/* TT-17501 */}
A new `TykCertificate` custom resource uploads a TLS certificate from a same-namespace Kubernetes Secret into [Tyk Certificate Store](/nightly/api-management/certificates#tyk-certificate-store). The resulting certificate ID, fingerprint, and expiry are exposed on `.status`.
...
- [Docker image v1.4.2](https://hub.docker.com/r/tykio/tyk-operator/tags?page=&page_size=&ordering=&name=v1.4.2)
  - ```bash
    ...
    ```
- Helm chart
  - [tyk-charts v5.3.0](/nightly/developer-support/release-notes/helm-chart#5-3-release-notes)
...
- [graphql.proxy.auth_headers](/nightly/api-management/graphql#creating-a-graphql-api-via-the-dashboard-ui)
- [graphql.proxy.subscription_type](/nightly/api-management/graphql#graphql-subscriptions)
- [graphql.proxy.request_headers](/nightly/api-management/graphql#request-headers)
