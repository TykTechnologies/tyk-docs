...
Tyk Operator 1.4.0 introduces support for managing [MCP (Model Context Protocol) Proxy](/nightly/ai-management/mcp-gateway/managing-mcp-proxies) definitions through Kubernetes custom resources. The new `TykMcpProxyDefinition` CRD allows you to manage MCP Proxies declaratively, and the `SecurityPolicy` CRD has been extended with MCP access rights for tool-based access control and per-primitive rate limiting.
