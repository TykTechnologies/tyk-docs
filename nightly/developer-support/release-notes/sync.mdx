...
keywords: "Tyk Sync, Release notes, changelog"
sidebarTitle: "Tyk Sync"
canonical: "https://tyk.io/docs/developer-support/release-notes/sync"
---

**Licensed Protected Product**

**This page contains all release notes for Tyk Sync displayed in reverse chronological order**

## Support Lifetime
Only the most recent Tyk Sync release is supported. We recommend staying current to minimise exposure to security vulnerabilities from third party dependencies.
...
This patch release updates the Go toolchain used to build Tyk Sync to Go 1.26 and addresses reported CVEs in dependent libraries.

For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v2.2.2) below.
...
For users currently on v2.2.0, we strongly recommend promptly upgrading to the latest release. If you are working with an older version, it is advisable to bypass version 2.2.0 and proceed directly to this latest patch release.

Go to the [Upgrading Tyk](/nightly/#upgrading-tyk) section for detailed upgrade Instructions.

#### Downloads
- [Docker image v2.2.2](https://hub.docker.com/r/tykio/tyk-sync/tags?page=&page_size=&ordering=-name&name=v2.2.2)
  - ```bash
    docker pull tykio/tyk-sync:v2.2.2
    ```

#### Changelog
<a id="Changelog-v2.2.2" data-scroll-offset></a>
...
{/* TT-17868 */}
Tyk Sync has been updated to build with Go 1.26, keeping the toolchain current with upstream Go security and performance updates.
</Accordion>
...
In this release, we have added support for the Client Identity Provider (IdP) registry, introduced in [Tyk 5.14.0](/nightly/developer-support/release-notes/dashboard#5-14-0-release-notes). This feature provides centralized management of IdPs and scope-to-policy mappings used with [JWT Auth](/nightly/basic-config-and-security/security/authentication-authorization/json-web-tokens). Adding support to Tyk Sync allows GitOps management of the new Client IdP resource.

For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v2.2.0) below.
...
#### Upgrade instructions
For users currently on v2.1.8, we strongly recommend promptly upgrading to the latest release. If you are working with an older version, it is advisable to bypass version 2.1.8 and proceed directly to this latest release.

Go to the [Upgrading Tyk](/nightly/#upgrading-tyk) section for detailed upgrade Instructions.

#### Downloads
- [Docker image v2.2.0](https://hub.docker.com/r/tykio/tyk-sync/tags?page=&page_size=&ordering=-name&name=v2.2.0)
  - ```bash
    docker pull tykio/tyk-sync:v2.2.0
    ```
...
<AccordionGroup>
<Accordion title='Add Client IdP resource support to dump, publish, and sync operations'>
Tyk Sync can now manage Client IdP registry entries as a first-class resource, alongside APIs, policies, and other assets.

- `tyk-sync dump` now exports each Client IdP registry resource to its own `clientidp-{id}.json` file and records them under a new `client_idps` section in `.tyk.json`, with an optional `--idps` flag to export a specific subset.
- `tyk-sync publish` (and `update`) recreates them on Tyk Dashboard.
- `tyk-sync sync` reconciles the full create, update, and delete lifecycle.

Required fields (`org_id`, `name`, and `jwks_uri`) are validated before any request is sent, and Tyk Dashboard versions that do not support the Client IdP registry are handled gracefully so existing workflows continue to run unchanged.
</Accordion>
...
<AccordionGroup>
<Accordion title='Fix MCP policy fields being omitted during policy dump'>
Resolved an issue where policies using Model Context Protocol (MCP) access control and rate limiting were not fully exported by `tyk-sync dump`, causing the `mcp_primitives`, `mcp_access_rights`, `json_rpc_methods`, and `json_rpc_methods_access_rights` fields to be silently dropped from the dumped definition.

Tyk Sync now preserves these MCP and JSON-RPC method fields, so policies can be dumped and synced without losing their MCP usage limits or primitive-based access rules.
...
#### Release Highlights
This release refreshes the embedded Tyk Classic, Tyk OAS API, and Policy definitions so that all fields introduced in Tyk Gateway 5.13.0 are correctly transferred during sync operations. It also ensures compatibility with the most recent Tyk LTS release [5.8.14](/nightly/developer-support/release-notes/dashboard#5-8-14-release-notes).

For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v2.1.8) below.
...
#### Upgrade instructions
For users currently on v2.1.7, we strongly recommend promptly upgrading to the latest release. If you are working with an older version, it is advisable to bypass version 2.1.7 and proceed directly to this latest patch release.
<br/>
Go to the [Upgrading Tyk](/nightly/#upgrading-tyk) section for detailed upgrade Instructions.

#### Downloads
- [Docker image v2.1.8](https://hub.docker.com/r/tykio/tyk-sync/tags?page=&page_size=&ordering=-name&name=v2.1.8)
  - ```bash
    docker pull tykio/tyk-sync:v2.1.8
    ```
...
#### Release Highlights
Tyk Sync 2.1.7 has updated the Golang version to 1.25, fixed CVEs and a performance issue, and ensures compatibility with the most recent Tyk LTS release [5.8.13](/nightly/developer-support/release-notes/dashboard#5-8-13-release-notes).

For a comprehensive list of changes, please refer to the detailed [changelog](/nightly/#Changelog-v2.1.7) below.
...
<Accordion title='MCP Proxy support'>
Tyk Sync recognises [MCP proxies](/nightly/ai-management/mcp-gateway/managing-proxies) as a distinct resource type.

The `dump` and `sync` commands have been extended to handle MCP proxies:

- Dump writes MCP proxies to `mcp-{id}.json` files.
- Command output reports a separate count for MCP proxies.
- Create, update, and delete operations are routed to the dedicated `/api/mcps` endpoint.

No changes to your existing workflow are required — MCP proxies are handled using the same `tyk-sync` commands.
</Accordion>
...
#### Release Highlights
Tyk Sync 2.1.6 is a version alignment release that ensures compatibility with the most recent Tyk LTS release [5.8.12](/nightly/developer-support/release-notes/dashboard#5-8-12-release-notes).
...
#### Downloads
- [Docker image v2.1.6](https://hub.docker.com/r/tykio/tyk-sync/tags?page=&page_size=&ordering=-name&name=v2.1.6)
  - ```bash
    docker pull tykio/tyk-sync:v2.1.6
    ```

#### Changelog
<a id="Changelog-v2.1.6" data-scroll-offset></a>
...
#### Release Highlights
Tyk Sync 2.1.5 is a version alignment release that ensures compatibility with the most recent Tyk LTS release [5.8.9](/nightly/developer-support/release-notes/dashboard#5-8-9-release-notes). It contains one bug fix.
...
#### Changelog
<a id="Changelog-v2.1.5" data-scroll-offset></a>
...
#### Release Highlights
Tyk Sync 2.1.4 is a version alignment release that ensures compatibility with the most recent Tyk LTS release [5.8.6](/nightly/developer-support/release-notes/dashboard#5-8-6-release-notes). No functional changes have been implemented in this release.
...
#### Changelog
<a id="Changelog-v2.1.4" data-scroll-offset></a>
...
#### Changelog
<a id="Changelog-v2.1.3" data-scroll-offset></a>
...
#### Changelog
<a id="Changelog-v2.1.2" data-scroll-offset></a>
...
#### Changelog
<a id="Changelog-v2.1.1" data-scroll-offset></a>
...
#### Changelog
<a id="Changelog-v2.0.4" data-scroll-offset></a>
...
#### Changelog
<a id="Changelog-v2.0.1" data-scroll-offset></a>
...
#### Changelog
<a id="Changelog-v1.4.3" data-scroll-offset></a>
