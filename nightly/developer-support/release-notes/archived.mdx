...
See our [Dashboard SSO documentation](/nightly/api-management/dashboard-configuration#single-sign-on-api) for more details.
...
See [Debugging Tab](/nightly/api-management/dashboard-configuration#debugging) for more information.
...
There is a [new section](/nightly/api-management/dashboard-configuration#dashboard-audit-logs) in the Tyk Dashboard config file where you can specify parameters for the audit log (containing audit records for all requests made to all endpoints under the `/api` route).
...
Instead of setting permissions per user, you can now [create a user group](/nightly/api-management/user-management#manage-tyk-dashboard-user-groups), and assign it to multiple users. It works for Single Sign-On too, just specify group ID during [SSO API](/nightly/api-management/dashboard-configuration#single-sign-on-api-1) flow.
...
From an API standpoint, user groups can be managed by [new Dashboard API](/nightly/api-management/dashboard-configuration#user-groups-api). The User object now has a new `group_id` field, and if it is specified, all permissions will be inherited from the specified group. [SSO API](/nightly/api-management/dashboard-configuration#single-sign-on-api-1) has been updated to include `group_id` field as well.
...
[Docs](/nightly/api-management/logs-metrics#statsd-deprecated)
...
* [API Docs](/nightly/api-management/dashboard-configuration#manage-apis-api-definition)
...
[Docs](/nightly/api-management/dashboard-configuration#activity-logs)
...
* Set permissions for your users, logged via SSO, via `sso_permission_defaults` in Dashboard config file. [Docs](/nightly/api-management/external-service-integration)
* Setup a login page redirect, using `sso_custom_login_url` and `sso_custom_portal_login_url` Dashboard config options to enable users login using a custom SSO login page. [Docs](/nightly/api-management/external-service-integration)
...
```
[Docs](/nightly/api-management/external-service-integration)
