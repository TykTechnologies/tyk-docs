...
canonical: "https://tyk.io/docs/developer-support/release-notes/archived"
---
...
See our [TCP Proxy Docs](/nightly/key-concepts/tcp-proxy) for more details.
...
See our [JWT Scope docs](/nightly/basic-config-and-security/security/authentication-authorization/json-web-tokens) for more details.
...
See our [Golang plugin documentation](/nightly/api-management/plugins/golang#) for more details.
...
See our [Distributed Tracing documentation](/nightly/api-management/traces) for more details.
...
See our updated [Getting Started](/nightly/tyk-self-managed/install) section and [new CLI documentation](/nightly/tyk-self-managed).
...
See our [Dashboard SSO documentation](https://tyk.io/docs/api-reference/single-sign-on/generate-authentication-token) for more details.

### Importing WSDL APIs

WSDL now is a first class citizen at Tyk. You can take your WSDL definition and simply import to the Dashboard, creating a nice boilerplate for your service. See [Import APIs](/nightly/api-management/gateway-config-managing-classic#import-an-api) for more details.
...
See [Debugging Tab](/nightly/api-management/endpoint-designer#debugging) for more information.
...
There is a [new section](/nightly/api-management/logs/audit-logs) in the Tyk Dashboard config file where you can specify parameters for the audit log (containing audit records for all requests made to all endpoints under the `/api` route).
...
> **TLDR**
> To get benefit or performance improvements ensure that you have `close_connections` set to `false` and set `max_idle_connections_per_host` according to our [production perfomance guide](/nightly/planning-for-production)
...
Instead of setting permissions per user, you can now [create a user group](/nightly/platform-management/user-groups), and assign it to multiple users. It works for Single Sign-On too, just specify group ID during [SSO API](https://tyk.io/docs/api-reference/single-sign-on/generate-authentication-token) flow.
...
From an API standpoint, user groups can be managed by [new Dashboard API](https://tyk.io/docs/api-reference/usergroup/list-user-groups). The User object now has a new `group_id` field, and if it is specified, all permissions will be inherited from the specified group. [SSO API](https://tyk.io/docs/api-reference/single-sign-on/generate-authentication-token) has been updated to include `group_id` field as well.

#### Added SMTP support
Now you can configure the Dashboard to send transactional emails using your SMTP provider. See [Outbound Email Configuration](/nightly/configure/outbound-email-configuration) for details.

#### <a name="upgrade"></a>Upgrading all new Components

For details on upgrading all Tyk versions, see [Upgrading Tyk](/nightly/developer-support/upgrading).
...
See the Keys section of the [Tyk Gateway REST API](/nightly/tyk-gateway-api) Swagger doc for more details.
...
See [List Key Requests](/nightly/tyk-apis/tyk-dashboard-api/manage-key-requests#list-key-requests) for more details.
...
See [Developer Profiles](/nightly/tyk-developer-portal/tyk-portal-classic/developer-profiles#reset-developer-password) for more details.
...
[Docs](/nightly/api-management/logs-metrics#statsd-instrumentation)
...
[Docs](/nightly/api-management/gateway-config-tyk-classic#tyk-classic-api-versioning)
...
[Docs](/nightly/tyk-oss-gateway/configuration#http_server_options)
...
[Docs](/nightly/tyk-oss-gateway/configuration#enable_key_logging)
...
[Docs](/nightly/api-management/certificates)
...
[JSVM Docs](/nightly/api-management/plugins/javascript#using-javascript-with-tyk)
[Plugin Data Structure Docs](/nightly/api-management/plugins/rich-plugins#rich-plugins-data-structures)
...
* [API Docs](https://tyk.io/docs/api-reference/apis/get-list-of-apis)
* [Policy Docs](/nightly/tyk-apis/tyk-dashboard-api/portal-policies)
...
[Docs](/nightly/transform-traffic/url-rewriting)
...
[Docs](/nightly/tyk-oss-gateway/configuration)
...
A major feature of this release is the implementation of Mutual TLS. Now you can protect your APIs by allow listing certificates, idenitfy users based on them, and increase security between Tyk and upstream API. For details, see [Mutual TLS](/nightly/api-management/implement-tls#secure-hosted-apis-with-mtls).
...
[Docs](/nightly/api-management/access-control/policies/applying-policies#partitioned-policies)
...
[Docs](/nightly/api-management/dashboard-analytics#activity-logs)
...
* Enable Tyk Identity Broker to apply LDAP filters to user search [Docs](/nightly/api-management/single-sign-on-ldap)
* Set permissions for your users, logged via SSO, via `sso_permission_defaults` in Dashboard config file. [Docs](/nightly/tyk-identity-broker/overview)
* Setup a login page redirect, using `sso_custom_login_url` and `sso_custom_portal_login_url` Dashboard config options to enable users login using a custom SSO login page. [Docs](/nightly/tyk-identity-broker/overview)
* For those who love to build everything in-house, we have added new API for custom dashboard authentication integrations. [Docs](/nightly/api-management/custom-auth-with-proxy-identity-provider)
...
#### Mutual TLS support
[Docs](/nightly/api-management/implement-tls#secure-hosted-apis-with-mtls)

#### Global API rate limits
[Docs](/nightly/api-management/rate-limit#rate-limiting-layers)
...
[Docs](/nightly/tyk-oss-gateway/configuration#control_api_port)
...
[Docs](/nightly/tyk-oss-gateway/configuration#log_level)
...
[Docs](/nightly/api-management/traffic-transformation/request-body)
...
#### Attaching Multiple Policies to the Keys
[Docs](/nightly/api-management/access-control/policies/applying-policies#applying-multiple-policies)
...
```
[Docs](/nightly/tyk-identity-broker/overview)
...
[Docs](/nightly/api-management/custom-auth-with-proxy-identity-provider)
...
Added support for Mutual TLS, mentioned by Gateway and Dashboard above. See [Docs](/nightly/api-management/implement-tls#secure-hosted-apis-with-mtls)
...
Tyk Identity Broker now fully support LDAP search with complex filters! [Docs](/nightly/api-management/single-sign-on-ldap)
...
Hybrid users should follow the upgrade instructions [here](/nightly/developer-support/upgrading#tyk-upgrade-guides-for-different-deployment-models).
