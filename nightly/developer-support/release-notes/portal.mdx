---
title: "Tyk Developer Portal Release Notes"
description: "Release notes documenting updates, enhancements and changes for Tyk Developer Portal"
keywords: "Developer Portal, Enterprise Developer Portal, EDP, Release notes, changelog"
order: 7
...
**This page contains all release notes for Tyk Developer Portal displayed in reverse chronological order**

## Support Lifetime
Only the most recent Tyk Developer Portal release is supported. We recommend staying current to minimise exposure to security vulnerabilities from third party dependencies.
...
Tyk Developer Portal now provides first-class support for Tyk MCP Proxies. MCP Proxies now sync to Portal automatically, MCP Products can be created, complete with documentation and published in Catalogues. Developers can find and request access to MCP Products in the live catalogue alongside REST, GraphQL and other supported API types, complete with a dedicated MCP badge. See [API Products](/nightly/portal/api-products).

Developers can now explore and test an MCP Proxy directly from its documentation page in Portal, without separate tooling, powered by a new embedded, MCPP-compatible reverse proxy that authenticates using the developer's Portal session. See [Test MCP Tools with the Embedded MCP Inspector](/nightly/portal/mcp-inspector-playground).

For MCP Products protected with OAuth 2.0, the Inspector detects the Protected Resource Metadata (RFC 9728) returned by the Gateway — the resource identifier, authorization servers, supported scopes, and discovery path — and uses it to initiate the OAuth 2.0 exchange. See [Access Protected MCP Proxies with OAuth 2.1](/nightly/portal/mcp-oauth-access).

Portal also provides a way to provision public OAuth 2.0 clients. Dynamic Client Registration now supports public clients that authenticate with PKCE rather than a client secret.

This release adds [Secrets Management](/nightly/tyk-configuration-reference/kv-store) to the Tyk Developer Portal configuration file for the first time, letting you reference values from external secrets stores in place of plain-text values.

For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v1.19.0) below.
...
For users currently on v1.18.0, we strongly recommend promptly upgrading to the latest release.

<a id="upgrade-1.19.0"></a>

<a id="upgrade-1.19.0"></a>

If you are upgrading to 1.19.0, please follow the detailed [upgrade instructions](#upgrading-tyk).

#### Downloads

- [Docker Image to pull](https://hub.docker.com/r/tykio/portal/tags?page=&page_size=&ordering=&name=v1.19.0)
  - ```bash
    docker pull tykio/portal:v1.19.0
    ```
- Helm charts
  - [tyk-charts](/nightly/developer-support/release-notes/helm-chart)

#### Changelog
<a id="Changelog-v1.19.0" data-scroll-offset></a>

##### Changed

<AccordionGroup>

<Accordion title='Renamed "API Products" to "Products" and updated auth-type badge colours'>
{/* TT-17330 */}
References to "API Products" in Portal Admin are now labelled "Products", and the "API's" tab is now labelled "Services". Auth-type badges in the catalogue and admin now use a consistent colour palette across API types.
</Accordion>

<Accordion title='Updated Golang version to 1.26'>
{/* TT-17868 */}
Tyk Portal has been updated to build with Go 1.26, keeping the toolchain current with upstream Go security and performance updates.
</Accordion>
...
<Accordion title='Full support for Tyk MCP Proxies'>
{/* TT-17195, TT-17196 */}
The Portal now synchronizes Tyk MCP Proxies from Tyk Dashboard and supports creation of Products to publish them in the Live Portal, complete with admin provided documentation

See [API Products](/nightly/portal/api-products).
</Accordion>

<Accordion title='Added an embedded reverse proxy for testing MCP servers'>
{/* TT-17197 */}
Portal now includes an embedded, MCPP-compatible reverse proxy so developers can connect to and test MCP servers directly through the Portal. It authenticates using the developer's Portal session, applies per-user rate limiting, and blocks requests to private, loopback, link-local and cloud metadata addresses by default.

See [How the Inspector Connects](/nightly/portal/mcp-inspector-playground#how-the-inspector-connects).
</Accordion>

<Accordion title='Embedded the MCP Inspector on MCP product documentation pages'>
{/* TT-17198 */}
The official MCP Inspector is now embedded directly on MCP Product documentation pages, letting developers explore and test an MCP server's tools, resources and prompts without leaving Portal.

See [Test MCP Tools with the Embedded MCP Inspector](/nightly/portal/mcp-inspector-playground).
</Accordion>

<Accordion title='MCP products now appear in the live developer catalogue'>
{/* TT-17365 */}
MCP Products are now shown in the live Portal catalogue and Product detail pages, with a dedicated MCP badge and a documentation view built for MCP APIs. Products that combine MCP with other API types show a badge and documentation link for each type.

See [View MCP Documentation in the Developer Portal](/nightly/portal/mcp-documentation).
</Accordion>

<Accordion title='Added Protected Resource Metadata support for MCP products'>
{/* TT-17591 */}
Portal now surfaces OAuth 2.0 Protected Resource Metadata (RFC 9728) for MCP products that have it enabled at the Gateway, or indicates that it's discovered at runtime for products using mirror mode.

See [Mirror and Static PRM](/nightly/portal/mcp-oauth-access#mirror-and-static-prm).
</Accordion>

<Accordion title='Added public OAuth client support for Dynamic Client Registration'>
{/* TT-17591 */}
Dynamic Client Registration now supports registering public OAuth clients that authenticate using PKCE instead of a client secret. This capability is available for all API and service types.
</Accordion>

<Accordion title='Secrets Management'>
{/* TT-16268, TT-17701 */}
Tyk Portal now supports Secrets Management, letting you reference values from external secrets stores: the Tyk config file, environment variables, local files (including Kubernetes-mounted secrets), Consul, Vault, AWS Secrets Manager, Azure Key Vault, and GCP Secret Manager, in place of plain-text values in Portal's configuration, using a consistent reference syntax:
...
References are resolved once, at startup. Individual secrets stores can be marked as `required` and, if they cannot be reached or initialized, Portal will fail to start with a clear error rather than starting with that store silently unavailable.

See [Secrets Management](/nightly/tyk-configuration-reference/kv-store) for full details.
 
The --show-configuration CLI flag does not expose the `kv` section: once secret-store references are resolved at startup, the KV store definitions (including credentials) are cleared from the in-memory configuration, so nothing about them appears in the output.
</Accordion>
...
<Accordion title='Fixed case-sensitive handling of user email addresses'>
{/* TT-14811 */}
Tyk Portal previously stored and compared user email addresses case-sensitively, which could prevent password reset emails from being delivered and could result in duplicate accounts for the same address in a different case — for example after an admin invitation, during self-registration, or via SSO.

Email address matching and lookups throughout Portal are now case-insensitive, and newly created or updated accounts store their email address in normalised (lowercase) form. This fix applies without requiring a data migration for existing accounts.
</Accordion>

<Accordion title="Fixed new Product incorrectly inheriting another Product's API documentation">
{/* TT-15912 */}
When creating a new Product via the Portal API, if the Product referenced an API that already had documentation attached to it in a different, unrelated Product, Portal would automatically link that documentation (for example an OpenAPI or GraphQL SDL document) to the new Product, even though it was never submitted in the request. This didn't happen when creating a Product through the Portal Admin UI.

The Portal API and Portal Admin UI now behave consistently: creating or updating a Product only associates documentation that is explicitly submitted for it.
</Accordion>

<Accordion title="Fixed Tyk OAS APIs using OpenAPI 3.1 documents breaking the Products APIs list">
{/* TT-18129 */}
Tyk Portal 1.18 and earlier did not support Tyk OAS APIs based on an OpenAPI 3.1 document. If a Product's APIs included one, the Products edit page's APIs tab showed no APIs at all, because Portal's underlying API definition dependency did not yet support OpenAPI 3.1.

This has been fixed by updating that dependency, so Tyk Portal now works seamlessly with Tyk OAS APIs based on OpenAPI 3.1 documents.
</Accordion>

<Accordion title="Fixed the update user endpoint returning a user's old and new teams">
{/* TT-14700 */}
Updating a user's teams via `PUT /portal-api/users/<user-id>` previously returned both the user's previous and newly assigned teams in the response, and the previous team remained associated with the user in the database even though the request was meant to fully replace it. A subsequent `GET` request on the user showed the same stale team.

The `Teams` field in a `PUT` request now fully replaces the user's existing teams. Both the `PUT` response and any subsequent `GET` request now reflect only the newly applied team(s).
</Accordion>
...
- <a href="https://nvd.nist.gov/vuln/detail/CVE-2026-5160" target="_blank">CVE-2026-5160</a>
...
- <a href="https://nvd.nist.gov/vuln/detail/CVE-2026-42505" target="_blank">CVE-2026-42505</a>
- <a href="https://nvd.nist.gov/vuln/detail/CVE-2026-46600" target="_blank">CVE-2026-46600</a>
- <a href="https://nvd.nist.gov/vuln/detail/CVE-2026-56852" target="_blank">CVE-2026-56852</a>
</Accordion>
...
In this release, we have implemented support for the Client IdP registry introduced in [Tyk 5.14.0](/nightly/developer-support/release-notes/dashboard#5-14-0-release-notes). This feature provides centralized management of IdPs and scope-to-policy mappings used with API Products secured with [JWT Authentication](/nightly/basic-config-and-security/security/authentication-authorization/json-web-tokens), resolving a historical issue with Dynamic Client Registration configuration not being properly cleaned up when associated API Products or Plans are deleted.

<Note>
This feature is available only from Tyk Gateway 5.14.0 and Tyk Dashboard 5.14.0 and must be enabled in the Developer Portal by setting the new configuration option [`TYK_PORTAL_ENABLEIDPREGISTRY`](/nightly/product-stack/tyk-enterprise-developer-portal/deploy/configuration#tyk_portal_enableidpregistry) to `true`.
</Note>
...
For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v1.18.0) below.
...
#### Upgrade instructions
<a id="upgrade-1.18.0"></a>

If you are upgrading to 1.18.0, please follow the detailed [upgrade instructions](#upgrading-tyk).

#### Downloads

- [Docker Image to pull](https://hub.docker.com/r/tykio/portal/tags?page=&page_size=&ordering=&name=v1.18.0)
  - ```bash
    docker pull tykio/portal:v1.18.0
    ```
- Helm charts
  - [tyk-charts](/nightly/developer-support/release-notes/helm-chart)
...
{/* TT-17453 */}
Tyk Developer Portal embeds Tyk Identity Broker. This has been updated to [version 1.7.3](/nightly/developer-support/release-notes/tib#1-7-3-release-notes).
</Accordion>
...
- Tyk Developer Portal version 1.18.0 or later with [`TYK_PORTAL_ENABLEIDPREGISTRY`](/nightly/product-stack/tyk-enterprise-developer-portal/deploy/configuration#tyk_portal_enableidpregistry) set to `true`

If [`TYK_PORTAL_ENABLEIDPREGISTRY`](/nightly/product-stack/tyk-enterprise-developer-portal/deploy/configuration#tyk_portal_enableidpregistry) is not set, then the Portal will continue to manage the Identity Provider details in the API definitions and is unable to remove these if the API Product is deleted.
</Accordion>
...
This release restores the environment variable behaviour that changed in 1.17.1 and 1.17.2, where some `PORTAL_*` variables were no longer recognised. Every legacy `PORTAL_*` variable now works exactly as it did in 1.17.0, and Portal also gains a new `TYK_PORTAL_` prefix with a consistent naming style that matches the convention used across other Tyk components.
...
#### Upgrade instructions
<a id="upgrade-1.17.3"></a>

If you are upgrading to 1.17.3, please follow the detailed [upgrade instructions](#upgrading-tyk).

#### Downloads

- [Docker Image to pull](https://hub.docker.com/r/tykio/portal/tags?page=&page_size=&ordering=&name=v1.17.3)
  - ```bash
    docker pull tykio/portal:v1.17.3
    ```
- Helm charts
  - [tyk-charts](/nightly/developer-support/release-notes/helm-chart)
...
Resolved a regression introduced in 1.17.1 where some configuration environment variables stopped taking effect. The most visible symptom was that `PORTAL_DISABLE_CSRF_CHECK` was silently ignored, causing a "bad request" error when users tried to log in. All legacy `PORTAL_*` variable names now work exactly as they did in 1.17.0. 
...
This release implements a fix to avoid the unintentional visibility of MCP proxy access policies on the Developer Portal.

For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v1.17.2) below.
...
Tyk Dashboard v5.13.0 introduces [MCP Proxy](/nightly/ai-management/mcp-gateway/managing-proxies) support. Products granting access to MCP Proxies are not currently supported from Developer Portal.

With the introduction of MCP Proxy support in the Gateway and Dashboard, however, Policies may be created on the Dashboard that grant access to MCP proxies. Prior to Developer Portal v1.17.2, such Policies would be visible in the Developer Portal and could be inadvertently published to Portal users.

See [MCP Gateway upgrade considerations](/nightly/developer-support/upgrading#mcp-upgrade-considerations) for the full component deployment order.
</Note>

#### Downloads

- [Docker Image to pull](https://hub.docker.com/r/tykio/portal/tags?page=&page_size=&ordering=&name=v1.17.2)
  - ```bash
    docker pull tykio/portal:v1.17.2
    ```
- Helm charts
  - [tyk-charts](/nightly/developer-support/release-notes/helm-chart)

#### Changelog
<a id="Changelog-v1.17.2" data-scroll-offset></a>

##### Fixed

<AccordionGroup>

<Accordion title='Avoid accidental exposure of MCP Proxies'>
Tyk Gateway and Dashboard 5.13.0 introduce support for MCP proxies to be deployed and managed on the MCP Gateway.

Developer Portal does not currently support the publication of Products granting access to MCP proxies, but prior to 1.17.2 the MCP proxy access policies created in the Dashboard will automatically be exposed as API Products.

From this release, those policies are no longer exposed on the Developer Portal, avoiding the risk of accidental publication of unsupported API Products in Catalogs.

First-class support for MCP Products will be introduced to the Developer Portal in a subsequent release.
</Accordion>
...
Tyk Developer Portal has been updated to Golang 1.25 and Debian 13 (Trixie) for enhanced security and performance. This patch also resolves some issues with the Portal API and synchronization of assets with the Tyk Dashboard.

For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v1.17.1) below.
...
#### Upgrade instructions
<a id="upgrade-1.17.1"></a>

If you are upgrading to 1.17.1, please follow the detailed [upgrade instructions](#upgrading-tyk).

#### Downloads

- [Docker Image to pull](https://hub.docker.com/r/tykio/portal/tags?page=&page_size=&ordering=&name=v1.17.1)
  - ```bash
    docker pull tykio/portal:v1.17.1
    ```
- Helm charts
  - [tyk-charts](/nightly/developer-support/release-notes/helm-chart)

#### Changelog
<a id="Changelog-v1.17.1" data-scroll-offset></a>

##### Changed

<AccordionGroup>

<Accordion title='Update Developer Portal to Golang 1.25'>
Updated the Tyk Developer Portal to use Golang 1.25, ensuring compatibility with the latest Golang runtime features and reducing exposure to potential security vulnerabilities in older versions.
</Accordion>

<Accordion title='Update base images to Debian 13 (Trixie)'>
Updated the base Docker images for Tyk Portal to Debian 13 (Trixie) to address multiple vulnerabilities in the underlying operating system.
</Accordion>

<Accordion title='MCP Proxies excluded from the Developer Portal'>
MCP Proxies are not surfaced in the Developer Portal as the Portal has no native MCP support.

Full support for MCP Proxies in the Developer Portal will be available in a future release.
</Accordion>
...
<Accordion title='Fixed Portal API Endpoints Returning Incorrect `Content-Type` Headers'>
Resolved an issue where several Portal API endpoints returned incorrect `Content-Type` headers. Previously, endpoints that didn't explicitly write a response body would default to `text/plain` instead of the expected `application/json`, potentially causing client applications to misinterpret API responses.

The fix addresses this issue across all affected Portal API endpoints, including product management endpoints (documentation, logos, previews, OpenAPI specs, GraphQL schemas, tags), user management endpoints, and other Portal API operations. All Portal API endpoints now return the correct `Content-Type` header that matches their actual response content.
</Accordion>

<Accordion title='Fixed Portal API Enum Field Handling for Null Values'>
Resolved an issue where the Portal API returned empty strings (`""`) instead of `null` values for optional enum fields when their values were not set. Previously, endpoints such as `/api/posts` and `/api/catalogues` would incorrectly serialize null enum values as empty strings, violating the API contract and potentially causing client-side parsing issues.

The Portal API now properly omits optional enum fields from responses when they are empty, ensuring consistent and correct API behavior across all endpoints, including posts, catalogues, access requests, and providers.
</Accordion>

<Accordion title='Fixed Plan Field Updates Not Propagating to Dashboard Policies'>
Resolved an issue where updates to certain Plan fields in the Developer Portal were not properly propagating to their corresponding Dashboard Policies. Previously, when administrators updated only the `KeyExpiresIn` (key expiration time) or `MetaData` fields through either the Portal UI or API, these changes would not sync to the Dashboard unless other fields such as rate limits or quotas were modified simultaneously. This occurred due to an incomplete change detection mechanism that omitted these fields from its validation checks. 

The Portal now correctly detects and propagates all plan field changes to Dashboard Policies, ensuring that updates to key expiration settings and metadata are immediately reflected in the Dashboard regardless of whether other fields are modified at the same time.
</Accordion>

<Accordion title='Fixed policy reference inconsistency in API definitions'>
Resolved an issue where the Portal incorrectly used database object IDs (_id) instead of custom Policy IDs (id) when linking policies to APIs through Products or Plans. This caused "policy not found" errors during JWT validation as the Gateway prioritizes custom Policy IDs. The Portal now consistently uses Policy IDs for all scope-to-policy mappings and properly cleans up old mappings during updates.
...
- <a href="https://nvd.nist.gov/vuln/detail/CVE-2026-27587" target="_blank">CVE-2026-27587</a>
...
- <a href="https://nvd.nist.gov/vuln/detail/CVE-2026-33815" target="_blank">CVE-2026-33815</a>
- <a href="https://nvd.nist.gov/vuln/detail/CVE-2026-33816" target="_blank">CVE-2026-33816</a>
...
- <a href="https://nvd.nist.gov/vuln/detail/CVE-2026-33809" target="_blank">CVE-2026-33809</a>
...
Developer Portal 1.17 brings enhanced experience and security when using mTLS secured API Products.

The new Auth Token-Certificate binding option enhances security by preventing the use of mismatched tokens and certificates. When client access to APIs in a Product are secured using Auth Token with mTLS, you can now link specific client certificates to each issued token. Tyk Gateway will only authorise access requests with matched tokens and certificates, mitigating against the risk of token theft.

We've added a new Certificate Management function in the Admin Portal, where you can upload client certificates, retrieve existing certificates from the [Tyk Certificate Store](/nightly/api-management/certificates#tyk-certificate-store), and associate certificates with Organisations.

When an Auth Token ([API Key](/nightly/portal/overview/concepts#access-credentials)) is issued to a Developer App, the appropriate certificate or certificates will automatically be linked to that token to ensure that the API Consumer can access the Product's APIs using a combination of the token with their client certificate.

For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v1.17.0) below.
...
If you are on version 1.16.0 or an older version, we recommend upgrading to this release ASAP.

To upgrade the Portal's theme, please follow the [upgrade instructions](/nightly/portal/customization/themes#upgrading-themes).

#### Download
- [Docker image v1.17.0](https://hub.docker.com/r/tykio/portal/tags?page=&page_size=&ordering=&name=v1.17.0)
  - ```bash
    docker pull tykio/portal:v1.17.0
    ```

- [The default theme package](https://github.com/TykTechnologies/portal-default-theme/releases/tag/1.17.0)

#### Changelog
<a id="Changelog-v1.17.0" data-scroll-offset></a>

##### Added

<AccordionGroup>

<Accordion title='Enhanced Security and Certificate Management with mTLS Secured Products'>
When client access to APIs in a Product is secured using Auth Token with mTLS, you can now link specific client certificates to each issued token. Tyk Gateway will only authorise access requests with matched tokens and certificates, mitigating against the risk of token theft.

To support this, the Admin Portal now has a dedicated certificate management interface to upload new certificates, synchronize them from Tyk Dashboard providers, and associate them with specific Organisations.

API Owners can now enable certificate-token binding on a per-Product basis for APIs using mTLS. When enabled, the system automatically binds the appropriate certificate to an access token upon approval, ensuring that only the correct token-certificate pair can be used to gain API access.

For Managed mTLS Products, the certificates associated with the API Consumer's Organisation will be bound to their token. For Self-Serve mTLS Products, the certificate provided by the API Consumer will be bound to the token.
</Accordion>
...
<Accordion title='Custom Credential Rotation and Revocation'>
The Developer Portal now properly supports rotation and revocation operations for [Tyk Managed Custom Credentials](/nightly/tyk-developer-portal/import-custom-credentials). Custom credentials now support the same lifecycle management as standard Tyk tokens.
</Accordion>

<Accordion title='API Selection Display and Filter Persistence in API Products'>
Fixed two issues affecting API selection when editing API Products:

First, when multiple APIs shared the same name, the Portal would incorrectly display which API was selected, potentially causing data corruption if users saved without noticing the error.

Second, applying search or category filters would cause all pending selections to be lost, forcing users to restart their selection process. Both issues have been resolved, ensuring accurate API selection display and persistent selection state during filtering operations.
</Accordion>

<Accordion title='mTLS Certificate Re-attachment Option Missing After Detachment'>
Fixed an issue where the "Attach Certificate" option could disappear after detaching a certificate from the Developer App, leaving the API Consumer unable to register their client certificate for access to the API Product. Developers can now properly re-attach certificates to their apps.
</Accordion>

<Accordion title='mTLS Certificate Attachment Visibility in App Details'>
Fixed an issue where the option to "Attach Mutual TLS Certificate" section was incorrectly displayed on the App Details page for Managed mTLS Products. This is now only displayed for Self-Serve mTLS Products, allowing the API Consumer to register their client certificate. For Managed mTLS Products, the permitted client certificates are registered by the API Owner in the Admin Portal.
</Accordion>
...
For more information, please visit [Requesting Access to an API Product](/nightly/tyk-developer-portal/direct-access-flow)
</Accordion>
...
For more information, please visit [Credential Management and Reusability](/nightly/tyk-developer-portal/single-credentials-multiple-api-products)
</Accordion>
...
Learn more about managing [API products with GraphQL documentation](/nightly/portal/api-products#api-reference-documentation)
...
We’ve added support for [Tyk Streams APIs](/nightly/api-management/event-driven-apis) in the Developer Portal, allowing API Providers to expose access to event-driven APIs alongside traditional REST APIs.
...
For the complete API specification, see [Tyk EDP API documentation](/nightly/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api).
...
To upgrade the portal's theme, please follow the [upgrade instructions](/nightly/portal/customization/themes#upgrading-themes) for the portal's themes.
...
#### Changelog
<a id="Changelog-v1.14.0" data-scroll-offset></a>
...
#### Changelog
<a id="Changelog-v1.13.2" data-scroll-offset></a>
...
Fixed an issue where middleware configuration for an API could be corrupted (for example, [Request header transform middleware](/nightly/api-management/traffic-transformation#request-header-transform) settings were changed in Developer Portal. This was due to a misalignment between Dashboard and Developer Portal in their interpretation of the API definition and has now been corrected.
</Expandable>
...
#### Changelog
<a id="Changelog-v1.13.1" data-scroll-offset></a>
...
#### Breaking Changes
<a id="breaking-changes-v1.13.0"></a>
...
#### Changelog
<a id="Changelog-v1.13.0" data-scroll-offset></a>
...
#### Changelog
<a id="Changelog-v1.12.0" data-scroll-offset></a>
...
You can read more about the supported SSO providers [here](/nightly/tyk-identity-broker/overview).
</Accordion>
...
- Added new TLS variables to set MinVersion ([portal_tls_min_version](/nightly/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_tls_min_version), MaxVersion ([PORTAL_TLSCONFIG_MAXVERSION](/nightly/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_tls_max_version), and CipherSuites ([PORTAL_TLS_CIPHER_SUITES](/nightly/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_tls_cipher_suites).
- Added a new configuration to manage the idle timeout of the portal's session ([PORTAL_SESSION_IDLE_TIMEOUT](/nightly/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_session_idle_timeout).
...
- Changed the default value of [PORTAL_DATABASE_CONNECTION_MAX_LIFETIME](/nightly/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_database_connection_max_lifetime) to 1800000 milliseconds.
...
- [Webhooks](/nightly/portal/customization) for events that happen in the portal.
- [Admin APIs](/nightly/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) for OAuth2.0 configuration. 
...
#### Changelog
##### Added
- Added [the webhooks](/nightly/portal/customization) capability that  enable real-time, automated data updates between the portal and 3rd party applications.
- Added [admin APIs](/nightly/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) for managing OAuth2.0 configuration. 
...
If you are on 1.8.3 or older version please follow the [upgrade instructions](/nightly/portal/customization/themes#upgrading-themes) to upgrade the portal's themes.
...
#### Changelog
##### Added
- Added [new configuration option](/nightly/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) for setting the `SameSite` attribute on the portal's cookie.
- Added [new welcome email for admin users](/nightly/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) that is sent when new admin account is created.
- Added [new welcome email for developers](/nightly/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) that is sent when new developer account is created.
...
- Changed the credential provisioning flow to automatically include DeveloperID, OrganizationID, ApplicationID, and TeamIDs in [the credential metadata](/nightly/portal/customization/user-model).
- Added warning regarding potential PII exposure to the [custom attributes menu](/nightly/portal/customization/user-model).
...
- Extended the `DELETE /users/:id` API endpoint by adding the [?force](/nightly/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) query parameter to force removal of a user even if some of their credentials have been removed from the credentials provider, or if the provider itself is down or unreachable.
...
- Extended [filesize limit](/nightly/portal/customization/themes#create-a-theme) for individual files in themes to 5 MB.
- Made the organization invite email's subject configurable via [the emails settings section](/nightly/portal/customization/email-notifications#supported-email-notifications).
...
#### Changelog
##### Added
- Added [new admin API](/nightly/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) for managing tags attached to API Products.
- Added [new admin API](/nightly/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) for managing OAuth2.0 client types attached to API Products.
...
##### Added
- Add a [config option](/nightly/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_enable_http_profiler) to expose the Golang profiling information which allows for debugging issues related to resource consumption.
...
- When the portal theme [manifest](/nightly/portal/customization/themes#manifest-file) has a reference to a template that is not present in the theme then the theme won't be loaded. This check that prevents admin users from uploading themes with potential errors was introduced in version [1.7.0](#content-blocks-validation).
...
All settings are available via the [admin API](/nightly/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) and the UI.
...
To set the CORS configuration please refer to the Portal's [configuration documentation](/nightly/product-stack/tyk-enterprise-developer-portal/deploy/configuration#cors-settings).
...
Now when the [PORTAL_DCR_LOG_ENABLED](/nightly/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_dcr_log_enabled) environment variable is set to `true`, the portal will output not only the status and status code of the request to the IdP, but also actual payload returned by the IdP: 
```yaml
...
- When [PORTAL_DCR_LOG_ENABLED](/nightly/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_dcr_log_enabled) is enabled the portal now returns not only the status and status code of the request to the IdP but also actual payload returned by the IdP
...
- Fixed the bug where the [Forgot Password page](/nightly/tyk-developer-portal/tyk-enterprise-developer-portal/api-consumer-portal/reset-password) did not reflect the current theme.
...
- Simplified the connection settings to the portal assets storage (where all images, themes, and other CMS files are stored) to help our customers get up to speed quicker. We are well aware that installing and configuring on-premise software can be tricky, especially when it comes to infrastructure, storage and databases. Hence, we have decided to ease this burden for you:
  - By default, the portal uses the `db` [storage type](/nightly/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_storage) for storing its themes and other CMS assets and it doesn't require any additional configuration. This means, you can start the portal right away without specifying any additional setting for the assets storage.
...
The Tyk Enterprise Developer Portal is now available in Kubernetes and customer can launch it using our [helm charts](/nightly/portal/install/kubernetes#legacy-helm-chart). This feature makes the portal Kubernetes friendly by adding liveness, readiness probes, graceful shutdown and changing the portal lifecycle so that it's possible to set an initial user and bootstrap the portal via APIs.

##### SSO for API Consumers and admins
API Providers can [configure Single Sign-on](/nightly/tyk-stack/tyk-developer-portal/enterprise-developer-portal/managing-access/enable-sso) for the Enterprise developer portal so that it's possible to login developers and admins to the portal user 3rd party IdP.
...
##### Add TLS support
This feature enables API Provides to secure the portal with [HTTPs](/nightly/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_tls_enable).

##### Add enhanced logging configuration
This new setting allows API Providers to set the logging [level](/nightly/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_log_level) and [format](/nightly/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_log_format). This offers API Providers more control over the logging behavior of their APIs.
...
##### Added
- Added Kubernetes support and [helm charts](/nightly/portal/install/kubernetes#legacy-helm-chart).
- Added [Single Sign-on](/nightly/tyk-stack/tyk-developer-portal/enterprise-developer-portal/managing-access/enable-sso) for API Consumers and admin users so that they can use their IdPs for managing admin users and developers.
...
- Added [TLS](/nightly/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_tls_enable) support for the portal's UI.
...
Now API Consumers can [create organizations](/nightly/tyk-stack/tyk-developer-portal/enterprise-developer-portal/managing-access/manage-api-consumer-organisations) and securely share credentials between their teammates. In greater detail:
...
##### Get started guides
API Providers can add [Get started guides](/nightly/portal/api-products#documentation-only-products) to API Products for better developer experiences:
...
API Providers can select which blogs posts to display on an API Product page using [the tags feature](/nightly/portal/api-products#documentation-only-products). To achieve that, an API Provider can specify tags for both API Products and blog posts. Blog posts that match tags with an API Product are displayed in the 'Related blog content' section in the API Product page. This offers API Providers greater control over what blog posts to display on their API Product page.

##### S3 support
We added [S3 support](/nightly/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_storage) for the portal assets storage (themes, images, OAS files). This update enhances the extensibility of our platform, allowing you to choose different storage solutions to better align with your specific needs.
...
##### Added
- Added the [organization management capability](/nightly/tyk-stack/tyk-developer-portal/enterprise-developer-portal/managing-access/manage-api-consumer-organisations) for API Consumers to safely share API access credentials between team members.
- Added the [Get started guides](/nightly/portal/api-products#documentation-only-products) for API Products so that admins can explain to their consumers how use their API Products.
- Added support for [S3 storage](/nightly/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_storage) for the portal's assets storage. Now our customers can use `s3` storage in addition to the filesystem which is especially important in Kubernetes environments.
- Added [tags](/nightly/portal/api-products#documentation-only-products) for API Products and blog posts so that API Providers have greater control over which blog posts to display on their API Product page.
