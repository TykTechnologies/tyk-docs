...
Tyk Developer Portal now provides first-class support for Tyk MCP Proxies. MCP Proxies now sync to Portal automatically, MCP Products can be created, complete with documentation and published in Catalogues. Developers can find and request access to MCP Products in the live catalogue alongside REST, GraphQL and other supported API types, complete with a dedicated MCP badge.

Developers can now explore and test an MCP Proxy directly from its documentation page in Portal, without separate tooling, powered by a new embedded, MCPP-compatible reverse proxy that authenticates using the developer's Portal session.

For MCP Products protected with OAuth 2.0, the Inspector detects the Protected Resource Metadata (RFC 9728) returned by the Gateway — the resource identifier, authorization servers, supported scopes, and discovery path — and uses it to initiate the OAuth 2.0 exchange.
