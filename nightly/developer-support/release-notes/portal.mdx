...
When viewing a documentation-only product, API specifications whose alias contains a hyphen (-) may not appear correctly in the API Specifications tab. This does not affect runtime functionality, or API Product docs in the Catalogue. We recommend not using a hyphen in the specification alias of a documentation-only product. A fix will be included in next release.
