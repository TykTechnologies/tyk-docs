...
keywords: "configuration files backup, backup tyk, tyk.conf, upgrade tyk, database backup"
sidebarTitle: "Upgrading Guide"
canonical: "https://tyk.io/docs/developer-support/upgrading"
---
...
- **Long Term Support:** Refer to our [versioning and long-term support policies](/nightly/developer-support/release-types/long-term-support) for details on major and minor releases, patches, and support dates.
- **Preparations:** Review the [preparation guidelines](/nightly/#upgrade-guidelines) before starting the upgrade.
- **Release notes:** Always check the "Upgrade Instructions" section in the relevant release notes.
- **Backups:** Follow our [comprehensive backup guide](/nightly/developer-support/faq#tyk-configuration) before starting the upgrade.
...
| :---------------------- | :---------------- | :----------- |
| **Tyk Cloud**        | [Cloud SaaS](/nightly/#tyk-cloud) | Guide to Tyk Cloud SaaS |
|                      | [Hybrid](/nightly/#tyk-hybrid) | Guide for Hybrid environments with Gateway Data Plane(s) deployed locally or with a third-party cloud provider |
|                      | [Go plugin](/nightly/#go-plugin) | Guide for upgrading Go plugin on the Tyk Cloud |
| **Tyk Self-Managed** | [RHEL and CentOS](/nightly/#upgrade-linux-redhat-centos) | Guide for RPM-based Linux distributions |
|                      | [Debian and Ubuntu](/nightly/#upgrade-linux-debian) | Guide for DEB-based Linux distributions |
|                      | [Docker](/nightly/#docker) | Guide for upgrading Docker images |
|                      | [Helm](/nightly/#helm) | Guide for upgrading Helm Charts |
|                      | [Kubernetes](/nightly/#kubernetes) | Guide for upgrading Kubernetes environment |
| **Tyk MDCB Self-Managed** | [MDCB](/nightly/#upgrade-mdcb) | Guide for upgrading Multi Data Center Bridge (MDCB) |
| **Tyk Open-Source**  | [Tyk Gateway](/nightly/#open-source-upgrade-guide) | Guide for upgrading Tyk open-source environment |
...
Use our [migration tool](/nightly/planning-for-production/database-settings#migrating-from-an-existing-mongodb-instance) to manage the switch from MongoDB to SQL.

##### Backup APIs Script

Utilize our bash [backup script](/nightly/developer-support/upgrading#backup-apis-and-policies) to export and restore all Tyk API Definitions and Policies.
...
Which strategy do you intend to use?
  ...
  - If following the [Blue-Green upgrade](/nightly/#blue-green-upgrade) strategy, has the green environment been configured and verified as production-ready?
  - If pursuing the [Rolling upgrade](/nightly/#rolling-upgrade) strategy, do all Tyk components have a second instance?
...
Have backups been performed?
  ...
  - Backup guide: Have you checked our [comprehensive guide for backing up Tyk](/nightly/developer-support/faq#tyk-configuration)?

#### Go plugins
Do you use custom Go plugins with your APIs?

  - Go plugin must be [recompiled](/nightly/#go-plugins-upgrade-guide) for the new version.
...
Use the [Upgrade Guides ToC](/nightly/#tyk-upgrade-guides-for-different-deployment-models) to choose the appropriate upgrade guide for
your platform.
...
3. **Upgrade Process:**
   ...
   - If using a new deployment process, follow the [deployment instructions](/nightly/tyk-self-managed) appropriate for your platform.
4. **Switch Environments:** Once the upgrade is complete, switch the traffic to the upgraded environment.

<Warning>
**Use the Same License Across All Tyk Dashboard Instances**

During rolling and blue-green upgrades, you can temporarily run more than one Tyk Dashboard instance against the same Redis and database backend. Every Dashboard instance connected to a shared Redis or database **must use the same license key**.

A Tyk Dashboard license defines the pool of gateway nodes that the Dashboard is allowed to register. If two instances share the same Redis but run different licenses, one Dashboard can reject gateways that were registered against the other license. Gateway registration then fails with an HTTP 404 response, and the Dashboard logs `level=error msg="GetSpareNode returned node not allowed by this dashboard's license"`. As a result, gateways cannot register, which causes 404 responses and prevents API creation.

Before you upgrade, confirm that the old and new Dashboard instances use the same license, and apply any license change to every instance at the same time. For more detail, see [Connecting Multiple Gateways to a Single Dashboard](/nightly/planning-for-production#connecting-multiple-gateways-to-a-single-dashboard) and [Troubleshooting](/nightly/api-management/troubleshooting-debugging#dashboard).
</Warning>
...
the [upgrade guidelines](/nightly/#upgrade-guidelines).
...
Follow our guide for [upgrading Cloud Control Planes](/nightly/tyk-cloud/environments-deployments/managing-control-planes#upgrade-cloud-control-planes).

**Step 2. Upgrade Go Plugins**

Follow our guide for deploying your [Go plugins on Tyk Cloud](/nightly/#go-plugin). Subsequently, follow the steps below according to the target upgrade version of the Gateway.

**Gateway Versions < 4.1.0.**

1. Proceed with [upgrading your Tyk Data Plane (Gateway)](/nightly/#upgrading-cloud-data-planes)
2. Update the [custom_middleware_bundle](/nightly/api-management/plugins/overview#plugin-bundles) field in the API Definitions of all APIs that use your plugin. The field should be updated to use the new bundle file containing your upgrade plugin.
3. Validate that your plugin is working per your expectations.

**Gateway Versions >= 4.1.0**

1. Update the [custom_middleware_bundle](/nightly/api-management/plugins/overview#plugin-bundles) field in the API Definitions of all APIs that use your plugin. The field should be updated to use the new bundle file containing your upgraded plugin.
...
3. Proceed with [upgrading your Tyk Data Plane (Gateway)](/nightly/#upgrading-cloud-data-planes). Given that you loaded your target version plugin in step 1, this version will be loaded automatically once you upgrade.
...
<a id="upgrading-cloud-data-planes"></a>

Follow our guide for [upgrading Cloud Data Planes](/nightly/tyk-cloud/environments-deployments/managing-gateways).
...
After following the guidelines for [preparing for upgrade](/nightly/#upgrade-guidelines),
...
See Tyk Guide for how to [upgrade Control Planes](/nightly/tyk-cloud/environments-deployments/managing-control-planes#upgrade-cloud-control-planes)

**Step 2 Upgrade Go plugins**

Follow our guide for [upgrading your Go plugins on Tyk Cloud](/nightly/#go-plugin). Subsequently, follow the steps below according to the target upgrade version of the Gateway.

**Gateway Versions < 4.1.0**

1. Proceed with upgrading your [Tyk Data Plane Hybrid Gateways](/nightly/#upgrading-data-plane-hybrid-gateways).
2. Update the [custom_middleware_bundle](/nightly/api-management/plugins/overview#plugin-bundles)
...
1. Update the [custom_middleware_bundle](/nightly/api-management/plugins/overview#plugin-bundles)
field in the API Definitions of all APIs that use your plugin. The field should be updated to use the new bundle file
...
3. Proceed with upgrading your [Tyk Data Plane Hybrid Gateways](/nightly/#upgrading-data-plane-hybrid-gateways). Given that you
...
<a id="upgrading-data-plane-hybrid-gateways"></a>

Follow the instructions for the component deployment type:

- [Docker](/nightly/#docker)
- [Helm](/nightly/#helm)
- [Linux Debian](/nightly/#upgrade-linux-debian)
- [Linux RHEL/CENTOS](/nightly/#upgrade-linux-redhat-centos)
...
2. [Compile your custom Go plugins](/nightly/#go-plugins-upgrade-guide).
...
| :--- | :--- | :--- |
| [Path 1](/nightly/#path-1)    | < 4.1.0 | < 4.1.0 |
| [Path 2](/nightly/#path-2)    | >= 4.1.0 | >= 4.2.0 |

**Path 1 - Current Version < 4.1.0 and Target Version < 4.1.0**

1. [Create a plugin bundle](/nightly/api-management/plugins/overview#plugin-bundles) that includes the newly compiled version
...
2. [Upload this bundle](/nightly/tyk-cloud/using-plugins#uploading-your-bundle) to your configured S3 bucket if using Cloud SaaS. If you're using Hybrid SaaS, upload this bundle to your bundle server.


**Path 2 - Current `Version >= 4`.1.0 and Target `Version >= 4`.2.0**

1. [Create a plugin bundle](/nightly/api-management/plugins/overview#plugin-bundles) that includes both your current version’s plugin along with the newly compiled version
...
the addition of the [Multi Data Center Bridge](/nightly/api-management/mdcb) (MDCB).
...
###### MCP Gateway (v5.13+)
<a id="mcp-upgrade-considerations"></a>

Tyk v5.13 introduces MCP Gateway, which adds specific ordering requirements on top of the standard upgrade sequence above. These apply only to deployments that will use MCP Gateway functionality.

**Self-managed (no MDCB)**

No additional ordering requirements. Tyk Pump writes MCP analytics directly to your configured storage backend (MongoDB or SQL). The standard Dashboard → Gateway → Pump upgrade order applies.

**Distributed deployments**

The standard MDCB-first ordering applies with one additional requirement: the `enable_mcp_aggregation` flag on Tyk Pump must remain `false` during the upgrade and only be enabled once all components are on the new version.

This flag tells the Hybrid Pump to send MCP analytics to MDCB using the new `PurgeAnalyticsDataMCPAggregated` RPC. If you enable it before MDCB is running the new version, the RPC calls will fail and MCP analytics records will be silently dropped.

The correct sequence for upgrading distributed deployments is:

1. Upgrade MDCB.
2. Upgrade Tyk Pump — with `enable_mcp_aggregation` still set to `false`.
3. Upgrade Tyk Gateway and Tyk Dashboard.
4. Only now set `enable_mcp_aggregation: true` on every Hybrid Pump and restart.

<Warning>
Do not set `enable_mcp_aggregation: true` before MDCB is running the new version. Doing so in a distributed deployment will cause RPC failures and silent loss of MCP analytics data.
</Warning>

The `enable_mcp_aggregation` flag is **not required** for single-DC self-managed deployments (no MDCB in the analytics path) or for Tyk Cloud (no customer-side Pump).

**Tyk Operator**

If you use [Tyk Operator](/nightly/tyk-stack/tyk-operator/installing-tyk-operator) to manage API definitions via `TykMCPProxyDefinition` custom resources, deploy the new Operator version only *after* Tyk Dashboard and Tyk Gateway are running the new version. The Operator controller calls the Dashboard `/api/mcps` endpoint, which only exists from v5.13 onwards. Deploying the Operator first will cause reconciliation failures on all MCP custom resources.

**Tyk Developer Portal**

If your deployment includes the [Tyk Developer Portal](/nightly/portal/overview/getting-started), upgrade Portal before or at the same time as Tyk Dashboard, and definitely before creating any MCP Proxy policies in the Dashboard. From v1.17.2, Portal prevents MCP Proxy policies from appearing as API Products in the Developer Portal so that they cannot accidentally be exposed in Catalogs. An older Portal version running against a new Dashboard with MCP Proxy policies configured will surface those policies which could then be exposed to API Consumers who will not be able to consume them.

**Tyk Cloud**

Tyk Cloud does not run a customer-side Pump — MCP analytics are written directly by the Gateway and MDCB to the managed store. The deployment order is:

1. MDCB
2. Tyk Gateway and Tyk Dashboard
3. Tyk Operator (if used) — after Dashboard is running
4. Tyk Developer Portal — before MCP Proxy policies are created in Dashboard

There is no `enable_mcp_aggregation` flag to configure in a Cloud deployment.
...
Follow our [Tyk OSS guide](/nightly/#go-plugins-upgrade-guide) for upgrading Tyk Gateway and Tyk Pump
...
For production environments, it is recommended to use [Helm charts](/nightly/#helm) to manage upgrades with zero downtime.

When upgrading Kubernetes Deployments, you should use a `RollingUpdate` strategy to ensure zero downtime. For more information on upgrade strategies, see the [Upgrade Strategies](/nightly/#upgrade-strategies) section.
...
After reviewing guidelines for [preparing for upgrade](/nightly/#upgrade-guidelines),
follow the instructions below to upgrade your Tyk components and plugins.

**Upgrade order:**

Please note that upgrade order is as explained in the upgrade [overview](/nightly/#tyk-self-managed-upgrade-guide)
...
Before executing the upgrade, ensure that you have consulted and performed all the necessary steps in the [pre-upgrade checklist](/nightly/#upgrade-guidelines).
...
#### Linux - RedHat (CentOS)
<a id="upgrade-linux-redhat-centos"></a>
...
the [self-managed](/nightly/#tyk-self-managed-upgrade-guide) section, and look for the instruction regarding Tyk Gateway.

Before proceeding with the upgrade process, ensure that you have thoroughly reviewed and completed the steps outlined in
the [pre-upgrade guidelines](/nightly/#upgrade-guidelines). Once you
...
For full installation details, check the usual [installation page](/nightly/apim/open-source/installation).
...
2. Update the image version in the `docker-compose.yaml` file.
   <br />
   For example, Tyk Gateway version `4.3.3` is defined in your `docker-compose.yaml` as `image: docker.tyk.io/tyk-gateway/tyk-gateway:v4.3.3`. Change `4.3.3` to the version you want to use.
...
3. Review and complete the steps outlined in the [pre-upgrade guidelines](/nightly/#upgrade-guidelines).
...
| :----------------- | :----------------- | :---------------- |
| [Path 1](/nightly/#path-1)    | < 4.1.0         | < 4.1.0        |
| [Path 2](/nightly/#path-2)    | < 4.1.0         | >= 4.1.0       |
| [Path 3](/nightly/#path-3)    | >= 4.1.0        | >= 5.1.0       |
...
If you are using Self-Managed deployment then we recommend that you use [Tyk Sync](/nightly/api-management/automations/sync) to backup your Tyk APIs and policies. 
