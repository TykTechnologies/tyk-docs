...
canonical: "https://tyk.io/docs/tyk-governance/core-concepts"
---
...
This page covers the conceptual model. For the workflows built on it, see [Rulesets](/nightly/tyk-governance/rulesets) for authoring standards and [Service Compliance](/nightly/tyk-governance/service-compliance) for acting on results.
...
- **API Category**: A tag applied to services in Tyk Dashboard, used to organize the service portfolio and to scope governance rulesets to specific subsets of services. See [Scoping Rulesets to Services](/nightly/tyk-governance/scoping-rulesets).
...
- **Deployment Warning**: A per-ruleset toggle that controls whether saving a non-compliant service produces a soft warning. Off by default, and it never hard-blocks. See [The Deployment Warning](/nightly/tyk-governance/remediate-issues#the-deployment-warning).
...
- **Remediation**: The structured process of addressing and resolving service governance issues using the per-rule guidance authored alongside the rule. See [Remediate Issues](/nightly/tyk-governance/remediate-issues).
...
- **Ruleset Template**: A pre-built ruleset that provides a starting point for common governance domains. See [Ruleset Templates](/nightly/tyk-governance/ruleset-templates).
...
Evaluation cost is driven by the size of the API definition, primarily the number of endpoints it declares, and to a lesser extent the weight of its schemas. It does not change with the volume of traffic your APIs receive. For measured timings, see [Evaluation Performance](/nightly/tyk-governance/service-compliance#evaluation-performance).
...
For how the feature itself is switched on for an organization, see [Availability and Licensing](/nightly/tyk-governance/overview#availability-and-licensing).
