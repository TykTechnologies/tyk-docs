...
canonical: "https://tyk.io/docs/tyk-governance/rulesets"
noindex: true
...
For a guided walkthrough that creates a ruleset from a template and reads its first result, see the [Quick Start](/nightly/tyk-governance/quick-start). For how a ruleset finds the services it governs, see [Scoping Rulesets to Services](/nightly/tyk-governance/scoping-rulesets).
...
- **Severity Levels**: Each rule carries a severity of Error, Warn, or Info that determines how strongly the rule's outcome affects compliance. See [Severity and Compliance](/nightly/tyk-governance/core-concepts#severity-and-compliance).
...
rules:
  rate-limit-exists:
    description: Ensure rateLimit is configured under upstream
    severity: error
...
For what each shipped template enforces, see [Ruleset Templates](/nightly/tyk-governance/ruleset-templates). For a step-by-step walkthrough with screenshots, see the [Quick Start](/nightly/tyk-governance/quick-start).
...
- **Deployment Warning toggle**: Off by default. When on, saving a non-compliant service in a matching category produces a soft warning at deploy time. See [The Deployment Warning](/nightly/tyk-governance/remediate-issues#the-deployment-warning).
...
- **From the Governance tab on a service**. Run any ruleset against that one service. Use this when you own the service. See [Service Compliance](/nightly/tyk-governance/service-compliance#see-compliance-for-a-service).
...
Once a ruleset is linked to a category, you can also run it against an API definition from your CI/CD pipeline, before that API is ever imported into Tyk Dashboard. See [CI/CD Governance Checks](/nightly/tyk-governance/cicd-checks).
