...
- **Which rulesets to check it against**, either `--categories`, meaning the categories your governance owner has linked rulesets to, resolved at check time, or `--ruleset-ids`, meaning specific ruleset IDs if you want a fixed set regardless of category changes. Pick one. See [Best Practices](/nightly/#best-practices) for when to use which.
