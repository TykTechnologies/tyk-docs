...
[Okta](https://www.okta.com/) supports both [OpenID Connect (OIDC)](/nightly/#sso-with-openid-connect) and [SAML 2.0](/nightly/#sso-with-saml).
...
- set `Key` to the Okta **Client ID**
- set `Secret` to the Okta **Client Secret**
- set `DashboardCredential` to the [PORTAL_API_SECRET](/nightly/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_api_secret) used to authenticate with the Portal's management API
