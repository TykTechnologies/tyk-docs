...
TIB requires valid credentials to [generate the one-time nonce](https://tyk.io/docs/api-reference/single-sign-on/generate-authentication-token) from the Tyk Dashboard API.
...
Dashboard users who need to create and manage TIB profiles require the **Identity Management (TIB)** [permission](/nightly/platform-management/user-permissions).
...
| [Default User Group](/nightly/#default-user-group) | Tyk Dashboard configuration | Assigns a fallback [Tyk Dashboard user group](/nightly/platform-management/user-groups) when no group mapping applies. |
...
When combined, the Default User Permissions provide the baseline and group permissions are merged on top. Where there is a conflict, the higher permission level wins. If the user is an [admin](/nightly/platform-management/user-permissions#admin-users), group permissions are skipped to preserve admin rights.
...
<Warning>
If TIB sends no group information to the Dashboard and no `sso_default_group_id` is configured, Tyk Dashboard treats the user as an [admin](/nightly/platform-management/user-permissions#admin-users). This applies when neither IdP permission claims nor a default group is configured.
</Warning>

Setting [`sso_default_group_id`](/nightly/tyk-dashboard/configuration#sso_default_group_id) in the Tyk Dashboard config assigns all SSO users that TIB has not assigned to a group to a specific [user group](/nightly/platform-management/user-groups), from which they will inherit permissions.
