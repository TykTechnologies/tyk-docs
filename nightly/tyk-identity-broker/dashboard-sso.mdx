...
canonical: "https://tyk.io/docs/tyk-identity-broker/dashboard-sso"
---
...
<Note>
We recommend that you read the [Tyk Identity Broker overview](/nightly/tyk-identity-broker/overview) before configuring SSO for Dashboard.
</Note>
...
|---|---|---|
| [`sso_enable_user_lookup`](/nightly/tyk-dashboard/configuration#sso_enable_user_lookup) | Tyk Dashboard configuration | Whether to match the user's email address against existing Tyk Dashboard accounts. |
...
TIB requires valid credentials to [generate the one-time nonce](/nightly/api-management/dashboard-configuration#single-sign-on-api) from the Tyk Dashboard API.
...
Dashboard users who need to create and manage TIB profiles require the **Identity Management (TIB)** [permission](/nightly/api-management/user-management#user-permissions).

## Profile Configuration

Dashboard SSO uses the `GenerateOrLoginUserProfile` [action](/nightly/tyk-identity-broker/overview#actions) in the TIB profile. The following profile fields are required for all Dashboard SSO configurations:

| Field | Value |
|---|---|
| `ActionType` | `GenerateOrLoginUserProfile` |
| `ReturnURL` | `http://{dashboard-host}/tap` |
| `IdentityHandlerConfig.DashboardCredential` | API key for the dedicated [TIB service account](/nightly/#tib-service-account) |

The `ProviderName`, `ProviderConfig`, and `Type` fields depend on your identity provider and are covered in the [IdP-specific guides](/nightly/#set-up-sso-with-your-identity-provider).

The user permission-related fields (`SSOOnlyForRegisteredUsers`, `UserGroupMapping`, `DefaultUserGroupID`, and `CustomUserGroupField`) are optional and used for [unregistered users](/nightly/#unregistered-user-login).
...
<Note>
The `GenerateOrLoginUserProfile` action type is also used to log admin users into Tyk Developer Portal. See [SSO into Tyk Developer Portal](/nightly/tyk-stack/tyk-developer-portal/enterprise-developer-portal/managing-access/enable-sso) for full details.
</Note>
...
|---|---|---|
| [IdP Permission Claims](/nightly/#idp-permission-claims) | TIB profile | Maps IdP group claims to Tyk Dashboard user groups, applying different permissions based on the user's IdP group membership. |
| [Default User Group](/nightly/#default-user-group) | Tyk Dashboard configuration | Assigns a fallback [Tyk Dashboard user group](/nightly/api-management/user-management#manage-tyk-dashboard-user-groups) when no group mapping applies. |
| [Default User](/nightly/#default-user) | Tyk Dashboard configuration | Sets a baseline set of permissions for all unregistered SSO users. |

When combined, the Default User Permissions provide the baseline and group permissions are merged on top. Where there is a conflict, the higher permission level wins. If the user is an [admin](/nightly/api-management/user-management#admin-users), group permissions are skipped to preserve admin rights.
...
<Warning>
If TIB sends no group information to the Dashboard and no `sso_default_group_id` is configured, Tyk Dashboard treats the user as an [admin](/nightly/api-management/user-management#admin-users). This applies when neither IdP permission claims nor a default group is configured.
</Warning>

Setting [`sso_default_group_id`](/nightly/tyk-dashboard/configuration#sso_default_group_id) in the Tyk Dashboard config assigns all SSO users that TIB has not assigned to a group to a specific [user group](/nightly/api-management/user-management#manage-tyk-dashboard-user-groups), from which they will inherit permissions.
...
These permissions will be merged with any configured for the [default user](/nightly/#default-user).

### Default User

In addition to the [default user group](/nightly/#default-user-group) mechanism, setting [`sso_permission_defaults`](/nightly/tyk-dashboard/configuration#sso_permission_defaults) in the Tyk Dashboard config assigns SSO users that have not been assigned to a user group by TIB to a specific set of permissions, for example:
...
These permissions will be merged with any configured for the [default user group](/nightly/#default-user-group).
...
| `host.secret` | [Shared secret](/nightly/tyk-identity-broker/standalone-tib#management-api-secret) between Tyk Dashboard and the standalone TIB instance. Must match the `Secret` field in `tib.conf`. |
...
For installation and configuration of standalone TIB, see [Install Standalone TIB](/nightly/tyk-identity-broker/standalone-tib).
...
|---|---|
| Microsoft Entra ID (OIDC or SAML), ADFS | [SSO with Microsoft Entra ID](/nightly/tyk-identity-broker/sso-entra-id) |
| Okta (OIDC or SAML) | [SSO with Okta](/nightly/tyk-identity-broker/sso-okta) |
| Auth0 | [SSO with Auth0](/nightly/tyk-identity-broker/sso-auth0) |
| Keycloak | [SSO with Keycloak](/nightly/tyk-identity-broker/sso-keycloak) |
| Active Directory, OpenLDAP | [SSO with LDAP](/nightly/api-management/single-sign-on-ldap) |
| Google, GitHub, LinkedIn, and other OAuth providers | [SSO with Social Providers](/nightly/api-management/single-sign-on-social-idp) |
| Custom or legacy authentication endpoints | [SSO with Proxy Provider](/nightly/api-management/custom-auth-with-proxy-identity-provider) |
