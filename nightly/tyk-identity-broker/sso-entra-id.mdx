...
[Microsoft Entra ID](https://www.microsoft.com/en-us/security/business/identity-access/microsoft-entra-id) (formerly Azure AD) supports both [OpenID Connect (OIDC)](/nightly/#sso-with-openid-connect) and [SAML 2.0](/nightly/#sso-with-saml).
...
For more details on profile configuration, see the [Dashboard SSO](/nightly/tyk-identity-broker/dashboard-sso#profile-configuration) documentation.
...
Active Directory Federation Services (ADFS) is Microsoft's on-premises federation service. It uses the same SAML 2.0 protocol as Entra ID. Follow the [SSO with SAML](/nightly/#sso-with-saml) instructions above, with the following differences:
