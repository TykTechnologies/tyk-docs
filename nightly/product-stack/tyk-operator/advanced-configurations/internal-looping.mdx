...
[URL rewriting](/nightly/transform-traffic/url-rewriting) enables the transformation of incoming API request paths to align with the expected endpoint format of your backend services.
...
[Triggers](/nightly/transform-traffic/url-rewriting#how-it-works) are configurations that specify actions based on certain conditions present in HTTP headers, query parameters, path parameters etc.
