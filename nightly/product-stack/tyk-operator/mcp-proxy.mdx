...
| :---- | :--- | :---------- |
| `contextRef` | object (optional) | Reference (`name`, `namespace`) to an [OperatorContext](/nightly/api-management/automations/operator#multi-tenancy-in-tyk) used when reconciling this resource. Use this for multi-tenant setups. |
...
The example above proxies a remote MCP server. The same `TykMcpProxyDefinition` and `ConfigMap` pattern also manages a proxy [generated directly from a Tyk-managed REST API](/nightly/ai-management/mcps/api-to-mcp), which the Operator reconciles identically. Two things differ in the OAS document itself:

- `upstream.url` is an adapter target (for example `tyk://a1b2c3d4e5f647a8b9c0d1e2f3a4b5c6/mcp`) instead of a remote `https://` URL. See [The upstream adapter target](/nightly/ai-management/mcp-gateway/api-to-mcp-definitions#the-upstream-adapter-target) for what this value means.
...
<Note>
Authoring this via the Operator means hand-writing the tool catalog in the `x-tyk-mcp-server` extension yourself. The [Dashboard's guided wizard](/nightly/ai-management/mcp-gateway/managing-proxies#tyk-api) has no Operator equivalent. See [REST API to MCP x-tyk-mcp-server extension](/nightly/ai-management/mcp-gateway/api-to-mcp-definitions) for the full field reference.
</Note>
...
For background on the underlying gateway feature, see the [MCP proxy policies documentation](/nightly/ai-management/mcp-gateway/policies).
