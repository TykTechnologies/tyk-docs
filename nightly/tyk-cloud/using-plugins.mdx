...
* [Golang](/nightly/tyk-cloud/using-plugins)
...
If you use a **Hybrid Data Plane** (a self-managed gateway that you deploy and connect to your Tyk Cloud Control Plane), use the Gateway's built-in [plugin bundle downloader](/nightly/api-management/plugins/overview#plugin-bundles) instead. Host your bundle on your own web server or S3 bucket, then set `bundle_base_url` in the Hybrid Gateway's `tyk.conf` to point to it.
</Note>
...
Next you'll [set up the Python authentication code bundle](/nightly/#create-a-python-code-bundle).
...
1. Firstly you will need a local Tyk Gateway installation that allows you to create your Python plugin bundle. We recommend [installing our Self-Managed version on Ubuntu Bionic 18.04](/nightly/tyk-self-managed/install/linux#install-tyk-on-debian-or-ubuntu).
