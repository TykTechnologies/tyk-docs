...
Next you'll [set up your plugin development environment](/nightly/#set-up-your-plugin-development-environment).
...
Next you'll [create a Python code bundle](/nightly/#create-a-python-code-bundle).
...
Plugins must be enabled on your Tyk Cloud Control Plane, as described in [Setup Control Plane](/nightly/#setup-control-plane). Tyk Cloud only deploys the File Server, and exposes its API, once plugins are enabled on the Control Plane.
...
`bundle_base_url` must point at your Tyk Cloud Control Plane's File Server endpoint, with the `/mserv/api/mw/bundle` path and your File Server API key passed as the `api_key` query parameter. You can find both values on your Control Plane, as described in [Uploading your Bundle](/nightly/#uploading-your-bundle).
