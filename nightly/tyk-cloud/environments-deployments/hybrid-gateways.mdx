...
canonical: "https://tyk.io/docs/tyk-cloud/environments-deployments/hybrid-gateways"
---
...
- as [Cloud Gateways](/nightly/tyk-cloud/environments-deployments/managing-gateways): Deployed and managed in *Tyk Cloud*, in any of our available regions. These are SaaS gateways, so there are no deployment or operational concerns.
...
1. Clone the [tyk-install](https://github.com/TykTechnologies/tyk-install) repository and navigate to the hybrid directory:

    ```bash
    git clone https://github.com/TykTechnologies/tyk-install.git
    cd tyk-install/docker/hybrid
    ```
...
| -------- | ----------- |
| `TYK_GW_SLAVEOPTIONS_CONNECTIONSTRING` | MDCB endpoint from the control plane |
| `TYK_GW_SLAVEOPTIONS_RPCKEY` | Organization ID |
| `TYK_GW_SLAVEOPTIONS_APIKEY` | Dashboard User API Key |
| `TYK_GW_SLAVEOPTIONS_GROUPID` | Data plane cluster identifier (used when running multiple data planes) |

To enable API sharding (optional), add to `confs/tyk.env`:

```bash
TYK_GW_DBAPPCONFOPTIONS_NODEISSEGMENTED=true
TYK_GW_DBAPPCONFOPTIONS_TAGS=qa,uat
```
...
Then verify in your control plane:

Navigate to **Deployments** > your Control Plane. Your hybrid data plane should appear under **Hybrid data planes**.

  <img src="/img/cloud/hybrid-data-plane-added.png" alt="Hybrid Data Plane Added" />
...
* Connection details to remote control plane from the above [section](/nightly/tyk-cloud/environments-deployments/hybrid-gateways).
...
Set the below environment variables and replace values with connection details to your Tyk Cloud remote control plane. See the above [section](/nightly/tyk-cloud/environments-deployments/hybrid-gateways) on how to get the connection details.
...
<Warning>
`tyk-hybrid` chart is deprecated. Please use our [Tyk Data Plane helm chart](/nightly/product-stack/tyk-charts/tyk-data-plane-chart) instead. 
