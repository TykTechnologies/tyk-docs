...
canonical: "https://tyk.io/docs/docs/tyk-cloud/using-plugins"
---
...
* [Python Plugins](/nightly/docs/api-management/plugins/rich-plugins#overview)
* [JSVM](/nightly/docs/api-management/plugins/javascript#)
* [Golang](/nightly/docs/tyk-cloud/using-plugins)
...
| File              | Description                                                                                                                                                                                                                                                                                       |
| :------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| custom_middleware | contains the middleware settings like the plugin driver you want to use (driver) and the hooks that your plugin will expose. We use the   **auth_check** for this tutorial. For other hooks see [here](/nightly/docs/api-management/plugins/rich-plugins#coprocess-dispatcher---hooks). |
...
* You import decorators from the Tyk module that gives us the Hook decorator, and we import [Tyk Python API helpers](/nightly/docs/api-management/plugins/rich-plugins#tyk-python-api-methods)
...
* A plugin bundle is a packaged version of the plugin, it may also contain a cryptographic signature of its contents. The -y flag tells the Tyk CLI tool to skip the signing process in order to simplify this tutorial. For more information on the Tyk CLI tool, see [here](/nightly/docs/api-management/plugins/overview#how-plugin-bundles-work).
...
1. Firstly you will need a local Tyk Gateway installation that allows you to create your Python plugin bundle. We recommend [installing our Self-Managed version on Ubuntu Bionic 18.04](/nightly/docs/tyk-self-managed/install#install-tyk-on-debian-or-ubuntu).
