...
canonical: "https://tyk.io/docs/docs/api-management/gateway-config-managing-classic"
---
...
To embark on your API journey with Tyk Cloud, we recommend going to our [Quick Start guide](/nightly/docs/tyk-cloud#quick-start-tyk-cloud). This guide will walk you through the process of creating your very first API in Tyk Cloud.
For an advanced step by step guide we recommend visiting our [Getting Started guide](/nightly/docs/tyk-cloud#comprehensive-tyk-cloud-setup). This will explain advanced configuration steps relating to how to distribute your API across nodes, in addition to adding and testing your API.
...
We just sent an API definition to the Tyk `/apis` endpoint. See [API definition objects](/nightly/docs/api-management/gateway-config-tyk-classic) for details of all the available objects. These objects encapsulate all of the settings for an API within Tyk.
...
For details on using Tyk OAS with Tyk Gateway, check out our guide to [working with Tyk OAS APIs](/nightly/docs/api-management/gateway-config-managing-oas).
</Note>


**Prerequisites**

Before you continue this tutorial, you will need a running [Tyk OSS gateway](/nightly/docs/tyk-oss-gateway). Click the button for instructions on how to install Tyk Gateway:
...
1. [Create an API with the Tyk Gateway API](/nightly/#using-tyk-gateway-api) - Tyk Gateway has its own APIs which provides various services including the registering of Tyk API Definitions on the Gateway.
2. [Create an API in File-based Mode](/nightly/#create-an-api-in-file-based-mode) - alternatively you can create a Tyk API Definition in a file and then load it to the Gateway.
...
Tyk API definitions encapsulate all of the settings for an API within Tyk Gateway and are discussed in detail in the [API section](/nightly/docs/api-management/gateway-config-tyk-classic) of this documentation.
...
See [What is a Security Policy?](/nightly/docs/api-management/policies#what-is-a-security-policy) for more details.
...
See [Versioning](/nightly/docs/api-management/gateway-config-tyk-classic#tyk-classic-api-versioning) for more details.
...
4. **Click "Generate API"**
    ...
    Your API will appear in your APIs list. If you select **EDIT** from the **ACTIONS** drop-down list, you can see the endpoints (from the [Endpoint Designer](/nightly/docs/api-management/dashboard-configuration#exploring-api-endpoint-designer)) that have been created as part of the import process.
