...
canonical: "https://tyk.io/docs/docs/api-management/authentication/jwt-signature-validation"
---
...
When storing the key or secret in the API definition, it is first base64 encoded and then configured in `server.authentication.securitySchemes.<jwtAuthScheme>.source` (in Tyk Classic, this is [jwt_source](/nightly/docs/api-management/gateway-config-tyk-classic#configuring-authentication-for-tyk-classic-apis)).
...
Refer to the [Tyk OAS API Definition](/nightly/docs/api-management/gateway-config-tyk-oas#jwt) reference for details.

### External Key Value Store

For improved separation of concerns and flexibility, the key/secret can be placed in an [external key value store](/nightly/docs/tyk-configuration-reference/kv-store), with the appropriate reference configured in the API definition.
...
Tyk can retrieve public keys from JSON Web Key Sets (JWKS) endpoints to validate the signature of incoming JWTs. Tyk supports configuring [single](/nightly/#single-jwks-endpoint) or [multiple](/nightly/#multiple-jwks-endpoints) JWKS endpoints with [caching](/nightly/#jwks-caching) capabilities.
...
From **Tyk 5.10.0** onwards, we have introduced enhanced JWKS caching for [Tyk OAS APIs](/nightly/docs/api-management/gateway-config-tyk-oas) with the following improvements:
...
For more details, refer to the [Tyk OAS API definition reference](/nightly/docs/api-management/gateway-config-tyk-oas#jwk).
...
New [Gateway API](/nightly/docs/tyk-gateway-api) endpoints are available from **Tyk 5.10.0** to manage JWKS caches programmatically. These endpoints work for both Tyk OAS and Tyk Classic APIs:
...
**Note:** These endpoints are currently available only through the Tyk [Gateway API](/nightly/docs/tyk-gateway-api) and are not yet extended to the Tyk [Dashboard API](/nightly/docs/tyk-dashboard-api).
