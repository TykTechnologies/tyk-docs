...
[JSON Web Tokens (JWT)](https://www.jwt.io/introduction) are a popular method for client authentication and authorization that can be used to secure access to your APIs via Tyk's [JWT Auth](/nightly/docs/basic-config-and-security/security/authentication-authorization/json-web-tokens) method.

After the JWT signature has been [validated](/nightly/docs/basic-config-and-security/security/authentication-authorization/json-web-tokens#signature-validation), Tyk uses the **claims** within the token to determine which security policies (access rights, rate limits and quotas) should be applied to the request.

From Tyk 5.10, Tyk can perform optional [validation](/nightly/docs/api-management/authentication/jwt-claim-validation) of these claims.
...
2. **Policy Resolution**: Tyk determines which [policy](/nightly/docs/api-management/policies#what-is-a-security-policy) to apply to the request:
   - From scope-to-policy mapping
   - From default policies

3. **Update Session**: The [session](/nightly/docs/api-management/policies#what-is-a-session-object) is updated with the identity and policies.
...
3. The `sub` [registered claim](/nightly/docs/api-management/authentication/jwt-claim-validation#registered-vs-custom-claims).
...
When creating multiple policies that might be applied to the same JWT, we recommend using [partitioned policies](/nightly/docs/api-management/policies#partitioned-policies) - policies that control specific aspects of API access rather than trying to configure everything in a single policy.
...
The following [session attributes](/nightly/docs/api-management/policies#session-object) are modified based on the policies:
...
In addition to updating the session, Tyk extracts claims from the JWT and makes them available as context variables for use in other [middleware](/nightly/docs/api-management/traffic-transformation).
