...
canonical: "https://tyk.io/docs/docs/api-management/traffic-transformation/go-templates"
---

Tyk's [request](/nightly/docs/api-management/traffic-transformation/request-body) and [response](/nightly/docs/api-management/traffic-transformation/response-body) body transform middleware use the [Go template language](https://golang.org/pkg/text/template/) to parse and modify the provided input.

Go templates are also used by Tyk's [webhook event handler](/nightly/docs/api-management/gateway-events#event-handling-with-webhooks) to produce the payload for the HTTP request sent to the target system.
