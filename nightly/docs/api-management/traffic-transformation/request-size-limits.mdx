...
<Note>
The system level request size limit is the only size limit applied to [TCP](/nightly/docs/key-concepts/tcp-proxy) and [Websocket](/nightly/docs/advanced-configuration/websockets) connections.
</Note>
...
You can configure a request size limit (in bytes) that will be applied to all APIs on your Tyk Gateway by adding `max_request_body_size` to the `http_server_options` [element](/nightly/docs/tyk-oss-gateway/configuration#http_server_optionsmax_request_body_size) of your `tyk.conf` Gateway configuration. For example:
```yaml
...
The [request size limit](/nightly/docs/api-management/traffic-transformation/request-size-limits) middleware enables you to apply limits to the size of requests made to your HTTP APIs. You might use this feature to protect your Tyk Gateway or upstream services from excessive memory usage or brute force attacks.
...
You can work around this by implementing a combination of endpoint-level size limits and [allow](/nightly/docs/api-management/traffic-transformation/allow-list#api-definition) or [block](/nightly/docs/api-management/traffic-transformation/block-list#api-designer) lists.
...
There are three different levels of granularity that can be used when configuring a request size limit.
- [system-wide](/nightly/#applying-a-system-level-size-limit): affecting all APIs deployed on the gateway
- [API-level](/nightly/docs/api-management/traffic-transformation/request-headers#tyk-classic-api): affecting all endpoints for an API
