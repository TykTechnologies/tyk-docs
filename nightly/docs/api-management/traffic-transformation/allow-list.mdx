...
canonical: "https://tyk.io/docs/docs/api-management/traffic-transformation/allow-list"
---
...
Note that this is not the same as Tyk's [IP allow list](/nightly/docs/api-management/gateway-config-tyk-classic#ip-access-control) feature, which is used to restrict access to APIs based upon the IP of the requestor.
...
When using the allow list middleware, we recommend that you familiarize yourself with Tyk's [URL matching](/nightly/docs/getting-started/key-concepts/url-matching) options.
...
The [allow list](/nightly/docs/api-management/traffic-transformation/allow-list) is a feature designed to restrict access to only specific API endpoints. It rejects requests to endpoints not specifically "allowed", returning `HTTP 403 Forbidden`. This enhances the security of the API by preventing unauthorized access to endpoints that are not explicitly permitted.
...
- `method_actions`: a shared object used to configure the [mock response](/nightly/docs/api-management/traffic-transformation/mock-response#configuring-mock-response-using-tyk-dashboard-ui) middleware
