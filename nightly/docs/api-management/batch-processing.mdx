...
canonical: "https://tyk.io/docs/docs/api-management/batch-processing"
---
...
If sequential execution is in use, Tyk will work through the entire `requests` array regardless of whether any requests return errors. All responses (success and failure) will be logged and returned to the client as described [below](/nightly/docs/api-management/batch-processing#batch-response-format).
...
You can make requests to the logical batch request endpoint from within [custom JavaScript middleware](/nightly/docs/api-management/plugins/javascript) via the `TykBatchRequest` function that is included in Tyk's  [JavaScript API](/nightly/docs/api-management/plugins/javascript#javascript-api).
...
- using [Internal Looping](/nightly/docs/advanced-configuration/transform-traffic/looping) to put the batch request API behind a protected API
