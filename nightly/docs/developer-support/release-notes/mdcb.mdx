...
canonical: "https://tyk.io/docs/docs/developer-support/release-notes/mdcb"
---
...
This release updates MDCB to maintain compatibility with recent changes to the [URL Rewrite Middleware schema](/nightly/docs/developer-support/release-notes/gateway#release-highlights-1), ensuring seamless interoperability with the latest Gateway and Dashboard versions.
...
We have updated MDCB to align with the recent [URL Rewrite Middleware schema](/nightly/docs/developer-support/release-notes/gateway#release-highlights-1). This update ensures that MDCB remains fully compatible and behaves consistently with the latest Gateway and Dashboard releases.
</Expandable>
</li>
</ul>
...
- Migrate to new [health check](/nightly/docs/api-management/mdcb#health-check) endpoints in order to get more detailed information. For Kubernetes users, use Helm Charts v1.6 to upgrade MDCB to set liveness and readiness probes of MDCB deployment to the new health check endpoints.
...
Two new [health check](/nightly/docs/api-management/mdcb#health-check) endpoints have been added to improve monitoring and diagnostics:
...
Two new `/config` and `/env` [endpoints](/nightly/docs/api-management/mdcb#check-mdcb-configurations) have been implemented, allowing developers to access the current configuration state of the MDCB instance in real-time. This feature provides:
...
[MongoDB supported versions](/nightly/docs/planning-for-production/database-settings#supported-versions) 
...
#### Added
- Added a new configuration option [enable_ownership](/nightly/docs/tyk-multi-data-centre/mdcb-configuration-options#enable_ownership) that allows MDCB filter APIs by API Ownership. 
