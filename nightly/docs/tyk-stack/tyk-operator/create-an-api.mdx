...
canonical: "https://tyk.io/docs/docs/tyk-stack/tyk-operator/create-an-api"
---
...
You can configure your API for any Tyk supported authentication method by following the [Client Authentication](/nightly/docs/api-management/client-authentication) documentation.
...
First, specify the details of your API using the [ApiDefinition CRD](/nightly/docs/api-management/automations/operator#apidefinition-crd), then deploy it to create the corresponding Kubernetes resource. Tyk Operator will take control of the CRD and create the actual API in the Tyk data plane.
...
- [HTTP Proxy example](/nightly/docs/tyk-stack/tyk-operator/create-an-api#set-up-manifest-for-http)
...
You can visit the [ApiDefinition CRD](/nightly/docs/api-management/automations/operator#apidefinition-crd) page to see all the latest API Definitions fields and features we support.
...
<Note>
Tyk Operator supported authentication types are listed in the [API Definition features](/nightly/docs/api-management/automations/operator#apidefinition-crd) section.
</Note>


#### Create an API key

You need to generate a key to access the `httpbin` API now. Follow [this guide](/nightly/docs/getting-started/configure-first-api#create-an-api-key) to see how to create an API key for your installation. 
...
This configuration allows [keyless (open)](/nightly/docs/basic-config-and-security/security/authentication-authorization/open-keyless) access to the API without any authentication.
...
This setup requires a [bearer token](/nightly/docs/api-management/authentication/bearer-token) for access.
...
- `validate_signature`: Enable [signature validation](/nightly/docs/api-management/authentication/bearer-token#auth-token-with-signature).
...
This configuration uses [JWT tokens](/nightly/docs/basic-config-and-security/security/authentication-authorization/json-web-tokens) for authentication.

Users can configure JWT authentication by defining the following fields:

- `jwt_signing_method`: Specify the method used to sign the JWT. Refer to the documentation on [JWT Signatures](/nightly/docs/basic-config-and-security/security/authentication-authorization/json-web-tokens#signature-validation) for supported methods.
...
This configuration uses [Basic Authentication](/nightly/docs/api-management/authentication/basic-authentication), requiring a username and password for access.
...
This configuration uses a [Golang plugin](/nightly/docs/api-management/plugins/golang#) for custom authentication. The following example shows how to create an API definition with a Golang custom plugin for `httpbin-go-auth`.

For an example of Golang authentication middleware, see [Performing custom authentication with a Golang plugin](/nightly/docs/api-management/plugins/golang#performing-custom-authentication-with-a-golang-plugin).
...
This configuration uses a [gRPC plugin](/nightly/docs/api-management/plugins/golang#) for custom authentication. The following example shows how to create an API definition with a gRPC custom plugin for `httpbin-grpc-auth`.
...
This setup allows for [multiple authentication](/nightly/docs/basic-config-and-security/security/authentication-authorization/multiple-auth) methods to be chained together, requiring clients to pass through each specified authentication provider.

To enable multiple (chained) auth, you should set `base_identity_provided_by` field to one of the supported chained enums. Consult the [Multi (Chained) Authentication](/nightly/docs/basic-config-and-security/security/authentication-authorization/multiple-auth) section for the supported auths.
...
To enable [IP Allowlist](/nightly/docs/api-management/gateway-config-tyk-classic#ip-access-control), set the following fields:
...
To enable [IP Blocklist](/nightly/docs/api-management/gateway-config-tyk-classic#ip-access-control), set the following fields:
...
In the example below we can see that the configuration is contained within the `graphql` configuration object. A GraphQL schema is specified within the `schema` field and the execution mode is set to `proxyOnly`. The [GraphQL public playground](/nightly/docs/api-management/graphql#enabling-public-graphql-playground) is enabled with the path set to `/playground`.
...
If you are on Tyk 3.2 and above, you can use the following manifest to create an UDG API. This example configures a Universal Data Graph from a [GraphQL datasource](/nightly/docs/api-management/data-graph#graphql) and a [REST Datasource](/nightly/docs/api-management/data-graph#rest).
...
Below, we take you through how to define the security policy but you can also find [Security Policy Example](/nightly/docs/tyk-stack/tyk-operator/create-an-api#security-policy-example) below.
...
In this example, the security policy will apply to an `ApiDefinition` resource named `httpbin` in the `default` namespace, a `TykOasApiDefinition` resource named `petstore` in the `default` namespace, and a `TykStreamsApiDefinition` resource named `http-to-kafka` in the `default` namespace. Note that with Tyk Operator, you do not need to specify API ID as in the raw [Policy definition](/nightly/docs/api-management/policies#policies-guide). Tyk Operator will automatically retrieve the API ID of referenced API Definition resources for you.
...
You can secure your APIs by specifying [allowed URLs](/nightly/docs/api-management/policies#secure-your-apis-by-method-and-path) (methods and paths) for each API within a security policy. This is done using the `allowed_urls` field under `access_rights_array`.
