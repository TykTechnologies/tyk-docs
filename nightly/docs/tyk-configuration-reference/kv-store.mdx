...
canonical: "https://tyk.io/docs/docs/tyk-configuration-reference/kv-store"
---
...
Storing local settings within the Tyk Gateway's configuration file allows you to have per instance variables, such as a machine ID, and inject these into API requests and responses using [transformation middleware](/nightly/docs/api-management/traffic-transformation).
...
The `secrets` section in the [Tyk Gateway configuration file](/nightly/docs/tyk-oss-gateway/configuration#secrets) allows you to store settings that are specific to a single Tyk Gateway instance. This is useful for storing instance-specific configuration to be injected into API middleware or if you prefer using configuration files.
- to retrieve the value assigned to a `KEY` in the `secrets` config you will use `secrets://KEY` or `$secret_conf.KEY` notation depending on the [location](/nightly/docs/tyk-oss-gateway/configuration#secrets) of the reference
...
In Tyk Gateway's configuration file (`tyk.conf`), you can retrieve values from KV stores for the following [fields](/nightly/docs/tyk-oss-gateway/configuration):
- `secret`
- `node_secret`
- `storage.password`
...
```
When the Gateway starts, Tyk will read the *Value* from Vault and use this as the `node_secret`, which is used to [secure connection to the Tyk Dashboard](/nightly/docs/tyk-oss-gateway/configuration#node_secret).
...
When the Gateway starts, Tyk will read the *Value* from the environment variable and use this as the [Target URL](/nightly/docs/api-management/gateway-config-tyk-oas#upstream).
...
- [request body transform](/nightly/docs/api-management/traffic-transformation/request-body)
- [request header transform](/nightly/docs/api-management/traffic-transformation/request-headers)
- [URL rewrite](/nightly/docs/transform-traffic/url-rewriting#url-rewrite-middleware)
- [response body transform](/nightly/docs/api-management/traffic-transformation/response-body)
- [response header transform](/nightly/docs/api-management/traffic-transformation/response-headers)
...
When the Gateway starts, Tyk will read the *Value* from the `secrets` section in the Gateway config file and use this to identify the header where Tyk Gateway should look for the [Authentication](/nightly/docs/api-management/gateway-config-tyk-oas#authentication) token in requests to your Tyk OAS API.
...
Alternatively, you can configure it using the equivalent [environment variables](/nightly/docs/tyk-oss-gateway/configuration#kvconsuladdress).
...
Alternatively, you can configure it using the equivalent [environment variables](/nightly/docs/tyk-oss-gateway/configuration#kvvaulttoken).
