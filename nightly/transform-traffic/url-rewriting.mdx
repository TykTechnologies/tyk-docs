...
When the regex includes capturing groups - subexpressions wrapped in parentheses - each matched group is assigned an index: `$1` for the first, `$2` for the second, and so on. These captured values can appear anywhere in the [rewrite target](/nightly/#the-rewrite-target).
...
For example, in the configuration shown in the [Worked Example](/nightly/#worked-example) below, advanced trigger 1 (the first in the list, so `n=0`) tests the `region` query parameter. When it fires on a request with `?region=us`, the value `us` is available as:
...
**Key-value storage** - using the [appropriate KV reference notation](/nightly/tyk-configuration-reference/kv-store#transformation-middleware).
...
The target can be a path on the same upstream, a fully qualified URL on a different host, or a `tyk://` internal address to route the request to another API on the same Tyk Gateway instance. See [Internal Routing](/nightly/advanced-configuration/transform-traffic/looping).

## Matching Behaviour

The URL rewrite middleware applies its own regular expression matching to the request path. It does not use the endpoint matching mode (wildcard, prefix, suffix, or exact) configured at the Gateway level, and is not affected by changes to those settings. See [Request Matching](/nightly/getting-started/key-concepts/url-matching) for details on endpoint matching modes.
...
If using Tyk Classic, add a `url_rewrites` object to the `extended_paths` section of the API definition. To reference context variables in `rewrite_to`, [enable context variables](/nightly/api-management/traffic-transformation/request-context-variables#enabling-context-variables-for-use-with-tyk-classic-apis) for the API.
...
See [Tyk Operator](/nightly/api-management/automations/operator) for details on creating and managing API definitions with Tyk Operator.
