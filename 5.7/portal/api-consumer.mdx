...
'og:description': "How to configure API Consumers in Tyk developer portal"
...
If your use case requires adding a new organization, see [step by step guide](/5.7/portal/api-consumer#manage-api-consumer-organizations). When an organization is created, a default team tied to that organization is automatically generated too. Teams and organizations can have different visibility when it comes to API Products and plans. This can be managed within the [catalog section](/5.7/portal/api-provider#create-a-new-catalog).
...
6. To generate the invite email, click **More Options** in the Overview section and then **Send invite**.
    ...
    To customize the invite email, please refer to the [Email customization section](/5.7/portal/customization#configure-email-notifications) for guidance.
...
Please refer to the [email notification section](/5.7/portal/settings#email-configuration) for further instructions for setting up the email server.
...
2. **Approve or reject organization registration requests**
    ...
    <br/><br/>Both emails are customizable. Refer to [the email customization documentation](/5.7/portal/customization#configure-email-notifications) for further information on the email customization.
