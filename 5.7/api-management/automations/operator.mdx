...
**TykOasApiDefinition**: Available from Tyk Operator v1.0. It represents a [Tyk OAS API configuration](/5.7/api-management/gateway-config-tyk-oas#tyk-oas-api-definition-object). Tyk OAS API is based on the OpenAPI specification (OAS) and is the recommended format for standard HTTP APIs. Tyk Operator supports all [Tyk OAS API feature](/5.7/api-management/gateway-config-tyk-oas#tyk-oas-api-feature-status) as they become available on the Gateway.

**ApiDefinition**: Available on all versions of Tyk Operator. It represents a [Tyk Classic API configuration](/5.7/api-management/gateway-config-tyk-classic). Tyk Classic API is the traditional format used for defining all APIs in Tyk, and now the recommended format for non-HTTP APIs such as TCP, GraphQL, and Universal Data Graph (UDG). Tyk Operator supports the major features of Tyk Classic API and the feature support details can be tracked [here](/5.7/#apidefinition-crd).

**TykStreamsApiDefinition**: Available from Tyk Operator v1.1. It represents an [Async API configuration](/5.7/api-management/event-driven-apis#configuration-as-code) which is based on [Tyk OAS API Definition](/5.7/api-management/gateway-config-tyk-oas#tyk-oas-api-definition-object). Tyk Operator supports all [Tyk Streams](/5.7/api-management/event-driven-apis#) and [Tyk OAS API feature](/5.7/api-management/gateway-config-tyk-oas#tyk-oas-api-feature-status) as they become available on the Gateway.

**SecurityPolicy**: Available on all versions of Tyk Operator. It represents a [Tyk Security Policy configuration](/5.7/#security-policy-example). Security Policies in Tyk provide a way to define and enforce security controls, including authentication, authorization, and rate limiting for APIs managed in Tyk. Tyk Operator supports essential features of Security Policies, allowing users to centrally manage access control and security enforcement for all APIs across clusters.
...
The following custom resources can be used to configure APIs and policies at [Tyk Gateway](/5.7/tyk-oss-gateway) or [Tyk Dashboard](/5.7/tyk-dashboard).

| Kind               | Group       | Version   | Description                                                                                       |
| :-------------------- | :------------- | :----------- | :--------------------------------------------------------------------------------------------------- |
| TykOasApiDefinition| tyk.tyk.io  | v1alpha1  | Defines configuration of [Tyk OAS API Definition object](/5.7/api-management/gateway-config-tyk-oas#tyk-oas-api-definition-object)                                 |
| ApiDefinition      | tyk.tyk.io  | v1alpha1  | Defines configuration of [Tyk Classic API Definition object](/5.7/api-management/gateway-config-tyk-classic)                                 |
| TykStreamsApiDefinition| tyk.tyk.io  | v1alpha1  | Defines configuration of [Tyk Streams](/5.7/api-management/event-driven-apis#configuration-as-code)                                 |
| SecurityPolicy     | tyk.tyk.io  | v1alpha1  | Defines configuration of [security policies](/5.7/api-management/policies#what-is-a-security-policy). Operator supports linking ApiDefinition custom resources in SecurityPolicy's access list so that API IDs do not need to be hardcoded in the resource manifest.        |
| SubGraph           | tyk.tyk.io  | v1alpha1  | Defines a [GraphQL federation subgraph](/5.7/api-management/graphql#subgraphs-and-supergraphs).                                           |
| SuperGraph         | tyk.tyk.io  | v1alpha1  | Defines a [GraphQL federation supergraph](/5.7/api-management/graphql#subgraphs-and-supergraphs).                                        |
| OperatorContext    | tyk.tyk.io  | v1alpha1  | Manages the context in which the Tyk Operator operates, affecting its overall behavior and environment. See [Operator Context](/5.7/#multi-tenancy-in-tyk) for details. |

#### Tyk Classic Developer Portal

The following custom resources can be used to configure [Tyk Classic Developer Portal](/5.7/tyk-developer-portal/tyk-portal-classic).

| Kind               | Group       | Version   | Description                                                                                       |
| :-------------------- | :------------- | :----------- | :--------------------------------------------------------------------------------------------------- |
| APIDescription     | tyk.tyk.io  | v1alpha1  | Configures [Portal Documentation](/5.7/tyk-apis/tyk-portal-api/portal-documentation). |
| PortalAPICatalogue | tyk.tyk.io  | v1alpha1  | Configures [Portal API Catalogue](/5.7/getting-started/key-concepts/api-catalogue). |
| PortalConfig       | tyk.tyk.io  | v1alpha1  | Configures [Portal Configuration](/5.7/tyk-apis/tyk-portal-api/portal-configuration). |
...
### Operator User
Tyk Operator is a Kubernetes Controller that manages Tyk Custom Resources (CRs) such as API Definitions and Security Policies. Developers define these resources as [Custom Resource (CRs)](/5.7/#custom-resources-in-tyk), and Tyk Operator ensures that the desired state is reconciled with the Tyk Gateway or Dashboard. This involves creating, updating, or deleting API configurations until the target state matches the desired state.
...
Tyk Dashboard is multi-tenant capable, which means you can use a single Tyk Dashboard instance to host separate [organizations](/5.7/api-management/dashboard-configuration#organizations) for each team or department. Each organization is a completely isolated unit with its own:
...
| :------------------ | :------------- | :--------- | :--------- |
| Client certifates | ✅ [Client mTLS](/5.7/api-management/client-authentication#setup-static-mtls-in-tyk-operator-using-the-tyk-classic-api-definition) | ✅ [Client mTLS](/5.7/api-management/client-authentication#setup-static-mtls-in-tyk-operator-using-tyk-oas-api-definition) | Certificate ID can be set in the API Definition but configuring certificates from Secrets in CRD is not supported. |
| Custom domain certificates | ✅ [TLS and SSL](/5.7/api-management/certificates#dynamically-setting-ssl-certificates-for-custom-domains) | ✅ [TLS and SSL](/5.7/api-management/certificates#dynamically-setting-ssl-certificates-for-custom-domains) | Certificate ID can be set in the API Definition but configuring certificates from Secrets in CRD is not supported. |
| Public keys pinning | ✅ [Certificate pinning](/5.7/api-management/certificates#certificate-pinning) | ✅ [Certificate pinning](/5.7/api-management/certificates#certificate-pinning) | Certificate ID can be set in the API Definition but configuring certificates from Secrets in CRD is not supported. |
| Upstream mTLS | ✅ [Upstream mTLS via Operator](/5.7/api-management/client-authentication#via-tyk-operator-using-the-tyk-classic-api-definition) | ✅ [Upstream mTLS via Operator](/5.7/api-management/client-authentication#tyk-operator-oas) | Certificate ID can be set in the API Definition but configuring certificates from Secrets in CRD is not supported. |


## Install and Configure Tyk Operator

We assume you have already installed Tyk. If you don’t have it, check out [Tyk
Cloud](/5.7/tyk-cloud#quick-start-tyk-cloud) or [Tyk Self
Managed](/5.7/getting-started/installation) page. [Tyk Helm
Chart](/5.7/product-stack/tyk-charts/overview) is the preferred (and easiest) way to install Tyk on Kubernetes.
...
Tyk Operator needs a [user credential](/5.7/#operator-user) to connect with
...
[Using Tyk Operator to enable GitOps with Tyk](/5.7/api-management/automations) about
...
If you are using [Tyk Stack](/5.7/product-stack/tyk-charts/tyk-stack-chart), [Tyk Control
Plane](/5.7/product-stack/tyk-charts/tyk-control-plane-chart), or [Tyk Open
Source Chart](/5.7/product-stack/tyk-charts/tyk-oss-chart), you can install Tyk Operator alongside other Tyk
...
another name, configure it through Helm Chart [envFrom](/5.7/#install-tyk-operator-and-custom-resource-definitions-crds) value.
...
You can configure [Tyk Operator as Ingress Controller](/5.7/#control-kubernetes-ingress-resources) so
...
[WATCH_INGRESS_CLASS](/5.7/#configure-tyk-operator-via-environment-variable-or-tyk-operator-conf-secret) through `tyk-operator-conf` or the environment variable.
...
You can configure your API for any Tyk supported authentication method by following [Authentication with Tyk OAS](/5.7/api-management/gateway-config-tyk-oas#authentication-with-tyk-oas) documentation.
...
First, specify the details of your API using the [ApiDefinition CRD](/5.7/#apidefinition-crd), then deploy it to create the corresponding Kubernetes resource. Tyk Operator will take control of the CRD and create the actual API in the Tyk data plane.
...
- [HTTP Proxy example](/5.7/#set-up-manifest-for-http)
- [TCP Proxy example](/5.7/#set-up-manifest-for-tcp)
- [GraphQL Proxy example](/5.7/#set-up-manifest-for-graphql)
- [UDG example](/5.7/#set-up-manifest-for-udg)
...
You can visit the [ApiDefinition CRD](/5.7/#apidefinition-crd) page to see all the latest API Definitions fields and features we support.
...
<Note>
Tyk Operator supported authentication types are listed in the [API Definition features](/5.7/#apidefinition-crd) section.
</Note>
...
This configuration allows [keyless (open)](/5.7/api-management/client-authentication#open-no-authentication) access to the API without any authentication.
...
This setup requires a [bearer token](/5.7/api-management/client-authentication#use-auth-tokens) for access.
...
- `validate_signature`: Enable [signature validation](/5.7/api-management/client-authentication#auth-token-signature-validation).
...
This configuration uses [JWT tokens](/5.7/basic-config-and-security/security/authentication-authorization/json-web-tokens) for authentication.

Users can configure JWT authentication by defining the following fields:

- `jwt_signing_method`: Specify the method used to sign the JWT. Refer to [JWT Signing Method](/5.7/basic-config-and-security/security/authentication-authorization/json-web-tokens#set-up-jwt-signing-method) for supported methods.
...
This configuration uses [Basic Authentication](/5.7/api-management/client-authentication#use-basic-authentication), requiring a username and password for access.
...
This configuration uses a [Golang plugin](/5.7/api-management/plugins/golang#) for custom authentication. The following example shows how to create an API definition with a Golang custom plugin for `httpbin-go-auth`.

For an example of Golang authentication middleware, see [Performing custom authentication with a Golang plugin](/5.7/api-management/plugins/golang#performing-custom-authentication-with-a-golang-plugin).
...
This configuration uses a [gRPC plugin](/5.7/api-management/plugins/golang#) for custom authentication. The following example shows how to create an API definition with a gRPC custom plugin for `httpbin-grpc-auth`.
...
This setup allows for [multiple authentication](/5.7/api-management/client-authentication#combine-authentication-methods) methods to be chained together, requiring clients to pass through each specified authentication provider.

To enable multiple (chained) auth, you should set `base_identity_provided_by` field to one of the supported chained enums. Consult [Enable Multi (Chained) Authentication in your API Definition](/5.7/api-management/client-authentication#enable-multi-chained-authentication-in-your-api-definition) for the supported auths.
...
To enable [IP Allowlist](/5.7/api-management/gateway-config-tyk-classic#ip-allowlist-middleware), set the following fields:
...
To enable [IP Blocklist](/5.7/api-management/gateway-config-tyk-classic#ip-blocklist-middleware), set the following fields:
...
In the example below we can see that the configuration is contained within the `graphql` configuration object. A GraphQL schema is specified within the `schema` field and the execution mode is set to `proxyOnly`. The [GraphQL public playground](/5.7/api-management/graphql#enabling-public-graphql-playground) is enabled with the path set to `/playground`.
...
If you are on Tyk 3.2 and above, you can use the following manifest to create an UDG API. This example configures a Universal Data Graph from a [GraphQL datasource](/5.7/api-management/data-graph#graphql) and a [REST Datasource](/5.7/api-management/data-graph#rest).
...
To secure your Tyk Streams API, configure security fields in the OAS definition just as you would for a standard Tyk OAS API. For more details, refer to the [Secure your OAS API](/5.7/#secure-your-oas-api) guide.

## Add a Security Policy to your API
To further protect access to your APIs, you will want to add a security policy. 
Below, we take you through how to define the security policy but you can also find [Security Policy Example](/5.7/#security-policy-example) below.
...
**<span id="per-endpoint-rate-limit">Key-Level Per-Endpoint Rate Limits</span>**
...
**<span id="path-based-permissions">Path based permissions</span>**

You can secure your APIs by specifying [allowed URLs](/5.7/api-management/policies#secure-your-apis-by-method-and-path) (methods and paths) for each API within a security policy. This is done using the `allowed_urls` field under `access_rights_array`.
...
[Partitioned policies](/5.7/api-management/policies#partitioned-policies) allow you to selectively enforce different segments of a security policy, such as quota, rate limiting, access control lists (ACL), and GraphQL complexity rules. This provides flexibility in applying different security controls as needed.
...
Tyk Operator by default looks for the value `tyk` and will ignore all other ingress classes. If you wish to override this default behavior,
 you may do so by setting the environment variable `WATCH_INGRESS_CLASS` in the operator manager deployment. [See Installing Tyk Operator](/5.7/#install-and-configure-tyk-operator) for further information.
...
Each path in an Ingress must have its own particular path type. Kubernetes offers three types of path types: `ImplementationSpecific`, `Exact`, and `Prefix`. Currently, not all path types are supported. The below table shows the unsupported path types for [Sample HTTP Ingress Resource](/5.7/#set-up-manifest-for-http) based on the examples in the [Kubernetes Ingress documentation](https://kubernetes.io/docs/concepts/services-networking/ingress/#examples).
...
Please bear in mind that if `proxy.strip_listen_path` is set to true on API Definition, Tyk strips the listen-path (for example, the listen-path for the Ingress under [Sample HTTP Ingress Resource](/5.7/#set-up-manifest-for-http) is /httpbin) with an empty string.
...
Tyk, with release *v4.0* offers [GraphQL federation](/5.7/api-management/graphql#federation-version-support) that allows you to divide GraphQL implementation across multiple back-end
...
Tyk Operator uses Custom Resources called [SubGraph](/5.7/#subgraph) and [SuperGraph](/5.7/#supergraph), that allows users to model the relationship between Subgraphs and Supergraphs.
...
Let's assume that a developer responsible for the [Users SubGraph](/5.7/#users-subgraph) would like to delete `username` field from the Users SubGraph.
Also, the [Supergraph](/5.7/#supergraph-1) called Social Media already uses the Users Subgraph.
...
each of the following [hooks](/5.7/api-management/plugins/plugin-types#plugin-and-hook-types) in the API request
lifecycle: [Pre (Request)](/5.7/api-management/plugins/plugin-types#request-plugins), [Authentication](/5.7/api-management/plugins/plugin-types#authentication-plugins),
[Post (Request)](/5.7/api-management/plugins/plugin-types#request-plugins), [Post
Authentication](/5.7/api-management/plugins/plugin-types#request-plugins), [Response](/5.7/api-management/plugins/plugin-types#response-plugins)
and [Analytics](/5.7/api-management/plugins/plugin-types#analytics-plugins). Subsequently, users can execute, or “hook”, their
...
Please refer to [Analytics Plugins](/5.7/api-management/plugins/plugin-types#analytics-plugins) to learn how to configure Analytics
...
languages](/5.7/api-management/plugins/overview#plugin-driver-names) section for list of supported plugin driver names.
...
[per-endpoint](/5.7/api-management/plugins/plugin-types#per-endpoint-custom-plugins) level plugins are not currently
...
2. **Create API Definition Resource**
    ...
    *http://httpbin.org*.
    ...
    - The `driver` configuration parameter is set to `otto` at line 15, since our plugin is a Javascript plugin. For other
      valid values please refer to the [plugins driver page](/5.7/api-management/plugins/overview#plugin-driver-names).
    - A plugin hook configuration block is specified at line 16, containing the `name` and `path` for our plugin. The plugin
      ...
      middleware. Valid values are the same as the [Tyk Classic API Definition](/5.7/#how-it-works) equivalent, i.e.
...
request lifecycle](/5.7/api-management/traffic-transformation#request-middleware-chain).
...
bundles](/5.7/api-management/plugins/overview#plugin-bundles), load them into the Tyk Gateway, and configure an API
...
An [OperatorContext](/5.7/#multi-tenancy-in-tyk) specifies the parameters for connecting and authenticating with a Tyk Dashboard. Below is an example of how to define an `OperatorContext`:
...
The concept of [internal looping](/5.7/advanced-configuration/transform-traffic/looping) allows you to use URL Rewriting to redirect your URL to *another API endpoint* or to *another API* in the Gateway. In Tyk, looping is generally targeted using the `tyk://<API_ID>/<path>` scheme, which requires prior knowledge of the `API_ID`. Tyk Operator simplifies the management and transformation of API traffic within Kubernetes environments by abstracting APIs as objects, managing them and dynamically assigning `API_ID`s by its Kubernetes metedata name and namespace.

### Configuring looping to internal ApiDefinition resources

Looping can be configured within Tyk Operator for [URL Rewrites](/5.7/#url-rewrites), [URL Rewrite Triggers](/5.7/#url-rewrite-triggers) and [Proxy to internal APIs](/5.7/#proxy-to-internal-apis) by configuring the `rewrite_to_internal` in `url_rewrite`, `rewrite_to_internal` in `triggers`, and `proxy.target_internal` fields respectively with these properties:
...
[URL rewriting](/5.7/transform-traffic/url-rewriting#url-rewrite-middleware) in Tyk enables the alteration of incoming API request paths to align with the expected endpoint format of your backend services.
...
[Triggers](/5.7/transform-traffic/url-rewriting#url-rewrite-triggers) are configurations that specify actions based on certain conditions present in HTTP headers, query parameters, path parameters etc.
...
The process for configuring internal looping in triggers to is similar to that explained in section [URL Rewrites](/5.7/#url-rewrites).
...
Internal looping can also be used for [proxying to internal APIs](/5.7/advanced-configuration/transform-traffic/looping).
...
API name can be set through `x-tyk-api-gateway.info.name` field in [Tyk OAS API Definition](/5.7/api-management/gateway-config-tyk-oas#tyk-oas-api-definition-object) object.
...
API active state can be set through `x-tyk-api-gateway.info.state.active` field in [Tyk OAS API Definition](/5.7/api-management/gateway-config-tyk-oas#tyk-oas-api-definition-object) object.
...
API accessibility can be set through `x-tyk-api-gateway.info.state.internal` field in [Tyk OAS API Definition](/5.7/api-management/gateway-config-tyk-oas#tyk-oas-api-definition-object) object.
...
If you already have API configurations created in the Tyk Dashboard and want to start using Tyk Operator to manage these APIs, you can include the existing API ID in the manifest under the `x-tyk-api-gateway.info.id` field in [Tyk OAS API Definition](/5.7/api-management/gateway-config-tyk-oas#tyk-oas-api-definition-object) object.
...
### API Categories
[API categories](/5.7/api-management/dashboard-configuration#governance-using-api-categories) are configured differently for Tyk OAS APIs and Tyk Classic APIs. Please see below for examples.
...
For a Tyk Classic API, you can specify the category name using the `name` field with a `#` qualifier. This will categorize the API in the Tyk Dashboard. See [How API categories work](/5.7/api-management/dashboard-configuration#tyk-classic-apis) to learn about limitations on API names.
...
`versioning` is configured at `order-api` (v1), the Base API, and it has similiar structure as [Tyk OAS API Definition](/5.7/api-management/api-versioning#configuring-api-versioning-in-the-tyk-oas-api-definition):
...
For Tyk Classic API, versioning can be configured via `ApiDefinition` custom resource definition (CRD). See [Tyk Classic versioning](/5.7/api-management/api-versioning#tyk-classic-api-versioning-1) for a comprehensive example of configuring API versioning for Tyk Classic API with Tyk Operator.

### API Ownership

Please consult the [API Ownership](/5.7/api-management/user-management#api-ownership) documentation for the fundamental concepts of API Ownership in Tyk and [Operator Context](/5.7/#multi-tenancy-in-tyk) documentation for an overview of the use of OperatorContext to manage resources for different teams effectively.
...
The TykOasApiDefinition Custom Resource Definition (CRD) manages [Tyk OAS API Definition object](/5.7/api-management/gateway-config-tyk-oas#tyk-oas-api-definition-object) within a Kubernetes environment. This CRD enables the integration and management of Tyk API definitions using Kubernetes-native tools, simplifying the process of deploying and managing OAS APIs on the Tyk Dashboard.

##### TykOasApiDefinition Features

`TykOasApiDefinition` can support all [features of the Tyk OAS API](/5.7/api-management/gateway-config-tyk-oas#tyk-oas-api-feature-status). You just need to provide the Tyk OAS API definition via a ConfigMap. In addition to managing the CRUD (Create, Read, Update, Delete) of Tyk OAS API resources, the Tyk Operator helps you better manage resources through object linking to Ingress, Security Policies, and certificates stored as Kubernetes secrets. See below for a list of Operator features and examples:

| Features | Support | Supported From | Comments | Example |
| :---------- | :--------- | :----------------- | :---------- | :-------- |
| API Category | ✅      | v1.0 | - | [Manage API Categories](/5.7/#api-categories) |
| API Version | ✅      | v1.0 | - | [Manage API versioning](/5.7/#api-versioning) |
| API Ownership via OperatorContext | ✅      | v1.0 | - | [API Ownership](/5.7/api-management/user-management#when-to-use-api-ownership) |
| Client Certificates | ✅      | v1.0 | - | [Manage TLS certificate](/5.7/#tls-certificates) |
| Custom Domain Certificates | ✅      | v1.0 | - | [Manage TLS certificate](/5.7/#tls-certificates) |
| Public keys pinning | ✅      | v1.0 | - | [Manage TLS certificate](/5.7/#tls-certificates) |
| Upstream mTLS | ✅      | v1.0 | - | [Manage TLS certificate](/5.7/#tls-certificates) |
| Kubernetes Ingress | ✅      | v1.0 | - | [Kubernetes Ingress Controller](/5.7/#control-kubernetes-ingress-resources) |
| Link with SecurityPolicy | ✅      | v1.0 | - | [Protect an API](/5.7/#add-a-security-policy-to-your-api) |

#### TykStreamsApiDefinition CRD
The TykStreamsApiDefinition Custom Resource Definition (CRD) manages [Async API configuration](/5.7/api-management/event-driven-apis#configuration-as-code) within a Kubernetes environment.

##### TykStreamsApiDefinition Features

`TykStreamsApiDefinition` can support all features of [Tyk Streams](/5.7/api-management/event-driven-apis#) and [Tyk OAS API](/5.7/api-management/gateway-config-tyk-oas#tyk-oas-api-feature-status). You just need to provide the Tyk Streams API definition via a ConfigMap. In addition to managing the CRUD (Create, Read, Update, Delete) of Tyk Streams API resources, the Tyk Operator helps you better manage resources through object linking to Security Policies. See below for a list of Operator features and examples:
...
See [Release notes](/5.7/developer-support/release-notes/operator) to check for each Tyk Operator release,
...
#### Security Policy CRD
The SecurityPolicy custom resource defines configuration of [Tyk Security Policy object](/5.7/api-management/policies).

Here are the supported features:

| Features                       | Support   | Supported From | Example |
| :-------------------------------- | :----------- | :---------------- | :--------- |
| API Access                     | ✅        | v0.1           | [API Access](/5.7/#define-the-security-policy-manifest)        |
| Rate Limit, Throttling, Quotas | ✅        | v0.1           | [Rate Limit, Throttling, Quotas](/5.7/#define-the-security-policy-manifest)        |
| Meta Data & Tags               | ✅        | v0.1           | [Tags and Meta-data](/5.7/#define-the-security-policy-manifest)        |
| Path and Method based permissions | ✅     | v0.1           | [Path based permission](/5.7/#security-policy-example)        |
| Partitions                     | ✅        | v0.1           | [Partitioned policies](/5.7/#security-policy-example)       |
| Per API limit                  | ✅        | v1.0           | [Per API Limit](/5.7/#security-policy-example)        |
| Per-Endpoint limit             | ✅        | v1.0           | [Per Endpoint Limit](/5.7/#security-policy-example)        |
