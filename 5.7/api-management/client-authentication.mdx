...
'og:description': "Learn how to apply the most appropriate authentication method to secure access your APIs with Tyk. Here you will find everything there is to know about authenticating and authorizing API clients with Tyk."
...
<Card title="OAuth 2.0" href="/5.7/api-management/client-authentication/#use-tyk-as-an-oauth-20-authorization-server">
Delegate authentication using one of the most widely used open standard protocols
</Card>

<Card title="JWT" href="/5.7/basic-config-and-security/security/authentication-authorization/json-web-tokens">
Securely transmit information between parties.
</Card>

<Card title="Basic Auth" href="/5.7/api-management/client-authentication/#use-basic-authentication">
Secure APIs with username and password credentials.
</Card>

<Card title="Auth Tokens" href="/5.7/api-management/client-authentication/#use-auth-tokens">
Implement token-based authentication for API access.
</Card>

<Card title="mTLS" href="/5.7/api-management/client-authentication/#use-mutual-tls">
Establish secure channels with two-way certificate verification.
</Card>

<Card title="HMAC" href="/5.7/api-management/client-authentication/#sign-requests-with-hmac">
Verify message integrity using shared secret keys.
</Card>

{/* To be added
<Card title="RSA" href="/5.7/api-management/client-authentication/#sign-requests-with-rsa">
Verify message integrity using shared secret certificates.
</Card> */}

<Card title="Custom Authentication" href="/5.7/api-management/client-authentication/#custom-authentication">
Create custom plugins to implement specific authentication requirements.
</Card>

<Card title="Open Access" href="/5.7/api-management/client-authentication/#open-no-authentication">
...
- **Multi-API Access**: Enable access to multiple APIs using a single OAuth token; configure one API for OAuth 2.0 token issuance and the other APIs with the [Auth Token](/5.7/#use-auth-tokens) method, linking them through a common policy

*Tyk as OAuth authorization server* supports the following *grant types*:

- [Authorization Code Grant](/5.7/#using-the-authorization-code-grant): the *client* is redirected to an *identity server* where the *user* must approve access before an *access token* will be issued
- [Client Credentials Grant](/5.7/#using-the-client-credentials-grant): used for machine-to-machine access, authentication is performed using only the *client Id* and *client secret*
- [Resource Owner Password Grant](/5.7/#using-the-resource-owner-password-grant) (a.k.a. Password Grant): only for use where the *client* is highly trusted, as the *client* must provide the *Resource Owner*'s own credentials during authentication
...
- [register a client app](/5.7/#client-app-registration) for each client that needs to access the API
- [configure your API proxy](/5.7/#configuring-your-api-proxy) to use the *Tyk OAuth 2.0* authentication method
...
1. *Client apps* are registered per-API, so the first step is to [configure Tyk OAuth 2.0](/5.7/#configuring-your-api-proxy) as the security method to be used for the API. With this done, you can navigate to the OAuth Client management screen for the API from the **Actions** menu on the **Created APIs** screen:
...
- redirect URI
- [optional] [security policies](/5.7/#manage-client-access-policies) to be applied to access tokens generated for the client
- [optional] [metadata](/5.7/api-management/policies#what-is-a-session-metadata) to be added to the access tokens
...
| :--- | :--- | :--- |
| Register a new client app | `POST /api/apis/oauth/{{api-id}}` | [link](/5.7/api-management/dashboard-configuration#create-a-new-oauth20-client) |
| Get a list of registered client apps | `GET /api/apis/oauth/{{api-id}}` | [link](/5.7/api-management/dashboard-configuration#list-oauth-clients) |
| Get the details of a client app | `GET /api/apis/oauth/{{api-id}}/{{client_id}}` | [link](/5.7/api-management/dashboard-configuration#get-an-oauth20-client) |
| Delete a client app | `DELETE /api/apis/oauth/{{api-id}}/{{client_id}}` | [link](/5.7/api-management/dashboard-configuration#delete-oauth-client) |


### Using the Authorization Code Grant

When using Tyk as the Authorization Server with the Authorization Code grant, the following steps are followed after [registering the Client App](/5.7/#client-app-registration):

<img src="/img/diagrams/diagram_docs_authorization-code-grant-type@2x.png" alt="Authorization grant type flow" />

**Explanatory notes:**

(1) *client* makes a request to the [authorization endpoint](/5.7/#authorization-request) on the *Auth Server*
...
(6) when the *user* successfully authenticates and authorizes the request, the *Identity Server* must request an [Authorization Code](/5.7/#authorization-code-request) from the *Auth Server*

(8) The *Identity Server* provides the *Authorization Code* to the *client*

(9) The *client* exchanges the *Authorization Code* for an [Access Token](/5.7/#exchange-the-authorization-code-for-an-access-token) from the *Auth Server*

(10) The *client* uses the *Access Token* to authenticate with the protected API using the [Auth Token](/5.7/#use-auth-tokens) method
...
The identity server will need access to the Tyk Dashboard API to [obtain an Authorization Code](/5.7/api-management/dashboard-configuration#oauth20-authorization-code).
...
This endpoint is protected using [Basic Authentication](/5.7/#use-basic-authentication)) where the username is the *client Id* and the password is the *client secret*.
...
This command, issued by the *client* is the final step to obtain an access token for the `/my-api` proxy deployed on a Tyk Gateway at `https://tyk.cloud.tyk.io`. The basic auth key is the base64 encoded representation of `my-client-id:my-client-secret` The `client_id` and `redirect_uri` match those provided in the initial [authorization request](/5.7/#authorization-request). The `code` is the *authorization code* provided to the *identity server* in the [authorization code request](/5.7/#authorization-code-request).
...
- `token_type`: set to `bearer` indicating that the access token should be provided in an [Auth Token](/5.7/#use-auth-tokens) request to the protected API
- `refresh_token`: [optional] a special token that can be used in the [Refresh Token](/5.7/#using-refresh-tokens) flow
...
When using Tyk as the *authorization server* with the Client Credentials grant, the *client* accesses resources on behalf of itself rather than on behalf of a *user*, so there is no user login/authorization step (as seen with [Authorization Code grant](/5.7/#using-the-authorization-code-grant)). This flow is ideal for server-to-server interactions.

After [registering the Client App](/5.7/#client-app-registration), the *client* simply requests an access token directly from the authorization server:
...
When using Tyk as the *authorization server* with the Resource Owner Password grant, the *client* provides the *user's* credentials when requesting an access token. There is no user login/authorization step (as seen with [Authorization Code grant](/5.7/#using-the-authorization-code-grant)). **This flow is not recommended and is provided only for integration with legacy clients.**
...
As explained [previously](/5.7/#how-does-tyk-implement-authentication-and-authorization), the AuthN/Z methods to be used to secure an API proxy are configured in the API definition. This permits granular application of the most appropriate method to each API deployed on Tyk Gateway.
...
- [Notifications](/5.7/#oauth-token-notifications) of token issuance will be sent to `http://notifyme.com` with the `X-Tyk-Shared-Secret` header set to `oauth-shared-secret`
...
You configure whether Tyk should issue a refresh token within the [API proxy definition](/5.7/#configuring-your-api-proxy).
...
- [retrieve a list of all tokens for a client app](/5.7/api-management/dashboard-configuration#retrieve-all-current-tokens-for-specified-oauth20-client)
- [revoke a single token](/5.7/api-management/dashboard-configuration#revoke-a-single-oauth-client-token)
- [revoke all tokens for a client app](/5.7/api-management/dashboard-configuration#revoke-all-oauth-client-tokens)
...
The example given [above](/5.7/#using-the-tyk-oas-api-definition) includes the configuration necessary to issue notifications for token issuance (see lines 48-51 in the example).
...
Authentication type is configured within your API Definition; this can be done via the [Tyk Dashboard](/5.7/#enable-basic-authentication-using-the-tyk-dashboard) or directly within the [API Definition file](/5.7/#enable-basic-authentication-in-your-file-based-api-definition).
...
```
We have full tutorials to guide you to [create an API Key](/5.7/api-management/gateway-config-managing-classic#access-an-api) via the Dashboard. 
...
[See Basic Authentication via the Dashboard API](/5.7/api-management/dashboard-configuration#basic-authentication-api)
...
For the server in `common name` specify a domain, or just pass `-subj "/CN=localhost"` to OpenSSL command. Then follow our [TLS and SSL Guide](/5.7/api-management/certificates).
...
<h4 id="tyk-operator-oas">Via Tyk Operator using Tyk OAS API Definition</h4>
...
To learn more about using Tyk Golang Plugins, go [here](/5.7/api-management/plugins/golang)
...
* See [Custom Authentication with a Python plugin](/5.7/api-management/plugins/rich-plugins#custom-authentication-plugin-tutorial) for a detailed example of a custom Python plugin.
* See [JavaScript Middleware](/5.7/api-management/plugins/javascript#) for more details on using JavaScript Middleware. 
...
<Note>
Keyless APIs cannot be selected for [Access Rights](/5.7/api-management/gateway-config-managing-classic#secure-an-api) in a security policy.
</Note>
...
<Note>
Tyk has previously offered two types of OAuth authentication flow; [Tyk as the authorization server](/5.7/#use-tyk-as-an-oauth-20-authorization-server) and Tyk connecting to an external *auth server* via a dedicated *External OAuth* option. The dedicated external *auth server* option was deprecated in Tyk 5.7.0.
<br />

For third-party OAuth integration we recommend using the JSON Web Token (JWT) middleware which is described [above](/5.7/basic-config-and-security/security/authentication-authorization/json-web-tokens), which offers the same functionality with a more streamlined setup and reduced risk of misconfiguration.
<br />
...
- `issuedAtValidationSkew` , `notBeforeValidationSkew`, `expiresAtValidationSkew` can be used to [configure clock skew](/5.7/basic-config-and-security/security/authentication-authorization/json-web-tokens#adjust-jwt-clock-skew-configuration) for json web token validation.
...
For integration with a third-party OIDC provider we recommend using the JSON Web Token (JWT) middleware which is described [above](/5.7/basic-config-and-security/security/authentication-authorization/json-web-tokens), which offers the same functionality with a more streamlined setup and reduced risk of misconfiguration.
<br />
...
[OpenID Connect](https://openid.net/developers/how-connect-works) (OIDC) builds on top of OAuth 2.0, adding authentication. You can secure your APIs on Tyk by integrating with any standards compliant OIDC provider using [JSON Web Tokens](/5.7/basic-config-and-security/security/authentication-authorization/json-web-tokens) (JWTs).
...
Note that where we talk about keys here, we are referring to [Session Objects](/5.7/api-management/policies#what-is-a-session-object), also sometimes referred to as Session Tokens

### Key expiry

Tyk's API keys ([token session objects](/5.7/api-management/policies#session-object)) have an `expires` field. This is a UNIX timestamp and, when this date/time is reached, the key will automatically expire; any subsequent API request made using the key will be rejected.
...
<Note>
There is a risk, when configuring API-level lifetime, that a key will be deleted before it has expired, as `session_lifetime` is applied regardless of whether the key is active or expired. To protect against this, you can configure the [session_lifetime_respects_key_expiration](/5.7/tyk-oss-gateway/configuration#session_lifetime_respects_key_expiration) parameter in your `tyk.conf`, so that keys that have exceeded their lifetime will not be deleted from Redis until they have expired.
</Note>


This feature works nicely with [JWT](/5.7/basic-config-and-security/security/authentication-authorization/json-web-tokens) or [OIDC](/5.7/#integrate-with-openid-connect-deprecated) authentication methods, as the keys are created in Redis the first time they are in use so you know when they will be removed. Be extra careful in the case of keys created by Tyk (Auth token or JWT with individual secrets) and set a long `session_lifetime`, otherwise the user might try to use the key **after** it has already been removed from Redis.

#### Gateway-level key lifetime control

You can set a global lifetime for all keys created in the Redis by setting [global_session_lifetime](/5.7/tyk-oss-gateway/configuration#global_session_lifetime) in the `tyk.conf` file; this parameter is an integer value in seconds.

To enable this global lifetime, you must also set the [force_global_session_lifetime](/5.7/tyk-oss-gateway/configuration#force_global_session_lifetime) parameter in the `tyk.conf` file.
...
Defining Access Policies: Use Tyk’s policies to refine API access controls, rate limits, and quotas. This lets you align your security model with business needs and enhance user experience through granular permissions. You can learn more about policies [here](/5.7/api-management/policies).

Exploring API Analytics: Leverage Tyk’s analytics to monitor access patterns, track usage, and gain insights into potential security risks or high-demand endpoints. Understanding usage data can help in optimizing API performance and enhancing security measures. You can learn more about analytics [here](/5.7/tyk-dashboard-analytics).