...
Explore our guides for [Datadog](/5.7/api-management/logs-metrics#datadog), [Dynatrace](/5.7/api-management/logs-metrics#dynatrace), [Jaeger](/5.7/api-management/logs-metrics#using-docker) and [New Relic](/5.7/api-management/logs-metrics#new-relic) for further info on how to integrate with 3rd party observability vendors.
...
Tyk allows you to capture and analyze logs related to API requests and responses in the [Log Browser](/5.7/api-management/dashboard-configuration#activity-logs) . You can optionally enable detailed recording for the requests per API level or per Key level to store inbound request and outbound response data. You can [enable debug modes](/5.7/api-management/troubleshooting-debugging#capturing-detailed-logs) for selected APIs and send the detail logs to one or more Pump backend instances.
...
If configured, then a [logging event handler](/5.7/api-management/gateway-events#logging-api-events-1) will also report **API events** to the configured log output.

When contacting support, you may be asked to change the logging level as part of the support handling process. See [Support Information](/5.7/api-management/troubleshooting-debugging#support-information) for more details.
...
1. Via an Environment Variable to affect [all Tyk components](/5.7/api-management/logs-metrics#setting-log-verbosity-for-all-tyk-components)
2. Just for the [Gateway](/5.7/api-management/logs-metrics#setting-log-verbosity-for-the-gateway-only) via your `tyk.conf` config file 
...
Tyk Gateway can be configured to generate a record of every request made to APIs deployed on the gateway and the response sent back to the originating client. The details of these transactions will be stored in the Redis storage, from which they can be transferred to persistent storage using [Tyk Pump](/5.7/tyk-pump). In Tyk these transaction logs are also referred to as traffic analytics or simply analytics.
...
Traffic analytics logging is enabled in the Gateway configuration using the `enable_analytics` [option](/5.7/tyk-oss-gateway/configuration#enable_analytics) (or using the equivalent environment variable `TYK_GW_ENABLEANALYTICS`).

The transaction records generated by the Gateway are stored in Redis, from which Tyk Pump can be configured to transfer them to the desired persistent storage. When using Tyk Dashboard, the [Aggregate Pump](/5.7/api-management/tyk-pump#tyk-dashboard) can be used to collate aggregated data that is presented in the [analytics](/5.7/api-management/dashboard-configuration#traffic-analytics) screens of the Tyk Dashboard.

The Gateway will not, by default, include the request and response payloads in the transaction records. This minimizes the size of the records and also avoids logging any sensitive content. The [detailed recording](/5.7/api-management/logs-metrics#enable-detailed-recording) option is provided if you need to capture the payloads in the records.

You can suppress the generation of transaction records for any endpoint by enabling the [do-not-track middleware](/5.7/api-management/traffic-transformation#do-not-track-overview) for that endpoint. This provides granular control over request tracking.

You can find details of all the options available to you when configuring analytics in the Gateway in the [reference documentation](/5.7/tyk-oss-gateway/configuration#analytics_config).



### Enable Detailed Recording

When [traffic analytics](/5.7/api-management/logs-metrics#logging-api-traffic) are enabled the Gateway will not, by default, include the request and response payloads in these transaction records. This minimizes the size of the records and also avoids logging any sensitive content.
...
1. [API level](/5.7/api-management/logs-metrics#configure-at-api-level)
2. [Key level](/5.7/api-management/logs-metrics#configure-at-key-level)
3. [Gateway level](/5.7/api-management/logs-metrics#configure-at-gateway-level)
...
<h5 id="tyk-classic">Tyk Classic APIs</h5>
...
The process for configuring detailed recording using Tyk Operator is similar to that explained in [Detailed recording with Tyk Classic APIs](/5.7/#tyk-classic).
...
An alternative approach to controlling detailed recording is to enable it only for specific [access keys](/5.7/api-management/policies#what-is-a-session-object). This is particularly useful for debugging purposes where you can configure detailed recording only for the key(s) that are reporting issues.
...
Detailed recording can be configured at the system level, affecting all APIs deployed on the Gateway, by enabling the [detailed recording](/5.7/tyk-oss-gateway/configuration#analytics_configenable_detailed_recording) option in `tyk.conf`.
...
Refer this [docs](/5.7/api-management/traffic-transformation#do-not-track-overview) for more information.
...
For further guidance on configuring your observability back-end, explore our guides for [Datadog](/5.7/#datadog), [Dynatrace](/5.7/#dynatrace), [Jaeger](/5.7/#jaeger) and [New Relic](/5.7/#new-relic).
...
1. **Enable at Gateway Level**
    ...
    By default, OpenTelemetry spans are exported using the `gRPC` protocol to `localhost:4317`. For more configuration options and default values, refer to the [OpenTelemetry configuration details](/5.7/tyk-oss-gateway/configuration#opentelemetry). 
...
For more in-depth information on the configuration options available, please refer to our [OpenTelemetry Configuration Details Page](/5.7/tyk-oss-gateway/configuration#opentelemetry).
...
1. **Configure Tyk API Gateway**
    ...
    * In your docker-compose.yml file for Tyk Gateway, add the following environment variables:
    ...
    After enabling OpenTelemetry at the Gateway level, you can activate [detailed tracing](/5.7/api-management/logs-metrics#opentelemetry) for specific APIs by editing their respective API definitions. Set the `detailed_tracing` option to either true or false. By default, this setting is false.
...
3. **Testing and Verifying Traces**
    ...
    3. Create a simple `httpbin` API using Tyk Dashboard. You can follow the [Tyk Dashboard documentation](/5.7/api-management/gateway-config-managing-classic#create-an-api) for more information.
...
For Kubernetes instructions, please refer to [How to integrate with Jaeger on Kubernetes](/5.7/#using-kubernetes).
...
For Docker instructions, please refer to [How to integrate with Jaeger on Docker](/5.7/#using-docker).
...
OpenTracing is now deprecated. We have introduced support of [OpenTelemetry](/5.7/api-management/logs-metrics#opentelemetry) in v5.2. We recommend users to migrate to OpenTelemetry for better supports of your tracing needs.
</Warning>


### Supported observability tools
- [Jaeger](/5.7/api-management/logs-metrics#jaeger-1)
- [Zipkin](/5.7/api-management/logs-metrics#zipkin)
- [New Relic](/5.7/api-management/logs-metrics#new-relic-1)
...
<Note>
[Tyk Gateway 5.2](/5.7/developer-support/release-notes/gateway.md) now includes OpenTelemetry Tracing. We recommend migrating to OpenTelemetry for better trace insights and more comprehensive support. This change will offer you significant advantages in managing your distributed tracing needs.

Subsequently, we recommend following this guide [Exporting OpenTelemetry Distributed Traces to Jaeger](/5.7/#using-docker).
</Note>
...
OpenTracing is now deprecated. We have introduced support of [OpenTelemetry](/5.7/{{ref "api-management/logs-metrics#opentelemetry"}}) in v5.2. We recommend users to migrate to OpenTelemetry for better supports of your tracing needs.
</Note>
