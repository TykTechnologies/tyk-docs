...
2. [Set the cryptographic signing method](/5.7/#set-up-jwt-signing-method) to `HMAC (shared)` and the public secret as `tyk123`
...
If Tyk cannot find a `pol` claim, it will apply this Default Policy. Select a policy that gives access to this API we are protecting, or [go create one first](/5.7/api-management/gateway-config-managing-classic#secure-an-api) if it doesn't exist.
...
If you are looking to POC this functionality in Kubernetes, you can run a fully worked-out example using our tyk-k8s-demo library. You can read more [here](/5.7/tyk-self-managed#kubernetes-demo).
...
- HMAC shared secret
- ECDSA
- [Public JWKS URL](/5.7/#enable-dynamic-public-key-rotation-using-jwks)
