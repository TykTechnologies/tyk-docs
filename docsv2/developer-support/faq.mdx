...
1. **If there's a configuration error or an unwanted change, can I easily revert to a previous API or Policy configuration? What are the options or best practices for effective rollbacks?**
    ...
    You can configure [Tyk-Sync](/docsv2/docsv2/api-management/automations/sync) to synchronise APIs and Policies with any version control system, like GitHub or GitLab and use it to perform roll back. Keys are not synchronised with Tyk Sync.
...
5. **How to Backup Middleware Directory for Custom Plugins ?**
    ...
    Details on middleware installation for [Tyk Self-Managed users](/docsv2/docsv2/api-management/plugins/javascript#installing-middleware-on-tyk-self-managed) (users of the licensed product).
    Details on middleware installation for [Tyk Open Source users](/docsv2/docsv2/api-management/plugins/javascript#installing-middleware-on-tyk-oss)
...
6. **How to Backup Tyk API Definitions Directory - For Tyk Open source users ?**
    ...
    **Note:** Tyk's default name is `/apps`. You might use a different name for it based on this [config field](/docsv2/docsv2/tyk-stack/tyk-gateway/important-prerequisites#tyk-config)
...
7. **How to Backup Tyk Policies Directory - For Tyk Open source users ?**
    ...
    **Note:** Tyk's default name is `/policies`. You might use a different name for it based on this [config field](/docsv2/docsv2/tyk-stack/tyk-gateway/important-prerequisites#path-to-policies-file)
