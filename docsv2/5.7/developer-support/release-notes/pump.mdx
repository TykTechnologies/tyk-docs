...
Tyk Pump now offers [FIPS 140-2](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.140-2.pdf) compliance. For further details please consult [Tyk API Management FIPS support](/docsv2/5.7/developer-support/release-types/fips-release)
...
Added [FIPS compliance](/docsv2/5.7/developer-support/release-types/fips-release) for Tyk Pump.
</Expandable>
</li>
</ul>
...
- When [`omit_configfile`](/docsv2/5.7/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables.md#omit_config_file) is set to `true`, Pump will not try to load the config file and spit out error logs

##### Updated
- Updated the default Hybrid Pump RPC pool size from 20 to 5 connections in order to reduce default CPU and memory footprint. See [Pump configurations](/docsv2/5.7/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables.md#pumpshybridmetarpcpoolsize)
