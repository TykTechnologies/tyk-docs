...
The compatibility matrix table shown above will be part of upcoming [Gateway release notes](/docsv2/5.7/developer-support/release-notes/gateway) for versions x.x and beyond. Additionally, these release notes will list tested third-party dependencies like *PostgreSQL, MongoDB, Redis* and more. */}
...
When selecting the "Latest" option, users can only choose the latest feature branch ([Gateway Release Notes](/docsv2/5.7/developer-support/release-notes/gateway)).
...
We have step-by-step install guides for various architectures and installation types. Refer to [upgrading tyk](/docsv2/5.7/developer-support/upgrading) for further details.
