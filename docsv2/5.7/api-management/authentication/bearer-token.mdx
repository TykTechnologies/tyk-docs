...
You can optionally [strip the auth token](/docsv2/5.7/api-management/client-authentication#managing-authorization-data) from the request prior to proxying to the upstream using the `authentication.stripAuthorizationData` field  (Tyk Classic: `strip_auth_data`).
...
The Auth Token method can support [Dynamic mTLS](/docsv2/5.7/api-management/client-authentication#dynamic-mtls) where the client can provide a TLS certificate in lieu of a standard Auth Token. This can be configured for an API using the [enableClientCertificate](/docsv2/5.7/api-management/gateway-config-tyk-oas#token)  option (Tyk Classic: `auth.use_certificate`).

## Auth Token with Signature

If you are migrating from platforms like Mashery, which use request signing, you can enable signature validation alongside auth token by configuring the additional [signatureValidation](/docsv2/5.7/api-management/gateway-config-tyk-oas#token) field (Tyk Classic: `auth.signature`).
...
If you have your own identity provider you may want to use that to generate and manage the access tokens, rather than having Tyk generate the tokens. You can use the `POST /tyk/keys/{keyID}` endpoint in the [Tyk Gateway API](/docsv2/5.7/tyk-gateway-api) to import those tokens to Tyk, off-loading access control, quotas and rate limiting from your own application.
...
- select **Strip authorization data** to remove the auth token locations from the request prior to proxying to the upstream, as described [here](/docsv2/5.7/api-management/client-authentication#managing-authorization-data)
- optionally select **Enable client certificate** to enable [Dynamic mTLS](/docsv2/5.7/api-management/client-authentication#dynamic-mtls) for the API, so the client can provide a certificate in place of the token

Note that the [auth token + signature](/docsv2/5.7/api-management/authentication/bearer-token#auth-token-with-signature) option is not available in the Tyk Dashboard API Designer.


## Using Tyk Classic APIs

As noted in the Tyk Classic API [documentation](/docsv2/5.7/api-management/gateway-config-tyk-classic#configuring-authentication-for-tyk-classic-apis), a new Tyk Classic API will use the auth (bearer) token method by default with the token expected in the `Authorization` header, so configuration is slightly different as there is no need to `enable` this method. You should configure the `auth` object for any non-default settings, such as a different token location or Dynamic mTLS.
