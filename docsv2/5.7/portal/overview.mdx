---
title: "Tyk Enterprise Developer Portal -This is 5.7"
...
1. [Connect to a provider (Tyk Self Managed)](/docsv2/5.7/portal/overview#connect-to-a-provider)
2. [Create and import API Products and Plans in your Tyk Self Managed Instance](/docsv2/5.7/portal/overview#create-api-products-and-plans)
3. [Create catalogs and developer audiences](/docsv2/5.7/portal/overview#organization-and-catalog)
4. [Publish API Products and Plans to the Developer portal so that API consumers can access them](/docsv2/5.7/portal/overview#optional-customize-visual-appearance-of-api-products-and-plans)

After the last step, you can start serving APIs to your developer audience.

### Install Developer Portal

Refer the [install guide](/docsv2/5.7/portal/install#installation-options-for-enterprise-developer-portal).
...
When the connection is established, the portal will import policies as API Products to the portal. The [Getting started guide](/docsv2/5.7/portal/overview#connect-to-a-provider) explains how to set up a policy and import it to the portal.
...
| Policy tags              | This is an optional field that can be used to define which policies from Tyk will be imported to the portal. If a tag is defined here, it needs to also be defined in the Policy section in the [Tyk Dashboard](/docsv2/5.7/portal/overview#create-and-import-an-api-product-from-tyk). If this field is left empty in both this provider section and in the policies within Tyk, then all policies will be imported from the Tyk instance. How to include the label in the policy section inside Tyk, is explained in [Publish API Products and plans](/docsv2/5.7/portal/overview#publish-an-api-product) for the public-facing portal. |
...
- A Tyk Self-Managed [installation](/docsv2/5.7/tyk-self-managed#installation-options-for-tyk-self-managed)
- Tyk Self-Managed [added as a provider](/docsv2/5.7/portal/overview#connect-to-a-provider)
- Have APIs [created in your Tyk installation](/docsv2/5.7/api-management/gateway-config-managing-classic#create-an-api).

#### Import API Product and Plan

When integrating with Tyk, the Tyk policies will be imported into the Developer Portal. Depending on the configuration that’s been set in the policy section, the policy will either be imported as an API Product or a Plan. For further details check the [portal key concepts](/docsv2/5.7/portal/overview#developer-portal-concepts) document.
...
2. Select a unique name for the API Product and complete the [product details for customization](/docsv2/5.7/portal/overview#optional-customize-visual-appearance-of-api-products-and-plans) in the **Details** tab. The product name will be the name assigned to the created policy in the Tyk Self-Managed selected provider
...
- A Tyk Self-Managed [installation](/docsv2/5.7/tyk-self-managed#installation-options-for-tyk-self-managed)
- Tyk Self-Managed [added as a provider](/docsv2/5.7/portal/overview#connect-to-a-provider)
- [Created and imported API Products and Plans from Tyk](/docsv2/5.7/portal/overview#create-api-products-and-plans)
...
The Tyk Enterprise Developer Portal uses Organization and Catalogs to segment access to APIs and plans. Therefore, the first thing is to create an organization for your customers. If you don't want to provision organizations manually, you can leverage the [Admin APIs](/docsv2/5.7/product-stack/tyk-enterprise-developer-portal/api-documentation/tyk-edp-api) or enable the [self-service organization registration](/docsv2/5.7/portal/api-consumer#manage-api-consumer-organizations).
...
The default catalogs that are featured when the portal is [bootstrapped](/docsv2/5.7/portal/install#bootstrapping-enterprise-developer-portal) are:
...
1. Navigate to the **API Products** menu, select the product that you want to customize (the Payment API in example below). In this menu you can customize the following aspects of API Products:
    ...
    From that page you can also manage [OAuth2.0 settings](/docsv2/5.7/portal/api-provider#dynamic-client-registration) and add [Get started guides](/docsv2/5.7/portal/api-provider#documentation-for-your-api-products) to your API Products, which is covered in the relevant sections.
...
1. To customize visual appearance of plans, open the **Plans** menu and select the plan you want to customize. You can customize the following settings:
    ...
    - **Scope**: Scope for the [OAuth2.0 settings](/docsv2/5.7/portal/api-provider#dynamic-client-registration). Please refer to the [OAuth2.0 documentation](/docsv2/5.7/portal/api-provider#dynamic-client-registration) for further guidance.
    - **Catalogs**: The catalogs in which this plan is available. Catalogs and organization are [covered](/docsv2/5.7/portal/overview#organization-and-catalog) later within this getting started guide.
