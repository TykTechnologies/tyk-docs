...
This guide covers the essential first steps to take within the Tyk AI Studio UI after successfully deploying the platform using the [Installation Guide](/docsv2/ai-management/ai-studio/deployment-helm-k8s).
...
4.  **Enter Details:**
    ...
    *   **API Key (Using Secrets):**
        *   **IMPORTANT:** Do *not* paste your API key directly here. Instead, use [Secrets Management](/docsv2/ai-management/ai-studio/secrets).
...
This LLM is now available for use within Tyk AI Studio, subject to [User/Group permissions](/docsv2/ai-management/ai-studio/user-management).

For more details, see the [LLM Management](/docsv2/ai-management/ai-studio/llm-management) documentation.
...
*   **Email Configuration:** If using features like user invites or notifications, ensure SMTP settings are correctly configured and test email delivery if possible ([Notifications](/docsv2/ai-management/ai-studio/notifications)).
...
Refer to the **Configuration Options** detailed within the [Installation Guide](/docsv2/ai-management/ai-studio/deployment-helm-k8s) for specifics on setting these values during the deployment process.

## Next Steps

With the initial configuration complete, you can now:

*   Explore [User Management](/docsv2/ai-management/ai-studio/user-management) to create users and groups.
*   Set up [Tools](/docsv2/ai-management/ai-studio/tools) for external API integration.
*   Configure [Data Sources](/docsv2/ai-management/ai-studio/datasources-rag) for RAG.
*   Define [Filters](/docsv2/ai-management/ai-studio/filters) for custom request/response logic.
*   Try out the [Chat Interface](/docsv2/ai-management/ai-studio/chat-interface).
