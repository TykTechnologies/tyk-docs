...
The Tyk Cloud trial does not include access to [Hybrid deployments](#deploy-hybrid-gateways) or the [Developer Portal](/docsv2/portal/overview#).
To try out these capabilities, please get in touch for a [guided evaluation](https://tyk.io/guided-evaluation/) with our team.
</Note>


Welcome to the [Tyk Cloud Platform](/docsv2/tyk-cloud)!
This guide will lead you through the following steps:
1. Signing up with [Tyk Cloud ](/docsv2/tyk-cloud).
2. Creating your first [API](/docsv2/api-management/gateway-config-introduction) using the [Tyk Dashboard](/docsv2/tyk-dashboard).
3. Setting up a [Policy](/docsv2/api-management/policies#what-is-a-security-policy) and Key to secure your APIs.
...
To create Tyk Cloud account follow this [guide](/docsv2/getting-started/create-account).

2. **Get started with your first API with Tyk Dashboard**

To create your API using Tyk Dashboard follow this [guide](/docsv2/getting-started/configure-first-api).
...
<Card title="Configuration" href="/docsv2/tyk-cloud#add-custom-authentication">
**Python custom plugins**

Implement your own custom logic with Python based plugins
</Card>

<Card title="Configuration" href="/docsv2/tyk-cloud#configure-custom-domains">
**Using custom domains**

Configure custom domain for your Dashboard and Developer Portal
</Card>

<Card title="Administration" href="/docsv2/tyk-cloud#managing-environments">
**Manage environments**

Create and manage multiple environments within your Tyk Cloud organization
</Card>

<Card title="Administration" href="/docsv2/tyk-cloud#managing-control-planes">
**Manage deployments**

Create and manage your Cloud Control Plane and Cloud Data Plane deployments
</Card>

<Card title="Administration" href="/docsv2/tyk-cloud#managing-teams-and-users">
**Manage teams & users**

Create teams in your organization, define roles and manage user access
</Card>

<Card title="Account" href="/docsv2/tyk-cloud#manage-accounts-and-billing">
**Manage billing**
...
For further details, please refer to the [release notes](/docsv2/developer-support/release-notes/gateway#530-release-notes) for Tyk Gateway v5.3.0.
</Warning>
...
See [Tyk Classic API versioning](/docsv2/api-management/gateway-config-tyk-classic#tyk-classic-api-versioning) for more details.
...
Your API will appear in your APIs list. If you select **EDIT** from the **ACTIONS** drop-down list, you can see the endpoints (from the [Endpoint Designer](/docsv2/api-management/dashboard-configuration#endpoint-designer)) that have been created as part of the import process.
...
Your Organization is your "container" for all your Environments, Control Planes and Cloud Data Planes. When you setup your Organization when [creating your account](/docsv2/getting-started/create-account), you assign it to a Home Region where all your data is stored. You cannot change this home region after creating your organization.
...
Control Planes are situated in your Organization's home region and provide links to an instance of the [Tyk Dashboard](/docsv2/tyk-dashboard) and the [Developer Portal](/docsv2/tyk-developer-portal). The Dashboard is where you perform all your API tasks. The developer portal allows your 3rd party developers access to your APIs. Cloud Data Planes are then connected to your Control Planes.
...
The following quick start guide explains how to use the [Tyk Data Plane Helm chart](/docsv2/product-stack/tyk-charts/tyk-data-plane-chart) to configure Tyk Gateway that includes:
...
For the complete installation guide and configuration options, please see [Tyk Data Plane Chart](/docsv2/product-stack/tyk-charts/tyk-data-plane-chart).
...
* [Adding APIs](/docsv2/api-management/gateway-config-managing-classic#create-an-api) to Tyk, including REST and GraphQL APIs
* Applying Quotas and Rate limits via [Security Policies](/docsv2/api-management/gateway-config-managing-classic#secure-an-api) and [Keys](/docsv2/api-management/gateway-config-managing-classic#access-an-api)
* [Securing](/docsv2/api-management/security-best-practices#securing-apis-with-tyk) your APIs
* Viewing granular [Analytics](/docsv2/api-management/dashboard-configuration#traffic-analytics) for your Tyk managed APIs
* [Transform traffic](/docsv2/api-management/traffic-transformation#) with the Tyk API Designer
* Add integration options such as [SSO](/docsv2/api-management/external-service-integration#single-sign-on-sso) and [3rd Party IdentityProviders](/docsv2/api-management/external-service-integration)
...
* [Team Admin](#assign-user-roles) - Can only manage the users of the team they are a member of.
    ...
    You can also [add users to the Tyk Dashboard](/docsv2/api-management/user-management#manage-tyk-dashboard-users) itself instead of inviting them as Tyk Cloud users. These users would likely be your API Developers and Engineers who manage the APIs.
    </Note>
...
You can [add users to the Tyk Dashboard](/docsv2/api-management/user-management#manage-tyk-dashboard-users) itself instead of inviting them as Tyk Cloud users. These users would likely be your API Developers and Engineers who manage the APIs.   
...
| Grow <p>[Standard Support SLA](/docsv2/developer-support/support)</p> | For single teams with high traffic. | This plan includes all of the features of the Tyk Stack. In this plan, you have **[Hybrid Gateways](#deploy-hybrid-gateways)** as an add on, along with standard support. |
| Scale <p>[Enhanced Support SLA](/docsv2/developer-support/support)</p> | For global organizations with multiple teams, requiring gateway deployments in multiple locations. | This plan includes all of the features of the Tyk Stack. **Enhanced(silver) support** will be provided. |
...
Our Developer Portal is completely customizable. See [Portal Customization](/docsv2/tyk-developer-portal/customise) for more details.
...
* [Python Plugins](/docsv2/api-management/plugins/rich-plugins#overview)
* [JSVM](/docsv2/api-management/plugins/javascript#)
...
Next you'll [set up the Python authentication code bundle](/docsv2/tyk-cloud#create-a-python-code-bundle).
...
| File              | Description                                                                                                                                                                                                                                                                                       |
|-------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| custom_middleware | contains the middleware settings like the plugin driver you want to use (driver) and the hooks that your plugin will expose. We use the   **auth_check** for this tutorial. For other hooks see [here](/docsv2/api-management/plugins/rich-plugins#coprocess-dispatcher---hooks). |
...
* You import decorators from the Tyk module that gives us the Hook decorator, and we import [Tyk Python API helpers](/docsv2/api-management/plugins/rich-plugins#tyk-python-api-methods)
...
* A plugin bundle is a packaged version of the plugin, it may also contain a cryptographic signature of its contents. The -y flag tells the Tyk CLI tool to skip the signing process in order to simplify this tutorial. For more information on the Tyk CLI tool, see [here](/docsv2/api-management/plugins/overview#how-plugin-bundles-work).
...
1. Firstly you will need a local Tyk Gateway installation that allows you to create your Python plugin bundle. We recommend [installing our Self-Managed version on Ubuntu Bionic 18.04](/docsv2/tyk-self-managed#install-tyk-on-debian-or-ubuntu).
...
We recommend that all users to migrate to the `tyk-data-plane` Chart. Please review the [Configuration](/docsv2/product-stack/tyk-charts/tyk-data-plane-chart#configuration) section of the new helm chart and cross-check with your existing configurations while planning for migration.
</Warning>
...
<Note>
Please make sure you are installing Redis versions that are supported by Tyk. Please refer to Tyk docs to get list of [supported versions](/docsv2/tyk-self-managed#redis-1).
</Note>
...
#### Tyk Cloud Storage
When a client makes a request to a Tyk Gateway deployment, the details of the request and response are captured and [stored in Redis](/docsv2/api-management/dashboard-configuration#traffic-analytics). Tyk Pump processes the records from Redis and forwards them to MongoDB. Finally, Tyk Cloud reads that data from MongoDB and displays its size(bytes) in the _Storage_ section of _Monitoring_. 
...
Since this functionality is powered by Tyk Gateway's OpenTelemetry integration, we recommend reviewing our comprehensive [OpenTelemetry documentation](/docsv2/api-management/logs-metrics#opentelemetry)
