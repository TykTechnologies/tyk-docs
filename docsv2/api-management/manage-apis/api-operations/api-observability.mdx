...
Explore our guides for [Datadog](/docsv2/docsv2/api-management/logs-metrics#datadog), [Dynatrace](/docsv2/docsv2/api-management/logs-metrics#dynatrace), [Jaeger](/docsv2/docsv2/api-management/logs-metrics#using-docker) and [New Relic](/docsv2/docsv2/api-management/logs-metrics#new-relic) for further info on how to integrate with 3rd party observability vendors.
...
Tyk allows you to capture and analyze logs related to API requests and responses in the [Log Browser](/docsv2/docsv2/api-management/dashboard-configuration#activity-logs) . You can optionally enable detailed recording for the requests per API level or per Key level to store inbound request and outbound response data. You can [enable debug modes](/docsv2/docsv2/api-management/troubleshooting-debugging#capturing-detailed-logs) for selected APIs and send the detail logs to one or more Pump backend instances.
