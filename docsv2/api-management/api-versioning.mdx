...
- [Version identifiers](/docsv2/docsv2/api-management/api-versioning#version-identifiers)
- [Default version](/docsv2/docsv2/api-management/api-versioning#default-version)
- [Base and child versions](/docsv2/docsv2/api-management/api-versioning#base-and-child-apis)
- [Controlling access to versions](/docsv2/docsv2/api-management/api-versioning#controlling-access-to-versioned-apis)

### Version Identifiers

The version identifier is the method by which the API client specifies which version of an API it is addressing with each request. Tyk supports multiple [locations](/docsv2/docsv2/api-management/api-versioning#version-identifier-location) within the request where this identifier can be placed. Typically the value assigned in the version identifier will be matched to the list of versions defined for the API and, assuming that the client is [authorized to access](/docsv2/docsv2/api-management/api-versioning#controlling-access-to-versioned-apis) that version, Tyk will apply the version specific processing to the request.
...
- [Request URL (path)](/docsv2/docsv2/api-management/api-versioning#request-url-path)
- [Query parameter](/docsv2/docsv2/api-management/api-versioning#query-parameter)
- [Request header](/docsv2/docsv2/api-management/api-versioning#request-header)
...
The upstream (target) URL will be constructed by combining the configured `upstream.url` (`target_url` for Tyk Classic APIs) with the full request path unless configured otherwise (for example, by using the [strip listen path](/docsv2/docsv2/api-management/gateway-config-tyk-oas#listenpath) feature).
...
When using the [Request URL](/docsv2/docsv2/api-management/api-versioning#request-url-path) for the versioning identifier, if Tyk is configured to strip the versioning identifier then the first URL fragment after the `listenPath` (`listen_path` for Tyk Classic APIs) will be deleted prior to creating the proxy URL. If the request does not include a versioning identifier and Tyk is configured to [fallback to default](/docsv2/docsv2/api-management/api-versioning#fallback-to-default), this may lead to undesired behaviour as the first URL fragment of the endpoint will be deleted.
...
Note that any version (*child* or *Base*) can be set as the [default version](/docsv2/docsv2/api-management/api-versioning#default-version).
...
Tyk's access control model supports very granular permissions to versioned APIs using the `access_rights` assigned in the [access keys/tokens](/docsv2/docsv2/api-management/policies#what-is-a-session-object) or [security policies](/docsv2/docsv2/api-management/policies#what-is-a-security-policy) that are applied to keys.

This means that you could restrict client access to only the [Base API](/docsv2/docsv2/api-management/api-versioning#base-and-child-apis), while allowing developers to create and test new versions independently. These will only be added to the "routing table" in the Base API when the API owner is ready and access keys could then be updated to grant access to the new version(s).

Note that an access key will only have access to the `default` version if it explicitly has access to that version (e.g. if `v2` is set as default, a key must have access to `v2` to be able to [fallback to the default](/docsv2/docsv2/api-management/api-versioning#fallback-to-default) if the versioning identifier is not correctly provided in the request.

When using Tyk OAS APIs each version of the API has a unique [API Id](/docsv2/docsv2/api-management/gateway-config-tyk-oas#info), so you simply need to identify the specific versions in the `access_rights` list in the key in the same way that you would add multiple different APIs to a single key.
...
These are largely due to the fact that a separate API definition is generated for each version of a Tyk OAS API, with one designated as the [base](/docsv2/docsv2/api-management/api-versioning#base-and-child-apis) version (which should be exposed on Tyk Gateway with the other (child) versions set to [internal](/docsv2/docsv2/api-management/traffic-transformation#internal-endpoint-1) visibility) whereas all versions of a Tyk Classic API are described by a single API definition.
...
You can configure a Tyk OAS API as a [Base API](/docsv2/docsv2/api-management/api-versioning#base-and-child-apis) by adding the `info.versioning` [object](/docsv2/docsv2/api-management/gateway-config-tyk-oas#versioning) to the [Tyk Vendor Extension](/docsv2/docsv2/api-management/gateway-config-tyk-oas#tyk-vendor-extension).
...
<Note>
If you are using Tyk Classic APIs, please see [this section](/docsv2/docsv2/api-management/api-versioning#versioning-with-tyk-classic-apis).
</Note>
...
2. **Configure the new version**
    ...
    By default, Tyk will have configured the [versioning identifier location](/docsv2/docsv2/api-management/api-versioning#version-identifier-location) as Request Header, with the key `x-api-version`. You can modify this if required, but if you're happy with this then that's it - your new API version is ready for use.
...
- set the [versioning identifier pattern](/docsv2/docsv2/api-management/api-versioning#version-identifier-pattern) (optional, if using URL path location)
- configure [fallback to default behaviour](/docsv2/docsv2/api-management/api-versioning#fallback-to-default)
- configure [version identifier stripping](/docsv2/docsv2/api-management/api-versioning#stripping-version-identifier)
...
All configuration for versioning of Tyk Classic APIs is documented [here](/docsv2/docsv2/api-management/gateway-config-tyk-classic#tyk-classic-api-versioning).
...
4. **Set the default version**
    ...
    You can choose any of your API versions to act as the [default](/docsv2/docsv2/api-management/api-versioning#default-version).
...
As explained, you can directly [configure the version settings](/docsv2/docsv2/api-management/api-versioning#configuring-api-versioning-in-the-api-definition) within the Tyk OAS API definition using the `info.versioning` section of the Tyk Vendor Extension.
...
When creating the [base version](/docsv2/docsv2/api-management/api-versioning#base-and-child-apis) of your API, you do not need to do anything special - the version details will be added when you later create the first child version.

### Creating Child Versions

When you want to create a [child version](/docsv2/docsv2/api-management/api-versioning#base-and-child-apis) for an existing API using the Tyk Dashboard API or Tyk Gateway API, you must provide additional query parameters to link the child and base APIs.
...
These options are also available when [updating an existing API definition](/docsv2/docsv2/api-management/gateway-config-managing-oas#updating-an-api) using the `PATCH /api/apis/oas` or `PATCH /tyk/apis/oas` endpoints.
...
If you need to change these, you should do so within the `info.versioning` section of the API definition for the base version as explained [previously](/docsv2/docsv2/api-management/api-versioning#configuring-api-versioning-in-the-api-definition).
