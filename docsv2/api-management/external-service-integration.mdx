...
This page introduces general features of Tyk Identity Broker (TIB) and how to configure them. If you are looking for global configurations of the TIB deployment refer this [config file](/docsv2/docsv2/tyk-configuration-reference/tyk-identity-broker-configuration).
...
Refer to this [document](/docsv2/docsv2/tyk-identity-broker/tib-rest-api)
...
7. More Docs for the flow can be found on our [GitHub TIB repo README](https://github.com/TykTechnologies/tyk-identity-broker) and our [3rd Party integration docs](/docsv2/docsv2/api-management/external-service-integration)
...
- `/admin/sso` - See [Dashboard Admin API SSO](/docsv2/docsv2/api-management/dashboard-configuration#single-sign-on-api-1) for more details.
- `/api/sso` -  See [Dashboard API SSO](/docsv2/docsv2/api-management/dashboard-configuration#single-sign-on-api) for more details.
...
You already have authorized access to Tyk's Dashboard. If you haven't, get the authorization key by following this [guide](/docsv2/docsv2/api-management/user-management#using-dashboard-api).
...
For more information on how to set and change user permissions, head to this [guide](/docsv2/docsv2/api-management/user-management#using-dashboard-ui-1)
...
* You already have authorized access to Tyk's Dashboard. If you haven't, [get the authorization key by following this doc](/docsv2/docsv2/api-management/user-management#using-dashboard-api).
...
7. Start TIB by running the binary (`profiles.json` is in the same CWD)
   See [Install TIB](/docsv2/docsv2/api-management/external-service-integration) for detailed instructions on how to install TIB
...
```
  ...
  - See [TIB REST API](/docsv2/docsv2/tyk-identity-broker/tib-rest-api) for more details.
...
The Tyk Identity Broker (TIB) is an open-source project which can be used to integrate Tyk authentication with 3rd party identity providers (IDPs). You can use this to enable your Dashboard to authenticate users with your LDAP-powered identity providers such as Active Directory. TIB has been designed as a glue-code solution, so it can integrate with almost any identity provider (IDP). See [Tyk Identity Broker Configuration](/docsv2/docsv2/tyk-configuration-reference/tyk-identity-broker-configuration) for details on configuring the TIB.
...
This guide assumes you already have a Tyk environment set up, with a Gateway and Dashboard. If you don't, please follow the [Tyk Self-Managed getting started guide](/docsv2/docsv2/tyk-self-managed#installation-options-for-tyk-self-managed).
...
4. **Set up the LDAP profile**
    ...
    * `OrgId`: The organization id which the profile is connected to - make sure this is the correct id for your organization (see the [Dashboard Admin API documentation](/docsv2/docsv2/api-management/dashboard-configuration#organizations-api) for details on how to retrieve this)
...
4. **Enable JWE**
  - [Updated the TIB profile via API](/docsv2/docsv2/tyk-identity-broker/tib-rest-api#update-profile)
