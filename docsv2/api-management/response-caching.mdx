...
There are two approaches to configure caching for an API deployed with Tyk:
 ...
 - [Basic](/docsv2/api-management/response-caching#basic-caching) or [Safe Request](/docsv2/api-management/response-caching#global-cache-safe-requests) caching is applied at the API level for all requests for which it is safe to do so.
 - [Advanced](/docsv2/api-management/response-caching#endpoint-caching) caching options can be applied at the endpoint level.
...
Tyk does support safe request caching at the more granular, per-endpoint level, as described [here](/docsv2/api-management/response-caching#request-selective-cache-control) - but `cache_all_safe_requests` must be set to `false` in that scenario.
...
_On this page we describe the use of Tyk's API response cache at the API level (Global); for details on the more advanced Endpoint level cache you should refer to [this](/docsv2/api-management/response-caching#endpoint-caching) page._
...
For more advanced use of the API-level cache we also have:
 ...
 - `enable_upstream_cache`: used to allow your [upstream service](/docsv2/api-management/response-caching#upstream-cache-control-1) to identify the responses to be cached
...
2. **Set the Cache Options for the Global Cache**
    ...
    2.  **Cache timeout** to set the [TTL](/docsv2/api-management/response-caching#cache-timeout) (in seconds) for cached requests
    3.  **Cache only these status codes** to set which [response codes](/docsv2/api-management/response-caching#cache-response-codes) to cache (ensure that you click **ADD** after entering each response code so that it is added to the list)
    4.  **Cache all safe requests** to enable the [global cache](/docsv2/api-management/response-caching#global-cache-safe-requests)

## Endpoint Caching

### Overview

On this page we describe how to configure Tyk's API response cache per endpoint within an API. This gives granular control over which paths are cached and allows you to vary cache configuration across API versions. For details on the API level (Global) cache you should refer to the [global-cache](/docsv2/api-management/response-caching#basic-caching) configuration page.
...
For details on the API-level cache you should refer to the [API-level cache](/docsv2/api-management/response-caching#basic-caching) configuration page.
...
<Note>
It's important to note that the [cache all safe requests](/docsv2/api-management/response-caching#global-cache-safe-requests) feature of the API-level cache will overrule the per-endpoint configuration so you must ensure that both are not enabled for the same API.
</Note>
...
If you're using Tyk OAS APIs, then you can find details and examples of how to configure the Endpoint Caching middleware [here](/docsv2/api-management/response-caching#using-tyk-oas-api).

If you're using Tyk Classic APIs, then you can find details and examples of how to configure the Endpoint Caching middleware [here](/docsv2/api-management/response-caching#using-classic-api).
...
The [Endpoint Caching](/docsv2/api-management/response-caching#endpoint-caching) middleware allows you to perform selective caching for specific endpoints rather than for the entire API, giving you granular control over which paths are cached.
...
If you're using the legacy Tyk Classic APIs, then check out the [Tyk Classic](/docsv2/api-management/response-caching#using-classic-api) page.
...
2. **Enable and configure the middleware for the specific endpoint**
    ...
    - `cacheByRegex`: Pattern match for [selective caching by body value](/docsv2/api-management/response-caching#request-selective-cache-control)
...
3. **Configure the middleware**
    ...
    <Note>
    Body value match or [request selective](/docsv2/api-management/response-caching#request-selective-cache-control) caching is not currently exposed in the Dashboard UI, so it must be enabled though either the raw API editor or the Dashboard API.
    </Note>
...
If you're using the newer Tyk OAS APIs, then check out the [Tyk OAS](/docsv2/api-management/response-caching#using-tyk-oas-api) page.
...
- a pattern match to cache only requests containing specific data in the [request body](/docsv2/api-management/response-caching#request-selective-cache-control)
...
3. **Configure the Advanced Cache plugin**
    ...
    <Note>
    Body value match or [request selective](/docsv2/api-management/response-caching#request-selective-cache-control) caching is not currently exposed in the Dashboard UI, so it must be configured through either the raw API editor or the Dashboard API.
    </Note>
...
For [multi-data center](/docsv2/api-management/mdcb#redis) deployments, the Data Planes have a locally deployed Redis. This enables them to have a localised cache close to the traffic-serving Gateways.

The [cache key](/docsv2/api-management/response-caching#cache-key) is used as the Redis key, for quick lookups.
...
The configuration of the separate Redis Cache is the same (and uses the same underlying driver) as the regular configuration, so [Redis Cluster](/docsv2/tyk-open-source#configure-redis-cluster) is fully supported. If you set `enable_cluster` to `false`, you only need to set one entry in `addrs`.
