...
Here you can see the various fields as they are applied to Tyk keys, these are all described in the Keys section of the [Gateway API](/docsv2/tyk-gateway-api).
...
[With the Dashboard API](/docsv2/api-management/gateway-config-managing-classic#create-a-security-policy-with-the-api)

[With the Gateway API - Open Source tab](/docsv2/api-management/gateway-config-managing-classic#secure-an-api)

[With the Dashboard](/docsv2/api-management/gateway-config-managing-classic#create-a-security-policy-with-the-dashboard)
...
See the [Gateway Configuration Options](/docsv2/tyk-oss-gateway/configuration) for more details.
...
See the Keys section of [Tyk Gateway API Swagger page](/docsv2/tyk-gateway-api) for more details.

### Access Key Level Security

Tyk supports the concept of access control at the key level. Access control is managed via three important settings in a session object. In order to be fully clear on how Tyk handles access control, it's worth looking at the key settings that go into a user session object. A full description of each of the options can be found in the [Tyk Gateway API documentation](/docsv2/tyk-gateway-api).
...
Creating new keys is done by POSTing an object such as the above to the Tyk create key API endpoint. See the keys section of the [Tyk Gateway API OpenAPI/Swagger](/docsv2/tyk-gateway-api) page.
...
A session object is just a JSON object. For more details of each parameter in the session object, see [Tyk Token Session Object Details](/docsv2/api-management/policies#session-object).
...
* `org_id`: The organization this user belongs to, this can be used in conjunction with the `org_id` setting in the API Definition object to have tokens "owned" by organizations. See the Organizations Quotas section of the [Tyk Gateway API](/docsv2/tyk-gateway-api).
...
As described in [What is a Session Object?](/docsv2/api-management/policies#what-is-a-session-object), all Tyk tokens can contain a metadata field. This field is a string key/value map that can store any kind of information about the underlying identity of a session.
...
- values can be used in dynamic [JavaScript](/docsv2/api-management/plugins/javascript#accessing-external-and-dynamic-data) middleware and Virtual Endpoints for further validation or request modification
...
- [URL Rewrite](/docsv2/transform-traffic/url-rewriting#pattern)
- [Request Header Transformation](/docsv2/api-management/traffic-transformation#injecting-dynamic-data-into-headers)
- [Response Header Transformation](/docsv2/api-management/traffic-transformation#injecting-dynamic-data-into-headers)
- [Request Body Transformation](/docsv2/api-management/traffic-transformation#data-accessible-to-the-middleware)
- [Response Body Transformation](/docsv2/api-management/traffic-transformation#data-accessible-to-the-middleware)
- [Virtual Endpoints](/docsv2/api-management/traffic-transformation#virtual-endpoints-overview)

You can also access and update metadata from your [custom plugins](/docsv2/api-management/plugins/overview#).  For an example of this, take a look at this [gRPC enabled GO Server](https://github.com/TykTechnologies/tyk-grpc-go-basicauth-jwt).  It's a PoC middleware that injects a JWT value into metadata and then accesses it later in the stream.
