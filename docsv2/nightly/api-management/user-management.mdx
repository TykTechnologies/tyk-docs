...
Dashboard users are not the same as developer portal users (a.k.a. [developers](/docsv2/nightly/docsv2/nightly/tyk-developer-portal/tyk-portal-classic/portal-concepts#developers)). The credentials are stored independently and have different mechanics relating to registration, management and access. For example, it is not possible to log into the developer portal using a dashboard account.
...
You can [create a user](/docsv2/nightly/docsv2/nightly/api-management/dashboard-configuration#add-user) with a call to the `POST /api/users` endpoint, for example:
...
<br/>
You will need to configure your [outbound email settings](/docsv2/nightly/docsv2/nightly/configure/outbound-email-configuration) to enable this feature.
</Note>
...
Tyk has a flexible [user permissions](/docsv2/nightly/docsv2/nightly/api-management/user-management#user-permissions) system that provides Role Based Access Control (RBAC) for your Tyk Dashboard.
...
You can also manage User Groups via our [Dashboard API](/docsv2/nightly/docsv2/nightly/api-management/dashboard-configuration#user-groups-api). The following functions are available:

* [List all User Groups](/docsv2/nightly/docsv2/nightly/api-management/dashboard-configuration#list-user-groups)
* [Get a User Group via the User Group ID](/docsv2/nightly/docsv2/nightly/api-management/dashboard-configuration#get-user-group)
* [Add a User Group](/docsv2/nightly/docsv2/nightly/api-management/dashboard-configuration#add-user-group)
* [Update a User Group](/docsv2/nightly/docsv2/nightly/api-management/dashboard-configuration#update-user-group)
* [Delete a User Group](/docsv2/nightly/docsv2/nightly/api-management/dashboard-configuration#delete-user-group)
...
There are other security options available from the Dashboard config file. See the [security section](/docsv2/nightly/docsv2/nightly/tyk-dashboard/configuration#security) for more details.
...
- [security.forbid_admin_view_access_token](/docsv2/nightly/docsv2/nightly/tyk-dashboard/configuration#securityforbid_admin_view_access_token): This parameter restricts admin users from viewing other users' Dashboard API Access Credentials, both in the API and the UI.
  
- [security.forbid_admin_reset_access_token](/docsv2/nightly/docsv2/nightly/tyk-dashboard/configuration#securityforbid_admin_reset_access_token): This parameter prevents admin users from resetting the access tokens of other users.
...
Selecting the **Account is Admin** checkbox from the Dashboard gives the user full access (it has the same [effect](/docsv2/nightly/docsv2/nightly/api-management/user-management#admin-users) as the `IsAdmin` property).

### Custom User Permissions

You can create your own custom permissions for use with the [Open Policy Agent (OPA)](/docsv2/nightly/docsv2/nightly/api-management/dashboard-configuration#extend-permissions-using-open-policy-agent-opa) using the [Additional Permissions](/docsv2/nightly/docsv2/nightly/api-management/dashboard-configuration#additional-permissions-api) endpoint in the Tyk Dashboard Admin API. This allows you to add and delete (CRUD) a list of additional (custom) permissions for your Dashboard users. Once created, a custom permission will be added to standard list of user permissions. 

You can also configure these custom permissions in the `security.additional_permissions` [map](/docsv2/nightly/docsv2/nightly/tyk-dashboard/configuration#securityadditional_permissions) in the Tyk Dashboard configuration file. 
...
<Note>
The availability of this feature [depends on your license](/docsv2/nightly/docsv2/nightly/api-management/user-management#).
</Note>
...
The [user owned analytics](/docsv2/nightly/docsv2/nightly/api-management/user-management#owned-analytics) feature allows you to prevent users from seeing the traffic to (and existence of) APIs for which they are not responsible. This reduces the opportunity for data leakage within your business.
...
To implement this structure, you would create three user groups and assign [permissions](/docsv2/nightly/docsv2/nightly/api-management/user-management#user-permissions) as indicated:
...
Currently, only [API Usage](/docsv2/nightly/docsv2/nightly/api-management/dashboard-configuration#activity-by-api) and [Error Counts](/docsv2/nightly/docsv2/nightly/api-management/dashboard-configuration#activity-by-error) are available to users with the `owned_analytics` permission.
...
The [Tyk Dashboard API](/docsv2/nightly/docsv2/nightly/tyk-dashboard-api) provides endpoints to manage API ownership directly, if you are not using the API Designer.
...
If you have deployed multiple [Tyk Organizations](/docsv2/nightly/docsv2/nightly/api-management/dashboard-configuration#organizations), you may have users that need access to more than one Organization (known as a "multi-org user"). **This functionality requires a specific Tyk license.**
...
You can integrate your existing identity management server with the Tyk Dashboard, as explained in our detailed [Single Sign-On (SSO) guide](/docsv2/nightly/docsv2/nightly/api-management/external-service-integration#single-sign-on-sso). **This functionality is available with all Tyk licenses except Tyk Classic Cloud.**

By default all users who login via SSO are granted admin permissions. You can change this behavior by setting either default permissions for *[users](/docsv2/nightly/docsv2/nightly/api-management/user-management#manage-tyk-dashboard-users)* or by creating a default *[user group](/docsv2/nightly/docsv2/nightly/api-management/user-management#manage-tyk-dashboard-user-groups)* to which all new users are assigned. With some IDPs you can automatically assign different SSO users to different *user groups* by dynamically mapping the IDP's user groups, for example with [Azure AD](/docsv2/nightly/docsv2/nightly/api-management/external-service-integration#user-group-mapping).
