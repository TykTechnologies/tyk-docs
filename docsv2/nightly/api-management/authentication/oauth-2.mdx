...
- **Multi-API Access**: Enable access to multiple APIs using a single OAuth token; configure one API for OAuth 2.0 token issuance and the other APIs with the [Auth Token](/docsv2/nightly/docsv2/nightly/api-management/authentication/bearer-token) method, linking them through a common policy
...
- [optional] [metadata](/docsv2/nightly/docsv2/nightly/api-management/policies#what-is-a-session-metadata) to be added to the access tokens
...
| --- | --- | --- |
| Register a new client app | `POST /api/apis/oauth/{{api-id}}` | [link](/docsv2/nightly/docsv2/nightly/api-management/dashboard-configuration#create-a-new-oauth20-client) |
| Get a list of registered client apps | `GET /api/apis/oauth/{{api-id}}` | [link](/docsv2/nightly/docsv2/nightly/api-management/dashboard-configuration#list-oauth-clients) |
| Get the details of a client app | `GET /api/apis/oauth/{{api-id}}/{{client_id}}` | [link](/docsv2/nightly/docsv2/nightly/api-management/dashboard-configuration#get-an-oauth20-client) |
| Delete a client app | `DELETE /api/apis/oauth/{{api-id}}/{{client_id}}` | [link](/docsv2/nightly/docsv2/nightly/api-management/dashboard-configuration#delete-oauth-client) |
...
(10) The *client* uses the *Access Token* to authenticate with the protected API using the [Auth Token](/docsv2/nightly/docsv2/nightly/api-management/authentication/bearer-token) method
...
The identity server will need access to the Tyk Dashboard API to [obtain an Authorization Code](/docsv2/nightly/docsv2/nightly/api-management/dashboard-configuration#oauth20-authorization-code).
...
This endpoint is protected using [Basic Authentication](/docsv2/nightly/docsv2/nightly/api-management/authentication/basic-authentication) where the username is the *client Id* and the password is the *client secret*.
...
- `token_type`: set to `bearer` indicating that the access token should be provided in an [Auth Token](/docsv2/nightly/docsv2/nightly/api-management/authentication/bearer-token) request to the protected API
...
The OpenAPI Specification indicates the use of [OAuth 2.0 authentication](https://swagger.io/docs/specification/v3_0/authentication/oauth2/) in the `components.securitySchemes` object using the `type: oauth2`. Tyk supports the [authorizationCode](/docsv2/nightly/docsv2/nightly/api-management/authentication/oauth-2#using-the-authorization-code-grant), [clientCredentials](#using-the-client-credentials-grant) and [password](#using-the-resource-owner-password-grant) flows and implements Relative Endpoint URLs for the `authorizationUrl`, `tokenUrl` and `refreshUrl`.
...
You can optionally [strip the user credentials](/docsv2/nightly/docsv2/nightly/api-management/client-authentication#managing-authorization-data) from the request prior to proxying to the upstream using the `authentication.stripAuthorizationData` field (Tyk Classic: `strip_auth_data`).

With the OAuth method selected, you'll need to configure Tyk to handle the specific configuration of OAuth grants that you will support. All of the OAuth specific configuration is performed within the [authentication.securitySchemes.oauth](/docsv2/nightly/docsv2/nightly/api-management/gateway-config-tyk-oas#oauth) object in the Tyk Vendor Extension.
...
As noted in the Tyk Classic API [documentation](/docsv2/nightly/docsv2/nightly/api-management/gateway-config-tyk-classic#configuring-authentication-for-tyk-classic-apis), you can select the Tyk as OAuth Server method using the `use_oauth2` option.
...
- [retrieve a list of all tokens for a client app](/docsv2/nightly/docsv2/nightly/api-management/dashboard-configuration#retrieve-all-current-tokens-for-specified-oauth20-client)
- [revoke a single token](/docsv2/nightly/docsv2/nightly/api-management/dashboard-configuration#revoke-a-single-oauth-client-token)
- [revoke all tokens for a client app](/docsv2/nightly/docsv2/nightly/api-management/dashboard-configuration#revoke-all-oauth-client-tokens)
