...
An input is a source of data piped through an array of optional [processors](/docsv2/nightly/docsv2/nightly/api-management/stream-config#overview-3):
...
Only one input is configured at the root of a Tyk Streams config. However, the root input can be a [broker](/docsv2/nightly/docsv2/nightly/api-management/stream-config#broker) which combines multiple inputs and merges the streams:
...
It's possible to configure a [batch policy](/docsv2/nightly/docsv2/nightly/api-management/stream-config#batch-policy) with a broker using the `batching` fields. When doing this the feeds from all child inputs are combined. Some inputs do not support broker based batching and specify this in their documentation.
...
Allows you to configure a [batching policy](/docsv2/nightly/docsv2/nightly/api-management/stream-config#batch-policy).
...
An optional [rate limit](/docsv2/nightly/docsv2/nightly/api-management/rate-limit#rate-limiting-with-tyk-streams) to throttle requests by.
...
The [scanner](/docsv2/nightly/docsv2/nightly/api-management/stream-config#overview-4) by which the stream of bytes consumed will be broken out into individual messages. Scanners are useful for processing large sources of data without holding the entirety of it within memory. For example, the `csv` scanner allows you to process individual CSV rows without loading the entire CSV file in memory at once.
...
{/* TODO add link to service wide HTTP server If the `address` config field is left blank the [service-wide HTTP server](/docsv2/nightly/docs/components/http/about) will be used. */}

{/* TODO add rate limit The field `rate_limit` allows you to specify an optional [`rate_limit` resource](/docsv2/nightly/docs/components/rate_limits/about), which will be applied to each HTTP request made and each websocket payload received.
...
An optional [rate limit](/docsv2/nightly/docs/components/rate_limits/about) to throttle requests by. */}
...
This field contains sensitive information that usually shouldn't be added to a config directly, read our [secrets page for more info](/docsv2/nightly/docs/configuration/secrets).
::: */}
...
Instead of using a static `access_token` allows you to query a [`cache`](/docsv2/nightly/docs/components/caches/about) resource to fetch OAUTHBEARER tokens from */}
...
An output is a sink where we wish to send our consumed data after applying an optional array of [processors](/docsv2/nightly/docsv2/nightly/api-management/stream-config#overview-3). Only one output is configured at the root of a Tyk Streams config. However, the output can be a [broker](/docsv2/nightly/docsv2/nightly/api-management/stream-config#broker-1) which combines multiple outputs under a chosen brokering pattern.
...
This output benefits from sending messages as a [batch](/docsv2/nightly/docsv2/nightly/api-management/stream-config#batching-6) for improved performance. Batches can be formed at both the input and output level.
...
Allows you to configure a [batching policy](/docsv2/nightly/docsv2/nightly/api-management/stream-config#batching-6).
...
{/* TODO add link here  If the `address` config field is left blank the [service-wide HTTP server](/docsv2/nightly/docs/components/http/about) will be used. */}
...
{/* TODO add link here - This behavior can be overridden by [archiving your batches](/docsv2/nightly/docs/configuration/batching#post-batch-processing). */}
...
{/* Add links to bloblang queries : Both the `key` and `topic` fields can be dynamically set using function interpolations described [here](/docsv2/nightly/docs/configuration/interpolation#bloblang-queries). */}
...
The `threads` field in the pipeline section determines how many parallel processing threads are created. You can read more about parallel processing in the [pipeline guide](/docsv2/nightly/docsv2/nightly/api-management/stream-config#processing-pipelines).
...
However, a small number of inputs such as [kafka](/docsv2/nightly/docsv2/nightly/api-management/stream-config#kafka) must be consumed sequentially (in this case by partition) and therefore benefit from specifying your batch policy at the input level instead:
...
Sometimes you may prefer to create your batches before processing, in which case if your input doesn't already support [a batch policy](#batch-policy) you can instead use a [broker](/docsv2/nightly/docsv2/nightly/api-management/stream-config#broker), which also allows you to combine inputs with a single batch policy:
...
This also works the same with [output brokers](/docsv2/nightly/docsv2/nightly/api-management/stream-config#broker-1).
