...
From Tyk 5.8 we recommend that any REST APIs are migrated to the newer [Tyk OAS API](/docsv2/nightly/docsv2/nightly/api-management/gateway-config-tyk-oas) style, in order that they can benefit from simpler configuration and future enhancements.

For Tyk Dashboard users with an existing portfolio of Tyk Classic API definitions, we provide a [migration tool](/docsv2/nightly/docsv2/nightly/api-management/migrate-from-tyk-classic), available via the Dashboard API and UI.
...
- common versioning configuration is mostly contained within the [definition](/docsv2/nightly/docsv2/nightly/api-management/gateway-config-tyk-classic#common-versioning-configuration) object
- configuration for the different versions is contained within the [version_data](/docsv2/nightly/docsv2/nightly/api-management/gateway-config-tyk-classic#version-specific-configuration) object
...
Each entry must include the `method` and `path` (identifying the endpoint) for which the middleware should be run. The other options for each middleware are documented in the [Traffic Transformation](#) section. Note that mock response functionality is provided via the `black_list[]`, `white_list[]` and `ignore[]` middleware.
...
**Field: `jwt_source`**
Must either be a base64 encoded valid RSA/HMAC key or a url to a resource serving JWK, this key will then be used to validate inbound JWT and throttle them according to the centralised JWT options and fields set in the configuration. See [Dynamic public key rotation using public JWKs URL](/docsv2/nightly/docsv2/nightly/basic-config-and-security/security/authentication-authorization/json-web-tokens#enable-dynamic-public-key-rotation-using-jwks) for more details on JWKs.
...
If set to true, this will override the endpoint level settings in [Ignore](/docsv2/nightly/docsv2/nightly/api-management/traffic-transformation#case-sensitivity-2), [Allowlist](/docsv2/nightly/docsv2/nightly/api-management/traffic-transformation#case-sensitivity) and [Blocklist](/docsv2/nightly/docsv2/nightly/api-management/traffic-transformation#case-sensitivity-1) middleware. This setting can be overriden at the Tyk Gateway level, and so applied to all APIs, by setting `ignore_endpoint_case` to `true` in your `tyk.conf` file. See [ignore_endpoint_case](/docsv2/nightly/docsv2/nightly/tyk-oss-gateway/configuration#ignore_endpoint_case) for details.
...
The [API-level rate limit](/docsv2/nightly/docsv2/nightly/api-management/rate-limit#rate-limiting-layers) aggregates the traffic coming into an API from all sources and ensures that the overall rate limit is not exceeded. It is composed of a `rate` (number of requests) and `per` (interval). If either is set to `0` then no API-level limit is applied.
