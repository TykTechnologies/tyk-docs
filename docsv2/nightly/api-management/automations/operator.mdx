...
**TykOasApiDefinition**: Available from Tyk Operator v1.0. It represents a [Tyk OAS API configuration](/docsv2/nightly/api-management/gateway-config-tyk-oas). Tyk OAS API is based on the OpenAPI specification (OAS) and is the recommended format for standard HTTP APIs.

**ApiDefinition**: Available on all versions of Tyk Operator. It represents a [Tyk Classic API configuration](/docsv2/nightly/api-management/gateway-config-tyk-classic). Tyk Classic API is the traditional format used for defining all APIs in Tyk, and now the recommended format for non-HTTP APIs such as TCP, GraphQL, and Universal Data Graph (UDG). Tyk Operator supports the major features of Tyk Classic API and the feature support details can be tracked [here](#apidefinition-crd).

**TykStreamsApiDefinition**: Available from Tyk Operator v1.1. It represents an [Async API configuration](/docsv2/nightly/api-management/event-driven-apis#configuration-options) which is based on [Tyk OAS API Definition](/docsv2/nightly/api-management/gateway-config-tyk-oas). Tyk Operator supports all [Tyk Streams](/docsv2/nightly/api-management/event-driven-apis#) features as they become available on the Gateway.
...
The following custom resources can be used to configure APIs and policies at [Tyk Gateway](/docsv2/nightly/tyk-oss-gateway) or [Tyk Dashboard](/docsv2/nightly/tyk-dashboard).

| Kind               | Group       | Version   | Description                                                                                       |
|--------------------|-------------|-----------|---------------------------------------------------------------------------------------------------|
| TykOasApiDefinition| tyk.tyk.io  | v1alpha1  | Defines configuration of [Tyk OAS API Definition object](/docsv2/nightly/api-management/gateway-config-tyk-oas)                                 |
| ApiDefinition      | tyk.tyk.io  | v1alpha1  | Defines configuration of [Tyk Classic API Definition object](/docsv2/nightly/api-management/gateway-config-tyk-classic)                                 |
| TykStreamsApiDefinition| tyk.tyk.io  | v1alpha1  | Defines configuration of [Tyk Streams](/docsv2/nightly/api-management/event-driven-apis#configuration-options)                                 |
| SecurityPolicy     | tyk.tyk.io  | v1alpha1  | Defines configuration of [security policies](/docsv2/nightly/api-management/policies#what-is-a-security-policy). Operator supports linking ApiDefinition custom resources in SecurityPolicy's access list so that API IDs do not need to be hardcoded in the resource manifest.        |
| SubGraph           | tyk.tyk.io  | v1alpha1  | Defines a [GraphQL federation subgraph](/docsv2/nightly/api-management/graphql#subgraphs-and-supergraphs).                                           |
| SuperGraph         | tyk.tyk.io  | v1alpha1  | Defines a [GraphQL federation supergraph](/docsv2/nightly/api-management/graphql#subgraphs-and-supergraphs).                                        |
...
The following custom resources can be used to configure [Tyk Classic Developer Portal](/docsv2/nightly/tyk-developer-portal/tyk-portal-classic).

| Kind               | Group       | Version   | Description                                                                                       |
|--------------------|-------------|-----------|---------------------------------------------------------------------------------------------------|
| APIDescription     | tyk.tyk.io  | v1alpha1  | Configures [Portal Documentation](/docsv2/nightly/tyk-apis/tyk-portal-api/portal-documentation). |
| PortalAPICatalogue | tyk.tyk.io  | v1alpha1  | Configures [Portal API Catalogue](/docsv2/nightly/getting-started/key-concepts/api-catalogue). |
| PortalConfig       | tyk.tyk.io  | v1alpha1  | Configures [Portal Configuration](/docsv2/nightly/tyk-apis/tyk-portal-api/portal-configuration). |
...
Tyk Dashboard is multi-tenant capable, which means you can use a single Tyk Dashboard instance to host separate [organizations](/docsv2/nightly/api-management/dashboard-configuration#organizations) for each team or department. Each organization is a completely isolated unit with its own:
...
|------------------|-------------|---------|---------|
| Client certifates | ✅ [Client mTLS](/docsv2/nightly/api-management/client-authentication#setup-static-mtls-in-tyk-operator-using-the-tyk-classic-api-definition) | ✅ [Client mTLS](/docsv2/nightly/api-management/client-authentication#setup-static-mtls-in-tyk-operator-using-tyk-oas-api-definition) | Certificate ID can be set in the API Definition but configuring certificates from Secrets in CRD is not supported. |
| Custom domain certificates | ✅ [TLS and SSL](/docsv2/nightly/api-management/certificates#dynamically-setting-ssl-certificates-for-custom-domains) | ✅ [TLS and SSL](/docsv2/nightly/api-management/certificates#dynamically-setting-ssl-certificates-for-custom-domains) | Certificate ID can be set in the API Definition but configuring certificates from Secrets in CRD is not supported. |
| Public keys pinning | ✅ [Certificate pinning](/docsv2/nightly/api-management/upstream-authentication#using-tyk-operator-to-configure-mtls-for-tyk-classic-apis) | ✅ [Certificate pinning](/docsv2/nightly/api-management/upstream-authentication#certificate-pinning) | Certificate ID can be set in the API Definition but configuring certificates from Secrets in CRD is not supported. |
| Upstream mTLS | ✅ [Upstream mTLS via Operator](/docsv2/nightly/api-management/upstream-authentication#using-tyk-operator-to-configure-mtls-for-tyk-classic-apis) | ✅ [Upstream mTLS via Operator](/docsv2/nightly/api-management/upstream-authentication#using-tyk-operator-to-configure-mtls) | Certificate ID can be set in the API Definition but configuring certificates from Secrets in CRD is not supported. |


## Install and Configure Tyk Operator

We assume you have already installed Tyk. If you don’t have it, check out [Tyk
Cloud](/docsv2/nightly/tyk-cloud#quick-start-tyk-cloud) or [Tyk Self
Managed](/docsv2/nightly/tyk-self-managed) page. [Tyk Helm
Chart](/docsv2/nightly/product-stack/tyk-charts/overview) is the preferred (and easiest) way to install Tyk on Kubernetes.
...
[Using Tyk Operator to enable GitOps with Tyk](/docsv2/nightly/api-management/automations) about
...
If you are using [Tyk Stack](/docsv2/nightly/product-stack/tyk-charts/tyk-stack-chart), [Tyk Control
Plane](/docsv2/nightly/product-stack/tyk-charts/tyk-control-plane-chart), or [Tyk Open
Source Chart](/docsv2/nightly/product-stack/tyk-charts/tyk-oss-chart), you can install Tyk Operator alongside other Tyk
...
You can configure your API for any Tyk supported authentication method by following the [Client Authentication](/docsv2/nightly/api-management/client-authentication) documentation.
...
You need to generate a key to access the `httpbin` API now. Follow [this guide](/docsv2/nightly/getting-started/configure-first-api#create-an-api-key) to see how to create an API key for your installation. 
...
This configuration allows [keyless (open)](/docsv2/nightly/api-management/client-authentication#open-no-authentication) access to the API without any authentication.
...
This setup requires a [bearer token](/docsv2/nightly/api-management/authentication/bearer-token) for access.
...
- `validate_signature`: Enable [signature validation](/docsv2/nightly/api-management/authentication/bearer-token#auth-token-with-signature).
...
This configuration uses [JWT tokens](/docsv2/nightly/basic-config-and-security/security/authentication-authorization/json-web-tokens) for authentication.

Users can configure JWT authentication by defining the following fields:

- `jwt_signing_method`: Specify the method used to sign the JWT. Refer to [JWT Signing Method](/docsv2/nightly/basic-config-and-security/security/authentication-authorization/json-web-tokens#set-up-jwt-signing-method) for supported methods.
...
This configuration uses [Basic Authentication](/docsv2/nightly/api-management/authentication/basic-authentication), requiring a username and password for access.
...
This configuration uses a [Golang plugin](/docsv2/nightly/api-management/plugins/golang#) for custom authentication. The following example shows how to create an API definition with a Golang custom plugin for `httpbin-go-auth`.

For an example of Golang authentication middleware, see [Performing custom authentication with a Golang plugin](/docsv2/nightly/api-management/plugins/golang#performing-custom-authentication-with-a-golang-plugin).
...
This configuration uses a [gRPC plugin](/docsv2/nightly/api-management/plugins/golang#) for custom authentication. The following example shows how to create an API definition with a gRPC custom plugin for `httpbin-grpc-auth`.
...
This setup allows for [multiple authentication](/docsv2/nightly/api-management/client-authentication#combine-authentication-methods) methods to be chained together, requiring clients to pass through each specified authentication provider.

To enable multiple (chained) auth, you should set `base_identity_provided_by` field to one of the supported chained enums. Consult the [Multi (Chained) Authentication](/docsv2/nightly/api-management/client-authentication#combine-authentication-methods) section for the supported auths.
...
To enable [IP Allowlist](/docsv2/nightly/api-management/gateway-config-tyk-classic#ip-access-control), set the following fields:
...
To enable [IP Blocklist](/docsv2/nightly/api-management/gateway-config-tyk-classic#ip-access-control), set the following fields:
...
In the example below we can see that the configuration is contained within the `graphql` configuration object. A GraphQL schema is specified within the `schema` field and the execution mode is set to `proxyOnly`. The [GraphQL public playground](/docsv2/nightly/api-management/graphql#enabling-public-graphql-playground) is enabled with the path set to `/playground`.
...
If you are on Tyk 3.2 and above, you can use the following manifest to create an UDG API. This example configures a Universal Data Graph from a [GraphQL datasource](/docsv2/nightly/api-management/data-graph#graphql) and a [REST Datasource](/docsv2/nightly/api-management/data-graph#rest).
...
In this example, the security policy will apply to an `ApiDefinition` resource named `httpbin` in the `default` namespace, a `TykOasApiDefinition` resource named `petstore` in the `default` namespace, and a `TykStreamsApiDefinition` resource named `http-to-kafka` in the `default` namespace. Note that with Tyk Operator, you do not need to specify API ID as in the raw [Policy definition](/docsv2/nightly/api-management/policies#policies-guide). Tyk Operator will automatically retrieve the API ID of referenced API Definition resources for you.
...
You can secure your APIs by specifying [allowed URLs](/docsv2/nightly/api-management/policies#secure-your-apis-by-method-and-path) (methods and paths) for each API within a security policy. This is done using the `allowed_urls` field under `access_rights_array`.
...
Tyk, with release *v4.0* offers [GraphQL federation](/docsv2/nightly/api-management/graphql#federation-version-support) that allows you to divide GraphQL implementation across multiple back-end
...
each of the following [hooks](/docsv2/nightly/api-management/plugins/plugin-types#plugin-and-hook-types) in the API request
lifecycle: [Pre (Request)](/docsv2/nightly/api-management/plugins/plugin-types#request-plugins), [Authentication](/docsv2/nightly/api-management/plugins/plugin-types#authentication-plugins),
[Post (Request)](/docsv2/nightly/api-management/plugins/plugin-types#request-plugins), [Post
Authentication](/docsv2/nightly/api-management/plugins/plugin-types#request-plugins), [Response](/docsv2/nightly/api-management/plugins/plugin-types#response-plugins)
and [Analytics](/docsv2/nightly/api-management/plugins/plugin-types#analytics-plugins). Subsequently, users can execute, or “hook”, their
...
Please refer to [Analytics Plugins](/docsv2/nightly/api-management/plugins/plugin-types#analytics-plugins) to learn how to configure Analytics
...
languages](/docsv2/nightly/api-management/plugins/overview#plugin-driver-names) section for list of supported plugin driver names.
...
[per-endpoint](/docsv2/nightly/api-management/plugins/plugin-types#per-endpoint-custom-plugins) level plugins are not currently
...
2. **Create API Definition Resource**
    ...
    *http://httpbin.org*.
    ...
    - The `driver` configuration parameter is set to `otto` at line 15, since our plugin is a Javascript plugin. For other
      valid values please refer to the [plugins driver page](/docsv2/nightly/api-management/plugins/overview#plugin-driver-names).
...
request lifecycle](/docsv2/nightly/api-management/traffic-transformation#request-middleware-chain).
...
bundles](/docsv2/nightly/api-management/plugins/overview#plugin-bundles), load them into the Tyk Gateway, and configure an API
...
The concept of [internal looping](/docsv2/nightly/advanced-configuration/transform-traffic/looping) allows you to use URL Rewriting to redirect your URL to *another API endpoint* or to *another API* in the Gateway. In Tyk, looping is generally targeted using the `tyk://<API_ID>/<path>` scheme, which requires prior knowledge of the `API_ID`. Tyk Operator simplifies the management and transformation of API traffic within Kubernetes environments by abstracting APIs as objects, managing them and dynamically assigning `API_ID`s by its Kubernetes metedata name and namespace.
...
[URL rewriting](/docsv2/nightly/transform-traffic/url-rewriting#url-rewrite-middleware) in Tyk enables the alteration of incoming API request paths to align with the expected endpoint format of your backend services.
...
[Triggers](/docsv2/nightly/transform-traffic/url-rewriting#url-rewrite-triggers) are configurations that specify actions based on certain conditions present in HTTP headers, query parameters, path parameters etc.
...
Internal looping can also be used for [proxying to internal APIs](/docsv2/nightly/advanced-configuration/transform-traffic/looping).
...
API name can be set through `x-tyk-api-gateway.info.name` field in [Tyk OAS API Definition](/docsv2/nightly/api-management/gateway-config-tyk-oas) object.
...
API active state can be set through `x-tyk-api-gateway.info.state.active` field in [Tyk OAS API Definition](/docsv2/nightly/api-management/gateway-config-tyk-oas) object.
...
API accessibility can be set through `x-tyk-api-gateway.info.state.internal` field in [Tyk OAS API Definition](/docsv2/nightly/api-management/gateway-config-tyk-oas) object.
...
If you already have API configurations created in the Tyk Dashboard and want to start using Tyk Operator to manage these APIs, you can include the existing API ID in the manifest under the `x-tyk-api-gateway.info.id` field in [Tyk OAS API Definition](/docsv2/nightly/api-management/gateway-config-tyk-oas) object.
...
### API Categories
[API categories](/docsv2/nightly/api-management/dashboard-configuration#governance-using-api-categories) are configured differently for Tyk OAS APIs and Tyk Classic APIs. Please see below for examples.
...
For a Tyk Classic API, you can specify the category name using the `name` field with a `#` qualifier. This will categorize the API in the Tyk Dashboard. See [How API categories work](/docsv2/nightly/api-management/dashboard-configuration#tyk-classic-apis) to learn about limitations on API names.
...
`versioning` is configured at `order-api` (v1), the Base API, and it has similiar structure as [Tyk OAS API Definition](/docsv2/nightly/api-management/api-versioning):
...
For Tyk Classic API, versioning can be configured via `ApiDefinition` custom resource definition (CRD). See [Tyk Classic versioning](/docsv2/nightly/api-management/gateway-config-tyk-classic#tyk-classic-api-versioning) for a comprehensive example of configuring API versioning for Tyk Classic API with Tyk Operator.

### API Ownership

Please consult the [API Ownership](/docsv2/nightly/api-management/user-management#api-ownership) documentation for the fundamental concepts of API Ownership in Tyk and [Operator Context](#multi-tenancy-in-tyk) documentation for an overview of the use of OperatorContext to manage resources for different teams effectively.
...
Tyk supports secure upstream connections through mutual TLS, certificate pinning, and public key verification to ensure data integrity between the gateway and backend services. For full details, please see the [Upstream Authentication](/docsv2/nightly/api-management/upstream-authentication) section.
...
The TykOasApiDefinition Custom Resource Definition (CRD) manages [Tyk OAS API Definition objects](/docsv2/nightly/api-management/gateway-config-tyk-oas) within a Kubernetes environment. This CRD enables the integration and management of Tyk API definitions using Kubernetes-native tools, simplifying the process of deploying and managing OAS APIs on the Tyk Dashboard.
...
| API Ownership via OperatorContext | ✅      | v1.0 | - | [API Ownership](/docsv2/nightly/api-management/user-management#when-to-use-api-ownership) |
...
The TykStreamsApiDefinition Custom Resource Definition (CRD) manages [Async API configuration](/docsv2/nightly/api-management/event-driven-apis#configuration-options) within a Kubernetes environment.

##### TykStreamsApiDefinition Features

`TykStreamsApiDefinition` can support all features of [Tyk Streams](/docsv2/nightly/api-management/event-driven-apis#). You just need to provide the Tyk Streams API definition via a ConfigMap. In addition to managing the CRUD (Create, Read, Update, Delete) of Tyk Streams API resources, the Tyk Operator helps you better manage resources through object linking to Security Policies. See below for a list of Operator features and examples:
...
See [Release notes](/docsv2/nightly/developer-support/release-notes/operator) to check for each Tyk Operator release,
...
#### Security Policy CRD
The SecurityPolicy custom resource defines configuration of [Tyk Security Policy object](/docsv2/nightly/api-management/policies).
