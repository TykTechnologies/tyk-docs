...
- Migrate to new [health check](/docsv2/nightly/api-management/mdcb#health-check) endpoints in order to get more detailed information. For Kubernetes users, use Helm Charts v1.6 to upgrade MDCB to set liveness and readiness probes of MDCB deployment to the new health check endpoints.
...
Two new [health check](/docsv2/nightly/api-management/mdcb#health-check) endpoints have been added to improve monitoring and diagnostics:
...
Two new `/config` and `/env` [endpoints](/docsv2/nightly/api-management/mdcb#check-mdcb-configurations) have been implemented, allowing developers to access the current configuration state of the MDCB instance in real-time. This feature provides:
...
[MongoDB supported versions](/docsv2/nightly/tyk-self-managed#supported-versions) 
...
#### Added
- Added a new configuration option [enable_ownership](/docsv2/nightly/tyk-multi-data-centre/mdcb-configuration-options#enable_ownership) that allows MDCB filter APIs by API Ownership. 
