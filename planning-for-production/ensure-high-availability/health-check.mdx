...
| **Dashboard** | `/hello` | N/A | [`health_check_endpoint_name`](/tyk-dashboard/configuration#health_check_endpoint_name) | - |
...
- **warn**: Indicates that one of the components is having an outage but your component is able to keep processing traffic. The impact is medium (i.e. no quotas are applied, no analytics, no RPC connection to [MDCB](/api-management/mdcb)).
...
For more details on configuring these endpoints, refer to the configuration references for [Gateway](/tyk-oss-gateway/configuration), [Dashboard](/tyk-dashboard/configuration), and [Pump](/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables).

## Kubernetes Probes Example

For more details on deploying Tyk in Kubernetes, refer to the [Kubernetes Installation Guide](/tyk-self-managed/install/kubernetes).
