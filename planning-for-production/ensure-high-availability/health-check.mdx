...
canonical: "https://tyk.io/docs/planning-for-production/ensure-high-availability/health-check"
---
...
Tyk Gateway exposes two separate probe endpoints, which map to Kubernetes' liveness and readiness probe semantics:

| Endpoint | Purpose | Configurable via | Returns 503 when |
| :-- | :-- | :-- | :-- |
| `/hello` | Liveness — is the Gateway process alive and able to answer requests? | `health_check_endpoint_name` | The Gateway is not running or is deadlocked. It returns `HTTP 200 OK` in all normal states, including when Redis or the Dashboard is unavailable, so it does not cause Kubernetes to restart the pod during dependency outages. Detailed component status (Redis, Dashboard, RPC) is included in the response body. |
| `/ready` | Readiness — is the Gateway ready to receive traffic? | `readiness_check_endpoint_name` | The Gateway has received a `SIGTERM`, `SIGINT`, or `SIGQUIT` and is entering [graceful shutdown](/planning-for-production/ensure-high-availability/graceful-shutdown), or has not finished loading its API definitions on startup. |

The `/ready` endpoint was added in Tyk Gateway v5.8.3 alongside the configurable graceful shutdown mechanism (see the [Gateway release notes](/developer-support/release-notes/gateway)), and its shutdown behavior is extended by `graceful_shutdown_delay_seconds` from Tyk Gateway v5.14.0.

<Note>
Use `/hello` for Kubernetes liveness probes and `/ready` for Kubernetes readiness probes. Using `/hello` for both, or using `/ready` for liveness, will cause pods to be restarted during a graceful shutdown or when a downstream dependency is temporarily unavailable.
</Note>
