...
Redis is not intended for long time storage so, as with traffic logs, [Tyk Pump](/api-management/tyk-pump) should be configured to transfer the logs to the persistent database in the Control Plane, from which they can be accessed by Tyk Dashboard.
...
The SQL Uptime Pump generates aggregated data from the uptime test logs, storing them in a table called `tyk_uptime_analytics`, or one table per day if `table_sharding` is enabled (see [Table Sharding](/tyk-configuration-reference/sql#table-sharding)).
