...
In [MDCB](/api-management/mdcb) deployments, worker Gateways store API definition backups in Redis to ensure resilience when the control plane is unavailable. For deployments with many or large API definitions, this can consume significant Redis memory.

You can enable Zstandard compression to reduce the storage footprint by setting [`storage.compress_api_definitions`](/tyk-oss-gateway/configuration#storage-compress_api_definitions) to `true` in your worker Gateway configuration:
...
This feature:
- Compresses API definitions before storing them in Redis backups
- Automatically decompresses when loading backups
- Is backward compatible - compressed and uncompressed data are handled transparently
- Can significantly reduce Redis memory usage depending on your API definitions
