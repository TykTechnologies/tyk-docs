...
* `OrgId`: The organization id which the profile is connected to - make sure this is the correct id for your organization (see the [Dashboard Admin API documentation](/dashboard-admin-api/organisations) for details on how to retrieve this)
...
When TIB is running you can use its REST API. See the [TIB REST API documentation](/tyk-identity-broker/tib-rest-api) for a full description of each endpoint.
