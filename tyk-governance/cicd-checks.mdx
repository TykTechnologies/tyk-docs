...
The other governance pages cover the rest of the picture. [Rulesets](/tyk-governance/rulesets) covers how platform teams define the standards being checked. [Service Compliance](/tyk-governance/service-compliance) covers how API owners see and resolve compliance issues for APIs already saved in Tyk Dashboard.
...
For the underlying endpoint itself, including request and response schemas, limits, and error codes, see the [Governance Evaluation API](/tyk-governance/evaluation-api) reference.
...
If you would rather call the endpoint directly, from another language or a CI system's native HTTP step, use the [Governance Evaluation API](/tyk-governance/evaluation-api) reference instead. Read [Gating Correctly](/tyk-governance/evaluation-api#gating-correctly) first: there are two traps that make a naive implementation pass every build.
...
For fixing the same issues inside Tyk Dashboard instead, see [Remediate Issues](/tyk-governance/remediate-issues).
...
- **Treat `truncated: true` as a failure, not a warning.** If a response is truncated you are only seeing the top 1,000 issues, and there could be more errors past the cutoff. Fail the build and narrow the ruleset or category scope rather than evaluating against partial data.
...
Almost always because the check is gating on the HTTP status code, which is `200` whenever the evaluation ran successfully, even with Error-severity issues. Violations live in the response body. See [Gating Correctly](/tyk-governance/evaluation-api#gating-correctly).
</Accordion>

<Accordion title='How many categories or ruleset IDs can I pass in one call?'>
Twenty of each, and category resolution is also capped at 20 resolved rulesets. This limit applies to the sandbox endpoint only, not to Tyk Dashboard. See [Limits](/tyk-governance/evaluation-api#limits).
</Accordion>
</AccordionGroup>
