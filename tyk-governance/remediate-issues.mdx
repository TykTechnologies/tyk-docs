...
To find the issues in the first place, see [Service Compliance](/tyk-governance/service-compliance).
...
If guidance on a rule is missing or unclear, that is a ruleset problem rather than a service problem. Raise it with the ruleset's owner, who can edit the `howToFix` text. See [Configure a Ruleset](/tyk-governance/rulesets#raw-definition).
...
For the full severity model, see [Severity and Compliance](/tyk-governance/core-concepts#severity-and-compliance).
...
If the count has not moved, allow for evaluation time on large definitions before assuming the fix failed. See [Evaluation Performance](/tyk-governance/service-compliance#evaluation-performance).
...
The Compliance Notification banner on the Tyk Dashboard home page is what surfaces this when you were not watching. See [Compliance Notifications](/tyk-governance/service-compliance#compliance-notifications).
