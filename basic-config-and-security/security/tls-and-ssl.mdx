...
Finally, set the [host_config.generate_secure_paths](/tyk-dashboard/configuration#host_configgenerate_secure_paths) flag to `true` in your `tyk_analytics.conf`
...
From v2.9.3 you can also specify a custom proxy and set the minimum TLS versions and any SSL ciphers within your API definitions. See [Internal Proxy Setup](/tyk-apis/tyk-gateway-api/api-definition-objects/proxy-settings.md#internal-proxy-setup) for more details.
