...
If Tyk cannot find a `pol` claim, it will apply this Default Policy. Select a policy that gives access to this API we are protecting, or [go create one first](/api-management/gateway-config-managing-classic#secure-an-api) if it doesn't exist.
