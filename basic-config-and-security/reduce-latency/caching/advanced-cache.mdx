...
For details on the API-level cache you should refer to the [API-level cache](/basic-config-and-security/reduce-latency/caching/global-cache) configuration page.
...
<Note>
It's important to note that the [cache all safe requests](/basic-config-and-security/reduce-latency/caching#global-cache-safe-requests) feature of the API-level cache will overrule the per-endpoint configuration so you must ensure that both are not enabled for the same API.
</Note>
...
If you're using Tyk OAS APIs, then you can find details and examples of how to configure the Endpoint Caching middleware [here](/product-stack/tyk-gateway/middleware/endpoint-cache-tyk-oas).

If you're using Tyk Classic APIs, then you can find details and examples of how to configure the Endpoint Caching middleware [here](/product-stack/tyk-gateway/middleware/endpoint-cache-tyk-classic).
