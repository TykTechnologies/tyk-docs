...
External Service Configuration gives you control over how Tyk Gateway securely connects to the [external services](/5.11/configure/external-service#external-service) that it needs to function correctly. You can configure proxy settings, mTLS authentication, and other connection parameters for Tyk's outbound connections to services such as OAuth providers, analytics systems, and webhook endpoints.
...
Refer to the [Tyk Gateway Configuration Reference](/5.11/tyk-oss-gateway/configuration#external_services) for more details on this setting.
...
| :-------------- | :------------- | :------------ |
| `oauth` | [OAuth/JWT token validation and introspection](/5.11/basic-config-and-security/security/authentication-authorization/json-web-tokens) | JWT auth interactions with Auth Server (Identity Provider) |
| `storage` | [External storage operations](/5.11/planning-for-production/database-settings) | Redis connections, database interactions |
| `webhooks` | [Webhook event notifications](/5.11/api-management/gateway-events#event-handling-with-webhooks) | Event handlers, notification delivery |
| `health` | [Health check requests](/5.11/planning-for-production/ensure-high-availability/health-check) | Host checker, uptime monitoring |  
| `discovery` | [Service discovery requests](/5.11/planning-for-production/ensure-high-availability/service-discovery) | Load balancer, service registry |
...
| `cert_id` | String | No | Certificate ID from [Tyk Certificate Store](/5.11/api-management/certificates#using-tyk-certificate-storage) |
| `ca_cert_ids` | Array | No | Array of CA certificate IDs from [Tyk Certificate Store](/5.11/api-management/certificates#using-tyk-certificate-storage) |
...
Refer to the [Tyk Gateway Configuration Reference](/5.11/tyk-oss-gateway/configuration#external_services) for more details.
...
4. **Default settings** - If a setting is not specified, Tyk uses sensible defaults. You can find these in the [Tyk Gateway Configuration Reference](/5.11/tyk-oss-gateway/configuration#external_services).
...
**What this does:**
- Similar to previous example, but uses [Tyk Certificate Store](/5.11/api-management/certificates#using-tyk-certificate-storage) instead of file paths
...
**What this does:**
- Similar to example 4, but uses [Tyk Certificate Store](/5.11/api-management/certificates#using-tyk-certificate-storage) for all mTLS configurations
