...
API requests can be redirected to other endpoints or APIs deployed on Tyk using the URL rewrite functionality. This allows requests to be redirected to internal APIs that are not directly exposed on the Gateway (for example to reduce complexity of the external interface or to perform additional processing or security checks before reaching sensitive upstream APIs). We refer to this practice as [looping](/5.11/advanced-configuration/transform-traffic/looping). By performing the looping internally using the URL rewrite middleware, latency is reduced because the redirection is handled entirely within Tyk with no unnecessary external network calls.
...
Tyk's URL rewrite middleware uses the concepts of [triggers](/5.11/transform-traffic/url-rewriting#url-rewrite-triggers) and [rules](/5.11/transform-traffic/url-rewriting#url-rewrite-rules). These can be combined in flexible ways to create sophisticated logic to direct requests made to a single endpoint to various upstream services (or other APIs internally exposed within Tyk).
...
More detail on URL Rewrite triggers and rules can be found [here](/5.11/transform-traffic/url-rewriting#url-rewriting-overview).

{/* proposed "summary box" to be shown graphically on each middleware page

## URL Rewrite middleware summary
 - The URL Rewrite middleware is an optional stage in Tyk's API Request processing chain, sitting between the [Request Header Transform](/5.11/api-management/traffic-transformation/request-headers) and [Response Caching](/5.11/api-management/response-caching) middleware.
 ...
 - URL Rewrite can access both [session metadata](/5.11/api-management/policies#what-is-a-session-metadata) and [request context variables](/5.11/api-management/traffic-transformation/request-context-variables). */}


## URL Rewrite middleware

Tyk's [URL rewrite](/5.11/transform-traffic/url-rewriting#url-rewrite-middleware) middleware uses the concepts of [triggers](/5.11/#url-rewrite-triggers) and [rules](/5.11/#url-rewrite-rules) to determine if the request (target) URL should be modified. These can be combined in flexible ways to create sophisticated logic to direct requests made to a single endpoint to various upstream services (or other APIs internally exposed within Tyk Gateway through [looping](/5.11/advanced-configuration/transform-traffic/looping)).

### URL rewrite rules

The URL rewrite middleware compares a [key](/5.11/#key) with a [pattern](/5.11/#pattern) to determine if there is a match; the rules define the location of the key and the structure of the pattern.
...
When using the request path location, you can use wildcards in the key name (which is the URL path) - for example `/asset/{type}/author/`. The URL rewrite middleware will treat the wildcard as a `(.*)` regex so that any value matches. The wildcard value itself will be ignored, is not extracted from the key, and is not available for use in constructing the [rewrite path](/5.11/#creating-the-rewrite-path).
...
- [context variables](/5.11/api-management/traffic-transformation/request-context-variables), extracted from the request at the start of the middleware chain, can be injected into the pattern regex using the `$tyk_context.` namespace
- [session metadata](/5.11/api-management/policies#what-is-a-session-metadata), from the Tyk Session Object linked to the request, can be injected into the pattern regex using the `$tyk_meta.METADATA_KEY` namespace 
...
- values from the successful [pattern match](/5.11/#using-data-from-the-key-in-the-rewrite-path)
- values from [key-value (KV) storage](/5.11/#using-data-from-kv-storage-in-the-rewrite-path)
...
You can retrieve a value from KV storage by including a reference in the [appropriate notation](/5.11/tyk-configuration-reference/kv-store#transformation-middleware) for the KV location where the key-value pair is stored.

If you use a value retrieved from [Consul](/5.11/tyk-configuration-reference/kv-store#using-consul-as-a-kv-store) or [Vault](/5.11/tyk-configuration-reference/kv-store#using-vault-as-a-kv-store), this must be the <b>last</b> part in the `rewriteTo` URL.
...
This limitation does not apply to KV accessed from the other [supported KV stores](/5.11/tyk-configuration-reference/kv-store) (environment variable or Gateway `secrets`).


## Using Tyk OAS
<a id="url-rewriting-using-tyk-oas"></a>

Tyk's [URL rewriter](/5.11/transform-traffic/url-rewriting#url-rewrite-middleware) uses the concepts of triggers and rules to determine if the request (target) URL should be modified. These can be combined in flexible ways to create sophisticated logic to direct requests made to a single endpoint to various upstream services (or other APIs internally exposed within Tyk).

URL rewrite triggers and rules are explained in detail [here](/5.11/transform-traffic/url-rewriting#url-rewriting-overview).

When working with Tyk OAS APIs the rules and triggers are configured in the [Tyk OAS API Definition](/5.11/api-management/gateway-config-tyk-oas#operation); this can be done manually within the `.json` file or from the API Designer in the Tyk Dashboard.

If you're using the legacy Tyk Classic APIs, then check out [this](/5.11/transform-traffic/url-rewriting#url-rewriting-using-classic) page.
...
If you want to use dynamic data from context variables, you must [enable](/5.11/api-management/traffic-transformation/request-context-variables#enabling-context-variables-for-use-with-tyk-classic-apis) context variables for the API to be able to access them from the request header transform middleware.

If you're using the newer Tyk OAS APIs, then check out [this](/5.11/transform-traffic/url-rewriting#url-rewriting-using-tyk-oas) page.

If you're using Tyk Operator then check out the [configuring the URL rewriter in Tyk Operator](/5.11/#tyk-operator) section below.
...
For further examples check out the [internal looping](/5.11/product-stack/tyk-operator/advanced-configurations/internal-looping) page.
