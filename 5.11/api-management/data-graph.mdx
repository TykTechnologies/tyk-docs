...
- [How to create UDG schema](/5.11/api-management/data-graph#creating-schema)
- [How to connect data sources](/5.11/api-management/data-graph#connect-datasource)
- [How to secure the data graph](/5.11/api-management/data-graph#security)
...
Head over to the [connect data source](/5.11/api-management/data-graph#udg) section to learn about the supported data sources and how to connect them to Tyk.
...
* [UDG Arguments](/5.11/api-management/data-graph#arguments)
* [UDG Datasources](/5.11/api-management/data-graph#udg)
...
See [Concept: Arguments](/5.11/api-management/data-graph#arguments) to learn more about it.
 ```
...
Global headers now have access to all [request context variables](/5.11/api-management/traffic-transformation/request-context-variables).
...
Data source headers now have access to all [request context variables](/5.11/api-management/traffic-transformation/request-context-variables).
...
<Note>
UDG subscriptions (including SSE) are only supported for data sources of kind **GraphQL** or **Kafka**. REST data sources do not support subscriptions. If your upstream service exposes subscriptions via SSE, configure it as a **GraphQL** data source with the appropriate `subscription_type`. See [GraphQL Subscriptions](/5.11/api-management/graphql#graphql-subscriptions) for configuration details.
</Note>
...
Above steps are explained in detail in our [Getting started pages](/5.11/api-management/data-graph#connect-datasource).
...
To use GraphQL subscriptions with UDG (including SSE), you must use a data source of kind **GraphQL** or **Kafka**. See [GraphQL Subscriptions](/5.11/api-management/graphql#graphql-subscriptions) for details on configuring SSE subscriptions with a GraphQL data source.
</Note>
...
You can learn more about it [here](/5.11/#udg)
...
To learn more about arguments click [here](/5.11/#arguments)

To learn more about reusing response fields click [here](/5.11/#reusing-response-fields)
...
You can learn more about field mapping [here](/5.11/#field-mappings)
...
For this tutorial we'll mitigate these risks using `Query Depth Limit` but you can also use common strategies like rate limiting and throttling, which you can read more about [here](/5.11/api-management/rate-limit)
...
2. **Applying to query depth**
    ...
    Although we can directly create keys by selecting this API but we'll use policy as it will make it easier to update keys for this API in future. You can read more about policies [here](/5.11/api-management/policies#what-is-a-security-policy)
...
We offer 3 ways of importing an example into Tyk:
 - Using [tyk-sync](/5.11/api-management/sync/use-cases#synchronize-api-configurations-with-github-actions)
 - Manually import via [Dashboard API Import](/5.11/api-management/gateway-config-managing-classic#import-an-api)
- Using Tyk Dashboard to browse and import the examples directly

#### Import via tyk-sync

Please follow the [tyk-sync documentation](/5.11/product-stack/tyk-sync/commands#examples-publish-command) to learn more about this approach.
...
You can find more detailed instructions in the [Dashboard API Import documentation section](/5.11/api-management/gateway-config-managing-classic#import-an-api).
