...
This middleware changes only the payload and not the headers. You can, however, combine this with the [Response Header Transform](/5.11/api-management/traffic-transformation/response-headers) to apply more complex transformation to responses.

There is a closely related [Request Body Transform](/5.11/api-management/traffic-transformation/request-body) middleware that provides the same functionality on the request sent by the client prior to it being proxied to the upstream.
...
A common use of the response body transform middleware is when surfacing a legacy SOAP service with a REST API. Full details of how to perform this conversion using Tyk are provided [here](/5.11/advanced-configuration/transform-traffic/soap-rest).
...
{/* proposed "summary box" to be shown graphically on each middleware page
 # Response Body Transform middleware summary
  ...
  - Response Body Transform can access both [session metadata](/5.11/api-management/policies#what-is-a-session-metadata) and [request context variables](/5.11/api-management/traffic-transformation/request-context-variables). */}


## Using Tyk OAS
<a id="response-body-using-tyk-oas"></a>

The [response body transform](/5.11/api-management/traffic-transformation/response-body) middleware provides a way to modify the payload of API responses before they are returned to the client.
...
If you're using the legacy Tyk Classic APIs, then check out the [Tyk Classic](/5.11/#response-body-using-classic) page.
...
If you're using the newer Tyk OAS APIs, then check out the [Tyk OAS](/5.11/#response-body-using-tyk-oas) page.
