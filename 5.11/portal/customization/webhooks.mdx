...
2. **Select Event Types for the Webhook**
    ...
    To finish configuration, select types of events that should be sent to the **Target URL** and save the changes. Refer the docs below to know more about [supported event types](/5.11/#supported-portal-events)
...
- [UserRegistered](/5.11/portal/customization/webhooks#new-user-registered) when a new user is registered.
- [UserAccountActivated](/5.11/portal/customization/webhooks#user-account-activated) when a user is activated.
- [UserAccountDeactivated](/5.11/portal/customization/webhooks#user-account-deactivated) when a user is deactivated.
- [PasswordReset](/5.11/portal/customization/webhooks#password-reset) when a user tries to reset a password.
- [ApplicationRegistered](/5.11/portal/customization/webhooks#new-application-registered) when a new API consumer application is created.
- [CredentialRegistered](/5.11/portal/customization/webhooks#new-credential-is-created) when a new API credential is created.
- [AccessRequestCreated](/5.11/portal/customization/webhooks#new-access-request-created) when a new API access request is created.
- [AccessRequestApproved](/5.11/portal/customization/webhooks#an-access-request-is-approved) when an API access request is approved.
- [AccessRequestRejected](/5.11/portal/customization/webhooks#an-access-request-is-rejected) when an API access request is rejected.
- [OrganizationRegistered](/5.11/portal/customization/webhooks#new-organization-registered) when an API consumer organization is created.
- [OrganizationRequestCreated](/5.11/portal/customization/webhooks#new-organization-registration-request-created) when a new API consumer organization registration request is created.
- [OrganizationRequestApproved](/5.11/portal/customization/webhooks#organization-registration-request-is-approved) when an API consumer organization registration request is approved.
- [OrganizationRequestRejected](/5.11/portal/customization/webhooks#organization-request-is-rejected) when an API consumer organization registration request is rejected.
...
This event is fired whenever a new consumer organization is created via the admin APIs, the live portal ([the become an organization flow](/5.11/portal/api-consumer)) or the admin UI.
...
This event is fired whenever a new organization request is created via the live portal ([the become an organization flow](/5.11/portal/api-consumer)) or the admin UI.
