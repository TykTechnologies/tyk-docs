...
Tyk Docs uses a versioning system that aligns with the Tyk components. The versioning follows the Semantic Versioning of Tyk [Gateway](/5.11/developer-support/release-notes/gateway).
...
Tyk maintains [LTS versions](/5.11/developer-support/release-types/long-term-support#current-lts-releases-timeline). During some releases, we need to update the LTS release alongside the latest version. For example, you may need to maintain both 5.7.2 (latest) and 5.3.2 (LTS).
