...
In this release, we have fixed some priority CVEs. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.11.1).
...
| ---------------------- | --------------- | ------------------- | -------- | 
| [GoLang](https://go.dev/dl/)          | 1.24 | 1.24 | [Go plugins](/5.11/api-management/plugins/golang) must be built using Go 1.24 | 
...
| [OpenAPI Specification](https://spec.openapis.org/oas/v3.0.3) | v3.0.x | v3.0.x | Supported by [Tyk OAS](/5.11/api-management/gateway-config-tyk-oas#tyk-vendor-extension-reference)|
...
If you are upgrading to 5.11.1, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
- [Docker Image to pull](https://hub.docker.com/r/tykio/tyk-dashboard/tags?page=&page_size=&ordering=&name=v5.11.1)
  - ```bash
    ...
    ```
- Helm charts
  - [tyk-charts v5.0.0](/5.11/developer-support/release-notes/helm-chart#5-0-0-release-notes)
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.11.0).
...
In this release we've deprecated the [policies.allow_explicit_policy_id](/5.11/tyk-oss-gateway/configuration#policies-allow-explicit-policy-id) configuration option. This was previously added to allow the use of custom policy IDs, which is now the default behaviour so this option is redundant.

#### Upgrade instructions
<a id="upgrade-5.11.0"></a>

If you are upgrading to 5.11.0, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
This patch release fixes an issue in the API Designer where JWKS cache timeout values could not be saved properly. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.10.2).
...
If you are upgrading to 5.10.2, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
- [Docker Image to pull](https://hub.docker.com/r/tykio/tyk-dashboard/tags?page=&page_size=&ordering=&name=v5.10.2)
  - ```bash
    ...
    ```
- Helm charts
  - [tyk-charts v4.1.0](/5.11/developer-support/release-notes/helm-chart#4-1-0-release-notes)
...
This patch release upgrades the Go build environment and delivers UI, analytics, and security fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.10.1).
...
If you are upgrading to 5.10.1, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
- [Docker Image to pull](https://hub.docker.com/r/tykio/tyk-dashboard/tags?page=&page_size=&ordering=&name=v5.10.1)
  - ```bash
    ...
    ```
- Helm charts
  - [tyk-charts v4.0.0](/5.11/developer-support/release-notes/helm-chart#4-0-0-release-notes)
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.10.0).
...
For more details, please see the dedicated [Gateway events](/5.11/api-management/gateway-events) section.
...
If you are upgrading to 5.10.0, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
Both interfaces now consistently accept either `null` or `[]` (empty array) values in the `versions` field of the access control list, ensuring seamless interoperability between API and UI workflows for policy and key management. Tyk OAS APIs use a [different approach](/5.11/api-management/api-versioning#how-api-versioning-works-with-tyk) to versioning, with each (base or child) version having a unique API ID that is added to the access list.
</Accordion>
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.9.2).
...
| Third Party Dependency | Tested Versions | Compatible Versions | Comments | 
| :---------------------- | :--------------- | :------------------- | :-------- | 
| [GoLang](https://go.dev/dl/)          | 1.23 | 1.23 | [Go plugins](/5.11/api-management/plugins/golang) must be built using Go 1.23 | 
| [Redis](https://redis.io/download/)   | 5.x, 6.x, 7.x    |  5.x, 6.x, 7.x    | | 
| [Valkey](https://valkey.io/download/) | 8.0.x, 8.1.x    | 7.2.x, 8.0.x, 8.1.x    | | 
| [MongoDB](https://www.mongodb.com/try/download/community)  | 6, 7, 8  | 5, 6, 7, 8  | | 
| [DocumentDB](https://aws.amazon.com/documentdb/)  | 4, 5  | 4, 5  | | 
...
If you are upgrading to 5.9.2, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
Resolved a compatibility issue introduced via a fix in Dashboard v5.9.0 (LTS v5.8.3) to account for DocumentDB's handling of endpoint paths containing dots (`.`). The Dashboard now escapes dot characters (e.g., `\u002e`) when storing Tyk OAS API definitions in the database, but MDCB failed to unescape them when reading from the database. This caused the [request validation](/5.11/api-management/traffic-transformation/request-validation) and [mock response](/5.11/api-management/traffic-transformation/mock-response) middleware configured on affected endpoints not to be applied. 

To align MDCB's dot handling mechanism with Tyk Dashboard, a new configuration option, `escape_dots_in_oas_paths`, has been introduced in both [Dashboard](/5.11/tyk-dashboard/configuration#escape_dots_in_oas_paths) and [MDCB](/5.11/tyk-multi-data-centre/mdcb-configuration-options#escape_dots_in_oas_paths):
...
Check the [Upgrade and Compatibility section](/5.11/#upgrade-5.9.2) for details on the recommended upgrade path.
</Expandable>
...
If you are upgrading to 5.9.1, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
This release builds on the recent release of [Tyk 5.8.3](/5.11/developer-support/release-notes/dashboard#583-release-notes), adding a collection of new capabilities. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.9.0).
...
If you are upgrading to 5.9.0, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
In this release, we have fixed some priority CVEs. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.8.11).
...
| Third Party Dependency                                       | Tested Versions        | Compatible Versions    | Comments | 
| :------------------------------------------------------------ | :---------------------- | :---------------------- | :-------- | 
| [Go](https://go.dev/dl/)                                     | 1.24  |  1.24  | [Go plugins](/5.11/api-management/plugins/golang) must be built using Go 1.24 | 
| [Redis](https://redis.io/download/)  | 6.2.x, 7.x  | 6.2.x, 7.x  | Used by Tyk Gateway | 
| [OpenAPI Specification](https://spec.openapis.org/oas/v3.0.3)| v3.0.x                 | v3.0.x                 | Supported by [Tyk OAS](/5.11/api-management/gateway-config-tyk-oas) |
...
If you are upgrading to 5.8.11, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk).
...
This patch release addresses a CVE present in 5.8.9. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.8.10).
...
If you are upgrading to 5.8.10, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk).
...
This patch release contains various bug fixes and introduces new OPA helper functions to identify the type of API being processed (Tyk OAS, Tyk Streams, Tyk Classic). For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.8.9).
...
In this release we've deprecated the [allow_explicit_policy_id](/5.11/tyk-dashboard/configuration#allow-explicit-policy-id) configuration option. This was previously added to allow the use of custom policy IDs, which is now the default behaviour so this option is redundant.

#### Upgrade instructions

If you are upgrading to 5.8.9, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
This patch release upgrades the Go build environment and fixes some high-priority CVEs. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.8.8).
...
If you are upgrading to 5.8.8, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
This patch release contains various bug fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.8.7).
...
If you are upgrading to 5.8.7, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
Resolved an issue where the [Dashboard API](/5.11/tyk-dashboard-api) allowed creation of empty or duplicate categories when updating Tyk OAS APIs through the `PUT /api/apis/oas/{API_ID}/categories` endpoint.
...
This patch release contains various bug fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.8.6).
...
If you are upgrading to 5.8.6, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.8.5).
...
| Third Party Dependency                                       | Tested Versions        | Compatible Versions    | Comments | 
| :------------------------------------------------------------ | :---------------------- | :---------------------- | :-------- | 
| [Go](https://go.dev/dl/)                                     | 1.23  |  1.23  | [Go plugins](/5.11/api-management/plugins/golang) must be built using Go 1.23 | 
| [Redis](https://redis.io/download/)  | 6.2.x, 7.x  | 6.2.x, 7.x  | | 
| [MongoDB](https://www.mongodb.com/try/download/community)  | 5.0.x, 6.0.x, 7.0.x  | 5.0.x, 6.0.x, 7.0.x  | | 
| [DocumentDB](https://aws.amazon.com/documentdb/)  | 4, 5  | 4, 5  | | 
| [PostgreSQL](https://www.postgresql.org/download/)         | 13.x - 17.x        | 13.x - 17.x            | | 
...
If you are upgrading to 5.8.5, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk).
...
- [Docker image to pull](https://hub.docker.com/r/tykio/tyk-gateway/tags?page=&page_size=&ordering=&name=v5.8.5)
  - ```bash
    ...
    ``` 
- Helm charts
  - [tyk-charts v3.0.0](/5.11/developer-support/release-notes/helm-chart#300-release-notes)
...
Check the [Upgrade and Compatibility section](/5.11/#upgrade-5.8.5) for details on the recommended upgrade path.
</Expandable>
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.8.4).
...
If you are upgrading to 5.8.4, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
This patch release contains various bug fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.8.3) below.
...
If you are upgrading to 5.8.3, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
- The Tyk OAS API designer now fully supports the use of Key-Value storage [references](/5.11/tyk-configuration-reference/kv-store#from-api-definitions) that are valid in the API definition.
...
If you are upgrading to 5.8.2, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
This patch release contains various bug fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.8.1) below.
...
If you are upgrading to 5.8.1, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
With Tyk 5.8.0 we are delighted to unlock the power and flexibility of Tyk OAS for all users, with full feature parity with the legacy Tyk Classic style for REST APIs. We are thrilled to announce new updates and improvements in Tyk 5.8.0, delivering more control, flexibility, and performance.  For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.8.0) below.
...
If you are upgrading to 5.8.0, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
| Third Party Dependency                                     | Tested Versions        | Compatible Versions    | Comments | 
| :---------------------------------------------------------- | :---------------------- | :---------------------- | :-------- | 
| [GoLang](https://go.dev/dl/)                               | 1.22       | 1.22       | [Go plugins](/5.11/api-management/plugins/golang) must be built using Go 1.22 | 
| [Redis](https://redis.io/download/)  | 6.2.x, 7.x  | 6.2.x, 7.x  | | 
| [MongoDB](https://www.mongodb.com/try/download/community)  | 5.0.x, 6.0.x, 7.0.x  | 5.0.x, 6.0.x, 7.0.x  | | 
| [DocumentDB](https://aws.amazon.com/documentdb/)  | 4, 5  | 4, 5  | | 
...
If you are upgrading to 5.7.3, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
- [Docker Image to pull](https://hub.docker.com/r/tykio/tyk-dashboard/tags?page=&page_size=&ordering=&name=v5.7.3)
  - ```bash
    ...
    ```
- Helm charts
  - [tyk-charts v2.2.0](/5.11/developer-support/release-notes/helm-chart#220-release-notes)
...
This release focuses mainly on a security fix. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.7.2) below.
...
#### Upgrade instructions
If you are upgrading to 5.7.2, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
This release focuses mainly on bug fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.7.1) below.
...
#### Upgrade instructions
If you are upgrading to 5.7.1, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
We are thrilled to announce new updates and improvements in Tyk 5.7.0, bringing more control, flexibility, and performance.  For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.7.0) below.
...
* **[External OAuth](/5.11/api-management/client-authentication#integrate-with-external-authorization-server-deprecated)**

  * Tyk Classic: `external_oauth`
  * Tyk OAS: `server.authentication.securitySchemes.externalOAuth`

* **[OpenID Connect (OIDC)](/5.11/api-management/client-authentication#integrate-with-openid-connect-deprecated)**

  * Tyk Classic: `auth_configs.oidc`
  * Tyk OAS: `server.authentication.oidc`

We recommend migrating to **[JWT Authentication](/5.11/basic-config-and-security/security/authentication-authorization/json-web-tokens)** for improved flexibility and long-term support.
...
#### Upgrade instructions
If you are upgrading to 5.7.0, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
[Setup guide for JWE OIDC SSO](/5.11/api-management/single-sign-on-social-idp#log-into-an-app-with-github-oauth)
</Accordion>
...
For detail usage of the `/audit-logs` endpoint, please see [Dashboard API documentation](/5.11/tyk-dashboard-api).
</Accordion>
...
If you are upgrading to 5.6.1, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
- ```bash
  ...
  ```
- Helm charts
  - [Tyk Charts v2.0.0](/5.11/developer-support/release-notes/helm-chart#200-release-notes)
...
We are thrilled to announce new updates and improvements in Tyk 5.6.0, bringing more control, flexibility, and performance.  For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.6.0) below.

##### Per endpoint Rate Limiting for clients

Now you can configure rate limits at the [endpoint level per client](/5.11/api-management/rate-limit#key-level-rate-limiting), using new configuration options in the access key. Use Tyk's powerful [security policies](/5.11/api-management/policies#what-is-a-security-policy) to create templates to set appropriate rate limits for your different categories of user.
...
We encourage users to switch to the recommended alternatives. For more detailed information, please refer to the [Documentation](/5.11/api-management/client-authentication#integrate-with-openid-connect-deprecated)
...
{/* ###### Future deprecations */}
#### Upgrade instructions
If you are upgrading to 5.6.0, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
- ```bash
  ...
  ```
- Helm charts
  - [tyk-charts v2.1.0](/5.11/developer-support/release-notes/helm-chart#210-release-notes)
...
| Third Party Dependency                                     | Tested Versions        | Compatible Versions    | Comments | 
| :---------------------------------------------------------- | :---------------------- | :---------------------- | :-------- | 
| [GoLang](https://go.dev/dl/)                               | 1.21       | 1.21       | [Go plugins](/5.11/api-management/plugins/golang#) must be built using Go 1.21 | 
| [Redis](https://redis.io/download/)  | 6.2.x, 7.x  | 6.2.x, 7.x  | | 
| [MongoDB](https://www.mongodb.com/try/download/community)  | 5.0.x, 6.0.x, 7.0.x  | 5.0.x, 6.0.x, 7.0.x  | | 
| [DocumentDB](https://aws.amazon.com/documentdb/)  | 4, 5  | 4, 5  | | 
| [PostgreSQL](https://www.postgresql.org/download/)         | 12.x - 16.x        | 12.x - 16.x            | | 
...
If you are upgrading to 5.5.2, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
If you are upgrading to 5.5.1, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
We are excited to announce Tyk Dashboard 5.5, featuring a brand-new dashboard identity, advanced rate-limiting capabilities, and enhanced security options. For a comprehensive list of changes, please refer to the [changelog](/5.11/#Changelog-v5.5.0) below.
...
Now configure rate limits at the endpoint level for both [Tyk OAS](/5.11/api-management/rate-limit#tyk-oas-api-definition) and [Tyk Classic APIs](/5.11/api-management/rate-limit#tyk-classic-api-definition), providing granular protection for upstream services against overloading and abuse.
...
{/* ###### Future deprecations */}
#### Upgrade instructions
If you are upgrading to 5.5.0, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
- ```bash
  ...
  ```
- Helm charts
  - [tyk-charts v1.6](/5.11/developer-support/release-notes/helm-chart#160-release-notes)
...
{/* ###### Future deprecations */}
#### Upgrade instructions
If you are upgrading to 5.4.0, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk).
...
- ```bash
  ...
  ```
- Helm charts
  - [tyk-charts v1.5](/5.11/developer-support/release-notes/helm-chart#150-release-notes)
...
If you are upgrading to 5.3.12, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk).
...
This patch release contains various bug fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.3.11) below.
...
If you are upgrading to 5.3.11, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
In this release, we upgraded the Golang version to `v1.23` and fixed a [CVE-2025-21613](https://nvd.nist.gov/vuln/detail/CVE-2025-21613). For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.3.10) below.
...
#### Upgrade Instructions
If you are upgrading to 5.3.10, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk).
...
#### Release Highlights
This release contains bug fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.3.9) below.
...
#### Upgrade Instructions
If you are upgrading to 5.3.9, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk).
...
#### Release Highlights
This release focuses mainly on bug fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.3.8) below.
...
This is an advanced notice that the dedicated External OAuth, OpenID Connect (OIDC) authentication options, and SQLite support will be deprecated starting in version 5.7.0. We recommend that users of the [External OAuth](/5.11/api-management/client-authentication#integrate-with-external-authorization-server-deprecated) and [OpenID Connect](/5.11/api-management/client-authentication#integrate-with-openid-connect-deprecated) methods migrate to Tyk's dedicated [JWT Auth](/5.11/basic-config-and-security/security/authentication-authorization/json-web-tokens) method. Please review your API configurations, as the Gateway logs will provide notifications for any APIs utilizing these methods.

#### Upgrade Instructions
If you are upgrading to 5.3.8, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk).
...
The UI now displays a deprecation notice for the dedicated [External OAuth](/5.11/api-management/client-authentication#integrate-with-external-authorization-server-deprecated) and [OpenID Connect (OIDC)](/5.11/api-management/client-authentication#integrate-with-openid-connect-deprecated) authentication mechanisms. This provides advanced notification that these authentication options will be deprecated in version 5.7.0. Users are advised to migrate to the [JWT Auth](/5.11/basic-config-and-security/security/authentication-authorization/json-web-tokens) method, which supports integration with both OAuth and OIDC providers, in preparation for future upgrade.
</Expandable>
...
If you are upgrading to 5.3.7, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
This release primarily focuses on bug fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.3.6) below.
...
If moving from a version of Tyk older than 5.3.0 please read the explanation provided with [5.3.0 release](/5.11/#TykOAS-v5.3.0).

#### Deprecations
There are no deprecations in this release.

#### Upgrade Instructions
When upgrading to 5.3.6, please follow the [detailed upgrade instructions](/5.11/#upgrading-tyk).
...
With MongoDB 4.4 reaching [EOL](https://www.mongodb.com/legal/support-policy/lifecycles) in February 2024, we can no longer guarantee full compatibility with this version of the database. If you are [using MongoDB](/5.11/planning-for-production/database-settings#mongodb) we recommend that you upgrade to a version that we have tested with, as indicated [below](/5.11/#3rdPartyTools-v5.3.6).


With PostgreSQL v11 reaching [EOL](https://www.postgresql.org/support/versioning/) in November 2023, we can no longer guarantee full compatibility with this version of the database. If you are [using PostgreSQL](/5.11/planning-for-production/database-settings#postgresql) we recommend that you upgrade to a version that we have tested with, as indicated [below](/5.11/#3rdPartyTools-v5.3.6).
...
- [Docker Image to pull](https://hub.docker.com/r/tykio/tyk-dashboard/tags?page=&page_size=&ordering=&name=v5.3.6)
 - ```bash
   ...
   ```
- Helm charts
 - [tyk-charts v2.0](/5.11/developer-support/release-notes/helm-chart#200-release-notes)
...
There are no breaking changes in this release, however, if moving from a version of Tyk older than 5.3.0 please read the explanation provided with [5.3.0 release](/5.11/#TykOAS-v5.3.0).
...
When upgrading to 5.3.5, please follow the [detailed upgrade instructions](/5.11/#upgrading-tyk).
...
With MongoDB 4.4 reaching [EOL](https://www.mongodb.com/legal/support-policy/lifecycles) in February 2024, we can no longer guarantee full compatibility with this version of the database. If you are [using MongoDB](/5.11/planning-for-production/database-settings#mongodb) we recommend that you upgrade to a version that we have tested with, as indicated [below](/5.11/#3rdPartyTools-v5.3.5).
...
#### Upgrade Instructions
When upgrading to 5.3.4 please follow the [detailed upgrade instructions](/5.11/#upgrading-tyk).


#### Release Highlights
For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.3.4) below.
...
With MongoDB 4.4 reaching [EOL](https://www.mongodb.com/legal/support-policy/lifecycles) in February 2024, we can no longer guarantee full compatibility with this version of the database. If you are [using MongoDB](/5.11/planning-for-production/database-settings#mongodb) we recommend that you upgrade to a version that we have tested with, as indicated [below](/5.11/#3rdPartyTools-v5.3.3).
...
- [Docker Image to pull](https://hub.docker.com/r/tykio/tyk-dashboard/tags?page=&page_size=&ordering=&name=v5.3.4)
 - ```bash
   ...
   ```
- Helm charts
 - [tyk-charts v1.4](/5.11/developer-support/release-notes/helm-chart#140-release-notes)
...
#### Upgrade Instructions
When upgrading to 5.3.3 please follow the [detailed upgrade instructions](/5.11/#upgrading-tyk).

#### Release Highlights

For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.3.3) below.
...
#### Upgrade Instructions
When upgrading to 5.3.2 please follow the [detailed upgrade instructions](/5.11/#upgrading-tyk).


#### Release Highlights
This release primarily focuses on bug fixes.
For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.3.2) below.
...
With MongoDB 4.4 reaching [EOL](https://www.mongodb.com/legal/support-policy/lifecycles) in February 2024, we can no longer guarantee full compatibility with this version of the database. If you are [using MongoDB](/5.11/planning-for-production/database-settings#mongodb) we recommend that you upgrade to a version that we have tested with, as indicated [below](/5.11/#3rdPartyTools-v5.3.2).
...
#### Upgrade Instructions
When upgrading to 5.3.1, please follow the [detailed upgrade instructions](/5.11/#upgrading-tyk).


#### Release Highlights
This release primarily focuses on bug fixes.
For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.3.1) below.
...
With MongoDB 4.4 reaching [EOL](https://www.mongodb.com/legal/support-policy/lifecycles) in February 2024, we can no longer guarantee full compatibility with this version of the database. If you are [using MongoDB](/5.11/planning-for-production/database-settings#mongodb) we recommend that you upgrade to a version that we have tested with, as indicated [below](/5.11/#3rdPartyTools-v5.3.1).
...
- ```bash
  ...
  ```
- Helm charts
  - [tyk-charts v1.3](/5.11/developer-support/release-notes/helm-chart#130-release-notes)
...
This upgrade transitions Tyk OAS APIs out of [Early Access](/5.11/developer-support/release-types/early-access-feature).
...
- **Not Backwards Compatible**
  ...
  - This means that if you wish to downgrade or revert to your previous version of Tyk, you will need to restore these API definitions from a backup. Please go to the [backup](/5.11/#upgrade-instructions) section for detailed instructions on backup before upgrading to v5.3.0.
...
**Important:** Please go to the [backup](/5.11/#upgrade-instructions) section for essential instructions on how to backup before upgrading to v5.3.0
...
With MongoDB 4.4 reaching [EOL](https://www.mongodb.com/legal/support-policy/lifecycles) in February 2024, we can no longer guarantee full compatibility with this version of the database and recommend upgrading to a version that we have tested with, as indicated [below](/5.11/#3rdPartyTools-v5.3.0).
...
1. For Self Managed deployments - Backup Your environment using the [usual guidance](/5.11/developer-support/upgrading#tyk-upgrade-guides-for-different-deployment-models) documented with every release (this includes backup config file and database).
2. For all deployments - Backup all your API definitions (Tyk OAS API and Classic Definitions):
   - For Tyk Cloud deployments - To perform the backup please use our guide for [exporting APIs and policies](/5.11/developer-support/upgrading#backup-apis-and-policies).
   - For Self-Managed deployments -  To perform the backup please use [Tyk Sync](/5.11/api-management/automations/sync).
4. Performing the upgrade - For all deployments, follow the instructions in the [upgrade guide](/5.11/#upgrading-tyk) when upgrading Tyk.

#### Release Highlights

We are excited to announce the release of 5.3.0, packed with new features, improvements and bug fixes to enhance your experience with Tyk Dashboard. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.3.0) below.

##### Tyk OAS Feature Maturity

Tyk OAS is now out of [Early Access](/5.11/developer-support/release-types/early-access-feature) as we have reached feature maturity. You are now able to make use of the majority of Tyk's features from your Tyk OAS APIs, so they are a credible alternative to the legacy Tyk Classic APIs.
...
Tyk uses `mongo-go` as the default MongoDB driver from v5.3. This provides support for MongoDB 4.4.x, 5.0.x, 6.0.x and 7.0.x. If you are using older MongoDB versions e.g. 3.x, please set MongoDB driver to `mgo`. The [MongoDB supported versions](/5.11/planning-for-production/database-settings#supported-versions) page provides details on how to configure MongoDB drivers in Tyk.
</Accordion>
</AccordionGroup>
...
Relaxed the strict validation for mock response so that the `Description` field is now optional for `response`, `responses` and `schema` within the OpenAPI description. Automatically configuring mock responses when using [Tyk OAS APIs](/5.11/api-management/traffic-transformation/mock-response#mock-responses-using-openapi-metadata) is now even easier.
</Accordion>
...
#### Upgrade Instructions
If you are using a 5.2.x version, we advise you to upgrade ASAP to this latest release. If you are on an older version, you should skip 5.2.0 and upgrade directly to this release. Go to the [Upgrading Tyk](/5.11/#upgrading-tyk) section for detailed upgrade Instructions.

#### Release Highlights
Dashboard 5.2.5 was version bumped only, to align with Gateway 5.2.5. Subsequently, no changes were encountered in release 5.2.5. Gateway 5.2.5 was a critical patch release. For further information please see the release notes for Gateway [v5.2.5](/5.11/developer-support/release-notes/gateway) 
...
#### Release Highlights
This release primarily focuses on bug fixes. 
For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.2.4) below.
...
Fixed two UI issues with the [OPA editor](/5.11/api-management/dashboard-configuration#using-the-open-policy-agent-in-the-dashboard) in the Tyk Dashboard to improve experience when using this feature. Scrolling beyond the end of the OPA window does not now start to scroll the API Designer window, and minimizing then re-expanding the OPA editor no longer limits the text to one line.
</Accordion>
...
#### Release Highlights
This release primarily focuses on bug fixes. 
For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.2.3) below.
...
Fixed an issue where the [OPA editor](/5.11/api-management/dashboard-configuration#using-the-open-policy-agent-in-the-dashboard) was not resizable. The fix ensures the floating OPA editor is now resizable and the resizing operation is smooth, improving user experience.
</Accordion>

<Accordion title='User Search not working unless you enter the full email address'>
Fixed an issue where the [User Search](/5.11/api-management/user-management#search-users) was not working unless the full email address was entered. The fix restores the functionality of showing suggestions for names as they are typed in, improving user experience and search efficiency.
</Accordion>
...
Fixed an issue in the Classic API Designer where the 'use_standard_auth' value was constantly reverting to 'true' when editing an API with an [external OAuth flow](/5.11/api-management/client-authentication#integrate-with-external-authorization-server-deprecated). This fix ensures the 'use_standard_auth' value remains consistent, enabling the use of external OAuth via the Raw API editor.
</Accordion>
...
#### Release Highlights
This release primarily focuses on bug fixes. 
For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.2.2) below.
...
Added a new Dashboard configuration option `allow_unsafe_oas`. This permits the modification of Tyk OAS APIs via the Tyk Classic API endpoints. This is not a recommended action due to the risk of inconsistent behavior and potential for breaking changes while Tyk OAS is in [Early Access](/5.11/developer-support/release-types/early-access-feature). This is provided for early adopters and will be deprecated later, once Tyk OAS reaches full maturity.
</Expandable>
...
#### Upgrade Instructions
If you are on a 5.2.0 we advise you to upgrade ASAP and if you are on an older version skip 5.2.0 and upgrade directly to this release. Go to the [Upgrading Tyk](/5.11/#upgrading-tyk) section for detailed upgrade Instructions.

#### Release Highlights
This release primarily focuses on bug fixes. 
For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.2.0) below.
...
We're thrilled to bring you some exciting enhancements and crucial fixes to improve your experience with Tyk Dashboard. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.2.0) below.

Configure Caching Timeouts Per API Endpoint and Enable Advanced Caching Options From Within Dashboard

We’ve added the ability to [configure](/5.11/api-management/response-caching#configuring-the-middleware-in-the-tyk-oas-api-definition) per-endpoint timeouts for Tyk’s response cache, giving you increased flexibility to tailor your APIs to your upstream services. While doing this, we’ve also fixed a longstanding issue within the *Tyk Dashboard* so that you can configure more of the [advanced caching](/5.11/api-management/response-caching#configuring-the-middleware-in-the-api-designer) options from within the UI.

##### Added Body Transform Middleware to Tyk OAS API Definition

With this release, we are adding the much requested *Body Transformations* to *Tyk OAS API Definition*. You can now [configure](/5.11/api-management/gateway-config-tyk-oas#transformbody) middleware for both [request](/5.11/api-management/traffic-transformation/request-body) and [response](/5.11/api-management/traffic-transformation/response-body) *Body Transformations* and - as a *Tyk Dashboard* user - you’ll be able to do so from within our simple and elegant API Designer tool. Visually test and preview *Body Transformations* from within the API Designer.
...
Added a new [endpoint](/5.11/tyk-dashboard-api), */system/stats*, to provide insight and operational statistics on total and active APIs deployed. The endpoint's flexible date filtering options, equip users to obtain comprehensive insights into usage trends.
</Expandable>
...
Fixed an issue where the [Enforced Timeout](/5.11/planning-for-production/ensure-high-availability/enforced-timeouts) configuration parameter of an API endpoint accepted negative values, without displaying validation errors. With this fix, users receive clear feedback and prevent unintended configurations.
</Accordion>
...
documentation for a full description of this new [user role](/5.11/api-management/user-management#user-permissions).

##### Import API examples from within the Dashboard

In 5.0 we introduced the possibility to import API examples manually or via
[_Tyk Sync_](/5.11/api-management/automations/sync). We have now extended this feature and it is now possible to do this without
...
We added a [Dashboard API Endpoint](/5.11/api-management/data-graph#automatically-creating-rest-udg-configuration-based-on-oas-specification) that is capable of taking an OAS 3.0.0 document and converting it into a UDG API.
...
size using [slave_options.rpc_pool_size](/5.11/tyk-oss-gateway/configuration#slave_optionsrpc_pool_size)
...
- Added a feature to display a full [_Tyk Sync_](/5.11/api-management/automations/sync) command that will allow a user to import an example UDG into their Dashboard
...
If you are upgrading to 5.0.15, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk). 
...
Go to the [Upgrading Tyk](/5.11/developer-support/release-notes/gateway#upgrading-tyk) section for detailed upgrade instructions.
...
See how to [Choose a MongoDB driver](/5.11/planning-for-production/database-settings#choose-a-mongodb-driver)
...
##### Release Highlights
This release primarily focuses on bug fixes. 
For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.0.1) below.
...
We’ve improved support for [OAS Mock Responses](/5.11/api-management/traffic-transformation/mock-response), with the Tyk OAS API definition now allowing you to register multiple Mock Responses in a single API, providing you with increased testing flexibility.
...
- via command line [using tyk-sync](/5.11/api-management/data-graph#udg-examples)
...
With this release we are also giving our users [improved headers for GQL APIs](/5.11/api-management/graphql#graphql-apis-headers). It is now possible to use context variables in request headers and persist headers needed for introspection separately for improved security.
...
- Now you can turn a Kafka topic into a GraphQL subscription by simply [importing your AsyncAPI definition](/5.11/api-management/dashboard-configuration#data-graphs-api)
...
[Importing OAS v3 via the Dashboard](/5.11/api-management/gateway-config-managing-oas#importing-an-openapi-description-to-create-an-api)
...
Follow the [standard upgrade guide](/5.11/developer-support/upgrading), there are no breaking changes in this release.

If you want switch from MongoDB to SQL, you can [use our migration tool](/5.11/planning-for-production/database-settings#migrating-from-an-existing-mongodb-instance), but keep in mind that it does not yet support the migration of your analytics data.


<Note>
Note: Upgrading the Golang version implies that all the Golang custom plugins that you are using need to be recompiled before migrating to 4.3 version of the Gateway. Check our docs for more details [Golang Plugins](/5.11/api-management/plugins/golang).
</Note>
...
Tyk has always had a proprietary specification for defining APIs. From Tyk v4.1 we now support defining APIs using the Open API Specification (OAS) as well, which can offer significant time and complexity savings. [This is an early access capability](/5.11/developer-support/release-types/early-access-feature).
...
This capability is available in both the open source and paid versions of Tyk. See our [Tyk OAS documentation](/5.11/api-management/gateway-config-tyk-oas) for more details.
...
- Added option to configure certificates for Tyk Dashboard using [environment variables](/5.11/tyk-dashboard/configuration#http_server_optionscertificates).
...
[Federation docs](/5.11/api-management/graphql#overview-1)

[Subgraphs and Supergraphs docs](/5.11/api-management/graphql#subgraphs-and-supergraphs)
...
[Subscriptions docs](/5.11/api-management/graphql#graphql-subscriptions)
...
While SQL support for Tyk products does not depend on specific database features, with this release, we will provide official support for [PostgreSQL DB for production purposes](/5.11/planning-for-production/database-settings#postgresql), and SQLite for development and PoC environments. Note that SQL support is available for self-managed setups only.
...
[MongoDB to SQL migration docs](/5.11/planning-for-production/database-settings#migrating-from-an-existing-mongodb-instance)
...
Check our DCR docs [here](/5.11/tyk-developer-portal/tyk-portal-classic/dynamic-client-registration)
...
We've updated the GraphQL functionality of our [Universal Data Graph](/5.11/api-management/data-graph#overview). You’re now able to deeply nest GraphQL & REST APIs and stitch them together in any possible way.

Queries are now possible via WebSockets and Subscriptions are coming in the next Release (3.3.0).

You're also able to configure [upstream Headers dynamically](/5.11/api-management/data-graph#header-forwarding), that is, you’re able to inject Headers from the client request into UDG upstream requests. For example, it can be used to access protected upstreams. 
...
#### Upgrade process
If you already have GraphQL or UDG APIs you need to follow this [upgrade guide](/5.11/api-management/graphql#migrating-to-32).
...
This follows the recent changes that we have made to embed TIB (Tyk Identity Broker)in the dashboard. See 3.0 [release notes](/5.11/developer-support/release-notes/dashboard#tyk-identity-broker-now-built-in-to-the-dashboard) for more information regarding this. 

To learn more [see the documentation](/5.11/api-management/external-service-integration)
...
Docs [here](/5.11/api-management/graphql#syncing-gql-schema)
...
Docs [here](/5.11/api-management/graphql#graphql-playground)
...
[Docs here](/5.11/tyk-developer-portal/tyk-portal-classic/key-requests)
...
See our updated [Tutorials](/5.11/tyk-self-managed) section.
...
Read more about the [GraphQL](/5.11/api-management/graphql) and [Universal Data Graph](/5.11/api-management/data-graph#overview)
...
See updated tutorials on how to [create a policy](/5.11/api-management/gateway-config-managing-classic#secure-an-api) and [keys](/5.11/api-management/gateway-config-managing-classic#access-an-api)
...
See [updated flow details](/5.11/api-management/external-service-integration#what-is-tyk-identity-broker-tib)
...
[Read more](/5.11/tyk-configuration-reference/kv-store)
...
At the moment the Response hook is supported for [Python and gRPC plugins](/5.11/api-management/plugins/rich-plugins#coprocess-dispatcher---hooks).
...
[Read More](/5.11/planning-for-production/ensure-high-availability/health-check)
...
[Read More](/5.11/api-management/troubleshooting-debugging#capturing-detailed-logs)
...
[Read More](/5.11/planning-for-production/ensure-high-availability/load-balancing#configure-load-balancing-and-weighting-via-the-dashboard)
...
[Read More](/5.11/api-management/tyk-pump#tyk-pump-configuration)
...
If you are upgrading from version 2.8, please [read this guide](/5.11/developer-support/release-notes/archived#upgrading-from-version-28)


## Further Information

### Upgrading Tyk
Please refer to the [upgrading Tyk](/5.11/developer-support/upgrading) page for further guidance on the upgrade strategy.
...
- [OpenAPI Document](/5.11/tyk-dashboard-api)
- [Postman Collection](https://www.postman.com/tyk-technologies/workspace/tyk-public-workspace/overview)

### FAQ

Please visit our [Developer Support](/5.11/developer-support/community) page for further information relating to reporting bugs, upgrading Tyk, technical support and how to contribute.
