...
- [Docker image to pull](https://hub.docker.com/r/tykio/tyk-gateway/tags?page=&page_size=&ordering=&name=v5.10.2)
  - ```bash
    ...
    ``` 
- Helm charts
  - [tyk-charts v4.0.0](/5.11/developer-support/release-notes/helm-chart#4-1-0-release-notes)
...
This patch release contains various bug and security fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.10.1).
...
Fixed an issue where [Custom Authentication](/5.11/api-management/authentication/custom-auth) could fall back to a previously configured alternative authentication method if the custom plugin bundle was not loaded. Now this is treated as for any other failed plugin load, and requests to the API will be rejected with `HTTP 500 Internal Server Error` to prevent access to an improperly configured endpoint.
</Accordion>
...
Added a new configuration option, [HttpServerOptions.MaxResponseBodySize](/5.11/tyk-oss-gateway/configuration/#http_server_optionsmax_response_body_size) to limit the maximum size of the response bodies processed during any response body transformations.  When the limit is exceeded, the Gateway returns `HTTP 500 Response Body Too Large` instead of attempting to process the oversized content.
</Accordion>
...
For more details, please see the dedicated [Multi Auth](/5.11/basic-config-and-security/security/authentication-authorization/multiple-auth) section.
...
For more details, please see the dedicated [JWT Auth](/5.11/api-management/authentication/jwt-authorization) section.
...
For more details, please see the [JWT Auth](/5.11/basic-config-and-security/security/authentication-authorization/json-web-tokens#signature-validation) section.
...
For more details, please see the dedicated [section](/5.11/configure/external-service).
...
Added a new `external_services` section in the [Gateway configuration](/5.11/configure/external-service) to provide centralized configuration for proxy settings and mTLS certificates when communicating with external services. This includes connections to persistent and temporal storage, OAuth 2.0 Authorization Servers, and webhook targets.
...
Introduced a proactive event system to warn administrators when mTLS certificates are approaching expiry. The Gateway now emits two new [API events](/5.11/api-management/gateway-events#api-events) to provide visibility into certificate status:
...
Fixed an issue where Tyk OAS mock response middleware failed to execute when internal API proxying was enabled. Mock responses configured in the target API are now correctly returned when a request is redirected to another API on the same Tyk Gateway instance via [internal looping](/5.11/advanced-configuration/transform-traffic/looping).
</Accordion>
...
This is a version bump to align with Dashboard v5.9.2, no changes have been implemented in this release. For further information, please see the release notes for Dashboard [v5.9.2](/5.11/developer-support/release-notes/dashboard#592-release-notes).
...
This release builds on the recent release of [Tyk 5.8.3](/5.11/developer-support/release-notes/gateway#583-release-notes), adding a collection of new capabilities. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.9.0).
...
We've added support for additional processors, inputs and outputs for [Tyk Streams event driven APIs](/5.11/api-management/event-driven-apis), extending the flexibility of this powerful feature.
...
A breaking change has been identified in Tyk 5.9.0 regarding [URL rewrite rules](/5.11/transform-traffic/url-rewriting). The `negate` field, which was optional in previous versions, is now mandatory in all URL rewrite rule configurations.

##### What Changed

In Tyk 5.8.2 and earlier, the `negate` field in [URL rewrite rules](/5.11/transform-traffic/url-rewriting) included an omitempty tag, making it optional in JSON. If not provided, it would default to false
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.8.11).
...
This patch release addresses a CVE present in 5.8.9 and fixes an issue where API keys remained active when set to inactive status. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.8.10).
...
This patch release contains various bug fixes and introduces enhanced DNS monitoring for MDCB deployments. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.8.9).
...
<Accordion title='Resolved Panic Triggered by DRL Updates in Mixed Rate-Limiter Environments'>
Fixed an issue where Gateways using [Redis Rate Limiters](/5.11/api-management/rate-limit#redis-rate-limiter) would crash when sharing Redis with Gateways using [Distributed Rate Limiting](/5.11/api-management/rate-limit#distributed-rate-limiter) (DRL). Non-DRL Gateways now properly ignore DRL update messages instead of attempting to process them, enabling mixed rate-limiter deployments across shared Redis instances.
</Accordion>
...
This patch release fixes some high-priority CVEs. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.8.8).
...
Gateway 5.8.5 was version bumped only to align with Dashboard 5.8.5. Subsequently, no changes were encountered in release 5.8.5. For further information, please see the release notes for Dashboard [v5.8.5](/5.11/developer-support/release-notes/dashboard#585-release-notes).
...
A breaking change has been identified in Tyk 5.8.3 regarding [URL rewrite rules](/5.11/transform-traffic/url-rewriting). The `negate` field, which was optional in previous versions, is now mandatory in all URL rewrite rule configurations.
...
Fixed support for `dns:///` protocol for load balancing when using [gRPC plugins](/5.11/api-management/plugins/rich-plugins#load-balancing-between-grpc-servers). Setting the new configuration option `TYK_GW_COPROCESSOPTIONS_GRPCROUNDROBINLOADBALANCING` to `true` will cause Tyk to balance the load between multiple gRPC servers; the default behavior (`false`) is to use a sticky connection to a single server.
</Accordion>
...
This patch release contains fixes to some bugs experienced when using MDCB and distributed data planes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.8.2) below.
...
Fixed an issue where an [enforced timeout](/5.11/planning-for-production/ensure-high-availability/enforced-timeouts) set for a specific API endpoint could be overruled by the configured [proxy_default_timeout](/5.11/tyk-oss-gateway/configuration#proxy_default_timeout). Now if an endpoint-level timeout is set then this will be honoured, regardless of any default timeout that is configured.
</Accordion>
...
With Tyk 5.8.0 we are delighted to unlock the power and flexibility of Tyk OAS for all users, with full feature parity with the legacy Tyk Classic API definition. We are also bringing other updates and improvements, delivering more control, flexibility, and performance. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.8.0) below.
...
This patch release contains a bug fix. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.7.3) below.
...
| Third Party Dependency                                       | Tested Versions        | Compatible Versions    | Comments | 
| :------------------------------------------------------------ | :---------------------- | :---------------------- | :-------- | 
| [Go](https://go.dev/dl/)                                     | 1.22  |  1.22  | [Go plugins](/5.11/api-management/plugins/golang) must be built using Go 1.22 | 
| [Redis](https://redis.io/download/)  | 6.2.x, 7.x  | 6.2.x, 7.x  | Used by Tyk Gateway | 
| [OpenAPI Specification](https://spec.openapis.org/oas/v3.0.3)| v3.0.x                 | v3.0.x                 | Supported by [Tyk OAS](/5.11/api-management/gateway-config-tyk-oas) |
...
This patch release contains a bug fix. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.7.2) below.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-vX.Y.0) below. */}
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.6.1) below.
...
[changelog](/5.11/#Changelog-v5.6.0) below.

##### Per endpoint Rate Limiting for clients

Building on the [per-endpoint upstream rate
limits](/5.11/api-management/rate-limit#api-level-rate-limiting) introduced in Tyk 5.5.0 we have
now added [per-endpoint client
rate limits](/5.11/api-management/rate-limit#key-level-rate-limiting). This new feature allows
...
rate limits](/5.11/api-management/rate-limit#key-level-rate-limiting). This new feature
...
| Third Party Dependency                                       | Tested Versions        | Compatible Versions    | Comments | 
| :------------------------------------------------------------ | :---------------------- | :---------------------- | :-------- | 
| [Go](https://go.dev/dl/)                                     | 1.21  |  1.21  | [Go plugins](/5.11/api-management/plugins/golang#) must be built using Go 1.21 | 
| [Redis](https://redis.io/download/)  | 6.2.x, 7.x  | 6.2.x, 7.x  | Used by Tyk Gateway | 
| [OpenAPI Specification](https://spec.openapis.org/oas/v3.0.3)| v3.0.x                 | v3.0.x                 | Supported by [Tyk OAS](/5.11/api-management/gateway-config-tyk-oas) |
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.5.1) below.
...
We have introduced two new options in the `http_server_options` [Gateway configuration](/5.11/tyk-oss-gateway/configuration#http_server_options) that will enforce prefix and/or suffix matching when Tyk performs checks on whether middleware or other logic should be applied to a request:
...
This release primarily focuses on bug fixes.
For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-vX.Y.0) below. */}
We are thrilled to introduce Tyk Gateway 5.5, bringing advanced rate-limiting capabilities, enhanced certificate authentication, and performance optimizations. For a comprehensive list of changes, please refer to the [changelog](/5.11/#Changelog-v5.5.0) below.
...
Simplify certificate management with support for root Certificate Authority (CA) certificates, enabling clients to authenticate using certificates signed by the [configured root CA](/5.11/basic-config-and-security/security/mutual-tls/client-mtls#faq).
...
| Third Party Dependency                                       | Tested Versions        | Compatible Versions    | Comments | 
| :------------------------------------------------------------ | :---------------------- | :---------------------- | :-------- | 
| [Go](https://go.dev/dl/)                                     | 1.19 (GQL), 1.21 (GW)  | 1.19 (GQL), 1.21 (GW)  | [Go plugins](/5.11/api-management/plugins/golang#) must be built using Go 1.21 | 
| [Redis](https://redis.io/download/)  | 6.2.x, 7.x  | 6.2.x, 7.x  | Used by Tyk Gateway | 
| [OpenAPI Specification](https://spec.openapis.org/oas/v3.0.3)| v3.0.x                 | v3.0.x                 | Supported by [Tyk OAS](/5.11/api-management/gateway-config-tyk-oas) |
...
We've introducing a [Rate Limit Smoothing](/5.11/api-management/rate-limit#rate-limit-smoothing) option for the spike arresting Redis Rate Limiter to give the upstream time to scale in response to increased request rates.
...
###### Action Required
Customers should clear their edge Redis instances of any potentially affected keys to maintain data consistency and ensure proper synchronization across their environments. Please refer to the item in the [fixed](/5.11/#fixed) section of the changelog for recommended actions.

##### Fixed Window Rate Limiter

Ideal for persistent connections with load-balanced gateways, the [Fixed Window Rate Limiter](/5.11/api-management/rate-limit#fixed-window-rate-limiter) algorithm mechanism ensures fair handling of requests by allowing only a predefined number to pass per rate limit window. It uses a simple shared counter in Redis so requests do not need to be evenly balanced across the gateways.

##### Event handling with Tyk OAS

We’ve added support for you to [register webhooks](/5.11/api-management/gateway-events#event-handling-with-webhooks) with your Tyk OAS APIs so that you can handle events triggered by the Gateway, including circuit breaker and quota expiry. You can also assign webhooks to be fired when using the new [smoothing rate limiter](/5.11/api-management/rate-limit#rate-limit-smoothing) to notify your systems of ongoing traffic spikes.
...
This patch release contains bug fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.3.12) below.
...
In this release, we upgraded the Golang version to `v1.23` for security enhancement and fixed an API authentication issue with redirects. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.3.10) below.
...
| Third Party Dependency                                        | Tested Versions       | Compatible Versions   | Comments                                                                                   |
| :------------------------------------------------------------- | :--------------------- | :--------------------- | :------------------------------------------------------------------------------------------ |
| [Go](https://go.dev/dl/)                                      |  1.23 (GW)            |  1.23 (GW)            | [Go plugins](/5.11/api-management/plugins/golang) must be built using Go 1.23 |
| [Redis](https://redis.io/download/)                           | 6.2.x, 7.x            | 6.2.x, 7.x            | Used by Tyk Gateway                                                                        |
| [OpenAPI Specification](https://spec.openapis.org/oas/v3.0.3) | v3.0.x                | v3.0.x                | Supported by [Tyk OAS](/5.11/api-management/gateway-config-tyk-oas)           |
...
| Third Party Dependency                                        | Tested Versions       | Compatible Versions   | Comments                                                                                   |
| :------------------------------------------------------------- | :--------------------- | :--------------------- | :------------------------------------------------------------------------------------------ |
| [Go](https://go.dev/dl/)                                      |  1.22 (GW)            |  1.22 (GW)            | [Go plugins](/5.11/api-management/plugins/golang#) must be built using Go 1.22 |
| [Redis](https://redis.io/download/)                           | 6.2.x, 7.x            | 6.2.x, 7.x            | Used by Tyk Gateway                                                                        |
| [OpenAPI Specification](https://spec.openapis.org/oas/v3.0.3) | v3.0.x                | v3.0.x                | Supported by [Tyk OAS](/5.11/api-management/gateway-config-tyk-oas)           |
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.3.7) below.
...
When upgrading to 5.3.7 please follow the [detailed upgrade instructions](/5.11/#upgrading-tyk).
...
[changelog](/5.11/#Changelog-v5.3.6) below.
...
If moving from an version of Tyk older than 5.3.0 please read the explanation provided with [5.3.0 release](/5.11/#TykOAS-v5.3.0).
...
We have resolved an issue where custom [response plugins](/5.11/api-management/plugins/plugin-types#response-plugins) were not being
...
configuration](/5.11/tyk-oss-gateway/configuration#http_server_options) that will enforce prefix and/or suffix matching
...
Permissions](/5.11/api-management/policies#secure-your-apis-by-method-and-path) in access policies and keys that led to authorization
...
[v5.3.4](/5.11/developer-support/release-notes/dashboard#530-release-notes)
...
This release primarily focuses on bug fixes. For a comprehensive list of changes, please refer to the detailed
[changelog](/5.11/#Changelog-v5.3.3) below.
...
FIPS support](/5.11/developer-support/release-types/fips-release).
...
Added [FIPS compliance](/5.11/developer-support/release-types/fips-release) for Tyk Gateway.
</Expandable>
...
This release primarily focuses on bug fixes. For a comprehensive list of changes, please refer to the detailed
[changelog](/5.11/#Changelog-v5.3.2) below.
...
This release primarily focuses on bug fixes. For a comprehensive list of changes, please refer to the detailed
[changelog](/5.11/#Changelog-v5.3.1) below.
...
For licensed deployments (Tyk Cloud, Self Managed including MDCB), please refer to the [release notes of Tyk Dashboard 5.3.0](/5.11/developer-support/release-notes/dashboard#530-release-notes).
...
- **Not Backwards Compatible**
  ...
  - This means that if you wish to downgrade or revert to your previous version of Tyk, you will need to restore these
    API definitions from a backup. Please go to the [backup](/5.11/#upgrade-instructions) section for detailed
...
**Important:** Please go to the [backup](/5.11/#upgrade-instructions) section for detailed instructions on
...
the image](/5.11/api-management/plugins/rich-plugins#install-the-python-development-packages)
...
If you are upgrading to 5.3.0, please follow the detailed [upgrade instructions](/5.11/#upgrading-tyk).

**The following steps are essential to follow before upgrading** Tyk Cloud (including Hybrid Gateways) and Self Managed
users - Please refer to the [release notes of Tyk Dashboard 5.3.0](/5.11/developer-support/release-notes/dashboard#530-release-notes).

For OSS deployments -

1. Backup Your environment using the [usual guidance](/5.11/developer-support/upgrading) documented with every release (this includes
...
3. Performing the upgrade - follow the instructions in the [upgrade
   guide](/5.11/developer-support/upgrading) when upgrading Tyk.
...
[changelog](/5.11/#Changelog-v5.3.0) below.

##### Tyk OAS Feature Maturity

Tyk OAS is now out of [Early
Access](/5.11/developer-support/release-types/early-access-feature) as we have reached feature maturity.
...
the [documentation](/5.11/tyk-configuration-reference/kv-store) of this powerful feature.
...
key](/5.11/tyk-stack/tyk-developer-portal/enterprise-developer-portal/api-access/configuring-custom-rate-limit-keys)
...
Rate Limiter](/5.11/api-management/rate-limit#redis-rate-limiter)). Previously the complete
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.2.5) below.
...
Added new option for Tyk to use the default version of an API if the requested version does not exist. This is referred to as falling back to default and is enabled using a [configuration](/5.11/api-management/gateway-config-tyk-oas#versioning) flag in the API definition; for Tyk OAS APIs the flag is `fallbackToDefault`, for Tyk Classic APIs it is `fallback_to_default`.
</Accordion>
...
Fixed an issue where [enforced timeouts](/5.11/planning-for-production/ensure-high-availability/enforced-timeouts) values were incorrect on a per-request basis. Since we enforced timeouts only at the transport level and created the transport only once within the value set by [max_conn_time](/5.11/tyk-oss-gateway/configuration#max_conn_time), the timeout in effect was not deterministic. Timeouts larger than 0 seconds are now enforced for each request.
</Accordion>
...
The [Logstash](/5.11/api-management/logs-metrics#logstash) formatter timestamp is now in [RFC3339Nano](https://www.rfc-editor.org/rfc/rfc3339) format.
</Accordion>
...
Fixed a performance issue encountered when Tyk Gateway retrieves a key via MDCB for a JWT API. The token is now validated against [JWKS or the public key](/5.11/basic-config-and-security/security/authentication-authorization/json-web-tokens) in the API Definition.
</Accordion>
...
- `resource_sync.retry_attempts` - defines the number of [retries](/5.11/tyk-oss-gateway/configuration#resource_syncretry_attempts) that the Gateway should perform during a resource sync (APIs or policies), defaulting to zero which means no retries are attempted
- `resource_sync.interval` - setting the [fixed interval](/5.11/tyk-oss-gateway/configuration#resource_syncinterval) between retry attempts (in seconds)
</Accordion>
...
Resolved a bug with HTTP GraphQL APIs where, when the [Persist GraphQL middleware](/5.11/api-management/graphql#persisting-graphql-queries) was used in combination with [Response Body Transform](/5.11/api-management/traffic-transformation/response-body), the response's body transformation was not being executed.
...
We're thrilled to bring you some exciting enhancements and crucial fixes to improve your experience with Tyk Gateway. For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.2.0) below.

##### Added Body Transform Middleware to Tyk OAS API Definition

With this release, we are adding the much requested *Body Transformations* to *Tyk OAS API Definition*. You can now [configure](/5.11/api-management/gateway-config-tyk-oas#transformbody) middleware for both [request](/5.11/api-management/traffic-transformation/request-body) and [response](/5.11/api-management/traffic-transformation/response-body) body transformations and - as a Tyk Dashboard user - you’ll be able to do so from within our simple and elegant API Designer tool.
...
We’ve added the ability to [configure](/5.11/api-management/response-caching#configuring-the-middleware-in-the-tyk-oas-api-definition) per-endpoint timeouts for Tyk’s response cache, giving you increased flexibility to tailor your APIs to your upstream services.

##### Added Header Management in Universal Data Graph

With this release we are adding a concept of [header management](/5.11/api-management/data-graph#header-management) in *Universal Data Graph*. With multiple upstream data sources, data graphs need to be sending the right headers upstream, so that our users can effectively track the usage and be able to enforce security rules at each stage. All *Universal Data Graph* headers now have access to *request context* variables like *JWT claims*, *IP address* of the connecting client or *request ID*. This provides extensive configurability of customizable information that can be sent upstream.

##### Added Further Support For GraphQL WebSocket Protocols

Support for [WebSocket](/5.11/api-management/graphql#graphql-websockets) protocols between client and the *Gateway* has also been expanded. Instead of only supporting the *graphql-ws protocol*, which is becoming deprecated, we now also support [graphql-transport-ws](https://github.com/enisdenjo/graphql-ws/blob/master/PROTOCOL.md) by setting the *Sec-WebSocket-Protocol* header to *graphql-transport-ws*.

##### Added OpenTelemetry Tracing

In this version, we're introducing the support for *OpenTelemetry Tracing*, the new [open standard](https://opentelemetry.io/) for exposing observability data. This addition gives you improved visibility into how API requests are processed, with no additional license required. It is designed to help you with monitoring and troubleshooting APIs, identify bottlenecks, latency issues and errors in your API calls. For detailed information and guidance, you can check out our [OpenTelemetry Tracing](/5.11/api-management/logs-metrics#opentelemetry) resource.
...
*OpenTelemetry* functionality is also available in [Go Plugins](/5.11/api-management/plugins/advance-config#instrumenting-plugins-with-opentelemetry). Developers can write code to add the ability to preview *OpenTelemetry* trace attributes, error status codes etc., for their Go Plugins.

We offer support for integrating *OpenTelemetry* traces with supported open source tools such [Jaeger](/5.11/api-management/logs-metrics#using-docker), [Dynatrace](/5.11/api-management/logs-metrics#dynatrace) or [New Relic](/5.11/api-management/logs-metrics#new-relic). This allows API owners and developers to gain troubleshooting and performance insights from error logs, response times etc.
...
Added support for [configuring](/5.11/tyk-oss-gateway/configuration#opentelemetry) distributed tracing behavior of *Tyk Gateway*. This includes enabling tracing, configuring exporter types, setting the URL of the tracing backend to which data is to be sent, customizing headers, and specifying enhanced connectivity for *HTTP*, *HTTPS* and *gRPC*. Subsequently, users have precise control over tracing behavior in *Tyk Gateway*.
</Accordion>

<Accordion title='Added support for configuring OpenTelemetry'>
Added support to configure *OpenTelemetry* [sampling types and rates](/5.11/tyk-oss-gateway/configuration#opentelemetrysampling) in the *Tyk Gateway*. This allows users to manage the need for collected detailed tracing information against performance and resource usage requirements.
</Accordion>
...
Added a new feature that allows clients to retrieve the trace ID from response headers. This feature is available when *OpenTelemetry* is [enabled](/5.11/tyk-oss-gateway/configuration#opentelemetryenabled) and simplifies debugging API requests, empowering users to seamlessly correlate and analyze data for a specific trace in any *OpenTelemetry* backend like [Jaeger](https://www.jaegertracing.io/).
</Accordion>

<Accordion title='Allow detailed tracing to be enabled/disabled at API level'>
Added configuration parameter to enable/disable [detailed_tracing](/5.11/api-management/logs-metrics#capturing-detailed-logs) for *Tyk Classic API*.
</Accordion>

<Accordion title='Add OpenTelemetry support for GraphQL'>
Added *OpenTelemetry* support for GraphQL. This is activated by setting [opentelemetry.enabled](/5.11/tyk-oss-gateway/configuration#opentelemetryenabled) to *true*. This integration enhances observability by enabling GQL traces in any OpenTelemetry backend, like [Jaeger](https://www.jaegertracing.io/), granting users comprehensive insights into the execution process, such as request times.
</Accordion>

<Accordion title='Add support for configuring granular control over cache timeouts at the endpoint level'>
Added a new [timeout option](/5.11/api-management/response-caching#configuring-the-middleware-in-the-tyk-oas-api-definition), offering granular control over cache timeout at the endpoint level.
</Accordion>

<Accordion title='Enable request context variables in UDG global or data source headers'>
Added support for using [request context variables](/5.11/api-management/traffic-transformation/request-context-variables) in *UDG* global or data source headers. This feature enables much more advanced [header management](/5.11/api-management/data-graph#header-management) for UDG and allows users to extract header information from an incoming request and pass it to upstream data sources.
</Accordion>

<Accordion title='Add support for configuration of global headers for any UDG'>
Added support for configuration of [global headers](/5.11/api-management/data-graph#header-management) for any *UDG*. These headers will be forwarded to all data sources by default, enhancing control over data flow.
</Accordion>
...
#### Early Access Features:
Please note that the `Tyk OAS APIs` feature, currently marked as *Early Access*, is subject to breaking changes in subsequent releases. Please refer to our [Early Access guide](/5.11/developer-support/release-types/early-access-feature) for specific details. Upgrading to a new version may introduce changes that are not backward-compatible. Downgrading to a previous version after upgrading may result in a broken installation.
...
description of how to use these powerful [features](/5.11/api-management/traffic-transformation/request-size-limits).
...
Go to the [Upgrading Tyk](/5.11/developer-support/release-notes/gateway#upgrading-tyk)
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.11/#Changelog-v5.0.15) below.
...
ensure proper synchronization across their environments. Please refer to the item in the [fixed](/5.11/#fixed) section of the
...
This release primarily focuses on bug fixes. For a comprehensive list of changes, please refer to the detailed
[changelog](/5.11/#Changelog-v5.0.2) below.
...
This release primarily focuses on bug fixes. For a comprehensive list of changes, please refer to the detailed
[changelog](/5.11/#Changelog-v5.0.1) below.
...
Tyk’s OSS users can now make use of extensive [custom middleware](/5.11/api-management/plugins/overview) options with your OAS
...
[Versioning your Tyk OAS APIs](/5.11/api-management/api-versioning) is easier than ever, with the
...
Mock Responses](/5.11/api-management/traffic-transformation/mock-response), with the Tyk OAS API
...
This can be achieved by configuring the new External OAuth authentication mechanism. Find out more here [External OAuth Integration](/5.11/api-management/client-authentication#integrate-with-external-authorization-server-deprecated)
...
2. Support for allow list in field-based permissions - so far Tyk was offering field-based permissions as a “block list” only. That meant that any new field/query added to a graph was by default accessible for all consumers until API manager explicitly blocked it on key/policy level. Adding support for “allow list” gives API managers much more control over changing schemas and reduces the risk of unintentionally exposing part of the graph that are not ready for usage. See [Introspection](/5.11/api-management/graphql#introspection) for more details.
...
- Added a way to modify Tyk analytics record via Go plugins [configurable with API definition](/5.11/api-management/plugins/plugin-types#analytics-plugins). Can be used to sanitise analytics data. 
- Added new policy API REST endpoints
- Added option to configure certificates for Tyk Gateway using [environment variable](/5.11/tyk-oss-gateway/configuration#http_server_optionscertificates)
...
Find out more about [plugins](/5.11/api-management/plugins/overview#) and how to write [Go response plugins](/5.11/api-management/plugins/golang#creating-a-custom-response-plugin).
...
If you are upgrading from version 2.8, please [read this guide](/5.11/developer-support/release-notes/archived#290-release-notes)
