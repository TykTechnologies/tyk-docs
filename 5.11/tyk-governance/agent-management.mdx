...
4. **Deploy the Agent**
    ...
    Run the agent using Docker or as a Kubernetes Deployment. See [Installation](/5.11/tyk-governance/installation#deploy-the-agent) for details.
...
6. **Trigger Initial Synchronization**
    ...
    Trigger a [synchronization](/5.11/#synchronization-process) to verify the agent is working correctly.
...
2. **Scheduled Sync**: At regular intervals configured in the agent. See [Understanding Scheduled Synchronization](/5.11/#understanding-scheduled-synchronization).
