...
For those who need more deployment flexibility, Tyk Cloud also supports a [Hybrid Gateway](/5.11/tyk-cloud/environments-deployments/hybrid-gateways) option. In this setup, the control plane remains hosted and managed by Tyk on AWS, while the gateways can be deployed on your preferred cloud provider or on-premises environment—allowing you to meet data locality and compliance needs without sacrificing control.
...
For a list of compatible operating systems and versions, please refer to the [Linux documentation](/5.11/tyk-self-managed/install/linux#compatible-operating-systems).
...
5. Run ansible-playbook to install the following:
    ...
    <Note>
    For a production environment, we recommend that the Gateway, Dashboard and Pump are installed on separate machines. If installing multiple Gateways, you should install each on a separate machine. See [Planning for Production](/5.11/planning-for-production) For more details.
    </Note>
...
The [Dashboard configuration](/5.11/tyk-dashboard/configuration) can be changed by either editing the `tyk_analytics.conf` file or injecting them as [environment variables](/5.11/tyk-oss-gateway/configuration) via `heroku config`. In this guide we'll use the latter for simplicity of demonstration but there is merit to both methods.
...
All these files serve the same purpose as with the Dasboard and the Pump. [Configuration](/5.11/tyk-oss-gateway/configuration) can either be edited in `tyk.conf` or [injected](/5.11/tyk-oss-gateway/configuration) with `heroku config`.
...
You're ready to follow the guide on [creating and managing your APIs](/5.11/api-management/gateway-config-managing-classic#create-an-api) with this Heroku deployment.

<Note>
To use the [geographic log distribution](/5.11/api-management/dashboard-configuration#activity-by-location) feature in the Dashboard please supply the GeoLite2 DB in the `gateway` directory, uncomment the marked line in `Dockerfile.web` and set the `analytics_config.enable_geo_ip` setting (or `TYK_GW_ANALYTICSCONFIG_ENABLEGEOIP` env var) to `true`.
</Note>
...
In order to enable [rich plugins](/5.11/api-management/plugins/rich-plugins#) for the Gateway, please set the following Heroku config option to either `python` or `lua` depending on the type of plugins used:
```bash
...
1. Via our [Ubuntu Setup](/5.11/tyk-self-managed/install#debian-ubuntu-install-gateway) on an installed Ubuntu Server on Azure.
2. Via our [Docker Installation](/5.11/deployment-and-operations/tyk-self-managed/tyk-demos-and-pocs/overview#docker-compose-setup) using Azure's Docker support.
...
1. Via our [Ubuntu Setup](/5.11/tyk-self-managed/install#debian-ubuntu-install-gateway) on an installed Ubuntu Server within Google Cloud.
2. Via our [Docker Installation](/5.11/deployment-and-operations/tyk-self-managed/tyk-demos-and-pocs/overview#docker-compose-setup) using Google Cloud's Docker support.
...
2. Update the Tyk Gateway [configuration](/5.11/tyk-oss-gateway/configuration#analytics_configstorage_expiration_time) to keep the stats for 3 mins to allow Tyk Pump to process them. This value should be greater than the Pump [purge delay](/5.11/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#purge_delay) to ensure the analytics data exists long enough in Redis to be processed by the Pump. 
