...
- [Granular access control](/api-management/access-control/sessions-and-keys/access-rights#granular-endpoint-access)
...
The [TYK_GW_HTTPSERVEROPTIONS_ENABLESTRICTROUTES](/tyk-oss-gateway/configuration#http_server_options-enable_strict_routes) option in the Tyk Gateway configuration is provided to avoid nearest-neighbour requests on overlapping routes. If this is set to `true` then Tyk will perform an exact match of the request against the configured *listen path* including the trailing `/` used to mark the end of the *listen path*. For example:
