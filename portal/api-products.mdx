...
In the Tyk Developer Portal, Products serve as the primary unit of discovery, access control, and [monetization](/portal/overview/intro#api-monetization-with-tyk-developer-portal) of your Product portfolio.

Since Tyk 5.7.0, you have been able to create Products to publish your [Tyk Streams APIs](/api-management/event-driven-apis) on the Developer Portal.
...
- When you [synchronize](/portal/api-provider#synchronizing-developer-portal-with-providers) the Developer Portal with the Provider, a new Product will be created automatically
...
Refer to [JWT Signature Validation](/api-management/authentication/jwt-signature-validation) for API configuration details.
...
3. Select one or more [Client Profiles](/tyk-stack/tyk-developer-portal/enterprise-developer-portal/api-access/dynamic-client-registration#client-profiles) that describe the methods API Consumers will use to obtain access tokens from the IdP.
4. Select **Save Changes** and then publish the Product to a Catalog to make it available to API Consumers.

<Warning>
The scope must [exist in your Identity Provider](/tyk-stack/tyk-developer-portal/enterprise-developer-portal/api-access/dynamic-client-registration#define-oauth-2-0-scopes) when the Product is published, so that the IdP is able to issue tokens containing the scope.
</Warning>
...
- **Format**: [Markdown](/portal/customization/markdown) or HTML content with multiple pages
...
For full details, step-by-step creation instructions, and known limitations, see [Documentation-Only Products](/portal/api-products/documentation-only-products).
