...
Tyk Developer Portal includes an integrated GraphQL Playground for exploring and testing GraphQL queries directly within the portal.
...
As shown here, developers can view the schema and explore the GraphQL API using the interactive playground.

<Note>
GraphQL Playground is available from Developer Portal version 1.15.0 and above.
</Note>

### When is it Available?
Developers will be able to access the GraphQL Playground from an API Product in the Catalogue if:
...
- **Format**: Upload API specification files in multiple formats:
    ...
    - *((Optional))* GraphQL Server URL: Enter the runtime GraphQL endpoint to enable live query support in the GraphQL Playground
