...
<Warning>
The scope must [exist in your Identity Provider](/tyk-stack/tyk-developer-portal/enterprise-developer-portal/api-access/dynamic-client-registration#define-oauth-2-0-scopes) when the API Plan is published, so that the IdP is able to issue tokens containing the scope.
</Warning>
...
- **Behavior**: This configuration is required when the Plan is used with an API Product for which the [Dynamic Client Registration](/tyk-stack/tyk-developer-portal/enterprise-developer-portal/api-access/dynamic-client-registration) flow has been activated
    ...
    - When a JWT presenting the Plan's scope is received by Tyk Gateway, the Gateway [applies the Plan to the session](/basic-config-and-security/security/authentication-authorization/json-web-tokens).
