...
When a synchronization event occurs, for each Provider in turn, the Portal will retrieve all access and limit policies that have matching tags with those configured in the [Provider settings](/api-provider#policy-tags).
...
- Verify policies have the correct [policy tags](/api-provider#policy-tags)
