...
On a Product's detail page, the tab previously labelled **API specification** is now called **Service Explorer**. What it renders depends on the Service's type:
...
There is no interactive tool testing on this page. Executing an MCP proxy's tools from the Portal is covered by the embedded MCP Inspector, described separately.
