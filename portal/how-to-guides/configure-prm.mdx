...
1. **Enable the Identity Provider Registry**
   ...
   If not already enabled, set [`TYK_PORTAL_ENABLEIDPREGISTRY=true`](/product-stack/tyk-enterprise-developer-portal/deploy/configuration#tyk_portal_enableidpregistry) and restart the Portal. See [Identity Provider Registry](/api-management/client-idp-registry) for why this is the recommended approach.
...
- [Dynamic Client Registration](/tyk-stack/tyk-developer-portal/enterprise-developer-portal/api-access/dynamic-client-registration) - full DCR setup reference, including provider-specific instructions for Keycloak, Okta, Auth0, Curity, Descope, and Gluu
