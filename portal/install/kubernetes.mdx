...
To omit a block entirely so that the cluster assigns its own IDs, set `enabled: false` on it. See [Deploy Tyk on OpenShift](/deployment-and-operations/openshift).
</Note>

The bootstrap job has its own independent security context, configured under `bootstrapJob.securityContext` and `bootstrapJob.containerSecurityContext`.
...
**`valueFrom` is available from Tyk Helm Chart v5.4.0.**
