...
To omit a block entirely so that the cluster assigns its own IDs, set `enabled: false` on it. See [Deploy Tyk on OpenShift](/tyk-self-managed/install/openshift).
</Note>
