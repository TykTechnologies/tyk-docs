...
When a consumer-admin deletes a team-member or another admin within an organization, the user is not actually deleted, but transferred to the `default_org`. The deleted user's apps are reassigned to the admin performing the deletion.
