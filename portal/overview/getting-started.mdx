...
When you configure a Provider, you're establishing a secure API-to-API connection between the Developer Portal and the Tyk Dashboard. The Portal uses the Dashboard's REST API to:

- **Query policies**: `GET /api/portal/policies` - Retrieves security policies that define access rights and rate limits
- **Create credentials**: `POST /api/keys` - Generates API keys when consumers request access
- **Manage policies**: `POST/PUT /api/portal/policies` - Creates or updates policies for new Products and Plans

The credentials you provide (Secret and Organization ID) are sent as HTTP headers (`Authorization` and `x-tyk-authorization`) with each API request to authenticate the Portal.
</Note>
...
When you click **Synchronize**, the following API interactions occur between the Portal and Dashboard:

1. **Policy Discovery**: The Portal calls `GET /api/portal/policies` on the Dashboard to retrieve all security policies matching the configured policy tags
2. **Policy Analysis**: Each policy is analyzed to determine if it's an "access" policy (for API Products) or a "rate limit" policy (for Subscription Plans)
3. **Local Mapping**: The Portal creates or updates its internal Products and Plans to match the retrieved policies
4. **Bidirectional Updates**: If Products or Plans were created in the Portal, their corresponding policies are pushed to the Dashboard via `POST /api/portal/policies`

This synchronization runs automatically based on the configured `PORTAL_REFRESHINTERVAL` (default: 10 minutes), ensuring the Portal stays in sync with any policy changes made directly in the Dashboard.
</Note>
