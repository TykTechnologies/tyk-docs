...
### Access Flow Types
The Developer Portal supports two different access flows for API Products. This is configured from the Admin Portal by API Owners:
...
A streamlined single-product access experience without a shopping cart interface.

##### Key Features
- Immediate access requests without cart management
- Credential reuse and compatibility checking
- Simplified user experience with fewer steps

#### Cart-Based Flow
A traditional multi-product shopping cart experience allowing users to bundle multiple API products into a single request.

##### Key Features
- Multi-product selection and bundling
- Cart review and modification capabilities
- Single checkout process for multiple products
