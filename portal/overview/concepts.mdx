...
Access credentials are the security tokens that allow API Consumers to authenticate with your APIs. These depend upon the configuration of those APIs in the API definition managed by Tyk Dashboard.

The Enterprise Developer Portal supports the following **Standalone Authentication Types**:
- **Standard Auth Token** (`authToken` / API keys)
- **JSON Web Token** (`jwt`)
- **Basic Auth** (`basic`)
- **Custom Auth** (`custom`)

The portal also supports specific **Multi-Auth Combinations**. For `multiAuth`, Mutual TLS (`mutualTLS`) is mandatory and can only be combined with:
- **Auth Token + mTLS**
- **JWT + mTLS**

*Note on OAuth 2.0 / OIDC:* The portal supports OAuth via **Dynamic Client Registration (DCR)**. In this setup, the portal acts as a client registry that integrates with a third-party Identity Provider (IdP) to provision OAuth credentials for the developer's app, rather than acting as the OAuth provider itself.
...
The Developer Portal leverages Tyk's [partitioned policies](/api-management/access-control/policies/applying-policies#partitioned-policies) in two distinct ways:
