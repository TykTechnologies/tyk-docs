...
Access credentials are the security tokens that allow API Consumers to authenticate with your APIs. These depend upon the configuration of those APIs in the API definition managed by Tyk Dashboard. For the full list of supported authentication types and credential management modes, see [Developer Portal Authentication Support](/portal/portal-auth-types).
