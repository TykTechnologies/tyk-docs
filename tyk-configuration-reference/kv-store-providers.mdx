...
description: "Configuration reference for each secret store type supported by Tyk's Secrets Management system."
...
<Note>
Tyk Gateway automatically registers a built-in store named `env` - in addition to any store you register yourself - configured with `prefix: "TYK_SECRET_"` and `uppercase: true`. This preserves the behavior of legacy `env://` references. A reference `kv://env/MY_VAR` (or `$kv{env:MY_VAR}`) therefore reads the environment variable `TYK_SECRET_MY_VAR`. See [Legacy Key-Value Syntax](/tyk-configuration-reference/kv-store-legacy) for how this differs from the legacy `env://` notation.
</Note>
...
| `http_auth.username` / `http_auth.password` | HTTP Basic auth credentials, for a Consul API sitting behind a proxy that requires it - not Consul's own access control. |
...
| `tls_config.insecure_skip_verify` | Disables server certificate verification. Test clusters only - never production. |
...
| `version_stage` | Read the secret version carrying this staging label. Cannot combine with `version_id`; any label other than `AWSCURRENT` makes the store read-only. |
