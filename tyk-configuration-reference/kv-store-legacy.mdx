...
Before [Secrets Management](/tyk-configuration-reference/kv-store)'s `kv://` and `$kv{}` notation, Tyk Gateway used a set of URL-scheme prefixes and `$secret_*` labels to read external values into `tyk.conf` and API definitions. This notation is **exclusive to Tyk Gateway** - it was never available in Tyk Dashboard, Tyk Portal or Tyk MDCB, which only support the newer syntax.

It continues to work alongside the new approach - it can be freely mixed with the newer syntax in the same API definition or the same running Gateway. This page is a reference for that existing behavior; if you're setting up secrets for the first time, use [Secrets Management](/tyk-configuration-reference/kv-store) instead.
...
| Prefix | Reads From |
| :--- | :--- |
| `env://<VAR>` | The environment variable `<VAR>`, read exactly as named. |
...
| Label | Reads From |
| :--- | :--- |
| `$secret_env.<VAR>` | The environment variable `<VAR>`. |
...
<Note>
`env://` reads the environment variable name exactly as written. This differs from the newer [`kv://env/...`](/tyk-configuration-reference/kv-store-providers#environment-variables-env) notation, which adds a `TYK_SECRET_` prefix and uppercases the key. If you switch an environment-variable reference from the old notation to the new one, be careful to ensure you account for this difference, or the reference may resolve to nothing.
</Note>
