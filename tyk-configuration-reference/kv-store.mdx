...
With Tyk Gateway you can store configuration data (typically authentication secrets or upstream server locations) in Key-Value (KV) systems such as [Vault](/deployment-and-operations/tyk-self-managed/deployment-lifecycle/deployment-to-production/key-value-storage/vault), and [Consul](/deployment-and-operations/tyk-self-managed/deployment-lifecycle/deployment-to-production/key-value-storage/consul) and then reference these values during configuration of the Tyk Gateway or APIs deployed on the Gateway.
...
Storing local settings within the Tyk Gateway's configuration file allows you to have per instance variables, such as a machine ID, and inject these into API requests and responses using [transformation middleware](/advanced-configuration/transform-traffic).
...
The exception to this is for specific [transformation middleware](/tyk-configuration-reference/kv-store#transformation-middleware) where the value will be retrieved for each call to the API, during the processing of the API request or response.
...
- to retrieve the value assigned to a `KEY` in Consul you will use `consul://KEY` or `$secret_consul.KEY` notation depending on the [location](/tyk-configuration-reference/kv-store#how-to-access-the-externally-stored-data) of the reference
...
- to retrieve the value assigned to a `KEY` in Vault you will use `vault://KEY` or `$secret_vault.KEY` notation depending on the [location](/tyk-configuration-reference/kv-store#how-to-access-the-externally-stored-data) of the reference
...
- to retrieve the value assigned to a `KEY` in the `secrets` config you will use `secrets://KEY` or `$secret_conf.KEY` notation depending on the [location](/tyk-configuration-reference/kv-store#how-to-access-the-externally-stored-data) of the reference
...
- if you want to set the local "secrets" section (equivalent to the `secrets` section in the [Gateway config file](/tyk-configuration-reference/kv-store#tyk-gateway-config-file)) using environment variables, you should use the following notation: `TYK_GW_SECRETS=key:value,key2:value2`
...
- to retrieve the value assigned to an environment variable `VAR_NAME` you will use `env://VAR_NAME` or `$secret_env.VAR_NAME` notation depending on the [location](/tyk-configuration-reference/kv-store#how-to-access-the-externally-stored-data) of the reference
...
For example, if you create a Key-Value pair in [Vault](/deployment-and-operations/tyk-self-managed/deployment-lifecycle/deployment-to-production/key-value-storage/vault#how-key-value-data-is-stored-in-vault) with the *key* `shared-secret` in *secret* `gateway-dashboard` within directory `tyk-secrets/` then you could use the *Value* as the `node_secret` in your Gateway config by including the following in your `tyk.conf` file:
``` .json
{
...
From Tyk Gateway v5.3.0 onwards, you can store [any string field](/tyk-configuration-reference/kv-store#other-string-fields) from the API definition in any of the supported KV storage options; for earlier versions of Tyk Gateway only the [Target URL and Listen Path](/tyk-configuration-reference/kv-store#target-url-and-listen-path) fields and [certain transformation middleware](/tyk-configuration-reference/kv-store#transformation-middleware) configurations were supported. 
...
When the Gateway starts, Tyk will read the *Value* from the environment variable and use this as the [Target URL](/tyk-apis/tyk-gateway-api/oas/x-tyk-oas-doc#upstream).
...
- [request body transform](/transform-traffic/request-body)
- [request header transform](/transform-traffic/request-headers)
- [URL rewrite](/transform-traffic/url-rewriting)
- [response body transform](/advanced-configuration/transform-traffic/response-body)
- [response header transform](/advanced-configuration/transform-traffic/response-headers)
...
- Environment variables: `$secret_env.key` or `env://key` (see [here](/tyk-configuration-reference/kv-store#using-environment-variables-with-transformation-middleware))
...
When accessing KV references from the `/tyk-apis` directory on Consul or Vault, you should not provide the `path/to/` the key except for Target URL and Listen Path (as described [above](/tyk-configuration-reference/kv-store#target-url-and-listen-path)).
</Note>
...
When the Gateway starts, Tyk will read the *Value* from the `secrets` section in the Gateway config file and use this to identify the header where Tyk Gateway should look for the [Authentication](/tyk-apis/tyk-gateway-api/oas/x-tyk-oas-doc#authentication) token in requests to your Tyk OAS API.