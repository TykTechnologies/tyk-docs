...
```json
{
  "storage": {
    "main": {
      "type": "postgres",
      "write_connection_string": "user=root password=admin database=tyk-primary-db host=tyk-db port=5432",
      "read_connection_string": "user=root password=admin database=tyk-primary-db host=tyk-db-replica port=5432"
    },
    "logs": {
      "type": "postgres",
      "connection_string": "user=root password=admin database=tyk-logs-db host=tyk-db port=5432",
      "table_sharding": true
    }
  }
}
