...
- **Tyk Cloud**: Navigate to **Deployments** > your Control Plane. Your hybrid data plane should appear under **Hybrid data planes**.
- **Self-Managed**: Navigate to **System Management** > **Gateway Nodes** in your Tyk Dashboard.

<AccordionGroup>
  <Accordion title="Gateway not connecting to MDCB">
    ...
    Common causes:
    - Incorrect MDCB connection string, Organization ID, or API key
    - Outbound network/firewall blocking the connection to the MDCB endpoint
  </Accordion>
  <Accordion title="APIs not loading (404 responses)">
    1. Verify the API is published in the control plane Dashboard.
    2. If using Group IDs, ensure the API is assigned to this data plane's group.
    3. Check the gateway logs for sync errors:
