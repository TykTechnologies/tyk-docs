...
Telemetry in Tyk Cloud supports three types of observability data:

- **Distributed Traces**: Track how requests flow through your system, identify bottlenecks, and troubleshoot issues
- **Gateway Metrics**: Export RED (Rate, Error, Duration) metrics to monitor API performance and track error rates
- **Application Logs**: Stream application logs in real time to your chosen provider for monitoring and troubleshooting

Traces and metrics can be enabled independently for `Cloud Data Plane` deployments. Logs can be enabled for both `Control Plane` and `Cloud Data Plane` deployments.

Since this functionality is powered by Tyk Gateway's OpenTelemetry integration, we recommend reviewing our comprehensive [OpenTelemetry documentation](/api-management/traces#opentelemetry)
to understand the underlying architecture, best practices for implementation, and how to maximize the value of telemetry in your API infrastructure. The documentation provides detailed insights into configuration options, sampling strategies, and metric collection.
...
The telemetry export configuration provides three independent toggles:

- **Enable Traces**: Enable/disable distributed tracing
- **Enable Metrics**: Enable/disable gateway metrics export (available from Gateway v5.13.0)
- **Enable Logs**: Enable/disable application log streaming (logs are streamed in real time to your configured provider)

#### Default Metrics Exported

When metrics are enabled, Tyk exports the following RED (Rate, Error, Duration) metrics by default:

| Metric Dimension | Description | Example Values |
|-----------------|-------------|----------------|
| **HTTP Method** | The HTTP method used for the request | `GET`, `POST`, `PUT`, `DELETE` |
| **API ID** | The unique identifier of the API being called | `api-12345` |
| **HTTP Response Code** | The HTTP status code returned | `200`, `404`, `500` |

These metrics are exported using OpenTelemetry's cumulative temporality model, which tracks the total count of events over time.

##### Custom Metrics

<Note>
Metric customization (additional dimensions, filtering) is not self-service. Customers must submit a support request to Tyk at [support.tyk.io](https://support.tyk.io/).

Custom metric dimensions can significantly increase cardinality. This impacts both the gateway's memory usage and your observability platform's storage costs.
</Note>

#### Sampling Rate Override

Choose what percentage of trace data to export (default: 10%). This setting applies to traces only, not metrics or logs.
