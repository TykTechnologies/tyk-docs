...
- **Application Logs**: Stream gateway application logs in real time to your chosen provider for monitoring and troubleshooting
- **Gateway Metrics**: Export RED (Rate, Error, Duration) metrics to monitor API performance and track error rates

Each can be enabled independently for `Cloud Data Plane` deployments.
...
Gateway metrics can be exported to the same providers as traces and logs:
- Datadog
- Dynatrace
- New Relic
- Elastic
