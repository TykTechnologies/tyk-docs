...
Tyk Cloud supports the use of [Python](/api-management/plugins/rich-plugins#overview), [JavaScript Middleware](/api-management/plugins/javascript) and [Golang](/tyk-cloud/using-plugins) based plugins.
...
If you use a **Hybrid Data Plane** (a self-managed gateway that you deploy and connect to your Tyk Cloud Control Plane), use the Gateway's built-in [plugin bundle downloader](/api-management/plugins/overview#plugin-bundles) instead. You can either host your own bundle on your own web server or S3 bucket, or point the Hybrid Gateway directly at your Tyk Cloud Control Plane's File Server. See [Serving Plugin Bundles To Hybrid Gateways](#serving-plugin-bundles-to-hybrid-gateways) for the File Server option.
</Note>
...
1. A local Tyk Gateway installation to create your Python plugin bundle. We recommend [installing our Self-Managed version on Ubuntu Bionic 18.04](/tyk-self-managed/install/linux#install-tyk-on-debian-or-ubuntu).
...
If your plugin bundle includes rich plugins (Python, Lua, or gRPC), also set `"enable_coprocess": true` in the Hybrid Gateway's `tyk.conf`, as described in [Gateway configuration](/api-management/plugins/overview#gateway-configuration).
