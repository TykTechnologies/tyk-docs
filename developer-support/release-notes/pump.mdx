...
#### Release Highlights
In this release, we are introducing MCP (Model Context Protocol) analytics support, enabling MCP Gateway traffic to be captured and processed alongside HTTP API traffic across MongoDB, PostgreSQL, Elasticsearch, and Prometheus. 
For more details, see the [Managing MCP proxies documentation](/ai-management/mcp-gateway/managing-mcp-proxies).

We have also improved the stdout pump with cleaner, more readable JSON logs by properly formatting escaped characters in request and response fields. 
