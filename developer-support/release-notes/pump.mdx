...
**Pump crashes on startup due to default MongoDB configuration**

If you configure Tyk Pump using only environment variables without setting `TYK_PMP_OMITCONFIGFILE=true`, the Pump will crash on startup after a 10-second timeout.

This happens because Pump attempts to load its default configuration file, which includes a `mongo-pump` connecting to `localhost`. The connection error already existed in previous versions, but due to underlying OS and Golang 1.25 updates in this release, `localhost` now resolves to an IPv6 address (`::1`). The OS signal changed to actively refuse the connection, which now causes the Pump to crash.

**Workaround:** Set the environment variable [TYK_PMP_OMITCONFIGFILE](/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#omit_config_file) to `true` to completely ignore the default configuration file.
