...
This happens because Pump attempts to load its default configuration file, which includes a `mongo-pump` connecting to `localhost`. Due to underlying Golang 1.25 updates in this release, `localhost` now resolves to an IPv6 address (`::1`). In environments without IPv6 support (like AWS Fargate), this connection attempt hangs and times out, causing the crash.
