...
**mongo-pump crashing when using only environment variables**

In Tyk Pump 1.14.1, due to an upgrade to Go 1.25 and Debian 13, `localhost` now resolves to the IPv6 address `[::1]` instead of IPv4 `127.0.0.1`. If you are using only environment variables (e.g., in Fargate deployments), not using mongo-pump, and haven't set `TYK_PMP_OMITCONFIGFILE=true`, the `mongo-pump` is initialized from the default config. Because of the IPv6 resolution change, the connection is actively refused, and after a 10-second timeout, `mongo-pump` calls `log.Fatal`, crashing the process. (In 1.14.0, it resolved to 127.0.0.1 and didn't crash).

**Workaround/Fix**: Set the environment variable `TYK_PMP_OMITCONFIGFILE=true` to omit the default configuration file if you are using only environment variables.
