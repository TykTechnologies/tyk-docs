...
Tyk Pump 1.15.1 standardizes the format of its application logs so that output is consistent with other Tyk components and aligned with industry observability standards. A new `log_format` option introduces a `text` default and a `json` option, both using RFC3339 timestamps and a renamed `message` field, with a `legacy` option to preserve the previous output. This is a breaking change for tooling that depends on the old log format.

This patch also fixes pre-aggregated analytics not being written to sharded PostgreSQL tables, and addresses CVEs to strengthen security.

For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v1.15.1).
...
The [log_format](/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#log_format) configuration option (and the `TYK_PMP_LOGFORMAT` environment variable) now accepts three explicit values:
...
- `legacy`: preserves the previous behavior exactly, including the historical timestamp format and the `msg` field name.

**What changes under the new `text` and `json` formats**
 
- **Consistent RFC3339 timestamps.** All log entries now use the industry-standard RFC3339 timestamp format (for example, `2024-12-12T13:59:08Z`), replacing the previous default `logrus` timestamp format.
- **Log message field renamed.** The log message field is renamed from `msg` to `message`, for consistency with other Tyk components.

**Migration**

- To keep the previous behavior exactly, the historical timestamps and the `msg` field name, set `log_format` to `legacy` (or `TYK_PMP_LOGFORMAT=legacy`). The `legacy` option is the supported fallback for customers whose observability tooling depends on the previous Tyk Pump log output.
...
#### Downloads
- [Docker Image v1.15.1](https://hub.docker.com/r/tykio/tyk-pump-docker-pub/tags?page=&page_size=&ordering=&name=v1.15.1)
  - ```bash
    docker pull tykio/tyk-pump-docker-pub:v1.15.1
    ```
- Source code tarball for OSS - [GH Tyk Pump Repo](https://github.com/TykTechnologies/tyk-pump/releases/tag/v1.15.1)
...
Tyk Pump application logs now match the standardized format used across other Tyk components, using a consistent, RFC3339-compliant timestamp across all application log entries.

The [log_format](/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#log_format) configuration option (and the `TYK_PMP_LOGFORMAT` environment variable) now accepts three values:

- `text` (the new default, RFC3339 plain text)
- `json` (RFC3339 JSON)
- `legacy` (the previous behavior, including the historical timestamp format and the `msg` field name)

Under `text` and `json`, the log message field is also renamed from `msg` to `message` for consistency with other Tyk components.

This is a **breaking change**: customers whose monitoring or log-parsing tools rely on the previous timestamp format or the `msg` field name should set `log_format` to `legacy`. See the Breaking Changes section for details.
</Accordion>
