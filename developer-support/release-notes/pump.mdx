...
<Accordion title='Improve readability of stdout pump JSON logs'>
Enhanced the stdout pump to output cleaner, more readable JSON logs by default. Escaped characters such as newlines and carriage returns are now properly formatted in the raw request and response fields, making the logs easier to read and analyze. A new configuration option `use_legacy_payload_format` has been added to revert to the previous behavior if needed.
</Accordion>
