...
<Accordion title='Fixed: Pump crashed when using only environment variables without TYK_PMP_OMITCONFIGFILE=true'>
This release fixes a bug where Tyk Pump loaded its default configuration file even when configured using only environment variables. The default configuration file includes a `mongo-pump` entry connecting to `localhost`.

Due to the Golang 1.25 update in this release, `localhost` now resolves to an IPv6 address (`::1`). In environments without IPv6 support, this connection is actively refused, causing Pump to crash on startup after a 10-second timeout.

**Action required:** If you configure Tyk Pump using only environment variables, you must set [TYK_PMP_OMITCONFIGFILE](/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#omit_config_file) to `true` to prevent Pump from loading the default configuration file.
</Accordion>
