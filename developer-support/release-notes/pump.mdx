...
Tyk Pump 1.15.0 introduces full MCP analytics support, enabling you to store, query, and monitor Model Context Protocol traffic independently of your standard API analytics. New MCP-specific pump backends are available for MongoDB, PostgreSQL, Prometheus, and Elasticsearch, with dedicated aggregation support for hybrid deployments.
...
<Accordion title='MCP analytics storage across all supported backends'>
Tyk Pump now stores MCP-specific analytics records as a distinct record type, separate from regular API analytics. New MCP pump backends are available for MongoDB, PostgreSQL, Prometheus, and Elasticsearch, allowing you to query and report on MCP traffic independently of your existing API analytics.

To enable MCP analytics, configure an MCP pump backend in your Pump configuration alongside your existing pumps. MCP records include primitive name, primitive type (tool, resource, or prompt), and JSON-RPC method, giving you full visibility into how AI agents are consuming your MCP proxies.

When using the hybrid pump in a multi-data-centre deployment, set `EnableMCPAggregation: true` to route MCP aggregate data through a dedicated channel, keeping it separate from standard analytics aggregation.
</Accordion>

<Accordion title='MCP labels and filtering for Prometheus metrics'>
Three new labels are available for Prometheus metric configuration: `mcp_method`, `mcp_primitive_type`, and `mcp_primitive_name`. These are populated for MCP records and empty for non-MCP records, allowing you to slice existing metrics by MCP dimensions.

You can also set `mcp_only: true` on any custom metric to restrict it to MCP records only, keeping MCP traffic observations separate from standard API traffic in your dashboards.
</Accordion>
...
<Accordion title='Duplicate log entries for non-MCP API requests'>
Resolved an issue where enabling an MCP pump caused duplicate log entries to be written for requests made to standard (non-MCP) APIs. Each request now produces a single log entry regardless of which pump backends are configured.
</Accordion>
