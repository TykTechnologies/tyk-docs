...
#### Release Highlights
In this release, we have improved the stdout pump with cleaner, more readable JSON logs by properly formatting escaped characters in request and response fields. We have also introduced MCP analytics support, enabling Tyk Pump to store and process MCP Gateway traffic records across MongoDB, PostgreSQL, Elasticsearch, and Prometheus backends.
...
The stdout pump now outputs cleaner JSON logs by default: escaped newline (`\n`), tab (`\t`), and carriage return (`\r`) characters are no longer present in the `raw_request` and `raw_response` fields. While the output remains valid JSON, customers whose log-processing pipelines, dashboards, or alerting rules rely on the previous escaped-character format will need to either update their tooling to handle the new format or opt back into the previous behaviour.

To preserve the previous behaviour, set the following in your stdout pump configuration:

`use_legacy_payload_format: true`

When set to `true`, the stdout pump output is identical to previous releases. When unset or set to `false`, the new clean formatting is applied.
...
<Accordion title='Cleaner JSON formatting for stdout pump logs by default'>
The stdout pump now outputs cleaner, more readable JSON logs by default. Escaped newline, tab, and carriage return characters are no longer present in the `raw_request` and `raw_response` fields, making the logs significantly easier to read and analyse while remaining valid JSON. A new `use_legacy_payload_format` configuration option (default `false`) is available to revert to the previous escaped-character output for customers whose downstream tooling depends on the original format. This is a breaking change in default behaviour, see the `Breaking Changes` section for details.
</Accordion>
...
Tyk Pump 1.15.0 adds support for storing and processing MCP (Model Context Protocol) Gateway analytics records:
...
- **Hybrid pump aggregation**: A new `EnableMCPAggregation` flag on the hybrid pump enables pre-aggregation of MCP records before they are forwarded to the Control Plane, reducing data volume for high-traffic deployments.
...
- **Bug fix**: Resolved a duplicate log entry issue where MCP records were being written twice to analytics storage.
