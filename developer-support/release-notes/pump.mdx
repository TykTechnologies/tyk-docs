...
This release adds support for referencing secrets and configuration values in the Pump configuration file from an external secret store, using the same unified `kv://` / `$kv{}` syntax introduced across other Tyk components this wave — including the ability to reference secrets from within a pump's own `meta` block (for example, a backend's connection string or credentials). As part of this change, the legacy `PMP_MONGO*` environment variable format is now deprecated. This release also extends GCP IAM-based authentication for Redis/Valkey to Pump's storage connection, updates the Go toolchain to Go 1.26, and fixes two issues affecting the Kinesis pump. This release also addresses reported CVEs in dependent libraries.
