...
- Bootstrap jobs gain configurable `backoffLimit` and support for `imagePullSecrets`, for private image registries.
- The Developer Portal chart's `extraEnvs` now supports `valueFrom` in addition to a literal `value`, and its bootstrap job's image, image pull secrets, and security context are now configurable.
