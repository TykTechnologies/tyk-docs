...
You can now configure Kubernetes Pod Topology Spread Constraints directly through the Tyk Gateway Helm Chart, allowing you to control how Gateway pods are distributed across availability zones or topology domains. This enables more resilient, highly available deployments and helps prevent all Gateway replicas from being scheduled in the same zone or node group. Existing users are unaffected, as the feature defaults to disabled unless explicitly configured.
</Accordion>
