...
<Accordion title='Renewed Default TLS Certificate with Backward-Compatible Transition'>
The default self-signed TLS certificate shipped with the Tyk Gateway Helm chart has been renewed. To ensure a smooth transition without breaking existing deployments, we have introduced a new configuration option `tyk-gateway.gateway.tls.useNewDefaultCertificate`. When enabled, the Gateway will serve both the legacy and the new certificate simultaneously, allowing clients to continue connecting while their trust stores are updated.

To enable the new certificate alongside the legacy one:
```yaml
tyk-gateway:
  gateway:
    tls:
...
This change only affects users who rely on the default Tyk certificate (`useDefaultTykCertificate: true`). Users who provide their own certificates via `secretName` are not impacted.
</Accordion>
