...
In this patch release, we've improved visibility into your data planes by including the specific APIs and Policies loaded by each gateway in the MDCB `/dataplanes` endpoint. For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v2.9.0).
...
Gateway is updated to support the enhanced MDCB `/dataplanes` endpoint. The enhanced MDCB endpoint will provide visibility into your Tyk deployment. This now returns a detailed list of the APIs and policies loaded on each connected data plane gateway. You can retrieve the results for a specific Gateway using the `node_id` query parameter. This update provides a clear picture of what is running on each Gateway, simplifying the monitoring and troubleshooting of your deployed Data Planes.
</Accordion>
