...
This release adds support for referencing secrets and configuration values in the MDCB configuration file from an external secret store, using the same unified `kv://` / `$kv{}` syntax introduced across other Tyk components this wave. This release also extends GCP IAM-based authentication for Redis/Valkey to MDCB's storage connections, and updates the Go toolchain to Go 1.26. This release also addresses reported CVEs in dependent libraries.
...
{/* TT-16268, TT-17699 */}
Tyk MDCB now supports the same unified syntax for referencing secrets and configuration values held in an external store that other Tyk components use: `kv://<store-name>/<path>` to substitute a whole value, or `$kv{<store-name>:<path>}` to substitute a value inline within a string. Declare one or more named secret stores in a new top-level `kv` section of the MDCB configuration file, then reference them from any string configuration value — for example, the analytics database connection string.
References are resolved once at startup, including nested configuration fields and references supplied via `TYK_MDCB_*` environment variables. If a referenced store is undefined or unreachable, MDCB fails to start with a clear error rather than starting with a missing or invalid value. Secret stores themselves can only be declared in the configuration file, not via environment variables. Configurations with no `kv` section behave exactly as before.
</Accordion>
