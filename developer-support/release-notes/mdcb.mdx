...
Tyk MDCB has been updated to Go 1.25 for enhanced security and performance. This release also addresses some CVEs and adds a new `omit_analytics_index_creation` configuration option to prevent PostgreSQL deadlocks when running multiple MDCB instances.
...
<Accordion title='Added option to disable analytics index creation in horizontally scaled MDCB deployments'>
Added a new `omit_analytics_index_creation` configuration option to prevent PostgreSQL deadlocks in horizontally scaled MDCB deployments. When enabled, MDCB instances skip automatic index creation while maintaining full analytics functionality, eliminating database contention issues when multiple instances attempt to create the same indexes concurrently.
</Accordion>

<Accordion title='Added support for `ignore_aggregations` configuration option'>
Added the ability to configure the `ignore_aggregations` option for the MongoDB aggregates pump. This allows users to better control high cardinality data, such as per-key analytics, which helps prevent performance degradation and reduces database storage costs.
</Accordion>

<Accordion title='Added support for self-healing aggregate pump'>
Added the ability to enable the `enable_aggregate_self_healing` option for the MongoDB aggregates pump. When enabled, this feature automatically splits aggregate documents that exceed database size limits, preventing analytics data loss and reducing network and compute resource consumption.
</Accordion>
