...
MDCB 2.11.0 brings MCP analytics support to hybrid deployments. Data Planes can now route MCP traffic records to the Control Plane through dedicated, isolated channels, keeping MCP analytics separate from standard API analytics and supporting both raw and aggregated record storage.
...
<Accordion title='Add dedicated MCP analytics RPC dispatchers for hybrid deployments'>
MDCB 2.11.0 introduces two new RPC dispatcher methods that give hybrid Data Planes an isolated channel for routing MCP analytics to the Control Plane:
...
These dispatchers work alongside the existing `PurgeAnalyticsData` and `PurgeAnalyticsDataAggregated` methods. MCP traffic is routed separately, so MCP analytics are not mixed with standard API analytics in hybrid deployments.
