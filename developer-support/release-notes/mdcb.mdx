...
- `PurgeAnalyticsDataMCP`: routes raw MCP analytics records from the Data Plane to the Control Plane's configured analytics storage backend (MongoDB, PostgreSQL, etc.)
- `PurgeAnalyticsDataMCPAggregated`: routes pre-aggregated MCP analytics records, enabling efficient storage of high-volume MCP traffic without raw record overhead
