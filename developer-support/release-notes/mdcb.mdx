...
MDCB 2.11.0 brings MCP analytics support to distributed deployments. Data Planes can now route MCP traffic records to the Control Plane through dedicated, isolated channels, keeping MCP analytics separate from standard API analytics and supporting both raw and aggregated record storage.
...
For distributed deployments, deploy MDCB 2.11.0 before upgrading Tyk Pump to 1.15.0 and Tyk Gateway to 5.13.0. The new MCP analytics RPC handlers are additive and do not affect existing Pump or Gateway versions until they are also upgraded. See [MCP Gateway upgrade considerations](/developer-support/upgrading#mcp-upgrade-considerations) for the full deployment order.
...
<Accordion title='Add dedicated MCP analytics RPC dispatchers for distributed deployments'>
MDCB 2.11.0 introduces two new RPC dispatcher methods that give distributed Data Planes an isolated channel for routing MCP analytics to the Control Plane:

- [PurgeAnalyticsDataMCP](/tyk-multi-data-centre/mdcb-configuration-options#purgeanalyticsdatamcp): routes raw MCP analytics records from the Data Plane to the Control Plane's configured analytics storage backend (MongoDB, PostgreSQL, etc.)
- [PurgeAnalyticsDataMCPAggregated](/tyk-multi-data-centre/mdcb-configuration-options#purgeanalyticsdatamcpaggregated): routes pre-aggregated MCP analytics records, enabling efficient storage of high-volume MCP traffic without raw record overhead

These dispatchers work alongside the existing `PurgeAnalyticsData` and `PurgeAnalyticsDataAggregated` methods. MCP traffic is routed separately, so MCP analytics are not mixed with standard API analytics in distributed deployments.

For details, see the [MCP analytics documentation](/ai-management/mcp-gateway/mcp-analytics).
</Accordion>
