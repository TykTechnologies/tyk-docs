...
MDCB 2.11.0 introduces a dedicated analytics channel for MCP traffic in hybrid deployments, ensuring raw MCP analytics records are routed cleanly to MCP-specific pump backends without mixing with standard API analytics data.
...
<Accordion title='Dedicated MCP analytics channels for hybrid deployments'>
MDCB now includes two dedicated RPC dispatchers for MCP analytics in hybrid deployments:

- `PurgeAnalyticsDataMCP` — handles raw (non-aggregated) MCP analytics records, routing them exclusively to MCP-specific pump backends (`sqlMCPPump` / `mcpPump`) and bypassing the standard analytics and graph analytics pumps.
- `PurgeAnalyticsDataMCPAggregated` — handles pre-aggregated MCP analytics, following the same separation pattern already established for standard aggregated data.

Together these ensure MCP analytics are cleanly isolated from regular API traffic records at every stage of the analytics pipeline.
</Accordion>
