...
The MDCB `/dataplanes` endpoint has been enhanced to provide visibility into your Tyk deployment. The endpoint response now includes a detailed list of the specific APIs and Policies loaded on each connected data plane gateway. Additionally, you can now filter the results for a specific gateway by using the new `node_id` query parameter. This allows for easier monitoring and troubleshooting of your environment by providing a clear picture of what is running on each gateway.
</Accordion>
