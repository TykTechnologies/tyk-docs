...
- **API Designer Integration**: Import OAS 3.1 compliant OpenAPI descriptions to create Tyk OAS APIs. The raw API editor does not yet provide live syntax validation and autocomplete for OAS 3.1. Version detection is automatic, so there is no loss in experience when working with OAS 3.0.
