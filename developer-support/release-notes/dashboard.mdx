...
<Accordion title='Improved Visibility of Versioned API Endpoints'>
Tyk Dashboard now displays the correct URLs for reaching your APIs (including versioned APIs), making it clear how to invoke each API without having to dig into configuration settings. The servers section now shows proper endpoints based on your versioning method (URL path, header, or query parameter) and supports custom domains and edge endpoints.
</Accordion>
