...
|         | Pump v1.16.1      | Pump all versions |
|         | TIB (if using standalone) v1.7.4 | TIB all versions |
...
<Accordion title='Fix API Debugger requests rejected by strict service meshes (e.g. Istio)'>
The Dashboard's API Debugger, used to test an API definition against the Gateway before it's saved and deployed, forwarded debug requests to the Gateway using the Dashboard's own `Host` header instead of the target Gateway's. Strict routing layers such as Istio reject requests where the `Host` header doesn't match the destination, causing debug requests to fail with a routing error before ever reaching the Gateway.

The Dashboard now sets the `Host` header to match the target Gateway when forwarding debug requests, so the API Debugger works correctly behind Istio or similar strict reverse proxies, without requiring custom service mesh workarounds.
</Accordion>

<Accordion title='Fix HTTP 403 error when creating users or user groups with separate read/write database connections'>
When the Dashboard is configured with separate read and write connection strings for its SQL database, creating a user, admin user, or user group could fail with an HTTP 403 error, even though the object had actually been created successfully. This happened because the Dashboard immediately read the newly created object back from the read connection, which could momentarily lag behind the write connection.

Creating a user, admin user, or user group no longer performs this unnecessary read-after-write; the Dashboard now returns success using the object it just created. This resolves the intermittent 403 errors under replication lag and has no impact on deployments using a single connection string or MongoDB.
</Accordion>

<Accordion title='Add `key_hash` field to API key update responses for consistency with other key endpoints'>
When updating an API key by hash (`?hashed=1`) via the Dashboard API, the response returned the hash value in the `key_id` field, unlike the create and get key endpoints, which already returned both `key_id` and `key_hash`. This inconsistency made it harder for API consumers to reliably distinguish between the two identifiers when handling update responses.

The update endpoint now also returns a `key_hash` field alongside `key_id`, matching the response shape of the other key endpoints. To preserve backward compatibility, `key_id` continues to contain the hash value when updating via `?hashed=1`, so existing integrations that read the hash from `key_id` are unaffected. `key_hash` is omitted from the response when key hashing is disabled on the Gateway and Dashboard.
</Accordion>

<Accordion title='Mask Provider Configuration Secrets and Dashboard API Access Token in the UI'>
The Provider Configuration Secret in Identity Profiles (the OAuth/OIDC client secret used by Social and OpenID Connect providers) and a user's Dashboard API Access Token on the Users detail page were displayed in plain text, carrying the same risk of accidental disclosure via screen share or shoulder-surfing previously addressed for secret fields in the API Designer.

Both values are now masked by default, with a reveal/hide toggle to unmask them when needed, and re-mask automatically on page refresh. This applies to both the create and edit flows for Identity Profiles. The Dashboard API Access Token continues to respect existing view permissions — users without access still see the permission-denied message rather than a masked value.
</Accordion>
