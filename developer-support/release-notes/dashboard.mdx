...
In this patch, we have fixed a number of small issues relating to the management of API Keys (Auth Tokens) for APIs secured using mTLS when using Dashboard UI. We have also fixed the filtering applied when requesting the details of all keys that grant access to a specific API.  For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v5.8.12).
