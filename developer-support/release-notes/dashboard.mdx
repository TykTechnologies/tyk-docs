...
#### Release Date xx February 2025

#### Release Highlights

In this release, we have improved the Dashboard user experience by fixing certificate tab display issues for static mTLS APIs, updating the expired license modal with correct contact information, and resolving the API-specific key retrieval endpoint to return only relevant keys instead of all environment keys. For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v5.8.12).
...
| :---- | :---- | :---- |
| 5.8.12   | MDCB v2.9.0     | MDCB v2.0.0 |
...
<Accordion title='Fixed Incorrect Certificate Tab Display for Static mTLS APIs'>
Resolved an issue in the Dashboard UI where the Authentication tab incorrectly appeared when creating keys for APIs protected with Auth Token and static mTLS, forcing users to unnecessarily link client certificates. This fix eliminates the confusing certificate linking requirement for static mTLS APIs, where no certificate-token binding should occur, and provides clearer separation between different mTLS authentication modes in the Dashboard interface.
</Accordion>
