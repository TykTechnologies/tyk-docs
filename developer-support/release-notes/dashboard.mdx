...
<Accordion title='Certificate Authentication Method'>
- **Redesigned certificate authentication configuration** by separating it from Auth Token authentication into a standalone method called "Certificate Authentication" (previously Dynamic mTLS). 
- **Updated both Tyk Classic and OAS API designers** to include Certificate Authentication as an independent authentication option available for multi-auth configurations. 
- **Enhanced the key creation interface** with clearer labeling, improved guidance text, and documentation links.
</Accordion>
