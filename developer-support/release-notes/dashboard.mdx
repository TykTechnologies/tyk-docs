...
**OpenAPI Import and Management Improvements**

This release addresses several critical issues with OpenAPI specification handling, including fixes for JavaScript regex Unicode escape sequence processing and duplicate parameter reference errors that were causing OAS update failures. Additionally, the dashboard certificate filter has been enhanced to properly display all API-linked certificates, improving visibility and management of SSL certificates.

**Enhanced Session and Memory Management**

New configurable options have been introduced for Redis memory optimization through improved session object management. Users can now configure session expiry management for both keys and policies, providing better control over resource utilization and system performance.

**Dashboard User Experience Updates**

The dashboard interface has been refined with configurable toast message duration settings, allowing users to customize notification timing. The confirmation modal for Universal Data Graph (UDG) schema updates has been removed to streamline the workflow, and the dashboard has been updated to Go 1.25 for improved performance and security.

For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v5.7.1) below.
...
Added the ability to configure the display duration for toast notifications in the Dashboard UI. Previously, all toast messages displayed for a fixed 3-second duration, which could cause screen clutter when multiple notifications appeared simultaneously from batch operations.
...
<Accordion title='Added Session Expiry Management Configuration for Keys and Policies'>
Added support for configuring session object expiry behavior in Redis through the Dashboard API. This enhancement introduces two new fields that provide more intuitive control over how Tyk manages expired session objects:

- `post_expiry_action` - defines the behavior when a key expires (accepts "retain" or "delete")
- `post_expiry_grace_period` - specifies the retention period beyond key expiry in seconds (supports -1 for forever, 0 for legacy behavior, or positive integers)

Both Keys (Session Objects) and Policies now support these configuration options through the Dashboard API with full validation and persistence. The implementation maintains complete backward compatibility by defaulting to legacy behavior (`post_expiry_action: "retain"` and `post_expiry_grace_period: 0`) when these fields are omitted from new objects or when retrieving existing objects that don't have these fields configured.
</Accordion>
...
<Accordion title='Removed Confirmation Modal for UDG Schema Updates'>
Removed the confirmation modal that previously appeared when updating Universal Data Graph (UDG) schema in the Dashboard. Users can now save schema changes directly without additional confirmation prompts, streamlining the UDG development workflow and eliminating the need to dismiss confirmation dialogs for routine schema updates.
</Accordion>
...
<Accordion title='Fixed Dashboard Certificate Filter to Show All API-Linked Certificates'>
Resolved an issue where the dashboard filter did not return all certificates linked to an API. Previously, the "Filter by API" function on the TLS/SSL Certificates page only displayed certificates used for client-to-gateway (static) mTLS, ignoring certificates used for gateway-to-upstream mTLS, custom domain certificates, and certificate pinning. This resulted in incomplete and misleading results where users could not see all certificates actively used by a given API. The dashboard now properly displays all certificate types associated with an API, including client-to-gateway mTLS, gateway custom domain certificates, gateway-to-upstream client certificates, and pinned public key certificates.
</Accordion>

<Accordion title='Fixed OAS Update Failures Due to Duplicated Parameter References'>
Resolved an issue where APIs imported from OpenAPI specifications with external parameter references would fail validation during subsequent update operations. Previously, when importing OAS files that referenced multiple distinct parameters from external files, the Dashboard's import process incorrectly collapsed all external references into a single generic parameter component and duplicated that reference multiple times, creating invalid OpenAPI specifications with multiple parameters having the same name and location.

The Dashboard now properly resolves external parameter references to distinct parameter definitions during import, ensuring that each `$ref` becomes a separate parameter component and maintains OpenAPI 3.0 compliance for successful updates.
</Accordion>
