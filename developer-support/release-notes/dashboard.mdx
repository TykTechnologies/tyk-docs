...
Fixed an issue where the Create Key and Edit Key screens would incorrectly require the user to register a client certificate with the key when granting access to an API secured with static mTLS.

Fixed a separate issue where the option to assign the client certificate was not possible when creating a key for an API which has the authentication method changed to use dynamic mTLS.
</Accordion>
