...
Tyk Dashboard 5.13.0 adds comprehensive management API and access control support for the MCP (Model Context Protocol) Gateway. A dedicated `/api/mcps` endpoint set provides full CRUD operations for MCP Proxy definitions. A new `mcp` RBAC permission group (deny/read/write) controls access to MCP management independently of the existing `apis` permission. MCP Gateway is now generally available with no feature flag required — visibility is controlled solely by the `mcp` RBAC permission. Keys and policies can now carry four new MCP-specific access-right fields for configuring tool-based access control and per-primitive rate limiting.
