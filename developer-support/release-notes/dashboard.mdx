...
We have added support for [Client Certificate-Token Binding](/api-management/authentication/auth-token#client-certificate-token-binding), adding an additional layer of security by forming a binding association between the Auth Token issued to a client and their client certificate.

When creating or modifying a key that grants access to an API secured using Auth Token with a static mTLS allow list, you now have the option to bind a client certificate with the key (token).

Note that this certificate must also be added to the client certificate allow list in the API definition to ensure that the certificate validation that occurs after the mTLS handshake is successful.

Multiple client certificates can be bound to each key to support certificate rotation scenarios, and the feature maintains full backward compatibility with existing keys that don't specify certificate bindings.
</Accordion>
