...
Now you can configure rate limits at the [endpoint level per client](/api-management/rate-limit#key-level-rate-limiting), using new configuration options in the access key. Use [Policies](/api-management/access-control/policies/understanding-policies) to create templates to set appropriate rate limits for your different categories of user.
