...
Resolved filtering issues where applying multiple filters (name + category) would drop previously applied filters instead of combining them. Users can now apply multiple filters that work together as expected, with all active filters preserved during pagination and sorting. Also fixed the Data Graph onboarding wizard incorrectly appearing when filters returned zero results - it now only shows when no APIs actually exist in the system, keeping filters visible and clearable when results are empty.
</Accordion>
...
Resolved multiple issues preventing successful API testing in the Quick Start onboarding flow. Users can now test APIs with any response type (JSON, HTML, XML) without encountering 500 errors or blank response displays.
