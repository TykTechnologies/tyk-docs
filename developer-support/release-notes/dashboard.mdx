...
This release introduces Tyk Governance, a new capability built directly into the Dashboard for defining API standards and continuously checking your Tyk OAS APIs against them — catching design, security, and compliance issues before an API reaches production instead of after. Platform teams can create rulesets from a ready-to-use template or their own definition, apply them to APIs via categories, and see results directly on a new Governance tab for every API, complete with severity-graded issues and specific remediation guidance. Compliance status is also surfaced across the API list and on the Dashboard home page, and the same checks can be run from a CI/CD pipeline before an API is ever imported. Tyk Governance is an Enterprise Edition capability, enabled per organization — contact your Tyk account team to turn it on.

**Flexible configuration with a unified Key-Value reference syntax**
 
This release also introduces a single, unified way to reference secrets and configuration values from an external store: `kv://<store>/<path>` for a whole value, or `$kv{<store>:<path>}` inline within a string. The Dashboard's own configuration file can now use this same syntax to source sensitive values — such as database credentials, the admin secret, and the license key — from an external secret store, in addition to its existing support for API definitions. Existing provider-specific references keep working unchanged.
 
This release also adds GCP IAM-based authentication to Redis/Valkey for Google Cloud Memorystore deployments, updates the Go toolchain to Go 1.26, and includes a set of smaller fixes covering the API Debugger, user and API key management, policy update performance, secret masking, Listen Path validation, and license expiry visibility. This release also migrates off an unmaintained security library and addresses several CVEs in dependent libraries. For a comprehensive list of changes, please refer to the detailed changelog below.
...
- **Continuous and on-demand evaluation** — APIs are evaluated automatically whenever a service is deployed, moves category, or a linked ruleset changes, and again at the moment of deployment. You can also test any ruleset against any API on demand, without affecting its stored compliance status, before deciding to apply it.
...
<Accordion title='Added a unified `kv://` syntax for referencing secrets and configuration values'>
{/* TT-16268, TT-17102, TT-17623, TT-17697, TT-17698, TT-17702 */}
Tyk now supports a single, unified syntax for referencing secrets and configuration values held in an external store, anywhere a value can appear in an API definition: `kv://<store-name>/<path>` to substitute a whole value, or `$kv{<store-name>:<path>}` to substitute a value inline within a string. The store referenced by `<store-name>` — Vault, Consul, a plain environment variable, or a generic secrets provider — is declared once in a new Key-Value registry in the Gateway configuration.
The Dashboard's own configuration file can now use this same syntax to source its own sensitive values — such as database credentials, the admin secret, and the license key — from an external secret store, via a new top-level `kv` configuration section. If the Dashboard cannot resolve a configured secret reference at startup, it fails to start rather than continuing with an invalid value.
 
Existing provider-specific references (`vault://`, `consul://`, `env://`, `secrets://`, and the `$secret_<provider>.<key>` inline form) continue to work exactly as before, so no existing API definition or Dashboard configuration needs to change.
</Accordion>
 
<Accordion title='Added a new goja-based driver for JavaScript plugins'>
{/* TT-16948, TT-17761 */}
Tyk Gateway's JavaScript plugin engine now offers a `goja`-based driver as an opt-in alternative to the existing `otto` engine, improving performance and adding support for post-auth and response hooks in JS plugins — hook points the `otto` driver does not support. The `otto` driver remains the default, so existing JS plugins keep working unchanged; the new driver can be selected per API, including from the plugin driver dropdown in the Tyk OAS API designer.
</Accordion>

<Accordion title='Added GCP IAM authentication for Redis/Valkey'>
{/* TT-17611 */}
Tyk components can now authenticate to Google Cloud Memorystore for Redis/Valkey using GCP IAM-based authentication, as an alternative to a static Redis password. This is supported for Memorystore for Valkey and for Memorystore for Redis Cluster; legacy non-cluster Memorystore for Redis has no IAM support from Google and continues to use a static password. The new authentication method is opt-in via configuration and is fully backward compatible with existing static-password setups.
</Accordion>

<Accordion title='Added a configuration option to limit idle PostgreSQL connection time'>
{/* TT-16212 */}
Tyk Dashboard now exposes a configuration option to cap the maximum idle time for PostgreSQL database connections, so idle connections can be released rather than held indefinitely. This helps deployments such as blue/green rollouts avoid exhausting the available PostgreSQL connections; the option is opt-in and preserves existing behavior when left unset.
</Accordion>
...
{/* TT-10248 */}
We have resolved an issue where the Dashboard's API Debugger forwarded requests to the Gateway's `/tyk/debug` endpoint carrying the Dashboard's own Host header rather than the Gateway's. Strict routing layers such as Istio could reject these requests before they reached the Gateway. The Dashboard now sets the correct Host header when proxying debug requests, so the API Debugger works without custom service mesh workarounds.
</Accordion>

<Accordion title='Fixed HTTP 403 error when creating users or user groups with separate read/write database connections'>
{/* TT-15506 */}
We have resolved an issue affecting Dashboard deployments configured with separate read and write database connection strings (SQL databases only). Creating a user or user group could return a `403` error if replication lag meant the newly written record wasn't yet visible on the read replica, even though the record had been created successfully. The Dashboard no longer performs this redundant read-after-write check, so creation succeeds reliably regardless of replication lag. This applies to user, user group, and admin user creation.
</Accordion>
...
{/* TT-16937 */}
We have resolved a performance issue where updating a Policy containing a large number of Tyk OAS APIs (for example, 60 or more) caused high latency, due to the Dashboard repeatedly re-parsing each API's OpenAPI document for validation, audit logging, and response serialization. Policy updates and the Gateway's `/system/policies` synchronization endpoint now scale linearly with the number of APIs in a policy instead of degrading sharply as policies grow.
</Accordion>
...
{/* TT-17631 */}
We have resolved an issue where the Dashboard accepted and saved a Listen Path without a leading slash — through the API Designer, the legacy Classic API designer, and all import flows (Blueprint, Swagger, WSDL, and OpenAPI/AsyncAPI data source import). A Listen Path missing its leading slash causes the Gateway to fail to load that API, so every request to it silently returns HTTP 404. The Dashboard now validates this at both the UI and API level, and rejects the save or import with a clear error message before an invalid definition can reach the Gateway. An existing API is left unchanged if an update with an invalid Listen Path is attempted.
</Accordion>

<Accordion title='Fixed duplicate upstream token requests and log noise in Upstream OAuth'>
{/* TT-16039 */}
We've resolved an issue in Upstream OAuth (Enterprise Edition), where Tyk acts as the OAuth client to an upstream identity provider, that sent a redundant, always-failing token request when the identity provider required client credentials in the request body rather than in a header. This removed the extra round trip and the associated log noise generated every time an upstream token was obtained.
</Accordion>
