...
{/* TT-17611 */}
Tyk Dashboard can now authenticate to Google Cloud Memorystore for Redis/Valkey using [GCP IAM-based authentication](/tyk-configuration-reference/redis-cluster-sentinel#iam-authentication), as an alternative to a static Redis password.
...
The IAM configuration is set in Tyk Dashboard's new top-level [`redis_iam_auth`](/tyk-configuration-reference/redis-cluster-sentinel#iam-authentication) configuration block.
</Accordion>
...
You can now select the [client authentication method](/api-management/upstream-authentication/oauth#authentication-method) that Tyk should adopt via a dropdown in the API Designer:
