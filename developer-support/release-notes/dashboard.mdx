...
Only the most recent Tyk Dashboard release and the most recent on the [Long Term Support (LTS)](/developer-support/release-types/long-term-support) branch are supported. We recommend staying current on the feature or LTS branch to minimise exposure to security vulnerabilities from third party dependencies.
...
#### Release Date xx September 2026

#### Release Highlights

This is a security-only patch release. It contains no functional or behavioral changes; it addresses several CVEs identified in dependent libraries

For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v5.15.1) below.
...
#### Dependencies 
<a id="dependencies-5.15.1"></a>

<a id="dependencies-5.15.1"></a>

| Dashboard Version | Recommended Releases | Backwards Compatibility |
| :------ | :---------------- | :---------------- |
| 5.15.1  | MDCB v2.13.0      | MDCB v2.13.0      |
...
| [OpenAPI Specification](https://spec.openapis.org/oas/v3.1.2.html) | v3.1.x, v3.0.x | v3.1.x, v3.0.x | Supported by [Tyk OAS](/api-management/gateway-config-tyk-oas)|

Given the potential time difference between your upgrade and the release of this version, we recommend users verify the ongoing support of third-party dependencies they install, as their status may have changed since the release.

#### Upgrade instructions
<a id="upgrade-5.15.1"></a>

<a id="upgrade-5.15.1"></a>

If you are upgrading to 5.15.1, please follow the detailed [upgrade instructions](#upgrading-tyk).

#### Downloads

- [Docker Image to pull](https://hub.docker.com/r/tykio/tyk-dashboard/tags?page=&page_size=&ordering=&name=v5.15.1)
  - ```bash
    docker pull tykio/tyk-dashboard:v5.15.1
    ```
- Helm charts
  - [tyk-charts](/developer-support/release-notes/helm-chart)

<Note>
Tyk Helm Charts are configured to install the LTS version of Tyk Dashboard. You will need to modify them to install v5.15.1.
</Note>

#### Changelog
<a id="Changelog-v5.15.1" data-scroll-offset></a>

##### Security Fixes

<AccordionGroup>
<Accordion title='Resolved CVEs'>
This is a security-only patch release. We have addressed the following CVEs reported in dependent libraries, providing increased protection against security vulnerabilities:
- <a href="https://nvd.nist.gov/vuln/detail/CVE-2026-14456" target="_blank">CVE-2026-14456</a>
- <a href="https://nvd.nist.gov/vuln/detail/CVE-2026-43871" target="_blank">CVE-2026-43871</a>
- <a href="https://nvd.nist.gov/vuln/detail/CVE-2026-56855" target="_blank">CVE-2026-56855</a>
- <a href="https://nvd.nist.gov/vuln/detail/CVE-2026-78662" target="_blank">CVE-2026-78662</a>
- <a href="https://nvd.nist.gov/vuln/detail/CVE-2026-79921" target="_blank">CVE-2026-79921</a>
- <a href="https://nvd.nist.gov/vuln/detail/CVE-2026-84445" target="_blank">CVE-2026-84445</a>
</Accordion>
</AccordionGroup>
...
This release introduces [Tyk Governance](/tyk-governance/overview), a new capability built directly into the Dashboard for defining API standards and continuously checking your Tyk OAS APIs against them, catching design, security, and compliance issues before an API reaches production instead of after.

Platform teams can create [rulesets](/tyk-governance/rulesets) from a [ready-to-use template](/tyk-governance/ruleset-templates) or their own definition, [apply them to APIs via categories](/tyk-governance/scoping-rulesets), and see results directly on a new Governance tab for every API, complete with severity-graded issues and specific [remediation guidance](/tyk-governance/remediate-issues). Compliance status is also surfaced across the API list and on the Dashboard home page, and the same checks can be [run from a CI/CD pipeline](/tyk-governance/cicd-checks) before an API is ever imported.

Tyk Governance is enabled by a new claim in the Dashboard license; contact your Tyk account team to turn it on.

**Configuration Secrets Management**
 
This release adds [Secrets Management](/tyk-configuration-reference/kv-store) to the Tyk Dashboard configuration file for the first time, letting you reference values from external secrets stores in place of plain-text values.
 
**IAM-based authentication for Google Cloud Memorystore**

Tyk Dashboard now supports [IAM-based authentication](/tyk-configuration-reference/redis-cluster-sentinel#iam-authentication) for Google Cloud Memorystore for Redis Cluster and Valkey, as an alternative to a static password.
...
For more details see [Managing MCP proxies using the Dashboard](/ai-management/mcp-gateway/managing-proxies).
</Accordion>
...
- **[Create rulesets](/tyk-governance/rulesets)** — a set of governance rules — either from a [ready-to-use template](/tyk-governance/ruleset-templates) (an OWASP API Security ruleset, or a general OpenAPI design-best-practices ruleset), or from your own definition by uploading or pasting a JSON or YAML ruleset file.
- **[Apply rulesets via API categories](/tyk-governance/scoping-rulesets)** — link a ruleset to one or more of your existing API categories, and every API carrying that category is automatically governed by it; an API can be governed by more than one ruleset at once.
...
- **[CI/CD governance checks](/tyk-governance/cicd-checks)** — run the same evaluation from a pipeline (GitHub Actions, GitLab CI, Jenkins, or any tool that can call an HTTP endpoint) directly against an OpenAPI document, before the API is even imported into Tyk.
...
In this release, Tyk Governance supports Tyk OAS APIs only. Tyk Governance is disabled by default — contact your Tyk account team to enable it for your organization. For more information, see [Tyk Governance](/tyk-governance/overview).
</Accordion>
...
See [Secrets Management](/tyk-configuration-reference/kv-store) for full details.
</Accordion>
...
- choose Tyk Gateway's new [JavaScript plugin engine](/api-management/plugins/javascript#choosing-a-javascript-engine) by selecting the `JavaScript` driver
- when using JavaScript plugins, provide your [plugin code inline](/api-management/plugins/overview#inline-code-javascript-only) as an alternative to giving a path to a file containing the code
- assign JavaScript plugins to [any plugin hook](/api-management/plugins/overview#plugin-hook-types)
- assign [multiple plugin bundles](/api-management/plugins/overview#composing-multiple-bundles) to an API with a simple comma separated list
- include [analytics plugins](/api-management/plugins/plugin-types#loading-from-a-bundle) in the custom plugin bundle

For more details see [Custom Plugins](/api-management/plugins/overview).
</Accordion>
...
For more details see [IAM Authentication](/tyk-configuration-reference/redis-cluster-sentinel#iam-authentication).
</Accordion>
...
For more details see [SQL Connection Pool Management](/tyk-configuration-reference/sql#connection-pool-management).
</Accordion>
...
For more details see [Upstream OAuth 2.0](/api-management/upstream-authentication/oauth#authentication-method).
</Accordion>
...
| Third Party Dependency                                    | Tested Versions     | Compatible Versions | Comments | 
| :-------------------------------------------------------- | :------------------ | :------------------ | :------- | 
| [Golang](https://go.dev/dl/)                              | 1.25                |  1.25               | [Go plugins](/api-management/plugins/golang) must be built using Go 1.25 | 
| [Redis](https://redis.io/download/)                       | 6.2.x, 7.x, 7.4.x, 8.x (up to 8.4)   | 6.2.x, 7.x, 7.4.x, 8.x (up to 8.4)   | | 
| [Valkey](https://valkey.io/download/)                     | 7.2.x, 8.0.x, 8.1.x | 7.2.x, 8.0.x, 8.1.x | | 
| [MongoDB](https://www.mongodb.com/try/download/community)  | 7.0.x, 8.0.x  | 6.0.x, 7.0.x, 8.0.x  | | 
| [DocumentDB](https://aws.amazon.com/documentdb/)          | 4, 5                | 4, 5                | | 
...
{/* TT-17497 */}
The new `file://` storage option added to Tyk Gateway's [Key-Value secret storage](/tyk-configuration-reference/kv-store#supported-secret-stores) feature can be configured in the API editor.
...
{/* TT-16953 */}
The Dashboard now has a dedicated [application log verbosity](/api-management/logs#severity-and-verbosity) setting ([`log_level`](/tyk-dashboard/configuration)) to match those of other Tyk components. It accepts the same values: `error`, `warn`, `info` (the default), and `debug`. This applies to all Dashboard application logs.
...
| Third Party Dependency                                    | Tested Versions     | Compatible Versions | Comments | 
| :-------------------------------------------------------- | :------------------ | :------------------ | :------- | 
| [Golang](https://go.dev/dl/)                              | 1.26                |  1.26               | [Go plugins](/api-management/plugins/golang) must be built using Go 1.26 | 
| [Redis](https://redis.io/download/)                       | 6.2.x, 7.x, 7.4.x, 8.x (up to 8.4)   | 6.2.x, 7.x, 7.4.x, 8.x (up to 8.4)   | | 
| [Valkey](https://valkey.io/download/)                     | 7.2.x, 8.0.x, 8.1.x | 7.2.x, 8.0.x, 8.1.x | | 
| [MongoDB](https://www.mongodb.com/try/download/community)  | 7.0.x, 8.0.x  | 6.0.x, 7.0.x, 8.0.x  | |
| [DocumentDB](https://aws.amazon.com/documentdb/)          | 4, 5                | 4, 5                | | 
...
Tyk Dashboard 5.13.0 adds comprehensive support for the MCP (Model Context Protocol) Gateway. A dedicated `/api/mcps` endpoint set provides full CRUD operations for MCP Proxy definitions. See [MCP API extensions](/ai-management/mcp-gateway/mcp-api-extensions).

- A new `mcp` RBAC permission group (deny/read/write) controls access to MCP management independently of the existing `apis` permission. Visibility is controlled solely by the `mcp` RBAC permission. See [Permissions](/ai-management/mcp-gateway/managing-proxies#permissions).
- Sessions (Keys) and Policies can now carry four new MCP-specific access-right fields for configuring tool-based access control and per-primitive rate limiting. See [MCP Gateway Policies](/ai-management/mcp-gateway/policies).
...
Dashboard 5.13.0 introduces policies that grant access to MCP Proxies. Tyk Developer Portal versions prior to 1.17.2 may automatically retrieve these policies and present them as API Products when [synchronizing with the Dashboard](/portal/api-products#dashboard-first-approach).
...
For details, see [MCP API extensions](/ai-management/mcp-gateway/mcp-api-extensions).
</Accordion>
...
For details, see [Permissions](/ai-management/mcp-gateway/managing-proxies#permissions).
</Accordion>
...
- [post_expiry_action](/api-management/access-control/sessions-and-keys/session-lifecycle#2-define-the-post-expiry-action) - determines what happens to the data in Redis after the `expires` timestamp is reached.
- [post_expiry_grace_period](/api-management/access-control/sessions-and-keys/session-lifecycle#3-configure-the-grace-period) - defines how long (in seconds) the Session is kept in Redis after expiration (if the `post_expiry_actions` is to retain the Session)
...
| Third Party Dependency | Tested Versions | Compatible Versions | Comments | 
| ---------------------- | --------------- | ------------------- | -------- | 
| [GoLang](https://go.dev/dl/)          | 1.25 | 1.25 | [Go plugins](/api-management/plugins/golang) must be built using Golang 1.25 | 
| [Redis](https://redis.io/download/)   | 5.x, 6.x, 7.x, 8.x (up to 8.4) | 5.x, 6.x, 7.x, 8.x (up to 8.4) | | 
| [Valkey](https://valkey.io/download/) | 8.0.x, 8.1.x    | 7.2.x, 8.0.x, 8.1.x    | | 
...
In this release we've deprecated the [policies.allow_explicit_policy_id](/tyk-oss-gateway/configuration#policies-allow_explicit_policy_id) configuration option. This was previously added to allow the use of custom policy IDs, which is now the default behaviour so this option is redundant.
...
This release builds on the recent release of [Tyk 5.8.3](/developer-support/release-notes/dashboard#5-8-3-release-notes), adding a collection of new capabilities. For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v5.9.0).
...
In this release we've deprecated the [allow_explicit_policy_id](/tyk-dashboard/configuration#allow_explicit_policy_id) configuration option. This was previously added to allow the use of custom policy IDs, which is now the default behaviour so this option is redundant.
...
- [Docker image to pull](https://hub.docker.com/r/tykio/tyk-dashboard/tags?page=&page_size=&ordering=&name=v5.8.5)
  - ```bash
    ...
    ``` 
- Helm charts
  - [tyk-charts v3.0.0](/developer-support/release-notes/helm-chart#3-0-0-release-notes)
...
- The Tyk OAS API designer now fully supports the use of Key-Value storage [references](/tyk-configuration-reference/kv-store#how-it-works) that are valid in the API definition.
...
- [Docker Image to pull](https://hub.docker.com/r/tykio/tyk-dashboard/tags?page=&page_size=&ordering=&name=v5.7.3)
  - ```bash
    ...
    ```
- Helm charts
  - [tyk-charts v2.2.0](/developer-support/release-notes/helm-chart#2-2-0-release-notes)
...
[Setup guide for JWE OIDC SSO](/api-management/single-sign-on-social-idp#json-web-encryption-jwe)
</Accordion>
...
- ```bash
  ...
  ```
- Helm charts
  - [Tyk Charts v2.0.0](/developer-support/release-notes/helm-chart#2-0-0-release-notes)
...
Now you can configure rate limits at the [endpoint level per client](/api-management/rate-limit#key-level-rate-limiting), using new configuration options in the access key. Use [Policies](/api-management/policies) to create templates to set appropriate rate limits for your different categories of user.
...
- ```bash
  ...
  ```
- Helm charts
  - [tyk-charts v2.1.0](/developer-support/release-notes/helm-chart#2-1-0-release-notes)
...
- ```bash
  ...
  ```
- Helm charts
  - [tyk-charts v1.6](/developer-support/release-notes/helm-chart#1-6-0-release-notes)
...
- ```bash
  ...
  ```
- Helm charts
  - [tyk-charts v1.5](/developer-support/release-notes/helm-chart#1-5-0-release-notes)
...
With MongoDB 4.4 reaching [EOL](https://www.mongodb.com/legal/support-policy/lifecycles) in February 2024, we can no longer guarantee full compatibility with this version of the database. If you are [using MongoDB](/tyk-configuration-reference/mongodb) we recommend that you upgrade to a version that we have tested with, as indicated [below](#3rdPartyTools-v5.3.6).


With PostgreSQL v11 reaching [EOL](https://www.postgresql.org/support/versioning/) in November 2023, we can no longer guarantee full compatibility with this version of the database. If you are [using PostgreSQL](/tyk-configuration-reference/sql) we recommend that you upgrade to a version that we have tested with, as indicated [below](#3rdPartyTools-v5.3.6).
...
- [Docker Image to pull](https://hub.docker.com/r/tykio/tyk-dashboard/tags?page=&page_size=&ordering=&name=v5.3.6)
 - ```bash
   ...
   ```
- Helm charts
 - [tyk-charts v2.0](/developer-support/release-notes/helm-chart#2-0-0-release-notes)
...
- [Docker Image to pull](https://hub.docker.com/r/tykio/tyk-dashboard/tags?page=&page_size=&ordering=&name=v5.3.4)
 - ```bash
   ...
   ```
- Helm charts
 - [tyk-charts v1.4](/developer-support/release-notes/helm-chart#1-4-0-release-notes)
...
- ```bash
  ...
  ```
- Helm charts
  - [tyk-charts v1.3](/developer-support/release-notes/helm-chart#1-3-0-release-notes)
...
Tyk uses `mongo-go` as the default MongoDB driver from v5.3. This provides support for MongoDB 4.4.x, 5.0.x, 6.0.x and 7.0.x. If you are using older MongoDB versions e.g. 3.x, please set MongoDB driver to `mgo`. The [MongoDB supported versions](/tyk-configuration-reference/mongodb#supported-versions) page provides details on how to configure MongoDB drivers in Tyk.
</Accordion>
</AccordionGroup>
...
Relaxed the strict validation for mock response so that the `Description` field is now optional for `response`, `responses` and `schema` within the OpenAPI description. Automatically configuring mock responses when using [Tyk OAS APIs](/api-management/traffic-transformation/mock-response#tyk-oas) is now even easier.
</Accordion>
...
Fixed two UI issues with the [OPA editor](/platform-management/open-policy-agent#using-the-open-policy-agent-in-the-dashboard) in the Tyk Dashboard to improve experience when using this feature. Scrolling beyond the end of the OPA window does not now start to scroll the API Designer window, and minimizing then re-expanding the OPA editor no longer limits the text to one line.
</Accordion>
...
Fixed an issue where the [OPA editor](/platform-management/open-policy-agent#using-the-open-policy-agent-in-the-dashboard) was not resizable. The fix ensures the floating OPA editor is now resizable and the resizing operation is smooth, improving user experience.
</Accordion>

<Accordion title='User Search not working unless you enter the full email address'>
Fixed an issue where the [User Search](/platform-management/dashboard-users#search-users) was not working unless the full email address was entered. The fix restores the functionality of showing suggestions for names as they are typed in, improving user experience and search efficiency.
</Accordion>
...
documentation for a full description of this new [user role](/platform-management/user-permissions).
...
size using [slave_options.rpc_pool_size](/tyk-oss-gateway/configuration#slave_options-rpc_pool_size)
...
See how to [Choose a MongoDB driver](/tyk-configuration-reference/mongodb)
...
We’ve improved support for [OAS Mock Responses](/api-management/traffic-transformation/mock-response#mock-response), with the Tyk OAS API definition now allowing you to register multiple Mock Responses in a single API, providing you with increased testing flexibility.
...
- Now you can turn a Kafka topic into a GraphQL subscription by simply [importing your AsyncAPI definition](/api-management/data-graph#data-graphs-api)
...
If you want switch from MongoDB to SQL, you can [use our migration tool](/tyk-configuration-reference/sql#migrating-tyk-dashboard-from-mongodb-to-sql), but keep in mind that it does not yet support the migration of your analytics data.
...
- Added option to configure certificates for Tyk Dashboard using [environment variables](/tyk-dashboard/configuration#http_server_options-certificates).
...
While SQL support for Tyk products does not depend on specific database features, with this release, we will provide official support for [PostgreSQL DB for production purposes](/tyk-configuration-reference/sql), and SQLite for development and PoC environments. Note that SQL support is available for self-managed setups only.
...
[MongoDB to SQL migration docs](/tyk-configuration-reference/sql#migrating-tyk-dashboard-from-mongodb-to-sql)
...
#### Upgrade process
If you already have GraphQL or UDG APIs you need to follow this [upgrade guide](/api-management/graphql#migrating-to-3-2).
...
To learn more [see the documentation](/tyk-identity-broker/overview)
...
See [updated flow details](/tyk-identity-broker/overview)
...
[Read More](/planning-for-production/ensure-high-availability/load-balancing#load-balancing)
...
[Read More](/api-management/tyk-pump#configuring-tyk-pump)
...
If you are upgrading from version 2.8, please [read this guide](/developer-support/release-notes/archived#upgrading-from-version-2-8)
