...
<Note>
There is no support for management of the Client IdP Registry from the Dashboard UI at this time.
</Note>
...
- human readable name for display in UI (when supported)
...
These client IdP assets extract the IdP details (issuer value and JWKS endpoint) and the scope-to-policy mapping from the API definitions, instead holding a central configuration per IdP with a list of APIs to which these should be applied.
...
Both API-level and endpoint-level timeouts accept granular configuration using using human-readable values such as `500ms`, `1.5s`, or `2m`. Previously, the endpoint-level timeouts could only be configured in seconds.
...
<Accordion title='Improve OAuth 2.0 authentication configuration UI'>
{/* TT-17385 */}
A number of UI improvements were made to the OAuth 2.0 authentication configuration in the API Designer to support the new `oauth2` scheme and guide users through configuring it correctly. The authentication method selector has been reorganised — OAuth 2.0 is now grouped with JWT, since the two are typically configured together. Deprecated methods have been moved to the bottom to steer users towards current options. Warnings and guidance have been added throughout to help users configure scope enforcement and token exchange correctly.
</Accordion>
...
In the API designer, if a TLS certificate is located using a reference, the Dashboard UI will direct the user to the API editor to view or modify the certificate.
</Accordion>

<Accordion title='Added Dashboard-specific application log level control'>
{/* TT-16953 */}
The Dashboard now has a dedicated [application log verbosity](/api-management/logs#log-verbosity) setting ([`log_level`](/tyk-dashboard/configuration#log_level)) to match those of other Tyk components. It accepts the same values — `error`, `warn`, `info` (the default), and `debug` — and applies to all Dashboard application logs.
...
{/* TT-17058 */}
The Dashboard now grenerates an entry in the application log when an authenticated request is rejected because the Dashboard user account is inactive.

This log entry, which is generated as a `warning`, includes the user's ID and group ID to aid with debugging, without including any personaly identifiable information in the log.
</Accordion>
