...
We have implemented a [change](/developer-support/release-notes/dashboard#Fixed-v5.9.0) to the behaviour of the `GET /api/streams/{apiID}` endpoint, which now expects an `Accept` header, not `Content-Type`.
...
Check the [Upgrade and Compatibility section](#upgrade-instructions) for details on the recommended upgrade path.
</Expandable>
</li>
</ul>
