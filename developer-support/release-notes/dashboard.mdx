...
This release significantly strengthens security posture with the introduction of default masking for sensitive configuration values throughout the Dashboard UI. Secrets, passwords, and tokens are now masked by default with role-based reveal toggles, reducing the risk of accidental exposure. Additionally, strict validation has been implemented for policy ID characters to prevent URL parsing issues, and comprehensive CVE fixes have been applied to dependent libraries for enhanced protection against security vulnerabilities.

**Improved Session and Resource Management**
New configurable session object management controls have been added to help optimize Redis memory usage and prevent memory bloat. Users can now configure session expiry behavior for both keys and policies through intuitive Dashboard UI controls, with options to delete or retain expired sessions based on specific requirements. This enhancement provides better control over resource utilization while maintaining full backward compatibility with existing configurations.

**Streamlined User Experience and Interface Improvements**
The Dashboard interface has been enhanced with several user experience improvements, including configurable toast notification durations, an optional confirmation skip for UDG schema updates, and improved certificate ID visibility across all certificate-related screens. Certificate IDs now use smart truncation to preserve distinctive portions, and the interface provides consistent behavior across the Certificate Store, API designers, and policy management screens.

**OpenAPI Specification and Migration Enhancements**
Fixes have been implemented for OpenAPI specification handling, resolving issues with JavaScript regex Unicode escape sequences and duplicate parameter references that were causing import and update failures. Additionally, JWT configuration field migration has been improved to ensure seamless upgrades from older Dashboard versions, automatically populating new field structures without requiring manual intervention.

**MCP Gateway Management**
Tyk Dashboard 5.13.0 adds comprehensive management API and access control support for the MCP (Model Context Protocol) Gateway. A dedicated `/api/mcps` endpoint set provides full CRUD operations for MCP Proxy definitions. A new `mcp` RBAC permission group (deny/read/write) controls access to MCP management independently of the existing `apis` permission. The `enable_mcp` configuration flag has been removed — MCP is now generally available and visibility is gated solely on the `mcp` permission. Keys and policies can now carry four new MCP-specific access-right fields for configuring tool-based access control and per-primitive rate limiting.

For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v5.13.0) below.

#### Breaking Changes

**MCP Proxy policies visible in Enterprise Developer Portal when running Portal versions earlier than v1.17.1**

Dashboard 5.13.0 introduces MCP Proxy definitions and the policies that grant access to them. If you are running the Enterprise Developer Portal (EDP) at a version earlier than v1.17.1 alongside Dashboard 5.13.0, MCP Proxy policies are not filtered from the developer catalogue. An administrator could inadvertently publish an MCP server to portal users.

Upgrade to **EDP v1.17.1 or later** before or immediately after upgrading the Dashboard. EDP v1.17.1 filters MCP Proxy policies from the developer catalogue until full portal support for MCP is available.
...
- **MDCB v2.11.0** — MCP analytics routing in hybrid deployments
- **Operator v1.4.0** — MCP Proxy management via Kubernetes CRDs
- **Sync v2.1.7** — MCP Proxy support in dump and sync operations
- **EDP v1.17.1** — prevents MCP Proxy policies from being inadvertently published to the developer catalogue
</Note>
...
<Accordion title='Added MCP Proxy management API'>
Introduces the `/api/mcps` endpoint set, providing full CRUD operations for MCP Proxy API definitions:
...
A schema endpoint at `/api/schemas/apidefs/mcp` returns the MCP API definition JSON schema. MCP Proxy definitions are excluded from general API list responses by default; standard API listing endpoints no longer return MCP Proxy definitions.

For details, see the [Managing MCP proxies documentation](/ai-management/mcp-gateway/managing-mcp-proxies).
</Accordion>

<Accordion title='Added dedicated MCP RBAC permission group'>
Introduces a dedicated `mcp` permission group (deny/read/write) for controlling access to MCP management endpoints in the Dashboard. Previously, `/api/mcps` endpoints fell under the `apis` permission.

Users who need to manage MCP Proxies must be granted `mcp: write`. Read-only access to MCP listings requires `mcp: read`. This change allows organisations to grant access to MCP management without also granting broader API management permissions.

For details, see the [Managing MCP proxies documentation](/ai-management/mcp-gateway/managing-mcp-proxies).
</Accordion>

<Accordion title='Added MCP access-right fields to keys and policies'>
Keys and policies now support four new access-right fields for configuring Tool-Based Access Control (TBAC) and per-primitive rate limiting on MCP Proxy APIs:
...
Validation rejects these fields on non-MCP APIs with a `400 Bad Request` response. Unknown API IDs are skipped during validation to avoid false negatives at Gateway startup.

For details, see the [MCP proxy policies documentation](/ai-management/mcp-gateway/policies).
</Accordion>

<Accordion title='Added validation for unsupported middleware in MCP Proxy definitions'>
Creating or updating an MCP Proxy definition with unsupported middleware now returns a `400 Bad Request` response with a descriptive error message listing the specific violations.

The following middleware types are not supported in MCP Proxy definitions:
- **Global level**: batch processing, global cache
- **Per-primitive level**: method transform, response body transform, URL rewrite, internal, cache, validate request, mock response

Unsupported middleware is also stripped from the definition as a defence-in-depth measure.
</Accordion>

<Accordion title='MCP Gateway is now generally available'>
The `enable_mcp` configuration flag and its associated page-data injection have been removed. MCP Gateway is production-ready and access to MCP management in the Dashboard is now controlled solely by the `mcp` RBAC permission.

No configuration change is required for users who do not use MCP. Users who previously set `enable_mcp: true` in their Dashboard configuration can safely remove the flag — it has no effect.
</Accordion>

<Accordion title='Added option to skip UDG schema update confirmations'>
Added a checkbox option to suppress confirmation modals when updating Universal Data Graph (UDG) schemas. When selected, the confirmation dialog will be skipped for the remainder of the browser session, allowing for faster iterative development while maintaining safety defaults for new sessions.
</Accordion>

<Accordion title='Added Configurable Toast Message Duration in Dashboard UI'>
Added the ability to configure the display duration for toast notifications in the Dashboard UI through the `ui.notifications.duration` configuration option. Previously, all toast messages displayed for a fixed 3-second duration, which could cause screen clutter when multiple notifications appeared simultaneously from batch operations.

You can now configure separate display durations for different message types:
- Success toast messages
- Warning toast messages
- Info toast messages
- Error toast messages

This provides better control over the user experience, allowing administrators to reduce visual noise from frequent success notifications while ensuring critical error messages remain visible for appropriate durations.
</Accordion>

<Accordion title='Added Configurable Session Object Management for Redis Memory Optimization'>
Added new session object management controls to the Dashboard UI and API to help prevent Redis memory bloat. This feature introduces two new configuration options that can be set on keys and policies:

- `post_expiry_action` - defines whether to delete or retain session objects when keys expire
- `post_expiry_grace_period` - sets the retention period beyond key expiry (supports "forever" when set to -1)

In the Dashboard UI, these options are available on the Configurations tab when creating or editing keys and policies. The interface uses progressive disclosure, showing expiry behavior options only when keys are configured to expire, with additional retention settings displayed when "Retain" is selected.

The feature maintains full backward compatibility with existing session lifetime configurations. If `post_expiry_action=retain` and `post_expiry_grace_period=0`, Tyk will continue using the legacy approach with `session_lifetime`, `global_session_lifetime`, and related Gateway configuration options.
</Accordion>

<Accordion title='Improved certificate ID visibility and consistency across Dashboard UI'>
Enhanced the display of certificate IDs throughout the Dashboard UI to improve usability when working with multiple certificates. Previously, certificate IDs were inconsistently truncated across different screens, often cutting off distinctive portions that help users identify and match certificates.

**What's Improved:**
- **Full ID display**: Certificate IDs are now shown in full whenever sufficient space is available
- **Smart truncation**: When space is limited, IDs are truncated from the left (e.g., "...bc4f:9a2d") to preserve the distinctive fingerprint portion on the right
- **Consistent behavior**: Standardized truncation rules applied across all certificate-related areas including the Certificate Store, API designers, Keys and Policies screens
- **Enhanced tooltips**: Full certificate IDs are always available on hover, even when truncated

**Areas Updated:**
- Certificate Store listing and management screens
- API-level certificate selection and configuration
- Keys and Policies certificate references
- All certificate dropdowns and selection components

This improvement reduces user errors when selecting certificates and makes certificate management more predictable across different areas of the Dashboard.
</Accordion>

<Accordion title='Added masking for sensitive configuration values with reveal toggle'>
Enhanced security by implementing default masking for sensitive configuration values throughout the Dashboard UI. Previously, secrets, passwords, and tokens were displayed in plain text, increasing the risk of accidental exposure through screen sharing, screenshots, or shoulder surfing.

**What's Protected:**
- **Authentication secrets**: HMAC shared secrets, JWT public keys, OAuth client secrets
- **Upstream authentication**: Basic auth passwords, OAuth client credentials and password grants
- **API tokens**: Dashboard API access tokens and validation sources
- **GraphQL data sources**: Kafka SASL passwords and other sensitive connection details

**How It Works:**
- All sensitive fields are **masked by default** using password-style input fields or dedicated masking components
- **Reveal/hide toggle** allows authorized users to view values when needed
- Values are **automatically re-masked** on page refresh for additional security
- Consistent behavior across both Classic APIs and OAS APIs in the API Designer

**Coverage:**
- API Designer authentication and upstream configuration screens
- Classic API and OAS API configuration forms
- Universal Data Graph (UDG) data source configurations
- Enhanced input components with built-in visibility controls

This improvement aligns with security best practices and helps organizations maintain better control over sensitive configuration data exposure.
</Accordion>
...
<Accordion title='Fixed OAS update failures due to duplicated parameter references during import'>
Resolved an issue where APIs imported from OpenAPI specifications with external parameter references would fail validation during subsequent update operations. Previously, when importing OAS files that referenced multiple distinct parameters from external files (e.g., `$ref: './parameters.yaml#/ID'`, `$ref: './parameters.yaml#/Name'`), the Dashboard's import process incorrectly collapsed all external references into a single generic parameter component and duplicated that reference multiple times, creating invalid OpenAPI specifications with multiple parameters having the same name and location.

The Dashboard now properly resolves external parameter references to distinct parameter definitions during import, ensuring that each `$ref` becomes a separate, uniquely-named parameter component and maintains OpenAPI 3.0 compliance for successful updates.
</Accordion>

<Accordion title='Added Strict Validation for Policy ID Characters'>
Resolved an issue where policy IDs containing special characters could cause problems when parsing API endpoint requests. Previously, policy IDs with characters such as `#`, `?`, `%`, and `/` would interfere with URL parsing in Dashboard and Gateway API endpoints that use the policy ID as a path parameter, potentially causing request failures or unexpected behavior.

Strict validation has been introduced across both the Dashboard and Gateway APIs to restrict policy identifiers to a safe character set (alphanumeric characters plus `_`, `-`, `.`, `~`). The validation occurs during policy creation and updates via the following endpoints:
...
<Accordion title='Fixed JWT Subject and Policy Claim Field Migration'>
Resolved an issue where JWT subject and policy claim fields were not being migrated when upgrading to newer versions of Tyk. Previously, when upgrading from v5.8.x to v5.10.x, existing JWT APIs would show empty values for the new `subjectClaims`, `basePolicyClaims`, and `jwksURIs` fields in the Dashboard UI, even though the legacy `identityBaseField` and `policyFieldName` values remained in the raw API definition. This caused validation errors when attempting to update APIs without making changes, requiring users to manually populate the `subjectClaims` field.

The migration logic has been refactored to properly include these extra fields during the upgrade process, ensuring existing `identityBaseField` values automatically populate the `subjectClaims` array and `policyFieldName` values populate `basePolicyClaims`. Users can now update APIs seamlessly without validation errors or manual field population.
</Accordion>

<Accordion title='Fixed API expiry drifting back one day on save in positive UTC timezones'>
Resolved an issue affecting both Tyk Classic APIs and Tyk OAS APIs where the API expiry could drift back by one day each time the API was saved in the API Designer for users in positive UTC timezones (such as UTC+8). Saving an API without changing the expiry is now fully idempotent for both API types, and the expiry value remains stable across repeated saves regardless of the user's timezone.
</Accordion>

<Accordion title='Improved Gateway registration handling at scale'>
Resolved an issue where mass Gateway registrations, such as a rolling upgrade of a large Gateway fleet, could fail to converge because the Dashboard serialised every registration request through a single shared lock. Gateways competing for the lock were rejected with a `409 Conflict` and could end up without the credentials needed to serve traffic. Registrations and deregistrations are now isolated per Gateway session, allowing Gateway fleets to register concurrently across multiple Dashboard instances without contention.
</Accordion>

<Accordion title='Reduced Redis load during Gateway registration on unlimited node licenses'>
Resolved an issue where Gateway registration could place excessive load on Redis, particularly during a rolling upgrade of a large Gateway fleet. Under load, this could block other Redis traffic and contribute to registration failures.

This fix addresses the issue for deployments using an **unlimited node license**: Gateway registration no longer performs the unnecessary Redis lookup, and Redis activity returns to baseline during registration storms. A dedicated fix will be provided for deployments using a **limited node license** in the upcoming release.
</Accordion>

<Accordion title='Fixed Gateway registration loop triggered by 409 Conflict during mass registration'>
Resolved an issue where Gateways could become permanently stuck in a registration loop after receiving a `409 Conflict` from the Dashboard during mass registrations or rolling upgrades. The Gateway treated the 409 as a successful registration, leaving heartbeat and policy synchronisation unable to authenticate and triggering an indefinite cycle of failed retries.

The Gateway now correctly distinguishes between a duplicate-session 409 (where it adopts the credentials returned by the Dashboard and proceeds normally) and a transient lock-contention 409 (where it retries with backoff). Gateways recover cleanly from any 409 condition during registration.
These changes eliminate the infinite recovery loop and ensure Gateways recover cleanly from any transient 409 condition during registration. </Accordion>
...
- <a href="https://cvereports.com/reports/GHSA-5c6j-r48x-rmvq" target="_blank">GHSA-5c6j-r48x-rmvq</a>
