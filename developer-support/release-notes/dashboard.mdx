...
In this release, . For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v5.12.0).
...
Please note that the Tyk Helm Charts are configured to install the LTS version of Tyk Dashboard. You will need to modify them to install v5.12.0.
...
<Accordion title='Enhanced Base API Management for Versioned Tyk OAS APIs'>
Improved the experience when managing versioned Tyk OAS APIs by adding flexible options for base API deletion and reassignment. When deleting a base API that has child versions, users are now presented with three clear options:
- Promote a child API to become the new base (automatically transferring versioning configuration and converting the original base to a child)
- Delete all associated child APIs together
- Leave child APIs orphaned (previous behavior)
The Dashboard now includes a new API endpoint to convert child APIs into base APIs, preserving all routing configurations and version identifiers while ensuring seamless traffic handling. Additionally, added a `purge` parameter to the delete API endpoint that allows bulk deletion of base APIs along with all their child versions. These enhancements eliminate the manual work previously required to manage orphaned APIs and provide administrators with clear, safe options for restructuring versioned API hierarchies without service disruption.
</Accordion>

<Accordion title='JSON Log Format Support for Dashboard'>
Added JSON log format option for the Tyk Dashboard to improve log processing and integration with monitoring tools. Users can now configure the Dashboard to output logs in structured JSON format instead of the default text format by setting the `TYK_LOGFORMAT` or `TYK_DB_LOGFORMAT` environment variables to "json", or by configuring `log_format: "json"` in the Dashboard configuration file. This feature provides consistency with the Gateway's JSON logging capabilities and enables better log ingestion by centralized logging systems, SIEM tools, and observability platforms. The configuration is case-insensitive and falls back to the standard text format for any invalid values, ensuring backward compatibility and reliable operation.
</Accordion>

<Accordion title='Full CRUD Operations Support for Custom Policy IDs in Dashboard API'>
Enhanced the Dashboard API to support complete CRUD operations (create, read, update, delete) using custom policy IDs in addition to existing database IDs. Users can now perform all policy management operations using the custom `id` field they assign to policies, providing more intuitive and consistent policy management workflows. The system includes configurable uniqueness enforcement for custom policy IDs within organizations through the new `EnforceUniquePolicyID` configuration option, preventing duplicate IDs and ensuring data integrity. API endpoints now prioritize custom ID lookups while maintaining full backward compatibility with database `_id` operations for existing integrations. When ambiguous states occur (multiple policies with the same custom ID), the system returns clear error messages with appropriate HTTP status codes. This enhancement streamlines policy management for users who prefer meaningful, custom identifiers over auto-generated database IDs, while ensuring existing workflows remain unaffected and providing administrators with flexible control over ID uniqueness enforcement.
</Accordion>

<Accordion title='API Definition Compression for Reduced Redis Storage Usage'>
Added configurable compression for API definitions stored in Redis to significantly reduce storage costs and improve performance for large-scale deployments. The Gateway now supports Zstd compression for API definition backups, achieving large size reduction for large Tyk OAS APIs with embedded OpenAPI specifications. This feature is particularly beneficial for enterprises with hundreds or thousands of APIs, where storage costs and Redis memory usage can become substantial. Compression is controlled via the new `storage.enable_api_definition_compression` configuration option (default: false) and operates during API reloads rather than in the request hot path, ensuring no impact on API response times. The implementation includes automatic format detection for backward compatibility, allowing mixed compressed and uncompressed data during upgrades.
</Accordion>

<Accordion title='Certificate-Token Binding Support in Dashboard API for Static mTLS'>
Added Dashboard API support for certificate-token binding with static mTLS authentication to enhance security and prevent cross-organizational credential misuse. The Dashboard now supports the new `mtls_static_certificate_bindings` field in key objects, allowing administrators to specify which client certificates can be used with specific access tokens. This field can be configured when creating or updating keys through all Dashboard API endpoints (policy-based keys, API-specific keys) and is properly persisted in Redis alongside session state data. The enhancement enables fine-grained control over certificate-token associations, preventing scenarios where one organization's token could be combined with another organization's certificate to gain unauthorized access. Multiple certificates can be bound to each key to support certificate rotation scenarios, and the feature maintains full backward compatibility with existing keys that don't specify certificate bindings. This Dashboard-level support complements the Gateway's certificate-token validation, providing a complete solution for secure static mTLS implementations in multi-tenant environments.
</Accordion>

<Accordion title='Enhanced Dashboard UI for API Management and Authentication'>
Improved the Dashboard user interface across multiple areas of API management and authentication:
- API Debug Panel Visualization: Enhanced the OAS API Designer's debug panel to clearly differentiate between request and response middleware processing stages. Added visual distinction for the Reverse Proxy step and implemented color-coded headers for request and response middleware sections. Users can now click on middleware steps to scroll to their position in the timeline, providing better visibility into the complete request processing flow.
- Base API Management for Versioned APIs: Added comprehensive UI support for managing versioned Tyk OAS APIs, including base API reassignment and intelligent deletion options. Users can now promote any child API to become the new base API directly from the versions table, with automatic transfer of versioning and routing configurations. When deleting a base API, the Dashboard presents three clear options: promote a child API to become the new base, delete all associated child APIs together, or leave child APIs as independent APIs.
- Certificate Authentication Method: Redesigned certificate authentication configuration by separating it from Auth Token authentication into a standalone method called "Certificate Authentication" (previously Dynamic mTLS). Updated both Tyk Classic and OAS API designers to include Certificate Authentication as an independent authentication option available for multi-auth configurations. Enhanced the key creation interface with clearer labeling, improved guidance text, and documentation links.
</Accordion>

<Accordion title='OpenAPI Specification 3.1 Support in Dashboard and Gateway'>
Added comprehensive support for OpenAPI Specification 3.1 across the Tyk platform, enabling users to work with the latest OpenAPI standard alongside existing 3.0 specifications:
- API Designer Integration: Enhanced the Dashboard's API Designer to automatically detect and support both OpenAPI 3.0.x and 3.1.x specifications. The raw API editor now provides correct syntax validation, highlighting, and autocomplete for both versions without requiring manual configuration. Version detection is automatic, and the UI functions identically regardless of OpenAPI version while maintaining full backward compatibility.
- Document Validation Engine: Implemented version-aware validation that automatically selects the appropriate validator based on the detected OpenAPI specification version. OpenAPI 3.0 documents continue using existing validation rules, while 3.1 documents leverage the JSON Schema 2020-12 validator to support new features, including type arrays with support, updated JSON Schema keywords, webhooks for callback operations, and SPDX license identifiers.
- Enhanced Numeric Validation: Added support for OpenAPI 3.1's updated `exclusiveMinimum` and `exclusiveMaximum` properties, which changed from boolean flags (3.0) to numeric values (3.1). The system now correctly handles both syntaxes: the legacy boolean format alongside minimum/maximum properties for 3.0 compatibility, and the direct numeric format for 3.1 specifications, with automatic precedence handling when both formats are present.
This comprehensive OpenAPI 3.1 support enables users to leverage modern API specification features while ensuring seamless backward compatibility with existing 3.0 APIs, providing a future-ready foundation for API design and management.
</Accordion>
...
Resolved an issue where the Dashboard's `/api/apis/{api_id}/keys` endpoint incorrectly returned all keys in the environment instead of only keys associated with the specified API. Users can now accurately retrieve and manage keys for individual APIs, eliminating confusion when working with multi-API environments. The fix ensures that only keys with explicit access rights to the requested API are returned, while maintaining performance safeguards to prevent resource exhaustion during large-scale key operations. This enhancement enables proper API-specific credential management and improves the accuracy of key administration workflows.
</Accordion>

<Accordion title='[UI] Fixed Certificate Authentication Configuration Issues for mTLS APIs and Policy ID Display'>
Resolved multiple Dashboard UI issues affecting mTLS configuration and policy management:
- Certificate Authentication Configuration: Fixed an issue where the Authentication tab incorrectly appeared when creating keys for APIs protected with Auth Token and static mTLS, forcing users to unnecessarily link client certificates. Additionally, resolved a configuration timing issue where enabling "Client Certificate" authentication after initial API creation failed to properly set required certificate flags (`auth.use_certificate` and `auth_configs.authToken.use_certificate`), preventing certificate sections from being editable in subsequently created keys. 
...
Resolved an issue where swapping certificates in multi-auth (mTLS + Basic auth) keys prevented the original certificate from being reused. Previously, when updating a key's certificate, the original certificate remained incorrectly associated with the key internally, causing "key with given certificate already found" errors when attempting to reuse that certificate. The Gateway now properly detaches certificates during key updates, allowing certificates to be freely reused across different keys after being removed from their original association.
</Accordion>

<Accordion title='Updated Expired License Modal with Correct Contact Information'>
Improved the expired license modal in the Dashboard to provide accurate guidance and working links for users with expired licenses. Previously, the "Renew License" button redirected to an outdated URL with no content, and the modal text incorrectly suggested users could get another free trial. The modal now displays a "Contact Tyk" button that directs users to the proper contact page, along with updated messaging that explains license expiration and directs users to reach out to Tyk or their Account Manager for next steps.
</Accordion>

<Accordion title='Monaco Editor JSON Schema Support'>
Fixed validation errors in the Monaco editor's JSON schema view when working with OpenAPI 3.1 documents. The editor now properly supports the new JSON Schema 2020-12 specification used by OpenAPI 3.1, eliminating false validation errors and providing accurate syntax highlighting for modern JSON schema features.
</Accordion>
...
In this release we've deprecated the [policies.allow_explicit_policy_id](/tyk-oss-gateway/configuration#policies-allow-explicit-policy-id) configuration option. This was previously added to allow the use of custom policy IDs, which is now the default behaviour so this option is redundant.
...
In this release we've deprecated the [allow_explicit_policy_id](/tyk-dashboard/configuration#allow-explicit-policy-id) configuration option. This was previously added to allow the use of custom policy IDs, which is now the default behaviour so this option is redundant.
