...
Tyk Dashboard 5.13.0 adds comprehensive management API and access control support for the MCP (Model Context Protocol) Gateway. A dedicated `/api/mcps` endpoint set provides full CRUD operations for MCP Proxy definitions.

A new `mcp` RBAC permission group (deny/read/write) controls access to MCP management independently of the existing `apis` permission. MCP Gateway is now generally available with no feature flag required. Visibility is controlled solely by the `mcp` RBAC permission.

Keys and policies can now carry four new MCP-specific access-right fields for configuring tool-based access control and per-primitive rate limiting.
...
**MCP Proxy policies visible in Developer Portal when running Portal versions earlier than v1.17.2**

Dashboard 5.13.0 introduces MCP Proxy definitions and the policies that grant access to them. If you are running the Developer Portal (EDP) at a version earlier than v1.17.2 alongside Dashboard 5.13.0, MCP Proxy policies may not be fully filtered from the developer catalogue. An administrator could inadvertently publish an MCP server to portal users.

Upgrade to **EDP v1.17.2** before or immediately after upgrading the Dashboard:

- **EDP v1.17.1** provides a partial mitigation: it filters MCP Proxy policies that have Tool-Based Access Control (TBAC) or per-primitive rate limiting configured. Basic MCP Proxy policies — those that grant API access without TBAC fields — are not filtered.
- **EDP v1.17.2** provides the complete fix: it detects MCP Proxy APIs by type and filters all MCP Proxy policies from the developer catalogue, regardless of whether TBAC is configured.
...
- **EDP v1.17.2**: filters all MCP Proxy policies from the developer catalogue; v1.17.1 provides a partial mitigation covering policies with TBAC or per-primitive rate limiting configured
</Note>
...
If your deployment uses MCP Gateway, additional component ordering applies. In hybrid deployments, MDCB 2.11.0 must be deployed before Gateway and Dashboard. Tyk Operator 1.4.0 must be deployed after Gateway and Dashboard are running. Portal 1.17.2 should be deployed before any MCP Proxy policies are created in Dashboard. See [MCP Gateway upgrade considerations](/developer-support/upgrading#mcp-upgrade-considerations) for the full sequence.
...
We have introduced the `/api/mcps` endpoint set, providing full CRUD operations for MCP Proxy API definitions:
...
<Accordion title='Add MCP access-right fields to keys and policies'>
...
<Accordion title='Add validation for unsupported middleware in MCP Proxy definitions'>
...
<Accordion title='Add Configurable Session Object Management for Redis Memory Optimization'>
We have added new session object management controls to the Dashboard UI and API to help prevent Redis memory bloat. This feature introduces two new configuration options that can be set on keys and policies:

- [post_expiry_action](/tyk-oss-gateway/configuration#post_expiry_action) - defines whether to delete or retain session objects when keys expire
- [post_expiry_grace_period](/tyk-oss-gateway/configuration#post_expiry_grace_period) - sets the retention period beyond key expiry (supports "forever" when set to -1)
...
The feature maintains full backward compatibility with existing session lifetime configurations. If [post_expiry_action](/tyk-oss-gateway/configuration#post_expiry_action) is set to `retain` and [post_expiry_grace_period](/tyk-oss-gateway/configuration#post_expiry_grace_period) is set to `0`, Tyk will continue using the legacy approach with [session_lifetime](/tyk-oss-gateway/configuration#session_lifetime), [global_session_lifetime](/tyk-oss-gateway/configuration#global_session_lifetime), and related Gateway configuration options.
</Accordion>
...
<Accordion title='Fix JavaScript Regex Unicode Escape Sequence Handling in OpenAPI Import'>
...
Previously, when regex patterns containing Unicode escape sequences (like `\u0000-\u017f`) were defined with single quotes in YAML, the YAML-to-JSON conversion process would double-escape the backslashes, transforming valid `\u` sequences into invalid `\\u` sequences that were rejected by Tyk's regex validator.
...
Previously, the filter only displayed certificates used for client-to-gateway static mTLS, ignoring certificates used for gateway-to-upstream mTLS, custom domain certificates, and certificate pinning. This resulted in incomplete and misleading results for users trying to view all certificates actively used by a given API.

The Dashboard now correctly returns all certificate types when filtering by API.
</Accordion>

<Accordion title='Fix OAS update failures due to duplicated parameter references during import'>
We have resolved an issue where APIs imported from OpenAPI specifications with external parameter references would fail validation during subsequent update operations.
...
<Accordion title='Fix JWT Subject and Policy Claim Field Migration'>
We have resolved an issue where JWT subject and policy claim fields were not being migrated when upgrading to newer versions of Tyk.

Previously, when upgrading from v5.8.x to v5.10.x, existing JWT APIs would show empty values for the new `subjectClaims`, `basePolicyClaims`, and `jwksURIs` fields in the Dashboard UI, even though the legacy `identityBaseField` and `policyFieldName` values remained in the raw API definition. This caused validation errors when attempting to update APIs without making changes, requiring users to manually populate the `subjectClaims` field.
