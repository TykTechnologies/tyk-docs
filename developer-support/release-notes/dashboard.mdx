...
**MCP Proxy policies visible in Developer Portal when running Portal versions earlier than v1.17.1**

Dashboard 5.13.0 introduces MCP Proxy definitions and the policies that grant access to them. If you are running the Developer Portal (EDP) at a version earlier than v1.17.1 alongside Dashboard 5.13.0, MCP Proxy policies are not filtered from the developer catalogue. An administrator could inadvertently publish an MCP server to portal users.
...
If your deployment uses MCP Gateway, additional component ordering applies. In hybrid deployments, MDCB 2.11.0 must be deployed before Gateway and Dashboard. Tyk Operator 1.4.0 must be deployed after Gateway and Dashboard are running. Portal 1.17.1 should be deployed before any MCP Proxy policies are created in Dashboard. See [MCP Gateway upgrade considerations](/developer-support/upgrading#mcp-upgrade-considerations) for the full sequence.
