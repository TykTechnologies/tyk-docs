...
The Dashboard API has been extended with:
- the `PATCH /api/apis/oas/{current-base-api-id}/{next-base-api-id}` endpoint that converts a child API into the base API
- an optional `purge` parameter to the `DELETE /api/apis/oas/{apiId}` endpoint that enables bulk deletion of all child APIs when the base API is deleted.
