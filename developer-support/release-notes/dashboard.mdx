...
Added optional compression for API definitions stored in Redis to significantly reduce storage costs and improve performance for distributed deployments. Up to 75% compression ratios have been observed with Tyk OAS APIs.

The new `storage.enable_api_definition_compression` configuration option will apply compression to API definitions when they are stored in the Data Plane Redis cache. This does not take place in the hot path (during request handling) so has no impact on API response times.

This feature is only applicable to distributed deployments using MDCB and the default is `false`.
</Accordion>
