...
Several critical stability issues have been resolved in this release, including fixes for JavaScript regex handling with Unicode patterns in OpenAPI specifications, Dashboard certificate filtering, and JWT claim migration during version upgrades. The release introduces enhanced Redis session management with configurable cleanup options and expiry behavior to improve memory efficiency. New MCP analytics capabilities have been added with a dedicated Dashboard page and supporting API endpoints. Additional Dashboard improvements include Go 1.25 support, configurable toast message duration, and an optional suppression of GraphQL schema update confirmation modals.

#### Breaking Changes
There are no breaking changes in this release.

#### Dependencies {#dependencies-5.13.0}
...
#### Upgrade instructions {#upgrade-5.13.0}
...
- [Docker image to pull](https://hub.docker.com/r/tykio/tyk-dashboard/tags?page=&page_size=&ordering=&name=v5.13.0)
  - ```bash
    ...
    ``` 
- Helm charts
  - [tyk-charts]({{<ref "developer-support/release-notes/helm-chart" >}})

#### Changelog {#Changelog-v5.13.0} 
...
<li>
<details>
<summary>Improved management of session objects and keys in Redis</summary>

Enhanced Redis storage management to prevent unlimited growth of expired keys and session objects. Tyk now provides better handling of session object lifecycle and TTL management, eliminating the need for manual scripts to clean up unnecessary entries. This improvement ensures more efficient Redis storage utilization and provides an enterprise-grade solution for managing session data persistence.
</details>
</li>
<li>
<details>
<summary>Added option to suppress GraphQL schema update confirmation modal</summary>

Added a checkbox option in the GraphQL schema update confirmation modal that allows users to skip confirmation dialogs for the remainder of their browser session. When users select this option before confirming their first schema update, subsequent "Save and Update API" actions will proceed automatically without showing the confirmation modal. The confirmation requirement resets when starting a new browser session, maintaining safety for potentially destructive operations while improving workflow efficiency for experienced users.
</details>
</li>
<li>
<details>
<summary>Updated Tyk Dashboard to Go 1.25</summary>

Updated the Tyk Dashboard to use Go 1.25, including modifications to CI pipelines and build configurations. This upgrade maintains security best practices by staying current with Go versions and reduces exposure to potential vulnerabilities. The update applies to both feature branches and the LTS 5.3 branch.
</details>
</li>
<li>
<details>
<summary>Added configurable toast message duration in the Dashboard UI</summary>

Implemented configurable duration settings for toast notification messages that appear in the Dashboard UI. Users can now adjust how long success messages and other notifications remain visible, addressing scenarios where multiple toast messages would fill up the screen. This enhancement provides better control over the notification experience and reduces visual clutter during operations that generate multiple responses.
</details>
</li>
<li>
<details>
<summary>Added dedicated MCP analytics page to the dashboard</summary>

Added a new "Activity by MCP" analytics page that provides API owners with comprehensive visibility into MCP (Message Control Protocol) primitive usage, performance, and error metrics. The page features seven analytical charts and an interactive filter bar, allowing users to monitor and analyze MCP-specific API activity. This dedicated view complements existing REST and GraphQL analytics pages and is automatically available to users with appropriate MCP permissions when the MCP feature is enabled.
</details>
</li>
<li>
<details>
<summary>Added new MCP analytics API endpoints to the dashboard backend</summary>

Added four new API endpoints to the dashboard backend for querying Model Context Protocol (MCP) analytics data. These endpoints enable users to access MCP traffic data stored by the pump, covering overall MCP traffic by API, activity by specific primitive, activity by consumer key, and error breakdowns by primitive. The endpoints follow the same response format as existing REST and GraphQL analytics endpoints, making MCP analytics data accessible through the dashboard interface.
</details>
</li>
<li>
<details>
<summary>Added configurable session cleanup options for Redis memory management</summary>

Added new Dashboard configuration options for managing expired session objects in Redis to address memory bloat issues. Users can now configure post-expiry behavior (delete or retain keys) and set grace periods for key retention, including permanent retention if needed. These settings provide better control over Redis memory usage while maintaining full backward compatibility with existing session lifetime configurations.
</details>
</li>
<li>
<details>
<summary>Added improved management of session objects and keys in Redis with configurable expiry behavior</summary>

Added new Dashboard backend support for enhanced Redis session management through two configurable fields: post_expiry_action (delete or retain expired keys) and post_expiry_grace_period (retention period beyond expiry). These improvements provide more intuitive control over session object lifecycle while maintaining full backward compatibility with existing session lifetime configurations.
</details>
</li>
</ul>
...
<li>
<details>
<summary>Fixed JavaScript regex issue with Unicode patterns in OpenAPI specifications</summary>

Resolved a bug where JavaScript-style Unicode regex patterns in OpenAPI specifications failed validation due to incompatibility with Tyk's internal Go RE2 engine. The issue occurred when Unicode escape sequences were double-escaped during YAML-to-JSON conversion, causing valid patterns like `^[\u0000-\u017f]*$` to become invalid. Tyk now automatically translates these escape sequences during API ingestion to ensure proper regex validation.
</details>
</li>
<li>
<details>
<summary>Fixed Dashboard filter to return all certificates linked to an API</summary>

Resolved an issue where the Dashboard's "Search by API" filter on the TLS/SSL Certificates page only displayed certificates used for client-to-gateway mTLS, ignoring certificates used for gateway-to-upstream mTLS. The fix introduces a dedicated endpoint to accurately retrieve all certificates associated with a given API, providing users with complete visibility into all certificates actively used by their APIs.
</details>
</li>
<li>
<details>
<summary>Fixed OAS update failures caused by duplicated parameter references during Dashboard import</summary>

Resolved a bug in the Dashboard's OpenAPI import process that incorrectly transformed distinct external parameter references into duplicated identical references. This caused APIs to import successfully but fail validation during subsequent update operations. The import logic now properly preserves distinct parameter references, preventing the creation of invalid OpenAPI specifications with duplicate parameter names and locations.
</details>
</li>
<li>
<details>
<summary>Fixed policy ID validation to prevent use of special characters that break API endpoints</summary>

Resolved an issue where policy IDs containing special characters like #, ?, %, and / would cause problems when used in Dashboard and Gateway API endpoint URLs. Strict validation has been implemented across both Dashboard and Gateway APIs to restrict policy identifiers to a safe character set, preventing URL parsing errors and ensuring reliable API functionality.
</details>
</li>
<li>
<details>
<summary>Fixed JWT subject and policy claim fields not migrating when upgrading Tyk versions</summary>

Resolved an issue where JWT authentication configurations were not properly migrated when upgrading from Tyk v5.8.x to v5.10.x. The upgrade process failed to migrate existing identityBaseField and policyFieldName values to the new subjectClaims and basePolicyClaims array format, leaving these fields empty in the Dashboard UI despite being properly configured in the backend. Users can now upgrade seamlessly without losing their JWT authentication settings or encountering validation errors.
</details>
</li>
</ul>
