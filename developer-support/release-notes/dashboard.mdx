...
<Accordion title='Change the Tyk OAS Base API Version'>
We have improved the experience when managing versioned Tyk OAS APIs by adding the ability to promote a child API version to become the base (routing API. The existing base API will be reassigned as a child version.

When deleting a base API from the API Designer, users are now presented with three clear options:
