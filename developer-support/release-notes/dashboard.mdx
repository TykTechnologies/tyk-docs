...
To provide an additional layer of security for your APIs, we've introduced Dashboard support for [Client Certificate-Token Binding](/api-management/authentication/auth-token#client-certificate-token-binding). This feature allows you to form a strict binding association between an Auth Token issued to a client and their specific client certificate.

By ensuring that a token can only be used in conjunction with its bound certificate, you can significantly reduce the risk of token theft or misuse. You can now easily manage these bindings directly from the Dashboard when creating or modifying keys, with full support for certificate rotation scenarios by allowing multiple certificates to be bound to a single key.
...
In this release, we are delighted to bring foundational observability capabilities that dramatically reduce troubleshooting time and improve operational visibility for API teams. These new capabilities include enhanced log correlation, detailed error context, and deployment verification capabilities.
With these capabilities, teams can now diagnose and resolve issues in minutes instead of hours.
...
<Accordion title='Added basic support for OpenAPI 3.1 (OAS v3.1.x)'>
