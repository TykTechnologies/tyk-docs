...
The Dashboard now includes a new API endpoint to convert child APIs into base APIs, preserving all routing configurations and version identifiers while ensuring seamless traffic handling. Additionally, added a `purge` parameter to the delete API endpoint that enables bulk deletion of base APIs and all their child versions.

These enhancements eliminate the manual work previously required to manage orphaned APIs and provide administrators with clear, safe options for restructuring versioned API hierarchies without service disruption.
</Accordion>

<Accordion title='JSON Log Format Support for Dashboard'>
Added JSON log format option for the Tyk Dashboard to improve log processing and integration with monitoring tools. Users can now configure the Dashboard to output logs in structured JSON format instead of the default text format by setting the `TYK_LOGFORMAT` or `TYK_DB_LOGFORMAT` environment variables to "json", or by configuring `log_format: "json"` in the Dashboard configuration file. 

This feature provides consistency with the Gateway's JSON logging capabilities and enables better log ingestion by centralized logging systems, SIEM tools, and observability platforms. The configuration is case-insensitive and falls back to the standard text format for any invalid values, ensuring backward compatibility and reliable operation.
</Accordion>
...
<Accordion title='Multiple high-severity CVEs have been fixed'> </Accordion>
