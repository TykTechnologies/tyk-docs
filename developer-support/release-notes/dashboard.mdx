...
This release fixes several Dashboard issues affecting user and API key management under load, corrects a Host header problem that broke the API Debugger behind strict service meshes like Istio, adds Dashboard-side validation to prevent APIs with an invalid Listen Path from silently failing to load on the Gateway, masks additional secrets in the Dashboard UI, and improves performance on policy updates involving many Tyk OAS APIs. This release also migrates off an unmaintained security library and updates the underlying Go toolchain to Go 1.26. This release also addresses several CVEs in dependent libraries.
