...
Two new fields have been added to the Session object, which can be directly configured when creating Sessions using the [Keys API](/api-reference/keys/create-custom-key) or [Policy API](/api-reference/policies/create-policy-definition), or from the key/policy management screens in the Dashboard UI:
