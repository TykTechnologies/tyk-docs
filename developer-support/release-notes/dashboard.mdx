...
This patch upgrades Tyk Dashboard to Go 1.24 and addresses vulnerabilities in dependent libraries that were present in 5.3.12. For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v5.3.13) below.
...
| Dashboard Version | Recommended Releases | Backwards Compatibility |
| :---- | :---- | :---- |
| 5.3.13 | MDCB v2.8.8     | MDCB v2.8.0 |
|         | Operator v1.2.0  | Operator v0.17 |
|         | Sync v2.1.5    | Sync v2.1.0 |
|         | Helm Chart v5.0  | Helm all versions |
| | EDP v1.16 | EDP all versions |
| | Pump v1.13.3| Pump all versions |
| | TIB (if using standalone) v1.7.0 | TIB all versions |
...
<Accordion title='Updated Tyk Dashboard to Go 1.24'>
Updated the Tyk Dashboard to use Go 1.24, reducing exposure to potential security vulnerabilities in older versions.
</Accordion>
...
<Accordion title='Resolved Gateway registration loop during upgrades'>
Fixed an issue where Gateways could enter an infinite registration loop during rolling upgrades, which caused them to enter a degraded state and fail to synchronize policies. The registration process has been stabilized to ensure Gateways successfully connect to the Dashboard without causing excessive load.
</Accordion>

<Accordion title='Fixed Gateway handling of registration conflicts'>
Resolved an issue where Gateways incorrectly handled conflict responses during mass registrations, leading to missing authentication nonces and subsequent connection failures. Gateways now properly process these responses and recover their state correctly.
</Accordion>

<Accordion title='Resolved Redis performance degradation during Gateway registration'>
Fixed a performance issue where Gateway registrations could cause significant latency spikes in Redis. The registration process has been optimized to eliminate blocking operations, ensuring stable performance even during mass Gateway deployments or upgrades.
</Accordion>
...
- <a href="https://github.com/advisories/GHSA-hg79-j56m-fxgv" target="_blank">GHSA-hg79-j56m-fxgv</a>
