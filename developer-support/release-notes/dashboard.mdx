...
|        | Helm Chart v4.2   | Helm all versions |
...
- [Docker Image to pull](https://hub.docker.com/r/tykio/tyk-dashboard/tags?page=&page_size=&ordering=&name=v5.11.0)
  - ```bash
    ...
    ```
- Helm charts
  - [tyk-charts v4.2.0](/developer-support/release-notes/helm-chart#420-release-notes)
...
<AccordionGroup>
<Accordion title='Storage library v1.3.0 Update Across All Tyk Components'>
Updated the storage library to v1.3.0 across all Tyk components (Gateway, Dashboard, MDCB, Tyk Pump, and Portal) to include MongoDB connection fixes and foundational SQL support. All components now benefit from improved connection management and enhanced storage layer reliability.
</Accordion>
...
Introduced a new Dashboard API endpoint `GET /certs/{certificateId}/apis` that efficiently retrieves all APIs linked to a specific certificate. The endpoint returns API names, IDs, and usage types (client, upstream, server) with proper pagination support, enabling the frontend to display certificate impact information without expensive client-side filtering across all APIs.
</Accordion>
...
- **Improve API Designer UI to clearly display versioned API URLs and access methods**: The Tyk OAS API Designer now displays additional information for the base path URLs where published APIs can be accessed, including versioning information, custom domains, and distributed data plane gateways (when declared in the Dashboard's `edge_endpoints` configuration). To accompany and support this enhanced visibility, Tyk will now add all base paths to the `servers` section in the OpenAPI description. We have introduced a new Dashboard API endpoint (`GET /api/apis/oas/{apiID}/urls`) that returns the list of base path URLs extracted from the Tyk OAS API definition presented in a structured format including detailed components (protocol, domain, port, listen-path).
...
<Accordion title='Add Trace ID to Traffic Logs for Improved Observability'>
...
<Accordion title='Added support for certificate-to-token binding with Auth Token authentication when using static mTLS'>
When enabled via the new `enable_token_certificate_binding` Gateway configuration, Tyk will verify that the mTLS certificate presented with a request matches the certificate registered for the access token. This prevents security vulnerabilities where tokens from one organization could be used with certificates from another organization. Multiple certificates can be registered per token, and the feature is backward compatible (disabled by default).
</Accordion>
...
<Accordion title='Fixed API Filter Preservation and Data Graph Onboarding Display Issues'>
Resolved filtering issues in the Dashboard where applying multiple filters (name + category) would drop previously applied filters instead of combining them. Refactored the filter state management to preserve all active filters across interactions, pagination, and sorting operations. Fixed the Data Graph onboarding wizard to only appear when no UDG APIs exist in the system, rather than incorrectly displaying when filters return zero results. Enhanced filter handling with centralized utilities for building GraphQL variables and URL parameters, ensuring filters remain visible and clearable even when results are empty. All changes maintain backward compatibility and existing GraphQL contracts.
</Accordion>

<Accordion title='Fix Certificate Search to Query Full Certificate Store in OAS API Designer'>
Fixed an issue in the Tyk OAS API Designer when selecting a TLS certificate to attach to the API (for example, a client certificate for client-gateway mTLS). To improve performance when scrolling through the list of existing certificates, the UI uses pagination to load a subset of certificates at a time. The certificate ID search would only match against the currently loaded page of certificates, so a certificate might not appear to exist when it does, but just hasn't been loaded yet. Now the search functionality has been improved to look through the full list of certificates in the Tyk Certificate Store, rather than relying on the paginated data.
</Accordion>

<Accordion title='Dashboard UI Fixes'>
- **Fixed Incorrect Data Range Display in Total API Traffic Dashboard Chart**: Fixed an issue where the **Total API Traffic** chart on the **Nodes & Licenses** screen displays data from the wrong time period.
- **Resolved Issue Where TIB User Role Mappings Disappeared on Update**: Fixed a bug that caused TIB profiles to vanish on updates made to `Provider Configuration` advanced settings.
- **Fix API Ownership User Selection from Filtered Dropdown in OAS APIs**: Fixed an issue where users and categories could not be selected from filtered dropdown lists in the API ownership section of OAS APIs.
</Accordion>

<Accordion title='Fix Dashboard Proxy Endpoint 500 Error in Quick Start API Testing'>
Fixed a 500 error that occurred when testing APIs in the Quick Start onboarding flow. The issue was caused by the dashboard proxy endpoint not handling edge endpoints configured without URL schemas (e.g., "gateway.example.com" instead of "https://gateway.example.com"). The proxy now automatically sanitizes and validates URLs to ensure successful API testing regardless of whether the `TYK_DB_EDGEENDPOINTS` environment variable includes a schema.
</Accordion>

<Accordion title='Fix Non-JSON Response Display in Quick Start API Testing'>
Fixed an issue in the Quick Start onboarding flow where non-JSON responses would cause a 500 error or fail to display in the "Test your API" step. The dashboard proxy endpoint now properly handles all response content types, and the UI correctly displays both JSON and non-JSON responses in the response viewer without getting stuck on previous responses.
</Accordion>
...
- [Docker Image to pull](https://hub.docker.com/r/tykio/tyk-dashboard/tags?page=&page_size=&ordering=&name=v5.10.2)
  - ```bash
    ...
    ```
- Helm charts
  - [tyk-charts v4.1.0](/developer-support/release-notes/helm-chart#410-release-notes)
