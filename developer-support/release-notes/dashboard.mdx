...
<Accordion title='Strict enforcement of TYK_DB_FORCEAPIDEFAULTS'>
{/* DX-2394 */}
In Tyk 5.14, the API management logic was unified so that all API creations and updates (including those managed by the Tyk Operator) pass through central functions. As a result, if `force_api_defaults` (`TYK_DB_FORCEAPIDEFAULTS`) is enabled, the Dashboard strictly enforces default settings globally.

This means `strip_listen_path` is forcibly set to `true` on all APIs, overriding any `strip_listen_path: false` values sent in the payload (e.g., via ApiDefinition CRDs).

To disable this behavior and allow custom settings like `strip_listen_path: false` to be respected, set `TYK_DB_FORCEAPIDEFAULTS=false`.
</Accordion>
