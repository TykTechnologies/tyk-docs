...
This release ensures that the embedded Identity Broker honours the Tyk Dashboard's configured log verbosity, so TIB log output is consistent with the rest of the Dashboard's application logs. We have also addressed CVEs to strengthen security.
