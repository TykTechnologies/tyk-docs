...
keywords: "Tyk Identity Broker, Release notes, changelog, v1.6, v1.6.1, v1.7.1"
...
In this patch, we have ensured the embedded Identity Broker honors the Tyk Dashboard's configured log verbosity, so TIB log output stays consistent with the rest of the Dashboard's application logs. We have also addressed several CVEs to strengthen security.
...
<Accordion title='Honor the Tyk Dashboard log level when running embedded'>
When running embedded in the Tyk Dashboard, the Identity Broker now honors the Dashboard's configured application log verbosity.

Previously, TIB emitted its log output at a fixed verbosity regardless of the Dashboard setting, so TIB messages could appear even when the Dashboard was configured to a less verbose level.

TIB log output now respects the Dashboard's log level, including the new `TYK_DB_LOGLEVEL` environment variable and the global `TYK_LOGLEVEL`, keeping all Dashboard logs consistent.
</Accordion>
