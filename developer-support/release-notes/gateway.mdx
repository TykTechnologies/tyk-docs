...
Also, it was possible to [create a custom key](/api-reference/gateway/keys/create-custom-key-import-key) shorter than the configured minimum length. The Session would be created, but the key would be unusable as any attempt to use it in a request would be rejected due to the token's length.
...
Also, it was possible to [create a custom key](https://tyk.io/docs/api-reference/gateway/keys/create-custom-key-import-key) shorter than the configured minimum length. The Session would be created, but the key would be unusable as any attempt to use it in a request would be rejected due to the token's length.
...
Two new fields have been added to the Session object, which can be directly configured when creating Sessions using the [Keys API](https://tyk.io/docs/api-reference/gateway/keys/create-a-key) or [Policy API](https://tyk.io/docs/api-reference/gateway/policies/create-a-policy):
