...
In this release, we have fixed a memory leak in the gateway that could occur when using JWT authentication, and we have resolved a performance issue with bundle verification that significantly impacted resource consumption when using plugin bundles. For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v5.11.1) below.
...
<Accordion title='Fixed Memory Leak in Gateway When Using JWKS URL Cache'>
Resolved a memory leak issue in the Gateway that could occur when APIs used JWT authentication with JWKS URL cache.
</Accordion>
...
Additionally, introduced a new Gateway configuration option `skip_verify_existing_plugin_bundle` that allows you to skip cryptographic verification when loading signed plugin bundles from disk. When set to `true`, this option reduces the performance overhead for environments with large numbers of APIs using signed bundles, while still maintaining security by validating signatures during the initial bundle download.
