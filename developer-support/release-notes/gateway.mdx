...
A change has been made to improve the security of the legacy **Auth Token + Dynamic mTLS** method for securing access to APIs deployed on Tyk. This removes the option to authenticate using only the auth token and enforces the mTLS handshake.
...
<Note>
Tyk Helm Charts are configured to install the LTS version of Tyk Gateway. You will need to modify them to install v5.12.0.
</Note>
