...
Resolved an issue where a Tyk Gateway acting as a client (using upstream mTLS) would fail to authenticate against another mTLS-protected Tyk Gateway or upstream server, resulting in `HTTP 403 Forbidden: "Client TLS certificate is required"` errors. Previously, the gateway's TLS client could withhold the configured certificate if the target server's requested Certificate Authorities did not perfectly match the certificate's issuer chain.
...
<Accordion title='Fixed Default Gateway TLS Version Regression to Restore TLS 1.3'>
Resolved a regression introduced in version 5.8 where the Gateway's default maximum TLS version was incorrectly set to TLS 1.2 instead of TLS 1.3. Previously, users upgrading from version 5.7 to 5.8+ experienced unexpected TLS version limitations that required manual configuration to restore TLS 1.3 support.

The Gateway now follows Go's native TLS defaults (TLS 1.2 minimum, TLS 1.3 maximum), restoring the previous behavior and aligning with industry security standards. This maintains full backward compatibility for existing deployments that explicitly configure TLS versions.

If you need to limit the maximum TLS version to 1.2, you must now explicitly set `TYK_GW_HTTPSERVEROPTIONS_MAXVERSION` for client-to-Gateway connections or `TYK_GW_PROXYSSLMAXVERSION` for Gateway-to-upstream connections.
</Accordion>
...
The Gateway now properly recognizes CORS preflight requests and allows them to bypass AllowList middleware checks when Tyk is configured to handle OPTIONS internally, restoring the expected behavior where CORS preflight handling works automatically without requiring explicit OPTIONS endpoint definitions.
</Accordion>
...
Tyk now correctly generates analytics for OAS API mock endpoints while maintaining the existing behavior for Classic APIs, which intentionally do not generate analytics for mock endpoints as documented.
</Accordion>

<Accordion title='Fixed OpenTelemetry Configuration File Settings Being Ignored'>
Resolved an issue where OpenTelemetry settings in the Gateway configuration file were not being applied. Previously, when users configured OpenTelemetry via the `opentelemetry` section in the Gateway config file (including `enabled`, `exporter`, and `endpoint` fields), these settings were ignored and only environment variables like `TYK_GW_OPENTELEMETRY_ENABLED` would take effect. The Gateway now properly reads and applies OpenTelemetry configuration from the config file as documented.
</Accordion>
