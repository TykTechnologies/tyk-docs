...
{/* TT-16039 */}
We've resolved an issue in Upstream OAuth, where Tyk acts as the OAuth client to an upstream identity provider, that sent a redundant, always-failing token request when the identity provider required client credentials in the request body rather than in a header. This removed the extra round trip and the associated log noise generated every time an upstream token was obtained.
</Accordion>
