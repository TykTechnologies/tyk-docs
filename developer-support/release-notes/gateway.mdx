...
<Accordion title='Fixed cross-user JWT and response leakage in federated GraphQL supergraphs'>
{/* TT-17921 */}
Resolved an issue affecting federated GraphQL supergraphs configured with per-request auth propagation (`strip_auth_data: false`). Under certain conditions, the Gateway could apply one user's JWT to a subgraph request made on behalf of a different user, and identical GraphQL queries from different users could receive each other's cached HTTP responses. Authorization context is now strictly isolated per caller: each request's JWT is resolved and applied at request time instead of being cached with the execution plan, and response caching/deduplication is now partitioned by the caller's authorization context so responses are never shared across users. If you run federated GraphQL supergraphs with `strip_auth_data: false`, we strongly recommend upgrading as soon as possible.
</Accordion>
