...
Tyk now correctly generates analytics for OAS API mock endpoints while maintaining the existing behavior for Classic APIs, which intentionally do not generate analytics for mock endpoints, as it is documented.
</Accordion>
...
The Gateway now properly reads and applies OpenTelemetry configuration from the config file as documented.
</Accordion>
