...
A new next-generation (`-ng`) [plugin compiler](/api-management/plugins/plugin-compiler-ng) is now available, as an alternative to the existing compiler, with a significantly reduced the OS-level CVE surface compared to the current compiler.
