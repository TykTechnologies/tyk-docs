...
Added configuration inspection endpoints to the Gateway Control API to enable troubleshooting and configuration auditing. The new `/config` and `/env` endpoints provide programmatic access to the Gateway's actual runtime configuration, including environment variable mappings and resolved values from multiple configuration sources (file, environment variables, defaults). Platform engineers and support teams can now quickly verify configuration settings, debug deployment issues, and audit security configurations through authenticated API calls. The feature includes sensitive data protection, automatically redacting passwords, secrets, and connection strings while preserving configuration structure visibility. Both full configuration dump and single-field queries are supported, which help you clarify configuration precedence, naming conventions, and targeted troubleshooting (e.g., `/config?field=storage.host`). This enhancement could be used to significantly reduce support friction by eliminating manual configuration file sharing and supports automated configuration drift detection for enterprise deployments. The feature is disabled by default and requires explicit enablement via `enable_config_inspection: true` with proper authentication using the Gateway's Control API secret.
</Accordion>

<Accordion title='Support Enhanced MDCB `/dataplane` Endpoint with Information on Loaded APIs and Policies'>
Gateway is updated to support the enhanced MDCB `/dataplanes` endpoint. The enhanced MDCB endpoint will provide visibility into your Tyk deployment. This now returns a detailed list of the APIs and policies loaded on each connected data plane gateway. You can retrieve the results for a specific Gateway using the `node_id` query parameter. This update provides a clear picture of what is running on each Gateway, simplifying monitoring and troubleshooting of your deployed Data Planes.
</Accordion>
