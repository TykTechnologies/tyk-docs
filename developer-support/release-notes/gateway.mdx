...
A new next-generation (`-ng`) [plugin compiler](/api-management/plugins/golang/compiling-a-plugin) is now available, as an alternative to the existing compiler, with a significantly reduced the OS-level CVE surface compared to the current compiler.

This compiler adds [native linux/arm64 support](/api-management/plugins/golang/compiling-a-plugin#native-arm64-support). Plugins built on Apple Silicon or other arm64 hosts no longer need to run the compiler under emulation.
...
For more details see [Next-Generation Plugin Compiler](/api-management/plugins/golang/compiling-a-plugin) and [migrating from the existing compiler](/api-management/plugins/golang/compiling-a-plugin#migrating-from-the-existing-compiler).
</Accordion>
