...
Added JWKS cache invalidation endpoint `DELETE /api/cache/jwks/{apiID}` to the Dashboard API to flush JWKS cache for specific APIs across all connected Gateways. This extends the existing Gateway-level JWKS cache flush functionality to work through the Dashboard API with proper MDCB propagation to data plane Gateways.
</Accordion>
