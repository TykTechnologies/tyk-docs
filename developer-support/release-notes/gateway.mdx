...
- Tyk OAS APIs can be configured as [MCP proxies](/ai-management/mcps/api-to-mcp), giving each API a catalogue entry and [configuration foundation](/ai-management/mcp-gateway/api-to-mcp-definitions) that describes it as a set of MCP tools, resources, and prompts.
...
RFC 8693 Token Exchange now supports the [Microsoft Entra On-Behalf-Of (OBO)](/api-management/authentication/token-exchange-microsoft-entra-obo) grant as a token exchange provider, allowing Tyk Gateway to exchange an inbound token for one scoped to a downstream Microsoft Entra ID-protected resource.
</Accordion>

<Accordion title='Added observability for Token Exchange'>
{/* TT-17186 */}

Token Exchange now emits additional [observability data](/api-management/authentication/token-exchange#observability) covering exchange attempts, giving operators visibility into token exchange activity and failures.
</Accordion>
...
{/* TT-6176 */}
Custom analytics plugins (Go plugins that modify analytics records before they leave the Gateway) can now be packaged and distributed as part of a plugin bundle, instead of requiring a separate file path on every Gateway. Add a [`traffic_logs` section](/api-management/plugins/plugin-types#loading-from-a-bundle) under `custom_middleware` in the plugin bundle manifest to include an analytics plugin alongside your other custom middleware. Loading an analytics plugin from a direct file path continues to work as before.
</Accordion>

<Accordion title='Enhancements to custom JavaScript plugins'>
{/* TT-16948, TT-17761 */}
A new Goja-based [JavaScript plugin engine](/api-management/plugins/javascript#choosing-a-javascript-engine) is available by selecting the `javascript` driver, significantly improving performance.
...
{/* TT-17611 */}
Tyk Gateway can now authenticate to Google Cloud Memorystore for Redis/Valkey using [GCP IAM-based authentication](/tyk-configuration-reference/redis-cluster-sentinel#iam-authentication), as an alternative to a static Redis password.
...
The IAM configuration is set in the new [`iam_auth` section](/tyk-configuration-reference/redis-cluster-sentinel#iam-authentication) of Gateway's storage config.
</Accordion>
...
We have added the ability to choose the [client authentication method](/api-management/upstream-authentication/oauth#authentication-method) that Tyk should use when requesting the token with the new `method` field in the Upstream Authentication config:
