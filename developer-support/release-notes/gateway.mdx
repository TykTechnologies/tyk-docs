...
Tyk 5.13.0 delivers comprehensive observability, enhanced error handling, and operational excellence, empowering teams to achieve complete visibility into their API ecosystem while streamlining API operations and troubleshooting.

**Complete Observability with OpenTelemetry Metrics**
This release introduces comprehensive real-time metrics export through OpenTelemetry, completing Tyk's observability stack alongside existing distributed tracing and logging. Teams now have complete operational visibility with automatic RED metrics (Rate, Errors, Duration), Go runtime health monitoring, and configuration state tracking.
...
**Centralized Error Response Customization**
...
Configure error overrides at multiple levels - globally, per API, or per specific endpoint - with flexible matching by status codes, error types, message patterns, or response content. Support for dynamic templates with validation context enables RFC 7807 compliance and actionable error feedback, while seamless integration with existing analytics and logging ensures complete observability of overridden responses.
Enhanced Developer Experience & Operational Excellence
This release brings significant improvements to day-to-day operations with configurable rate limit headers that now correctly display rate limit information (rather than quota data) in HTTP 429 responses, addressing industry standard compliance. Enhanced session management provides automatic cleanup of expired Redis objects, while TLS certificate rotation support eliminates the need for Gateway restarts during routine certificate updates.
Additional quality-of-life improvements include underscore support in Vault/Consul secret paths, configurable API compression limits, and numerous stability fixes addressing CORS preflight handling, analytics generation for OAS mock endpoints, and client mTLS authentication between Gateways.

**MCP Gateway**
Tyk 5.13.0 introduces the MCP (Model Context Protocol) Gateway, enabling teams to proxy, secure, and manage AI agent tool servers within Tyk API management. MCP Gateway is generally available in this release.

MCP APIs now support Server-Sent Events (SSE) streaming, enabling long-lived agent sessions that survive beyond the configured write timeout. A configurable `sse_write_timeout` terminates idle streams cleanly, and the Gateway sends a structured error event to the client when an upstream connection drops unexpectedly.

Tool-Based Access Control (TBAC) enforces fine-grained access rules at two levels: JSON-RPC method level (e.g., `tools/call`, `resources/read`) and MCP primitive level (individual tool, resource, or prompt by name). Per-primitive rate limiting sets independent rate limits for each method or named primitive within a key or policy. When multiple policies apply, access lists merge as a union and rate limits use the most permissive value.

MCP traffic now appears in Gateway access logs with dedicated fields (`mcp_method`, `mcp_primitive_type`, `mcp_primitive_name`) alongside a new `api_type` field present on all access log entries. OpenTelemetry custom metrics expose the same four MCP dimensions — including `mcp_error_code` — aligned with the OpenTelemetry `mcp.*` semantic conventions.

For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v5.13.0) below.

#### Breaking Changes

**Default maximum TLS version** is now inherited from the underlying Golang library and so will be TLS 1.3 unless overridden using `TYK_GW_HTTPSERVEROPTIONS_MAXVERSION`. Previously this defaulted to TLS 1.2. See [here](/api-management/implement-tls#controlling-tls-version-&-cipher-suites) for details how to control TLS version and cipher suites.
...
- **Dashboard v5.13.0** — MCP Proxy management and access control configuration
...
<Accordion title='Added MCP Gateway'>
Tyk 5.13.0 introduces the MCP (Model Context Protocol) Gateway, enabling teams to proxy, secure, and manage AI agent tool servers using Tyk API management. MCP Proxy definitions use OpenAPI Specification (OAS) format and support the JSON-RPC over HTTP transport used by MCP clients and servers.

This release includes:
- **SSE streaming**: MCP APIs now support long-lived Server-Sent Events connections. The write deadline is cleared for SSE streams so connections survive beyond the configured write timeout. A configurable `sse_write_timeout` idle timeout terminates stale connections cleanly. A structured `event: error` message is sent to the client if the upstream connection drops unexpectedly.
- **Protected Resource Metadata (PRM)**: The `/.well-known/oauth-protected-resource` endpoint is now served as a middleware in the Gateway chain, making `$tyk_context.*` substitution variables available for use in PRM resource field values.

For configuration details, see the [MCP Gateway documentation](/ai-management/mcp-gateway/overview).
</Accordion>

<Accordion title='Added Tool-Based Access Control (TBAC) for MCP Proxy APIs'>
MCP Proxy APIs now support granular access control at the JSON-RPC method and MCP primitive levels. Two new middleware components enforce access rules independently for each request:
...
<Accordion title='Added per-primitive rate limiting for MCP Proxy APIs'>
MCP Proxy APIs now support independent rate limits for each JSON-RPC method and for individual tools, resources, and prompts. Two new fields are available in key and policy access definitions:
...
When multiple policies apply to a key, rate limits merge using most-permissive semantics — the highest rate allowed by any policy applies.

For configuration details, see the [MCP proxy policies documentation](/ai-management/mcp-gateway/policies).
</Accordion>

<Accordion title='Added MCP-specific fields to Gateway access logs'>
Gateway access logs now include MCP-specific fields for requests handled by MCP Proxy APIs:
...
<Accordion title='Added MCP dimensions to OpenTelemetry custom metrics'>
...
<Accordion title='Added Centralized Error Response Customization'>
Added a comprehensive mechanism to customise HTTP error responses generated by both the Tyk Gateway and upstream services. This enhancement allows operators to fully control error outputs to standardise formats, mask sensitive information, and comply with industry standards.

**Key Capabilities:**
- **Multi-level Configuration:** Configure error overrides globally (gateway-wide), per API, or per specific endpoint with clear precedence rules
...
**Configuration:** Error overrides use a map-based structure indexed by HTTP status code in the new `error_overrides` configuration.

**Template Support:** Response templates support dynamic variables including `{{.StatusCode}}`, `{{.Message}}`, and validation-specific context like `{{.invalid_params}}` for detailed error feedback.

**Precedence Order:**
1. Endpoint-level overrides (highest priority)
2. API-level overrides
3. Gateway-level overrides
4. Default Tyk responses (fallback)
...
<Accordion title='Added OpenTelemetry Metrics Export'>
This release introduces comprehensive OpenTelemetry metrics export, providing real-time operational visibility into Gateway performance, health, and business KPIs. This complements existing distributed tracing and logging to deliver a complete observability solution.

**Key Features:**
- **Default Gateway Metrics:** Automatic export of RED metrics (Rate, Errors, Duration), Go runtime health metrics (CPU, memory, goroutines), and configuration state tracking (loaded APIs/policies, reload operations)
...
**Configuration:** Metrics are configured under the new `opentelemtry.metrics` object with independent control from tracing
...
- **Runtime Metrics:** Go memory usage, goroutine count, GC performance
- **Configuration Metrics: Loaded APIs/policies count, reload operations and duration
...
For more details and examples, see the LINK
</Accordion>
...
Resolved an issue where a Go plugin returning an error status code would resulting in malformed response bodies that concatenated the original plugin response with additional Gateway error messages. 

The Gateway now correctly handles plugin-generated error responses without double-writing headers, ensuring response bodies contain only the original plugin content and eliminating superfluous warnings in logs.
</Accordion>
...
Resolved an issue where Tyk OAS APIs with mock endpoints stopped generating analytics data. This functionality was inadvertently broken while fixing an unrelated internal API proxying issue in Tyk Gateway 5.8.6.
...
<Accordion title='Fixed VEM path preservation for undefined MCP primitives'>
Resolved an issue where the Virtual Endpoint Method (VEM) path was not preserved during version checking when a request referenced an MCP primitive not defined in the API. The Gateway now correctly retains the path through the version check, preventing requests to undefined primitives from being incorrectly routed or rejected.
</Accordion>
