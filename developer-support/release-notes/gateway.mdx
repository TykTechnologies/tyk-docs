...
Tyk Gateway can now turn an existing Tyk OAS API into a fully functional MCP proxy, so REST APIs already registered with Tyk can be exposed to AI agents as MCP tools without rebuilding them. This release adds the catalogue and configuration foundation for defining MCP proxies, REST-to-MCP protocol translation for tool calls, session lifecycle and Gateway runtime support, and policy enforcement over MCP proxy traffic.

**Enhanced Secrets Management**
 
This release also introduces an enhanced Secrets Management approach, replacing the legacy Key-Value reference syntax for most use cases.
...
For Enterprise Edition users, the new system adds support for new providers: AWS Secrets Manager, Azure Key Vault, and GCP Secret Manager.
...
RFC 8693 Token Exchange now supports the Microsoft Entra On-Behalf-Of (OBO) grant, allowing Tyk Gateway to exchange an inbound client token for a downstream-scoped token issued by Microsoft Entra ID before forwarding the request upstream.
Token Exchange also gains improved observability into the exchange process (Enterprise Edition).

**Enhanced experience with JavaScript custom plugins**
 
We've added a new JavaScript plugin engine that uses the Goja Golang JavaScript runtime. This is available as a new `driver` option and provides significant improvement in JS plugin performance. We've also extended the usability of JS plugins so that they can be assigned to all plugin hooks. 
 
 **Next-generation (-ng) plugin compiler with native arm64 support**
 
A new next-generation (`-ng`) plugin compiler is now available, as an alternative to the existing compiler, with a significantly reduced the OS-level CVE surface compared to the current compiler.
...
- Tyk OAS APIs can be configured as MCP proxies, giving each API a catalogue entry and configuration foundation that describes it as a set of MCP tools, resources, and prompts.
...
- Access policies can be enforced against MCP proxy traffic, so the same policy-based access controls used for REST APIs apply consistently to the MCP tools, resources, and prompts an MCP proxy exposes.
</Accordion>
...
JavaScript plugins can also now be attached to post-auth and response hooks increasing the viability of JS as the plugin language of choice.
