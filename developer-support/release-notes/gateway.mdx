...
- `legacy`: preserves the format used prior to Gateway 5.8.15, with the historical timestamp format and the `msg` field name.
 
This is a **breaking change** for any user whose monitoring or log-parsing tools rely on the specific format of the Tyk Gateway application logs. If you require the previous timestamp format or the `msg` field name, you should set `log_format` to `legacy`.
</Accordion>
