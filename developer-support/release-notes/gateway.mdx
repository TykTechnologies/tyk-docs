...
For more details, please see the [JWT Auth](/api-management/authentication/jwt-signature-validation) section.
...
The [Logstash](/api-management/logs-metrics) formatter timestamp is now in [RFC3339Nano](https://www.rfc-editor.org/rfc/rfc3339) format.
</Accordion>
...
We offer support for integrating *OpenTelemetry* traces with supported open source tools such [Jaeger](/api-management/logs-metrics), [Dynatrace](/api-management/logs-metrics) or [New Relic](/api-management/logs-metrics). This allows API owners and developers to gain troubleshooting and performance insights from error logs, response times etc.
...
Added configuration parameter to enable/disable [detailed_tracing](/api-management/logs-metrics) for *Tyk Classic API*.
</Accordion>
