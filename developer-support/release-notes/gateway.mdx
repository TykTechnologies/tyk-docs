...
Tyk Gateway can now turn an existing Tyk OAS API into a fully functional [MCP proxy](/ai-management/mcp-gateway/api-to-mcp), so REST APIs already registered with Tyk can be exposed to AI agents as MCP tools without rebuilding them. This release adds the catalogue and [configuration foundation](/ai-management/mcp-gateway/api-to-mcp-definitions) for defining MCP proxies, REST-to-MCP protocol translation for tool calls, session lifecycle and Gateway runtime support, and [policy enforcement](/ai-management/mcp-gateway/policies) over MCP proxy traffic.
...
For more details see [REST API to MCP](/ai-management/mcp-gateway/api-to-mcp).
</Accordion>
