...
This release introduces the next-generation (`-ng`) plugin compiler with native arm64 support, alongside a set of fixes: request validation error responses are now always valid JSON, the GraphQL Playground now loads the schema for authenticated APIs, a race condition that could prevent session deletion under heavy load is resolved, shutdown signal log entries now follow the configured log format, and JWT scope-to-policy mapping produces less noisy error logging. The underlying Go toolchain has been updated to Go 1.26. This release also addresses several CVEs in dependent libraries. For a comprehensive list of changes, please refer to the detailed changelog below.
...
CE, EE, and FIPS variants are available under the `-ng` tag suffix for both 5.15.0 (latest) and 5.13.2 (LTS) releases, giving you a safe path to evaluate the new compiler before any eventual replacement of the current one.
</Accordion>
