...
In this release, we have updated the self-signed TLS certificate in gateway Docker images with extended validity and modern SAN support. For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v5.3.13) below.
...
<AccordionGroup>
<Accordion title='Updated Gateway TLS Certificate'>
Renewed the self-signed certificate shipped in gateway Docker images to prevent expiration (previous certificate expires March 2026). The updated certificate now uses Subject Alternative Names (SAN) instead of the deprecated Common Name (CN) field, ensuring compatibility with modern TLS standards and Go 1.17+. The certificate has a 200-day validity period to meet current security standards. 

This change affects deployments using `global.tls.gateway=true` in Helm Chart configurations. No immediate action required for existing deployments.
</Accordion>
</AccordionGroup>
