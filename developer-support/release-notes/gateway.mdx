...
<Accordion title='MCP Request Handling & JSON-RPC Routing'>
Added support for routing JSON-RPC requests to Model Context Protocol (MCP) endpoints. The Gateway now detects JSON-RPC requests, extracts methods and primitive names, and routes them to the appropriate Virtual Endpoints for Methods (VEMs) while executing configured middleware.
</Accordion>

<Accordion title='MCP Definition Gateway Load & VEM Generation'>
Added the ability for the Gateway to load MCP definitions and generate internal routing structures (VEMs) for MCP primitives such as tools, resources, and prompts.
</Accordion>

<Accordion title='Building CRUD Operations for MCP Definitions'>
Added dedicated endpoints for managing MCP definitions in the Dashboard (`/api/mcps`) and Gateway (`/tyk/mcps`). This allows for creating, reading, updating, and deleting MCP definitions with proper validation and synchronization.
</Accordion>

<Accordion title='MCP Definition Data Structure'>
Added the foundational data structure and schema to represent MCP definitions in the Gateway, including support for `mcpTools`, `mcpResources`, and `mcpPrompts` sections.
</Accordion>

<Accordion title='MDCB Dataplane Loaded APIs/Policies'>
Extended the MDCB `/dataplanes` endpoint to include loaded API and Policy IDs in the response, enabling better visibility into the resources loaded on connected data planes.
</Accordion>

<Accordion title='Gateway Configuration Dump via Management API'>
Added new `/config` and `/env` endpoints to the Gateway Management API, allowing users to retrieve the actual deployment configuration and environment variable mappings for easier troubleshooting and auditing.
</Accordion>

<Accordion title='Custom Trace Header Configuration for Gateway'>
Added support for configuring custom trace headers (e.g., `X-Correlation-ID`) for OpenTelemetry context propagation. This allows organizations with legacy correlation ID systems to maintain trace continuity when requests pass through the Gateway.
</Accordion>

<Accordion title='Implement Document Validation for OAS 3.1'>
Added support for validating OpenAPI Specification (OAS) 3.1 documents, including features like type arrays with null support and JSON Schema 2020-12 keywords.
</Accordion>

<Accordion title='Add TraceID and SpanID to Gateway Application Logs'>
Added `trace_id` and `span_id` fields to all request-scoped application logs when OpenTelemetry is enabled, allowing developers to correlate operational events with distributed traces.
</Accordion>

<Accordion title='Add TraceID to Gateway Access Logs'>
Added the ability to include the OpenTelemetry `trace_id` in Gateway access logs written to stdout, enabling correlation of HTTP request logs with distributed traces.
</Accordion>

<Accordion title='Support Configurable BatchSpanProcessor Settings'>
Added a new `span_batch_config` section to the OpenTelemetry configuration, allowing users to tune BatchSpanProcessor settings (`max_queue_size`, `max_export_batch_size`, `batch_timeout`) to prevent span loss in high-throughput environments.
</Accordion>

<Accordion title='Implement Compression of API Definitions'>
Added support for Zstd compression of API definitions stored in Redis. This significantly reduces storage costs and improves performance for deployments with large or numerous API definitions.
</Accordion>
...
<Accordion title='Selectively Perform Plugin Bundle Verification on Load'>
Improved the performance of plugin bundle verification by adding a new configuration option (`skip_verify_existing_plugin_bundle`). When enabled, cryptographic bundle verification is skipped when loading from disk, reducing resource consumption for deployments with many APIs.
</Accordion>

<Accordion title='Improve AccessLogs Error Observability for 5XX Errors'>
Enhanced access logs to include structured error classification fields (`response_flag`, `response_code_details`, `error_source`, etc.) for 5XX upstream errors, making it easier to distinguish between different types of failures such as connection timeouts, TLS certificate expiration, and circuit breaker events.
</Accordion>

<Accordion title='Update OAS Field for Dynamic MTLS'>
Restructured Certificate Authentication as a separate authentication method in Tyk OAS API definitions, replacing the previous Dynamic mTLS configuration under the Auth Token scheme. This improves API design and usability.
</Accordion>

<Accordion title='Extend Certificate Expiry Events to All Gateway Certs'>
Extended certificate expiry monitoring to cover all certificate types used by the Gateway, including server certificates, CA certificates, and upstream mTLS certificates. The Gateway now fires `CertificateExpiringSoon` and `CertificateExpired` events for all monitored certificates.
</Accordion>

<Accordion title='Warn if Policy ID Duplicated in Single Org'>
Added a warning log message when multiple policies with the same ID are loaded within a single organization, helping users identify and resolve potential configuration conflicts.
</Accordion>
...
<Accordion title='Improve OpenTelemetry Trace Support for Compliant Multi-Auth APIs'>
Resolved an issue where OpenTelemetry traces for APIs using compliant multi-auth with JWT did not include the `alias` attribute, making it difficult to identify the consumer of a request.
</Accordion>

<Accordion title='List APIs Excludes MCP Proxies by Default'>
Fixed an issue where the `/api/apis` and `/tyk/apis` endpoints returned MCP Proxies by default. MCP Proxies are now excluded unless explicitly requested using the `include_types=mcp` query parameter.
</Accordion>
...
<Accordion title='Address Gateway and Dashboard CVEs'>
Addressed multiple Critical, High, and Medium severity Common Vulnerabilities and Exposures (CVEs) identified in the Gateway and Dashboard components by updating dependent libraries.
</Accordion>
