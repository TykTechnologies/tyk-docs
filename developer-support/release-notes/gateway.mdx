...
<Accordion title='Fix log flooding for unmatched JWT scope-to-policy mappings'>
When using JWT authentication with scope-to-policy mapping, the Gateway logged an `ERROR` for every scope in a token that didn't match a policy, even when other scopes in the same token matched successfully and the request was authorized without issue. This flooded logs with false-positive errors on otherwise healthy traffic.

Unmatched scopes are now reported in a single consolidated log entry instead of one per scope: at `DEBUG` level when at least one scope matched a policy and the request was authorized, and at `ERROR` level only when none of the token's scopes matched any policy and the request is rejected. This significantly reduces log noise for APIs using scope-to-policy mapping, while preserving visibility into requests that fail authorization entirely.
</Accordion>

<Accordion title='Fix race condition preventing reliable session deletion under heavy load'>
Resolved a race condition between session update and deletion operations that could occur when deleting a session with an associated quota while under heavy load. Previously, this could prevent the session key from being deleted at all, or worse, cause it to appear deleted in the control plane while remaining valid for API access in the data plane, allowing continued use of an authorization token that should have been revoked.

Session deletion now completes reliably regardless of load, with the key immediately invalidated across both control and data planes. This applies consistently to single-Gateway, MDCB, and shared-Redis multi-Gateway deployments.
</Accordion>

<Accordion title='Fix shutdown signal log not following configured log format'>
When the Gateway received a `SIGINT`, `SIGTERM`, or `SIGQUIT` signal, it logged a raw, unstructured `interrupt` message that ignored the configured `log_format`, no log level, no structured message field. With JSON logging enabled, this malformed entry could cause log parsers to fail and stop processing subsequent log lines.

The shutdown signal is now logged consistently with the configured `log_format` (`text`, `json`, or `legacy`), including a proper log level and message, alongside the existing shutdown log entries.
</Accordion>
