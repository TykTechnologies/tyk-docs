...
Added support for TLS certificate rotation in External Service configurations without requiring Gateway restarts. This enhancement allows organizations with strict certificate rotation policies to update certificates seamlessly during runtime.
...
<Accordion title='Added customizable error response overrides for gateway and API-level configuration'>
Introduced a comprehensive error response override system that allows administrators to customize HTTP error responses returned by the Tyk Gateway. This feature enables standardization, transformation, or masking of error responses from both gateway-generated errors and upstream service responses.

**Key Capabilities:**
- **Gateway-level overrides**: Configure global error response rules in `tyk.conf`
- **API-level overrides**: Define custom error responses per API in both Classic and OAS API definitions
- **Upstream error handling**: Intercept and customize 4xx/5xx responses from upstream services
- **Flexible matching**: Support for exact status codes (401, 500), patterns (4xx, 5xx), error flags, message patterns, and JSON body field matching

**Configuration Options:**
- **Response customization**: Override status codes, response bodies, headers, and content types
- **Template support**: Use inline Go templates, external template files, or message-only overrides
- **Precedence rules**: API-level overrides take priority over gateway-level overrides
- **Match conditions**: Filter overrides based on error flags, regex patterns, or JSON response fields
...
This feature works seamlessly with existing analytics, logging, and observability systems, ensuring overridden responses are properly tracked and monitored.
</Accordion>

<Accordion title='Added Gateway Go Runtime Metrics via OpenTelemetry'>
Added comprehensive Go runtime metrics collection through OpenTelemetry to provide visibility into Gateway resource utilization and performance characteristics. This enhancement enables teams to proactively monitor Gateway health and troubleshoot performance issues before they impact users.
...
Added OpenTelemetry metrics to continuously monitor Gateway configuration state, enabling platform teams to track deployment success and detect configuration drift. This addresses observability gaps where API definitions or policies could disappear during deployments without immediate detection.
