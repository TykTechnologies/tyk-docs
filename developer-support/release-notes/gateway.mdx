...
**Application log timestamps now use RFC3339 by default**
Tyk Gateway now emits a single, consistent timestamp format across all application log entries. Previously, the same log file could contain two different timestamp formats because some internal components bypassed the Tyk logger and wrote their own format, making logs harder to parse, alert on, and triage.

The `log_format` configuration option (and the `TYK_GW_LOGFORMAT` environment variable) now accepts three explicit values:

`text` (**new default**): plain-text logs with RFC3339-compliant timestamps (e.g. `2024-12-12T13:59:08Z`), applied consistently across all Gateway components.
`json`: JSON logs with RFC3339-compliant timestamps.
`legac`y: preserves the previous mixed/historical behaviour, including the custom `Jan 02 15:04:05` timestamp format used by the standard Tyk logger.

To keep the previous behaviour exactly, set `log_format` to `legacy` (or `TYK_GW_LOGFORMAT=legacy`). 
