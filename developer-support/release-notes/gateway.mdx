...
**Create MCP Proxies from Your Existing REST APIs** *(Enterprise Edition)*
...
**Microsoft Entra On-Behalf-Of Support for Token Exchange** *(Enterprise Edition)*
...
Tyk OAS APIs can now be configured as MCP proxies, giving each API a catalogue entry and configuration foundation that describes it as a set of MCP tools, resources, and prompts an AI agent can discover and call.
</Accordion>

<Accordion title='Added REST-to-MCP protocol translation for MCP proxy tool calls'>
{/* TT-17320 */}
<Note>
Enterprise Edition only
</Note>

When an AI agent calls an MCP tool backed by an MCP proxy, Tyk Gateway translates the JSON-RPC tool call into the equivalent REST request against the underlying API, and translates the REST response back into the MCP response format expected by the agent.
</Accordion>

<Accordion title='Added session lifecycle and Gateway runtime support for MCP proxies'>
{/* TT-17321 */}
<Note>
Enterprise Edition only
</Note>

Tyk Gateway now manages the MCP session lifecycle for MCP proxies end-to-end, including session initialization, keep-alive, and teardown, as part of the Gateway's runtime request handling.
</Accordion>

<Accordion title='Added policy enforcement for MCP proxy traffic'>
{/* TT-17322 */}
<Note>
Enterprise Edition only
</Note>

Access policies can now be enforced against MCP proxy traffic, so the same policy-based access controls used for REST APIs apply consistently to the MCP tools, resources, and prompts exposed through an MCP proxy.
</Accordion>
...
{/* TT-15516 */}
We've resolved an issue where the bundled GraphQL Playground could not load the schema (Docs and Explorer) for a GraphQL API that requires authentication, even after supplying valid credentials in the Playground's Request Headers editor. The Playground now re-runs schema introspection whenever the selected API or the supplied headers change, and clears the schema if credentials are invalid, removed, or introspection is disabled for that key. As part of this fix, the Playground's bundled GraphiQL asset has also been upgraded, resolving a known cross-site scripting vulnerability (CVE-2021-41248) present in the previous version.
</Accordion>
...
<Accordion title='Fixed inability to delete a Session using a quota under heavy load'>
{/* TT-16259 */}
We have resolved a race condition between session update and session deletion operations that, under heavy load, could either prevent a session (API key) with an associated quota from being deleted, or leave it appearing deleted on the Control Plane while still valid on the Data Plane. Session deletion now completes reliably under load, and deleted sessions are immediately invalidated across both planes.
</Accordion>

<Accordion title='Fixed shutdown signal log entry not following the configured log format'>
{/* TT-17316 */}
We have resolved an issue where the log entry produced when the Gateway received a `SIGINT`, `SIGTERM`, or `SIGQUIT` shutdown signal did not follow the configured `log_format`. With JSON logging enabled, this unstructured entry could cause downstream log parsers to fail on an unparseable line. The shutdown signal is now logged in the configured format like any other application log entry.
</Accordion>
