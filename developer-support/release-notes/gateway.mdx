...
<Accordion title='Optimized Redis Storage for API Keys and Session Objects'>
Significantly reduced Redis memory consumption for API keys and session objects by implementing intelligent storage optimization. The Gateway now automatically omits empty and zero-value fields when storing session data, resulting in up to 20% reduction in Redis memory usage for typical API keys. This optimization particularly benefits environments with thousands of keys, where minimal keys (API key only, no policies) now consume approximately 500-700 bytes instead of storing unnecessary empty fields. The enhancement is storage-agnostic, supporting future in-memory or Redis-free deployments, while maintaining full backward compatibility with existing keys and ensuring no impact on Gateway performance. Users with large-scale API key deployments will see immediate storage cost savings and improved Redis efficiency without any configuration changes or migration requirements.
</Accordion>

<Accordion title='Certificate-Token Binding for Static mTLS Authentication'>
Added certificate-token binding support for Auth Token authentication when using static mTLS to prevent cross-organizational token and certificate misuse. The Gateway now validates that the client certificate presented in a request matches one of the certificates specifically bound to the access token, providing an additional security layer beyond the existing API-level certificate allowlist. This feature addresses scenarios where multiple organizations share the same API (such as through the Developer Portal), preventing one organization's token from being used with another organization's certificate. Multiple certificates can be bound to each token to support certificate rotation scenarios, and the feature operates seamlessly alongside existing authentication flows. When no certificate bindings are configured, the system maintains backward compatibility with current static mTLS behavior. Failed binding attempts are logged with clear error messages, and the feature operates independently from dynamic mTLS authentication, ensuring no impact on existing workflows.
</Accordion>

<Accordion title='Configurable Gateway-Default JWKS Cache Timeout'>
Added Gateway-level configuration for JWKS cache timeout to provide enterprise-wide control over key caching duration. Administrators can now set a default JWKS cache timeout across all APIs using the `jwks.cache.timeout` configuration parameter or the `TYK_GW_JWKS_CACHE_TIMEOUT` environment variable, replacing the previous fixed 240-second timeout with a configurable value. This enhancement enables organizations to enforce security policies and compliance requirements at the Gateway level without requiring individual API configuration changes. The feature maintains full backward compatibility with the existing 240-second default when no Gateway-level timeout is configured, and preserves the existing precedence hierarchy where API-level JWKS cache settings override Gateway defaults when explicitly configured. Both Tyk OAS and Tyk Classic APIs automatically inherit the Gateway-default timeout, simplifying JWKS cache management across large API deployments while providing flexibility for APIs that require specific caching behaviors.
</Accordion>

<Accordion title='Unified Path Matching Algorithm for OAS Middleware'>
Implemented a unified path matching algorithm that ensures consistent behavior between Classic and OAS APIs across all middleware. Previously, OAS-specific middleware (ValidateRequest, MockResponse) used Gorilla Mux for exact path matching only, while Classic APIs used regex-based matching that supported subpaths and wildcards. The Gateway now compiles OAS middleware operations into the same RxPaths system used by Classic APIs, enabling full regex flexibility, wildcard support, and consistent subpath matching behavior across all API types and middleware configurations.
</Accordion>
...
<Accordion title='Configurable BatchSpanProcessor Settings to Prevent OpenTelemetry Span Loss'>
Added configurable BatchSpanProcessor settings for OpenTelemetry tracing to prevent span loss in high-throughput production environments. Users can now tune parameters, including `max_queue_size`, `max_export_batch_size`, and `batch_timeout` through the new `span_batch_config` section in the OpenTelemetry configuration. This addresses a known upstream OpenTelemetry Go SDK issue where default settings (2048 queue size, 512 batch size, 5s timeout) cause silent span loss and incomplete traces under heavy load, resulting in up to 70% trace loss in production scenarios. The enhancement allows users to increase queue sizes to 8192+ and adjust batch parameters to match their traffic patterns, significantly reducing orphaned child spans and improving trace completeness. Configuration is optional and backward compatible, with zero or omitted values defaulting to SDK standards.
</Accordion>

<Accordion title='TraceID Support in Gateway Access Logs for OpenTelemetry Correlation'>
Added `trace_id` field support to Gateway access logs to enable seamless correlation between HTTP request logs and distributed traces in observability platforms. When OpenTelemetry is enabled, access logs written to stdout now include the W3C trace ID (32 hex characters) that matches the `X-Tyk-Trace-Id` response header, allowing DevOps engineers and SREs to easily correlate Gateway access logs with traces in platforms like Grafana Tempo, Jaeger, and OpenSearch. The feature is backward compatible, only including the `trace_id` field when OpenTelemetry is enabled and a valid trace ID exists, with no impact on existing log consumers or performance. Users can now configure access log templates to include or exclude trace IDs as needed, enabling automatic log-trace correlation in modern observability stacks and streamlining production debugging workflows.
</Accordion>

<Accordion title='TraceID and SpanID Support in Gateway Application Logs for Enhanced Observability'>
Added `trace_id` and `span_id` fields to all Gateway application logs to enable comprehensive correlation between operational events and distributed traces. When OpenTelemetry is enabled, middleware execution logs, error messages, and debugging information now include W3C-compliant trace and span identifiers, allowing developers and SREs to see detailed Gateway execution flow when viewing traces in observability platforms. The implementation automatically enriches all request-scoped logs through a central logging function, ensuring consistent trace correlation across auth middleware, rate limiting, transforms, and error handling without affecting non-request logs like startup or configuration messages. Users can now follow the complete request journey from access logs through detailed operational events to trace spans, providing unprecedented visibility into Gateway behavior and significantly improving troubleshooting capabilities in complex distributed systems.
</Accordion>

<Accordion title='Policy ID Duplication Warning for Enhanced Troubleshooting'>
Added warning logs to identify policy ID conflicts within organizations to improve troubleshooting and prevent unexpected behavior. The Gateway now monitors for policies sharing the same custom ID within a single organization during policy loading and generates clear warning messages listing the conflicting policy database IDs. The warnings are organization-scoped, ensuring that legitimate use of identical policy IDs across different organizations does not trigger false alerts.
</Accordion>

<Accordion title='Comprehensive Certificate Expiry Monitoring for All Gateway Certificate Types'>
Extended certificate expiry monitoring to cover all Gateway certificate types, significantly improving operational visibility and preventing unexpected service disruptions. The Gateway now monitors upstream mTLS certificates used for backend connections and provides startup logging for server and CA certificates. Certificate expiry events now include a `cert_role` field to distinguish between "client" and "upstream" certificates, enabling administrators to route alerts appropriately and understand which part of the system is affected. Upstream certificate monitoring operates at the API level with periodic checking and fires `CertificateExpiringSoon` and `CertificateExpired` events with proper `api_id` attribution when certificates approach or exceed their expiry dates. Server and CA certificates are logged during Gateway startup with expiry information, providing visibility into file-based server certificates, Certificate Store server certificates, and CA certificates used for client verification.
</Accordion>

<Accordion title='Custom Trace Header Configuration for Legacy Correlation ID Systems'>
Added support for custom trace header configuration to enable seamless integration with existing correlation ID systems that use non-standard header names. Organizations can now configure the Gateway to recognize custom headers like `X-Correlation-ID` or `X-Request-ID` as trace context sources, eliminating the need to modify legacy systems to adopt standard W3C trace headers. The feature supports three propagation modes: 
...
This flexibility allows gradual migration from legacy correlation systems to standard OpenTelemetry tracing while maintaining trace continuity across the entire request flow. The implementation includes automatic fallback to standard propagators when custom headers are missing or invalid, ensuring robust trace handling in mixed environments. Custom trace headers are validated to conform to OpenTelemetry trace ID standards (32 hex characters) and integrate seamlessly with existing OpenTelemetry exporters and observability platforms, providing organizations with a smooth transition path to modern distributed tracing.
</Accordion>

<Accordion title='Enhanced 5XX Error Observability in Access Logs'>
Added structured error classification to access logs for 5XX upstream errors to significantly improve troubleshooting capabilities and reduce time-to-resolution for production incidents. Access logs now include detailed error context through new fields:
- `response_flag` (standardized error codes like TLE for TLS certificate expired, UCF for connection refused) 
- `response_code_details` (human-readable error descriptions)
- `error_source` (originating component)
- `error_target` (upstream address). 
The enhancement covers comprehensive error scenarios, including TLS certificate issues (expired, invalid, hostname mismatch), connection problems (refused, timeout, reset), infrastructure failures (DNS resolution, circuit breaker open, no healthy upstreams), and client disconnections. Additional context fields provide specific details such as TLS certificate expiry dates and subjects for certificate-related errors, and circuit breaker states for service protection scenarios. This eliminates the need to cross-reference multiple log sources when investigating 5XX errors, enabling operators to quickly identify whether failures stem from expired certificates, network connectivity issues, or backend service problems directly from access logs.
</Accordion>

<Accordion title='Enhanced 4XX Error Observability in Access Logs'>
Added structured error classification to access logs for 4XX gateway errors to provide clear visibility into client request rejections and authentication failures. Access logs now include `response_flag`, `response_code_details`, and `error_source` fields that distinguish between different types of gateway-level errors, such as rate limiting (RLT) versus quota exhaustion (QEX), missing authentication headers (AMF) versus invalid tokens (TKI), and request validation failures like oversized bodies (BTL) or malformed JSON (BIV). The implementation maps existing TykErrors to standardized response flags while preserving all current client response behavior, ensuring no breaking changes to API consumers. JWT-specific errors are classified by failure type (missing header, invalid claims, signing method issues), and rate limiting errors clearly differentiate between temporary rate limits and quota exhaustion scenarios. This enhancement enables operators to quickly understand why requests were rejected at the gateway level without needing to correlate access logs with application logs, streamlining troubleshooting workflows for authentication issues, request validation failures, and quota management.
</Accordion>

<Accordion title='Gateway Configuration Dump via Management API for Enhanced Troubleshooting'>
Added comprehensive configuration inspection endpoints to the Gateway Control API to enable troubleshooting and configuration auditing without requiring SSH access. The new `/config` and `/env` endpoints provide programmatic access to the Gateway's actual runtime configuration, including environment variable mappings and resolved values from multiple configuration sources (file, environment variables, defaults). Platform engineers and support teams can now quickly verify configuration settings, debug deployment issues, and audit security configurations through authenticated API calls. The feature includes intelligent, sensitive data protection, automatically redacting passwords, secrets, and connection strings while preserving configuration structure visibility. Single-field queries support targeted troubleshooting (e.g., `/config?field=storage.host`), and environment variable mappings help clarify configuration precedence and naming conventions. The endpoints reflect live configuration state, including hot-reloaded changes, and maintain consistency with MDCB's configuration inspection format. This enhancement significantly reduces support friction by eliminating manual configuration file sharing and provides automated configuration drift detection capabilities for enterprise deployments. The feature is disabled by default and requires explicit enablement via `enable_config_inspection: true` with proper authentication using the Gateway's Control API secret.
</Accordion>

<Accordion title='Enhanced MDCB `/dataplane` Endpoint with Information on Loaded APIs and Policies'>
The MDCB `/dataplanes` endpoint has been enhanced to provide visibility into your Tyk deployment. This now returns a detailed list of the APIs and policies loaded on each connected data plane gateway. You can retrieve the results for a specific Gateway using the `node_id` query parameter. This update provides a clear picture of what is running on each Gateway, simplifying monitoring and troubleshooting of your deployed Data Planes.
</Accordion>
...
<Accordion title='Certificate Authentication as Standalone Method in OAS API Definitions'>
Restructured Certificate Authentication (formerly Dynamic mTLS) as a dedicated authentication method in Tyk OAS API definitions, separate from Auth Token authentication. Certificate authentication is now configured using the new `authentication.certificateAuth.enabled` field, aligning it with other Tyk proprietary authentication methods like HMAC and Custom Auth, rather than as an adjunct property of token authentication. This change improves API design consistency and usability while maintaining full backward compatibility with the deprecated `authentication.securitySchemes.authToken.enableClientCertificate` field. When both fields are present, the new `certificateAuth.enabled` field takes precedence, and bidirectional conversion between OAS and classic API formats works seamlessly.
</Accordion>

<Accordion title='Model Context Protocol (MCP) Support for AI Agent Integration'>
- **MCP & OpenAPI Support**: 
  - Added comprehensive support for Model Context Protocol (MCP) to enable seamless integration with AI agents and LLM applications.
  - The Gateway now supports MCP definitions via the OpenAPI Specification format with MCP-specific extensions.
- **MCP Primitives & Configuration**:
  - MCP primitives (tools, resources, prompts) are configurable as manageable API primitives.
  - Each MCP primitive supports individual middleware configuration, including:
    - Rate limiting
    - Authentication
    - Request validation
  - This provides granular control over AI agent interactions.
- **Management & APIs**:
  - Dedicated CRUD endpoints for MCP management:
    - Dashboard: `/api/mcps`
    - Gateway: `/tyk/mcps`
  - Automatic generation of Virtual Endpoint Methods (VEMs) for MCP primitive routing.
  - Intelligent JSON-RPC request handling that routes calls to the appropriate primitives based on method names and parameters.
- **Advanced Capabilities**:
  - Support for wildcard resource patterns in MCP definitions for flexible file system access control.
  - Tool-specific rate limiting to support cost management.
  - Prompt-based request routing to enable structured AI interactions.
- **Compatibility & API Listing**:
  - MCP APIs are excluded by default from API listing endpoints:
    - `/api/apis`
    - `/tyk/apis`
  - To include MCP APIs, clients must use:
    - `include_types=mcp`
  - Maintains full backward compatibility with existing API definitions.
- **Protocol & Security**:
  - Specialized handling for JSON-RPC 2.0 protocol requirements.
  - Enables organizations to securely expose backend services and data sources to AI agents through Tyk’s comprehensive API management capabilities.
</Accordion>
...
<Accordion title='Fixed Gateway Panic with Missing Vault Paths in API Definitions'>
Resolved an issue where the Gateway would crash with a panic when API definitions contained `vault://` references, but the required Vault path (`secret/data/tyk-apis`) did not exist. Previously, this configuration error caused the entire Gateway process to exit, resulting in a complete service outage during API loads, hot reloads, or Dashboard saves. The Gateway now gracefully handles missing Vault paths by logging a clear error message and continuing to operate normally.
</Accordion>

<Accordion title='Fixed OpenAPI multipleOf Validation for Floating-Point Numbers'>
Resolved a floating-point precision issue in OpenAPI request validation where mathematically valid multipleOf values were incorrectly rejected due to binary representation limitations. Previously, when validating decimal numbers against multipleOf constraints (e.g., validating 2.3 against multipleOf: 0.1), the Gateway's validation logic would fail. The Gateway now properly handles floating-point precision in multipleOf validation, ensuring that all mathematically valid decimal multiples pass validation consistently while continuing to correctly reject invalid values.
</Accordion>

<Accordion title='Fixed Incomplete Validation of Multi-Value HTTP Headers in OAS APIs'>
Resolved an issue where Tyk's OpenAPI request validation only checked the first instance of multi-value headers, allowing invalid header values to bypass schema constraints. Previously, when multiple headers with the same name were sent or when header values were provided in different orders, only the first header was validated against the OpenAPI schema, creating a validation loophole. The Gateway now properly normalizes and validates all header values according to HTTP standards, ensuring that every value in multi-value headers complies with the defined OpenAPI schema constraints.
</Accordion>

<Accordion title='Fixed API Routing Issues with Custom Domains and Similar Listen Paths'>
Resolved a routing issue where APIs returned 404 Not Found errors despite being properly configured, with different behaviors between Classic and OAS APIs depending on custom domain settings. Previously, when APIs had similar listen path prefixes (e.g., `/caa` and `/caas2itsamu0456w2ayl9`), the Gateway's routing logic would incorrectly match requests, causing legitimate API calls to fail. The issue affected OAS APIs when custom domains were disabled and Classic APIs when custom domains were enabled. The Gateway now properly sorts and matches API specifications by listen path length while correctly considering domain configuration options, ensuring all APIs are accessible via their configured paths regardless of custom domain settings or API type. 
</Accordion>

<Accordion title='Fixed Missing Request Duration Logging for Gateway Error Responses'>
Resolved an issue where the Gateway incorrectly logged 0ms duration for error responses, including 504 Gateway Timeout, 499 Client Closed Request, and 500 Internal Server Error, creating gaps in API observability and monitoring. Previously, these error responses were hardcoded with zero latency values, making it impossible to determine actual processing time, gateway saturation, or connection utilization for failed requests. The Gateway now accurately calculates and logs the actual request duration from start to error occurrence for all error responses, providing complete timing visibility across successful and failed API requests. This enhancement improves observability for performance monitoring, capacity planning, and troubleshooting workflows.
</Accordion>

<Accordion title='Fixed Mock Response Middleware for Versioned OAS APIs'>
Resolved an issue where mock response middleware failed to execute when calling child versions of Tyk OAS APIs, preventing users from setting up version-specific mock responses. Previously, the operation context containing mock response configuration was not properly set for child API versions, causing the middleware to be skipped entirely while base versions worked correctly. The Gateway now properly handles URL path adjustments between parent and child API listen paths for OAS APIs, ensuring that mock response middleware executes correctly for all API versions.
</Accordion>

<Accordion title='Fixed Path Matching Issues for OAS API Middleware'>
Resolved multiple path matching inconsistencies between Classic and OAS APIs that prevented middleware from executing correctly. Fixed an issue where wildcard path matching (`/.*` or `blank paths`) did not work for ValidateRequest and MockResponse middleware in OAS APIs, preventing customers from applying middleware to entire APIs as they could with Classic APIs. Additionally, resolved an issue where middleware configured for the root path (`/`) failed to execute when OAS APIs were called without a trailing slash (e.g., `/httpbin` instead of `/httpbin/`), while the same configuration worked correctly for Classic APIs. These fixes were implemented through a unified path matching algorithm that ensures consistent middleware behavior across all API types and path configurations.
</Accordion>

<Accordion title='Fixed Missing Identity Source in OTEL Traces for JWT Protected APIs'>
Resolved an issue where OpenTelemetry traces were missing the "alias" field when using JWT-protected APIs, making it impossible to identify API consumers in tracing data. Previously, while the alias was correctly populated in Redis sessions and pump metrics, it was not included in OTEL spans for JWT-authenticated requests. The Gateway now ensures that OTEL spans consistently include the alias attribute for all authentication methods, enabling proper consumer identification and request attribution in distributed tracing systems.
</Accordion>
