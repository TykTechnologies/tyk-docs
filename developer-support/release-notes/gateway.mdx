...
In this release, we have fixed a memory leak in the gateway that occurred when using JWKS URL authentication. For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v5.11.1) below.
...
- [Source code tarball of Tyk Gateway v5.10.1](https://github.com/TykTechnologies/tyk/releases/tag/v5.11.1)
...
<Accordion title='Fixed Memory Leak in Gateway When Using JWKS URL Authentication'>
Resolved a memory leak issue in the gateway that occurred when APIs used JWT authentication with JWKS URLs.
</Accordion>
