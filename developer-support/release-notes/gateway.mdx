...
This release also introduces a single, unified way to reference secrets and configuration values from an external store anywhere in an API definition: `kv://<store>/<path>` for a whole value, or `$kv{<store>:<path>}` inline within a string. The store itself — Vault, Consul, a plain environment variable, or a generic secrets provider — is defined once, by name, in a new Key-Value registry in the Gateway configuration. Existing `vault://`, `consul://`, `env://`, and `secrets://` references keep working unchanged, so there's no forced migration.
...
**A modernized JavaScript plugin engine**
...
{/* TT-9550, TT-17641 */}
A new next-generation (`-ng`) plugin compiler is now available alongside the existing plugin compiler, as an opt-in parallel option. It is a dual-layer, BusyBox-based image that significantly reduces the OS-level CVE surface compared to the current compiler, and adds native `linux/arm64` support — plugins built on Apple Silicon or other arm64 hosts no longer need to run the compiler under emulation.
CE, EE, and FIPS variants are available under the `-ng` tag suffix for both feature and LTS releases, giving you a safe path to evaluate the new compiler before any eventual replacement of the current one.
</Accordion>
...
{/* TT-14798 */}
We've resolved an issue where error responses from Tyk's OAS request validation middleware could contain invalid JSON when the underlying error message included an apostrophe or other special character — for example, when an OpenAPI schema's regex pattern contained a single quote. These responses are now valid JSON by default, so downstream systems parsing them no longer fail. If you rely on the previous JavaScript-style escaping, set the `X-Skip-Escaper: 1` request header, or the new `error_message_escaper: skip` configuration option, to restore it. XML and SOAP+XML error responses are unaffected.
</Accordion>
