...
**Note: This change does not affect you unless you specifically rely on the current Gateway log format**: for example, if your monitoring, parsing, or alerting tools are built around the existing timestamp format.

This release makes several changes to the format and content of Tyk Gateway application logs so that output is consistent across all components and aligned with industry observability standards. All of the changes below are controlled by a single setting, and every one of them can be reverted by selecting the `legacy` log format.
 
The `log_format` configuration option (and the `TYK_GW_LOGFORMAT` environment variable) now accepts three explicit values:
...
- `legacy`: preserves the previous behaviour exactly, including the historical mixed timestamp formats and the previous 404 log fields.
**What changes under the new `text` and `json` formats**
 
- **Consistent RFC3339 timestamps.** All log entries now use the industry-standard RFC3339 timestamp format (e.g. `2024-12-12T13:59:08Z`). Previously, the same log file could contain two different timestamp formats, because some internal components bypassed the Tyk logger and emitted their own format. The default changes from the custom `Jan 02 15:04:05` format to RFC3339.
...
- **Log message field renamed from `msg` to `message`.** The main log message field is now `message`, aligning the Gateway with Tyk Pump and common logging conventions and removing the previous inconsistency between components.

**Migration**
- To keep the previous behaviour exactly, historical timestamps and the previous 404 log fields, set `log_format` to `legacy` (or `TYK_GW_LOGFORMAT=legacy`). The `legacy` option is the supported fallback for customers whose observability tooling depends on the historical log output.
...
The Gateway can now wait for a configurable period after receiving a shutdown signal before it stops accepting new connections, giving load balancers and orchestrators time to detect that the node is no longer ready and remove it from the pool. This prevents the `502` errors that could otherwise occur during scale-down in high-traffic deployments.
...
Because the certificate is now a string field in the API definition, it can also be populated from a KV/secret reference, keeping private key material out of the management plane. Existing API definitions that reference certificates by ID or file path continue to work unchanged.
</Accordion>

<Accordion title='Add file-based KV/secret references'>
The Gateway can now resolve KV/secret references from the contents of a local file, making it straightforward to consume secrets mounted as files on the data plane without storing the secret material in the management plane. A new `file://<path>` scheme resolves at API load time, alongside the existing `env://`, `vault://`, `consul://`, and `secrets://` schemes, and a new `$secret_file.<key>` prefix resolves at request time in the header-modification, URL-rewrite, and body-transform middleware, alongside the existing `$secret_env.`, `$secret_vault.`, and `$secret_consul.` prefixes.
...
<Accordion title='Add OAuth 2.0 security scheme for external IdP integration'>
{/* TT-17172 */}
A new `oauth2` security scheme is available for Tyk OAS APIs, enabling authentication against an external identity provider such as Keycloak, Okta, or Auth0. Tyk acts as the OAuth 2.0 protected resource, validating inbound bearer tokens against the IdP's JWKS endpoint.

This is distinct from `tyk_oauth`, which issues tokens to clients, and from the deprecated `externalOAuth` scheme, which validated external IdP tokens but was limited to a single provider.

The new scheme is natively integrated with the Tyk OAS API definition and is the foundation for scope enforcement, Protected Resource Metadata, and token exchange.
</Accordion>

<Accordion title='Add OAuth 2.0 scope enforcement'>
{/* TT-17173, TT-17174 */}
Scope enforcement can now be configured for APIs using the new `oauth2` security scheme, requiring that inbound tokens carry specific OAuth scopes before a request is permitted. Scopes can be enforced at the API level, applying to all requests, or overridden per-operation and per-MCP-primitive for fine-grained access control over individual endpoints and tools.
</Accordion>

<Accordion title='Move Protected Resource Metadata into the OAuth 2.0 scheme'>
{/* TT-17175 */}
...
We fixed a bug where Tyk Gateway application logs were inconsistent: the same log file could contain two different timestamp formats.
Tyk Gateway now outputs a consistent, RFC3339-compliant timestamp across all application log entries. 
The `log_format` configuration option (and the `TYK_GW_LOGFORMAT` environment variable) now accepts three values: 
- `text` (the new default, RFC3339 plain text)
- `json` (RFC3339 JSON)
- `legacy` (the previous mixed behaviour, including the custom `Jan 02 15:04:05` format).
This is a **breaking change**: customers whose monitoring or log-parsing tools rely on the previous timestamp format should set `log_format` to `legacy`. 
