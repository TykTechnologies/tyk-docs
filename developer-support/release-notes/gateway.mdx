...
In this release, we have updated the gateway TLS certificate to extend validity and improve compatibility with modern TLS standards. For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v5.3.13) below.
...
Renewed the self-signed certificate shipped in gateway Docker images with extended validity until March 2029. The updated certificate now uses Subject Alternative Names (SAN) instead of the deprecated Common Name (CN) field, ensuring compatibility with modern TLS standards and Go 1.17+. This change affects deployments using `global.tls.gateway=true` in Helm configurations. No action required, existing deployments will automatically use the new certificate on upgrade.
</Accordion>
</AccordionGroup>
