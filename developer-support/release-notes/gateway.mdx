...
<Accordion title='Custom Trace Header Configuration for Custom Correlation ID Systems'>
...
<Accordion title='Enhanced Error Observability in Access Logs'>
Added structured error context to access logs for 4XX or 5XX gateway and upstream errors to significantly improve troubleshooting capabilities and reduce time-to-resolution for production incidents. Access logs now include detailed error context through new fields:
...
- `tls_cert_expiry` (TLS certificate expiration date, only present on TLS certificate errors).
- `tls_cert_subject` (TLS certificate subject, only present on TLS certificate errors).
- `circuit_breaker_state` (Circuit breaker state, only present on circuit breaker errors).
The `response_flag` helps you identify the root cause of error, including TLS certificate issues (expired, invalid, hostname mismatch), connection problems (refused, timeout, reset), infrastructure failures (DNS resolution, circuit breaker open, no healthy upstreams), rate limiting (RLT), quota exhaustion (QEX), missing authentication headers (AMF), etc. The full list of response flag codes are documented in the [Access Logs](https://tyk.io/docs/api-management/logs#access-logs) section in the Docs. Additional context fields provide specific details such as TLS certificate expiry dates and subjects for certificate-related errors, and circuit breaker states for service protection scenarios. This eliminates the need to cross-reference multiple log sources when investigating API errors, enabling operators to quickly identify whether failures stem from expired certificates, network connectivity issues, or backend service problems directly from access logs.
</Accordion>
...
Resolved critical path matching inconsistencies that prevented Tyk OAS-specific middleware from executing correctly in scenarios that worked properly with Classic APIs.

- Fixed an issue where ValidateRequest and MockResponse middleware failed to trigger for subpaths (e.g., middleware configured for `/users` would not execute for `/users/123`), breaking feature parity with Classic API behavior.
- Resolved wildcard path matching problems where patterns like `/.*` or root paths `/` did not apply middleware to all endpoints as expected, preventing customers migrating from other API gateways from using global middleware configurations.
- Additionally, fixed mock response failures for child versions of OAS APIs and resolved middleware execution issues when APIs were accessed without trailing slashes.

The fix implements a unified path matching algorithm that ensures OAS APIs use the same regex-based matching system as Classic APIs, providing consistent middleware behavior across all API types while maintaining full backward compatibility and regex flexibility.
</Accordion>
