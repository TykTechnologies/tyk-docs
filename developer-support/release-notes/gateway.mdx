...
Tyk Gateway's JavaScript plugin engine now offers a new `goja`-based driver as an opt-in alternative to the existing `otto` engine, improving performance and adding support for post-auth and response hooks in JS plugins. The `otto` driver remains the default, so existing JS plugins are unaffected.
