...
This patch release focuses on enhanced observability capabilities and critical bug fixes to improve gateway stability and monitoring.

**OpenTelemetry Metrics Integration**

Tyk Gateway now provides comprehensive metrics collection through OpenTelemetry, including runtime metrics, endpoint-level metrics, and gateway configuration metrics. The integration includes cardinality control, exemplar support, and configurable resource attributes for better gateway identification. Configuration has been restructured to separate traces and metrics, with support for config-driven API metrics and centralized dimension sources.

**Error Response Management**

A new centralized error overrides infrastructure enables per-API custom error response configuration, giving you greater control over error handling and user experience across different APIs.

**Enhanced Configuration Options**

New configuration capabilities include rate limit header customization, configurable session TTL management for Redis, TLS certificate rotation for external services, and API compression size limits. The gateway now supports underscores in Vault and Consul secret reference paths for improved flexibility.

The release also addresses several critical issues including TLS certificate authentication errors, client mTLS authentication between gateways, CORS preflight request handling, and analytics generation for OAS mock endpoints. Go runtime has been updated to version 1.25 for improved performance and security.

For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v5.8.1) below.
...
|--------|-------------------|-------------|
| 5.13.0 | MDCB vX.Y.Z       | MDCB vX.Y.Z |
...
<Accordion title='Added Rate Limit Header Configuration Option'>
Added a new Gateway configuration option `rate_limit_response_headers` that allows `X-RateLimit-*` headers to be populated from rate limit data instead of quota data. This addresses industry standard compliance and reduces customer confusion when receiving HTTP 429 responses.

Previously, when APIs exceeded their rate limits and returned HTTP 429 status codes, the `X-RateLimit-Limit` and `X-RateLimit-Remaining` headers incorrectly displayed quota information (e.g., monthly limits) rather than rate limit information (e.g., per-minute limits). This violated RFC standards and confused customers who would see remaining quota capacity while receiving rate limit errors.

When `rate_limit_response_headers` is set to `"rate_limits"`, the headers now correctly show:
...
This change maintains backward compatibility, with the default behavior unchanged when the configuration option is not specified.
</Accordion>

<Accordion title='Added Configurable Session Object TTL Management for Redis'>
Tyk now provides improved management of session objects and keys stored in Redis through enhanced TTL (Time To Live) configuration options. This addresses the previous limitation where expired session objects could accumulate indefinitely in Redis storage, requiring manual cleanup scripts.

Key improvements include:

- Enhanced `session_lifetime` configuration at the API level and `global_session_lifetime` at the Gateway level to properly manage Redis object TTL
- Added `session_lifetime_respects_key_expiration` setting to ensure session objects are not deleted from Redis before their associated keys have expired
- Improved coordination between key expiry periods and Redis TTL to prevent premature deletion of valid tokens
- Automatic cleanup of expired session objects, reducing Redis storage requirements and eliminating the need for manual maintenance scripts

This provides an enterprise-grade solution for Redis storage management while maintaining backward compatibility with existing key expiration behavior. Session objects will now be automatically removed from Redis when appropriate, based on the configured TTL settings.
</Accordion>

<Accordion title='Added TLS Certificate Rotation Support for External Service Config'>
Added support for TLS certificate rotation in External Service configurations without requiring Gateway restarts. This enhancement allows organizations with strict certificate rotation policies (such as financial institutions) to update certificates seamlessly during runtime.

Key improvements include:

- Hot-reload capability for TLS certificates in External Service configurations, eliminating the need for Gateway pod restarts during certificate rotation
- New optional configuration toggle allowing the Gateway to intelligently reuse mTLS certificates already configured for upstream connections when making OAuth external service calls
- Reduced operational overhead by removing the requirement for infrastructure changes and custom image rebuilds during routine certificate rotation
- Enhanced certificate management flexibility, supporting scenarios where multiple certificates may be configured for different external service endpoints

This feature is particularly valuable for environments with short certificate validity periods (e.g., 90-day rotation cycles) and maintains full backward compatibility with existing External Service configurations.
</Accordion>

<Accordion title='Added Underscore Support in Vault and Consul Secret Reference Paths'>
Added support for underscore characters in `$secret_vault` and `$secret_consul` secret reference paths. Previously, the underscore character was not supported in these KV store reference paths due to regex pattern limitations, causing references to be truncated at the first underscore. For example, `$secret_vault.kv-v2/path.API_KEY` would only match up to `.API`, silently dropping `_KEY`.
...
<Accordion title='Added Gateway Go Runtime Metrics via OpenTelemetry'>
Added comprehensive Go runtime metrics collection through OpenTelemetry to provide visibility into Gateway resource utilization and performance characteristics. This enhancement enables SRE and DevOps teams to proactively monitor Gateway health and troubleshoot performance issues before they impact users.

The new metrics include:
- CPU utilization and processing time
- Memory usage breakdown (heap, stack, allocations)
- Active goroutine count and lifecycle tracking
- Garbage collection duration and frequency
- Thread count and management

These runtime metrics complement existing request-level RED metrics to provide a complete operational picture of Gateway performance. Teams can now correlate system-level resource constraints with API latency patterns, detect potential goroutine or memory leaks early, and optimize infrastructure sizing based on actual resource utilization patterns.
</Accordion>

<Accordion title='Added Endpoint-Level Metrics via OpenTelemetry'>
Added support for endpoint-level granularity in OpenTelemetry metrics, allowing users to distinguish between different endpoints within the same API. This enhancement provides more detailed observability for API monitoring and analytics.

Key improvements include:

- Renamed the `route` metadata dimension to `listen_path` to accurately reflect that it returns the API's static listen path rather than a dynamic route template
- Added a new `endpoint` metadata dimension that exposes the tracked endpoint path from the TrackEndpointMiddleware, providing per-endpoint visibility in OTel metrics
- Decoupled TrackEndpointMiddleware from the `EnableAnalytics` setting, enabling endpoint tracking in OTel-only deployments without requiring the full analytics pipeline
- Maintained cardinality safety by keeping the `endpoint` dimension empty by default and only populating it when `track_endpoints` is explicitly configured on the API

This feature works with both Classic and OAS APIs that have endpoint tracking configured and follows the same opt-in pattern used by other API gateway solutions for controlling metric cardinality.
</Accordion>

<Accordion title='Added Gateway Configuration Metrics via OpenTelemetry'>
Added OpenTelemetry metrics to continuously monitor Gateway configuration state, enabling platform teams to track deployment success and detect configuration drift. This addresses critical observability gaps where API definitions or policies could disappear during deployments without immediate detection.

The Gateway now exports the following configuration metrics via OTLP:

- `tyk_apis_loaded` - Number of APIs currently loaded on the Gateway
- `tyk_policies_loaded` - Number of policies currently loaded on the Gateway
- `tyk_config_reload_total` - Total count of configuration reload operations
- Configuration reload duration and timestamp metrics

These metrics enable continuous monitoring and alerting scenarios such as detecting when API counts drop below expected thresholds (`tyk_apis_loaded < 90`), identifying complete policy wipes (`tyk_policies_loaded == 0`), and tracking deployment propagation across multiple Gateway instances. Teams can now set up proactive alerts for configuration drift and silent failures that previously went undetected until impacting production traffic.
</Accordion>

<Accordion title='Added OpenTelemetry Metrics Support to Library'>
Extended the existing OpenTelemetry library to support OTLP metrics export alongside tracing capabilities. This provides a unified configuration surface for both traces and metrics exported to the same OTLP backend, meeting industry expectations for OpenTelemetry as the standard observability solution.

Key features include:

- Opt-in metrics via independent `metrics.enabled` configuration flag, leaving existing trace-only consumers unaffected
- Reuses existing exporter settings (gRPC/HTTP), endpoint, TLS, and header configurations already established for tracing
- Provides nil-safe instrument API (counters, histograms, gauges) that consumers can call without defensive checks
- Exposes health and export statistics for monitoring the metrics pipeline itself
- Configurable export intervals, temporality settings, shutdown timeouts, and retry policies

The metrics subsystem maintains full backward compatibility with existing trace-only configurations while enabling customers to consolidate their observability data through a single OpenTelemetry configuration.
</Accordion>

<Accordion title='Added OpenTelemetry Metrics Cardinality Control'>
Added a new `cardinality_limit` configuration option to the OpenTelemetry metrics configuration that allows operators to set a provider-wide cap on the number of unique time series per metric instrument. This prevents runaway memory growth and storage costs when high-cardinality attributes (such as API keys, OAuth IDs, or unique request identifiers) are attached to metric instruments.

When the cardinality limit is reached, new attribute combinations are automatically collapsed into a synthetic overflow data point with the attribute `otel.metric.overflow=true`, preserving total count and sum accuracy while preventing excessive memory usage and series proliferation. This provides protection against backends that enforce series limits, such as Prometheus and Datadog.
</Accordion>

<Accordion title='Added OTel Metrics Exemplar Support'>
Exemplars are now automatically enabled for OTel metrics, providing metric-to-trace correlation capabilities. When a trace is active during a metric measurement, the OTel Go SDK automatically attaches the current `trace_id` and `span_id` to histogram observations.

This enables users to click on exemplar annotations in Grafana to jump directly from metric anomalies (such as latency spikes in the `http.server.duration` histogram) to the specific trace that caused the measurement, eliminating the need to manually hunt for related traces during incident investigation.
</Accordion>

<Accordion title='Added OpenTelemetry Metrics Integration'>
Added initial OpenTelemetry metrics support to the Tyk Gateway with a foundational `tyk.http.requests` counter to validate the end-to-end metrics pipeline. This integration introduces a new metrics provider that supports OTLP export to external collectors for enhanced observability.

The implementation includes:

- A new `Instruments` struct in `internal/otel/metrics.go` that encapsulates the OpenTelemetry metrics provider and gateway instruments
- Safe-to-call methods that no-op when metrics are disabled
- Integration with the existing `config.OpenTelemetry.Metrics` configuration block

Operators can enable metrics collection by setting `"metrics": {"enabled": true}` in their existing `opentelemetry` configuration. The counter increments on every completed HTTP request, providing a foundation for more detailed metrics attributes in future releases.
</Accordion>

<Accordion title='Added Config-Driven OpenTelemetry API Metrics'>
Added comprehensive support for config-driven OpenTelemetry metrics with custom dimensions and per-instrument scoping. This enables operators to define metric instruments (counters, histograms) directly through Gateway configuration without requiring code changes.

Key capabilities include:

- **Custom dimension sources**: Extract dimensions from request metadata, headers, session data, context variables, and upstream response headers using a unified `{source, key, label, default}` configuration format
- **Per-instrument dimension scoping**: Each metric instrument declares its own dimensions in config, ensuring optimal cardinality control and performance
- **Conditional data loading**: The Gateway only loads expensive data sources (session lookups, context variables, upstream responses) when at least one configured instrument actually requires that data
- **Built-in cardinality protection**: Each instrument maintains N≤10 dimensions by design, staying under OpenTelemetry SDK fast-path thresholds

This addresses multi-tenant observability requirements where operators need customer-specific dimensions (customer ID, tier, organization) in Prometheus metrics while maintaining high performance on the request path. The feature builds on the OpenTelemetry infrastructure introduced in Phase 1 and provides a native alternative to custom plugins that can introduce unacceptable latency.
</Accordion>

<Accordion title='Added Configurable OTel Resource Attributes for Gateway Identity'>
Added support for configurable OpenTelemetry resource attributes to help identify individual Gateway instances in multi-node deployments. Resource attributes are set once at provider initialization and carried in the ResourceMetrics envelope alongside all exported metrics, enabling platform teams to distinguish between Gateway instances, filter metrics by edge cluster, and compare behavior across a fleet.

The Gateway now supports attaching gateway-specific identity information such as node ID, group ID, and segment tags to exported telemetry. How these attributes surface as queryable labels depends on the downstream backend configuration - for example, Prometheus requires enabling `resource_to_telemetry_conversion` in the OTel Collector, while New Relic automatically attaches resource attributes as dimensions.

This enhancement provides the foundation for more granular observability and monitoring in distributed Gateway deployments.
</Accordion>

<Accordion title='Restructured OpenTelemetry Configuration to Separate Traces and Metrics'>
Restructured the OpenTelemetry configuration in the Gateway to provide better developer experience when configuring traces and metrics independently. The configuration now uses separate `opentelemetry.traces` and `opentelemetry.metrics` objects, allowing users to enable and configure each telemetry type independently.

The new structure includes:
- `opentelemetry.traces` object containing all trace-specific configuration including `enabled`, `exporter`, `endpoint`, `headers`, `connection_timeout`, `resource_name`, `tls`, `span_processor_type`, `span_batch_config`, `context_propagation`, `custom_trace_header`, and `sampling` fields
- `opentelemetry.metrics` object for metrics configuration with `enabled`, `exporter`, `endpoint`, `export_interval`, and `api_metrics` fields
- Full backward compatibility with existing root-level trace configuration fields (now marked as deprecated)

This change provides clearer separation of concerns and simplifies configuration management for deployments that require only traces or only metrics telemetry.
</Accordion>

<Accordion title='Added Configurable API Compression Size Limit'>
Added a new Gateway configuration option `storage.max_decompressed_size` to control the maximum size limit for API definition decompression. This enhances the API compression feature by allowing operators to adjust the decompression limit based on their deployment requirements.

When `storage.max_decompressed_size` is not configured, the Gateway maintains the default 100MB limit for backward compatibility. When set to a positive value ≥ 1MB, the Gateway uses that value as the decompression cap. Values below 1MB are automatically clamped to 1MB with a warning logged to prevent potential security issues while avoiding service disruption.

This configuration change applies only to Gateway-level storage settings and does not affect the public API definition schema or Dashboard contracts.
</Accordion>

<Accordion title='Added Centralised Error Overrides Infrastructure'>
Added a foundational infrastructure for gateway-level error overrides that allows operators to standardise, transform, or mask error responses returned by the gateway. This infrastructure provides centralised rule compilation, efficient runtime matching, and flexible response generation.

The new error override system features:

- Map-based configuration indexed by HTTP status code for O(1) lookups and efficient fallback matching
- Support for exact status code overrides (`401`, `500`) and pattern matching (`4xx`, `5xx`)
- Multiple response override modes including explicit body, inline body template, file-based template, and message-only options
- Custom headers and content-type aware template rendering (JSON/XML)
- Additional matching logic for error flags, response body field/value patterns, and message regex matching

All gateway-generated errors flow through the centralised `HandleError()` function in the gateway request lifecycle, enabling error overrides to be applied without requiring additional request middleware. The matching and override engine is designed to be shared between gateway-generated errors and upstream service errors (4xx/5xx responses from upstream services).
</Accordion>

<Accordion title='Added Per-API Error Response Overrides'>
Added support for API-level error response overrides, allowing individual APIs to customize HTTP error responses independently of global gateway configuration. This feature extends the existing gateway-level error override infrastructure to the API definition level, enabling API owners to define their own override rules using the same configuration structure.

The system supports customization of:
- Tyk-generated errors (authentication failures, rate limits, validation errors)
- Reverse proxy failures (timeouts, DNS errors, circuit breaker events)
- Inline body templates and template file rendering
- Custom HTTP headers and status code overrides
- Pattern matching by status code and additional matching criteria

API-level overrides follow the same configuration structure as gateway-level overrides, providing consistency while allowing granular control over error responses for specific APIs.
</Accordion>

<Accordion title='Added ConfigData Dimension Source for OpenTelemetry Metrics'>
Added support for using API-level custom metadata from `config_data` as OpenTelemetry metric dimensions. Users can now configure a new dimension source `config_data` that extracts values from an API definition's `config_data` map into metric attributes.

For example, if an API has `config_data: {"environment": "staging", "team": "platform"}`, users can define dimensions like `{"source": "config_data", "key": "environment"}` and the metric will include `configdata.environment="staging"` as an attribute.

- Follows the same pattern as existing dynamic dimension sources like `context` and `header`
- Accepts any key from the `config_data` map and converts non-string values using `fmt.Sprint`
- Respects the existing `config_data_disabled` flag - when disabled, dimensions fall back to their configured default values
- Only populates `config_data` when needed by configured dimensions, avoiding unnecessary processing overhead
</Accordion>
...
Updated the Tyk Gateway and Plugin Compiler to use Go 1.25.5, ensuring compatibility with the latest Go runtime features and reducing exposure to potential security vulnerabilities in older versions. This update includes fixes for nil-pointer handling and TLS behavior changes introduced in Go 1.25.
</Accordion>

<Accordion title='Upgraded OpenTelemetry SDK Dependencies'>
...
<Accordion title='Reduced Log Noise for TLS Certificate Authentication Errors'>
...
Resolved an issue where a Tyk Gateway acting as a client (using upstream mTLS) would fail to authenticate against another mTLS-protected Tyk Gateway or upstream server, resulting in a `403 Forbidden: "Client TLS certificate is required"` error. This occurred because the gateway's TLS client could withhold the configured certificate if the target server's requested Certificate Authorities did not perfectly match the certificate's issuer chain. The gateway has been updated to reliably present the configured upstream client certificate whenever requested by the target server, ensuring seamless mTLS communication between gateways and other upstream services.
</Accordion>

<Accordion title='Fixed Gateway TLS Version Default to Follow Go Standards'>
Resolved a regression where the Gateway was defaulting to TLS 1.2 instead of TLS 1.3 when no explicit TLS version configuration was provided. This restores the previous behavior and aligns with Go's native TLS defaults, ensuring better security by default while following industry standards.

Previously, when `TYK_GW_HTTPSERVEROPTIONS_MAXVERSION` or `TYK_GW_PROXYSSLMAXVERSION` were not explicitly configured, the Gateway would force TLS 1.2 as the maximum version instead of allowing Go's default TLS 1.3.

The Gateway now uses Go's native TLS defaults when no explicit version limits are configured. Users who need to restrict TLS to version 1.2 (or other specific versions) must now explicitly set `TYK_GW_HTTPSERVEROPTIONS_MAXVERSION` for client-to-Gateway connections or `TYK_GW_PROXYSSLMAXVERSION` for Gateway-to-upstream connections to their desired version.
</Accordion>

<Accordion title='Fixed CORS Preflight Requests Blocked by AllowList Middleware'>
Resolved an issue where CORS preflight OPTIONS requests were incorrectly blocked by the AllowList middleware when `options_passthrough` was disabled. Previously, when APIs had CORS enabled with `options_passthrough: false` and AllowList middleware applied, browsers' CORS preflight requests would fail with "Requested endpoint is forbidden" errors because OPTIONS endpoints were not explicitly defined in the AllowList configuration.

Tyk now properly recognizes CORS preflight requests and allows them to bypass AllowList middleware checks when `options_passthrough` is disabled, restoring the expected behavior where users don't need to explicitly define OPTIONS endpoints in their AllowLists for CORS to function correctly.
</Accordion>

<Accordion title='Fixed Policy ID Validation to Prevent Special Characters'>
Resolved an issue where policy IDs containing special characters (`#`, `?`, `%`, `/`) could cause problems when parsing Dashboard and Gateway API endpoint requests. Previously, users could create policies with IDs containing these characters, which would then fail when used in URL paths for policy management endpoints like `/api/portal/policies/{id}` and `/tyk/policies/{polID}`.

Tyk now enforces strict validation of policy IDs across both Dashboard and Gateway APIs, restricting policy identifiers to a safe character set (alphanumeric characters, `_`, `-`, `.`, `~`). The validation occurs when creating or updating policies via POST and PUT operations, and returns an error if unacceptable characters are detected in the `id` field.
</Accordion>

<Accordion title='Fixed Analytics Generation for OAS API Mock Endpoints'>
Resolved a regression introduced in v5.8.6 where OAS APIs with mock endpoints stopped generating analytics data. Previously, analytics were correctly captured for mock responses in OAS APIs since the Mock Middleware executes at the end of the middleware chain for OAS APIs, making analytics generation expected behavior.

The Gateway now properly restores analytics generation for OAS API mock endpoints while maintaining the existing behavior for Classic APIs, which intentionally do not generate analytics for mock endpoints. This fix ensures consistency with pre-v5.8.6 behavior for OAS APIs without affecting the resolution of the internal API proxying issue that was addressed in v5.8.6.
</Accordion>

<Accordion title='Fixed OpenTelemetry Configuration File Settings Being Ignored'>
Resolved an issue where OpenTelemetry settings in the Gateway configuration file were being ignored. Previously, configuring OpenTelemetry via the `opentelemetry` section in the Gateway config file (including `enabled`, `exporter`, and `endpoint` fields) had no effect, and the Gateway would only respond to environment variable settings like `TYK_GW_OPENTELEMETRY_ENABLED`. The Gateway now properly reads and applies OpenTelemetry configuration from the config file as documented.
</Accordion>

<Accordion title='Fixed Missing Default Configuration for Span Batch Processing'>
Resolved an issue where SpanBatchConfig was not initialized with proper default values. Previously, this could cause span batch processing to behave unpredictably or fail when no explicit configuration was provided. The Gateway now properly sets appropriate defaults for span batch processing configuration, ensuring consistent behavior across all installations.
</Accordion>

<Accordion title='Fixed Missing API Metrics and Access Logs for Cache Middleware'>
Resolved an issue where cache hits served by the Redis cache middleware were missing observability signals. Previously, when a response was served from cache, the middleware would short-circuit the request chain and only record analytics, failing to produce access log entries or OpenTelemetry API metrics. This made cached responses invisible to operators monitoring the Gateway. The cache middleware now properly generates all observability signals for cached responses, ensuring complete visibility into both cache hits and cache misses.
</Accordion>

<Accordion title='Fixed Missing API and Policy Count Metrics When Gateway Fails MDCB Connection'>
Resolved an issue where the `tyk.gateway.apis.loaded` and `tyk.gateway.policies.loaded` gauge metrics were not emitted when the Gateway failed to connect to MDCB during startup. Previously, if the Gateway could not establish an MDCB connection, these metrics would not be reported at all, making it difficult to detect configuration or connection failures. The Gateway now properly emits these metrics with a value of 0 when no APIs or policies are loaded due to MDCB connection failures, providing a clear indicator of startup issues.
</Accordion>
...
The [Logstash](/api-management/logs#logstash) formatter timestamp is now in [RFC3339Nano](https://www.rfc-editor.org/rfc/rfc3339) format.
</Accordion>
...
We offer support for integrating *OpenTelemetry* traces with supported open source tools such [Jaeger](/api-management/traces#using-docker), [Dynatrace](/api-management/traces#dynatrace) or [New Relic](/api-management/traces#new-relic). This allows API owners and developers to gain troubleshooting and performance insights from error logs, response times etc.
...
Added configuration parameter to enable/disable [detailed_tracing](/api-management/tyk-pump#configure-detailed-recording) for *Tyk Classic API*.
</Accordion>
