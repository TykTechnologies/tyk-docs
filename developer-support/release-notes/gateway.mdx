...
Resolved critical path matching inconsistencies that prevented OAS-specific middleware from executing correctly in scenarios that worked properly with Classic APIs. Fixed an issue where ValidateRequest and MockResponse middleware failed to trigger for subpaths (e.g., middleware configured for `/users` would not execute for `/users/123`), breaking feature parity with Classic API behavior. Resolved wildcard path matching problems where patterns like `/.*` or root paths `/` did not apply middleware to all endpoints as expected, preventing customers migrating from other API gateways from using global middleware configurations. Additionally, fixed mock response failures for child versions of OAS APIs and resolved middleware execution issues when APIs were accessed without trailing slashes. The fix implements a unified path matching algorithm that ensures OAS APIs use the same regex-based matching system as Classic APIs, providing consistent middleware behavior across all API types while maintaining full backward compatibility and regex flexibility.
</Accordion>
