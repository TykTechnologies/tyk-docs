...
This release introduces the next-generation (`-ng`) plugin compiler with native arm64 support, alongside several changes and fixes to improve the stability and correct the behavior of the Gateway.

The underlying Go toolchain has been updated to Go 1.26 - all Go plugins must be recompiled with this Go version.
