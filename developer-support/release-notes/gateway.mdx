...
This release also adds GCP IAM-based authentication to Redis/Valkey for Google Cloud Memorystore deployments, ships the next-generation (`-ng`) plugin compiler with native arm64 support, updates the Go toolchain to Go 1.26, and includes a set of smaller fixes covering request validation, GraphQL Playground, JWT logging, session deletion under load, and shutdown log formatting. This release also addresses several CVEs in dependent libraries.
...
Existing provider-specific references (`vault://`, `consul://`, `env://`, `secrets://`, and the `$secret_<provider>.<key>` inline form) continue to work exactly as before, so no existing API definition needs to change. The new syntax is purely additive, giving you one consistent way to reference configuration values going forward regardless of which store they come from.
</Accordion>
