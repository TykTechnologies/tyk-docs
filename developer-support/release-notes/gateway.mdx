...
Tyk Gateway now supports a first-class [OAuth 2.0 authentication method](/api-management/authentication/oauth-2) for Tyk OAS APIs, enabling token validation against an external Identity Provider such as Keycloak, Okta, or Auth0 that acts as the Authorization Server. Tyk acts as the OAuth 2.0 protected resource, validating inbound bearer tokens against the IdP's JWKS endpoint. No token issuance, just enforcement.
