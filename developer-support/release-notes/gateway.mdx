...
Teams now have complete operational visibility with automatic RED metrics (Rate, Errors, Duration), Go runtime health monitoring, and configuration state tracking.
...
**MCP Gateway**
---
We have introduced the MCP (Model Context Protocol) Gateway, enabling teams to proxy, secure, and manage AI agent tool servers within Tyk API management. MCP Gateway is generally available in this release.
...
Per-primitive rate limiting sets independent rate limits for each method or named primitive within a key or policy. When multiple policies apply, access lists merge as a union and rate limits use the most permissive value.

MCP traffic now appears in Gateway access logs with dedicated fields (`mcp_method`, `mcp_primitive_type`, `mcp_primitive_name`) alongside a new `api_type` field present on all access log entries.
...
You must set [http_server_options.max_version](/tyk-oss-gateway/configuration#http_server_options-max_version) to `771` (or the equivalent environment variable [TYK_GW_HTTPSERVEROPTIONS_MAXVERSION](/tyk-oss-gateway/configuration#http_server_options-max_version)) if you require an upper limit of TLS 1.2.
...
<Accordion title='Add Tool-Based Access Control (TBAC) for MCP Proxy APIs'>
...
<Accordion title='Add per-primitive rate limiting for MCP Proxy APIs'>
...
<Accordion title='Add Rate Limit Header Configuration Option'>
We have added a new Gateway configuration option [rate_limit_response_headers](/tyk-oss-gateway/configuration#rate_limit_response_headers) that allows `X-RateLimit-*` headers to be populated from rate limit data instead of quota data. This addresses industry standard compliance and reduces customer confusion when receiving HTTP 429 responses.
...
When [rate_limit_response_headers](/tyk-oss-gateway/configuration#rate_limit_response_headers) is set to `"rate_limits"`, the headers now correctly show:
...
<Accordion title='Add Configurable Session Object TTL Management for Redis'>
...
- Enhanced [session_lifetime](/tyk-oss-gateway/configuration#session_lifetime) configuration at the API level and [global_session_lifetime](/tyk-oss-gateway/configuration#global_session_lifetime) at the Gateway level to properly manage Redis object TTL
- Added [session_lifetime_respects_key_expiration](/tyk-oss-gateway/configuration#session_lifetime_respects_key_expiration) setting to ensure session objects are not deleted from Redis before their associated keys have expired
...
<Accordion title='Add TLS Certificate Rotation Support for External Service Config'>
We have added support for TLS certificate rotation in External Service configurations without requiring Gateway restarts. This enhancement allows organizations with strict certificate rotation policies to update certificates seamlessly during runtime.
...
<Accordion title='Add Underscore Support in Vault and Consul Secret Reference Paths'>
...
<Accordion title='Add Configurable API Compression Size Limit'>
We have added a new Gateway configuration option [storage.max_decompressed_size](/tyk-oss-gateway/configuration#storage-max_decompressed_size) to control the maximum size limit for API definition decompression. This enhances the API compression feature by allowing operators to adjust the decompression limit based on their deployment requirements.

When [storage.max_decompressed_size](/tyk-oss-gateway/configuration#storage-max_decompressed_size) is not configured, the Gateway maintains the default 100MB limit for backward compatibility. When set to a positive value ≥ 1MB, the Gateway uses that value as the decompression cap. Values below 1MB are automatically clamped to 1MB with a warning logged to prevent potential security issues while avoiding service disruption.
...
<Accordion title='Add Centralized Error Response Customization'>
We have added a comprehensive mechanism to customize HTTP error responses generated by both the Tyk Gateway and upstream services. This enhancement allows operators to fully control error outputs to standardize formats, mask sensitive information, and comply with industry standards.
...
<Accordion title='Add Gateway Go Runtime Metrics via OpenTelemetry'>
We have added comprehensive Go runtime metrics collection through OpenTelemetry to provide visibility into Gateway resource utilization and performance characteristics. This enhancement enables teams to proactively monitor Gateway health and troubleshoot performance issues before they impact users.
...
<Accordion title='Fix query parameter handling in Tyk internal loops (tyk://)'>
We have resolved inconsistent query parameter handling in URL rewrites using Tyk internal loops (`tyk://api-id/path`). Previously, custom query parameters specified in the `rewrite_to` URL were silently dropped, while original request parameters were unexpectedly preserved.
...
We have resolved an issue where a Tyk Gateway acting as a client (using upstream mTLS) would fail to authenticate against another mTLS-protected Tyk Gateway or upstream server, resulting in `HTTP 403 Forbidden: "Client TLS certificate is required"` errors.
...
<Accordion title='Fix Default Gateway TLS Version to TLS 1.3'>
...
We have resolved an issue where OpenTelemetry settings in the Gateway configuration file were not being applied. Previously, when users configured OpenTelemetry via the `opentelemetry` section in the Gateway config file (including `enabled`, `exporter`, and `endpoint` fields), these settings were ignored, and only environment variables like `TYK_GW_OPENTELEMETRY_ENABLED` would take effect.

Tyk Gateway now properly reads and applies OpenTelemetry configuration from the config file as documented.
</Accordion>

<Accordion title='Resolve issue with Gateway entering an unresponsive state during registration failures'>
...
<Accordion title='Fix Missing Default Configuration for Span Batch Processing'>
We have resolved an issue where SpanBatchConfig was not initialized with proper default values. Previously, this could cause span batch processing to behave unpredictably or fail when no explicit configuration was provided. The Gateway now properly sets appropriate defaults for span batch processing configuration, ensuring consistent behavior across all installations.
</Accordion>

<Accordion title='Fix Missing API Metrics and Access Logs for Cache Middleware'>
We have resolved an issue where cache hits served by the Redis cache middleware were missing observability signals.

Previously, when a response was served from cache, the middleware would short-circuit the request chain and only record analytics, failing to produce access log entries or OpenTelemetry API metrics. This made cached responses invisible to operators monitoring the Gateway.

The cache middleware now properly generates all observability signals for cached responses, ensuring complete visibility into both cache hits and cache misses.
</Accordion>

<Accordion title='Fix Missing API and Policy Count Metrics When Gateway Fails MDCB Connection'>
We have resolved an issue where the `tyk.gateway.apis.loaded` and `tyk.gateway.policies.loaded` gauge metrics were not emitted when the Gateway failed to connect to MDCB during startup.

Previously, if the Gateway could not establish an MDCB connection, these metrics would not be reported at all, making it difficult to detect configuration or connection failures. The Gateway now properly emits these metrics with a value of 0 when no APIs or policies are loaded due to MDCB connection failures, providing a clear indicator of startup issues.
</Accordion>

<Accordion title='Fix VEM path preservation for undefined MCP primitives'>
We have resolved an issue where the Virtual Endpoint Method (VEM) path was not preserved during version checking when a request referenced an MCP primitive not defined in the API. The Gateway now correctly retains the path through the version check, preventing requests to undefined primitives from being incorrectly routed or rejected.
</Accordion>
