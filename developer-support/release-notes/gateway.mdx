...
<Accordion title='Expose Gateway-Only latency in Tyk metrics'>
- Implemented request-level timing by tracking the exact timestamp when a request enters the gateway and using it for accurate end-to-end latency calculations. Previously was only measuring the proxy.
- Added `gateway` field to the `latency` struct in traffic logs.
</Accordion>
...
<Accordion title='Enhanced `X-Forwarded-For` IP Selection with Configurable Depth Control'>
Added `xff_depth` configuration parameter to the Gateway's HTTP server options, enabling operators to specify which IP address from the `X-Forwarded-For` header chain should be considered the real client IP. The feature counts from the rightmost (most trusted) end of the IP chain for improved security while maintaining backward compatibility. When `xff_depth` is set to 0 or not configured, Tyk continues using the first IP address as before. Updated default configurations across Tyk Demo, Helm Charts, and example files to use `xff_depth=1` for enhanced security in new deployments.
</Accordion>
...
Implemented OpenTelemetry trace context propagation to maintain request tracing visibility as requests flow through plugins, with specific support for gRPC plugins. Enhanced the Protocol Buffer definitions and Dispatcher interface to include trace context fields, updated the CoProcessor and GRPCDispatcher to preserve trace information, and added OpenTelemetry gRPC interceptors for seamless context propagation. This eliminates observability blind spots in plugin processing, allowing customers to see complete end-to-end traces of API requests, including all plugin activities. Includes helper utilities, examples, and documentation for plugin developers while maintaining backward compatibility with existing plugins.
</Accordion>
...
Fixed an issue where Gateways with other Rate Limiters than DRL would panic when receiving a DRL update from Redis.
</Accordion>
...
Fixed an issue where Tyk advertised leaf subject DNs instead of CA DNs, causing mTLS handshake failures with strict TLS clients. Handshakes now work according to RFC, ensuring compatibility with standards-compliant clients like `Apache mod_ssl`.
</Accordion>
...
<Accordion title='Fixed OAuth Client Key Retrieval Failure in JWT APIs on Hybrid Gateways'>
Resolved performance and caching issues in hybrid gateway deployments where JWT APIs with Keycloak IDP failed to retrieve OAuth client keys from local Redis storage. The gateway was unable to find keys with the `oauth-clientid.` prefix in the local keystore. Fixed the key caching mechanism to ensure OAuth client data is properly stored and retrieved from local Redis. The fix ensures OAuth client keys are correctly synchronized and cached locally in hybrid deployments, providing consistent performance without impacting existing functionality.
</Accordion>
...
- [Docker image to pull](https://hub.docker.com/r/tykio/tyk-gateway/tags?page=&page_size=&ordering=&name=v5.10.2)
  - ```bash
    ...
    ``` 
- Helm charts
  - [tyk-charts v4.0.0](/developer-support/release-notes/helm-chart#410-release-notes)
