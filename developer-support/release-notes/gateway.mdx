...
**Local filesystem storage option for key-value references**
...
** Easier TLS certificates rotation using key-value storage**

API definitions can now carry a PEM-encoded certificate as a string stored inline in any of the TLS certificate fields. Combining this with Tyk's existing [key-value secret storage]() unlocks the ability to store certificates directly in the secure secret store locally to the data plane. Using a reference to the stored PEM file in the API definition, Tyk will automatically retrieve the certificate when resolving secrets at Gateway load time. You can use a hot reload of the Gateeway to rotate certificates without making any change to the API definition by changing the certificate in the secret store.
...
Configure the per-endpoint timeout in the Tyk Vendor Extension usng the new `duration` field in the per-operation [`enforceTimeout`](/api-management/gateway-config-tyk-oas#enforcetimeout) object. This accepts human-readable durations in milliseconds, seconds, or minutes (for example `500ms`, `1.5s`, or `2m`).
...
- `legacy`: preserves the format used in Gateway 5.13.0 and earler, with the historical timestamp format and the `msg` field name.
...
Followng the delay period, the existing [graceful shutdown](/planning-for-production/ensure-high-availability/graceful-shutdown#tyk-gateway) process continues.
</Accordion>
