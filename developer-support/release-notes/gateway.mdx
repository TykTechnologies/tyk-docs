...
This release significantly enhances observability capabilities with comprehensive OpenTelemetry metrics support, including Go runtime metrics, gateway configuration metrics, and API metrics with custom dimensions. Key improvements include better error handling with centralized error override infrastructure, enhanced MCP (Model Context Protocol) support for AI API traffic visibility, and TLS certificate rotation without requiring gateway restarts. The release also resolves critical authentication issues with client-mTLS and CORS preflight handling, while addressing several regressions in analytics generation and OpenTelemetry configuration processing.
...
<li>
<details>
<summary>Added option to populate X-RateLimit headers from rate limits rather than quotas</summary>

Added a new configuration option to populate X-RateLimit-* headers with rate limit information instead of quota data, aligning with industry standards. This resolves customer confusion when receiving HTTP 429 responses, as the headers now correctly show rate limit data (requests allowed, remaining, and reset time) rather than quota information. The change helps customers implement proper retry logic by providing accurate rate limiting context.
</details>
</li>
<li>
<details>
...
Enhanced Redis storage management to provide better control over expired keys and session objects, addressing potential unlimited growth in Redis storage requirements. This improvement reduces the need for manual cleanup scripts and provides a more enterprise-grade approach to managing Tyk's Redis storage footprint. The enhancement ensures that session objects created for API requests and access tokens are properly managed with appropriate TTL settings.
</details>
</li>
<li>
<details>
<summary>Added support for TLS certificate rotation in External Service Config without requiring Gateway restart</summary>

Enhanced the External Service configuration to support automatic TLS certificate rotation without requiring a Gateway restart. This improvement eliminates the operational overhead of manual certificate updates and reduces infrastructure complexity for organizations with strict certificate rotation policies, particularly benefiting financial institutions and other enterprises that require frequent certificate updates for security compliance.
</details>
</li>
<li>
<details>
<summary>Added support for underscores in Vault and Consul secret reference paths</summary>

Tyk now supports underscores in $secret_vault.* and $secret_consul.* key-value store reference paths. Previously, references containing underscores were truncated at the first underscore character, forcing users to modify their secret naming conventions. This enhancement aligns Vault and Consul secret references with the existing behavior of $secret_env, $tyk_context, and $tyk_meta patterns, which already supported underscores.
</details>
</li>
<li>
<details>
<summary>Updated Gateway and Plugin Compiler to Go 1.25</summary>

Updated the Tyk Gateway and Plugin Compiler from Go 1.24.6 to Go 1.25.5 to maintain security best practices and stay current with the latest Go version. The update includes compatibility fixes for compiler changes, TLS behavior adjustments, and resolution of any test failures. This ensures Tyk continues to benefit from the latest Go security patches and performance improvements.
</details>
</li>
<li>
<details>
<summary>Added Go runtime metrics via OpenTelemetry for enhanced gateway observability</summary>

Added comprehensive Go runtime metrics collection through OpenTelemetry, providing visibility into CPU usage, memory consumption, goroutine counts, garbage collection performance, and thread utilization. This enhancement enables SRE and DevOps teams to proactively monitor gateway health, plan capacity scaling, and troubleshoot performance issues before they impact API traffic. The metrics follow OpenTelemetry Go Runtime Semantic Conventions for standardized observability.
</details>
</li>
<li>
<details>
<summary>Added gateway configuration metrics via OpenTelemetry</summary>

Added new OpenTelemetry metrics that continuously export gateway configuration state, including API and policy counts, reload timestamps, and reload duration. Platform teams can now monitor configuration drift, set up alerts for missing APIs or policies, and track deployment propagation in their observability platforms like Prometheus, Grafana Cloud, or DataDog. This complements the existing configuration API with time-series metrics suitable for ongoing monitoring and alerting.
</details>
</li>
<li>
<details>
<summary>Added OpenTelemetry metrics support to complement existing tracing capabilities</summary>

Extended the OpenTelemetry library to support OTLP metrics export alongside the existing tracing functionality. This enhancement allows users to configure both traces and metrics through a single interface, exporting to the same OTLP backend such as Datadog, Grafana Cloud, or New Relic. The metrics subsystem is opt-in via configuration and reuses existing exporter settings, providing a unified observability solution without affecting current trace-only deployments.
</details>
</li>
<li>
<details>
<summary>Upgraded OpenTelemetry SDK to latest versions for improved performance</summary>

Updated OpenTelemetry dependencies from mixed legacy versions (some 22 minor versions behind) to the latest releases, bringing substantial performance improvements to span lifecycle methods, trace context propagation, and metrics recording. These optimizations enhance hot path operations that are executed on every HTTP request through Tyk Gateway, while also eliminating potential runtime compatibility issues between mismatched core and exporter versions.
</details>
</li>
<li>
<details>
<summary>Added OpenTelemetry metrics support to Tyk Gateway</summary>

Integrated the OpenTelemetry metrics library into Tyk Gateway with a basic HTTP request counter to validate the end-to-end metrics pipeline. The gateway now records a `tyk.http.requests` counter that increments on every completed request when OpenTelemetry metrics are enabled in the configuration. This establishes the foundation for comprehensive metrics collection and export to OpenTelemetry collectors.
</details>
</li>
<li>
<details>
<summary>Added config-driven OpenTelemetry API metrics with custom dimensions</summary>

Tyk now supports configurable OpenTelemetry metrics instruments (counters, histograms) that can be defined entirely through gateway configuration. Operators can specify custom dimensions sourced from request metadata, headers, session data, context variables, and upstream response headers without requiring code changes. This enables multi-tenant observability scenarios where metrics need to include customer-specific attributes like customer ID, tier, or organization data.
</details>
</li>
<li>
<details>
<summary>Added OpenTelemetry resource attributes to identify gateway instances</summary>

Tyk Gateway now attaches gateway-specific resource attributes to exported OpenTelemetry metrics, enabling platform teams to distinguish between gateway instances in multi-node deployments. These resource attributes provide gateway identity information that allows filtering metrics by edge cluster and comparing behavior across a fleet of gateways. The attributes are set once at provider initialization and carried with all exported metrics in the ResourceMetrics envelope.
</details>
</li>
<li>
<details>
<summary>Added configurable compression size limit for API definitions</summary>

Added a new gateway configuration option `storage.max_decompressed_size` that allows administrators to set a custom limit for uncompressed API definition sizes when using the compression feature. This replaces the previous hard-coded 100MB limit, enabling deployments with larger API definition sets to configure an appropriate threshold while maintaining protection against decompression bomb attacks.
</details>
</li>
<li>
<details>
<summary>Added centralised error override infrastructure for customising HTTP error responses</summary>

Implemented a new centralised infrastructure that allows users to customise HTTP error responses returned by the Tyk Gateway. All gateway-generated errors now flow through a central HandleError() function where custom overrides can be applied. This foundation enables consistent error response customisation across the gateway without requiring additional middleware, with future support planned for upstream service error handling.
</details>
</li>
<li>
<details>
<summary>Added per-API error response overrides</summary>

Tyk now supports custom error response overrides at the individual API level, extending the existing gateway-level capability. APIs can define their own override rules using the same configuration structure, allowing different APIs to customize HTTP error responses independently. This provides granular control over error messaging and formatting for Tyk-generated errors on a per-API basis.
</details>
</li>
<li>
<details>
<summary>Added MCP identity capture in analytics records for better visibility into AI API traffic</summary>

Enhanced analytics to capture MCP (Model Context Protocol) specific identity data, allowing API owners to see which MCP primitives (tools, resources, prompts) are being called instead of only seeing generic POST requests. This provides meaningful visibility into AI API traffic patterns, usage metrics, and performance data. Previously, all MCP requests appeared as identical HTTP POST requests to a single endpoint, making it impossible to distinguish between different tools or operations in analytics dashboards.
</details>
</li>
<li>
<details>
<summary>Added MCP-specific dimensions to OpenTelemetry metrics</summary>

Added four new metadata extractors that enable MCP-specific dimensions on existing OpenTelemetry metrics. Users can now observe MCP traffic patterns including tool calls, resource reads, and prompt invocations through detailed metrics that align with official OTel semantic conventions. This enhancement provides better visibility into MCP proxy usage without requiring additional configuration changes.
</details>
</li>
<li>
<details>
<summary>Added MCP-specific fields and API type classification to access logs</summary>

Added an `api_type` field to all access log entries that classifies traffic as "mcp", "graphql", "oas", or "classic" based on the API configuration. For MCP APIs, four additional structured fields are now included in access logs to capture MCP-specific request details. This enhancement enables operators to filter and analyze traffic by protocol type directly from access logs, improving observability for multi-protocol environments.
</details>
</li>
</ul>
...
<li>
<details>
<summary>Fixed logging level for TLS certificate authentication errors</summary>

Resolved an issue where "Client TLS certificate is required" errors were logged at DEBUG level instead of INFO level. This change ensures that TLS certificate authentication failures are properly visible in production monitoring and security auditing, as these errors represent legitimate security events that administrators need to track.
</details>
</li>
<li>
<details>
<summary>Fixed client-mTLS authentication failure when Tyk Gateway acts as a client</summary>

Resolved an issue where a Tyk Gateway acting as a client would fail to authenticate against another mTLS-protected Tyk Gateway or upstream server, resulting in a "403 Forbidden: Client TLS certificate is required" error. The problem occurred because the gateway's TLS client would withhold the configured certificate if the target server's requested Certificate Authorities did not perfectly match the certificate's issuer chain. The gateway now reliably presents the configured upstream client certificate whenever requested by the target server, ensuring seamless mTLS communication between gateways and upstream services.
</details>
</li>
<li>
<details>
<summary>Fixed CORS preflight OPTIONS requests being blocked by AllowList middleware</summary>

Resolved a regression where CORS preflight OPTIONS requests were incorrectly blocked by the AllowList middleware when options_passthrough was disabled. This occurred because OPTIONS endpoints were not explicitly defined in AllowList configurations, causing preflight requests to fail with "Requested endpoint is forbidden" errors and preventing legitimate API calls from completing.
</details>
</li>
<li>
<details>
<summary>Fixed policy ID validation to prevent special characters that break API endpoints</summary>

Resolved an issue where policy IDs containing special characters like #, ?, %, and / would cause API endpoint parsing failures when used in URL paths. Tyk now enforces strict validation on policy IDs across both Dashboard and Gateway APIs, restricting them to a safe character set. This prevents broken API requests while maintaining the ability to use custom policy identifiers within the validated character constraints.
</details>
</li>
<li>
<details>
<summary>Fixed regression where OAS APIs no longer generated analytics on mock endpoints</summary>

Resolved a regression introduced in v5.8.6 where mock endpoints in OpenAPI Specification (OAS) APIs stopped generating analytics data. This affected users who rely on analytics to monitor mock endpoint usage during development and testing workflows. Mock endpoints in OAS APIs now correctly generate analytics data as they did in versions prior to v5.8.6.
</details>
</li>
<li>
<details>
<summary>Fixed OpenTelemetry configuration settings being ignored in Gateway config file</summary>

Resolved an issue where OpenTelemetry settings specified in the Gateway configuration file were not being applied. Previously, the Gateway would ignore the opentelemetry configuration block in the config file and only respond to environment variables. The Gateway now properly reads and applies OpenTelemetry settings from both the configuration file and environment variables as expected.
</details>
</li>
<li>
<details>
<summary>Fixed missing API metrics and access logs for cache middleware responses</summary>

Resolved a bug where responses served from the Redis cache middleware were missing observability signals. When the cache middleware served a response from cache, it would short-circuit the request chain and only record analytics, skipping access log entries and OpenTelemetry API metrics. Cached responses now properly generate all observability data, ensuring operators have complete visibility into gateway traffic including cache hits.
</details>
</li>
</ul>
