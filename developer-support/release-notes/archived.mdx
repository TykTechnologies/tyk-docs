...
See our [Dashboard SSO documentation](/platform-management/dashboard-api#single-sign-on-api) for more details.
...
There is a [new section](/platform-management/dashboard-admin-api#dashboard-audit-logs) in the Tyk Dashboard config file where you can specify parameters for the audit log (containing audit records for all requests made to all endpoints under the `/api` route).
...
Instead of setting permissions per user, you can now [create a user group](/api-management/user-management#manage-tyk-dashboard-user-groups), and assign it to multiple users. It works for Single Sign-On too, just specify group ID during [SSO API](/platform-management/dashboard-admin-api#single-sign-on-api) flow.
...
From an API standpoint, user groups can be managed by [new Dashboard API](/platform-management/dashboard-api#user-groups-api). The User object now has a new `group_id` field, and if it is specified, all permissions will be inherited from the specified group. [SSO API](/platform-management/dashboard-admin-api#single-sign-on-api) has been updated to include `group_id` field as well.
...
* [API Docs](/platform-management/dashboard-api#manage-apis-api-definition)
