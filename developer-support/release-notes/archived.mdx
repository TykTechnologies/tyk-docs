...
See our [Distributed Tracing documentation](/api-management/traces) for more details.
...
See [Debugging Tab](/api-management/endpoint-designer#debugging) for more information.
...
There is a [new section](/api-management/logs/audit-logs) in the Tyk Dashboard config file where you can specify parameters for the audit log (containing audit records for all requests made to all endpoints under the `/api` route).
...
Instead of setting permissions per user, you can now [create a user group](/platform-management/user-groups), and assign it to multiple users. It works for Single Sign-On too, just specify group ID during [SSO API](https://tyk.io/docs/api-reference/single-sign-on/generate-authentication-token) flow.
...
[Docs](/transform-traffic/url-rewriting)
...
A major feature of this release is the implementation of Mutual TLS. Now you can protect your APIs by allow listing certificates, idenitfy users based on them, and increase security between Tyk and upstream API. For details, see [Mutual TLS](/api-management/implement-tls#secure-hosted-apis-with-mtls).
...
[Docs](/api-management/access-control/policies/applying-policies#partitioned-policies)
...
[Docs](/api-management/dashboard-analytics#activity-logs)
...
* Set permissions for your users, logged via SSO, via `sso_permission_defaults` in Dashboard config file. [Docs](/tyk-identity-broker/overview)
* Setup a login page redirect, using `sso_custom_login_url` and `sso_custom_portal_login_url` Dashboard config options to enable users login using a custom SSO login page. [Docs](/tyk-identity-broker/overview)
...
#### Mutual TLS support
[Docs](/api-management/implement-tls#secure-hosted-apis-with-mtls)
...
#### Attaching Multiple Policies to the Keys
[Docs](/api-management/access-control/policies/applying-policies#applying-multiple-policies)
...
```
[Docs](/tyk-identity-broker/overview)
...
Added support for Mutual TLS, mentioned by Gateway and Dashboard above. See [Docs](/api-management/implement-tls#secure-hosted-apis-with-mtls)
