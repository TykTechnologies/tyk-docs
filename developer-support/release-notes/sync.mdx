...
#### Release Highlights
In this release, we have added support for managing Client Identity Provider (IdP) registry entries in Tyk Sync, so the JWKS URI and scope-to-policy mappings used for Dynamic Client Registration can be version-controlled and promoted between environments through GitOps workflows. We have also addressed CVEs to strengthen security.
...
`tyk-sync dump` now exports each Client IdP to its own `clientidp-{id}.json` file and records them under a new `client_idps` section in `.tyk.json`, with a `--idps` flag to export a specific subset.

`tyk-sync publish` (and `update`) recreates them on the Dashboard, and `tyk-sync sync` reconciles the full create, update, and delete lifecycle.

Required fields (`org_id`, `name`, and `jwks_uri`) are validated before any request is sent, and Dashboard versions that do not support the Client IdP registry are handled gracefully so existing workflows continue to run unchanged.
</Accordion>
