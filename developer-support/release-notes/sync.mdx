...
<Accordion title='MCP Proxy support in tyk-sync'>
Tyk Sync now recognises MCP proxies as a distinct resource type alongside OAS APIs. The `dump` and `sync` commands handle MCP proxies correctly:

- Dump writes MCP proxies to `mcp-{id}.json` files, separate from `oas-{id}.json` files for OAS APIs.
- Command output reports MCP proxies and OAS APIs as separate counts: `X OAS APIs` and `Y MCP Proxies`.
- Create, update, and delete operations are routed to the `/api/mcps` endpoint rather than `/api/apis/oas`.
- MCP proxies are included automatically when fetching all resources.

No changes to your existing workflow are required — MCP proxies are handled alongside your OAS APIs in the same `tyk-sync` commands.
</Accordion>
...
<Accordion title='MCP proxies being recreated as OAS APIs after sync'>
Resolved an issue where MCP proxies were incorrectly recreated as plain OAS APIs after a `tyk-sync sync` operation. The dump command was discarding the `application_protocol: mcp` marker before writing the file, causing sync to route the resource to `/api/apis/oas` instead of `/api/mcps` on the next run.

The dump now preserves the MCP marker so that subsequent sync operations correctly identify and route MCP proxies.
</Accordion>
