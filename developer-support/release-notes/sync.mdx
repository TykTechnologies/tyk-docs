...
Resolved an issue where policies using Model Context Protocol (MCP) access control and rate limiting were not fully exported by `tyk-sync dump`, causing the `mcp_primitives`, `mcp_access_rights`, `json_rpc_methods`, and `json_rpc_methods_access_rights` fields to be silently dropped from the dumped definition. Tyk Sync now preserves these MCP and JSON-RPC method fields, so policies can be dumped and synced without losing their MCP usage limits or primitive-based access rules.
