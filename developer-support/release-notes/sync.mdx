...
This release updates the Go toolchain used to build Tyk Sync to Go 1.26 and addresses reported CVEs in dependent libraries.
...
For users currently on v2.2.0, we strongly recommend promptly upgrading to the latest release. If you are working with an older version, it is advisable to bypass version 2.2.0 and proceed directly to this latest release.
...
In this release, we have added support for the Client Identity Provider (IdP) registry, introduced in [Tyk 5.14.0](/developer-support/release-notes/dashboard#5-14-0-release-notes). This feature provides centralized management of IdPs and scope-to-policy mappings used with [JWT Auth](/basic-config-and-security/security/authentication-authorization/json-web-tokens). Adding support to Tyk Sync allows GitOps management of the new Client IdP resource.
...
Tyk Sync recognises [MCP proxies](/ai-management/mcp-gateway/managing-proxies) as a distinct resource type.
