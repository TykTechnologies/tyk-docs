...
#### Release Highlights
This release refreshes the embedded Tyk Classic and Tyk OAS API and Policy definitions so that all fields introduced in Tyk Gateway 5.13.0 — including the new `errorOverrides` Tyk OAS extension — are correctly transferred during sync operations. It also ensures compatibility with the most recent Tyk LTS release [5.8.14](/developer-support/release-notes/dashboard#5-8-14-release-notes).
