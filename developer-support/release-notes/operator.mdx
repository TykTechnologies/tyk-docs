...
**Full declarative management of Tyk Dashboard resources**
 
Tyk Operator now manages several new Kubernetes Custom Resources, closing key gaps that previously prevented full programmatic control of Tyk Dashboard configuration and required manual Dashboard configuration or ad hoc Kubernetes Jobs calling the Admin API:
 
- **`TykSession`**: Client access control and tracking [Session](/api-management/access-control/sessions-and-keys/understanding-sessions).
...
- **`TykIdentityProfile`**: Tyk Identity Broker (TIB) profile for [Dashboard Single Sign-On](/tyk-identity-broker/dashboard-sso).
- **`TykOAuthClient`**: OAuth 2.0 clients bound to a specific Tyk OAuth-enabled API
- **`TykWebhook`**: reusable Dashboard webhook targets
- **`TykUserGroup`**: Dashboard [user groups](/platform-management/user-groups)
<Note>  While user groups can now be defined declaratively, individual Dashboard user accounts are not yet managed via CRDs and continue to be configured through the Dashboard UI or Admin API. </Note>
...
| :--------------------------------------------------------- | :--------------------- | :--------------------- | :------- | 
| [Kubernetes](https://kubernetes.io)                        | 1.30.x to 1.36.x       | 1.28.x to 1.36.x       |          | 
...
<Accordion title='Added TykSession CRD for declarative management of API access tokens'>
{/* TT-17235 */}
A new `TykSession` custom resource lets you declare API credentials, opaque access tokens, and HTTP Basic Auth users as Kubernetes resources, instead of provisioning them with one-off Kubernetes Jobs against the Admin API. The Operator creates, updates, and revokes the corresponding credential in the Gateway as the resource changes.
...
{/* TT-17503 */}
`TykCertificate` and `TykSession` resources (see above) now detect when their referenced Kubernetes Secret is rotated by an external system such as cert-manager or Vault, and reconcile the change automatically without needing a manual edit to trigger a sync.
...
<Accordion title='API ownership is now applied for Tyk OAS APIs'>
{/* TT-16776 */}
The Tyk Operator now applies API ownership from `OperatorContext` to Tyk OAS APIs, matching existing behavior for Tyk Classic APIs. `user_owners` and `user_group_owners` are applied on create and update, and updating these fields on an `OperatorContext` reconciles every Tyk OAS API that references it, keeping ownership consistent between Kubernetes and the Dashboard.
</Accordion>
...
- <a href="https://nvd.nist.gov/vuln/detail/CVE-2026-84304" target="_blank">CVE-2026-84304</a>
