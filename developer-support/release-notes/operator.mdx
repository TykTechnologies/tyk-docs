...
Tyk Operator v1.3.1 resolves a blocking installation issue introduced in v1.3.0. This release fixes the Helm chart template error that prevented users from successfully installing the operator via Helm charts.

For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v1.3.1) below.
...
#### Downloads
- [Docker image v1.3.1](https://hub.docker.com/r/tykio/tyk-operator/tags?page=&page_size=&ordering=&name=v1.3.1)
  - ```bash
    docker pull tykio/tyk-operator:v1.3.1
    ```
- Helm chart
  - [tyk-charts v5.1.0](/developer-support/release-notes/helm-chart#510-release-notes)

#### Changelog
<a id="Changelog-v1.3.1" data-scroll-offset></a>
...
Resolved an issue introduced in the v1.3.0 chart where installing the Tyk Operator via Helm failed due to an incorrect webhook annotation reference in the Helm template. Users can now successfully install the Tyk Operator by updating their Helm repositories and deploying the `tyk-operator` chart version 1.3.1.
</Accordion>
...
<Note>
**Do not use this release.** Tyk Operator v1.3.0 contains a bug that prevents installation via Helm charts. This issue has been resolved in v1.3.1. Please upgrade directly to v1.3.1.
</Note>
