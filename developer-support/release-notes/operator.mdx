...
Tyk Operator 1.4.0 introduces support for managing MCP (Model Context Protocol) Proxy API definitions through Kubernetes custom resources. The new `TykMcpProxyDefinition` CRD allows you to manage MCP Proxy APIs declaratively, and the `SecurityPolicy` CRD has been extended with MCP access rights for tool-based access control and per-primitive rate limiting.
...
Deploy Tyk Operator 1.4.0 only after Tyk Gateway 5.13.0 and Tyk Dashboard 5.13.0 are running. The `TykMcpProxyDefinition` controller calls Dashboard's `/api/mcps` endpoint, which does not exist in earlier Dashboard versions. Deploying Operator before Dashboard and Gateway will cause reconciliation failures on all `TykMcpProxyDefinition` resources. See [MCP Gateway upgrade considerations](/developer-support/upgrading#mcp-upgrade-considerations) for details.
...
<Accordion title='Add MCP Proxy support to TykMcpProxyDefinition and SecurityPolicy CRDs'>
This release introduces two extensions to Tyk Operator's custom resource support for MCP (Model Context Protocol) Gateway:

**TykMcpProxyDefinition CRD**

A new `TykMcpProxyDefinition` CRD allows you to manage MCP Proxy API definitions declaratively in Kubernetes. The OAS definition is supplied via a ConfigMap reference (`spec.tykMCP.configmapRef`). The CRD status surface exposes three hashes — `CRDSpecHash`, `ConfigMapHash`, and `TykSpecHash` — to track synchronisation state between the Kubernetes resource, the ConfigMap, and the Tyk Gateway definition.
