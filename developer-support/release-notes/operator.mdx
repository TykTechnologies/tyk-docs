...
Tyk Operator v1.4.0 introduces native management of [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) servers through the new `TykMcpProxyDefinition` custom resource, and extends `SecurityPolicy` with MCP-aware access controls (allow/block lists for tools, resources, prompts, and JSON-RPC methods, plus per-primitive and per-method rate limits). The Operator also moves to Go 1.25.9 and picks up dependency updates that clear the outstanding Trivy CVEs flagged for the v1.4.0 release.

For details on the new MCP resource, see [Manage MCP Servers with Tyk Operator](/product-stack/tyk-operator/mcp-proxy). For a comprehensive list of changes, refer to the [changelog](#Changelog-v1.4.0) below.
...
Deploy Tyk Operator 1.4.0 only after Tyk Gateway 5.13.0 and Tyk Dashboard 5.13.0 are running. The `TykMcpProxyDefinition` controller calls Dashboard's `/api/mcps` endpoint, which does not exist in earlier Dashboard versions — deploying the Operator before Dashboard and Gateway will cause reconciliation failures on all `TykMcpProxyDefinition` resources. See [MCP Gateway upgrade considerations](/developer-support/upgrading#mcp-upgrade-considerations) for details.
...
<Accordion title='MCP Proxy support (TykMcpProxyDefinition)'>
A new `TykMcpProxyDefinition` custom resource (`tyk.tyk.io/v1alpha1`, short name `tykmcp`) lets you declaratively manage MCP servers through Tyk. The Operator reads the MCP OAS document from a referenced `ConfigMap` and reconciles it onto Tyk, surfacing status fields such as `listenPath`, `domain`, `enabled`, and `latestTransaction.status`. See [Manage MCP Servers with Tyk Operator](/product-stack/tyk-operator/mcp-proxy) for the full reference and examples.
</Accordion>

<Accordion title='MCP-aware Security Policies'>
`SecurityPolicy` access entries now accept MCP-specific access controls when targeting a `TykMcpProxyDefinition`:
...
- `mcp_primitives` — per-primitive rate limits (`type: tool | resource | prompt`).
- `json_rpc_methods` — rate limits keyed by JSON-RPC method, e.g. `tools/call`, `resources/read`.

See [Securing an MCP proxy](/product-stack/tyk-operator/mcp-proxy#securing-an-mcp-proxy-with-a-securitypolicy) and the [MCP proxy policies documentation](/ai-management/mcp-gateway/policies).
</Accordion>
...
<Accordion title='Go 1.25.9 runtime'>
The Operator binary and container images are now built with Go 1.25.9, picking up the latest upstream security and performance fixes.
</Accordion>
...
<Accordion title='Prerelease Docker image tags include the v prefix'>
Prerelease tags such as `v1.4.0-rc1` now publish matching `tykio/tyk-operator:v1.4.0-rc1` images, so release-candidate workflows can pull images by the same tag name used in Git.
</Accordion>

<Accordion title='Dependency updates clearing v1.4.0 CVEs'>
Bumped third-party dependencies to clear outstanding Trivy-reported CVEs for the v1.4.0 release.
</Accordion>
