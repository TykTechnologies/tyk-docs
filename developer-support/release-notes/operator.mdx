...
Tyk Operator 1.4.0 introduces support for managing [MCP (Model Context Protocol) Proxy](/ai-management/mcp-gateway/managing-mcp-proxies) definitions through Kubernetes custom resources. The new `TykMcpProxyDefinition` CRD allows you to manage MCP Proxies declaratively, and the `SecurityPolicy` CRD has been extended with MCP access rights for tool-based access control and per-primitive rate limiting.
...
**Admission Webhook Removal:** The admission webhook for security policy (`validate-tyk-tyk-io-v1alpha1-securitypolicy`) has been removed.
