...
Tyk Operator v1.3.0 delivers improvements for API monitoring and ingress management in Kubernetes environments. This release adds native uptime test configuration support for both Classic and OAS API definitions, eliminating the need for manual Dashboard configuration. The ingress controller now properly respects custom strip listen path settings from OAS templates, and the operator has been updated to Go 1.25 for improved performance and enhanced security. For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v1.3.0) below.
...
<AccordionGroup>
<Accordion title='Go 1.25 Update'>
The Tyk Operator now runs on Go 1.25, providing improved performance, enhanced security, and access to the latest language features. This update ensures the Operator remains current with supported Go versions and reduces exposure to security vulnerabilities found in older runtime versions.
</Accordion>
...
<Accordion title='Added Uptime Test Configuration Support in Tyk Operator'>
The Tyk Operator now supports configuring uptime tests directly in your API definitions through both Classic and OAS CRDs. You can now define multiple uptime tests with custom URLs, methods, timeouts, headers, and commands without needing to manually configure them in the Dashboard.
</Accordion>
...
<Accordion title='Fixed Strip Listen Path Configuration in Tyk Operator Ingress Controller'>
The Tyk Operator ingress controller now properly respects the `strip` listen path configuration defined in your OAS API templates. This fix ensures that your template configurations are preserved while maintaining backward compatibility, as templates without an explicit strip setting will continue to default to `true`, and ACME challenge paths will still force `strip: false` as required.
</Accordion>
