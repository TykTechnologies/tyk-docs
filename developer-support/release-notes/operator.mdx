...
Tyk Operator v1.4.0 introduces native Kubernetes support for MCP proxies. A new `TykMcpProxyDefinition` CRD lets you declare, manage, and version-control MCP proxies as Kubernetes resources, with full GitOps support for MCP access control through an extended `SecurityPolicy` CRD.
...
<Accordion title='TykMcpProxyDefinition CRD for managing MCP proxies in Kubernetes'>
A new `TykMcpProxyDefinition` custom resource definition (CRD) allows you to declare and manage MCP proxies as Kubernetes resources. The MCP proxy configuration (an OAS document) is stored in a ConfigMap, which the CR references via `spec.tykMCP.configmapRef`. The Operator watches both the CR and the referenced ConfigMap and reconciles any changes against the Tyk Gateway API automatically.

The CR status surface includes the proxy ID, listen path, linked policies, and separate hashes of the last reconciled CR spec, ConfigMap, and Tyk API definition — letting you detect drift between each layer from `kubectl`.

Sample manifests are available in the [operator repository](https://github.com/TykTechnologies/tyk-operator) under `config/samples/mcp/`.
</Accordion>

<Accordion title='MCP access rights in SecurityPolicy CRD'>
The `SecurityPolicy` CRD now supports MCP-specific access rights. You can define per-tool, per-resource, and per-prompt allow and deny lists, and set per-primitive rate limits, directly in your policy manifests. This matches the access control model available in Tyk Dashboard and Tyk Gateway, bringing full GitOps support to MCP proxy policy management.
</Accordion>
