...
The admission webhook for security policy (`validate-tyk-tyk-io-v1alpha1-securitypolicy`) has been removed as it was not performing any functional validation. This change simplifies the operator deployment and reduces webhook overhead. No action is required from users, and existing `SecurityPolicy` CRDs and resources remain fully supported and unaffected.
</Expandable>
