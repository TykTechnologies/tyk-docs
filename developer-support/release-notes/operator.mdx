...
Following on from API and Policy management, Tyk Operator now lets you declare several more Dashboard-managed resource types as Kubernetes Custom Resources, closing several of the key gaps that previously required manual Dashboard configuration or ad hoc Kubernetes Jobs calling the Admin API:
 
- **`TykSession`**: API access tokens and HTTP Basic Auth credentials
