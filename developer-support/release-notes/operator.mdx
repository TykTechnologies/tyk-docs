...
In this release, we have fixed issues with the reliability of SecurityPolicy reconciliation under high load that could lead to infinite reconciliation loops, orphaned Dashboard policies, and inconsistent state between Kubernetes and Dashboard. The fix introduces a new configuration option that enables tuning of Tyk Operator's HTTP connection pool for different deployments.
...
- [Docker image v1.4.2](https://hub.docker.com/r/tykio/tyk-operator/tags?page=&page_size=&ordering=&name=v1.4.2)
  - ```bash
    ...
    ```
- Helm chart
  - [tyk-charts v5.2.1](/developer-support/release-notes/helm-chart#5-2-release-notes)
...
This fix adds a new environment variable (`TYK_OPERATOR_HTTP_MAX_IDLE_CONNS_PER_HOST`) that enables users to tune the size of the Operator's HTTP connection pool to the Dashboard. This overrides the existing Go default setting and defaults to `100` connections. This can be tuned for the user's specific operating conditions: it should be increased for high-throughput clusters or reduced for resource-constrained environments.
</Accordion>
