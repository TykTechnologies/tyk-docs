...
This release.. For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v1.30.3). 
...
<Accordion title='Upgraded sonarqube-scanners to v7'>
The `sonarqube-scanners` dependency has been updated to v7, improving the overall security of the system.
</Accordion>
...
<Accordion title='Added entitlements for configuring MongoDB pool size parameters'>
Introduced new entitlements that allow users to customize MongoDB connection pool settings for MDCB and Tyk Dashboard deployments. This provides greater control over database connection management and performance.
**Configuration**: Added new entitlements to configure `maxPoolSize` and `maxIdleTimeMS` for both MDCB and Tyk Dashboard. When not set, the default values are used.
</Accordion>
