...
When viewing a documentation-only product, GraphQL specifications whose alias contains a hyphen (-) may not appear correctly in the API Specifications tab. This does not affect runtime functionality, API Product docs in the Catalogue, or GraphQL Playground behavior. We recommend not using a hyphen for a specification alias in this case. A fix will be included in v1.16.0
