...
Tyk Developer Portal addresses several issues in the Portal API, including incorrect Content-Type headers and improved enum field handling for null values. Additionally, we've resolved a scope mapping cleanup issue that occurred when deleting DCR products from the Portal.

The release also includes infrastructure updates with Go 1.25 support, enhanced Portal Dashboard synchronization logic, and proper exclusion of MCP Proxies from the Enterprise Developer Portal to improve system stability and performance.
...
<Accordion title='Excluded MCP Proxies from Enterprise Developer Portal'>
Added functionality to exclude Tyk MCP (Multi-Cloud Proxy) servers from the Enterprise Developer Portal (EDP) until full EDP support for MCP is implemented. This ensures API consumers cannot discover or subscribe to MCP servers through the portal.

MCP proxies are now excluded from:
- API Products list - MCP ACL policies do not appear and cannot be selected or configured as API products
- Plans list - MCP rate limit policies do not appear and cannot be selected or configured as plans
- Manual product association - MCP proxies are not listed as available options when creating or editing API products
- EDP API operations - Direct API calls attempting to associate MCP policies with products return validation errors

This change prevents unintended exposure of MCP servers to portal users while maintaining the existing EDP functionality for standard APIs.
</Accordion>
