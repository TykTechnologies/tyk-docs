...
Tyk Developer Portal has been updated to Go 1.25 and Debian 13 (Trixie) for enhanced security and performance. This release resolves several Portal API issues, including incorrect `Content-Type` headers, improper null value handling for enum fields, policy reference inconsistencies, and scope mapping cleanup when deleting DCR products. 
...
Resolved an issue where multiple Portal API endpoints returned incorrect `Content-Type` headers. Previously, endpoints that didn't explicitly write a response body would default to `text/plain` instead of the expected `application/json`, potentially causing client applications to misinterpret API responses.
...
Resolved an issue where the Portal API returned empty strings (`""`) instead of `null` values for optional enum fields when their values were not set. Previously, endpoints like `/api/posts` and `/api/catalogues` would incorrectly serialize null enum values as empty strings, violating the API contract and potentially causing client-side parsing issues.
...
Resolved an issue where deleting Dynamic Client Registration (DCR) products or plans from the Developer Portal did not properly clean up JWT scope-to-policy mappings from their associated API definitions. Previously, when a DCR product was removed from the Portal, the access policy was correctly deleted from the Dashboard, but the scope mappings referencing that policy remained in the API definitions, creating orphaned references to non-existent policies.

The Portal now properly removes scope-to-policy mappings that reference deleted products during the deletion process. This fix addresses two underlying issues: the `removePolicyFromScopeMappings` function now correctly handles both OAS and non-OAS API definitions using the appropriate scope mapping paths, and the deletion handler now properly populates API details to ensure scope cleanup occurs across all affected APIs.
</Accordion>

<Accordion title='Fixed Plan Field Updates Not Propagating to Dashboard Policies'>
Resolved an issue where updates to certain Plan fields in the Developer Portal were not properly propagating to their corresponding Dashboard Policies. Previously, when administrators updated only the `KeyExpiresIn` (key expiration time) or `MetaData` fields through either the Portal UI or API, these changes would not sync to the Dashboard unless other fields like rate limits or quotas were modified simultaneously. This occurred due to an incomplete change detection mechanism that omitted these fields from its validation checks. 
