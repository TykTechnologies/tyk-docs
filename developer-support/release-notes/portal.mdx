...
**Fixed Portal API Content-Type and Enum Handling**

This release addresses several critical issues with Portal API endpoints. Content-Type headers are now correctly set for API responses, and enum field handling has been improved to properly manage null values, ensuring more reliable API interactions.

**Enhanced Portal Dashboard Synchronization**

The Portal Dashboard synchronization logic has been updated to provide better reliability and performance when syncing data between the portal and dashboard components.

**MCP Server Exclusion for Enterprise Portals**

MCP servers are now properly excluded from Enterprise Developer Portal deployments, providing better resource management and deployment consistency for enterprise customers.

Additional improvements include updating the Developer Portal to Go 1.25 for better performance and security, and fixing scope mapping cleanup issues when deleting DCR products from the Portal.

For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v1.5.1) below.
...
<Accordion title='Excluded MCP Servers from Enterprise Developer Portal'>
Added exclusion of Tyk MCP (Multi-Cloud Proxy) servers from the Enterprise Developer Portal to prevent API consumers from discovering or subscribing to MCP servers until full EDP support is implemented. This ensures that:

- MCP ACL policies do not appear in the API Products list and cannot be selected or configured as API products
- MCP proxies are not listed as available options when creating or editing API products in the EDP admin interface
- Direct API calls attempting to associate MCP partitioned ACL policies with API products return validation errors
- MCP rate limit policies do not appear in the Plans list and cannot be selected or configured as plans

This change maintains portal functionality while preventing premature exposure of MCP servers to API consumers through the developer portal.
</Accordion>
...
Updated the Tyk Developer Portal to use Go 1.25, ensuring compatibility with the latest Go runtime features and reducing exposure to potential security vulnerabilities in older versions.
</Accordion>

<Accordion title='Updated Portal Dashboard Synchronization Logic'>
Restructured the Portal's data synchronization logic with the Dashboard to prevent data loss and improve reliability. Previously, the Portal used a `Save()` operation with partially filled Product objects, which overwrote entire database rows and caused Portal-managed fields like logos, descriptions, and content to be zeroed out during Dashboard-to-Portal synchronization.

The synchronization now uses targeted `Updates()` operations that only modify Dashboard-owned fields such as `name`, `auth_type`, and `remote_scopes`, preserving all Portal-managed customizations. Additionally, products are now matched using unique, stable identifiers (`ReferenceID`/`AccessObjectID`) instead of name-based matching, eliminating conflicts where products with identical names could be incorrectly updated.
</Accordion>
...
<Accordion title='Fixed Incorrect Content-Type Headers in Portal API Endpoints'>
Resolved an issue where numerous Portal API endpoints were returning incorrect `Content-Type` headers. Previously, endpoints that didn't explicitly write a response body would default to `text/plain` instead of the expected `application/json`, causing client-side parsing issues and API contract violations.

The fix addresses over 20 affected endpoints across Product and User APIs, including document management, logo/preview operations, OpenAPI specification handling, and user management endpoints. All Portal API endpoints now return the correct `Content-Type` header that matches their actual response content.
</Accordion>

<Accordion title='Fixed Portal API Enum Field Handling for Null Values'>
Resolved an issue where the Portal API incorrectly returned empty strings (`""`) instead of `null` values for optional enum fields when those fields had no value set. Previously, endpoints like `/api/posts` and `/api/catalogues` would return enum fields such as `Status` and `VisibilityStatus` as empty strings, which violated the API contract specification.

The Portal API now properly omits optional enum fields from responses when they are empty, ensuring consistent null handling across all affected endpoints including access requests, posts, catalogues, and providers.
</Accordion>

<Accordion title='Fixed Scope Mapping Cleanup When Deleting DCR Products from Portal'>
Resolved an issue where deleting DCR (Dynamic Client Registration) products or plans from the Developer Portal did not remove their scope mappings from associated API definitions. Previously, when a DCR product was deleted, the access policy was correctly removed from the Dashboard, but the scope-to-policy mappings remained in the API definitions, causing stale references to non-existent policies.

The Portal now properly cleans up scope mappings during product deletion by:

- Fixed the `removePolicyFromScopeMappings` function to work correctly with both OAS and legacy API definitions
- Ensured that `DeletePlanHandler` populates the necessary API details to enable proper cleanup
- Added support for the updated JWKS endpoint fields when manipulating API definitions

This ensures that API definitions remain consistent after product deletions and prevents authentication issues caused by references to deleted policies.
</Accordion>
