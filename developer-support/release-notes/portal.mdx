...
<Note>
If you are also upgrading Tyk Dashboard to 5.13.0, upgrade to EDP v1.17.1 at the same time. Dashboard 5.13.0 introduces MCP Proxy definitions and policies that grant access to them. Without this release, those policies will not be filtered from the developer catalogue and may be inadvertently published to portal users. See [MCP Gateway upgrade considerations](/developer-support/upgrading#mcp-upgrade-considerations) for the full component deployment order.
</Note>
