...
Tyk Dashboard v5.13.0 introduces [MCP Proxy](/ai-management/mcp-gateway/managing-mcp-proxies) support. Products granting access to MCP Proxies are not currently supported from Developer Portal.
...
Policies granting access to MCP Proxies are no longer synchronised to the Developer Portal as API Products. Prior to this release, such Policies would be included in synchronisation with the Dashboard and could inadvertently appear in Catalogs, where they would fail to operate correctly.

Support for the publication of MCP Proxies on the Developer Portal and management of access requests for Products containing these will be available in a future release.
</Accordion>
