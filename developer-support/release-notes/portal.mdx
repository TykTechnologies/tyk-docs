...
In this release, we’re excited to introduce a fully integrated GraphQL Playground experience in the Enterprise Developer Portal. We've also upgraded Tyk Portal to Golang 1.24 and fixed a CVE.
...
<li>
<details>
<summary>Tyk Portal now runs on Golang 1.24</summary>
...
- **Automatic Schema Loading via Introspection**: Playground automatically loads API schemas through introspection when a server endpoint is available, enabling instant exploration of types, queries, and mutations
- **Support for SDL Uploads**: Admins can optionally upload a Schema Definition Language (SDL) file (.graphql, .graphqls, or JSON format) when introspection is unavailable. This allows schema exploration, autocompletion, and validation features to still function.
- **Documentation-Only GraphQL Products**: Admins can now upload GraphQL schema documentation for documentation-only API products
...
Fixed the following high-priority CVE identified in the Tyk Portal, providing increased protection against security
vulnerabilities:<br />
