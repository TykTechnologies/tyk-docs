...
When adding an API Product to an existing Auth (Bearer) Token credential for the first time, the quota usage counters for that credential may be reset to zero.
**Details:**
- Affects Auth (Bearer) Token authentication only
