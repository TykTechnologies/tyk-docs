...
In this release, we’re excited to introduce a fully integrated GraphQL Playground experience in the Enterprise Developer Portal. We've also upgraded Tyk Portal to Golang 1.24 and fixed a priority CVE.
...
- **Enhanced Schema Documentation**: Admins can upload GraphQL Schema Definition Language (SDL) files (.graphql, .graphqls, or JSON format) to provide rich documentation and enable full playground functionality.
- **Flexible Schema Loading**: The playground loads the schema information when SDL files are provided during API product configuration, ensuring developers have access to comprehensive API documentation.
