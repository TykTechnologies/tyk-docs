...
The Developer Portal receives several important fixes that improve API consistency and data integrity. Portal API endpoints now return correct Content-Type headers and proper null values for optional enum fields, while Dynamic Client Registration (DCR) product deletion has been enhanced to properly clean up scope mappings. Additionally, Portal Dashboard synchronization has been improved to prevent data loss during sync operations, and MCP servers are now properly excluded from the Enterprise Developer Portal for better security and functionality.
...
#### Dependencies
<a id="dependencies-1.17.1"></a>

| Portal Version | Recommended Releases | Backwards Compatibility |
|--------|-------------------|-------------|
| 1.17.1 | MDCB vX.Y.Z       | MDCB vX.Y.Z |
...
<Accordion title='MCP servers are now excluded from the Enterprise Developer Portal'>
MCP (Multi-Cloud Proxy) servers are now automatically excluded from appearing in the Enterprise Developer Portal's API Products list. This prevents API consumers from discovering or subscribing to MCP servers through the portal until full EDP support for MCP is implemented. Platform administrators can continue managing MCP policies through the Tyk Dashboard without them being exposed to portal users.
</Accordion>
...
<Accordion title='Updated Developer Portal to Go 1.25'>
The Enterprise Developer Portal has been upgraded to use Go 1.25, improving security and keeping current with the latest Go language features. This update includes modifications to build processes and CI pipelines to ensure compatibility with the new Go version while maintaining all existing functionality.
</Accordion>

<Accordion title='Improved Portal Dashboard synchronization to prevent data loss during sync operations'>
Enhanced the synchronization logic between the Portal and Dashboard to prevent data loss that previously occurred when Dashboard data overwrote Portal-managed content. The updated synchronization now only modifies Dashboard-owned fields (such as name, auth_type, and remote_scopes) while preserving Portal-managed fields like logos, descriptions, and documentation. This resolves issues where partial product data would overwrite complete records and eliminates conflicts from name-based product matching that could cause data mix-ups.
</Accordion>
...
Resolved an issue where multiple Portal API endpoints were returning incorrect Content-Type headers, specifically returning "text/plain" instead of the expected "application/json" for JSON responses. The problem affected endpoints like POST /products/{product_id}/docs and DELETE /products/{product_id}/docs/{doc_id}, among others, and occurred when API handlers didn't explicitly write a response body. All Portal API endpoints have been audited and corrected to return the appropriate Content-Type headers that match their response content.
</Accordion>

<Accordion title='Fixed Portal API returning empty strings instead of null for optional enum fields'>
Resolved an issue where the Portal API incorrectly returned empty strings ("") instead of null values for optional enum fields when no value was set. This affected various API endpoints including post status and catalogue visibility status fields. The API now properly adheres to its contract by returning null for unset optional enum values.
</Accordion>

<Accordion title='Fixed DCR product deletion to properly clean up scope mappings from API definitions'>
Resolved an issue where deleting DCR products or plans from the developer portal would leave orphaned scope-to-policy mappings in the API definitions. The system now properly removes scope mappings that reference deleted policies, preventing authentication issues with stale configuration. JWKS endpoints are preserved when shared by other active products to maintain functionality for remaining integrations.
</Accordion>
...
The Developer Portal has been upgraded to Go 1.25 and now includes enhanced dashboard synchronization capabilities that prevent data loss during sync operations. Critical fixes address incorrect Content-Type headers in Portal API responses and resolve issues with empty strings being returned instead of null values for optional enum fields. Additionally, DCR product deletion now properly cleans up scope mappings from API definitions, and MCP servers are now excluded from Enterprise Developer Portal deployments for improved security and performance.
...
<Accordion title='Updated Developer Portal to Go 1.25'>
Updated the Enterprise Developer Portal to use Go 1.25, enhancing security and maintaining compatibility with the latest Go runtime features. This upgrade ensures the Developer Portal stays current with Go language improvements and reduces exposure to potential security vulnerabilities in older versions.
</Accordion>

<Accordion title='Improved Portal Dashboard synchronization to prevent data loss during sync operations'>
Enhanced the synchronization logic between the Portal and Dashboard to prevent data loss that previously occurred when Dashboard data overwrote Portal-managed content. The updated logic ensures that Dashboard-to-Portal sync only updates Dashboard-owned fields like name, auth_type, and remote_scopes, while preserving Portal-managed fields such as logos, descriptions, and documentation. This resolves issues where Portal customizations were being lost during synchronization processes.
</Accordion>

<Accordion title='Added exclusion of MCP servers from Enterprise Developer Portal'>
MCP (Multi-Cloud Proxy) servers are now automatically excluded from the Enterprise Developer Portal to prevent API consumers from discovering or subscribing to them. This ensures that MCP proxies remain hidden from portal users until full EDP support for MCP servers is implemented in a future release. Platform administrators will no longer see MCP ACL policies in the API Products list or be able to configure them as API products.
</Accordion>
...
<Accordion title='Fixed incorrect Content-Type headers returned by Portal API endpoints'>
Resolved an issue where multiple Portal API endpoints were returning incorrect Content-Type headers, specifically returning text/plain instead of the expected application/json. The problem occurred when API handlers didn't explicitly write a response body, causing the server to default to text/plain. This fix ensures that all Portal API endpoints now return the correct Content-Type header that matches their actual response content.
</Accordion>

<Accordion title='Fixed Portal API returning empty strings instead of null for optional enum fields'>
Resolved an issue where the Portal API incorrectly returned empty strings ("") instead of null values for optional enum fields when no value was set. This affected various API responses including post status and catalogue visibility status fields. The API now properly respects its contract by returning null values as expected, ensuring consistent data handling for client applications.
</Accordion>

<Accordion title='Fixed DCR product deletion to properly clean up scope mappings from API definitions'>
Resolved an issue where deleting DCR (Dynamic Client Registration) products or plans from the developer portal left orphaned scope-to-policy mappings in the associated API definitions. When a DCR product was removed, the access policy was correctly deleted from the Dashboard, but the JWT scope mappings remained in the API definition, referencing non-existent policies. The system now properly removes these orphaned scope mappings when products are deleted, ensuring API definitions remain clean and functional.
</Accordion>
