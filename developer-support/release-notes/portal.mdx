...
This patch release addresses an important issue with Dynamic Client Registration (DCR) product deletion functionality. Previously, when a DCR product was deleted, associated scope mappings were not properly removed from the system, which could lead to orphaned data and potential inconsistencies.

**DCR Product Deletion Fix**
The DCR product deletion process now correctly removes all associated scope mappings when a product is deleted. This ensures complete cleanup of related data and maintains system integrity when managing DCR products and their associated OAuth2 scopes.
...
- [Docker Image to pull](https://hub.docker.com/r/tykio/portal/tags?page=&page_size=&ordering=&name=v5.13.0)
  - ```bash
    docker pull tykio/portal:v5.13.0
    ```
- Helm charts
...
<Accordion title='Fixed DCR Product Deletion Not Removing Scope Mappings'>
Resolved an issue where deleting Dynamic Client Registration (DCR) products or plans from the developer portal did not remove their associated scope-to-policy mappings from API definitions. Previously, when a DCR product was deleted, the access policy was correctly removed from the Dashboard, but the JWT scope-to-policy mappings remained in the API definitions, creating orphaned references to non-existent policies.

The fix addresses two underlying problems: the `removePolicyFromScopeMappings` function was broken for non-OAS APIs due to incorrect path handling, and the `DeletePlanHandler` was not properly populating API details during the deletion process. Tyk now properly cleans up scope mappings when DCR products are deleted, ensuring API definitions remain consistent and free of stale policy references.
</Accordion>
