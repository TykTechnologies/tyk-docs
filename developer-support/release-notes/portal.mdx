...
Fixed two issues affecting API selection when editing API Products: First, when multiple APIs shared the same name, the Portal would incorrectly display which API was selected, potentially causing data corruption if users saved without noticing the error. Second, applying search or category filters would cause all pending selections to be lost, forcing users to restart their selection process. Both issues have been resolved, ensuring accurate API selection display and persistent selection state during filtering operations.
</Accordion>
