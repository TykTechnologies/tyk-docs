...
The Developer Portal has received significant reliability improvements, including fixes for incorrect Content-Type headers and empty string handling in API responses, as well as enhanced dashboard synchronization to prevent data loss. Scope mapping cleanup has been resolved to properly remove mappings when DCR products are deleted from the portal. Additionally, MCP proxies are now excluded from Enterprise Developer Portal publication, and the portal has been updated to Go 1.25 for improved performance and security.
...
<Accordion title='Excluded MCP proxies from Enterprise Developer Portal publication'>
MCP (Multi-Cloud Proxy) servers are now automatically excluded from the Enterprise Developer Portal to prevent API consumers from discovering or subscribing to them. This temporary measure ensures a clean portal experience while full EDP support for MCP is being developed. MCP partitioned policies will not appear in the API Products list and cannot be configured as API products in the portal.
</Accordion>
...
Updated the Enterprise Developer Portal to use Go 1.25 to maintain compatibility with the latest Go version and minimize exposure to security vulnerabilities. This update includes changes to the build configuration and CI pipelines to ensure all systems run on the newer Go version while maintaining existing functionality.
</Accordion>

<Accordion title='Improved Portal Dashboard synchronization to prevent data loss during sync operations'>
Updated the synchronization logic between the Portal and Dashboard to eliminate data loss that occurred when Dashboard-managed fields overwrote Portal-specific content. The sync process now only updates explicitly Dashboard-owned fields (such as name, auth_type, and remote_scopes) while preserving Portal-managed fields like logos, descriptions, and documentation. This change also resolves conflicts from name-based product matching that could cause data mix-ups between products with identical names.
</Accordion>
...
Resolved an issue where multiple Portal API endpoints were returning incorrect Content-Type headers, specifically returning text/plain instead of the expected application/json. The problem affected endpoints like POST /products/{product_id}/docs and DELETE /products/{product_id}/docs/{doc_id}, and occurred when API handlers didn't explicitly write a response body, causing the server to default to text/plain. All Portal API endpoints have been audited and corrected to return appropriate Content-Type headers that match their response content.
</Accordion>

<Accordion title='Fixed Portal API returning empty strings instead of null for optional enum fields'>
Resolved an issue where the Portal API returned empty strings ("") instead of null values for optional enum fields when no value was set. This affected various API responses including post status and catalogue visibility status fields. The API now correctly follows its contract by returning null for unset optional enum values, ensuring consistent data handling for client applications.
</Accordion>

<Accordion title='Fixed scope mappings not being removed from APIs when DCR products are deleted from the developer portal'>
Resolved an issue where deleting Dynamic Client Registration (DCR) products or plans from the developer portal would leave orphaned scope-to-policy mappings in the API definitions. When a DCR product was removed, the associated access policy was correctly deleted from the Dashboard, but the JWT scope mappings remained in the API configuration, referencing non-existent policies. The system now properly cleans up these scope mappings when products are deleted, ensuring API definitions remain consistent and functional.
</Accordion>
