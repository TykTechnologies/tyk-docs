...
**Enhanced Portal Product Management**

We've resolved an issue with DCR (Dynamic Client Registration) scope mapping cleanup that occurred when deleting Portal products. This fix ensures that all associated scope mappings are properly removed during product deletion, preventing orphaned references and maintaining data integrity within your Portal configuration.
...
- [Docker Image to pull](https://hub.docker.com/r/tykio/portal/tags?page=&page_size=&ordering=&name=v5.8.14)
  - ```bash
    docker pull tykio/portal:v5.8.14
    ```
- Helm charts
...
<Accordion title='Fixed DCR Scope Mapping Cleanup When Deleting Portal Products'>
Resolved an issue where deleting DCR (Dynamic Client Registration) products or plans from the developer portal did not properly remove their associated scope mappings from API definitions. Previously, when a DCR product was deleted from the portal, the access policy was correctly removed from the Dashboard, but the scope-to-policy mappings remained in the affected API definitions, creating orphaned references to non-existent policies.

The portal now properly cleans up scope mappings during product deletion by:

- Fixing the `removePolicyFromScopeMappings` function to correctly handle both OAS and non-OAS API definitions when removing scope mappings
- Ensuring the cleanup process uses the appropriate helper functions that properly branch between OAS and legacy API definition formats
- Populating API details during the deletion process to enable proper scope mapping cleanup

This prevents orphaned scope-to-policy mappings from accumulating in API definitions when DCR products are removed from the portal.
</Accordion>
