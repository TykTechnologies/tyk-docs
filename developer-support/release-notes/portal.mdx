...
To support this, administrators now have a dedicated certificate management interface where they can upload new certificates, synchronize them from Tyk Dashboard providers, and associate them with specific organizations.

Additionally, administrators can now enable certificate-token binding on a per-product basis for APIs using mTLS. When enabled, the system automatically binds the appropriate certificate to an access token upon approval, ensuring that only the correct token-certificate pair can be used to gain API access.
</Accordion>
