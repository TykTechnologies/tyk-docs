...
Portal 1.16.0 introduces the ability to update plans on existing credentials. However, due to limitations in how Auth Token credentials handle rate limits and quotas, the “Change Plan” option has been disabled for this authentication type.

##### Quota Counter Reset When Adding API Products to Existing Credentials
When adding an API Product to an existing Auth Token credential for the first time, the quota usage counters for that credential may be reset to zero.
**Details:**
- Affects Auth Token authentication only
- Occurs only on the first API product addition
- Subsequent API product additions are not affected
- Quota counters automatically return to normal behaviour after the next quota reset cycle (hourly/daily)
