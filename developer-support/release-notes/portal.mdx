...
<Accordion title='Certificate Management for Auth Token-Certificate Binding'>
The Developer Portal now includes comprehensive certificate management capabilities to support secure Auth Token-Certificate binding for mTLS authentication. Portal administrators can now manage certificates through a dedicated interface under API Consumers, including uploading new certificates, synchronizing existing certificates from Tyk Dashboard providers, and associating certificates with organizations for token binding. This feature enables administrators to control which certificates are bound to access tokens issued within their organizations, enhancing security by preventing token-certificate mismatches in mTLS scenarios.
</Accordion>
...
The Developer Portal now correctly displays and manages API selections when editing API Products. Previously, when multiple APIs shared the same name, the Portal would show the wrong API as selected, potentially causing data corruption if users saved without noticing the error. Additionally, applying search or category filters would cause all pending selections to be lost, forcing users to start over. These issues have been resolved, ensuring accurate API selection display and persistent selection state during filtering operations.
</Accordion>

<Accordion title='mTLS Certificate Re-attachment Option Missing After Detachment'>
The Developer Portal now correctly displays the "Attach Certificate" option for mTLS credentials after a certificate has been detached. The issue that prevented developers from re-attaching certificates to their applications has been resolved.
</Accordion>
