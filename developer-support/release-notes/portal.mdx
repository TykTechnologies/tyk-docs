...
This patch release addresses several critical issues in the Portal API, including incorrect Content-Type headers and improved enum field handling for null values. Additionally, we've resolved a scope mapping cleanup issue that occurred when deleting DCR products from the Portal.
...
Updated the Portal Dashboard synchronization logic to prevent data loss during sync operations. Previously, the synchronization process used `Save()` with partially filled Product objects, which overwrote entire database rows and caused Portal-managed fields like logos, descriptions, and content to be zeroed out during Dashboard-to-Portal sync operations.

The Portal now uses targeted `Updates()` operations that only modify explicitly Dashboard-owned fields such as `name`, `auth_type`, and `remote_scopes`, preserving all Portal-managed customizations. Additionally, products are now matched using stable identifiers (`ReferenceID`/`AccessObjectID`) instead of name-based matching, eliminating conflicts where products with identical names could be incorrectly updated.
</Accordion>
