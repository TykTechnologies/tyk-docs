...
##### Plan Changes Disabled for Auth Token Credentials
Due to some limitations when updating Auth Token credentials, the ability to change plans has been disabled in Portal 1.16.0. Users can still add or remove API Products from existing credentials, but plan changes will remain unavailable until the issues are resolved.

##### Quota Counter Reset When Adding API Products to Existing Credentials
With the introduction of the new Portal feature that allows adding and removing API Products from existing credentials, there is a known limitation affecting Auth Token–based credentials.
When adding an API Product to an existing Auth Token credential for the first time, the quota usage counters associated with that credential may be reset to zero.
**Details:**
- It affects Auth Token authentication only
- The quota reset happens only on the first product addition
- Subsequent product additions do not trigger the issue
- Quota counters automatically return to normal behaviour after the next quota reset period (hourly/daily)
