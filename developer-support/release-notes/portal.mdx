...
When adding an API Product to an existing Auth (Bearer) Token credential for the first time, the quota usage counters for that credential may be reset to zero, allowing temporary quota bypass until next reset cycle. 
**Details:**
