...
<Accordion title='MCP Proxy policies excluded from the developer catalogue'>
Policies containing MCP access rights are no longer displayed in the developer catalogue. Previously, these policies appeared as API Products or Plans in the catalogue but could not be consumed correctly because the portal has no native MCP Proxy support.

MCP Proxy policies will appear in the catalogue once full portal support for MCP is available in a future release.
</Accordion>
