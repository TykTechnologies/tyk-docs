...
This release adds [Secrets Management](/tyk-configuration-reference/kv-store) to the Tyk Developer Portal configuration file for the first time, letting you reference values from external secrets stores in place of plain-text values.
...
See [API Products](/portal/api-products).
</Accordion>
...
See [How the Inspector Connects](/portal/mcp-inspector-playground#how-the-inspector-connects).
</Accordion>
...
See [Test MCP Tools with the Embedded MCP Inspector](/portal/mcp-inspector-playground).
</Accordion>
...
See [View MCP Documentation in the Developer Portal](/portal/mcp-documentation).
</Accordion>
...
See [Mirror and Static PRM](/portal/mcp-oauth-access#mirror-and-static-prm).
</Accordion>
...
In this release, we have implemented support for the Client IdP registry introduced in [Tyk 5.14.0](/developer-support/release-notes/dashboard#5-14-0-release-notes). This feature provides centralized management of IdPs and scope-to-policy mappings used with API Products secured with [JWT Authentication](/basic-config-and-security/security/authentication-authorization/json-web-tokens), resolving a historical issue with Dynamic Client Registration configuration not being properly cleaned up when associated API Products or Plans are deleted.

<Note>
This feature is available only from Tyk Gateway 5.14.0 and Tyk Dashboard 5.14.0 and must be enabled in the Developer Portal by setting the new configuration option [`TYK_PORTAL_ENABLEIDPREGISTRY`](/product-stack/tyk-enterprise-developer-portal/deploy/configuration#tyk_portal_enableidpregistry) to `true`.
</Note>
...
See [MCP Gateway upgrade considerations](/developer-support/upgrading#mcp-upgrade-considerations) for the full component deployment order.
</Note>
...
We've added a new Certificate Management function in the Admin Portal, where you can upload client certificates, retrieve existing certificates from the [Tyk Certificate Store](/api-management/certificates#tyk-certificate-store), and associate certificates with Organisations.

When an Auth Token ([API Key](/portal/overview/concepts#access-credentials)) is issued to a Developer App, the appropriate certificate or certificates will automatically be linked to that token to ensure that the API Consumer can access the Product's APIs using a combination of the token with their client certificate.
...
The Developer Portal now properly supports rotation and revocation operations for [Tyk Managed Custom Credentials](/tyk-developer-portal/import-custom-credentials). Custom credentials now support the same lifecycle management as standard Tyk tokens.
</Accordion>
...
You can read more about the supported SSO providers [here](/tyk-identity-broker/overview).
</Accordion>
...
Now when the [PORTAL_DCR_LOG_ENABLED](/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_dcr_log_enabled) environment variable is set to `true`, the portal will output not only the status and status code of the request to the IdP, but also actual payload returned by the IdP: 
```yaml
...
- When [PORTAL_DCR_LOG_ENABLED](/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_dcr_log_enabled) is enabled the portal now returns not only the status and status code of the request to the IdP but also actual payload returned by the IdP
...
The Tyk Enterprise Developer Portal is now available in Kubernetes and customer can launch it using our [helm charts](/portal/install/kubernetes#legacy-helm-chart). This feature makes the portal Kubernetes friendly by adding liveness, readiness probes, graceful shutdown and changing the portal lifecycle so that it's possible to set an initial user and bootstrap the portal via APIs.
...
##### Added
- Added Kubernetes support and [helm charts](/portal/install/kubernetes#legacy-helm-chart).
