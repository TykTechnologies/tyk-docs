...
In this release, we have resolved several issues across Portal Admin and the Developer Portal API, including more reliable cleanup of Dynamic Client Registration configuration when products or plans are deleted. We have also addressed CVEs to strengthen security.
...
Resolved an issue where searching on the Teams page (API Consumers > Teams) in Portal Admin returned a database error whenever the search term was not numeric, such as a Team or Organisation name.
...
<Accordion title='Fix scope mappings and JWKS configuration left behind when deleting DCR products or plans'>
Resolved an issue where deleting a Dynamic Client Registration (DCR) product or plan from the Developer Portal removed the associated access policy from the Dashboard but left the linked API definitions unchanged. The scope-to-policy mappings and the JWKS endpoint that DCR had written to those APIs remained in place, pointing at a policy and identity provider that no longer existed.

Deleting a product or plan, or removing an individual API from a product, now removes the corresponding scope-to-policy mappings from each affected API, and clears the JWKS endpoint when no other active product or plan still references it.

For Tyk OAS APIs, the Portal now writes this configuration to the dedicated OAS JWT fields rather than the legacy fields.
</Accordion>
