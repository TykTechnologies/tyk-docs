...
In this release, MCP Proxy APIs are no longer exposed to end-developers through the Enterprise Developer Portal. Previously, MCP Proxy APIs and the policies targeting them could sync into the portal as Products, making them visible even though the portal has no first-class MCP support. This fix ensures MCP Proxies are filtered out at every sync entry point — both via policy sync and direct API attachment — so they do not appear as Products for end-developers. First-class MCP support will be introduced in a subsequent release.
