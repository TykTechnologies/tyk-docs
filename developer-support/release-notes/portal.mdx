...
Following the temporary exclusion of MCP Proxies flagged in earlier releases, Tyk Enterprise Developer Portal now provides first-class support for Tyk MCP Proxy APIs. MCP APIs and their products now sync into the Portal catalogue automatically, admins can add documentation for them from Portal Admin, and developers can find them in the live catalogue alongside REST, GraphQL and other supported API types, complete with a dedicated MCP badge.

Developers can now explore and call an MCP server directly from its documentation page in Portal, without separate tooling. This is powered by a new embedded, MCPP-compatible reverse proxy that authenticates using the developer's Portal session, applies per-user rate limiting, and blocks server-side request forgery (SSRF) by default, together with the official MCP Inspector, now vendored and embedded on product documentation pages.

For MCP products protected with OAuth 2.0, Portal now surfaces Protected Resource Metadata (RFC 9728) - the resource identifier, authorization servers, supported scopes and discovery path - or indicates when this is discovered at runtime instead. Dynamic Client Registration also now supports public clients that authenticate with PKCE rather than a client secret.

Other changes in this release include updates to the Product Administration UI, with "API Products" renamed to "Products" and the "API's" tab renamed to "Services", plus consistent auth-type badge colours across the catalogue and admin.

This release also adds support for referencing secrets and configuration values in the Portal configuration file from an external secret store, using the same unified `kv://` / `$kv{}` syntax introduced across other Tyk components this wave, and updates the Go toolchain to Go 1.26. Two bugs are also fixed: developer email addresses are now handled case-insensitively throughout Portal, and creating a product via the Portal API no longer inherits unrelated API documentation from another product.
...
<Accordion title='MCP Proxy APIs and their products now sync into the Portal catalogue'>
{/* TT-17195 */}
The Portal's sync process now recognises Tyk MCP Proxy APIs using the same detection Tyk Gateway uses, and syncs them along with their products and plans. This removes the temporary exclusion introduced in earlier releases.
</Accordion>

<Accordion title='Admins can add documentation to MCP APIs from Portal Admin'>
{/* TT-17196 */}
A new "MCP (Model Context Protocol)" documentation type is available when adding documentation to a product, with a Markdown editor for authoring content.
</Accordion>
...
<Accordion title='Added Protected Resource Metadata and public OAuth client support for MCP products'>
{/* TT-17591 */}
Portal now surfaces OAuth 2.0 Protected Resource Metadata (RFC 9728) for MCP products that have it enabled at the Gateway, or indicates that it's discovered at runtime for products using mirror mode. Dynamic Client Registration also supports registering public OAuth clients that authenticate using PKCE instead of a client secret.
</Accordion>

<Accordion title='Added a unified `kv://` syntax for referencing secrets and configuration values'>
{/* TT-16268, TT-17701 */}
Tyk Portal now supports the same unified syntax for referencing secrets and configuration values held in an external store that other Tyk components use: `kv://<store-name>/<path>` to substitute a whole value, or `$kv{<store-name>:<path>}` to substitute a value inline within a string. Declare one or more named secret stores in a new `kv` section of the Portal configuration file — in `.conf`/`.json`, `.yaml`/`.yml`, or `.toml` format — then reference them from any string configuration value.
Supported store types include Hashicorp Vault, Hashicorp Consul, environment variables, local files, inline values, and cloud provider secret managers (AWS, GCP, and Azure). References are resolved once at startup. Provider configuration keys are matched case-insensitively but punctuation-exact, while store names and secret key names are matched exactly — a mismatch always fails startup with a clear error rather than silently resolving to a missing or incorrect value. A secret store marked as `required` causes startup to fail if it's unreachable.
 
To help verify configuration without exposing secrets, Portal also adds a `--show-configuration` CLI flag and an admin `/config` HTTP endpoint. Both display the resolved configuration with each configured secret store's own provider credentials redacted, while still showing the store's name, type, and `required` flag. Configurations with no `kv` section behave exactly as before.
</Accordion>
...
<Accordion title='Fixed case-sensitive handling of developer email addresses'>
{/* TT-14811 */}
Tyk Portal previously stored and compared developer email addresses case-sensitively, which could prevent password reset emails from being delivered and could result in duplicate accounts for the same address in a different case — for example after an admin invitation, during self-registration, or via SSO. Email address matching and lookups throughout Portal are now case-insensitive, and newly created or updated accounts store their email address in normalised (lowercase) form. This fix applies without requiring a data migration for existing accounts.
</Accordion>

<Accordion title="Fixed a new product incorrectly inheriting another product's API documentation">
{/* TT-15912 */}
When creating a new product via the Portal API, if the product referenced an API that already had documentation attached to it in a different, unrelated product, Portal would automatically link that documentation (for example an OpenAPI or GraphQL SDL document) to the new product, even though it was never submitted in the request. This didn't happen when creating a product through the Portal Admin UI. The Portal API and Portal Admin UI now behave consistently: creating or updating a product only associates documentation that is explicitly submitted for it.
