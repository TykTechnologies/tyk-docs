...
<Accordion title='Fix case-sensitive email matching causing failed password resets and duplicate accounts'>
Resolved an issue where the Portal stored and compared email addresses case-sensitively, unlike the Tyk Dashboard. This caused password reset emails to go unsent when a different casing was used than at registration, and could create duplicate accounts when the same email was used with different casing during an admin invite or self-registration.

Email addresses are now normalized and matched case-insensitively across login, password reset, admin invites, self-registration, and SSO, so users can reset their password and log in regardless of casing, and duplicate registrations are correctly rejected as "already registered." If multiple accounts already exist for the same email differing only in case, the system will match the most recently created account and log a warning that the email is duplicated.
</Accordion>

<Accordion title='Fix new API Products inheriting documentation from unrelated Products via the Portal API'>
Resolved an issue where creating a new API Product via the Portal API could automatically link OpenAPI or GraphQL documentation that had actually been attached to a different, unrelated API Product, granting access to the same underlying API, even though the new request never referenced that documentation. This did not occur when creating Products through the Portal UI, so the two paths now behave consistently.

Creating an API Product via the Portal API no longer inherits another Product's documentation. Existing documentation continues to be preserved correctly when a Product is updated without changing its own documentation.
</Accordion>
