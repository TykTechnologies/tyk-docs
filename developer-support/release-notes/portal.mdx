...
In this release, we’ve introduced a new Direct Access Flow as an alternative to the shopping cart flow, and we've enabled single credential management for multiple API products, along with enhanced SSO integration, improved mTLS certificate management, and several improvements and security fixes. 
...
<Accordion title='Direct Access Flow for API Product Requests'>
The Developer Portal now offers a streamlined Direct Access Flow that can substitute (if enabled in Admin General Settings) the shopping cart flow for API product access requests. This new flow allows developers to request access to API products immediately without adding them to a cart, reducing friction and improving the developer experience.

For more information, please visit [Requesting Access to an API Product](https://tyk.io/docs/product)
</Accordion>

<Accordion title='Single Credential for Multiple API Products'>
Developers can now add multiple API products to a single credential set, eliminating credential sprawl. This feature allows for better credential management while maintaining security and access control.

For more information, please visit [Credential Management and Reusability](https://tyk.io/docs/product)
</Accordion>

<Accordion title='Custom Login URL Support'>
The Developer Portal now supports configuring a custom login URL that redirects users to external identity providers instead of the default password login page. When configured, all authentication flows automatically redirect to the specified external login system. This can be configured by adding the `SSOCustomLoginURL` setting to your portal configuration or setting the environment variable `PORTAL_SSO_CUSTOM_LOGIN_URL` when starting the portal.
</Accordion>

<Accordion title='Centralised Management for mTLS APIs'>
...
<Accordion title='Added Developer App Credentials Access on Product Documentation Pages'>
Enhanced the Developer Portal by making developer app credentials available directly within API product documentation pages. This eliminates the need for users to navigate away from documentation to retrieve their API keys, significantly reducing friction when testing APIs.
</Accordion>

<Accordion title='Enhanced GraphQL URL Handling for Trailing Slash Consistency'>
Improved GraphQL endpoint URL handling to automatically support both trailing slash and non-trailing slash variations, ensuring seamless compatibility when users copy API endpoints from the Dashboard to the GraphQL server URL in the Developer Portal.
</Accordion>

<Accordion title='Added Auto-Population of GraphQL Server Endpoints in API Product Documentation'>
Enhanced the API Product creation workflow by automatically pre-populating the GraphQL server endpoint field when a GraphQL API is selected. The system now combines the Gateway host with the API's Listen Path to generate the correct endpoint format, while keeping the field editable for custom configurations.
</Accordion>
...
<Accordion title='Private Catalogue Documentation Access Control'>
Fixed a security vulnerability where API documentation for documentation-only products in private catalogues could be accessed directly via URL without authentication. All content in private catalogues now consistently requires authentication, regardless of whether the API Product contains associated APIs.
</Accordion>
...
When viewing a documentation-only product, API specifications whose alias contains a hyphen (-) may not appear correctly in the API Specifications tab. This does not affect runtime functionality, or API Product docs in the Catalogue. We recommend not using a hyphen in the specification alias of a documentation-only product.
