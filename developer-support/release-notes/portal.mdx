...
Developer Portal v1.16.0 addresses credential sprawl that impacts large-scale API programs. Developers can now reuse a single credential across multiple API products. We've also streamlined the access request flow from 5 steps to 2. For admins, you get instant central control: add or remove API access from any credential in seconds, change plans without generating new keys, and manage mTLS certificates centrally for compliance.

**Key benefits:**
- **One key, multiple APIs:** Reuse credentials when auth requirements match
- **Streamlined access requests:** Direct flow eliminates the shopping cart (now optional)
- ** Change plans on existing credentials:** Upgrade rate limits without regenerating keys or redeploying apps
...
Both API consumers and portal admins can now change plans for products on active credentials within an app, allowing access level updates without regenerating keys or redeploying apps.
</Accordion>
