...
Extended the mTLS certificate management controls introduced in Portal 1.16.0 to the App Details page. The "Attach Mutual TLS Certificate" section now correctly appears only for credentials associated with self-serve mTLS products, maintaining consistency with the checkout flow behavior. For managed mTLS products, this section is appropriately hidden since certificate management is handled by administrators.
</Accordion>
