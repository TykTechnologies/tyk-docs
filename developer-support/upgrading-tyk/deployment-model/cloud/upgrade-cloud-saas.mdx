...
the [upgrade guidelines](/developer-support/upgrading-tyk/preparations/upgrade-guidelines).
...
Follow our guide for [upgrading Cloud Control Planes](/tyk-cloud/environments-&-deployments/managing-control-planes#upgrade-control-planes).

---

## Step #2: Upgrade Go Plugins

Follow our guide for deploying your [Go plugins on Tyk Cloud](/developer-support/upgrading-tyk/deployment-model/cloud/upgrade-go-plugin). Subsequently, follow the steps below according to the target upgrade version of the Gateway.

##### Gateway Versions < 4.1.0.

1. Proceed with [upgrading your Tyk Data Plane (Gateway)](#upgrading-cloud-data-planes)
2. Update the [custom_middleware_bundle](/plugins/how-to-serve-plugins/plugin-bundles#per-api--local-parameters) field in the API Definitions of all APIs that use your plugin. The field should be updated to use the new bundle file containing your upgrade plugin.
3. Validate that your plugin is working per your expectations.

##### Gateway Versions >= 4.1.0

1. Update the [custom_middleware_bundle](/plugins/how-to-serve-plugins/plugin-bundles#per-api--local-parameters) field in the API Definitions of all APIs that use your plugin. The field should be updated to use the new bundle file containing your upgraded plugin.
...
Follow our guide for [upgrading Cloud Data Planes](/tyk-cloud/environments-&-deployments/managing-gateways#upgrade-cloud-data-planes).
