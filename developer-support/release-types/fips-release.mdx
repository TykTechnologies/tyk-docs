...
For Tyk Pump, the FIPS artifact is the separately distributed `tyk-pump-fips` build. Pump FIPS releases follow the same version-specific delivery model as the other FIPS Products: 5.13.x and later are available as Docker Hardened Images or native RPM/DEB packages, while 5.8.x FIPS releases are available as native RPM/DEB packages only. Pump FIPS compliance applies to the Tyk-built Pump binary and its approved cryptographic operations; customers remain responsible for the compliance of the surrounding deployment and external data stores used by Pump. Pump release builds use the Go toolchain floor declared by the Pump module, currently Go 1.25 or later.
<!-- Documents: STK-REQ-002, SYS-REQ-024, SYS-REQ-026, SYS-REQ-036 -->
