...
Tyk Sync does not have a dedicated FIPS-specific Docker image variant (such as `tykio/tyk-sync-fips`). It is a CLI tool and is not listed as a FIPS-supported product in the primary FIPS documentation, but standard images (e.g., `tykio/tyk-sync:v2.1.8`) can be used, and it is compiled with Go 1.25.
