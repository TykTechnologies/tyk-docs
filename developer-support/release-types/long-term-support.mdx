...
Non-LTS releases, often called "feature releases", receive full support from Tyk. However, their support lifecycle is shorter than LTS releases.

A non-LTS minor version reaches its End-of-Life (EOL) and is no longer supported once the next minor version is released.

For example:
- Tyk Gateway v5.1.x was fully supported until v5.2.0 was released.
- Upon the release of v5.2.0, the v5.1.x branch became End-of-Life (EOL).

This means that bug fixes and security patches will only be applied to the latest active minor release branch. For users who require a longer support window with regular patching for stability, we strongly recommend using our LTS releases.
