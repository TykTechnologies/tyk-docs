...
2.  **Sync component versions to tyk-install:**
    Merging the release notes PR into `main` automatically triggers the [Sync Component Versions](https://github.com/TykTechnologies/tyk-docs/actions/workflows/sync-versions-to-tyk-install.yml) workflow, which opens a PR in the [tyk-install](https://github.com/TykTechnologies/tyk-install) repository to update the component image tags in `docker/self-managed/.env.example`. Review and merge that PR.
...
4.  **Link the release notes to the feature documentation:**
    **Description:** Release notes merge before the feature documentation is ready. The links are left out at release time.\
    **Steps:** Wait until the feature documentation merges. Then follow [Link Release Notes to Feature Documentation](#link-release-notes-to-feature-documentation).
...
**Note:** Check whether a `docs-X.Y` staging branch exists for this release (used to stage large feature docs ahead of the version bump). If it has content not yet merged into `main`, merge it first, or explicitly confirm with the team it's intentionally held back - otherwise the new release branch (cut from `main`) will be missing that content.
...
6.  **Deploy release:**
    ...
    The PRs mentioned in the prerequisites can now be merged into `main` and the latest release branch. To backport a PR already merged into `main` onto the new release branch, comment `/release to release-X.Y` on it - a bot cherry-picks the change into a new PR against that branch automatically.

7. **Sync component versions to tyk-install:**
    Merging the release notes PR into `main` automatically triggers the [Sync Component Versions](https://github.com/TykTechnologies/tyk-docs/actions/workflows/sync-versions-to-tyk-install.yml) workflow, which opens a PR in the [tyk-install](https://github.com/TykTechnologies/tyk-install) repository to update the component image tags in `docker/self-managed/.env.example`. Review and merge that PR.


8. **Update Postman Collections:**\
...
10. **Verify:**
    Once the deployment PR (step 4) merges, check the live site: [tyk.io/docs](https://tyk.io/docs) should serve the new latest version's content at the root, and the outdated-version banner on an older version's page (e.g. `tyk.io/docs/5.13/...`) should reference the new latest version, not the previous one.

11.  **Link the release notes to the feature documentation:**
    **Description:** Release notes merge before the feature documentation is ready. The links are left out at release time.\
    **Steps:** Wait until the feature documentation merges. Then follow [Link Release Notes to Feature Documentation](#link-release-notes-to-feature-documentation).

## Link Release Notes to Feature Documentation

Release notes merge before the feature documentation is ready. A link added at that point can fail the `validate` CI job, because the target page does not exist yet. It can also ship as a link to nowhere. For this reason, the links are left out at release time. They are then forgotten.

Do this pass after the release is live and all feature documentation PRs have merged.

### What to Link

| Release notes section | Add a link? |
| :-------------------- | :---------- |
| **Release Highlights** | Yes. Use inline links in the prose. |
| Changelog, **Added** | Yes. Add a trailing `For more details see [Page](/path).` line. |
| Changelog, **Changed** | Only if the change alters documented behavior. |
| Changelog, **Fixed** and **Security Fixes** | No. |
| Dependency and Go version bumps | No. |

Cover every component in the release, not only Tyk Gateway and Tyk Dashboard. Check the release announcement for the full list.

### Rules

*   Use markdown `[]()` links with root-relative paths, for example `/tyk-configuration-reference/kv-store`.
*   Do not use an absolute `https://tyk.io/docs/...` URL. The build adds the version prefix to root-relative paths. An absolute URL always goes to the latest version. This is wrong for a reader on `/docs/nightly` or on an older version.
*   Link to the most specific section that answers the entry. Use an anchor where one exists.
*   If a feature has no documentation, leave the entry unlinked and raise the gap. Do not link to an unrelated page.

### Raise the PR

Raise one PR for all components, against `main`. Label it `release-X.Y` so the bot cherry-picks it to the release branch, which is what puts the links on the released version of the docs. Never commit to `production`.

Check the links locally before you push:

```bash
pip install 'requests>=2.31.0,<3' 'pyyaml>=6.0,<7'
