...
<Note>
The `tyk.mcp.requests.total` counter uses 6 dimensions. Keep the total dimension count at 10 or fewer per instrument to stay on the OTel SDK fast path. See [Cardinality Considerations](/api-management/logs#cardinality-considerations) for guidance.
</Note>
...
- [Custom metrics](/api-management/logs) — How to define custom metric instruments and dimensions
- [Default gateway metrics](/api-management/logs) — Standard RED metrics exported by the gateway
- [Observability best practices](/api-management/logs) — Cardinality control, sampling strategy, and export topology guidance
