...
<Note>
The `tyk.mcp.requests.total` counter uses 6 dimensions. Keep the total dimension count at 10 or fewer per instrument to stay on the OTel SDK fast path. See [Performance Considerations](/api-management/logs#performance-considerations) for guidance.
</Note>
