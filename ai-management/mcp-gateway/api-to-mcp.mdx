...
- **A tool catalogue**: derived automatically from the source API and kept in sync on every gateway reload. See [Tool Catalogue](#tool-catalogue) below.
...
Tyk builds the tool catalogue by reading the source API's OpenAPI operations.
...
The interface you use doesn't change how the proxy behaves at runtime: the tool catalogue, enrichment overrides, and everything else here apply the same way regardless of how the definition was authored.
