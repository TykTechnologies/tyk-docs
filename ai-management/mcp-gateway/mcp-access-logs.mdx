...
Nothing links these two entries together: there's no shared request ID or tag connecting them. To correlate them, match on timestamp and note that the proxy's own `latency_total` includes the time spent on the source API call, so it's always the larger of the two figures.
