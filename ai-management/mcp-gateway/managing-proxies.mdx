...
<Note>
Review this list carefully before continuing, especially for APIs with write or destructive operations. Exposing every endpoint by default is a convenience, not a security recommendation — deselect any endpoint an agent shouldn't be able to call, rather than relying on downstream policy alone to restrict access.
</Note>
...
![MCP Designer tabs](/img/ai-management/mcp-designer-source-api-panel.png)
...
![Actions menu open on an MCP proxy, showing Delete MCP Proxy](/img/ai-management/view-mcp-proxy-definition.png)
