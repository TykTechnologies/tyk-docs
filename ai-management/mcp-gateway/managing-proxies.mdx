...
Click **Save MCP Proxy** to apply your changes. These map to `x-tyk-mcp-server.primitives[]` in the proxy definition — see [Tool name and parameter enrichment](/ai-management/mcp-gateway/api-to-mcp-enrichment) for the full field reference.
...
| `x-tyk-mcp-server` | REST API to MCP proxies only. Holds the tool catalogue: which source operations are exposed, and any name, description, or parameter overrides. See [Tool name and parameter enrichment](/ai-management/mcp-gateway/api-to-mcp-enrichment). |
