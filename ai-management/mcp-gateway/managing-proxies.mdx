...
Only Tyk OAS APIs appear in this list. Tyk Classic API definitions cannot be used as an API to MCP source — see [Requirements](/ai-management/mcp-gateway/overview#requirements-and-limitations).
...
Unlike the remote MCP server wizard, deployment targets for an API to MCP proxy are inherited from the source API and can't be set here. Set the initial **MCP proxy Status** (**Active** or **Inactive**).
...
Clicking a proxy in the list opens the MCP Designer. For a proxy fronting a remote MCP server, the MCP Designer has two tabs: **Settings** and **Primitives**. For an API to MCP proxy, the Designer inserts a third tab, **Tool mapping**, between them.

![MCP Designer tabs](/img/ai-management/mcp-designer-tabs.png)

{/* TODO: This screenshot may only show the two-tab (Settings/Primitives) layout - check whether it needs replacing with a three-tab (Settings/Tool mapping/Primitives) view for an API to MCP proxy, or whether both variants need showing */}
...
The **Tool mapping** tab appears only on API to MCP proxies. It lists every operation available from the source API on the left; only the operations you select are exposed as tools on this proxy. A counter shows how many of the total operations are currently exposed, and a search box filters the list.
...
Click **Save MCP Proxy** to apply your changes. These map to `x-tyk-mcp-server.primitives[]` in the proxy definition — see [API to MCP: enrichment](/ai-management/mcp-gateway/api-to-mcp-enrichment) for the full field reference.
...
| `x-tyk-api-gateway.upstream` | Upstream URL, load balancing, upstream authentication, and mTLS. For an API to MCP proxy, `upstream.url` holds an adapter target (for example `tyk://orders-rest/mcp`) instead of a remote `https://` URL — see [MCP OAS definition](/ai-management/mcp-gateway/mcp-proxy-definitions#info-server-and-upstream) for what this value means. |
| `x-tyk-mcp-server` | API to MCP proxies only. Holds the tool catalogue: which source operations are exposed, and any name, description, or parameter overrides. See [API to MCP: enrichment](/ai-management/mcp-gateway/api-to-mcp-enrichment). |
...
| `x-tyk-api-gateway.middleware.mcpResources` | Per-resource middleware: same options as tools, keyed by resource URI or URI pattern. Not used by API to MCP proxies, since REST APIs have no resource or prompt primitives. |
| `x-tyk-api-gateway.middleware.mcpPrompts` | Per-prompt middleware: same options as tools, keyed by prompt name. Not used by API to MCP proxies. |
