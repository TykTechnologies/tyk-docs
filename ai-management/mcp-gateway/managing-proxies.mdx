...
| `x-tyk-api-gateway.upstream` | Upstream URL, load balancing, upstream authentication, and mTLS. For an API to MCP proxy, `upstream.url` holds an adapter target pointing at the paired Tyk API (for example `tyk://orders-rest/mcp`) instead of a remote `https://` URL. |
