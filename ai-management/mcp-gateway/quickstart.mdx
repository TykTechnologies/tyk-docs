...
- **Set up token exchange**: Use RFC 8693 token exchange so the inbound agent token is replaced with a backend-scoped token before reaching the upstream MCP server. See [Token exchange](/api-management/authentication/token-exchange).
