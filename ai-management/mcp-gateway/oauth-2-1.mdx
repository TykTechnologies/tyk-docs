...
For every PRM field, its default, and the automatic migration from the pre-5.14 location, see [Protected Resource Metadata](/api-management/authentication/oauth2-authentication#protected-resource-metadata).
...
For the `scopeCheck` fields, the `scopeSource` modes, and per-operation and per-primitive exemptions, see [Scope enforcement](/api-management/authentication/oauth2-authentication#scope-enforcement).
...
For the provider fields and multi-tenant issuer matching, see [Provider fields](/api-management/authentication/token-exchange#provider-fields).
