...
Token exchange is Enterprise Edition only and is configured within the `oauth2` security scheme. For full configuration details — including provider setup, caching, actor tokens, and per-primitive overrides — see [Token exchange](/api-management/authentication/token-exchange).
