...
description: "Expose different subsets of a single Tyk-managed REST API's operations as separate, independently governed MCP proxies — for example, a read-only proxy for a reporting agent and a write-enabled proxy for an operations agent."
...
A single REST API often needs to serve more than one kind of agent, each with a different level of trust. A reporting agent might only need read access; an operations agent might need to update or cancel things. Rather than exposing every operation to every agent through one proxy and relying on per-consumer policy to restrict access, you can [generate more than one MCP proxy from the same REST API](/ai-management/mcp-gateway/api-to-mcp), each with its own tool catalogue, credential, and policy.

This guide slices an orders API into two proxies: a read-only **Orders Reporting** proxy exposing only lookup operations, and a write-enabled **Orders Operations** proxy exposing order updates and cancellations.
...
The two proxies below both target the same source API (ID `a1b2c3d4e5f647a8b9c0d1e2f3a4b5c6`), but each has its own allow-list:

**Orders Reporting** — read-only lookups only:

```json
{
  "x-tyk-api-gateway": {
    "info": { "name": "Orders Reporting", "state": { "active": true } },
    "server": { "listenPath": { "value": "/orders-reporting/", "strip": true } },
    "upstream": { "url": "tyk://a1b2c3d4e5f647a8b9c0d1e2f3a4b5c6/mcp" }
  },
  "x-tyk-mcp-server": {
    "primitives": [
      { "source": { "operationId": "getOrderStatus" }, "name": "get_order_status", "allow": true },
...
**Orders Operations** — the write operations, none of the read-only ones:

```json
{
  "x-tyk-api-gateway": {
    "info": { "name": "Orders Operations", "state": { "active": true } },
    "server": { "listenPath": { "value": "/orders-operations/", "strip": true } },
    "upstream": { "url": "tyk://a1b2c3d4e5f647a8b9c0d1e2f3a4b5c6/mcp" }
  },
  "x-tyk-mcp-server": {
    "primitives": [
      ...
      {
        "source": { "method": "PATCH", "path": "/orders/{id}/shipping" },
...
Because they're separate proxies, **Orders Reporting** and **Orders Operations** each get their own credential and [security policy](/ai-management/mcp-gateway/policies) — issue keys scoped to whichever proxy an agent should reach, rather than relying on per-tool RBAC within a single shared proxy. A reporting agent's key never needs to be trusted with write access at all, because the write tools don't exist on the proxy it connects to.
