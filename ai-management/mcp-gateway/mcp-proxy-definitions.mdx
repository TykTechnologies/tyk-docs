...
An MCP proxy [generated directly from a Tyk-managed REST API](/ai-management/mcp-gateway/api-to-mcp) has one further structure: `x-tyk-mcp-server`, a vendor extension alongside `x-tyk-api-gateway` rather than nested inside it, holding the tool catalogue Tyk derives from the source API's OpenAPI operations. See [REST API to MCP x-tyk-mcp-server extension](/ai-management/mcp-gateway/api-to-mcp-definitions) for the full field reference, the allow-list selection rule, tool name validation limits, and worked examples.
