...
For an MCP proxy [generated directly from a Tyk-managed REST API](/ai-management/mcps/api-to-mcp), `upstream.url` holds an adapter target instead of a remote server's url. See [The upstream adapter target](/ai-management/mcp-gateway/api-to-mcp-definitions#the-upstream-adapter-target) for what this value means and why it takes this form.
