...
<Note>
For an MCP proxy [generated directly from a Tyk-managed REST API with API to MCP](/ai-management/mcp-gateway/api-to-mcp), `upstream.url` holds an adapter target pointing at the paired Tyk API (for example `tyk://orders-rest/mcp`) instead of a remote server's `https://` URL. See [API to MCP: definitions](/ai-management/mcp-gateway/api-to-mcp-definitions).
</Note>
