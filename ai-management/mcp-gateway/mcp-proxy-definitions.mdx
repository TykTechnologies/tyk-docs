...
All operations become tools by default. To narrow that down, add entries with `allow: true` for only the operations you want exposed — as soon as one entry has `allow: true`, Tyk switches to that explicit allow-list and every other operation is excluded. Entries without `allow: true` are still valid: they apply their `name`/`description`/`parameters`/`annotations` overrides as metadata, without affecting whether the operation is exposed.
...
This proxy exposes exactly two tools — `get_order_status` and `cancel_order` — because at least one entry has `allow: true`. Every other operation on the source API is excluded.
