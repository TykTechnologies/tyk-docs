...
| **Distributed tracing** | — | ❌ Not implemented | The `TRACING_ENABLED` flag exists but MCP spans are not emitted. Use metrics and access logs for observability. |
...
- [MCP API configuration](/ai-management/mcp-gateway/mcp-api-extensions) — Complete field reference for every option in the `x-tyk-api-gateway` extension for MCP proxies.
