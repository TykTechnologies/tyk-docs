...
**When an MCP proxy is [generated directly from a Tyk-managed REST API with API to MCP](/ai-management/mcp-gateway/api-to-mcp)**, tool names are derived from the `operationId` field of each OpenAPI operation. An operation with `operationId: getWeatherForecast` becomes a tool named `getWeatherForecast`. If an operation has no `operationId`, Tyk derives a deterministic tool name from its HTTP method and path instead — for example, `GET /orders/{id}` becomes `get_orders_id` — which stays stable across reloads. The exception: if the source API's OAS already uses an operation-level allow-list (`x-tyk-api-gateway.middleware.operations[...].allow`), operations without `operationId` are excluded from the MCP tool catalogue entirely, since that allow-list is keyed by `operationId`.
