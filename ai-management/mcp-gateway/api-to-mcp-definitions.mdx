...
This proxy exposes exactly one tool, `get_order_status`, the only entry marked `allow: true`. `cancel_order` isn't exposed, since it isn't marked `allow: true` — and neither is any other operation on the source API. Its `description` and `annotations` overrides are still saved in the definition, and take effect only if you later add `allow: true` to that same entry.
