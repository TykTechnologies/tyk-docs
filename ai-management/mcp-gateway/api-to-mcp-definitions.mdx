...
This proxy exposes exactly one tool, `get_order_status`, the only entry marked `allow: true`. `cancel_order` isn't exposed, since it isn't marked `allow: true` — and neither is any other operation on the source API. Its `description` and `annotations` overrides still apply as metadata, even though it isn't a tool.
