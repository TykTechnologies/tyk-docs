...
keywords: "AI Management, Platform Teams, Tyk AI Studio"
...
Tyk AI Studio is available in two editions to suit different organizational needs:

### Community Edition (CE)

The Community Edition is open source and available to everyone. It provides a robust foundation for AI management, including:

- **Core AI Gateway**: Full LLM proxy capabilities, chat interfaces, and tool integrations.
- **Visibility**: Cost tracking and analytics dashboards.
- **Deployment**: Hub-and-spoke architecture with edge gateway management and configuration synchronization.
- **Basic Security**: Plugin system with path whitelisting and checksum validation.
- **Marketplace**: Access to the official Tyk plugin marketplace.

**How to get it**: The Community Edition is publicly available. You can build it directly from the open-source repository or use the provided Docker images.

### Enterprise Edition (ENT)

The Enterprise Edition operates under a proprietary commercial license and includes everything in the Community Edition, plus advanced features for enterprise governance, security, and scale:

- **Budget Management**: Real-time budget enforcement, forecasting, and automated email alerts at usage thresholds.
- **Advanced Security & Identity**: SSO integration (SAML, OIDC, LDAP, Social), automated user provisioning, and IdP group mapping. Enhanced plugin security with GRPC host whitelisting and OCI signature verification.
- **Multi-Tenancy**: Unlimited multi-tenant namespaces with full configuration isolation and per-namespace operations.
- **Granular Access Control**: Group-based access control, custom catalog management (LLM, Data, Tool), and resource filtering by user group membership.
- **Customization & Governance**: Support for multiple custom plugin marketplaces, audit logging, and priority support.

**How to get it**: The Enterprise Edition requires a commercial license and access to the private enterprise repository. Contact support@tyk.io, enterprise@tyk.io, or your account manager to request your AI Studio Enterprise license and repository access.
