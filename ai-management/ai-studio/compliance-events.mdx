...
- **Custom analytics handlers**: the `AnalyticsHandler` interface now takes `context.Context` on its recording methods, including the new `RecordComplianceEvents`. Custom implementations need to update their signatures. See [Plugin SDK Changes in v2.1](/ai-management/ai-studio/plugins/sdk-changes-v2-1#changed-analyticshandler-interface).
...
- [Plugin SDK Changes in v2.1](/ai-management/ai-studio/plugins/sdk-changes-v2-1): the `AnalyticsHandler` context change and related SDK updates.
