...
- **Custom analytics handlers**: the `AnalyticsHandler` interface now takes `context.Context` on its recording methods, including the new `RecordComplianceEvents`. Custom implementations need to update their signatures. See the [Tyk AI Studio release notes](/developer-support/release-notes/ai-studio) for the migration.
