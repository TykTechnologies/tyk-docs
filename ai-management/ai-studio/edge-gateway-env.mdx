...
import EdgeGatewayConfig from '/snippets/5.15/edge-gateway-config.mdx';
...
<Note>
The Edge Gateway binary, its Helm values, and its container images use the older name `microgateway` (for example `./microgateway`, the `microgateway.enabled` Helm value, and the `tykio/tyk-microgateway-ent` image). These docs use "Edge Gateway" for the same component.
</Note>
