...
```mermaid
graph TD
    A[Client Request] -->|Model: gpt-4| B[Model Router]
    B --> C{Match Model Pattern}
    C -->|Pattern: gpt-*| D[Model Pool]
    D --> E{Select Vendor}
    E -->|Round Robin / Weighted| F[Selected Vendor]
    F --> G{Apply Model Mapping}
    G -->|gpt-4 -> claude-3-opus| H[Rewrite Request]
    H --> I[AI Gateway]
    I --> J[Target LLM Provider]
```
