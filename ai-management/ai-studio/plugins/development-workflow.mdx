...
flowchart TD
    subgraph "PLUGIN DEVELOPMENT LOOP"
        Step1["1. Edit Code<br/>└── main.go, manifest.json, etc."]
        Step2["2. Build<br/>└── go build -o my-plugin ."]
        Step3["3. Reload (ONE command!)<br/>└── curl -X POST .../plugins/{id}/reload"]
        Step4["4. Test<br/>└── Make requests, check logs, verify behavior"]
        Step5["5. Repeat from Step 1"]
