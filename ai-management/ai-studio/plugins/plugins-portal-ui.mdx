...
```mermaid
graph LR
    A[AI Portal] -->|Loads| B[Custom Page]
    B -->|RPC| C[Plugin Backend]
```mermaid
graph LR
    A[Portal User] --> B[Portal UI]
    B --> C[Plugin WebComponent]
    C -->|RPC| D[Plugin Backend]
```
...
- [AI Studio UI Plugins Guide](plugins-studio-ui) - Admin UI plugin development
- [Plugin Examples](plugins-examples) - Browse working examples
- [Service API Reference](plugins-service-api) - KV storage, logging, and management APIs
