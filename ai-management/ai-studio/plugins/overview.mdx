...
Plugins can also run as standalone gRPC services in a sidecar or elsewhere on the network, rather than as local sub-processes. See [Plugin Deployment](deployment) for details.
...
[Learn more about Service API →](service-api)
...
[Learn more about manifests →](manifests)
...
5. Build and test with `file://` deployment (see [Development Workflow Guide](development-workflow) for fast iteration)
6. Deploy with `grpc://` or `oci://` for production

**Pro tip**: Use the reload API (`POST /api/v1/plugins/{id}/reload`) to test changes instantly without reinstalling. See the [Development Workflow Guide](development-workflow) for the fastest iteration loop.
...
- **[Development Workflow Guide](development-workflow)** - Fast iteration with file:// and reload API
- **[Plugin SDK Reference](sdk)** - Complete SDK documentation
- **[Plugin Examples](examples)** - Browse working examples (including production-ready community plugins)
- **[Object Hooks Guide](object-hooks)** - Intercept CRUD operations
- **[Custom Endpoints Guide](custom-endpoints)** - Serve custom HTTP endpoints (MCP, OAuth, webhooks)
- **[Microgateway Plugins Guide](microgateway)** - Gateway-specific patterns
- **[AI Studio UI Plugins Guide](studio-ui)** - Build admin plugin UIs
- **[AI Portal UI Plugins Guide](portal-ui)** - Build portal-facing plugin pages and forms
- **[AI Studio Agent Plugins Guide](studio-agent)** - Build conversational agents
- **[Service API Reference](service-api)** - Complete API documentation
- **[Plugin Deployment](deployment)** - Production deployment options
- **Migration Guide** - Upgrade from old SDKs
