...
| **[Custom Endpoints](custom-endpoints)** | `CustomEndpointHandler` | Gateway | Serve custom HTTP endpoints under `/plugins/{slug}/` |
| **UI Provider** | `UIProvider` | Studio only | Serve admin web UI assets |
| **[Portal UI](portal-ui)** | `PortalUIProvider` | Studio only | Portal-facing pages and forms with user context |
...
| **[Resource Provider](resource-types)** | `ResourceProvider` | Studio only | Register custom resource types for Apps with privacy scoring |
...
Conversational AI agent with streaming support. See [Agent Plugins Guide](studio-agent) for details.
...
Intercept CRUD operations on AI Studio objects (LLMs, Datasources, Tools, Users). See [Object Hooks Guide](object-hooks) for complete details.
...
Serve web UI assets for AI Studio plugins. See [UI Plugins Guide](studio-ui) for details.
...
Receive data from edge (Microgateway) plugins. This enables the hub-and-spoke communication pattern where edge plugins can send data back to the control plane. See [Edge-to-Control Communication](edge-to-control) for complete details.
...
- See [Service API Reference](service-api#event-service) for complete documentation
