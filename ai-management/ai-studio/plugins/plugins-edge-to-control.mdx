...
```mermaid
graph TD
    A[Edge Plugin] -->|Batched Data| B[Control Plane Plugin]
    B --> C[Database/Analytics]
```mermaid
graph TD
    subedge[Edge Gateway]
    A[Microgateway Plugin] -->|Batches Data| B[Edge-to-Control Client]
    end
    subcontrol[Control Plane]
    C[Studio Plugin] -->|Receives Data| D[Database/Analytics]
    end
    B -->|gRPC Stream| C
```
...
- [Microgateway Plugins Guide](plugins-microgateway) - Gateway plugin development
- [Service API Reference](plugins-service-api) - KV storage and logging APIs
- [Hub-and-Spoke Architecture](architecture-hub-spoke) - System architecture overview
