...
This API key is specifically used for programmatic access to the Tyk AI Studio management APIs (the Admin API) and the AI Portal. It allows developers to authenticate their applications against Tyk AI Studio to consume the configured LLM endpoints. 

*Note: This is **not** the API key for the underlying LLM providers (like OpenAI or Anthropic). The actual LLM provider keys are stored securely in the backend and are never exposed to users.*
