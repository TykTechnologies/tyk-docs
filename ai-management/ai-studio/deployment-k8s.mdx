...
<Note>
The Helm chart, its values, and its container images use the older name `microgateway` for the Edge Gateway component (for example the `microgateway:` values block and the `tykio/tyk-microgateway-ent` image). This guide uses "Edge Gateway" in prose for the same component.
</Note>
