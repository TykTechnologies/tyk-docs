...
After generating the configuration files and before starting up the containers, you **must** update these files with the secrets created in the [Generate Secrets](#generate-secrets) step. Additionally, as this is an Enterprise deployment, you must update the license key environment variable.
