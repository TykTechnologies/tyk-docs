...
<Note>
The Edge Gateway's image, binary, and directory names use the older name `microgateway` or the abbreviation `mgw` (for example the `tykio/tyk-microgateway-ent` image and the `mgw-data`/`mgw-plugins` directories below). This guide uses "Edge Gateway" in prose for the same component.
</Note>
