...
*   **Selecting Call Settings:** Choosing an existing [Call Setting](/ai-management/ai-studio/call-settings) for the assigned LLM. A Call Setting defines runtime parameters, such as temperature and max tokens. AI Studio seeds this list with settings for current models, so a default is usually available.
