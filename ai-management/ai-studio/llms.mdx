...
---
#### **Use Cases**
- **[Chat Room Features](/ai-management/ai-studio/chats)**:
...
- **[AI Gateway](/ai-management/ai-studio/proxy)**:
...
3. **Don't Log Bodies** *(Toggle)*:
   Allows administrators to disable the storage of request and response bodies in proxy logs on a per-LLM basis for privacy and compliance.
   - **Enabled**: Request and response bodies are cleared/redacted before storage in ProxyLog, LLMChatLogEntry, and analytics events across both the embedded gateway and microgateway.
   - **Disabled**: Request and response bodies are logged and stored normally.
