...
Before you start configuring a profile, gather the following from your Identity Provider (IdP) side. Some of it depends on information from the AI Studio side that you only get partway through setup, so expect to go back and forth with your IdP admin once.

* **A callback URL to register with your IdP.** Every profile gets a callback URL in the form `https://<your-ai-studio-domain>/auth/<profile-id>/<provider>/callback`. The `<profile-id>` is only assigned once you create the profile in AI Studio, so the usual order is: create the profile with placeholder values, note its ID from the URL or profile list, register the callback URL with your IdP, then come back and finish the profile configuration.
* **For OIDC:** Client ID, Client Secret, and either an Issuer URL or a Discovery Endpoint from your IdP.
* **For SAML:** your IdP's SSO URL, IdP Issuer or Entity ID, and the IdP's public certificate. You will also need to give your IdP the SP Entity ID (Tyk AI Studio's own identifier).
* **Certificate format for SAML (current limitation):** AI Studio's certificate manager is file system based. The certificate must be saved as a file that AI Studio's server process can read, and the profile configuration must point to that file's path. There is currently no way to upload a certificate through the UI.
