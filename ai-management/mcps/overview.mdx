...
keywords: "AI MCP, MCPs in Tyk, Model Context Protocol"
sidebarTitle: "MCP Servers"
---

<Note>
To govern AI agent traffic to any MCP server reachable over HTTP, whether a vendor runs it or your own team does, see [MCP Gateway](/ai-management/mcp-gateway/overview). It covers authentication, per-tool access control, rate limiting and observability.
</Note>
...
With Tyk MCP Servers, your AI agents can:
...
<a id="mcp-for-enterprise-use"></a>

Tyk extends the MCP model for enterprise deployments with the following capabilities:

- **Remote MCP catalogs and server support** – Expose internal APIs and tools to AI assistants securely without requiring local installations.
