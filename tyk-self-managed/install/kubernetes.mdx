...
<Note>
Deploying to OpenShift? See [Deploy Tyk on OpenShift](/tyk-self-managed/install/openshift). Deploying through Argo CD or Flux CD? See [Deploy Tyk Helm Charts with GitOps](/product-stack/tyk-charts/gitops-deployment).
</Note>
...
<Note>
Please make sure you are installing Redis versions that are supported by Tyk. Please refer to Tyk docs to get list of [supported versions](/tyk-configuration-reference/redis-cluster-sentinel#supported-versions).
</Note>
