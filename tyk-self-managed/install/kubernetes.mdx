...
<Warning>
This deployment is NOT designed for production use or performance testing. The Tyk Install is our full, [Self-Managed](/tyk-self-managed/install) solution, which includes our Gateway, Dashboard and analytics processing pipeline. 
...
- Our Pro Demo Docker [GitHub repo](https://github.com/TykTechnologies/tyk-install)
...
If you are deploying the **Tyk Developer Portal**, set the appropriate values under the `enterprisePortal` section in your `values.yaml`. Please visit [Tyk Developer Portal installation](/portal/install/kubernetes) for a step by step guide.
