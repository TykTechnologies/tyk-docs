...
4. Clone the Tyk PoCs repo

    ```.bash
    git clone https://github.com/TykTechnologies/tyk-pocs
    cd tyk-pocs/docker/self-managed/
    ```

5. Add the license key to `confs/tyk_analytics.env` into the `TYK_DB_LICENSEKEY` variable using "vi" or "nano", etc
...
Inside `confs/tyk_analytics.env`, `TYK_DB_LICENSEKEY` should look something like this, with a real license however:

`
TYK_DB_LICENSEKEY=eyJhbGciOiJSUzI1NiIsInR5cCI...WQ
`
