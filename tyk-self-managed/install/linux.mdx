...
Tyk Gateway has a [dependency](/api-management/in-memory-database) on Redis. Follow the steps provided by Red Hat to make the installation of Redis, conducting a [search](https://access.redhat.com/search/?q=redis) for the correct version and distribution.

#### Storage Database

Tyk Dashboard has a dependency on a storage database that can be [PostgreSQL](/tyk-configuration-reference/sql) or [MongoDB](/tyk-configuration-reference/mongodb).

<Tabs>

<Tab title="PostgreSQL">

Check the PostgreSQL supported [versions](/tyk-configuration-reference/sql). Follow the steps provided by [PostgreSQL](https://www.postgresql.org/download/linux/redhat/) to install it.
...
Check the MongoDB supported [versions](/tyk-configuration-reference/mongodb). Follow the steps provided by [MongoDB](https://www.mongodb.com/docs/manual/tutorial/install-mongodb-on-red-hat/) to install it.
...
See [SQL configuration](/tyk-configuration-reference/sql) for details on installing SQL in a production environment.
</Tab>
</Tabs>
