...
Under high traffic, consider isolating Redis workloads across dedicated instances. Tyk Gateway supports a [separate analytics store](/planning-for-production/database-settings#separate-analytics-storage) for traffic log buffering and a [separate cache store](/planning-for-production/database-settings#separate-cache-storage) for API response caching, keeping them isolated from session and rate limiting data.
