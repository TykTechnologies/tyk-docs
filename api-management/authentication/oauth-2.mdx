...
- [optional] [metadata](/api-management/access-control/sessions-and-keys/understanding-sessions#metadata-and-context) to be added to the access tokens
...
| :--- | :--- | :--- |
| Register a new client app | `POST /api/apis/oauth/{{api-id}}` | [link](/api-management/dashboard-configuration#create-a-new-oauth2-0-client) |
...
| Get the details of a client app | `GET /api/apis/oauth/{{api-id}}/{{client_id}}` | [link](/api-management/dashboard-configuration#get-an-oauth2-0-client) |
...
The identity server will need access to the Tyk Dashboard API to [obtain an Authorization Code](/api-management/dashboard-configuration#oauth2-0-authorization-code).

[Tyk Identity Broker (TIB)](/tyk-identity-broker/overview) can fulfill this role, authenticating users against an external IdP and handling the authorization code exchange with Tyk Dashboard on their behalf. See [Issue API Access Tokens via TIB](/api-management/access-control/sessions-and-keys/issuing-tokens-via-tib) for details.
...
### Complete Example: OAuth 2.0 Authorization Code Grant for SSO

This section provides a complete, step-by-step example of implementing the OAuth 2.0 Authorization Code Grant flow with Tyk, suitable for Single Sign-On (SSO) scenarios.

#### Prerequisites

- Tyk Dashboard running at `http://localhost:3000`
- Tyk Gateway running at `http://localhost:8080`
- Dashboard API credentials (admin secret)
- An upstream API to protect (this example uses `http://httpbin.org`)

#### Step 1: Create the API Definition with OAuth 2.0

First, create an API that uses OAuth 2.0 with the Authorization Code grant type.
...
curl -X POST http://localhost:3000/api/apis/oas \
  ...
  -d '{
    "openapi": "3.0.3",
    "info": {
      "title": "OAuth2 Protected API",
      "version": "1.0.0"
    },
    "servers": [
      {
        "url": "http://localhost:8080/oauth2-api/"
      }
    ],
    "security": [
      {
...
The response includes the API ID that you'll need for subsequent steps:

```json
{
  "key": "b5eb9e7d72e34bdab5baa0260958feb9",
  "status": "ok",
  "action": "added"
}
```

Save the API ID (e.g., `b5eb9e7d72e34bdab5baa0260958feb9`) for the next steps.

#### Step 2: Create an OAuth Client

Register a client application that will be authorized to access the API:

```bash
curl -X POST http://localhost:3000/api/apis/oauth/<API_ID> \
  -H "Authorization: <DASHBOARD_API_KEY>" \
  -H "Content-Type: application/json" \
  -d '{
    "redirect_uri": "http://localhost:3000/identity-server/callback",
    "meta_data": {
      "app_name": "My SSO Application"
    }
  }'
```
...
```json
{
  "client_id": "c4a3b79a12d34e5f6789ab0c1d2e3f4a",
  "secret": "MTIzNDU2Nzg5MGFiY2RlZmdoaWprbG1ub3BxcnN0dXZ3eHl6",
  "redirect_uri": "http://localhost:3000/identity-server/callback",
  "meta_data": {
    "app_name": "My SSO Application"
  }
...
Note the `client_id` and `secret` values - you'll need these for authentication.

#### Step 3: Initiate the Authorization Request

The client application redirects the user to start the OAuth flow:
...
Tyk responds with a redirect to your identity server:

```
HTTP/1.1 307 Temporary Redirect
Location: http://localhost:3000/identity-server/login?client_id=c4a3b79a12d34e5f6789ab0c1d2e3f4a&redirect_uri=http%3A%2F%2Flocalhost%3A3000%2Fidentity-server%2Fcallback&response_type=code
```
...
```bash
# First, encode the client credentials for Basic Auth
# client_id:secret -> base64
# c4a3b79a12d34e5f6789ab0c1d2e3f4a:MTIzNDU2Nzg5MGFiY2RlZmdoaWprbG1ub3BxcnN0dXZ3eHl6
# Base64: YzRhM2I3OWExMmQzNGU1ZjY3ODlhYjBjMWQyZTNmNGE6TVRJek5EVTJOemc1TUdGaVkyUmxabWRvYVdwcmJHMXViM0J4Y25OMGRYWjNlSGw2
...
The complete Authorization Code Grant flow for SSO follows this sequence:

1. **User initiates login** → Client redirects to Tyk's authorization endpoint
2. **Tyk redirects to Identity Server** → User authenticates and authorizes
3. **Identity Server requests authorization code** → Tyk issues code via Dashboard API
4. **Identity Server redirects user back** → Client receives authorization code
5. **Client exchanges code for tokens** → Tyk issues access and refresh tokens
6. **Client accesses protected API** → Uses access token in Authorization header
7. **Token refresh (when needed)** → Client uses refresh token for new access token

<Note>
**Identity Server Integration**: In a production SSO setup, your Identity Provider (IdP) such as Keycloak, Auth0, Okta, or a custom solution handles user authentication (step 2-3). The IdP must be configured with access to the Tyk Dashboard API to request authorization codes on behalf of authenticated users.
</Note>
...
- [retrieve a list of all tokens for a client app](/api-management/dashboard-configuration#retrieve-all-current-tokens-for-specified-oauth2-0-client)
...
- [`oauth_token_expire`](/tyk-oss-gateway/configuration#oauth_token_expire): Overrides the default expiration time (in seconds) for newly generated access tokens. This value sets the logical `expires` timestamp in the access token's [Session](/api-management/access-control/sessions-and-keys/understanding-sessions).
- [`oauth_refresh_token_expire`](/tyk-oss-gateway/configuration#oauth_refresh_token_expire): Overrides the default expiration time (in seconds) for newly generated refresh tokens. If not set, it defaults to 14 days (1,209,600 seconds). When this TTL expires, the refresh token is physically deleted from Redis.
- [`oauth_token_expired_retain_period`](/tyk-oss-gateway/configuration#oauth_token_expired_retain_period): Controls a background cleanup job that removes expired tokens from an internal tracking list (a Redis sorted set used to track all tokens issued to a specific OAuth Client). It does **not** control the Redis TTL or the retention of the actual Session data.
...
For more details on how to control the physical retention of access token session data, see the [Session Lifecycle](/api-management/access-control/sessions-and-keys/session-lifecycle) documentation.
