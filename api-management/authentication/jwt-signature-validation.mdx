...
Tyk caches the [JSON Web Key Set](https://datatracker.ietf.org/doc/html/rfc7517) (JWKS) retrieved from JWKS endpoints to reduce the performance impact of contacting external services during request handling. Separate caches are maintained for each JWKS endpoint for each API, with a [default validity period of 240 seconds](/api-management/authentication/jwt-signature-validation#configuring-the-jwks-cache-validity-period).
...
### Configuring the JWKS cache validity period
