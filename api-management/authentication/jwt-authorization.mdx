...
After the JWT signature has been [validated](/api-management/authentication/jwt-signature-validation), Tyk uses the **claims** within the token to determine which security policies (access rights, rate limits and quotas) should be applied to the request.
