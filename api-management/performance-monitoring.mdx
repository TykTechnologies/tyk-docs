...
Internal load tests (Tyk Gateway `v5.13.0-alpha3` on GCP `c2-standard-4`, 30-minute runs at a 15k rps target against 10 routes, with a real OpenTelemetry collector on a 5s export interval) and Go micro-benchmarks consistently show the same picture:
...
| Analytics only (DB) | +28.1% | +78.5% | +6.9% | +1.5% | -1.0% |
...
- **Enable runtime metrics by default.** They give you GC, goroutine and heap signal at no measurable cost — the load test reports CPU −2% and memory 0% versus metrics-only, well inside run-to-run variance. There is no operational reason to leave them off.
...
- **Size pod memory by API count first.** The dominant memory lever is the number of loaded APIs, not whether OTel is on. 10→100 routes is +71% memory with OTel completely disabled; metrics add only ~+6% on top of that at 100 routes. Provision against `routes × pods` before you treat the metrics toggle as a memory mitigation.
