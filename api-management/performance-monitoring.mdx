...
The table below summarizes the per-feature overhead measured against the OTel-off baseline (10 routes, 15k rps target, 30-minute runs):
...
- **Prefer per-API detailed tracing over global enablement.** The Tyk OAS API [server.detailedTracing](/api-management/gateway-config-tyk-oas#detailedtracing) field (and its [Tyk Classic equivalent](/api-management/gateway-config-tyk-classic#opentelemetry)) lets you turn on middleware-level spans only for the APIs that need them, multiplying span volume only where it is justified rather than across the whole gateway.
