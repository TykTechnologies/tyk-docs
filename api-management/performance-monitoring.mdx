...
Tyk Gateway generates transaction records for each API request and response, containing [analytics data](/api-management/tyk-traffic-log-fields) relating to: the originating host (where the request is coming from), which Tyk API version was used, the HTTP method requested and request path etc.

The transaction records are transmitted to Redis and subsequently transferred to a persistent [data store](/api-management/traces/external-data-sinks) of your choice via Tyk Pump. Furthermore, Tyk Pump can also be configured to [aggregate](/api-management/dashboard-analytics#how-aggregation-works) the transaction records (using different data keys - API ID, access key, endpoint, response status code, location) and write to a persistent data store. Tyk Dashboard uses this data for:
