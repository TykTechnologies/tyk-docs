...
Internal load tests (Tyk Gateway `v5.13.0` on GCP `c2-standard-4`, 30-minute runs at a 15k rps target against 10 routes, with a real OpenTelemetry collector on a 5s export interval) and Go micro-benchmarks consistently show the same picture. Horizontal pod autoscaling was bounded to 2–12 pods and the fleet sat near the cap in every scenario (~11.8 pods on average), so the comparisons below hold pod count essentially constant — but the cost figures reflect a fleet already operating at its scale-out ceiling, and a higher HPA cap might absorb part of the tracing overhead as additional pods rather than per-pod CPU. With that caveat noted:
