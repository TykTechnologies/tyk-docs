...
<Note>
It is assumed that the APIs being protected with a rate limit are using the [auth token](/api-management/authentication/auth-token) client authentication method and policies have already been created.
</Note>
