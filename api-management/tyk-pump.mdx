...
Tyk Gateway writes a traffic log, a structured record of the request and response, for every API call it processes, into Redis. Redis is a temporary, high-throughput buffer, not a long-term store, so something has to move those traffic logs out before they expire.
...
- **Forwarding traffic logs, unaltered, to external systems** such as Splunk or Datadog. See [External Data Sinks](/api-management/traces/external-data-sinks).
...
<Note>
For the full field-by-field reference, environment variable names, and default values for every pump type, see the [Tyk Pump configuration reference](/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables). For the fields contained in a traffic log itself, see [Tyk Traffic Log Fields](/api-management/tyk-traffic-log-fields).
</Note>
...
`max_record_size` caps the size, in bytes, of the `RawRequest` and `RawResponse` fields written by a pump. Defaults to `0` (unlimited). Set it at the top level of `pump.conf` to apply a default to every pump:
...
When [detailed recording](/api-management/dashboard-analytics#detailed-recording) is configured in Tyk Gateway, the full request and response will be base64-encoded and stored in the [`raw_request`](/api-management/tyk-traffic-log-fields#rawrequest) and [`raw_response`](/api-management/tyk-traffic-log-fields#rawresponse) fields respectively in the traffic log.
...
| Type | Description |
| :-- | :-- |
| [`csv`](/api-management/traces/external-data-sinks#csv) | Writes traffic logs to local CSV files |
| [`elasticsearch`](/api-management/traces/external-data-sinks#elasticsearch) | Indexes traffic logs in Elasticsearch |
| [`graylog`](/api-management/traces/external-data-sinks#graylog) | Forwards traffic logs to Graylog |
| [`influx`](/api-management/traces/external-data-sinks#influxdb) | Writes traffic logs to InfluxDB v1 |
| [`influx2`](/api-management/traces/external-data-sinks#influx2) | Writes traffic logs to InfluxDB v2 |
| [`kafka`](/api-management/traces/external-data-sinks#kafka) | Publishes traffic logs to a Kafka topic |
| [`kinesis`](/api-management/traces/external-data-sinks#kinesis) | Publishes traffic logs to an Amazon Kinesis stream |
| [`logzio`](/api-management/traces/external-data-sinks#logzio) | Forwards traffic logs to Logz.io |
| [`moesif`](/api-management/traces/external-data-sinks#moesif) | Forwards traffic logs to Moesif |
| [`resurfaceio`](/api-management/traces/external-data-sinks#resurfaceio) | Forwards traffic logs to Resurface.io |
| [`segment`](/api-management/traces/external-data-sinks#segment) | Forwards traffic logs to Segment |
| [`splunk`](/api-management/traces/external-data-sinks#splunk) | Forwards traffic logs to Splunk |
| [`sqs`](/api-management/traces/external-data-sinks#sqs) | Publishes traffic logs to an Amazon SQS queue |
| [`stdout`](/api-management/traces/external-data-sinks#stdout) | Writes traffic logs to standard output, useful for container log collectors |
| [`syslog`](/api-management/traces/external-data-sinks#syslog) | Forwards traffic logs to a syslog server |
| [`timestream`](/api-management/traces/external-data-sinks#timestream) | Writes traffic logs to Amazon Timestream |
