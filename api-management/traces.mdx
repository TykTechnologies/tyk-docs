...
By default, spans are exported via gRPC to `localhost:4317`. Configure the protocol and endpoint using [`opentelemetry.traces.exporter`](/tyk-oss-gateway/configuration#opentelemetry-exporter) (`http` or `grpc`) and [`opentelemetry.traces.endpoint`](/tyk-oss-gateway/configuration#opentelemetry-endpoint).

TODO: Add a table here with all config options

<Note>
The root-level `opentelemetry.enabled`, `opentelemetry.exporter`, and `opentelemetry.endpoint` fields (and their environment variable equivalents `TYK_GW_OPENTELEMETRY_ENABLED`, `TYK_GW_OPENTELEMETRY_EXPORTER`, `TYK_GW_OPENTELEMETRY_ENDPOINT`) are deprecated. They continue to work for backward compatibility but should be replaced with the `opentelemetry.traces.*` equivalents in new deployments.
</Note>
...
|---------------------|---------|-------------|
| [MAXQUEUESIZE](/tyk-oss-gateway/configuration#opentelemetry-span_batch_config-max_queue_size) | `2048` | maximum number of spans buffered before export |
| [MAXEXPORTBATCHSIZE](/tyk-oss-gateway/configuration#opentelemetry-span_batch_config-max_export_batch_size) | `512` | maximum number of spans per export batch |
| [BATCHTIMEOUT](/tyk-oss-gateway/configuration#opentelemetry-span_batch_config-batch_timeout) | `5` | seconds to wait before forcing an export |
...
This setting allows you to specify the type of context propagator to use for trace data. It’s essential for ensuring compatibility and data integrity between different services in your architecture.
...
To customize, you can either set the [TYK_GW_OPENTELEMETRY_TRACES_SAMPLING_TYPE](/tyk-oss-gateway/configuration#opentelemetry-sampling-type) environment variable or modify the `opentelemetry.traces.sampling.type` field in the Tyk Gateway configuration file. Valid values for this setting are:

| Value | Behaviour |
|-------|-----------|
| `AlwaysOn` (default) | All traces are sampled |
| `AlwaysOff` | No traces are sampled |
| `TraceIDRatioBased` | Samples a fraction of traces based on `opentelemetry.traces.sampling.rate` |
