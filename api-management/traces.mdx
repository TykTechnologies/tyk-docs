...
If your upstream systems use a proprietary correlation header (for example, `X-Correlation-ID`), set `custom_trace_header` to that header name. The gateway will extract the trace ID from this header and propagate it as a standard `traceparent` header to upstream services.
...
<Note>
`custom_trace_header` is available from Tyk Gateway v5.12.0.
</Note>
