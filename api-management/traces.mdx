...
Since v5.2, Tyk Gateway has supported the [OpenTelemetry](/api-management/traces#opentelemetry) standard for distributed tracing. You can configure Tyk to work with an [OpenTelemetry collector](https://opentelemetry.io/docs/collector/) or integrate it with any [observability vendor supporting OpenTelemetry](https://opentelemetry.io/ecosystem/vendors/) to capture traces of API requests as they flow through Tyk Gateway and any upstream services.

Explore our guides for [Datadog](/api-management/traces#datadog), [Dynatrace](/api-management/traces#dynatrace), [Jaeger](/api-management/traces#using-docker) and [New Relic](/api-management/traces#new-relic) for further info on how to integrate with 3rd party observability vendors.

Tyk also supports the legacy [OpenTracing](/api-management/traces#opentracing-deprecated) approach (now deprecated), but we recommend users to adopt OpenTelemetry for a comprehensive, vendor-neutral technology with wide industry support.



## OpenTelemetry

Starting from Tyk Gateway version 5.2, you can leverage the power of [OpenTelemetry](https://opentelemetry.io/docs/what-is-opentelemetry/), an open-source observability framework designed for cloud-native software. This enhances your API monitoring with end-to-end distributed tracing. Tyk also supports [OpenTelemetry metrics export](/api-management/metrics/opentelemetry-configuration) from Tyk Gateway.
...
For further guidance on configuring your observability back-end, explore our guides for [Datadog](/api-management/traces#datadog), [Dynatrace](/api-management/traces#dynatrace), [Jaeger](/api-management/traces#jaeger) and [New Relic](/api-management/traces#new-relic).
...
OpenTelemetry tracing must be enabled at the Gateway level by adding the following to the Tyk Gateway configuration file (typically `tyk.conf`):
...
Alternatively, set the environment variable `TYK_GW_OPENTELEMETRY_TRACES_ENABLED=true`.

<Note>
By default, OpenTelemetry spans are exported to the collector using the `gRPC` protocol to `localhost:4317`. You can choose between HTTP and gRPC protocols by configuring the [opentelemetry.traces.exporter](/tyk-oss-gateway/configuration#opentelemetry-exporter) field to `http` or `grpc`. You can specify an alternative target using the [opentelemetry.traces.endpoint](/tyk-oss-gateway/configuration#opentelemetry-endpoint) field.
</Note>

<Note>
The root-level `opentelemetry.enabled`, `opentelemetry.exporter`, and `opentelemetry.endpoint` fields and their corresponding `TYK_GW_OPENTELEMETRY_ENABLED`, `TYK_GW_OPENTELEMETRY_EXPORTER`, `TYK_GW_OPENTELEMETRY_ENDPOINT` environment variables are deprecated. They continue to work for backward compatibility but should be replaced with the `opentelemetry.traces.*` equivalents in new deployments.
</Note>
...
- **Environment Variable**: Use `TYK_GW_OPENTELEMETRY_TRACES_CONTEXTPROPAGATION` to specify the context propagator type.
- **Configuration File**: Navigate to the `opentelemetry.traces.context_propagation` field in your configuration file to set your preferred option.
...
To customize, you can either set the `TYK_GW_OPENTELEMETRY_TRACES_SAMPLING_TYPE` environment variable or modify the `opentelemetry.traces.sampling.type` field in the Tyk Gateway configuration file. Valid values for this setting are:
...
- **Environment Variable**: Use `TYK_GW_OPENTELEMETRY_TRACES_SAMPLING_RATE`.
...
- **Environment Variable**: Use `TYK_GW_OPENTELEMETRY_TRACES_SAMPLING_PARENTBASED`.
...
1. **Configure Tyk API Gateway**
    ...
    * In your docker-compose.yml file for Tyk Gateway, add the following environment variables:
    ...
    After enabling OpenTelemetry at the Gateway level, you can activate [detailed tracing](/api-management/traces#opentelemetry) for specific APIs by editing their respective API definitions. Set the `detailed_tracing` option to either true or false. By default, this setting is false.
...
We introduced support for [OpenTelemetry](/api-management/traces#opentelemetry) in Tyk v5.2. We recommend that users migrate to OpenTelemetry for better support of your tracing needs.
...
- [Jaeger](/api-management/traces#jaeger-1)
- [Zipkin](/api-management/traces#zipkin)
- [New Relic](/api-management/traces#new-relic-1)
...
<Note>
Tyk's OpenTelemetry Tracing works with Jaeger and we recommend following our guide to [use OpenTelemetry with Jaeger](/api-management/traces#jaeger) rather than the following deprecated Open Tracing method.
</Note>
...
<Note>
Tyk's OpenTelemetry Tracing works with New Relic and we recommend following our guide to [use OpenTelemetry with New Relic](/api-management/traces#new-relic) rather than the following deprecated Open Tracing method.
</Note>
