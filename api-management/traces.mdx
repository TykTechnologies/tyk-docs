...
Since v5.2, Tyk Gateway has supported the [OpenTelemetry](/api-management/logs-metrics) standard for distributed tracing. You can configure Tyk to work with an [OpenTelemetry collector](https://opentelemetry.io/docs/collector/) or integrate it with any [observability vendor supporting OpenTelemetry](https://opentelemetry.io/ecosystem/vendors/) to capture traces of API requests as they flow through Tyk Gateway and any upstream services.

Explore our guides for [Datadog](/api-management/logs-metrics), [Dynatrace](/api-management/logs-metrics), [Jaeger](/api-management/logs-metrics) and [New Relic](/api-management/logs-metrics) for further info on how to integrate with 3rd party observability vendors.

Tyk also supports the legacy [OpenTracing](/api-management/logs-metrics) approach (now deprecated), but we recommend users to adopt OpenTelemetry for a comprehensive, vendor-neutral technology with wide industry support.
...
For further guidance on configuring your observability back-end, explore our guides for [Datadog](/api-management/logs-metrics), [Dynatrace](/api-management/logs-metrics), [Jaeger](/api-management/logs-metrics) and [New Relic](/api-management/logs-metrics).
...
1. **Configure Tyk API Gateway**
    ...
    * In your docker-compose.yml file for Tyk Gateway, add the following environment variables:
    ...
    After enabling OpenTelemetry at the Gateway level, you can activate [detailed tracing](/api-management/logs-metrics) for specific APIs by editing their respective API definitions. Set the `detailed_tracing` option to either true or false. By default, this setting is false.
...
We introduced support for [OpenTelemetry](/api-management/logs-metrics) in Tyk v5.2. We recommend that users migrate to OpenTelemetry for better support of your tracing needs.
...
- [Jaeger](/api-management/logs-metrics)
- [Zipkin](/api-management/logs-metrics)
- [New Relic](/api-management/logs-metrics)
...
<Note>
Tyk's OpenTelemetry Tracing works with Jaeger and we recommend following our guide to [use OpenTelemetry with Jaeger](/api-management/logs-metrics) rather than the following deprecated Open Tracing method.
</Note>
...
<Note>
Tyk's OpenTelemetry Tracing works with New Relic and we recommend following our guide to [use OpenTelemetry with New Relic](/api-management/logs-metrics) rather than the following deprecated Open Tracing method.
</Note>
