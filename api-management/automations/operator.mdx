...
**TykOasApiDefinition**: Available from Tyk Operator v1.0. It represents a [Tyk OAS API configuration](/api-management/gateway-config-tyk-oas#tyk-oas-api-definition-object). Tyk OAS API is based on the OpenAPI specification (OAS) and is the recommended format for standard HTTP APIs. Tyk Operator supports all [Tyk OAS API feature](/api-management/gateway-config-tyk-oas#tyk-oas-api-feature-status) as they become available on the Gateway.
...
**TykStreamsApiDefinition**: Available from Tyk Operator v1.1. It represents an [Async API configuration](/api-management/event-driven-apis#configuration-as-code) which is based on [Tyk OAS API Definition](/api-management/gateway-config-tyk-oas#tyk-oas-api-definition-object). Tyk Operator supports all [Tyk Streams](/api-management/event-driven-apis#) and [Tyk OAS API feature](/api-management/gateway-config-tyk-oas#tyk-oas-api-feature-status) as they become available on the Gateway.
...
| :-------------------- | :------------- | :----------- | :--------------------------------------------------------------------------------------------------- |
| TykOasApiDefinition| tyk.tyk.io  | v1alpha1  | Defines configuration of [Tyk OAS API Definition object](/api-management/gateway-config-tyk-oas#tyk-oas-api-definition-object)                                 |
...
| TykStreamsApiDefinition| tyk.tyk.io  | v1alpha1  | Defines configuration of [Tyk Streams](/api-management/event-driven-apis#configuration-as-code)                                 |
...
| Public keys pinning | ✅ [Certificate pinning](/api-management/certificates#certificate-pinning) | ✅ [Certificate pinning](/api-management/certificates#certificate-pinning) | Certificate ID can be set in the API Definition but configuring certificates from Secrets in CRD is not supported. |
| Upstream mTLS | ✅ [Upstream mTLS via Operator](/api-management/client-authentication#via-tyk-operator-using-the-tyk-classic-api-definition) | ✅ [Upstream mTLS via Operator](/api-management/client-authentication#tyk-operator-oas) | Certificate ID can be set in the API Definition but configuring certificates from Secrets in CRD is not supported. |
...
You can configure your API for any Tyk supported authentication method by following [Authentication with Tyk OAS](/api-management/gateway-config-tyk-oas#authentication-with-tyk-oas) documentation.
...
This setup requires a [bearer token](/api-management/client-authentication#use-auth-tokens) for access.
...
- `validate_signature`: Enable [signature validation](/api-management/client-authentication#auth-token-signature-validation).
...
This configuration uses [Basic Authentication](/api-management/client-authentication#use-basic-authentication), requiring a username and password for access.
...
To enable multiple (chained) auth, you should set `base_identity_provided_by` field to one of the supported chained enums. Consult [Enable Multi (Chained) Authentication in your API Definition](/api-management/client-authentication#enable-multi-chained-authentication-in-your-api-definition) for the supported auths.
...
To enable [IP Allowlist](/api-management/gateway-config-tyk-classic#ip-allowlist-middleware), set the following fields:
...
To enable [IP Blocklist](/api-management/gateway-config-tyk-classic#ip-blocklist-middleware), set the following fields:
...
API name can be set through `x-tyk-api-gateway.info.name` field in [Tyk OAS API Definition](/api-management/gateway-config-tyk-oas#tyk-oas-api-definition-object) object.
...
API active state can be set through `x-tyk-api-gateway.info.state.active` field in [Tyk OAS API Definition](/api-management/gateway-config-tyk-oas#tyk-oas-api-definition-object) object.
...
API accessibility can be set through `x-tyk-api-gateway.info.state.internal` field in [Tyk OAS API Definition](/api-management/gateway-config-tyk-oas#tyk-oas-api-definition-object) object.
...
If you already have API configurations created in the Tyk Dashboard and want to start using Tyk Operator to manage these APIs, you can include the existing API ID in the manifest under the `x-tyk-api-gateway.info.id` field in [Tyk OAS API Definition](/api-management/gateway-config-tyk-oas#tyk-oas-api-definition-object) object.
...
`versioning` is configured at `order-api` (v1), the Base API, and it has similiar structure as [Tyk OAS API Definition](/api-management/api-versioning#configuring-api-versioning-in-the-tyk-oas-api-definition):
...
For Tyk Classic API, versioning can be configured via `ApiDefinition` custom resource definition (CRD). See [Tyk Classic versioning](/api-management/api-versioning#tyk-classic-api-versioning-1) for a comprehensive example of configuring API versioning for Tyk Classic API with Tyk Operator.
...
The TykStreamsApiDefinition Custom Resource Definition (CRD) manages [Async API configuration](/api-management/event-driven-apis#configuration-as-code) within a Kubernetes environment.

##### TykStreamsApiDefinition Features

`TykStreamsApiDefinition` can support all features of [Tyk Streams](/api-management/event-driven-apis#) and [Tyk OAS API](/api-management/gateway-config-tyk-oas#tyk-oas-api-feature-status). You just need to provide the Tyk Streams API definition via a ConfigMap. In addition to managing the CRUD (Create, Read, Update, Delete) of Tyk Streams API resources, the Tyk Operator helps you better manage resources through object linking to Security Policies. See below for a list of Operator features and examples:
