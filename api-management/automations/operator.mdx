...
Tyk Dashboard is multi-tenant capable, which means you can use a single Tyk Dashboard instance to host separate [organizations](/platform-management/dashboard-admin-api#organizations) for each team or department. Each organization is a completely isolated unit with its own:
...
| API Ownership via OperatorContext | ✅      | v1.0 | - | [API Ownership](/platform-management/api-ownership#when-to-use-api-ownership) |
