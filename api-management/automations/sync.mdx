...
Starting with Sync v1.5+ and Dashboard v5.3.2+, Tyk Sync supports both [Tyk OAS APIs](/api-management/gateway-config-managing-oas#) and [Tyk Classic APIs](/api-management/gateway-config-introduction#api-definition-types) when working with the Tyk Dashboard, without requiring special flags or configurations.
