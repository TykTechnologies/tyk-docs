...
The Ignore Authentication middleware instructs Tyk Gateway to skip a major part of the middleware processing chain for calls to an endpoint, even if authentication is enabled for the API. This is not limited to just authentication, but also includes authorization and other security checks.
