...
When using the ignore authentication middleware, we recommend that you familiarize yourself with [Request Matching](/getting-started/key-concepts/url-matching) options.
...
- `method_actions`: a shared object used to configure the [mock response](/api-management/traffic-transformation/mock-response#tyk-classic) middleware for Tyk Classic APIs
