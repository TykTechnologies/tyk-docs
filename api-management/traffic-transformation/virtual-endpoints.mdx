...
The virtual endpoint middleware provides a JavaScript engine that runs the custom code that you provide either inline within the API definition or in a source code file accessible to Tyk Gateway. From v5.14, Tyk Gateway runs JavaScript on the [goja engine](/api-management/plugins/javascript#the-goja-engine), a pure-Go ECMAScript engine whose guaranteed language level is ECMAScript 5.1 (with much of ES6 and later available rather than guaranteed). Earlier releases ran JavaScript on the legacy otto engine, which is limited to ES5.1 and is retained for backward compatibility. The goja engine is not Node.js: there is no `require`/`import`, no `async`/`await` and no Node standard library.

The engine used for a virtual endpoint follows the [plugin driver](/api-management/plugins/javascript#selecting-the-engine) set on its API: a virtual endpoint runs on goja only when the API driver is `javascript`. With no driver set, virtual endpoints default to the legacy otto engine, so set the driver to `javascript` explicitly to run them on goja. In Tyk OAS the driver is `x-tyk-api-gateway.middleware.global.pluginConfig.driver`; in a Tyk Classic API definition it is `custom_middleware.driver`.
...
The [middleware scripting guide](/api-management/plugins/javascript#the-plugin-contract) provides guidance on writing JS functions for your virtual endpoints, including how to access key session data and custom attributes from the API definition.
...
In this example, we demonstrate how you can access different [external Tyk objects](/api-management/plugins/javascript#the-request-object) (API request, session key, API definition).
...
You can add [custom attributes](/api-management/plugins/javascript#the-config-object) to the API definition and access these from within your Virtual Endpoint.
...
One of the most common use cases for virtual endpoints is to provide some form of aggregate data to your users, combining the responses from multiple upstream service calls. This virtual endpoint function will do just that using the batch processing function from the [JavaScript API](/api-management/plugins/javascript#the-javascript-api)
