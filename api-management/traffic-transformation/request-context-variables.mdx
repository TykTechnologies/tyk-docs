...
<Note>
Context variables must be enabled for your API to access them in transformation middleware.

- In **Tyk OAS APIs**, context variables default to enabled when creating or importing an API, but can be enabled or disabled under global middleware settings (`x-tyk-api-gateway.middleware.global.contextVariables.enabled`). If disabled or omitted in custom API definitions, context variables are not populated for downstream middleware.
...
See [Enabling Context Variables](#enabling-context-variables) for configuration instructions.
</Note>
...
Context variables are exposed in three middleware plugins but are accessed differently depending on the caller:
...
In Tyk OAS API definitions, you can configure these context-aware middlewares at:

- **Global level**: Under `x-tyk-api-gateway.middleware.global` (such as `transformHeaders`, `transformResponseHeaders`, or `urlRewrite`) to apply across all endpoints in the API.
- **Operation level**: Under `x-tyk-api-gateway.middleware.operations.<operationId>` (such as `transformHeaders`, `transformResponseHeaders`, `transform`, `transformResponse`, or `urlRewrite`) to apply to a specific endpoint operation.

If using Tyk Classic API definitions, configure these middlewares under `version_data.versions.<version_name>.extended_paths` or `global_headers` / `global_response_headers`.
