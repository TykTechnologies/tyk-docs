...
In Tyk OAS APIs, context variables are enabled by default when creating or importing an API.

Context-aware middleware sits under `x-tyk-api-gateway.middleware`:

- **Global level** (`x-tyk-api-gateway.middleware.global`): Enable or disable context variables (`contextVariables.enabled`), and apply middleware (such as `transformRequestHeaders` or `transformResponseHeaders`) across all endpoints in the API.
- **Operation level** (`x-tyk-api-gateway.middleware.operations.<operationId>`): Apply middleware (such as `transformRequestHeaders`, `transformResponseHeaders`, `transformRequestBody`, `transformResponseBody`, or `urlRewrite`) to a specific endpoint operation.
