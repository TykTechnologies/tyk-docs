...
In Tyk OAS APIs, context variables default to enabled when creating or importing an API. You can enable or disable them under global middleware settings (`x-tyk-api-gateway.middleware.global.contextVariables.enabled`).

In Tyk OAS API definitions, you can configure context-aware middleware at:
...
Configure context-aware middleware under `version_data.versions.<version_name>.extended_paths` or `global_headers` / `global_response_headers`.
