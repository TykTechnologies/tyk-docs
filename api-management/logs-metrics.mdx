...
This analytics pipeline uses [Tyk Pump](/api-management/tyk-pump) or [Tyk MDCB](/api-management/mdcb) to aggregate [traffic logs](/api-management/logs-metrics#api-traffic-logs) from Tyk Gateway and store them in the Dashboard's [analytics storage](/api-management/dashboard-configuration#data-storage-solutions). You can also use Tyk Pump to export analytics to [external backends](/api-management/tyk-pump#external-data-stores) such as Elasticsearch, InfluxDB, and Kafka.
...
| [`opentelemetry.metrics.endpoint`](/tyk-oss-gateway/configuration#opentelemetry-metrics-endpoint) | OTLP collector endpoint. Defaults to `opentelemetry.traces.endpoint` if not set | — |
...
| [`opentelemetry.metrics.headers`](/tyk-oss-gateway/configuration#opentelemetry-metrics-headers) | Additional HTTP headers sent with each OTLP export request | — |
| [`opentelemetry.metrics.tls`](/tyk-oss-gateway/configuration#opentelemetry-metrics-tls) | TLS configuration for the OTLP connection | — |
...
| [`opentelemetry.metrics.export_timeout`](/tyk-oss-gateway/configuration#opentelemetry-metrics-export_timeout) | Maximum time allowed for a single export request, in seconds | `30` |
...
| [`opentelemetry.metrics.api_metrics`](/tyk-oss-gateway/configuration#opentelemetry-metrics-api_metrics) | Custom metric instrument definitions. See [Custom Metrics](/api-management/metrics/custom-metrics) | — |
