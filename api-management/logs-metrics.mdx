...
| [New Relic Instrumentation](#new-relic-instrumentation) | Tyk Gateway | [Deprecated](/developer-support/deprecation#understanding-deprecation-and-end-of-life-eol) | Legacy New Relic agent integration available since Gateway v2.5. |
| [StatsD Instrumentation](#statsd-instrumentation) | Tyk Gateway, Pump, Dashboard | [Deprecated](/developer-support/deprecation#understanding-deprecation-and-end-of-life-eol) | Legacy StatsD protocol export for Gateway, Pump, and Dashboard. |
...
[Tyk Pump](/api-management/tyk-pump) can expose metrics derived from the traffic logs that the Gateway stores in Redis, the same traffic logs used for [Dashboard Analytics](/api-management/dashboard-analytics). This is an older, separate mechanism: computed by Tyk Pump after the fact from traffic log records, not by Tyk Gateway in real time, so it can only produce traffic-derived counters and histograms (status codes, latency, and whatever traffic log fields you tag). It has no visibility into Gateway's own runtime health or configuration state.
