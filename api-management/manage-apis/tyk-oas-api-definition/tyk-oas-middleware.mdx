...
<Note>
Tyk OAS API support is currently in [Early Access](/developer-support/special-releases-and-features/early-access-features) and some Tyk features are not yet supported. You can see the status of what is and isn't yet supported [here](/getting-started/using-oas-definitions/oas-reference.md).
</Note>
...
For example, given the base Tyk OAS API used [here](/getting-started/using-oas-definitions/update-an-oas-api#tutorial-3-update-tyk-oas-api-definition-with-an-updated-openapi-definition) we can enable the `allowList` middleware with this `PATCH` command:
...
The OpenAPI Document that you used in the example [above](#validateRequest-check-that-the-API-request-payload-meets-a-defined-schema), we also defined a JSON schema that describes the response format for any request that hits the `GET /pet/{petId}` path.
