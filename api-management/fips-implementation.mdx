...
> If your organisation requires a certified base image, supply chain attestations, or STIG compliance, you must use Docker Hardened Images.

Version 5.8.x (LTS-1) is available as RPM/DEB packages only, targeting FIPS 140-2. See the [FIPS Policy](/docs/developer-support/release-types/fips-release) page for a full breakdown of compliance posture by version.
...
The override follows the same pattern for each component: set the image repository to the `-fips` suffixed equivalent and pin the tag to your target version. Full per-component key paths are documented in the [Tyk Helm chart reference](https://tyk.io/docs/product-stack/tyk-charts/overview/).
...
To perform signature and attestation verification, you will need the Tyk signing identity and OIDC issuer values. These are available from your account manager or via [Tyk support](https://support.tyk.io/). Once you have those values, verification follows the standard `cosign verify` workflow for image signatures, SBOM attestations, and build provenance.
...
Without VEX filtering applied, scanners will report CVEs that are present in the image but not exploitable in this context. This is expected behaviour, not a signal that the image is insecure. We strongly recommend configuring VEX filtering before running compliance scans.
...
Tyk's FIPS compliance attestation covers the Tyk components listed on the [FIPS Policy](/docs/developer-support/release-types/fips-release) page when deployed and configured as described in this documentation. It does not extend to:
