...
This page covers the [Tyk Pump types](/api-management/tyk-pump#pump-type-catalog) that write traffic logs and aggregated analytics into Tyk Dashboard's [persistent database](/api-management/persistent-database): the pumps that ultimately feed Tyk Dashboard's Log Browser and Traffic Analytics screens.
...
For Tyk Dashboard's Traffic Analytics screens (Traffic Analytics graphs, Activity by Graph, Activity by MCP), data is written to the [Aggregate Analytics](/api-management/persistent-database#aggregate-analytics) category in the database using:
...
For Tyk Dashboard's Log Browser, data is written to the [Logs](/api-management/persistent-database#traffic-logs) category in the database using:
...
Enable [detailed recording](/api-management/logs/traffic-logs#detailed-recording) first, so that GraphQL information can be parsed from the request body and response; this applies globally across all APIs on the Gateway.
...
<Note>
Tyk no longer supports SQLite as of Tyk 5.7.0. Transition to [PostgreSQL](/tyk-configuration-reference/sql), [MongoDB](/tyk-configuration-reference/mongodb), or a listed compatible alternative.
</Note>
...
| `table_sharding` | `false` | If `true`, records are stored in one table per day instead of a single table. See [Table Sharding](/tyk-configuration-reference/sql#table-sharding). |
...
| `migrate_sharded_tables` | `false` | See [Table Sharding](/tyk-configuration-reference/sql#table-sharding). |
...
Accepts the same `track_all_paths`, `ignore_tag_prefix_list`, `store_analytics_per_minute`, and `omit_index_creation` fields as the [SQL Aggregate Pump](#sql-aggregate-pump) above. The table name is fixed at `tyk_mcp_aggregated`; there's no `table_name` override, though [table sharding](/tyk-configuration-reference/sql#table-sharding) still applies, storing per-day tables such as `tyk_mcp_aggregated_20230327`.
