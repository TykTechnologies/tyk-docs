...
As a guideline, every 3 million requests generates roughly 1GB of traffic log data. [Detailed recording](/api-management/dashboard-analytics#detailed-recording), which captures the full request and response body on every record, multiplies this considerably, and is the single biggest driver of storage growth where it's enabled.
...
You must ensure that the equivalent `table_sharding` configuration for each relevant section of Tyk Dashboard's `storage` configuration (`main`, `analytics`, `logs`) matches.
