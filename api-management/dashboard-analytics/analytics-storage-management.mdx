...
Tyk Pump writes two different kinds of data to the Control Plane's [persistent database](/api-management/persistent-database), from which it's used by Tyk Dashboard's Traffic Analytics and Log Browser, and they behave very differently when it comes to storage growth.
...
Whether Tyk creates one automatically for you depends on the pump type. The [Aggregate pump](/api-management/dashboard-analytics/control-plane-pumps#mongo-aggregate-pump) and the [Per-Organisation Mongo Pump](/api-management/dashboard-analytics/control-plane-pumps#per-organisation-mongo-pump) both create one automatically on any brand-new collection, the same way they create their other indexes. The [Standard Mongo Pump](/api-management/dashboard-analytics/control-plane-pumps#standard-mongo-pump) doesn't create one automatically. You must create it manually on `tyk_analytics`.
