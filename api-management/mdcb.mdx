...
For deployments with many or large API definitions, you can enable compression to reduce Redis memory usage. Set [`storage.compress_api_definitions`](/tyk-oss-gateway/configuration#storagecompress_api_definitions) to `true` in your worker Gateway configuration. This uses Zstandard compression for API definition backups stored in Redis, which can significantly reduce memory requirements while maintaining full compatibility with existing uncompressed data.
...
To reduce Redis memory usage for API definition backups, you can optionally enable compression:
...
This uses Zstandard compression for storing API definitions and is recommended for deployments with many or large APIs. See [`storage.compress_api_definitions`](/tyk-oss-gateway/configuration#storagecompress_api_definitions) for details.
