...
#### Force updating the entitlement plan

When changing an organisation's entitlement plan, the system validates that the new plan is compatible with the resources currently in use. If the validation fails (for example, because the organisation is consuming more resources than the new plan allows), the update will be rejected.

To bypass this validation and force the entitlement plan change, add the `force_update=true` query parameter to the PUT request:

```curl
curl -X PUT "$DASH_URL/admin/organisations/$ORG_ID?force_update=true" \
  -H "Admin-Auth: $DASH_ADMIN_SECRET" \
  -d @myorg.json
```

<Warning>
Use `force_update=true` with caution. Bypassing the entitlement validation may result in the organisation using resources that exceed the limits of the new plan.
</Warning>
