...
You can [create a user](/platform-management/dashboard-api#add-user) with a call to the `POST /api/users` endpoint, for example:
...
You can create your own custom permissions for use with the [Open Policy Agent (OPA)](/platform-management/open-policy-agent#extend-permissions-using-open-policy-agent-opa) using the [Additional Permissions](/platform-management/dashboard-api#additional-permissions-api) endpoint in the Tyk Dashboard Admin API. This allows you to add and delete (CRUD) a list of additional (custom) permissions for your Dashboard users. Once created, a custom permission will be added to standard list of user permissions. 
...
Currently, only [API Usage](/api-management/dashboard-analytics#activity-by-api) and [Error Counts](/api-management/dashboard-analytics#activity-by-error) are available to users with the `owned_analytics` permission.
...
If you have deployed multiple [Tyk Organizations](/platform-management/dashboard-admin-api#organizations), you may have users that need access to more than one Organization (known as a "multi-org user"). **This functionality requires a specific Tyk license.**
...
You can integrate your existing identity management server with the Tyk Dashboard, as explained in our detailed [Single Sign-On (SSO) guide](/api-management/external-service-integration#single-sign-on-sso). **This functionality is available with all Tyk licenses.**
