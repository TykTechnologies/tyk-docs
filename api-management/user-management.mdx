...
By default all users who login via SSO are granted admin permissions. You can change this behavior by setting either default permissions for *[users](/platform-management/dashboard-users#manage-tyk-dashboard-users)* or by creating a default *[user group](/platform-management/user-groups)* to which all new users are assigned. With some IDPs you can automatically assign different SSO users to different *user groups* by dynamically mapping the IDP's user groups, for example with [Azure AD](/api-management/single-sign-on-oidc#user-group-mapping).
