...
description: "Overview of Tyk's native OpenTelemetry support for distributed tracing, metrics export, and log collection. The recommended observability approach for Tyk Gateway."
...
flowchart LR
    ...
    GW -->|"Logs (stdout)"| OC["OTel Collector"]
    OC --> LC["Log backend\nElasticsearch · Loki"]
```
...
|:----------|:--------------|:------------|
| `tyk.gw.id` / `service.instance.id` | Yes | Unique node ID for this gateway instance. |
| `tyk.gw.dataplane` | Yes | Whether the gateway is running as an edge (dataplane) node. |
| `tyk.gw.group.id` | No | Edge group ID. Populated only for hybrid/dataplane gateways. |
| `tyk.gw.tags` | No | Segment tags. Populated only when the gateway is segmented. |
...
- **[Best Practices](/api-management/observability-best-practices):** Production guidance on export topology, cardinality control, trace sampling, and log collection.
- **[Gateway Observability Playbook](/api-management/gateway-observability-playbook):** Diagnosing common failures using RED metrics, response flags, and PromQL alert rules.
