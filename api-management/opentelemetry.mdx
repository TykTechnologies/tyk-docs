...
Traces and metrics both carry the same [resource attributes](#resource-attributes) set at gateway startup. The key correlating attribute is `tyk.gw.id` (also exported as `service.instance.id`), which uniquely identifies the gateway node that produced each signal.

When running multiple gateway replicas, this lets you filter metrics to a specific node using `tyk.gw.id` and find that same node's traces using the same attribute.
