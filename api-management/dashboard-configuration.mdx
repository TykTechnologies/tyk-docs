...
##### Validate Rules Before Switching
OPA governs access to the Tyk Dashboard API, so a faulty ruleset can lock users out. Validate `dashboard.rego` with the OPA CLI before relying on it. Add stubs for the Tyk-injected `TykAPIGet` and `TykDiff` functions, which exist only at runtime:

```bash
opa check dashboard.rego        # compiles?
opa eval -d dashboard.rego -i input.json 'data.dashboard_users.allow'   # admin -> true?
```

<Warning>
**Recovery if a ruleset locks you out:** disable OPA at the config layer and restart, which bypasses both sources:

```json
"security": { "open_policy": { "enabled": false } }
```

The default ruleset's `is_admin` rule also keeps admin users from being locked out in most cases.
</Warning>
