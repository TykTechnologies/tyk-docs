...
import OpaRules from '/snippets/5.14/opa-rules.mdx';
...
- [Detailed logging](/api-management/logs/traffic-logs#detailed-recording)
...
<Note>
For the Tyk Dashboard's analytics functionality to work, you must configure both per-request and aggregated pumps for the database platform that you are using. For more details see the [Control Plane Pumps](/api-management/dashboard-analytics/control-plane-pumps#choosing-a-pump-type) section.
</Note>
...
Not every endpoint necessarily appears here: see [Controlling Which Endpoints Are Tracked](/api-management/dashboard-analytics#controlling-which-endpoints-are-tracked) for what controls that.
...
For more information, see [MCP Analytics](/ai-management/mcp-gateway/mcp-analytics).
...
- you save rules through the [OPA API](/api-management/dashboard-configuration#open-policy-agent-api) (`PUT /api/org/opa`) or the Tyk Dashboard UI.

To check which rules are currently stored at the organization level, use the [List OPA rules and settings](/api-management/dashboard-configuration#list-opa-rules-and-settings) endpoint.
