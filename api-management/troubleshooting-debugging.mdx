...
20. ##### GetSpareNode returned node not allowed by this dashboard's license

    **Description**

    Gateways fail to register with the Tyk Dashboard, and the Dashboard logs the following error:

    ```
    level=error msg="GetSpareNode returned node not allowed by this dashboard's license"
    ```

    The Dashboard returns an HTTP 404 response ("No valid nodes available for this license") to the gateway. Gateways that cannot register cause 404 responses on the Gateway and prevent API creation.

    **Cause**

    More than one Tyk Dashboard instance is connected to the same Redis and database backend while running different license keys. A Dashboard license defines the pool of gateway nodes that the Dashboard is allowed to register, and this node information is shared through Redis. When the licenses differ, a Dashboard can be assigned a node from Redis that belongs to another license and then rejects it.

    This commonly happens during a rolling or blue-green upgrade, or while updating a license, when old and new Dashboard instances run side by side with different keys.

    **Solution**

    Ensure that every Tyk Dashboard instance connected to the same Redis and database backend uses the same license key. When you update a license, apply the new key to all Dashboard instances at the same time. For upgrade guidance, see the [Upgrading Guide](/developer-support/upgrading#upgrade-strategies) and [Connecting Multiple Gateways to a Single Dashboard](/planning-for-production#connecting-multiple-gateways-to-a-single-dashboard).
...
1. ##### Capturing detailed logs
    ...
    **What is detailed request logging?**
    ...
    When [detailed request logging](/api-management/tyk-pump#configure-detailed-recording) is enabled, Tyk will record the request and response in wire-format in the analytics database. This can be very useful when trying to debug API requests to see what went wrong for a user or client.

    This mode is configured in the gateway and can be enabled at the [system](/api-management/tyk-pump#gateway-level), [API](/api-management/tyk-pump#api-level) or [access key](/api-management/tyk-pump#key-level) level.
