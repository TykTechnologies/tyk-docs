...
<Note>
Error messages like `Wrong Password`, `Empty Signature Header`, `Empty Signature Path`, `Invalid Signature Header`, and `Invalid Signature Path` are dynamically constructed or handled by specific plugins, and might not be exact static strings.
</Note>
