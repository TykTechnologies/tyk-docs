...
1. ##### Capturing detailed logs
    ...
    **What is detailed request logging?**
    ...
    When [detailed request logging](/api-management/dashboard-analytics#configure-detailed-recording) is enabled, Tyk will record the request and response in wire-format in the analytics database. This can be very useful when trying to debug API requests to see what went wrong for a user or client.

    This mode is configured in the gateway and can be enabled at the [system](/api-management/dashboard-analytics#gateway-level), [API](/api-management/dashboard-analytics#api-level) or [access key](/api-management/dashboard-analytics#key-level) level.
