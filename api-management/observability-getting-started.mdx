...
- Distributed traces exporting to Tempo with trace-to-log correlation
...
- [Docker](https://docs.docker.com/get-docker/) and [Docker Compose](https://docs.docker.com/compose/install/) v2 installed
- A running Tyk Self-Managed deployment (or a new one, see [Docker installation](/tyk-self-managed/install/docker))
...
```yaml expandable
receivers:
  # Receives traces and metrics pushed from Tyk Gateway
  otlp:
    protocols:
      grpc:
  ...
  # Reads Tyk Gateway logs from the Docker container log path
  filelog/tyk-gateway:
    ...
    operators:
      ...
      # Parse inner Tyk JSON log payload and extract timestamp and severity
      - type: json_parser
        parse_from: body
...
processors:
  ...
  # Promote gateway resource attributes into metric datapoint labels
  # so you can filter by gateway instance in Grafana
...
The `tracesToLogsV2` block wires up trace-to-log correlation in Grafana: clicking a trace ID in Tempo will automatically query Loki for matching log lines.
...
Send a few test requests through Tyk Gateway, then open Grafana at **http://localhost:3001** and verify:

**Metrics**: in Explore, select the Prometheus datasource and run:
```promql
tyk_http_requests_total
```
You should see request counts labelled by `api_id`, `response_code`, and `service_name`.
...
**Traces**: in Explore, select the Tempo datasource and search for recent traces. Each trace should show two spans: a parent span for the full request lifecycle and a child span for the upstream call.
