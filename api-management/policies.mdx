...
* `mtls_static_certificate_bindings`: Only applicable to [Auth Token authentication](/api-management/authentication/auth-token#client-certificate-token-binding), from **Tyk 5.12** onward this is a list of certificate IDs for client certificates that are bound to the session object, one of which must be presented with the token. 
