...
<Note>
**Note: Policy ID restrictions**

From 5.13. 0, when creating a policy with a custom ID, the ID can only contain the following characters: `a-z`, `A-Z`, `0-9`, `.`, `_`, `-`, `~`. If you need to bypass this restriction, you can set the `allow_unsafe_policy_ids` configuration flag to `true` in your Gateway and Dashboard configuration.
</Note>
...
[With the Dashboard](/api-management/gateway-config-managing-classic#tyk-cloud-1)
