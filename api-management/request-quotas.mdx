...
In this tutorial, we will configure Request Quotas on a [Policy](/api-management/access-control/policies/understanding-policies) to limit the number of requests an API key can make over a defined period. Unlike [rate limits](/api-management/rate-limit) (requests per second), quotas control overall volume. We'll set a low quota limit with a short renewal period for easy testing, associate a key with the policy, observe blocked requests once the quota is exhausted, and verify that the quota resets after the period elapses. This guide primarily uses the Tyk Dashboard for configuration.
...
The Tyk Dashboard provides a straightforward interface to set request quota parameters using [Policies](/api-management/access-control/policies/understanding-policies) or directly in the [Session](/api-management/access-control/sessions-and-keys/understanding-sessions) when creating a Key.
...
*   **Policy Precedence:** Quotas set on a [Policy](/api-management/access-control/policies/understanding-policies) apply to all keys using that policy *unless* overridden by a specific quota set directly on the key (using the "Set per API Limits and Quota" option).
