...
<Note>
Debug log level generates a significant volume of data and is not recommended except when debugging.
</Note>
...
Access logs are simple, traditional server logs that record basic information about each request to your API Gateway, written directly to stdout/stderr.

As of Tyk Gateway v5.8.0, you can configure the Gateway to log individual API request. To enable this feature, set the `TYK_GW_ACCESSLOGS_ENABLED` environment variable to `true`.
...
<Note>
API Traffic logs are not written to stdout/stderr like application logs and access logs. Instead, they are stored in Redis and then processed and forwarded by Tyk Pump to the configured data sinks, such as databases or other external tools.
</Note>
...
You can configure Tyk Gateway to capture request and response payloads in API traffic log when required. Tyk refers to this capability as **detailed recording**. 
