...
Access logs automatically include structured error classification fields when requests result in errors. These fields help operators quickly identify the root cause of failures — such as TLS certificate expiry, connection refused, rate limiting, or authentication errors — directly from the access log, without needing to cross-reference application logs or API Traffic Logs.
