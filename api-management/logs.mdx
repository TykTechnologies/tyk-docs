...
<Note>
`trace_id` and `span_id` enrichment in application logs is available from Tyk Gateway v5.12.0.
</Note>
...
<ParamField path="trace_id">
  The OpenTelemetry trace ID for the request (32-character hex W3C trace ID). Only present when OpenTelemetry is enabled and a trace ID is available. Use this to navigate from an access log entry to the corresponding trace in your observability backend (for example, Grafana Tempo or Jaeger).

  <Note>
  Available from Tyk Gateway v5.12.0. Requires OpenTelemetry to be enabled.
  </Note>
</ParamField>
