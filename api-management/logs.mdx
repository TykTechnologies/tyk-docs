...
Access logs automatically include structured error classification fields when requests result in errors. These fields help operators quickly identify the root cause of failures — such as TLS certificate expiry, connection refused, rate limiting, or authentication errors — directly from the access log, without needing to cross-reference system logs or analytics records.

Error classification fields are **only present when errors occur**. Successful requests do not include them.

#### Error classification field reference
...
#### Example: Upstream connection refused (502)

When the upstream server refuses the TCP connection, the access log includes:
...
#### Example: TLS certificate expired (502)
...
#### Example: Circuit breaker open (503)

When a [circuit breaker](/planning-for-production/ensure-high-availability/circuit-breakers) has tripped for an endpoint:
...
Note that `error_target` is absent for gateway-level errors (authentication, rate limiting, validation) because the request never reached the upstream.

#### Example: Upstream returned 503 (5XX passthrough)

When the upstream successfully received the request but responded with an error status:
...
Key distinction: `error_source=Upstream` and `upstream_status=503` tell you the upstream *responded* with an error, as opposed to a connection failure where no response was received.

#### Response flags: Upstream and proxy errors (5XX)

These flags indicate errors that occur when the gateway attempts to proxy the request to the upstream service.
...
These flags indicate errors that occur within the gateway's middleware pipeline before the request reaches the upstream. The `error_source` field identifies which middleware rejected the request.
...
<Note>
The HTTP status code for `AMF` varies by middleware: the Auth Token middleware returns `401`, while OAuth2 and JWT middleware return `400`. The `response_flag` is the same in both cases, but you can use `error_source` to identify which middleware generated the error.
</Note>
...
The `error_source` field identifies the gateway component that generated the error. This helps you understand where in the request processing pipeline the failure occurred.
...
#### Filtering access logs by response flag

You can filter access logs to focus on specific error types. For example, using `grep`:

```bash
# Find all TLS-related errors
grep 'response_flag=TL' /var/log/tyk/access.log

# Find all connection failures
grep -E 'response_flag=(UCF|UCT|URR)' /var/log/tyk/access.log

# Find all rate limiting events
grep -E 'response_flag=(RLT|QEX)' /var/log/tyk/access.log

# Find all auth failures for a specific API
grep 'api_name=payments-api' /var/log/tyk/access.log | grep -E 'response_flag=(AMF|AKI|TKI|TKE)'
```

If you are using JSON log format (`TYK_LOGFORMAT=json`), you can query these fields with tools like `jq`:

```bash
# Find all connection refused errors with their targets
cat /var/log/tyk/access.log | jq 'select(.response_flag == "UCF") | {time, error_target, path}'

# Count errors by response flag
cat /var/log/tyk/access.log | jq -r '.response_flag // empty' | sort | uniq -c | sort -rn
```
