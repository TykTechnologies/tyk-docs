...
<ParamField path="mcp_error_code">
  JSON-RPC error code when the MCP request fails (for example, `-32601` for method not found, `-32602` for invalid params). Only present when an MCP error occurs.
</ParamField>
...
| `mcp_error_code` | String | JSON-RPC error code (e.g. `-32601`, `-32602`) | MCP error responses |
