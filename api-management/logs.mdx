...
Tyk generates four types of log:
...
| :--- | :--- | :--- |
| Verbosity | `TYK_LOGLEVEL` | All Tyk components |
...
| [Tyk MDCB](/tyk-multi-data-centre/mdcb-configuration-options#log_level) | `TYK_MDCB_LOGLEVEL` | — | Legacy text only |
...
<Note>
The `legacy` format was introduced in Tyk 5.14.0 to maintain backward compatibility for existing users with log pipelines reliant on the precise log content. See [legacy format](/api-management/logs#legacy-format).
</Note>
...
By default, Tyk components write all log output to `stderr`.

Tyk Gateway can also forward application logs and access logs to a third-party aggregator. This is in addition to `stderr` output, which continues regardless. Add the relevant settings to `tyk.conf` (or use the equivalent environment variables):

<Tabs>
<Tab title="Sentry">

- `use_sentry`: Set to `true` to enable output to [Sentry](https://sentry.io/product/logs/).
- `sentry_code`: The Sentry-assigned DSN (endpoint URL) to which the Gateway will send the logs.
