...
The input section defines the publisher source of the data stream. Each input type has specific configuration parameters.

Tyk Streams supports the following input and output connectors:

- `kafka`
- `http_client`
- `http_server` (supports plain HTTP, WebSocket and streamed responses)
- `mqtt`
- `amqp_0_9`
- `amqp_1`
- `broker` (combines multiple inputs or outputs)

Other Bento connectors, such as Amazon SQS, are not supported and will be rejected by Tyk Streams configuration validation. To consume from an unsupported source, bridge it into one of the supported connectors (for example, relay SQS messages to Kafka) before connecting Tyk Streams.
