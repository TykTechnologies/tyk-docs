---
title: "Try Tyk Observability with the OTel Demo"
...
- Adapting the configuration to your own Docker Compose environment. See [Get Started with Tyk Observability](/api-management/observability-getting-started)
...
Tyk Gateway writes structured logs to `stderr`. Two settings work together to make Gateway logs useful in Grafana:
...
<Note>
If you want to add observability to an existing Tyk deployment without running the full demo stack, the [Get Started with Tyk Observability](/api-management/observability-getting-started) guide walks through enabling logs, traces, and metrics with a minimal Docker Compose setup.
</Note>
