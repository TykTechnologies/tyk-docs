...
- [`session_lifetime_respects_key_expiration`](/tyk-oss-gateway/configuration#session_lifetime_respects_key_expiration): as for the per-API [respect validity](/api-management/access-control/sessions-and-keys/session-lifecycle#legacy-controls) configuration, this ensures that Sessions are not deleted before they have expired. If this is set to `true`, the Session's TTL is the later of `expires` and the per-API custom key lifetime. This is `customKeyLifetime.value` (Tyk OAS) or `session_lifetime` (Tyk Classic). The per-API setting is not ignored.
