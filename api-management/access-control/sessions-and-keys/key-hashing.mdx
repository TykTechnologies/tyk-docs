...
By default, if key hashing is in use, then requests to the [Gateway API](/api-reference/keys/list-keys) or [Dashboard API](/api-reference/keys/list-all-the-keys) to retrieve a list of keys will be rejected - this functionality is disabled for security reasons.
...
- `enable_hashed_keys_listing`: Set to true to allow the `GET /keys` endpoint to return hashes.
