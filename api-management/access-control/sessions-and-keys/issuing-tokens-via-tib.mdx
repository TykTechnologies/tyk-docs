...
Because LDAP is a passthrough flow, users submit their credentials via a form `POST` directly to TIB; no browser redirect to an external IdP is involved. See [Login Page](/tyk-identity-broker/sso-ldap#login-page) for how to create the login form.
...
See the [LDAP field reference](/tyk-identity-broker/sso-ldap#tib-profile) for all available `ProviderConfig` options.
