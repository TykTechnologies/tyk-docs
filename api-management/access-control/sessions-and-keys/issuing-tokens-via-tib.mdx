...
This example uses [Keycloak](https://www.keycloak.org/) as an OpenID Connect identity provider to issue OAuth 2.0 tokens for API access. It implements the *identity server* role described in [Integration with Identity Server](/api-management/authentication/oauth-2#integration-with-identity-server). TIB authenticates the user against Keycloak, then requests the Tyk access token on the client's behalf.
