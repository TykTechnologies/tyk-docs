...
- **Policy ID (`id`)**: this is the identifier that should be used in all operations via the Gateway API, Dashboard API, in the Session's `apply_policies` array, and in the API definition's scope-to-policy mapping. This can be set by the user.
...
6. **Session Persistence:** During the request, if the client consumes quota, the Gateway updates the quota counters in the temporary Session. After the request completes, the Gateway saves this updated Session object back to Redis so the new quota values are persisted. (Note: Rate limit counters are tracked in separate Redis keys and do not modify the Session object).
...
<Note>
    Because `quota_renewal_rate` is the duration in seconds before the quota resets, taking the maximum value results in the most infrequent renewal period so be careful when combining Policies that set different quota limits.
</Note>


**Proposed Diagram: Policy Combinations and Merging**

Note for the docs team: We should add a visual diagram here. I propose a layered "stack" diagram:
- Bottom Layer: The Base Session object.
- Middle Layer: A Monolithic Policy (e.g., "Standard Tier") applying global rate limits and access rights.
- Top Layers: Multiple Partitioned Policies (e.g., "Reporting Add-on", "Premium Support") stacked side-by-side on top.
- Arrows/Callouts: Show how the Gateway merges these layers downwards to create the "Effective Session". Callouts should explicitly state: "Access Rights: Union (Combined)" and "Rate Limits: Most Permissive (Maximum)".
