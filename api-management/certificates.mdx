...
The Tyk Certificate Store centralizes certificates within Tyk’s data layer for use by multiple APIs. It is available in both licensed and OSS installations.
...
The primary use case is supplying certificates from a secret management system without storing them in the management plane. Any supported [KV reference](/tyk-configuration-reference/kv-store) can supply the PEM content at load time, for example, a `file://` reference can read a certificate mounted from a Kubernetes secret volume without the management plane ever storing the private key. See [Using Local Files as a KV Store](/tyk-configuration-reference/kv-store#local-files) for configuration details.
