...
Any of these forms can be used in the certificate fields in the API definition; the inline PEM content form is not supported in the `cert_file, key_file, ca_file` fields in the [External Services mTSL Config](/configure/external-service#mutual-tls-mtls-configuration) in the Gateway config file (`tyk.conf`).
...
The primary use case is supplying certificates from a secret management system without storing them in the Control Plane. Any supported [KV reference](/tyk-configuration-reference/kv-store) can supply the PEM content at load time, for example, a `file` store can read a certificate mounted from a Kubernetes secret volume without the Control Plane ever storing the private key. See [Local Files](/tyk-configuration-reference/kv-store-providers#local-files-file) for configuration details, or [Legacy Key-Value Syntax](/tyk-configuration-reference/kv-store-legacy#local-files) if you're using the older `file://` notation.
