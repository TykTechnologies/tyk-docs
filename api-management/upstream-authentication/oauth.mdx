...
<Note>
Any error encountered in the communication with the OAuth server will generate an `UpstreamOAuthError` event. This event can be used to trigger an event handler, for example you could use a [webhook](/basic-config-and-security/report-monitor-trigger-events/webhooks) to alert the system administrator of the issue.
</Note>
