...
*Global webhook definitions* are registered with the Dashboard using the [UI](#creating-a-global-webhook-definition-using-tyk-dashboard) or [Dashboard API](/platform-management/dashboard-api#web-hooks-api) and assigned a unique *webhook id* that can be obtained via the [Dashboard API](/platform-management/dashboard-api#list-web-hooks) or via drop-down selection within the UI.
...
- `type`: the type of event handler, in this case should be set to `webhook`
- `cooldownPeriod`: the [webhook cooldown](/api-management/gateway-events#webhook-cooldown) for duplicate events (in duration format, e.g. 10s, 1m30s); use this to prevent flooding of the target endpoint when multiple events are fired in quick succession
- `id`: the *webhook id* assigned by Tyk to the global webhook when it was created (this can be determined using the [list webhooks](/platform-management/dashboard-api#list-web-hooks) endpoint in the Tyk Dashboard API)
