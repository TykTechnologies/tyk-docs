...
- `AuthFailure`: a key has failed authentication (e.g. 401 Unauthorized) or has attempted access and was denied (e.g. 403 Forbidden)
- `KeyExpired`: an attempt has been made to access an API using an expired key
- `UpstreamOAuthError`: an error occurred when trying to authenticate with an upstream using an OAuth providerr
