...
When a request is made to either the Control API or a Hosted API, Tyk will check the hostname in the SNI (Server Name Indication) in the request and will look for a matching Subject Alternative Name (SAN) among the server certificate pairs registered for the Gateway. Having located the appropriate server certificate pair, this will be used to complete the TLS handshake.
...
| Parameter | Description |
|-----------|-------------|
| [`ssl_certificates`](/tyk-oss-gateway/configuration#http-server-options-ssl-certificates) | An array of references to Tyk's server certificate pairs, which can be either Certificate IDs from the Tyk Certificate Store or paths to PEM files |
...
When acting as the TLS client, Tyk Gateway will skip the verification of the server certificate if you configure:
...
### Configuring mTLS for the Gateway Control API

The Gateway can enforce mutual TLS for requests to its Control API for additional security:
...
| `certificates.control_api` | An array of references to CA certificates, which can be either Certificate IDs from the Tyk Certificate Store or paths to PEM files |
...
You can upload an issuing certificate to the Tyk Certificate Store and include this in the static allow list. Tyk will then validate client certificates signed by the registered CA.
...
An alternative to the allow list statically declared in the API definition, is Tyk's powerful dynamic allow list. This is a separate authentication method historically called Dynamic mTLS but, from Tyk 5.12, referred to as [Certificate Auth](/api-management/authentication/certificate-auth). Changes were implemented with the introduction of Certificate Auth to address some [limitations](/api-management/implement-tls#the-problem-with-legacy-dynamic-mtls-mode) in Dynamic mTLS.

The *dynamic allow list* refers to a list of authorized client certificates registered in Redis. This can be managed without making any changes to the API definition, unlike the *static allow list* described [above](/api-management/implement-tls#configuring-mtls-with-a-static-allow-list).
...
| [`ssl_force_common_name_check`](/tyk-oss-gateway/configuration#ssl-force-common-name-check) | Validate that the upstream hostname matches the Common Name (CN) in the server certificate ([legacy support](/api-management/implement-tls#common-name-check)) |
...
| `forceCommonNameCheck` | Validate that the upstream hostname matches the Common Name (CN) in the server certificate ([legacy support](/api-management/implement-tls#common-name-check)) |
...
- Gateway config: `SSLForceCommonNameCheck`
- Tyk OAS: `x-tyk-api-gateway.upstream.tlsTransport.forceCommonNameCheck`
- Tyk Classic: `proxy.transport.ssl_force_common_name_check`  

**Behavior**

When enabled, Tyk performs manual hostname verification against the certificate's Common Name field:
...
If TLS is in use for the Dashboard's control API, then the Gateways will need to verify the server certificate presented by the Dashboard (unless configured to [skip verification](/api-management/implement-tls#skipping-server-certificate-verification) of the server certificate).
