...
`hmac_enabled`
When set to `true` this indicates generation of a [HMAC signature](/api-management/client-authentication#sign-requests-with-hmac) using the secret provided in `hmac_secret`. If the generated signature matches the signature provided in the *Authorization* header then authentication of the request has passed.
