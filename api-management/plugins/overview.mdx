...
1. **Configure plugin type and custom data**
    ...
    In the *Plugins Configuration* section, select the *Plugin Driver*, which tells Tyk which type of plugin to expect: Go, gRPC, JavaScript, Lua or Python. For JavaScript, the `javascript` driver selects the [goja engine](/api-management/plugins/javascript#the-goja-engine) (Tyk Gateway v5.14+) and the `otto` driver selects the legacy engine, retained for backward compatibility.
...
- `driver`: Indicates the type of plugin, e.g. `golang`, `grpc`, `javascript`, `lua`, `otto` or `python`. For JavaScript, `javascript` selects the [goja engine](/api-management/plugins/javascript#the-goja-engine) (Tyk Gateway v5.14+) and `otto` selects the legacy engine, retained for backward compatibility.
...
*   [JavaScript](/api-management/plugins/javascript#the-plugin-contract): A plugin written in Javascript uses JavaScript Virtual Machine (JSVM) interpreter.
...
| :---------------------- | :--------- |
| GoLang                  | goplugin   |
| JavaScript (v5.14+)     | javascript |
| JavaScript (legacy)     | otto      |
| gRPC                    | grpc       |
| Python                  | python     |
| Lua                     | lua        |
...
| Built-in SDK | ✅<br />All Gateway Functionality | ✅<br />[Yes](/api-management/plugins/javascript#the-javascript-api)	| ❌	| ✅<br />[Yes](/api-management/plugins/rich-plugins#tyk-python-api-methods)	| ❌
...
*   **Performance**: JSVM is performant, but is not easy to optimize. From Tyk Gateway v5.14 the `javascript` driver runs on the [goja engine](https://github.com/dop251/goja), a pure-Go ECMAScript engine; the legacy `otto` driver runs on the [otto interpreter](https://github.com/robertkrimen/otto). The JSVM also runs each request on a fresh runtime, which can increase memory footprint.
