...
Custom plugins are usually referred to by the location where they can be *hooked* into the middleware processing chain. These hooks execute alongside Tyk's built-in middleware. For a complete list of built-in middleware and their execution order, see the [Built-in Middleware Reference](/api-management/plugins/built-in-middleware-reference).

The available custom plugin hooks are:

1. **Pre**: Executed at the very beginning of the request lifecycle, before any authentication or rate limiting.
2. **AuthCheck**: Executed during the authentication phase. Used to implement custom authentication mechanisms.
3. **PostKeyAuth**: Executed immediately after successful authentication.
4. **Post**: Executed after authentication and rate limiting, just before the request is proxied to the upstream.
5. **Response**: Executed on the response returned from the upstream before it is sent back to the client.
6. **Analytics**: Executed asynchronously after the response has been sent to the client, used for custom analytics processing.
