...
import { ResponsiveGrid } from '/snippets/5.15/ResponsiveGrid.mdx';
...
- **Volume-mounted files**: the plugin file is mounted into the Gateway's file system at runtime, for example using a Kubernetes ConfigMap, Secret, or CSI volume. See [Custom Plugins with Tyk Operator](/product-stack/tyk-operator/advanced-configurations/custom-plugins) for an example that mounts a plugin this way.
...
<Note>
Tyk Cloud Data Planes only support plugin bundles for custom plugins. If you use Tyk Cloud, see [Configure Custom Plugins in Tyk Cloud](/tyk-cloud/using-plugins).
</Note>
...
<Warning>
Inline code only works with the `javascript` driver, which must be set explicitly. Setting `code` while `driver` is `otto` loads nothing, silently - no error is raised. See [Inline Plugin Code](/api-management/plugins/javascript#inline-plugin-code) for details.
</Warning>
...
- Use **plugin bundles** if you need the same plugin shared across many APIs or Gateways, want to roll out plugin changes without restarting pods, or require [signed bundles](/api-management/plugins/overview#gateway-configuration) for supply-chain integrity.
...
- If you use [Tyk Cloud](/tyk-cloud/using-plugins), plugin bundles are the only supported option for custom plugins.
...
| :---------- | :--------- |
| GoLang     | goplugin  |
| JavaScript | `javascript` (from Tyk 5.15.0, recommended) or `otto` (legacy) - see [Choosing a JavaScript Engine](/api-management/plugins/javascript#choosing-a-javascript-engine) |
| gRPC       | grpc      |
| Python 		 | python    |
| Lua        | lua	     |
...
1. `javascript` (Goja) plugins can perform a small set of atomic operations (get/set/delete/increment) against Tyk's own Redis instance - see [Atomic Storage Bindings](/api-management/plugins/javascript#atomic-storage-bindings). This isn't general TCP or socket access, doesn't extend to other databases, and isn't available on `otto`.
...
*   **TCP Access**: The JSVM has no general socket access, so working with DB drivers directly is not possible. The `javascript` driver is a partial exception: it can perform a small set of atomic operations against Tyk's own Redis instance - see [Atomic Storage Bindings](/api-management/plugins/javascript#atomic-storage-bindings) - but this doesn't extend to other databases, and isn't available on `otto`.
...
- JavaScript [dynamic event handlers](/api-management/gateway-events) always run on the legacy `otto` engine, regardless of an API's configured plugin driver. This doesn't affect custom plugins or virtual endpoints, both of which support either driver.
