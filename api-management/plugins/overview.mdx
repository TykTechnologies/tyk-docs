...
Both variants share the same underlying constraint: Tyk Gateway only loads the plugin file that is present in its file system when the API definition referencing it is loaded. Any change to the plugin therefore requires the file to be replaced and Tyk Gateway to be restarted or the pod rolled out.
...
- Every plugin change requires a new image build or an updated volume, followed by a Gateway restart or pod rollout. This can interrupt live traffic if it is not managed carefully.
...
| Update workflow | Rebuild the image or update the mounted files, then restart or roll out the Gateway | Publish a new bundle file, update the API definition, then trigger a Gateway reload |
...
| Behavior if the plugin source is unavailable at startup | The Gateway fails to load the affected API | Tyk Gateway keeps running with the last cached bundle if the download fails |
