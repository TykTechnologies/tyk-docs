...
| `EDITION` | `ce`, `ee` or `ee-fips`. Defaults to the edition baked into the image. A higher-privilege value than the image supports fails the build immediately. A lower one, for example `ce` on an EE or FIPS image, compiles but produces a plugin that fails to load in that Gateway. See [Things to know before you switch](#things-to-know-before-you-switch). `FIPS=1` is accepted as an alias for `EDITION=ee-fips`. |
