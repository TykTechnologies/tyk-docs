...
| `EDITION` | Defaults to the edition baked into the image, so you do not normally set it. A value that matches the image is accepted. A value that does not match fails the build, because `EDITION` cannot switch an image to another edition. `FIPS=1` is accepted as an alias for `EDITION=ee-fips`. |
