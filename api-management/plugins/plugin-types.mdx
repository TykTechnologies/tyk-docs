...
From Tyk 5.15.0, analytics plugins can also be loaded from a [plugin bundle](/api-management/plugins/overview#plugin-bundles) instead of a bare file path. Add a `traffic_logs` entry to the `custom_middleware` block of the bundle's `manifest.json`:
