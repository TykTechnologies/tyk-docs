...
Tyk Gateway exports **metrics and traces** via the OpenTelemetry Protocol (OTLP) and writes **logs** to stdout/stderr for external collection. This page covers production-ready configuration for each signal — which export topology to use, how to control metrics cardinality, how to tune trace sampling, and how to correlate logs with traces.

For configuration references, see [Distributed Tracing with Tyk](/api-management/traces), [OpenTelemetry Metrics Configuration](/api-management/metrics/opentelemetry-configuration), and [Logging in Tyk](/api-management/logs).
...
Tyk caps each metric instrument at **2,000 unique label-value combinations** by default, controlled by the `cardinality_limit` field in the [OpenTelemetry metrics configuration](/api-management/metrics/opentelemetry-configuration#cardinality-control). When the limit is exceeded, additional combinations are aggregated into an overflow bucket marked with the attribute `otel.metric.overflow=true`. This protects the gateway but means data is being silently dropped.

Alert on overflow to catch cardinality issues early:

```promql
increase(some_metric{otel_metric_overflow="true"}[5m]) > 0
```

The limit can be raised, but the correct fix is reducing cardinality in your [custom metrics configuration](/api-management/metrics/custom-metrics).
...
Configured via `opentelemetry.traces.sampling.type` and `opentelemetry.traces.sampling.rate` in the [gateway configuration](/api-management/traces#sampling-strategies). The sampling decision is made at the start of each trace, before any span data exists.
...
The sampling decision is made after the full trace is collected. The OTel Collector buffers spans in memory, then applies policies to decide what to keep. This allows you to guarantee 100% retention of error traces and slow requests regardless of the overall sample rate.

To use tail-based sampling, send all traces from Tyk to the Collector (set `type: AlwaysOn`, which is the default), then apply policies in the Collector:
...
Tyk Gateway writes logs to stdout/stderr — not via OTLP. They must be collected by an external agent.
...
Enable in the gateway configuration:
...
JSON format has lower parsing overhead than the default text format and is directly indexable by log backends (Loki, Elasticsearch, CloudWatch). Enable it with:
