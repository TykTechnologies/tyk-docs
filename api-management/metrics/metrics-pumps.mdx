...
The base metrics above cover a fixed set of dimensions. You can define your own counters and histograms instead, built from any of the [traffic log fields](/api-management/logs/traffic-logs#traffic-log-field-reference). Use the snake_case version of a field's name as the label, for example `api_key`, `response_code`, or `ip_address`.

A few labels don't follow that pattern directly:

- `code`, `key`, `api`, and `client_id` are shorter aliases for `response_code`, `api_key`, `api_id`, and `oauth_id`.
- `latency_total`, `latency_upstream`, and `latency_gateway` map to the `total`, `upstream`, and `gateway` fields of the [`latency`](/api-management/logs/traffic-logs#traffic-log-field-reference) object.
- `mcp_method`, `mcp_primitive_type`, and `mcp_primitive_name` map to the `jsonrpc_method`, `primitive_type`, and `primitive_name` fields of `mcp_stats`, populated only for [MCP Gateway](/ai-management/mcp-gateway/overview) traffic.
- `time_stamp` is the label for the `timestamp` field. This is the only field where the label doesn't match the field name.
