...
You can set the `api_metrics` array with the `TYK_GW_OPENTELEMETRY_METRICS_APIMETRICS` environment variable instead of the config file. The value must be the JSON-encoded array as a single string:

```bash
export TYK_GW_OPENTELEMETRY_METRICS_APIMETRICS='[{"name":"tyk.requests.by_customer","type":"counter","description":"Request count by customer ID","dimensions":[{"source":"header","key":"X-Customer-ID","label":"customer_id","default":"unknown"}]}]'
```

Note that the variable name ends in `APIMETRICS` with no underscore between `API` and `METRICS`. `TYK_GW_OPENTELEMETRY_METRICS_API_METRICS` is not recognized.
...
| `response_flag` | `metadata` | A 3-letter error classification code set by the gateway, e.g. `URS` (upstream returned 5XX), `AKI` (API key invalid), `CBO` (circuit breaker open). Falls back to the HTTP status code string (e.g. `"200"`) when no error classification applies. See the [full list of response flags](/api-management/logs/access-logs#error-classification). |
