...
| `tyk.response_flag` | Tyk 3-letter error classification code (e.g., `AKI`, `URS`, `CBO`), or the HTTP status code string (e.g., `"200"`) for successful requests or when error classification is absent. See the [full list of response flags](/api-management/logs/access-logs#error-classification). |
...
These instruments cover every [token exchange](/api-management/authentication/token-exchange) decision the Gateway makes when exchanging an inbound token for a downstream token (RFC 8693).
...
| Dimension | Description |
|-----------|-------------|
| `outcome` | One of `ok`, `idp_error`, `misconfig`, `no_matching_provider`, or `step_up_required` (providers using the [`jwt-bearer` grant](/api-management/authentication/token-exchange#grant-type) only). Deliberately bounded to a small enum — the IdP's own error code is never a label (unbounded cardinality); it's available in [structured logs and audit events](/api-management/authentication/token-exchange#observability) instead. |
