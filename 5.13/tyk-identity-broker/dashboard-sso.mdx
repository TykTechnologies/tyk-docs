...
<Note>
We recommend that you read the [Tyk Identity Broker overview](/5.13/tyk-identity-broker/overview) before configuring SSO for Dashboard.
</Note>
...
|---|---|---|
| [`sso_enable_user_lookup`](/5.13/tyk-dashboard/configuration#sso_enable_user_lookup) | Tyk Dashboard configuration | Whether to match the user's email address against existing Tyk Dashboard accounts. |
...
TIB requires valid credentials to [generate the one-time nonce](/5.13/api-management/dashboard-configuration#single-sign-on-api) from the Tyk Dashboard API.
...
Dashboard users who need to create and manage TIB profiles require the **Identity Management (TIB)** [permission](/5.13/api-management/user-management#user-permissions).

## Profile Configuration

Dashboard SSO uses the `GenerateOrLoginUserProfile` [action](/5.13/tyk-identity-broker/overview#actions) in the TIB profile. The following profile fields are required for all Dashboard SSO configurations:

| Field | Value |
|---|---|
| `ActionType` | `GenerateOrLoginUserProfile` |
| `ReturnURL` | `http://{dashboard-host}/tap` |
| `IdentityHandlerConfig.DashboardCredential` | API key for the dedicated [TIB service account](/5.13/#tib-service-account) |

The `ProviderName`, `ProviderConfig`, and `Type` fields depend on your identity provider and are covered in the [IdP-specific guides](/5.13/#set-up-sso-with-your-identity-provider).

The user permission-related fields (`SSOOnlyForRegisteredUsers`, `UserGroupMapping`, `DefaultUserGroupID`, and `CustomUserGroupField`) are optional and used for [unregistered users](/5.13/#unregistered-user-login).
...
<Note>
The `GenerateOrLoginUserProfile` action type is also used to log admin users into Tyk Developer Portal. See [SSO into Tyk Developer Portal](/5.13/tyk-stack/tyk-developer-portal/enterprise-developer-portal/managing-access/enable-sso) for full details.
</Note>
...
|---|---|---|
| [IdP Permission Claims](/5.13/#idp-permission-claims) | TIB profile | Maps IdP group claims to Tyk Dashboard user groups, applying different permissions based on the user's IdP group membership. |
| [Default User Group](/5.13/#default-user-group) | Tyk Dashboard configuration | Assigns a fallback [Tyk Dashboard user group](/5.13/api-management/user-management#manage-tyk-dashboard-user-groups) when no group mapping applies. |
| [Default User](/5.13/#default-user) | Tyk Dashboard configuration | Sets a baseline set of permissions for all unregistered SSO users. |

When combined, the Default User Permissions provide the baseline and group permissions are merged on top. Where there is a conflict, the higher permission level wins. If the user is an [admin](/5.13/api-management/user-management#admin-users), group permissions are skipped to preserve admin rights.
...
<Warning>
If TIB sends no group information to the Dashboard and no `sso_default_group_id` is configured, Tyk Dashboard treats the user as an [admin](/5.13/api-management/user-management#admin-users). This applies when neither IdP permission claims nor a default group is configured.
</Warning>

Setting [`sso_default_group_id`](/5.13/tyk-dashboard/configuration#sso_default_group_id) in the Tyk Dashboard config assigns all SSO users that TIB has not assigned to a group to a specific [user group](/5.13/api-management/user-management#manage-tyk-dashboard-user-groups), from which they will inherit permissions.
...
These permissions will be merged with any configured for the [default user](/5.13/#default-user).

### Default User

In addition to the [default user group](/5.13/#default-user-group) mechanism, setting [`sso_permission_defaults`](/5.13/tyk-dashboard/configuration#sso_permission_defaults) in the Tyk Dashboard config assigns SSO users that have not been assigned to a user group by TIB to a specific set of permissions, for example:
...
These permissions will be merged with any configured for the [default user group](/5.13/#default-user-group).
...
| `host.secret` | [Shared secret](/5.13/tyk-identity-broker/standalone-tib#management-api-secret) between Tyk Dashboard and the standalone TIB instance. Must match the `Secret` field in `tib.conf`. |
...
For installation and configuration of standalone TIB, see [Install Standalone TIB](/5.13/tyk-identity-broker/standalone-tib).
...
|---|---|
| Microsoft Entra ID (OIDC or SAML), ADFS | [SSO with Microsoft Entra ID](/5.13/tyk-identity-broker/sso-entra-id) |
| Okta (OIDC or SAML) | [SSO with Okta](/5.13/tyk-identity-broker/sso-okta) |
