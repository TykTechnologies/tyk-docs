...
import RedisCalculator from '/snippets/5.13/RedisCalculator.mdx';
import DatabaseCalculator from '/snippets/5.13/DatabaseCalculator.mdx';

Visit the following sections to see how to configure the Database for Production:
* [Redis](/5.13/planning-for-production/database-settings#redis)
* [MongoDB](/5.13/planning-for-production/database-settings#mongodb-sizing-guidelines)
* [PostgreSQL](/5.13/planning-for-production/database-settings#postgresql)

Please consult the [data storage configuration](/5.13/api-management/dashboard-configuration#data-storage-solutions) guide for further information relating to how to configure Tyk's data storage across different database engines.
...
**Capping Analytics**
Tyk Gateways can generate a lot of analytics data. Be sure to read about [capping your Dashboard analytics](/5.13/api-management/tyk-pump#tyk-pump-capping-analytics-data-storage)

**API Definition Compression for MDCB Deployments**

In [MDCB](/5.13/api-management/mdcb) deployments, Data Plane Gateways store API definition backups in Redis to ensure resilience when the Control Plane is unavailable. For deployments with many or large API definitions, this can consume significant Redis memory.

You can reduce the size of these backups using optional Zstandard compression. Typically you can expect to see approximately 75% compression, though this can vary depending on the specific configuration of your API definitions and policies.

Set [`storage.compress_api_definitions`](/5.13/tyk-oss-gateway/configuration#storage-compress_api_definitions) and [`storage.compress_policies`](/5.13/tyk-oss-gateway/configuration#storage-compress_policies) in your Data Plane Gateway configuration to compress your API definition and policy backups respectively:
...
```
API definition compression is available from Tyk 5.12.0 onwards.
Policy compression is available from Tyk 5.13.0 onwards.

The Gateway will seamlessly read uncompressed API definitions and policies if they are already stored in Redis in that format. When the Data Plane is reloaded, the definitions and policies will be compressed before being stored.

To mitigate the risk of decompression bombs, the maximum combined uncompressed size for the API definitions is limited to the value you configure for the `storage.max_decompressed_size`.
...
* [Configure Dashboard MongoDB driver](/5.13/tyk-dashboard/configuration#mongo_driver)
* [Configure MDCB MongoDB driver](/5.13/tyk-multi-data-centre/mdcb-configuration-options#analytics-driver)
...
<Note>
If you are using [DocumentDB](https://aws.amazon.com/documentdb/), [capped collections](/5.13/api-management/tyk-pump#tyk-pump-capping-analytics-data-storage) are not supported. See [here](https://docs.aws.amazon.com/documentdb/latest/developerguide/mongo-apis.html) for more details.
</Note>
...
More information on these configuration variables [here](/5.13/tyk-dashboard/configuration). 
