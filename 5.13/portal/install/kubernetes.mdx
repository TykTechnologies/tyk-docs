...
1. **As part of Tyk Self-Managed** - Enable `global.components.devPortal` during Tyk Self-Managed deployment using the [tyk-stack chart](/5.13/product-stack/tyk-charts/tyk-stack-chart)
...
2. **Config settings**
    ...
    In addition to `values.yaml`, you can also define the environment variables described in the [configuration section](/5.13/product-stack/tyk-enterprise-developer-portal/deploy/configuration) to further customize your portal deployment. These environment variables can also be listed as a name value list under the `extraEnvs` section of the helm chart.
...
For the full list of configurable values, refer to the [tyk-stack chart guide](/5.13/product-stack/tyk-charts/tyk-stack-chart). The sections below cover common production configuration scenarios.
...
**Workaround:** Disable the automatic bootstrap and run it manually after deployment. See [Bootstrap Job](/5.13/#bootstrap-job) below.
</Warning>

### Bootstrap Job

The bootstrap job runs once after `helm install`. It waits for the Portal pod to become ready, then calls `POST /portal-api/bootstrap` to create the bootstrap admin ([API Owner](/5.13/portal/api-owner)) user. The Portal blocks its startup sequence until this call succeeds.
...
For bootstrap job failures, crash-loops, database connectivity issues, and license key errors, see [Kubernetes Bootstrap Failures](/5.13/portal/troubleshooting/kubernetes-bootstrap-failures).
...
It is recommended to use new helm charts instead of legacy charts. Guide for new charts can be found [here](/5.13/portal/install/kubernetes)
</Warning>
...
3. **Launch the portal using the helm chart**
    ...
    <Note>
    In case this is the first time you are launching the portal, it will be necessary to bootstrap it before you can use it. For detailed instructions, please refer to the [bootstrapping documentation](/5.13/#bootstrapping-enterprise-developer-portal).
    </Note>
