...
When a synchronization event occurs, for each Provider in turn, the Portal will retrieve all access and limit policies that have matching tags with those configured in the [Provider settings](/5.13/portal/api-provider#policy-tags).
...
The endpoint URL for the Tyk Dashboard. This is where the Developer Portal will interact with the [Tyk Dashboard API](/5.13/api-management/dashboard-configuration#exploring-the-dashboard-api) using its allocated [access credentials](/5.13/portal/api-provider#tyk-dashboard-api-access-credentials).
...
- **Best Practice**: Use a dedicated Dashboard user account with [minimal required permissions](/5.13/portal/api-provider#tyk-dashboard-api-access-credentials)

#### Organisation ID

The organization identifier in [multi-tenant Tyk deployments](/5.13/tyk-cloud/environments-deployments/managing-organisations).
...
- **Note**: This is not the same as a Tyk Developer Portal [Organisation](/5.13/portal/overview/concepts#api-consumers)
...
- Ensure the user account has appropriate [permissions](/5.13/portal/api-provider#tyk-dashboard-api-access-credentials)
- Check for organization ID mismatches

**Missing Products or Plans**

- Verify the corresponding policies exist in the Dashboard
- Verify policies have the correct [policy tags](/5.13/portal/api-provider#policy-tags)
