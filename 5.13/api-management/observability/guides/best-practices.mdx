...
**Via OTel Collector**: recommended for production. Decouples Tyk from the backend, adds buffering and retry, enables tail-based sampling (see [Trace Sampling](/5.13/#traces-sampling-strategy)), and fans out to multiple backends simultaneously.
...
For vendor-specific configuration (Datadog, Dynatrace, New Relic, Elastic, Jaeger), see the [Traces](/5.13/api-management/traces) configuration guide.
...
Tyk ordinarily caps each metric instrument at **2,000 unique label-value combinations**. A different limit can be set using the [cardinality control](/5.13/api-management/logs-metrics#cardinality-control).
...
If you are hitting the cardinality cap then the limit can be raised, but the recommended action is to reduce cardinality in your [custom metrics configuration](/5.13/api-management/metrics/custom-metrics).
...
When defining [custom metrics](/5.13/api-management/metrics/custom-metrics), choose dimension sources based on their cardinality characteristics:
...
To use tail-based sampling, send all traces from Tyk to the Collector using the `AlwaysOn` approach as described in the [trace sampling](/5.13/api-management/traces#sampling) section, then apply policies in the Collector. This is the default setting.
...
Access logs are enabled in the Gateway configuration or the equivalent [environment variable](/5.13/tyk-oss-gateway/configuration#access_logs-enabled):
...
See [Access Logs](/5.13/api-management/logs#access-logs) for the full list of configurable fields and custom template options.

### Use JSON Log Format

JSON format has lower parsing overhead than the default text format and is directly indexable by log backends (Loki, Elasticsearch, CloudWatch). Enable this mode in the Gateway configuration or the equivalent [environment variable](/5.13/tyk-oss-gateway/configuration#log_format):
...
| Kubernetes | OTel Collector with `filelog` receiver; see [Collecting Gateway Logs with OTel on Kubernetes](/5.13/api-management/collecting-gateway-logs-otel-kubernetes) |
...
| **Analytics (Tyk Pump)** | Medium–High (Redis writes) | ~13% RPS reduction when tracking all requests | Use [Do-Not-Track](/5.13/api-management/traffic-transformation/do-not-track) middleware for non-critical endpoints |
