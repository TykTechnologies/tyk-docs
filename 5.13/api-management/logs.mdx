...
<Note>
Tyk Gateway also generates **API Traffic Logs**, which are written into Redis for detailed information about API traffic and analysis. For more information on API Traffic Logs, refer to this [documentation](/5.13/api-management/tyk-pump#api-traffic-logs).
</Note>
...
| [Tyk Gateway](/5.13/tyk-oss-gateway/configuration#log_level)   | `log_level`   | `TYK_GW_LOGLEVEL`    | `info`                   |
| [Tyk Pump](/5.13/tyk-pump/tyk-pump-configuration/tyk-pump-environment-variables#log_level)     | `log_level`   | `TYK_PMP_LOGLEVEL`   | `info`                   |
| [Tyk Developer Portal](/5.13/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_log_level) | `logLevel`  | `PORTAL_LOG_LEVEL`   | `info`   |
...
| `CBO` | Circuit Breaker Open | 503 | The [circuit breaker](/5.13/planning-for-production/ensure-high-availability/circuit-breakers) is open |
...
When a [circuit breaker](/5.13/planning-for-production/ensure-high-availability/circuit-breakers) trips for an endpoint, the access log shows:
...
The [track_404_logs](/5.13/tyk-oss-gateway/configuration#track_404_logs) is a configuration in Tyk Gateway. When you set it to `true`, the gateway will log all incoming HTTP requests that result in a 404 `Not Found` error because they don't match any of your configured API listen paths.
...
You can enable this feature by setting the [track_404_logs](/5.13/tyk-oss-gateway/configuration#track_404_logs) option to `true` in your `tyk.conf` file or by setting the [TYK_GW_TRACK404LOGS](/5.13/tyk-oss-gateway/configuration#track_404_logs) environment variable to `true`.
...
[Custom logging event handlers](/5.13/api-management/gateway-events#logging-api-events) can be registered against **Gateway events** to customise the logs that are generated for those events.
