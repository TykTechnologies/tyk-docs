...
- **Developer Portal:** When [managing access requests](/5.13/tyk-stack/tyk-developer-portal/enterprise-developer-portal/api-access/approve-requests), Tyk will automatically manage the linking of Policies relating to API Products and Plans to the client's Session. 
...
1. **Session Retrieval:** The Gateway extracts the client's [Key](/5.13/api-management/access-control/overview#identifying-the-client-keys-and-authentication) from the request and uses this to retrieve their Session object from the local cache or the Redis database.
...
You can easily [disable a session](/5.13/api-management/access-control/sessions-and-keys/session-lifecycle#temporarily-revoking-access-to-a-key) temporarily by linking it to a Policy that has `is_inactive: true`. 
