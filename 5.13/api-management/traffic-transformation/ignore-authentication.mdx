...
description: "Learn how to skip authentication for an API endpoint"
...
You can also set case sensitivity for the entire Tyk Gateway in its [configuration file](/5.13/tyk-oss-gateway/configuration#ignore_endpoint_case) `tyk.conf`. If case insensitivity is configured at the gateway level, this will override the endpoint-level setting.

#### Endpoint parsing

When using the ignore authentication middleware, we recommend that you familiarize yourself with [Request Matching](/5.13/getting-started/key-concepts/url-matching) options.
...
The [Ignore Authentication](/5.13/api-management/traffic-transformation/ignore-authentication) middleware instructs Tyk Gateway to skip the authentication step for calls to an endpoint, even if authentication is enabled for the API.
...
If you're using the legacy Tyk Classic APIs, then check out the [Tyk Classic](/5.13/#ignore-authentication-using-classic) page.
...
If you're using the newer Tyk OAS APIs, then check out the [Tyk OAS](/5.13/#ignore-authentication-using-tyk-oas) page.
