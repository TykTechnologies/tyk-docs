...
After [verifying](/5.13/basic-config-and-security/security/authentication-authorization/json-web-tokens) that the token hasn't been tampered with, Tyk processes claims in this order:

1. **Claims Extraction**: All claims from the JWT payload are extracted and stored in [context variables](/5.13/api-management/traffic-transformation/request-context-variables) with the format `jwt_claims_CLAIMNAME`. For example, a claim named `role` becomes accessible as `jwt_claims_role`.

2. **Claims Validation**:
   - [Registered Claims Validation](/5.13/#registered-claims-validation): Checks standard claims against your configuration
   - [Custom Claims Validation](/5.13/#custom-claims-validation): Applies your business rules to custom claims
   - [Authorization](/5.13/api-management/authentication/jwt-authorization): Uses validated claims to determine API access and apply policies
...
[Registered Claims](/5.13/#registered-vs-custom-claims) are grouped into:
...
- `notBeforeValidationSkew` allows tokens to be valid before their `nbf` time
    ...
    <Note>
    Temporal claim validation and the associated clock skew controls were supported by Tyk before 5.10.0 and also for [Tyk Classic APIs](/5.13/api-management/gateway-config-tyk-classic#configuring-authentication-for-tyk-classic-apis)
    </Note>
...
Complex object claims can be validated, though typically you'll want to validate specific nested properties using [dot notation](/5.13/#nested-claims).
