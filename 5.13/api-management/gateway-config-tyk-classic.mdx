...
import ApiDefGraphql from '/snippets/5.13/api-def-graphql.mdx';
...
From Tyk 5.8 we recommend that any REST APIs are migrated to the newer [Tyk OAS API](/5.13/api-management/gateway-config-tyk-oas) style, in order that they can benefit from simpler configuration and future enhancements.

For Tyk Dashboard users with an existing portfolio of Tyk Classic API definitions, we provide a [migration tool](/5.13/api-management/migrate-from-tyk-classic), available via the Dashboard API and UI.
...
Note that there are some specific differences between Tyk Classic and Tyk OAS APIs, in particular with respect to [default authentication method](/5.13/#configuring-authentication-for-tyk-classic-apis) and [API versioning](/5.13/#tyk-classic-api-versioning).
...
- common versioning configuration is mostly contained within the [definition](/5.13/api-management/gateway-config-tyk-classic#common-versioning-configuration) object
- configuration for the different versions is contained within the [version_data](/5.13/api-management/gateway-config-tyk-classic#version-specific-configuration) object
...
Each entry must include the method and path (identifying the endpoint) where the middleware runs. You can find full documentation for each middleware in the [Traffic Transformation](/5.13/api-management/traffic-transformation) section including configuration instructions for the Tyk Classic API definition, for example the [allow list](/5.13/api-management/traffic-transformation/allow-list#api-definition-1). When using Tyk Classic, the mock response functionality is configured via the `black_list[]`, `white_list[]` or `ignore[]` middleware.
</ParamField>
...
If set to true, this will override the endpoint level settings in [Ignore](/5.13/api-management/traffic-transformation/ignore-authentication#case-sensitivity), [Allowlist](/5.13/api-management/traffic-transformation/allow-list#case-sensitivity) and [Blocklist](/5.13/api-management/traffic-transformation/block-list#case-sensitivity) middleware. This setting can be overriden at the Tyk Gateway level, and so applied to all APIs, by setting `ignore_endpoint_case` to `true` in your `tyk.conf` file. See [ignore_endpoint_case](/5.13/tyk-oss-gateway/configuration#ignore_endpoint_case) for details.
</ParamField>
...
This field controls the exposure of the API on the Gateway. When set to `true`, the API will not be made available for external access and will not be included in API listings returned by the Gateway's management APIs; it will be accessible only via [internal routing](/5.13/advanced-configuration/transform-traffic/looping).
</ParamField>
...
The [API-level rate limit](/5.13/api-management/rate-limit#rate-limiting-layers) aggregates the traffic coming into an API from all sources and ensures that the overall rate limit is not exceeded. It is composed of a `rate` (number of requests) and `per` (interval). If either is set to `0` then no API-level limit is applied.
</ParamField>
...
This must start with a leading slash `/`. See [Request Matching](/5.13/getting-started/key-concepts/url-matching#api-level-matching-the-listen-path) for detail.

See [Tyk Classic API versioning](/5.13/#tyk-classic-api-versioning) for guidance on structuring API definitions when versions live at different URL paths.

By default, the listen path is kept in the forwarded request path. See [`strip_listen_path`](/5.13/#param-proxy-strip-listen-path) to remove it, and [Request Forwarding](/5.13/planning-for-production/ensure-high-availability/load-balancing) for how the forwarded URL is constructed.
</ParamField>

<ParamField path="proxy.strip_listen_path">
Set to `true` to remove the listen path from the forwarded request path instead of keeping it. See [Request Forwarding](/5.13/planning-for-production/ensure-high-availability/load-balancing) for an example.
</ParamField>
