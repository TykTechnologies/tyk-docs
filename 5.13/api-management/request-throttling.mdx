...
Tyk's Request Throttling feature provides a mechanism to manage traffic spikes by queuing and automatically retrying client requests that exceed [rate limits](/5.13/api-management/rate-limit), rather than immediately rejecting them. This helps protect upstream services from sudden bursts and improves the resilience of API interactions during temporary congestion.
...
Request Throttling is configured using [Policies](/5.13/api-management/policies) or directly on individual [Sessions](/5.13/api-management/access-control/sessions-and-keys/understanding-sessions).
...
Tyk's Request Throttling intercepts API requests *after* they have exceeded a configured [Rate Limit](/5.13/api-management/rate-limit). 
...
Refer to this [documentation](/5.13/#configuration-options).
</Accordion>
