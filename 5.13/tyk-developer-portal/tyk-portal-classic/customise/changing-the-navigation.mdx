...
<ul class="nav navbar-nav">
  ...
  <li><a href="/5.13/portal/{{$menuItem.URL}}">{{$menuItem.Title}}</a></li>
  {{ end }}
  <li><a href="/5.13/portal/apis/">API Catalog</a></li>
  {{ if not .PortalConfig.DisableSignup }}
    {{ if not .UserData }}
    <li><a href="/5.13/portal/register/">Register</a></li>
    {{ end }}
  {{ end }}
  {{ if not .PortalConfig.DisableLogin }}
    {{ if not .UserData }}
    <li><a href="/5.13/portal/login/">Log in</a></li>
    {{ end }}
  {{ end }}
</ul>
```
