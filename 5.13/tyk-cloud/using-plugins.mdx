...
* [Python Plugins](/5.13/api-management/plugins/rich-plugins#overview)
* [JSVM](/5.13/api-management/plugins/javascript#)
* [Golang](/5.13/tyk-cloud/using-plugins)
...
Next you'll [set up the Python authentication code bundle](/5.13/#create-a-python-code-bundle).
...
Next you will [create an API](/5.13/#test-middleware) from our Control Plane and see our middleware in action.
...
| File              | Description                                                                                                                                                                                                                                                                                       |
| :------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| custom_middleware | contains the middleware settings like the plugin driver you want to use (driver) and the hooks that your plugin will expose. We use the   **auth_check** for this tutorial. For other hooks see [here](/5.13/api-management/plugins/rich-plugins#coprocess-dispatcher-hooks). |
...
* You import decorators from the Tyk module that gives us the Hook decorator, and we import [Tyk Python API helpers](/5.13/api-management/plugins/rich-plugins#tyk-python-api-methods)
...
* A plugin bundle is a packaged version of the plugin, it may also contain a cryptographic signature of its contents. The -y flag tells the Tyk CLI tool to skip the signing process in order to simplify this tutorial. For more information on the Tyk CLI tool, see [here](/5.13/api-management/plugins/overview#how-plugin-bundles-work).
* You should now have a `bundle.zip` file in the plugin working directory.
* Next you will configure [uploading your plugin bundle file](/5.13/#uploading-your-bundle) to your Amazon S3 bucket.
...
1. Firstly you will need a local Tyk Gateway installation that allows you to create your Python plugin bundle. We recommend [installing our Self-Managed version on Ubuntu Bionic 18.04](/5.13/tyk-self-managed/install/linux#install-tyk-on-debian-or-ubuntu).
