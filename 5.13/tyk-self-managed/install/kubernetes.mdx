...
- [Enterprise Edition License](/5.13/apim#licensing)

<Note>
Running on Podman, containerd, or another container runtime? See [Container Runtimes](/5.13/deployment-and-operations/container-runtimes).
</Note>
...
5. **Done!**
    ...
    You are now ready to [create an API](/5.13/api-management/gateway-config-managing-classic#create-an-api).

    For the complete installation guide and configuration options, please see [Tyk Stack Helm Chart](/5.13/product-stack/tyk-charts/tyk-stack-chart).
...
<Note>
If you want to enable Tyk Developer Portal, please use [PostgreSQL](/5.13/#install-tyk-stack-with-helm-chart-postgresql). MongoDB is not supported in Developer Portal.
</Note>
...
2. **Install Redis (if you don't have a Redis instance)**
    ...
    <Note>
    Please make sure you are installing Redis versions that are supported by Tyk. Please refer to Tyk docs to get list of [supported versions](/5.13/planning-for-production/database-settings#redis).
    </Note>
...
3. **Install MongoDB (if you don't have a MongoDB instance)**
    ...
    <Note>
    Please make sure you are installing MongoDB versions that are supported by Tyk. Please refer to Tyk docs to get list of [supported versions](/5.13/api-management/dashboard-configuration#supported-database).
    </Note>
    ...
    <Note>
    Ensure that you are installing MongoDB versions that are supported by Tyk. Please consult the list of [supported versions](/5.13/api-management/dashboard-configuration#supported-database) that are compatible with Tyk.
    </Note>
...
This demo will run Tyk Self-Managed on your machine, which contains 5 containers: Tyk Gateway, Tyk Dashboard, Tyk Pump, Redis and either MongoDB or one of our supported [SQL databases](/5.13/api-management/dashboard-configuration#supported-database).
...
Now you have your prerequisites, follow the instructions from our [Tyk Helm Chart](/5.13/#use-legacy-helm-chart) page.
...
To get started go to [Tyk Operator](/5.13/api-management/automations/operator).

### Developer Portal

If you are deploying the **Tyk Developer Portal**, set the appropriate values under the `enterprisePortal` section in your `values.yaml`. Please visit [Tyk Developer Portal installation](/5.13/portal/install/kubernetes) for a step by step guide.
...
Check [Tyk Control plane](/5.13/api-management/mdcb) for more configuration details.
...
See [TIB](/5.13/tyk-identity-broker/standalone-tib) for more details.
...
<Warning>
`tyk-pro` chart is deprecated. Please use our [Tyk Stack helm chart](/5.13/product-stack/tyk-charts/tyk-stack-chart) instead. 

We recommend all users migrate to the `tyk-stack` Chart. Please review the [Configuration](/5.13/product-stack/tyk-charts/tyk-stack-chart) section of the new helm chart and cross-check with your existing configurations while planning for migration.
</Warning>
...
2. **Data stores**
    ...
    You can find supported MongoDB and SQL versions [here](/5.13/planning-for-production/database-settings).
...
<Note>
Please make sure you are installing PostgreSQL versions that are supported by Tyk. Please refer to Tyk docs to get list of [supported versions](/5.13/api-management/dashboard-configuration#supported-database).
</Note>
...
Check [Tyk Gateway Sharding](/5.13/api-management/multiple-environments#what-is-api-sharding-) for more details.
