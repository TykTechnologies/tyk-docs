...
External Service Configuration gives you control over how Tyk Gateway securely connects to the [external services](/5.13/configure/external-service#external-service) that it needs to function correctly. You can configure proxy settings, mTLS authentication, and other connection parameters for Tyk's outbound connections to services such as OAuth providers, analytics systems, and webhook endpoints.
...
Refer to the [Tyk Gateway Configuration Reference](/5.13/tyk-oss-gateway/configuration#external_services) for more details on this setting.
...
| :-------------- | :------------- | :------------ |
| `oauth` | [OAuth/JWT token validation and introspection](/5.13/basic-config-and-security/security/authentication-authorization/json-web-tokens) | JWT auth interactions with Auth Server (Identity Provider) |
| `storage` | [External storage operations](/5.13/planning-for-production/database-settings) | Redis connections, database interactions |
| `webhooks` | [Webhook event notifications](/5.13/api-management/gateway-events#event-handling-with-webhooks) | Event handlers, notification delivery |
| `health` | [Health check requests](/5.13/planning-for-production/ensure-high-availability/health-check) | Host checker, uptime monitoring |  
| `discovery` | [Service discovery requests](/5.13/planning-for-production/ensure-high-availability/service-discovery) | Load balancer, service registry |
...
| `cert_id` | String | No | Certificate ID from [Tyk Certificate Store](/5.13/api-management/certificates#tyk-certificate-store) |
| `ca_cert_ids` | Array | No | Array of CA certificate IDs from [Tyk Certificate Store](/5.13/api-management/certificates#tyk-certificate-store) |
...
From Tyk 5.13.0, the certificate paths (`cert_file`, `key_file`, `ca_file`) for service type `oauth`, used for integration with external Identity Providers (IdPs) for [client](/5.13/api-management/authentication/jwt-signature-validation#remotely-stored-keys-jwks-endpoint) and [upstream](/5.13/api-management/upstream-authentication/oauth#upstream-oauth-2-0) authentication, can be set using [external references](/5.13/tyk-configuration-reference/kv-store#from-the-tyk-gateway-configuration-file). These references are resolved when the Gateway starts up and on each hot reload. This allows certificate rotation without a Gateway restart; however, updated certificates are applied only after a hot reload.

The certificate paths for other integrations do not support external references at this time.

<Note>
If [multiple Identity Providers](/5.13/api-management/authentication/jwt-signature-validation#multiple-jwks-endpoints) are configured, they share the same External Service Configuration.
</Note>
...
Refer to the [Tyk Gateway Configuration Reference](/5.13/tyk-oss-gateway/configuration#external_services) for more details.
...
4. **Default settings** - If a setting is not specified, Tyk uses sensible defaults. You can find these in the [Tyk Gateway Configuration Reference](/5.13/tyk-oss-gateway/configuration#external_services).
...
**What this does:**
- Similar to previous example, but uses [Tyk Certificate Store](/5.13/api-management/certificates#tyk-certificate-store) instead of file paths
...
**What this does:**
- Similar to example 4, but uses [Tyk Certificate Store](/5.13/api-management/certificates#tyk-certificate-store) for all mTLS configurations
