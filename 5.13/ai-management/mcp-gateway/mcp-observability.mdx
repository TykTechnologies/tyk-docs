...
OpenTelemetry must be enabled on your Tyk Gateway. See [OpenTelemetry configuration](/5.13/api-management/traces) for setup instructions.
...
|---|---|---|
| **Metrics** | Custom OTel metric instruments with MCP dimensions for counters and histograms | [MCP metrics](/5.13/ai-management/mcp-gateway/mcp-metrics) |
| **Access logs** | Structured per-request log records with MCP fields included when non-empty | [MCP access logs](/5.13/ai-management/mcp-gateway/mcp-access-logs) |
