...
<Note>
For the full OpenAPI specifications of the Gateway API and Dashboard API, see [Tyk APIs](/5.13/tyk-apis).
</Note>
...
Returns the full Tyk OAS API definition. See [Get an MCP proxy: Gateway API](/5.13/#get-an-mcp-proxy) for the response shape.
...
For a conceptual explanation of what the policy fields do and how to configure access tiers, see [MCP proxy policies](/5.13/ai-management/mcp-gateway/policies).
