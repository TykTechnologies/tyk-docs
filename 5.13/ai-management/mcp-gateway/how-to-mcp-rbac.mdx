...
After [securing your MCP proxy](/5.13/ai-management/mcp-gateway/how-to-proxy-remote-mcp), the next step is controlling what each consumer can do. Role-based access control (RBAC) in Tyk MCP lets you give different agents different views of the same proxy, without creating separate proxy definitions. A policy bound to a key determines which tools that agent can invoke.
...
For the complete policy reference, see [MCP policies](/5.13/ai-management/mcp-gateway/policies).

---

## Before you begin

- The Mock MCP Server running on `http://localhost:7878`. Set up in the [quickstart](/5.13/ai-management/mcp-gateway/quickstart).
- An MCP proxy named **Mock MCP Server** with authentication enabled. See [How to secure an MCP proxy](/5.13/ai-management/mcp-gateway/how-to-proxy-remote-mcp).
