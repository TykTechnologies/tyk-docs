...
- **Long Term Support:** Refer to our [versioning and long-term support policies](/5.13/developer-support/release-types/long-term-support) for details on major and minor releases, patches, and support dates.
- **Preparations:** Review the [preparation guidelines](/5.13/#upgrade-guidelines) before starting the upgrade.
- **Release notes:** Always check the "Upgrade Instructions" section in the relevant release notes.
- **Backups:** Follow our [comprehensive backup guide](/5.13/developer-support/faq#tyk-configuration) before starting the upgrade.
...
| :---------------------- | :---------------- | :----------- |
| **Tyk Cloud**        | [Cloud SaaS](/5.13/#tyk-cloud) | Guide to Tyk Cloud SaaS |
|                      | [Hybrid](/5.13/#tyk-hybrid) | Guide for Hybrid environments with Gateway Data Plane(s) deployed locally or with a third-party cloud provider |
|                      | [Go plugin](/5.13/#go-plugin) | Guide for upgrading Go plugin on the Tyk Cloud |
| **Tyk Self-Managed** | [RHEL and CentOS](/5.13/#upgrade-linux-redhat-centos) | Guide for RPM-based Linux distributions |
|                      | [Debian and Ubuntu](/5.13/#upgrade-linux-debian) | Guide for DEB-based Linux distributions |
|                      | [Docker](/5.13/#docker) | Guide for upgrading Docker images |
|                      | [Helm](/5.13/#helm) | Guide for upgrading Helm Charts |
|                      | [Kubernetes](/5.13/#kubernetes) | Guide for upgrading Kubernetes environment |
| **Tyk MDCB Self-Managed** | [MDCB](/5.13/#upgrade-mdcb) | Guide for upgrading Multi Data Center Bridge (MDCB) |
| **Tyk Open-Source**  | [Tyk Gateway](/5.13/#open-source-upgrade-guide) | Guide for upgrading Tyk open-source environment |
...
Use our [migration tool](/5.13/planning-for-production/database-settings#migrating-from-an-existing-mongodb-instance) to manage the switch from MongoDB to SQL.

##### Backup APIs Script

Utilize our bash [backup script](/5.13/developer-support/upgrading#backup-apis-and-policies) to export and restore all Tyk API Definitions and Policies.
...
Which strategy do you intend to use?
  ...
  - If following the [Blue-Green upgrade](/5.13/#blue-green-upgrade) strategy, has the green environment been configured and verified as production-ready?
  - If pursuing the [Rolling upgrade](/5.13/#rolling-upgrade) strategy, do all Tyk components have a second instance?
...
Have backups been performed?
  ...
  - Backup guide: Have you checked our [comprehensive guide for backing up Tyk](/5.13/developer-support/faq#tyk-configuration)?

#### Go plugins
Do you use custom Go plugins with your APIs?

  - Go plugin must be [recompiled](/5.13/#go-plugins-upgrade-guide) for the new version.
...
Use the [Upgrade Guides ToC](/5.13/#tyk-upgrade-guides-for-different-deployment-models) to choose the appropriate upgrade guide for
your platform.
...
3. **Upgrade Process:**
   ...
   - If using a new deployment process, follow the [deployment instructions](/5.13/tyk-self-managed) appropriate for your platform.
...
the [upgrade guidelines](/5.13/#upgrade-guidelines).
...
Follow our guide for [upgrading Cloud Control Planes](/5.13/tyk-cloud/environments-deployments/managing-control-planes#upgrade-cloud-control-planes).

**Step 2. Upgrade Go Plugins**

Follow our guide for deploying your [Go plugins on Tyk Cloud](/5.13/#go-plugin). Subsequently, follow the steps below according to the target upgrade version of the Gateway.

**Gateway Versions < 4.1.0.**

1. Proceed with [upgrading your Tyk Data Plane (Gateway)](/5.13/#upgrading-cloud-data-planes)
2. Update the [custom_middleware_bundle](/5.13/api-management/plugins/overview#plugin-bundles) field in the API Definitions of all APIs that use your plugin. The field should be updated to use the new bundle file containing your upgrade plugin.
3. Validate that your plugin is working per your expectations.

**Gateway Versions >= 4.1.0**

1. Update the [custom_middleware_bundle](/5.13/api-management/plugins/overview#plugin-bundles) field in the API Definitions of all APIs that use your plugin. The field should be updated to use the new bundle file containing your upgraded plugin.
...
3. Proceed with [upgrading your Tyk Data Plane (Gateway)](/5.13/#upgrading-cloud-data-planes). Given that you loaded your target version plugin in step 1, this version will be loaded automatically once you upgrade.
...
Follow our guide for [upgrading Cloud Data Planes](/5.13/tyk-cloud/environments-deployments/managing-gateways).
...
After following the guidelines for [preparing for upgrade](/5.13/#upgrade-guidelines),
...
See Tyk Guide for how to [upgrade Control Planes](/5.13/tyk-cloud/environments-deployments/managing-control-planes#upgrade-cloud-control-planes)

**Step 2 Upgrade Go plugins**

Follow our guide for [upgrading your Go plugins on Tyk Cloud](/5.13/#go-plugin). Subsequently, follow the steps below according to the target upgrade version of the Gateway.

**Gateway Versions < 4.1.0**

1. Proceed with upgrading your [Tyk Data Plane Hybrid Gateways](/5.13/#upgrading-data-plane-hybrid-gateways).
2. Update the [custom_middleware_bundle](/5.13/api-management/plugins/overview#plugin-bundles)
...
1. Update the [custom_middleware_bundle](/5.13/api-management/plugins/overview#plugin-bundles)
field in the API Definitions of all APIs that use your plugin. The field should be updated to use the new bundle file
...
3. Proceed with upgrading your [Tyk Data Plane Hybrid Gateways](/5.13/#upgrading-data-plane-hybrid-gateways). Given that you
...
- [Docker](/5.13/#docker)
- [Helm](/5.13/#helm)
- [Linux Debian](/5.13/#upgrade-linux-debian)
- [Linux RHEL/CENTOS](/5.13/#upgrade-linux-redhat-centos)
...
2. [Compile your custom Go plugins](/5.13/#go-plugins-upgrade-guide).
...
| :--- | :--- | :--- |
| [Path 1](/5.13/#path-1)    | < 4.1.0 | < 4.1.0 |
| [Path 2](/5.13/#path-2)    | >= 4.1.0 | >= 4.2.0 |

**Path 1 - Current Version < 4.1.0 and Target Version < 4.1.0**

1. [Create a plugin bundle](/5.13/api-management/plugins/overview#plugin-bundles) that includes the newly compiled version
...
2. [Upload this bundle](/5.13/tyk-cloud/using-plugins#uploading-your-bundle) to your configured S3 bucket if using Cloud SaaS. If you're using Hybrid SaaS, upload this bundle to your bundle server.


**Path 2 - Current `Version >= 4`.1.0 and Target `Version >= 4`.2.0**

1. [Create a plugin bundle](/5.13/api-management/plugins/overview#plugin-bundles) that includes both your current version’s plugin along with the newly compiled version
...
the addition of the [Multi Data Center Bridge](/5.13/api-management/mdcb) (MDCB).
...
If you use [Tyk Operator](/5.13/tyk-stack/tyk-operator/installing-tyk-operator) to manage API definitions via `TykMCPProxyDefinition` custom resources, deploy the new Operator version only *after* Tyk Dashboard and Tyk Gateway are running the new version. The Operator controller calls the Dashboard `/api/mcps` endpoint, which only exists from v5.13 onwards. Deploying the Operator first will cause reconciliation failures on all MCP custom resources.

**Tyk Developer Portal**

If your deployment includes the [Tyk Developer Portal](/5.13/portal/overview/getting-started), upgrade Portal before or at the same time as Tyk Dashboard, and definitely before creating any MCP Proxy policies in the Dashboard. From v1.17.2, Portal prevents MCP Proxy policies from appearing as API Products in the Developer Portal so that they cannot accidentally be exposed in Catalogs. An older Portal version running against a new Dashboard with MCP Proxy policies configured will surface those policies which could then be exposed to API Consumers who will not be able to consume them.
...
Follow our [Tyk OSS guide](/5.13/#go-plugins-upgrade-guide) for upgrading Tyk Gateway and Tyk Pump
...
For production environments, it is recommended to use [Helm charts](/5.13/#helm) to manage upgrades with zero downtime.

When upgrading Kubernetes Deployments, you should use a `RollingUpdate` strategy to ensure zero downtime. For more information on upgrade strategies, see the [Upgrade Strategies](/5.13/#upgrade-strategies) section.
...
After reviewing guidelines for [preparing for upgrade](/5.13/#upgrade-guidelines),
follow the instructions below to upgrade your Tyk components and plugins.

**Upgrade order:**

Please note that upgrade order is as explained in the upgrade [overview](/5.13/#tyk-self-managed-upgrade-guide)
...
Before executing the upgrade, ensure that you have consulted and performed all the necessary steps in the [pre-upgrade checklist](/5.13/#upgrade-guidelines).
...
the [self-managed](/5.13/#tyk-self-managed-upgrade-guide) section, and look for the instruction regarding Tyk Gateway.

Before proceeding with the upgrade process, ensure that you have thoroughly reviewed and completed the steps outlined in
the [pre-upgrade guidelines](/5.13/#upgrade-guidelines). Once you
...
For full installation details, check the usual [installation page](/5.13/apim/open-source/installation).
...
3. Review and complete the steps outlined in the [pre-upgrade guidelines](/5.13/#upgrade-guidelines).
...
| :----------------- | :----------------- | :---------------- |
| [Path 1](/5.13/#path-1)    | < 4.1.0         | < 4.1.0        |
| [Path 2](/5.13/#path-2)    | < 4.1.0         | >= 4.1.0       |
| [Path 3](/5.13/#path-3)    | >= 4.1.0        | >= 5.1.0       |
...
If you are using Self-Managed deployment then we recommend that you use [Tyk Sync](/5.13/api-management/automations/sync) to backup your Tyk APIs and policies. 
