...
Only the most recent Tyk Dashboard release and the most recent on the [Long Term Support (LTS)](/5.13/developer-support/release-types/long-term-support) branch are supported. We recommend staying current on the feature or LTS branch to minimise exposure to security vulnerabilities from third party dependencies.
...
| [OpenAPI Specification](https://spec.openapis.org/oas/v3.1.2.html) | v3.1.x, v3.0.x | v3.1.x, v3.0.x | Supported by [Tyk OAS](/5.13/api-management/gateway-config-tyk-oas)|
...
{/* TT-17453 */}
Tyk Dashboard embeds Tyk Identity Broker. This has been updated to [version 1.7.3](/5.13/developer-support/release-notes/tib#1-7-3-release-notes).
</Accordion>
...
{/* TT-16816 */}
Resolved an issue where importing a [multipart OpenAPI document](/5.13/api-management/gateway-config-managing-oas#multi-part-openapi-documents) from a ZIP archive left temporary files on the Dashboard's local disk after the import completed. Over time this could exhaust disk space and, because the temporary location was derived from the archive filename, a later import using the same filename could reuse stale contents resulting in unexpected API configuration.
...
Dashboard 5.13.0 introduces policies that grant access to MCP Proxies. Tyk Developer Portal versions prior to 1.17.2 may automatically retrieve these policies and present them as API Products when [synchronizing with the Dashboard](/5.13/portal/api-products#dashboard-first-approach).
...
See [MCP Gateway upgrade considerations](/5.13/developer-support/upgrading#mcp-upgrade-considerations) for the full sequence.
...
For details, see the [Managing MCP Proxies documentation](/5.13/ai-management/mcp-gateway/managing-proxies).
</Accordion>
...
For details, see the [MCP Proxy policies documentation](/5.13/ai-management/mcp-gateway/policies).
</Accordion>
...
- **Configurable toast notification duration**: The display duration of toast notifications can now be configured separately for success, warning, info, and error messages via the new [ui.notifications.duration](/5.13/tyk-dashboard/configuration#ui-notifications-duration) configuration option, replacing the previous fixed three-second duration. This helps reduce visual noise from frequent notifications while keeping critical messages visible for longer.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.12.1).
...
| Third Party Dependency | Tested Versions | Compatible Versions | Comments | 
| ---------------------- | --------------- | ------------------- | -------- | 
| [GoLang](https://go.dev/dl/)          | 1.25 | 1.25 | [Go plugins](/5.13/api-management/plugins/golang) must be built using Golang 1.25 | 
| [Redis](https://redis.io/download/)   | 5.x, 6.x, 7.x    |  5.x, 6.x, 7.x    | | 
| [Valkey](https://valkey.io/download/) | 8.0.x, 8.1.x    | 7.2.x, 8.0.x, 8.1.x    | | 
| [MongoDB](https://www.mongodb.com/try/download/community)  | 6, 7, 8  | 5, 6, 7, 8  | | 
| [DocumentDB](https://aws.amazon.com/documentdb/)  | 4, 5  | 4, 5  | | 
| [PostgreSQL](https://www.postgresql.org/download/) | 13.x - 17.x | 13.x - 17.x  | | 
| [OpenAPI Specification](https://spec.openapis.org/oas/v3.1.2.html) | v3.1.x | v3.1.x | Supported by [Tyk OAS](/5.13/api-management/gateway-config-tyk-oas)|
...
To provide an additional layer of security for your APIs, we've introduced Dashboard support for [Client Certificate-Token Binding](/5.13/api-management/authentication/bearer-token#client-certificate-token-binding). This feature allows you to form a strict binding association between an Auth Token issued to a client and their specific client certificate.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.12.0).
...
The Tyk Dashboard now supports [application logging](/5.13/api-management/logs#application-logs) in structured JSON format. To enable this feature, set the environment variable `TYK_DB_LOGFORMAT` to `json`.
...
In this release, we have fixed some priority CVEs. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.11.1).
...
This patch release fixes an issue in the API Designer where JWKS cache timeout values could not be saved properly. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.10.2).
...
- [Docker Image to pull](https://hub.docker.com/r/tykio/tyk-dashboard/tags?page=&page_size=&ordering=&name=v5.10.2)
  - ```bash
    ...
    ```
- Helm charts
  - [tyk-charts v4.1.0](/5.13/developer-support/release-notes/helm-chart#4-1-0-release-notes)
...
This patch release upgrades the Go build environment and delivers UI, analytics, and security fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.10.1).
...
Both interfaces now consistently accept either `null` or `[]` (empty array) values in the `versions` field of the access control list, ensuring seamless interoperability between API and UI workflows for policy and key management. Tyk OAS APIs use a [different approach](/5.13/api-management/api-versioning#how-api-versioning-works-with-tyk) to versioning, with each (base or child) version having a unique API ID that is added to the access list.
</Accordion>
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.9.2).
...
| Third Party Dependency | Tested Versions | Compatible Versions | Comments | 
| :---------------------- | :--------------- | :------------------- | :-------- | 
| [GoLang](https://go.dev/dl/)          | 1.23 | 1.23 | [Go plugins](/5.13/api-management/plugins/golang) must be built using Go 1.23 | 
| [Redis](https://redis.io/download/)   | 5.x, 6.x, 7.x    |  5.x, 6.x, 7.x    | | 
| [Valkey](https://valkey.io/download/) | 8.0.x, 8.1.x    | 7.2.x, 8.0.x, 8.1.x    | | 
| [MongoDB](https://www.mongodb.com/try/download/community)  | 6, 7, 8  | 5, 6, 7, 8  | | 
| [DocumentDB](https://aws.amazon.com/documentdb/)  | 4, 5  | 4, 5  | | 
...
Resolved a compatibility issue introduced via a fix in Dashboard v5.9.0 (LTS v5.8.3) to account for DocumentDB's handling of endpoint paths containing dots (`.`). The Dashboard now escapes dot characters (e.g., `\u002e`) when storing Tyk OAS API definitions in the database, but MDCB failed to unescape them when reading from the database. This caused the [request validation](/5.13/api-management/traffic-transformation/request-validation) and [mock response](/5.13/api-management/traffic-transformation/mock-response#mock-response) middleware configured on affected endpoints not to be applied. 

To align MDCB's dot handling mechanism with Tyk Dashboard, a new configuration option, `escape_dots_in_oas_paths`, has been introduced in both [Dashboard](/5.13/tyk-dashboard/configuration#escape_dots_in_oas_paths) and [MDCB](/5.13/tyk-multi-data-centre/mdcb-configuration-options#escape_dots_in_oas_paths):
...
Check the [Upgrade and Compatibility section](/5.13/#upgrade-5.9.2) for details on the recommended upgrade path.
</Expandable>
...
This release builds on the recent release of [Tyk 5.8.3](/5.13/developer-support/release-notes/dashboard#5-8-3-release-notes), adding a collection of new capabilities. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.9.0).
...
In this release, we have fixed some priority CVEs. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.8.11).
...
This patch release addresses a CVE present in 5.8.9. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.8.10).
...
In this release we've deprecated the [allow_explicit_policy_id](/5.13/tyk-dashboard/configuration#allow_explicit_policy_id) configuration option. This was previously added to allow the use of custom policy IDs, which is now the default behaviour so this option is redundant.
...
This patch release contains various bug fixes and introduces new OPA helper functions to identify the type of API being processed (Tyk OAS, Tyk Streams, Tyk Classic). For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.8.9).
...
This patch release upgrades the Go build environment and fixes some high-priority CVEs. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.8.8).
...
Resolved an issue where the [Dashboard API](/5.13/tyk-dashboard-api) allowed creation of empty or duplicate categories when updating Tyk OAS APIs through the `PUT /api/apis/oas/{API_ID}/categories` endpoint.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.8.5).
...
Check the [Upgrade and Compatibility section](/5.13/#upgrade-5.8.5) for details on the recommended upgrade path.
</Expandable>
...
- The Tyk OAS API designer now fully supports the use of Key-Value storage [references](/5.13/tyk-configuration-reference/kv-store#from-api-definitions) that are valid in the API definition.
...
With Tyk 5.8.0 we are delighted to unlock the power and flexibility of Tyk OAS for all users, with full feature parity with the legacy Tyk Classic style for REST APIs. We are thrilled to announce new updates and improvements in Tyk 5.8.0, delivering more control, flexibility, and performance.  For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.8.0) below.
...
| Third Party Dependency                                     | Tested Versions        | Compatible Versions    | Comments | 
| :---------------------------------------------------------- | :---------------------- | :---------------------- | :-------- | 
| [GoLang](https://go.dev/dl/)                               | 1.22       | 1.22       | [Go plugins](/5.13/api-management/plugins/golang) must be built using Go 1.22 | 
| [Redis](https://redis.io/download/)  | 6.2.x, 7.x  | 6.2.x, 7.x  | | 
| [MongoDB](https://www.mongodb.com/try/download/community)  | 5.0.x, 6.0.x, 7.0.x  | 5.0.x, 6.0.x, 7.0.x  | | 
| [DocumentDB](https://aws.amazon.com/documentdb/)  | 4, 5  | 4, 5  | | 
...
This release focuses mainly on a security fix. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.7.2) below.
...
* **[External OAuth](/5.13/api-management/client-authentication#integrate-with-external-authorization-server-deprecated)**

  * Tyk Classic: `external_oauth`
  * Tyk OAS: `server.authentication.securitySchemes.externalOAuth`

* **[OpenID Connect (OIDC)](/5.13/api-management/client-authentication#integrate-with-openid-connect-deprecated)**

  * Tyk Classic: `auth_configs.oidc`
  * Tyk OAS: `server.authentication.oidc`

We recommend migrating to **[JWT Authentication](/5.13/basic-config-and-security/security/authentication-authorization/json-web-tokens)** for improved flexibility and long-term support.
...
[Setup guide for JWE OIDC SSO](/5.13/api-management/single-sign-on-social-idp#json-web-encryption-jwe)
</Accordion>
...
For detail usage of the `/audit-logs` endpoint, please see [Dashboard API documentation](/5.13/tyk-dashboard-api).
</Accordion>
...
We are thrilled to announce new updates and improvements in Tyk 5.6.0, bringing more control, flexibility, and performance.  For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.6.0) below.

##### Per endpoint Rate Limiting for clients

Now you can configure rate limits at the [endpoint level per client](/5.13/api-management/rate-limit#key-level-rate-limiting), using new configuration options in the access key. Use [Policies](/5.13/api-management/policies) to create templates to set appropriate rate limits for your different categories of user.
...
We encourage users to switch to the recommended alternatives. For more detailed information, please refer to the [Documentation](/5.13/api-management/client-authentication#integrate-with-openid-connect-deprecated)
...
| Third Party Dependency                                     | Tested Versions        | Compatible Versions    | Comments | 
| :---------------------------------------------------------- | :---------------------- | :---------------------- | :-------- | 
| [GoLang](https://go.dev/dl/)                               | 1.21       | 1.21       | [Go plugins](/5.13/api-management/plugins/golang#) must be built using Go 1.21 | 
| [Redis](https://redis.io/download/)  | 6.2.x, 7.x  | 6.2.x, 7.x  | | 
| [MongoDB](https://www.mongodb.com/try/download/community)  | 5.0.x, 6.0.x, 7.0.x  | 5.0.x, 6.0.x, 7.0.x  | | 
| [DocumentDB](https://aws.amazon.com/documentdb/)  | 4, 5  | 4, 5  | | 
| [PostgreSQL](https://www.postgresql.org/download/)         | 12.x - 16.x        | 12.x - 16.x            | | 
...
We are excited to announce Tyk Dashboard 5.5, featuring a brand-new dashboard identity, advanced rate-limiting capabilities, and enhanced security options. For a comprehensive list of changes, please refer to the [changelog](/5.13/#Changelog-v5.5.0) below.
...
In this release, we upgraded the Golang version to `v1.23` and fixed a [CVE-2025-21613](https://nvd.nist.gov/vuln/detail/CVE-2025-21613). For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.3.10) below.
...
The UI now displays a deprecation notice for the dedicated [External OAuth](/5.13/api-management/client-authentication#integrate-with-external-authorization-server-deprecated) and [OpenID Connect (OIDC)](/5.13/api-management/client-authentication#integrate-with-openid-connect-deprecated) authentication mechanisms. This provides advanced notification that these authentication options will be deprecated in version 5.7.0. Users are advised to migrate to the [JWT Auth](/5.13/basic-config-and-security/security/authentication-authorization/json-web-tokens) method, which supports integration with both OAuth and OIDC providers, in preparation for future upgrade.
</Expandable>
...
If you are upgrading to 5.3.7, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk). 
...
This release primarily focuses on bug fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.3.6) below.
...
If moving from a version of Tyk older than 5.3.0 please read the explanation provided with [5.3.0 release](/5.13/#TykOAS-v5.3.0).
...
With MongoDB 4.4 reaching [EOL](https://www.mongodb.com/legal/support-policy/lifecycles) in February 2024, we can no longer guarantee full compatibility with this version of the database. If you are [using MongoDB](/5.13/planning-for-production/database-settings#mongodb) we recommend that you upgrade to a version that we have tested with, as indicated [below](/5.13/#3rdPartyTools-v5.3.6).


With PostgreSQL v11 reaching [EOL](https://www.postgresql.org/support/versioning/) in November 2023, we can no longer guarantee full compatibility with this version of the database. If you are [using PostgreSQL](/5.13/planning-for-production/database-settings#postgresql) we recommend that you upgrade to a version that we have tested with, as indicated [below](/5.13/#3rdPartyTools-v5.3.6).
...
- [Docker Image to pull](https://hub.docker.com/r/tykio/tyk-dashboard/tags?page=&page_size=&ordering=&name=v5.3.6)
 - ```bash
   ...
   ```
- Helm charts
 - [tyk-charts v2.0](/5.13/developer-support/release-notes/helm-chart#2-0-0-release-notes)
...
There are no breaking changes in this release, however, if moving from a version of Tyk older than 5.3.0 please read the explanation provided with [5.3.0 release](/5.13/#TykOAS-v5.3.0).
...
#### Release Highlights
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.3.4) below.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.3.3) below.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.3.2) below.
...
#### Release Highlights
This release primarily focuses on bug fixes.
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.3.1) below.
...
- **Not Backwards Compatible**
  ...
  - This means that if you wish to downgrade or revert to your previous version of Tyk, you will need to restore these API definitions from a backup. Please go to the [backup](/5.13/#upgrade-instructions) section for detailed instructions on backup before upgrading to v5.3.0.
...
**Important:** Please go to the [backup](/5.13/#upgrade-instructions) section for essential instructions on how to backup before upgrading to v5.3.0
...
With MongoDB 4.4 reaching [EOL](https://www.mongodb.com/legal/support-policy/lifecycles) in February 2024, we can no longer guarantee full compatibility with this version of the database and recommend upgrading to a version that we have tested with, as indicated [below](/5.13/#3rdPartyTools-v5.3.0).
...
1. For Self Managed deployments - Backup Your environment using the [usual guidance](/5.13/developer-support/upgrading#tyk-upgrade-guides-for-different-deployment-models) documented with every release (this includes backup config file and database).
2. For all deployments - Backup all your API definitions (Tyk OAS API and Classic Definitions):
   - For Tyk Cloud deployments - To perform the backup please use our guide for [exporting APIs and policies](/5.13/developer-support/upgrading#backup-apis-and-policies).
   - For Self-Managed deployments -  To perform the backup please use [Tyk Sync](/5.13/api-management/automations/sync).
4. Performing the upgrade - For all deployments, follow the instructions in the [upgrade guide](/5.13/#upgrading-tyk) when upgrading Tyk.

#### Release Highlights

We are excited to announce the release of 5.3.0, packed with new features, improvements and bug fixes to enhance your experience with Tyk Dashboard. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.3.0) below.

##### Tyk OAS Feature Maturity

Tyk OAS is now out of [Early Access](/5.13/developer-support/release-types/early-access-feature) as we have reached feature maturity. You are now able to make use of the majority of Tyk's features from your Tyk OAS APIs, so they are a credible alternative to the legacy Tyk Classic APIs.
...
Tyk uses `mongo-go` as the default MongoDB driver from v5.3. This provides support for MongoDB 4.4.x, 5.0.x, 6.0.x and 7.0.x. If you are using older MongoDB versions e.g. 3.x, please set MongoDB driver to `mgo`. The [MongoDB supported versions](/5.13/planning-for-production/database-settings#supported-versions) page provides details on how to configure MongoDB drivers in Tyk.
</Accordion>
</AccordionGroup>
...
Relaxed the strict validation for mock response so that the `Description` field is now optional for `response`, `responses` and `schema` within the OpenAPI description. Automatically configuring mock responses when using [Tyk OAS APIs](/5.13/api-management/traffic-transformation/mock-response#tyk-oas) is now even easier.
</Accordion>
...
#### Release Highlights
Dashboard 5.2.5 was version bumped only, to align with Gateway 5.2.5. Subsequently, no changes were encountered in release 5.2.5. Gateway 5.2.5 was a critical patch release. For further information please see the release notes for Gateway [v5.2.5](/5.13/developer-support/release-notes/gateway) 
...
Fixed two UI issues with the [OPA editor](/5.13/api-management/dashboard-configuration#using-the-open-policy-agent-in-the-dashboard) in the Tyk Dashboard to improve experience when using this feature. Scrolling beyond the end of the OPA window does not now start to scroll the API Designer window, and minimizing then re-expanding the OPA editor no longer limits the text to one line.
</Accordion>
...
Fixed an issue where the [OPA editor](/5.13/api-management/dashboard-configuration#using-the-open-policy-agent-in-the-dashboard) was not resizable. The fix ensures the floating OPA editor is now resizable and the resizing operation is smooth, improving user experience.
</Accordion>

<Accordion title='User Search not working unless you enter the full email address'>
Fixed an issue where the [User Search](/5.13/api-management/user-management#search-users) was not working unless the full email address was entered. The fix restores the functionality of showing suggestions for names as they are typed in, improving user experience and search efficiency.
</Accordion>
...
Fixed an issue in the Classic API Designer where the 'use_standard_auth' value was constantly reverting to 'true' when editing an API with an [external OAuth flow](/5.13/api-management/client-authentication#integrate-with-external-authorization-server-deprecated). This fix ensures the 'use_standard_auth' value remains consistent, enabling the use of external OAuth via the Raw API editor.
</Accordion>
...
Added a new Dashboard configuration option `allow_unsafe_oas`. This permits the modification of Tyk OAS APIs via the Tyk Classic API endpoints. This is not a recommended action due to the risk of inconsistent behavior and potential for breaking changes while Tyk OAS is in [Early Access](/5.13/developer-support/release-types/early-access-feature). This is provided for early adopters and will be deprecated later, once Tyk OAS reaches full maturity.
</Expandable>
...
We're thrilled to bring you some exciting enhancements and crucial fixes to improve your experience with Tyk Dashboard. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.2.0) below.

Configure Caching Timeouts Per API Endpoint and Enable Advanced Caching Options From Within Dashboard

We’ve added the ability to [configure](/5.13/api-management/response-caching#configuring-the-middleware-in-the-tyk-oas-api-definition) per-endpoint timeouts for Tyk’s response cache, giving you increased flexibility to tailor your APIs to your upstream services. While doing this, we’ve also fixed a longstanding issue within the *Tyk Dashboard* so that you can configure more of the [advanced caching](/5.13/api-management/response-caching#configuring-the-middleware-in-the-api-designer) options from within the UI.

##### Added Body Transform Middleware to Tyk OAS API Definition

With this release, we are adding the much requested *Body Transformations* to *Tyk OAS API Definition*. You can now [configure](/5.13/api-management/gateway-config-tyk-oas#transformbody) middleware for both [request](/5.13/api-management/traffic-transformation/request-body) and [response](/5.13/api-management/traffic-transformation/response-body) *Body Transformations* and - as a *Tyk Dashboard* user - you’ll be able to do so from within our simple and elegant API Designer tool. Visually test and preview *Body Transformations* from within the API Designer.
...
Added a new [endpoint](/5.13/tyk-dashboard-api), */system/stats*, to provide insight and operational statistics on total and active APIs deployed. The endpoint's flexible date filtering options, equip users to obtain comprehensive insights into usage trends.
</Expandable>
...
documentation for a full description of this new [user role](/5.13/api-management/user-management#user-permissions).

##### Import API examples from within the Dashboard

In 5.0 we introduced the possibility to import API examples manually or via
[_Tyk Sync_](/5.13/api-management/automations/sync). We have now extended this feature and it is now possible to do this without
...
We added a [Dashboard API Endpoint](/5.13/api-management/data-graph#automatically-creating-rest-udg-configuration-based-on-oas-specification) that is capable of taking an OAS 3.0.0 document and converting it into a UDG API.
...
- Added a feature to display a full [_Tyk Sync_](/5.13/api-management/automations/sync) command that will allow a user to import an example UDG into their Dashboard
...
If you are upgrading to 5.0.15, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk). 
...
Go to the [Upgrading Tyk](/5.13/developer-support/release-notes/gateway#upgrading-tyk) section for detailed upgrade instructions.
...
See how to [Choose a MongoDB driver](/5.13/planning-for-production/database-settings#choose-a-mongodb-driver)
...
##### Release Highlights
This release primarily focuses on bug fixes. 
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.0.1) below.
...
We’ve improved support for [OAS Mock Responses](/5.13/api-management/traffic-transformation/mock-response#mock-response), with the Tyk OAS API definition now allowing you to register multiple Mock Responses in a single API, providing you with increased testing flexibility.
...
- via command line [using tyk-sync](/5.13/api-management/data-graph#udg-examples)
...
With this release we are also giving our users [improved headers for GQL APIs](/5.13/api-management/graphql#graphql-apis-headers). It is now possible to use context variables in request headers and persist headers needed for introspection separately for improved security.
...
- Now you can turn a Kafka topic into a GraphQL subscription by simply [importing your AsyncAPI definition](/5.13/api-management/dashboard-configuration#data-graphs-api)
...
[Importing OAS v3 via the Dashboard](/5.13/api-management/gateway-config-managing-oas#importing-an-openapi-description-to-create-an-api)
...
- Added option to configure certificates for Tyk Dashboard using [environment variables](/5.13/tyk-dashboard/configuration#http_server_options-certificates).
...
While SQL support for Tyk products does not depend on specific database features, with this release, we will provide official support for [PostgreSQL DB for production purposes](/5.13/planning-for-production/database-settings#postgresql), and SQLite for development and PoC environments. Note that SQL support is available for self-managed setups only.
...
[MongoDB to SQL migration docs](/5.13/planning-for-production/database-settings#migrating-from-an-existing-mongodb-instance)
...
Check our DCR docs [here](/5.13/tyk-developer-portal/tyk-portal-classic/dynamic-client-registration)
...
See our updated [Tutorials](/5.13/tyk-self-managed) section.
...
See updated tutorials on how to [create a policy](/5.13/api-management/gateway-config-managing-classic#secure-an-api) and [keys](/5.13/api-management/gateway-config-managing-classic#access-an-api)
...
See [updated flow details](/5.13/tyk-identity-broker/overview)
...
[Read More](/5.13/planning-for-production/ensure-high-availability/load-balancing#load-balancing)
...
If you are upgrading from version 2.8, please [read this guide](/5.13/developer-support/release-notes/archived#upgrading-from-version-2-8)
