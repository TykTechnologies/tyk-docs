...
For a comprehensive list of changes, please refer to the detailed [changelog](#Changelog-v2.2.2) below.
...
In this release, we have added support for the Client Identity Provider (IdP) registry, introduced in Tyk 5.14.0. This feature provides centralized management of IdPs and scope-to-policy mappings used with [JWT Auth](/5.13/basic-config-and-security/security/authentication-authorization/json-web-tokens). Adding support to Tyk Sync allows GitOps management of the new Client IdP resource.

<Note>
Client IdP registry support requires Tyk Gateway and Tyk Dashboard v5.14.0 or higher. On Tyk v5.13.x LTS deployments, `tyk-sync dump` skips IdP definitions silently, and `tyk-sync sync` fails if your repository contains any `clientidp-*.json` files.
</Note>
...
#### Release Highlights
This release refreshes the embedded Tyk Classic, Tyk OAS API, and Policy definitions so that all fields introduced in Tyk Gateway 5.13.0 are correctly transferred during sync operations. It also ensures compatibility with the most recent Tyk LTS release [5.8.14](/5.13/developer-support/release-notes/dashboard#5-8-14-release-notes).
...
#### Release Highlights
Tyk Sync 2.1.7 has updated the Golang version to 1.25, fixed CVEs and a performance issue, and ensures compatibility with the most recent Tyk LTS release [5.8.13](/5.13/developer-support/release-notes/dashboard#5-8-13-release-notes).
...
Tyk Sync recognises [MCP proxies](/5.13/ai-management/mcp-gateway/managing-proxies) as a distinct resource type.
...
#### Release Highlights
Tyk Sync 2.1.6 is a version alignment release that ensures compatibility with the most recent Tyk LTS release [5.8.12](/5.13/developer-support/release-notes/dashboard#5-8-12-release-notes).
...
#### Release Highlights
Tyk Sync 2.1.5 is a version alignment release that ensures compatibility with the most recent Tyk LTS release [5.8.9](/5.13/developer-support/release-notes/dashboard#5-8-9-release-notes). It contains one bug fix.
...
#### Release Highlights
Tyk Sync 2.1.4 is a version alignment release that ensures compatibility with the most recent Tyk LTS release [5.8.6](/5.13/developer-support/release-notes/dashboard#5-8-6-release-notes). No functional changes have been implemented in this release.
...
This patch release uplifts a dependent library for better interoperability with the rest of the Tyk stack. Please refer to the [changelog](#Changelog-v2.1.2) below for detailed explanation.
...
Recommended usage:
Tyk Dashboard setting: [allow-unsafe-oas](/5.13/tyk-dashboard/configuration#allow_unsafe_oas)
Tyk Sync: use the [--allow-unsafe-oas](/5.13/api-management/automations/sync) when invoking the CLI
