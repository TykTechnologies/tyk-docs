...
Only the most recent Tyk Gateway release and the most recent on the [Long Term Support (LTS)](/5.13/developer-support/release-types/long-term-support) branch are supported. We recommend staying current on the feature or LTS branch to minimise exposure to security vulnerabilities from third party dependencies.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.13.2) below.
...
| :--------------------------------------- | :------------------ | :------------------ | :------- | 
| [Golang](https://go.dev/dl/)             | 1.26                |  1.26               | [Go plugins](/5.13/api-management/plugins/golang) must be built using Go 1.26 | 
...
| [OpenAPI Specification](https://spec.openapis.org/oas/v3.1.2.html) | 3.1.x, 3.0.x | 3.1.x, 3.0.x | Supported by [Tyk OAS](/5.13/api-management/gateway-config-tyk-oas)|
...
If you are upgrading to 5.13.2, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
- [Docker Image to pull](https://hub.docker.com/r/tykio/tyk-gateway/tags?page=&page_size=&ordering=&name=v5.13.2)
  - ```bash
    ...
    ```
- Helm charts
  - [tyk-charts](/5.13/developer-support/release-notes/helm-chart)
...
For more details see [Golang Plugin Compiler](/5.13/api-management/plugins/golang#plugin-compiler).
</Accordion>
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.13.1) below.
...
**Mitigation:** A new `legacy` option has been added to the [`log_format`](/5.13/tyk-oss-gateway/configuration#log_format) configuration. This setting preserves the format used in Tyk Gateway 5.13.0 and earlier, with the historical timestamp format and lack of a field identifying for the originating API.
...
|         | Helm Chart v5.2.1 | Helm all versions |
...
| Third Party Dependency                   | Tested Versions     | Compatible Versions | Comments | 
| :--------------------------------------- | :------------------ | :------------------ | :------- | 
| [Golang](https://go.dev/dl/)             | 1.25                |  1.25               | [Go plugins](/5.13/api-management/plugins/golang) must be built using Go 1.25 | 
...
If you are upgrading to 5.13.1, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
The [`log_format`](/5.13/tyk-oss-gateway/configuration#log_format) configuration option now accepts three explicit values:
...
{/* TT-4023 */}
When the Gateway control API is moved to a separate port via [`control_api_port`](/5.13/tyk-oss-gateway/configuration#control_api_port), requests to the proxy/data API port that did not match any configured API (generating an `HTTP 404` response) were silently dropped: no log entry was generated even with [`track_404_logs`](/5.13/tyk-oss-gateway/configuration#track_404_logs) enabled. This has been fixed; 404 logs are now correctly emitted for unmatched requests on all listening ports.

Additionally, a `host` field has been added to 404 log entries (when using `text` or `json` [`log_format`](/5.13/tyk-oss-gateway/configuration#log_format)). This records the Host header from the request, allowing users to distinguish whether the unmatched request was directed at the proxy API or the control API.
</Accordion>
...
- [`regexp_cache_max_entries`](/5.13/tyk-oss-gateway/configuration#regexp_cache_max_entries)  caps the regular-expression cache (default: `5000`)
- [`disable_regexp_cache_bound`](/5.13/tyk-oss-gateway/configuration#disable_regexp_cache_bound) restores the previous unbounded behaviour for deployments with a naturally limited set of patterns (default:`false`)
- [`disable_auto_max_procs`](/5.13/tyk-oss-gateway/configuration#disable_auto_max_procs) restores the previous default behavior of GOMAXPROCS (default:`false`)
</Accordion>
...
Transform your API's error experience with the new comprehensive [error override](/5.13/api-management/custom-error-responses#error-overrides) system. This feature provides complete control over HTTP error responses from both the Gateway and upstream services, enabling standardized, branded error formats that guide users toward solutions rather than leaving them confused.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.13.0) below.
...
You must set [http_server_options.max_version](/5.13/tyk-oss-gateway/configuration#http_server_options-max_version) (or the equivalent environment variable)  to `771` if you require an upper limit of TLS 1.2.

  See [here](/5.13/api-management/implement-tls#controlling-tls-version-and-cipher-suites) for details how to control TLS version and cipher suites.
...
If you are upgrading to 5.13.0, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).

If your deployment uses MCP Gateway, additional component ordering applies. In distributed deployments, MDCB 2.11.0 must be deployed before Gateway and Dashboard. Tyk Operator 1.4.0 must be deployed after Gateway and Dashboard are running. See [MCP Gateway upgrade considerations](/5.13/developer-support/upgrading#mcp-upgrade-considerations) for the full sequence.
...
For configuration details, see the [MCP Gateway documentation](/5.13/ai-management/mcp-gateway/overview).
</Accordion>
...
For configuration details, see the [MCP proxy policies documentation](/5.13/ai-management/mcp-gateway/policies).
</Accordion>
...
For details, see the [MCP access logs documentation](/5.13/ai-management/mcp-gateway/mcp-access-logs).
</Accordion>
...
For details, see the [MCP metrics documentation](/5.13/ai-management/mcp-gateway/mcp-metrics).
</Accordion>
...
**Configuration:** Error overrides use a map-based structure indexed by HTTP status code in the new [error_overrides](/5.13/tyk-oss-gateway/configuration#error_overrides) configuration.
...
This release adds a new Gateway configuration option [rate_limit_response_headers](/5.13/tyk-oss-gateway/configuration#rate_limit_response_headers). When set to "rate_limits", the `X-RateLimit-*` headers are populated from rate-limit data instead of quota data:
...
We have added a new, simplified, approach to configuring [Session lifetime](/5.13/api-management/access-control/sessions-and-keys/session-lifecycle) within Redis.
...
- [post_expiry_action](/5.13/api-management/access-control/sessions-and-keys/session-lifecycle#2-define-the-post-expiry-action) - determines what happens to the data in Redis after the `expires` timestamp is reached.
- [post_expiry_grace_period](/5.13/api-management/access-control/sessions-and-keys/session-lifecycle#3-configure-the-grace-period) - defines how long (in seconds) the Session is kept in Redis after expiration (if the `post_expiry_actions` is to retain the Session)

The existing Gateway-wide [global session lifetime](/5.13/api-management/access-control/sessions-and-keys/session-lifecycle#gateway-level-settings) override is still respected.

The [legacy API level controls](/5.13/api-management/access-control/sessions-and-keys/session-lifecycle#legacy-controls) can still be used if both new fields are set to `0` (or unset) so **there is no change in behavior for existing Sessions**.
</Accordion>
...
Compression is opt-in per asset type via independent configuration flags: [`storage.compress_api_definitions`](/5.13/tyk-oss-gateway/configuration#storage-compress_api_definitions) (introduced in 5.12.0) for API definitions, and the new [`storage.compress_policies`](/5.13/tyk-oss-gateway/configuration#storage-compress_policies) for security policies. Both are disabled by default and remain backward compatible — Gateways transparently load both compressed and uncompressed data from Redis.

The decompression size limit is now configurable via the new [`storage.max_decompressed_size`](/5.13/tyk-oss-gateway/configuration#storage-max_decompressed_size) option (default 100MB, minimum 1MB) and is applied independently to API definitions and policies, so each asset type has its own ceiling. As with the original feature, compression and decompression occur only during Gateway reloads, with no impact on the request hot path.
</Accordion>
...
**Configuration:** Metrics are configured under the new [opentelemetry.metrics](/5.13/tyk-oss-gateway/configuration#opentelemetry-metrics) object with independent control from tracing.
...
<Note>
It is important to note that **query parameters provided with the original request are no longer automatically forwarded**. You must update your [URL rewrite configuration](/5.13/transform-traffic/url-rewriting) to explicitly include any required parameters in the `rewrite_to` URL.
</Note>
</Accordion>
...
To change the maximum TLS version, you must explicitly set [TYK_GW_HTTPSERVEROPTIONS_MAXVERSION](/5.13/tyk-oss-gateway/configuration#http_server_options-max_version) for client-to-Gateway connections or [TYK_GW_PROXYSSLMAXVERSION](/5.13/tyk-oss-gateway/configuration#proxy_ssl_max_version) for Gateway-to-upstream connections.

To change the minimum TLS version, you must explicitly set [TYK_GW_HTTPSERVEROPTIONS_MINVERSION](/5.13/tyk-oss-gateway/configuration#http_server_options-min_version) for client-to-Gateway connections or [TYK_GW_PROXYSSLMINVERSION](/5.13/tyk-oss-gateway/configuration#proxy_ssl_min_version) for Gateway-to-upstream connections.

For full details of TLS version configuration see [here](/5.13/api-management/implement-tls#controlling-tls-version-and-cipher-suites).
</Accordion>
...
Now OpenTelemetry can be configured via the [`opentelemetry`](/5.13/tyk-oss-gateway/configuration#opentelemetry) section in the Gateway config file (including `enabled`, `exporter`, and `endpoint` fields) or their equivalent environment variables.
</Accordion>
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.12.1) below.
...
| Third Party Dependency | Tested Versions | Compatible Versions | Comments | 
| ---------------------- | --------------- | ------------------- | -------- | 
| [Go](https://go.dev/dl/)               | 1.25                   |  1.25  | [Go plugins](/5.13/api-management/plugins/golang) must be built using Go 1.25 | 
| [Redis](https://redis.io/download/)    | 6.2.x, 7.x, 7.4.x      | 6.2.x, 7.x, 7.4.x      | | 
| [Valkey](https://valkey.io/download/)  | 7.2.x, 8.0.x, 8.1.x    | 7.2.x, 8.0.x, 8.1.x    | | 
| [OpenAPI Specification](https://spec.openapis.org/oas/v3.1.2.html) | 3.1.x, 3.0.x | 3.1.x, 3.0.x | Supported by [Tyk OAS](/5.13/api-management/gateway-config-tyk-oas)|
...
If you are upgrading to 5.12.1, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
- [Docker image to pull](https://hub.docker.com/r/tykio/tyk-gateway/tags?page=&page_size=&ordering=&name=v5.12.1)
  - ```bash
    ...
    ``` 
- Helm charts
  - [tyk-charts v5.1.0](/5.13/developer-support/release-notes/helm-chart#5-1-0-release-notes)
...
We do not yet have support for all new features. For more details, see the [documentation](/5.13/api-management/gateway-config-tyk-oas#openapi-specification-3-1)
...
Troubleshooting API errors in production just got significantly faster. We've enhanced Gateway [access logs](/5.13/api-management/logs#access-logs) to include rich, structured error context for 4XX and 5XX errors, eliminating the need to cross-reference multiple log sources during an incident.
...
To provide an additional layer of security for your APIs, we've introduced [Client Certificate-Token Binding](/5.13/api-management/authentication/bearer-token#client-certificate-token-binding). This feature allows you to form a strict binding association between an Auth Token issued to a client and their specific client certificate.
...
We have restructured [Certificate Authentication](/5.13/api-management/authentication/certificate-auth) (formerly known as Dynamic mTLS) to be a dedicated, standalone authentication method in Tyk OAS API definitions. 
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.12.0) below.

#### Breaking Changes

A change has been made to improve security of the legacy [**Auth Token + Dynamic mTLS**](/5.13/api-management/implement-tls#legacy-dynamic-mtls-mode) method for securing access to APIs deployed on Tyk. This removes the option to authenticate using only the auth token and enforces the mTLS handshake.
...
| Third Party Dependency | Tested Versions | Compatible Versions | Comments | 
| ---------------------- | --------------- | ------------------- | -------- | 
| [Go](https://go.dev/dl/)               | 1.24                   |  1.24  | [Go plugins](/5.13/api-management/plugins/golang) must be built using Go 1.24 | 
| [Redis](https://redis.io/download/)    | 6.2.x, 7.x, 7.4.x      | 6.2.x, 7.x, 7.4.x      | | 
| [Valkey](https://valkey.io/download/)  | 7.2.x, 8.0.x, 8.1.x    | 7.2.x, 8.0.x, 8.1.x    | | 
| [OpenAPI Specification](https://spec.openapis.org/oas/v3.1.2.html) | 3.1.x, 3.0.x | 3.1.x, 3.0.x | Supported by [Tyk OAS](/5.13/api-management/gateway-config-tyk-oas)|
...
- Added the `trace_id` field to Gateway [access logs](/5.13/api-management/logs#access-logs) when OpenTelemetry is enabled, matching the `X-Tyk-Trace-Id` response header.
- Added `trace_id` and `span_id` fields to all request-scoped Gateway [application logs](/5.13/api-management/logs#application-logs) (middleware execution, errors, and debugging).
...
The full list of response flag codes is available in the [Access Logs documentation](/5.13/api-management/logs#access-logs).
</Accordion>
...
This feature is disabled by default and is enabled using [`enable_config_inspection`](/5.13/tyk-oss-gateway/configuration#enable_config_inspection).
</Accordion>
...
This release introduces a usage-aware certificate synchronization system for distributed deployments (MDCB). Data Planes can now be configured to only sync and store certificates that are actually required by their loaded APIs when using the [MDCB Synchroniser](/5.13/api-management/mdcb#mdcb-synchroniser), rather than pulling all certificates from the Control Plane:
...
These inconsistencies could cause the [Request Validation](/5.13/api-management/traffic-transformation/request-validation) and [Mock Response](/5.13/api-management/traffic-transformation/mock-response#mock-response) middleware to be skipped in certain scenarios when using Tyk OAS APIs.
...
| [Redis](https://redis.io/download/)    | 6.2.x, 7.x, 7.4.x      | 6.2.x, 7.x, 7.4.x      | | 
| [Valkey](https://valkey.io/download/)  | 7.2.x, 8.0.x, 8.1.x    | 7.2.x, 8.0.x, 8.1.x    | | 
| [OpenAPI Specification](https://spec.openapis.org/oas/v3.0.3)| v3.0.x  | v3.0.x | Supported by [Tyk OAS](/5.13/api-management/gateway-config-tyk-oas) |
...
If you are upgrading to 5.11.1, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
- [Docker image to pull](https://hub.docker.com/r/tykio/tyk-gateway/tags?page=&page_size=&ordering=&name=v5.11.1)
  - ```bash
    ...
    ``` 
- Helm charts
  - [tyk-charts v5.0.0](/5.13/developer-support/release-notes/helm-chart#5-0-0-release-notes)
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.11.0).
...
In this release we've deprecated the [policies.allow_explicit_policy_id](/5.13/tyk-oss-gateway/configuration#policies-allow_explicit_policy_id) configuration option. This was previously added to allow the use of custom policy IDs, which is now the default behaviour so this option is redundant.

#### Upgrade instructions
<a id="upgrade-5.11.0"></a>

If you are upgrading to 5.11.0, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
If you are upgrading to 5.10.2, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
- [Docker image to pull](https://hub.docker.com/r/tykio/tyk-gateway/tags?page=&page_size=&ordering=&name=v5.10.2)
  - ```bash
    ...
    ``` 
- Helm charts
  - [tyk-charts v4.0.0](/5.13/developer-support/release-notes/helm-chart#4-1-0-release-notes)
...
This patch release contains various bug and security fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.10.1).
...
If you are upgrading to 5.10.1, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
- [Docker image to pull](https://hub.docker.com/r/tykio/tyk-gateway/tags?page=&page_size=&ordering=&name=v5.10.1)
  - ```bash
    ...
    ``` 
- Helm charts
  - [tyk-charts v4.0.0](/5.13/developer-support/release-notes/helm-chart#4-0-0-release-notes)
...
Fixed an issue where [Custom Authentication](/5.13/api-management/authentication/custom-auth) could fall back to a previously configured alternative authentication method if the custom plugin bundle was not loaded. Now this is treated as for any other failed plugin load, and requests to the API will be rejected with `HTTP 500 Internal Server Error` to prevent access to an improperly configured endpoint.
</Accordion>
...
Added a new configuration option, [HttpServerOptions.MaxResponseBodySize](/5.13/tyk-oss-gateway/configuration#http_server_options-max_response_body_size) to limit the maximum size of the response bodies processed during any response body transformations.  When the limit is exceeded, the Gateway returns `HTTP 500 Response Body Too Large` instead of attempting to process the oversized content.
</Accordion>
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.10.0).
...
For more details, please see the dedicated [Multi Auth](/5.13/basic-config-and-security/security/authentication-authorization/multiple-auth) section.
...
For more details, please see the dedicated [JWT Auth](/5.13/api-management/authentication/jwt-authorization) section.
...
For more details, please see the [JWT Auth](/5.13/basic-config-and-security/security/authentication-authorization/json-web-tokens) section.
...
For more details, please see the dedicated [section](/5.13/configure/external-service).
...
For more details, please see the dedicated [Gateway events](/5.13/api-management/gateway-events) section.
...
If you are upgrading to 5.10.0, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
Added a new `external_services` section in the [Gateway configuration](/5.13/configure/external-service) to provide centralized configuration for proxy settings and mTLS certificates when communicating with external services. This includes connections to persistent and temporal storage, OAuth 2.0 Authorization Servers, and webhook targets.
...
Introduced a proactive event system to warn administrators when mTLS certificates are approaching expiry. The Gateway now emits two new [API events](/5.13/api-management/gateway-events#api-events) to provide visibility into certificate status:
...
Fixed an issue where Tyk OAS mock response middleware failed to execute when internal API proxying was enabled. Mock responses configured in the target API are now correctly returned when a request is redirected to another API on the same Tyk Gateway instance via [internal looping](/5.13/advanced-configuration/transform-traffic/looping).
</Accordion>
...
This is a version bump to align with Dashboard v5.9.2, no changes have been implemented in this release. For further information, please see the release notes for Dashboard [v5.9.2](/5.13/developer-support/release-notes/dashboard#5-9-2-release-notes).
...
| Third Party Dependency | Tested Versions | Compatible Versions | Comments | 
| :---------------------- | :--------------- | :------------------- | :-------- | 
| [Go](https://go.dev/dl/)               | 1.23                   |  1.23  | [Go plugins](/5.13/api-management/plugins/golang) must be built using Go 1.23 | 
| [Redis](https://redis.io/download/)    | 6.2.x, 7.x, 7.4.x      | 6.2.x, 7.x, 7.4.x      | | 
| [Valkey](https://valkey.io/download/)  | 7.2.x, 8.0.x, 8.1.x    | 7.2.x, 8.0.x, 8.1.x    | | 
| [OpenAPI Specification](https://spec.openapis.org/oas/v3.0.3)| v3.0.x  | v3.0.x | Supported by [Tyk OAS](/5.13/api-management/gateway-config-tyk-oas) |
...
If you are upgrading to 5.9.2, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.9.1).
...
If you are upgrading to 5.9.1, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
This release builds on the recent release of [Tyk 5.8.3](/5.13/developer-support/release-notes/gateway#5-8-3-release-notes), adding a collection of new capabilities. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.9.0).
...
We've added support for additional processors, inputs and outputs for [Tyk Streams event driven APIs](/5.13/api-management/event-driven-apis), extending the flexibility of this powerful feature.
...
A breaking change has been identified in Tyk 5.9.0 regarding [URL rewrite rules](/5.13/transform-traffic/url-rewriting). The `negate` field, which was optional in previous versions, is now mandatory in all URL rewrite rule configurations.

##### What Changed

In Tyk 5.8.2 and earlier, the `negate` field in [URL rewrite rules](/5.13/transform-traffic/url-rewriting) included an omitempty tag, making it optional in JSON. If not provided, it would default to false
...
If you are upgrading to 5.9.0, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.8.15) below.
...
**Mitigation:** A new `legacy` option has been added to the [`log_format`](/5.13/tyk-oss-gateway/configuration#log_format) configuration. This setting preserves the format used prior to Tyk Gateway 5.8.15, with the historical timestamp format and lack of a field identifying for the originating API.
...
If you are upgrading to 5.8.15, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
- [Docker Image to pull](https://hub.docker.com/r/tykio/tyk-gateway/tags?page=&page_size=&ordering=&name=v5.8.15)
  - ```bash
    ...
    ```
- Helm charts
  - [tyk-charts v5.2.0](/5.13/developer-support/release-notes/helm-chart#5-2-0-release-notes)
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.8.14) below.
...
If you are upgrading to 5.8.14, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
We have resolved inconsistent query parameter handling in URL rewrites when [internally routing](/5.13/advanced-configuration/transform-traffic/looping) using the `tyk://` scheme. Previously, custom query parameters specified in the `rewrite_to` URL were silently dropped, while original request parameters were unexpectedly preserved.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.8.13).
...
If you are upgrading to 5.8.13, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.8.12).
...
If you are upgrading to 5.8.12, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
These inconsistencies could see the [Request Validation](/5.13/api-management/traffic-transformation/request-validation) and [Mock Response](/5.13/api-management/traffic-transformation/mock-response#mock-response) middleware being skipped in certain scenarios when using Tyk OAS APIs.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.8.11).
...
If you are upgrading to 5.8.11, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
This patch release addresses a CVE present in 5.8.9 and fixes an issue where API keys remained active when set to inactive status. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.8.10).
...
If you are upgrading to 5.8.10, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
This patch release contains various bug fixes and introduces enhanced DNS monitoring for MDCB deployments. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.8.9).
...
If you are upgrading to 5.8.9, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
<Accordion title='Resolved Panic Triggered by DRL Updates in Mixed Rate-Limiter Environments'>
Fixed an issue where Gateways using [Redis Rate Limiters](/5.13/api-management/rate-limit#redis-rate-limiter) would crash when sharing Redis with Gateways using [Distributed Rate Limiting](/5.13/api-management/rate-limit#distributed-rate-limiter) (DRL). Non-DRL Gateways now properly ignore DRL update messages instead of attempting to process them, enabling mixed rate-limiter deployments across shared Redis instances.
</Accordion>
...
This patch release fixes some high-priority CVEs. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.8.8).
...
If you are upgrading to 5.8.8, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
This patch release contains various bug fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.8.7).
...
If you are upgrading to 5.8.7, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
This patch release contains various bug fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.8.6).
...
If you are upgrading to 5.8.6, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
Gateway 5.8.5 was version bumped only to align with Dashboard 5.8.5. Subsequently, no changes were encountered in release 5.8.5. For further information, please see the release notes for Dashboard [v5.8.5](/5.13/developer-support/release-notes/dashboard#5-8-5-release-notes).
...
If you are upgrading to 5.8.5, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
- [Docker image to pull](https://hub.docker.com/r/tykio/tyk-gateway/tags?page=&page_size=&ordering=&name=v5.8.5)
  - ```bash
    ...
    ``` 
- Helm charts
  - [tyk-charts v3.0.0](/5.13/developer-support/release-notes/helm-chart#3-0-0-release-notes)
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.8.4).
...
If you are upgrading to 5.8.4, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
This patch release contains various bug fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.8.3) below.
...
A breaking change has been identified in Tyk 5.8.3 regarding [URL rewrite rules](/5.13/transform-traffic/url-rewriting). The `negate` field, which was optional in previous versions, is now mandatory in all URL rewrite rule configurations.
...
If you are upgrading to 5.8.3, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
Fixed support for `dns:///` protocol for load balancing when using [gRPC plugins](/5.13/api-management/plugins/rich-plugins#load-balancing-between-grpc-servers). Setting the new configuration option `TYK_GW_COPROCESSOPTIONS_GRPCROUNDROBINLOADBALANCING` to `true` will cause Tyk to balance the load between multiple gRPC servers; the default behavior (`false`) is to use a sticky connection to a single server.
</Accordion>
...
This patch release contains fixes to some bugs experienced when using MDCB and distributed data planes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.8.2) below.
...
If you are upgrading to 5.8.2, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
This patch release contains various bug fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.8.1) below.
...
If you are upgrading to 5.8.1, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
Fixed an issue where an [enforced timeout](/5.13/planning-for-production/ensure-high-availability/enforced-timeouts) set for a specific API endpoint could be overruled by the configured [proxy_default_timeout](/5.13/tyk-oss-gateway/configuration#proxy_default_timeout). Now if an endpoint-level timeout is set then this will be honoured, regardless of any default timeout that is configured.
</Accordion>
...
With Tyk 5.8.0 we are delighted to unlock the power and flexibility of Tyk OAS for all users, with full feature parity with the legacy Tyk Classic API definition. We are also bringing other updates and improvements, delivering more control, flexibility, and performance. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.8.0) below.
...
If you are upgrading to 5.8.0, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
This patch release contains a bug fix. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.7.3) below.
...
| Third Party Dependency                                       | Tested Versions        | Compatible Versions    | Comments | 
| :------------------------------------------------------------ | :---------------------- | :---------------------- | :-------- | 
| [Go](https://go.dev/dl/)                                     | 1.22  |  1.22  | [Go plugins](/5.13/api-management/plugins/golang) must be built using Go 1.22 | 
| [Redis](https://redis.io/download/)  | 6.2.x, 7.x  | 6.2.x, 7.x  | Used by Tyk Gateway | 
| [OpenAPI Specification](https://spec.openapis.org/oas/v3.0.3)| v3.0.x                 | v3.0.x                 | Supported by [Tyk OAS](/5.13/api-management/gateway-config-tyk-oas) |
...
If you are upgrading to 5.7.3, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
- [Docker image to pull](https://hub.docker.com/r/tykio/tyk-gateway/tags?page=&page_size=&ordering=&name=v5.7.3)
  - ```bash
    ...
    ``` 
- Helm charts
  - [tyk-charts v2.2.0](/5.13/developer-support/release-notes/helm-chart#2-2-0-release-notes)
...
This patch release contains a bug fix. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.7.2) below.
...
#### Upgrade instructions
If you are upgrading to 5.7.2, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
This release focuses mainly on bug fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.7.1) below.
...
#### Upgrade instructions
If you are upgrading to 5.7.1, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
We are thrilled to announce new updates and improvements in Tyk 5.7.0, bringing more control, flexibility, and performance.  For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.7.0) below.
...
#### Deprecations
{/* Required. Use the following statement if there are no deprecations, or explain if there are */}
In 5.7.0, we have deprecated the dedicated [External OAuth](/5.13/api-management/client-authentication#integrate-with-external-authorization-server-deprecated)  (Tyk Classic: `external_oauth`, Tyk OAS: `server.authentication.securitySchemes.externalOAuth`) and [OpenID Connect](/5.13/api-management/client-authentication#integrate-with-openid-connect-deprecated)  (Tyk Classic: `auth_configs.oidc`, Tyk OAS: `server.authentication.oidc`) authentication methods. We advise users to switch to [JWT Authentication](/5.13/basic-config-and-security/security/authentication-authorization/json-web-tokens).


#### Upgrade instructions
If you are upgrading to 5.7.0, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-vX.Y.0) below. */}
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.6.1) below.
...
If you are upgrading to 5.6.1, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
- Helm charts
  ...
  - [tyk-charts v2.1.0](/5.13/developer-support/release-notes/helm-chart#2-1-0-release-notes)
...
[changelog](/5.13/#Changelog-v5.6.0) below.

##### Per endpoint Rate Limiting for clients

Building on the [per-endpoint upstream rate
limits](/5.13/api-management/rate-limit#api-level-rate-limiting) introduced in Tyk 5.5.0 we have
now added [per-endpoint client
rate limits](/5.13/api-management/rate-limit#key-level-rate-limiting). This new feature allows
...
If you are upgrading to 5.6.0, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
rate limits](/5.13/api-management/rate-limit#key-level-rate-limiting). This new feature
...
| Third Party Dependency                                       | Tested Versions        | Compatible Versions    | Comments | 
| :------------------------------------------------------------ | :---------------------- | :---------------------- | :-------- | 
| [Go](https://go.dev/dl/)                                     | 1.21  |  1.21  | [Go plugins](/5.13/api-management/plugins/golang#) must be built using Go 1.21 | 
| [Redis](https://redis.io/download/)  | 6.2.x, 7.x  | 6.2.x, 7.x  | Used by Tyk Gateway | 
| [OpenAPI Specification](https://spec.openapis.org/oas/v3.0.3)| v3.0.x                 | v3.0.x                 | Supported by [Tyk OAS](/5.13/api-management/gateway-config-tyk-oas) |
...
#### Upgrade instructions
If you are upgrading to 5.5.2, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
- [Docker image to pull](https://hub.docker.com/r/tykio/tyk-gateway/tags?page=&page_size=&ordering=&name=v5.5.2)
  - ```bash
    ...
    ``` 
- Helm charts
  - [tyk-charts v2.0.0](/5.13/developer-support/release-notes/helm-chart#2-0-0-release-notes)
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.5.1) below.
...
#### Upgrade instructions
If you are upgrading to 5.5.1, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
We have introduced two new options in the `http_server_options` [Gateway configuration](/5.13/tyk-oss-gateway/configuration#http_server_options) that will enforce prefix and/or suffix matching when Tyk performs checks on whether middleware or other logic should be applied to a request:
...
This release primarily focuses on bug fixes.
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-vX.Y.0) below. */}
We are thrilled to introduce Tyk Gateway 5.5, bringing advanced rate-limiting capabilities, enhanced certificate authentication, and performance optimizations. For a comprehensive list of changes, please refer to the [changelog](/5.13/#Changelog-v5.5.0) below.

##### Per Endpoint Rate Limiting

Now configure rate limits at the endpoint level for both [Tyk OAS](/5.13/api-management/rate-limit#tyk-oas-api-definition) and [Tyk Classic APIs](/5.13/api-management/rate-limit#tyk-classic-api-definition), providing granular protection for upstream services against overloading and abuse.

##### Root CA Support for Client Certificates

Simplify certificate management with support for root Certificate Authority (CA) certificates, enabling clients to authenticate using certificates signed by the [configured root CA](/5.13/api-management/implement-tls#frequently-asked-questions).
...
#### Upgrade instructions
If you are upgrading to 5.5.0, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
- [Docker image to pull](https://hub.docker.com/r/tykio/tyk-gateway/tags?page=&page_size=&ordering=&name=v5.5.0)
  - ```bash
    ...
    ``` 
- Helm charts
  - [tyk-charts v1.6](/5.13/developer-support/release-notes/helm-chart#1-6-0-release-notes)
...
| Third Party Dependency                                       | Tested Versions        | Compatible Versions    | Comments | 
| :------------------------------------------------------------ | :---------------------- | :---------------------- | :-------- | 
| [Go](https://go.dev/dl/)                                     | 1.19 (GQL), 1.21 (GW)  | 1.19 (GQL), 1.21 (GW)  | [Go plugins](/5.13/api-management/plugins/golang#) must be built using Go 1.21 | 
| [Redis](https://redis.io/download/)  | 6.2.x, 7.x  | 6.2.x, 7.x  | Used by Tyk Gateway | 
| [OpenAPI Specification](https://spec.openapis.org/oas/v3.0.3)| v3.0.x                 | v3.0.x                 | Supported by [Tyk OAS](/5.13/api-management/gateway-config-tyk-oas) |
...
#### Upgrade instructions
If you are upgrading to 5.4.0, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
We've introducing a [Rate Limit Smoothing](/5.13/api-management/rate-limit#rate-limit-smoothing) option for the spike arresting Redis Rate Limiter to give the upstream time to scale in response to increased request rates.
...
###### Action Required
Customers should clear their edge Redis instances of any potentially affected keys to maintain data consistency and ensure proper synchronization across their environments. Please refer to the item in the [fixed](/5.13/#fixed) section of the changelog for recommended actions.

##### Fixed Window Rate Limiter

Ideal for persistent connections with load-balanced gateways, the [Fixed Window Rate Limiter](/5.13/api-management/rate-limit#fixed-window-rate-limiter) algorithm mechanism ensures fair handling of requests by allowing only a predefined number to pass per rate limit window. It uses a simple shared counter in Redis so requests do not need to be evenly balanced across the gateways.

##### Event handling with Tyk OAS

We’ve added support for you to [register webhooks](/5.13/api-management/gateway-events#event-handling-with-webhooks) with your Tyk OAS APIs so that you can handle events triggered by the Gateway, including circuit breaker and quota expiry. You can also assign webhooks to be fired when using the new [smoothing rate limiter](/5.13/api-management/rate-limit#rate-limit-smoothing) to notify your systems of ongoing traffic spikes.
...
- [Docker image to pull](https://hub.docker.com/r/tykio/tyk-gateway/tags?page=&page_size=&ordering=&name=v5.4.0)
  - ```bash
    ...
    ``` 
- Helm charts
  - [tyk-charts v1.5](/5.13/developer-support/release-notes/helm-chart#1-5-0-release-notes)
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.3.13) below.
...
| Third Party Dependency                                     | Tested Versions        | Compatible Versions    | Comments | 
| :---------------------------------------------------------- | :---------------------- | :---------------------- | :-------- | 
| [GoLang](https://go.dev/dl/)                               | 1.24       | 1.24       | [Go plugins](/5.13/api-management/plugins/golang) must be built using Go 1.24 | 
| [Redis](https://redis.io/download/)  | 6.2.x, 7.x  | 6.2.x, 7.x  | Used by Tyk Gateway | 
| [OpenAPI Specification](https://spec.openapis.org/oas/v3.0.3) | v3.0.x      | v3.0.x          | Supported by [Tyk OAS](/5.13/api-management/gateway-config-tyk-oas#tyk-vendor-extension-reference)|
...
If you are upgrading to 5.3.13, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
This patch release contains bug fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.3.12) below.
...
If you are upgrading to 5.3.12, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
This patch release contains various bug fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.3.11) below.
...
If you are upgrading to 5.3.11, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
In this release, we upgraded the Golang version to `v1.23` for security enhancement and fixed an API authentication issue with redirects. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.3.10) below.
...
| Third Party Dependency                                        | Tested Versions       | Compatible Versions   | Comments                                                                                   |
| :------------------------------------------------------------- | :--------------------- | :--------------------- | :------------------------------------------------------------------------------------------ |
| [Go](https://go.dev/dl/)                                      |  1.23 (GW)            |  1.23 (GW)            | [Go plugins](/5.13/api-management/plugins/golang) must be built using Go 1.23 |
| [Redis](https://redis.io/download/)                           | 6.2.x, 7.x            | 6.2.x, 7.x            | Used by Tyk Gateway                                                                        |
| [OpenAPI Specification](https://spec.openapis.org/oas/v3.0.3) | v3.0.x                | v3.0.x                | Supported by [Tyk OAS](/5.13/api-management/gateway-config-tyk-oas)           |
...
If you are upgrading to 5.3.10, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
This release contains bug fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.3.9) below.
...
| Third Party Dependency                                        | Tested Versions       | Compatible Versions   | Comments                                                                                   |
| :------------------------------------------------------------- | :--------------------- | :--------------------- | :------------------------------------------------------------------------------------------ |
| [Go](https://go.dev/dl/)                                      |  1.22 (GW)            |  1.22 (GW)            | [Go plugins](/5.13/api-management/plugins/golang#) must be built using Go 1.22 |
| [Redis](https://redis.io/download/)                           | 6.2.x, 7.x            | 6.2.x, 7.x            | Used by Tyk Gateway                                                                        |
| [OpenAPI Specification](https://spec.openapis.org/oas/v3.0.3) | v3.0.x                | v3.0.x                | Supported by [Tyk OAS](/5.13/api-management/gateway-config-tyk-oas)           |
...
If you are upgrading to 5.3.9, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
This release focuses mainly on bug fixes. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.3.8) below.
...
This is an advanced notice that the dedicated External OAuth, OpenID Connect (OIDC) authentication options, and SQLite support will be deprecated starting in version 5.7.0. We recommend that users of the [External OAuth](/5.13/api-management/client-authentication#integrate-with-external-authorization-server-deprecated) and [OpenID Connect](/5.13/api-management/client-authentication#integrate-with-openid-connect-deprecated) methods migrate to Tyk's dedicated [JWT Auth](/5.13/basic-config-and-security/security/authentication-authorization/json-web-tokens) method. Please review your API configurations, as the Gateway logs will provide notifications for any APIs utilizing these methods.


#### Upgrade Instructions

If you are upgrading to 5.3.8, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.3.7) below.
...
When upgrading to 5.3.7 please follow the [detailed upgrade instructions](/5.13/#upgrading-tyk).
...
[changelog](/5.13/#Changelog-v5.3.6) below.
...
If moving from an version of Tyk older than 5.3.0 please read the explanation provided with [5.3.0 release](/5.13/#TykOAS-v5.3.0).
...
When upgrading to 5.3.6 please follow the [detailed upgrade instructions](/5.13/#upgrading-tyk).
...
We have resolved an issue where custom [response plugins](/5.13/api-management/plugins/plugin-types#response-plugins) were not being
...
When upgrading to 5.3.5 please follow the [detailed upgrade instructions](/5.13/#upgrading-tyk).
...
configuration](/5.13/tyk-oss-gateway/configuration#http_server_options) that will enforce prefix and/or suffix matching
...
[v5.3.4](/5.13/developer-support/release-notes/dashboard#5-3-0-release-notes)
...
When upgrading to 5.3.4 please follow the [detailed upgrade instructions](/5.13/#upgrading-tyk).
...
- [Docker image to pull](https://hub.docker.com/r/tykio/tyk-gateway/tags?page=&page_size=&ordering=&name=v5.3.4)
  - ```bash
    ...
    ```
- Helm charts
  - [tyk-charts v1.4](/5.13/developer-support/release-notes/helm-chart#1-4-0-release-notes)
...
When upgrading to 5.3.3 please follow the [detailed upgrade instructions](/5.13/#upgrading-tyk).
...
This release primarily focuses on bug fixes. For a comprehensive list of changes, please refer to the detailed
[changelog](/5.13/#Changelog-v5.3.3) below.
...
FIPS support](/5.13/developer-support/release-types/fips-release).
...
Added [FIPS compliance](/5.13/developer-support/release-types/fips-release) for Tyk Gateway.
</Expandable>
...
When upgrading to 5.3.2 please follow the [detailed upgrade instructions](/5.13/#upgrading-tyk).

#### Release Highlights

This release primarily focuses on bug fixes. For a comprehensive list of changes, please refer to the detailed
[changelog](/5.13/#Changelog-v5.3.2) below.
...
When upgrading to 5.3.1 please follow the [detailed upgrade instructions](/5.13/#upgrading-tyk).

#### Release Highlights

This release primarily focuses on bug fixes. For a comprehensive list of changes, please refer to the detailed
[changelog](/5.13/#Changelog-v5.3.1) below.
...
- [Docker image to pull](https://hub.docker.com/r/tykio/tyk-gateway/tags?page=&page_size=&ordering=&name=v5.3.1)
  - ```bash
    ...
    ```
- Helm charts
  - [tyk-charts v1.3](/5.13/developer-support/release-notes/helm-chart#1-3-0-release-notes)
...
Fixed an issue with GraphQL APIs, where [headers](/5.13/api-management/graphql#graphql-apis-headers) were not properly forwarded upstream for [GQL/UDG subscriptions](/5.13/api-management/graphql#graphql-subscriptions).
</Accordion>
...
This upgrade transitions Tyk OAS APIs out of [Early Access](/5.13/developer-support/release-types/early-access-feature).

For licensed deployments (Tyk Cloud, Self Managed including MDCB), please refer to the [release notes of Tyk Dashboard 5.3.0](/5.13/developer-support/release-notes/dashboard#5-3-0-release-notes).
...
- **Not Backwards Compatible**
  ...
  - This means that if you wish to downgrade or revert to your previous version of Tyk, you will need to restore these
    API definitions from a backup. Please go to the [backup](/5.13/#upgrade-instructions) section for detailed
...
**Important:** Please go to the [backup](/5.13/#upgrade-instructions) section for detailed instructions on
...
the image](/5.13/api-management/plugins/rich-plugins#install-the-python-development-packages)
...
If you are upgrading to 5.3.0, please follow the detailed [upgrade instructions](/5.13/#upgrading-tyk).

**The following steps are essential to follow before upgrading** Tyk Cloud (including Hybrid Gateways) and Self Managed
users - Please refer to the [release notes of Tyk Dashboard 5.3.0](/5.13/developer-support/release-notes/dashboard#5-3-0-release-notes).

For OSS deployments -

1. Backup Your environment using the [usual guidance](/5.13/developer-support/upgrading) documented with every release (this includes
...
3. Performing the upgrade - follow the instructions in the [upgrade
   guide](/5.13/developer-support/upgrading) when upgrading Tyk.
...
[changelog](/5.13/#Changelog-v5.3.0) below.

##### Tyk OAS Feature Maturity

Tyk OAS is now out of [Early
Access](/5.13/developer-support/release-types/early-access-feature) as we have reached feature maturity.
...
the [documentation](/5.13/tyk-configuration-reference/kv-store) of this powerful feature.
...
key](/5.13/tyk-stack/tyk-developer-portal/enterprise-developer-portal/api-access/configuring-custom-rate-limit-keys)
...
Rate Limiter](/5.13/api-management/rate-limit#redis-rate-limiter)). Previously the complete
...
#### Upgrade Instructions
If you are using a 5.2.x version, we advise you to upgrade ASAP to this latest release. If you are on an older version, you should skip 5.2.0 and upgrade directly to this release. Go to the [Upgrading Tyk](/5.13/#upgrading-tyk) section for detailed upgrade instructions.

#### Release Highlights
This release implements a bug fix.
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.2.5) below.
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.2.4) below.
...
#### Release Highlights
This release enhances security, stability, and performance.
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.2.3) below.
...
Added new option for Tyk to use the default version of an API if the requested version does not exist. This is referred to as falling back to default and is enabled using a [configuration](/5.13/api-management/gateway-config-tyk-oas#versioning) flag in the API definition; for Tyk OAS APIs the flag is `fallbackToDefault`, for Tyk Classic APIs it is `fallback_to_default`.
</Accordion>
...
#### Release Highlights
This release primarily focuses on bug fixes.
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.2.2) below.
...
Fixed a performance issue encountered when Tyk Gateway retrieves a key via MDCB for a JWT API. The token is now validated against [JWKS or the public key](/5.13/basic-config-and-security/security/authentication-authorization/json-web-tokens) in the API Definition.
</Accordion>
...
#### Upgrade Instructions
If you are on a 5.2.0 we advise you to upgrade ASAP and if you are on an older version skip 5.2.0 and upgrade directly to this release. Go to the [Upgrading Tyk](/5.13/#upgrading-tyk) section for detailed upgrade instructions.

#### Release Highlights
This release primarily focuses on bug fixes.
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.2.0) below.
...
- `resource_sync.retry_attempts` - defines the number of [retries](/5.13/tyk-oss-gateway/configuration#resource_sync-retry_attempts) that the Gateway should perform during a resource sync (APIs or policies), defaulting to zero which means no retries are attempted
- `resource_sync.interval` - setting the [fixed interval](/5.13/tyk-oss-gateway/configuration#resource_sync-interval) between retry attempts (in seconds)
</Accordion>
...
Resolved a bug with HTTP GraphQL APIs where, when the [Persist GraphQL middleware](/5.13/api-management/graphql#persisting-graphql-queries) was used in combination with [Response Body Transform](/5.13/api-management/traffic-transformation/response-body), the response's body transformation was not being executed.
...
We're thrilled to bring you some exciting enhancements and crucial fixes to improve your experience with Tyk Gateway. For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.2.0) below.

##### Added Body Transform Middleware to Tyk OAS API Definition

With this release, we are adding the much requested *Body Transformations* to *Tyk OAS API Definition*. You can now [configure](/5.13/api-management/gateway-config-tyk-oas#transformbody) middleware for both [request](/5.13/api-management/traffic-transformation/request-body) and [response](/5.13/api-management/traffic-transformation/response-body) body transformations and - as a Tyk Dashboard user - you’ll be able to do so from within our simple and elegant API Designer tool.
...
We’ve added the ability to [configure](/5.13/api-management/response-caching#configuring-the-middleware-in-the-tyk-oas-api-definition) per-endpoint timeouts for Tyk’s response cache, giving you increased flexibility to tailor your APIs to your upstream services.

##### Added Header Management in Universal Data Graph

With this release we are adding a concept of [header management](/5.13/api-management/data-graph#header-management) in *Universal Data Graph*. With multiple upstream data sources, data graphs need to be sending the right headers upstream, so that our users can effectively track the usage and be able to enforce security rules at each stage. All *Universal Data Graph* headers now have access to *request context* variables like *JWT claims*, *IP address* of the connecting client or *request ID*. This provides extensive configurability of customizable information that can be sent upstream.

##### Added Further Support For GraphQL WebSocket Protocols

Support for [WebSocket](/5.13/api-management/graphql#graphql-websockets) protocols between client and the *Gateway* has also been expanded. Instead of only supporting the *graphql-ws protocol*, which is becoming deprecated, we now also support [graphql-transport-ws](https://github.com/enisdenjo/graphql-ws/blob/master/PROTOCOL.md) by setting the *Sec-WebSocket-Protocol* header to *graphql-transport-ws*.

##### Added OpenTelemetry Tracing

In this version, we're introducing the support for *OpenTelemetry Tracing*, the new [open standard](https://opentelemetry.io/) for exposing observability data. This addition gives you improved visibility into how API requests are processed, with no additional license required. It is designed to help you with monitoring and troubleshooting APIs, identify bottlenecks, latency issues and errors in your API calls. For detailed information and guidance, you can check out our [OpenTelemetry Tracing](/5.13/api-management/traces) resource.
...
*OpenTelemetry* functionality is also available in [Go Plugins](/5.13/api-management/plugins/advance-config#instrumenting-plugins-with-opentelemetry). Developers can write code to add the ability to preview *OpenTelemetry* trace attributes, error status codes etc., for their Go Plugins.

We offer support for integrating *OpenTelemetry* traces with supported open source tools such [Jaeger](/5.13/api-management/traces/jaeger), [Dynatrace](/5.13/api-management/traces/dynatrace) or [New Relic](/5.13/api-management/traces#new-relic). This allows API owners and developers to gain troubleshooting and performance insights from error logs, response times etc.
...
Added support for [configuring](/5.13/tyk-oss-gateway/configuration#opentelemetry) distributed tracing behavior of *Tyk Gateway*. This includes enabling tracing, configuring exporter types, setting the URL of the tracing backend to which data is to be sent, customizing headers, and specifying enhanced connectivity for *HTTP*, *HTTPS* and *gRPC*. Subsequently, users have precise control over tracing behavior in *Tyk Gateway*.
</Accordion>

<Accordion title='Added support for configuring OpenTelemetry'>
Added support to configure *OpenTelemetry* [sampling types and rates](/5.13/tyk-oss-gateway/configuration#opentelemetry-sampling) in the *Tyk Gateway*. This allows users to manage the need for collected detailed tracing information against performance and resource usage requirements.
</Accordion>
...
Added a new feature that allows clients to retrieve the trace ID from response headers. This feature is available when *OpenTelemetry* is [enabled](/5.13/tyk-oss-gateway/configuration#opentelemetry-enabled) and simplifies debugging API requests, empowering users to seamlessly correlate and analyze data for a specific trace in any *OpenTelemetry* backend like [Jaeger](https://www.jaegertracing.io/).
</Accordion>

<Accordion title='Allow detailed tracing to be enabled/disabled at API level'>
Added configuration parameter to enable/disable [detailed_tracing](/5.13/api-management/tyk-pump#configure-detailed-recording) for *Tyk Classic API*.
</Accordion>

<Accordion title='Add OpenTelemetry support for GraphQL'>
Added *OpenTelemetry* support for GraphQL. This is activated by setting [opentelemetry.enabled](/5.13/tyk-oss-gateway/configuration#opentelemetry-enabled) to *true*. This integration enhances observability by enabling GQL traces in any OpenTelemetry backend, like [Jaeger](https://www.jaegertracing.io/), granting users comprehensive insights into the execution process, such as request times.
</Accordion>

<Accordion title='Add support for configuring granular control over cache timeouts at the endpoint level'>
Added a new [timeout option](/5.13/api-management/response-caching#configuring-the-middleware-in-the-tyk-oas-api-definition), offering granular control over cache timeout at the endpoint level.
</Accordion>

<Accordion title='Enable request context variables in UDG global or data source headers'>
Added support for using [request context variables](/5.13/api-management/traffic-transformation/request-context-variables) in *UDG* global or data source headers. This feature enables much more advanced [header management](/5.13/api-management/data-graph#header-management) for UDG and allows users to extract header information from an incoming request and pass it to upstream data sources.
</Accordion>

<Accordion title='Add support for configuration of global headers for any UDG'>
Added support for configuration of [global headers](/5.13/api-management/data-graph#header-management) for any *UDG*. These headers will be forwarded to all data sources by default, enhancing control over data flow.
</Accordion>
...
Fixed an issue where the [Enforced Timeout](/5.13/planning-for-production/ensure-high-availability/enforced-timeouts) configuration parameter of an API endpoint accepted negative values, without displaying validation errors. With this fix, users receive clear feedback and prevent unintended configurations.
</Accordion>
...
Our Gateway is using [Golang 1.19](https://tip.golang.org/doc/go1.19) programming language starting with the 5.1 release. This brings improvements to the code base and allows us to benefit from the latest features and security enhancements in Go. Don’t forget that, if you’re using GoPlugins, you'll need to [recompile](/5.13/api-management/plugins/golang#upgrading-your-tyk-gateway) these to maintain compatibility with the latest Gateway.

#### Early Access Features:
Please note that the `Tyk OAS APIs` feature, currently marked as *Early Access*, is subject to breaking changes in subsequent releases. Please refer to our [Early Access guide](/5.13/developer-support/release-types/early-access-feature) for specific details. Upgrading to a new version may introduce changes that are not backward-compatible. Downgrading to a previous version after upgrading may result in a broken installation.
...
### Upgrade Instructions
Go to the [Upgrading Tyk](/5.13/#upgrading-tyk) section for detailed upgrade instructions.
...
description of how to use these powerful [features](/5.13/api-management/traffic-transformation/request-size-limits).
...
size using [slave_options.rpc_pool_size](/5.13/tyk-oss-gateway/configuration#slave_options-rpc_pool_size)
...
Go to the [Upgrading Tyk](/5.13/developer-support/release-notes/gateway#upgrading-tyk)
...
For a comprehensive list of changes, please refer to the detailed [changelog](/5.13/#Changelog-v5.0.15) below.
...
ensure proper synchronization across their environments. Please refer to the item in the [fixed](/5.13/#fixed) section of the
...
This release primarily focuses on bug fixes. For a comprehensive list of changes, please refer to the detailed
[changelog](/5.13/#Changelog-v5.0.2) below.
...
This release primarily focuses on bug fixes. For a comprehensive list of changes, please refer to the detailed
[changelog](/5.13/#Changelog-v5.0.1) below.
...
Tyk’s OSS users can now make use of extensive [custom middleware](/5.13/api-management/plugins/overview) options with your OAS
...
[Versioning your Tyk OAS APIs](/5.13/api-management/api-versioning) is easier than ever, with the
...
We’ve improved support for [Mock Responses](/5.13/api-management/traffic-transformation/mock-response#mock-response), with the Tyk OAS API
...
This can be achieved by configuring the new External OAuth authentication mechanism. Find out more here [External OAuth Integration](/5.13/api-management/client-authentication#integrate-with-external-authorization-server-deprecated)
...
2. Support for allow list in field-based permissions - so far Tyk was offering field-based permissions as a “block list” only. That meant that any new field/query added to a graph was by default accessible for all consumers until API manager explicitly blocked it on key/policy level. Adding support for “allow list” gives API managers much more control over changing schemas and reduces the risk of unintentionally exposing part of the graph that are not ready for usage. See [Introspection](/5.13/api-management/graphql#introspection) for more details.
...
Follow the [standard upgrade guide](/5.13/developer-support/upgrading), there are no breaking changes in this release.

If you want switch from MongoDB to SQL, you can [use our migration tool](/5.13/planning-for-production/database-settings#migrating-from-an-existing-mongodb-instance), but keep in mind that it does not yet support the migration of your analytics data.


<Note>
Note: Upgrading the Golang version implies that all the Golang custom plugins that you are using need to be recompiled before migrating to 4.3 version of the Gateway. Check our docs for more details [Golang Plugins](/5.13/api-management/plugins/golang).
</Note>
...
Tyk has always had a proprietary specification for defining APIs. From Tyk v4.1 we now support defining APIs using the Open API Specification (OAS) as well, which can offer significant time and complexity savings. [This is an early access capability](/5.13/developer-support/release-types/early-access-feature).
...
This capability is available in both the open source and paid versions of Tyk. See our [Tyk OAS documentation](/5.13/api-management/gateway-config-tyk-oas) for more details.
...
- Added a way to modify Tyk analytics record via Go plugins [configurable with API definition](/5.13/api-management/plugins/plugin-types#analytics-plugins). Can be used to sanitise analytics data. 
- Added new policy API REST endpoints
- Added option to configure certificates for Tyk Gateway using [environment variable](/5.13/tyk-oss-gateway/configuration#http_server_options-certificates)
...
[Federation docs](/5.13/api-management/graphql#overview-1)

[Subgraphs and Supergraphs docs](/5.13/api-management/graphql#subgraphs-and-supergraphs)
...
[Subscriptions docs](/5.13/api-management/graphql#graphql-subscriptions)
...
We've updated the GraphQL functionality of our [Universal Data Graph](/5.13/api-management/data-graph#overview). You’re now able to deeply nest GraphQL & REST APIs and stitch them together in any possible way.

Queries are now possible via WebSockets and Subscriptions are coming in the next Release (3.3.0).

You're also able to configure [upstream Headers dynamically](/5.13/api-management/data-graph#header-forwarding), that is, you’re able to inject Headers from the client request into UDG upstream requests. For example, it can be used to access protected upstreams. 
...
Find out more about [plugins](/5.13/api-management/plugins/overview#) and how to write [Go response plugins](/5.13/api-management/plugins/golang#creating-a-custom-response-plugin).
...
#### Upgrade process
If you already have GraphQL or UDG APIs you need to follow this [upgrade guide](/5.13/api-management/graphql#migrating-to-3-2)
...
This follows the recent changes that we have made to embed TIB (Tyk Identity Broker)in the dashboard. See 3.0 [release notes](/5.13/developer-support/release-notes/dashboard#tyk-identity-broker-now-built-in-to-the-dashboard) for more information regarding this. 

To learn more [see the documentation](/5.13/tyk-identity-broker/overview)
...
Docs [here](/5.13/api-management/graphql#syncing-gql-schema)
...
Docs [here](/5.13/api-management/graphql#graphql-playground)
...
[Docs here](/5.13/tyk-developer-portal/tyk-portal-classic/key-requests)
...
Read more about the [GraphQL](/5.13/api-management/graphql) and [Universal Data Graph](/5.13/api-management/data-graph#overview)
...
[Read more](/5.13/tyk-configuration-reference/kv-store)
...
At the moment the Response hook is supported for [Python and gRPC plugins](/5.13/api-management/plugins/rich-plugins#coprocess-dispatcher-hooks).
...
[Read More](/5.13/planning-for-production/ensure-high-availability/health-check)
...
[Read More](/5.13/api-management/troubleshooting-debugging#capturing-detailed-logs)
...
[Read More](/5.13/api-management/tyk-pump#tyk-pump-configuration)
...
If you are upgrading from version 2.8, please [read this guide](/5.13/developer-support/release-notes/archived#2-9-0-release-notes)

## Further Information

### Upgrading Tyk
Please refer to the [upgrading Tyk](/5.13/developer-support/upgrading) page for further guidance on the upgrade strategy.
...
- [OpenAPI Document](/5.13/tyk-dashboard-api)
- [Postman Collection](https://www.postman.com/tyk-technologies/workspace/tyk-public-workspace/overview)

### FAQ

Please visit our [Developer Support](/5.13/developer-support/community) page for further information relating to reporting bugs, upgrading Tyk, technical support and how to contribute.
