...
Also, it was possible to [create a custom key](/5.13/api-reference/keys/create-custom-key-import-key) shorter than the configured minimum length. The Session would be created, but the key would be unusable as any attempt to use it in a request would be rejected due to the token's length.
