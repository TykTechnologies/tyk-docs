...
This release adds [Secrets Management](/5.13/tyk-configuration-reference/kv-store) to the Tyk MDCB configuration file for the first time, letting you reference values from external secrets stores in place of plain-text values.
...
In this release, we have added support for the Client Identity Provider (IdP) registry, introduced in Tyk 5.14.0. This feature provides centralized management of IdPs and scope-to-policy mappings used with [JWT Auth](/5.13/basic-config-and-security/security/authentication-authorization/json-web-tokens). Adding support to Tyk MDCB allows the use of the new Client IdP resource in distributed deployments.

<Note>
Client IdP registry support requires Tyk Gateway and Tyk Dashboard v5.14.0 or higher. This feature stays inert on Tyk v5.13.x LTS deployments.
</Note>
...
For distributed deployments, deploy MDCB 2.11.0 before upgrading Tyk Pump to 1.15.0 and Tyk Gateway to 5.13.0. MCP analytics support is additive and does not affect existing Pump or Gateway versions until they are also upgraded. See [MCP Gateway upgrade considerations](/5.13/developer-support/upgrading#mcp-upgrade-considerations) for the full deployment order.
...
No additional MDCB configuration is required. For details, see the [MCP analytics documentation](/5.13/ai-management/mcp-gateway/mcp-observability).
</Accordion>
...
The [`omit_analytics_index_creation`](/5.13/tyk-multi-data-centre/mdcb-configuration-options#omit_analytics_index_creation) configuration option is used to prevent deadlocks in horizontally scaled deployments that could occur if multiple MDCB instances created the same index concurrently.
When this option is enabled, MDCB does not [automatically create indexes](/5.13/api-management/tyk-pump#omitting-indexes), eliminating database contention issues, while maintaining full analytics functionality.
...
This release updates MDCB to maintain compatibility with recent changes to the [URL Rewrite Middleware schema](/5.13/developer-support/release-notes/gateway#release-highlights-1), ensuring seamless interoperability with the latest Gateway and Dashboard versions.
...
We have updated MDCB to align with the recent [URL Rewrite Middleware schema](/5.13/developer-support/release-notes/gateway#release-highlights-1). This update ensures that MDCB remains fully compatible and behaves consistently with the latest Gateway and Dashboard releases.
</Expandable>
...
- Migrate to new [health check](/5.13/api-management/mdcb#health-check) endpoints in order to get more detailed information. For Kubernetes users, use Helm Charts v1.6 to upgrade MDCB to set liveness and readiness probes of MDCB deployment to the new health check endpoints.
...
Two new [health check](/5.13/api-management/mdcb#health-check) endpoints have been added to improve monitoring and diagnostics:
...
Two new `/config` and `/env` [endpoints](/5.13/api-management/mdcb#check-mdcb-configurations) have been implemented, allowing developers to access the current configuration state of the MDCB instance in real-time. This feature provides:
...
[MongoDB supported versions](/5.13/planning-for-production/database-settings#supported-versions) 
...
#### Added
- Added a new configuration option [enable_ownership](/5.13/tyk-multi-data-centre/mdcb-configuration-options#enable_ownership) that allows MDCB filter APIs by API Ownership. 
