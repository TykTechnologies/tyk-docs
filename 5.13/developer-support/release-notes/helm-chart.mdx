...
This release includes improvements to support for Redis Sentinel deployments and updates the default charts to install the most recent Tyk LTS release [5.8.5](/5.13/developer-support/release-notes/dashboard#5-8-5-release-notes) and Developer Portal [1.14.0](/5.13/developer-support/release-notes/portal#1-14-0-release-notes).
...
Resolved an issue where the Redis Sentinel password ([TYK_GW_STORAGE_SENTINELPASSWORD](/5.13/tyk-oss-gateway/configuration#storage-sentinel_password)) was not correctly picked up by Helm charts. Previously, both Redis and Sentinel passwords referenced the same secret key, leading to the Sentinel password defaulting to the regular Redis password. This fix ensures correct handling of distinct Sentinel passwords.
</Accordion>
</AccordionGroup>
...
To configure, see [Tyk Stack](/5.13/product-stack/tyk-charts/tyk-stack-chart#audit-log-configurations) documentation.
</Accordion>
...
To configure, see [Tyk Stack](/5.13/product-stack/tyk-charts/tyk-stack-chart#opa-configurations) documentation.
</Accordion>
...
To configure, see [Tyk Stack](/5.13/product-stack/tyk-charts/tyk-stack-chart#gateway-probes) documentation.
</Accordion>
...
However, if you are upgrading to [Tyk Operator v1.0](/5.13/developer-support/release-notes/operator#1-0-0-release-notes) using the Helm Chart, please read the [license requirement](/5.13/developer-support/release-notes/operator#breaking-changesv1.0.0) and Tyk Operator [installation and upgrade instructions](/5.13/tyk-stack/tyk-operator/installing-tyk-operator) carefully.
...
- Familiarize yourself with Request Matching in Tyk [here](/5.13/getting-started/key-concepts/url-matching).
- For production setup guidance, see [this guide](/5.13/planning-for-production#ensure-you-are-matching-only-the-url-paths-that-you-want-to-match).
...
Learn more about the settings in the [Request Matching](/5.13/getting-started/key-concepts/url-matching) documentation.
</Expandable>
...
MDCB v2.7.0 release introduces `/liveness` and `/readiness` probes which give more accurate and detail health check information. MDCB deployment has been updated to use the new endpoints. See [MDCB Health Check](/5.13/api-management/mdcb#health-check) section for information about the new probes.
...
For more details about new endpoints, check [MDCB Health check](/5.13/api-management/mdcb#health-check) section.
</Expandable>
...
To leverage this stable release and simplify your Tyk deployments, we invite you to explore our example setup for MDCB Control Plane using Helm Chart. Simply follow our [MDCB Control Plane setup guide](/5.13/api-management/mdcb#setup-mdcb-control-plane) to get started.
...
#### Breaking Changes
{/* Required. Use the following statement if there are no breaking changes, or explain if there are */}
For MongoDB users: Tyk Charts 1.3.0 uses `mongo-go` as the default driver to connect to MongoDB. `mongo-go` driver is compatible with MongoDB 4.4.x and above. For MongoDB versions prior to 4.4, please set `global.mongo.driver` to `mgo`. We recommend reading [Choose a MongoDB driver](/5.13/planning-for-production/database-settings#choose-a-mongodb-driver) when you need to change driver setting.
...
##### Graph Pump
Tyk Charts 1.3 adds support for Graph MongoDB Pump, Graph SQL Pump and Graph SQL Aggregate Pump. see [Graph Pump setup](/5.13/api-management/tyk-pump#graph-pump-setup) to learn more about the GraphQL-specific metrics available.

##### Enable Tyk Identity Broker (TIB) in Tyk Dashboard
Tyk Charts 1.3 adds a field to enable Internal [Tyk Identity Broker (TIB)](/5.13/tyk-identity-broker/overview) in Tyk Dashboard by field `tyk-dashboard.tib.enabled` to `true`.
...
When enabled at `gateway.control.ingress.enabled`, an Ingress resource will be created to allow external access to gateway's [control service](/5.13/planning-for-production#change-your-control-port).
</Accordion>

<Accordion title='Gateway: Configure Gateway to work with MDCB synchroniser'>
Allow users to configure worker gateway to work with [Tyk MDCB synchroniser](/5.13/api-management/mdcb#mdcb-synchroniser) easily by setting `global.mdcbSynchronizer.enabled` in `tyk-data-plane`.
...
You can enable Internal [Tyk Identity Broker (TIB)](/5.13/tyk-identity-broker/overview) in Tyk Dashboard by field `tyk-dashboard.tib.enabled` to `true`.
</Accordion>
...
read [Tyk Control Plane chart](/5.13/product-stack/tyk-charts/tyk-control-plane-chart).
</Accordion>
