...
The concept of [internal looping](/5.13/advanced-configuration/transform-traffic/looping) allows you to use URL Rewriting to redirect your URL to *another API endpoint* or to *another API* in the Gateway. In Tyk, looping is generally targeted using the `tyk://<API_ID>/<path>` scheme, which requires prior knowledge of the `API_ID`. Tyk Operator simplifies the management and transformation of API traffic within Kubernetes environments by abstracting APIs as objects, managing them and dynamically assigning `API_ID`s by its Kubernetes metedata name and namespace.

## Configuring looping to internal ApiDefinition resources

Looping can be configured within Tyk Operator for [URL Rewrites](/5.13/#url-rewrites), [URL Rewrite Triggers](/5.13/#url-rewrite-triggers) and [Proxy to internal APIs](/5.13/#proxy-to-internal-apis) by configuring the `rewrite_to_internal` in `url_rewrite`, `rewrite_to_internal` in `triggers`, and `proxy.target_internal` fields respectively with these properties:
...
[URL rewriting](/5.13/transform-traffic/url-rewriting) enables the transformation of incoming API request paths to align with the expected endpoint format of your backend services.
...
[Triggers](/5.13/transform-traffic/url-rewriting#how-it-works) are configurations that specify actions based on certain conditions present in HTTP headers, query parameters, path parameters etc.
...
The process for configuring internal looping in triggers to is similar to that explained in section [URL Rewrites](/5.13/#url-rewrites).
...
Internal looping can also be used for [proxying to internal APIs](/5.13/advanced-configuration/transform-traffic/looping).
