...
each of the following [hooks](/5.13/api-management/plugins/plugin-types#plugin-and-hook-types) in the API request
lifecycle: [Pre (Request)](/5.13/api-management/plugins/plugin-types#request-plugins), [Authentication](/5.13/api-management/plugins/plugin-types#authentication-plugins),
[Post (Request)](/5.13/api-management/plugins/plugin-types#request-plugins), [Post
Authentication](/5.13/api-management/plugins/plugin-types#request-plugins), [Response](/5.13/api-management/plugins/plugin-types#response-plugins)
and [Analytics](/5.13/api-management/plugins/plugin-types#analytics-plugins). Subsequently, users can execute, or “hook”, their
...
Please refer to [Analytics Plugins](/5.13/api-management/plugins/plugin-types#analytics-plugins) to learn how to configure Analytics
...
languages](/5.13/api-management/plugins/overview#plugin-driver-names) section for list of supported plugin driver names.
...
[per-endpoint](/5.13/api-management/plugins/plugin-types#per-endpoint-custom-plugins) level plugins are not currently
...
2. **Create API Definition Resource**
    ...
    *http://httpbin.org*.
    ...
    - The `driver` configuration parameter is set to `otto` at line 15, since our plugin is a Javascript plugin. For other
      valid values please refer to the [plugins driver page](/5.13/api-management/plugins/overview#plugin-driver-names).
    - A plugin hook configuration block is specified at line 16, containing the `name` and `path` for our plugin. The plugin
      ...
      middleware. Valid values are the same as the [Tyk Classic API Definition](/5.13/#how-it-works) equivalent, i.e.
...
request lifecycle](/5.13/api-management/traffic-transformation#request-middleware-chain).
...
bundles](/5.13/api-management/plugins/overview#plugin-bundles), load them into the Tyk Gateway, and configure an API
